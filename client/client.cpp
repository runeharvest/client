...
//
// RYZOM_TRY and RYZOM_CATCH aim is to catch differently in dev and final
// version
//    In final version, we catch everything and nlerror the problem to display a
//    NeL message box In dev version, we just catch EFatalError() and we leave
//    the OS to catch the exception to enable us to cancel/debug it
//

// We don't catch(...) because these exception are already trapped with the
// se_translation that generate the NeL message box

#define RYZOM_TRY(_block)                                                      \
  try {                                                                        \
    nlinfo(_block " of Ryzom...");
#define RYZOM_CATCH(_block)                                                    \
  nlinfo(_block " of Ryzom success");                                          \
  }                                                                            \
...
#endif
{
  ...
  Args.addArg("p", "profile", "id",
              "Use this profile to determine what directory to use by default");
  ...
  // extract the 2 or 3 first param (argv[1], argv[2] and argv[3]) it must be
  // <login> <password> [shardId]
#ifdef NL_OS_WINDOWS
  if (!Args.parse(cmdline))
    return 1;
#else
  ...
  if (Args.haveArg("p") || !CFile::isExists("client_default.cfg")) {
    ...
    if (!CPath::setCurrentPath(currentPath))
      return 1;
  }
  ...
  // Get the bitmap size
  HRSRC hrsrc =
      FindResource(HInstance, MAKEINTRESOURCE(IDB_SLASH_SCREEN), RT_BITMAP);
  nlassert(hrsrc);
  ...
  SlashScreen = CreateDialog(hInstance, MAKEINTRESOURCE(IDD_SLASH_SCREEN), NULL,
                             MyDialogProc);
  RECT rect;
  RECT rectDesktop;
  ...
  SetWindowPos(
      SlashScreen, HWND_TOP, (rectDesktop.right - rectDesktop.left - width) / 2,
      (rectDesktop.bottom - rectDesktop.top - height) / 2, width, height, 0);
  ...
  {
    vector<string> files;
    NLMISC::CPath::getPathContent("data", false, false, true, files, NULL,
                                  true);
    uint i;
  ...
  if (steamClient.init())
    LoginCustomParameters =
        "&steam_auth_session_ticket=" + steamClient.getAuthSessionTicket();
#endif
...
#if !FINAL_VERSION
      // display the file access logger stats and clear out memory allocated for
      // the file access logger
...
#if !FINAL_VERSION
  // display the file access logger stats and clear out memory allocated for the
  // file access logger
  // ICommand::execute("iFileAccessLogDisplay",*NLMISC::InfoLog);
  // ICommand::execute("iFileAccessLogStop",*NLMISC::InfoLog);
  // ICommand::execute("iFileAccessLogClear",*NLMISC::InfoLog);
...
#ifdef NL_OS_WINDOWS
  /// always do a sanity check for early (in the sense of production days)
  /// memory coruption detection
  // _CrtCheckMemory();
#endif
