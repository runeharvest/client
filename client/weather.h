...
// Description of light cycle in ryzom (for each season) , should be initialized
// from a georges sheet
extern CLightCycle WorldLightCycle;
/** This setup light of objects in the world depending on the hour. It has a
 * light cycle descriptor which must be setuped at each season change (see
 * buildLightCycleDesc). It may interract with a weather manager to know when to
 * override lighting of the scene (in case of bad weather).
 */
extern CLightCycleManager LightCycleManager;
// This manage precipitations and blend weather setups. This allow to know the
// current weather (if fog or light must be overriden)
extern CWeatherManagerClient WeatherManager;
// The last season computed from computeCurrSeason, updated in the main loop /
// in teleport. Update code based on season should use this value
