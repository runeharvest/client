...
NLMISC::CMatrix CTimedFX::getInstanceMatrix() const {
  ...
  if (FXSheet && FXSheet->InheritRot)
    result.rotate(Rot);
  if (FXSheet && FXSheet->InheritScale)
    result.scale(Scale);
  return result;
}
...
CTimedFXManager::CTimedFXManager() {
  FPU_CHECKER
  H_AUTO_USE(RZ_TimedFX)
  _Scene = NULL, _DayLength = 24.f, _InitDone = false;
...
// *******************************************************************************************
void CTimedFXManager::init(NL3D::UScene *scene, const CClientDate &startDate,
                           float /* dayLength */, float noiseFrequency,
                           uint maxNumberOfFXInstances,
                           float sortDistanceInterval, float maxDist) {
  FPU_CHECKER
  H_AUTO_USE(RZ_TimedFX)
  nlassert(!_InitDone);
  if (!scene) {
  ...
  _CandidateFXListSortedByDist.resize(
      (int)ceilf(maxDist / sortDistanceInterval));
  _CandidateFXListSortedByDistTmp.resize(
      (int)ceilf(maxDist / sortDistanceInterval));
...
CTimedFXManager::add(const std::vector<CTimedFX> &fxs,
  ...
  for (uint k = 0; k < fxs.size(); ++k) {
    ...
    if (!(fi.FXSheet && fi.FXSheet->Mode == CSeasonFXSheet::AlwaysStarted)) {
      if (fi.FXSheet && fi.FXSheet->Mode == CSeasonFXSheet::Spawn) {
        ...
        fi.computeStartHour(cycle - 1, startDate, cycleLength, _DayLength,
                            _Noise);
        if (startDate < _CurrDate) {
          fi.computeStartHour(cycle, startDate, cycleLength, _DayLength,
                              _Noise);
          if (startDate < _CurrDate) {
            fi.computeStartHour(cycle + 1, startDate, cycleLength, _DayLength,
                                _Noise);
        ...
        // the system is not always instanciated, so must check if it currently
        // is.
        float cycleLength = fi.FXSheet ? fi.FXSheet->CycleDuration : _DayLength;
        sint32 cycle = dateToCycle(_CurrDate, cycleLength, _DayLength);
        ...
        if (startDate > _CurrDate) {
          // Let see if it did not start the previous cycle and then ended that
          // cycle
          fi.computeEndHour(cycle - 1, endDate, cycleLength, _DayLength,
                            _Noise);
          if (endDate > _CurrDate) {
            CClientDate prevStartDate;
            fi.computeStartHour(cycle - 1, prevStartDate, cycleLength,
                                _DayLength, _Noise);
          ...
          if (endDate > _CurrDate) {
            ...
            fi.computeStartHour(_CurrDate.Day + 1, endDate, cycleLength,
                                _DayLength, _Noise);
    ...
    if (instanciate) {
      ...
      // Add the system to the queue of systems to be removed, only if it is not
      // always started
      if (!(fi.FXSheet && fi.FXSheet->Mode == CSeasonFXSheet::AlwaysStarted)) {
        // As the system is currenlty instanciated, we won't deals with it again
        // until it shutdowns, so, we save it in the priority queue.
        CTimeStampedFX tsf;
        tsf.Date = endDate;
        // tsf.DebugDay = 666;
        tsf.FX = &fi; // yes, we keep a pointer on a vector element, but we will
                      // never grow that vector
        fi.SetHandle = _FXToRemove.insert(tsf).first;
        fi.State = CManagedFX::InRemoveList;
      }
    } else {
      // The system is not instanciated yet, but will be later, so add to the
      // queue of systems that "will be instanciated later"
      CTimeStampedFX tsf;
      tsf.Date = startDate;
      tsf.FX = &fi; // yes, we keep a pointer on a vector element, but we will
                    // never grow that vector
...
#ifdef DEBUG_FX
      nlinfo("==== added fx %s, start day = %day, start hour = %f, pos=(%f, "
             "%f, %f)",
             fi.FXSheet ? fi.FXSheet->FXName.c_str() : "???",
             (int)startDate.Day, startDate.Hour, fi.SpawnPosition.x,
             fi.SpawnPosition.y, fi.SpawnPosition.z);
#endif
    }
  }
  CHECK_INTEGRITY
...
void CTimedFXManager::remove(TFXGroupHandle handle) {
  ...
  for (uint k = 0; k < group.size(); ++k) {
    ...
    case CManagedFX::InAddList:
      _FXToAdd.erase(fi.SetHandle);
      break;
    case CManagedFX::InRemoveList:
      _FXToRemove.erase(fi.SetHandle);
      break;
    default:
      break;
    }
...
// *******************************************************************************************
void CTimedFXManager::update(const CClientDate &date,
                             EGSPD::CSeason::TSeason /* season */,
  ...
  while (!_FXToRemove.empty()) {
    ...
    // removes from candidate & instanciated list (this causes fx to be shutdown
    ...
    newTsf.FX = tsf.FX;
    float cycleLength =
        newTsf.FX->FXSheet ? newTsf.FX->FXSheet->CycleDuration : _DayLength;
    sint32 cycle = dateToCycle(tsf.Date, cycleLength, _DayLength);
    tsf.FX->computeStartHour(cycle, newTsf.Date, cycleLength, _DayLength,
                             _Noise);
    if (newTsf.Date <= tsf.Date) {
      // already started for that day, so compute start hour for next day.
      tsf.FX->computeStartHour(cycle + 1, newTsf.Date, cycleLength, _DayLength,
                               _Noise);
    }
...
// compute next shutdown date
#ifdef DEBUG_FX
    nlinfo("===== compute end date for %s, debug day = %d",
           tsf.FX->FXSheet ? tsf.FX->FXSheet->FXName.c_str() : "???",
           /*tsf.DebugDay*/ 666);
#endif
    CTimeStampedFX newTsf;
    newTsf.FX = tsf.FX;
    if (tsf.FX->FXSheet && tsf.FX->FXSheet->Mode == CSeasonFXSheet::Spawn) {
      // for a spawned fx, it is shutdown as soon the frame after it is created,
      // so we add it to the remove list with the current date
      newTsf.Date = date;
    } else {
#ifdef DEBUG_FX
      nlinfo("start day = %d, start hour = %f", (int)tsf.Date.Day,
             tsf.Date.Hour);
#endif
      float cycleLength =
          tsf.FX->FXSheet ? tsf.FX->FXSheet->CycleDuration : _DayLength;
      sint32 cycle = dateToCycle(tsf.Date, cycleLength, _DayLength);
      tsf.FX->computeEndHour(cycle - 1, newTsf.Date, cycleLength, _DayLength,
                             _Noise);
#ifdef DEBUG_FX
      nlinfo("try 0 : end day = %d, end hour = %f", (int)newTsf.Date.Day,
             newTsf.Date.Hour);
#endif
      if (newTsf.Date < tsf.Date) {
        tsf.FX->computeEndHour(cycle, newTsf.Date, cycleLength, _DayLength,
                               _Noise);
#ifdef DEBUG_FX
        nlinfo("try 1 : end day = %d, end hour = %f", (int)newTsf.Date.Day,
               newTsf.Date.Hour);
#endif
        if (newTsf.Date < tsf.Date) {
          tsf.FX->computeEndHour(cycle + 1, newTsf.Date, cycleLength,
                                 _DayLength, _Noise);
#ifdef DEBUG_FX
          nlinfo("try 2 : end day = %d, end hour = %f", (int)newTsf.Date.Day,
                 newTsf.Date.Hour);
#endif
        }
      }
  ...
  if ((_CurrDate.Day == 0 && _CurrDate.Hour == 0.f) ||
      (camPos - _LastCamPos).sqrnorm() > _SortDistance * _SortDistance) {
    _LastCamPos = camPos;
...
void CTimedFXManager::shutDown(TFXGroupHandle handle) {
  ...
  // remove all fxs of that group from both queues, and shutdown those that are
  // instanciated
...
// *******************************************************************************************
void CTimedFXManager::CManagedFX::shutDown(NL3D::UScene *scene,
                                           CFXManager &fxManager) {
  FPU_CHECKER
  H_AUTO_USE(RZ_TimedFX)
  if (!Instance.empty()) {
    // should not be already shutting down
    // if the fx is spaned, it will end by itself, so we just add it to the
    // shutdown list
    if (FXSheet && FXSheet->Mode == CSeasonFXSheet::Spawn) {
      ...
      if (Instance.isSystemPresent() && Instance.isValid()) {
        if (!Instance.removeByID(NELID("main")) &&
            !Instance.removeByID(NELID("STOP"))) {
          // if a specific emitter has not be tagged, just stop all emitters if
...
// *******************************************************************************************
void CTimedFXManager::cycleToDate(sint32 cycle, float hour, float cycleLength,
                                  float dayLength, CClientDate &result) {
  FPU_CHECKER
  H_AUTO_USE(RZ_TimedFX)
  if (dayLength == 0.f) {
    result.Day = 0;
...
// *******************************************************************************************
sint32 CTimedFXManager::dateToCycle(const CClientDate &date, float cycleLength,
                                    float dayLength) {
  FPU_CHECKER
  H_AUTO_USE(RZ_TimedFX)
  if (cycleLength == 0.f)
    return 0;
  if (dayLength == cycleLength)
    return date.Day;
...
// *******************************************************************************************
void CTimedFXManager::CManagedFX::computeHour(
    sint32 cycle, float bias, CClientDate &resultDate, float cycleLength,
    float dayLength, const NLMISC::CNoiseValue &nv, float minHour,
    float maxHour) const {
  FPU_CHECKER
  H_AUTO_USE(RZ_TimedFX)
  // add an offset in z to get a different noise value each day
  CVector pos = SpawnPosition + (float)cycle * CVector::K +
                bias * nv.Frequency * CVector::I;
  float delta = computeUniformNoise(nv, pos);
  if (minHour <= maxHour) {
    // convert cycle:hour to day:hour (a cycle may be something other than 24
    // hour)
    cycleToDate(cycle, minHour + delta * (maxHour - minHour), cycleLength,
                dayLength, resultDate);
  } else {
    float hourDiff = dayLength - minHour + maxHour;
    float hour = minHour + delta * hourDiff;
    if (hour >= cycleLength) {
      cycleToDate(cycle + 1, hour - dayLength, cycleLength, dayLength,
                  resultDate);
    } else {
...
// *******************************************************************************************
void CTimedFXManager::CManagedFX::computeStartHour(
    sint32 cycle, CClientDate &resultDate, float cycleLength, float dayLength,
    const NLMISC::CNoiseValue &nv) const {
  FPU_CHECKER
  H_AUTO_USE(RZ_TimedFX)
  if (!FXSheet) {
  ...
  computeHour(cycle, 0.f, resultDate, cycleLength, dayLength, nv,
              FXSheet->StartHourMin, FXSheet->StartHourMax);
}

// *******************************************************************************************
void CTimedFXManager::CManagedFX::computeEndHour(
    sint32 cycle, CClientDate &resultDate, float cycleLength, float dayLength,
    const NLMISC::CNoiseValue &nv) const {
  FPU_CHECKER
  H_AUTO_USE(RZ_TimedFX)
  ...
  if (!(FXSheet->Mode == CSeasonFXSheet::UseDuration)) {
    if (FXSheet->EndHourMax < FXSheet->StartHourMin) {
      computeHour(cycle + 1, 1.f, resultDate, cycleLength, dayLength, nv,
                  FXSheet->EndHourMin, FXSheet->EndHourMax);
    } else {
      computeHour(cycle, 1.f, resultDate, cycleLength, dayLength, nv,
                  FXSheet->EndHourMin, FXSheet->EndHourMax);
    }
  } else {
    ...
    computeHour(cycle, 0.f, resultDate, cycleLength, dayLength, nv,
                FXSheet->StartHourMin, FXSheet->StartHourMax);
#ifdef DEBUG_FX
    nlinfo("computeEndHour for day %d: start day = %d, start hour = %f, "
           "pos=(%f, %f, %f)",
           (int)day, (int)resultDate.Day, resultDate.Hour, SpawnPosition.x,
    ...
    CVector pos = SpawnPosition + (float)cycle * CVector::K +
                  bias * nv.Frequency * CVector::I;
    float delta = computeUniformNoise(nv, pos);
    resultDate.Hour += delta * (FXSheet->MaxDuration - FXSheet->MinDuration) +
                       FXSheet->MinDuration;
...
void CTimedFXManager::dumpFXToAdd() const {
  ...
  nlinfo("FX to add");
  nlinfo("=========");
  for (TTimeStampedFXPtrSet::const_iterator it = _FXToAdd.begin();
       it != _FXToAdd.end(); ++it) {
    nlinfo("%s : day=%d, hour=%f, pos=(%.1f, %.1f, %.1f)",
           it->FX->FXSheet ? it->FX->FXSheet->FXName.c_str() : "???",
           (int)it->Date.Day, it->Date.Hour, it->FX->SpawnPosition.x,
           it->FX->SpawnPosition.y, it->FX->SpawnPosition.z);
  }
}
...
void CTimedFXManager::dumpFXToRemove() const {
  ...
  nlinfo("FX to add");
  nlinfo("=========");
  for (TTimeStampedFXPtrSet::const_iterator it = _FXToRemove.begin();
       it != _FXToRemove.end(); ++it) {
    nlinfo("%s : day=%d, hour=%f, pos=(%.1f, %.1f, %.1f)",
           it->FX->FXSheet ? it->FX->FXSheet->FXName.c_str() : "???",
           (int)it->Date.Day, it->Date.Hour, it->FX->SpawnPosition.x,
           it->FX->SpawnPosition.y, it->FX->SpawnPosition.z);
...
void CTimedFXManager::dumpFXInfo() const {
  ...
  uint numInstance = 0;
  for (TManagedFXGroupList::const_iterator it = _FXGroups.begin();
       it != _FXGroups.end(); ++it) {
...
void CTimedFXManager::checkIntegrity() {
  ...
  for (TManagedFXGroupList::const_iterator it = _FXGroups.begin();
    ...
    for (uint k = 0; k < group.size(); ++k) {
      if (!group[k].Instance.empty()) {
        if (!(group[k].FXSheet &&
              group[k].FXSheet->Mode == CSeasonFXSheet::AlwaysStarted)) {
          ++numInstance;
        }
      }
    }
...
#ifdef NL_DEBUG
  for (TTimeStampedFXPtrSet::iterator it = _FXToAdd.begin();
       it != _FXToAdd.end(); ++it) {
    nlassert(it->FX->Magic == 0xbaadcafe);
  }
  for (TTimeStampedFXPtrSet::iterator it = _FXToRemove.begin();
       it != _FXToRemove.end(); ++it) {
    nlassert(it->FX->Magic == 0xbaadcafe);
  }
#endif
  ...
  if (fi.FXSheet) {
    for (uint k = 0; k < 4; ++k) {
      if (fi.FXSheet->UserParamsMax[k] != fi.FXSheet->UserParamsMin[k]) {
        float delta = computeUniformNoise(
            _Noise, fi.SpawnPosition + (float)1.23564f * k * CVector::I +
                        (float)cycle * 2.564848f * CVector::J);
        fi.Instance.setUserParam(k, fi.FXSheet->UserParamsMin[k] +
                                        delta * (fi.FXSheet->UserParamsMax[k] -
                                                 fi.FXSheet->UserParamsMin[k]));
      } else {
        fi.Instance.setUserParam(k, fi.FXSheet->UserParamsMin[k]);
      }
    }
...
const char *timedFXModeToStr(CSeasonFXSheet::TMode mode) {
  ...
  H_AUTO_USE(RZ_TimedFX)
  switch (mode) {
  case CSeasonFXSheet::AlwaysStarted:
    return "always started";
  case CSeasonFXSheet::UseEndHour:
    return "use end hour";
  case CSeasonFXSheet::UseDuration:
    return "use duration";
  case CSeasonFXSheet::Spawn:
    return "spawn";
  default:
    break;
  }
...
void CTimedFXManager::displayFXBoxes(TDebugDisplayMode displayMode) const {
  ...
  float textSize = 2.5f;
  // display fx to add
  for (TManagedFXGroupList::const_iterator groupIt = _FXGroups.begin();
       groupIt != _FXGroups.end(); ++groupIt) {
    for (uint k = 0; k < groupIt->Group.size(); ++k) {
      ...
      switch (mf.State) {
      case CManagedFX::Permanent:
        color = (!mf.Instance.empty()) ? CRGBA::Magenta : CRGBA::Cyan;
        break;
      case CManagedFX::InAddList:
        color = CRGBA::Blue;
        break;
      case CManagedFX::InRemoveList:
        color = (!mf.Instance.empty()) ? CRGBA::Red : CRGBA::Yellow;
        break;
      case CManagedFX::Unknown:
        break;
      }
      drawBox(CVector(-size, -size, -size), CVector(size, size, size), color);
      ...
      case SpawnDate:
        switch (mf.State) {
        case CManagedFX::Permanent:
          textToDisplay = NLMISC::toString("PERMANENT");
          break;
        case CManagedFX::InAddList:
        case CManagedFX::InRemoveList:
          textToDisplay = NLMISC::toString(
              "day:%d, hour:%d:%d", (int)mf.SetHandle->Date.Day,
              (int)mf.SetHandle->Date.Hour,
              (int)(60.f * fmodf(mf.SetHandle->Date.Hour, 1.f)));
          break;
        default:
          break;
        }
...
// *******************************************************************************************
void CTimedFXManager::linkCandidateFX(TCandidateFXListSortedByDist &targetList,
                                      float dist, CManagedFX *fx) {
  FPU_CHECKER
  H_AUTO_USE(RZ_TimedFX)
  if (!fx)
    return;
  ...
  fx->_PrevCandidateFX =
      &targetList[rank]; // NB the vector won't grow so no prb
  targetList[rank] = fx;
}

// *******************************************************************************************
void CTimedFXManager::linkInstanciatedFX(CManagedFX *&listHead,
                                         CManagedFX *fx) {
  FPU_CHECKER
  H_AUTO_USE(RZ_TimedFX)
  if (!fx)
...
void CTimedFXManager::updateInstanciatedFXList() {
  ...
  if (!_CandidateFXListTouched)
    return;
  // algo : we take at most '_MaxNumberOfFXInstances' from the start of the
  // updated list of candidate fxs (they are roughly sorted by distance) these
  // fx are inserted in new list of instanciated fxs. All fx that haven't been
  // inserted (they remains in the previous list) are discarded. At the end the
  // new list replaces the previous one
  ...
  sint maxNumPossibleInstance =
      (sint)(_MaxNumberOfFXInstances - _FXManager.getNumFXtoRemove());
  if (maxNumPossibleInstance > 0) {
    // NB: Ideally _CandidateFXListSortedByDist is small as a vector, so it's ok
    // to traverse it in its entirety.
    for (uint k = 0; k < numDistanceRanges; ++k) {
      ...
      while (currCandidate) {
        ...
        ++numInstances;
        if (numInstances == (uint)maxNumPossibleInstance)
  ...
  // removes old instances (those that were instanciated at previous frame, but
  // are not anymore)
  ...
  if (maxNumPossibleInstance > 0) {
    while (currFXToInstanciate) {
      ...
      // special case : if the fx must be spawned, then it must be a valid
      // candidate at the frame it is inserted, otherwise it is not created at
      // all (this is to avoid it to spawn at a later date, when it's too late)
      if (currFXToInstanciate->FXSheet &&
          currFXToInstanciate->FXSheet->Mode == CSeasonFXSheet::Spawn) {
        if (currFXToInstanciate->SetHandle->Date != _CurrDate) {
          oktoCreate = false;
          // nb : the fx will be removed from the 'instanciated list' at next
          // pass, because spawned fx are shutdown one frame after their
          // creation (see CTimedFXManager::update)
        }
      }
      if (oktoCreate) {
        ...
        NL3D::UInstance ts =
            _Scene->createInstance(currFXToInstanciate->FXSheet->FXName);
        if (ts.empty()) {
          ...
          if (currFXToInstanciate->Instance.empty()) {
            ...
            currFXToInstanciate->Instance.setTransformMode(
                NL3D::UTransform::DirectMatrix);
            currFXToInstanciate->Instance.setMatrix(
                currFXToInstanciate->getInstanceMatrix());
            if (currFXToInstanciate->State == CManagedFX::Permanent) {
              setupUserParams(*currFXToInstanciate, 0);
            } else {
              setupUserParams(*currFXToInstanciate,
                              currFXToInstanciate->SetHandle->Date.Day);
            }
  ...
  if (prevFXToInstanciate) {
    ...
    if (alreadyInstanciatedListHead) {
      alreadyInstanciatedListHead->_PrevInstanciatedFX =
          &prevFXToInstanciate->_NextInstanciatedFX;
    }
    // set new list
...
void CTimedFXManager::updateCandidateFXListSorting() {
  ...
  nlassert(_CandidateFXListSortedByDist.size() ==
           _CandidateFXListSortedByDistTmp.size());
  uint numDistanceRanges = (uint)_CandidateFXListSortedByDist.size();
  for (uint k = 0; k < numDistanceRanges; ++k) {
    CManagedFX *currCandidate = _CandidateFXListSortedByDist[k];
    while (currCandidate) {
...
// *******************************************************************************************
// temp, for debug
NLMISC_COMMAND(setMaxNumTimedFXs,
               "set the max number of timed fx that are visible at a time",
               "<numInst>") {
  if (args.size() != 1)
    return false;
