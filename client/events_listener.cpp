...
// Destructor
//---------------------------------------------------
CEventsListener::~CEventsListener() {} // ~CEventsListener //
...
void CEventsListener::operator()(const CEvent &event) {
  ...
  if (event == EventDestroyWindowId) {
    ...
    /* YOYO:
            ...
            Id did not succeed to reproduce the crash in
       CContinentMngr::select(). Hence I let it to quit "safely", calling
       release methods
    */
  ...
  if (event == EventCloseWindowId) {
    CAHManager::getInstance()->runActionHandler(
        "enter_modal", NULL, "group=ui:interface:quit_dialog");
  }
  // Event from the Mouse (MOVE)
    ...
    if (!MouseFreeLook) {
      ...
      sint scX = (sint32)(mouseEvent->X * (float)drW) -
                 ((sint32)drW >> 1); // in pixels, centered
      sint scY = (sint32)(mouseEvent->Y * (float)drH) - ((sint32)drH >> 1);
      if (!s_MouseFreeLookReady) {
        sint pscX = (sint32)(_MouseX * (float)drW) -
                    ((sint32)drW >> 1); // in pixels, centered
      ...
      // NOTE: No 0, 0 center mouse message in Windows (lower mouse message
      // rate), but safe to assume any movement messages are requeued relative
      // to our new position
      ...
      s_MouseFreeLookFrameY +=
          (scY - s_MouseFreeLookLastY) * (ClientCfg.FreeLookInverted ? -1 : 1);
  ...
  // Event from the Mouse (DOWN BUTTONS)
    ...
    TMouseButton mouseButton =
        (TMouseButton)((mousebutton->Button) &
  ...
  _MouseButtonReleased =
      (NLMISC::TMouseButton)(_MouseButtonReleased & ~_HandledUIButtonMask);
  _MouseButtonPushed =
      (NLMISC::TMouseButton)(_MouseButtonPushed & ~_HandledUIButtonMask);
...
// ***********************************************************************
void CEventsListener::smoothMouseCoordinates(float &x, float &y,
                                             float smoothingPeriod) {
  if (smoothingPeriod > 0.001f) {
    _MouseSmoother.setSamplingPeriod(smoothingPeriod);
    CVector2f smoothedPos =
        _MouseSmoother.samplePos(CVector2f(x, y), TimeInSec);
    x = smoothedPos.x;
    y = smoothedPos.y;
  }
}
...
void CEventsListener::updateMouseSmoothing() {
  if (_LastFreeLookUpdateDate != TimeInSec) {
    updateFreeLookPos((float)s_MouseFreeLookFrameX,
                      (float)s_MouseFreeLookFrameY);
...
void CEventsListener::updateFreeLookPos(float x, float y) {
  if (ClientCfg.FreeLookSmoothingPeriod == 0 || !_MouseSmoothingOn) {
    ...
    smoothMouseCoordinates(_CurrSmoothedMouseAX, _CurrSmoothedMouseAY,
                           ClientCfg.FreeLookSmoothingPeriod);
...
//---------------------------------------------------
float CEventsListener::getMousePosX() { return _MouseX; } // getMouseMoveX //

//---------------------------------------------------
// return a float the mousePosY.
//---------------------------------------------------
float CEventsListener::getMousePosY() { return _MouseY; } // getMouseMoveY //
