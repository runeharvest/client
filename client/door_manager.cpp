...
// ***************************************************************************
std::string CDoorManager::transformName(uint /* index */,
                                        const std::string & /* instanceName */,
                                        const std::string &shapeName) {
  if (shapeName.rfind('.') == string::npos)
    return shapeName;
  string sExt = toLowerAscii(
      shapeName.substr(shapeName.rfind('.') + 1, shapeName.size()));
  if (sExt != "pacs_prim")
    return shapeName;
...
void CDoorManager::loadedCallback(NL3D::UInstanceGroup *ig) {
  ...
  for (uint k = 0; k < numInstances; ++k) {
    ...
    if (sShapeName.rfind('.') == string::npos)
      continue;

    string sExt = toLowerAscii(
        sShapeName.substr(sShapeName.rfind('.') + 1, sShapeName.size()));
    if (sExt != "pacs_prim")
      continue;
    ...
    if (!sShapeName.empty()) {
      CUniquePtr<NLPACS::UPrimitiveBlock> pb(
          NLPACS::UPrimitiveBlock::createPrimitiveBlockFromFile(sShapeName));
      ...
      NLPACS::UMoveContainer::getPACSCoordsFromMatrix(pos, angle,
                                                      instanceMatrix);
      PACS->addCollisionnablePrimitiveBlock(pPB, 0, 1, &pDoor->Prims, angle,
                                            pos, true, scale);

      // Complete the user data of all the 'door primitives' with a pointer to
      // the door structure
        ...
        if (i != k) {
          ...
          if (sInstName == pDoor->Name) {
            ...
            case SDoor::Matis3PartBourgeon: {
              ...
              sDebug = toLowerAscii(
                  sDebug.substr(sDebug.rfind('_') + 1, sDebug.size()));
              if (sDebug == "gauche")
                pDoor->Instances[0] = i;
...
void CDoorManager::getPACSTriggers() {
  ...
  for (uint i = 0; i < nNbTrig; ++i) {
    ...
    if (((rTI.Object0 & 0xffff) == UserDataDoor) ||
        ((rTI.Object1 & 0xffff) == UserDataDoor)) {
