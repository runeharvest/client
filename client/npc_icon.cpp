...
// Time after which the state of a NPC is considered obsolete and must be
// refreshed (because it's in gamecycle, actual time increases if server slows
// down, to avoid server congestion)
NLMISC::TGameCycle CNPCIconCache::_CacheRefreshTimerDelay =
    NPC_ICON::DefaultClientNPCIconRefreshTimerDelayGC;

// Time between updates of the "catchall timer"
NLMISC::TGameCycle CNPCIconCache::_CatchallTimerPeriod =
    NPC_ICON::DefaultClientNPCIconRefreshTimerDelayGC;
...
CNPCIconCache::CNPCIconCache()
    : _LastRequestTimestamp(0), _LastTimerUpdateTimestamp(0), _Enabled(true) {
  _Icons[NPC_ICON::IconNone].init("", "");
  _Icons[NPC_ICON::IconNotAMissionGiver].init("", "");
  _Icons[NPC_ICON::IconListHasOutOfReachMissions].init("mission_available.tga",
                                                       ""); //"MP_Blood.tga"
  _Icons[NPC_ICON::IconListHasAlreadyTakenMissions].init("ICO_Task_Generic.tga",
                                                         "r2ed_tool_redo");
  _Icons[NPC_ICON::IconListHasAvailableMission].init(
      "mission_available.tga", "", CViewRadar::MissionList); //"MP_Wood.tga"
  _Icons[NPC_ICON::IconAutoHasUnavailableMissions].init("spe_com.tga", "");
  _Icons[NPC_ICON::IconAutoHasAvailableMission].init(
      "spe_com.tga", "", CViewRadar::MissionAuto); //"MP_Oil.tga"
  _Icons[NPC_ICON::IconStepMission].init(
      "mission_step.tga", "", CViewRadar::MissionStep); //"MP_Shell.tga"
...
const CNPCIconCache::CNPCIconDesc &
CNPCIconCache::getNPCIcon(const CEntityCL *entity, bool bypassEnabled) {
  // Not applicable? Most entities (creatures, characters) have a null key here.
  BOMB_IF(!entity, "NULL entity in getNPCIcon",
  ...
  // This method must be reasonably fast, because it constantly gets called by
  // the radar view
  ...
  if (!entity->canHaveMissionIcon())
    return _Icons[NPC_ICON::IconNone];
  if (!entity->isFriend()) // to display icons in the radar, we need the
                           // Contextual property to be received as soon as
                           // possible
    return _Icons[NPC_ICON::IconNone];

  // Temporarily not shown if the player is in interaction with the NPC
  if (UserEntity->interlocutor() != CLFECOMMON::INVALID_SLOT) {
    CEntityCL *interlocutorEntity =
        EntitiesMngr.entity(UserEntity->interlocutor());
    if (interlocutorEntity &&
        (entityToKey(interlocutorEntity) == npcIconCacheKey))
      return _Icons[NPC_ICON::IconNone];
  }
  ...
  if (img != _MissionGivers.end()) {
    ...
    if (giver.getState() != NPC_ICON::AwaitingFirstData) {
      ...
      // but only known mission givers that have a chance to propose new
      // missions
      if ((giver.getState() != NPC_ICON::NotAMissionGiver) &&
          //				(giver.getState() !=
          //NPC_ICON::ListHasAlreadyTakenMissions) && // commented out because
          //it would not refresh in case an auto mission become available
          (!giver.isDescTransient())) {
        NLMISC::TGameCycle informationAge =
            NetMngr.getCurrentServerTick() - giver.getLastUpdateTimestamp();
    ...
    CMissionGiverMap::iterator itg =
        _MissionGivers.insert(make_pair(npcIconCacheKey, giver)).first;
    queryMissionGiverData(npcIconCacheKey);
...
void CNPCIconCache::addObservers() {
  ...
  IngameDbMngr.addBranchObserver(IngameDbMngr.getNodePtr(), "MISSIONS",
  ...
  IngameDbMngr.addBranchObserver(
      IngameDbMngr.getNodePtr(), "CHARACTER_INFO:SKILLS",
      MissionPrerequisitEventObserver, skillLeavesToMonitor,
      getArraySize(skillLeavesToMonitor));

  // Owned Items
  static const char *bagLeavesToMonitor[1] = {
      "SHEET"}; // just saves 2000 bytes or so (500 * observer pointer entry in
                // vector) compared to one observer per bag slot
  IngameDbMngr.addBranchObserver(IngameDbMngr.getNodePtr(), "INVENTORY:BAG",
                                 MissionPrerequisitEventObserver,
                                 bagLeavesToMonitor,
                                 getArraySize(bagLeavesToMonitor));

  // Worn Items
  IngameDbMngr.addBranchObserver("INVENTORY:HAND",
                                 &MissionPrerequisitEventObserver);
  IngameDbMngr.addBranchObserver("INVENTORY:EQUIP",
                                 &MissionPrerequisitEventObserver);

  // Known Bricks
  IngameDbMngr.addBranchObserver("BRICK_FAMILY",
                                 &MissionPrerequisitEventObserver);

  // For other events, search for calls of
...
void CNPCIconCache::removeObservers() {
  ...
  // Mission Journal
  IngameDbMngr.getNodePtr()->removeBranchObserver("MISSIONS",
                                                  MissionStartStopObserver);
  IngameDbMngr.getNodePtr()->removeBranchObserver("MISSIONS",
                                                  MissionNpcAliasObserver);

  // Skills
  IngameDbMngr.getNodePtr()->removeBranchObserver(
      "CHARACTER_INFO:SKILLS", MissionPrerequisitEventObserver);

  // Owned Items
  IngameDbMngr.getNodePtr()->removeBranchObserver(
      "INVENTORY:BAG", MissionPrerequisitEventObserver);

  // Worn Items
  IngameDbMngr.getNodePtr()->removeBranchObserver(
      "INVENTORY:HAND", MissionPrerequisitEventObserver);
  IngameDbMngr.getNodePtr()->removeBranchObserver(
      "INVENTORY:EQUIP", MissionPrerequisitEventObserver);

  // Known Bricks
  IngameDbMngr.getNodePtr()->removeBranchObserver(
      "BRICK_FAMILY", MissionPrerequisitEventObserver);
}

void CNPCIconCache::CMissionStartStopObserver::update(ICDBNode *node) {
  // Every time a mission in progress is started or stopped, refresh the icon
  // for visible NPCs (including mission giver information)
...
void CNPCIconCache::CMissionPrerequisitEventObserver::update(ICDBNode *node) {
  // Every time a mission in progress changes, refresh the icon for the related
  // npc
...
void CNPCIconCache::queryMissionGiverData(TNPCIconCacheKey npcIconCacheKey) {
  ...
  // nldebug("%u: queryMissionGiverData           %u (total %u)",
  // NetMngr.getCurrentServerTick(), npcIconCacheKey, requests1.size());
}

void CNPCIconCache::queryAllVisibleMissionGiverData(
    NLMISC::TGameCycle olderThan) {
  // Request an update for all npcs (qualifying, i.e. that have missions) in
  // vision
  for (uint i = 0; i < EntitiesMngr.entities().size(); ++i) {
    ...
    CMissionGiverMap::iterator img = _MissionGivers.find(npcIconCacheKey);
    if (img == _MissionGivers.end())
      continue; // if the NPC does not have an entry yet, it will be created by
                // getNPCIcon()

    // Refresh only known mission givers that have a chance to propose new
    // missions
    CNPCMissionGiverDesc &giver = (*img).second;
    if (giver.getState() == NPC_ICON::NotAMissionGiver)
      continue;
    //		if (giver.getState() == NPC_ICON::ListHasAlreadyTakenMissions)
    //			continue; // commented out because it would not refresh
    //in case an auto mission becomes available
    ...
    if (olderThan != 0) {
      ...
      // Don't refresh NPC being involved in a mission goal (the step icon has
      // higher priority over the giver icon) If later the NPC is no more
      // involved before the information is considered old, it will show the
      // same giver state until the information is considered old. That's why we
      // let refresh the NPC when triggered by an event (olderThan == 0).
      if (isNPCaCurrentGoal(npcIconCacheKey))
        continue;
    }
    ...
    // nldebug("%u: queryAllVisibleMissionGiverData %u (total %u)",
    // NetMngr.getCurrentServerTick(), npcIconCacheKey, requests2.size());
  }
}

void CNPCIconCache::update() {
  // Every CatchallTimerPeriod, browse visible entities and refresh the ones
  // with outdated state (e.g. the ones not displayed in radar).
  if (NetMngr.getCurrentServerTick() >
      _LastTimerUpdateTimestamp + _CatchallTimerPeriod) {
  ...
  // Every tick update at most (2 cycles actually, cf. server<->client
  // communication frequency), send all pending requests in a single message.
  if (NetMngr.getCurrentServerTick() > _LastRequestTimestamp) {
    if (!_DescriptionsToRequest.empty()) {
      ...
      uint8 nb8 = uint8(_DescriptionsToRequest.size() &
                        0xff); // up to vision size (255 i.e. 256 minus user)
      out.serial(nb8);
      for (CSmallKeyList::const_iterator ikl = _DescriptionsToRequest.begin();
           ikl != _DescriptionsToRequest.end(); ++ikl) {
        TNPCIconCacheKey key = *ikl;
        out.serial(key);
      }
      NetMngr.push(out);
      // nldebug("%u: Pushing %hu NPC desc requests",
      // NetMngr.getCurrentServerTick(), nb8);
...
void CNPCIconCache::onEventForMissionInProgress() {
  ...
  // Ask the server to update availability status (will refresh icons if there
...
bool CNPCIconCache::isNPCaCurrentGoal(TNPCIconCacheKey npcIconCacheKey) const {
  ...
  for (CSmallKeyList::const_iterator ikl = _KeysOfCurrentGoals.begin();
       ikl != _KeysOfCurrentGoals.end(); ++ikl) {
...
void CNPCIconCache::storeKeysOfCurrentGoals() {
  // This event is very unfrequent, and the number of elements of
  // _KeysOfCurrentGoals is usually very small (typically 0 to 3, while
  // theoretical max is 15*20) so we don't mind rebuilding the list.
  _KeysOfCurrentGoals.clear();
  CCDBNodeBranch *missionNode = safe_cast<CCDBNodeBranch *>(
      IngameDbMngr.getNodePtr()->getNode(ICDBNode::CTextId("MISSIONS")));
  ...
  for (uint i = 0; i != nbCurrentMissionSlots; ++i) {
    ...
    CCDBNodeBranch *stepsToDoNode = safe_cast<CCDBNodeBranch *>(
        missionEntry->getNode(ICDBNode::CTextId("GOALS")));
    ...
    for (uint j = 0; j != nbGoals; ++j) {
      ...
      CCDBNodeLeaf *aliasNode = safe_cast<CCDBNodeLeaf *>(
          stepNode->getNode(ICDBNode::CTextId("NPC_ALIAS")));
      BOMB_IF(!aliasNode, "NPC_ALIAS node missing in MISSIONS DB", return);
      TNPCIconCacheKey npcIconCacheKey =
          (TNPCIconCacheKey)aliasNode->getValue32();
      if (npcIconCacheKey != 0)
...
void CNPCIconCache::refreshIconsOfScene(bool force) {
  ...
  for (uint i = 0; i < EntitiesMngr.entities().size(); ++i) {
    ...
    CMissionGiverMap::iterator it =
        _MissionGivers.find(CNPCIconCache::entityToKey(entity));
...
bool CNPCIconCache::onReceiveMissionAvailabilityForThisChar(
    TNPCIconCacheKey npcIconCacheKey, NPC_ICON::TNPCMissionGiverState state) {
  CMissionGiverMap::iterator img = _MissionGivers.find(npcIconCacheKey);
  BOMB_IF(img == _MissionGivers.end(),
          "Mission Giver " << npcIconCacheKey << "not found", return false);
...
bool CNPCMissionGiverDesc::updateMissionAvailabilityForThisChar(
...
std::string CNPCIconCache::getDump() const {
  string s = toString("System %s\nCurrent timers: %u %u\n",
                      ...
                      _CacheRefreshTimerDelay, _CatchallTimerPeriod);
  s += toString("%u NPCs in mission giver map:\n", _MissionGivers.size());
  for (CMissionGiverMap::const_iterator img = _MissionGivers.begin();
       img != _MissionGivers.end(); ++img) {
...
std::string CNPCMissionGiverDesc::getDump() const {
  return toString("%u [%u s ago]", _MissionGiverState,
                  (NetMngr.getCurrentServerTick() - _LastUpdateTimestamp) / 10);
}
...
NLMISC_COMMAND(queryMissionGiverData,
