...
std::string getAppBundlePath() {
  ...
  if (bundle) {
    ...
    if (url) {
      ...
      CFStringRef str;
      str = CFURLCopyFileSystemPath(CFURLCopyAbsoluteURL(url),
                                    kCFURLPOSIXPathStyle);
      CFRelease(url);

      if (str) {
        s_cachedPathToBundle =
            CFStringGetCStringPtr(str, CFStringGetSmallestEncoding(str));
        CFRelease(str);
      } else {
  ...
  if (GetModuleFileNameA(NULL, buffer, MAX_PATH))
    s_cachedPathToBundle =
        NLMISC::CPath::standardizePath(NLMISC::CFile::getPath(buffer), false);
