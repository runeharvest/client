...
// ***************************************************************************
COutpost::COutpost() { _OutpostId = -1; }

// ***************************************************************************
COutpost::~COutpost() { removeOutpost(); }
...
// ***************************************************************************
bool COutpost::setupOutpost(const CContinentParameters::CZC &zone,
                            sint32 outpostId, CVillage *village) {
  // Yoyo. legacy code. should no more be needed now. Still let for check
  // (useful?)
...
// ***************************************************************************
void COutpost::setBuildingPosition(uint building, const NLMISC::CQuat &rot,
...
void COutpost::initOutpost() {
  ...
  if (_OutpostId > -1) {
    // Register RZ_MAX_BUILDING_PER_OUTPOST observers for the
    // RZ_MAX_BUILDING_PER_OUTPOST buildings
    uint i;
    for (i = 0; i < RZ_MAX_BUILDING_PER_OUTPOST; i++) {
      // Put the ZC pacs_prim
      TPacsPrimMap::iterator pbIt = PacsPrims.find(NLMISC::toLowerAscii(
          NLMISC::CFile::getFilenameWithoutExtension(ClientCfg.ZCPacsPrim)));
      if (pbIt != PacsPrims.end()) {
        ...
        if (PACS)
          PACS->addCollisionnablePrimitiveBlock(pbIt->second, 0, 1,
                                                &_AddedPrims, angle, pos, true,
                                                CVector(1, 1, 1));
      }
    }
  }
