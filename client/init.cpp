...
//	if (XmlAllocUsesSTL)
        {
                int size = *(((int *) ptr) - 1);
                xmlStlAlloc.deallocate((uint8 *) ptr - sizeof(int), size +
sizeof(int));
        }
// 	else
// 	{
...
static INT_PTR CALLBACK ExitClientErrorDialogProc(HWND hwndDlg, UINT uMsg,
                                                  WPARAM wParam,
                                                  LPARAM /* lParam */) {
  switch (uMsg) {
  case WM_INITDIALOG: {
    if (CI18N::hasTranslation("TheSagaOfRyzom")) {
      if (!SetWindowTextW(hwndDlg,
                          nlUtf8ToWide(CI18N::get("TheSagaOfRyzom").c_str()))) {
        nlwarning("SetWindowText failed: %s",
                  formatErrorMessage(getLastError()).c_str());
      }
    }
    SetDlgItemTextW(hwndDlg, IDC_ERROR_MSG_TEXT,
                    (WCHAR *)CurrentErrorMessage.c_str());
    ...
    SetWindowPos(
        ...
        (rectDesktop.right - rectDesktop.left - rect.right + rect.left) / 2,
        (rectDesktop.bottom - rectDesktop.top - rect.bottom + rect.top) / 2, 0,
        0, SWP_NOSIZE);
    HICON exitClientDlgIcon =
        LoadIcon(HInstance, MAKEINTRESOURCE(IDI_MAIN_ICON));
    ::SendMessageA(hwndDlg, WM_SETICON, (WPARAM)ICON_BIG,
                   (LPARAM)exitClientDlgIcon);
  } break;
  case WM_COMMAND:
...
void ExitClientError(const char *format, ...) {
  ...
  DialogBox(HInstance, MAKEINTRESOURCE(IDD_ERROR_HELP_MESSAGE_BOX), NULL,
            ExitClientErrorDialogProc);
  /*
          MessageBoxW (NULL, nlUtf8ToWide(str.c_str()),
     nlUtf8ToWide(CI18N::get("TheSagaOfRyzom").c_str()), MB_OK|MB_ICONERROR);
  */
#else
  fprintf(stderr, "%s\n", str);
#endif
  ...
  quitCrashReport();
  NLMISC::NL3D_BlockMemoryAssertOnPurge =
...
#ifdef NL_OS_WINDOWS
  MessageBoxW(NULL, nlUtf8ToWide(message.c_str()),
              nlUtf8ToWide(CI18N::get("TheSagaOfRyzom").c_str()),
...
#ifdef NL_OS_WINDOWS
  return MessageBoxW(NULL, nlUtf8ToWide(message.c_str()),
                     nlUtf8ToWide(CI18N::get("TheSagaOfRyzom").c_str()),
...
// For nel memory
void outOfMemory() { nlstopex(("OUT OF MEMORY")); }
...
void displayCPUInfo() {
  nlinfo("CPUInfo: CPUMask before change: %x, after change: %x, CPUID: %x, "
         "hasHyperThreading: %s",
         (uint32)Debug_OldCPUMask, (uint32)Debug_NewCPUMask,
         CSystemInfo::getCPUID(),
         (CSystemInfo::hasHyperThreading() ? "YES" : "NO"));
}

string getVersionString(uint64 version) {
  return toString("%u.%u.%u.%u", (unsigned int)(version >> 48),
                  (unsigned int)((version >> 32) & 0xffff),
                  (unsigned int)((version >> 16) & 0xffff),
...
string getSystemInformation() {
  string s;
  s += "Memory: " +
       bytesToHumanReadable(CSystemInfo::availablePhysicalMemory()) + "/" +
       bytesToHumanReadable(CSystemInfo::totalPhysicalMemory()) + "\n";
  s += "Process Virtual Memory: " +
       bytesToHumanReadable(CSystemInfo::virtualMemory()) + "\n";
...
void checkDriverVersion() {
  ...
  uint64 driverVersion;
  if (CSystemInfo::getVideoInfo(deviceName, driverVersion)) {
    static uint64 driversVersion[] = {NVIDIA_RECOMMANDED_DRIVERS,
                                      ATI_RECOMMANDED_DRIVERS};
    static const char *driversTest[] = {NVIDIA_RECOMMANDED_DRIVERS_STRING_TEST,
                                        ATI_RECOMMANDED_DRIVERS_STRING_TEST};
    static const char *driversNTest[] = {
        NVIDIA_RECOMMANDED_DRIVERS_STRING_NTEST, NULL};
    ...
    for (i = 0; i < sizeofarray(driversVersion); i++) {
      ...
      if ((lwr.find(driversTest[i]) != string::npos) &&
          (driversNTest[i] == NULL ||
           lwr.find(driversNTest[i]) == string::npos)) {
        if (driverVersion < driversVersion[i]) {
          string message =
              CI18N::get("uiUpdateDisplayDriversNotUpToDate") + "\n\n";
          // message += CI18N::get ("uiUpdateDisplayDriversVendor") +
          // driversVendor[i] + "\n";
          message +=
              CI18N::get("uiUpdateDisplayDriversCard") + deviceName + "\n";
          message += CI18N::get("uiUpdateDisplayDriversCurrent") +
                     getVersionString(driverVersion) + "\n";
          message += CI18N::get("uiUpdateDisplayDriversRecommanded") +
                     getVersionString(driversVersion[i]) + "\n\n";
...
void listStereoDisplayDevices(std::vector<NL3D::CStereoDeviceInfo> &devices) {
  ...
  for (std::vector<NL3D::CStereoDeviceInfo>::iterator it(devices.begin()),
       end(devices.end());
       it != end; ++it) {
    std::string name =
        toString("%s - %s - %s", IStereoDisplay::getLibraryName(it->Library),
                 it->Manufacturer.c_str(), it->ProductName.c_str());
    std::string fullname =
        toString("[%s] [%s]", name.c_str(), it->Serial.c_str());
    nlinfo("VR [C]: Stereo Display: %s", name.c_str());
    if (cache) {
      VRDeviceCache.push_back(
          std::pair<std::string, std::string>(name, it->Serial)); // VR_CONFIG
    }
  }
}
    ...
    if (ClientCfg.VRDisplayDevice == std::string("Auto")) {
      ...
      for (std::vector<NL3D::CStereoDeviceInfo>::iterator it(devices.begin()),
        ...
        std::string name = toString(
            "%s - %s - %s", IStereoDisplay::getLibraryName(it->Library),
            it->Manufacturer.c_str(), it->ProductName.c_str());
...
static void addPaths(IProgressCallback &progress,
                     const std::vector<std::string> &paths, bool recurse) {
  ...
  directoryPrefixes.push_back(
      CPath::makePathAbsolute(getAppBundlePath() + "/..", ".", true));
#elif defined(NL_OS_UNIX)
  // check in same directory as executable
  directoryPrefixes.push_back(Args.getProgramPath());

  // check in installed directory
  if (CFile::isDirectory(getRyzomSharePrefix()))
    directoryPrefixes.push_back(CPath::standardizePath(getRyzomSharePrefix()));
#endif

  std::set<std::string> directoriesToProcessSet;
  std::vector<std::string> directoriesToProcess;

  // first pass, build a map with all existing directories to process in second
  // pass
  for (uint j = 0; j < directoryPrefixes.size(); j++) {
    ...
    for (uint i = 0; i < paths.size(); i++) {
      ...
      if (!directory.empty() && !directoryPrefix.empty() &&
          !CPath::isAbsolutePath(directory))
        directory = directoryPrefix + directory;

      // only process existing directories
      if (CFile::isExists(directory)) {
        if (directoriesToProcessSet.find(directory) ==
            directoriesToProcessSet.end()) {
  ...
  for (size_t i = 0; i < directoriesToProcess.size(); ++i) {
    ...
    progress.pushCropedValues((float)i / (float)directoriesToProcess.size(),
                              (float)(i + 1) /
                                  (float)directoriesToProcess.size());
...
void initStreamedPackageManager(NLMISC::IProgressCallback &progress) {
  ...
  nlassert(
      !spm.Provider); // If this asserts, init was called twice without release
...
void addSearchPaths(IProgressCallback &progress) {
  ...
  InterfaceAddOnManager.addSearchFiles("uiaddon", "*.xml;*.lua;*.tga",
                                       "login_*.xml;out_v2_*.xml", &progress);
...
void addPreDataPaths(NLMISC::IProgressCallback &progress) {
  ...
  // nlinfo ("PROFILE: %d seconds for Add search paths Predata",
  // (uint32)(ryzomGetLocalTime ()-initPaths)/1000);
}
...
static bool addRyzomIconBitmap(const std::string &directory,
                               vector<CBitmap> &bitmaps) {
  if (CFile::isDirectory(directory)) {
    // build filename from directory and default ryzom client icon name
    std::string filename = NLMISC::toString("%s/%s.png", directory.c_str(),
                                            getRyzomClientIcon().c_str());
...
void initLog() {
  ...
  CFileDisplayer *ClientLogDisplayer =
  ...
  nlinfo("Memory: %s/%s",
         bytesToHumanReadable(CSystemInfo::availablePhysicalMemory()).c_str(),
         bytesToHumanReadable(CSystemInfo::totalPhysicalMemory()).c_str());
  ...
  if (setrlimit(RLIMIT_NOFILE, &rlp2)) {
    if (errno == EINVAL) {
      nlwarning(
          "Unable to set rlimit with error: the specified limit is invalid");
    } else if (errno == EPERM) {
      nlwarning("Unable to set rlimit with error: the limit specified would "
  ...
  nlinfo("rlimit after %llu %llu", (uint64)rlp3.rlim_cur,
         (uint64)rlp3.rlim_max);
...
void prelogInit() {
  try {
    ...
    // Due to Bug #906, we disable the stl xml allocation
    // nlverify (xmlMemSetup (XmlFree4NeL, XmlMalloc4NeL, XmlRealloc4NeL,
    // XmlStrdup4NeL) == 0);
    ...
    // create the save dir.
    if (!CFile::isExists("save"))
      CFile::createDirectory("save");

    // create the user dir.
    if (!CFile::isExists("user"))
      CFile::createDirectory("user");

#if !FINAL_VERSION
      // if we're not in final version then start the file access logger to keep
      // track of the files that we read as we play
      // ICommand::execute("iFileAccessLogStart",*NLMISC::InfoLog);
#endif

    // check "BuildName" in ClientCfg
    // nlassert(!ClientCfg.BuildName.empty()); // TMP comment by nico do not
    // commit

    // Start memory allocation log
    // 		if (ClientCfg.LogMemoryAllocation)
    // 			NLMEMORY::StartAllocationLog ("alloc.memlog",
    // ClientCfg.LogMemoryAllocationSize);
    ...
    FPU_CHECKER_ONCE {
      ...
      STRING_MANAGER::CStringManagerClient::initI18NSpecialWords(
    ...
    // Initialize the VR devices (even more important than the most important
    // part of the client)
...
#ifdef NL_OS_WINDOWS
    uintptr_t icon =
        (uintptr_t)LoadIcon(HInstance, MAKEINTRESOURCE(IDI_MAIN_ICON));
#else
    uintptr_t icon = 0;
#endif // NL_OS_WINDOWS
    ...
    CLoginProgressPostThread::getInstance().step(
        CLoginStep(LoginStep_VideoModeSetup, "login_step_video_mode_setup"));
    ...
    CLoginProgressPostThread::getInstance().step(
        CLoginStep(LoginStep_VideoModeSetupHighColor,
    ...
    if (homeDirectory)
      directoryPrefixes.push_back(CPath::standardizePath(homeDirectory) +
                                  ".local");
    ...
    for (size_t j = 0; j < iconSizes.size(); ++j) {
      ...
      for (size_t i = 0; i < directoryPrefixes.size(); ++i) {
        ...
        std::string directory =
            toString("%s/share/icons/hicolor/%ux%u/apps",
                     directoryPrefixes[i].c_str(), size, size);

        // if found, skip other directories for this icon size
        if (addRyzomIconBitmap(directory, bitmaps))
          break;
      }
    }
    ...
    // for background downloader : store this window handle in shared memory for
    // later access (we use SendMessage to communicate with the background
    // downloader)
    ...
    if (Driver->getAvailableVertexAGPMemory() == 0) {
      ...
      // for radeon 7200, patch because agp crash with agp with OpenGL -> don't
      display the message if (!(Driver->getNbTextureStages() <= 3 &&
      strstr(deviceName.c_str(), "radeon")))
      {*/
    ...
    // Create a text context. We need to put the full path because we not
    // already add search path
    ...
    CWidgetManager::getInstance()->setWindowSnapInvert(
        ClientCfg.WindowSnapInvert);
    CWidgetManager::getInstance()->setWindowSnapDistance(
        ClientCfg.WindowSnapDistance);

    // Yoyo: initialize NOW the InputHandler for Event filtering.
    CInputHandlerManager *InputHandlerManager =
    ...
    // Create the loading texture. We can't do that before because we need to
    // add pre search path first and driver
    ...
    if (!ClientCfg.TestBrowser) {
      ...
      // nlinfo ("PROFILE: %d seconds for Add search paths Data",
      // (uint32)(ryzomGetLocalTime ()-initPaths)/1000);
    }
    ...
    nlinfo("PROFILE: %d seconds for prelogInit",
            ...
            if ((ext == "item") || (ext == "sitem")) {
              ...
              if (itemId.buildSheetId(NLMISC::toLowerAscii(strShape))) {
                ...
                CItemSheet *itemSheet =
                    dynamic_cast<CItemSheet *>(SheetMngr.get(itemId));
                if (itemSheet) {
...
void postlogInit() {
  ...
  try {
    ...
    NLMISC::TTime initCurrent = initLast;
    {
      ...
      // nlinfo ("PROFILE: %d seconds (%d total) for Initializing network",
...

    {
      ...
      initLast = initCurrent;
      initCurrent = ryzomGetLocalTime();
      // nlinfo ("PROFILE: %d seconds (%d total) for Initializing chat manager",
      // (uint32)(initCurrent-initLast)/1000,
      // (uint32)(initCurrent-initStart)/1000);
    }

    {
      ...
      if (!LigoConfig.readPrimitiveClass(ClientCfg.LigoPrimitiveClass.c_str(),
                                         false)) {
        nlwarning("Can't load primitive class file %s",
                  ClientCfg.LigoPrimitiveClass.c_str());
      }

      initLast = initCurrent;
      initCurrent = ryzomGetLocalTime();
      // nlinfo ("PROFILE: %d seconds (%d total) for Initializing primitive
      // classes", (uint32)(initCurrent-initLast)/1000,
      // (uint32)(initCurrent-initStart)/1000);
    }
...

    {
      ...
      SheetMngr.load(ProgressBar, ClientCfg.UpdatePackedSheet,
                     ClientCfg.NeedComputeVS, ClientCfg.DumpVSIndex);

      initLast = initCurrent;
      initCurrent = ryzomGetLocalTime();
      // nlinfo ("PROFILE: %d seconds (%d total) for Initializing sheets",
      // (uint32)(initCurrent-initLast)/1000,
      // (uint32)(initCurrent-initStart)/1000);
    }

    {
      ...
      // STRING_MANAGER::CStringManagerClient::specialWordsMemoryCompress(); //
      // Must be done after brick manager init

      initLast = initCurrent;
      initCurrent = ryzomGetLocalTime();
      // nlinfo ("PROFILE: %d seconds (%d total) for Initializing bricks",
      // (uint32)(initCurrent-initLast)/1000,
      // (uint32)(initCurrent-initStart)/1000);
    }
    ...
    if (!ClientCfg.Light) {
      ...
      initLast = initCurrent;
      initCurrent = ryzomGetLocalTime();
      // nlinfo ("PROFILE: %d seconds (%d total) for Initializing Color Slot
      // etc.", (uint32)(initCurrent-initLast)/1000,
      // (uint32)(initCurrent-initStart)/1000);
    } else {
    ...
    // Initialize MovieShooter
    {
      ...
      initLast = initCurrent;
      initCurrent = ryzomGetLocalTime();
      // nlinfo ("PROFILE: %d seconds (%d total) for Initializing Movie
      // Shooter", (uint32)(initCurrent-initLast)/1000,
      // (uint32)(initCurrent-initStart)/1000);
    }

    {
      ...
      initLast = initCurrent;
      initCurrent = ryzomGetLocalTime();
      // nlinfo ("PROFILE: %d seconds (%d total) for Initializing primitives",
      // (uint32)(initCurrent-initLast)/1000,
      // (uint32)(initCurrent-initStart)/1000);
    }

    {
      ...
      initLast = initCurrent;
      initCurrent = ryzomGetLocalTime();
      // nlinfo ("PROFILE: %d seconds (%d total) for Executing cfg file start
      // commands", (uint32)(initCurrent-initLast)/1000,
      // (uint32)(initCurrent-initStart)/1000);
    }
    ...
    nlinfo("PROFILE: %d seconds for postlogInit",
           (uint32)(ryzomGetLocalTime() - initStart) / 1000);
  } catch (const Exception &e) {
    ExitClientError(e.what());
  }
