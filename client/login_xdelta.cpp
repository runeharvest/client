...
// ---------------------------------------------------------------------------
bool CXDPFileReader::init(const std::string &sFilename, sint32 nLowerBound,
                          sint32 nUpperBound, bool bCompressed) {
...
#endif
    {
      nlwarning("%s: corrupt or truncated delta: cannot seek to %d",
                sFilename.c_str(), nLowerBound);
      return false;
    }
...
bool CXDPFileReader::readUInt(uint32 &val) {
  // This is mostly because I dislike endian, and less to save space on small
  // ints
  uint8 c;
  uint8 arr[16];
  int i = 0;
...
bool SXDeltaCtrl::SSourceInfo::read(CXDPFileReader &fr) {
  if (!fr.readString(Name))
    return false;
  if (!fr.read(MD5.Data, 16))
    return false;
  if (!fr.readUInt(Len))
    return false;
  if (!fr.readBool(IsData))
    return false;
  if (!fr.readBool(Sequential))
    return false;
...
bool SXDeltaCtrl::SInstruction::read(CXDPFileReader &fr) {
  if (!fr.readUInt(Index))
    return false;
  if (!fr.readUInt(Offset))
    return false;
  if (!fr.readUInt(Length))
    return false;
...
bool SXDeltaCtrl::read(CXDPFileReader &fr) {
  ...
  if (!fr.readUInt32(nSize))
    return false;
  nSize = netToHost(nSize);

  // ----

  if (!fr.read(ToMD5.Data, 16))
    return false;
  if (!fr.readUInt(ToLen))
    return false;
  if (!fr.readBool(HasData))
    return false;

  uint32 i, nSourceInfoLen, nInstLen;

  if (!fr.readUInt(nSourceInfoLen))
    return false;
  SourceInfo.resize(nSourceInfoLen);
  for (i = 0; i < nSourceInfoLen; ++i)
    SourceInfo[i].read(fr);

  if (!fr.readUInt(nInstLen))
    return false;
...
bool CXDeltaPatch::load(const string &sFilename) {
  ...
  // Check the version
  if (strncmp((const char *)&vMagicBuffer[0], XDELTA_110_PREFIX,
              XDELTA_PREFIX_LEN) != 0) {
  ...
  // if the flag patch compressed is on the part from nCtrlOffset to
  // nEndCtrlOffset is the delta compressed
  CXDPFileReader frFile;
  if (!frFile.init(sFilename, nCtrlOffset, nEndCtrlOffset,
                   (_Flags & XDELTA_FLAG_PATCH_COMPRESSED) != 0)) {
...
// ---------------------------------------------------------------------------
CXDeltaPatch::TApplyResult CXDeltaPatch::apply(const std::string &sFileToPatch,
                                               const std::string &sFileOutput,
                                               std::string &errorMsg) {
  if ((_Flags & XDELTA_FLAG_FROM_COMPRESSED) ||
      (_Flags & XDELTA_FLAG_TO_COMPRESSED)) {
  ...
  if (_Ctrl.SourceInfo.size() == 1) {
    ...
    if (rInfo.IsData) {
      ...
      if (!XDFR[0].init(_FileName, _HeaderOffset, _CtrlOffset,
                        isPatchCompressed())) {
        fclose(outFILE);
  ...
  if (!checkIntegrity(xdfrOut, _Ctrl.ToMD5, _Ctrl.ToLen)) {
    errorMsg =
        toString("integrity problem with output file %s", sFileOutput.c_str());
...
// ---------------------------------------------------------------------------
bool CXDeltaPatch::checkIntegrity(CXDPFileReader &rFR, const CHashKeyMD5 &md5,
                                  uint32 nLength) {
...
// ---------------------------------------------------------------------------
CXDeltaPatch::TApplyResult CXDeltaPatch::apply(
    const string &sPatchFilename, const string &sFileToPatchFilename,
    const string &sOutputFilename, std::string &errorMsg, ICallBack *pCB) {
  CXDeltaPatch patch;
  ...
  _CallBack = pCB;
  TApplyResult ar =
      patch.apply(sFileToPatchFilename, sOutputFilename, errorMsg);
...
bool CXDeltaPatch::info(const std::string &sPatchFilename) {
  ...
  for (uint32 i = 0; i < patch.getCtrl().SourceInfo.size(); ++i) {
    ...
    nlinfo("%s\t%d\t%d\t%d\t%s\t%s\n", rSI.MD5.toString().c_str(), rSI.Len,
           rSI.Copies, rSI.CopyLength, rSI.Sequential ? "yes" : "no",
           rSI.Name.c_str());
  }
  return true;
}
