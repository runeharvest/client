...
void CHeightGrid::addTri(const CVector2f corners[3], float z)
{
        ...
        for(sint y = 0; y < numRasters; ++y)
        {
                ...
                for (ge.X = r.first; ge.X <= r.second; ++ge.X)
                {
                        ...
                        {
                                _ZGrid(ge.X - _PosX, ge.Y - _PosY) = std::max(z,
_ZGrid(ge.X - _PosX, ge.Y - _PosY));
                        }
                }
        }
}
...
//
***********************************************************************************
void CHeightGrid::init(float cellSize, uint heightGridSize, uint
wrappedHeightGridSize, float minZ)
{
...
void CHeightGrid::updateBBox()
{
        _BBox.setMinMax(CVector(_PosX * _CellSize, _PosY * _CellSize, _MinZ),
                            CVector((_PosX + _GridSize) * _CellSize, (_PosY +
_GridSize) * _CellSize, -_MinZ));
}
...
//
***********************************************************************************
void CHeightGrid::addCollisionMesh(const
...
void CHeightGrid::updateRect(const NLMISC::CRect &rect)
{
        ...
        CAABBox bbox;
        bbox.setMinMax(CVector(rect.X * _CellSize, rect.Y * _CellSize, _MinZ),
CVector((rect.X + rect.Width) * _CellSize, (rect.Y + rect.Height) * _CellSize, -
_MinZ)); CollisionManager->getMeshs(bbox, _UpdateMeshs); for(uint k = 0; k <
_UpdateMeshs.size(); ++k)
        {
        ...
        {
                for (uint x = (uint) rect.X; x < (uint) (rect.X + rect.Width);
++x)
                {
                        updateCell(x, y);
                }
...
void CHeightGrid::discardRect(const CRect &rect)
{
        ...
        CAABBox bbox;
        bbox.setMinMax(CVector(rect.X * _CellSize, rect.Y * _CellSize, _MinZ),
CVector((rect.X + rect.Width) * _CellSize, (rect.Y + rect.Width) * _CellSize, -
_MinZ)); CollisionManager->getMeshs(bbox, _UpdateMeshs); for(uint k = 0; k <
_UpdateMeshs.size(); ++k)
        {
                if (!_UpdateMeshs[k].WorldBBox->intersect(_BBox))
                {
                        removeCollisionMesh(_UpdateMeshs[k].ID); // remove mesh
...
void CHeightGrid::updateCell(sint px, sint py)
{
        ...
        float z = _MinZ;
        const CHeightGridWrapped::TGridElemList &pl =
_ZGridWrapped.getGridElemList(worldX, worldY);
        for(CHeightGridWrapped::TGridElemList::const_iterator it = pl.begin();
it != pl.end(); ++it)
        {
...
void	CHeightGrid::display(NL3D::UDriver &drv) const
{
        ...
        for (sint y = _PosY; y < (sint)  (_PosY + _GridSize - 1); ++y)
        {
                for (sint x = _PosX; x < (sint) (_PosX + _GridSize - 1); ++x)
                {
                        ...
                        CVector pos1 = gridCoordToWorld(x + 1, y) + z1 *
CVector::K; CVector pos2 = gridCoordToWorld(x + 1, y + 1) + z2 * CVector::K;
                        CVector pos3 = gridCoordToWorld(x, y + 1) + z3 *
CVector::K; drv.drawLine(CLine(pos0, pos1), m); drv.drawLine(CLine(pos1, pos2),
m); drv.drawLine(CLine(pos2, pos3), m); drv.drawLine(CLine(pos3, pos0), m);
                }
        }
        ...
        //	{
        //		const CPointGrid::TPointList &pl =
_PointGrid.getPointList(x ,y);
        //		for(CPointGrid::TPointList::const_iterator it =
pl.begin(); it != pl.end(); ++it)
        //		{
        ...
        nlwarning("Max list length = %d", (int)
_ZGridWrapped.getListMaxLength());
}
...
{
        uint sizeMax = 0;
        for(CArray2D<TGridElemList>::const_iterator it = _Grid.begin(); it !=
_Grid.end(); ++it)
        {
...
//=====================================================================
void CPrecipitationClipGrid::initGrid(uint size, float gridEltSizeX,
                                      float gridEltSizeY) {
...
//=====================================================================
void CPrecipitationClipGrid::updateGrid(const NLMISC::CVector & /* userPos */,
  ...
  /*
          ...
          if (_Touched)
          {
                  ...
                  updateGridPart(newX, newY, 0, 0, _Size + 1, _Size + 1, gr);
                          ...
                          double freq = (double)
     CSystemInfo::getProcessorFrequency(); double msPerTick = 1000 / (double)
     freq; nldebug("Updated precipitation clip grid : time = %.2f ms", (float)
     (clock.getNumTicks() * msPerTick));
          ...
          if (newX > _XPos) // moved right ?
          {
                  ...
                  if (height > 0)
                  {
                          ...
                          updateGridPart(newX + _Size + 1 - width, newY -
     offsetY, _Size + 1 - width, std::max(-offsetY, 0), width, height, gr);
                  }
          }
          else if (newX < _XPos) // moved left ?
          {
                  ...
                  {
                          // complete column on the right
                          updateGridPart(newX, newY - offsetY, 0,
     std::max(-offsetY, 0), width, height, gr);
                  }
          }
          ...
          if (newY > _YPos)
          {
                  ...
                  updateGridPart(newX, newY + _Size + 1 - height, 0, _Size + 1 -
     height, _Size + 1, height, gr);
          }
          else
          if (newY < _YPos)
          {
                  ...
                  nldebug("Updated precipitation clip grid : time = %.2f ms",
     (float) (clock.getNumTicks() * msPerTick));
                  // display the height in the clip grid
                  printGridValues();
          #endif
...
void CPrecipitationClipGrid::printGridValues() const {
  for (uint k = 0; k <= _Size; ++k) {
    ...
    for (uint l = 0; l <= _Size; ++l) {
      ...
      outStr +=
          toString("%5d%c ", (uint)gridIt->MeanHeight, l == _Size ? ' ' : ',');
    }
    nlinfo(outStr.c_str());
  }
}
...
void CPrecipitationClipGrid::moveGrid(sint offsetX, sint offsetY) {
  static TGrid otherGrid; // the grid is likely to be rather small so it's ok to
                          // keep it static
  ...
  if (width > 0 && height > 0) {
    ...
    TGrid::iterator dstIt =
        getGridIt(otherGrid, srcX + offsetX, srcY + offsetY, _Size);
    do {
...
//=====================================================================
const CPrecipitationClipGrid::CGridPoint *
CPrecipitationClipGrid::get(sint x, sint y) const {
  // pos in the grid
  sint gx = x - _XPos;
  sint gy = y - _YPos;
  if ((uint)gx > (_Size + 1) || (uint)gy > (_Size + 1))
    return NULL;
...
//=====================================================================
void CPrecipitationClipGrid::updateGridPart(sint worldX, sint worldY,
                                            uint gridX, uint gridY, uint width,
                                            uint height,
                                            NLPACS::UGlobalRetriever *gr) {
    ...
    for (uint x = 0; x < width; ++x) {
      if (!gr) {
        ...
        NLPACS::UGlobalPosition gpos =
            gr->retrievePosition(currWorldPos, 20000.f);
...
void CPrecipitationClipGrid::display(NL3D::UDriver &drv) const {
  ...
  for (uint y = 0; y < _Size + 1; ++y) {
    for (uint x = 0; x < _Size + 1; ++x) {
      ...
      drv.drawLine((float)(x * stepX - 1), (float)(y * stepY),
                   (float)(x * stepX + 1), (float)(y * stepY), col);
      drv.drawLine((float)(x * stepX), (float)(y * stepY - 1),
                   (float)(x * stepX), (float)(y * stepY + 1), col);
    }
  }
