...
void CFactionWarManager::addFactionWar(PVP_CLAN::CFactionWar fw) {
  ...
  nldebug("<CFactionWarManager::addFactionWar> Adding faction war %s vs %s",
          PVP_CLAN::toString(fw.Clan1).c_str(),
          PVP_CLAN::toString(fw.Clan2).c_str());
...
void CFactionWarManager::stopFactionWar(PVP_CLAN::CFactionWar fw) {
  ...
  for (it = _FactionWarOccurs.begin(); it != _FactionWarOccurs.end(); ++it) {
    if ((*it).inPvPFaction(fw.Clan1, fw.Clan2)) {
      nldebug(
          "<CFactionWarManager::stopFactionWar> Stopping faction war %s vs %s",
          PVP_CLAN::toString(fw.Clan1).c_str(),
          PVP_CLAN::toString(fw.Clan2).c_str());
...
//----------------------------------------------------------------------------
bool CFactionWarManager::areFactionsInWar(PVP_CLAN::TPVPClan clan1,
...
//----------------------------------------------------------------------------
CFactionWarManager::~CFactionWarManager() { _FactionWarOccurs.clear(); }
