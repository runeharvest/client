      ...
      if (cp) {
        ...
        for (sint32 y = 0; y < (sint32)Screen.getHeight(); ++y) {
          for (sint32 x = 0; x < (sint32)Screen.getWidth(); ++x) {
            if (cp->contains(
                    CVector2f((float)(x - cornerX), (float)(y - cornerY)))) {
              ((CRGBA *)&Screen
                   .getPixels()[0])[x + (Screen.getHeight() - 1 - y) *
                                            Screen.getWidth()] = CRGBA::Magenta;
            }
          }
        }
      }
...
void tempDumpColPolys() {
  ...
  if (pw) {
    ...
    if (island && island->Island != currIsland) {
      ...
      texturedMaterial.texEnvArg0RGB(0, CMaterial::Texture,
                                     CMaterial::SrcColor);
      ...
      mat.setScale(scale);
      mat.setPos(
          CVector(-island->XMin * scale.x, -island->YMin * scale.y, 0.f));
      //
      CMatrix uvScaleMat;
      //
    ...
    //
    driver->setFrustum(frust.Left, frust.Right, frust.Bottom, frust.Top,
                       frust.Near, frust.Far, frust.Perspective);
    ...
    const CVector localFrustCorners[8] = {
        ...
        CVector(frust.Left * frust.Far / frust.Near, frust.Far,
                frust.Top * frust.Far / frust.Near),
        CVector(frust.Right * frust.Far / frust.Near, frust.Far,
                frust.Top * frust.Far / frust.Near),
        CVector(frust.Right * frust.Far / frust.Near, frust.Far,
                frust.Bottom * frust.Far / frust.Near),
        CVector(frust.Left * frust.Far / frust.Near, frust.Far,
                frust.Bottom * frust.Far / frust.Near)};
    ...
    */
    ...
    for (uint k = 0; k < zones.size(); ++k) {
      zones[k]->render(vb, *driver, texturedMaterial, wiredMaterial,
                       MainCam.getMatrix(), TRI_BATCH_SIZE, localFrustCorners);
    }
  }
}
