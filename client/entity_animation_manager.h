...
public:
   ...
   * \return CMovingAutomatonState * : pointer on the moving automaton state or
   * 0.
   */
  const CAutomatonStateSheet *mState(const std::string &automaton,
                                     TAnimStateKey key);
  ...
  /**
   * Process all logical tracks(sound, etc..) which come along with the
   * animation. \param string animListName : the animation list in witch to
   * search. \param TAnimStateKey animationId : the id of the animation. \param
   * CAnimationTime startTimeOffset :the start time in the animation play.
   * \param CAnimationTime endTimeOffset : the end time in the animation play.
   */
  void processLogic(const std::string &animListName, TAnimStateKey animationId,
                    CAnimationTime startTimeOffset,
                    CAnimationTime endTimeOffset, NL3D::UTrack *&soundTrack,
                    std::vector<NL3D::CAnimationTime> &result);
   ...
   */
  bool interpolate(uint idAnim, double timeOffset,
                   NLMISC::CVector &result) const {
   ...
   */
  double getAnimMinSpeedFactor(const std::string &animSet,
                               const TAnimStateId &animStateId,
                               const CAnimation::TAnimId &idAnim) const;
   ...
   */
  double getAnimMaxSpeedFactor(const std::string &animSet,
                               const TAnimStateId &animStateId,
                               const CAnimation::TAnimId &idAnim) const;
   ...
   * \return UAnimationSet * : a pointer on the animation set or 0 if there is
   * not animation set.
   */
   ...
   * \return bool : true if the result has been filled, false if the index is
   * invalid.
   */
   ...
   */
  //	NL3D::UPlayList *CEntityAnimationManager::createFacePlayList(const
  //CTypeEntity& type);
   ...
   * \return UAnimationSet * : a pointer on the animation set or 0 if there is
   * not animation set for this type.
   */
   ...
   */
  //	uint chooseFaceAnim(const CTypeEntity& type, const std::string
  //&emotion);
};
