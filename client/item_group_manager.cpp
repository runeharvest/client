...
bool CItemGroup::contains(CDBCtrlSheet *other,
  ...
  for (int i = 0; i < Items.size(); i++) {
    CItem item = Items[i];
    if (item.useCreateTime() && item.createTime == other->getItemCreateTime() &&
        item.serial == other->getItemSerial()) {
      slot = item.slot;
      return true;
    }
    // Present for compatibility reasons
    NLMISC::CSheetId sheet = NLMISC::CSheetId(other->getSheetId());
    if (sheet.toString() == item.sheetName &&
        other->getQuality() == item.quality &&
        other->getItemWeight() == item.weight &&
        other->getItemColor() == item.color &&
        (!item.usePrice || (other->getItemPrice() >= item.minPrice &&
                            other->getItemPrice() <= item.maxPrice))) {
      slot = item.slot;
      return true;
    }
  }
...
void CItemGroup::addItem(sint32 createTime, sint32 serial,
                         SLOT_EQUIPMENT::TSlotEquipment slot) {
  // Don't add an item if it already exists, this could cause issue
  //  It's happening either if we are creating a group with a 2 hands items (and
  //  the item is found both in handR and handL) Or if an user incorrectly edit
  //  his group file
  for (int i = 0; i < Items.size(); i++) {
    if (Items[i].createTime == createTime && Items[i].serial == serial) {
      nldebug("Not adding duplicate item, createTime: %d, serial: %d",
              createTime, serial);
      // In this case, we are adding the duplicate item for a 2 hands item
      // If it's saved as a left hand item, save it as a right hand item instead
      // (so we have only 1 correct item)
      if (Items[i].slot == SLOT_EQUIPMENT::HANDL &&
          slot == SLOT_EQUIPMENT::HANDR)
...
void CItemGroup::writeTo(xmlNodePtr node) {
  xmlNodePtr groupNode =
      xmlNewChild(node, NULL, (const xmlChar *)"group", NULL);
  xmlSetProp(groupNode, (const xmlChar *)"name", (const xmlChar *)name.c_str());
  for (int i = 0; i < Items.size(); i++) {
    CItem item = Items[i];
    xmlNodePtr itemNode =
        xmlNewChild(groupNode, NULL, (const xmlChar *)"item", NULL);
    if (item.useCreateTime()) {
      xmlSetProp(itemNode, (const xmlChar *)"createTime",
                 (const xmlChar *)NLMISC::toString(item.createTime).c_str());
      xmlSetProp(itemNode, (const xmlChar *)"serial",
                 (const xmlChar *)NLMISC::toString(item.serial).c_str());
    }
    // Present for compatibility reasons
    else {
      xmlSetProp(itemNode, (const xmlChar *)"sheetName",
                 (const xmlChar *)item.sheetName.c_str());
      xmlSetProp(itemNode, (const xmlChar *)"quality",
                 (const xmlChar *)NLMISC::toString(item.quality).c_str());
      xmlSetProp(itemNode, (const xmlChar *)"weight",
                 (const xmlChar *)NLMISC::toString(item.weight).c_str());
      xmlSetProp(itemNode, (const xmlChar *)"color",
                 (const xmlChar *)NLMISC::toString(item.color).c_str());
      xmlSetProp(itemNode, (const xmlChar *)"minPrice",
                 (const xmlChar *)NLMISC::toString(item.minPrice).c_str());
      xmlSetProp(itemNode, (const xmlChar *)"maxPrice",
                 (const xmlChar *)NLMISC::toString(item.maxPrice).c_str());
    }
    // We need to save slot only if it's useful for clarity
    // if(item.slot == SLOT_EQUIPMENT::HANDL || item.slot ==
    // SLOT_EQUIPMENT::HANDR)
    xmlSetProp(itemNode, (const xmlChar *)"slot",
               (const xmlChar *)SLOT_EQUIPMENT::toString(item.slot).c_str());
  }
  for (int i = 0; i < removeBeforeEquip.size(); i++) {
    xmlNodePtr removeNode =
        xmlNewChild(groupNode, NULL, (const xmlChar *)"remove", NULL);
    xmlSetProp(
        removeNode, (const xmlChar *)"slot",
        (xmlChar *)SLOT_EQUIPMENT::toString(removeBeforeEquip[i]).c_str());
  }
}
...
void CItemGroup::readFrom(xmlNodePtr node) {
  ...
  while (curNode) {
    if (strcmp((char *)curNode->name, "item") == 0) {
      ...
      if (ptrName)
        NLMISC::fromString((const char *)ptrName, item.createTime);
      ptrName = (char *)xmlGetProp(curNode, (xmlChar *)"serial");
      if (ptrName)
        NLMISC::fromString((const char *)ptrName, item.serial);
      ...
      if (ptrName)
        NLMISC::fromString((const char *)ptrName, item.sheetName);
      ptrName = (char *)xmlGetProp(curNode, (xmlChar *)"quality");
      if (ptrName)
        NLMISC::fromString((const char *)ptrName, item.quality);
      ptrName = (char *)xmlGetProp(curNode, (xmlChar *)"weight");
      if (ptrName)
        NLMISC::fromString((const char *)ptrName, item.weight);
      ptrName = (char *)xmlGetProp(curNode, (xmlChar *)"color");
      if (ptrName)
        NLMISC::fromString((const char *)ptrName, item.color);
      ptrName = (char *)xmlGetProp(curNode, (xmlChar *)"minPrice");
      if (ptrName)
        NLMISC::fromString((const char *)ptrName, item.minPrice);
      ptrName = (char *)xmlGetProp(curNode, (xmlChar *)"maxPrice");
      if (ptrName)
        NLMISC::fromString((const char *)ptrName, item.maxPrice);
      item.usePrice = (item.minPrice != 0 ||
                       item.maxPrice != std::numeric_limits<uint32>::max());
...
void CItemGroupManager::linkInterface() {
  ...
  CGroupMenu *pRootMenu = dynamic_cast<CGroupMenu *>(
      pWM->getElementFromId("ui:interface:item_menu_in_bag"));
  CGroupSubMenu *pMenu = pRootMenu->getRootMenu();
  // get item subgroup
  CGroupMenu *pGroupMenu = dynamic_cast<CGroupMenu *>(
      pWM->getElementFromId("ui:interface:item_menu_in_bag:item_group_menu"));
  ...
  if (pMenu && pGroupSubMenu)
    ...
    nlwarning("Couldn't link group submenu to item_menu_in_bag, check your "
              "widgets.xml file");
}
...
void CItemGroupManager::saveGroups() {
  std::string userGroupFileName =
      "save/groups_" + PlayerSelectedFileName + ".xml";
  if (PlayerSelectedFileName.empty()) {
    nlwarning(
        "Trying to save group with an empty PlayerSelectedFileName, aborting");
    return;
  }
  try {
    NLMISC::COFile f;
    if (f.open(userGroupFileName, false, false, true)) {
      ...
      xmlNodePtr node =
          xmlNewDocNode(doc, NULL, (const xmlChar *)"item_groups", NULL);
    ...
    nlwarning("Error while writing the file %s : %s.",
              userGroupFileName.c_str(), e.what());
  }
}
...
bool CItemGroupManager::loadGroups() {
  ...
  if (PlayerSelectedFileName.empty()) {
    nlwarning(
        "Trying to load group with an empty PlayerSelectedFileName, aborting");
    return false;
  }
  if (!NLMISC::CFile::fileExists(userGroupFileName) ||
      NLMISC::CFile::getFileSize(userGroupFileName) == 0) {
...
void CItemGroupManager::update() {
  if (_StartInvalidAction != 0 &&
      _StartInvalidAction <= NetMngr.getCurrentServerTick()) {
    invalidActions(_StartInvalidAction, _EndInvalidAction);
    _StartInvalidAction = 0;
  }
  if (_EndInvalidAction != 0 &&
      _EndInvalidAction <= NetMngr.getCurrentServerTick()) {
    _EndInvalidAction = 0;
    validActions();
  }
  ...
  if (!_MigrationDone && pIM) {
    NLMISC::CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
...
bool CItemGroupManager::migrateGroups() {
  ...
  // This is not very optimised, but this will be executed only once (and
  // removed in the near future)
  for (int i = 0; i < _Groups.size(); i++) {
    CItemGroup group = _Groups[i];
    // Migrate the group only if there is items inside, and the first one hasn't
    // been migrated
    bool needMigration =
        group.Items.size() > 0 && !group.Items[0].useCreateTime();
    if (!needMigration) {
...
CItemGroup CItemGroupManager::migrateGroup(CItemGroup group) {
  ...
  for (int i = 0; i < INVENTORIES::NUM_ALL_INVENTORY; i++) {
    ...
    for (int j = 0; j < items.size(); j++) {
      SLOT_EQUIPMENT::TSlotEquipment slot = SLOT_EQUIPMENT::UNDEFINED;
      // slot might be undefined here, but we want it for clarity purpose in the
      // xml (to easily find lines)
      if (items[j].slot != SLOT_EQUIPMENT::UNDEFINED)
        slot = items[j].slot;
      // We can't get a perfect match (can't know if it's a right/left jewel for
      // example), but it's good enough
      else {
        ...
        else if (sheet->hasSlot(SLOTTYPE::HEADDRESS))
          slot = SLOT_EQUIPMENT::HEADDRESS;
        else if (sheet->hasSlot(SLOTTYPE::NECKLACE))
          slot = SLOT_EQUIPMENT::NECKLACE;
        else if (sheet->hasSlot(SLOTTYPE::FINGERS))
          slot = SLOT_EQUIPMENT::FINGERL;
        else if (sheet->hasSlot(SLOTTYPE::ANKLE))
          slot = SLOT_EQUIPMENT::ANKLEL;
        else if (sheet->hasSlot(SLOTTYPE::WRIST))
          slot = SLOT_EQUIPMENT::WRISTL;
        else if (sheet->hasSlot(SLOTTYPE::EARS))
          slot = SLOT_EQUIPMENT::EARL;
        // Armor
        // Helmet
        else if (sheet->hasSlot(SLOTTYPE::HEAD))
          slot = SLOT_EQUIPMENT::HEAD;
        // Gloves
        else if (sheet->hasSlot(SLOTTYPE::HANDS))
          slot = SLOT_EQUIPMENT::HANDS;
        // Sleeves
        else if (sheet->hasSlot(SLOTTYPE::ARMS))
          slot = SLOT_EQUIPMENT::ARMS;
        // Vest
        else if (sheet->hasSlot(SLOTTYPE::CHEST))
          slot = SLOT_EQUIPMENT::CHEST;
        // Boots
        else if (sheet->hasSlot(SLOTTYPE::FEET))
          slot = SLOT_EQUIPMENT::FEET;
        // pants
        else if (sheet->hasSlot(SLOTTYPE::LEGS))
          slot = SLOT_EQUIPMENT::LEGS;
        else
          slot = SLOT_EQUIPMENT::UNDEFINED;
      }
      out.addItem(items[j].pCS->getItemCreateTime(),
                  items[j].pCS->getItemSerial(), slot);
    }
  }
  return out;
}
...
void CItemGroupManager::fakeInvalidActions(NLMISC::TGameCycle time) {
  // We cannot directly ivnalidate action or our invalidate will be overriden by
  // the server (and that means we won't actually have one because it's buggy
  // with multiple equip in a short time) So we wait a bit (currently 6 ticks is
  // enough) to do it
...
void CItemGroupManager::invalidActions(NLMISC::TGameCycle begin,
                                       NLMISC::TGameCycle end) {
  NLGUI::CDBManager *pDB = NLGUI::CDBManager::getInstance();
  NLMISC::CCDBNodeLeaf *node;
  // This are the db update server sends when an user equip an item, see
  // egs/player_manager/gear_latency.cpp CGearLatency::setSlot
...
bool CItemGroupManager::moveGroup(std::string name,
  ...
  for (int i = 0; i < INVENTORIES::NUM_ALL_INVENTORY; i++) {
    ...
    if (inventory != dst && pIM->isInventoryAvailable(inventory)) {
      ...
      for (int i = 0; i < items.size(); i++) {
        ...
        // Workaround: sometimes item are marked as equipped by
        // pIM->isBagItemWeared() even tho they aren't really Because of a
        // synchronisation error between client and server
        if (isItemReallyEquipped(item.pCS))
          continue;
        CAHManager::getInstance()->runActionHandler("move_item", item.pCS,
                                                    moveParams);
      }
    }
  }
...
bool CItemGroupManager::equipGroup(std::string name, bool pullBefore) {
  ...
  for (int i = 0; i < group->removeBeforeEquip.size(); i++) {
    ...
    std::string dbPath;
    // For hands equip, dbPath obviously starts at 0, we need to offset
    // correctly
    if (slot == SLOT_EQUIPMENT::HANDL || slot == SLOT_EQUIPMENT::HANDR)
      dbPath = "LOCAL:INVENTORY:HAND:" +
               NLMISC::toString((uint32)slot - SLOT_EQUIPMENT::HANDL);
    else
  ...
  for (int i = 0; i < items.size(); i++) {
    ...
    // We'll equip items in left hand later (the right hand will be normally
    // equipped) This way, if we switch from 2 hands to 2 * 1 hands, both hands
    // will be equipped correctly (first right, which will remove the 2 hands,
    // then left) If we don't, we might try to equip the left hand first, which
    // will do nothing because we have a 2 hands equipped
    if (item.slot == SLOT_EQUIPMENT::HANDL) {
      duals.push_back(item);
      continue;
    }

    // If the item can be weared 2 times, don't automatically equip the second
    // one Or else it will simply replace the first. We'll deal with them later
  ...
  // For some reason, there is no (visual) invalidation (server still blocks any
  // action), force one Unfortunately, there is no clean way to do this, so
  // we'll simulate one
  if (maxEquipTime > 0)
...
bool CItemGroupManager::createGroup(std::string name, bool removeUnequiped) {
  ...
  for (i = 0; i < MAX_EQUIPINV_ENTRIES; ++i) {
    ...
    // Instead of doing two separate for, just be a bit tricky for hand
    // equipment
...
bool CItemGroupManager::isItemReallyEquipped(CDBCtrlSheet *item) {
  ...
  for (uint32 i = 0; i < MAX_EQUIPINV_ENTRIES; ++i) {
    ...
    if (!pCS)
      continue;
    // Can't directly compare ID (as pCS is like
    // "ui:interface:inv_equip:content:equip:armors:feet" and item is like
    // "ui:interface:inv_pa3:content:iil:bag_list:list:sheet57") Instead check
    // inventory + slot
...
std::vector<CInventoryItem>
CItemGroupManager::matchingItems(CItemGroup *group,
  ...
  if (dbPath.empty()) {
    nldebug("Inventory type %s not supported",
            INVENTORIES::toString(inventory).c_str());
    return out;
  }
  ...
  for (uint i = 0; i < MAX_BAGINV_ENTRIES; i++) {
    ...
    if (group->contains(pCS, slot)) {
      // Sometimes, index in the list differ from the index in DB, and we need
      // the index in DB, not the one from the list
