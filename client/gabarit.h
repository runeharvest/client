...
struct CSkeletonGabarit {
  ...
  void buildFromSkeleton(NL3D::USkeleton src, const std::string &skelName,
                         bool bMale);
...
public:
   ...
   * \param finalHeightScale if not NULL, will be filled with height scale of
   * the skeleton (factor)
   */
  void applyGabarit(NL3D::USkeleton dest, uint gender,
                    EGSPD::CPeople::TPeople race, float height,
                    float torsoWidth, float armsWidth, float legsWidth,
                    float breastSize, float *finalHeightScale);
