...
// It is a bitfield indexed by BGDownloader::TDownloadID that indicate the parts
// that are available for patch
uint32 AvailablePatchs = 0;
// next wanted patch for the background downloader
BGDownloader::TDownloadID BGDownloaderWantedPatch =
...
#define CTRL_EDITBOX_CREATEACCOUNT_LOGIN                                       \
  "ui:login:create_account:content:submit_gr:eb_login:eb"
#define CTRL_EDITBOX_CREATEACCOUNT_PASSWORD                                    \
  "ui:login:create_account:content:submit_gr:eb_password:eb"
#define CTRL_EDITBOX_CREATEACCOUNT_CONFIRMPASSWORD                             \
  "ui:login:create_account:content:submit_gr:eb_confirm_password:eb"
#define CTRL_EDITBOX_CREATEACCOUNT_EMAIL                                       \
  "ui:login:create_account:content:submit_gr:eb_email:eb"
...
// ***************************************************************************
// Pop a fatal error message box, giving the option to 'quit' the client, plus a
// help button
...
// ***************************************************************************
// Pop an error message box, giving the option to go back to main menu, plus a
// help button
...
void createOptionalCatUI() {
  ...
  if (pList == NULL) {
    nlwarning("element " GROUP_LIST_CAT
  ...
  for (uint i = 0; i < InfoOnPatch.OptCat.size(); i++) {
    ...
    CInterfaceGroup *pNewLine =
        CWidgetManager::getInstance()->getParser()->createGroupInstance(
            "t_cat", GROUP_LIST_CAT, params);
    if (pNewLine != NULL) {
      CViewText *pVT = dynamic_cast<CViewText *>(pNewLine->getView("name"));
      if (pVT != NULL)
        pVT->setText(InfoOnPatch.OptCat[i].Name);
...
void initEula() {
  ...
  if (!ClientCfg.SkipEULA &&
    ...
    NLGUI::CDBManager::getInstance()
        ->getDbProp("UI:VARIABLES:SCREEN")
        ->setValue32(UI_VARIABLES_SCREEN_EULA);

    // if we display the eula, it means we make a patch so we clean the cache
    // directory (clear cache after each patch)
...
static void setDataScanLog(const std::string &text) {
  ...
  CViewText *pVT =
      dynamic_cast<CViewText *>(CWidgetManager::getInstance()->getElementFromId(
          "ui:login:datascan:content:log_txt:log"));
  if (pVT != NULL) {
    pVT->setText(text);
  }
}
...
// ***************************************************************************
static void setDataScanState(const std::string &text,
                             const std::string &progress = string()) {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  CViewText *pVT =
      dynamic_cast<CViewText *>(CWidgetManager::getInstance()->getElementFromId(
          "ui:login:datascan:content:state"));
  if (pVT != NULL)
    pVT->setText(text);

  pVT =
      dynamic_cast<CViewText *>(CWidgetManager::getInstance()->getElementFromId(
          "ui:login:datascan:content:progress"));
  if (pVT != NULL)
    pVT->setText(progress);
}
...
void initCatDisplay() {
  ...
  if ((!InfoOnPatch.NonOptCat.empty()) || (!InfoOnPatch.OptCat.empty()) ||
      (!InfoOnPatch.ReqCat.empty())) {
    createOptionalCatUI();
    NLGUI::CDBManager::getInstance()
        ->getDbProp("UI:VARIABLES:SCREEN")
        ->setValue32(UI_VARIABLES_SCREEN_CATDISP);
  } else {
    LoginSM.pushEvent(CLoginStateMachine::ev_run_patch);
    //		CAHManager::getInstance()->runActionHandler("login_patch",
    //NULL);
  }
}
...
void initReboot() {
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("UI:VARIABLES:SCREEN")
      ->setValue32(UI_VARIABLES_SCREEN_REBOOT);
}

// ***************************************************************************
static void setPatcherStateText(const std::string &baseUIPath,
                                const std::string &str) {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  CViewText *pVT =
      dynamic_cast<CViewText *>(CWidgetManager::getInstance()->getElementFromId(
          baseUIPath + ":content:state"));
  if (pVT != NULL) {
    pVT->setText(str);
  }
}
...
// ***************************************************************************
static void setPatcherProgressText(const std::string &baseUIPath,
                                   const std::string &str) {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  CViewText *pVT =
      dynamic_cast<CViewText *>(CWidgetManager::getInstance()->getElementFromId(
          baseUIPath + ":content:progress"));
  if (pVT != NULL) {
    pVT->setText(str);
  }
}
...
static void updatePatchingInfoText(const std::string &baseUIPath) {
  ...
  if (isBGDownloadEnabled()) {
    bgDownloader.update();
    if (bgDownloader.getDownloadThreadPriority() ==
      ...
      if (bgDownloader.getTotalFilesToGet() != 0) {
        setPatcherProgressText(
            baseUIPath,
            toString("%d/%d", (int)bgDownloader.getCurrentFilesToGet(),
                     (int)bgDownloader.getTotalFilesToGet()));
      } else {
        setPatcherProgressText(baseUIPath, string());
      }
    }
...
#endif
  {
    ...
    if (pPM->getThreadState(state, log)) {
      ...
      if (pPM->getTotalFilesToGet() != 0) {
        setPatcherProgressText(baseUIPath,
                               toString("%d/%d", pPM->getCurrentFilesToGet(),
                                        pPM->getTotalFilesToGet()));
      } else {
        setPatcherProgressText(baseUIPath, string());
      }
    }
...
void loginMainLoop() {
  ...
  //	while (loginFinished == false)
  {
    ...
    //		sint32 screen =
    //NLGUI::CDBManager::getInstance()->getDbProp("UI:VARIABLES:SCREEN")->getValue32();
    if (LoginSM.getCurrentState() == CLoginStateMachine::st_check_patch)
    //		if (screen == UI_VARIABLES_SCREEN_CHECKING) // If we are in
    //checking mode
    {
      ...
      if (finished) {
        ...
        if (isBGDownloadEnabled()) {
          ...
          case BGDownloader::TaskResult_Success:
            if (AvailablePatchs &
                (1 << BGDownloaderWantedPatch)) // is there a patch for what we
                                                // want now ?
            {
    ...
    //		else if (screen == UI_VARIABLES_SCREEN_DATASCAN) // If we are in
    //ScanData mode
      ...
      if (pPM->isScanDataThreadEnded(res)) {
        ...
        if (NLGUI::CDBManager::getInstance()
                ->getDbProp("UI:VARIABLES:DATASCAN_RUNNING")
                ->getValue32() != 0) {
          // no more running
          NLGUI::CDBManager::getInstance()
              ->getDbProp("UI:VARIABLES:DATASCAN_RUNNING")
        ...
        if (pPM->getThreadState(state, log)) {
          // set state
          setDataScanState(state, toString("%d/%d", pPM->getCurrentFilesToGet(),
                                           pPM->getTotalFilesToGet()));
        }
    ...
    //		else if (screen == UI_VARIABLES_SCREEN_PATCHING) // If we are in
    //patching mode
      ...
      if (isBGDownloadEnabled()) {
        ...
        if (finished) {
          // bgDownloader.hideDownloader();
          //  restore the search paths (all bnp files were removed from CPath to
          //  allows the patcher to overwrite them)
    ...
    //		else if (screen == UI_VARIABLES_SCREEN_CATDISP) // If we are
    //displaying patch info
    else if (LoginSM.getCurrentState() == CLoginStateMachine::st_display_cat) {
      // Non optional stuff (non opt cat and req cat)

      // Add req cat size : given the optional cat we determines the required
      // cat
      ...
      if (pList != NULL) {
        for (uint i = 0; i < InfoOnPatch.OptCat.size(); i++) {
          ...
          if (pLine != NULL) {
            CCtrlButton *pCB =
                dynamic_cast<CCtrlButton *>(pLine->getCtrl("on_off"));
            if ((pCB != NULL) && (pCB->getPushed())) {
              ...
              if (InfoOnPatch.OptCat[i].Req != -1) {
                ...
                if (j == ReqCat.size()) {
                  ...
                  nNonOptSize +=
                      InfoOnPatch.ReqCat[InfoOnPatch.OptCat[i].Req].Size;
                }
              }
            }
          }
      ...
      // Total size of the patches is optional cats + required cat (f(optCat)) +
      // non opt cat

      CViewText *pVT = dynamic_cast<CViewText *>(
          CWidgetManager::getInstance()->getElementFromId(VIEW_TOTAL_SIZE));
      if (pVT != NULL)
        pVT->setText(BGDownloader::getWrittenSize(TotalPatchSize));

      pVT = dynamic_cast<CViewText *>(
          CWidgetManager::getInstance()->getElementFromId(
              VIEW_NON_OPTIONAL_SIZE));
      if (pVT != NULL)
        pVT->setText(BGDownloader::getWrittenSize(nNonOptSize));
    }
  }
}
...
void initLoginScreen() {
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("UI:VARIABLES:SCREEN")
      ->setValue32(UI_VARIABLES_SCREEN_CHECKPASS);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("UI:VARIABLES:DISPLAY_ACCOUNT_BUTTONS")
      ->setValue32(ClientCfg.DisplayAccountButtons);
  ...
  CViewText *pV =
      dynamic_cast<CViewText *>(CWidgetManager::getInstance()->getElementFromId(
          "ui:login:checkpass:content:ver_value"));
  if (pV)
  ...
  if (!l.empty()) {
    ...
    CAHManager::getInstance()->runActionHandler("set_keyboard_focus", NULL,
  ...
  CCtrlTextButton *pCB = dynamic_cast<CCtrlTextButton *>(
...
void initAutoLogin() {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  CGroupEditBox *pGEBLog = dynamic_cast<CGroupEditBox *>(
      CWidgetManager::getInstance()->getElementFromId(CTRL_EDITBOX_LOGIN));
  CGroupEditBox *pGEBPwd = dynamic_cast<CGroupEditBox *>(
  ...
  if (ClientCfg.R2Mode) {
    ...
    if (ShardSelected == -1) {
      ...
      LoginSM.pushEvent(CLoginStateMachine::ev_login_ok);
      //		CAHManager::getInstance()->runActionHandler("login_connect_2",
      //NULL);
    }
  }
}
...
bool login() {
  CLoginProgressPostThread::getInstance().step(
      CLoginStep(LoginStep_LoginScreen, "login_step_login_screen"));
  ...
  //		CInterfaceElement *backToLogin =
  //CWidgetManager::getInstance()->getElementFromId(CTRL_BUTTON_BACKTOLOGIN); 		if
  //(backToLogin) 			backToLogin->setActive(false);
  //
  //		// show 'reload test page' button
  //		CInterfaceElement *reloadTest =
  //CWidgetManager::getInstance()->getElementFromId(CTRL_BUTTON_RELOADTESTPAGE);
  //		if (reloadTest)
  //			reloadTest->setActive(true);
  //
  //		// start the browser
  //		CGroupHTML *pGH =
  //dynamic_cast<CGroupHTML*>(CWidgetManager::getInstance()->getElementFromId(GROUP_BROWSER));
  //
  ...
  //	{
  //		CGroupEditBox *pGEB =
  //dynamic_cast<CGroupEditBox*>(CWidgetManager::getInstance()->getElementFromId(CTRL_EDITBOX_LOGIN));
  //		if (pGEB != NULL)
  //			pGEB->setInputString(l);
  //		CAHManager::getInstance()->runActionHandler("set_keyboard_focus",
  //NULL, "target=" CTRL_EDITBOX_PASSWORD "|select_all=false");
  //	}
  //	else
  //	{
  //		CAHManager::getInstance()->runActionHandler("set_keyboard_focus",
  //NULL, "target=" CTRL_EDITBOX_LOGIN "|select_all=false");
  //	}

  ShardSelected = -1;
  //	CCtrlTextButton *pCB =
  //dynamic_cast<CCtrlTextButton*>(CWidgetManager::getInstance()->getElementFromId(CTRL_BUTTON_CONNECT));
  ...
  //	{
  //		CInterfaceManager *pIM = CInterfaceManager::getInstance();
  //		CGroupEditBox *pGEBLog =
  //dynamic_cast<CGroupEditBox*>(CWidgetManager::getInstance()->getElementFromId(CTRL_EDITBOX_LOGIN));
  //		CGroupEditBox *pGEBPwd =
  //dynamic_cast<CGroupEditBox*>(CWidgetManager::getInstance()->getElementFromId(CTRL_EDITBOX_PASSWORD));
  //		pGEBLog->setInputString(LoginLogin);
  //		pGEBPwd->setInputString(LoginPassword);
  //		CAHManager::getInstance()->runActionHandler("on_login", NULL,
  //"");
  ...
  //
  //		if (ShardSelected == -1)
  //			pIM->messageBox(CI18N::get("uiErrServerLost"),
  //"ui:login"); 		else
  //			CAHManager::getInstance()->runActionHandler("login_connect_2",
  //NULL);
  //	}
...
// ***************************************************************************
static void getPatchParameters(std::string &url, std::string &ver,
...
std::string getBGDownloaderCommandLine() {
#ifdef NL_DEBUG
  CConfigFile::CVar *bgdCommandLine =
      ClientCfg.ConfigFile.getVarPtr("BackgroundDownloaderCommandLine");
  ...
  std::string commandLine =
      /*R2ServerVersion + " " + VersionName + " " + */ url + " " + ver;
...
void initPatchCheck() {
  ...
  if (!ClientCfg.R2Mode) {
    ...
    // In previous code, the following was not set in R2Mode, possible bug
    // ?...let as before anyway ... store the selected shard for restarting
    // after patch
...
#endif
  {
    ...
    CBGDownloaderAccess::getInstance().requestDownloadThreadPriority(
        BGDownloader::ThreadPriority_Normal, false);
    CBGDownloaderAccess::getInstance().startTask(
        taskDesc, getBGDownloaderCommandLine(), true /* showDownloader */);
  }
#endif
  NLGUI::CDBManager::getInstance()
      ->getDbProp("UI:VARIABLES:SCREEN")
      ->setValue32(UI_VARIABLES_SCREEN_CHECKING);
...
void initShardDisplay() {
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("UI:VARIABLES:SCREEN")
      ->setValue32(UI_VARIABLES_SCREEN_SHARDDISP);

  CInterfaceGroup *pList = dynamic_cast<CInterfaceGroup *>(
      CWidgetManager::getInstance()->getElementFromId(GROUP_LIST_SHARD));
  if (pList == NULL) {
    nlwarning("element " GROUP_LIST_SHARD
              " not found probably bad login_main.xml");
    return;
  }
  ...
  {
          CShard s (	toString("%05d",fff), fff%3, fff+32,
  toString("%s%d","pipo",fff), 32*fff%46546, "32.32.32.32",
  "http://www.ryzom.com" ); Shards.push_back(s);
  }*/
  ...
  for (uint i = 0; i < Shards.size(); i++) {
    ...
    CInterfaceGroup *pNewLine =
        CWidgetManager::getInstance()->getParser()->createGroupInstance(
            "t_shard", GROUP_LIST_SHARD, params);
    if (pNewLine != NULL) {
      CViewText *pVT = dynamic_cast<CViewText *>(pNewLine->getView("name"));
      if (pVT != NULL)
        pVT->setText(Shards[i].Name);

      pVT = dynamic_cast<CViewText *>(pNewLine->getView("version"));
      if (pVT != NULL)
        pVT->setText(Shards[i].Version);
      ...
      if (pVT != NULL)
        pVT->setText(toString(Shards[i].NbPlayers));
...
void onlogin(bool vanishScreen = true) {
  ...
  string res =
      checkLogin(LoginLogin, LoginPassword, ClientApp, LoginCustomParameters);
  if (res.empty()) {
    ...
    if (ClientCfg.R2Mode) {
      ...
      //
      //				pPM->init(R2PatchURLs, R2BackupPatchURL,
      //R2ServerVersion);
      //
    ...
    //		CInterfaceGroup *pList =
    //dynamic_cast<CInterfaceGroup*>(CWidgetManager::getInstance()->getElementFromId(GROUP_LIST_SHARD));
    //		if (pList == NULL)
    //		{
    //			nlwarning("element "GROUP_LIST_SHARD" not found probably
    //bad login_main.xml"); 			return;
    //		}
    //		/* // To test more servers
    //		for (uint fff = 0; fff < 20; ++fff)
    //		{
    ...
    //			vector< pair < string, string > > params;
    //			params.clear();
    //			params.push_back(pair<string,string>("id",
    //"s"+toString(i))); 			if (i>0) 				params.push_back(pair<string,string>("posref",
    //"BL TL"));
    //
    //			CInterfaceGroup *pNewLine
    //=pIM->createGroupInstance("t_shard", GROUP_LIST_SHARD, params); 			if
    //(pNewLine != NULL)
    //			{
    //				CViewText *pVT =
    //dynamic_cast<CViewText*>(pNewLine->getView("name")); 				if (pVT != NULL)
    //pVT->setText(Shards[i].Name);
    //
    //				pVT =
    //dynamic_cast<CViewText*>(pNewLine->getView("version")); 				if (pVT != NULL)
    //pVT->setText(Shards[i].Version);
    //
    //				CViewBase *pVBon = pNewLine->getView("online");
    //				CViewBase *pVBoff =
    //pNewLine->getView("offline"); 				if ((pVBon != NULL) && (pVBoff != NULL))
    //				{
    //					pVBon->setActive (Shards[i].Online);
    //					pVBoff->setActive (!Shards[i].Online);
    //				}
    //
    //				pVT =
    //dynamic_cast<CViewText*>(pNewLine->getView("nbplayer")); 				if (pVT != NULL)
    //pVT->setText(toString(Shards[i].NbPlayers));
    //
    //
    //				// Add to the list
    ...
    //		{
    //			CCtrlButton *pCB =
    //dynamic_cast<CCtrlButton*>(CWidgetManager::getInstance()->getElementFromId(GROUP_LIST_SHARD
    //":s0:but")); 			if (pCB != NULL) 				pCB->setPushed(true);
    //			CAHManager::getInstance()->runActionHandler
    //(pCB->getActionOnLeftClick(), pCB, pCB->getParamsOnLeftClick());
    //
    //		}
    ...
    //		if (LoginLogin.empty())
    //			CAHManager::getInstance()->runActionHandler("set_keyboard_focus",
    //NULL, "target=" CTRL_EDITBOX_LOGIN "|select_all=false"); 		else
    //			CAHManager::getInstance()->runActionHandler("set_keyboard_focus",
    //NULL, "target=" CTRL_EDITBOX_PASSWORD "|select_all=false");
  }
}
...
class CAHOnLogin : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    ...
    if ((pGEBLog == NULL) || (pGEBPwd == NULL)) {
      nlwarning("element " CTRL_EDITBOX_LOGIN " or " CTRL_EDITBOX_PASSWORD
                " not found probably bad login_main.xml");
      return;
    }
...
class CAHOnGameConfiguration : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    ...
    if (CFile::fileExists(Configurator)) {
      ...
      nlwarning("<CAHOnGameConfiguration::execute> can't find ryzom "
                "configurator : %s",
                Configurator.c_str());
    }
  }
...
class CAHLoginTab : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    ...
    } else if (NLGUI::CDBManager::getInstance()
                   ->getDbProp("UI:VARIABLES:SCREEN")
                   ->getValue32() == UI_VARIABLES_SCREEN_CREATE_ACCOUNT) {
      CCtrlBase *pCB = CWidgetManager::getInstance()->getCaptureKeyboard();
      if (pCB != NULL) {
        CCtrlBase *pNewCB;
        string sID = pCB->getId();
        if (sID == CTRL_EDITBOX_CREATEACCOUNT_LOGIN)
          ...
          pNewCB = dynamic_cast<CCtrlBase *>(
              CWidgetManager::getInstance()->getElementFromId(
                  CTRL_EDITBOX_CREATEACCOUNT_EMAIL));
        else
          pNewCB = dynamic_cast<CCtrlBase *>(
              CWidgetManager::getInstance()->getElementFromId(
  ...
  virtual void execute(CCtrlBase *pCaller, const string & /* Params */) {
    ...
    if (ShardSelected != -1) {
      pCB = dynamic_cast<CCtrlButton *>(
          CWidgetManager::getInstance()->getElementFromId(
              GROUP_LIST_SHARD ":s" + toString(ShardSelected) + ":but"));
      if (pCB != NULL)
        pCB->setPushed(false);
    }
    ...
    CCtrlTextButton *pCTB = dynamic_cast<CCtrlTextButton *>(
        CWidgetManager::getInstance()->getElementFromId(CTRL_BUTTON_CONNECT));
    if (pCTB != NULL)
      pCTB->setActive(true);
  }
...
class CAHLoginConnect : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    ...
    //		std::vector<std::string>	patchURIs =
    //Shards[ShardSelected].PatchURIs; 		string url =
    //Shards[ShardSelected].EmergencyPatchURL; 		string ver =
    //Shards[ShardSelected].Version;
    //
    //		if (!ClientCfg.PatchUrl.empty())
    //			url = ClientCfg.PatchUrl;
    //
...
class CAHLoginConnect2 : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    ...
    if (Shards[ShardSelected].PatchURIs.empty() &&
        Shards[ShardSelected].EmergencyPatchURL.empty()) {
    ...
    //		else
    //		{
    //			// Version is good, eula then connect and launch the
    //client 			showEULA();
    //		}
  }
};
...
void initPatch() {
...
#endif
  {
    ...
    for (uint i = 0; i < InfoOnPatch.OptCat.size(); i++) {
      // Ok for the moment all optional categories must be patched even if the
      // player does not want it. Because we cant detect that a continent have
      // to be patched ingame.
      ...
      {
              CCtrlButton *pCB =
      dynamic_cast<CCtrlButton*>(pLine->getCtrl("on_off")); if (pCB != NULL)
              {
                      if (pCB->getPushed())
    ...
    BGDownloader::CTaskDesc taskDesc(BGDownloader::DLState_GetAndApplyPatch,
                                     (1 << BGDownloaderWantedPatch));
    CBGDownloaderAccess::getInstance().startTask(
        taskDesc, string(),
        true /* showDownloader */); // no command line since bg downloader
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("UI:VARIABLES:SCREEN")
      ->setValue32(UI_VARIABLES_SCREEN_PATCHING);

  CInterfaceElement *closeBtn =
...
// ***************************************************************************
// Called after the check has been done. The page is full of optional categories
// that must be selected for patching
class CAHLoginPatch : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    ...
    //
    //		CInterfaceGroup *pList =
    //dynamic_cast<CInterfaceGroup*>(CWidgetManager::getInstance()->getElementFromId(GROUP_LIST_CAT));
    //		if (pList == NULL)
    //		{
    //			nlwarning("element "GROUP_LIST_CAT" not found probably
    //bad login_main.xml"); 			return;
    //		}
    //
    //		for(uint i = 0; i < InfoOnPatch.OptCat.size(); i++)
    //		{
    //			// Ok for the moment all optional categories must be
    //patched even if the player
    //			// does not want it. Because we cant detect that a
    //continent have to be patched ingame.
    //			vCategories.push_back(InfoOnPatch.OptCat[i].Name);
    //
    //			/*
    //			// Code to check if the player wants an optional
    //category or not 			CInterfaceGroup *pLine = pList->getGroup("c"+toString(i));
    //			if (pLine != NULL)
    //			{
    //				CCtrlButton *pCB =
    //dynamic_cast<CCtrlButton*>(pLine->getCtrl("on_off")); 				if (pCB != NULL)
...
class CAHSetReleaseNote : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string &sParams) {
    ...
    CGroupHTML *pQH = dynamic_cast<CGroupHTML *>(
    ...
    if (ClientCfg.R2Mode) {
      // ring release note
      sURL = ClientCfg.RingReleaseNotePath + "?version=" +
             (VersionName.empty() ? R2ServerVersion : VersionName) +
...
class CAHOpenURL : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string &sParams) {
    ...
    if (installTag.length() > 1) {
      ...
      // ...
      // Display the string.
      nlwarning("RegQueryValue for '%s' : %s", KeyName,
...
class CAHInitResLod : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */,
    ...
    getRyzomModes(VideoModes, StringModeList, stringFreqList, CurrentMode,
                  currentFreq);

    // getRyzomModes() expects empty list, so we need to insert 'Windowed' after
    // mode list is filled
    StringModeList.insert(StringModeList.begin(), "uiConfigWindowed");

    // If the client is in windowed mode, still in windowed mode and do not
    // change anything
    if (ClientCfg.Windowed)
      CurrentMode = 0;
    // If we have not found the mode so it can be an error or machine change, so
    // propose the first available
    ...
    CViewText *pVT = dynamic_cast<CViewText *>(
        CWidgetManager::getInstance()->getElementFromId(
            "ui:login:checkpass:content:res_value"));
    if (pVT != NULL)
    ...
    for (uint32 i = 0; i < CfgPresetList.size(); ++i) {
      CConfigFile::CVar *cfgVarPtr =
          ClientCfg.ConfigFile.getVarPtr(CfgPresetList[i].first);
      if (cfgVarPtr == NULL)
        continue;
      ...
      for (uint32 j = 0; j < 4; ++j) // CInterfaceDDX::NumPreset
      {
        ...
        CConfigFile::CVar *presetVarPtr =
            ClientCfg.ConfigFile.getVarPtr(sPresetName);
        if (presetVarPtr) {
    ...
    pVT = dynamic_cast<CViewText *>(
        CWidgetManager::getInstance()->getElementFromId(
            "ui:login:checkpass:content:lod_value"));
    if (pVT != NULL)
  ...
  virtual void execute(CCtrlBase * /* pCaller */,
    ...
    if (CurrentPreset != 4) // CInterfaceDDX::CustomPreset
    {
      for (uint32 i = 0; i < CfgPresetList.size(); ++i) {
        ...
        string sPresetName =
            CfgPresetList[i].first + "_ps" + toString(CurrentPreset);
        CConfigFile::CVar *presetVarPtr =
            ClientCfg.ConfigFile.getVarPtr(sPresetName);
        if (presetVarPtr) {
          if (CfgPresetList[i].second) // Is it a double ?
...
void initDataScan() {
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("UI:VARIABLES:SCREEN")
      ->setValue32(UI_VARIABLES_SCREEN_DATASCAN);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("UI:VARIABLES:DATASCAN_RUNNING")
      ->setValue32(1);
}
...
class CAHOnScanDataClose : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    ...
    if (NLGUI::CDBManager::getInstance()
      ...
      //			if(pGEB)
      //				loginEB=
      //pGEB->getInputStringAsStdString();
      //			// if none entered
      //			if (loginEB.empty())
      //				CAHManager::getInstance()->runActionHandler("set_keyboard_focus",
      //NULL, "target=" CTRL_EDITBOX_LOGIN "|select_all=false"); 			else
...
inline string parseCommentError(const string &initString,
...
bool initCreateAccount() {
  ...
  // reset UI
  CInterfaceGroup *createAccountUI = dynamic_cast<CInterfaceGroup *>(
      CWidgetManager::getInstance()->getElementFromId(
          "ui:login:create_account"));
  if (createAccountUI) {
    // show "submit interface", hide "login interface"
    CInterfaceGroup *grSubmit = dynamic_cast<CInterfaceGroup *>(
        createAccountUI->findFromShortId("submit_gr"));
    if (grSubmit)
      grSubmit->setActive(true);

    CInterfaceGroup *grLogin = dynamic_cast<CInterfaceGroup *>(
        createAccountUI->findFromShortId("login_gr"));
    if (grLogin)
    ...
    for (uint i = 0; i < editBoxes.size(); i++) {
      CGroupEditBox *eb = dynamic_cast<CGroupEditBox *>(
          createAccountUI->findFromShortId(editBoxes[i] + ":eb"));
      if (eb)
        eb->setInputString(std::string());
    }

    // conditions button
    CCtrlBaseButton *but = dynamic_cast<CCtrlBaseButton *>(
        createAccountUI->findFromShortId("accept_cond"));
    if (but)
      but->setPushed(true);
    ...
    CurlHttpClient.verifyServer(true); // set this to false if you need to
    ...
    rules[0] = std::pair<std::string, std::string>("rules_login",
                                                   "id=tooltip-Username");
    rules[1] = std::pair<std::string, std::string>("rules_password",
                                                   "id=tooltip-Password");
    rules[2] = std::pair<std::string, std::string>("rules_password_conf",
                                                   "id=tooltip-ConfirmPass");
    rules[3] =
        std::pair<std::string, std::string>("rules_email", "id=tooltip-Email");
    rules[4] = std::pair<std::string, std::string>("rules_conditions",
                                                   "id=tooltip-TaC");

    for (uint i = 0; i < rules.size(); i++) {
      CViewText *text = dynamic_cast<CViewText *>(
          createAccountUI->findFromShortId(rules[i].first));
      if (text) {
    ...
    // welcome text
    CViewText *text = dynamic_cast<CViewText *>(
        createAccountUI->findFromShortId("errors_list"));
    if (text) {
      ...
      CInterfaceGroup *group = dynamic_cast<CInterfaceGroup *>(
          createAccountUI->findFromShortId("erros_txt"));
      if (group) {
        group->updateCoords();

        CInterfaceGroup *groupScroll = dynamic_cast<CInterfaceGroup *>(
            createAccountUI->findFromShortId("err_back_scrollbar"));
        if (groupScroll)
          groupScroll->setActive(group->getHReal() > group->getMaxHReal());
        CCtrlScroll *scroll = dynamic_cast<CCtrlScroll *>(
            createAccountUI->findFromShortId("err_scroll_bar"));
        if (scroll)
    ...
    // hide rules
    CInterfaceGroup *rulesGr = dynamic_cast<CInterfaceGroup *>(
        createAccountUI->findFromShortId("rules_gr"));
    if (rulesGr)
      rulesGr->setActive(false);

    // must be done after hide rules
    CAHManager::getInstance()->runActionHandler(
        "set_keyboard_focus", NULL,
        "target=" CTRL_EDITBOX_CREATEACCOUNT_LOGIN "|select_all=false");
  }

  NLGUI::CDBManager::getInstance()
      ->getDbProp("UI:VARIABLES:SCREEN")
      ->setValue32(UI_VARIABLES_SCREEN_CREATE_ACCOUNT);
...
class CAHCreateAccountRules : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string &Params) {
    ...
    if (createAccountUI) {
      ...
      for (uint i = 0; i < rules.size(); i++) {
        CViewText *text = dynamic_cast<CViewText *>(
            createAccountUI->findFromShortId(rules[i]));
        if (text) {
...
class CAHOnCreateAccountSubmit : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    ...
    if (createAccountUI) {
      ...
      // text
      CViewText *text = dynamic_cast<CViewText *>(
          createAccountUI->findFromShortId("email_adress"));
      if (text)
      ...
      std::string params =
          "Username=" + results[0] + "&Password=" + results[1] +
...
class CAHCreateAccountLogin : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    ...
    if (createAccountUI) {
      CGroupEditBox *eb = dynamic_cast<CGroupEditBox *>(
          createAccountUI->findFromShortId("eb_login:eb"));
      if (eb)
        LoginLogin = eb->getInputString();

      eb = dynamic_cast<CGroupEditBox *>(
          createAccountUI->findFromShortId("eb_password:eb"));
      if (eb)
...
// ***************************************************************************
string checkLogin(const string &login, const string &password,
                  const string &clientApp,
  ...
  std::string url = ClientCfg.ConfigFile.getVar("StartupHost").asString() +
                    ClientCfg.ConfigFile.getVar("StartupPage").asString();

  // don't use login with alt method
  if (!login.empty()) {
    // ask server for salt
    if (!HttpClient.sendGet(url + "?cmd=ask&cp=2&login=" + login +
                                "&lg=" + ClientCfg.LanguageCode,
                            "", pPM->isVerboseLog()))
      return std::string("Can't send (error code 60) ") +
             HttpClient.lastError();

    if (pPM->isVerboseLog())
      nlinfo("Sent request for password salt");
  ...
  if (ClientCfg.R2Mode) {
    ...
    if (!login.empty()) {
      ...
      if (!HttpClient.sendGet(
              url + "?cmd=login&login=" + login + "&password=" +
              cryptedPassword + "&clientApplication=" + clientApp + "&cp=2" +
              "&lg=" + ClientCfg.LanguageCode + customParameters))
        return std::string("Can't send (error code 2) ") +
               HttpClient.lastError();
    } else {
      // don't send login and password if empty
      if (!HttpClient.sendGet(
              url + "?cmd=login&clientApplication=" + clientApp + "&cp=2" +
              "&lg=" + ClientCfg.LanguageCode + customParameters))
        return std::string("Can't send (error code 2) ") +
               HttpClient.lastError();
    }
    ...
    if (res[0] == 'H') {
      ...
      if (lines.size() != 2) {
        return toString("Invalid server return, found %u lines, want 2",
                        lines.size());
      }
    ...
    if (!HttpClient.sendGet(url + "?login=" + login +
                            "&password=" + cryptedPassword +
                            "&clientApplication=" + clientApp + "&cp=2"))
    ...
    */
    ...
    if (res[0] == 'H') {
      ...
      if (lines.size() != nbs + 1) {
        nlwarning("bad shard lines number %u != %d", (uint)lines.size(),
                  nbs + 1);
        nlwarning("'%s'", res.c_str());
      ...
      for (uint i = 1; i < lines.size(); i++) {
        ...
        Shards.push_back(
            CShard(res[0], online, shardId, res[3], nbPlayers, res[5], res[6]));
        nlinfo("Shard %u, addr = %s, id = %u, name = %s, PatchURIs = %s", i,
               res[5].c_str(), shardId, res[3].c_str(), res[6].c_str());
        if (res.size() == 8) {
...
string selectShard(uint32 shardId, string &cookie, string &addr) {
  ...
  if (!HttpClient.connectToLogin())
    return "Can't connect (error code 7)";

  if (LoginLogin.empty())
    return "Empty Login (error code 8)";
  if (LoginPassword.empty())
    return "Empty Password (error code 9)";
  if (ClientApp.empty())
    return "Empty Client Application (error code 10)";
  ...
  if (!HttpClient.sendGet(url + "?cmd=login&shardid=" + toString(shardId) +
                          "&login=" + LoginLogin +
                          "&password=" + cryptedPassword +
                          "&clientApplication=" + ClientApp + "&cp=2"))
  ...
  if (res[0] == 'H') {
    ...
    /*
                    if (shard != NULL && line.size() >= 3)
                    {
                            ...
                            nlinfo("received %d main patch server URIs:",
       shard->PatchURIs.size()); uint	i; for (i=0; i<shard->PatchURIs.size();
       ++i) nlinfo("%d: '%s'", i, shard->PatchURIs[i].c_str());
                    }
    */
  } else {
    // server returns ???
...
/*void mainLandPatch()
{
        ...
        nlassert(AvailablePatchs & (1 << BGDownloader::DownloadID_MainLand)); //
only handled case for now
        // no-op
        ...
        // there should have been a reboot there, so quit if something went
wrong... release(); exit(0);
}
*/
...
void loginIntro() {
  // Display of nevrax logo is done at init time (see init.cpp) just before
  // addSearchPath (second one)
#if 0
...
void loginDebug() {
  ...
  if (DebugLoginNextScreen) {
    ...
    node = NLGUI::CDBManager::getInstance()->getDbProp(
        "UI:VARIABLES:DEBUG:SCREEN_MAX", false);
    if (node)
      maxValue = node->getValue32();
    node = NLGUI::CDBManager::getInstance()->getDbProp("UI:VARIABLES:SCREEN",
                                                       false);
    if (node)
      node->setValue32(std::min(maxValue, node->getValue32() + 1));
  }

  if (DebugLoginPrevScreen) {
    DebugLoginPrevScreen = false;
    CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
        "UI:VARIABLES:SCREEN", false);
    if (node)
      node->setValue32(std::max(0, node->getValue32() - 1));
  }

  if (DebugLoginReloadUI) {
    DebugLoginReloadUI = false;
    CInterfaceManager *pIM = CInterfaceManager::getInstance();
    sint32 activeScreen = NLGUI::CDBManager::getInstance()
                              ->getDbProp("UI:VARIABLES:SCREEN")
                              ->getValue32();
    if (activeScreen == -1)
      activeScreen = UI_VARIABLES_SCREEN_CHECKPASS;
    pIM->uninitLogin();
    pIM->initLogin();

    NLGUI::CDBManager::getInstance()
        ->getDbProp("UI:VARIABLES:SCREEN")
        ->setValue32(activeScreen);
    NLGUI::CDBManager::getInstance()
        ->getDbProp("UI:VARIABLES:DISPLAY_ACCOUNT_BUTTONS")
        ->setValue32(ClientCfg.DisplayAccountButtons);
...
#define DISP_TEXT(x, text)                                                     \
  ...
  line += lineStep;
    ...
    DISP_TEXT(0.0f,
              getActionKey("debug_login_prev_screen") + " : Previous screen");
    DISP_TEXT(0.0f, getActionKey("debug_login_next_screen") + " : Next screen");
    DISP_TEXT(0.0f,
              getActionKey("debug_login_ui") + " : Debug elements under mouse");
