...
void initPACS(const char *rbank, const char *gr,
  ...
  if (rbank != 0 && gr != 0) {
    ...
    if (GR) {
      ...
      PACS = UMoveContainer::createMoveContainer(GR, gw, gh,
                                                 RYZOM_ENTITY_SIZE_MAX, 2);
    } else
  ...
  if (PACS == 0)
    PACS = UMoveContainer::createMoveContainer(
        15000.0, -25000.0, 20000.0, -20000.0, 16, 16, RYZOM_ENTITY_SIZE_MAX, 2);
...
UInstanceGroup *getCluster(const UGlobalPosition &gp) {
  ...
  std::map<std::string, UInstanceGroup *>::const_iterator igIt =
      IGLoaded.find(toLowerAscii(strPos));
...
void addPacsPrim(const std::string &fileName) {
  std::string ppName = NLMISC::toLowerAscii(
      NLMISC::CFile::getFilenameWithoutExtension(fileName));
  if (PacsPrims.find(ppName) != PacsPrims.end()) {
    nlwarning(
        ("Pacs primitive " + ppName + " already has been inserted").c_str());
    return;
  }
  CUniquePtr<NLPACS::UPrimitiveBlock> pb(
      NLPACS::UPrimitiveBlock::createPrimitiveBlockFromFile(fileName));
...
void deletePrimitiveBlocks() {
  for (TPacsPrimMap::iterator it = PacsPrims.begin(); it != PacsPrims.end();
       ++it) {
    delete it->second;
  }
  PacsPrims.clear();
...
void initPrimitiveBlocks() {
  ...
  std::string lookupPathPrimList =
      CPath::lookup(primitiveListFile, false, true);
  if (lookupPathPrimList.empty()) {
    nlwarning("Unable to find the file containing pacs primitives list : %s",
              primitiveListFile);
    return;
  }
  //
