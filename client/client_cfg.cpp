...
#define READ_FLOAT2_DEV(cfgVariableName, variableName)                         \
  _READ_FLOAT2(cfgVariableName, variableName)
...
#define READ_ENUM_ASINT_DEV(type, variableName)                                \
  _READ_ENUM_ASINT(type, variableName)
...
#define READ_FLOAT2_FV(cfgVariableName, variableName)                          \
  _READ_FLOAT2(cfgVariableName, variableName)
...
#define READ_ENUM_ASINT_FV(type, variableName)                                 \
  _READ_ENUM_ASINT(type, variableName)
...
CClientConfig::CClientConfig() {
  ...
  CreateAccountURL =
      RYZOM_CLIENT_CREATE_ACCOUNT_URL; // "https://open.ryzom.dev/ams/";
  EditAccountURL =
      RYZOM_CLIENT_EDIT_ACCOUNT_URL; // "https://open.ryzom.dev/ams/";
  ForgetPwdURL =
      RYZOM_CLIENT_FORGET_PASSWORD_URL; // "https://open.ryzom.dev/ams/";
  ...
  FreeLookSmoothingPeriod =
      0.f; // when in absolute mode, free look factor is used instead of speed,
           // the mouse gives the absolute angle
  FreeLookInverted = false;
  FreeLookTablet =
      false; // Mouse reports absolute coordinates, so avoid mouse recentering
  AutomaticCamera = true;
  DblClickMode = true;  // when in dbl click mode, a double click is needed to
                        // execute default contextual action
  AutoEquipTool = true; // when true player will auto-equip last used weapon or
                        // forage tool when doing an action
  ...
  ScreenAspectRatio = 0.f; // Default commmon Screen Aspect Ratio (no relation
                           // with the resolution) - 0.f = auto
  ...
  LoadingMusic = "main menu loop.ogg"; // Main loading used after leaving
                                       // character selection, and when going
  ...
  //	PreDataPath.push_back("data/gamedev/language/");	// Default Path
  //for the language data

  //	DataPath.push_back("data/");					//
  //Default Path for the Data. 	DataPath.push_back("data_leveldesign/");
  //// Default Path for the Level Design Directory.
  //	DataPath.push_back("data_common/");				//
  //Default Path for the Level Design Directory.
  ...
  EndScreenTimeOut =
      0.f; // Default time out for the screen at the end of the application.
  Loading_BG =
      "loading_bg.tga"; // Default name for the loading background file.
  LoadingFreeTrial_BG =
      "loading_free_trial_bg.tga"; // Default name for the loading background
                                   // file in FreeTrial mode.
  ...
  ShadowZDirClampLandscape =
      -0.5f; // On landscape, allow a minimum Phi angle of -30 degrees
  ShadowZDirClampInterior =
      -0.86f; // On Interior, allow a minimum Phi angle of -60 degrees
...
void CClientConfig::setValues() {
  ...
  CConfigFile::CVar *pcvFullScreen =
      ClientCfg.ConfigFile.getVarPtr("FullScreen");
  if (pcvFullScreen) {
  ...
  clamp(ClientCfg.InterfaceScale, ClientCfg.InterfaceScale_min,
  ...
  if (varPtr) {
    if (nlstricmp(varPtr->asString(), "Auto") == 0 ||
        nlstricmp(varPtr->asString(), "0") == 0)
      ClientCfg.Driver3D = CClientConfig::DrvAuto;
    else if (nlstricmp(varPtr->asString(), "OpenGL") == 0 ||
             nlstricmp(varPtr->asString(), "1") == 0)
      ClientCfg.Driver3D = CClientConfig::OpenGL;
    else if (nlstricmp(varPtr->asString(), "Direct3D") == 0 ||
             nlstricmp(varPtr->asString(), "2") == 0)
      ClientCfg.Driver3D = CClientConfig::Direct3D;
    else if (nlstricmp(varPtr->asString(), "OpenGLES") == 0 ||
             nlstricmp(varPtr->asString(), "3") == 0)
      ClientCfg.Driver3D = CClientConfig::OpenGLES;
  } else
  ...
  // Background Color
#if !FINAL_VERSION
  CConfigFile::CVar *pcvBackColor =
      ClientCfg.ConfigFile.getVarPtr("Background");
  ...
  // to be backward compatible, suppose a value<1 is for the old System!!! =>
  // invert it!
  ...
  if (!ClientCfg.CurlCABundle.empty() &&
      ClientCfg.CurlCABundle[0] ==
          '%') // Path is relative to client_default.cfg path (used by ryzom
               // patch)
  {
    string defaultConfigFileName;
    if (ClientCfg.getDefaultConfigLocation(defaultConfigFileName))
      ClientCfg.CurlCABundle = CFile::getPath(defaultConfigFileName) +
                               ClientCfg.CurlCABundle.substr(1);
  }
  ...
  if (varPtr) {
    if (nlstricmp(varPtr->asString(), "Auto") == 0)
      ClientCfg.DriverSound = CClientConfig::SoundDrvAuto;
    else if (nlstricmp(varPtr->asString(), "FMod") == 0)
      ClientCfg.DriverSound = CClientConfig::SoundDrvFMod;
    else if (nlstricmp(varPtr->asString(), "OpenAL") == 0)
      ClientCfg.DriverSound = CClientConfig::SoundDrvOpenAL;
    else if (nlstricmp(varPtr->asString(), "DirectSound") == 0)
      ClientCfg.DriverSound = CClientConfig::SoundDrvDirectSound;
    else if (nlstricmp(varPtr->asString(), "XAudio2") == 0)
      ClientCfg.DriverSound = CClientConfig::SoundDrvXAudio2;
  } else
  ...
  // USER COLORS //
  // Shout Color
  CConfigFile::CVar *pcvColorShout =
      ClientCfg.ConfigFile.getVarPtr("ColorShout");
  ...
  if (!sic) {
    ...
    if (sic->size() & 1) {
      cfgWarning(
          "Expecting odd size for SystemInfoColors. Last entry ignored.");
    }
    ...
    for (uint k = 0; k < numCol; ++k) {
      ...
      char fx[64];
      fx[0] = '\0';
      if (sscanf(sic->asString((2 * k) + 1).c_str(), "%d %d %d %d %s %s", &r,
                 &g, &b, &a, mode, fx) < 5) {
        if (DisplayCFGWarning)
          nlwarning("Can't parse color for entry %s",
                    sic->asString(2 * k).c_str());
      } else {
        SSysInfoParam p;
        ...
        if (stricmp(mode, "over") == 0)
          p.Mode = SSysInfoParam::Over;
        else if (stricmp(mode, "overonly") == 0)
          p.Mode = SSysInfoParam::OverOnly;
        else if (stricmp(mode, "center") == 0)
          p.Mode = SSysInfoParam::Center;
        else if (stricmp(mode, "centeraround") == 0)
          p.Mode = SSysInfoParam::CenterAround;
        else if (stricmp(mode, "around") == 0)
          p.Mode = SSysInfoParam::Around;
  ...
  for (uint p = 0; p < 2; p++) {
    CConfigFile::CVar *pc =
        ClientCfg.ConfigFile.getVarPtr(printfCommands[p].c_str());
    if (pc) {
      if (pc->size() % 5 == 0 && pc->size() >= 5) {
        for (uint i = 0; i < pc->size(); i += 5) {
          ...
          if (p == 0)
            ClientCfg.PrintfCommands.push_back(pcom);
          else
            ClientCfg.PrintfCommandsFreeTrial.push_back(pcom);
        }
      } else {
        cfgWarning(
            ("Missing or too many parameters in " + printfCommands[p]).c_str());
      }
    }
  }
#endif
  ...
  // Read the camera height and distance at load time only, cause prefer keep
  // ingame player setup
  if (firstTimeSetValues) {
  ...
  // HelpFontColor
  {
    CConfigFile::CVar *cvHelpFontColor =
        ClientCfg.ConfigFile.getVarPtr("HelpFontColor");
...
#if !FINAL_VERSION
  // DebugFontColor
  CConfigFile::CVar *pcvDebugFontColor =
      ClientCfg.ConfigFile.getVarPtr("DebugFontColor");
  ...
  if (ClientCfg.ConfigFile.exists("ExtendedCommands") &&
      ClientCfg.ConfigFile.getVar("ExtendedCommands").asString() == "Enable")
...
#if FINAL_VERSION
  {
    CConfigFile::CVar *var =
        ClientCfg.ConfigFile.getVarPtr("R2EDExtendedDebug");
    if (var) {
      var->setAsInt(0);
    }
  ...
  if (ClientCfg.FPExceptions)
    _control87(_EM_DENORMAL /*|_EM_INVALID|_EM_ZERODIVIDE|_EM_OVERFLOW*/ |
  ...
  // Init Verbose Modes (at the beginning to be able to display them as soon as
  // possible).
  ...
  if (ClientCfg.Local) {
    uint32 tickOffset =
        (uint32)(ClientCfg.RyzomDay * RYZOM_HOURS_IN_TICKS * 24 +
                 ClientCfg.RyzomTime * RYZOM_HOURS_IN_TICKS);
    RT.resetTickOffset();
  ...
  CConfigFile::CVar *pcvHardwareCursors =
      ClientCfg.ConfigFile.getVarPtr("HardwareCursors");
  if (pcvHardwareCursors) {
    ClientCfg.HardwareCursors.clear();
    int iSz = pcvHardwareCursors->size();
    for (int i = 0; i < iSz; i++)
      ClientCfg.HardwareCursors.insert(
          toLowerAscii(pcvHardwareCursors->asString(i)));
  } else {
  ...
  // vl: BackgroundDownloader is hardcoded to false and we don't want to run it,
  // even if the cfg wants it
...
void CClientConfig::init(const string &configFileName) {
  ...
  if (!CFile::fileExists(configFileName)) {
    ...
    if (fp == NULL)
      nlerror("CFG::init: Can't create config file '%s'",
              configFileName.c_str());
    else
      nlwarning("CFG::init: creating '%s' with default values",
                configFileName.c_str());
  ...
  size_t pos = 0;
  while ((pos = contentUtf8.find("RootConfigFilename")) !=
         configFileName.npos) {
  ...
  configFile.serialBuffer((uint8 *)contentUtf8.c_str(),
                          (uint)contentUtf8.size());
  configFile.close();

  // now we can continue loading and parsing the config file

  // if the config file will be modified, it calls automatically the function
  // setValuesOnFileChange()
  ...
  if (varPtr) {
    ...
    if (!CI18N::isLanguageCodeSupported(lang)) {
      nlinfo("Unsupported language code \"%s\" fallback on default",
             lang.c_str());
  ...
  if (varPtr) {
    ...
    if (str != getVersion() && ClientCfg.SaveConfig) {
      nlinfo(
          "Update and save the ClientVersion variable in config file %s -> %s",
          str.c_str(), getVersion().c_str());
...
void CClientConfig::writeBool(const std::string &varName, bool bVal,
                              bool bForce) {
  CConfigFile::CVar *varPtr =
      bForce ? ConfigFile.insertVar(varName, CConfigFile::CVar())
             : ConfigFile.getVarPtr(varName);
  if (varPtr)
...
void CClientConfig::writeDouble(const std::string &varName, double dVal,
                                bool bForce) {
  CConfigFile::CVar *varPtr =
      bForce ? ConfigFile.insertVar(varName, CConfigFile::CVar())
             : ConfigFile.getVarPtr(varName);
...
void CClientConfig::writeString(const std::string &varName,
                                const std::string &strVal, bool bForce) {
  CConfigFile::CVar *varPtr =
      bForce ? ConfigFile.insertVar(varName, CConfigFile::CVar())
             : ConfigFile.getVarPtr(varName);
  if (varPtr)
...
#else
  // unders Windows or Linux, search client_default.cfg is same directory as
  // executable
