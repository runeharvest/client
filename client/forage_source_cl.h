...
public:
  ...
  uint8 getKnowledge() const {
    return (_KnowledgeLevel & 0x80) != 0 ? 0 : _KnowledgeLevel;
  }

  /// Return the icon filename (valid if the knowledge > 0, otherwise returns
  /// NULL)
  ...
  /// Return true if the source is in "prospection extra time" (not bonus extra
  /// time or normal extra time)
  bool isInProspectionExtraTime() const {
    return _BarDestValues[0] < _ExtraTime - _InclBonusExtraTime;
  }
...
private:
  ...
  /// 0=nothing 1=group 2=family. Bit 7 set means "knowledge info not received
  /// yet" (by visual fx)
  ...
  /// When the source is in "bonus extra time", change the display of the time
  /// bar
