...
#else
  HKEY hKey;
  if (RegCreateKeyW(HKEY_CURRENT_USER, REGKEY_RYZOM_PATH, &hKey) ==
      ERROR_SUCCESS) {
    DWORD permanentBan = on ? 1 : 0;
    LONG result = RegSetValueExW(hKey, REGKEY_PERMANENT_BAN, 0, REG_DWORD,
                                 (LPBYTE)&permanentBan, 4);
...
#else
  HKEY hKey;
  if (RegOpenKeyExW(HKEY_CURRENT_USER, REGKEY_RYZOM_PATH, 0, KEY_READ, &hKey) ==
      ERROR_SUCCESS) {
    DWORD permanentBan;
    DWORD type;
    DWORD dataSize = sizeof(DWORD);

    RegQueryValueExW(hKey, REGKEY_PERMANENT_BAN, 0, &type,
                     (LPBYTE)&permanentBan, &dataSize);
...
static void setPermanentBanFileMarker(const std::string &path, bool on) {
  if (on) {
    try {
      ...
      COFile f(path);
#ifdef NL_OS_WINDOWS
      SetFileAttributesW(nlUtf8ToWide(path),
...
void setPermanentBanMarkers(bool on) {
  ...
  setPermanentBanFileMarker(CPath::getWindowsDirectory() + PERMANENT_BAN_FILE1,
                            on);
}
...
bool testPermanentBanMarkers() {
  /*#ifndef NL_OS_WINDOWS
          ...
          if (getPermanentBanFileMarker(CPath::getWindowsDirectory() +
  PERMANENT_BAN_FILE1)) return true; #endif return false;
  */
  return false;
}

// mark a bnp file without corrupting its datas (this force the patch to be
// applied the next time the client is launched, thus delaying the trouble
// maker)
...
void applyPermanentBanPunishment() {
  // go in the data directory & touch all bnp files so that the client should
  // repatch all its datas
