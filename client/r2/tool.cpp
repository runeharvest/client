...
static const CVector cardinals[] = {
    CVector(1.f, 0.f, 0.f),  CVector(1.f, 1.f, 0.f).normed(),
    CVector(0.f, 1.f, 0.f),  CVector(-1.f, 1.f, 0.f).normed(),
    CVector(-1.f, 0.f, 0.f), CVector(-1.f, -1.f, 0.f).normed(),
    CVector(0.f, -1.f, 0.f), CVector(1.f, -1.f, 0.f).normed()};
...
bool CTool::checkDoubleClick() {
  ...
  if (_DoubleClickStartTime == -1)
    return false;
  if (T0 - _DoubleClickStartTime >=
  ...
  return abs(mx - _DoubleClickX) <= moveThrehsold &&
         abs(my - _DoubleClickY) <= moveThrehsold;
}
...
bool CTool::isMouseOnUI() {
  ...
  return CWidgetManager::getInstance()->getWindowUnder(getMouseX(),
                                                       getMouseY()) != NULL;
}
...
CGroupMap *CTool::isMouseOnWorldMap() {
  ...
  const std::vector<CInterfaceGroup *> &groupsUnder =
      CWidgetManager::getInstance()->getGroupsUnderPointer();
  if (groupsUnder.empty())
    return NULL;
...
void CTool::computeWorldViewRay(sint32 posX, sint32 posY, CWorldViewRay &dest) {
  ...
  if (gm) {
    ...
    if (mapPos.x >= 0.f && mapPos.y >= 0.f && mapPos.x <= 1.f &&
        mapPos.y <= 1.f) {
      CVector2f worldPos;
  ...
  // In this case, false intersection with the ground may be detected, so move
  // forward in ray direction to resolve that problem
...
// ***************************************************************
CTool::TRayIntersectionType
CTool::getPacsType(const NLMISC::CVector &pos, float threshold,
                   NLPACS::UGlobalPosition &destPos) {
...
// ***************************************************************
bool CTool::raytrace(const NLMISC::CVector &segmentStart,
                     const NLMISC::CVector &dir, NLMISC::CVector &inter) {
  // H_AUTO(R2_CTool_raytrace)
  //  try precise collision first if ray not vertical (not supported by
  //  CollisionManager)
...
// ***************************************************************
CTool::TRayIntersectionType
CTool::computeWorldMapIntersection(float x, float y, NLMISC::CVector &inter) {
  // H_AUTO(R2_CTool_computeWorldMapIntersection)
  const CArray2D<sint16> &heightMap =
      getEditor().getIslandCollision().getHeightMap();
  if (!heightMap.empty()) {
    // here we take only the surface with a valid z, so the 'InvalidPacsPocs'
    // case is not possible
    if (computeNearestValidSurfaceFromHeightMap(x, y, inter))
      return ValidPacsPos;
    return NoIntersection;
  }
  if (!GR)
    return NoIntersection;
  // if heightmap not present then relies on old pacs test method...(transition
  // code before all goes through the heightmap)
  CVector pos(x, y, 0.f);
  //
  ...
  float deltaZ = 2.f;
  float bias =
      0.001f; // if (dx == 0) and (dy == 0), the camera collision fails (not
              // supported, visibly) workaround it by adding a small bias
  ...
  //
  float dist = CollisionManager->getCameraCollision(refinedStart, refinedEnd,
                                                    0.1f, false);
  if (dist == 1.f)
    return NoIntersection;
  //
...
// ***************************************************************
inline bool
CTool::isIslandValidPos(const CArray2D<sint16> &heightMap,
                        const CScenarioEntryPoints::CCompleteIsland &islandDesc,
                        float x, float y) {
...
// do several raytracing test on the small region until a hit is found (at
// boundary of zones, welding is not perfect so it helps to remove this problem)
static bool areaRaytrace(CPackedWorld &pw, const CVector &start,
                         const CVector &end, CVector &inter, CVector *normal) {
  // H_AUTO(R2_areaRaytrace)
  static volatile float distMax = 0.2f;
  static volatile float distStep = 0.05f;
  if (pw.raytrace(start, end, inter, NULL, normal))
    return true;
...
// ***************************************************************
bool CTool::computeNearestValidSurfaceFromHeightMap(float x, float y,
  ...
  if (mapX < 0 || mapY < 0 || mapX >= (islandDesc->XMax - islandDesc->XMin) ||
      mapY >= (islandDesc->YMax - islandDesc->YMin))
    return false;
  sint hmZ = heightMap(mapX, mapY);
  if (hmZ >= 0x7ffe)
    return false; // not an accessible pos
  ...
  bool inter1Found = areaRaytrace(
      *pw, origin - bias, origin + 10000.f * CVector::K, inter1, &normal1);
  bool inter2Found = areaRaytrace(
      *pw, origin + bias, origin - 10000.f * CVector::K, inter2, &normal2);
  ...
  if (inter1Found && inter2Found) {
    // because z in heightmap in usually a 'ceil' of real height, tends to favor
    // surface below to avoid special case were the cliff top is very close to
    // the ground. add a bias for this (equal to the heightmap precision)
    origin.z -= 2.f;
    inter =
        ((inter1 - origin).norm() < (inter2 - origin).norm()) ? inter1 : inter2;
  } else if (inter1Found) {
    inter = inter1;
  } else {
    inter = inter2;
...
// ***************************************************************
CTool::TRayIntersectionType
CTool::computeLandscapeRayIntersection(const CWorldViewRay &worldViewRay,
                                       NLMISC::CVector &inter) {
  // H_AUTO(R2_CTool_computeLandscapeRayIntersection)
  if (!worldViewRay.Valid)
    return NoIntersection;
  if (worldViewRay.OnMiniMap) {
    return computeWorldMapIntersection(worldViewRay.Origin.x,
                                       worldViewRay.Origin.y, inter);
  }
  // Compute collision point with landscape
  if (!raytrace(worldViewRay.Origin, worldViewRay.Dir, inter)) {
    // NB following code is intended to fix the problem of holes betweens zone
    // in the packed collision
    static volatile float bias = 0.15f;
    bool found = false;
    for (uint k = 0; k < sizeofarray(cardinals) && !found; ++k) {
      CVector delta = bias * (cardinals[k].x * worldViewRay.Right +
                              cardinals[k].y * worldViewRay.Up);
  ...
  const CArray2D<sint16> &heightMap =
      getEditor().getIslandCollision().getHeightMap();
  if (!heightMap.empty()) {
    // if heightmap is present, use it because it gives us more reliable
    // information
    CVector surfPos;
    if (!computeNearestValidSurfaceFromHeightMap(inter.x, inter.y, surfPos))
      return InvalidPacsPos;
...
void CTool::handleMouseOverPlayer(bool over) {
  ...
  CCDBNodeLeaf *pNL =
      _UserCharFade
          ? &*_UserCharFade
          : &*(_UserCharFade = NLGUI::CDBManager::getInstance()->getDbProp(
...
// ***************************************************************
CInstance *
CTool::checkInstanceUnderMouse(IDisplayerUIHandle **miniMapHandle /*= NULL*/) {
  ...
  if (isMouseOnUI()) {
    ...
    if (gm) {
      ...
      CDisplayerVisual *currSelectedDV =
          inst ? inst->getDisplayerVisual() : NULL;
      ...
      const std::vector<CCtrlBase *> &ctrlsUnder =
          CWidgetManager::getInstance()->getCtrlsUnderPointer();
      for (sint k = (sint)ctrlsUnder.size() - 1; k >= 0; --k) {
        IDisplayerUIHandle *handle =
            dynamic_cast<IDisplayerUIHandle *>(ctrlsUnder[k]);
        if (handle != NULL) {
          CDisplayerVisual *dv =
              handle->getDisplayedInstance().getDisplayerVisual();
          if (ctrlsUnder[k]->getRenderLayer() > bestCandidateLayer ||
              (ctrlsUnder[k]->getRenderLayer() == bestCandidateLayer &&
               currSelectedDV == dv)) {
    ...
    // (so that we don't need to know of the UI ..., world map is a special
    // case)
    {
    ...
    if (isInScreen(x, y)) {
      ...
      CInstance *inst = getEditor().getInstanceUnderPos(cursX, cursY, 2000.f,
                                                        isPlayerUnderCursor);
...
// ***************************************************************
void CTool::handleMouseOverInstance(const char *cursorDefault,
...
bool CTool::defaultRightButtonDownHandling() {
  ...
  /*if (!getEditor().getFocusedInstance())
  {
          // cancel any current selection, this will cause the default context
  menu to beshown getEditor().setSelectedInstance(NULL); return false;
  }
  getEditor().setSelectedInstance(getEditor().getFocusedInstance());
  getEditor().displayContextMenu();
  return true;	*/
...
void CTool::handleWorldMapAutoPan(sint32 &outDx, sint32 &outDy) {
  ...
  if (currFrameCounter == _AutoPanLastHandlingFrame)
  ...
  sint32 delay = (_NumPans > CV_MapAutoFastPanNumTicks.get())
                     ? CV_MapAutoFastPanDeltaInMs.get()
...
NLMISC::CRGBA CTool::getInvalidPosColor() {
  ...
  if (duration <= 0)
    duration = 0.1;
  return blend(CV_InaccessiblePosColor0.get(), CV_InaccessiblePosColor1.get(),
               0.5f + 0.5f * (float)cos(NLMISC::Pi *
                                        fmod((double)T1, duration) / duration));
}
