...
// ***************************************************************
CToolChoosePos::CToolChoosePos(sint ghostSlot, const std::string &cursValid,
  ...
  _GhostSlot = ghostSlot, _Valid = false;
...
// ***************************************************************
void CToolChoosePos::enableMultiPos(
    const std::string &cursValidMulti /*="curs_create_multi.tga"*/) {
...
void CToolChoosePos::hideMapSelectionAxis() {
  ...
  if (worldMap)
    worldMap->setSelectionAxis(false);
}
...
void CToolChoosePos::updateBeforeRender() {
  ...
  case ValidPacsPos:
    ...
    if (worldMap)
      worldMap->setSelectionAxis(true, inter);
    showPolys(true);
    break;
  case InvalidPacsPos:
    entityPos = inter;
  ...
  if (entity) {
      ...
      */
      ...
      if (inst.empty()) {
        if (!entity->instances().empty()) {
          ...
          // loading = !entity->instances()[0].Loading.empty() &&
      ...
      CVector skelRootRelativePos = entity->skeleton()->getMatrix().getPos() -
      entity->pos().asVector();
      // combine quat for front face xform & anim quat
      CQuat frontQuat(CVector::K,  _CreateAngle);
      entity->skeleton()->setRotQuat(frontQuat *
      entity->skeleton()->getRotQuat()); entity->skeleton()->setPos(entityPos +
      skelRootRelativePos); mat.setRot(frontQuat); mat.setPos(entityPos);
      */
      // relative position to skeleton root
      CVector skelRootRelativePos =
          entity->skeleton()->getMatrix().getPos() - entity->pos().asVector();
      // combine quat for front face xform & anim quat
      CQuat frontQuat(CVector::K, _CreateAngle);
      entity->skeleton()->setRotQuat(frontQuat *
                                     entity->skeleton()->getRotQuat());
      CMatrix frontMat;
    ...
    if (shown && entity->getPrimitive())
    {
            float w;
            float h;
            if (entity->getPrimitive()->getPrimitiveType() ==
    UMovePrimitive::_2DOrientedBox)
            ...
            //unitBoxToLocalBox.setPos(CVector(localBox.getMin().x / favoid0(w),
    localBox.getMin().y / favoid0(h), 1.f); unitBoxToLocalPrim.setPos(CVector(-
    0.5f * w, - 0.5f * h, 0.f)); CMatrix worldMat = mat * unitBoxToLocalPrim;
            ...
            else
            {
                    ...
                    _BadPlaceDecal.setCustomUVMatrix(true,
  ...
  if (!_PolyRender.empty() && _PolyRender[0].getActive()) {
    ...
    for (uint k = 0; k < _PolyRender.size(); ++k) {
      ...
      bool validPoly =
          ic.isValidPoly(tmpPoly); // Test all position because entities may be
                                   // created in that zone

      // also test zone vertices because in the end, a valid zone is a zone for
      // which all vertices are valid
      for (uint l = 0; validPoly && l < verts.size(); ++l) {
        if (!ic.isValidPos(verts[l]))
          validPoly = false;
      }
      ...
      */
      ...
      if (_PolyRender[k].getLook().Shape == CPrimLook::ClosedPolyLine) {
        ...
        if (showPoly) {
          CLandscapePolyDrawer::getInstance().computeBBoxFromPolygon(tmpPoly,
                                                                     bbox);
          CLandscapePolyDrawer::getInstance().addPoly(
              tmpPoly, _PolyRender[k].getLook().EdgeLook.WorldMapColor, bbox);
        }
        _PolyRender[k].setWorldMapPolyColor(
            _PolyRender[k].getLook().EdgeLook.WorldMapColor);
      }
    }
  }
...
void CToolChoosePos::updateAfterRender() {
  ...
  case SelectPos: {
    ...
    if (_Valid) {
      ...
      if (stopAfterCommit() && (!_MultiPos || !isShiftDown()) &&
          !_MultiPosLocked) {
        if (_GhostSlot != -1) {
          // EntitiesMngr.remove(_GhostSlot,  true);
          CEntityCL *ghost = EntitiesMngr.entity(_GhostSlot);
          if (ghost) {
            // just hide the slot because creation may happen just after, so
            // keep a ref on textures and shape
            ghost->show(false);
          }
        }
...
void CToolChoosePos::cancel() {
  ...
  showPolys(false);
  // TODO nico : potential crash here if called just before render (in
  // updateBeforeRender)
