...
bool CToolMaintainedAction::onMouseLeftButtonDown() {
  ...
  CTool::TSmartPtr holdThis(
      this); // prevent deletion by son (if they set another tool)
             // TODO nico : find a more general system than this, because
  if (_State == Action)
    return true; // may happen if focus was lost
  if (_State == Canceling) {
    releaseMouse();
    _State = Idle; // better for ergonomy (of if focus was lost, in which case
                   // we can't detect the button was up)
  }
...
bool CToolMaintainedAction::onMouseLeftButtonUp() {
  ...
  if (_State == Idle) {
    // NB : we need to release the mouse here and signal the event as handled,
    // because a previous tool such as CToolDrawPrim terminates on a
    // 'leftMouseDown' event then switch to the default 'SelectMove' tool Not
    // handling the event would trigger 'the mouseLeftClick' event which result
    // in the default action 'unselect'
  ...
  _LeftButtonDown = false;
  if (_State == Canceling) {
    if (!_RightButtonDown) // both buttons must be released before mouse is
                           // released
    {
      releaseMouse();
      _State = Idle;
...
bool CToolMaintainedAction::onMouseRightButtonDown() {
  ...
  if (_State == Canceling) {
    ...
    _State = Idle; // better for ergonomy (if focus was lost, in which case we
                   // can't detect the button was up)
  }
  _LeftButtonDown = true;
  if (_State != Action) {
  ...
  // Enter 'canceling' state : both button must be released before any other
  // action can be done
  _State = Canceling;
  captureMouse();
  return true;
...
bool CToolMaintainedAction::onMouseRightButtonUp() {
  ...
  _LeftButtonDown = false;
  if (_State == Canceling) {
    if (!_LeftButtonDown) // both buttons must be released before before mouse
                          // is released
    {
      releaseMouse();
      _State = Idle;
...
bool CToolMaintainedAction::onMouseLeftButtonClicked() {
  ...
  //  if this message reach us, then the user clicked somewhere in the view, so
  ...
  // we say that the click has been handled only if mouse was already captured
  // by the map
  return true;
}
...
void CToolMaintainedAction::onFocusGained() {
  if (_State == Idle) {
    updateFocusedInstance(); // if alt-tab was used then udpdate focused
                             // instanceright now
...
bool CToolMaintainedAction::onMouseMove() {
  if (_LostFocusRecover) {
    ...
    // must update focused instance as a MouseMove is received, so that next
    // 'mouse down' event can be on the right entity. events sequence is :
    // -  focus gained
    // -  mouse move
    // - possibly 'mouse down' <- here focused instance needs to be good, hence
    // the following code  !
...
void CToolMaintainedAction::updateFocusedInstance() {
  ...
  if (instanceUnder != getEditor().getSelectedInstance()) {
    setMouseCursor(
        isMouseOnWorldMap()
            ? "curs_default.tga"
            : getPickCursor()); // do not show the selection circle on the
...
void CToolMaintainedAction::updateAfterRender() {
  ...
  case Canceling:
    setMouseCursor(DEFAULT_CURSOR); //\TODO nico Maybe a special cursor here
