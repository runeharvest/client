...
/** This class contained common behaviour for tools that follow the following
 * mechanism :
 * - when the left bouton if maintained, an action if performerd  on highlighted
 * instance if the instance wasn't the selection then it becomes the selection
 * - when the right button is pushed down while an action is performed then the
 * action is canceled. The mouse is then captured until both right and left
 * buttons are released
 * - when right button is pushed down outside of action then the context menu
 * for current selected instance is shown. If the mouse was over an highlighted
 * instance that is not the selection then it become the selection
 * - when right or left button are pushed then maintained and the mouse wasn't
 * on an entity then the camera handling begins.
 */
...
public:
  ...
  // when another tool finishes with a (double) click, it may switch to the
  // default select move tool so the last click may be interpreted as a click
  // "in scene", which does an unselect. Call this after switching to prevent
  // this behavior
