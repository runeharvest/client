...
// ***************************************************************
CToolDrawPrim::CToolDrawPrim(TPrimType primType,
                             CInstance *extending /*= NULL*/) {
  _NumPoints = 0;
  _ValidPos = false;
  _PrimType = primType;
  _MustClose = false;
...
bool CToolDrawPrim::canTerminate() const {
  ...
  return ((_PrimType == Road && _NumPoints >= 1) || _NumPoints >= 3) &&
         _ValidPrim;
}
...
bool CToolDrawPrim::init(const CLuaObject &parameters) {
  ...
  */
  ...
  _PrimLookInaccessible.init(
      getEditor().getEnv()["PrimRender"]["RoadLookInaccessible"]);
  //
  ...
  if (vertices.isTable()) {
    ...
    ENUM_LUA_TABLE(vertices, it) {
      ...
      _Points.push_back(NLMISC::CVector((float)vertex["x"].toNumber(),
                                        (float)vertex["y"].toNumber(),
                                        (float)vertex["z"].toNumber()));
    }
  }
  if (!parameters["ExtendedPrimitiveId"].isNil()) {
    ...
    if (!_ExtendedPrimitive) {
      _Extending = false;
      nlwarning("Can't extend primitive with id %s",
                extendedPrimitiveId.c_str());
      return false;
    }
    _Extending = true;
  }
...
void CToolDrawPrim::cancel() {
  ...
  if (_ExtendedPrimitive) {
    CDisplayerVisualGroup *dv = dynamic_cast<CDisplayerVisualGroup *>(
        _ExtendedPrimitive->getDisplayerVisual());
    if (dv) {
...
void CToolDrawPrim::setPrimLook(bool closed, bool lastEdgeIsValid, bool valid) {
  ...
  CPrimLook *look;
  if (!valid)
    look = &_PrimLookInvalid;
  else if (closed)
    look = &_PrimLookCanClose;
  else
    look = &_PrimLook;
...
void CToolDrawPrim::updateBeforeRender() {
  ...
  CGroupMap *worldMap = getWorldMap();
  if (worldMap && !_InaccessibleParts &&
      _InaccessiblePrim.isAddedToWorldMap()) {
...
void CToolDrawPrim::doUpdateBeforeRender() {
  ...
  CVector inter;
  CVector wpPos;
  TRayIntersectionType interType =
      computeLandscapeRayIntersection(worldViewRay, inter);
  switch (interType) {
  case NoIntersection:
  ...
  // If there are at least 3 vertices and the primitive is a region, then see
  // whether mouse is near the first vertex -> in this case, propose to the user
  // to close the primitive If landscape is intersected too, then the vertex
  // must be nearer then the first vertex
    ...
    if (gm) {
      ...
      if (!_Prim.getVerticesShapeInstance().empty()) {
        if (!_Prim.getVerticesShapeInstance()[0].empty()) {
          ...
          if (vertexBBox.intersect(worldViewRay.Origin,
                                   worldViewRay.Origin +
                                       200.f * worldViewRay.Dir)) {
            // if vertex nearer than landscape
            bool landscapeNearest = false;
            if (interType != NoIntersection) {
              // TODO nico : the test is not exact : should use intersection
              // with the bbox to do length comparison over the same line ...
              landscapeNearest = (wpPos - worldViewRay.Origin).norm() <
                                 (_Points[0] - worldViewRay.Origin).norm();
            }
  ...
  if (!isMouseOnUI() || isMouseOnWorldMap() || !_ValidPrim) {
    setMouseCursor(_ValidPos /* && !_InaccessibleParts */ ? "curs_create.tga"
                                                          : "curs_stop.tga");
  } else {
  ...
  if (_InaccessibleParts) {
    /*if (!testQuad)
    {
            ...
            testQuad->setQuad(CVector(0.f, thickness, 0.f), CVector(thickness
    * 2.f, thickness, 0.f), thickness);
  ...
  if (testDecal) {
    R2::CScenarioEntryPoints::CCompleteIsland *id =
    ...
    if (id) {
      ...
      _TestDecal.setCustomUVMatrix(
          true,
          getEditor().getIslandCollision().getWorldToAccessibilityTexMat(true));
...
void CToolDrawPrim::updateInaccessiblePrimRenderLook(CPrimRender &dest) {
  ...
  if (duration <= 0)
    duration = 0.1;
  CRGBA color =
      blend(CV_InaccessiblePosColor0.get(), CV_InaccessiblePosColor1.get(),
            0.5f + 0.5f * (float)cos(NLMISC::Pi * fmod((double)T1, duration) /
                                     duration));
  CPrimLook look = dest.getLook();
  look.EdgeLook.WorldMapColor = color;
  look.EdgeLook.DecalColor = color;
  dest.setLook(look);
  dest.setCustomWorldMapEdgeUVMatrix(
      true, getEditor().getIslandCollision().getWorldToAccessibilityTexMat());
  dest.setCustomDecalEdgeUVMatrix(
      true, getEditor().getIslandCollision().getWorldToAccessibilityTexMat(
                true)); // cropped version for decal rendering
}
...
bool CToolDrawPrim::onMouseLeftButtonDown() {
  ...
  if (canTerminate()) {
    ...
    commit();
    CToolMaintainedAction *tma =
        dynamic_cast<CToolMaintainedAction *>(getEditor().getCurrentTool());
    if (tma) {
...
void CToolDrawPrim::commit() {
  ...
  CObject *desc = NULL;
  if (!_Extending) {
    getDMC().newAction(NLMISC::CI18N::get(_PrimType == Road
                                              ? "uiR2EDCreateRouteAction"
                                              : "uiR2EDCreateZoneAction"));
  } else {
    getDMC().newAction(NLMISC::CI18N::get(_PrimType == Road
                                              ? "uiR2EDExtendRouteAction"
                                              : "uiR2EDExtendZoneAction"));
  }
  if (!_Extending) {
  ...
  if (desc || _Extending) {
    ...
    if (!_Extending) {
      ...
      if (!instanceId.empty()) {
        // ask a flag to prompt user to enter name for this object when it is
        // created
      ...
      for (uint k = 0; k < _Points.size(); ++k) {
        CObject *wp = getDMC().newComponent(_PrimType == Road ? "WayPoint"
                                                              : "RegionVertex");
        if (!wp)
          continue;
    ...
    if (!_Extending) {
      // set readable name
      string readableName = NLMISC::CI18N::get(
          _PrimType == Road ? "uiR2EDNameBotRoad" : "uiR2EDNameBotRegion");
      ...
      if (getEditor().getDefaultFeature()) {
        getDMC().requestInsertNode(
            getEditor().getDefaultFeature(getEditor().getBaseAct())->getId(),
            "Components", -1, "", desc);
      }
      //
      delete desc;
      // NB : if display of primitives is 'hide all', then force to 'show all',
      // else the new prim wouldn't be visible
      if (_ForceShowPrims) {
        if (getEditor().getEnv()["PrimDisplayVisible"].toBoolean() == false ||
            getEditor()
                    .getEnv()["PrimDisplayContextualVisibility"]
                    .toBoolean() == true) {
      ...
      for (uint k = _StartNumPoints - 1; k > _Points.size() - 1; --k) {
        getDMC().requestEraseNode(sons[k]->getDisplayedInstance()->getId(), "",
                                  -1);
      }

      for (uint k = 0; k < std::min(_StartNumPoints, uint(_Points.size()));
           ++k) {
        // change modified vertices
        CVector newWorldPos = CVectorD(_Points[k]).asVector();
        if (newWorldPos != _InitialPoints[k]) {
          CObject *wp = getDMC().newComponent(
              _PrimType == Road ? "WayPoint" : "RegionVertex");
          if (wp) {
            CObject *newPos = buildVector(offset + CVectorD(_Points[k]));
            getDMC().requestSetNode(sons[k]->getDisplayedInstance()->getId(),
                                    "Position", newPos);
            delete newPos;
          }
        }
      ...
      for (uint k = _StartNumPoints; k < _Points.size(); ++k) {
        ...
        getDMC().requestInsertNode(_ExtendedPrimitive->getId(), "Points", -1,
                                   "", wp);
        delete wp;
      }
      if (dv) {
...
bool CToolDrawPrim::onDeleteCmd() {
  ...
  if (_NumPoints == 0) {
    if (_Extending) {
      getDMC().newAction(CI18N::get("uiR2EDDeleteRoad") +
                         _ExtendedPrimitive->getDisplayName());
      // just erase the road
...
// ***************************************************************
bool CToolDrawPrim::isValidPolyShape(bool ignoreLast,
                                     std::list<CPolygon> &splitPoly) const {
  ...
  if (poly2D.selfIntersect())
    return false;
  CPolygon poly;
  poly.Vertices = _Points;
  if (poly.Vertices.back() == poly.Vertices[poly.Vertices.size() - 2] ||
      ignoreLast) {
    poly.Vertices.pop_back(); // duplicated point at the end will always fails
                              // the test, so remove it
  }
...
bool CToolDrawPrim::testAccessibleEdges(bool ignoreLast) {
  ...
  for (sint k = 0; k < (sint)numPoints; ++k) {
    if (!getEditor().getIslandCollision().isValidSegment(
            _Points[k], _Points[(k + 1) % _Points.size()])) {
      return false;
    }
  }
  return true;
...
{
        nlassert(0); // old code
        if (_LastPoints.size() == _Points.size() && std::equal(_Points.begin(),
_Points.end(), _LastPoints.begin())) return; // not modified, so last flag
remains valid _InaccessibleParts = false; _LastPoints = _Points;
        ...
        // change : valid to create if current mouse pos is valid (test 0.5f
meters around) CVector2f testPos = _Points[testPointIndex]; _ValidPrim =
getEditor().getIslandCollision().isValidPos(testPos);
...
void CToolDrawPrim::onActivate() {
  ...
  if (_ExtendedPrimitive) {
    ...
    if (!dv) {
      ...
      dv->setActiveRecurse(
          false); // hide the primitive being extended, because we display
