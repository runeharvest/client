...
CInstance *CToolSelectMove::createGhost(CInstance &instance) {
  ...
  if (luaProj.callMethodByNameNoThrow("copy", 0, 1)) {
    // now we got a table that is an exact (canonical) copy of the original
    // object, with the same instance ids.. prepare for new insertion by
    // renaming these instance id's (which 'newCopy' does)
    if (classDef["newCopy"].callNoThrow(1, 1)) {
      ...
      if (classDef["pasteGhost"].callNoThrow(1, 1)) {
        ...
        CInstance *newInst =
            getEditor().getInstanceFromId(ghost["InstanceId"].toString());
        if (newInst) {
          if (!newInst->getGhost()) {
            nlwarning(
                "When duplicating an object using the 'select/move' tool, temporary duplicate should be inserted \
								   as a ghost in the scene, removing object...");
            getEditor().getDMC().requestEraseNode(newInst->getId(), "", -1);
          }
          // set the flag so that the cost of this object isn't taken in account
          // in the displayed quotas
          newInst->getLuaProjection()["User"].setValue("GhostDuplicate", true);
          getEditor().setSelectedInstance(newInst);
          newInst->getDisplayerVisual()->setDisplayFlag(
              CDisplayerVisual::FlagHideActivities, true);
          nlwarning(
              "CToolSelectMove: beginning duplicate with instance with id %s",
              newInst->getId().c_str());
          // show in "frozen" state
          {
            /*CObjectNumber *numberValue = new CObjectNumber(2); // 2 = frozen
            state getEditor().getDMC().requestSetNode(newInst->getId(),
            "DisplayMode", numberValue); delete numberValue;
            */
            newInst->getDisplayerVisual()->setDisplayMode(
                CDisplayerVisual::DisplayModeFrozen);
...
void CToolSelectMove::beginAction(CInstance &instance) {
  ...
  // must work with eval link point because world pos may be realy far from link
  // pos (for regions)
  ...
  // nlwarning("starting to move instance at pos %s",
  // NLMISC::toString(_StartPos.asVector()).c_str());
  ...
  /** Compute delta in screen space between instance position and the click
   * position We maintain that delta when the instance moves
   */
  uint32 w, h;
  getScreenSize(w, h);
  ...
  computeLandscapeRayIntersection(rayOrigin, rayOrigin + 1000.f * rayDir,
  debugPos);
  */
  ...
  if (isShiftDown()) {
    // see in advance if there will be room left to create a duplicate for this
    // object
...
void CToolSelectMove::cancelAction(CInstance &instance) {
  ...
  if (_Duplicating) {
    ...
    if (_GhostInstance) {
      nlassert(_GhostInstance->getGhost()); // should have been inserted as a
                                            // ghost in the scene
      //
...
void CToolSelectMove::commitAction(CInstance &instance) {
  ...
  if (!_ValidPos) {
    ...
    if (_Duplicating && _Moved) {
      ...
      CLuaObject classDef = instance.getClass();
      std::string oldName =
          luaProj["Name"].toString(); // keep name, because the newCopy call
                                      // will generate a newone, causing indices
                                      // to increase 2 by 2

      // restore default value for display mode
      // getEditor().getDMC().requestEraseNode(_GhostInstance->getId(),
      // "DisplayMode", -1);
      if (_GhostInstance->getDisplayerVisual()) {
        _GhostInstance->getDisplayerVisual()->setDisplayMode(
            CDisplayerVisual::DisplayModeVisible);
      }
        ...
        if (luaProj.callMethodByNameNoThrow("copy", 0, 1)) {
          {
            std::string posInstanceId = instance.getPosInstanceId();
            if (!posInstanceId.empty()) {
              getEditor().requestRollbackLocalNode(
                  posInstanceId, ""); // Locally modified coords have
                                      // (of a local object ...)
                                      // have been read during the copy,
                                      // get rid of them...
            }
          }
          getDMC().newAction(CI18N::get("uiR2EDCopyAction") +
                             _GhostInstance->getDisplayName());
          getEditor().getDMC().requestEraseNode(_GhostInstance->getId(), "",
                                                -1);
          ...
          if (classDef["newCopy"].callNoThrow(1, 1)) {
            ...
            if (_AutoGroup.getGroupingCandidate()) {
              newCopy.push();
              CUniquePtr<CObject> desc(
                  CComLuaModule::getObjectFromLua(ls.getStatePointer()));
              _AutoGroup.group(desc.get(), _FinalPos);
            } else {
              newCopy.push();
              ls.push(false); // second parameter is for "not a new place"
              ls.push(
                  instance.getId()); // last param give the original instance id
                                     // of the object being copied
          ...
          if (!posInstanceId.empty()) {
            getEditor().requestRollbackLocalNode(
                posInstanceId, ""); // Locally modified coords
                                    // (of a local object ...)
                                    // have been read during the copy,
                                    // get rid of them...
          }
      ...
      if (!_ValidPos) {
        ...
        // getDMC().newAction(CI18N::get("uiR2EDMoveAction") +
        ...
        std::string instanceId =
            instance.getObjectTable()->getAttr("InstanceId")->toString();
...
void CToolSelectMove::updateAction(CInstance &instance) {
  ...
  CDisplayerVisual *vd = _GhostInstance ? _GhostInstance->getDisplayerVisual()
                                        : instance.getDisplayerVisual();
  if (!vd)
    return;
  ...
  if (mx != _MouseX || my != _MouseY || autoPanDx || autoPanDy) {
    if (!autoPanDx && !autoPanDy) {
      ...
      const sint32 timeThreshold =
          300; // if some time elapsed, then do the move
      if (!_Moved && abs(mx - _MouseRefX) < moveThreshold &&
          abs(my - _MouseRefY) < moveThreshold &&
          (T1 - _StartTime) < timeThreshold) {
        // discard small displacement when clicking (move is unwanted most of
        // the time)
        return;
      }
    }
    ...
    if (_ValidPos) {
      ...
      case NoIntersection:
        ...
        break;
      case ValidPacsPos:
        // nlwarning("moving instance at pos %s",
        // NLMISC::toString(inter).c_str());
        _FinalPos = inter;
  ...
  if (_InitiallyAccessible) {
    // NB : do not do the test if initially inaccessible to allow user to go to
    // an 'accessible' state more easily
    if (!vd->isAccessible()) {
      _ValidPos = false;
    }
  ...
  if (_InitiallyValidShape) {
    // is started from an invalid shape, allow to use intermediate invalid
    // shapes to correct it
  ...
  /*
  {
          ...
          drawBox(_StartPos - CVector(0.02f, 0.02f, 0.02f), _StartPos +
  CVector(0.02f, 0.02f, 0.02f), CRGBA::Magenta); Driver->setMatrixMode2D11();
          Driver->drawLine((_RefX - 5) / (float) w, _RefY / (float) h, (_RefX +
  5) / (float) w, _RefY / (float) h, CRGBA::Red); Driver->drawLine(_RefX /
  (float) w, (_RefY - 5) / (float) h, _RefX / (float) w, (_RefY + 5) / (float)
  h, CRGBA::Red);
          //
          Driver->drawLine((_PosRefX - 5) / (float) w, _PosRefY / (float) h,
  (_PosRefX + 5) / (float) w, _PosRefY / (float) h, CRGBA::Green);
          Driver->drawLine(_PosRefX / (float) w, (_PosRefY - 5) / (float) h,
  _PosRefX / (float) w, (_PosRefY + 5) / (float) h, CRGBA::Green);
          //
          Driver->drawLine((_MouseRefX - 5) / (float) w, _MouseRefY / (float) h,
  (_MouseRefX + 5) / (float) w, _MouseRefY / (float) h, CRGBA::Yellow);
          Driver->drawLine(_MouseRefX / (float) w, (_MouseRefY - 5) / (float) h,
  _MouseRefX / (float) w, (_MouseRefY + 5) / (float) h, CRGBA::Yellow);
          //
  }
  */
}
...
// ***************************************************************
void CToolSelectMove::setInstancePos(const NLMISC::CVectorD &pos,
                                     CInstance &instance) {
  ...
  getEditor().requestSetLocalNode(_GhostInstance ? _GhostInstance->getId()
                                                 : instance.getId(),
                                  "Position", newPos);
  delete newPos;
}
...
bool CToolSelectMove::isActionPossibleOn(const CInstance &instance) const {
  ...
  if (isShiftDown()) {
    ...
    mutableInstance.getLuaProjection().callMethodByNameNoThrow(
        "isCopyable", 0,
        1); // isCopyable should be 'const', so no worries here ...
...
void CToolSelectMove::updateBeforeRender() {
  ...
  if (_Duplicating && _GhostInstance) {
    _AutoGroup.update(_FinalPos, _GhostInstance->getPaletteId(),
                      _ValidPos && !isCtrlDown());
    _GhostInstance->getDisplayerVisual()->setDisplayFlag(
        CDisplayerVisual::FlagHideActivities,
        _AutoGroup.getGroupingCandidate() != NULL);
  } else {
    if (!_Duplicating && _GhostInstance)
      _GhostInstance->getDisplayerVisual()->setDisplayFlag(
          CDisplayerVisual::FlagHideActivities, false);
  }
}
