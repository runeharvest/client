...
// *********************************************************************************************************
CToolNewVertex::CToolNewVertex()
    : CToolChoosePos(-1, "curs_create.tga", "curs_create_vertex_invalid.tga"),
      _CurrEdge(-1), _CurrPos(CVector::Null) {
  lockMultiPos();
}
...
bool CToolNewVertex::isValidChoosePos(const CVector2f &pos) const {
  ...
  CDisplayerVisualGroup *dvg =
      dynamic_cast<CDisplayerVisualGroup *>(selection->getDisplayerVisual());
  if (!dvg)
    return false;
...
// *********************************************************************************************************
void CToolNewVertex::commit(const NLMISC::CVector &createPosition,
                            float /* createAngle */) {
  ...
  if (!selection->getDisplayerVisual())
    return;
  getDMC().newAction(NLMISC::CI18N::get(
      selection->isKindOf("Road") ? "uiR2EDInsertNewWayPointAction"
                                  : "uiR2EDInsertNewZoneVertexAction"));
  CObject *wp = getDMC().newComponent(
      selection->isKindOf("Road") ? "WayPoint" : "RegionVertex");
  if (!wp)
    return;
  wp->setObject(
      "Position",
      buildVector(CVectorD(createPosition -
                           selection->getDisplayerVisual()->getWorldPos())));
  if (_CurrEdge == (sint)(points->getSize() - 1)) {
    getDMC().requestInsertNode(selection->getId(), "Points", -1, "", wp);
  } else {
    getDMC().requestInsertNode(selection->getId(), "Points", _CurrEdge + 1, "",
                               wp);
  }
  delete wp;
}
