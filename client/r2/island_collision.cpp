...
const char *CIslandCollision::_AccessibilityTextureId =
        ...
        for (uint x = 0; x < width; ++x) {
          ...
          for (uint sy = 0; sy < CIslandCollision::MapSizeDivisor; ++sy) {
            for (uint sx = 0; sx < CIslandCollision::MapSizeDivisor; ++sx) {
              ...
              if (srcX < hm.getWidth() && srcY < hm.getHeight()) {
...
// *********************************************************************************************************
CAccessibilityTexture::CAccessibilityTexture(CIslandCollision *islandCol)
    : _IslandCol(islandCol) {
...
// *********************************************************************************************************
CIslandCollision::~CIslandCollision() { release(); }
...
// *********************************************************************************************************
R2::CScenarioEntryPoints::CCompleteIsland *
CIslandCollision::getCurrIslandDesc() {
  ...
  if (dumpPACSCorrepondance) {
    ...
    for (uint y = 0; y < height * zoom; ++y) {
      for (uint x = 0; x < width * zoom; ++x) {
        ...
        // get value from pacs
        CTool::TRayIntersectionType interType =
            CTool::getPacsType(CVector((float)x / zoom + _IslandDesc.XMin,
                                       (float)y / zoom + _IslandDesc.YMin, 0.f),
                               100000.f, pacsPos);
        ...
        *dest2 = *dest;
        bool accessible =
            _HeightMap(clamped((sint)((x + 0.5f) / zoom), width), iy) <
                0x7ffe &&
            _HeightMap(clamped((sint)((x - 0.5f) / zoom), width), iy) <
                0x7ffe &&
            _HeightMap(ix, clamped((sint)((y + 0.5f) / zoom), height)) <
                0x7ffe &&
...
// *********************************************************************************************************
bool CIslandCollision::findEmptyPlace(const CVector2f &startPos,
                                      NLMISC::CVector &dest) {
  ...
  /*if (dumpSearch)
  {
          ...
          for (uint y = 0; y < _HeightMap.getHeight(); ++y)
          {
                  ...
                  {
                          if (_HeightMap(x, y) <= 0x7ffe) *dest++ =
  CRGBA::Magenta; else (dest++)->set((uint8) (127 + _HeightMap(x, y)), (uint8)
  (127 + _HeightMap(x, y)), (uint8) (127 + _HeightMap(x, y)), 255);
                  }
          }
  }*/
  ...
  for (;;) {
    ...
    if (isValidPos(currPos)) {
      ...
      {
        CTool::TRayIntersectionType interType =
            CTool::computeWorldMapIntersection(currPos.x, currPos.y, dest);
...
void CIslandCollision::updateCurrPackedIsland() {
  ...
  if (fabsf(newPos.x - _LastPackedIslandTestPos.x) >= 80.f ||
    ...
    loadEntryPoints();
    CVectorD playerPos = UserEntity->pos();
    CScenarioEntryPoints::CCompleteIsland *island =
        sep.getCompleteIslandFromCoords(
            CVector2f((float)playerPos.x, (float)playerPos.y));
    if (!island) {
      _PackedIslandLoadingFailure = true;
      _PackedIsland = NULL;
      return;
    }
    if (_PackedIsland && _PackedIslandName == island->Island)
      return; // same than before, no-op
    reloadPackedIsland(
        *island); // first island or entered a new island -> reload
  }
}

// *********************************************************************************************************
CPackedWorld *CIslandCollision::reloadPackedIsland(
  ...
  _HeightMap.clear();
  {
    ...
    _AccessibilityTexture =
        new NL3D::CTextureUser(new CAccessibilityTexture(this));
    _AccessibilityTexture->setFilterMode(UTexture::Nearest,
                                         UTexture::NearestMipMapOff);
    _AccessibilityTexture->setWrapS(UTexture::Clamp);
    _AccessibilityTexture->setWrapT(UTexture::Clamp);
    // make this texture accessible in the ui view renderer from a simple
    // filename this allow to display it in the world map
    ...
    vr.setExternalTexture(_AccessibilityTextureId, _AccessibilityTexture,
    ...
    if (vr.getTextureIdFromName(_AccessibilityTextureId) == -1) {
      ...
      nlwarning("Creating accessibility texture with id %d ",
                (int)vr.getTextureIdFromName(_AccessibilityTextureId));
    } else {
      vr.updateTexturePos(_AccessibilityTextureId, 0, 0, width, height);
    }
    // scale resumes to dividing by real coords, because heightmap resolution is
    // hardcoded to 1 meter
    CVector scale(1.f / std::max(_HeightMap.getWidth(), 1u),
                  1.f / std::max(_HeightMap.getHeight(), 1u), 0.f);
    _WorldToAccessibilityTexMat.setScale(scale);
    _WorldToAccessibilityTexMat.setPos(
        CVector((float)-islandDesc.XMin * scale.x,
                (float)-islandDesc.YMin * scale.y, 0.f));
    ...
    _WorldToAccessibilityTexMatCropped.setPos(
        CVector((float)-islandDesc.XMin * scale.x,
                (float)-islandDesc.YMin * scale.y, 0.f));
  }
  return _PackedIsland;
...
void CIslandCollision::mergeWaterSurfaces() {
  ...
  for (std::list<CWaterSurf>::iterator it = _WaterSurfs.begin();
       it != _WaterSurfs.end(); ++it) {
    ...
    for (uint k = 0; k < tris.size(); ++k) {
      ...
      clippedPoly.clip(&waterPlane, 1);
      {
        ...
        std::copy(clippedPoly.Vertices.begin(), clippedPoly.Vertices.end(),
                  clippedPoly2D.Vertices.begin());
...
void CIslandCollision::addInaccessibleRegion(const NLMISC::CPolygon2D &poly) {
  ...
  for (sint y = minY; y < (sint)(minY + borders.size()); ++y) {
    if (y < 0 || y >= (sint)_HeightMap.getHeight())
      continue;
    for (sint x = borders[y - minY].first; x <= borders[y - minY].second; ++x) {
      if (x < 0 || x >= (sint)_HeightMap.getWidth())
        continue;
...
bool CIslandCollision::isValidPoly(const NLMISC::CPolygon2D &poly) {
  ...
  if (!islandDesc)
    return true;
  // apply offset to polygon to  avoid overflow (coords of rasterized poly can
  // be 32000 at most)
  ...
  for (uint k = 0; k < poly.Vertices.size(); ++k) {
    ...
    offsetedPoly.Vertices[k].set(src.x - (float)islandDesc->XMin,
                                 src.y - (float)islandDesc->YMin);
    ...
    //
    if (offsetedPoly.Vertices[k].x < -32000.f)
      return false;
    if (offsetedPoly.Vertices[k].x >= 32000.f)
      return false;
    if (offsetedPoly.Vertices[k].y < -32000.f)
      return false;
    if (offsetedPoly.Vertices[k].y >= 32000.f)
      return false;
  }
  ...
  if (dumpTestBitmap) {
    ...
    for (sint y = 0; y < (sint)borders.size(); ++y) {
      if (y + minY < 0 || y + minY >= (sint)bm.getHeight())
        continue;
      sint xmin = borders[y].first;
      sint xmax = borders[y].second;
      if (xmin > xmax)
        continue;
      if (xmax < 0 || xmin >= (sint)hm.getWidth())
        continue;
  ...
  for (sint y = 0; y < (sint)borders.size(); ++y) {
    if (y + minY < 0 || y + minY >= (sint)hm.getHeight())
      return false;
    sint xmin = borders[y].first;
    sint xmax = borders[y].second;
    if (xmin > xmax)
...
// *********************************************************************************************************
bool CIslandCollision::isValidSegment(const NLMISC::CVector2f start,
...
// *********************************************************************************************************
void CIslandCollision::waterSurfaceAdded(const NLMISC::CPolygon2D &shape,
  ...
  for (std::list<CWaterSurf>::iterator it = _WaterSurfs.begin();
    ...
    if (shape.Vertices.size() == it->Shape.Vertices.size()) {
      if (std::equal(shape.Vertices.begin(), shape.Vertices.end(),
                     it->Shape.Vertices.begin())) {
        float oldMat[16];
        float newMat[16];
...
bool CIslandCollision::isValidPos(const NLMISC::CVector2f &pos) {
  ...
  if (!isValidSegment(pos - CVector2f(0.5f, 0.f), pos + CVector2f(0.5f, 0.f)))
    return false;
  CVector dummy;
  return CTool::computeWorldMapIntersection(pos.x, pos.y, dummy) ==
         CTool::ValidPacsPos;
}
