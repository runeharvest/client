...
struct ISelectableObject {
  ...
  // ask if current intersection with mouse ray in in the projection of the
  // object (must call isInProjection)
...
// ***********************************************************************************************************************
// Decoration in a CGroupMap that expose this interface (through dynamic_cast)
// are representation of CInstance in the island map
struct IDisplayerUIHandle {
  virtual CInstance &getDisplayedInstance() = 0;
  // test that the given position intesersect the displayer (default is true,
  // because this is call only if bounding rect test succeeded)
  virtual bool contains(sint32 /* mouseXInWindow */,
                        sint32 /* mouseYInWindow */) const {
    return true;
  }
  ...
  virtual uint getEdgeIndex() const {
    nlassert(0);
    return 0;
  }
};
...
public:
  ...
  enum TDisplayFlags {
    ...
    FlagHasFocus,
    FlagHighlighted,
    FlagBadPos, // a 'stop' icon is displayed on entity when it is on a bad
                // landscape position may happen whn a group is moved as a whole
    FlagHideActivities,
    FlagCount
  };

  // Current display mode. Display mode is retrieved from the lua property
  // 'DisplayMode', and is thus saved with the object
  enum TDisplayMode {
   ...
   * If not visible then "setActive(false)" will be called to remove the
   * displayed object from the view. Modification messages (if any ...) should
   * then be ignored by derivers until onCreate is called again
   */
  ...
  /** NB : derivers should not usually react to the 'onPostCreate' or 'onErase'
   * methods because default behavior of these is to call 'setActive' as
   * necessary when the user select an act in which the displayed instance is
  ...
  /** Derivers note : 'onAttrModifier' takes care of updating position of the
   * displayer, hence derivers should call their parent version before updating
   * real position in the 3D scene
   */
   ...
   * Because object may have been made son of an new object with another world
   * pos
   */
  ...
  // get display mode, (not taking possible inheritance in account), sint32 for
  // lua export
  ...
  // return true if the object is currently visible *in current act* (mean it is
  // not and'ed with the getActive() flag)
  bool getActualVisibility() const {
    return getActualDisplayMode() != DisplayModeHidden;
  }
  ...
  /** Eval a point at which the displayed object may be linked to when
   * displaying groups Must be expressed in world coordinates
   */
  virtual NLMISC::CVector evalLinkPoint(bool /* leader */ = false) {
    return getWorldPos().asVector();
  }
  // Eval enter point (useful for zones)
  virtual bool evalEnterPoint(const NLMISC::CVector &startPoint,
                              NLMISC::CVector &result);
  // Eval exit point (for objects such as roads), default resume to
  // evalEnterPoint
  virtual NLMISC::CVector evalExitPoint();
  // From ISelectableObject
  virtual CInstance *getInstanceInEditor() const {
   ...
   * May have sens for entity such as region which are just projected over the
   * scene and don't depend on PACS for their display
   */
  virtual bool isInvalidPacsPosAcceptable() const { return false; }

  /** For the move tool : test if all parts of the object are accessible (e.g
   * not on an unreachable part of the map) NB : this is a display, flag, not
   * updated if the current displayer is not active !!
   */
  virtual bool isAccessible() { return true; }

  // test if the current shape for this displayer is valid (may be false for
  // self-intersecting polys)
  ...
  // called by parent when the display mode has changed, so sons may want to
  // update their display if their inherit it
  virtual void onParentDisplayModeChanged() {}

  /////////////////
  // POSITION(S) //
  /////////////////
  // TODO nico : should really move the position into "CInstance", or better,
  // find a way to derive CWorldObject in C++ rather than in lua. Position is a
  // property of the object, not a display property!! Should not be the
  // responsability of the displayer to maintain this ...

  // Get 3D position relative to parent.
  NLMISC::CVectorD getPos() const { return _Pos; }
  // Get world 2D position
  NLMISC::CVector2f getWorldPos2f() const {
    return NLMISC::CVector2f((float)_WorldPos.x, (float)_WorldPos.y);
  }
  ...
  // Eval all sub-positions in world as CVector2f (default is to clear the
  // vector)
...
private:
  ...
  CGroupInScene *_IconInScene; // a "stop" icon showing that the object has an
                               // invalid position
...
protected:
  ...
  getBlinkColor(NLMISC::CRGBA defaultColor,
                NLMISC::CRGBA blinkColor = NLMISC::CRGBA(192, 192, 192)) const;

  // See if the z need to be reevaluated. May happen after a tp or when the
  // camera moves (used by shapes) Calling this will update the invalidity flag
  bool testNeedZEval();
