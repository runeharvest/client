...
public:
  ...
  enum TSelectionDisplayMode {
    AutoSelection = 0,
    BoxSelection = 1,
    CircleSelection = 2,
    SelectionDisplayModeCount
  };
  ...
  void setSelectionDisplayMode(sint32 value) {
    _SelectionDisplayMode =
        (TSelectionDisplayMode)((uint32)value % SelectionDisplayModeCount);
  }
  sint32 getSelectionDisplayMode() const {
    return (sint32)_SelectionDisplayMode;
  }
  ...
  REFLECT_SINT32("SelectionDisplayMode", getSelectionDisplayMode,
  ...
  // return radius of the cylinder for the collision primitive, or -1 if a box
  // is used
  ...
  // manage clipping of entity to display then earest entities (and thus bypass
  // the 255 entities limit)
  void setClipBlend(float amount);

  // test if the entity was created by the user right now (not by another user
  // or undo/reload/reload operation)
...
private:
  ...
  float _Angle;
  bool _AddedToMap;
  bool _RegisteredInEditor; // entitiy displayer register themselves in editor
                            // (it sorts entities by distance and display the
                            // nearest one by updating their clipping by calling
                            // 'setClipAmount')
  float _ClipBlend;
  ...
  //
  CDisplayerVisualShape *_PlaceHolder; // place holder used when there are too
...
private:
  ...
  void drawBBox(const NLMISC::CMatrix &modelMatrix, const NLMISC::CAABBox &bbox,
                NLMISC::CRGBA colOverZ, NLMISC::CRGBA colUnderZ);
  void showSelected();
