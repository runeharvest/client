...
// *********************************************************************************************************
/** this class forward modification events from the network so that the editor
 * can update its state to match the current state of the map
 */
class CDynamicMapClientEventForwarder : public CDynamicMapClient {
public:
  CDynamicMapClientEventForwarder(const std::string &eid,
                                  NLNET::IModuleSocket *clientGateway,
                                  lua_State *luaState);
  virtual void nodeErased(const std::string &instanceId,
                          const std::string &attrName, sint32 position);
  virtual void nodeSet(const std::string &instanceId,
                       const std::string &attrName, CObject *value);
  virtual void nodeInserted(const std::string &instanceId,
                            const std::string &attrName, sint32 position,
                            const std::string &key, CObject *value);
  virtual void nodeMoved(const std::string &instanceId,
                         const std::string &attrName, sint32 position,
                         const std::string &destInstanceId,
                         const std::string &destAttrName, sint32 destPosition);
  virtual void scenarioUpdated(CObject *highLevel, bool willTP,
                               uint32 initialActIndex);

  virtual void onAnimationModeConnected(
      const CClientMessageAdventureUserConnection &connected);
  virtual void onEditionModeConnected(uint32 userSlotId, uint32 adventureId,
                                      CObject *highLevel,
                                      const std::string &versionName,
                                      bool willTP, uint32 initialActIndex);
  virtual void onEditionModeDisconnected();
  virtual void onResetEditionMode();
  virtual void onTestModeConnected();
  virtual void onTestModeDisconnected(TSessionId sessionId, uint32 lasAct,
                                      TScenarioSessionType sessionType);
};

CDynamicMapClientEventForwarder::CDynamicMapClientEventForwarder(
    const std::string &eid, NLNET::IModuleSocket *clientGateway,
    lua_State *luaState)
    : CDynamicMapClient(eid, clientGateway, luaState) {}

void CDynamicMapClientEventForwarder::nodeErased(const std::string &instanceId,
                                                 const std::string &attrName,
                                                 sint32 position) {
  // H_AUTO(R2_CDynamicMapClientEventForwarder_nodeErased)
  if (getEditor().getMode() != CEditor::EditionMode) {
    nlassert(getEditor().getMode() !=
             CEditor::AnimationModeLoading); /* Probably should not happen */
...
void CDynamicMapClientEventForwarder::nodeSet(const std::string &instanceId,
                                              const std::string &attrName,
                                              CObject *value) {
  // H_AUTO(R2_CDynamicMapClientEventForwarder_nodeSet)
  if (getEditor().getMode() != CEditor::EditionMode && getEditor().getMode() !=
                                                           CEditor::
                                                               AnimationModeLoading /* Loading animation scenario from terminal, ghost nodes created by translator */) {
...
void CDynamicMapClientEventForwarder::nodeInserted(
...
void CDynamicMapClientEventForwarder::nodeMoved(
    const std::string &instanceId, const std::string &attrName, sint32 position,
    const std::string &destInstanceId, const std::string &destAttrName,
    sint32 destPosition) {
  ...
  getEditor().nodeMoved(instanceId, attrName, position, destInstanceId,
                        destAttrName, destPosition);
}

void CDynamicMapClientEventForwarder::scenarioUpdated(CObject *highLevel,
                                                      bool willTP,
                                                      uint32 initialActIndex) {
...
void CDynamicMapClientEventForwarder::onEditionModeConnected(
    uint32 userSlotId, uint32 adventureId, CObject *highLevel,
    const std::string &versionName, bool willTP, uint32 initialActIndex) {
  // H_AUTO(R2_CDynamicMapClientEventForwarder_onEditionModeConnected)
  getEditor().onEditionModeConnected(userSlotId, adventureId, highLevel,
                                     versionName, willTP, initialActIndex);
}
...
void CDynamicMapClientEventForwarder::onTestModeDisconnected(
    TSessionId sessionId, uint32 lastAct, TScenarioSessionType sessionType) {
...
void CDynamicMapClientEventForwarder::onAnimationModeConnected(
    const CClientMessageAdventureUserConnection &connected) {
...
void CEditor::setForceDesktopReset(bool force) {
  ...
  CHECK_EDITOR
  std::fill(_ForceDesktopReset,
            _ForceDesktopReset + sizeofarray(_ForceDesktopReset), force);
}
...
void CEditor::autoConfigInit(bool serverIsRingSession) {
  ...
  if (ClientCfg.R2EDEnabled) {
    ...
    case R2::TUserRole::ur_player:
      ...
      // NOTE: I'm not sure if 'editor' is the right mode here - but I can't
      // find anything better for now If I try 'AccessModeUnknown' the code
      // asserts later on.
      initialAccessMode = R2::CEditor::AccessEditor;
      break;
    default:
      nlwarning("Unexpected user role %u while editor enabled",
                (uint)(UserRoleInSession.getValue()));
      initialMode = R2::CEditor::EditionMode;
      initialAccessMode = R2::CEditor::AccessEditor;
    }
    ...
    if (UserRoleInSession.getValue() != R2::TUserRole::ur_player)
      nlwarning("Unexpected user role %u while editor disabled",
                (uint)(UserRoleInSession.getValue()));
  }
}
...
void CEditor::initModules(bool connectDMC) {
  ...
  CCommandRegistry::getInstance().execute("clientGw.transportAdd FEClient fec",
                                          *InfoLog);
  if (!ClientCfg.Local) {
    CCommandRegistry::getInstance().execute("clientGw.transportCmd fec(open)",
                                            *InfoLog);
  }
  ...
  if (connectDMC)
    _DMC = new CDynamicMapClientEventForwarder("Client0", socketClientGw,
                                               getLua().getStatePointer());
  else
    _DMC = new CDynamicMapClient("Client0", socketClientGw,
                                 getLua().getStatePointer());
...
// *********************************************************************************************************
CEditor::~CEditor() { release(); }

// ***************************************************************
void CEditor::requestSetLocalNode(const std::string &instanceId,
                                  const std::string &attrName,
                                  const CObject *value) {
  ...
  CObject *obj = NULL;
  {
    ...
    if (!src) {
      nlwarning("Can't find object with id '%s' (setting attribute '%s')",
                instanceId.c_str(), attrName.c_str());
      return;
    }
    if (!attrName.empty()) {
      ...
      if (!subObj) {
        ...
        if (!valueBase) {
          nlwarning("Can't find attribute %s inside object (or is base) with "
                    "InstanceId =  %s",
                    attrName.c_str(), instanceId.c_str());
          return;
        }
      ...
      if (!subObj) {
        nlwarning("Can't find attribute %s inside object with InstanceId =  %s",
                  attrName.c_str(), instanceId.c_str());
        return;
      }
...
void CEditor::requestCommitLocalNode(const std::string &instanceId,
                                     const std::string &attrName) {
  ...
  /*
  // when requestSetNode is called, the old value can't be retrieved because it
  will
...
// ***************************************************************
void CEditor::requestRollbackLocalNode(const std::string &instanceId,
                                       const std::string &attrName) {
  ...
  getDMC().getPropertyAccessor().rollbackValue(obj);
  // force the displayer to update their state from the shadowed value in place
  // of the local value
  onAttrModified(obj);
}
...
void CEditor::clearDebugWindow() {
  ...
  CGroupList *gl = dynamic_cast<CGroupList *>(
      CWidgetManager::getInstance()->getElementFromId(
          "ui:interface:debug_info:content:cb:text_list"));
  if (gl) {
...
int CEditor::luaSetSelectedInstanceId(CLuaState &ls) {
  ...
  if (!inst) {
    nlwarning("setSelectedInstance : Instance with Id %s not found",
              ls.toString(2));
    return 0;
  }
  if (!inst->getSelectableFromRoot()) {
    nlwarning("Instance with id %s or one of its ancestor is not selectable",
              ls.toString(2));
    return 0;
  }
...
int CEditor::luaSetCurrentTool(CLuaState &ls) {
  ...
  CHECK_EDITOR
  CLuaIHM::check(ls, ls.getTop() == 2 || ls.getTop() == 3,
                 "luaSetCurrentTool (method)");
...
int CEditor::luaGetCurrentTool(CLuaState &ls) {
  ...
  CHECK_EDITOR
  CLuaIHM::checkArgCount(ls, "getCurrentTool",
                         1); // 1 because of the method call
...
int CEditor::luaGetSelectedInstance(CLuaState &ls) {
  ...
  if (!getEditor().getSelectedInstance()) {
    ...
    getEditor().projectInLua(
        getEditor().getSelectedInstance()->getObjectTable());
  }
  return 1;
}
...
int CEditor::luaGetVisualPropertiesFromInstanceId(CLuaState &ls) {
  ...
  std::string strVPABC = NLMISC::toString(
      "VPA:%016.16" NL_I64 "x\nVPB:%016.16" NL_I64 "x\nVPC:%016.16" NL_I64 "x",
      uVPA, uVPB, uVPC);
...
int CEditor::luaSetEntityCustomSelectBox(CLuaState &ls) {
  ...
  CHECK_EDITOR
  CLuaIHM::checkArgCount(ls, "setEntityCustomSelectBox",
                         3); // 3 because of method call
  ...
  if (dumpMap)
  {
          for (TEntityCustomSelectBoxMap::const_iterator watchIt =
  boxMap.begin(); watchIt != boxMap.end(); ++ watchIt)
          {
...
int CEditor::luaGetEntityCustomSelectBox(CLuaState &ls) {
  ...
  CHECK_EDITOR
  CLuaIHM::checkArgCount(ls, "getEntityCustomSelectBox",
                         2); // 3 because of method call
...
int CEditor::luaChoosePos(CLuaState &ls) {
  ...
  //
  getEditor().setCurrentTool(new CToolChoosePosLua(
      ghostSlot, validFunc, cancelFunc, toolName, cursValid, cursInvalid, polys,
      validLook, invalidLook));
  return 0;
}
...
int CEditor::luaSnapPosToGround(CLuaState &ls) {
  ...
  //
  /*NLPACS::UGlobalPosition gpos = GR->retrievePosition(CVector((float)
  ls.toNumber(2), (float) ls.toNumber(3), 2000.f)); if (gpos.InstanceId != -1)
  {
  ...
  // new algo : to avoid to snapping to a cliff, start from the approximate
  // (valid) height found in the height map
  CVector inter;
  if (CTool::computeWorldMapIntersection((float)ls.toNumber(2),
                                         (float)ls.toNumber(3),
                                         inter) != CTool::NoIntersection) {
    ls.push(inter.x);
    ls.push(inter.y);
    ls.push(inter.z);
    return 3;
...
int CEditor::luaSetCurrentActFromId(CLuaState &ls) {
  ...
  if (!act->isKindOf("Act")) {
    nlwarning("%s : instance with id %s is not an act", funcName,
              ls.toString(1));
    return 0;
  }
...
int CEditor::luaGenInstanceName(CLuaState &ls) {
  ...
  CLuaIHM::checkArgType(ls, funcName, 2, LUA_TSTRING);
  ls.push(getEditor()
              .genInstanceName(ucstring::makeFromUtf8(ls.toString(2)))
              .toUtf8()); // FIXME: Ring UTF-8
#endif
  return 1;
}
...
#else
  CLuaIHM::checkArgType(ls, funcName, 2, LUA_TSTRING);
  ls.push(getEditor().isPostFixedByNumber(
      ucstring::makeFromUtf8(ls.toString(2)))); // FIXME: Ring UTF-8
#endif
  return 1;
}
...
int CEditor::luaIsCreature(CLuaState &ls) {
  ...
  const CCharacterSheet *sheet = dynamic_cast<const CCharacterSheet *>(
      SheetMngr.get(NLMISC::CSheetId(inst->getSheet())));
  ...
  bool creature =
      !(isNPC || (!isNPC && (sheet->Race == EGSPD::CPeople::Kami ||
                             sheet->Race == EGSPD::CPeople::Karavan)));
  ls.push(creature);
...
int CEditor::luaSetEditorSeason(CLuaState &ls) {
  ...
  if (getEditor().getMode() != CEditor::EditionMode &&
    ...
    CLuaIHM::fails(ls, "%s : cannot set weather value outside of edit mode",
                   funcName);
  }
  CLuaIHM::checkArgCount(ls, funcName, 2);
  CLuaIHM::checkArgType(ls, funcName, 2,
                        LUA_TSTRING); // this is a method (self + 1 param)
  if (strcmp(ls.toString(2), "Automatic") == 0)
    getEditor()._Season = Automatic;
  else if (strcmp(ls.toString(2), "Spring") == 0)
    getEditor()._Season = Spring;
  else if (strcmp(ls.toString(2), "Summer") == 0)
    getEditor()._Season = Summer;
  else if (strcmp(ls.toString(2), "Autumn") == 0)
    getEditor()._Season = Autumn;
  else if (strcmp(ls.toString(2), "Winter") == 0)
    getEditor()._Season = Winter;
  else
    CLuaIHM::fails(ls, "%s, invalid season name : %s", ls.toString(2));
  // If season is changed while scenario is being received, we do not have
  // received the tp command yet, so signal the weather system that the season
...
int CEditor::luaSetFixedLighting(CLuaState &ls) {
  ...
  CLuaIHM::checkArgCount(ls, funcName, 2);
  CLuaIHM::checkArgType(ls, funcName, 2,
                        LUA_TBOOLEAN); // this is a method (self + 1 param)
...
int CEditor::luaSetPlotItemInfos(CLuaState &ls) {
  ...
  CLuaIHM::checkArgType(ls, funcName, 5, LUA_TSTRING);
#endif
  CItemSheet *item = dynamic_cast<CItemSheet *>(
      SheetMngr.get(CSheetId((uint32)ls.toInteger(2))));
...
int CEditor::luaIsCurrentSelectionPlayer(CLuaState &ls) {
  ...
  CPlayerCL *player = dynamic_cast<CPlayerCL *>(
      EntitiesMngr.entity(UserEntity->getTargetSlotNoLag()));
  ls.push(player != NULL);
  return 1;
}
...
int CEditor::luaFindEmptyPlace(CLuaState &ls) {
  ...
  CLuaIHM::checkArgType(ls, funcName, 3, LUA_TNUMBER);
  CVector result;
  if (getEditor().getIslandCollision().findEmptyPlace(
          CVector2f((float)ls.toNumber(2), (float)ls.toNumber(3)), result)) {
    ls.push(result.x);
    ls.push(result.y);
    ls.push(result.z);
    return 3;
...
int CEditor::luaIsInIslandRect(CLuaState &ls) {
  ...
  CLuaIHM::checkArgType(ls, funcName, 2, LUA_TNUMBER);
  CLuaIHM::checkArgType(ls, funcName, 3, LUA_TNUMBER);
  const R2::CScenarioEntryPoints::CCompleteIsland *island =
  ...
  ls.push(x >= island->XMin && y >= island->YMin && x <= island->XMax &&
          y <= island->YMax);
  return 1;
}
...
int CEditor::luaGetCurrentIslandName(CLuaState &ls) {
  ...
  R2::CScenarioEntryPoints::CCompleteIsland *ci =
      getEditor().getIslandCollision().getCurrIslandDesc();
...
int CEditor::luaKickCharacter(CLuaState &ls) {
  ...
  sb.kickCharacter(
      sb.getCharId(),
      R2::getEditor().getDMC().getEditionModule().getCurrentAdventureId(),
...
int CEditor::luaTeleportToCharacter(CLuaState &ls) {
  ...
  cem.requestTeleportOneCharacterToAnother(
      cem.getCurrentAdventureId(),
      CSessionBrowserImpl::getInstance().getCharId(), (uint32)ls.toInteger(2));
  return 0;
}
...
//H_AUTO(R2_CEditor_luaIsValidPoly)
  const char *funcName = "isValidPoly";
        ...
        CLuaIHM::checkArgType(ls, funcName, 3, LUA_TTABLE); // method with no
args ls.pushValue(3); CLuaObject poly; NLMISC::CPolygon2D poly2D;
        poly.pop(luaState);
        ENUM_LUA_TABLE(table, it)
        {
                ...
                {
                        CLuaIHM::fails(ls, "%s expects CVector2f for poly
coordinates);
                }
...
ls.toNumber(2), (float) ls.toNumber(3), 2000.f)); if (gpos.InstanceId != -1)
        ...
        // new algo : to avoid to snapping to a cliff, start from the
approximate (valid) height found in the height map CVector inter; if
(CTool::computeWorldMapIntersection((float) ls.toNumber(2), (float)
ls.toNumber(3), inter) != CTool::NoIntersection)
        {
                ls.push(inter.x);
                ls.push(inter.y);
                ls.push(inter.z);
...
int CEditor::luaGetUserEntityName(CLuaState &ls) {
  ...
  CLuaIHM::checkArgCount(ls, funcName, 1); // this is a method
  if (UserEntity) {
    string name = UserEntity->getEntityName() +
...
int CEditor::luaEnumInstances(CLuaState &ls) {
    ...
    static int next(CLuaState &ls) {
      ...
      while (ie->Current != ie->InstMap->end()) {
        ...
        if (!inst->getGhost()) {
          ...
          if (!parentAct || parentAct == ed.getCurrentAct() ||
              parentAct == ed.getBaseAct()) {
  ...
  if (classIndex < 0) {
    CLuaIHM::fails(ls, "Trying to iterate over unknown class : %s",
                   ls.toString(2));
  }
  ls.newTable();
  CLuaObject mt;
...
void CEditor::waitScenarioScreen() {
  ...
  CInterfaceGroup *waitScreen = dynamic_cast<CInterfaceGroup *>(
      CWidgetManager::getInstance()->getElementFromId(
          "ui:interface:r2ed_connecting"));
  if (waitScreen) {
  ...
  CInterfaceElement *sysInfo = CWidgetManager::getInstance()->getElementFromId(
      "ui:interface:system_info");
  bool sysInfoActive = false;
  if (sysInfo)
    sysInfoActive = sysInfo->getActive();
  ...
  for (;;) {
    ...
    try {
      ...
      if (NetMngr.getConnectionState() == CNetManager::Disconnect) {
        ...
        CViewText *pVT = dynamic_cast<CViewText *>(
            CWidgetManager::getInstance()->getElementFromId(
                "ui:interface:r2ed_connecting:title"));
        if (pVT != NULL)
          pVT->setText(CI18N::get("uiFirewallAlert") + "...");

        // The mouse and fullscreen mode should be unlocked for the user to set
        // the firewall permission
        nlSleep(30); // 'nice' the client, and prevent to make too many send
                     // attempts
      }
    }
    ...
    if (state == WaitingTP && getEditor()._TPReceivedFlag) {
      break; // don't want an additionnal swap buffer here, so exit now (the
             // loading bitmap is good, has just been reloaded by the tp
             // screen...
    }
    ...
    if (!waitScreen) {
      ...
      TextContext->printfAt(0.5f, 0.5f,
    ...
    if (NetMngr.getConnectionState() == CNetManager::Disconnect) {
      if (firewallTimeout) {
        // Display the firewall error string instead of the normal failure
        // string
        CViewText *pVT = dynamic_cast<CViewText *>(
            CWidgetManager::getInstance()->getElementFromId(
                "ui:interface:r2ed_connecting:title"));
  ...
  if (sysInfo)
    sysInfo->setActive(sysInfoActive);
...
public:
  ...
  virtual void onAttrModified(CInstance &instance, const std::string &attrName,
                              sint32 attrIndex);
...
CInstanceObserverLua::CInstanceObserverLua(CLuaObject &receiver)
    : _Receiver(receiver) {
  ++_Count;
}
...
void CInstanceObserverLua::onInstanceCreated(CInstance &instance) {
  ...
  if (_Receiver["onInstanceCreated"].isNil())
    return; // no-op if not handled
...
void CInstanceObserverLua::onInstanceErased(CInstance &instance) {
  ...
  nlassert(_Receiver.isValid());
  if (_Receiver["onInstanceErased"].isNil())
    return; // no-op if not handled
  getEditor().projectInLua(instance.getObjectTable());
...
void CInstanceObserverLua::onPreHrcMove(CInstance &instance) {
  ...
  nlassert(_Receiver.isValid());
  if (_Receiver["onPreHrcMove"].isNil())
    return; // no-op if not handled
  getEditor().projectInLua(instance.getObjectTable());
...
void CInstanceObserverLua::onPostHrcMove(CInstance &instance) {
  ...
  nlassert(_Receiver.isValid());
  if (_Receiver["onPostHrcMove"].isNil())
    return; // no-op if not handled
  getEditor().projectInLua(instance.getObjectTable());
  _Receiver.callMethodByNameNoThrow("onPostHrcMove", 1, 0);
}

void CInstanceObserverLua::onAttrModified(CInstance &instance,
                                          const std::string &attrName,
                                          sint32 attrIndex) {
  // H_AUTO(R2_CInstanceObserverLua_onAttrModified)
  nlassert(_Receiver.isValid());
  if (_Receiver["onAttrModified"].isNil())
    return; // no-op if not handled
  getEditor().projectInLua(instance.getObjectTable());
...
int CEditor::luaAddInstanceObserver(CLuaState &ls) {
  ...
  ls.push(getEditor().addInstanceObserver(ls.toString(2),
                                          new CInstanceObserverLua(receiver)));
  return 1;
}
...
int CEditor::luaRemoveInstanceObserver(CLuaState &ls) {
  ...
  IInstanceObserver *observer =
      getEditor().getInstanceObserver((TInstanceObserverHandle)ls.toInteger(2));
  if (observer == NULL) {
    CLuaIHM::fails(ls, "Instance observer not found for handle = %" NL_I64 "d",
                   ls.toInteger(2));
  }
  CInstanceObserverLua *luaObserver =
      dynamic_cast<CInstanceObserverLua *>(observer);
  if (luaObserver == NULL) {
    CLuaIHM::fails(ls,
                   "Instance observer found for handle %" NL_I64
                   "d, but has bad type, it wasn't registered from lua.",
                   ls.toInteger(2));
  }
...
bool CEditor::isInstanceObserver(IInstanceObserver *observer) const {
  ...
  for (TInstanceObserverMap::const_iterator it = _InstanceObservers.begin();
       it != _InstanceObservers.end(); ++it) {
...
void CEditor::registerLuaFunc() {
  ...
  registerEnvMethod("getVisualPropertiesFromInstanceId",
                    luaGetVisualPropertiesFromInstanceId);
  ...
  registerEnvMethod("getStartingAnimationFilename",
                    luaGetStartingAnimationFilename);
...
void CEditor::loadKeySet(const std::string &keySet) {
  ...
  std::string userKeyFileName =
      "save/" + keySet + "_" + PlayerSelectedFileName + ".xml";
  if (CFile::fileExists(userKeyFileName) &&
      CFile::getFileSize(userKeyFileName) > 0) {
  ...
  // Load the default key (but don't replace existings bounds, see keys.xml
  // "key_def_no_replace")
  xmlFilesToParse.push_back(keySet + ".xml");

  if (!CInterfaceManager::getInstance()->parseInterface(xmlFilesToParse,
                                                        true)) {
...
void CEditor::saveCurrentKeySet() {
  ...
  std::string filename =
      "save/" + prefix + "_" + PlayerSelectedFileName + ".xml";
...
bool CEditor::isDMing() const {
  ...
  if (getMode() == AnimationModePlay)
    return false; // masterless mode -> not a dm
  return getMode() == DMMode || getMode() == AnimationModeDm ||
         (getMode() != NotInitialized && getAccessMode() == AccessDM);
}
...
void CEditor::setMode(TMode mode) {
  switch (mode) {
  case NotInitialized:
    nlwarning("*R2* Setting mode to NotInitialized");
    break;
  case EditionMode:
    nlwarning("*R2* Setting mode to EditionMode");
    break;
  case GoingToDMMode:
    nlwarning("*R2* Setting mode to GoingToDMMode");
    break;
  case GoingToEditionMode:
    nlwarning("*R2* Setting mode to GoingToEditionMode");
    break;
  case TestMode:
    nlwarning("*R2* Setting mode to TestMode");
    break;
  case DMMode:
    nlwarning("*R2* Setting mode to DMMode");
    break;
  case AnimationModeLoading:
    nlwarning("*R2* Setting mode to AnimationModeLoading");
    break;
  case AnimationModeWaitingForLoading:
    nlwarning("*R2* Setting mode to AnimationModeWaitingForLoading");
    break;
  case AnimationModeDm:
    nlwarning("*R2* Setting mode to AnimationModeDm");
    break;
  case AnimationModePlay:
    nlwarning("*R2* Setting mode to AnimationModePlay");
    break;
  case AnimationModeGoingToDm:
    nlwarning("*R2* Setting mode to AnimationModeGoingToDm");
    break;
  case AnimationModeGoingToPlay:
    nlwarning("*R2* Setting mode to AnimationModeGoingToPlay");
    break;
  default:
  ...
  if (mode == _Mode && !_ForceDesktopReset[mode])
    return;
  ...
  if (mode != _Mode) {
    ...
    if (_Mode == EditionMode || _Mode == AnimationModeLoading) {
      ...
      removeAllEntitySlots(); // when leaving test or dm, remove the entities
                              // slots
    }
    ...
    if (mode == DMMode || mode == AnimationModeDm) {
      ...
      getEditor().getDMC().getEditionModule().askUpdateCharMode(
          TCharMode::Editer);
  ...
  if ((!IngameDbMngr.initInProgress()) &&
      UserEntity) // prevent from calling selection() if coming from a FarTP,
                  // otherwise the server will mess up the action counter
                  // because it can't know our datasetrow so early
                  // (initInProgress(): heuristic to ensure the setTarget() will
                  // work on the EGS)
  {
  ...
  CWidgetManager::getInstance()->setCaptureKeyboard(NULL);
  // Season is now unknown, until server force it (in test mode), or first set
  // act set it (in edit mode)
  _Season = UnknownSeason;
  //
  switch (_Mode)
  ...
  case EditionMode:
    ...
    /**
     * nb we manage mouse bitmap ourselves, so there's only one cursor context
     * here, and it never changes: its sole purpose is to route messages to the
     * current tool
     */
    ContextCur.add(false, "STAND BY", DEFAULT_CURSOR, 0.0f,
                   CEditor::checkCursor, CEditor::mouseClick);
  ...
  case AnimationModeDm:
    PeopleInterraction
        .updateAllFreeTellerHeaders(); // if invitation were received during
                                       // loading, add 'invite' button to the
                                       // new windows
    nlassert(CDisplayerBase::ObjCount == 0);
    ActionsContext.setContext("r2ed_anim_dm");
    // set new mode in lua
  ...
  case AnimationModePlay: {
    ...
    hideRingWindows();
    // setUIMode(1); // don't change desktop here, because virtual desktops are
    ...
    if (getDMC().getEditionModule().isSessionOwner()) {
      // if we are not a player, pop the ui to pop control player window & to
      // quit
  ...
  // display the bars over the player (not available during edition, but
...
void CEditor::hideRingWindows() {
  ...
  static const char *ringWindows[] = {"r2ed_palette",
                                      "r2ed_connect",
...
void CEditor::init(TMode initialMode, TAccessMode accessMode) {
  ...
  for (uint k = 0; k < ClientCfg.R2EDReloadFiles.size(); ++k) {
    CFile::removeFileChangeCallback(
        ClientCfg.R2EDReloadFiles[k]); // avoid to add twice when a reset ...
    CFile::addFileChangeCallback(ClientCfg.R2EDReloadFiles[k],
  ...
  _Env.setValue(
      "InClient",
      true); // TMP : signal to the script that it is initialised by the client
  //
  switch (accessMode) {
  case AccessEditor:
    _Env.setValue("AccessMode", "Editor");
    break;
  case AccessDM:
    _Env.setValue("AccessMode", "DM");
    break;
  case AccessOutlandOwner:
    _Env.setValue("AccessMode", "OutlandOwner");
    break;
  default:
    nlassert(0);
    break;
  ...
  //
  initObjectProjectionMetatable(); // init system to access to scenary objects
                                   // from lua
  // init client/server stuffs

  CConfigVarBase::getConfigFileTimeStamp()++; // invalidate all var taken from
                                              // the config file
  ...
  if (_SerializeUIConfig) {
    ...
    bool configLoaded = false;
    {
      ...
      // if access mode is DM, then load the whole ui (else it has been
      // previuously loaded)
    ...
    // if not found then reset all virtual desktops (one for each mode of the
    // editor)
    if (!configLoaded) {
      nlwarning("No interface config found, resetting editor windows to their "
                "default");
...
uint CEditor::getMaxNumPlotItems() {
  ...
  uint ret;
  fromString(CWidgetManager::getInstance()->getParser()->getDefine(
                 "r2ed_max_num_plot_item_sheets"),
             ret);
  return ret;
}
...
CCDBNodeLeaf *CEditor::getRefPlotItemSheetDBLeaf(uint index) {
  ...
  return NLGUI::CDBManager::getInstance()->getDbProp(
      toString("LOCAL:R2:REFERENCE_PLOT_ITEMS:%d:SHEET", (int)index), false);
}
...
CCDBNodeLeaf *CEditor::getPlotItemSheetDBLeaf(uint index) {
  ...
  return NLGUI::CDBManager::getInstance()->getDbProp(
      toString("LOCAL:R2:PLOT_ITEMS:%d:SHEET", (int)index), false);
}
...
void CEditor::setReferencePlotItemSheet(uint index, uint32 sheetId) {
  ...
  leaf = NLGUI::CDBManager::getInstance()->getDbProp(
      toString("LOCAL:R2:AVAILABLE_PLOT_ITEMS:%d:SHEET", (int)index), false);
  if (leaf) {
    leaf->setValue32(sheetId);
  }
}
...
void CEditor::initReferencePlotItems() {
  // H_AUTO(R2_CEditor_initReferencePlotItems)
  {
    ...
    for (CSheetManager::TEntitySheetMap::const_iterator it = sheets.begin();
         it != sheets.end(); ++it) {
      if (it->second.EntitySheet &&
          it->second.EntitySheet->Type == CEntitySheet::ITEM) {
...
void CEditor::saveUIConfig() {
  ...
  std::string filename =
      "save/" + getUIPrefix(_Mode) + PlayerSelectedFileName + ".icfg";
...
// *********************************************************************************************************
void CEditor::showPrimRender(CPrimRender &dest, const NLMISC::CAABBox &localBox,
                             const NLMISC::CMatrix &worldMat,
                             const CDecalAnim &refDecalAnim) {
  ...
  float animRatio =
      refDecalAnim.DurationInMs == 0
          ? 0.f
          : (T1 % refDecalAnim.DurationInMs) / (float)refDecalAnim.DurationInMs;
  animRatio = 0.5f * cosf(2.f * (float)Pi * animRatio) + 0.5f;
  CRGBA color =
      blend(refDecalAnim.StartDiffuse, refDecalAnim.EndDiffuse, animRatio);
  CPrimLook pl = dest.getLook();
  pl.EdgeLook.DecalColor = pl.VertexLook.DecalColor =
      pl.FirstVertexLook.DecalColor = color;
  dest.setLook(pl);
  dest.addDecalsToRenderList();
  static volatile bool showPoly = true;
  if (showPoly) {
    CLandscapePolyDrawer::getInstance().addPoly(
        poly2D, dest.getLook().EdgeLook.WorldMapColor, bbox);
  }
}

// *********************************************************************************************************
void CEditor::showSelectBox(const NLMISC::CAABBox &localBox,
                            const NLMISC::CMatrix &worldMat) {
...
// *********************************************************************************************************
void CEditor::showHighlightBox(const NLMISC::CAABBox &localBox,
                               const NLMISC::CMatrix &worldMat) {
...
// *********************************************************************************************************
void CEditor::showDecal(const NLMISC::CVector2f &pos, float scale,
                        CDecal &decal, const CDecalAnim &decalAnim) {
  // H_AUTO(R2_CEditor_showDecal)
  CHECK_EDITOR
  float animRatio = decalAnim.DurationInMs == 0
                        ? 0.f
                        : ((T1 - _DecalRefTime) % decalAnim.DurationInMs) /
                              (float)decalAnim.DurationInMs;
  animRatio = 0.5f * cosf(2.f * (float)Pi * animRatio) + 0.5f;
...
void CEditor::updateSelectingDecals() {
  ...
  CHECK_EDITOR
  if (_SelectingDecalAnim.DurationInMs == 0)
    return;
  for (uint k = 0; k < _SelectingDecals.size(); ++k) {
    ...
    if (decal) {
      if (decal->EndDate > T1) {
        float animRatio =
            (float)(T1 - (decal->EndDate - _SelectingDecalAnim.DurationInMs)) /
            (float)_SelectingDecalAnim.DurationInMs;
        _SelectingDecalAnim.updateDecal(decal->Pos, animRatio, decal->Decal,
                                        decal->Scale);
...
void CEditor::reset() {
  ...
  if (_Mode == EditionMode) {
    R2::getEditor().getLua().executeScriptNoThrow(
        "r2.Version.save(\"save/r2_buffer.dat\")");
  }
  release();
  clearDebugWindow();
  if (ReloadUIFlag) {
    loadLanguageFile();
    reloadUI();
    setForceDesktopReset(true); // next changes of desktop should force a reset,
                                // since ui has been reloaded
    ReloadUIFlag = false;
  }
...
void CEditor::reloadUI(const char *filename) {
  ...
  if (fileDate > lastUIModificationDate ||
      !ClientCfg
           .R2EDDontReparseUnchangedUIFiles) // TMP (init not complete else ...)
  {
    // reload the ui files
  ...
  struct CProjector {
    ...
    static int index(CLuaState &ls) {
      ...
      if (!checkTag(ls))
        return false;
#endif
      CObjectTable::TRefPtrConst &obj =
          *(CObjectTable::TRefPtrConst *)ls.toUserData(1);
      ...
      // NLMISC::operator==(obj, NULL))
      {
        ...
        throw ELuaWrappedFunctionException(
            &ls, "Attempt to access an erased object");
      }
      ...
      if (!ls.isString(2)) {
        nlwarning("String expected when accessing an object property, %s found "
                  "instead",
                  ls.getTypename(2));
      ...
      const char *strId = ls.toString(2); // strings are shared in lua, so a
                                          // pointer comparison is sufficient

      // special members
      // NB nico : this was added before the R2::CInstance class derived from
      // CReflectable,
      // -> see if some of the following properties may better  fit into
      // R2::CInstance exported properties

      if (OPERATOR_EQUAL(lstr_isNil, strId)) {
        ls.push(false);
        return 1;
      } else if (OPERATOR_EQUAL(strId, lstr_Parent)) {
        if (!obj->getParent()) {
          ls.pushNil();
          return 1;
        }
      ...
      if (OPERATOR_EQUAL(strId, lstr_IndexInParent)) {
        ...
        return 1;
      } else if (OPERATOR_EQUAL(strId, lstr_DisplayerUI)) {
      ...
      } else if (OPERATOR_EQUAL(strId, lstr_DisplayerVisual)) {
        CInstance *instance = getEditor().getInstanceFromObject(obj);
        if (!instance) {
          ls.pushNil();
          return 1;
      ...
      } else if (OPERATOR_EQUAL(strId, lstr_DisplayerProperties)) {
        CInstance *instance = getEditor().getInstanceFromObject(obj);
        if (!instance) {
          ls.pushNil();
          return 1;
      ...
      // NOTE : calling a method on an editor object in the script can be done
      // by doing r2:getClass(instance).methodName(instance, parameters ...)
      // with this trick, one can do instance:methodName(parameters ...)
      CObject *className = obj->findAttr("Class");
      if (className) {
        CLuaObject method =
            getEditor().getClasses()[className->toString()][index];
      ...
      // Is is ok to test thos properties after those defined in lua classes
      // definition because the init check that no "native" property is declared
      // by the user

      CInstance *instance = getEditor().getInstanceFromObject(obj);
      if (instance) {
        const CReflectedProperty *prop =
            instance->getReflectedProperty(index, false);
        if (prop) {
    ...
    static int newIndex(CLuaState &ls) {
      ...
      if (obj == NULL) {
        throw ELuaWrappedFunctionException(
            &ls, "Trying to set a property in an erased object, returning nil");
      }
#ifdef NL_DEBUG
      if (!checkTag(ls))
        return false;
      ...
      if (instance) {
        const CReflectedProperty *prop =
            instance->getReflectedProperty(ls.toString(2));
        if (prop) {
      ...
      throw ELuaWrappedFunctionException(
          &ls,
          "Property %s of editor object is read-only. You must use "
          "'r2.requestSetNode' function to change it",
          ls.toString(2));
    }
    static int gc(CLuaState &ls) {
#ifdef NL_DEBUG
    ...
    static int next(CLuaState &ls) {
      ...
      if (obj == NULL) {
        throw ELuaWrappedFunctionException(
            &ls, "editor object '__next' metatmethod : Attempt to access an "
                 "erased object");
      }
      if (ls.isNil(2)) {
        ...
        if (obj->getSize() == 0) {
          ...
          // look for duplicated keys (ignoring empty keys, because in this case
          // th index is the key)
          ...
          for (uint k = 0; k < obj->getSize(); ++k) {
            ...
            if (!key.empty()) {
              if (keys.count(key)) {
                nlwarning("Duplicated key of type string found while "
                          "attempting to enumerate an instance content.");
    ...
    static int equal(CLuaState &ls) {
      ...
      if (!checkTag(ls))
        return false;
      CObjectTable::TRefPtrConst &lhs =
          *(CObjectTable::TRefPtrConst *)ls.toUserData(1);
      CObjectTable::TRefPtrConst &rhs =
          *(CObjectTable::TRefPtrConst *)ls.toUserData(2);
...
// *********************************************************************************************************
void CEditor::getLuaUserTableFromObject(CLuaState &ls,
                                        const CObjectTable &table) {
...
void CEditor::setCurrentAct(CInstance *act) {
  ...
  if (_ClearingContent)
    return;
  bool currentActSelected =
      _SelectedInstance && (_SelectedInstance == _CurrentAct);
  CInstance *previousAct = _CurrentAct;
  _CurrentAct = act ? act : getBaseAct();
  //
  struct CPreActChangedVisitor : public IInstanceVisitor {
    virtual void visit(CInstance &inst) { inst.onPreActChanged(); }
  };
  ...
  struct CActChangedVisitor : public IInstanceVisitor {
    virtual void visit(CInstance &inst) { inst.onActChanged(); }
  };
...
void CEditor::projectInLua(const CObjectTable *table) {
  ...
  CHECK_EDITOR
  nlassert(table);
  const CObjectTableClient *otc =
      NLMISC::safe_cast<const CObjectTableClient *>(table);
...
void CEditor::backupRequestCommands() {
  ...
  nlassert(!_OldLuaRequestInsertNode
  ...
  struct CIgnoreRequestCall {
    static int requestSomething(CLuaState &) {
      ...
      /*
      CLuaIHM::debugInfo("Can't call a 'r2.request' command while object are
      erased in displayers, or are just being created (in this case, use
      onPostCreate instead) !"); CLuaIHM::debugInfo("Callstack is :");
  ...
  getEnv().setValue("requestInsertGhostNode",
                    CIgnoreRequestCall::requestSomething);
...
void CEditor::restoreRequestCommands() {
  ...
  nlassert(
      _OldLuaRequestInsertNode.isValid()); // backupRequestCommands not called ?
  nlassert(_OldLuaRequestInsertGhostNode.isValid());
  nlassert(_OldLuaRequestSetNode.isValid());
  nlassert(_OldLuaRequestEraseNode.isValid());
  nlassert(_OldLuaRequestMoveNode.isValid());
...
void CEditor::clearContent() {
  ...
  delete _NewScenario; // will be not NULL only if quit is called during the
                       // scenario connection screen
  _NewScenario = NULL;
  ...
  _DMC->CDynamicMapClient::scenarioUpdated(NULL, false,
                                           1); // clear scenario for real
  _ClearingContent = false;
...
const TMissionItem *CEditor::getPlotItemInfos(uint32 sheetId) const {
  ...
  std::map<uint32, TMissionItem>::const_iterator it =
      _PlotItemInfos.find(sheetId);
  if (it == _PlotItemInfos.end())
    return NULL;
  return &it->second;
}
...
void CEditor::release() {
  ...
  CWidgetManager::getInstance()
      ->hideAllWindows(); // make sure all action handlers are called while the
                          // r2 lua environment is still active
  clearContent();
...
void CEditor::forceSetSelectedInstance(CInstance *inst) {
  ...
  CHECK_EDITOR
  if (_EnteredInSetSelectedInstance)
    return; // prevent recursive call (may happen because selection highlight an
            // item in a menu, which trigger selection in turn)
...
bool CEditor::hasInstance(const CInstance *instance) const {
  ...
  for (TInstanceMap::const_iterator it = _Instances.begin();
...
// *********************************************************************************************************
sint CEditor::getGeneratedNameIndex(const std::string &nameUtf8,
                                    const std::string &baseNameUtf8) {
...
ucstring CEditor::genInstanceName(const ucstring &baseName) {
  ...
  for (TInstanceMap::const_iterator it = _Instances.begin();
       it != _Instances.end(); ++it) {
    maxIndex = (uint)std::max(
        (sint)maxIndex,
        getGeneratedNameIndex(it->second->getName(), baseNameUtf8));
  }

  nlinfo("CEditor::genInstanceName newVersion");

  // Old part : used when there was a delay between net command and effect
  // did not work in some parts if genInstance is not followed by creation of an
  // instance with that name cf RT 11560
  /*
  // now, look in local generated names (instance that have not been already
  added to the scene) TGeneratedNameMap::iterator nameSetIt =
  _LocalGeneratedNames.find(baseNameUtf8); if (nameSetIt !=
  _LocalGeneratedNames.end())
  {
          if (!nameSetIt->second.empty())
          {
                  maxIndex = std::max(maxIndex, *(nameSetIt->second.rbegin()));
  // take bigger element of the set (the last element)
          }
  }
...
// *********************************************************************************************************
void CEditor::setCookie(const TInstanceId &instanceId, const std::string &key,
                        CLuaObject &value) {
  // H_AUTO(R2_CEditor_setCookie)
  CHECK_EDITOR
  TCookieList &cl = _Cookies[instanceId];
  cl.push_front(CCookie());
  CCookie &cookie = cl.front(); // don't build struct and push to avoid costly
                                // copy of a CLuaObject
  cookie.Key = key;
  cookie.Value = value;
}
...
// *********************************************************************************************************
bool CEditor::callEnvFunc(const char *funcName, int numArgs,
                          int numRet /*=0*/) {
  ...
  if (dumpStackWanted)
    getLua().dumpStack();
  if (result != 0) {
    nlwarning("Error while calling function %s : %s", funcName,
              getLua().toString());
  ...
  if (dumpStackWanted)
    getLua().dumpStack();
  nlassert(newSize == initialStackSize + 1 - numArgs +
                          numRet); // -1 is because of the r2 table
  if (dumpStackWanted)
    getLua().dumpStack();
...
// *********************************************************************************************************
bool CEditor::callEnvMethod(const char *funcName, int numArgs,
                            int numRet /*= 0*/) {
  ...
  getEnv().push();
  getLua().insert(-1 -
                  numArgs); // put the table before the args (self parameter)
...
bool CEditor::doLuaScript(const char *filename, const char *fileDescText) {
  ...
  if (0 && FINAL_VERSION == 1) // disabled for the moment because there are lua
                               // file that must be loaded from example
  {
    ...
    if (filePath.size() < len || filePath.substr(0, len) != path) {
      nlwarning("Can't find %s : %s in ('%s')", fileDescText, filename,
                path.c_str());
      return false;
    }
  }
  ...
  try {
    if (!getLua().executeFile(filePath)) {
      nlwarning("Couldn't open file %s : %s for R2 is not loaded", filename,
                fileDescText);
    }
    CLuaStackChecker ls(&getLua());
    return true;
  } catch (const NLMISC::EStream &e) {
    nlwarning("Error while loading R2 %s (file = %s) : %s", fileDescText,
              filename, e.what());
  } catch (const ELuaError &e) {
    // char filename[MAX_PATH];
    std::string msg = e.what();
    if (testWildCard(msg, "*.lua:*:*")) // TODO nico : more accurate testing for
                                        // filename format
    {
      std::string::size_type extPos = msg.find(".lua");
      nlassert(extPos != std::string::npos);
      std::string filename =
          msg.substr(0, extPos + 4); // extract filename including extension
      int line;
      fromString(&*(msg.begin() + extPos + 5), line); // line number follows
      nlwarning((CLuaIHMRyzom::createGotoFileButtonTag(filename.c_str(), line) +
                 e.what())
                    .c_str());
    } else {
      // no filename / line in the result, so maybe the parse error was in an
      // 'inline' script
      nlwarning("Error in R2 %s (file = %s) : %s", fileDescText, filename,
                e.what());
    }
  }
  return false;
...
void CEditor::updatePreCamera() {
  ...
  if (_Mode == EditionMode) {
    ...
    {
      if ((CTime::getLocalTime() - _LastAutoSaveTime) / 1000 >
          ClientCfg.R2EDAutoSaveWait) // 5 minutes if not  change in Confile
      {
        autoSave();
      }
      loop = 0;
...
void CEditor::updatePrimitiveContextualVisibility() {
  ...
  if (!_SelectedInstance)
    return;
  // if selected instance remains in last contextual prim list, then they may
  // remain visible
  bool ok = false;
  ...
  if (seq) {
    ...
    if (activities) {
      ...
      for (uint k = 0; k < activities->getSize(); ++k) {
        ...
        if (primitive) {
          CDisplayerVisualGroup *primDisp =
              dynamic_cast<CDisplayerVisualGroup *>(
                  primitive->getDisplayerVisual());
          if (primDisp) {
...
// *********************************************************************************************************
CEntitySorter *CEditor::getEntitySorter() const { return _EntitySorter; }
...
void CEditor::updateBeforeRender() {
  ...
  for (TInstanceMap::iterator it = _Instances.begin(); it != _Instances.end();
       ++it) {
  ...
  if (UserEntity) {
    if (_Mode == EditionMode || isDMing()) {
      ...
      showDecal(
          CVector2f((float)UserEntity->pos().x, (float)UserEntity->pos().y),
          1.f, _PionneerDecal, _PionneerDecalAnim);
    }
  }
...
// *********************************************************************************************************
void CEditor::updateDecalBlendRegion(CDecal &decal,
  ...
  decal.setTopBlend(pos.z + topBlendDist, pos.z + topBlendDist + blendLength);
}
...
void CEditor::updateAfterRender() {
  if (_EntitySorter)
    _EntitySorter->clipEntitiesByDist();
...
bool CEditor::handleEvent(const NLGUI::CEventDescriptor &eventDesc) {
  ...
  CHECK_EDITOR
  if (ConnectionWanted || !_CurrentTool)
    return false; // TMP special case for connection
  if (eventDesc.getType() == NLGUI::CEventDescriptor::system) {
    const NLGUI::CEventDescriptorSystem &eds =
        (const NLGUI::CEventDescriptorSystem &)eventDesc;
    if (eds.getEventTypeExtended() == NLGUI::CEventDescriptorSystem::setfocus) {
      const NLGUI::CEventDescriptorSetFocus &edsf =
          (const NLGUI::CEventDescriptorSetFocus &)eds;
...
// *********************************************************************************************************
CEntityCL *CEditor::createEntity(uint slot, const NLMISC::CSheetId &sheetId,
                                 const NLMISC::CVector &pos, float heading,
                                 const std::string &permanentStatutIcon) {
  ...
  CEntityCL *entity =
      EntitiesMngr.create(slot, sheetId.asInt(), emptyEntityInfo);
  if (!entity) {
  ...
  node = NLGUI::CDBManager::getInstance()->getDbProp(
      "SERVER:Entities:E" + NLMISC::toString("%d", slot) + ":P" +
          NLMISC::toString("%d", CLFECOMMON::PROPERTY_POSX),
      false);
  if (node) {
    ...
    node->setValue64(x);
    node = NLGUI::CDBManager::getInstance()->getDbProp(
        "SERVER:Entities:E" + toString("%d", slot) + ":P" +
            toString("%d", CLFECOMMON::PROPERTY_POSY),
        false);
    if (node) {
      node->setValue64(y);
      node = NLGUI::CDBManager::getInstance()->getDbProp(
          "SERVER:Entities:E" + toString("%d", slot) + ":P" +
              toString("%d", CLFECOMMON::PROPERTY_POSZ),
          false);
      if (node)
        node->setValue64(z);
    }
  ...
  node = NLGUI::CDBManager::getInstance()->getDbProp(
      "SERVER:Entities:E" + toString("%d", slot) + ":P" +
          toString("%d", CLFECOMMON::PROPERTY_ORIENTATION),
      false);
  if (node) {
    C64BitsParts parts;
    parts.f[0] = heading;
  ...
  node = NLGUI::CDBManager::getInstance()->getDbProp(
      "SERVER:Entities:E" + toString("%d", slot) + ":P" +
          toString("%d", CLFECOMMON::PROPERTY_MODE),
      false);
  if (node) {
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:Entities:E" + toString("%d", slot) + ":P" +
...
void CEditor::displayContextMenu() {
  ...
  if (menu.empty())
    return; // no menu for that instance ?
  //
...
// *********************************************************************************************************
void CEditor::triggerInstanceObserver(const TInstanceId &id,
                                      IObserverAction &action) {
  ...
  if (lb == ub)
    return;
  // must do a copy, because an observer may erase himself from the list when it
  // is triggered
  ...
  for (std::vector<IInstanceObserver::TRefPtr>::iterator it = dest.begin();
       it != dest.end(); ++it) {
    if (*it)
      action.doAction(**it);
  }
...
// *********************************************************************************************************
void CEditor::onErase(CObject *root, bool &foundInBase,
  ...
  // rather a change of attribute to the "default value' (may happen in an undo
  // operation)
  ...
  if (parent) {
    ...
    if (sonIndex != -1) {
      ...
      if (!nameInParent.empty() &&
          getDMC().getPropertyAccessor().hasValueInBase(parent, nameInParent)) {
        foundInBase = true;
      }
    }
  }
  ...
  if (root->isTable()) {
    // special patch: ref ids under this object should not be triggered any more
    // for that object
  ...
  if (!_ClearingContent) {
    if (inst) {
      ...
      // nlwarning("Instance with id %s deleted, but not inserted",
      // inst->getId().c_str());
    }
  }
  // really remove object
  // nlwarning("Removing instance with id %s (table = 0x%s)",
  // inst->getId().c_str(), (int) root);
  if (inst) {
...
// *********************************************************************************************************
void CEditor::nodeErased(const std::string &instanceId,
                         const std::string &attrName, sint32 position) {
  ...
  if (foundInBase) {
    ...
    if (parentInstance) {
      ...
      nlwarning("Can't found instance in which %s was modified",
                nameInParent.c_str());
    }
  }
  // warn the parent that it has been modified
  onAttrModified(parent);
  // NB : msg for deleted attribute does not exist yet, so the parent is warned
  // that it is modified, but 'onTableModified' is not called (because key
...
void CEditor::onEditionModeConnected(uint32 userSlotId, uint32 adventureId,
                                     CObject *highLevel,
                                     const std::string &versionName,
                                     bool willTP, uint32 initialActIndex) {
  ...
  CInterfaceGroup *currentSessionGroup = dynamic_cast<CInterfaceGroup *>(
      CWidgetManager::getInstance()->getElementFromId(
          "ui:interface:r2ed_current_session"));
  if (currentSessionGroup) {
    CViewText *text = dynamic_cast<CViewText *>(
        currentSessionGroup->getView("current_session"));
    if (text) {
      text->setText(toString("Edition Session = %d (%s)", adventureId,
                             versionName.c_str()));
    }
  }
  setDefaultChatWindow(PeopleInterraction.ChatGroup.Window);
  _DMC->CDynamicMapClient::onEditionModeConnected(
      userSlotId, adventureId, highLevel, versionName, willTP, initialActIndex);
}
...
void CEditor::onAnimationModeConnected(
    const CClientMessageAdventureUserConnection &connected) {
  ...
  case 0:
    setMode(AnimationModeLoading);
    break;
  case 1:
    setMode(AnimationModeWaitingForLoading);
    break;
  case 2:
    setMode(AnimationModeDm);
    break;
  case 3:
    setMode(AnimationModePlay);
    break;
  default:
    nlwarning("Unhandled %u in Animation Session", connected.Mode);
  }
  ...
  if (currentSessionGroup) {
    CViewText *text = dynamic_cast<CViewText *>(
        currentSessionGroup->getView("current_session"));
    if (text) {
      text->setText(toString("Animation Session = %d (%s)",
                             connected.SessionId.asInt(),
                             connected.VersionName.c_str()));
    }
  }
  setDefaultChatWindow(PeopleInterraction.ChatGroup.Window);
...
void CEditor::onEditionModeDisconnected() {
  ...
  try {
    R2::getEditor().getLua().executeScript(
        "r2.Version.save(\"save/r2_buffer.dat\")");
...
void CEditor::onTestModeConnected() {
  ...
  CHECK_EDITOR
  // TODO nico : change the name of the function : should rather be
  // 'onAnimationModeConnected' start as a GM
  CAHManager::getInstance()->runActionHandler("r2ed_anim_dm_mode", NULL, "");
  _DMC->CDynamicMapClient::onTestModeConnected();
}

void CEditor::onTestModeDisconnected(TSessionId sessionId, uint32 lastAct,
                                     TScenarioSessionType sessionType) {
  // H_AUTO(R2_CEditor_onTestModeDisconnected)
  CHECK_EDITOR
  _DMC->CDynamicMapClient::onTestModeDisconnected(sessionId, lastAct,
                                                  sessionType);
}

// *********************************************************************************************************
void CEditor::nodeInserted(const std::string &instanceId,
                           const std::string &attrName, sint32 position,
                           const std::string &key, CObject *value) {
  // H_AUTO(R2_CEditor_nodeInserted)
  CHECK_EDITOR
  _DMC->CDynamicMapClient::nodeInserted(instanceId, attrName, position, key,
                                        value);
  if (value->isTable()) {
        ...
        onAttrModified(
            clonedValue
                ->getParent()); // parent table is modified
                                // so -> only parent of parent table will be
                                // notified of change maybe we need another msg
                                // "onInsert" that is sent to the parent table.
      }
    }
  }
}
  ...
  if (!id.empty()) {
    ...
    if (cookieList != _Cookies.end()) {
      if (obj->isTable()) {
        ...
        CLuaObject userTable(
            ls); // pop the table into a CLuaObject for convenience
        for (TCookieList::iterator it = cookieList->second.begin();
             it != cookieList->second.end(); ++it) {
    ...
    if (!classDesc.isNil()) {
      CLuaStackChecker lsc(&getLua());
      CDisplayerVisual *dispViz = createObjectFromClassName<CDisplayerVisual>(
          classDesc["DisplayerVisual"].toString());
      if (dispViz) {
        bool ok = dispViz->init(classDesc["DisplayerVisualParams"]);
        if (!ok) {
          nlwarning("Error when calling init on visual displayer of class %s",
                    classDesc["Name"].toString().c_str());
        }
      }
      inst->setDisplayerVisual(dispViz);
      //
      CDisplayerBase *dispUI = createObjectFromClassName<CDisplayerBase>(
          classDesc["DisplayerUI"].toString());
      if (dispUI) {
        bool ok = dispUI->init(classDesc["DisplayerUIParams"]);
        if (!ok) {
          nlwarning("Error when calling init on  ui displayer of class %s",
                    classDesc["Name"].toString().c_str());
        }
      }
      inst->setDisplayerUI(dispUI);
      //
      CDisplayerBase *dispProp = createObjectFromClassName<CDisplayerBase>(
          classDesc["DisplayerProperties"].toString());
      if (dispProp) {
        bool ok = dispProp->init(classDesc["DisplayerPropertiesParams"]);
        if (!ok) {
          nlwarning("Error when calling init on property displayer of class %s",
                    classDesc["Name"].toString().c_str());
        }
      }
    ...
    if (!className.empty() && !name.empty()) {
      for (TGeneratedNameMap::iterator it = _LocalGeneratedNames.begin();
           it != _LocalGeneratedNames.end(); ++it) {
...
// *********************************************************************************************************
void CEditor::onAttrModified(CInstance &parentInstance,
                             const std::string &attrName, sint32 indexInArray) {
  ...
  public:
    CAttrModifiedNotification(CInstance &instance, const std::string &key,
                              sint32 indexInArray)
  ...
  CAttrModifiedNotification attrModifiedNotification(parentInstance, attrName,
                                                     indexInArray);
...
void CEditor::onAttrModified(const CObject *value) {
  ...
  while (parent) {
    ...
    if (parentInstance) {
      ...
      // we are in an array in an instance -> memorize index in that array for
      // next call to "onAttrModified"...
...
// *********************************************************************************************************
void CEditor::nodeSet(const std::string &instanceId,
                      const std::string &attrName, CObject *value) {
  ...
  if (value->isTable()) {
    ...
    if (!id.empty()) {
      const CObject *clonedValue = _DMC->find(id);
      if (clonedValue) {
        createNewInstanceForObjectTable(
            clonedValue); // if the created object is a table, warn him that he
                          // has been created
...
// *********************************************************************************************************
void CEditor::scenarioUpdated(CObject *highLevel, bool willTP,
                              uint32 initialActIndex) {
  ...
  */
  ...
  if (_Scenario->getAttr("InstanceId")) {
    _ScenarioInstance =
        getInstanceFromId(_Scenario->getAttr("InstanceId")->toString());
  } else {
    _ScenarioInstance = NULL;
  ...
  onPostCreate(
      _Scenario); // post creation require that current act has been set
  ...
  if (ClientCfg.Local)
  {
          sint locationId = (uint)
  _ScenarioInstance->getLuaProjection()["Description"]["LocationId"].toInteger();

          CScenarioEntryPoints &sep = CScenarioEntryPoints::getInstance();
          _IslandCollision.loadEntryPoints();
          if (sep.getCompleteIslands().empty())
          {
                  nlwarning("Entry points not loaded, teleport not done (local
  mode)");
          }
          ...
          else
          {
                  ...
                  const CScenarioEntryPoints::CCompleteIsland &ci =
  sep.getCompleteIslands()[locationId]; CVectorD playerPos = UserEntity->pos();
                  //
                          ...
                          playerPos.y >= ci.YMax)
                  {
                          ...
                          {
                                  const CScenarioEntryPoints::CShortEntryPoint &
  shortEntryPoint = ci.EntryPoints[0]; CVector dest((float) shortEntryPoint.X,
  (float) shortEntryPoint.Y, 0.f);

                                  UserEntity->pos(dest); // change position in
  pacs
                                  // Select the closest continent from the new
  position. beginLoading (LoadingBackground); #define BAR_STEP_TP 2 // fixme :
  this define is duplicated.... ProgressBar.reset (BAR_STEP_TP); ucstring
  nmsg("Loading..."); ProgressBar.newMessage (
  ClientCfg.buildLoadingString(nmsg) ); ProgressBar.progress(0);
...
CInstance *CEditor::getDefaultFeature(CInstance *act) {
  ...
  if (!defaultFeature)
    return NULL; // 0 should be the default feature
...
// *********************************************************************************************************
void CEditor::nodeMoved(const std::string &instanceId,
                        const std::string &attrName, sint32 position,
                        const std::string &destInstanceId,
                        const std::string &destAttrName, sint32 destPosition) {
  ...
  // do the move
  _DMC->CDynamicMapClient::nodeMoved(instanceId, attrName, position,
                                     destInstanceId, destAttrName,
                                     destPosition);
  // warn the previous parent that it has been modified
  if (src->getParent() != oldParent) {
    onAttrModified(
        oldParent); // the old parent is modified so send appropriate msg
  }
  // else ... if new parent is the same then send message only at the end (else
  // object believe it has been modified twice in a row) tells object that he
  // has moved
  if (inst) {
    inst->onPostHrcMove();
...
void CEditor::dumpInstances() {
  ...
  for (TInstanceMap::iterator it = _Instances.begin(); it != _Instances.end();
       ++it) {
    nlwarning("Obj = %p, id = %s, instance = %p", it->first,
              it->second->getId().c_str(), &(*it->second));
  }
}
...
public:
  ...
  virtual float
  preciseIntersectionTest(const NLMISC::CVector &worldRayStart,
                          const NLMISC::CVector &worldRayDir) const {
    if (!UserEntity)
      return FLT_MAX;
    return CEditor::preciseEntityIntersectionTest(*UserEntity, worldRayStart,
                                                  worldRayDir);
  }
...
//
CInstance *CEditor::getInstanceUnderPos(float x, float y, float distSelection,
  ...
  //
  viewport.getRayWithPoint(x, y, worldViewRay.Origin, worldViewRay.Dir,
                           camMatrix, camFrust);
  ...
  validObjects.push_back(
      &SelectableUser); // add fake object for test with user entity
  for (TInstanceMap::iterator it = _Instances.begin(); it != _Instances.end();
       ++it) {
  ...
  // compute intersection of mouse with landscape / batiments for test with
  // projected object like region & decals
  CVector sceneInter;
  CTool::TRayIntersectionType rayInterType =
      CTool::computeLandscapeRayIntersection(worldViewRay, sceneInter);
  ...
  for (uint k = 0; k < validObjects.size(); ++k) {
    ...
    if (object->getLastClip())
      continue;
    float depth = FLT_MAX;
    ISelectableObject::TSelectionType selectionType =
        object->getSelectionType();
    ...
    case ISelectableObject::WorldSelectBox: {
      const CMatrix &rayMatrix =
          (selectionType == ISelectableObject::LocalSelectBox)
              ? object->getInvertedMatrix()
              : CMatrix::Identity;
      selectObj.RayStart = rayMatrix * worldViewRay.Origin;
      selectObj.RayStartClipped = selectObj.RayStart;
      selectObj.RayEndClipped =
          rayMatrix * (worldViewRay.Origin + worldViewRay.Dir * distSelection);
      // if intersect the bbox
      selectObj.SelectBox = object->getSelectBox();
      selectObj.SelectBoxInWorld = false;
      if (selectObj.SelectBox.clipSegment(selectObj.RayStartClipped,
                                          selectObj.RayEndClipped)) {
  ...
  // **** get best entity according to distance face-camera or box-ray if no
  // face intersection
  ...
  for (i = 0; i < intersectedObjects.size(); i++) {
    ...
    case ISelectableObject::GroundProjected: {
      ...
      bool keepSelection = objectSelected &&
                           objectSelected->getSelectionType() ==
                               ISelectableObject::GroundProjected &&
    ...
    case ISelectableObject::LocalSelectBox:
    case ISelectableObject::WorldSelectBox:
      // if (!object->intersectionTest(bbox, pos, dir, dist2D, distZ,
      // distSelection)) continue;
      distZ = object->preciseIntersectionTest(worldViewRay.Origin,
                                              worldViewRay.Dir);
      if (distZ != FLT_MAX) {
        preciseInterFound = true;
      }
    ...
    // *** if intersect face, then take the best face-intersection, else use
    // box-ray cost true face-col found?
    ...
    // else
      ...
      if (bestDistZ == FLT_MAX) {
        ...
        // take the middle of the clipped segment. suppose that this middle is
        // the "nearest ray point to center" This is false, but gives better
        // results.
        CVector m = (selectObj.RayStartClipped + selectObj.RayEndClipped) / 2;
        // get the distance to center. NB: small entities are preferred since
        // smaller mean lower cost
        float outBBoxCost = (m - c).norm();

        // the final cost is a weighted sum of the both. NB: distCamCost is in
        // meter, and outBBBoxCost is meters. Hence
        // ClientCfg.SelectionOutBBoxWeight is a factor
        float boxCost =
            distCamCost + outBBoxCost * ClientCfg.SelectionOutBBoxWeight;
  ...
  if (bestDistZ == FLT_MAX) {
    // if the last entity under pos is valid, prefer it among all other
    // approximate ones
    if (precInstanceUnderPos && precInstanceUnderPosValid)
      objectSelected = precInstanceUnderPos->getDisplayerVisual();
  }

  if (objectSelected && objectSelected->getSelectionType() ==
                            ISelectableObject::GroundProjected) {
    if (borderSelected && borderSelected != objectSelected) {
      if (!objectSelected || !objectSelected->isInProjectionBorder(
                                 CVector2f(sceneInter.x, sceneInter.y))) {
  ...
  _LastInstanceUnderPos =
      objectSelected ? objectSelected->getInstanceInEditor() : NULL;
  return _LastInstanceUnderPos;
} // getEntityUnderPos //

// *********************************************************************************************************
float CEditor::preciseEntityIntersectionTest(
    CEntityCL &entity, const NLMISC::CVector &worldRayStart,
    const NLMISC::CVector &worldRayDir) {
  ...
  if (!ClientCfg.Light) {
    if (skeleton) {
      if (skeleton->supportFastIntersect() &&
          skeleton->fastIntersect(worldRayStart, worldRayDir, dist2D, distZ,
                                  false)) {
        if (dist2D == 0.f) {
          return distZ;
        }
    ...
    else if (!entity.instances().empty() &&
             !entity.instances()[0].Current.empty()) {
      NL3D::UInstance inst = entity.instances()[0].Current;
      if (inst.supportFastIntersect()) {
        if (inst.fastIntersect(worldRayStart, worldRayDir, dist2D, distZ,
                               false)) {
          if (dist2D == 0.f) {
            return distZ;
          }
        ...
        // TMP
        // precise test was asked, but there's no better test than bbox, so
        // return the dist for bbox middle
        return (entity.pos().asVector() + entity.localSelectBox().getCenter() -
                worldRayStart)
            .norm();
      }
    }
...
const NLMISC::CAABBox &CEditor::getLocalSelectBox(CEntityCL &entity) const {
  ...
  CHECK_EDITOR
  const TEntityCustomSelectBoxMap &boxMap =
      getEditor().getEntityCustomSelectBoxMap();
  TEntityCustomSelectBoxMap::const_iterator it =
      boxMap.find(CSheetId(entity.sheetId()).toString());
  if (it != boxMap.end()) {
...
void CEditor::connectionMsg(const std::string &stringId) {
  ...
  CInterfaceGroup *r2ConnectWindow = dynamic_cast<CInterfaceGroup *>(
      CWidgetManager::getInstance()->getElementFromId(
          "ui:interface:r2ed_connect"));
  if (!r2ConnectWindow)
    return;
  if (stringId.empty()) {
    ...
    CViewText *vt =
        dynamic_cast<CViewText *>(r2ConnectWindow->getView("connexionMsg"));
    if (vt) {
  ...
  if (_Mode == EditionMode || _Mode == AnimationModeLoading) {
    ...
    if (_ScenarioInstance) {
      try {
        ...
        if (_ScenarioInstance->getLuaProjection().callMethodByNameNoThrow(
                "validateForTesting", 0, 1)) {
...
CEditor::addInstanceObserver(const TInstanceId &instanceId,
                             IInstanceObserver *observer) {
  ...
  const TInstanceObserverMap::const_iterator lb =
      _InstanceObservers.lower_bound(instanceId);
  const TInstanceObserverMap::const_iterator ub =
      _InstanceObservers.upper_bound(instanceId);
  // NB nico : removed the inserted twice stuff below because observer pointers
  // are not used as keys, so sharing is possible see if not inserted twice
  /*
  for (TInstanceObserverMap::const_iterator it = lb; it != ub; ++it)
  {
          if (it->second == observer)
          {
                  nlwarning("addInstanceObserver : Instance observer inserted
  twice"); return BadInstanceObserverHandle;
          }
  }
  */
  // insert the handle
  ...
  _InstanceObserverHandles[handle] = _InstanceObservers.insert(
      TInstanceObserverMap::value_type(instanceId, observer));
  nlassert(_InstanceObservers.size() == _InstanceObserverHandles.size());
  return handle;
}

// *********************************************************************************************************
CEditor::IInstanceObserver *
CEditor::removeInstanceObserver(TInstanceObserverHandle handle) {
  // H_AUTO(R2_CEditor_removeInstanceObserver)
  CHECK_EDITOR
  nlassert(_InstanceObservers.size() == _InstanceObserverHandles.size());
  TInstanceObserverHandleMap::iterator it =
      _InstanceObserverHandles.find(handle);
  if (it == _InstanceObserverHandles.end()) {
    nlwarning(
        "removeInstanceObserver : Instance observer handle not found : %d",
        (int)handle);
    return NULL;
  }
...
CEditor::TSeason CEditor::getSeason() const {
  ...
  if (_IsWaitingTPForSeasonChange)
    return UnknownSeason; // as long at the teleport message hasn't been
                          // received, don't change the season for nothing ->
                          // pretend that we don't know the season so that it
                          // remains unchnged
  return _Season;
}
...
void CEditor::checkMissingCollisions() {
  ...
  const CScenarioEntryPoints::TCompleteIslands &islands =
      sep.getCompleteIslands();
  for (uint k = 0; k < islands.size(); ++k) {
    bool found =
        !(CPath::lookup(islands[k].Island + ".packed_island", false, false)
              .empty());
    if (!found) {
      nlwarning("ISLAND COLLISION MISSING FOR : %s", islands[k].Island.c_str());
    }
    found = !(
        CPath::lookup(islands[k].Island + ".island_hm", false, false).empty());
    if (!found) {
...
NLMISC::CVectorD getVectorD(const CObject *obj) {
  return CVectorD(getNumber(obj, "x"), getNumber(obj, "y"),
                  getNumber(obj, "z"));
}

// *********************************************************************************************************
CObject *buildVector(const NLMISC::CVectorD &vector,
...
const CObject *getObject(const CObject *obj, const std::string &attrName) {
  if (!obj)
    return NULL;
  return getEditor().getDMC().getPropertyAccessor().getPropertyValue(obj,
                                                                     attrName);
}
...
void CEditor::onContinentChanged() {
  ...
  if (_ScenarioInstance) {
    struct CContinentChangedVisitor : public IInstanceVisitor {
      virtual void visit(CInstance &inst) { inst.onContinentChanged(); }
    };
...
//-----------------------------
bool CEditor::getVisualPropertiesFromObject(CObject *object, SPropVisualA &vA,
                                            SPropVisualB &vB,
                                            SPropVisualC &vC) {
  ...
  static const char *keys[] = {"GabaritHeight",     "GabaritTorsoWidth",
                               "GabaritArmsWidth",  "GabaritLegsWidth",
                               "GabaritBreastSize", "HairType",
                               "HairColor",         "Tattoo",
                               "EyesColor",         "MorphTarget1",
                               "MorphTarget2",      "MorphTarget3",
                               "MorphTarget4",      "MorphTarget5",
                               "MorphTarget6",      "MorphTarget7",
                               "MorphTarget8",      "JacketModel",
                               "TrouserModel",      "FeetModel",
                               "HandsModel",        "ArmModel",
                               "WeaponRightHand",   "WeaponLeftHand",
                               "JacketColor",       "ArmColor",
                               "HandsColor",        "TrouserColor",
                               "FeetColor"};
  ...
  if (entitySheet) {
    const CCharacterSheet *chSheet =
        dynamic_cast<const CCharacterSheet *>(entitySheet);
    if (chSheet) {
  ...
  if (itemNb > 0) {
    ...
    vA.PropertySubData.HatModel =
        (uint)SheetMngr.getVSIndex(itemFileName, SLOTTYPE::HEAD_SLOT);
  } else {
  ...
  if (itemNb > 0) {
    itemFileName = CSheetId(itemNb).toString();
    vA.PropertySubData.JacketModel =
        (uint)SheetMngr.getVSIndex(itemFileName, SLOTTYPE::CHEST_SLOT);
  } else {
  ...
  if (itemNb > 0) {
    itemFileName = CSheetId(itemNb).toString();
    vA.PropertySubData.TrouserModel =
        (uint)SheetMngr.getVSIndex(itemFileName, SLOTTYPE::LEGS_SLOT);
  } else {
  ...
  if (itemNb > 0) {
    itemFileName = CSheetId(itemNb).toString();
    vB.PropertySubData.FeetModel =
        (uint)SheetMngr.getVSIndex(itemFileName, SLOTTYPE::FEET_SLOT);
  } else {
  ...
  if (itemNb > 0) {
    itemFileName = CSheetId(itemNb).toString();
    vB.PropertySubData.HandsModel =
        (uint)SheetMngr.getVSIndex(itemFileName, SLOTTYPE::HANDS_SLOT);
  } else {
  ...
  if (itemNb > 0) {
    itemFileName = CSheetId(itemNb).toString();
    vA.PropertySubData.ArmModel =
        (uint)SheetMngr.getVSIndex(itemFileName, SLOTTYPE::ARMS_SLOT);
  } else {
  ...
  if (itemNb > 0) {
    itemFileName = CSheetId(itemNb).toString();
    vA.PropertySubData.WeaponRightHand =
        (uint)SheetMngr.getVSIndex(itemFileName, SLOTTYPE::RIGHT_HAND_SLOT);
  } else {
  ...
  if (itemNb > 0) {
    itemFileName = CSheetId(itemNb).toString();
    vA.PropertySubData.WeaponLeftHand =
        (uint)SheetMngr.getVSIndex(itemFileName, SLOTTYPE::LEFT_HAND_SLOT);
  } else {
...
bool CEditor::CSortedInstances::contains(CInstance *inst) const {
  ...
  if (it == _InstanceToIter.end()) {
    for (TInstanceByName::const_iterator it = _ByName.begin();
         it != _ByName.end(); ++it) {
...
bool CEditor::isRegisteredByDispName(CInstance *inst) const {
  ...
  for (uint k = 0; k < _InstancesByDispName.size(); ++k) {
    if (_InstancesByDispName[k].contains(inst))
...
// *********************************************************************************************************
void CEditor::registerInstanceDispName(const ucstring &displayName,
                                       CInstance *inst) {
  nlassert(inst);
  sint currClass = inst->getClassIndex();
  if (currClass < 0) {
    nlwarning("Classindex not found for class %s",
...
class CAHCreateEntity : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const std::string &sParams) {
    ...
    if (!paletteNode->isTable()) {
      nlwarning("Bad type for palette node %s (should be a table)",
                paletteId.c_str());
      return;
    }
    ...
    getEditor().setCurrentTool(
        NULL); // remove current to avoid to have ghost removed by that tool if
               // it was a "CToolCreateEntity" too.
    uint ghostSlot = 1; // TMP TMP
    CEntityCL *entity = NULL;
    if (!(entity =
              CEditor::createEntity(ghostSlot, sheetId, CVector::Null, 0.f))) {
      return;
    }
    ...
    if (dynamic_cast<CPlayerR2CL *>(entity)) {
      ...
      if (getEditor().getEnv().callMethodByNameNoThrow("randomNPCProperties", 2,
                                                       1)) {
        CLuaObject result(getEditor().getLua());
        std::map<std::string, sint64> visualProps;
        ...
        // vA.PropertySubData.Sex = (uint) visualProps["Sex"];
        const CEntitySheet *entitySheet =
            SheetMngr.get((CSheetId)sheetId.asInt());
        if (entitySheet) {
          const CCharacterSheet *chSheet =
              dynamic_cast<const CCharacterSheet *>(entitySheet);
          if (chSheet) {
        ...
        if (itemNb > 0) {
          itemFileName = CSheetId(itemNb).toString();
          vA.PropertySubData.WeaponRightHand = (uint)SheetMngr.getVSIndex(
              itemFileName, SLOTTYPE::RIGHT_HAND_SLOT);
        } else {
          vA.PropertySubData.WeaponRightHand = 0;
        }
        ...
        if (itemNb > 0) {
          itemFileName = CSheetId(itemNb).toString();
          vA.PropertySubData.WeaponLeftHand = (uint)SheetMngr.getVSIndex(
              itemFileName, SLOTTYPE::LEFT_HAND_SLOT);
        } else {
          vA.PropertySubData.WeaponLeftHand = 0;
        }
        ...
        prop = (sint64 *)&vA;
        NLGUI::CDBManager::getInstance()
            ->getDbProp("SERVER:Entities:E" + toString("%d", entity->slot()) +
        ...
        EntitiesMngr.updateVisualProperty(0, entity->slot(),
    ...
    getEditor().setCurrentTool(
        new CToolCreateEntity(ghostSlot, paletteId,
                              NLGUI::CDBManager::getInstance()
                                  ->getDbProp("UI:TEMP:R2_DRAW_ARRAY")
                                  ->getValueBool()));
  }
};
...
sint CEditor::classToIndex(const std::string &className) const {
  CHashMap<std::string, uint>::const_iterator it =
      _ClassNameToIndex.find(className);
  if (it == _ClassNameToIndex.end())
    return -1;
  return it->second;
}
...
bool CEditor::isKindOf(sint testedClassIndex, sint kindClassIndex) const {
  if (testedClassIndex < 0 || kindClassIndex < 0)
    return false;
...
sint CEditor::getBaseClass(sint derivedClass) const {
  if (derivedClass < 0 || derivedClass >= (sint)_BaseClassIndices.size())
    return -1;
...
void CEditor::setMaxVisibleEntityExceededFlag(bool on) {
  if (on == _MaxVisibleEntityExceededFlag)
    return;
...
class CAHStopTest : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */,
                       const std::string & /* sParams */) {
    CHECK_EDITOR
    if (getEditor().getAccessMode() != CEditor::AccessEditor)
      return;
    ConnectionWanted = true;
  }
};
...
class CAHOpenScenarioControl : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const std::string &sParams) {
    ...
    if (!R2::getEditor().isInitialized())
      wnd = dynamic_cast<CInterfaceGroup *>(
          CWidgetManager::getInstance()->getElementFromId(
              "ui:interface:ring_scenario_loading_window"));
    else
      wnd = dynamic_cast<CInterfaceGroup *>(
          CWidgetManager::getInstance()->getElementFromId(
              "ui:interface:r2ed_scenario_control"));
...
class CAHR2StopLive : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const std::string &sParams) {
    ...
    if (!confirmStopLive) {
      CInterfaceManager *pIM = CInterfaceManager::getInstance();
      pIM->validMessageBox(CInterfaceManager::QuestionIconMsg,
                           CI18N::get("uiR2EDconfirmStopLive"), "r2_stop_live",
                           "confirmStopLive=1", "", "", "ui:interface");
    } else {
      CSessionBrowserImpl &sb = CSessionBrowserImpl::getInstance();
      sb.closeSession(
          sb.getCharId(),
      ...
      if (sb.waitOneMessage(sb.getMessageName("on_invokeResult"))) {
        if (R2::getEditor().getDMC().getEditionModule().getEditSessionLink() !=
            0) {
          // Now we expect to receive an impulsion FAR_TP to mainland, triggered
          // by the DSS. We'll disobey it by FarTPing to the Edition session
          // instead!
          nldebug("Will return to editing session %u", R2::getEditor()
                                                           .getDMC()
                                                           .getEditionModule()
                                                           .getEditSessionLink()
                                                           .asInt());
        ...
        // otherwise, let accomplish the FAR_TP to mainland
        // (returnToPreviousSession) triggered by the DSS.
      } else {
  ...
  virtual void execute(CCtrlBase *pCaller, const std::string & /* sParams */) {
    ...
    if (pCaller) {
      ...
      if (fatherGC) {
        ...
        CGroupEditBox *geb = dynamic_cast<CGroupEditBox *>(
            fatherGC->getGroup("add_contact_eb:eb"));
        if (geb && !geb->getInputString().empty()) {
          string charName = geb->getInputString();
          CSessionBrowserImpl &sessionBrowser =
              CSessionBrowserImpl::getInstance();
          sessionBrowser.inviteCharacterByName(sessionBrowser.getCharId(),
                                               charName);

          if (!sessionBrowser.waitOneMessage(
                  sessionBrowser.getMessageName("on_invokeResult"))) {
          ...
          if (sessionBrowser._LastInvokeResult == 14) {
            CViewText *pVT = dynamic_cast<CViewText *>(
                CWidgetManager::getInstance()->getElementFromId(
                    "ui:interface:warning_free_trial:text"));
            if (pVT != NULL)
            ...
            CAHManager::getInstance()->runActionHandler(
                "enter_modal", pCaller, "group=ui:interface:warning_newcomer");
          }
...
class CAHCancelTool : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */,
                       const std::string & /* sParams */) {
    if (!ClientCfg.R2EDEnabled || !getEditor().isInitialized())
      return;
    CHECK_EDITOR
    getEditor().setCurrentTool(NULL);
  }
...
class CAHAnimTestMode : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */,
                       const std::string & /* sParams */) {
    CHECK_EDITOR
    if (getEditor().getAccessMode() == CEditor::AccessDM)
      return;
...
class CAHR2ContextCommand : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const std::string &sParams) {
    ...
    if (commandId == "delete") {
      if (getEditor().getCurrentTool() &&
          getEditor().getCurrentTool()->onDeleteCmd())
        return;
    }
...
class CAHR2Teleport : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */,
    ...
    if (R2::getEditor().getMode() != CEditor::EditionMode &&
        ...
        R2::getEditor().getMode() != CEditor::AnimationModeDm)
      return;
...
// *********************************************************************************************************
// signal the server that a dm gift has begun, so that the server can take note
// of the target entity (the current target)
class CAHR2DMGiftBegin : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */,
    ...
    if (GenericMsgHeaderMngr.pushNameToStream("DM_GIFT:BEGIN", out)) {
      ...
      nlwarning("<CHandlerInvalidateExchange::execute> unknown message name "
                "'DM_GIFT:BEGIN");
  }
};
  ...
  virtual void execute(CCtrlBase * /* pCaller */,
    ...
    if (GenericMsgHeaderMngr.pushNameToStream("DM_GIFT:VALIDATE", out)) {
      for (uint k = 0; k < 8; ++k) {
        uint32 sheetId = 0;
        uint8 quantity = 0;
        CCDBNodeLeaf *sheetLeaf = NLGUI::CDBManager::getInstance()->getDbProp(
            toString("LOCAL:R2:DM_GIFT:%d:SHEET", (int)k));
        if (sheetLeaf)
          sheetId = (uint32)sheetLeaf->getValue32();
        CCDBNodeLeaf *quantityLeaf =
            NLGUI::CDBManager::getInstance()->getDbProp(
                toString("LOCAL:R2:DM_GIFT:%d:QUANTITY", (int)k));
        if (quantityLeaf)
          quantity = (uint8)quantityLeaf->getValue8();
        out.serial(sheetId);
        out.serial(quantity);
      }
      NetMngr.push(out);
...
class CAHR2FreezeUnfreezeBotObjects : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */,
    ...
    CLuaManager::getInstance().executeLuaScript(
        "r2:freezeUnfreezeBotObjects()");
  }
};
REGISTER_ACTION_HANDLER(CAHR2FreezeUnfreezeBotObjects,
...
// *********************************************************************************************************
// write scenario rtdata to a file for debugging or stress testing by
// simulator_service
NLMISC_COMMAND(saveScenarioRtData, "save scenario RtData to file",
               "<filename>") {
  ...
  if (args.empty()) {
    ...
    if (description) {
      ...
      if (title) {
        ...
        fileName =
            "area" + locationId->toString() + "_" + instanceId->toString();
    }
  } else // filename given
  {
    fileName = args[0];
...
NLMISC_COMMAND(
    dumpValidPositions,
    "dump valid position to create objects from the current camera viewpoint",
    "<filename>") {
  if (args.size() != 1)
    return false;
  try {
    ...
    for (uint x = 0; x < result.getWidth(); ++x) {
      for (uint y = 0; y < result.getHeight(); ++y) {
        CVector ray(
            fru.Left + (fru.Right - fru.Left) * (x / (float)result.getWidth()),
            fru.Near,
        ...
        CVector inter;
        wvr.Dir = ray;
        R2::CTool::TRayIntersectionType rit =
            R2::CTool::computeLandscapeRayIntersection(wvr, inter);
        CRGBA resultCol;
        switch (rit) {
        case R2::CTool::NoIntersection:
          resultCol = CRGBA(0, 0, 0, 0);
          break;
        case R2::CTool::ValidPacsPos:
          resultCol = CRGBA(0, 0, 255, 100);
          break;
        default:
          resultCol = CRGBA(255, 0, 0, 100);
          break;
        }
        CRGBA *dest =
            ((CRGBA *)&result.getPixels(0)[0]) + x + y * result.getWidth();
...
//
*********************************************************************************************************
NLMISC_COMMAND(reloadScenario, "reset R2 editor, reload all scripts & xml
(possibly preserving scenario)", "")
{
        if (!args.empty()) return false;
        //
