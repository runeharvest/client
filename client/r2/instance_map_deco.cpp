...
// *********************************************************************************************************
void CInstanceMapDeco::CCtrlButtonEntity::getContextHelp(
...
// *********************************************************************************************************
bool CInstanceMapDeco::CCtrlButtonEntity::handleEvent(
    const NLGUI::CEventDescriptor & /* event */) {
...
// *********************************************************************************************************
void CInstanceMapDeco::setDisplayedInstance(CInstance *instance,
                                            bool orientable) {
  ...
  if (_GlowStarActive) {
    if (_GlowStar[0]) {
      ...
      for (uint k = 0; k < 2; ++k) {
        ...
        _GlowStar[k]->setQuad(
            _GlowStarPos, CV_MapGlowStarSize.get(),
            (float)(CV_MapGlowStarSpeed[k].get() * 0.001 * (double)T1));
  ...
  if (_Orient) {
    ...
    if (vd) {
      ...
      if (vd->getRotateInProgress()) {
        ...
        NLMISC::incrementalBlend(
            _OrientBlendFactor, closeView ? 0.5f : 0.f,
            DT * 1000.f / favoid0(CV_MapEntityOrientBlendTimeInMs.get()));
      }
    } else {
      _OrientBlendFactor = 0.f;
    }
    ...
    if (_OrientBlendFactor == 0.f) {
      ...
      _Orient->setColorRGBA(
          CRGBA(255, 255, 255, (uint8)(255 * _OrientBlendFactor)));
      sint32 x;
      sint32 y;
      groupMap.worldToWindowSnapped(x, y, getWorldPos());
      _Orient->setQuad(CV_MapEntityOrientTexture.get(),
                       CVector((float)x, (float)y, 0.f), vd->getAngle(),
                       closeView ? CV_MapEntityOrientOriginDist.get()
                                 : CV_MapEntityOrientOriginDistSmall.get());
...
void CInstanceMapDeco::onUpdate(CGroupMap &groupMap) {
  ...
  //
  bool closeView = _CloseTexture.empty() ? false
                                         : groupMap.getMeterPerPixel() <
                                               CV_MapEntityCloseDist.get();
  //
  ...
  _Main->setColor(
      (selected && !closeView)
          ? CV_MapEntitySelectColor.get()
          : vd->getDisplayModeColorInMap()); // if small icon, then change the
                                             // icon color directly, because no
  ...
  if (selected || hasFocus) {
    ...
    if (worldPos.x < wmin.x || worldPos.x > wmax.x || worldPos.y < wmin.y ||
        worldPos.y > wmax.y) {
      // OUT OF VISIBLE REGION CASE
      _Over->setActive(true);
      _Over->setColorRGBA(selected ? CV_MapEntitySelectColor.get()
                                   : CV_MapEntityHighlightColor.get());
      ...
      _Over->setRenderLayer(3);
      _Over->setQuad(winInter - (size + bias) * dir, winInter - bias * dir,
                     0.5f * size);
      //
      if (_GlowStar[0]) {
        sint32 screenInterX, screenInterY;
        groupMap.windowToScreen(screenInterX, screenInterY, (sint32)winInter.x,
                                (sint32)winInter.y);
        sint32 refCornerX, refCornerY;
        _GlowStar[0]->getParent()->getCorner(refCornerX, refCornerY,
                                             Hotspot_BL);
        _GlowStarPos.set((float)(screenInterX - refCornerX),
                         (float)(screenInterY - refCornerY), 0.f);
      ...
      if (closeView || hasFocus) {
        ...
        const std::string &tex = closeView
                                     ? CV_MapEntitySelectTexture.get()
                                     : CV_MapEntitySmallHighlightTexture.get();
  ...
  if (_OverInvalid->getActive()) {
    const std::string &tex = closeView ? CV_MapEntityInvalidTexture.get()
                                       : CV_MapEntityInvalidTextureSmall.get();
...
void CInstanceMapDeco::setActive(bool active) {
  ...
  if (active == _Active)
    return;
  if (_Main)
    _Main->setActive(active);
  if (_Over)
    _Over->setActive(active);
  if (_GlowStar[0])
    _GlowStar[0]->setActive(active);
  if (_GlowStar[1])
    _GlowStar[1]->setActive(active);
  if (_Orient)
    _Orient->setActive(active);
  _Active = active;
}
