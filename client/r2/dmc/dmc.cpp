...
//========
void CClientInstantActionFeedBack::doRequestInsertNode(
    const std::string &instanceId, const std::string &name, sint32 position,
    const std::string &key, CObject *value) {
  // H_AUTO(R2_CClientInstantActionFeedBack_doRequestInsertNode)
  //  server record (must be done before client feedback because 'unchanged
  //  value' optimisation would prevent the change, else)
...
//========
void CClientInstantActionFeedBack::doRequestSetNode(
    const std::string &instanceId, const std::string &attrName,
    CObject *value) {
...
//========
void CClientInstantActionFeedBack::doRequestEraseNode(
    const std::string &instanceId, const std::string &attrName,
    sint32 position) {
...
//========
void CClientInstantActionFeedBack::doRequestMoveNode(
  ...
  //  server record (must be done before client feedback because 'unchanged
  //  value' optimisation would prevent the change, else)
  _DMC.doRequestMoveNode(instanceId, attrName, position, destInstanceId,
                         destAttrName, destPosition);
  // client instant feed back
  _DMC.nodeMoved(instanceId, attrName, position, destInstanceId, destAttrName,
                 destPosition);
}
...
// For this we reuse the CActionHistoric class, but without undo capability (it
// doesn't hold a local copy of the scenario) At the end of the frame, commands
// are redirected directly to client, and set to server too. For now the server
// feedback is ignored, however (server just repeats the command in mono edition
// case)
class CFrameActionsRecorder : public IDynamicMapClient {
public:
  CFrameActionsRecorder(CDynamicMapClient &dmc) : _DMC(dmc), _Flushing(false) {
...
//========
void CFrameActionsRecorder::doRequestInsertNode(const std::string &instanceId,
...
//========
void CFrameActionsRecorder::doRequestSetNode(const std::string &instanceId,
                                             const std::string &attrName,
                                             CObject *value) {
...
//========
void CFrameActionsRecorder::doRequestEraseNode(const std::string &instanceId,
                                               const std::string &attrName,
                                               sint32 position) {
...
//========
void CFrameActionsRecorder::doRequestMoveNode(const std::string &instanceId,
                                              const std::string &attrName,
                                              sint32 position,
                                              const std::string &destInstanceId,
                                              const std::string &destAttrName,
  ...
  _FrameActions.requestMoveNode(instanceId, attrName, position, destInstanceId,
                                destAttrName, destPosition);
}
...
void CFrameActionsRecorder::flush() {
  ...
  if (_Flushing)
    return;
  if (!_FrameActions.isNewActionBeingRecorded())
    return;
  _Flushing = true;
...
CDynamicMapClient::CDynamicMapClient(const std::string & /* eid */,
  ...
  // Edition
  IModule *clientEditionModuleInterface =
      mm.createModule("ClientEditionModule", "ClientEditionModule", "");

  nlassert(clientEditionModuleInterface != NULL);

  _EditionModule =
      safe_cast<CClientEditionModule *>(clientEditionModuleInterface);
  ...
  if (!luaState) {
    loadFeatures(); // NB nico : when used by the client,  features are loaded
                    // externally for now ...
  }
...
bool CDynamicMapClient::loadAnimationFromBuffer(
    const std::string &content, const std::string &filename,
    std::string &errMsg, const CScenarioValidator::TValues &values) {
...
void CDynamicMapClient::addPaletteElement(const std::string &attrName,
...
CObject *
CDynamicMapClient::getPropertyValue(CObject *component,
...
void CDynamicMapClient::requestTranslateFeatures() {
  ...
  _ComLua->callTranslateFeatures(
      _EditionModule->getCurrentScenario()->getHighLevel());
}
...
void CDynamicMapClient::requestUploadCurrentScenario() {
  ...
  CObject *highLevel = const_cast<R2::CObject *>(
      getHighLevel()); // the object is not change but the api is broken

  if (highLevel) {
    requestUploadScenario(highLevel);
  }
}

void CDynamicMapClient::doRequestInsertNode(const std::string &instanceId,
                                            const std::string &name,
                                            sint32 position,
                                            const std::string &key,
                                            CObject *value) {
  ...
  nlwarning("DO REQUEST: insert node %s, %s, %d, %s", instanceId.c_str(),
            name.c_str(), (int)position, key.c_str());
  _EditionModule->requestInsertNode(instanceId, name, position, key, value);
}

void CDynamicMapClient::doRequestSetNode(const std::string &instanceId,
                                         const std::string &attrName,
                                         CObject *value) {
  // H_AUTO(R2_CDynamicMapClient_doRequestSetNode)
  nlwarning("DO REQUEST: set node %s, %s", instanceId.c_str(),
            attrName.c_str());
  _EditionModule->requestSetNode(instanceId, attrName, value);
}

void CDynamicMapClient::doRequestEraseNode(const std::string &instanceId,
                                           const std::string &attrName,
                                           sint32 position) {
  // H_AUTO(R2_CDynamicMapClient_doRequestEraseNode)
  nlwarning("DO REQUEST: erase node %s, %s, %d", instanceId.c_str(),
            attrName.c_str(), (int)position);
  _EditionModule->requestEraseNode(instanceId, attrName, position);
}

void CDynamicMapClient::doRequestMoveNode(const std::string &instanceId,
                                          const std::string &attrName,
                                          sint32 position,
                                          const std::string &destInstanceId,
                                          const std::string &destAttrName,
  ...
  _EditionModule->requestMoveNode(instanceId, attrName, position,
                                  destInstanceId, destAttrName, destPosition);
}

// undo / redo buffering

void CDynamicMapClient::requestInsertNode(const std::string &instanceId,
                                          const std::string &name,
                                          sint32 position,
                                          const std::string &key,
                                          CObject *value) {
  // H_AUTO(R2_CDynamicMapClient_requestInsertNode)
  nlwarning("REQUEST: insert node %s, %s, %d, %s", instanceId.c_str(),
            name.c_str(), (int)position, key.c_str());
  _ActionHistoric.requestInsertNode(instanceId, name, position, key, value);
}

void CDynamicMapClient::requestSetNode(const std::string &instanceId,
                                       const std::string &attrName,
                                       CObject *value) {
...
public:
  ...
  CNotifySonDeletion(CInstance &erasedInstance)
      : ErasedInstance(erasedInstance) {}
...
void CDynamicMapClient::requestEraseNode(const std::string &instanceId,
                                         const std::string &attrName,
                                         sint32 position) {
  ...
  nlwarning("REQUEST: erase node %s, %s, %d", instanceId.c_str(),
            attrName.c_str(), (int)position);
  _ActionHistoric.requestEraseNode(instanceId, attrName, position);
}

void CDynamicMapClient::requestMoveNode(const std::string &instanceId,
                                        const std::string &attrName,
                                        sint32 position,
                                        const std::string &destInstanceId,
                                        const std::string &destAttrName,
  ...
  nlwarning("REQUEST: move node node from [%s, %s, %d] to [%s, %s, %d]",
            instanceId.c_str(), attrName.c_str(), (int)position,
            destInstanceId.c_str(), destAttrName.c_str(), (int)destPosition);
  _ActionHistoric.requestMoveNode(instanceId, attrName, position,
                                  destInstanceId, destAttrName, destPosition);
}

void CDynamicMapClient::nodeSet(const std::string &instanceId,
                                const std::string &attrName, CObject *value) {
  // H_AUTO(R2_CDynamicMapClient_nodeSet)
  getCurrentScenario()->setNode(instanceId, attrName, value);
}

void CDynamicMapClient::nodeErased(const std::string &instanceId,
                                   const std::string &attrName,
                                   sint32 position) {
...
void CDynamicMapClient::nodeInserted(const std::string &instanceId,
                                     const std::string &attrName,
                                     sint32 position, const std::string &key,
                                     CObject *value) {
  // H_AUTO(R2_CDynamicMapClient_nodeInserted)
  getCurrentScenario()->insertNode(instanceId, attrName, position, key, value);
}

void CDynamicMapClient::nodeMoved(const std::string &instanceId,
                                  const std::string &attrName, sint32 position,
                                  const std::string &destInstanceId,
                                  const std::string &destAttrName,
                                  sint32 destPosition) {
  // H_AUTO(R2_CDynamicMapClient_nodeMoved)
  getCurrentScenario()->moveNode(instanceId, attrName, position, destInstanceId,
                                 destAttrName, destPosition);
}

void CDynamicMapClient::scenarioUpdated(CObject *highLevel, bool willTP,
...
CObject *CDynamicMapClient::find(const std::string &instanceId,
  ...
  return _EditionModule->getCurrentScenario()->find(instanceId, attrName,
                                                    position, key);
}
...
void CDynamicMapClient::onEditionModeConnected(
  ...
  scenarioUpdated(
      highLevel, willTP,
      initialActIndex); // handle by CEditor.cpp that call
                        // _EditionModule->updateScenario(highLevel);

  CEditor::connectionMsg("");
  //_EditionModule->requestSetPioneerRight( TPioneerRight::DM);
}

void CDynamicMapClient::onAnimationModeConnected(
  ...
  if (connected.Mode == 0 ||
      connected.Mode == 1) // DM Load game or Player wait game
  {
    scenarioUpdated(connected.HighLevel.getData(), true,
                    1); // handle by CEditor.cpp that call
                        // _EditionModule->updateScenario(highLevel);
  }
...
void CDynamicMapClient::onTestModeDisconnected(
    TSessionId /* sessionId */, uint32 /* lasAct */,
...
void CDynamicMapClient::onNpcAnimationTargeted(uint32 mode) {
  ...
  if (mode & CAnimationProp::Alive) {
    ...
    if (mode & CAnimationProp::Grouped) {
      lua.push("r2ed_anim_grp_kill");
    }
    lua.push("r2ed_anim_add_hp");
    if (mode & CAnimationProp::Grouped) {
      lua.push("r2ed_anim_grp_heal");
    }
  }
  ...
  //
  getEditor().callEnvMethod("updateAnimBarActions",
                            lua.getTop() - initialStackSize);
}
...
void CDynamicMapClient::setInstantFeedBackFlag(bool instantFeedBackFlag) {
  ...
  if (instantFeedBackFlag == _InstantFeedBackFlag)
    return;
  if (_InstantFeedBackFlag) {
    _FrameActionsRecorder->flush();
  }
  if (instantFeedBackFlag) {
    _ActionHistoric.setDMC(
        _FrameActionsRecorder); // forward to action recording
  } else {
