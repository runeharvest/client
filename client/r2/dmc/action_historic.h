...
public:
  ...
  /** Signal the beginning of a new action. Calling this twice in a row will not
   * create an empty action.
   */
  void newSingleAction(const ucstring &name);
  /** Begin a new action composed of several subactions. 'numSubActions' calls
   * to 'newAction' will be expected before the merge of the subactions is
   * pushed
   */
   ...
   * if some requests in the action depends on notifications sent by previous
   * requests)
   */
  ...
  void requestInsertNode(const std::string &instanceId, const std::string &name,
...
private:
   ...
   * Once redo / undo have been called, no other requests can be pushed into
   * this action, so calls to 'pushRequest' will assert.
   */
  ...
  public:
    ...
    // For pending action : flush the content that has already been pushed using
    // 'pushRequest' This should be called before 'setCompleted'
  ...
  public:
    CRequestSetNode(const std::string &instanceId, const std::string &attrName,
                    CObject *value);
    // from CRequestBase
  ...
  public:
    CRequestEraseNode(const std::string &instanceId,
                      const std::string &attrName, sint32 position);
    // from CRequestBase
    virtual void redo(IDynamicMapClient *dmc, CScenario &scenario);
    virtual void undo(IDynamicMapClient *dmc, CScenario &scenario);
  ...
  public:
    CRequestInsertNode(const std::string &instanceId,
                       const std::string &attrName, sint32 position,
  ...
  public:
    CRequestMoveNode(const std::string &srcInstanceId,
                     const std::string &srcAttrName, sint32 srcPosition,
                     const std::string &destInstanceId,
                     const std::string &destAttrName, sint32 destPosition);
    // from CRequestBase
    virtual void redo(IDynamicMapClient *dmc, CScenario &scenario);
...
private:
  ...
  /** Index of current action relative to the end of the stack (-1 for the last
   * action) Undo applies to this action, redo applies to the next action
   */
  ...
  //
private:
  // return previous action absolute index, or -1 if there's no previous action
  // that can be undone.
  sint getPreviousActionIndex() const;
  // return next action absolute index, or -1 if there's no next action that can
  // be undone.
