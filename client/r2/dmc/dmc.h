...
public:
  ...
  // If instant feedback flag has been set, flush the command queue (no-op
  // else);
  void flushActions();

  /** \param luaState NULL If the lua environment should be built by this object
   *        If the lua state is provided externally, then it won't be deleted by
   * this object
   */
  CDynamicMapClient(const std::string &eid, NLNET::IModuleSocket *clientGateway,
                    lua_State *luaState);
  ...
  virtual void addPaletteElement(const std::string &attrName,
                                 CObject *paletteElement);

  virtual CObject *getPropertyValue(CObject *component,
  ...
  virtual void onAnimationModeConnected(
      const R2::CClientMessageAdventureUserConnection &connected);
  ...
  // return true if loading was ok
  bool loadAnimationFromBuffer(const std::string &data,
  ...
  CObject *find(const std::string &instanceId, const std::string &attrName = "",
                sint32 position = -1, const std::string &key = "");
