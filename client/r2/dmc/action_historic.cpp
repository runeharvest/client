...
//====================================================================================
void CActionHistoric::newPendingMultiAction(const ucstring &name,
                                            uint actionCount) {
...
void CActionHistoric::endAction() {
  ...
  if (!_NewActionIsPending) {
    _NewAction->flush(
        _DMC,
        _Scenario); // redo may trigger other observers that complete the action
  }
...
void CActionHistoric::clear(CObject *newScenario) {
  ...
  _NewAction = NULL;
  _NewActionIsPending = false;
  _Scenario.setHighLevel(newScenario ? CRequestBase::cloneObject(newScenario)
                                     : NULL);
...
//====================================================================================
void CActionHistoric::requestInsertNode(const std::string &instanceId,
  ...
  if (_Scenario.getHighLevel()) {
    ...
    if (value->getGhost() || (target && target->getGhost())) {
      ...
      getEditor().getDMC().nodeInserted(instanceId, name, position, key,
                                        value->clone());
      return;
    }
  }
  if (!_NewAction)
    _NewAction = new CAction(_NewActionName);
  CRequestBase::TSmartPtr req =
      new CRequestInsertNode(instanceId, name, position, key, value);
  // if (_NewActionIsPending) req->redo(_DMC, _Scenario);
  _NewAction->pushRequest(req);
}

//====================================================================================
void CActionHistoric::requestSetNode(const std::string &instanceId,
  ...
  if (!_NewAction)
    _NewAction = new CAction(_NewActionName);
  CRequestBase::TSmartPtr req =
      new CRequestSetNode(instanceId, attrName, value);
  // if (_NewActionIsPending) req->redo(_DMC, _Scenario);
  _NewAction->pushRequest(req);
}

//====================================================================================
void CActionHistoric::requestEraseNode(const std::string &instanceId,
                                       const std::string &attrName,
                                       sint32 position) {
  ...
  if (!_NewAction)
    _NewAction = new CAction(_NewActionName);
  CRequestBase::TSmartPtr req =
      new CRequestEraseNode(instanceId, attrName, position);
  // if (_NewActionIsPending) req->redo(_DMC, _Scenario);
  _NewAction->pushRequest(req);
}

//====================================================================================
void CActionHistoric::requestMoveNode(const std::string &instanceId,
                                      const std::string &attrName,
                                      sint32 position,
  ...
  if (!_NewAction)
    _NewAction = new CAction(_NewActionName);
  CRequestBase::TSmartPtr req =
      new CRequestMoveNode(instanceId, attrName, position, destInstanceId,
...
//====================================================================================
CActionHistoric::CAction::CAction(const ucstring &name)
    : _Name(name), _FlushedCount(0), _Completed(false), _Flushing(false) {}
...
//====================================================================================
void CActionHistoric::CAction::flush(IDynamicMapClient *dmc,
                                     CScenario &scenario) {
...
//====================================================================================
void CActionHistoric::CAction::rollback(IDynamicMapClient *dmc,
                                        CScenario &scenario) {
...
//====================================================================================
void CActionHistoric::CAction::redo(IDynamicMapClient *dmc,
                                    CScenario &scenario) {
...
//====================================================================================
void CActionHistoric::CAction::undo(IDynamicMapClient *dmc,
                                    CScenario &scenario) {
...
//
void CActionHistoric::CRequestSetNode::redo(IDynamicMapClient *dmc,
                                            CScenario &scenario) {
...
//
void CActionHistoric::CRequestSetNode::undo(IDynamicMapClient *dmc,
                                            CScenario &scenario) {
  // H_AUTO(R2_CRequestSetNode_undo)
  nlassert(
      scenario.getHighLevel()); // if this assert fires, then 'clear' was called
                                // with a NULL pointer ! The action historic
                                // need a start scenario to have undo capability
  if (!_OldValue) {
...
//====================================================================================
CActionHistoric::CRequestEraseNode::CRequestEraseNode(
...
//
void CActionHistoric::CRequestEraseNode::redo(IDynamicMapClient *dmc,
                                              CScenario &scenario) {
  ...
  if (scenario.getHighLevel()) // undo allowed ?
  {
    ...
    if (!old->getNameInParent(_ParentInstanceId, _AttrNameInParent,
                              _PositionInParent)) {
      nlwarning(
          "Can't retrieve name in parent, requestEraseNode undo will fail!");
    }
    _OldValue = cloneObject(old);
...
//
void CActionHistoric::CRequestEraseNode::undo(IDynamicMapClient *dmc,
                                              CScenario &scenario) {
  ...
  // modify local version
  scenario.insertNode(_ParentInstanceId, _AttrNameInParent, _PositionInParent,
                      "", cloneObject(_OldValue));
  // send to network
  dmc->doRequestInsertNode(_ParentInstanceId, _AttrNameInParent,
                           _PositionInParent, "", _OldValue);
  _OldValue = NULL;
...
//====================================================================================
CActionHistoric::CRequestInsertNode::CRequestInsertNode(
...
void CActionHistoric::CRequestInsertNode::redo(IDynamicMapClient *dmc,
  ...
  if (scenario.getHighLevel()) {
    // modify local version
    scenario.insertNode(_InstanceId, _AttrName, _Position, _Key,
                        cloneObject(_Value));
  }
  // send to network
...
#ifdef RYZOM_LUA_UCSTRING
  CLuaIHM::push(getEditor().getLua(),
                ucstring::makeFromUtf8(nodeId->toString()));
#else
...
//====================================================================================
CActionHistoric::CRequestMoveNode::CRequestMoveNode(
    const std::string &srcInstanceId, const std::string &srcAttrName,
    sint32 srcPosition, const std::string &destInstanceId,
...
//
void CActionHistoric::CRequestMoveNode::redo(IDynamicMapClient *dmc,
                                             CScenario &scenario) {
  ...
  if (scenario.getHighLevel()) {
    ...
    if (!src->getNameInParent(_SrcInstanceIdInParent, _SrcAttrNameInParent,
                              _SrcPositionInParent)) {
      nlwarning("Can't find name of source object in its parent");
      return;
    }
    // modify local version
    scenario.moveNode(_SrcInstanceId, _SrcAttrName, _SrcPosition,
                      _DestInstanceId, _DestAttrName, _DestPosition);
    if (!src->getShortestName(_DestInstanceIdAfterMove, _DestAttrNameAfterMove,
                              _DestPositionAfterMove)) {
      nlwarning("Can't retrieve name of instance after move, undo will fail");
    }
  }
  // send to network
  dmc->doRequestMoveNode(_SrcInstanceId, _SrcAttrName, _SrcPosition,
                         _DestInstanceId, _DestAttrName, _DestPosition);
}
//
void CActionHistoric::CRequestMoveNode::undo(IDynamicMapClient *dmc,
                                             CScenario &scenario) {
  ...
  nlassert(dmc);
  // modify local version
  scenario.moveNode(_DestInstanceIdAfterMove, _DestAttrNameAfterMove,
                    _DestPositionAfterMove, _SrcInstanceIdInParent,
                    _SrcAttrNameInParent, _SrcPositionInParent);
  // send to network
  dmc->doRequestMoveNode(_DestInstanceIdAfterMove, _DestAttrNameAfterMove,
                         _DestPositionAfterMove, _SrcInstanceIdInParent,
                         _SrcAttrNameInParent, _SrcPositionInParent);
}
