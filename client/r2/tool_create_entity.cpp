...
// ***************************************************************
CToolCreateEntity::~CToolCreateEntity() { clearArray(); }
...
// ***************************************************************
CToolCreateEntity::CToolCreateEntity(uint ghostSlot,
...
void CToolCreateEntity::updateInvalidCursorOnUI() {
  ...
  const std::vector<CInterfaceGroup *> &groups =
      CWidgetManager::getInstance()->getGroupsUnderPointer();
  for (uint k = 0; k < groups.size(); ++k) {
    if (groups[k]->getId() ==
        "ui:interface:r2ed_palette") // hardcoded for now ...
    {
...
// ***************************************************************
void CToolCreateEntity::commit(const NLMISC::CVector &createPosition,
                               float createAngle) {
  ...
  cloneEntityIntoScenario(ghost, createPosition, createAngle,
                          true, /* new action */
                          false /* create ghost */);
  ...
  if (isMultiPos() && isShiftDown()) {
    ...
    CAHManager::getInstance()->runActionHandler("r2ed_create_entity", NULL,
                                                "PaletteId=" + _PaletteId);
  }
}

// ***************************************************************
bool CToolCreateEntity::isBotObjectSheet(
    const NLMISC::CSheetId &sheetId) const {
  // H_AUTO(R2_CToolCreateEntity_isBotObjectSheet)
  const CCharacterSheet *charSheet =
      dynamic_cast<const CCharacterSheet *>(SheetMngr.get(sheetId));
  if (charSheet) {
    std::string botobjectsPaletteRoot = "palette.entities.botobjects";
    return (_PaletteId.substr(0, botobjectsPaletteRoot.size()) ==
            botobjectsPaletteRoot);
  }
  return false;
}

// ***************************************************************
std::string CToolCreateEntity::cloneEntityIntoScenario(
    CEntityCL *clonee, const NLMISC::CVector &createPosition, float createAngle,
    bool newAction, bool createGhost) {
  ...
  if (!getEditor().verifyRoomLeft(isBotObject ? 0 : 1, isBotObject ? 1 : 0)) {
    return "";
  }
  ...
  if (readableName.empty()) {
    ...
    readableName =
        CI18N::get(isBotObject ? "uiR2EDNameBotObject" : "uiR2EDNameNPC");
  }
  ...
  if (desc.get()) {
    // TMP FIX : if the created entity is a custom npc, then retrieve look from
    // the clonee visual properties
    if (className == "NpcCustom") {
      SPropVisualA vA;
      SPropVisualB vB;
      SPropVisualC vC;
      const string propNameA = toString(
          "SERVER:Entities:E%d:P%d", clonee->slot(), CLFECOMMON::PROPERTY_VPA);
      const string propNameB = toString(
          "SERVER:Entities:E%d:P%d", clonee->slot(), CLFECOMMON::PROPERTY_VPB);
      const string propNameC = toString(
          "SERVER:Entities:E%d:P%d", clonee->slot(), CLFECOMMON::PROPERTY_VPC);
      CCDBNodeLeaf *leafA =
      ...
      if (vA.PropertySubData.HatModel == 0) {
        ...
        sheetId = vsManager->index2Sheet((uint32)vA.PropertySubData.HatModel,
                                         SLOTTYPE::HEAD_SLOT);
        if (sheetId) {
      ...
      if (vA.PropertySubData.JacketModel == 0) {
        ...
        sheetId = vsManager->index2Sheet((uint32)vA.PropertySubData.JacketModel,
                                         SLOTTYPE::CHEST_SLOT);
        if (sheetId) {
      ...
      if (vA.PropertySubData.TrouserModel == 0) {
        ...
        sheetId = vsManager->index2Sheet(
            (uint32)vA.PropertySubData.TrouserModel, SLOTTYPE::LEGS_SLOT);
        if (sheetId) {
      ...
      if (vB.PropertySubData.FeetModel == 0) {
        ...
        sheetId = vsManager->index2Sheet((uint32)vB.PropertySubData.FeetModel,
                                         SLOTTYPE::FEET_SLOT);
        if (sheetId) {
      ...
      if (vB.PropertySubData.HandsModel == 0) {
        ...
        sheetId = vsManager->index2Sheet((uint32)vB.PropertySubData.HandsModel,
                                         SLOTTYPE::HANDS_SLOT);
        if (sheetId) {
      ...
      if (vA.PropertySubData.ArmModel == 0) {
        ...
        sheetId = vsManager->index2Sheet((uint32)vA.PropertySubData.ArmModel,
                                         SLOTTYPE::ARMS_SLOT);
        if (sheetId) {
      ...
      if (vA.PropertySubData.WeaponRightHand == 0) {
        ...
        sheetId =
            vsManager->index2Sheet((uint32)vA.PropertySubData.WeaponRightHand,
      ...
      if (vA.PropertySubData.WeaponLeftHand == 0) {
        ...
        sheetId =
            vsManager->index2Sheet((uint32)vA.PropertySubData.WeaponLeftHand,
      ...
      if (player != NULL) {
        ...
        if (getEditor().getEnv().callMethodByNameNoThrow("randomNPCName", 2,
                                                         1)) {
          CLuaObject result(getEditor().getLua());
    ...
    if (getEditor().getDefaultFeature()) {
      ...
      CInstance *targetAct =
          isBotObject ? getEditor().getBaseAct() : getEditor().getCurrentAct();
      if (!targetAct) {
        nlwarning("Can't find act when creating an entity");
      } else {
        if (_AutoGroup.getGroupingCandidate()) {
          nlassert(!createGhost); // either autogroup or arraymode, both at the
                                  // same time not supported
          _AutoGroup.group(desc.get(), createPosition);
        } else {
          // create standalone
          desc->setGhost(createGhost);
          getDMC().requestInsertNode(
              getEditor().getDefaultFeature(targetAct)->getId(), "Components",
              -1, "", desc.get());
        }
      }
    }
...
void CToolCreateEntity::updateBeforeRender() {
  ...
  if (_CreateState != DrawArray) {
    ...
    setContextHelp(CI18N::get(_AutoGroup.getGroupingCandidate()
                                  ? "uiR2EDToolCreateEntityAutoGroup"
                                  : "uiR2EDToolCreateEntity"));
    return;
  }
  setContextHelp(CI18N::get("uiR2EDDrawArrayContextHelp"));
  ...
  _ValidArray = true;
  TRayIntersectionType rayIntersectionType =
  ...
  if (worldMap)
    worldMap->setSelectionAxis(_ValidArray);
...
void CToolCreateEntity::clearArray() {
  ...
  for (uint k = 0; k < _ArrayElements.size(); ++k) {
    if (_ArrayElements[k]) {
      getEditor().getDMC().requestEraseNode(
          _ArrayElements[k]->getDisplayedInstance()->getId(), "", -1);
    }
  }
}
  ...
  for (uint k = 0; k < _ArrayElements.size(); ++k) {
    ...
    if (!_ArrayElements[k]) {
      if (k < arraySize) {
        ...
        // create new element
        std::string instanceId =
            cloneEntityIntoScenario(clonee, pos, angle, false, /*new action*/
                                    true /*create ghost*/);
        CInstance *inst = getEditor().getInstanceFromId(instanceId);
        if (inst) {
          _ArrayElements[k] = dynamic_cast<CDisplayerVisualEntity *>(
              inst->getDisplayerVisual());
          if (_ArrayElements[k]) {
            _ArrayElements[k]->setDisplayMode(
                CDisplayerVisual::DisplayModeArray);
          }
        }
    ...
    if (_ArrayElements[k]) {
      ...
      // do a kind of 'reserve' on the list of entities : don't delete entities
      // in excess, but hide them instead
      ...
      if (active) {
        ...
        TInstanceId instanceId =
            _ArrayElements[k]->getDisplayedInstance()->getId();
        CVector worldPos = _ArrayElements[k]->getWorldPos();
        if (pos != worldPos) {
          CObject *newPos = buildVector(
              pos,
              _ArrayElements[k]->getDisplayedInstance()->getPosInstanceId());
...
void CToolCreateEntity::commitArray() {
  ...
  for (uint k = 0; k < _ArrayElements.size(); ++k) {
    if (_ArrayElements[k]) {
      cloneEntityIntoScenario(
          _ArrayElements[k]->getEntity(), _ArrayElements[k]->getWorldPos(),
          _ArrayElements[k]->getAngle(), k == 0, /*new action*/
          false /*create ghost*/);
    }
  }
  clearArray();
  ...
  virtual void execute(CCtrlBase *pCaller, const std::string & /* sParams */) {
    ...
    if (but) {
      NLGUI::CDBManager::getInstance()
          ...
          ->setValueBool(but->getPushed());
      CToolCreateEntity *tce =
          dynamic_cast<CToolCreateEntity *>(getEditor().getCurrentTool());
      if (tce) {
        CAHManager::getInstance()->runActionHandler(
            "r2ed_create_entity", NULL, "PaletteId=" + tce->getPaletteId());
      }
    }
  }
};
