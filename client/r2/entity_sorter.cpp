...
void CEntitySorter::registerEntityDisplayer(CDisplayerVisualEntity *dve) {
  ...
  nlassert(std::find(_AddRemoveDebug.begin(), _AddRemoveDebug.end(),
                     NLMISC::CRefPtr<CDisplayerVisualEntity>(dve)) ==
           _AddRemoveDebug.end());
...
#ifdef NL_DEBUG
  nlassert(!_EntityDisplayers.count(dve));
  TAddRemoveDebug::iterator debugIt =
      std::find(_AddRemoveDebug.begin(), _AddRemoveDebug.end(),
                NLMISC::CRefPtr<CDisplayerVisualEntity>(dve));
...
// *********************************************************************************************************
bool CEntitySorter::isWithinDisplayRange(
    CDisplayerVisualEntity *displayer) const {
  if (!displayer)
    return false;
  if (!_FarthestEntity.Displayer)
    return true;
  return CSortedEntity(
             displayer,
             (UserEntity->pos() - displayer->getWorldPos()).sqrnorm()) <
         _FarthestEntity;
}
...
void CEntitySorter::sortByEntitiesByDistance() {
  ...
  for (TDisplayers::iterator it = _EntityDisplayers.begin();
       it != _EntityDisplayers.end(); ++it) {
    CSortedEntity ed;
...
void CEntitySorter::updateFadeInOut() {
  ...
  while (k < (sint)_VisibleList.size()) {
    ...
    if (entity.State == CVisibleEntity::Appear) {
      ...
      if (entity.Displayer->getEntity() &&
          entity.Displayer->getEntity()->getLastClip()) {
...
// order doesn't matter for us, so copy the last entry at our position
  ...
  nlassert((uint)_EntityDisplayers[lastEntity.Displayer] ==
           _VisibleList.size() - 1);
#endif
  _EntityDisplayers[lastEntity.Displayer] =
      index; // remap index to the new position
  _EntityDisplayers[entity.Displayer] =
      -1; // should set to -1 last for the case where &lastEntity == &entity
  entity = lastEntity;
...
void CEntitySorter::updateVisibleList() {
  ...
  for (k = 0; k < candidateCount; ++k) {
    if (_SortTable[k].IndexInVisibleList ==
        -1) // not inserted in visible list ?
    {
      if (_VisibleList.size() < maxEntityCount) {
        ...
        if (dve->isCreatedThisFrame()) {
          _VisibleList.back().BlendValue =
              1.f; // directly visible if just created
        }
#ifdef NL_DEBUG
        nlassert(_EntityDisplayers.count(dve));
#endif
      ...
      _VisibleList[_SortTable[k].IndexInVisibleList].State =
          CVisibleEntity::Appear;
    }
  }
  if (!_SortTable.empty()) {
  ...
  for (; k < _SortTable.size(); ++k) {
    if (_SortTable[k].IndexInVisibleList != -1) {
      ...
      // until i'm removed from the visible list for real during
      // 'updateFadeInOut'
      _VisibleList[_SortTable[k].IndexInVisibleList].State =
          CVisibleEntity::Disappear;
    } else {
...
void CEntitySorter::clipEntitiesByDist() {
  ...
  if (bench) {
    ...
    nlwarning("clip time = %.2f ms",
              1000.f * CTime::ticksToSecond(endTime - startTime));
  }
  CHECK_INTEGRITY
}
...
void CEntitySorter::checkIntegrity() {
#ifdef NL_DEBUG
  for (TAddRemoveDebug::iterator it = _AddRemoveDebug.begin();
       it != _AddRemoveDebug.end(); ++it) {
    nlassert(*it); // if this assert fire, then a displayer was registered using
                   // 'registerEntityDisplayer", but deleted without calling
                   // unregisterEntityDisplayer
  }
  ...
  for (TDisplayers::iterator it = _EntityDisplayers.begin();
    ...
    if (indexInVisibleList != -1) {
      nlassert(indexInVisibleList >= 0 &&
               indexInVisibleList < (TIndexInVisibleList)_VisibleList.size());
      const CVisibleEntity &visibleEntity = _VisibleList[indexInVisibleList];
      nlassert(visibleEntity.Displayer == it->first);
      nlassert(visibleEntity.BlendValue >= 0.f &&
               visibleEntity.BlendValue <= 1.f);
