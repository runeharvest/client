...
public:
  CCtrlPolygonSelectable(CViewBase::TCtorParam &param, CInstance &instance)
      : CCtrlPolygon(param), Instance(instance) {}
  ...
  virtual bool contains(sint32 mouseXInWindow, sint32 mouseYInWindow) const {
    ...
    return CCtrlPolygon::contains(
        CVector2f(mouseXInWindow + 0.5f, mouseYInWindow + 0.5f));
  }
  // tooltip
...
public:
  CCtrlQuadSelectable(CViewBase::TCtorParam &param, CInstance &instance,
                      uint edgeIndex)
      : CCtrlQuad(param), Instance(instance), EdgeIndex(edgeIndex) {}
  // from IDisplayerUIHandle
  virtual CInstance &getDisplayedInstance() { return Instance; }
  // from IDisplayerUIHandle
...
CDisplayerVisualGroup::CSelectablePrimRender::newCtrlPolygon() const {
  ...
  CCtrlPolygonSelectable *result =
      new CCtrlPolygonSelectable(param, *DisplayedInstance);
...
// *********************************************************************************************************
CCtrlQuad *CDisplayerVisualGroup::CSelectablePrimRender::newCtrlQuad(
    uint edgeIndex) const {
  // H_AUTO(R2_CDisplayerVisualGroup_CSelectablePrimRender)
  nlassert(DisplayedInstance);
  CViewBase::TCtorParam param;
  CCtrlQuadSelectable *result =
      new CCtrlQuadSelectable(param, *DisplayedInstance, edgeIndex);
  result->setToolTipParent(CCtrlBase::TTMouse);
  result->setToolTipParentPosRef(Hotspot_BR);
  result->setToolTipPosRef(Hotspot_TL);
  return result;
...
// *********************************************************************************************************
void CDisplayerVisualGroup::onAttrModified(const std::string &attrName,
                                           sint32 attrIndex) {
  // H_AUTO(R2_CDisplayerVisualGroup_onAttrModified)
  CDisplayerVisual::onAttrModified(attrName, attrIndex);
  // necessary to handle this because there no event for 'element removed from
  // table' for now ...
...
// *********************************************************************************************************
bool CDisplayerVisualGroup::isInProjectionBorder(
...
void CDisplayerVisualGroup::onPostRender() {
  if (!_Active || !getActualVisibility())
    return;
  updatePrimVertices();
  updateAccessibility();
}
...
void CDisplayerVisualGroup::updateAccessibility() {
  ...
  {
          if
  (!getEditor().getIslandCollision().isValidSegment(_Prim.getVertices()[k],
  _Prim.getVertices()[(k + 1) % _Prim.getVertices().size()]))
          {
...
void CDisplayerVisualGroup::updatePrimVertices() {
  ...
  for (uint k = 0; k < _Instances.size(); ++k) {
    if (_Instances[k]) {
      vertices.push_back(_Instances[k]->evalLinkPoint(
          k == 0 && _Prim.getLook().Shape == CPrimLook::Star));
      _Poly2D.Vertices.push_back(
          CVector(vertices.back().x, vertices.back().y, 0.f));
    } else {
  ...
  // test against the collision map : all traversed opsition must be valid else
  // poly is invalid
  /*
  ...
  else
  {
          ...
          R2::CScenarioEntryPoints::CCompleteIsland *island =
  getEditor().getIslandCollision().getCurrIslandDesc(); const
  NLMISC::CArray2D<sint16> &heightMap =
  getEditor().getIslandCollision().getHeightMap(); if
...
void CDisplayerVisualGroup::updatePrimLook() {
  ...
  if (displayMode != DisplayModeHidden && _ContextualVisibilityActive &&
  ...
  CPrimLook &look =
      (_CurrPrimValid && !_CurrPrimInaccessible) ? _PrimLook : _PrimLookInvalid;
...
void CDisplayerVisualGroup::onPreRender() {
  ...
  if (!_Active)
    return;
  // see if render is on for primitives (don't use a config var because may be
  // dynamically changed)
  ...
  if (_Prim.getLook().Shape == CPrimLook::ClosedPolyLine && _CurrPrimValid) {
    ...
    if (getDisplayFlag(FlagSelected)) {
      ...
      polyColor = getBlinkColor(
          blend(baseColor, CV_FocusedRegionColor.get(), selectionFactor));
    }
  ...
  {
          _Prim.setEmissive(getBlinkColor(blend(CV_UnselectedInstanceColor.get(),
  CV_FocusedInstanceColor.get(), selectionFactor)));
  }
  */
  //
...
// *********************************************************************************************************
bool CDisplayerVisualGroup::evalEnterPoint(const NLMISC::CVector &startPoint,
  ...
  for (uint k = 0; k < _Instances.size(); ++k) {
    ...
    CVector v1 =
        _Instances[(k + 1) % _Instances.size()]->getWorldPos().asVector();
    CLine l(v0, v1);
    CVector proj;
...
// *********************************************************************************************************
void CDisplayerVisualGroup::getSonsWorldPos2f(
    std::vector<NLMISC::CVector2f> &result) {
...
// *********************************************************************************************************
void CDisplayerVisualGroup::getSons(
    std::vector<CDisplayerVisual *> &sons) const {
...
CDisplayerVisualGroup::getActualDisplayMode() const {
  ...
  if (dm == DisplayModeHidden)
    return dm;
  if (!_ContextualVisibilityActive ||
      const_cast<CDisplayerVisualGroup *>(this)->isContextuallyVisible())
    return dm;
...
bool CDisplayerVisualGroup::isContextuallyVisible() {
  ...
  if (selection) {
    if (this == selection->getDisplayerVisual())
      return true;
    if (selection->getDisplayerVisual() &&
        selection->getDisplayerVisual()->getParent() == this)
      return true;
  }
