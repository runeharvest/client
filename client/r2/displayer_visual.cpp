...
// ***************************************************************
NLMISC::CRGBA CDisplayerVisual::getBlinkColor(NLMISC::CRGBA defaultColor,
                                              NLMISC::CRGBA blinkColor) const {
...
// *********************************************************************************************************
void CDisplayerVisual::onAttrModified(const std::string &attrName,
                                      sint32 /* attrIndex */) {
...
/*void CDisplayerVisual::updateDisplayMode()
{
        ...
        if (value >= 0 && value < DisplayModeCount)
        {
                ...
                {
                        //if (newDisplayMode == DisplayModeFrozen &&
getEditor().getSelectedInstance() == getDisplayedInstance())
                        //{
        ...
        {
                nlwarning("Trying to set an invalid display mode : %d", (int)
_DisplayMode);
        }
}*/
...
void CDisplayerVisual::updateLocalPos() {
  ...
  if (getProps().findAttr("Position")) {
    ...
    if (!isValidDouble(_Pos.x) || !isValidDouble(_Pos.y) ||
        !isValidDouble(_Pos.z)) {
      //			nlassert(!wantAssert);
...
void CDisplayerVisual::updateWorldPos() {
  ...
  static volatile bool wantAssert = true;
  if (!isValidDouble(_WorldPos.x) || !isValidDouble(_WorldPos.y) ||
      !isValidDouble(_WorldPos.z)) {
    nlassert(!wantAssert);
  }
  updateValidPosFlag();
}
...
void CDisplayerVisual::updateValidPosFlag() {
  ...
  if (!isCompound()) {
    ...
    CScenarioEntryPoints::CCompleteIsland *islandDesc =
    ...
    setDisplayFlag(FlagBadPos, !getEditor().getIslandCollision().isValidPos(
                                   NLMISC::CVector(_WorldPos)));
  }
}
...
void CDisplayerVisual::onPostCreate() {
  ...
  _LastParentOk = false;
  updateLocalPos();
  updateWorldPos(); // do not recurse here because sons pos have not been
                    // initialized yet
  // updateDisplayMode();
...
void CDisplayerVisual::onContinentChanged() {
  ...
  if (getActive()) {
    setActive(false);
    setActive(
        true); // force a refresh of collision (EntitiesManager::changeContinent
               // updates the primitive, but broken for an unknown reason ...)
  }
}
...
bool CDisplayerVisual::isActiveInCurrentAct() const {
  ...
  if (parentAct == getEditor().getBaseAct() ||
      parentAct == getEditor().getCurrentAct()) {
    return true;
  }
  return false;
}
      ...
      if (!_IconInScene && !_IconInSceneCreationFailed) {
        ...
        CInterfaceGroup *group =
            CWidgetManager::getInstance()->getParser()->createGroupInstance(
                iconTemplateName, "ui:interface", NULL, 0);
        if (group) {
          _IconInScene = dynamic_cast<CGroupInScene *>(group);
          if (!_IconInScene) {
            nlwarning("Template %s has bad type : should be a derived group "
                      "from CGroupInScene",
                      iconTemplateName);
            delete group;
            ...
            CWidgetManager::getInstance()->addWindowToMasterGroup(
                "ui:interface", group);
...
void CDisplayerVisual::evalIconInScenePos(NLMISC::CVector &dest) const {
  ...
  NLMISC::CVector result = getWorldPos().asVector() -
                           radius * MainCam.getMatrix().getJ() +
                           selectBox.getHalfSize().z * CVector::K;
  static volatile bool wantAssert = true;
  if (!isValidDouble(result.x) || !isValidDouble(result.y) ||
      !isValidDouble(result.z)) {
    nlassert(!wantAssert);
    return;
  }
  dest = result;
...
// *********************************************************************************************************
bool CDisplayerVisual::evalEnterPoint(const NLMISC::CVector & /* startPoint */,
...
// *********************************************************************************************************
void CDisplayerVisual::getSonsWorldPos2f(
...
bool CDisplayerVisual::isSelectable() const {
  ...
  return getDisplayedInstance()->getSelectableFromRoot() &&
         (dm == DisplayModeVisible || dm == DisplayModeLocked);
}
...
CDisplayerVisual::TDisplayMode CDisplayerVisual::getActualDisplayMode() const {
  ...
  if (!getDisplayedInstance()->getSelectableFromRoot() &&
      dm != DisplayModeHidden) {
  ...
  // If it is not an animation but a scenario started from ring access do not
  // display things
...
CRGBA CDisplayerVisual::getDisplayModeColorInScene() const {
  ...
  case DisplayModeVisible:
    return CV_UnselectedInstanceColor.get();
  case DisplayModeFrozen:
    return CV_FrozenInstanceColor.get();
  case DisplayModeLocked:
    return CV_LockedInstanceColor.get();
  case DisplayModeArray:
    return CV_ArrayInstanceColor.get();
  default:
...
CRGBA CDisplayerVisual::getDisplayModeColorInMap() const {
  ...
  case DisplayModeVisible:
    return CRGBA::White;
  case DisplayModeFrozen:
    return CV_MapEntityFrozenColor.get();
  case DisplayModeLocked:
    return CV_MapEntityLockedColor.get();
  case DisplayModeArray:
    return CV_ArrayInstanceColor.get();
  default:
    return CRGBA(0, 0, 0, 0);
