...
inline bool operator==(const CVertexLook &lhs, const CVertexLook &rhs) {
  return lhs.DecalTexture == rhs.DecalTexture &&
         lhs.DecalSize == rhs.DecalSize && lhs.DecalColor == rhs.DecalColor &&
...
inline bool operator==(const CEdgeLook &lhs, const CEdgeLook &rhs) {
  return lhs.ShapeName == rhs.ShapeName && lhs.ShapeScale == rhs.ShapeScale &&
...
inline bool operator!=(const CPrimLook &lhs, const CPrimLook &rhs) {
  return !(lhs == rhs);
}

/** class to display a primitive (zone, roads, groups ...) in scene, and
 * possibly on the world map
 */
...
public:
  ...
  const CMeshArray &getVerticesShapeInstance() const {
    return _VertexShapeInstances;
  }
  ...
  // Return view in the world map associated with the nth vertex (or NULL if not
  // added in the map)
  ...
  /** For derivers : build a new CCtrlQuad for display of edges on the worldmap
   * (or maybe a derived class)
   */
  virtual CCtrlQuad *newCtrlQuad(uint edgeIndex) const;

  /** Set a custom matrix to compute uvs for both decal edge & world map edge
   * Useful to project the accessibility texture, to show the user were he can't
   * draw / move a primitive
   */
  void setCustomWorldMapEdgeUVMatrix(
      bool on, const NLMISC::CMatrix &matrix = NLMISC::CMatrix::Identity);
  void setCustomDecalEdgeUVMatrix(
      bool on, const NLMISC::CMatrix &matrix = NLMISC::CMatrix::Identity);
...
private:
  void update();
  void updatePos();
  void updateEdge(NL3D::UInstance edge, const NLMISC::CVector &start,
                  const NLMISC::CVector &end);
  void updateEdgeDecal(CDecal &edgeDecal, const NLMISC::CVector &start,
                       const NLMISC::CVector &end, float distToStartVertex,
