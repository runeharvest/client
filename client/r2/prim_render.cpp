...
void readFromLua(const CLuaObject &table, const char *key, float &dest) {
  if (table[key].isNumber())
    dest = (float)table[key].toNumber();
}
void readFromLua(const CLuaObject &table, const char *key, uint &dest) {
  if (table[key].isInteger())
    dest = (uint)table[key].toInteger();
}
void readFromLua(const CLuaObject &table, const char *key, std::string &dest) {
  if (table[key].isString())
    dest = table[key].toString();
}

void readFromLua(const CLuaObject &table, const char *key,
                 CStringShared &dest) {
  if (table[key].isString())
    dest = table[key].toString();
}
...
void readFromLua(const CLuaObject &table, const char *key, bool &dest) {
  if (table[key].isBoolean())
    dest = table[key].toBoolean();
}
void readFromLua(const CLuaObject &table, const char *key, CRGBA &dest) {
  if (table[key].isRGBA())
    dest = table[key].toRGBA();
}
...
void CEdgeLook::init(const CLuaObject &params) {
  ...
  if (decalWrapMode < WrapModeCount)
    DecalWrapMode = (TWrapMode)decalWrapMode;
  uint worldMapWrapMode = Scaled;
  readFromLua(params, "WorldMapWrapMode", worldMapWrapMode);
  if (worldMapWrapMode < WrapModeCount)
    WorldMapWrapMode = (TWrapMode)worldMapWrapMode;
}

// *********************************************************
bool operator==(const CPrimLook &lhs, const CPrimLook &rhs) {
  return lhs.Shape == rhs.Shape && lhs.VertexShapeName == rhs.VertexShapeName &&
...
CCtrlPolygon *CPrimRender::newCtrlPolygon() const {
  ...
  protected:
    // from CCtrlPolygon
    void computeScaledVertex(NLMISC::CVector2f &dest,
...
bool CPrimRender::contains(const NLMISC::CVector2f &pos) const {
  ...
  for (uint k = 0; k < _VertexDecals.size(); ++k) {
    if (_VertexDecals[k]->contains(pos))
      return true;
  }
  for (uint k = 0; k < _EdgeDecals.size(); ++k) {
    if (_EdgeDecals[k]->contains(pos))
      return true;
  }
  return false;
}
...
// *********************************************************
void CPrimRender::setCustomWorldMapEdgeUVMatrix(bool on,
                                                const NLMISC::CMatrix &matrix) {
...
// *********************************************************
void CPrimRender::setCustomDecalEdgeUVMatrix(bool on,
                                             const NLMISC::CMatrix &matrix) {
...
void CPrimRender::update() {
  ...
  case CPrimLook::PolyLine:
    _NumEdges = _Vertices.size() <= 1 ? 0 : (sint)_Vertices.size() - 1;
    if (!_Look.LastEdgeIsValid && _NumEdges != 0)
      --_NumEdges;
    break;
  case CPrimLook::ClosedPolyLine:
    _NumEdges = _Vertices.size() >= 3
                    ? (sint)_Vertices.size()
                    : std::max((sint)0, (sint)(_Vertices.size() - 1));
    if (!_Look.LastEdgeIsValid && _NumEdges != 0)
      --_NumEdges;
    break;
  default:
  ...
  if (!_Look.EdgeLook.DecalTexture.empty()) {
    ...
    for (sint k = 0; k < _NumEdges; ++k) {
      ...
      _EdgeDecals[k]->setTexture(_Look.EdgeLook.DecalTexture,
                                 _Look.EdgeLook.DecalWrapMode ==
                                     CEdgeLook::Centered,
                                 true, _Look.EdgeLook.DecalFiltered);
      _EdgeDecals[k]->setDiffuse(_Look.EdgeLook.DecalColor);
      _EdgeDecals[k]->setCustomUVMatrix(_CustomDecalEdgeUVMatrix.On,
                                        _CustomDecalEdgeUVMatrix.Matrix);
    }
  }
  //////////////////
  // POSITIONNING //
  ...
  nlinfo("%.2f seconds for : CPrimRender::update", (endTime - _StartTime) /
  1000.f);
  */
}
...
void CPrimRender::updatePos() {
  ...
  //  world map
  if (_AddedToWorldMap) {
    setWorldMapNumVertices(
        _Look.VertexLook.WorldMapTexture.empty() ? 0 : (uint)_Vertices.size());
  ...
  if (!_Vertices.empty()) {
    ...
    case CPrimLook::Star: {
      ...
      for (uint k = 1; k < _Vertices.size(); ++k) {
        ...
        if (!_EdgeDecals.empty()) {
          updateEdgeDecal(*(_EdgeDecals[k - 1]), centerPos, _Vertices[k],
                          _Look.FirstVertexLook.DecalDistToEdgeDecal,
    ...
    case CPrimLook::ClosedPolyLine: {
      for (sint k = 0; k < _NumEdges; ++k) {
        //
        if (!_EdgeShapeInstances.empty()) {
          updateEdge(_EdgeShapeInstances[k], _Vertices[k],
                     _Vertices[(k + 1) % _Vertices.size()]);
        }
        if (!_EdgeDecals.empty()) {
          updateEdgeDecal(*(_EdgeDecals[k]), _Vertices[k],
                          _Vertices[(k + 1) % _Vertices.size()],
                          k == 0 ? _Look.VertexLook.DecalDistToEdgeDecal
                                 : _Look.FirstVertexLook.DecalDistToEdgeDecal,
                          _Look.VertexLook.DecalDistToEdgeDecal);
        }
      }
  ...
  for (uint k = 0; k < _Vertices.size(); ++k) {
    ...
    if (!_VertexDecals.empty()) {
      _VertexDecals[k]->setWorldMatrixForSpot(
          CVector2f(_Vertices[k].x, _Vertices[k].y),
          k == 0 ? _Look.FirstVertexLook.DecalSize
                 : _Look.VertexLook.DecalSize);
    }
  }
  //
  if (_AddedToWorldMap) {
...
// *********************************************************
void CPrimRender::updateEdge(NL3D::UInstance edge, const NLMISC::CVector &start,
...
// *********************************************************
void CPrimRender::updateEdgeDecal(CDecal &edgeDecal,
  ...
  // compute real start coordinate that is at 'startRadius' dist from the
  // 'start' pos
  ...
  CMatrix uvMatrix;
  float uScale = _Look.EdgeLook.DecalUScale *
                 (length - (distToStartVertex + distToEndVertex));
...
void CPrimRender::setWorldMapNumEdges(uint count) {
  ...
  if (count < _WorldMapEdges.size()) {
    ...
    while (left--) {
      ...
      if (_WorldMapPoly) {
        cq->setId(_WorldMapPoly->getId() +
                  toString("_e%d", _WorldMapEdges.size() - 1));
      }
    }
  }
...
void CPrimRender::onUpdate(CGroupMap &worldMap) {
  ...
  if (_WorldMapPoly) {
    if (_Look.Shape == CPrimLook::ClosedPolyLine) {
      ...
      const CVector2f &origin = test1 ? worldMap.getWorldOffset() :
      CVector2f::Null;
      ...
      polyMatrix.setRot(CVector(scaleX, 0.f, 0.f),
                                              ...
                                              CVector::Null);
      polyMatrix.setPos(CVector(ref0.x - scaleX * origin.x, ref0.y - scaleY *
      origin.y, 0.f));
      */
      ...
      if (dumpPoly)
      {
              ...
              for(uint k = 0; k < verts.size(); ++k)
              {
                      ...
                      nlwarning("vert %d = (%.1f, %.1f, %.1f)", (int) k , pos.x,
      pos.y, pos.z);
              }
              dumpPoly = false;
      }*/
  ...
  for (uint k = 0; k < _WorldMapEdges.size(); ++k) {
    ...
    if (_CustomWorldMapEdgeUVMatrix.On) {
      CUV uvs[4];
      CVector startUV =
          _CustomWorldMapEdgeUVMatrix.Matrix * _Vertices[startIndex];
      ...
      case CEdgeLook::Scaled:
        ...
        break;
      case CEdgeLook::Repeat:
        _WorldMapEdges[k]->setPattern(
            0.f, _InvWorldTextureWidth * _Look.EdgeLook.WorldMapUScale * length,
            CCtrlQuad::Repeat);
        break;
      case CEdgeLook::Centered:
        _WorldMapEdges[k]->setPattern(
            0.5f - 0.5f * length * _InvWorldTextureWidth *
                       _Look.EdgeLook.WorldMapUScale,
            0.5f + 0.5f * length * _InvWorldTextureWidth *
                       _Look.EdgeLook.WorldMapUScale,
            CCtrlQuad::Clamp);
        break;
...
CViewBitmap *CPrimRender::getWorldMapVertexView(uint index) const {
  ...
  if (index >= _WorldMapVertices.size())
    return NULL;
...
void CPrimRender::setWorldMapPolyColor(NLMISC::CRGBA color) {
  ...
  if (_WorldMapPoly)
    _WorldMapPoly->setColorRGBA(color);
}
...
void CPrimRender::setActive(bool active) {
  ...
  for (uint k = 0; k < _WorldMapVertices.size(); ++k) {
    if (_WorldMapVertices[k])
      _WorldMapVertices[k]->setActive(active);
  }
  for (uint k = 0; k < _WorldMapEdges.size(); ++k) {
    if (_WorldMapEdges[k])
      _WorldMapEdges[k]->setActive(active);
  }
  if (_WorldMapPoly)
    _WorldMapPoly->setActive(active);
