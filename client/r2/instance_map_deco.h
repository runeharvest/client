...
/** Tool to display of an instance in current scenario in the world map (a world
 * map DECO This class derives from CGroupMap::IDeco, meaning it must be added /
 * removed to the world map by calling CGroupMap::addDeco /
 * CGroupMap::removeDeco
 */
...
private:
  /** special derived class from CBitmap that allows to know what is the current
   * instance when the mouse position is tested against the map (see
   * R2::CTool::checkInstanceUnderMouse for details)
   */
  class CCtrlButtonEntity : public CCtrlButton, public IDisplayerUIHandle {
  public:
    CCtrlButtonEntity(CInstance &instance)
