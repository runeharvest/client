...
/**
 * Display an instance in a R2 scenario as a primitive. (through the CPrimRender
 * class) Display is made in both the 3D scene and the world map.
 *
 * This displayer is intended to be used for compound objects display
 * The displayer will look into a sub-instance array in the displayed instance
 * to find the vertices of the primitive to display.
 *
 * The displayer is initialized from a lua table, (instance displayers are
 * defined in their lua class definition, see base_class.lua for details)
 *
 *
...
public:
   ...
   * The 'Shape' field is a string that give the shape (same values than the
   * TShape enum). The 'Array' field is a string that give the name of the
   * sub-instance array containing the primitive vertices
   */
  ...
  // see if a position if over an edge, return the edge index in this case, or
  // -1 if not found
   ...
   * In this mode, the road or the region is only displayed if some entity is
   * attached to it (no-op for groups)
   */
  void setContextualVisibilityActive(bool active);
  bool getContextualVisibilityActive() const {
    return _ContextualVisibilityActive;
  }
  /** If contextual is enabled, tells whether this object is visible with
   * current context Actually when one entity uses that road / region
   */
  ...
  REFLECT_BOOL("ContextualVisibilityActive", getContextualVisibilityActive,
               setContextualVisibilityActive);
  REFLECT_EXPORT_END

private:
  mutable std::vector<CDisplayerVisual *> _Instances;
  /** Special CRenderPrim : when 'in world map' intersection test is done, the
   * CCtrlPolygon will allow to know which CInstance is actually displayed by
   * this polygon. This is because the CPrimRender class does not now anything
   * about R2::CInstance or anything related to the scenario...
   */
  ...
  bool _CurrPrimValid; // is the current polygon valid (e.g can be split into
                       // convex polys ?)
  ...
  bool _Active;
  bool _ContextualVisibilityActive; // when true, visibility depends on current
                                    // selection : group must be part of
...
public:
  ...
  void setContextualVisibilityDate(sint64 date) {
    _ContextualVisibilityDate = date;
  }
};
