...
//-----------------------------------------------
NL3D::UParticleSystemInstance CFXManager::instantFX(const std::string &fxName,
                                                    float timeOut /* = 0.f*/) {
  H_AUTO_USE(RZ_FXManager)
  // Check FX Name.
  if (fxName.empty())
...
// addFX :
//-----------------------------------------------
void CFXManager::addFX(NL3D::UParticleSystemInstance fx, float timeOut,
...
void CFXManager::update() {
  ...
  while (!_DeferredFXByDate.empty()) {
    ...
    if (T1 < (uint)_DeferredFXByDate.begin()->first)
      break;
  ...
  while (itFx != _FX2RemoveList.end()) {
    ...
    if (!itTmp->Instance.isSystemPresent() || !itTmp->Instance.isValid() ||
...
// deferedFX :
//-----------------------------------------------
void CFXManager::deferFX(const std::string &fxName,
                         const NLMISC::CMatrix &matrix, float delayInSeconds,
                         float timeOut /*=FX_MANAGER_DEFAULT_TIMEOUT*/) {
  CDeferredFX fx;
  fx.FXName = fxName;
  fx.Matrix = matrix;
  fx.TimeOut = timeOut;
  _DeferredFXByDate.insert(
      std::make_pair(T1 + uint64(1000.f * delayInSeconds), fx));
}
