...
//===================================================================================================
static void applySkyMaterialSetup(UInstance instance, bool isNight, uint stage,
  ...
  if (!isNight)
    ContinentMngr.cur()->DaySkySetup.applyToInstance(instance, stage,
                                                     skipFirstMaterial);
  else
    ContinentMngr.cur()->NightSkySetup.applyToInstance(instance, stage,
                                                       skipFirstMaterial);
}

//===================================================================================================
static void applySkyTex(UInstance instance, uint stage,
                        const std::string &name) {
  H_AUTO_USE(RZ_SkyRender)
  if (instance.empty())
    return;
  if (instance.getNumMaterials() < 1)
    return;
 ...
 * the diffuseAlpha gives a modulate factor for the sky, which is then blended
 * additively
 */
static void setSkyAlpha(UInstance instance, float constantAlpha,
                        float diffuseAlpha, bool opaque) {
  H_AUTO_USE(RZ_SkyRender)
  if (instance.empty())
    return;
  ...
  instance.getMaterial(0).setDstBlend(opaque ? UInstanceMaterial::zero
...
static void renderStandardSky(float dayNight) {
  ...
  applySkyMaterialSetup(Sky, false,
...
//===================================================================================================
/** setup a sky dome for a blend between night & day, with eventually a weather
 * blend
 */
static void setupSkyDomeTextures(UInstance instance,
                                 const std::string &weatherDay,
                                 const std::string &weatherNight) {
  H_AUTO_USE(RZ_SkyRender)
  if (instance.empty())
    return;
  // setup stage 0 (day)
...
void renderSky(const CLightCycleManager &lcm, NLMISC::CRGBA fogColor) {
  ...
  // build a set of 4 background between which to blend depending on weather &
  // hour
  //                  00    01
  // bg for setup 0    +-----+    ---> background depending on hour (day, dusk,
  // night)
  //                   |     |
  //                   |     |
  //  bg for setup 1   +-----+
  ...
  case CLightCycleManager::DayToNight:
    if (lightLevel <= duskRatio) {
      ...
      blendFactor =
          duskRatio != 1.f ? (lightLevel - duskRatio) / (1.f - duskRatio) : 0.f;
  ...
  if (!ContinentMngr.cur()->Indoor) {
    if (lightLevel == 0.f || lightLevel == 1.f) {
      ...
      if (!bg1.empty() || !bg2.empty()) {
        ...
        if (!bg1.empty()) {
          ...
          {
            applySkyMaterialSetup(
                Sky, isNight, 0,
                true); // duplicate current setup in both stages
          ...
          applySkyMaterialSetup(Sky, isNight,
                                0); // duplicate current setup in both stages
          applySkyMaterialSetup(Sky, isNight, 1, true);
          applySkyTex(Sky, 1, bg2);
        }
    ...
    } else // We are in a transition between day and night
    {
      ...
      if (bg00->empty() && bg01->empty() && bg10->empty() && bg11->empty()) {
        // no weather texture
        renderStandardSky(blendFactor);
      } else // 2 pass rendering
      {
        ...
        setSkyAlpha(Sky2ndPass, blendFactor, ws.BlendFactor,
                    false /* alpha additif */);
        showSkyParts(true, false, false);
        SkyScene->render();
    ...
    if (!SkyFogPart.empty()) {
      ...
      if (SkyFogPart.getNumMaterials() > 0) {
        ...
        CMatrix uvMat;
        uvMat.scale(CVector(
            1.f, ws.FogGradientFactor != 0.f ? 1.f / ws.FogGradientFactor : 0.f,
            1.f)); // scale the fog part
