...
const float *CProjectileManager::getProjectileFXUserParams(uint power) {
  if (power < 1 || power > MAGICFX::NUM_SPELL_POWER)
    return NULL;
...
void CProjectileManager::addProjectile(const CProjectileBuild &pb) {
  ...
  // eval start position
  CCharacterCL *casterEntity =
      dynamic_cast<CCharacterCL *>(EntitiesMngr.entity(pb.TargeterInfo.Slot));
  if (casterEntity) {
    proj.LastEntityOrientation = casterEntity->front();
    evalFXPosition(&pb.TargeterInfo.StickMode, *casterEntity,
                   proj.TargeterInfo.DefaultPos, pb.TargeterInfo.StickOffset);
  ...
  if (targetEntity) {
    evalFXPosition(&(pb.ProjectileAimingPoint), *targetEntity,
                   proj.Target.TargetPos);
  }
  // else ... if target entity is not valid, default pos given in the target is
  // used

  // create actual fxs (unless hit is direct)
  if (pb.EndDate > pb.StartDate) {
    if (pb.ProjectileAspect) {
      if (pb.AttackInfo.Intensity >= 1 &&
          pb.AttackInfo.Intensity <= MAGICFX::NUM_SPELL_POWER) {
        ...
        for (uint k = 0; k < projectile.FX.size(); ++k) {
          ...
          proj.FX[k] = sheet.createMatchingInstance(
              ProjectileUserParams[pb.AttackInfo.Intensity - 1]);
          if (!proj.FX[k].empty()) {
            // Force transform to be linked to the root, and not clusterized
            // This is because we don't compute the cluster system the
            // projectile is in, and as a result, projectile can't be seen when
            // player is in a cluster system. To compute the cluster system we
            // would need a PACS primitive, and thus collisions.
  ...
  proj.ParabolaHeight =
      dist * 0.01f * BOMB_MISSILE_PARABOLA_HEIGHT_FOR_100_METERS;
}
...
void CProjectileManager::update() {
  ...
  for (std::list<CProjectile>::iterator it = _ToShutDownProjectiles.begin();
       it != _ToShutDownProjectiles.end(); ++it) {
    if (it->LetProjectileStickedOnTarget) {
      CCharacterCL *target = NULL;
      if (it->Target.Slot != CLFECOMMON::INVALID_SLOT) {
        target =
            dynamic_cast<CCharacterCL *>(EntitiesMngr.entity(it->Target.Slot));
      }
      it->shutDown(target);
    } else {
      it->shutDown(NULL);
  ...
  while (it != _ActiveProjectiles.end()) {
    ...
    double segmentRatio = totalDeltaT != 0 ? deltaT / totalDeltaT : 1;
    if (segmentRatio >= 1.f) {
      impact = true;
      segmentRatio = 1.f;
    }
    ...
    // For a bomb, Pos(t) = t * E + (1 - t) * S + h * (0 0 -4t^2 + 4t) with S =
    // start point, E = end point, h is the height of parabola, and t ranging
    // from 0 to 1 so The speed direction at a given date t is Speed(t) =
    // dPos(t)/dt = E - S + h * (0 0 -8t + 4) NB : anim track is not taken in
    // account
    NLMISC::CVector I;
    if (proj.Mode == MAGICFX::Bomb) {
      I.set(deltaPos.x, deltaPos.y,
            deltaPos.z +
                proj.ParabolaHeight * (4.f - 8.f * (float)segmentRatio));
      I.normalize();
    } else {
      I = deltaPos.normed();
    }
    ...
    if (proj.Mode == MAGICFX::Bomb) {
      // for simplicity, the parabola reach its max at the middle of the
      // trajectory so we got something like f(x) = -4x^2 + 4x
      projPos.z +=
          (float)(proj.ParabolaHeight *
                  (-4 * segmentRatio * segmentRatio + 4 * segmentRatio));
    }
    m.setPos(projPos);
    proj.setMatrix(m);
    // see if target reached
    if (impact || TimeInSec > proj.EndDate) {
      // stop projectiles and add them to the fx manager so that they are
      // removed when there are no particle lefts
      ...
      if (target) {
        ...
        if (ccl) {
          if (proj.AttackInfo.Intensity >= 1 &&
              proj.AttackInfo.Intensity <= MAGICFX::NUM_SPELL_POWER) {
            if (!proj.MagicResist) {
              if (proj.ImpactAspect) {
                CAttachedFX::CBuildInfo bi;
                for (uint k = 0; k < proj.ImpactAspect->FX.size(); ++k) {
                  CFXStickMode replaceStickMode =
                      proj.ImpactAspect->FX[k].Sheet->StickMode;
                  bi.StickMode = NULL;
                  if (proj.LocalizedImpact) {
                    nlassert(proj.ImpactAspect->Sheet);
                    // impact must have same localization than impact damages
                    // (when the sheet allows it)
                    ...
                    if (proj.ImpactAspect->Sheet->CanReplaceStickBone[k]) {
                      replaceStickMode.UserBoneName =
                          proj.ProjectileAimingPoint.UserBoneName;
                    }
                  ...
                  if (!fx->FX.empty()) {
                    /** Force to create each instance, even if not visible, so
                     * that the impact won't be deleted at start because it
                     * isn't visible.. Though this isn't needed in other cases,
                     * spell impact can be quite long. and must remain in sync
                     * with the action
                     */
                    ...
                    for (uint l = 0; l < 4; ++l) {
                      fx->FX.setUserParam(
                          l,
                          ImpactUserParams[proj.AttackInfo.Intensity - 1][l]);
                    }
                    ccl->attachFX(fx);
                  }
                }
                ...
                if (proj.PlayImpactAnim) {
                  ...
                  if (ccl->animState(MOVE) == CAnimationStateSheet::Idle ||
                      proj.ForcePlayImpact) {
              ...
              if (!fx->FX.empty()) {
                fx->FX
                    .forceInstanciate(); // TODO : if the fx isn't too long, is
                                         // it useful to force to create it ?
                ccl->attachFX(fx);
              }
            }
          }
        ...
        if (proj.AttackInfo.Intensity >= 1 &&
          ...
          if (!proj.MagicResist) {
            if (proj.ImpactAspect) {
              ...
              for (uint k = 0; k < proj.ImpactAspect->FX.size(); ++k) {
                NL3D::UParticleSystemInstance ps =
                    proj.ImpactAspect->FX[k].createMatchingInstance(
                        ImpactUserParams[proj.AttackInfo.Intensity - 1]);
                if (!ps.empty()) {
                  ps.forceInstanciate();
                  ps.setPos(impactPos);
            ...
            NL3D::UParticleSystemInstance ps =
                getResistFX(proj.AttackInfo.Intensity)
                    ->createMatchingInstance();
            if (!ps.empty()) {
              ps.forceInstanciate(); // TODO : resist can be can be quite short,
                                     // so may be not useful to force its
                                     // instanciation ?
              ps.setPos(impactPos);
              // add to fx manager for fx to be removed when finished
              FXMngr.addFX(ps);
      ...
      _ToShutDownProjectiles.splice(_ToShutDownProjectiles.begin(),
                                    _ActiveProjectiles, tmpIt);
    }
  }
}
...
// *****************************************************************************************
void CProjectileManager::evalFXPosition(
    const CFXStickMode *stickMode, CEntityCL &entity, NLMISC::CVector &result,
  ...
  if (!stickMode) {
    ...
    case CFXStickMode::UserBone: {
      if (entity.skeleton()) {
        sint boneID = entity.skeleton()->getBoneIdByName(
            NLMISC::CStringMapper::unmap(stickMode->UserBoneName));
        if (boneID != -1) {
          ...
          nlwarning(
              "Invalid bone : %s. Using entity pos instead",
              NLMISC::CStringMapper::unmap(stickMode->UserBoneName).c_str());
        }
      } else {
        nlwarning(
            "Can't retrieve entity skeleton. Using entity pos instead of bone "
            "pos",
            NLMISC::CStringMapper::unmap(stickMode->UserBoneName).c_str());
      }
...
sint CProjectileManager::CProjectile::getImpactBoneID(CEntityCL &target) {
  ...
  if (!target.skeleton())
    return -1;
  sint TargetBoneID = target.skeleton()->getBoneIdByName(
      NLMISC::CStringMapper::unmap(ProjectileAimingPoint.UserBoneName));
  return TargetBoneID;
}
  ...
  if (target) {
    if (ProjectileAimingPoint.Mode == CFXStickMode::UserBone ||
      ...
      if (!target->getLastClip()) {
        ...
        if (impactBoneID != -1) {
          if (target->skeleton() &&
              target->skeleton()->isBoneComputed(impactBoneID)) {
            ...
            if (TargetBoneRelativePosBlendFactor == 0.f) {
              ...
              // express last clipped position in world (taking in account the
              // ne orientation & position of model)
              ...
              clippedPos.set(
                  newEntityPos.x + front.y * relPos.x - front.x * relPos.y,
                  newEntityPos.y + front.x * relPos.x + front.y * relPos.y,
                  newEntityPos.z + relPos.z);
              // do the blend
              const NLMISC::CVector &visiblePos =
                  bone.getLastWorldMatrixComputed().getPos();
              ...
              TargetBoneRelativePosBlendFactor -=
                  DT * BLEND_FATCOR_DECREASE_OVER_TIME;
              if (TargetBoneRelativePosBlendFactor <= 0.f)
                TargetBoneRelativePosBlendFactor = 0.f;
            }
      ...
      if (!targetBoneVisible) {
        // the target bone is not visible -> use last computed bone position
        // (and make it relative to the parent skeleton)
        if (TargetBoneVisibleLastFrame) {
          // the relative position of the bone in its skeleton hasn't been
          // computed yet (because the bone was visible at last frame or because
          // it is the first visible frame)
          const NLMISC::CVector &front = LastEntityOrientation;
          const NLMISC::CVector deltaPos =
              impactPos - target->lastFramePos().asVector();
          // we except that front vector of the entity is normed and (thus its z
          // is 0)
          ClippedTargetBoneRelativePos.set(
              front.y * deltaPos.x - front.x * deltaPos.y,
              deltaPos.x * front.x + deltaPos.y * front.y, deltaPos.z);
        }
        // Snap the target model if necessary. Note that the skeleton may be
        // visible the bone may have been not computed (because of clod)
        if (target->getLastClip()) {
          target->snapToGround();
        }
      ...
      if (target->getLastClip()) {
        target->snapToGround(); // TODO : avoid to do this each frame for
                                // targetted non-visible entities ?
      }
...
static void convertFreeFXToAttachedFX(NL3D::UParticleSystemInstance instance,
                                      CCharacterCL &holder, float timeOut,
...
void CProjectileManager::CProjectile::shutDown(CCharacterCL *target) {
  ...
  for (uint k = 0; k < MaxNumFX; ++k) {
    if (!FX[k].empty()) {
      ...
      // must set user matrix of target, if an emitter emit 'on death' using
      // 'user' matrix
      if (target) {
      ...
      if (!FX[k].removeByID(NELID("STOP")) &&
          !FX[k].removeByID(NELID("main"))) {
        FX[k].activateEmitters(false);
        // nlwarning("Projectile with a particle system that has no 'STOP'
        // emitter");
        if (target) {
          convertFreeFXToAttachedFX(FX[k], *target, FX_ERROR_TIMEOUT,
                                    ProjectileAspect ? &ProjectileAspect->FX[k]
                                                     : NULL,
                                    ProjectileAimingPoint);
        } else {
          FXMngr.addFX(
              FX[k], FX_ERROR_TIMEOUT); // use  short timeout to avoid problems
        }
      } else {
        if (target) {
          convertFreeFXToAttachedFX(FX[k], *target, FX_DEFAULT_TIMEOUT,
                                    ProjectileAspect ? &ProjectileAspect->FX[k]
                                                     : NULL,
                                    ProjectileAimingPoint);
        } else {
          FXMngr.addFX(
              FX[k], FX_DEFAULT_TIMEOUT); // gives a time out to avoid remaining
                                          // particles if the fx main emitter
                                          // has not been flaged with 'STOP'
        }
      }
      FX[k] = NULL;
    }
...
void CProjectileManager::entityRemoved(uint slot) {
  // TODO nico : adding anchor objects  (that can be "CRefPtr'ed" ...) to define
  // rays / projectile paths would be a more elegant solution now (and a
  // CAnchorRef object to dlte anchor automatically)
  ...
  if (!entity)
    return;
  // projectile queue.
  for (std::multiset<CProjectileBuild>::iterator it = _ProjectileQueue.begin();
       it != _ProjectileQueue.end(); ++it) {
    // note : we do not change field that would modify ordering in the set (but
    // const_cast is needed there anyway ..)
    CProjectileBuild &pb = const_cast<CProjectileBuild &>(*it);
    if (it->TargeterInfo.Slot == slot) {
      // eval start pos
      evalFXPosition(
          &it->TargeterInfo.StickMode, *entity,
          pb.TargeterInfo
              .DefaultPos); // note : insertion in the set depends not on this
                            // field, hence the const_cast
      pb.TargeterInfo.Slot = CLFECOMMON::INVALID_SLOT;
    }
    if (it->Target.Slot == slot) {
      // eval target pos
      evalFXPosition(
          &pb.ProjectileAimingPoint, *entity,
          pb.Target.TargetPos); // note : insertion in the set depends not on
                                // this field, hence the const_cast
      pb.TargeterInfo.Slot = CLFECOMMON::INVALID_SLOT;
    }
  }
  // currently active projectile.
  for (std::list<CProjectile>::iterator it = _ActiveProjectiles.begin();
       it != _ActiveProjectiles.end(); ++it) {
    if (it->Target.Slot == slot) {
      // eval target pos
