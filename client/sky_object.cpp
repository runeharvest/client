...
// *************************************************************************************************
void CSkyObject::CColorInfo::init(
    const CSkyObjectSheet::CColorInfoSheet &ci,
...
// *************************************************************************************************
CRGBA CSkyObject::CColorInfo::computeColor(float dayPart, float weatherLevel,
                                           CRGBA fogColor) {
  switch (Mode) {
  case Unused:
...
void CSkyObject::CColorGradientInfo::init(
    const CSkyObjectSheet::CColorGradientInfoSheet &cgis,
    std::map<std::string, CBitmap *> &bitmapByName,
    std::vector<CBitmap *> &builtBitmaps) {
  ...
  for (uint k = 0; k < cgis.WeatherToGradient.size(); ++k) {
    bool alreadyBuilt;
    WeatherToGradient[k] = buildSharedBitmap(
        cgis.WeatherToGradient[k], bitmapByName, builtBitmaps, alreadyBuilt);
    if (WeatherToGradient[k]) {
      if (!WeatherToGradient[k]->convertToType(CBitmap::RGBA)) {
        // can't use bitmap..
        WeatherToGradient[k] = NULL; // don't do a delete here because it'is
                                     // 'builtBitmaps' that has ownership
      } else {
        if (!alreadyBuilt) {
        ...
        if (lastSeenBitmap) {
          if (WeatherToGradient[k]->getWidth() != lastSeenBitmap->getWidth() ||
              WeatherToGradient[k]->getHeight() !=
                  lastSeenBitmap->getHeight()) {
...
static void dumpGrad(CBitmap &bm)
{
        ...
        {
                nlinfo("(r, g, b, a) = (%d, %d, %d, %d)", pixs[k].R, pixs[k].G,
pixs[k].B, pixs[k].A);
        }
}
*/
...
// *************************************************************************************************
void CSkyObject::CColorGradientInfo::setup(NL3D::UInstance instance,
                                           float dayPart, float weatherLevel,
                                           CBitmap &gradientCache,
                                           CBitmap &gradientCacheBlurred) {
  if (instance.empty())
    return;
  if (WeatherToGradient.empty())
    return;
  clamp(dayPart, 0.f, 1.f);
  clamp(weatherLevel, 0.f, 1.f);
  // takes 2 closest bitmaps to do the blend
  uint bm0Index = std::min(uint(weatherLevel * WeatherToGradient.size()),
                           uint(WeatherToGradient.size() - 1));
  uint bm1Index = std::min(uint(weatherLevel * WeatherToGradient.size() + 1),
                           uint(WeatherToGradient.size() - 1));
  ...
  if (bm0->getWidth() != bm1->getWidth() ||
      bm0->getHeight() != bm1->getHeight())
    return;
  ...
  Slice0[0].blend(Slice0[0], Slice0[1],
                  (uint)(256 * fmodf(dayPart * bm0->getHeight(), 1.f)), true);
  ...
  Slice1[0].blend(Slice1[0], Slice1[1],
                  (uint)(256 * fmodf(dayPart * bm0->getHeight(), 1.f)), true);
  Slice0[0].blend(
      Slice0[0], Slice1[0],
      (uint)(256 * fmodf(weatherLevel * WeatherToGradient.size(), 1.f)), true);
  // see if blended result differs from cache, if so, update texture of the
  // instance
  ...
  if (!std::equal(newGrad, newGrad + bm0->getWidth(), oldGrad)) {
    ...
    // build a blurred version of the gradient cache (improve quality of
    // gradient)
    ...
    *destPix++ = *srcPix++;
    ...
    while (srcPix != lastSrcPix) {
      destPix->R = (uint8)(((uint16)srcPix[-1].R + (uint16)srcPix->R +
                            (uint16)srcPix[1].R) *
                               (256 / 3) >>
                           8);
      destPix->G = (uint8)(((uint16)srcPix[-1].G + (uint16)srcPix->G +
                            (uint16)srcPix[1].G) *
                               (256 / 3) >>
                           8);
      destPix->B = (uint8)(((uint16)srcPix[-1].B + (uint16)srcPix->B +
                            (uint16)srcPix[1].B) *
                               (256 / 3) >>
                           8);
      destPix->A = (uint8)(((uint16)srcPix[-1].A + (uint16)srcPix->A +
                            (uint16)srcPix[1].A) *
                               (256 / 3) >>
                           8);
      ++destPix;
    ...
    *destPix++ = *srcPix++;
    ...
    for (uint k = 0; k < numMaterials; ++k) {
      ...
      instance.getMaterial(k).setTextureMem(
          TargetTextureStage, &gradientCacheBlurred.getPixels()[0],
          gradientCacheBlurred.getWidth() * gradientCacheBlurred.getHeight() *
              sizeof(uint32),
          false, false, gradientCacheBlurred.getHeight(),
          gradientCacheBlurred.getWidth());
      // clamp on v coordinate
      instance.getMaterial(k).setWrapT(TargetTextureStage,
...
void CSkyObject::init(const CSkyObjectSheet::CVersionSheet &sheet,
                      ...
                      std::map<std::string, CBitmap *> &bitmapByName,
                      std::vector<CBitmap *> &builtBitmaps,
                      bool visibleInMainScene, bool visibleInEnvMap) {
  if (instance.empty())
    return;
  Instance = instance;
  ...
  for (uint k = 0; k < SKY_MAX_NUM_STAGE; ++k) {
    ...
    if (!sheet.OffsetUBitmap[k].empty()) {
      OffsetUBitmap[k] = buildSharedBitmap(sheet.OffsetUBitmap[k], bitmapByName,
                                           builtBitmaps, alreadyBuilt);
    }
    if (!sheet.OffsetVBitmap[k].empty()) {
      OffsetVBitmap[k] = buildSharedBitmap(sheet.OffsetVBitmap[k], bitmapByName,
                                           builtBitmaps, alreadyBuilt);
    }
  }
  ...
  std::copy(sheet.OffsetFactor, sheet.OffsetFactor + SKY_MAX_NUM_STAGE,
            OffsetFactor);
  for (uint k = 0; k < SKY_MAX_NUM_FX_USER_PARAMS; ++k) {
    if (!sheet.FXUserParamBitmap[k].empty()) {
      bool alreadyBuilt;
      FXUserParams[k] = buildSharedBitmap(
          sheet.FXUserParamBitmap[k], bitmapByName, builtBitmaps, alreadyBuilt);
    }
  }
...
// *************************************************************************************************
bool CSkyObject::setup(const CClientDate &date,
                       const CClientDate &animationDate, float numHoursInDay,
                       float weatherLevel, CRGBA fogColor, bool envMapScene) {
  if (Instance.empty())
    return false;
  Active = true;
  ...
  if (DiffuseColor.Mode != Unused) {
    ...
    if (newDiffuseColor != LastDiffuseColor) {
      ...
      if (!PS.empty()) {
        ...
        // PS are hiden / shown, so must unfreeze hrc. (adding an isntance group
        // causes hrc to be frozen)
        PS.unfreezeHRC();
      } else {
    ...
    if (RefColor == DiffuseColorRef) // if this is the ref color, then the
                                     // object is not visible if alpha is 0
    {
  ...
  if (ParticleEmitters.Mode != Unused) {
    CRGBA newParticleEmittersColor =
        ParticleEmitters.computeColor(dayPart, weatherLevel, fogColor);
    ...
    if (RefColor ==
        ParticleEmittersColorRef) // if this is the ref color, then the object
                                  // is not visible if alpha is 0
    {
      if (LastParticleEmittersColor == CRGBA::Black) {
        if (!PS.hasParticles()) // can deactivate PS only when all particles are
                                // off
        {
          Active = false;
        }
  ...
  for (uint k = 0; k < SKY_MAX_NUM_STAGE; ++k) {
    if (ConstantColor[k].Mode != Unused) {
      CRGBA newConstantColor =
          ConstantColor[k].computeColor(dayPart, weatherLevel, fogColor);
  ...
  double animTime =
      animationDate.Hour + (double)animationDate.Day * (double)numHoursInDay;
  if (PS.empty()) {
    ////////////////////
    // gradient setup //
    ////////////////////
    ColorGradient.setup(Instance, dayPart, weatherLevel, GradientCache,
    ...
    for (uint k = 0; k < SKY_MAX_NUM_STAGE; ++k) {
      if (TexPanner[k].U != 0.f || TexPanner[k].V != 0.f ||
        ...
        if (OffsetUBitmap[k]) {
          offset.x +=
              OffsetFactor[k].U *
              OffsetUBitmap[k]->getColor(dayPart, weatherLevel, true, false).R;
        }
        if (OffsetVBitmap[k]) {
          offset.y +=
              OffsetFactor[k].V *
              OffsetVBitmap[k]->getColor(dayPart, weatherLevel, true, false).R;
        }
        CMatrix mat;
        mat.setPos(offset);
        for (uint l = 0; l < numMaterials; ++l) {
    ...
    for (uint k = 0; k < SKY_MAX_NUM_FX_USER_PARAMS; ++k) {
      if (FXUserParams[k]) {
        CRGBA color =
            FXUserParams[k]->getColor(dayPart, weatherLevel, true, false);
