...
public:
  ...
  /**
   * Build the animation set according to the georges file. (read in the sheet
   * manager)
   */
  ...
  /**
   * When in walk animation, after this speed the animation should switch in run
   * mode. \return double : a speed.
   */
  double speedToRun() const { return _SpeedToRun; }
  /**
   * When in run animation, before this speed the animation should switch in
   * walk mode. \return double : a speed.
   */
  double speedToWalk() const { return _SpeedToWalk; }

  /// Return the animation state from its Id. Your id must be valid, no check
