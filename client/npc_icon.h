...
public:
  /// Constructor
  CNPCMissionGiverDesc()
      : _MissionGiverState(NPC_ICON::AwaitingFirstData),
        _LastUpdateTimestamp(0), _IsDescTransient(true), _HasChanged(false) {}

  // Current assignment operator: bitwise copy

  /// Return the current state
  NPC_ICON::TNPCMissionGiverState getState() const {
    return _MissionGiverState;
  }

  /// Return the time of last update (only valid if getState() !=
  /// NPC_ICON::AwaitingFirstData)
  NLMISC::TGameCycle getLastUpdateTimestamp() const {
  ...
  /// Called on receival of update for this NPC. Return true if something has
  /// changed.
  bool
  updateMissionAvailabilityForThisChar(NPC_ICON::TNPCMissionGiverState state);
 ...
 * Tells which icons to display above NPCs, using data from either the server
 * database (for mission steps) or requesting and caching data from the server
 * (for mission givers).
 */
class CNPCIconCache {
public:
   ...
   * The spotId tells which small icon, and in which render layer, to display in
   * the radar.
   */
  class CNPCIconDesc {
  public:
    void init(const std::string &main, const std::string &over,
              CViewRadar::TRadarSpotId spotId = CViewRadar::Std) {
      _TextureMain = main;
      _TextureOver = over;
      _SpotId = spotId;
    }
  ...
  /**
   * Return the filename of the icon to display for the specified NPC, using the
   * following precedence:
   * 1. Icon for the case when the character has a mission step to do with the
   * NPC (Step icon)
   * 2. Icon for the case when the character may or may not take a mission with
   * the NPC (Auto or List icon)
   * 3. No mission related with this NPC (empty strings returned)
   */
  const CNPCIconCache::CNPCIconDesc &getNPCIcon(const CEntityCL *entity,
  ...
  /// Request an update about proposed missions of all qualifying npcs in
  /// vision, from the server
...
protected:
  ...
  /// Request an update about proposed missions of the specified npc, from the
  /// server.
  void queryMissionGiverData(TNPCIconCacheKey npcIconCacheKey);
  friend struct commands_queryMissionGiverDataClass;

  /** Request an update about proposed missions on all qualifying visible NPCs,
   * from the server. If olderThan==0, all qualifying (i.e. that have missions)
   * NPCs are included (and a request is sent even for transient descriptions),
   * whereas with a positive delay, only NPCs with information old than
   * Now-olderThan are included and transient descriptions are ignored.
   */
   ...
   * both missions actors (Step icon) and mission givers' proposed missions
   * (Auto/List icon):
  ...
  /// Update visible NPCs related to missions in progress, regarding missions
  /// actors (Step icon)
  ...
  /// Called on receival of update for this NPC. Return true if something has
  /// changed.
  bool onReceiveMissionAvailabilityForThisChar(
...
private:
  ...
  /// List of NPCs involved in current goals (typically, contains 0 to 3
  /// elements)
  CSmallKeyList _KeysOfCurrentGoals;

  /// List of NPCs for which the description is to be requested at the current
  /// update
