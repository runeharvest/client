...
public:
  enum TState {
    ...
    /// ingame state
    st_ingame,
    /// leave the current shard (the exit action progress, Far TP part 1.1;
    /// Server Hop part 1-2)
    st_leave_shard,
    ...
    st_reconnect_fs,
    /// after reconnecting, bypass character selection ui & select the same
    /// character (Far TP & Server Hop part 3.2)
    ...
    st_reconnect_error,
    /// Rate a ring session. should pop a web windows pointing the rate session
    /// page
    st_rate_session,
    /// create account
    st_create_account,
  ...
  enum TEvent {
    ...
    /// the lua script (from web usually) starts the client connection to a
    /// shard
    ev_connect,
...
public:
  ...
  // Note that, because it is a co-task, restarting the state machine after it
  // has finished require to delete then recreate this object
  CLoginStateMachine() : _CurrentState(st_start) {}
...
public:
  ...
  enum TJoinMode { LaunchEditor, JoinSession, JoinMainland, NbJoinModes };

  CFarTP()
      : _Reason(NULL), _SessionIdToJoinFast(0), _InGameMode(false),
        _PreviousR2EdEnabled(false), _ReselectingChar(false),
        _IsServerHopEmbedded(false), _HookedForEditor(false),
        _DSSDown(false) /*, _SelfReconnecting(false)*/ {}

  // Get the address of a front-end service via http, then begin Far TP
  bool requestFarTPToSession(TSessionId sessionId, uint8 charSlot,
                             CFarTP::TJoinMode joinMode,
                             bool bailOutIfSessionVanished);

  // Ask the EGS to pop the top position if applicable
  void
  requestReturnToPreviousSession(TSessionId rejectedSessionId = TSessionId(0));

  // Store the result of a JoinSession request for the next disconnection
  // request
  void setJoinSessionResult(TSessionId sessionId,
                            const CSecurityCode &securityCode);

  // Tell to ignore the next Far TP request, it will be replaced by a Far TP to
  // the edition session (allows to shortcut anim-mainland-edition by
  // anim-edition)
  ...
  bool isLeavingEGS() const {
    return (LoginSM.getCurrentState() == CLoginStateMachine::st_leave_shard);
  }
  ...
  void setMainLoopEntered() {
    setIngame();
    _ReselectingChar = false;
  }
  void beginEmbeddedServerHop() {
    _IsServerHopEmbedded = true;
    setOutgame();
    _ReselectingChar = false;
  }
  void endEmbeddedServerHop() {
    _IsServerHopEmbedded = false;
    setIngame();
...
private:
  ...
  void joinSessionResult(uint32 userId, TSessionId sessionId, uint32 result,
  ...
  TSessionId
      _SessionIdToJoinFast; // if non 0, fast disconnection allowed to session
  ...
  bool _IsServerHopEmbedded; // allows to embed a server hop inside a char
                             // reselect (e.g. click on Launch Editor of PLAY
                             // with a character on a remote shard)
  bool _HookedForEditor; // allows to Far TP to editor instead of following the
                         // next Far TP request
  bool _DSSDown;         // remembers a DSSDown event if occurs during a FarTP
  //	bool _SelfReconnecting; // allows to disconnect after character
  //selection and reconnect until the player can choose another character
};
