...
public:
  ...
  CQuicConnectionImpl(CQuicConnection *self)
      : Self(self), Api(NULL), Registration(NULL), Configuration(NULL),
        Connection(NULL), DesiredStateChange(NoChange), BufferWrites(0),
        BufferReads(0), State(CQuicConnection::Disconnected),
        ConnectedFlag(false), ShuttingDownFlag(false), ShutdownFlag(false),
        ReceiveEnable(0), MaxSendLength(0), SendBusy(false), SentCount(0) {}
  ...
  // It is filled by the receive callback (any thread), and emptied by the
  // receiveDatagram call (main thread only).
  CSynchronizedFIFO Buffer; // (any thread)
  CAtomicInt BufferWrites;  // (any thread)
  int BufferReads; // If BufferReads == BufferWrites, then there's no more
                   // datagrams (main thread only)

  // Current state of the connection
  CQuicConnection::TState
      State; // Updated by the update call (main thread only)
  CAtomicFlag
      ConnectedFlag; // Set by the callback thread, checked by update (main)
  CAtomicFlag ShuttingDownFlag; // Set by the callback thread, checked by the
                                // update call (main thread), reset by update on
                                // complete disconnection
  CAtomicFlag
      ShutdownFlag; // Set by the callback thread, checked by the update call
                    // (main thread), reset by update on complete disconnection

  bool ReceiveEnable; // Set when datagrams may be received, otherwise they'll
                      // be ignored, this is to avoid stray messages after
                      // disconnecting (set and accessed by main thread only)
  CAtomicInt
      MaxSendLength; // Set by the callback thread, used to check if we can send
...
void CQuicConnectionImpl::init() {
  ...
  // TestAndSet has acquire semantics, clear has release semantics, so we use
  // clear to flag the events
  ...
  // Open library
  QUIC_STATUS status =
      MsQuicOpenVersion(QUIC_API_VERSION_2, (const void **)&MsQuic);
...
void CQuicConnectionImpl::connect() {
  ...
  // Protocol feature levels, corresponds to Ryzom Core release versions, keep
  // datagram-only support to give users and server owners the option to keep
  // bandwidth restricted 4.1: Only datagram support (restricted bandwidth, same
  // behaviour as UDP) 4.2?: Add up to 4 unidirectional streams from the server
  // to client to send long impulses (keep bandwidth from client restricted)
  // (DB_INIT, STRING, STRING_MANAGER, MODULE_GATEWAY) 4.3?: Add a single
  // bidirectional stream opened by the client for the scenario editor gateway
  // (more efficient MODULE_GATEWAY replacement)
  static const CStringView protocolName41 = "ryzomcore/4.1";
  static const QUIC_BUFFER alpn = {(uint32)protocolName41.size(),
                                   (uint8 *)protocolName41.data()};

  // Configuration, initialized in start, but destroyed on release only (may
  ...
  if (!m->Configuration) {
    ...
    status =
        MsQuic->ConfigurationOpen(m->Registration, &alpn, 1, &settings,
    ...
    if (!ClientCfg.QuicCertValidation) {
      credConfig.Flags |=
          QUIC_CREDENTIAL_FLAG_NO_CERTIFICATE_VALIDATION; // Don't care for
                                                          // development
    }
    ...
    if (QUIC_FAILED(status)) {
      ...
      nlwarning("MsQuic->ConfigurationLoadCredential failed with status 0x%x",
                status);
      return;
    }
  }
  ...
  status = MsQuic->ConnectionOpen(m->Registration,
                                  CQuicConnectionImpl::connectionCallback,
                                  (void *)this, &m->Connection);
  if (QUIC_FAILED(status)) {
    close();
  ...
  // Start the connection (NOTE: The hostname lookup is a bit redundant here,
  // but it's okay I suppose...)
  status = MsQuic->ConnectionStart(m->Connection, m->Configuration,
                                   QUIC_ADDRESS_FAMILY_UNSPEC,
                                   nlUtf8ToMbcs(addr.hostname()), addr.port());
  if (QUIC_FAILED(status)) {
    close();
    nlwarning("MsQuic->ConnectionStart to %s failed with status 0x%x",
              addr.toStringLong().c_str(), status);
    return;
  }
  ...
  if (MsQuic && blocking) {
    try {
      ...
      if (INelContext::isContextInitialised()) {
        nlwarning("Exception while waiting for connection shutdown: %s",
                  e.what());
      }
    }
  }
...
void CQuicConnectionImpl::shutdown() {
  CQuicConnectionImpl *const m = this;
  if (m->Connection) {
    MsQuic->ConnectionShutdown(m->Connection,
...
void CQuicConnectionImpl::update() {
  ...
  if (!m->Connection && m->DesiredStateChange == CQuicConnectionImpl::Release) {
    nlassert(m->State == CQuicConnection::Disconnected);
    m->release();
...
void CQuicConnection::update() { m->update(); }
...
CQuicConnection::TState CQuicConnection::state() const { return m->State; }

uint32 CQuicConnection::maxSendLength() const { return m->MaxSendLength; }

QUIC_STATUS
#ifdef _Function_class_
_Function_class_(QUIC_CONNECTION_CALLBACK)
#endif
    CQuicConnectionImpl::connectionCallback(HQUIC connection, void *context,
                                            QUIC_CONNECTION_EVENT *ev) {
  ...
  case QUIC_CONNECTION_EVENT_CONNECTED: {
    ...
    nlinfo("Connected over QUIC protocol with ALPN '%s'",
           nlsvc(CStringView((const char *)ev->CONNECTED.NegotiatedAlpn,
                             ev->CONNECTED.NegotiatedAlpnLength)));
  ...
  case QUIC_CONNECTION_EVENT_SHUTDOWN_INITIATED_BY_TRANSPORT: {
    ...
    m->MaxSendLength = 0;
    if (INelContext::isContextInitialised()) {
      nlwarning("Shutdown initiated by transport, error code %llu, status 0x%x",
  ...
  case QUIC_CONNECTION_EVENT_DATAGRAM_RECEIVED:
    ...
    // YES PLEASE
    m->datagramReceived(ev->DATAGRAM_RECEIVED.Buffer->Buffer,
                        ev->DATAGRAM_RECEIVED.Buffer->Length);
    status = QUIC_STATUS_SUCCESS;
    break;
  case QUIC_CONNECTION_EVENT_DATAGRAM_STATE_CHANGED:
    m->MaxSendLength = ev->DATAGRAM_STATE_CHANGED.SendEnabled
                           ? ev->DATAGRAM_STATE_CHANGED.MaxSendLength
                           : 0;
    if (INelContext::isContextInitialised()) {
      nlinfo("Datagram state changed, max send length is now %i",
             (int)m->MaxSendLength);
    }
    status = QUIC_STATUS_SUCCESS;
    break;
  case QUIC_CONNECTION_EVENT_DATAGRAM_SEND_STATE_CHANGED:
    if (ev->DATAGRAM_SEND_STATE_CHANGED.State == QUIC_DATAGRAM_SEND_SENT &&
        (ptrdiff_t)ev->DATAGRAM_SEND_STATE_CHANGED.ClientContext ==
            (ptrdiff_t)m->SentCount.load()) {
...
//{
//	if (m->Connection && m->State && CQuicConnection::Connected && size <=
//m->MaxSendLength.load())
//	{
...
//		buf->Length = size;
//		QUIC_STATUS status = MsQuic->DatagramSend(m->Connection, buf, 1,
//QUIC_SEND_FLAG_NONE, this); 		if (QUIC_FAILED(status))
//		{
...
bool CQuicConnection::sendDatagramSwap(NLMISC::CBitMemStream &buffer,
                                       uint32 size) {
  if (m->Connection && m->State && CQuicConnection::Connected &&
      size <= m->MaxSendLength.load() && size <= buffer.length()) {
    ...
    QUIC_STATUS status = MsQuic->DatagramSend(
        m->Connection, &m->SendQuicBuffer, 1, QUIC_SEND_FLAG_NONE,
        (void *)(ptrdiff_t)m->SentCount.load());
    if (QUIC_FAILED(status)) {
      m->SendBusy.clear();
      nlwarning("DatagramSend failed with %d", status);
      return false;
...
bool CQuicConnection::isSupported() const { return true; }
...
CQuicConnection::TState CQuicConnection::state() const { return Disconnected; }

uint32 CQuicConnection::maxSendLength() const { return 0; }
...
bool CQuicConnection::datagramAvailable() const { return false; }

bool CQuicConnection::receiveDatagram(NLMISC::CBitMemStream &msgin) {
  return false;
}

bool CQuicConnection::isSupported() const { return false; }
