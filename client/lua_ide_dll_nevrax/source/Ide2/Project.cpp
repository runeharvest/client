...
CProject::~CProject() { RemoveProjectFiles(); }
...
void CProject::UpdateDirectories() {
  ...
  if (!PathIsRelative(m_strIntermediateDir)) {
    ...
    PathAppend(m_strIntermediateDirRoot.GetBuffer(MAX_PATH),
...
BOOL CProject::Load() {
  ...
  CFileDialog fd(TRUE, NULL, NULL, OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST,
                 "Project files (*.lpr)|*.lpr|All files (*.*)|*.*||",
                 AfxGetMainWnd());
...
BOOL CProject::Save() { return Save(m_strPathName); }
...
void CProject::SaveModified() {
  if (m_bModified) {
    if (AfxMessageBox("Your project has changed. Do you want to save it?",
                      MB_YESNO) == IDYES)
      Save();
  }
}
...
BOOL CProject::BuildOutputFiles() {
  ...
  long nFiles = m_files.GetSize();
  for (int i = 0; i < nFiles; ++i)
    if (theApp.FirstFileIsNewer(m_files[i]->GetOutputPathNameExt(),
                                GetDebugPathNameExt())) {
      pOutput->Write("Linking\n");

      strCmdLine.Format("\"%s\" -o \"%s\"",
                        theApp.GetModuleDir() + "\\" + "luac.exe",
                        GetDebugPathNameExt());
  ...
  if (theApp.FirstFileIsNewer(GetDebugPathNameExt(), GetReleasePathNameExt())) {
    strCmdLine.Format("\"%s\" -s -o \"%s\" \"%s\"",
                      theApp.GetModuleDir() + "\\" + "luac.exe",
                      GetReleasePathNameExt(), GetDebugPathNameExt());
  ...
  if (m_bGenerateListing &&
      theApp.FirstFileIsNewer(GetDebugPathNameExt(), GetListingPathNameExt())) {
...
const BOOL CProject::GetBreakPoint(const char *szFile, int nLine,
