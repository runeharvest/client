...
CMainFrame::CMainFrame() {
  m_hAccelNoProject = ::LoadAccelerators(theApp.m_hInstance,
                                         MAKEINTRESOURCE(IDR_ACCEL_NO_PROJECT));
  m_hAccelBuild =
      ::LoadAccelerators(theApp.m_hInstance, MAKEINTRESOURCE(IDR_ACCEL_BUILD));
  m_hAccelDebug =
      ::LoadAccelerators(theApp.m_hInstance, MAKEINTRESOURCE(IDR_ACCEL_DEBUG));
  m_hAccelDebugBreak = ::LoadAccelerators(
      theApp.m_hInstance, MAKEINTRESOURCE(IDR_ACCEL_DEBUG_BREAK));
...
BOOL CMainFrame::InitDockingWindows() {
  ...
  if (!m_wndWorkspace.Create(this, ID_VIEW_WORKSPACE, _T("Workspace"),
                             CSize(200, 100), CBRS_LEFT)) {
    TRACE0("Failed to create dialog bar m_wndWorkspace\n");
    return -1; // fail to create
  }

  if (!m_wndOutput.Create(this, ID_VIEW_OUTPUT, _T("Output"), CSize(300, 100),
                          CBRS_BOTTOM)) {
    TRACE0("Failed to create dialog bar m_wndOutput\n");
    return -1; // fail to create
  }

  if (!m_wndCallStack.Create(this, ID_VIEW_CALLSTACK, _T("Call Stack"),
                             CSize(300, 100), CBRS_RIGHT)) {
    TRACE0("Failed to create dialog bar m_wndCallStack\n");
    return -1; // fail to create
  }

  if (!m_wndLocals.Create(this, ID_VIEW_LOCALS, _T("Local Variables"),
                          CSize(200, 100), CBRS_BOTTOM)) {
    TRACE0("Failed to create dialog bar m_wndLocals\n");
    return -1; // fail to create
  }

  if (!m_wndWatches.Create(this, ID_VIEW_WATCHES, _T("Local Variables"),
                           CSize(200, 100), CBRS_BOTTOM)) {
...
void CMainFrame::OnFileSaveproject() { GetProject()->Save(); }

void CMainFrame::OnFileSaveprojectas() { GetProject()->SaveAs(); }

void CMainFrame::OnProjectAddFiles() { GetProject()->AddFiles(); }

void CMainFrame::OnProjectProperties() { GetProject()->Properties(); }
...
void CMainFrame::OnBuildClean() { m_project.CleanIntermediateAndOutputFiles(); }
...
LRESULT CMainFrame::DebugMessage(UINT nMsg, WPARAM wParam, LPARAM lParam) {
  ...
  case DMSG_WRITE_DEBUG:
    GetOutputWnd()
        ->GetOutput(COutputWnd::outputDebug)
        ->Write((const char *)wParam);
    break;
...
void CMainFrame::OnDebugGo() { m_debug.Go(); }
...
void CMainFrame::OnDebugStepinto() { m_debug.StepInto(); }

void CMainFrame::OnDebugStepover() { m_debug.StepOver(); }

void CMainFrame::OnDebugStepout() { m_debug.StepOut(); }

void CMainFrame::OnDebugRuntocursor() { m_debug.RunToCursor(); }

void CMainFrame::OnDebugBreak() { m_debug.Break(); }

void CMainFrame::OnDebugStopdebugging() { m_debug.Stop(); }
...
void CMainFrame::InitExternalDebug(const char *tmpProjectFile,
                                   lua_realloc_t reallocfunc,
                                   lua_free_t freefunc) {
...
void CMainFrame::OnHelpContactauthor() {
  ShellExecute(AfxGetMainWnd()->GetSafeHwnd(), "open",
               "mailto:rybak@gorlice.net.pl", NULL, NULL, SW_SHOWNA);
}

void CMainFrame::OnHelpLuahelppdf() {
  ShellExecute(AfxGetMainWnd()->GetSafeHwnd(), "open",
               theApp.GetModuleDir() + "\\docs\\refman-5.0.pdf", NULL, NULL,
               SW_SHOWNA);
}

void CMainFrame::OnHelpVisithomepage() {
  ShellExecute(AfxGetMainWnd()->GetSafeHwnd(), "open",
               "http://www.gorlice.net.pl/~rybak/luaide", NULL, NULL,
               SW_SHOWNA);
}

void CMainFrame::OnHelpLuahomepage() {
  ShellExecute(AfxGetMainWnd()->GetSafeHwnd(), "open", "http://www.lua.org",
               NULL, NULL, SW_SHOWNA);
}
...
void CMainFrame::OnNextDoc() {
  ...
  while (pos != NULL) {
    ...
    if (pDoc->GetView()->GetParentFrame() == activeWnd) {
      if (m_LastCycledDoc == NULL)
        m_LastCycledDoc = pDoc;
      if (pos == NULL) {
