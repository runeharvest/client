...
const CString &CLuaDoc::GetTitle() const {
  ...
  if (const_cast<CLuaDoc *>(this)->IsModified())
    m_TmpTitle += "*";
  return m_TmpTitle;
}
...
void CLuaDoc::CheckExternallyModified() {
  ...
  if (newModifTime > m_ModifTime) {
    ...
    if (theApp.GetMainFrame()->MessageBox((LPCTSTR)(GetPathName() + msg),
                                          (LPCTSTR)caption,
                                          MB_OKCANCEL) == IDOK) {
      CFile f;
...
CTime CLuaDoc::GetModifTime() const { return GetModifTime(GetPathName()); }
...
#ifdef _DEBUG
void CLuaDoc::AssertValid() const { CDocument::AssertValid(); }

void CLuaDoc::Dump(CDumpContext &dc) const { CDocument::Dump(dc); }
#endif //_DEBUG
...
BOOL CLuaDoc::IsInProject() { return GetProjectFile() != NULL; }
