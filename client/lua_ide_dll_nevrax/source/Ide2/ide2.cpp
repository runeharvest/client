...
BOOL CIdeApp::InitInstance() {
  ...
  m_pLuaTemplate = new CMultiDocTemplateEx(
      IDR_IDE2TYPE, RUNTIME_CLASS(CLuaDoc),
...
CMainFrame *CIdeApp::GetMainFrame() { return (CMainFrame *)m_pMainWnd; }
...
CLuaView *CIdeApp::LoadProjectFilesView(CProjectFile *pPF) {
  CLuaDoc *pDoc =
      (CLuaDoc *)m_pLuaTemplate->OpenDocumentFile(pPF->GetPathName(), TRUE);
  if (pDoc)
    return pDoc->GetView();
  else
    return NULL;
