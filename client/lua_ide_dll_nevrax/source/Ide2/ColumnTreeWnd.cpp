...
BOOL CColumnTreeWnd::OnEraseBkgnd(CDC *pDC) { return TRUE; }
...
void CColumnTreeWnd::OnHScroll(UINT nSBCode, UINT nPos,
...
int CColumnTreeWnd::OnCreate(LPCREATESTRUCT lpCreateStruct) {
  ...
  m_Tree.Create(WS_CHILD | WS_VISIBLE | TVS_NOHSCROLL | TVS_NOTOOLTIPS, CRect(),
                this, TreeID);
  m_Header.Create(WS_CHILD | WS_VISIBLE | HDS_FULLDRAG, CRect(), this,
                  HeaderID);
  ...
  if (hComCtlDll) {
    ...
    PFNDLLGETVERSION pfnDllGetVersion =
        (PFNDLLGETVERSION)GetProcAddress(hComCtlDll, "DllGetVersion");
