...
HWND CDebugger::GetMainWnd() { return theApp.GetMainFrame()->m_hWnd; }
...
BOOL CDebugger::Prepare(lua_realloc_t reallocfunc, lua_free_t freefunc) {
  ...
  if (pProject->PositionBreakPoints())
    AfxMessageBox("One or more breakpoints are not positioned on valid lines. "
...
void CDebugger::LineHook(const char *szFile, int nLine) {
  ...
  if (m_ExternalDebugging) {
    if (theApp.GetMainFrame()->GetProject()->IsBreakPointPossibleAtLine(
            nLine)) {
      hasBreakPoint =
          theApp.GetMainFrame()->GetProject()->HasBreakPoint(szFile, nLine);
    }
  } else {
    hasBreakPoint = ::SendMessage(m_hWndMainFrame, DMSG_HAS_BREAKPOINT,
                                  (WPARAM)szFile, (LPARAM)nLine);
  }

  if (m_nMode == DMOD_STEP_INTO || m_nMode == DMOD_BREAK ||
      (m_nMode == DMOD_STEP_OVER && m_nLevel <= 0) ||
      (m_nMode == DMOD_STEP_OUT && m_nLevel < 0) ||
      (m_nMode == DMOD_RUN_TO_CURSOR &&
       m_strPathName.CompareNoCase(szFile) == 0 && m_nLine == nLine)) {
    if (m_EvaluatingCondition)
      return;
...
void CDebugger::DebugBreak(const char *szFile, int nLine) {
  ...
  ::SendMessage(m_hWndMainFrame, DMSG_GOTO_FILELINE, (WPARAM)szFile,
                (LPARAM)nLine);
...
void CDebugger::Break() { m_nMode = DMOD_BREAK; }

void CDebugger::ClearStackTrace() {
  ::SendMessage(m_hWndMainFrame, DMSG_CLEAR_STACKTRACE, 0, 0);
}

void CDebugger::AddStackTrace(const char *szDesc, const char *szFile,
                              int nLine) {
  StackTrace st;
  st.szDesc = szDesc;
  st.szFile = szFile;
...
void CDebugger::StackLevelChanged() { m_lua.DrawLocalVariables(); }

void CDebugger::ClearLocalVariables() {
  ::SendMessage(m_hWndMainFrame, DMSG_CLEAR_LOCALVARIABLES, 0, 0);
}

void CDebugger::AddLocalVariable(const char *name, const char *type,
                                 const char *value) {
  Variable var;
  var.szName = name;
  var.szType = type;
...
void CDebugger::AddGlobalVariable(const char *name, const char *type,
                                  const char *value) {
  Variable var;
  var.szName = name;
  var.szType = type;
...
void CDebugger::Execute() {
  CString strInterpreter =
      /*"F:\\a ja tak\\lubie cie bardzo\\lua.exe";*/ theApp.GetModuleDir() +
      "\\" + "lua.exe";
  CProject *pProject = ((CMainFrame *)AfxGetMainWnd())->GetProject();

  _spawnl(_P_NOWAIT, strInterpreter, strInterpreter,
          "\"" + pProject->GetDebugPathNameExt() + "\"", NULL);
}
