...
void CMDIClientWnd::ResetMenu() {
  ...
  if (pFrame->GetMode() != pFrame->modeNoProject) {
    pMenu->InsertMenu(nPos, MF_BYPOSITION | MF_POPUP | MF_ENABLED,
                      (UINT)m_menuProject.GetSubMenu(0)->GetSafeHmenu(),
                      "&Project");
    ++nPos;
  }
  ...
  if (pFrame->GetMode() == pFrame->modeBuild)
    pMenu->InsertMenu(nPos, MF_BYPOSITION | MF_POPUP | MF_ENABLED,
                      (UINT)m_menuBuild.GetSubMenu(0)->GetSafeHmenu(),
                      "&Build");
  else if (pFrame->GetMode() == pFrame->modeDebug ||
           pFrame->GetMode() == pFrame->modeDebugBreak)
    pMenu->InsertMenu(nPos, MF_BYPOSITION | MF_POPUP | MF_ENABLED,
                      (UINT)m_menuDebug.GetSubMenu(0)->GetSafeHmenu(),
                      "&Debug");
}
