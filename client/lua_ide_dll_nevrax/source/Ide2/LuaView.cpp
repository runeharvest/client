...
void CLuaView::OnGotoLine() {
  ...
  if (gt.DoModal() == IDOK) {
    if (gt.m_Line == 0)
      gt.m_Line = 1;
    if ((int)gt.m_Line > m_editor.GetLineCount())
      gt.m_Line = m_editor.GetLineCount();
...
void CLuaView::find(bool forward, bool next) {
  CFindText &ft = theApp.GetMainFrame()->m_FindText;
  if (ft.m_TextToFind.GetLength() == 0)
    return;
  int startPos, endPos;
  int searchStartPos;
  int selStart, selEnd;
  ...
  m_editor.FindText((LPCTSTR)ft.m_TextToFind, ft.m_MatchCase, ft.m_WholeWord,
                    ft.m_RegExp, forward, searchStartPos, startPos, endPos);
  if (startPos != -1) {
    m_editor.SetSelection(startPos, endPos);
  }
}

void CLuaView::OnFindNext() { find(true, true); }

void CLuaView::OnFindPrevious() { find(false, true); }
...
#ifdef _DEBUG
void CLuaView::AssertValid() const { CView::AssertValid(); }

void CLuaView::Dump(CDumpContext &dc) const { CView::Dump(dc); }
...
void CLuaView::OnCheckSyntax() {
  ...
  CString errors;
  if (!theApp.GetMainFrame()->GetDebugger()->GetLuaHelper().CheckSyntax(
          code, errors)) {
    int pos = errors.Find(":");
    int errLine;
    if (pos != -1) {
    ...
    theApp.GetMainFrame()->MessageBox((LPCTSTR)errors, (LPCTSTR)caption,
