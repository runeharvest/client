...
//
CModuleVersion::~CModuleVersion() { delete[] m_pVersionInfo; }
...
BOOL CModuleVersion::GetFileVersionInfo(LPCTSTR modulename) {
  ...
  *(VS_FIXEDFILEINFO *)this = *(VS_FIXEDFILEINFO *)lpvi;
  ...
  if (VerQueryValue(m_pVersionInfo, "\\VarFileInfo\\Translation", &lpvi,
                    &iLen) &&
      iLen >= 4) {
...
CString CModuleVersion::GetValue(LPCTSTR lpKeyName) {
  ...
  if (m_pVersionInfo) {
    ...
    //
    CString query;
    query.Format(_T("\\StringFileInfo\\%04x%04x\\%s"), m_translation.langID,
                 m_translation.charset, lpKeyName);

    LPCTSTR pVal;
    UINT iLenVal;
    if (VerQueryValue(m_pVersionInfo, (LPTSTR)(LPCTSTR)query, (LPVOID *)&pVal,
                      &iLenVal)) {
