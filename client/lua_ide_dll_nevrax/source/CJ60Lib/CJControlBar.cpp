...
//			1.01	20 Oct 1998	- Fixed problem with gripper and
//buttons 								  disappearing when docking toggled. Overloaded 								  IsFloating() method from
//base class. 			1.02    22 Nov 1998 - Modified set cursor to display normal size
//								  cursor when
//static linked. 			2.00    12 Jan 1999 - Total class re-write, added
//multiple/side-by-side 								  controlbar docking! No longer uses CSizingControlBar
//								  base class.
// ==========================================================================
//
/////////////////////////////////////////////////////////////////////////////
...
CCJControlBar::CCJControlBar() {
  ...
  m_ImageList.Create(IDB_BUTTON_IMAGES, 10, 1, RGB(255, 255, 255));
...
CCJControlBar::~CCJControlBar() { m_ImageList.DeleteImageList(); }
...
BOOL CCJControlBar::Create(CWnd *pParentWnd, UINT nID, LPCTSTR lpszWindowName,
                           CSize sizeDefault, DWORD dwStyle) {
  ASSERT_VALID(pParentWnd); // must have a parent

  // Set initial control bar style.
  SetBarStyle(dwStyle & CBRS_ALL | CBRS_HIDE_INPLACE | CBRS_SIZE_DYNAMIC |
              CBRS_FLOAT_MULTI);
  ...
  return CWnd::Create(AFX_WNDCONTROLBAR, lpszWindowName, dwStyle,
...
void CCJControlBar::OnNcPaint() { EraseNonClient(); }
...
BOOL CCJControlBar::OnSetCursor(CWnd *pWnd, UINT nHitTest, UINT message) {
  return (!m_bOkToDrag && !m_bDragging)
             ? CControlBar::OnSetCursor(pWnd, nHitTest, message)
             : FALSE;
}
...
void CCJControlBar::OnLButtonUp(UINT nFlags, CPoint point) {
  ...
  if (m_bDragging) {
    ...
    if (IsVertDocked()) {
      ...
      m_sizeDesired.cy =
          m_sizeDesired.cy > GetMinExt() ? m_sizeDesired.cy : GetMinExt();
      m_sizeDesired.cy = m_sizeDesired.cy > rectAvail.Height()
                             ? rectAvail.Height()
                             : m_sizeDesired.cy;
    }
      ...
      m_sizeDesired.cx =
          m_sizeDesired.cx > GetMinExt() ? m_sizeDesired.cx : GetMinExt();
      m_sizeDesired.cy = m_sizeDesired.cx > rectAvail.Width()
                             ? rectAvail.Width()
                             : m_sizeDesired.cx;
    }
...
UINT CCJControlBar::OnNcHitTest(CPoint point) { return HTCLIENT; }
...
int CCJControlBar::OnCreate(LPCREATESTRUCT lpCreateStruct) {
  ...
  if (m_bButtons) {
    if (!m_btnClose.Create(NULL,
                           WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_NOTIFY |
...
void CCJControlBar::OnUpdateButtonClose(CCmdUI *pCmdUI) { pCmdUI->Enable(); }
...
void CCJControlBar::OnUpdateButtonMinimize(CCmdUI *pCmdUI) { pCmdUI->Enable(); }
...
void CCJControlBar::SetChild(CWnd *pWnd) { m_pChildWnd = pWnd; }
...
UINT CCJControlBar::GetMenuID() { return m_menuID; }

void CCJControlBar::SetMenuID(UINT nID) { m_menuID = nID; }
...
BOOL CCJControlBar::IsLeftDocked() {
  return m_pDockBar ? (m_pDockBar->GetDlgCtrlID() == AFX_IDW_SIZEBAR_LEFT)
                    : FALSE;
}

BOOL CCJControlBar::IsRightDocked() {
  return m_pDockBar ? (m_pDockBar->GetDlgCtrlID() == AFX_IDW_SIZEBAR_RIGHT)
                    : FALSE;
}

BOOL CCJControlBar::IsTopDocked() {
  return m_pDockBar ? (m_pDockBar->GetDlgCtrlID() == AFX_IDW_SIZEBAR_TOP)
                    : FALSE;
}

BOOL CCJControlBar::IsBottomDocked() {
  return m_pDockBar ? (m_pDockBar->GetDlgCtrlID() == AFX_IDW_SIZEBAR_BOTTOM)
                    : FALSE;
}
