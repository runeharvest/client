...
// Create coolbar
//
BOOL CCoolBar::Create(CWnd *pParentWnd, DWORD dwStyle, DWORD dwAfxBarStyle,
                      UINT nID) {
  ...
  CRect rc;
  rc.SetRectEmpty();
  return CWnd::CreateEx(WS_EX_TOOLWINDOW, REBARCLASSNAME, NULL, dwStyle, rc,
                        pParentWnd, nID);
}
...
//
BOOL CCoolBar::InsertBand(CWnd *pWnd, CSize szMin, int cx, LPCTSTR lpText,
                          int iPos, BOOL bNewRow) {
  CRebarBandInfo rbbi;
