...
BOOL CCJListView::SortTextItems(int nCol, BOOL bAscending, int low /*= 0*/,
                                int high /*= -1*/) {
  ...
  while (lo <= hi) {
    ...
    if (lo <= hi) {
      ...
      if (GetListCtrl().GetItemText(lo, nCol) !=
          GetListCtrl().GetItemText(hi, nCol)) {
        // swap the rows
        ...
        lvitemlo.stateMask = LVIS_CUT | LVIS_DROPHILITED | LVIS_FOCUSED |
                             LVIS_SELECTED | LVIS_OVERLAYMASK |
                             LVIS_STATEIMAGEMASK;
...
void CCJListView::SaveColumnWidths() {
  ...
  for (int nItem = 0; nItem < pHeader->GetItemCount(); nItem++) {
    CString strTemp;
    strTemp.Format("%d,", GetListCtrl().GetColumnWidth(nItem));
    strValue += strTemp;
  }
...
int CCJListView::GetRegColumnWidth(int iColumn) {
  ...
  strValue = AfxGetApp()->GetProfileString(m_strSection, strEntry, "");
...
void CCJListView::SetExtendedStyle(DWORD dwNewStyle) {
  ...
  DWORD dwStyle =
      ::SendMessage(GetListCtrl().m_hWnd, LVM_GETEXTENDEDLISTVIEWSTYLE, 0, 0);

  // Add the full row select and grid line style to the existing extended
  // styles.
  dwStyle |= dwNewStyle;
...
int CCJListView::OnCreate(LPCREATESTRUCT lpCreateStruct) {
  if (CListView::OnCreate(lpCreateStruct) == -1)
    return -1;

  // TODO: Add your specialized creation code here
  if (m_headerCtrl.SubclassWindow(GetListCtrl().GetDlgItem(0)->GetSafeHwnd()) ==
      0) {
  ...
  SaveColumnState("Settings", "Control");
