...
BOOL CCJListCtrl::SortTextItems(int nCol, BOOL bAscending, int low /*= 0*/,
                                int high /*= -1*/) {
...
void CCJListCtrl::SaveColumnWidths() {
  ...
  for (int nItem = 0; nItem < pHeader->GetItemCount(); nItem++) {
    CString strTemp;
    strTemp.Format("%d,", GetColumnWidth(nItem));
    strValue += strTemp;
  }
...
int CCJListCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct) {
  ...
  if (m_headerCtrl.SubclassWindow(GetDlgItem(0)->GetSafeHwnd()) == 0)
    return -1;
