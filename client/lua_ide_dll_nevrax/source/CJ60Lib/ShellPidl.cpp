 ...
 *
 ****************************************************************************/
BOOL CShellPidl::GetName(LPSHELLFOLDER lpsf, LPITEMIDLIST lpi, DWORD dwFlags,
                         LPSTR lpFriendlyName) {
  BOOL bSuccess = TRUE;
  STRRET str;
  ...
  if (NOERROR == lpsf->GetDisplayNameOf(lpi, dwFlags, &str)) {
    ...
    case STRRET_WSTR:
      ...
      WideCharToMultiByte(CP_ACP,         // CodePage
                          ...
                          MAX_PATH,
                          // sizeof(lpFriendlyName), // cchMultiByte, wrong.
                          // sizeof on a pointer, psk, psk
                          NULL,  // lpDefaultChar,
 ...
 *
 ****************************************************************************/
LPITEMIDLIST CShellPidl::GetFullyQualPidl(LPSHELLFOLDER lpsf,
                                          LPITEMIDLIST lpi) {
  ...
  MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, szBuff, -1, (USHORT *)szOleChar,
                      sizeof(szOleChar));

  hr = lpsfDeskTop->ParseDisplayName(NULL, NULL, szOleChar, &ulEaten, &lpifq,
                                     &ulAttribs);
...
BOOL CShellPidl::DoTheMenuThing(HWND hwnd, LPSHELLFOLDER lpsfParent,
  ...
  hr = lpsfParent->GetUIObjectOf(hwnd,
                                 ...
                                 IID_IContextMenu, 0, (LPVOID *)&lpcm);
  if (SUCCEEDED(hr)) {
