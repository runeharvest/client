...
public:
  ...
  LineLayout *Retrieve(int lineNumber, int lineCaret, int maxChars,
                       int styleClock_, int linesOnScreen, int linesInDoc);
...
protected: // ScintillaBase subclass needs access to much of Editor
   ...
   * whereas on Windows there is just one window with both scroll bars turned
   * on. */
  ...
  /** In bufferedDraw mode, graphics operations are drawn to a pixmap and then
   * copied to the screen. This avoids flashing but is about 30% slower. */
  bool bufferedDraw;
  ...
  void EnsureCaretVisible(bool useMargin = true, bool vert = true,
                          bool horiz = true);
  ...
  void DrawLine(Surface *surface, ViewStyle &vsDraw, int line, int lineVisible,
                int xStart, PRectangle rcLine, LineLayout *ll, int subLine = 0);
  ...
  /** PositionInSelection returns 0 if position in selection, -1 if position
   * before selection, and 1 if after. Before means either before any line of
   * selection or before selection on its line, with a similar meaning to after.
   */
