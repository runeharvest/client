...
public:
  ...
  uint32 indexInBag;
  SLOT_EQUIPMENT::TSlotEquipment slot; // Used to differentiate right/left
                                       // hands, and for clarity in the xml file
  CInventoryItem(
      CDBCtrlSheet *pCS, INVENTORIES::TInventory origin, uint32 indexInBag,
...
public:
  struct CItem {
    SLOT_EQUIPMENT::TSlotEquipment
        slot; // Used only for dagger (right/left hand slot)
    sint32 createTime;
    sint32 serial;
    ...
    CItem(std::string sheetName = "", uint16 quality = 0, uint32 weight = 0,
          uint8 color = 0, sint32 createTime = 0, sint32 serial = 0,
          SLOT_EQUIPMENT::TSlotEquipment slot = SLOT_EQUIPMENT::UNDEFINED,
          uint32 minPrice = 0,
          uint32 maxPrice = std::numeric_limits<uint32>::max(),
          bool usePrice = false)
        : sheetName(sheetName), quality(quality), weight(weight), color(color),
          createTime(createTime), serial(serial), slot(slot),
          minPrice(minPrice), maxPrice(maxPrice), usePrice(usePrice) {}
...
public:
  ...
  // return true if any item in the group match the parameter ; slot is
  // UNDEFINED unless the item has been found in the group
  bool contains(CDBCtrlSheet *other);
  bool contains(CDBCtrlSheet *other, SLOT_EQUIPMENT::TSlotEquipment &slot);
  void addItem(sint32 createTime, sint32 serial,
...
private:
  ...
  std::vector<CInventoryItem> matchingItems(CItemGroup *group,
  ...
  // Workaround: sometimes item are marked as equipped by pIM->isBagItemWeared()
  // even tho they aren't really Because of a synchronisation error between
  // client and server
  bool isItemReallyEquipped(CDBCtrlSheet *item);

  // Used to migrate old groups ; keep for compatibility purpose
  bool migrateGroups();
  // Return a new group who uses create time and serial (param group isn't
  // modified)
