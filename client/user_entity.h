...
public:
   ...
   * \return CLFECOMMON::TCLEntityId : slot currently selected
   * (CLFECOMMON::INVALID_SLOT for an empty selection). \warning Can be
   * different from the entity targeted (in combat mode for example).
   */
  virtual const CLFECOMMON::TCLEntityId &selection() const {
    return _Selection;
  }
  ...
  /** Return the trader entity that will be turned towards the user entity
   * (client-side) and other stuff such as distance checking (see also
   * interlocutor()) \return CLFECOMMON::TCLEntityId : slot of the trader
   * (CLFECOMMON::INVALID_SLOT if no trader).
   */
  virtual const CLFECOMMON::TCLEntityId &trader() const { return _Trader; }

  /** Set the current interlocutor entity that will be turned towards the user
   * entity (client-side). This one is for dyn-chat, while trader() is for
   * bot-chat. \param slot : interlocutor slot (CLFECOMMON::INVALID_SLOT if no
   * interlocutor).
   */
  void interlocutor(const CLFECOMMON::TCLEntityId &slot);
  /** Return the current interlocutor entity that will be turned towards the
   * user entity (client-side). This one is for dyn-chat, while trader() is for
   * bot-chat. \return CLFECOMMON::TCLEntityId : slot of the interlocutor
   * (CLFECOMMON::INVALID_SLOT if no interlocutor).
   */
  ...
  inline void setSpeedServerAdjust(float speed) {
    _SpeedFactor.addFactorValue(speed);
  }
  ...
  /// Return true if the user is indoor and the CFG want to force the FPV
  /// Indoor.
  bool forceIndoorFPV();
  ...
  void disableFollow();
  // when the user request to move the head pitch, stop the "follow mode" to
  // force it
  ...
  /// Method to move to someone else for a mission. NB: resetAnyMoveTo() is
  /// called first
  void moveToMission(CLFECOMMON::TCLEntityId slot, double dist, uint32 id);
  /// Method to move to someone else for a ring mission. NB: resetAnyMoveTo() is
  /// called first
   ...
   *	The caller MUST call after CSPhraseManager::clientExecute(), to
   *increment action counter NB: resetAnyMoveTo() is called first
   */
  void moveToExtractionPhrase(CLFECOMMON::TCLEntityId slot, double dist,
                              uint phraseMemoryLine, uint phraseMemorySlot,
                              bool cyclic);
  /** Method to begin a spire construction
   *	The caller MUST call after CSPhraseManager::clientExecute(), to
   *increment action counter NB: resetAnyMoveTo() is called first
   */
  void moveToTotemBuildingPhrase(CLFECOMMON::TCLEntityId slot, double dist,
                                 uint phraseMemoryLine, uint phraseMemorySlot,
                                 bool cyclic);
  /// Reset any MoveTo. NB: if the current moveTo is a moveToCombatPhrase() or a
  /// moveToExtractionPhrase(), decrement action counter
  void resetAnyMoveTo();
  ...
  /// After a few time, if the user is still in collision with someone else,
  /// remove collisions with other entitites.
  void startColTimer();
  /// Called when the user is no more in collision with another entity.
  void stopColTimer();

  /// Make the character transparent if the mouse is under it (params is if we
  /// must make it transparent or opaque)
  ...
  /// assert(target). NB: this engage (moveToCombatPhrase) if in melee and to
  /// far, or directly launch the action
  void executeCombatWithPhrase(CEntityCL *target, uint32 memoryLine,
                               uint32 memoryIndex, bool cyclic);

  /// equip with the last weapon used of with the best weapon that can be found
  /// in inventory
  void autoEquipWithLastUsedWeapons();

  /// save the last weapon(s) used in fight or weapons in hand before tool
  /// auto-equip
  ...
  /// \name R2 specific
  // @{
  // R2 char mode (player, dm, anim, edit) NB: automatically affect the camera
  // max distance and player run/walk speed
  void setR2CharMode(R2::TCharMode mode);
  R2::TCharMode getR2CharMode() const { return _R2CharMode; }
  // re-update view camera dist max, and walk/run speed according to current R2
  // char mode
  void flushR2CharMode() { setR2CharMode(_R2CharMode); }
  // @}
  // When a player control a npc (in a ring shard) then he must adapt his speed
  // on the Npc speed.
...
protected:
  ...
  /// Speed Factor to use for the user sent by the server to respect magical
  /// spells
  ...
  bool _Run;
  /// 'True' if the user & mount should run but are walking because of the mount
  /// unability to run
  bool _RunWhenAble;
  ...
  /// Last Global Position sent to the server (in fact it's a normal position
  /// sent).
  ...
  /// Time in MS when the User started beiing in collision with anything that
  /// avoid him to do an Action (and still is).
...
protected:
  ...
  virtual void updateVisualPropertyVpc(const NLMISC::TGameCycle &gameCycle,
                                       const sint64 &prop);
  /// Update Entity Mount
  virtual void
...
private:
  ...
  bool _ColOn;
  /// Time in MS when the User started beiing in collision with another entity
  /// (and still is).
  ...
  void moveToCheckStartDist(CLFECOMMON::TCLEntityId slot, double dist,
                            TMoveToAction action);

  /// forceLook an entity (follow or moveTo)
  CLFECOMMON::TCLEntityId _ForceLookSlot;
  void forceLookEntity(const NLMISC::CVectorD &dir2targ, bool updateHeadPitch,
                       bool start = false);
   ...
   *increment action counter NB: resetAnyMoveTo() is called first
   */
  void moveToCombatPhrase(CLFECOMMON::TCLEntityId slot, double dist,
                          uint phraseMemoryLine, uint phraseMemorySlot,
                          bool phraseCyclic);
  ...
  /// R2: to know in which mode is the current entity (affect camera view and
  /// run speed)
  ...
  /// previous items in hand before they have been changed by an auto-equip due
  /// to an action (ex: forage)
