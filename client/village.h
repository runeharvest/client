 ...
 * This class holds a village instance group, and manage its loading /
 * unloading.
 */
...
public:
   ...
   * \param loadedIGMap If not NULL, a map that will be filled with ig that are
   * currently loaded \return true if the build succeed
   */
  bool setupFromSheet(NL3D::UScene *scene, const CVillageSheet &sheet,
   ...
   * It it returns false, it means that the village is too far or that
   * asynchronous loading suffice. It it returns true, the next call to update
   * will return only when the loading is completed.
   */
  virtual bool needCompleteLoading(const NLMISC::CVector &pos) const;
  /** Test that village against the player position, and load / unload it (using
  ...
  bool setupOutpost(NL3D::UScene *scene, const CContinentParameters::CZC &zone,
                    sint32 outpost, CStreamableIG::TString2IG *loadedIGMap);
