...
/**
 * class for the basic chat control interface (double list, mute/unmute of
 * players, different colors can be assigned for each player) text can be saved
 * in .txt format. The list is sensible to move-over, left click and right click
 * \author David Fleury
...
public:
  ...
  /// Constructor
  CChatControl(uint id, float x, float y, float x_pixel, float y_pixel, float w,
               float h, float w_pixel, float h_pixel, uint leftFunc,
               uint rightFunc, const CPen &pen);
  CChatControl(uint id, float x, float y, float x_pixel, float y_pixel, float w,
               float h, float w_pixel, float h_pixel, uint leftFunc,
               uint rightFunc, uint32 fontSize, CRGBA color, bool shadow);

private:
  /**
   * add a line of text (private because this function, herited from CMultiList
   * should be avoided for chat box, use add( const NLMISC::CEntityId sid, const
   * ucstring &name, const ucstring &text) or add(const ucstring &text) for
   * player msg and system msg
   */
...
public:
   ...
   */
  void add(const NLMISC::CEntityId &sid, const ucstring &name,
           const ucstring &text);

  /**
   * display text from a system message (not a player message, for player use
   * the other public add method) \param ucstring& the message to display
   */
  ...
  void setSysColor(CRGBA &color) {
    _SysTextColor =
        color; // map the command dummy NLMISC::CEntityId with the display color
   ...
   * \return ucstring* pointer to the player name (or NULL if there is no player
   * selected)
   */
  ucstring *CChatControl::getSelectedPlayerName() const {
    return _SelectedPlayer;
  }

  /**
   * get adress of  the NLMISC::CEntityId of the currently selected player, or
   * NULL if no player selected \return NLMISC::CEntityId* pointer to the player
   * NLMISC::CEntityId (or NULL)
   */
  NLMISC::CEntityId *CChatControl::getSelectedPlayerSid() const {
    return _SelectedPlayerSid;
  }
   ...
   */
  bool isMuted(const NLMISC::CEntityId &sid) const {
    return (_MutedPlayers.find(sid) != _MutedPlayers.end());
  }
...
private:
   ...
   * \param NLMISC::CEntityId*& the variable that will receive the adress of the
   * NLMISC::CEntityId of the player (or NULL if not found) \param ucstring*&
   * the variable that will receive the pointer to the name of the player (or
   * NULL if not found) \return bool true if a player was found, false otherwise
   */
  bool searchPlayerAtPos(float y, NLMISC::CEntityId *&sid,
                         ucstring *&name) const;
...
private:
  ...
  /// the set of NLMISC::CEntityId (match the list of ucstring associated to
  /// player names and messages)
  ...
  /**
   * list Y coordinates of displayed player name, used when a user left click on
   * the control to determine if he clicked on a player name, and which one(to
   * mute that player) NB : the second type (ucstring*) is a pointer to the
   * player name in _ItemsList
   */
  typedef std::list<
      std::pair<std::pair<float, float>,
                std::pair<const ucstring *, const NLMISC::CEntityId *>>>
      TPairPFloatPStrSid;
  ...
  /// heigth of the last computed string, used to determine if the mouse if over
  /// a line or not
  ...
  CRGBA _SysTextColor;
  /// text color for muted players messages (the messages received before the
  /// player was muted)
  CRGBA _MutedColor;
  ...
  /// the currently selected character name (either by left click or right
  /// click) (pointer to the player name in _ItemsList)
  ucstring *_SelectedPlayer;

  /// the currently selected character NLMISC::CEntityId (either by left click
  /// or right click)
  ...
  /**
   * the ending position in the list of NLMISC::CEntityId for display, by
   * default it's the last item in the list (the newest one) if auto scroll mode
   * is 'on' (_AutoScroll == true), this iterator is allways equal to the last
   * item in the list this iterator is used to keep the NLMISC::CEntityId List
   * and the ucstring list coordinated
   */
