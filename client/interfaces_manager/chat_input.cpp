...
// Constructor.
//-----------------------------------------------
CChatInput::CChatInput(uint id) : CCapture(id) { init(); } // CChatInput //

//-----------------------------------------------
// CChatInput :
// Constructor.
//-----------------------------------------------
CChatInput::CChatInput(uint id, float x, float y, float x_pixel, float y_pixel,
                       float w, float h, float w_pixel, float h_pixel,
                       uint numFunc, const CPen &pen)
    : CCapture(id, x, y, x_pixel, y_pixel, w, h, w_pixel, h_pixel, numFunc,
               pen) {
  init();
} // CChatInput //
...
CChatInput::CChatInput(uint id, float x, float y, float x_pixel, float y_pixel,
                       float w, float h, float w_pixel, float h_pixel,
                       uint numFunc, uint32 fontSize, CRGBA color, bool shadow)
    : CCapture(id, x, y, x_pixel, y_pixel, w, h, w_pixel, h_pixel, numFunc,
               fontSize, color, shadow) {
  init();
} // CChatInput //
...
//-----------------------------------------------
void CChatInput::init() { _Prompt = "Say : "; } // init //
