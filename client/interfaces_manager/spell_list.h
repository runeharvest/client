...
public:
  /// Constructor
  CSpellList(uint id, float x, float y, float x_pixel, float y_pixel, float w,
             float h, float w_pixel, float h_pixel, uint16 spacing = 0,
             uint leftFunc = 0, uint rightFunc = 0);
   ...
   * \param CButtonBase& base the characteristics of the button (textures and
   * colors)
   */
   ...
   * \param CButtonBase::TBG &modeDisable button background display mode when
   * Disabled
   */
  void setButtonParam(float w, float h, uint tOn, const CRGBA &colorOn,
                      uint tOff, const CRGBA &colorOff, uint tDis,
                      const CRGBA &colorDis, const CButtonBase::TBG &modeOn,
                      const CButtonBase::TBG &modeOff,
                      const CButtonBase::TBG &modeDisable);
  ...
  /**
   * remove the specified spell from list (may be slow : check items in
   * insertion order until it find the right one) \param uint16 spellId id of
   * the spell to remove \return CSpellClient* adress of the removed spell or
   * NULL if the spell was not found
   */
  CSpellClient *removeSpell(uint16 spellId);

  /**
   * get the selected spell control (may be slow : check items in insertion
   * order until it find the right one) \return CSpellControl *spellControl the
   * selected control, or NULL if none is selected
   */
...
private:
  ...
  typedef std::list<std::pair<std::pair<float, float>, const CControl *>>
      TPairPosItem;
