...
// Constructor.
//-----------------------------------------------
CButton::CButton(uint id) : CControl(id) { init(0, 0, 0); } // CButton //

//-----------------------------------------------
// CButton :
// Constructor.
//-----------------------------------------------
CButton::CButton(uint id, float x, float y, float x_pixel, float y_pixel,
                 float w, float h, float w_pixel, float h_pixel, uint numFuncOn,
                 uint numFuncR, uint numFuncD, const CButtonBase &buttonBase)
...
CButton::CButton(uint id, float x, float y, float x_pixel, float y_pixel,
                 float w, float h, float w_pixel, float h_pixel, uint numFuncOn,
                 uint numFuncR, uint numFuncD, CRGBA on, CRGBA off,
                 CRGBA disable)
    : CControl(id, x, y, x_pixel, y_pixel, w, h, w_pixel, h_pixel),
      CButtonBase(on, off, disable) {
  init(numFuncOn, numFuncR, numFuncD);
} // CButton //
...
//-----------------------------------------------
void CButton::pen(const CPen &pen) { _Pen = pen; } // pen //
...
void CButton::display() {
  ...
  case BG_stretch: {
    ...
    if (utexture) {
      ...
      // Draw a quad
      Driver->drawQuad(_X_Display, _Y_Display, _X_Display + _W_Display,
                       _Y_Display + _H_Display, CRGBA(255, 0, 255));
      // Draw the debug msg.
  ...
  if (!_Text.empty()) {
    ...
    TextContext->printAt(_X_Display + _W_Display / 2,
                         _Y_Display + _H_Display / 2, _Text);
  }
} // display //
...
void CButton::click(float x, float y, bool &taken) {
  ...
  if (_Enable && (!taken)) {
    ...
    if (x >= _X_Display && x <= (_X_Display + _W_Display) && y >= _Y_Display &&
      ...
      /// \todo Malkav: TO DO : SIMULATE DOUBLE CLICK !!!!!!! SHOULD MAKE A REAL
      /// DOUBLE CLICK EVENT
