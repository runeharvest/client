 ...
 * Constructor
 */
CChatControl::CChatControl(uint id) : CMultiList(id, 2) { init(0, 0); }

//-----------------------------------------------
// CChatControl :
// Constructor.
//-----------------------------------------------
CChatControl::CChatControl(uint id, float x, float y, float x_pixel,
                           float y_pixel, float w, float h, float w_pixel,
                           float h_pixel, uint leftFunc, uint rightFunc,
                           const CPen &pen)
...
CChatControl::CChatControl(uint id, float x, float y, float x_pixel,
    ...
    : CMultiList(id, x, y, x_pixel, y_pixel, w, h, w_pixel, h_pixel, fontSize,
                 color, shadow, 2) {
  init(leftFunc, rightFunc);
} // CMultiList //
...
//-----------------------------------------------
void CChatControl::add(const std::list<ucstring> &strList) {} // add //

//-----------------------------------------------
// add :
// add text from player
//-----------------------------------------------
void CChatControl::add(const NLMISC::CEntityId &sid, const ucstring &name,
                       const ucstring &text) {
  /// \toto Malkav: ensure the name is different from system messages displayed
  /// text (_SystemDisplayText)
...
void CChatControl::display() {
  ...
  /// \todo GUIGUI : if the scissor is corrected -> draw the bitmap after the
  /// scissor.
  ...
  /*	float w = 0 , x_display = _X_Display;
          ...
          for (uint j = 0 ;  j < _NbCol ; ++j)
          {
                  ...
                  Driver->drawBitmap(x_display , yDisplay, w, hDisplay,
     *CInterfMngr::getTexture(_BackgroundTexture), true, _BackgroundColor);
                  x_display += w;
          }
  */

  /// \todo GUIGUI : initialize the scissor with oldScissor and remove tmp
  /// variables.
  ...
  // if the ending item is the last item (so the first item in reverse order),
  ...
  float maxH = 0;
  float y =
      _Y_Display + _LastStringHeight /
                       3.0f; // start a bit upper to have room to print a line
  float x = _X_Display;
  ...
  for (ritList = _EndingIterator; ritList != ritEndList; ++ritList) {
    ...
    // Test mouse position.
    if ((_MouseY >= y) && (_MouseY <= y + _LastStringHeight) &&
        (_MouseX >= x) && (_MouseX <= xLimit)) {
      // if player is muted
      if ((!_MutedPlayers.empty()) &&
          (_MutedPlayers.find(*sid) != _MutedPlayers.end())) {
    ...
    // insert player names and NLMISC::CEntityId coordinates in the _NamesYPos
    // map (only if it's not a system message)
    if ((*sid) != _CommandSid)
      _NamesYPos.push_back(
          std::make_pair(std::make_pair(y, y + info.StringHeight),
                         std::make_pair(&(*itText), sid)));
...
// searchNameAtPos :
//-----------------------------------------------
bool CChatControl::searchPlayerAtPos(float y, NLMISC::CEntityId *&sid,
                                     ucstring *&name) const {
  ...
  // we know the first name is at _Y_Display and the last one at
  // _Y_Display+_H_Display (approx.) so we make an interpolation of name
  // position in the list according to the y coordinate and the limits
  // coordinates
  for (uint32 start = static_cast<uint32>((y - _Y_Display) / (_H_Display)*size);
       start > 0; --start) {
    ++it;
    if (it == itEnd) {
      --it;
...
CRGBA CChatControl::getNewColor() const {
  ...
  while ((_UsedColors.find(color) != itColorsEnd) && (max < 180)) {
    ...
    // randomly add a number beetween 10 and 200 (in absolute) to each composant
    // (rgb)
