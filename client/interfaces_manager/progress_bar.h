...
public:
  /// Constructor
  CProgressBar(uint id, float x, float y, float x_pixel, float y_pixel, float w,
  ...
  /**
   * Advances the current position for the progress bar control by the specified
   * value \param uint32 offset the value added to current position \return
   * uint32 the old position of the progress bar
   */
  ...
  /**
   * Advances the current position for the progress bar control by the step
   * increment \return uint32 the previous position of the progress bar control
   */
...
protected:
  ...
  /// temporary position, used when smooth filling the control, only for
  /// internal use
  uint32 _TempPos;
  ...
  /// the smooth mode (smooth transition beetween two positions true = ON)
  /// (default = false)
  bool _Smooth;

  /// if in smooth mode, set the interval in ms (milliseconds) beetween the
  /// addition/soustraction of two units to the bar (defualt = 200ms =
  /// 5units/second)
