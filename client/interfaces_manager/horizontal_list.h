...
public:
  /// Constructor
  CHorizontalList(uint id, float x, float y, float x_pixel, float y_pixel,
                  float w, float h, float w_pixel, float h_pixel,
                  uint16 spacing = 3, uint texture = 0,
                  NLMISC::CRGBA rgba = CRGBA(255, 255, 255, 255));
   ...
   * >0 and left is scroll<0 ) \param sint32 scroll : the number of unit to
   ...
   * \param bool delElts if true all the controls in the lis will be deleted,
   * otherwise, they will just be removed from the list
   */
...
protected:
  ...
  /// boolean : indicate if controls are centered in the list or not
  /// (vertically) default = false
  bool _Centered;

  /// internal use only, set if the list is cleared during a skim through the
  /// controls in the list. clear() will be called on the next display
  bool _ClearRequest;

  /// internal use only, indicate if the elements of the list should be deleted
  /// ( ie : delete calld on controls pointers)
  bool _DeleteRequest;

  /// internal use only, lock the list during a skim through the controls (if a
  /// click on a control of the list clear the list for exemple)
  bool _Locked;

  /// internal use only, buffer the add() request when the control is locked,
  /// add all the elts in the buffer on the next display
