...
// Constructor.
//-----------------------------------------------
CChoiceList::CChoiceList(uint id) : CList(id) {} // CChoiceList //

//-----------------------------------------------
// CChoiceList :
// Constructor.
//-----------------------------------------------
CChoiceList::CChoiceList(uint id, float x, float y, float x_pixel,
...
CChoiceList::CChoiceList(uint id, float x, float y, float x_pixel,
                         ...
                         bool shadow)
    : CList(id, x, y, x_pixel, y_pixel, w, h, w_pixel, h_pixel, fontSize, color,
            shadow) {} // CChoiceList //
...
void CChoiceList::display() {
  ...
  for (ritList = _EndingIterator; ritList != ritEndList; ++ritList) {
    ...
    if (ritList == _ItSelected)
      Driver->drawQuad(_X_Display, y, _X_Display + wDisplay,
                       y + info.StringHeight, CRGBA(0, 0, 255, 255));
