...
// Constructor :
//-----------------------------------------------
CHorizontalList::CHorizontalList(uint id, float x, float y, float x_pixel,
                                 float y_pixel, float w, float h, float w_pixel,
                                 float h_pixel, uint16 spacing, uint texture,
                                 CRGBA rgba)
...
void CHorizontalList::hotSpot(THotSpot hs) {
  ...
  // Update chidren.
  for (TListControl::iterator it = _Children.begin(); it != _Children.end();
       ++it) {
    if ((*it)->parent() == this) {
      float x, y;
...
void CHorizontalList::display() {
  ...
  if (!_Locked) {
    for (TListControl::const_iterator itbuff = _AddBuffer.begin();
         itbuff != _AddBuffer.end(); ++itbuff) {
      add((*itbuff));
    }
  ...
  if (_TextureId != 0) {
    if (_Tiled && !_TexturePath.empty()) {
      ...
      drawBitmapTiled(Driver, xDisplay, yDisplay, wDisplay, hDisplay, w, h,
                      *_Texture, _TextureWidth, _TextureHeight, true, _RGBA);
    } else {
      Driver->drawBitmap(xDisplay, yDisplay, wDisplay, hDisplay, *_Texture,
                         true, _RGBA);
    }
  }
  ...
  for (it = _StartingIterator; it != itE; ++it) {
    ...
    /// \todo Malkav: set control hotspot according to current list hotspot (but
    /// not == ), centered or not
    (*it)->hotSpot(_HotSpot);
  ...
  if (_ScrollLeft != NULL || _ScrollRight != NULL) {
    ...
    case THotSpot::HS_TR:
      if (_ScrollLeft != NULL)
        _ScrollLeft->setPosition(_X, _Y + _H / 2, _X_Pixel,
                                 _Y_Pixel + _H_Pixel / 2);

      if (_ScrollRight != NULL)
        _ScrollRight->setPosition(_X + _W, _Y + _H / 2, _X_Pixel + _W_Pixel,
                                  _Y_Pixel + _H_Pixel / 2);
      break;
    ...
    case THotSpot::HS_TL:
      if (_ScrollLeft != NULL)
        _ScrollLeft->setPosition(_X - _W, _Y + _H / 2, _X_Pixel - _W_Pixel,
                                 _Y_Pixel + _H_Pixel / 2);

      if (_ScrollRight != NULL)
        _ScrollRight->setPosition(_X, _Y + _H / 2, _X_Pixel,
                                  _Y_Pixel + _H_Pixel / 2);
      break;
    ...
    case THotSpot::HS_MM:
      if (_ScrollLeft != NULL)
        _ScrollLeft->setPosition(_X - _W / 2, _Y, _X_Pixel - _W_Pixel / 2,
                                 _Y_Pixel);

      if (_ScrollRight != NULL)
        _ScrollRight->setPosition(_X + _W / 2, _Y, _X_Pixel + _W_Pixel / 2,
                                  _Y_Pixel);
      break;
    }
    if (_ScrollLeft != NULL)
