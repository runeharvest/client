...
protected:
  ...
  /// Calculate the display position of the control in relation to the position
  /// of the control (Hot Spot).
  virtual void calculateHS();

public:
  /// Constructor
  CText(uint id = 0);
  CText(uint id, float x, float y, float x_pixel, float y_pixel,
  ...
  // replace the methods of CPen class (the CComputedString needs to be
  // recomputed when we change color or size or shadow)
  /// Set the font size.
