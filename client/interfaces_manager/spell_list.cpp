...
// Constructor :
//-----------------------------------------------
CSpellList::CSpellList(uint id, float x, float y, float x_pixel, float y_pixel,
                       float w, float h, float w_pixel, float h_pixel,
                       uint16 spacing, uint leftFunc, uint rightFunc)
    : CControlList(id, x, y, x_pixel, y_pixel, w, h, w_pixel, h_pixel,
                   spacing) {
  _NumFuncOn = leftFunc;
...
// setButtonParam :
//-----------------------------------------------
void CSpellList::setButtonParam(float w, float h, uint tOn,
                                const CRGBA &colorOn, uint tOff,
                                const CRGBA &colorOff, uint tDis,
                                const CRGBA &colorDis,
                                const CButtonBase::TBG &modeOn,
...
void CSpellList::addSpell(CSpellClient *spell) {
  ...
  CSpellControl *ctrl = new CSpellControl(
      0, _X, _Y, _X_Pixel, _Y_Pixel, _W - w, _Line_H, _W_Pixel - w_pixel,
      _Line_H_Pixels, 8, _SpellPen, _CommentPen);

  // nlassert( ctrl );
  if (!ctrl) {
    nlwarning("<CSpellList::addSpell> : allocation of a new CSpellControl "
              "failed, abort");
    return;
  }
...
void CSpellList::ref(float x, float y, float w, float h)
{
        ...
        // Update chidren.
        for(TListControl::iterator it = _Children.begin(); it !=
_Children.end(); ++it)
        {
                if((*it)->parent() == this)
                {
