...
// constructor :
//-----------------------------------------------
CBrickControl::CBrickControl(uint id) { init(); } // constructor //
...
// constructor :
//-----------------------------------------------
CBrickControl::CBrickControl(uint id, float x, float y, float x_pixel,
                             float y_pixel, float w, float h, float w_pixel,
                             float h_pixel, uint numFuncOn, uint numFuncR)
...
// display :
//-----------------------------------------------
void CBrickControl::display() { // If the control is hide -> return
  if (!_Show)
    return;
  ...
  if (_AssociatedBrick->getTexture() == NULL) {
    ...
    if (_AssociatedBrick->LatencyEndDate > T1) {
      Driver->drawBitmap(
          _X_Display, _Y_Display, _W_Display, _H_Display,
          *CInterfMngr::getTexture(_AssociatedBrick->getTexture()), true,
  ...
  TextContext->printAt(_X_Display + _W_Display, _Y_Display,
                       ucstring(_AssociatedBrick->Text));
  //	TextContext->printAt( _X_Display + _W_Display/2, _Y_Display +
  //_H_Display/2, ucstring(_AssociatedBrick->Text) );
} // display //
