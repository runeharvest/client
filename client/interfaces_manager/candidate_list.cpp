...
// Constructor.
//-----------------------------------------------
CCandidateList::CCandidateList(uint id) : CMultiList(id, 2) {
...
// Constructor.
//-----------------------------------------------
CCandidateList::CCandidateList(uint id, float x, float y, float x_pixel,
                               float y_pixel, float w, float h, float w_pixel,
                               float h_pixel, const CPen &pen)
...
CCandidateList::CCandidateList(uint id, float x, float y, float x_pixel,
                               float y_pixel, float w, float h, float w_pixel,
                               float h_pixel, uint32 fontSize, CRGBA color,
                               bool shadow)
...
// Destructor.
//-----------------------------------------------
CCandidateList::~CCandidateList() {} // ~CCandidateList //
...
void CCandidateList::display() {
  ...
  for (ritList = _EndingIterator; ritList != ritEndList; ++ritList) {
    ...
    if (_SelectedCandidate != NULL && ((*itText) == *_SelectedCandidate)) {
      Driver->drawQuad(_X_Display, y - 5.0f / windowH, _X_Display + wDisplay,
                       y + info.StringHeight + 3.0f / windowH, _SelectedColor);
    }
    ...
    // insert candidate names and Sid coordinates in the _NamesYPos map (only if
    // it's not a system message)
    _NamesYPos.push_back(
        std::make_pair(std::make_pair(y, y + info.StringHeight), &(*itText)));
    ...
    if ((x >= _X_Display) && (x <= (_X_Display + _W_Display * _ColSize[0]))) {
      ...
      if (found) {
        CInterfMngr::runFuncCtrl(_LeftClickFunction, id(),
                                 (CInterfMngr::TParam)_SelectedCandidate);
        taken = true;
        return;
      }
    }
