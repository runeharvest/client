...
// Constructor :
//-----------------------------------------------
CSpellControl::CSpellControl(uint id, float x, float y, float x_pixel,
                             float y_pixel, float w, float h, float w_pixel,
                             float h_pixel, uint16 spacing,
                             const CPen &spellPen, const CPen &commentPen)
...
CSpellControl::CSpellControl(uint id, float x, float y, float x_pixel,
                             float y_pixel, float w, float h, float w_pixel,
                             float h_pixel, uint16 spacing, uint32 fontSize1,
                             const CRGBA &color1, bool shadow1,
                             uint32 fontSize2, const CRGBA &color2,
                             bool shadow2)
    : CControl(id, x, y, x_pixel, y_pixel, w, h, w_pixel, h_pixel) {
  init(spacing, CPen(fontSize1, color1, shadow1),
       CPen(fontSize2, color2, shadow2));
} // constructor //

//-----------------------------------------------
// Destructor :
//-----------------------------------------------
CSpellControl::~CSpellControl() {} // destructor //

//-----------------------------------------------
// init :
//-----------------------------------------------
void CSpellControl::init(uint16 spacing, const CPen &spellPen,
                         const CPen &commentPen) {
  _Spacing = spacing;
...
void CSpellControl::display() {
  ...
  /// \todo Malkav : found a way to refresh the comment line when it's edited
  /// somwhere else...
  if (_Spell)
