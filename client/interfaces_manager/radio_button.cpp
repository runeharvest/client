...
// Default Constructor.
//-----------------------------------------------
CRadioButton::CRadioButton(uint id) : CButton(id) {
  _Controller = NULL;
} // CRadioButton //
...
// Constructor.
//-----------------------------------------------
CRadioButton::CRadioButton(uint id, float x, float y, float x_pixel,
                           float y_pixel, float w, float h, float w_pixel,
                           float h_pixel, uint numFuncOn, uint numFuncR,
                           uint numFuncD, const CButtonBase &buttonBase)
    : CButton(id, x, y, x_pixel, y_pixel, w, h, w_pixel, h_pixel, numFuncOn,
              numFuncR, numFuncD, buttonBase) {
  _Controller = NULL;
} // CRadioButton //
...
CRadioButton::CRadioButton(uint id, float x, float y, float x_pixel,
                           float y_pixel, float w, float h, float w_pixel,
                           float h_pixel, uint numFuncOn, uint numFuncR,
                           uint numFuncD, CRGBA on, CRGBA off, CRGBA disable)
    : CButton(id, x, y, x_pixel, y_pixel, w, h, w_pixel, h_pixel, numFuncOn,
              numFuncR, numFuncD, on, off, disable) {
  _Controller = NULL;
} // CRadioButton //
...
void CRadioButton::radioClick(float x, float y, bool &taken) {
  ...
  if (_Enable && (!taken)) {
    ...
    if (x >= _X_Display && x <= (_X_Display + _W_Display) && y >= _Y_Display &&
      ...
      // TO DO : SIMULATE DOUBLE CLICK !!!!!!! SHOULD MAKE A REAL DOUBLE CLICK
      // EVENT
