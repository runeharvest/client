...
private:
  ...
  void init(uint id, float x, float y, float x_pixel, float y_pixel, float w,
...
public:
  ...
  enum TMode { Mode_0_1 = 0, Mode_Pixel };
...
protected:
  ...
  THotSpot _HotSpot;
  /// Useful for the Parent to know what reference position to give to the
  /// control.
  THotSpot _Origin;
...
public:
  /// Constructor
  CControl(uint id = 0);
  CControl(uint id, float x, float y, float x_pixel, float y_pixel, float w,
           float h, float w_pixel, float h_pixel);
  virtual ~CControl() {}
