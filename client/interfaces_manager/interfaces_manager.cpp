...
//-----------------------------------------------
void CInterfMngr::init(const CInterfMngr::TMapFuncCtrl &funcCtrlMap,
...
void CInterfMngr::release() {
  // Release OSD Look.
  for (TMapOSDBase::iterator itOSDLook = _OSDLook.begin();
       itOSDLook != _OSDLook.end(); ++itOSDLook) {
    if ((*itOSDLook).second) {
  ...
  // Delete OSDs.
  for (TMapWindows::iterator it = _Windows.begin(); it != _Windows.end();
       ++it) {
    if ((*it).second) {
      delete (*it).second;
      (*it).second = 0;
    ...
    while (!file.eof()) {
      ...
      if ((key != NULL) && (*key) != '/') {
        ...
        // ID is not Valide 0 is reserved for the OSD.
          ...
          nlerror(
              "\"ctrls.txt\": Line %d : 0 must not be used for a control ID !!",
              line);
        }
      }
    }
          ...
          if(file.is_open())
          {
                  ...
                  while(!file.eof())
                  {
                          ...
                          if(key != NULL)
                          {
                                  ...
                                  if(str != NULL)
                                  {
                                          _Textures.insert(TMapTextures::value_type(id,
     Driver->createTextureFile(string(str))));
                                  }
                          }
                  }
                  file.close();
    ...
    while (!file.eof()) {
      ...
      if (token != NULL) {
        ...
        // Set specific Pens.
          ...
          // Insert the text.
          _Pens.insert(TMapPens::value_type(
              id, new CPen(fontSize, CRGBA(r, g, b, a), shadow)));
        }
      }
    }
    file.close();
    ...
    while (!file.eof()) {
      ...
      if (token != NULL) {
        ...
        // Set specific Pens.
          ...
          // Insert the button.
          CButtonBase *button =
              new CButtonBase(tON, tOFF, tDIS, rgbaOn, rgbaOff, rgbaDisable);
          if (button) {
...
COSD *CInterfMngr::createOSD(uint id, bool popUp) {
  ...
  if (file.is_open()) {
    ...
    // Create the window.
    win = new COSD(id, x, y, 0.f, 0.f, 0.f, 0.f, width, height, minWidth,
                   minHeight, popUp);
    if (win != 0) {
...
void CInterfMngr::update(float x, float y) {
  ...
  for (TUint::iterator itID = _OSDWaitingDestruction.begin(); itID != itEID;
       ++itID) {
  ...
  for (TOSD::const_iterator itNew = _OSDWaitingInsertion.begin();
       itNew != itNewEnd; ++itNew) {
