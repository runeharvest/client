...
// Constructor.
//-----------------------------------------------
COSD::COSD(uint id, float x, float y, float x_pixel, float y_pixel, float w,
           float h, float w_pixel, float h_pixel, float minWidth,
           float minHeight, bool popUp) {
  // Common init for all constructors.
  init(id, x, y, x_pixel, y_pixel, w, h, w_pixel, h_pixel, minWidth, minHeight,
       popUp);
} // COSD //
...
//-----------------------------------------------
void COSD::init(uint id, float x, float y, float x_pixel, float y_pixel,
                float w, float h, float w_pixel, float h_pixel, float minWidth,
                float minHeight, bool popUp) {
  _Id = id;
  _PopUp = popUp;
  ...
  //	calculateDisplay(); // done into the 'resize' method, called by the
  //'open' method
...
COSD::~COSD() {
  ...
  for (TMapControls::iterator it = _Controls.begin(); it != _Controls.end();
       it++) {
...
void COSD::display() {
  ...
  for (TListControl::reverse_iterator it = _Children.rbegin();
  ...
  case selected:
    // Draw Borders.
    drawBorders(_HL_W, _HL_H, _X_Display, _Y_Display, _X_Display + _W_Display,
                _Y_Display + _H_Display, _HL_Color);
    break;

  // OSD with a Border to resize.
  case resizable:
    // Draw big Borders.
    drawBorders(_RS_W, _RS_H, _X_Display, _Y_Display, _X_Display + _W_Display,
                _Y_Display + _H_Display, _RS_Color);
    break;
  ...
  case movable:
    ...
    if (!_OSD_Name.empty())
      drawText(_X_Display + _W_Display / 2,
               _Y_Display + _H_Display - _RS_H - _TB_H_Display / 2, _OSD_Name,
               _TB_Pen);
    break;
  }
} // display //
...
void COSD::drawBG() {
  ...
  case BG_plain:
    Driver->drawQuad(_X_Display, _Y_Display, _X_Display + _W_Display,
                     _Y_Display + _H_Display, _BG_Color);
    break;

  // Background is stretch at the OSD size.
  case BG_stretch:
    Driver->drawBitmap(_X_Display, _Y_Display, _W_Display, _H_Display,
                       *CInterfMngr::getTexture(_BG), true, _BG_Color);
    break;
...
void COSD::drawTB() {
  ...
  // Backgroung is only made by 1 color (RGBA).
  case TB_plain:
    Driver->drawQuad(_X_Display + _RS_W,
                     _Y_Display + _H_Display - _RS_H - _TB_H_Display,
                     _X_Display + _W_Display - _RS_W,
                     _Y_Display + _H_Display - _RS_H, _TB_Color);
    break;

  // Background is stretch at the OSD size.
  case TB_stretch:
    Driver->drawBitmap(_X_Display + _RS_W,
                       _Y_Display + _H_Display - _RS_H - _TB_H_Display,
                       _W_Display - 2 * _RS_W, _TB_H_Display,
                       *CInterfMngr::getTexture(_TB), true, _TB_Color);
    break;
...
//-----------------------------------------------
void COSD::drawBorders(float bSizeW, float bSizeH, float x0, float y0, float x1,
                       float y1, const CRGBA &color) {
...
void COSD::resize(uint32 width, uint32 height) {
  ...
  for (TMapControls::iterator it = _Controls.begin(); it != _Controls.end();
       it++) {
    // If the control as a reference different than the OSD -> the Reference
    // will resize this control.
  ...
  if (fullUse) {
    if (_RS_Mode != no_resize) {
      ...
      default:
        ...
        if (x >= _X_Display && x < _X_Display + _W_Display && y >= _Y_Display &&
          ...
          // X is in the right part of the OSD (move, resize right, or resize
          // right corners)
          ...
          // X is in the left part of the OSD (move, resize left, or resize left
          // corners)
          ...
          // Y is in the Top part of the OSD (move, resize top, or resize top
          // corners)
          ...
          // Y is in the bottom part of the OSD (move, resize bottom, or resize
          // bottom corners)
          ...
          testMode(x, y, _X_Display + _RS_W * 2, _Y_Display + _RS_H * 2,
                   _X_Display + _W_Display - _RS_W * 2,
                   _Y_Display + _H_Display - _TB_H_Display - _RS_H * 2);
          // ...
          used = true;
        } else {
          _OSD_Mode = none;
...
void COSD::click(float x, float y, bool &taken) {
  ...
  if (_PopUp) {
    if ((x < _X_Display) || (x >= _X_Display + _W_Display) ||
        (y < _Y_Display) || (y >= _Y_Display + _H_Display)) {
...
void COSD::clickRight(float x, float y, bool &taken) {
  ...
  // if pop_up and cick was outside the window bounds: destroy that window
  if (_PopUp && ((x < _X_Display) || (x >= _X_Display + _W_Display) ||
                 (y < _Y_Display) || (y >= _Y_Display + _H_Display))) {
    CInterfMngr::deleteOSD(this->_Id);
    return;
  }
...
//-----------------------------------------------
void COSD::testMode(float x, float y, float rectX0, float rectY0, float rectX1,
                    float rectY1) {
  if (y > rectY1)
    _OSD_Mode = movable;
  else {
...
//-----------------------------------------------
void COSD::osdMode(TMode osdMode) { _OSD_Mode = osdMode; } // osdMode //

//-----------------------------------------------
// osdMode :
// Return the current OSD Mode.
//-----------------------------------------------
COSD::TMode COSD::osdMode() const { return _OSD_Mode; } // osdMode //
...
// Return the OSD Name.
//-----------------------------------------------
ucstring COSD::osdName() const { return _OSD_Name; } // osdName //
...
//-----------------------------------------------
uint COSD::osdUpdateFunc() const { return _OSD_Update_Func; } // updateFunc //
...
//-----------------------------------------------
COSD::TBG COSD::bgMode() const { return _BG_Mode; } // bgMode //

//-----------------------------------------------
// bg :
// Set the texture Id for the Background.
//-----------------------------------------------
void COSD::bg(uint b) { _BG = b; } // bg //

//-----------------------------------------------
// bg :
// Get background Id.
//-----------------------------------------------
uint COSD::bg() const { return _BG; } // bg //

//-----------------------------------------------
// bgColor :
// Set the Background RGBA.
//-----------------------------------------------
void COSD::bgColor(const CRGBA &bRGBA) { _BG_Color = bRGBA; } // bgColor //

//-----------------------------------------------
// bgColor :
// Get the background RGBA.
//-----------------------------------------------
CRGBA COSD::bgColor() const { return _BG_Color; } // bgColor //
...
//-----------------------------------------------
void COSD::tbMode(TTB mode) { _TB_Mode = mode; } // tbMode //

//-----------------------------------------------
// tbMode :
// Get the Title Bar display mode.
//-----------------------------------------------
COSD::TTB COSD::tbMode() const { return _TB_Mode; } // tbMode //

//-----------------------------------------------
// tb :
// Set the texture Id for the Title Bar.
//-----------------------------------------------
void COSD::tb(uint t) { _TB = t; } // tb //

//-----------------------------------------------
// tb :
// Get Title Bar Id.
//-----------------------------------------------
uint COSD::tb() const { return _TB; } // tb //

//-----------------------------------------------
// tbColor :
// Set the Title Bar RGBA.
//-----------------------------------------------
void COSD::tbColor(const CRGBA &tRGBA) { _TB_Color = tRGBA; } // tbColor //

//-----------------------------------------------
// tbColor :
// Get the Title Bar RGBA.
//-----------------------------------------------
CRGBA COSD::tbColor() const { return _TB_Color; } // tbColor //
...
//-----------------------------------------------
float COSD::tbHeight() const { return _TB_H; } // tbHeight //

//-----------------------------------------------
// pen :
// Set the Pen for the Title Bar.
//-----------------------------------------------
void COSD::tbPen(const CPen &pen) { _TB_Pen = pen; } // pen //

//-----------------------------------------------
// pen :
// Get the Pen of the Title Bar.
//-----------------------------------------------
CPen COSD::tbPen() const { return _TB_Pen; } // pen //
...
//-----------------------------------------------
void COSD::hlColor(const CRGBA &hlColor) { _HL_Color = hlColor; } // hlColor //

//-----------------------------------------------
// hlColor :
// Get the HighLight Color.
//-----------------------------------------------
CRGBA COSD::hlColor() const { return _HL_Color; } // hlColor //
...
//-----------------------------------------------
float COSD::hlSize() const { return _HL_Size; } // hlSize //
...
// Set the Resize Mode.
//-----------------------------------------------
void COSD::rsMode(TResize rsMode) { _RS_Mode = rsMode; } // rsMode //

//-----------------------------------------------
// rsMode :
// Get the Resize Mode.
//-----------------------------------------------
COSD::TResize COSD::rsMode() const { return _RS_Mode; } // rsMode //

//-----------------------------------------------
// rsColor :
// Set Resize borders Color
//-----------------------------------------------
void COSD::rsColor(CRGBA rsColor) { _RS_Color = rsColor; } // rsColor //

//-----------------------------------------------
// rsColor :
// Get Resize borders Color
//-----------------------------------------------
CRGBA COSD::rsColor() const { return _RS_Color; } // rsColor //
...
//-----------------------------------------------
float COSD::rsSize() const { return _RS_Size; } // rsSize //
...
void COSD::delChild(uint idCtrl) {
  ...
  if (it != _Controls.end()) {
    ...
    if (ctrl) {
      // Erase Control.
      for (TListControl::iterator itChild = _Children.begin();
           itChild != _Children.end(); ++itChild) {
        if ((*itChild) == ctrl) {
...
//-----------------------------------------------
bool COSD::testInRect(float x, float y, float rectX0, float rectY0,
...
bool COSD::resizeMode(float x, float y) {
  ...
  enum TXState { X_0 = 0x00, X_1 = 0x01, X_2 = 0x02, X_3 = 0x03, X_4 = 0x04 };

  enum TYState { Y_0 = 0x00, Y_1 = 0x10, Y_2 = 0x20, Y_3 = 0x30, Y_4 = 0x40 };

  TXState x_State;
  TYState y_State;

  // X POSITION in the OSD.
  // X is in the right part of the OSD (move, resize right, or resize right
  // corners)
  if (x >= _X_Display + _W_Display - cornerSizeX) {
    // Resize right or right corners.
    if (x >= _X_Display + _W_Display - _RS_W)
  ...
  // Y is in the bottom part of the OSD (move, resize bottom, or resize bottom
  // corners)
  else if (y < _Y_Display + cornerSizeY) {
    // Resize bottom or bottm corners.
    if (y < _Y_Display + _RS_H)
...
void COSD::getText(uint idCtrl) {
  ...
  CONTROL_SCRIPT_MACRO(
      // Get the Text.
      else if (strcmp(ptr, "Text:") == 0) idTxt = getInt();
      // Get the Pen.
      else if (strcmp(ptr, "Pen:") == 0) idPen = getInt();)

  // Create the list.
  CText *text =
      new CText(idCtrl, x, y, xPixel, yPixel, CInterfMngr::getText(idTxt),
...
void COSD::getCapture(uint idCtrl) {
  ...
  CONTROL_SCRIPT_MACRO(
      // Get the Function.
      else if (strcmp(ptr, "Function:") == 0) numFunc = getInt();
      // Get the Pen.
      else if (strcmp(ptr, "Pen:") == 0) idPen = getInt();
      // Get the Prompt.
      else if (strcmp(ptr, "Prompt:") == 0) idPrompt = getInt();)

  // Create the list.
  CCapture *capture = new CCapture(idCtrl, x, y, xPixel, yPixel, w, h, wPixel,
                                   hPixel, numFunc, CInterfMngr::getPen(idPen));
  if (capture) {
...
void COSD::getButton(uint idCtrl) {
  ...
  CONTROL_SCRIPT_MACRO(
      ...
      else if (strcmp(ptr, "FunctionRight:") == 0) numFuncR = getInt();

      // Get the Function for double click
      else if (strcmp(ptr, "FunctionDouble:") == 0) numFuncD = getInt();
  ...
  // Create the button.
  CButton *button =
      new CButton(idCtrl, x, y, xPixel, yPixel, w, h, wPixel, hPixel, numFunc,
                  numFuncR, numFuncD, CInterfMngr::getButton(idCtrl));
  if (button) {
...
void COSD::getRadioButton(uint idCtrl) {
  ...
  CRadioButton *button = new CRadioButton(
      idCtrl, x, y, xPixel, yPixel, w, h, wPixel, hPixel, numFunc, numFuncR,
      numFuncD, CInterfMngr::getButton(idCtrl));
  if (button) {
    button->origin(origin);
    button->hotSpot(hs);
    button->ref(_X_Display, _Y_Display, _W_Display, _H_Display);
    ...
    while (ptr != NULL) {
      ...
      if (strcmp(ptr, "Buttons:") == 0) {
        ...
        while ((ptr != NULL) && (strcmp(ptr, "End") != 0)) {
          ...
          if (it != _Controls.end()) {
            if (radioController->add(
                    dynamic_cast<CRadioButton *>((*it).second)) == false) {
              nlerror("Interface Error : control %d can't be add in the radio "
                      "button %d",
                      buttonId, idCtrl);
            }
          }
...
void COSD::getBitmap(uint idCtrl) {
  ...
  CONTROL_SCRIPT_MACRO(
      // Get the Texture.
      else if (strcmp(ptr, "Texture:") == 0) idtexture = getInt();
      // Get the RGBA.
      else if (strcmp(ptr, "RGBA:") == 0) rgba = getRGBA();
      // tiled or stretched
  ...
  // Create the list.
  CBitm *bitmap = new CBitm(idCtrl, x, y, xPixel, yPixel, w, h, wPixel, hPixel,
                            idtexture, rgba);
  if (bitmap) {
    bitmap->tiled(tiled);
...
void COSD::getList(uint idCtrl) {
  ...
  CONTROL_SCRIPT_MACRO(
      ...
      else if (strcmp(ptr, "UpTexture:") == 0) upTexture = getInt();
      // Get the Texture for the 'down' arrow
      else if (strcmp(ptr, "DownTexture:") == 0) downTexture = getInt();
      // Get the Texture for the scroll bar body
      else if (strcmp(ptr, "ScrollBarTexture:") == 0) scrollBarTexture =
          getInt();
      // Get the RGBA for the scroll bar
      else if (strcmp(ptr, "ScrollBarRGBA:") == 0) scrollBarRgba = getRGBA();)

  // Create the list.
  CList *list = new CList(idCtrl, x, y, xPixel, yPixel, w, h, wPixel, hPixel,
                          CInterfMngr::getPen(idPen));
  if (list) {
    list->origin(origin);
    list->hotSpot(hs);
...
void COSD::getMultiList(uint idCtrl) {
  ...
  CONTROL_SCRIPT_MACRO(
      ...
      else if (strcmp(ptr, "NbColumns:") == 0) nbCol = getInt();

      // Get the columns size.
      else if (strcmp(ptr, "ColSize:") == 0) colSize = getVectorOfFloat(nbCol);

      // Get the Pen.
      else if (strcmp(ptr, "Pen:") == 0) idPen = getInt();

      // Get the Spacing.
      else if (strcmp(ptr, "Spacing:") == 0) spacing = getInt();

      // Get the specified Line Height.
      else if (strcmp(ptr, "LineHeight:") == 0) lineHeight = getInt();
  ...
  // Create the list.
  CMultiList *list = new CMultiList(idCtrl, x, y, xPixel, yPixel, w, h, wPixel,
                                    hPixel, CInterfMngr::getPen(idPen), nbCol);
  if (list) {
    list->origin(origin);
    list->hotSpot(hs);
    list->ref(_X_Display, _Y_Display, _W_Display, _H_Display);
...
void COSD::getChatBox(uint idCtrl) {
  ...
  CONTROL_SCRIPT_MACRO(
      ...
      else if (strcmp(ptr, "LeftClickFunction:") == 0) leftFunc = getInt();

      // Get the function to execute on left click on player name
      else if (strcmp(ptr, "RightClickFunction:") == 0) rightFunc = getInt();
      // Get the Texture for the 'up' arrow
      else if (strcmp(ptr, "UpTexture:") == 0) upTexture = getInt();
      // Get the Texture for the 'down' arrow
      else if (strcmp(ptr, "DownTexture:") == 0) downTexture = getInt();
  ...
  // Create the list.
  CChatControl *chat =
      new CChatControl(idCtrl, x, y, xPixel, yPixel, w, h, wPixel, hPixel,
                       leftFunc, rightFunc, CInterfMngr::getPen(idPen));
  if (chat) {
    chat->origin(origin);
    chat->hotSpot(hs);
...
void COSD::getCandidateList(uint idCtrl) {
  ...
  CONTROL_SCRIPT_MACRO(
      // Get the columns size.
      else if (strcmp(ptr, "ColSize:") == 0) colSize = getVectorOfFloat(2);
      ...
      else if (strcmp(ptr, "SelectionRGBA:") == 0) selRgba = getRGBA();)

  // Create the list.
  CCandidateList *list =
      new CCandidateList(idCtrl, x, y, xPixel, yPixel, w, h, wPixel, hPixel,
                         CInterfMngr::getPen(idPen));
  if (list) {
    list->origin(origin);
    list->hotSpot(hs);
...
void COSD::getChatInput(uint idCtrl) {
  ...
  CChatInput *chat =
      new CChatInput(idCtrl, x, y, xPixel, yPixel, w, h, wPixel, hPixel,
                     numFunc, CInterfMngr::getPen(idPen));
  if (chat) {
    chat->origin(origin);
    chat->hotSpot(hs);
    chat->ref(_X_Display, _Y_Display, _W_Display, _H_Display);
...
void COSD::getChoiceList(uint idCtrl) {
  ...
  // Create the chat input
  CChoiceList *choiceList =
      new CChoiceList(idCtrl, x, y, xPixel, yPixel, w, h, wPixel, hPixel,
                      CInterfMngr::getPen(idPen));
  if (choiceList) {
...
void COSD::getHorizontalList(uint idCtrl) {
  ...
  CONTROL_SCRIPT_MACRO(
      ...
      else if (strcmp(ptr, "ScrollLeft:") == 0) scrollLeftId = getInt();

      else if (strcmp(ptr, "ScrollRight:") == 0) scrollRightId = getInt();
  ...
  // Create the list.
  CHorizontalList *list =
      new CHorizontalList(idCtrl, x, y, xPixel, yPixel, w, h, wPixel, hPixel,
                          spacing, idtexture, rgba);
  if (list) {
    list->tiled(tiled);
...
void COSD::getControlList(uint idCtrl) {
  ...
  // Create the list.
  CControlList *list = new CControlList(idCtrl, x, y, xPixel, yPixel, w, h,
                                        wPixel, hPixel, spacing);
  if (list) {
    CScrollBar *scroll = list->getVScroll();
    if (scroll) {
      scroll->setDownArrowTextureOn(downTexture);
...
void COSD::getSpellList(uint idCtrl) {
  ...
  CONTROL_SCRIPT_MACRO(
      ...
      // Get the Pen.
      else if (strcmp(ptr, "SpellPen:") == 0) idSpellPen = getInt();
      // Get the Pen.
      else if (strcmp(ptr, "CommentPen:") == 0) idCommentPen = getInt();
      // Get the button width
      else if (strcmp(ptr, "ButtonW:") == 0) buttonW = getFloat();
      // Get the button width
      else if (strcmp(ptr, "ButtonH:") == 0) buttonH = getFloat();
      // Get the function to execute on left click on candidate name
      else if (strcmp(ptr, "LeftClickFunction:") == 0) leftFunc = getInt();
      // Get the function to execute on left click on candidate name
      else if (strcmp(ptr, "RightClickFunction:") == 0) rightFunc = getInt();
      // Get the height of a line in the control (relative to the size of the
      // list)
      else if (strcmp(ptr, "Line_H:") == 0) line_H = getFloat();
      // Get the height of a line in the control (in pixels)
      else if (strcmp(ptr, "Line_H_Pixel:") == 0) line_H_Pixel = getFloat();
      // Get the Texture for the 'up' arrow
      else if (strcmp(ptr, "UpTexture:") == 0) upTexture = getInt();
      // Get the Texture for the 'down' arrow
      else if (strcmp(ptr, "DownTexture:") == 0) downTexture = getInt();
  ...
  // Create the list.
  CSpellList *list = new CSpellList(idCtrl, x, y, xPixel, yPixel, w, h, wPixel,
                                    hPixel, spacing, leftFunc, rightFunc);
...
void COSD::getProgressBar(uint idCtrl) {
  ...
  CONTROL_SCRIPT_MACRO(
      ...
      else if (strcmp(ptr, "BarTexture:") == 0) barTexture = getInt();
      // Get the Texture for the background
      else if (strcmp(ptr, "BackgroundTexture:") == 0) bkgTexture = getInt();
      // Get the RGBA for the progress bar
      else if (strcmp(ptr, "BarRGBA:") == 0) barRgba = getRGBA();
      // Get the RGBA for the background
      else if (strcmp(ptr, "BackgroundRGBA:") == 0) bkgRgba = getRGBA();
      // get the range
      else if (strcmp(ptr, "Range:") == 0) range = getInt();
      // get the step inc
      else if (strcmp(ptr, "Step:") == 0) step = getInt();
      // Get the Pen.
      else if (strcmp(ptr, "Pen:") == 0) idPen = getInt();
      // Get the Text
      else if (strcmp(ptr, "Text:") == 0) idText = getInt();

  )

  // Create the control
  CProgressBar *ctrl = new CProgressBar(idCtrl, x, y, xPixel, yPixel, w, h,
                                        wPixel, hPixel, range);
...
void COSD::getCastingBar(uint idCtrl) {
  ...
  // Create the control
  CCastingBar *ctrl = new CCastingBar(idCtrl, x, y, xPixel, yPixel, w, h,
                                      wPixel, hPixel, range);
...
void COSD::getBrickControl(uint idCtrl) {
  ...
  // Create the control.
  CBrickControl *brick = new CBrickControl(idCtrl, x, y, xPixel, yPixel, w, h,
                                           wPixel, hPixel, leftFunc, rightFunc);
  if (brick) {
    brick->origin(origin);
    brick->hotSpot(hs);
...
void COSD::open(ifstream &file) {
  ...
  while (!file.eof()) {
    ...
    // if the first char is a / then this is a comment line, skip it and go to
    // next line
    if ((key != NULL) && (*key) != '/') {
      // Make the id.
      ...
      default:
        nlerror("Line %d : ID %d is undeclared in \"ctrls.txt\" OR the type is "
                "unknown !!",
                line, idCtrl);
        break;
      }
