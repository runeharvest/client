 ...
 * Applying patch is copying from the source 'Index' at offset 'Offset' an
 * amount of data of length 'Length'. \author Matthieu 'TrapII' Besson \author
 * Nevrax France \date July 2004
 */
struct SXDeltaCtrl {
  struct SSourceInfo {
    std::string Name;
 ...
 * We read only the 1.1 version. 'Compressed From' and 'Compressed To' are not
 * supported. \author Matthieu 'TrapII' Besson \author Nevrax France \date July
 * 2004
 */
class CXDeltaPatch {
public:
  ...
  // Tools
  static TApplyResult apply(const std::string &sPatchFilename,
                            const std::string &sFileToPatchFilename,
                            const std::string &sOutputFilename,
                            std::string &errorMsg, ICallBack *pCallBack = NULL);
  static bool info(const std::string &sPatchFilename);

  // Load a patch file
  bool load(const std::string &sFilename);

  TApplyResult apply(const std::string &sFileToPatch,
                     const std::string &sFileOutput, std::string &errorMsg);

  bool isNoVerify() { return (_Flags & XDELTA_FLAG_NO_VERIFY) != 0; }
  bool isFromCompressed() {
    return (_Flags & XDELTA_FLAG_FROM_COMPRESSED) != 0;
  }
  bool isToCompressed() { return (_Flags & XDELTA_FLAG_TO_COMPRESSED) != 0; }
  bool isPatchCompressed() {
    return (_Flags & XDELTA_FLAG_PATCH_COMPRESSED) != 0;
  }
  ...
  bool checkIntegrity(CXDPFileReader &rFR, const NLMISC::CHashKeyMD5 &md5,
                      uint32 nLength);
...
public:
  CXDPFileReader();
  ~CXDPFileReader();
  bool init(const std::string &sFilename, sint32 nLowerBound,
