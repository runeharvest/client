...
NLMISC::CVariable<bool> VerboseAnimParsing("client", "VerboseAnimParsing",
                                           "log the animation parsing info",
                                           false, 0, true);
...
void CAnimationSheet::build(const NLGEORGES::UFormElm &item) {
  ...
  Reverse = bTmp;
  // In case of an animation that is a rotation but with no rotation (like
  // characters on a mount).
  if (!item.getValueByName(VirtualRot, "virtual rotation"))
    nlwarning("CAnimation:build: key 'virtual rotation' not found.");
  // Do we need to hide the entity at the end of the animation ? (useful for
  // Kami).
  ...
  for (uint i = 0; i < 7; i++) {
    ...
    if (next >= 0) {
      ...
      if (!item.getValueByName(nextWeight,
                               toString("next%d weight", i + 1).c_str())) {
...
void CAnimationSheet::serial(NLMISC::IStream &f) {
  // serialize the animation name to be able to compute the animation when
  // loading.
...
//-----------------------------------------------
TAnimStateId
CAnimationStateSheet::getAnimationStateId(const std::string &stateName) {
  if (stateName.empty())
    return UnknownState;

  std::map<std::string, TAnimStateId>::const_iterator ite =
      _StringToAnimStateId.find(stateName);
...
const string &CAnimationStateSheet::getAnimationStateName(TAnimStateId id) {
  ...
  if (id == UnknownState)
    ...
    if ((uint32)id >= _AnimStateIdToString.size()) {
      nlwarning("ASS:getAnimationStateName: id(%d) out of range (size=%u)",
                _AnimStateIdToString.size());
      return unknownString;
    }
...
void CAnimationStateSheet::build(const NLGEORGES::UFormElm &item) {
  ...
  if (!item.getNodeByName(&elmt, "animations"))
    nlwarning(
        "CAnimationState:build: the node 'animations' is not referenced.");
    ...
    if (arraySize > 0) {
      ...
      for (uint i = 0; i < arraySize; ++i) {
        ...
        if (animElmt) {
          ...
          nlwarning("CAnimationState::build : element (%d) should be here in "
                    "'animations'.");
      }
    }
  }
  ...
  if (!item.getValueByName(sTmp, "LodCharacterAnimation"))
    nlwarning("CAnimationState:build: the node 'LodCharacterAnimation' is not "
              "referenced.");
  ...
  if (!item.getValueByName(bTmp, "Display Objects"))
    nlwarning(
        "CAnimationState:build: the node 'Display Objects' is not referenced.");
  DisplayObjects = bTmp;

  // Get MeleeImpactDelay (relevant only for melee attack states)
  if (!item.getValueByName(MeleeImpactDelay, "MeleeImpactDelay"))
    nlwarning("CAnimationState:build: the node 'MeleeImpactDelay' is not "
              "referenced.");
...
void CAnimationStateSheet::serial(NLMISC::IStream &f) {
  ...
  string sStateName;
  if (!f.isReading())
    sStateName = getAnimationStateName((TAnimStateSheetId)State);
  f.serial(sStateName);
  ...
  // update statics maps
  if (f.isReading()) {
    _StringToAnimStateId.insert(
        pair<string, TAnimStateSheetId>(sStateName, (TAnimStateSheetId)State));
      ...
      if (rootElmt.getNodeByName(&elmt, stateName)) {
        ...
        if (elmt) {
          ...
          if (stateId != CAnimationStateSheet::UnknownState) {
            ...
            // Already exist ?
            if (AnimationStates[stateId].State ==
              ...
              nlwarning("CAnimationSet::build : Insertion failed state '%s' "
                        "already exit.",
                        stateName.c_str());
          } else
            nlwarning("CAnimationSet::build : Bad Animation State ID. -> state "
                      "'%s' not inserted.",
                      stateName.c_str());
        }
        ...
        if (!rootElmt.getValueByName(essential, essentialKey.c_str()))
          nlwarning("CAnimationSet:build: the node '%s' is not referenced.",
                    string(stateName + string(".Essential")).c_str());
        if (essential) {
          if (!animPresent)
            if (VerboseAnimParsing)
        ...
        nlwarning("Cannot get the state '%s' (element '%d').",
                  stateName.c_str(), i);
    }
      ...
      for (uint i = 0; i < size; ++i) {
        ...
        for (uint j = 0; j < modes.size(); ++j)
          for (uint k = 0; k < types.size(); ++k)
            if (fileForm->getRootNode().getValueByName(
                    useful, string(modes[j] + "." + types[k]).c_str())) {
              if (useful)
                nSize++;
            }
          ...
          for (uint k = 0; k < types.size(); ++k)
            if (fileForm->getRootNode().getValueByName(
              ...
              if (useful) {
                ...
                string animset = toString("%s_%s%s", entityName.c_str(),
                                          modes[j].c_str(), types[k].c_str());
                // Build
                CAnimationSetSheet animationSet;
                AnimSetList[nTotalSize + nSize].Name =
                    animset + ".animation_set";
                CSmartPtr<UForm> fileFormAS = formLoader->loadForm(
                    AnimSetList[nTotalSize + nSize].Name.c_str());
                if (fileFormAS != NULL) {
                  AnimSetList[nTotalSize + nSize].build(
                      fileFormAS->getRootNode());
                  nSize++;
                } else
                  nlwarning("Invalid Form : %s.",
                            AnimSetList[nTotalSize + nSize].Name.c_str());
              }
            }
