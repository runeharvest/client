...
void CFXSheet::build(const NLGEORGES::UFormElm &item) {
  ...
  if (result) {
    ...
    nlwarning(
        "CFXSheet::build: there is no element 'Trail_List' in the form %s.",
        Id.toString().c_str());
  }
  ...
  if (elmt) {
    ...
    for (uint i = 0; i < arrawSize; ++i) {
      ...
      if (psElmt) {
        ...
        // Weak Combo
        if (!psElmt->getValueByName(params.UserParam0,
                                    "weak combo.User Param 0"))
          debug("Key 'weak combo.User Param 0' not found.");
        if (!psElmt->getValueByName(params.UserParam1,
                                    "weak combo.User Param 1"))
          debug("Key 'weak combo.User Param 1' not found.");
        if (!psElmt->getValueByName(params.UserParam2,
                                    "weak combo.User Param 2"))
          debug("Key 'weak combo.User Param 2' not found.");
        if (!psElmt->getValueByName(params.UserParam3,
                                    "weak combo.User Param 3"))
          debug("Key 'weak combo.User Param 3' not found.");
        ps.Power.push_back(params);

        // Strng Combo
        if (!psElmt->getValueByName(params.UserParam0,
                                    "strong combo.User Param 0"))
          debug("Key 'strong combo.User Param 0' not found.");
        if (!psElmt->getValueByName(params.UserParam1,
                                    "strong combo.User Param 1"))
          debug("Key 'strong combo.User Param 1' not found.");
        if (!psElmt->getValueByName(params.UserParam2,
                                    "strong combo.User Param 2"))
          debug("Key 'strong combo.User Param 2' not found.");
        if (!psElmt->getValueByName(params.UserParam3,
                                    "strong combo.User Param 3"))
