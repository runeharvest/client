...
//-----------------------------------------------
void CCharacterSheet::readEquipment(const NLGEORGES::UFormElm &form,
                                    const string &key, CEquipment &slot) {
...
void CCharacterSheet::build(const NLGEORGES::UFormElm &item) {
  ...
  if (!item.getValueByName(raceStr, "Basics.Race"))
    ...
    if (EGSPD::CPeople::toString(Race) != raceStr) {
      debug(toString("In sheet '%s': invalid race '%s', race is set to unknow",
                     Id.toString().c_str(), raceStr.c_str()));
    }
  } else
  ...
  if (!ObjectInRightHand.IdItem) {
    ...
    if (!right.empty())
      ObjectInRightHand.IdItem =
          ClientSheetsStrings.add(NLMISC::toLowerAscii(right));
  }
  ...
  if (!ObjectInLeftHand.IdItem) {
    ...
    if (!left.empty())
      ObjectInLeftHand.IdItem =
          ClientSheetsStrings.add(NLMISC::toLowerAscii(left));
  }
  ...
  if (item.getValueByName(AnimSetBaseName, "3d data.AnimSetBaseName")) {
    if (AnimSetBaseName.empty())
      ...
      AnimSetBaseName = NLMISC::toLowerAscii(
          AnimSetBaseName); // Force the CASE in UPPER to not be CASE SENSITIVE.
  } else
  ...
  if (!item.getValueByName(Scale, "3d data.Scale"))
    ...
    if (Scale <= 0.0f) {
      nlwarning("CCharacterSheet:build: Scale(%f) <= 0.0 so fix scale to 1.0",
                Scale);
      Scale = 1.0f;
    }
  }
        ...
        for (uint i = 0; i < altClothesArraySize; ++i) {
          if (elm->getArrayValue(altClothes, i)) {
            if (!altClothes.empty()) {
              ...
              debug(toString("'%s' field empty for the index '%d'.",
                             alternativeClothesKey, i));
          } else
            debug(
                toString("'%s' cannot get the array value for the index '%d'.",
                         alternativeClothesKey, i));
        }
      } else
        debug(
            toString("'%s' cannot get the array size.", alternativeClothesKey));
    } else
        ...
        if (hairItemArraySize > 0) {
          ...
          for (uint i = 0; i < hairItemArraySize; ++i) {
            ...
            if (elm->getArrayNodeName(arrayNodeName, i))
              readEquipment(item,
                            string(hairItemList) + "[" + toString(i) + "]",
                            HairItemList[i]);
          }
        }
      } else
  ...
  for (uint k = 0; k < NumAttackLists; ++k) {
    ...
    if (item.getValueByName(attackListName,
                            toString("attack_list%d", (int)k).c_str()) &&
...
// ***************************************************************************
void CCharacterSheet::getWholeEquipmentList(
    std::vector<const CEquipment *> &equipList) const {
  equipList.clear();
