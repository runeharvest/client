...
// CAnimationFXSheet
//-----------------------------------------------
CAnimationFXSheet::CAnimationFXSheet(const std::string &psName,
...
void CAnimationFXSheet::build(const NLGEORGES::UFormElm &item,
  ...
  ok &=
      item.getValueByName(TrajectoryAnim, (prefix + "TrajectoryAnim").c_str());
  ...
  bool okRepeatMode =
      item.getValueByName(repeatMode, (prefix + "RepeatMode").c_str());
  if (okRepeatMode) {
