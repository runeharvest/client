...
void CAttackIDSheet::build(const NLGEORGES::UFormElm &item,
  ...
  case Range: {
    ...
    if (item.getValueByName(rangeWeaponType,
                            (prefix + "RangeWeaponType").c_str())) {
      RangeWeaponType =
  ...
  case Magic:
    ...
    break;
  case Creature:
    item.getValueByName(CreatureAttackIndex,
                        (prefix + "CreatureAttackIndex").c_str());
    break;
  case DamageShield:
    item.getValueByName(DamageShieldType,
                        (prefix + "DamageShieldType").c_str());
    break;
  default:
...
// *****************************************************************************************
void CAttackIDSheet::CSpellInfo::build(const NLGEORGES::UFormElm &item,
                                       const std::string &prefix) {
  uint32 spellID = 0;
...
bool operator==(const CAttackIDSheet &lhs, const CAttackIDSheet &rhs) {
  if (lhs.Type != rhs.Type)
    return false;
  switch (lhs.Type) {
  case CAttackIDSheet::Melee:
    return true;
  case CAttackIDSheet::Range:
    return lhs.RangeWeaponType == rhs.RangeWeaponType;
  case CAttackIDSheet::Magic:
    return lhs.SpellInfo == rhs.SpellInfo;
  case CAttackIDSheet::Creature:
    return lhs.CreatureAttackIndex == rhs.CreatureAttackIndex;
  case CAttackIDSheet::DamageShield:
    return lhs.DamageShieldType == rhs.DamageShieldType;
  default:
    return false;
  }
}
