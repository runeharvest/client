...
enum TSkyRefColor {
  ...
  DiffuseColorRef,
  ConstantColor0Ref,
  ConstantColorLastRef = ConstantColor0Ref + SKY_MAX_NUM_STAGE,
  ParticleEmittersColorRef = ConstantColorLastRef
}; // color serves as a reference to see if object must be hiden. This is the
   // case when the alpha is 0
enum TSkyColorMode {
  Unused = 0,
  BitmapColor,
  FogColor,
  BitmapColorModulatedByFogColor
};
...
public:
  ...
  public:
    sint32 TargetTextureStage; // the texture stage to which the gradient must
                               // be applied.
    /** each bitmap in the following list  gives the gradient depending on
     * weather valuet level. The V coordinate gives the gradient values, and U
     * is offseted by the weather value each bitmap match a weather state. First
     * bitmap maps to light level = 0 and last bitmap maps to light level = 1
     * for intermediary weather values, the two nearest bitmap are blended to
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  // A version of a sky object. If materials are too complex, a fallback version
  // may be used instead (on gpu with 2 stages) //
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  class CVersionSheet {
  public:
    ...
    // texture scaling : each bitmap gives a scaling factor depending on weather
  ...
  CVersionSheet
      FallbackPass[2]; // up to 2 fallback versions (for multi-pass fallback)
