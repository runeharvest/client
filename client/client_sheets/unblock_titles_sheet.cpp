      ...
      for (uint i = 0; i < NbTitlesUnblock; ++i) {
        ...
        if (!(arrayTUElt->getArrayNode(&TUElt, i) && TUElt)) {
          ...
          if (!TUElt->getValueByName(bReserved, "Reserved")) {
            nlwarning(
                "<CSkillsTreeSheet::build> can't get node Reserved in sheet");
          }

          string strSkillsNeeded;
          if (!TUElt->getValueByName(strSkillsNeeded, "SkillsNeeded")) {
            nlwarning("<CSkillsTreeSheet::build> can't get node SkillsNeeded "
                      "in sheet");
          }
          ...
          if (!TUElt->getValueByName(strMinFames, "MinFames")) {
            nlwarning(
                "<CSkillsTreeSheet::build> can't get node MinFames in sheet");
          }

          string strMaxFames;
          if (!TUElt->getValueByName(strMaxFames, "MaxFames")) {
            nlwarning(
                "<CSkillsTreeSheet::build> can't get node MaxFames in sheet");
          }

          // cf TPVPClan enum (pvp_clan.h)
          if (!TUElt->getValueByName(TitlesUnblock[title].CivNeeded,
                                     "CivNeeded")) {
            nlwarning(
                "<CSkillsTreeSheet::build> can't get node CivNeeded in sheet");
          }

          // cf TPVPClan enum (pvp_clan.h)
          if (!TUElt->getValueByName(TitlesUnblock[title].CultNeeded,
                                     "CultNeeded")) {
            nlwarning(
                "<CSkillsTreeSheet::build> can't get node CultNeeded in sheet");
          }

          if (!TUElt->getValueByName(TitlesUnblock[title].CharOldness,
                                     "CharOldness")) {
            nlwarning("<CSkillsTreeSheet::build> can't get node CharOldness in "
                      "sheet");
          }

          if (!TUElt->getValueByName(TitlesUnblock[title].CharPlayedTime,
                                     "CharPlayedTime")) {
            nlwarning("<CSkillsTreeSheet::build> can't get node CharPlayedTime "
                      "in sheet");
          }

          if (!TUElt->getValueByName(TitlesUnblock[title].AccountOldness,
                                     "AccountOldness")) {
            nlwarning("<CSkillsTreeSheet::build> can't get node AccountOldness "
                      "in sheet");
          }

          if (!TUElt->getValueByName(TitlesUnblock[title].AuthorRating,
                                     "AuthorRating")) {
            nlwarning("<CSkillsTreeSheet::build> can't get node AuthorRating "
                      "in sheet");
          }

          if (!TUElt->getValueByName(TitlesUnblock[title].AMRating,
                                     "AMRating")) {
            nlwarning(
                "<CSkillsTreeSheet::build> can't get node AMRating in sheet");
          }

          if (!TUElt->getValueByName(TitlesUnblock[title].OrganizerRating,
                                     "OrganizerRating")) {
            nlwarning("<CSkillsTreeSheet::build> can't get node "
                      "OrganizerRating in sheet");
          }

          string strItemsNeeded;
          if (!TUElt->getValueByName(strItemsNeeded, "ItemsNeeded")) {
            nlwarning("<CSkillsTreeSheet::build> can't get node ItemsNeeded in "
                      "sheet");
          }
          ...
          if (!strSkillsNeeded.empty()) {
            uint16 skillsNeededSz =
                (uint16)TitlesUnblock[title].SkillsNeeded.size();
            ...
            for (j = 0; j < vSkill.size() / 2; ++j) {
              TitlesUnblock[title].SkillsNeeded[skillsNeededSz].push_back(
                  vSkill[j * 2]);
              fromString(vSkill[j * 2 + 1], tmpLevel);
              TitlesUnblock[title].SkillsLevelNeeded[skillsNeededSz].push_back(
                  tmpLevel);
            }
          }
          ...
          if (!strBricksNeeded.empty()) {
            ...
            for (j = 0; j < vBrick.size(); ++j) {
              TitlesUnblock[title].BricksNeeded.push_back(
                  CSheetId(vBrick[j] + ".sbrick"));
            }
          }
          ...
          if (!strItemsNeeded.empty()) {
            uint16 itemsNeededSz =
                (uint16)TitlesUnblock[title].ItemsNeeded.size();
            ...
            for (j = 0; j < vItem.size() / 2; ++j) {
              TitlesUnblock[title].ItemsNeeded[itemsNeededSz].push_back(
                  CSheetId(vItem[j * 2] + ".sitem"));
              fromString(vItem[j * 2 + 1], tmpQuality);
              TitlesUnblock[title].ItemsQualityNeeded[itemsNeededSz].push_back(
                  tmpQuality);
            }
          }
        }
