...
CContinentParameters::CContinentParameters() {
  ...
  FogStart = 0.0f;
  FogEnd = 0.0f;
  RootFogStart = 0.0f, RootFogEnd = 0.0f;
...
//=========================================================================
CContinentSheet::CContinentSheet() { Type = CEntitySheet::CONTINENT; }
...
void CContinentParameters::build(const NLGEORGES::UFormElm &item) {
  ...
  if (item.getNodeByName(&elm, "LightLandscapeDusk") && elm) {
    if (!LandscapeLightDusk.build(*elm))
      LandscapeLightDusk.blend(LandscapeLightDay, LandscapeLightNight, 0.5f);
  } else
    LandscapeLightDusk.blend(LandscapeLightDay, LandscapeLightNight, 0.5f);
  ...
  if (item.getNodeByName(&elm, "LightEntityDusk") && elm) {
    if (!EntityLightDusk.build(*elm))
      EntityLightDusk.blend(EntityLightDay, EntityLightNight, 0.5f);
  } else
    EntityLightDusk.blend(EntityLightDay, EntityLightNight, 0.5f);
  ...
  if (item.getNodeByName(&elm, "LightRootDusk") && elm) {
    if (!RootLightDusk.build(*elm))
      RootLightDusk.blend(RootLightDay, RootLightNight, 0.5f);
  } else
    RootLightDusk.blend(RootLightDay, RootLightNight, 0.5f);
  ...
  item.getValueByName(EntitySunContributionMaxThreshold,
  ...
  for (uint season = 0; season < EGSPD::CSeason::Invalid; ++season) {
    item.getValueByName(
        CanopyIGfileName[season],
        (EGSPD::CSeason::toString((EGSPD::CSeason::TSeason)season) + "CanopyIG")
            .c_str());
  }
  ...
  for (uint season = 0; season < EGSPD::CSeason::Invalid; ++season) {
    item.getValueByName(
        TileColorMono[season],
        (EGSPD::CSeason::toString((EGSPD::CSeason::TSeason)season) +
         "TileColorMono")
            .c_str());
    item.getValueByName(
        TileColorFactor[season],
        (EGSPD::CSeason::toString((EGSPD::CSeason::TSeason)season) +
         "TileColorFactor")
            .c_str());
    item.getValueByName(
        StaticLightingFactor[season],
        (EGSPD::CSeason::toString((EGSPD::CSeason::TSeason)season) +
         "StaticLightingFactor")
            .c_str());
    item.getValueByName(
        SkySheet[season],
        ("SkySheet" + EGSPD::CSeason::toString((EGSPD::CSeason::TSeason)season))
            .c_str());
    ...
    string strSeason;
    item.getValueByName(
        strSeason, ("DisplayedSeasonFor" +
                    EGSPD::CSeason::toString((EGSPD::CSeason::TSeason)season))
                       .c_str());
    EGSPD::CSeason::TSeason readSeason = ForceDisplayedSeason[season] =
...
void CContinentParameters::buildFogMapBuild(const NLGEORGES::UFormElm &item) {
  ...
  item.getValueByName(FogMapBuild.Map[CFogMapBuild::NoPrecipitation],
                      "NoRainMap");
...
void CContinentSheet::build(const NLGEORGES::UFormElm &item) {
  ...
  static const char *seasonFuncName[] = {
      "SpringWeatherFunction", "SummerWeatherFunction", "AutumnWeatherFunction",
      "WinterWeatherFunction"};
  // added - 1 because there is an invalid season
  nlctassert(sizeof(seasonFuncName) / sizeof(seasonFuncName[0]) ==
