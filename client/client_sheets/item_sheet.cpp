...
void CItemSheet::build(const NLGEORGES::UFormElm &item) {
  ...
  SlotBF = 0;
  if (item.getNodeByName(&pElt, "basics.EquipmentInfo.EquipmentSlots") &&
      pElt) {
    // Get all slots.
    uint size;
    if (pElt->getArraySize(size)) {
      for (uint i = 0; i < size; ++i) {
        ...
        if (pElt->getArrayValue(slotName, i)) {
          ...
          SlotBF |=
              SINT64_CONSTANT(1)
              << (SLOTTYPE::stringToSlotType(NLMISC::toUpperAscii(slotName)));
        }
      }
    } else
  ...
  if (!item.getValueByName(family, "basics.family")) {
    ...
    Family = (ITEMFAMILY::EItemFamily)ITEMFAMILY::stringToItemFamily(
        NLMISC::toUpperAscii(family));
  ...
  if (!item.getValueByName(itemtype, "basics.ItemType")) {
    ...
    ItemType = (ITEM_TYPE::TItemType)ITEM_TYPE::stringToItemType(
        NLMISC::toUpperAscii(itemtype));
  ...
  TRANSLATE_ENUM(RequiredCharac, CHARACTERISTICS::Unknown,
                 CHARACTERISTICS::toCharacteristic, "basics.RequiredCharac");
  TRANSLATE_VAL(RequiredCharacLevel, "basics.MinRequiredCharacLevel");
  TRANSLATE_ENUM(RequiredSkill, SKILLS::unknown, SKILLS::toSkill,
                 "basics.RequiredSkill");
  TRANSLATE_VAL(RequiredSkillLevel, "basics.MinRequiredSkillLevel");

  // item Origin
  TRANSLATE_ENUM(ItemOrigin, ITEM_ORIGIN::UNKNOWN, ITEM_ORIGIN::stringToEnum,
                 "basics.origin");
  ...
  case ITEMFAMILY::COSMETIC: {
    ...
    if (pos == string::npos)
      nlwarning(
          "<loadCosmetics> Can't load the VPValue from sheet name in sheet %s",
          Id.toString().c_str());
    else {
      sint i = (sint)pos - 1;
      for (; i >= 0; i--) {
  ...
  case ITEMFAMILY::ARMOR: {
    // ArmorType
    TRANSLATE_ENUM(Armor.ArmorType, ARMORTYPE::UNKNOWN, ARMORTYPE::toArmorType,
                   "armor.Armor category");
  } break;
  // MELEE_WEAPON
  case ITEMFAMILY::MELEE_WEAPON: {
    // WeaponType
    TRANSLATE_ENUM(MeleeWeapon.WeaponType, WEAPONTYPE::UNKNOWN,
                   WEAPONTYPE::stringToWeaponType, "melee weapon.category");

    // Skill
    TRANSLATE_ENUM(MeleeWeapon.Skill, SKILLS::unknown, SKILLS::toSkill,
                   "melee weapon.skill");

    // DamageType
    TRANSLATE_ENUM(MeleeWeapon.DamageType, DMGTYPE::UNDEFINED,
                   DMGTYPE::stringToDamageType, "melee weapon.damage type");
  ...
  case ITEMFAMILY::RANGE_WEAPON: {
    // WeaponType
    TRANSLATE_ENUM(RangeWeapon.WeaponType, WEAPONTYPE::UNKNOWN,
                   WEAPONTYPE::stringToWeaponType, "range weapon.category");

    // Range weapon type
    TRANSLATE_ENUM(RangeWeapon.RangeWeaponType, RANGE_WEAPON_TYPE::Generic,
                   RANGE_WEAPON_TYPE::stringToRangeWeaponType,
                   "range weapon.RangeWeaponType");

    // Skill
    TRANSLATE_ENUM(RangeWeapon.Skill, SKILLS::unknown, SKILLS::toSkill,
                   "range weapon.skill");
  ...
  case ITEMFAMILY::AMMO: {
    // Skill
    TRANSLATE_ENUM(Ammo.Skill, SKILLS::unknown, SKILLS::toSkill,
                   "ammo.weapon type");

    // DamageType
    TRANSLATE_ENUM(Ammo.DamageType, DMGTYPE::UNDEFINED,
                   DMGTYPE::stringToDamageType, "ammo.damage type");
  ...
  case ITEMFAMILY::RAW_MATERIAL: {
    // Ecosystem
    TRANSLATE_ENUM(Mp.Ecosystem, ECOSYSTEM::unknown,
                   ECOSYSTEM::stringToEcosystem, "mp.Ecosystem");

    // MpCategory
    TRANSLATE_ENUM(Mp.MpCategory, MP_CATEGORY::Undefined,
                   MP_CATEGORY::stringToMPCategory, "mp.Category");

    // Skill
    TRANSLATE_ENUM(Mp.HarvestSkill, SKILLS::unknown, SKILLS::toSkill,
                   "mp.HarvestSkill");
    ...
    uint i;
    char keyTmp[256];
    // ensure that if you modify RM_FABER_TYPE, you have to rebuild the item
    // sheets.
    ...
    for (i = 0; i < RM_FABER_TYPE::NUM_FABER_TYPE; i++) {
      uint32 durability = 0;
      string sheetEntry =
          RM_FABER_TYPE::faberTypeToSheetEntry((RM_FABER_TYPE::TRMFType)i);
      ...
      if (durability) {
        ...
        TRANSLATE_ENUM(itemPart.OriginFilter, ITEM_ORIGIN::UNKNOWN,
                       ITEM_ORIGIN::stringToEnum, keyTmp);

        // read each stat
        for (uint j = 0; j < RM_FABER_STAT_TYPE::NumRMStatType; j++) {
          sprintf(
              keyTmp, "mp.MpParam.%s.%s", sheetEntry.c_str(),
              RM_FABER_STAT_TYPE::toString((RM_FABER_STAT_TYPE::TRMStatType)j)
                  .c_str());
  ...
  case ITEMFAMILY::SHIELD: {
    // ShieldType
    TRANSLATE_ENUM(Shield.ShieldType, SHIELDTYPE::NONE,
                   SHIELDTYPE::stringToShieldType, "shield.Category");
  } break;
  // TOOL: different for any tool
  case ITEMFAMILY::CRAFTING_TOOL: {
    // CraftingToolType
    TRANSLATE_ENUM(Tool.CraftingToolType, TOOL_TYPE::Unknown,
                   TOOL_TYPE::toToolType, "crafting tool.type");
  } break;
  case ITEMFAMILY::HARVEST_TOOL: {
    // Skill
    TRANSLATE_ENUM(Tool.Skill, SKILLS::unknown, SKILLS::toSkill,
                   "harvest tool.skill");
  } break;
  ...
  case ITEMFAMILY::TELEPORT: {
    // Type of teleport
    TRANSLATE_ENUM(Teleport.Type, TELEPORT_TYPES::NONE,
                   TELEPORT_TYPES::getTpTypeFromString, "teleport.Type");
  } break;
...
bool CItemSheet::isFaberisable() const {
  ...
  return Family == ITEMFAMILY::AMMO || Family == ITEMFAMILY::ARMOR ||
         Family == ITEMFAMILY::MELEE_WEAPON ||
         Family == ITEMFAMILY::RANGE_WEAPON || Family == ITEMFAMILY::JEWELRY ||
         Family == ITEMFAMILY::CRAFTING_TOOL ||
         Family == ITEMFAMILY::HARVEST_TOOL ||
         Family == ITEMFAMILY::TAMING_TOOL || Family == ITEMFAMILY::SHIELD;
}
...
SKILLS::ESkills CItemSheet::getRequiredSkill() const {
  ...
  case ITEMFAMILY::RANGE_WEAPON:
    return RangeWeapon.Skill;
  case ITEMFAMILY::AMMO:
    return Ammo.Skill;
    //		case ITEMFAMILY::SHIELD:		return
    //SHIELDTYPE::shieldTypeToSkill(Shield.ShieldType);
  case ITEMFAMILY::RAW_MATERIAL:
    return Mp.HarvestSkill;
  //
  case ITEMFAMILY::HARVEST_TOOL:
  case ITEMFAMILY::TAMING_TOOL:
    return Tool.Skill;
...
// ***************************************************************************
bool CItemSheet::canBuildItemPart(RM_FABER_TYPE::TRMFType e,
                                  ITEM_ORIGIN::EItemOrigin origin) const {
  if (e < RM_FABER_TYPE::NUM_FABER_TYPE) {
    if (Mp.ItemPartBF & (SINT64_CONSTANT(1) << e)) {
      ...
      if (itemPart.OriginFilter == ITEM_ORIGIN::COMMON ||
          itemPart.OriginFilter == origin || origin == ITEM_ORIGIN::COMMON)
        return true;
    }
  }
...
// ***************************************************************************
const CItemSheet::CMpItemPart &
CItemSheet::getItemPart(RM_FABER_TYPE::TRMFType e) const {
...
// ***************************************************************************
bool CItemSheet::hasCharacRequirement(
    uint itemLevel, CHARACTERISTICS::TCharacteristics &caracType,
    float &caracValue) const {
  ...
  for (uint i = 0; i < RM_FABER_TYPE::NUM_FABER_TYPE; i++) {
    ...
    if (canBuildItemPart(faberType)) {
      ...
      if (faberType != RM_FABER_TYPE::MPBT &&
          ...
          faberType != RM_FABER_TYPE::MPJH && faberType != RM_FABER_TYPE::MPCF)
        all = false;
    }
  }
  if (all) {
