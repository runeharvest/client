  ...
  struct SSkillData {
    ...
    void serial(NLMISC::IStream &f) {
      ...
      if (f.isReading()) {
        ...
        f.serial(size);
        for (std::vector<SKILLS::ESkills>::iterator it = ChildSkills.begin();
             it != ChildSkills.end(); ++it) {
          f.serialEnum((*it));
        }
      }
    }
...
public:
  CSkillsTreeSheet() { Type = SKILLS_TREE; }
  /// destructor
  ...
  /// serialize
  virtual void serial(NLMISC::IStream &f) { f.serialCont(SkillsTree); }
