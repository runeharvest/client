...
// ***************************************************************************
// The max number of different MP required for a faber sentence. This applies
// for ItemPart MPs, and Formula MPs
...
public:
  ...
  struct CProperty {
    ...
    std::string Text;
    // translated at runTime with CSBrickManager to PropId/Value (not
    // serialised)
    uint PropId;
    float Value;
    float Value2;
    ...
    void serial(NLMISC::IStream &f) { f.serial(Text); }
  };
  ...
  struct CFaberPlan {
    ...
    // FormulaMP: a specific MP (or can be any item) that the faber requires in
    // addition to CItemPartMPs
    struct CFormulaMP {
  ...
  public:
    ...
    void serial(NLMISC::IStream &f) { f.serialEnum(Skill); };
...
public:
  ...
  BRICK_TYPE::EBrickType getBrickType() const {
...
public:
  ...
  std::string getForbiddenDef() const {
    return ClientSheetsStrings.get(IdForbiddenDef);
  }
  std::string getForbiddenExclude() const {
    return ClientSheetsStrings.get(IdForbiddenExclude);
  }
