...
// *********************************************************************************************************
void CAttackSheet::build(const NLGEORGES::UFormElm &item,
  ...
  ok &= item.getValueByName(AttackStaticObjectCastFX,
                            (prefix + "AttackStaticObjectCastFX").c_str());
  ...
  //
  ok &= item.getValueByName(ProjectileDelay,
                            (prefix + "ProjectileDelay").c_str());
  ok &= item.getValueByName(StaticObjectProjectileDelay,
                            (prefix + "StaticObjectProjectileDelay").c_str());
  uint32 projectileMode = 0;
  ok &=
      item.getValueByName(projectileMode, (prefix + "ProjectileMode").c_str());
  ProjectileMode = (MAGICFX::TSpellMode)projectileMode;
  ok &= item.getValueByName(IsImpactLocalised,
                            (prefix + "IsImpactLocalised").c_str());
  ok &= item.getValueByName(PlayImpactFXOnlyOnMainTarget,
                            (prefix + "PlayImpactFXOnlyOnMainTarget").c_str());
  ok &=
      item.getValueByName(PlayImpactAnim, (prefix + "PlayImpactAnim").c_str());
  ok &= DefaultCastingPoint.build(item, prefix + "DefaultCastingPoint.");
  ok &= DefaultAimingPoint.build(item, prefix + "DefaultAimingPoint.");
  ok &= item.getValueByName(LetProjectileStickedOnTarget,
                            (prefix + "LetProjectileStickedOnTarget").c_str());
  ok &= item.getValueByName(ForceUseProjectileDelay,
                            (prefix + "ForceUseProjectileDelay").c_str());
  ok &= item.getValueByName(ApplyItemOffsetToWeaponBone,
                            (prefix + "ApplyItemOffsetToWeaponBone").c_str());
  ok &= item.getValueByName(AdditionnalStartOffset.x,
                            (prefix + "AdditionnalStartOffsetX").c_str());
  ok &= item.getValueByName(AdditionnalStartOffset.y,
                            (prefix + "AdditionnalStartOffsetY").c_str());
  ok &= item.getValueByName(AdditionnalStartOffset.z,
                            (prefix + "AdditionnalStartOffsetZ").c_str());
  if (!ok) {
...
void CAttackSheet::serial(NLMISC::IStream &f) {
  ...
  f.serial(
      PlayImpactFXOnlyOnMainTarget); // Only may target play the impact fx (but
