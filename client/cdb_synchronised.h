...
class CCDBSynchronised : public NLMISC::CCDBManager {
  ...
  /// True while the first database packet has not been completely processed
  /// (including branch observers)
...
public:
  ...
  void init(const std::string &fileName,
            class NLMISC::IProgressCallback &progressCallBack);
  ...
  /// Return true while the first database packet has not been completely
  /// received
  ...
  void setChangesProcessed() {
    if (allInitPacketReceived()) {
      ...
      writeInitInProgressIntoUIDB(); // replaced by
                                     // DECLARE_INTERFACE_USER_FCT(isDBInitInProgress)
    }
  }
...
private:
  ...
  bool allInitPacketReceived() const {
    return _InitDeltaReceived == 2;
