...
//=================================================================================
void CStreamableIG::init(NL3D::UScene *owningScene, const NLMISC::CVector &pos,
                         float forceLoadRadius, float loadRadius,
                         float unloadRadius) {
  H_AUTO_USE(RZ_StremableIG)
  nlassert(_Scene == NULL); // init should be called once
  nlassert(owningScene);
  _Scene = owningScene;
  if (!(unloadRadius >= loadRadius && loadRadius >= forceLoadRadius &&
        forceLoadRadius >= 0)) {
  ...
  if (!_Linked) {
    ...
    for (uint k = 0; k < _IGs.size(); ++k) {
      if (!_IGs[k].Loading && !_IGs[k].IG) {
        ...
        _Scene->createInstanceGroupAndAddToSceneAsync(
            _IGs[k].Name + ".ig", &_IGs[k].IG, _IGs[k].Pos, _IGs[k].Rot, season,
            &_Callback);
      } else {
        if (_IGs[k].Loading && _IGs[k].IG) {
    ...
    for (uint k = 0; k < _IGs.size(); ++k) {
      ...
      if (!_IGs[k].IG) {
        ...
        if (!_IGs[k].Loading) {
          ...
          _Callback.Owner = this;
          _Scene->createInstanceGroupAndAddToSceneAsync(
              _IGs[k].Name + ".ig", &_IGs[k].IG, _IGs[k].Pos, _IGs[k].Rot,
              season, &_Callback);
          _IGs[k].Loading = true;
        }

        _Scene->updateWaitingInstances(
            1000); /* set a high value to upload texture at a fast rate */
    ...
    if (_IGs[k].Loading) {
      if (_IGs[k].IG) {
        if (_IGs[k].IG != (NL3D::UInstanceGroup *)-1) {
          ...
          _IGs[k].IG->removeFromScene(*_Scene);
          // notifyIGAdded has not been called yet, so no need to call
          // notifyIGRemoved
        }
      } else {
        _Scene->stopCreatingAndAddingIG(&_IGs[k].IG);
      ...
      if (_IGs[k].IG &&
          _IGs[k].IG !=
              (NL3D::UInstanceGroup *)-1) // -1 signal that async loading failed
      {
        // nlinfo("unload 2");
        nlassert(_Scene);
        _IGs[k].IG->removeFromScene(*_Scene);
...
//===================================================================================
bool CStreamableIG::setIG(uint ig, const std::string &name,
  ...
  if (ig < _IGs.size()) {
    ...
    _IGs[ig].ParentName = NLMISC::toLowerAscii(
        NLMISC::CFile::getFilenameWithoutExtension(parentName));
    _IGs[ig].IG = NULL;
    _IGs[ig].Loading = false;
    _Linked = false;
    return true;
...
//=================================================================================
void CStreamableIG::addIG(const std::string &name,
                          const std::string &parentName,
  ...
  _IGs.back().Name =
      NLMISC::toLowerAscii(NLMISC::CFile::getFilenameWithoutExtension(name));
  _IGs.back().ParentName = NLMISC::toLowerAscii(
      NLMISC::CFile::getFilenameWithoutExtension(parentName));
  _IGs.back().IG = NULL;
...
void CStreamableIG::linkInstances() {
  ...
  if (_Linked)
    return;
  for (uint k = 0; k < _IGs.size(); ++k) {
    /** There are few igs at the same time, so a linear search should suffice
     * for now
     */
    if (_IGs[k].IG != (NL3D::UInstanceGroup *)-1) {
      // search the parent
      if (!_IGs[k].ParentName.empty()) {
        for (uint l = 0; l < _IGs.size(); ++l) {
          ...
          if (_IGs[l].IG != (NL3D::UInstanceGroup *)-1 &&
              _IGs[k].ParentName == _IGs[l].Name) {
            if (!_IGs[k].IG->linkToParentCluster(_IGs[l].IG)) {
              nlwarning("Failed to link cluster %s to its parent %s",
                        _IGs[k].Name.c_str(), _IGs[k].ParentName.c_str());
            }
          }
        }
      }
...
void CStreamableIG::addLoadedIGToMap() {
  ...
  for (uint k = 0; k < _IGs.size(); ++k) {
    if (_IGs[k].IG &&
        _IGs[k].IG !=
            (NL3D::UInstanceGroup *)-1) // is this a successfully loaded ig ?
    {
      // insert the new ig if it hasn't before..
      if (_IGMap
              ->insert(std::make_pair(NLMISC::toLowerAscii(_IGs[k].Name),
                                      _IGs[k].IG))
              .second)
        // if inserted, must notify IG Added, else already notifiyed by
        // loadAsync()
        this->notifyIGAdded(_IGs[k].IG);
    }
  }
...
bool CStreamableIG::enumIGs(IIGEnum *callback) {
  ...
  for (TIGArray::iterator it = _IGs.begin(); it != _IGs.end() && continueEnum;
       ++it) {
