...
struct IStreamableEntity : public NLMISC::CRefCount {
   ...
   * It it returns false, it means that the entity is too far or that
  ...
  /** Test that entity against the player position, and load / unload it (using
  ...
  virtual void forceUpdate(const NLMISC::CVector &pos,
                           class NLMISC::IProgressCallback &progress) = 0;
 ...
 * Such an entity is streamed when it is near the user, or its loading is forced
 * when it is too near. The derivers may choose what kind of object must be
 * loaded
 */
...
protected:
  ...
  void setPos(const NLMISC::CVector &pos) { _Pos = pos; }
  ...
  /** Force complete loading of that object. If an async loading was started, it
   * must be finished.
   */
