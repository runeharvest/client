...
struct CStatThread : public NLMISC::IRunnable {
  ...
  void get(const std::string &url) {
    ...
    curl_easy_setopt(curl, CURLOPT_REFERER,
                     string("http://www.ryzom.com/" + referer).c_str());
#else
    curl_easy_setopt(curl, CURLOPT_REFERER,
                     string("https://ryzom.dev/" + referer).c_str());
#endif
  ...
  void addParam(std::string &params, const std::string &name,
  ...
  string cookie() {
    ...
    return toUpperAscii(
        getMD5((const uint8 *)userid.c_str(), (uint32)userid.size())
            .toString());
  }
  ...
  bool connect() {
    ...
    std::string session =
        toString("%d%d", NetMngr.getUserId(), CTime::getSecondsSince1970());
    addParam(params, "sessioncookie",
             toUpperAscii(
                 getMD5((const uint8 *)session.c_str(), (uint32)session.size())
                     .toString()));
    ...
    addParam(params, "screen",
             toString("%dx%d", ClientCfg.ConfigFile.getVar("Width").asInt(),
                      ClientCfg.ConfigFile.getVar("Height").asInt()));
#ifdef RYZOM_FORGE
...
void waitForUserCharReceived() {
  if ((!ClientCfg.Local) /*ace&&(!ClientCfg.Light)*/) {
    // Get the position from the server (will fill either UserEntity or
    // UserEntityInitPos/Front)
...
void initMainLoop() {
  ...
  {    // profile init mainloop
    ...
    if (SoundMngr) {
      ...
      // CSoundManager::LoadingMusicXFade, true); no fadein, and not async
      // because don't work well because of loading in the main thread Force use
      // GameMusic volume
    ...
    // nlinfo ("PROFILE: %d seconds (%d total) for Uninitializing outgame",
    ...
    // Create the game interface database
    {
      ...
      NLGUI::CDBManager::getInstance()->getDB()->attachChild(
          IngameDbMngr.getNodePtr(), "SERVER");
      ...
      initLast = initCurrent;
      initCurrent = ryzomGetLocalTime();
      // nlinfo ("PROFILE: %d seconds (%d total) for Initializing XML database",
      // (uint32)(initCurrent-initLast)/1000,
      // (uint32)(initCurrent-initStart)/1000);
    }
    ...
    // Create interface database
    {
      ...
      initLast = initCurrent;
      initCurrent = ryzomGetLocalTime();
      // nlinfo ("PROFILE: %d seconds (%d total) for Initializing interface",
      // (uint32)(initCurrent-initLast)/1000,
      // (uint32)(initCurrent-initStart)/1000);
    }

    {
      // Ask and receive the user position to start (Olivier: moved here because
      // needed by initInGame())
      ...
      initLast = initCurrent;
      initCurrent = ryzomGetLocalTime();
      // nlinfo ("PROFILE: %d seconds (%d total) for Awaiting start position",
      // (uint32)(initCurrent-initLast)/1000,
      // (uint32)(initCurrent-initStart)/1000);
    }
...

    {
      ...
      // During load of the game, fade completely out SFX, and leave outgame
      // music When the game will begin, it will fade in slowly
      if (SoundMngr)
        SoundMngr->setupFadeSound(0.f, 1.f);

      initLast = initCurrent;
      initCurrent = ryzomGetLocalTime();
      // nlinfo ("PROFILE: %d seconds (%d total) for Initializing sound",
      // (uint32)(initCurrent-initLast)/1000,
      // (uint32)(initCurrent-initStart)/1000);
    }

    {
      ...
      initLast = initCurrent;
      initCurrent = ryzomGetLocalTime();
      // nlinfo ("PROFILE: %d seconds (%d total) for Initializing Entities
      // manager", (uint32)(initCurrent-initLast)/1000,
      // (uint32)(initCurrent-initStart)/1000);
    }

    {
      ...
      initLast = initCurrent;
      initCurrent = ryzomGetLocalTime();
      // nlinfo ("PROFILE: %d seconds (%d total) for Creating scene",
      // (uint32)(initCurrent-initLast)/1000,
      // (uint32)(initCurrent-initStart)/1000);
    }
...

    {
      ...
      initLast = initCurrent;
      initCurrent = ryzomGetLocalTime();
      // nlinfo ("PROFILE: %d seconds (%d total) for Initializing animation",
      // (uint32)(initCurrent-initLast)/1000,
      // (uint32)(initCurrent-initStart)/1000);
    }
    ...
    float aspectRatio;
    {
      ...
      cam2.setPerspective(fov, aspectRatio, SceneRootCameraZNear,
                          SceneRootCameraZFar);
      ...
      // Set this collision manager the one the scene must use for Shadow
      // Reception on Buildings
...

    {
      ...
      initLast = initCurrent;
      initCurrent = ryzomGetLocalTime();
      // nlinfo ("PROFILE: %d seconds (%d total) for Initializing Entity
      // Animation Manager", (uint32)(initCurrent-initLast)/1000,
      // (uint32)(initCurrent-initStart)/1000);
    }

    {
      ...
      pIM->initInGame(); // must be called after waitForUserCharReceived()
                         // because Ring information is used by initInGame()
#ifdef RYZOM_FORGE
      CItemGroupManager::getInstance()
          ->init(); // Init at the same time keys.xml is loaded
#endif
      initLast = initCurrent;
      initCurrent = ryzomGetLocalTime();
      // nlinfo ("PROFILE: %d seconds (%d total) for Initializing ingame",
      // (uint32)(initCurrent-initLast)/1000,
      // (uint32)(initCurrent-initStart)/1000);
...

    {
      ...
      // DO IT AFTER: Database, Collision Manager, PACS, scene, animations
      // loaded.
      ...
      initLast = initCurrent;
      initCurrent = ryzomGetLocalTime();
      // nlinfo ("PROFILE: %d seconds (%d total) for Initializing User sheet",
      // (uint32)(initCurrent-initLast)/1000,
      // (uint32)(initCurrent-initStart)/1000);
    }
...

    {
      ...
      initLast = initCurrent;
      initCurrent = ryzomGetLocalTime();
      // nlinfo ("PROFILE: %d seconds (%d total) for Initializing Weather",
      // (uint32)(initCurrent-initLast)/1000,
      // (uint32)(initCurrent-initStart)/1000);
    }

    {
...
#endif
      {
        ...
        if (!ClientCfg.Light) {
          ...
          // Default setup: 1/20. should work well for night/day transition in
          // 30 minutes.
          //	Because all patchs will be updated every 20 seconds => 90 steps.
          //	But enable it only when we enter Night/Day transition, because
          //landscape updateLighting is 	still slow (+10% if 1/20).
      ...
      initLast = initCurrent;
      initCurrent = ryzomGetLocalTime();
      // nlinfo ("PROFILE: %d seconds (%d total) for Initializing Landscape",
      // (uint32)(initCurrent-initLast)/1000,
      // (uint32)(initCurrent-initStart)/1000);
    }
    ...
    if (!ClientCfg.Light) {
      ...
      initLast = initCurrent;
      initCurrent = ryzomGetLocalTime();
      // nlinfo ("PROFILE: %d seconds (%d total) for Initializing Cloudscape",
      // (uint32)(initCurrent-initLast)/1000,
      // (uint32)(initCurrent-initStart)/1000);
    }
...

    {
      ...
      initLast = initCurrent;
      initCurrent = ryzomGetLocalTime();
      // nlinfo ("PROFILE: %d seconds (%d total) for Initializing season",
      // (uint32)(initCurrent-initLast)/1000,
      // (uint32)(initCurrent-initStart)/1000);
    }

    {
      ...
      CTimedFXManager::getInstance().init(
          Scene, CClientDate(RT.getRyzomDay(), (float)RT.getRyzomTime()),
          WorldLightCycle.NumHours, 15.45646f,
          ClientCfg.MaxNumberOfTimedFXInstances,
          2.f,    // roughly, the size of a tile
          300.f); // no env fx should be seen farther than that

      initLast = initCurrent;
      initCurrent = ryzomGetLocalTime();
      // nlinfo ("PROFILE: %d seconds (%d total) for Initializing Timed FX",
      // (uint32)(initCurrent-initLast)/1000,
      // (uint32)(initCurrent-initStart)/1000);
    }

    {
      ...
      initLast = initCurrent;
      initCurrent = ryzomGetLocalTime();
      // nlinfo ("PROFILE: %d seconds (%d total) for Loading continent",
      // (uint32)(initCurrent-initLast)/1000,
      // (uint32)(initCurrent-initStart)/1000);
    }

    {
      ...
      if (Landscape) {
        ...
        initLast = initCurrent;
        initCurrent = ryzomGetLocalTime();
        // nlinfo ("PROFILE: %d seconds (%d total) for Initializing collision
        // manager", (uint32)(initCurrent-initLast)/1000,
        // (uint32)(initCurrent-initStart)/1000);
      }
    }
    ...
    if (!ClientCfg.Light) {
        ...
        if (ClientCfg.HDTextureInstalled) {
          ...
          if (ClientCfg.HDEntityTexture) {
            if (ClientCfg.VideoMemory <= 0) {
              ...
              if (videoMemory <= 0)
                videoMemory = CSystemUtils::getTotalVideoMemory();

              // in the worst case, use default value of 128 MiB
              if (videoMemory <= 0)
                videoMemory = 128 * 1024;
              ...
              videoMemory =
                  ClientCfg.VideoMemory < 32 ? 32 : ClientCfg.VideoMemory;
          ...
          /* setup "v1 texture" (or 256*256), but take into account that they
             are already stored as V1...
                  => remove 1 from the right shit (=> 0,2)
          */
          Driver->setupAsyncTextureLod(ENTITY_TEXTURE_COARSE_LEVEL - 1,
                                       ENTITY_TEXTURE_NORMAL_LEVEL - 1);
          // Allow a big cache for them
          Driver->setupMaxTotalAsyncTextureSize(ENTITY_TEXTURE_LOW_MEMORY *
                                                1024 * 1024);
          // Allow low upload
          Driver->setupAsyncTextureMaxUploadPerFrame(ENTITY_TEXTURE_LOW_MAXUP *
                                                     1024);
        }
      }
      ...
      initLast = initCurrent;
      initCurrent = ryzomGetLocalTime();
      // nlinfo ("PROFILE: %d seconds (%d total) for Creating LOD managers",
      // (uint32)(initCurrent-initLast)/1000,
      // (uint32)(initCurrent-initStart)/1000);
    }
    ...
    if (!ClientCfg.Light && ClientCfg.PreCacheShapes) {
      ...
      // Add a PreLoad Cache the shape bank, with a maximum cache Size => never
      // deleted
...

      {
        ...
        if (CBigFile::getInstance().isBigFileAdded("characters.bnp")) {
          ...
          if (!DBG_DisablePreloadShape) {
            Driver->getShapeBank()->preLoadShapesFromBNP(
                PreLoadCacheName, "characters.bnp", "*.shape", &ProgressBar);
          }
          ...
          if (!DBG_DisablePreloadShape) {
            Driver->getShapeBank()->preLoadShapesFromBNP(
                PreLoadCacheName, "characters.bnp", "*.skel", &ProgressBar);
          }
          ProgressBar.popCropedValues();
        }
        // else Load Characters shapes from the new BNP
        else if (CBigFile::getInstance().isBigFileAdded(
                     "characters_shapes.bnp")) {
          ProgressBar.progress(0);
          ProgressBar.pushCropedValues(0, 0.25f);
          if (!DBG_DisablePreloadShape) {
            Driver->getShapeBank()->preLoadShapesFromBNP(
                PreLoadCacheName, "characters_shapes.bnp", "*.shape",
                &ProgressBar);
          }
          ProgressBar.popCropedValues();
          ProgressBar.progress(0.25f);
          ...
          if (!DBG_DisablePreloadShape) {
            Driver->getShapeBank()->preLoadShapesFromBNP(
                PreLoadCacheName, "characters_skeletons.bnp", "*.skel",
                &ProgressBar);
          }
          ProgressBar.popCropedValues();
        }
        // else load from shapes dir
          ...
          ProgressBar.pushCropedValues(0, 0.25f);
          if (!DBG_DisablePreloadShape) {
            Driver->getShapeBank()->preLoadShapesFromDirectory(
                PreLoadCacheName,
                ClientCfg.PreLoadPath.empty()
                    ? "data/3d/common/characters/shapes"
                    : ClientCfg.PreLoadPath + "/characters_shapes",
          ...
          if (!DBG_DisablePreloadShape) {
            Driver->getShapeBank()->preLoadShapesFromDirectory(
                PreLoadCacheName,
                ClientCfg.PreLoadPath.empty()
                    ? "data/3d/common/characters/skeletons"
                    : ClientCfg.PreLoadPath + "/characters_skeletons",
...

      {
        ...
        if (CBigFile::getInstance().isBigFileAdded("fauna.bnp")) {
          ...
          if (!DBG_DisablePreloadShape) {
            Driver->getShapeBank()->preLoadShapesFromBNP(
                PreLoadCacheName, "fauna.bnp", "*.shape", &ProgressBar);
          }
          ProgressBar.popCropedValues();
          ...
          if (!DBG_DisablePreloadShape) {
            Driver->getShapeBank()->preLoadShapesFromBNP(
                PreLoadCacheName, "fauna.bnp", "*.skel", &ProgressBar);
          }
          ProgressBar.popCropedValues();
        }
        // else load from shapes dir
          ...
          if (!DBG_DisablePreloadShape) {
            Driver->getShapeBank()->preLoadShapesFromBNP(
                PreLoadCacheName, "fauna_shapes.bnp", "*.shape", &ProgressBar);
          }
          ProgressBar.popCropedValues();
          ProgressBar.progress(0.75f);
          ProgressBar.pushCropedValues(0.75f, 1);
          if (!DBG_DisablePreloadShape) {
            Driver->getShapeBank()->preLoadShapesFromBNP(
                PreLoadCacheName, "fauna_skeletons.bnp", "*.skel",
                &ProgressBar);
          }
          ProgressBar.popCropedValues();
        }
        // else load from shapes dir
          ...
          if (!DBG_DisablePreloadShape) {
            Driver->getShapeBank()->preLoadShapesFromDirectory(
                PreLoadCacheName,
                ClientCfg.PreLoadPath.empty()
                    ? "data/3d/common/fauna/shapes"
                    : ClientCfg.PreLoadPath + "/fauna_shapes",
                "*.shape", false, &ProgressBar);
          }
          ProgressBar.popCropedValues();
          ProgressBar.progress(0.75f);
          ...
          if (!DBG_DisablePreloadShape) {
            Driver->getShapeBank()->preLoadShapesFromDirectory(
                PreLoadCacheName,
                ClientCfg.PreLoadPath.empty()
                    ? "data/3d/common/fauna/skeletons"
                    : ClientCfg.PreLoadPath + "/fauna_skeletons",
                "*.skel", false, &ProgressBar);
          }
          ProgressBar.popCropedValues();
        }
      ...
      initLast = initCurrent;
      initCurrent = ryzomGetLocalTime();
      // nlinfo ("PROFILE: %d seconds (%d total) for Loading characters shapes",
      // (uint32)(initCurrent-initLast)/1000,
      // (uint32)(initCurrent-initStart)/1000);

      {
        ...
        if (CBigFile::getInstance().isBigFileAdded("sfx.bnp")) {
          ...
          if (!DBG_DisablePreloadShape) {
            Driver->getShapeBank()->preLoadShapesFromBNP(
                PreLoadCacheNameFX, "sfx.bnp", "*.ps", &ProgressBar,
                preloadFXTextures);
          }
          ProgressBar.popCropedValues();
          ...
          if (!DBG_DisablePreloadShape) {
            Driver->getShapeBank()->preLoadShapesFromBNP(
                PreLoadCacheNameFX, "sfx.bnp", "*.shape", &ProgressBar,
                preloadFXTextures);
          }
          ProgressBar.popCropedValues();
        }
        // else load from shapes dir
          ...
          if (!DBG_DisablePreloadShape) {
            Driver->getShapeBank()->preLoadShapesFromDirectory(
                PreLoadCacheNameFX,
                ClientCfg.PreLoadPath.empty() ? "data/3d/common/sfx"
                                              : ClientCfg.PreLoadPath + "/sfx",
        ...
        initLast = initCurrent;
        initCurrent = ryzomGetLocalTime();
        // nlinfo ("PROFILE: %d seconds (%d total) for Loading FX",
        // (uint32)(initCurrent-initLast)/1000,
        // (uint32)(initCurrent-initStart)/1000);
      }
      {
        ...
        Driver->getShapeBank()->setShapeCacheSize(PreLoadCacheNameObjects,
                                                  1000000);
        if (CBigFile::getInstance().isBigFileAdded("objects.bnp")) {
          ProgressBar.pushCropedValues(0.0f, 1.f);
          if (!DBG_DisablePreloadShape) {
            Driver->getShapeBank()->preLoadShapesFromBNP(
                PreLoadCacheNameObjects, "objects.bnp", "*.shape", &ProgressBar,
        ...
        // else load from shapes dir
          ...
          if (!DBG_DisablePreloadShape) {
            Driver->getShapeBank()->preLoadShapesFromDirectory(
                PreLoadCacheNameObjects,
                ClientCfg.PreLoadPath.empty()
                    ? "data/3d/common/objects"
                    : ClientCfg.PreLoadPath + "/objects",
        ...
        initLast = initCurrent;
        initCurrent = ryzomGetLocalTime();
        // nlinfo ("PROFILE: %d seconds (%d total) for Loading object shapes",
        // (uint32)(initCurrent-initLast)/1000,
        // (uint32)(initCurrent-initStart)/1000);
      }
    } else {
...

    {
      ...
      initLast = initCurrent;
      initCurrent = ryzomGetLocalTime();
      // nlinfo ("PROFILE: %d seconds (%d total) for Initializing Contextual
      // Cursor", (uint32)(initCurrent-initLast)/1000,
      // (uint32)(initCurrent-initStart)/1000);
    }
    ...
    TextContext->setColor(CRGBA(255, 255, 255));
    {
      ...
      initLast = initCurrent;
      initCurrent = ryzomGetLocalTime();
      // nlinfo ("PROFILE: %d seconds (%d total) for Initializing Instances
      // group", (uint32)(initCurrent-initLast)/1000,
      // (uint32)(initCurrent-initStart)/1000);
    }
    ...
    if (!ClientCfg.Light) {
      if (Landscape) {
        ...
        Landscape->setThreshold(
            ClientCfg.getActualLandscapeThreshold()); // Set Threshold.
      }
    }
    ...
    initLast = initCurrent;
    initCurrent = ryzomGetLocalTime();
    // nlinfo ("PROFILE: %d seconds (%d total) for Initializing other
    // parameters", (uint32)(initCurrent-initLast)/1000,
    // (uint32)(initCurrent-initStart)/1000);
    ...
    if (!ClientCfg.Local) {
      {
        ...
        if (GenericMsgHeaderMngr.pushNameToStream("CONNECTION:READY", out)) {
          ...
          nlwarning(
              "initMainLoop : unknown message name : 'CONNECTION:READY'.");
      }
    }
    ...
    initLast = initCurrent;
    initCurrent = ryzomGetLocalTime();
    // nlinfo ("PROFILE: %d seconds (%d total) for Sending \"Ready\"",
    // (uint32)(initCurrent-initLast)/1000,
    // (uint32)(initCurrent-initStart)/1000);
    ...
    /*
            ...
            {
                    Driver->enableLowLevelKeyboard (true); // the return value
       has already been tested at startup
            }*/
    ...
    if (PeopleInterraction.ChatGroup.Window) {
      CGroupEditBox *eb = dynamic_cast<CGroupEditBox *>(
          PeopleInterraction.ChatGroup.Window->getEditBox());
    ...
    CWidgetManager::getInstance()->setCaptureKeyboard(
        NULL); // previous set editbox becomes '_OldCaptureKeyboard'
    ...
    initLast = initCurrent;
    initCurrent = ryzomGetLocalTime();
    // nlinfo ("PROFILE: %d seconds (%d total) for Launching",
    // (uint32)(initCurrent-initLast)/1000,
    // (uint32)(initCurrent-initStart)/1000);

    nlinfo("PROFILE: %d seconds for init main loop",
  ...
  if (WantStartupProfiling) {
    ...
    CLog log;
    CFileDisplayer fileDisplayer(
        NLMISC::CFile::findNewFile(getLogDirectory() + "profile_startup.log"));
    log.addDisplayer(&fileDisplayer);
    // display
    NLMISC::CHTimer::displayHierarchicalByExecutionPathSorted(
        &log, CHTimer::TotalTime, true, 48, 2);
...
void loadBackgroundBitmap(TBackground background) {
  ...
  for (uint i = 0; i < ClientCfg.Logos.size(); i++) {
    ...
    if (res.size() == 9)
      LogoBitmaps.push_back(Driver->createTextureFile(res[0]));
  }
...
void endLoading() { destroyLoadingBitmap(); }
...
void initWelcomeWindow() {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  CInterfaceGroup *welcomeWnd = dynamic_cast<CInterfaceGroup *>(
      CWidgetManager::getInstance()->getElementFromId(
          "ui:interface:welcome_info"));
  if (welcomeWnd) {
    bool welcomeDbProp = NLGUI::CDBManager::getInstance()
                             ->getDbProp("UI:SAVE:WELCOME")
                             ->getValueBool();
    CSessionBrowserImpl &sb = CSessionBrowserImpl::getInstance();
    welcomeWnd->setActive((sb.CurrentJoinMode != CFarTP::LaunchEditor) &&
                          welcomeDbProp);
  }
}
...
void initHardwareCursor(bool secondCall) {
  ...
  if (node) {
    if (!ClientCfg.HardwareCursor && !node->getValueBool()) {
      ...
      // else, only the first time after this patch, open popup to propose
      // hardare cursor mode
      else {
        CInterfaceGroup *cursorWnd = dynamic_cast<CInterfaceGroup *>(
            CWidgetManager::getInstance()->getElementFromId(
                "ui:interface:hardware_cursor"));
        if (cursorWnd) {
          cursorWnd->setActive((sb.CurrentJoinMode != CFarTP::LaunchEditor) ||
                               secondCall);
    ...
    if (!node->getValueBool() &&
        ((sb.CurrentJoinMode != CFarTP::LaunchEditor) || secondCall)) {
...
void initBloomConfigUI() {
  ...
  CCtrlBaseButton *button = dynamic_cast<CCtrlBaseButton *>(
      CWidgetManager::getInstance()->getElementFromId(
          "ui:interface:game_config:content:fx:bloom_gr:bloom:c"));
  if (button) {
    button->setFrozen(!supportBloom);
  }

  button = dynamic_cast<CCtrlBaseButton *>(
      CWidgetManager::getInstance()->getElementFromId(
          "ui:interface:game_config:content:fx:bloom_gr:square_bloom:c"));
  if (button) {
    button->setFrozen(!supportBloom);
  }
  ...
  button = dynamic_cast<CCtrlBaseButton *>(
      CWidgetManager::getInstance()->getElementFromId(
          "ui:interface:game_config:content:fx:fxaa:c"));
  if (button) {
    button->setFrozen(!supportBloom);
  }
  ...
  CCtrlScroll *scroll = dynamic_cast<CCtrlScroll *>(
      CWidgetManager::getInstance()->getElementFromId(
          "ui:interface:game_config:content:fx:bloom_gr:density_bloom:c"));
  if (scroll) {
  ...
  CInterfaceGroup *group = dynamic_cast<CInterfaceGroup *>(
      CWidgetManager::getInstance()->getElementFromId(
          "ui:interface:game_config:content:fx:bloom_gr"));
