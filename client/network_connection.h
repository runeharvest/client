...
/*
 * ENABLE_INCOMING_MSG_RECORDER for recording/replaying incoming network
 * messages
 */
...
public:
  ...
  struct TVPNodeClient : public CLFECOMMON::TVPNodeBase {
    ...
    void decodeDiscreetProperties(NLMISC::CBitMemStream &msgin) {
      ...
      if (BranchHasPayload) {
        if (isLeaf()) {
          SlotContext.NetworkConnection->decodeDiscreetProperty(msgin,
                                                                PropIndex);
        } else {
          if (a())
            a()->decodeDiscreetProperties(msgin);
          if (b())
  ...
  struct TPositionInfo {
    /**
     * Only for positions (property index 0): interval, in game cycle unit,
     * between the current position update and the next predicted one. In most
     * cases the next real position update will occur just before the predicted
     * interval elapses.
     *
     * If the interval cannot be predicted yet (e.g. before having received two
     * updates for the same entity), PredictedInterval is set to ~0.
     */
  ...
  public:
    /// Constructor
    CChange(CLFECOMMON::TCLEntityId id = 0, uint8 prop = 255,
            NLMISC::TGameCycle gc = 0)
    ...
    // These contextual additional properties must be set by hand (not by
    // constructor)
    union {
  ...
  /// The states of the connection to the server (if you change them, change
  /// ConnectionStateCStr)
  enum TConnectionState {
    ...
    Disconnect, // disconnect() called, or timeout, or connection closed by
                // frontend
    Quit        // quit() called
  };

  typedef void (*TImpulseCallback)(NLMISC::CBitMemStream &,
                                   CLFECOMMON::TPacketNumber packet, void *arg);
...
public:
   ...
   * \param cookie it s the cookie in string format that was passed to the exe
   * command line (given by the nel_launcher) \param addr it s the front end
   * address in string format that was passed to the exe command line (given by
   * the nel_launcher)
   */
  ...
  /**
   * Connects to the front-end (using or not the login system, depending on what
   * was specified at init()) \param result the message returned in case of an
   * error Returns true if no error occurred.
   */
   ...
   * the exception EBlockedByFirewall in thrown. The first time, a time-out is
   * armed.
  ...
  /**
   * Buffers an action to be sent at next update (or later updates if network
   * overload occurs)
   */
  void push(CLFECOMMON::CAction *action);

  /**
   * Buffers a target action (set targetOrPickup to true for target, false for
   * pick-up
   */
   ...
   */
  //	void			setReferencePosition(const NLMISC::CVectorD
  //&position) { _ImpulseDecoder.setReferencePosition(position); }
  ...
  // Return an estimated smooth server tick. Increment only and
  // accelerate/decelerate according to network
  ...
  uint32 getPartialSentBytes() {
    uint32 ret = _PartialSentBytes;
    _PartialSentBytes = 0;
    return ret;
  }
  uint32 getReceivedBytes() { return _TotalReceivedBytes; }
  uint32 getPartialReceivedBytes() {
    uint32 ret = _PartialReceivedBytes;
    _PartialReceivedBytes = 0;
    return ret;
  }
  bool getConnectionQuality() { return _ConnectionQuality; }
  NLMISC::TTime getTimeSinceLastReceivedPacket() {
    return NLMISC::CTime::getLocalTime() - _LastReceivedNormalTime;
  }

  /// Return the average billed download rate in kbps, including all headers
  /// (UDP+IP+Ethernet)
  float getMeanDownload() {
    return (_MeanDownload.mean(ryzomGetLocalTime()) + 20 + 8 + 14) * 0.008f;
  }

  /// Return the average billed upload rate in kbps, including all headers
  /// (UDP+IP+Ethernet)
  float getMeanUpload() {
    return (_MeanUpload.mean(ryzomGetLocalTime()) + 20 + 8 + 14) * 0.008f;
  }
  ...
  float getMeanPacketLoss() {
    ...
    float dpk = _MeanPackets.Values.empty()
                    ? 0.0f
                    : _MeanPackets.Values.back().second -
                          _MeanPackets.Values.front().second + 1;
  ...
  CLFECOMMON::TSheetId get(CLFECOMMON::TCLEntityId id) const {
    return _PropertyDecoder.sheet(id);
  }
  ...
  const NLNET::CLoginCookie &getLoginCookie() const { return _LoginCookie; }
  ...
  void decodeDiscreetProperty(NLMISC::CBitMemStream &msgin,
...
protected:
  ...
  /// Position update tick queues (used for statistical interval prediction),
  /// one per slot
  std::deque<NLMISC::TGameCycle> _PosUpdateTicks[256];

  /// Position update interval sorted sets (used for statistical interval
  /// prediction), one per slot
  ...
  // if prec don't work, those may (if the server run on windows, prec won't
  // work)
  ...
  CLFECOMMON::TPacketNumber
      _CurrentSendNumber; // number that will be used to send the next packet

  CLFECOMMON::TPacketNumber
      _LastReceivedNumber; // number of the last packet receive from the server
  uint32 _AckBitMask; // this mask contains ack of old messages received by the
                      // server
  uint32
      _LastAckBit; // a remember of the status of the previous received packet
  NLMISC::TTime _LastReceivedTime; // last time the client received something
                                   // from the server
  ...
  NLMISC::TTime _LastReceivedNormalTime; // last time the client received valid
  ...
  NLMISC::TTime _CurrentClientTime; // the time currently played at this frame
                                    // (in the past)
  NLMISC::TGameCycle
      _CurrentClientTick; // the tick for the current frame (also in the past)
  NLMISC::TGameCycle _CurrentServerTick; // the last tick sent by the server.
  NLMISC::TTime _MachineTimeAtTick; // the machine time at the beginning of the
                                    // current tick
  ...
  struct CGenericMultiPartTemp {
    ...
    void set(CLFECOMMON::CActionGenericMultiPart *agmp,
  ...
  struct CHash {
    ...
    inline bool operator()(const CLFECOMMON::TSheetId &id1,
                           const CLFECOMMON::TSheetId &id2) const {
      return (size_t)id1 < (size_t)id2;
    }
  };
...
private:
  ...
  //
  bool decodeHeader(NLMISC::CBitMemStream &msgin,
                    bool checkMessageNumber = true);
  ...
  bool associationBitsHaveChanged(CLFECOMMON::TCLEntityId slot,
                                  uint32 associationBits) {
    bool res =
        ((uint16)associationBits != _PropertyDecoder.associationBits(slot));
  ...
  NLMISC::CBitMemStream _NextMessageToReplay; // loaded at beginning of
                                              // update(), read in buildStream()
