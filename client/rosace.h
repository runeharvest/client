...
public:
  ...
  inline void getPos(float &x, float &y) {
    x = _X;
    y = _Y;
  }
};
...
private:
  /// Select the component under the position (x,y) or unselect all if nothing
  /// at this position.
  void cursorMode(float x, float y);
  /// Select a component according to the angle generated by position (x,y) and
  /// screen center.
