...
// TODO nico : that stuff was coded in a hurry, but could be good to add it to
// NL3D with a better packaging later...

// extern UCamera	MainCam;
extern CView View;

// state ofthe recorder
enum TState { Idle, Recording, PlayBack };
...
bool isRecordingCamera() { return State == Recording; }
...
void updateCameraRecorder() {
  ...
  case PlayBack: {
    ...
    CSample compVal;
    compVal.Date = date;
    TTrack::const_iterator it =
        std::lower_bound(Track.begin(), Track.end(), compVal);
    if (it == Track.end()) {
    ...
    /*
    CVector pos = lambda * s1.Matrix.getPos() + (1.f - lambda) *
    s0.Matrix.getPos(); CVector I = lambda * s1.Matrix.getI() + (1.f - lambda) *
    s0.Matrix.getI(); I.normalize(); CVector J = lambda * s1.Matrix.getJ() +
    (1.f - lambda) * s0.Matrix.getJ(); J = (J - (J * I) * I).normed(); CVector K
    = I ^ J; CMatrix mat; mat.setPos(pos); mat.setRot(I, J, K);
