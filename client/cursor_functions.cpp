...
void initContextualCursor() {
  ...
  ContextCur.add(false, "STAND BY", string("curs_default.tga"), 0.0f,
                 checkUnderCursor, 0);
  ContextCur.add(false, "SELECTABLE", string("curs_pick.tga"), 0.0f,
                 checkUnderCursor, contextSelect);
  //	ContextCur.add(false,	"ATTACK",
  //string("hand_attack.TGA"),			0.0f,	checkUnderCursor,
  //contextAttack); 	ContextCur.add(false,	"QUARTER",
  //string("hand_harvest.TGA"),			0.0f,	checkUnderCursor,
  //contextQuarter); 	ContextCur.add(false,	"LOOT",
  //string("hand_loot.TGA"),			0.0f,	checkUnderCursor,
  //contextLoot); 	ContextCur.add(false,	"PICKUP",
  //string("hand_loot.TGA"),			0.0f,	checkUnderCursor,
  //contextPickUp); 	ContextCur.add(false,	"TALK",
  //string("hand_speak.TGA"),			0.0f,	checkUnderCursor,
  //contextTalk);

  // String Cursors
  ContextCur.add(true, "ATTACK", string("uimGcmAttack"), 0.0f, checkUnderCursor,
                 contextAttack);
  ContextCur.add(true, "QUARTER", string("uimGcmQuartering"), 0.0f,
                 checkUnderCursor, contextQuarter);
  ContextCur.add(true, "LOOT", string("uimGcmLoot"), 0.0f, checkUnderCursor,
                 contextLoot);
  ContextCur.add(true, "PICKUP", string("uimGcmPickUp"), 0.0f, checkUnderCursor,
                 contextPickUp);
  ContextCur.add(true, "TRADE ITEM", string("uimGcmTrade"), 0.0f,
                 checkUnderCursor, contextTradeItem);
  ContextCur.add(true, "TRADE PHRASE", string("uimGcmTradePhase"), 0.0f,
                 checkUnderCursor, contextTradePhrase);
  ContextCur.add(true, "DYNAMIC MISSION", string("uimGcmDynamicMission"), 0.0f,
                 checkUnderCursor, contextDynamicMission);
  ContextCur.add(true, "STATIC MISSION", string("uimGcmMissions"), 0.0f,
                 checkUnderCursor, contextStaticMission);
  ContextCur.add(true, "TRADE PACT", string("uimGcmTradePact"), 0.0f,
                 checkUnderCursor, contextTradePact);
  ContextCur.add(true, "CREATE GUILD", string("uimGcmCreateGuild"), 0.0f,
                 checkUnderCursor, contextCreateGuild);
  ContextCur.add(true, "NEWS", string("uimGcmNews"), 0.0f, checkUnderCursor,
                 contextNews);
  ContextCur.add(true, "TELEPORT", string("uimGcmTeleport"), 0.0f,
                 checkUnderCursor, contextTeleport);
  ContextCur.add(true, "FACTION", string("uimGcmFaction"), 0.0f,
                 checkUnderCursor, contextFaction);
  ContextCur.add(true, "COSMETIC", string("uimGcmCosmetic"), 0.0f,
                 checkUnderCursor, contextCosmetic);
  //	ContextCur.add(true,	"TALK",
  //string("uimGcmTalk"),				0.0f,
  //checkUnderCursor,	contextTalk);
  ContextCur.add(true, "EXTRACT_RM", string("uimGcmExtractRM"), 0.0f,
                 checkUnderCursor, contextExtractRM);
  ContextCur.add(true, "MISSION", string(""), 0.0f, checkUnderCursor,
                 contextMission);
  ContextCur.add(true, "WEB PAGE", string(""), 0.0f, checkUnderCursor,
                 contextWebPage);
  ContextCur.add(true, "WEBIG", string(""), 0.0f, checkUnderCursor,
                 contextWebIG);
  ContextCur.add(true, "OUTPOST", string(""), 0.0f, checkUnderCursor,
                 contextOutpost);
  ContextCur.add(true, "RING MISSION", string(""), 0.0f, checkUnderCursor,
                 contextRingMission);
  ContextCur.add(true, "BUILD_TOTEM", string("uimGcmChooseBuilding"), 0.0f,
                 checkUnderCursor, contextBuildTotem);
} // initContextualCursor //
...
static bool testMissionOption(sint32 priorityWanted) {
  ...
  for (uint k = 0; k < NUM_MISSION_OPTIONS; ++k) {
    std::string nodeName = toString(
        "LOCAL:TARGET:CONTEXT_MENU:MISSIONS_OPTIONS:%d:PRIORITY", (int)k);
    CCDBNodeLeaf *pNL =
        NLGUI::CDBManager::getInstance()->getDbProp(nodeName, false);
    if (pNL) {
      sint32 priority = pNL->getValue32();
      if (priority == priorityWanted) {
        // Special for priority 2 (yoyo: don't know why...). fail if more than
        // one choice
        if (priorityWanted != 2 || textID == 0) {
          nodeName = toString(
              "LOCAL:TARGET:CONTEXT_MENU:MISSIONS_OPTIONS:%d:TITLE", (int)k);
        ...
        // here means that priorityWanted==2 and textId!=0
          ...
          // More than a choice for priorityWanted=2 mission options so nothing
          // to show.
          textID = 0;
          break;
        }
      ...
      nlwarning("checkUnderCursor: entry '%s' dose not exist.",
                nodeName.c_str());
  }
...
static bool testMissionRing() {
  ...
  for (uint i = 0; i < BOTCHATTYPE::MaxR2MissionEntryDatabase; i++) {
    ...
    CCDBNodeLeaf *pNL = NLGUI::CDBManager::getInstance()->getDbProp(
        toString("LOCAL:TARGET:CONTEXT_MENU:MISSION_RING:%d:TITLE", i), false);
    if (pNL && pNL->getValue32()) {
...
void checkUnderCursor() {
  ...
  CViewPointer *cursor =
      static_cast<CViewPointer *>(CWidgetManager::getInstance()->getPointer());
  if (cursor == 0)
    return;
  ...
  if (CWidgetManager::getInstance()->getWindowUnder(x, y) == NULL) {
    ...
    CEntityCL *entity;
    entity = EntitiesMngr.getEntityUnderPos(
        cursX, cursY, ClientCfg.SelectionDist, isPlayerUnderCursor);

    // If the mouse is over the player make the player transparent
    CCDBNodeLeaf *pNL =
        s_UserCharFade
            ? &*s_UserCharFade
            : &*(s_UserCharFade = NLGUI::CDBManager::getInstance()->getDbProp(
                     "UI:SAVE:USER_CHAR_FADE", false));
    ...
    if (entity) {
      ...
      float dist = (float)(entity->pos() - UserEntity->pos()).norm() -
                   entity->box().getRadius();
      // Entity Slot under the cursor.
      SlotUnderCursor = entity->slot();
      uint32 availablePrograms =
          (uint32)NLGUI::CDBManager::getInstance()
              ->getDbProp("LOCAL:TARGET:CONTEXT_MENU:PROGRAMMES")
              ->getValue32();
      bool entityAttackable =
          (availablePrograms & (1 << BOTCHATTYPE::Attackable)) ||
          entity->properties().attackable();
      ...
      if (SlotUnderCursor != UserEntity->selection()) {
        // If the user is fighting, clicking automatically attacks the new
        // target
        if (ClientCfg.AtkOnSelect && UserEntity->isFighting() &&
            entityAttackable) {
      ...
      // Entity Under the cursor is the entity selected.
        ...
        // Wait for the target is up to date. Do not display context cursor if
        // the user is mounted.
        if ((UserEntity->selection() == UserEntity->targetSlot()) &&
          ...
          // Alive
            ...
            if (availablePrograms) {
              ...
              // Dynamic Mission
              //								else
              //if(availablePrograms & (1 << BOTCHATTYPE::DynamicMissionFlag))
              //								{
              //									if(ContextCur.context("DYNAMIC
              //MISSION")) 										return;
              //								}
              // Trade Pact
              ...
              // WebPage
                ...
                if (pNL && pNL->getValue32()) {
                  ...
                  // if the string is not received display a temp string
                  string webPageText;
                  if (!STRING_MANAGER::CStringManagerClient::instance()
                           ->getDynString(textID, webPageText))
                    webPageText =
              ...
              // OutpostFlag
                ...
                if (pNL && pNL->getValue32()) {
                  ...
                  string outpostName;
                  outpostName = STRING_MANAGER::CStringManagerClient::
                      getOutpostLocalizedName(outpostSheet);
      ...
      sint32 instance_idx;
      CShapeInstanceReference instref =
...
// \Warning Be sure the checkUnderCursor and contextSelect are done after
// entites removed.
//-----------------------------------------------
void contextSelect(bool /* rightClick */, bool /* dblClick */) {
...
void contextTalk(bool rightClick, bool dblClick) {
  ...
  /*	else if(availablePrograms & (1 << BOTCHATTYPE::DynamicMissionFlag))
                  UserEntity->moveTo(SlotUnderCursor, 2.0,
     CUserEntity::DynamicMission);*/
...
void contextExtractRM(bool rightClick, bool dblClick) {
  ...
  if (ClientCfg.DblClickMode && !dblClick)
    return;
  UserEntity->moveToExtractionPhrase(SlotUnderCursor, MaxExtractionDistance,
...
void contextWebIG(bool rightClick, bool dblClick) {
  if (rightClick)
    return;
  CInterfaceManager *IM = CInterfaceManager::getInstance();
  CInterfaceElement *pGC = CWidgetManager::getInstance()->getElementFromId(
      "ui:interface:bot_chat_object");
  CInterface3DShape *el = dynamic_cast<CInterface3DShape *>(
  ...
  if (selectedInstanceURL.empty()) {
    ...
    CAHManager::getInstance()->runActionHandler(
        "browse", NULL,
        "name=ui:interface:webig:content:html|url=" + selectedInstanceURL);
  }
...
void contextBuildTotem(bool rightClick, bool dblClick) {
  ...
  if (ClientCfg.DblClickMode && !dblClick)
    return;
  UserEntity->moveToTotemBuildingPhrase(SlotUnderCursor, MaxExtractionDistance,
                                        std::numeric_limits<uint>::max(),
                                        std::numeric_limits<uint>::max(), true);
} // contextBuildTotem //
