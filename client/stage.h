...
public:
   ...
   */
  bool isPresent(uint prop) const {
    return (_Stage.find(prop) != _Stage.end());
  }
   ...
   * indicate if the value (second element) is valid. \param uint prop :
   * property asked (to get its value). \return pair<bool, sint64> : '.first' ==
   * 'true' if the property exist so value is valid. '.second' is the value if
   * '.first' == 'true'.
   */
  ...
  const NLMISC::TGameCycle &predictedInterval() const {
    return _PredictedInterval;
  }
  /// Set the Predicted Interval.
  void predictedInterval(const NLMISC::TGameCycle &pI) {
    if (pI != 0)
      _PredictedInterval = pI;
  }
  //@}
   ...
   *	It is used to remove the LCT for stages without POS (with some
   *restrictions). To avoid pop or stages skipped, the LCT impact is smoothly
   *reduced with time (see CCharacterCL::updateStages())
   */
...
public:
   ...
   * value. \warning If the Game Cycle is before the first element, this method
   * do not try to add a stage but will try to add a property to the first one.
   ...
   * value : value of the property. \param predictedI : predicted interval in
   * tick before the next position update. \return CStage * : pointer on the
   * stage or 0.
   */
  CStage *addStage(NLMISC::TGameCycle gameCycle, uint property, sint64 value);
  CStage *addStage(NLMISC::TGameCycle gameCycle, uint property, sint64 value,
                   NLMISC::TGameCycle predictedI);
