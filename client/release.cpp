...
// ***************************************************************************
// 3D element release, called from both releaseMainLoopReselect() and
// releaseMainLoop()
static void releaseMainLoopScenes() {
  ...
  if (Scene) {
    ...
    // Release the landscape. NB: all pending async loading zones are deleted
    // here
    if (Landscape) {
    ...
    if (SoundMngr != NULL)
      SoundMngr->getMixer()->initClusteredSound((UScene *)NULL, 0.01f, 100.0f,
                                                1.0f);
...
void releaseMainLoopReselect() {
  ...
  // save keys loaded and interface cfg (not done in releaseMainLoop() because
  // done at end of mainLoop()...)
  pIM->uninitInGame0();
#ifdef RYZOM_FORGE
  ...
  // Reset Fx manager (must be done after EntitiesMngr.release()) Important
  // because may still point to 3D elements
  FXMngr.reset();

  // Interface release
  CInterfaceManager::getInstance()->uninitInGame1();

  // Remove key / action / some interface stuff (done through
  // CInterfaceManager::destroy() in releaseMainLoop I think)
  ...
  // Yoyo: Don't release attack list manager, because I think it only owns
  // static data (and 3D data created from Driver, not Scenes) Note that in
  // initMainLoop(), CAttackListManager::getInstance().init() will do nothing
  // (since already created and not released here)
  // --CAttackListManager::getInstance().release();

  // Still release the AnimFXMisc, because btw will still be released at
  // AnimFXMisc.init() time init initMainLoop()
  AnimFXMisc.release();

  // Don't care!
  // --selectTipsOfTheDay (rand());

  // Reset the continent manager. NB: btw will be done at initMainLoop() (from
  // preloadSheets) and important to do before release of timedfx manager
  ContinentMngr.reset();
  ...
  // Release the Entities Animation Manager (Yoyo: fuckingly important because
  // keep a pointer on a _PlayListManager that is created from Scene)
  if (Driver) {
  ...
  // Not necessary I think because owns only static data (string + function
  // ptrs)
  ...
  if (Driver) {
    ...
    // Yoyo: important to do it else leak because recreated/reloaded in
    // initMainLoop() on same pointer (=> old one not released)!!!
    releaseMainLoopScenes();

    // release the auto animation (btw done in initMainLoop()...)
    releaseAutoAnimation();

    // Don't Release the shape bank! => optimisation: preloading of
    // character/objects shapes will not be redone!
  ...
  // Don't Purge memory because PointLight may still be present in other Scenes
  // (sky scene...) for instance
  // --UDriver::purgeMemory();

  // String manager: remove all waiting callbacks and removers
  // (if some interface stuff has not received its string yet, its remover will
  // get useless)
  ...
  // reset the client database and clear all observers. must do this while we
  // are disconnected! First remove the auto copy observers
  pIM->releaseServerToLocalAutoCopyObservers();
  // Then remove the SERVER and LOCAL database (NB: "UI" node was removed by
  // uninitIngame1())
  ...
  nlassert(IngameDbMngr.getNodePtr() ==
           NULL); // actually it is the "SERVER" node kept by CRefPtr => should
                  // be NULL
...
// Yoyo: actually, because of change by AJM, this method is called only when the
// user quit the app. see releaseMainLoopReselect() for the actual method called
// for reselection Btw the 2 methods should have strong similarities
void releaseMainLoop(bool closeConnection) {
  ...
  if (!ClientCfg.Local) {
    ...
    /*
                    ...
                    CBitMemStream out;
                    if(GenericMsgHeaderMngr.pushNameToStream("CONNECTION:QUIT_GAME",
       out))
                    {
                            NetMngr.push(out);
                    }
                    else
                            nlwarning(" unknown message name
...
void releaseOutGame() {
  ...
  // Delete the driver.
  if (Driver) {
    // Stop any async loading. Actually nohting should be async loaded in
    // outgame.
    CAsyncFileManager::terminate();
    ...
    EventsListener.removeFromServer(
        CInputHandlerManager::getInstance()->FilteredEventServer);
...
void release() {
  if (StartPlayTime != 0) {
    CLoginProgressPostThread::getInstance().step(CLoginStep(
        LoginStep_GameExit,
        "login_step_game_exit&play_time=" +
            toString((NLMISC::CTime::getLocalTime() - StartPlayTime) / 1000)));
  }
  ...
  SheetMngr.release();
  //	releaseWeather();	// AJM FIXME conflicting ownership with
  //SheetManager
