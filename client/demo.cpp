...
void initDemo() {
  for (uint i = 0; i < ClientCfg.StartCommands.size() / 7; ++i) {
    ...
    if (!sheetId.buildSheetId(ClientCfg.StartCommands[i * 7])) {
      nlwarning("initDemo: cannot create the entity %d '%s'.", i * 7,
                ClientCfg.StartCommands[i * 7].c_str());
      continue;
    }
    ...
    CEntityCL *entity =
        EntitiesMngr.create(entitySlot, sheetId.asInt(), emptyEntityInfo);
    if (entity) {
      ...
      uint prop = 0; // 0: Position ; 3: orientation ; 4: mode ; 5: behaviour ;
                     // 6: nameId ; 7: target ; 8: visual1 ; 9: visual2
      ...
      /*
                              ...
                              // MOVE //
                              //////////
                              gameCycle = NetMngr.getCurrentClientTick()+100;
         // 10sec later. x =
         (sint64)((entity->pos().x+UserEntity->front().x*50.0)*1000.0); y =
         (sint64)((entity->pos().y+UserEntity->front().y*50.0)*1000.0); z =
         (sint64)((entity->pos().z+UserEntity->front().z*50.0)*1000.0);
                              // Write the position in the DB.
                              IngameDbMngr.setProp("Entities:E" +
         toString(entitySlot) + ":P0", x); IngameDbMngr.setProp("Entities:E" +
         toString(entitySlot) + ":P1", y); IngameDbMngr.setProp("Entities:E" +
         toString(entitySlot) + ":P2", z);
                              // Update the position.
                              EntitiesMngr.updateVisualProperty(gameCycle,
         entitySlot, prop);
      */
    } else
      nldebug("initDemo: entity(%s) in slot %d cannot be created.",
              sheetId.toString().c_str(), entitySlot);
  }
} // initDemo //
