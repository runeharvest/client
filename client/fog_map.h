...
public:
  ...
  bool init(const CFogMapBuild &desc, const NLMISC::CVector &minCoord,
            const NLMISC::CVector &maxCoord);
  // Release datas
  void release();
  /// Compute the fog parameters at the given position.
  void getFogParams(float referenceStartDist, float referenceEndDist, float x,
                    float y, float dayNight, float duskRatio,
   ...
   */
  NLMISC::CRGBAF getMapValue(TMapType type, float x, float y,
                             NLMISC::CRGBAF defaultValue) const;
  //
  NLMISC::CBitmap &getMap(TMapType type) {
...
private:
  ...
  NLMISC::CRGBAF getMapValueFromMapCoord(TMapType type, float x, float y,
                                         NLMISC::CRGBAF defaultValue) const;
};
