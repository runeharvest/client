...
extern uint8
    ShowInfos; // 0=no info 1=text info 2=graph info 3=streaming info 4=fps only
...
/*
        nlwarning("The MemoryStatus structure is %ld bytes long.",
   stat.dwLength); nlwarning("It should be %d.", sizeof (stat)); nlwarning("%ld
   percent of memory is in use.", stat.dwMemoryLoad); nlwarning("There are %*ld
   total %sbytes of physical memory.", WIDTH, stat.dwTotalPhys/DIV, divisor);
        nlwarning("There are %*ld free %sbytes of physical memory.", WIDTH,
   stat.dwAvailPhys/DIV, divisor); nlwarning("There are %*ld total %sbytes of
   paging file.", WIDTH, stat.dwTotalPageFile/DIV, divisor); nlwarning("There
   are %*ld free %sbytes of paging file.", WIDTH, stat.dwAvailPageFile/DIV,
   divisor); nlwarning("There are %*lx total %sbytes of virtual memory.", WIDTH,
   stat.dwTotalVirtual/DIV, divisor); nlwarning("There are %*lx free %sbytes of
...
/*
        ...
        nlwarning("There are %*ld free %sbytes of physical memory.", WIDTH,
   stat.dwAvailPhys/DIV, divisor); nlwarning("There are %*ld free %sbytes of
   paging file.", WIDTH, stat.dwAvailPageFile/DIV, divisor); nlwarning("There
   are %*lx free %sbytes of virtual memory.\n", WIDTH, stat.dwAvailVirtual/DIV,
   divisor);
*/
#endif
...
CGraph SpfGraph("time per frame (100 ms)", 10.0f, 10.0f, 250.0f, 100.0f,
                CRGBA(0, 128, 0, 255), 0, 100.0f, 3);
CGraph CurrentTaskGraph("current task", 10.0f, 120.0f, 250.0f, 50.0f,
                        CRGBA(0, 0, 128, 255), 0, 1.0f, 3);
CGraph VRAMDownGraph("vram download (1Mo)", 10.0f, 180.0f, 250.0f, 50.0f,
                     CRGBA(128, 0, 0, 255), 0, 1.f, 3);
CGraph VRAMUpGraph("vram upload (1Mo)", 10.0f, 240.0f, 250.0f, 50.0f,
                   CRGBA(128, 0, 0, 255), 0, 1.f, 3);
CGraph OpenedFileGraph("fopen (10)", 10.0f, 300.0f, 250.0f, 50.0f,
                       CRGBA(128, 128, 0, 255), 0, 10.f, 3);
CGraph ByteReadGraph("byte read (500 ko)", 10.0f, 360.0f, 250.0f, 100.0f,
                     CRGBA(0, 128, 128, 255), 0, 500, 3);
CGraph ByteReadGraphInstant("instant byte read (500 ko)", 10.0f, 470.0f, 250.0f,
                            100.0f, CRGBA(64, 0, 128, 255), 0, 500, 3);
CGraph FileReadGraph("fread (10)", 10.0f, 580.0f, 250.0f, 50.0f,
                     CRGBA(128, 0, 128, 255), 0, 10, 3);
CGraph LuaMemGraph("Lua memory (mb)", 290.0f, 10.0f, 250.0f, 100.0f,
                   CRGBA(0, 128, 64, 255), 0, 64.0f, 3);
...
void displayStreamingDebug() {
  ...
  if (!FreezeGraph) {
    ...
    // lua memory
    LuaMemGraph.addOneValue(
        CLuaManager::getInstance().getLuaState()->getGCCount() / 1024.f);

    // Count of waitinf instance
    CurrentTaskGraph.addOneValue(
        CAsyncFileManager::getInstance().isTaskRunning() ? 1.f : 0.f);
  }
...
public:
  ...
  void debugDisplayConnectionEvent(string &str) {
    ...
    for (uint i = 0; i < EventQueue.size(); i++) {
      if (i > 0)
        str += ", ";
      // Avoid crash in the crash log: in the improbable case where EventQueue
      // has been crashed, at least don't crash here
...
const char *CDebugConnectionHistory::EventNames
    [CDebugConnectionHistory::NumDebugConnectionEvent] = {"HOP", "FTP", "RSL"};
const char *CDebugConnectionHistory::CounterNames
    [CDebugConnectionHistory::NumDebugConnectionEvent] = {
        "NumServerHOP", "NumFarTP", "NumReselectPerso"};

// extern methods
void crashLogAddServerHopEvent() {
  DebugConnectionHistory.debugAddConnectionEvent(
      CDebugConnectionHistory::ServerHopEvent);
}
void crashLogAddFarTpEvent() {
  DebugConnectionHistory.debugAddConnectionEvent(
      CDebugConnectionHistory::FarTPEvent);
}
void crashLogAddReselectPersoEvent() {
  DebugConnectionHistory.debugAddConnectionEvent(
      CDebugConnectionHistory::ReselectPersoEvent);
}
...
string getDebugInformation() {
  ...
  if (UserEntity) {
    ...
    str += toString("UserPosition: %.2f %.2f %.2f\n", UserEntity->pos().x,
                    UserEntity->pos().y, UserEntity->pos().z);
  } else {
    str += "No user entity information\n";
  }

  str += toString("ViewPosition: %.2f %.2f %.2f\n", View.viewPos().x,
                  View.viewPos().y, View.viewPos().z);
  uint64 timeInGame = ingameTime1();
  str += toString(
      "Time in game: %dh %dmin %dsec\n", (uint)(timeInGame / (60 * 60 * 1000)),
      (uint)(timeInGame / (60 * 1000)) % 60, (uint)(timeInGame / 1000) % 60);
  ...
  str += string("Client is ") + string((ClientCfg.Local ? "off" : "on")) +
         string("line\n");
  // FarTP/ReselectPerso
...
void resetIngameTime() { IngameEnterTime = T1; }

uint64 ingameTime0() { return T0 - IngameEnterTime; }

uint64 ingameTime1() { return T1 - IngameEnterTime; }
...
void displayNetDebug() {
  ...
  line += lineStep;
  // Inventory Counter
  val = NLGUI::CDBManager::getInstance()
  ...
  TextContext->printfAt(1.f, line, "NextAction  (loc/srv): %d/%d", locVal,
                        srvVal);
  line += lineStep;
  // CycleAction
  ...
  srvVal &= PHRASE_EXECUTE_COUNTER_MASK;
  locVal &= PHRASE_EXECUTE_COUNTER_MASK;
  TextContext->printfAt(1.f, line, "CycleAction (loc/srv): %d/%d", locVal,
                        srvVal);
  line += lineStep;
