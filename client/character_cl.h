...
public:
  ...
  CStageSet _Stages;
  enum { MaxNumAura = 2 };
  enum TAtkHeight { AtkUnkownHeight = 0, AtkLow, AtkMiddle, AtkHigh };
...
public:
  ...
  virtual void updatePostCollision(const NLMISC::TTime &time,
                                   CEntityCL *target);
  virtual void updateVisible(const NLMISC::TTime &currentTimeInMs,
                             CEntityCL *target);
  virtual void updateSomeClipped(const NLMISC::TTime &currentTimeInMs,
                                 CEntityCL *target);
  virtual void updateClipped(const NLMISC::TTime &currentTimeInMs,
                             CEntityCL *target);

  // Get the move for the MOVE channel with the blend.
  double getTheMove(double loopTimeStep, double oldMovingTimeOffset,
   ...
   * \param target : pointer on the current target.
   */
  virtual void updatePosCombatFloat(double frameTimeRemaining,
                                    CEntityCL *target);
  /**
   * Update the position of the entity after the motion.
   ...
   * \param target : pointer on the current target.
   */
  virtual void updatePos(const NLMISC::TTime &currentTimeInMs,
                         CEntityCL *target);
   ...
   * for example). This will remove the entity from the target. \param slot :
   * Slot of the entity that will be removed.
   */
  ...
  enum TOnRot { OnRotNone = 0, OnRotLeft, OnRotRight };
  enum TOnBigBend { OnBendNone = 0, OnBendLeft, OnBendRight };
  //
  TOnMove onMove(const CAutomatonStateSheet &curAnimState);
  //
  TOnRot onRotation(const CAutomatonStateSheet &curAnimState,
                    NLMISC::CVector &dirEndA);
  //
  TOnBigBend onBigBend(const CAutomatonStateSheet &curAnimState,
                       NLMISC::CVector &dirEndA);
  //
  ...
  /** Force an animation.
   */
  void setAnim(TAnimStateKey newKey,
               TAnimStateKey subKey = (TAnimStateKey)CAnimation::UnknownAnim,
               uint animID = NL3D::UAnimationSet::NotFound);
  ...
  virtual uint nbAttachedFXToRemove() {
    return (uint)_AttachedFXListToRemove.size();
  }
  ...
  std::vector<const CAnimationSet *> &currentAnimSet() {
    return _CurrentAnimSet;
  }
  ...
  bool isSwimming() const {
    return (_Mode == MBEHAV::SWIM || _Mode == MBEHAV::SWIM_DEATH ||
            _Mode == MBEHAV::MOUNT_SWIM);
  }
  /// Return true if the character is riding.
  bool isRiding() const {
    return (_Mode == MBEHAV::MOUNT_NORMAL || _Mode == MBEHAV::MOUNT_SWIM);
  }
  /// Is the entity in combat.
  bool isFighting() const {
    return (_Mode == MBEHAV::COMBAT || _Mode == MBEHAV::COMBAT_FLOAT);
  }
  /// Return true if the character is currently dead.
  virtual bool isDead() const {
    return (_Mode == MBEHAV::DEATH || _Mode == MBEHAV::SWIM_DEATH);
  }
  /// Return true if the character is really dead. With no lag because of anim
  /// or LCT
  virtual bool isReallyDead() const {
    return (_TheoreticalMode == MBEHAV::DEATH ||
            _TheoreticalMode == MBEHAV::SWIM_DEATH);
  }

  const CAutomatonStateSheet *currentState() { return _CurrentState; }
  void currentState(const CAutomatonStateSheet *newState) {
  ...
  // get scale from reference scale (male fyros). For creatures, returns the
  // same than _CharacterScalePos
  ...
  // align an fx to be positionned and oriented like that entity, and put in the
  // same cluster system than this entity
  void
  alignFX(NL3D::UParticleSystemInstance instance, float scale = 1.f,
          const NLMISC::CVector &localOffset = NLMISC::CVector::Null) const;

  // align a fx using the given matrix (also add scale & offset) , and put in
  // the same cluster system than this entity
  void
  alignFX(NL3D::UParticleSystemInstance instance, const NLMISC::CMatrix &matrix,
          float scale = 1.f,
          const NLMISC::CVector &localOffset = NLMISC::CVector::Null) const;
   ...
   * If pointer isn't NULL then a new aura is created. Any previously present
   * aura is immediatly deleted.
   */
  void setAuraFX(uint index, const CAnimationFX *fx);
  /** Set a new link fx.
   * If there's a previous link, the dispell fx is played to make it disappear
   * If fx ptr is NULL then no new link fx is created (but dispell is still
   * played)
   */
  void setLinkFX(const CAnimationFX *fx, const CAnimationFX *dispell);

  // From CEntityCL
  const char *getBoneNameFromBodyPart(BODY::TBodyPart part,
                                      BODY::TSide side) const;
  // ...
  virtual bool getBoneHeight(BODY::TBodyPart localisation, BODY::TSide side,
                             float &height) const;
  //
  ...
  void setAnimJobSpecialisation(uint32 state) {
    _AnimJobSpecialisation = state;
  }
...
protected:
  ...
  public:
    ...
    NL3D::UParticleSystemInstance
        AdvantageFX; // adantage fx linked to that item
  public:
    CWornItem() { Sheet = NULL; }
     ...
     */
    void startAttackFX(NL3D::USkeleton skeleton, uint intensity,
                       SLOTTYPE::EVisualSlot visualSlot, bool activateTrail);
    // stop attack fx
    void stopAttackFX();
    // start/stop the advantage fx (advantage fx appears on a weapon that is
    // mastered by user)
    void enableAdvantageFX(NL3D::UInstance parent);
    /** set size of trail (value clamped to [0, 15]). This will map to the
     * values given in the item sheet. 0 mean that there's no trail
     */
  ...
  double _DistToFirst;
  // Time at which the first run animation should start in order to avoid pop
  // (computed in updateStages()) It is used to accelerate animations that are
  ...
  ////// ANIMATION
  /// Used to adjust the rotation of the animation to finish with the perfect
  /// orientation.
  ...
  /// The job specialisation of the character (0 by default). visual prop from
  /// server
  uint32 _AnimJobSpecialisation;

  /** List of animation FXs currently being played. Those FXs are shutdown as
   * soon as an animation switch occurs. When theyareshutdown,theit emitter are
   * stopped, and they are put in _RemoveAnimFXList,waiting for deletion
   */
  std::list<CAttachedFX::TSmartPtr> _AttachedFXListForCurrentAnim;
  /// List of attached to remove as soon as possible (when there are no
  /// particles left)
  ...
  public:
    CAnimationFX AF; // ideally this field should be per sheet, not per
                     // instance, but simpler this way
  ...
  // if 0(default), LCT is fully applied, else this is the ServerTick when the
  // LCT impact decrease
  NLMISC::TGameCycle _StartDecreaseLCTImpact;

  // this time is the estimated time of the first important stage (excluding the
  // first one)
...
protected:
  ...
  NLMISC::TGameCycle adjustPI(float x, float y, float z,
   ...
   * This convert any attack encoded in current behaviour
   * (magic/melee/range/creature_attack) into a common representation
   * (CAttackInfo)
   */
   ...
   */
  void performCurrentAttackEnd(const CBehaviourContext &bc,
                               bool directOffensifSpell,
                               std::vector<double> &targetHitDates,
                               TAnimStateKey animForCombat);

  // update current attack from the current behaviour (this update the "_Attack"
  // field)
  void updateCurrentAttack();

  /** Create part of current attack (a single projectile or/and impact, total
   * attack may contain severals) IMPORTANT: the instance on which this method
   * is invoked is NOT always the caster (secondary missiles can be generated
   * after main target has been reached) NB : target stick mode isn't filled,
   * for this, should use computeTargetStickMode instead NB : once fully filled,
   * the CProjectileBuild object should be inserted in the projectile manager
   */
  bool createCurrentAttackEndPart(
  ...
  TAtkHeight getAttackHeight(CEntityCL *target, BODY::TBodyPart localisation,
                             BODY::TSide side) const;

  ////// FX

  /** Stop all attached FXs linked to current anim (this wait until FX have no
   * more particle, after what they are effectively deleted) \param
   * stopLoopingFX If set to true, looping fx are also stopped. A looping fx has
   * RepeatMode == Loop in its sheet.
   */
   ...
   * \param instIdx : if not CEntityCL::BadIndex, replace the equipement
   * instance at this index. \return uint32 : index of the instance or
   * CEntityCL::BadIndex if there were a problem. \todo GUIGUI : find a better
   * choice to avoid all these slotType tests
   */
  uint32 buildEquipment(const CCharacterSheet::CEquipment &slot,
                        SLOTTYPE::EVisualSlot visualSlot, sint color = -1,
                        uint32 instIdx = CEntityCL::BadIndex);
  ...
  virtual void updateVisualPropertyOrient(const NLMISC::TGameCycle &gameCycle,
                                          const sint64 &prop);
  ...
  updateVisualPropertyBehaviour(const NLMISC::TGameCycle &gameCycle,
                                const sint64 &prop);
  /// Update Entity Name.
  virtual void updateVisualPropertyName(const NLMISC::TGameCycle &gameCycle,
                                        const sint64 &prop);
  /// Update Entity Target.
  virtual void updateVisualPropertyTarget(const NLMISC::TGameCycle &gameCycle,
                                          const sint64 &prop);
  /// Update Entity Mode.
  virtual void updateVisualPropertyMode(const NLMISC::TGameCycle &gameCycle,
                                        const sint64 &prop);
  /// Update the NPC Alternative Look Property.
  virtual void updateVisualPropertyVpa(const NLMISC::TGameCycle &gameCycle,
                                       const sint64 &prop);
  virtual void updateVisualPropertyVpb(const NLMISC::TGameCycle &gameCycle,
                                       const sint64 &prop);
  /// Update Entity Mount
  virtual void
  ...
  updateVisualPropertyRiderEntity(const NLMISC::TGameCycle &gameCycle,
                                  const sint64 &prop);
  /// Update Entity Bars
  virtual void updateVisualPropertyBars(const NLMISC::TGameCycle &gameCycle,
                                        const sint64 &prop);
  virtual void
  ...
  updateVisualPropertyEventFactionID(const NLMISC::TGameCycle &gameCycle,
                                     const sint64 &prop);
  virtual void updateVisualPropertyPvpMode(const NLMISC::TGameCycle &gameCycle,
                                           const sint64 &prop);
  virtual void updateVisualPropertyPvpClan(const NLMISC::TGameCycle &gameCycle,
                                           const sint64 &prop);
  virtual void
  ...
  virtual void updateVisualPropertyStatus(const NLMISC::TGameCycle &gameCycle,
                                          const sint64 &prop);
  // Update target list
  virtual void
  updateVisualPropertyTargetList(const NLMISC::TGameCycle &gameCycle,
                                 const sint64 &prop, uint listIndex);
  // Update tvisual fw
  virtual void updateVisualPropertyVisualFX(const NLMISC::TGameCycle &gameCycle,
                                            const sint64 &prop);
  ...
  /// Get the direction that should have the character at the end of the
  /// animation.
  const NL3D::CVector &dirEndAnim() { return _DirEndAnim; }
  /// Set the direction that should have the character at the end of the
  /// animation.
   ...
   */
  uint32 addColoredInstance(const std::string &shapeName,
                            const std::string &stickPoint = "",
                            sint texture = -1, uint32 instIdx = BadIndex,
                            sint color = -1);
  ...
  ///
  double computeMotion(const double &oldMovingTimeOffset,
                       TAnimationType channel = MOVE) const;
  ...
  void endCast(const MBEHAV::CBehaviour &behaviour,
  ...
  /// destination and some more information.
  void updateStages();
   ...
   * animation. \param stopTime : time to stop processing events from the
   * current animation.
   */
  void animEventsProcessing(double startTime, double stopTime);

  void applyColorSlot(SInstanceCL &instance, sint skin, sint userColor,
                      sint hair, sint eyes);

  /// Return the current max speed for the entity in meter per sec (!= swim, !=
  /// mount, etc.)
  ...
  /// Return the Distance from the current entity position to the First
  /// Position.
...
private:
  ...
  /** Return the shape pointer from tha item and according to the character
   * gender. \param itemSheet : reference on the item sheet. \return string & :
   * reference on the shape name.
   */
  std::string shapeFromItem(const CItemSheet &itemSheet) const;

  /// Create the instance from an item
  uint32 createItemInstance(const CItemSheet &itemSheet, uint32 instIdx,
                            SLOTTYPE::EVisualSlot visualSlot,
                            const std::string &bindBone, sint8 texture,
                            sint color);
   ...
   * has to be between 0 and 1 to be valid). \return bool : true if the trigger
   * is valid. \warning This method does not check if the animation is Null.
   */
  bool beginImpact(NL3D::UAnimation *anim, NL3D::TAnimationTime currentTime,
                   const std::string &triggerName, bool &isActive,
                   float timeFactor);
  ...
  // Attach a CAttachedFX  object to that character.
  void attachFXInternal(const CAttachedFX::TSmartPtr fx,
                        TAttachedFXList wantedList);

  //  attach an externally created fx in the given list (alive fx or shutting
  //  down fxs)
  void attachFX(NL3D::UParticleSystemInstance instance,
                const CAnimationFX *sheet, const CFXStickMode *stickMode,
  ...
  /// Helper fct : remove any obsolete reference in a CAnimFX list to an entity
  /// slot that has just been removed.
  static void
  updateAnimFXListForSlotRemoved(std::list<CAttachedFX::TSmartPtr> &fxList,
  ...
  void computeTargetStickMode(const CAttackSheet &sheet,
                              const CAttackInfo &attackInfo, CFXStickMode &dest,
                              CEntityCL &target);

  // remove attached fxs that depends on another slot when it has been removed
  void updateAttachedFXListForSlotRemoved(
      std::list<CAttachedFX::TSmartPtr> &fxList,
      const CLFECOMMON::TCLEntityId &slotRemoved);

  // apply the flying HP related to behaviour
  void applyBehaviourFlyingHPs(const CBehaviourContext &bc,
                               const MBEHAV::CBehaviour &behaviour,
                               const std::vector<double> &targetHitDates);

protected:
  // retrieve attack from an id and a list of attack lists filenames
  const CAttack *
  getAttack(const CAttackIDSheet &id,
