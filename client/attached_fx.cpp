...
// Copyright (C) 2013  Matt RAYKOWSKI (sfb) <matt.raykowski@gmail.com>
//
...
// ***********************************************************************************************************************
void CAttachedFX::create(CCharacterCL &parent, const CBuildInfo &buildInfo,
                         const CTargeterInfo &targeterInfo) {
  clear();
  if (!buildInfo.Sheet)
    return;
  UParticleSystemInstance instance = buildInfo.Sheet->createMatchingInstance();
  // TODO nico : user params are not in the buildInfo, but are set by
  // createMatchingInstance then by the caller (they could be set directly by
...
void CAttachedFX::create(CCharacterCL &parent,
  ...
  const CFXStickMode *stickMode = buildInfo.StickMode
                                      ? buildInfo.StickMode
                                      : &buildInfo.Sheet->Sheet->StickMode;
  if (!instance.empty()) {
  ...
  case CFXStickMode::StaticMatrix: {
    const CMatrix &staticMatrix = buildInfo.StaticMatrix
                                      ? *buildInfo.StaticMatrix
  ...
  case CFXStickMode::UserBone: {
    if (parent.skeleton()) {
      sint boneID = parent.skeleton()->getBoneIdByName(
  ...
  case CFXStickMode::UserBoneOrientedTowardTargeter:
    if (parent.skeleton()) {
      UserBoneID = parent.skeleton()->getBoneIdByName(
          NLMISC::CStringMapper::unmap(stickMode->UserBoneName));
      if (UserBoneID == 0xff) {
        nlwarning(
            "Bad bone name : %s",
  ...
  case CFXStickMode::UserBoneRay:
    ...
    if (targeterInfo.StickMode.UserBoneName != 0) {
      ...
      if (parent.skeleton() && targeter && targeter->skeleton()) {
        TargeterUserBoneID = parent.skeleton()->getBoneIdByName(
            NLMISC::CStringMapper::unmap(TargeterInfo.StickMode.UserBoneName));
      }
    }
    update(parent, CMatrix::Identity);
    break;
...
// ***********************************************************************************************************************
void CAttachedFX::update(CCharacterCL &parent,
                         const NLMISC::CMatrix &alignMatrix) {
  if (AniFX && !FX.empty()) {
    ...
    case CFXStickMode::Follow:
      ...
      parent.alignFX(FX, alignMatrix,
                     AniFX->Sheet->ScaleFX ? parent.getScaleRef() : 1.f,
                     trackPos + SpawnPos);
      break;
    case CFXStickMode::FollowNoRotation: {
      // just update the pos
    ...
    case CFXStickMode::OrientedTowardTargeter: {
      ...
      if (targeter) {
        ...
        CVector J((float)orientD.x, (float)orientD.y,
                  0.f); // project on XY plane
        J.normalize();
