...
static void renderEnvMapScene(UScene *scene, UCamera &cam,
                              const CMatrix &camMatrix, float znear,
                              float zfar) {
  ...
  if (!cam.empty()) {
    ...
    scene->renderPart((UScene::TRenderPart)(UScene::RenderOpaque |
...
void CWaterEnvMapRdr::doRender(const CMatrix &camMatrix,
                               TGlobalAnimationTime time, UDriver &drv) {
  H_AUTO_USE(RZ_WaterEnvMapRDR)
  drv.clearZBuffer();
  ...
  if (Sky) {
    Sky->setup(_DateForRender, _AnimationDateForRender, _WeatherForRender,
               _FogColorForRender, CVector(0.5f, 0.f, -0.85f).normed(), true);
    if (CurrTime != -1) {
      if (Sky->getScene())
        Sky->getScene()->animate(
            CurrTime); // this will force the elapsed time to be set to  so that
                       // fx won't be animated twice
    }
  }
  drv.setCullMode(drv.getCullMode() == UDriver::CCW ? UDriver::CW
                                                    : UDriver::CCW);
  // render canopy
  ContinentMngr.getFogState(CanopyFog, LightCycleManager.getLightLevel(),
                            LightCycleManager.getLightDesc().DuskRatio,
                            LightCycleManager.getState(),
                            CVectorD(View.viewPos()), RootFogState);
  RootFogState.setupInDriver(drv);
  CMatrix mat = camMatrix;
  drv.setDepthRange(0.f, 0.5f);
  mat.setPos(_CurrCanopyCamPos);
  renderEnvMapScene(SceneRoot, WaterEnvMapCanopyCam, mat, SceneRootCameraZNear,
                    SceneRootCameraZFar);
  Driver->enableFog(false);
  // render sky
  if (Sky) {
    drv.setDepthRange(0.5f,
                      1.f); // force sky scene to appear behind canopy scene
