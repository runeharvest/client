...
/** Spawn/despawn microlife fx on the landscape. Those fx are created in the
 * world editor using 'patatoid' primitives. This manager look in which
 * primitive the player is, and add/remove microlife with the given density as
 * necessary. Microlife FXs are a special case of timed fx, and this manager
 * relies in fact on CTimedFXManager \TODO The container used is basically a
 * quadgrid tailored for polygons, so it may be good to put the container part
 * in NL3D for reuse (test for water intersection are similar) \author Nicolas
 * Vizerie \author Nevrax France \date 1/2004
 */
...
public:
   ...
   * \param cellSize size of grid cells used to test if a tile intersect a
   * primitive (also limit precision of tests)
   */
  void init(const NLMISC::CVector2f &minCorner,
            const NLMISC::CVector2f &maxCorner, float cellSize = 30.f);
...
private:
  ...
  public:
    ...
    bool IsFullyCovered; // the grid cell is fully contained into that polygon
                         // so fast test can be performed
  ...
  /** A grid that cover the continent and that give the list of primitives
   * polygons that cover each cell List of primitives are shared between each
   * cell. Each cell contains an index into _PossibleOverlapPolyLists.
   */
...
private:
  ...
  // add / remove a poly of a zone to a build grid. Poly coordinates are in
  // world.
  void drawPolyInBuildGrid(const std::vector<NLLIGO::CPrimVector> &primPoly,
                           uint primitiveIndex, TBuildGrid &buildGrid,
                           const CFloraSheet *sheet, bool isExcludeTri);
  // pack the build grid so that each cell only contains index into a small set
  // of poly list, instead of a full poly list (a lot of cells share the same
  // poly list)
  ...
  void addPrimitiveToBuildGrid(const std::string &fileName,
                               uint &primitiveIndex, TBuildGrid &buildGrid);
  //
