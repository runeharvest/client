...
// The possible not corrected error between actual ServerTick and estimated one
// (NB: equal to packet sent frequency)
#define SMOOTH_SERVER_TICK_WINDOW 100
// The min Difference between the estimated and the actual one. If <=, the
// estimation is reseted.
...
const char *ConnectionStateCStr[9] = {
    "NotInitialised", "NotConnected", "Authenticate",
    "Login",          "Synchronize",  "Connected",
    "Probe",          "Stalled",      "Disconnect"};
...
// Stat: array of vectors of cycles when a pos is received, indexed by
// TCLEntityId
struct TRDateState {
  TRDateState(TGameCycle gc, TGameCycle pdit, TTime ct)
      : ServerCycle(gc), PredictedInterval(pdit), LocalTime(ct) {}
...
void initReceiveLog() {
  try {
    ...
    // ReceiveLogger.displayNL( "Searching for param LogReceive in the config
    // file..." );
...
/*
 * startReceiveLog (the slots logged are all if no selection, or only the
 * selected slots (one at a time)
 */
 ...
 * stopReceiveLog
 */
void stopReceiveLog() { LogReceiveEnabled = false; }
...
void displayReceiveLog() {
  ...
  ReceiveLogger.displayNL(
      "ReceiveLog (ServerCycle, PredictedInterval, LocalTime(ms)):");

  // Display receive dates for all slots for which vector is not empty
  // (allows to trace several selected slots one after one, or all slots at the
  // same time)
  sint i;
  for (i = 0; i != 256; ++i) {
    if (!ReceivePosDateLog[i].empty()) {
      ReceiveLogger.displayRawNL("Entity %d: %u updates", i,
                                 ReceivePosDateLog[i].size());
      TReceiveDateLog::iterator idl;
      for (idl = ReceivePosDateLog[i].begin();
           idl != ReceivePosDateLog[i].end(); ++idl) {
        ReceiveLogger.displayRawNL("%u\t%u\t%" NL_I64 "u", (*idl).ServerCycle,
                                   (*idl).PredictedInterval, (*idl).LocalTime);
      }
    }
  }
...
NLMISC_COMMAND(startReceiveLog,
               "Starts logging the position receives (for all or only the "
               "watched entities when selected)",
               "") {
...
CVariable<bool>
    CheckXMLSignature("client", "CheckXMLSignature",
                      "enable client to check msg/database.xml signature", true,
                      0, true);

CSlotGraph *PosUpdateIntervalGraph = NULL;
CSlotGraph *PosUpdatePredictionGraph = NULL;

CGraph MsPerTickGraph("mspertick (ms)", 10.0f, 570.0f, 400.0f, 100.0f,
                      CRGBA(0, 0, 128, 128), 1000, 400.0f, 2);
CGraph PingGraph("ping (ms)", 10.0f, 460.0f, 400.0f, 100.0f,
                 CRGBA(128, 0, 0, 128), 100000, 1000.0f, 2);
CGraph PacketLossGraph("PacketLoss (pc)", 10.0f, 350.0f, 200.0f, 100.0f,
                       CRGBA(0, 128, 0, 128), 100000, 100.0f, 2);
CGraph UploadGraph("upload (byps)", 10.0f, 240.0f, 200.0f, 100.0f,
                   CRGBA(0, 128, 128, 128), 1000, 3000.0f, 2);
CGraph DownloadGraph("download (byps)", 10.0f, 130.0f, 200.0f, 100.0f,
                     CRGBA(0, 0, 128, 128), 1000, 3000.0f, 2);
...
/*float percentile( const multiset<uint32>& dataset, float p )
{
        ...
        itnext = it; ++itnext;
        return ((float)(ipIndex+1)-fpIndex)*(float)(*it) +
(fpIndex-(float)ipIndex)*(float)(*itnext);
}*/

// Reversed quantile function = percentile(dataset, 1-p) (optimized for p>0.5
// i.e. rp<=0.5)
...
void CNetworkConnection::init(const string &cookie, const string &addr) {
  if (_ConnectionState != NotInitialised && _ConnectionState != Disconnect) {
    nlwarning("Unable to init(): connection not properly closed yet.");
    return;
  }

  if (!_Registered) {
    CActionFactory::getInstance()->registerAction(ACTION_POSITION_CODE,
                                                  CActionPosition::create);
    CActionFactory::getInstance()->registerAction(ACTION_SYNC_CODE,
                                                  CActionSync::create);
    CActionFactory::getInstance()->registerAction(ACTION_DISCONNECTION_CODE,
                                                  CActionDisconnection::create);
    CActionFactory::getInstance()->registerAction(ACTION_ASSOCIATION_CODE,
                                                  CActionAssociation::create);
    CActionFactory::getInstance()->registerAction(ACTION_DUMMY_CODE,
                                                  CActionDummy::create);
    CActionFactory::getInstance()->registerAction(ACTION_LOGIN_CODE,
                                                  CActionLogin::create);
    CActionFactory::getInstance()->registerAction(ACTION_TARGET_SLOT_CODE,
                                                  CActionTargetSlot::create);
    CActionFactory::getInstance()->registerAction(ACTION_GENERIC_CODE,
                                                  CActionGeneric::create);
    CActionFactory::getInstance()->registerAction(
        ACTION_GENERIC_MULTI_PART_CODE, CActionGenericMultiPart::create);
    CActionFactory::getInstance()->registerAction(ACTION_SINT64,
...
void CNetworkConnection::initCookie(const string &cookie, const string &addr) {
...
#endif
  {
    ...
    nlinfo("Network initialisation with front end '%s' and cookie %s",
           _FrontendAddress.c_str(), _LoginCookie.toString().c_str());
  }
 ...
 */
void CNetworkConnection::setRecordingMode(bool onOff,
 ...
 */
void CNetworkConnection::setReplayingMode(bool onOff,
                                          const std::string &filename) {
  nlassert(ClientCfg.Local);
  if (onOff) {
...
bool CNetworkConnection::connect(string &result) {
  if (_ConnectionState != NotConnected) {
    nlwarning("Unable to connect(): connection not properly initialised (maybe "
  ...
  try {
    ...
    if (cfg.getVar("Use").asInt() == 1) {
      ...
      nlinfo("Try to find a shard that have fsaddr = '%s'",
             fsaddr.toStringLong().c_str());
      ...
      for (i = 0; i < shards.size(); i += 2) {
        try {
          ...
          if (net == fsaddr) {
            ...
            string srcPath = CPath::standardizeDosPath(
                CPath::getFullPath(shards.asString(i + 1)));
            ...
            for (uint j = 0; j < needToCopy.size(); j++) {
              ...
              string dstPath = CPath::standardizeDosPath(CPath::getFullPath(
                  CPath::lookup(CFile::getFilename(needToCopy.asString(j))),
                  false));
              try {
                ...
                if (arg1 != dstPath) {
                  nlinfo("Copying '%s' into '%s'", arg1.c_str(),
                         dstPath.c_str());
                  nlinfo("executing 'copy /Y %s %s", arg1.c_str(),
                         dstPath.c_str());
                  string str = "copy /Y " + arg1 + " " + dstPath;
                  int ret = system(str.c_str());
                  // int ret = _spawnlp (_P_WAIT, "copy", "copy", "/Y",
                  // arg1.c_str(), dstPath.c_str(), NULL);
                  if (ret != 0) {
                    nlwarning("the copy command seems failed with the error "
                              "code %d, errno %d: %s",
                ...
                nlwarning("Can't copy '%s' '%s', try the next file",
                          arg1.c_str(), dstPath.c_str());
              }
            }
            break;
          }
      ...
      if (i == shards.size()) {
        nlwarning("the fsaddr '%s' is not in the shards.cfg, can't copy "
                  "data_common files",
                  fsaddr.toStringLong().c_str());
      }
    }
    ...
    nlinfo(
        "There's no shards.cfg, or bad file format, can't copy common files");
  }

  nlinfo("CNET[%p]: Connecting to '%s' with cookie '%s'", this,
         _FrontendAddress.c_str(), _LoginCookie.toString().c_str());

  // then connect to the frontend using the udp sock
  // ace faut faire la nouveau login client 	result =
  // CLoginClient::connectToShard (_FrontendAddress, _Connection);

  m_QuicConnection.update();
  nlassert((!m_Connection.connected()) &&
           !(m_QuicConnection.state() == CQuicConnection::Connected));

  try {
    //
    // S12: connect to the FES. Note: In UDP mode, it's the user that have to
    ...
    if (m_UseQuic) {
      ...
      _FrontendHost.setPort(_FrontendHost.port() -
                            5000); // Same trick as SBS, just use a port offset
      nlinfo("Connecting to QUIC at port %d instead", _FrontendHost.port());
      m_QuicConnection.connect(_FrontendHost);
    } else {
      m_QuicConnection
          .release(); // Do clean up any resources if we're no longer using QUIC
...
void CNetworkConnection::setImpulseCallback(TImpulseCallback callback,
                                            void *argument) {
...
bool CNetworkConnection::isConnected() { return _ConnectionState == Connected; }
...
bool CNetworkConnection::dataToReplayAvailable() {
  ...
  // nldebug( "current=%u nextToReplay=%u", _CurrentClientTick,
  // _NextClientTickToReplay );
  return (_CurrentClientTick >=
          _NextClientTickToReplay); // when true => authorize entering
                                    // buildStream()...
}
#endif
...
bool CNetworkConnection::update() {
  ...
  if (res) {
    ...
    if (diff > 2000) {
      nlinfo("Reads by second: %.1f => LoopTime = %.2f ms LoopCount = %u Diff "
             "= %u ms",
             (float)loopcount * 1000.0f / (float)diff, (float)diff / loopcount,
             loopcount, diff);
      loopcount = 0;
      lastdisplay = tn;
    }
...
#define new DEBUG_NEW
      ...
      nlinfo("CNET[%p]: Connecting to next FS host address '%s'", this,
             frontendHost.address().asIPString().c_str());
      try {
        // Connect
    ...
    if (tres.empty()) {
      ...
      nlinfo("CNET[%p]: Client connected to shard again, attempting login",
             this);
    } else {
  ...
  try {
    ...
    do {
      ...
      default:
        // Nothing here !
        stateBroke = false; // will come here if a disconnection action is
                            // received inside a method that returns true
        break;
      }
  ...
  if (m_UseQuic &&
      (m_QuicConnection.state() == CQuicConnection::Disconnected)) {
    // Bye
    _ConnectionState = Disconnect;
  }
  ...
  if (!m_UseQuic && !m_Connection.connected()) {
    // if(!ClientCfg.Local)
    //	nlwarning("CNET[%p]: update() attempted whereas socket is not connected
    //!", this);
    _ConnectionState = Disconnect;
  }
...
bool CNetworkConnection::buildStream(CBitMemStream &msgin) {
  ...
  if (_ReplayIncomingMessagesOn) {
    ...
    msgin.clear();
    memcpy(msgin.bufferToFill(_NextMessageToReplay.length()),
           _NextMessageToReplay.buffer(), _NextMessageToReplay.length());
    // nldebug( "Reading message for tick %u (size %u)",
    // _NextClientTickToReplay, msgin.length() );
  ...
  if (m_UseQuic) {
    if (m_QuicConnection.receiveDatagram(msgin)) {
      ...
      // FIXME: Also get protocol stats QUIC_PARAM_CONN_STATISTICS
    ...
    // Under quic receiving a datagram never fails if a datagram is flagged as
    // available
    nlwarning("QUIC datagram available but receiving datagram failed, this is "
              "a bug.");
    return false;
  }
  ...
  if (m_Connection.receive((uint8 *)_ReceiveBuffer, len, false)) {
    ...
    if (_RecordIncomingMessagesOn) {
      ...
      _RecordedMessagesOut.serialMemStream(
          msgin); // shouldn't msgin's bufpos be resetted?
    }
#endif
    return true;
  } else {
    // Try the next address if we're still logging in
    bool tryNextAddress = (_ConnectionState == Login) &&
                          ((_FrontendHostIndex + 1) < _FrontendHost.size());

    // A receiving error means the front-end is down
    _ConnectionState = Disconnect;
    disconnect(); // won't send a disconnection msg because state is already
                  // Disconnect
    if (tryNextAddress) {
      _ConnectionState = Login;
...
void CNetworkConnection::sendSystemLogin() {
  ...
  try {
...
#ifdef NL_OS_WINDOWS
    // An exception (10004: Blocking operation interrupted) may occur if a
    // firewall such as Kerio is running (note: ZoneAlarm blocks connect() until
    // a decision is made by the user). Handle true network errors with a
    // nlerror dialog box
...
#endif
    // The first time, display info for the user to configure his personal
    // firewall
    ...
    // nldebug( "Attempt interrupted at %u ms",
    // (uint32)(CTime::getLocalTime()-attemptStartTime) );
    TTime currentTime = CTime::getLocalTime();
    if (currentTime - attemptStartTime >
        15000) // let 15 seconds for the user to allow the connection
    {
      nldebug("Login failed at %u ms",
              (uint32)(CTime::getLocalTime() - attemptStartTime));
  ...
  statsSend(length);
  nlinfo("CNET[%p]: sent LOGIN cookie=%s", this,
         _LoginCookie.toString().c_str());
  // nlinfo( "CNET[%p]: sent LOGIN cookie=%s at attempt %u at %u ms", this,
  // _LoginCookie.toString().c_str(), nbAttempts,
  // (uint32)(CTime::getLocalTime()-attemptStartTime) );
}
...
bool CNetworkConnection::stateLogin() {
...
#else
  while (m_UseQuic ? m_QuicConnection.datagramAvailable()
                   : m_Connection.dataAvailable()) // && _TotalMessages<5)
#endif
  {
    ...
    if (buildStream(msgin) && decodeHeader(msgin)) {
      if (_SystemMode) {
        ...
        default:
          ...
          nlwarning("CNET[%p]: received system %d in state Login", this,
                    message);
          break;
        }
      } else {
  ...
  if (_UpdateTime - _LatestLoginTime > 300) {
    ...
    if (m_LoginAttempts > 24) {
      ...
      disconnect(); // will send disconnection message
      nlwarning(
          "CNET[%p]: Too many LOGIN attempts, connection problem (using %s)",
          this, m_UseQuic ? "QUIC" : "UDP");
      return false; // exit now from loop, don't expect a new state
    } else
#endif
    {
...
void CNetworkConnection::receiveSystemSync(CBitMemStream &msgin) {
  ...
  if (CheckXMLSignature.get()) {
    ...
    try {
      ...
      // Since cannot now easily if the server run on Windows or unix, try the
      // both methods
      xmlInvalid =
          (checkMsgXml != _MsgXmlMD5 || checkDatabaseXml != _DatabaseXmlMD5);
      if (xmlInvalid)
        xmlInvalid = (checkMsgXml != _AltMsgXmlMD5 ||
                      checkDatabaseXml != _AltDatabaseXmlMD5);
    ...
    if (xmlInvalid && !alreadyWarned) {
      alreadyWarned = true;
      Driver->systemMessageBox("msg.xml and database.xml files are invalid "
                               "(server version signature is different)",
                               "XML files invalid");

      nlwarning("XML signature is invalid:");
      nlwarning("msg.xml client:%s,%s server:%s",
                _AltMsgXmlMD5.toString().c_str(), _MsgXmlMD5.toString().c_str(),
                checkMsgXml.toString().c_str());
      nlwarning("database.xml client:%s,%s server:%s",
                _AltDatabaseXmlMD5.toString().c_str(),
                _DatabaseXmlMD5.toString().c_str(),
...
#endif
  _CurrentClientTick =
      uint32(_CurrentServerTick - (_LCT + _MsPerTick) / _MsPerTick);
  _CurrentClientTime = _UpdateTime - (_LCT + _MsPerTick);

  // nlinfo( "CNET[%p]: received SYNC %" NL_I64 "u %" NL_I64 "u -
  // _CurrentReceivedNumber=%d _CurrentServerTick=%d", this,
  // (uint64)_Synchronize, (uint64)stime, _CurrentReceivedNumber,
  // _CurrentServerTick );
...
void CNetworkConnection::sendSystemAckSync() {
  ...
  // nlinfo("CNET[%p]: sent ACK_SYNC, _LastReceivedNumber=%d
  // _LastAckInLongAck=%d", this, _LastReceivedNumber, _LastAckInLongAck);
    ...
    if (buildStream(msgin) && decodeHeader(msgin)) {
      if (_SystemMode) {
        ...
        case SYSTEM_SERVER_DOWN_CODE:
          ...
          break;
        default:
          nlwarning("CNET[%p]: received system %d in state Synchronize", this,
                    message);
          break;
        }
      } else {
...
void CNetworkConnection::receiveNormalMessage(CBitMemStream &msgin) {
  // nlinfo("CNET[%p]: received normal message Packet=%d Ack=%d", this,
  // _LastReceivedNumber, _LastReceivedAck);

  vector<CAction *> actions;
  _ImpulseDecoder.decode(msgin, _CurrentReceivedNumber, _LastReceivedAck,
                         _CurrentSendNumber, actions);
#ifdef SHOW_PROPERTIES_RECEIVED
  ...
  while (!_Actions.empty() && _Actions.front().FirstPacket != 0 &&
         _Actions.front().FirstPacket <= _LastReceivedAck) {
  ...
  CActionFactory::getInstance()->unpack (msgin, commonActions,
  getCurrentServerTick()+1 ); // +1 because the current tick is set a few lines
  further
...
#endif
    // nldebug( "_CurrentServerTick=%d _CurrentReceivedNumber=%d
    // _Synchronize=%d", _CurrentServerTick, _CurrentReceivedNumber,
    // _Synchronize );
  }

  // remove useless stamps in queue
  while (!_PacketStamps.empty() &&
         _LastReceivedAck > _PacketStamps.front().first)
    _PacketStamps.pop_front();

#ifdef ENABLE_INCOMING_MSG_RECORDER
  if ((!_ReplayIncomingMessagesOn) &&
      (_PacketStamps.empty() || _PacketStamps.front().first > _LastReceivedAck))
#else
  if (_PacketStamps.empty() || _PacketStamps.front().first > _LastReceivedAck)
#endif
  {
    // nlwarning("Frontend ack'ed message %d not stamp dated",
    // _LastReceivedAck);
  } else {
    ...
    // earliest estimation of server packet send time and latest estimation
    // (based on ping and acknowledge)
    TTime earliest = ackedPacketTime + _BestPing / 2,
          latest = _UpdateTime - _BestPing / 2;

    // compute number of ticks between frame currently played by client and
    // packet received from server
    sint32 numStepTick = (sint32)(_CurrentServerTick - _CurrentClientTick);

    // if enough steps and times are valid
    if (numStepTick > 0 && earliest > _CurrentClientTime &&
        latest > _CurrentClientTime) {
      // exact formula for _MsPerTick =
      // (_CurrentServerTime-_CurrentClientTime)/numStepTick where
      // _CurrentServerTime is the actual server packet send time but as the
      // exact time is unknown, we use a predictive time window instead, based
      // on the acknwoledged packet send time, the received packet time and the
      // ping
      ...
      /// \todo we should instead of putting 1, returning in probe mode because
      /// it means that we had a very big lag
      if (_MsPerTick == 0) {
        nlwarning("_MsPerTick is 0 because server tick is too big %d compare "
                  "to the client tick is %d",
                  _CurrentServerTick, _CurrentClientTick);
        setMsPerTick(1);
        //_MsPerTick = 1;
      }
  ...
  uint sum = propReceived[4] + propReceived[5] + propReceived[6] +
             propReceived[7] + propReceived[8] + propReceived[9];
  if (sum != 0)
  ...
  str += "TOTAL: " + NLMISC::toString(propReceived[2]) + " + " +
         NLMISC::toString(propReceived[0] + propReceived[3] + sum);
  // ss << "TOTAL: " << propReceived[2] << " + " << propReceived[0] +
  // propReceived[3] + sum;
...
void CNetworkConnection::decodeVisualProperties(CBitMemStream &msgin) {
  try {
    ...
    while (true) {
      // nlinfo( "Reading pass %u, BEFORE HEADER: pos: %d  len: %u", ++i,
      // msgin.getPosInBit(), msgin.length() * 8 );
      ...
      if (associationBitsHaveChanged(slot, associationBits) &&
          (!IgnoreEntityDbUpdates || slot == 0)) {
        // displayBitStream( msgin, beginbitpos, msgin.getPosInBit() );
        //			   nlinfo ("Disassociating S%hu (AB %u)",
        //(uint16)slot, associationBits );
        if (_PropertyDecoder.isUsed(slot)) {
          ...
          //					nlinfo( "Cannot disassociate
          //slot %hu: sheet not received yet", (uint16)slot );
        }
      }

      // Read the timestamp delta if there's one (otherwise take
      // _CurrentServerTick)
      ...
      // Tree
      // nlinfo( "AFTER HEADER: posBit: %d pos: %d  len: %u",
      // msgin.getPosInBit(), msgin.getPos(), msgin.length() );
      ...
      if (currentNode->a()->BranchHasPayload) {
        CActionPosition *ap =
            (CActionPosition *)CActionFactory::getInstance()->create(
                slot, ACTION_POSITION_CODE);
        ap->unpack(msgin);
        ...
        if (_DataBase != NULL && (!IgnoreEntityDbUpdates || slot == 0)) {
          ...
          nodeRoot =
              dynamic_cast<CCDBNodeBranch *>(_DataBase->getNode((uint16)0));
          if (nodeRoot) {
            CCDBNodeLeaf *node;
            node = dynamic_cast<CCDBNodeLeaf *>(
                nodeRoot->getNode(slot)->getNode(0));
            nlassert(node != NULL);
            node->setValue64(ap->Position[0]);
            node = dynamic_cast<CCDBNodeLeaf *>(
                nodeRoot->getNode(slot)->getNode(1));
            nlassert(node != NULL);
            node->setValue64(ap->Position[1]);
            node = dynamic_cast<CCDBNodeLeaf *>(
                nodeRoot->getNode(slot)->getNode(2));
            nlassert(node != NULL);
            node->setValue64(ap->Position[2]);

            if (LoggingMode) {
              nlinfo("recvd position (%d,%d) for slot %hu, date %u",
                     (sint32)(ap->Position[0]), (sint32)(ap->Position[1]),
                     (uint16)slot, timestamp);
            }
          }
        }
        ...
        /*
         * Statistical prediction of time before next position update: set
         * PredictedInterval
         */
        ...
        if (puTicks.size() == MAX_POSUPDATETICKQUEUE_SIZE) {
          puIntervals.erase(puIntervals.find(
              puTicks[1] - puTicks[0])); // erase only one element, not all
                                         // corresponding to the value
          puTicks.pop_front();
        }

        // Add a new element to the tick queue and possibly to the interval
        // sorted set Still to choose: _CurrentServerTick or timestamp ?
        ...
        if (puIntervals.empty()) {
          ...
          predictedInterval =
              (TGameCycle)(percentileRev(puIntervals,
                                         PREDICTION_REV_PERCENTILE) +
                           1);

          // if ( predictedInterval > 100 )
          //	nlwarning( "Slot %hu: Predicted interval %u exceeds 100 ticks",
          //(uint16)slot, predictedInterval );

          if (PosUpdatePredictionGraph)
            PosUpdatePredictionGraph->addOneValue(slot,
                                                  (float)predictedInterval);
        }

        // nlinfo( "Slot %hu: Interval=%u Predicted=%u", (uint16)slot,
        // latestInterval, predictedInterval );
...
#ifdef MEASURE_RECEIVE_DATES
        // Stat log
        if (LogReceiveEnabled && (WatchedEntitySlot == 256) ||
      ...
      if (currentNode->BranchHasPayload) {
        ...
        if (currentNode->a()->BranchHasPayload) {
          CActionSint64 *ac =
              (CActionSint64 *)CActionFactory::getInstance()->createByPropIndex(
                  slot, PROPERTY_ORIENTATION);
          ac->unpack(msgin);
          ...
          if (_DataBase != NULL && (!IgnoreEntityDbUpdates || slot == 0)) {
            ...
            if (nodeRoot) {
              CCDBNodeLeaf *node = dynamic_cast<CCDBNodeLeaf *>(
                  nodeRoot->getNode(slot)->getNode(PROPERTY_ORIENTATION));
              nlassert(node != NULL);
              node->setValue64(ac->getValue());
              if (LoggingMode) {
                nlinfo("CLIENT: recvd property %hu (%s) for slot %hu, date %u",
                       (uint16)PROPERTY_ORIENTATION,
                       getPropText(PROPERTY_ORIENTATION), (uint16)slot,
                       timestamp);
              }
              // nldebug("CLPROPNET[%p]: received property %d for entity %d: %"
              // NL_I64 "u", this, action->PropIndex, action->CLEntityId,
              // action->getValue());
            }
          }
...
void CNetworkConnection::decodeDiscreetProperty(CBitMemStream &msgin,
                                                TPropIndex propIndex) {
  // nldebug( "Reading discreet property %hu at bitpos %d", (uint16)propIndex,
  // msgin.getPosInBit() );
  ...
  if (propIndex == PROPERTY_TARGET_LIST) {
    ...
    if (_DataBase != NULL && (!IgnoreEntityDbUpdates || slot == 0)) {
      ...
      nodeRoot = dynamic_cast<CCDBNodeBranch *>(_DataBase->getNode(0));
      if (nodeRoot) {
        CCDBNodeBranch *nodeEntity =
            dynamic_cast<CCDBNodeBranch *>(nodeRoot->getNode(slot));
        ...
        for (i = 0; i < listSize; ++i) {
          ...
          if (place == 8) {
            nodeProp =
                dynamic_cast<CCDBNodeLeaf *>(nodeEntity->getNode(writeProp));
      ...
      if (LoggingMode) {
        nlinfo("CLIENT: recvd property %hu (%s) for slot %hu, date %u",
               (uint16)propIndex, getPropText(propIndex), (uint16)slot,
               TVPNodeClient::SlotContext.Timestamp);
      }
    }
  ...
  case PROPERTY_SHEET: {
    ...
    thechange.NewEntityInfo.DataSetIndex =
        (TClientDataSetIndex)((ac->getValue() >> 32) & 0xffffffff);
  ...
  case PROPERTY_MODE: {
    ...
    if (_DataBase != NULL && (!IgnoreEntityDbUpdates || slot == 0)) {
      ...
      if (nodeRoot) {
        CCDBNodeLeaf *node = dynamic_cast<CCDBNodeLeaf *>(
            nodeRoot->getNode(slot)->getNode(propIndex));
        nlassert(node != NULL);
    ...
    // Set the position or orientation received along with the mode in the
    // database
    ...
    if (modeEnum == MBEHAV::COMBAT_FLOAT) {
      ...
      // Set 2D position (the position at TVPNodeClient::SlotContext.Timestamp
      // is not sent at the same time as the position for Mode)
      if (_DataBase != NULL && (!IgnoreEntityDbUpdates || slot == 0)) {
        uint16 x16 = (uint16)((ac->getValue() >> 12) & 0xFFFF);
        uint16 y16 = (uint16)((ac->getValue() >> 28) & 0xFFFF);
        if (!(x16 == 0 &&
              y16 == 0)) // don't set the position if it was not initialized yet
        {
          sint32 x, y;
          ...
          nldebug("%u: S%hu: Received mode with null pos", _CurrentServerTick,
                  (uint16)slot); // TEMP
        }
      }
    }
    ...
    */
  default: {
    // special for Bars, always take _CurrentServerTick timestamp (delta
    // timestamp decoded is mainly for position purpose...)
    NLMISC::TGameCycle timeStamp = TVPNodeClient::SlotContext.Timestamp;
    /* YOYO: i don't know what to do with others property that really use the
       gamecycle and are maybe buggued:
            ENTITY_MOUNTED_ID,RIDER_ENTITY_ID,BEHAVIOUR,TARGET_LIST,TARGET_ID,VISUAL_FX
            But bars timestamp accuracy is very important (else could take DB
       property with falsly newer timestamp)
    */
    ...
    if (_DataBase != NULL && (!IgnoreEntityDbUpdates || slot == 0)) {
      ...
      if (nodeRoot) {
        ...
        // nldebug("CLPROPNET[%p]: received property %d for entity %d: %" NL_I64
        // "u", this, action->PropIndex, action->CLEntityId,
        // action->getValue());
      }
    }
  }
  ...
  //		ss << propReceived[3] << " orient; ";
  //	uint sum = propReceived[4] + propReceived[5] + propReceived[6] +
  //propReceived[7] + propReceived[8] + propReceived[9]; 	if ( sum != 0 ) 		ss <<
  //sum << " discreet; "; 	if ( propReceived[16] != 0 ) 		ss << propReceived[16] <<
  //"assoc; "; 	if ( propReceived[17] != 0 ) 		ss << propReceived[17] << "disac; ";
  //	ss << "TOTAL: " << propReceived[2] << " + " << propReceived[0] +
  //propReceived[3] + sum;
...
void CNetworkConnection::sendNormalMessage() {
  // nlinfo("CNET[%p]: send normal message Packet=%d Ack=%d AckBits=%08X", this,
  // _CurrentSendNumber, _LastReceivedNumber, _AckBitMask);
  ...
  for (itblock = _Actions.begin(); itblock != _Actions.end(); ++itblock) {
    ...
    // if (message.getPosInBit() + (*itblock).bitSize() >
    // FrontEndInputBufferSize) // hard version 	break;

    if (block.FirstPacket == 0)
      block.FirstPacket = _CurrentSendNumber;

    // nlassertex((*itblock).Cycle > lastPackedCycle, ("(*itblock).Cycle=%d
    // lastPackedCycle=%d", (*itblock).Cycle, lastPackedCycle));
    ...
    // nldebug("CNET[%p]: packed block %d, message is currently %d bits long",
    // this, block.Cycle, message.getPosInBit());

    // Prevent to send a message too big
    // if (message.getPosInBit() + (*itblock).bitSize() >
    // FrontEndInputBufferSize) // hard version
...
bool CNetworkConnection::stateConnected() {
...
#endif
  {
    ...
    previousTime = now;
    if ((diff > 3000) && (!(m_UseQuic ? m_QuicConnection.datagramAvailable()
                                      : m_Connection.dataAvailable()))) {
      return false;
    }

    // update the current time;
    while (_CurrentClientTime < (TTime)(_UpdateTime - _MsPerTick - _LCT) &&
           _CurrentClientTick < _CurrentServerTick) {
    ...
    if (_CurrentClientTick >= _CurrentServerTick &&
        !(m_UseQuic ? m_QuicConnection.datagramAvailable()
                    : m_Connection.dataAvailable())) {
      return false;
    }
  }
    ...
    if (buildStream(msgin) && decodeHeader(msgin)) {
      if (_SystemMode) {
        ...
        case SYSTEM_SERVER_DOWN_CODE:
          ...
          break;
        default:
          nlwarning("CNET[%p]: received system %d in state Connected", this,
                    message);
          break;
        }
      } else {
...
bool CNetworkConnection::decodeHeader(CBitMemStream &msgin,
  ...
  if ((sint)_CurrentReceivedNumber > (sint)_LastReceivedPacketInBothModes &&
      checkMessageNumber) {
    _TotalLostPackets +=
        (_CurrentReceivedNumber - _LastReceivedPacketInBothModes - 1);
  ...
  /// \todo remove
  // nlinfo("DBG:BEN: decodeHeader, packet=%d, %s", _CurrentReceivedNumber,
  // _SystemMode ? "SYSTEM" : "NORMAL");
  ///
  ...
  if (_SystemMode) {
    ...
    uint32 UserLWatch, CycleWatch, ReceiveWatch, SendWatch;
    ...
    --counter;
    if (counter == 0) {
      //			nlinfo( "User:%u Cycle:%u Rcv:%u Snd:%u
      //PrioAmount:%.2f", 					UserLWatch, CycleWatch, ReceiveWatch, SendWatch,
      //PriorityAmount );
      counter = 128;
    }
  ...
  // nlinfo("CNET[%p] received packet %d, %s mode - LastReceivedAck=%d", this,
  // _CurrentReceivedNumber, _SystemMode ? "SYTEM" : "NORMAL",
  // _LastReceivedAck);
  ...
  if (_CurrentReceivedNumber > _LastReceivedNumber + 1) {
    ...
    nldebug("CNET[%p] lost messages server->client [%d; %d]", this,
            _LastReceivedNumber + 1, _CurrentReceivedNumber - 1);
    _MeanLoss.update((float)(_CurrentReceivedNumber - _LastReceivedNumber - 1),
                     ryzomGetLocalTime());
  } else if (_CurrentReceivedNumber == _LastReceivedNumber) {
    // we receive the same packet that the last one
    nldebug("CNET[%p] awaiting packet %d, received packet %d", this,
            _LastReceivedNumber + 1, _CurrentReceivedNumber);
    return false;
  } else if (_CurrentReceivedNumber < _LastReceivedNumber) {
    // it's an older message than the current
    nldebug("CNET[%p] received an old message, awaiting packet %d, received "
            "packet %d",
            this, _LastReceivedNumber + 1, _CurrentReceivedNumber);
    return false;
  }

  // don't acknowledge system messages and normal messages in
  // because this will disturb impulsion from frontend, that will interpret it
  // as if previous messages were ok
  bool ackBool = (!_SystemMode && (_ConnectionState == Connected ||
                                   _ConnectionState == Synchronize));
  ...
  if (_CurrentReceivedNumber - _LastReceivedNumber < 32) {
    ...
    _AckBitMask |= _LastAckBit
                   << (_CurrentReceivedNumber - _LastReceivedNumber - 1);
  } else {
    _AckBitMask =
        (_CurrentReceivedNumber - _LastReceivedNumber == 32 && _LastAckBit != 0)
            ? 0x80000000
            : 0x00000000;
  }
  ...
  _LongAckBitField.set(_CurrentReceivedNumber & (NumBitsInLongAck - 1),
                       ackBool);

  // no more than NumBitsInLongAck ack in field
  if ((TPacketNumber)_LastAckInLongAck <=
      (TPacketNumber)(_CurrentReceivedNumber - NumBitsInLongAck))
...
void CNetworkConnection::push(CBitMemStream &msg) {
  ...
  CActionGeneric *ag = (CActionGeneric *)CActionFactory::getInstance()->create(
      INVALID_SLOT, ACTION_GENERIC_CODE);
  ...
  if (impulseBitSize < maxImpulseBitSize) {
    ...
    // MultiPart impulsion
    CActionGenericMultiPart *agmp =
        (CActionGenericMultiPart *)CActionFactory::getInstance()->create(
            INVALID_SLOT, ACTION_GENERIC_MULTI_PART_CODE);
    sint32 minimumBitSizeForMP = CActionFactory::getInstance()->size(agmp);

    sint32 availableSize =
        (maxImpulseBitSize - minimumBitSizeForMP) / 8; // (in bytes)
#ifdef NL_DEBUG
    nlassert(availableSize >
    ...
    for (sint32 i = 0; i < nbBlock; i++) {
      if (i != 0)
        agmp = (CActionGenericMultiPart *)CActionFactory::getInstance()->create(
            INVALID_SLOT, ACTION_GENERIC_MULTI_PART_CODE);

      agmp->set(num, (uint16)i, msg.buffer(), bytelen, availableSize,
                (uint16)nbBlock);
      push(agmp);
    }
  }
...
void CNetworkConnection::pushTarget(TCLEntityId slot,
                                    LHSTATE::TLHState targetOrPickup) {
  CActionTargetSlot *ats =
      (CActionTargetSlot *)CActionFactory::getInstance()->create(
          INVALID_SLOT, ACTION_TARGET_SLOT_CODE);
  nlassert(ats != NULL);
  ats->Slot = slot;
...
void CNetworkConnection::push(CAction *action) {
  if (_Actions.empty() || _Actions.back().Cycle != 0) {
    // nlinfo("-BEEN- push back 2 [size=%d, cycle=%d]", _Actions.size(),
    // _Actions.empty() ? 0 : _Actions.back().Cycle);
...
void CNetworkConnection::send(TGameCycle cycle) {
  try {
    ...
    nlassertex(_LastSentCycle < cycle,
    ...
    */
    ...
    if (_Actions.empty() || _Actions.back().Cycle != 0) {
      //		nlinfo("-BEEN- push back 1 [size=%d, cycle=%d]",
      //_Actions.size(), _Actions.empty() ? 0 : _Actions.back().Cycle);
      //		_Actions.push_back();
    } else {
      CActionBlock &block = _Actions.back();
      /*
                              CAction	*dummy =
         CActionFactory::getInstance()->create(INVALID_SLOT, ACTION_DUMMY_CODE);
      ...
      */
      ...
      if (i < block.Actions.size()) {
        nldebug("Postponing %u actions exceeding max size in block %d (block "
                "size is %d bits long)",
                block.Actions.size() - i, cycle, bitSize);
    ...
    _ConnectionState = Disconnect;
    disconnect(); // won't send disconnection message as state is already
                  // Disconnect
  }
}
...
void CNetworkConnection::send() {
  try {
    ...
    // This way, we can say that at most 15 packets will be delivered each
    // second (5 send(tick), and 10 send() -- if you take getLocalTime()
    // inaccuracy into account)
    if (_ConnectionState == Connected &&
        CTime::getLocalTime() - _LastSendTime > 100) {
      sendNormalMessage();
    }
  } catch (const ESocket & /*e*/) {
    _ConnectionState = Disconnect;
...
void CNetworkConnection::disconnect() {
  ...
  if (_ConnectionState == NotInitialised || _ConnectionState == NotConnected ||
      _ConnectionState == Authenticate || _ConnectionState == Disconnect) {
    // nlwarning("Unable to disconnect(): not connected yet, or already
    // disconnected.");
        ...
        case SYSTEM_SYNC_CODE:
          ...
          /*
                                          case SYSTEM_PROBE_CODE:
                                                  ...
                                                  _ConnectionState = Probe;
                                                  nldebug("CNET[%p]:
             stalled->probe", this); receiveSystemProbe(msgin); break; case
             SYSTEM_STALLED_CODE:
                                                  // receive stalled, decode
             stalled _ConnectionState = Stalled; receiveSystemStalled(msgin);
                                                  return true;
                                                  break;
          */
        case SYSTEM_SERVER_DOWN_CODE:
...
void CNetworkConnection::displayAllocationStats() {
  nlinfo("CNET[%p]: %d queued blocks, %d changes", this, _Actions.size(),
         _Changes.size());
}

string CNetworkConnection::getAllocationStats() {
  char buf[128];
  sprintf(buf, "CNET[%p]: %u queued blocks, %u changes", this,
          (uint)_Actions.size(), (uint)_Changes.size());
  return string(buf);
}
...
void CNetworkConnection::genericAction(CActionGeneric *ag) {
  ...
  // nldebug("CNET: Calling impulsion callback (size %u) :'%s'", this,
  // bms.length(), toHexaString(bms.bufferAsVector()).c_str());
  // nldebug("CNET[%p]: Calling impulsion callback (size %u)", this,
  // bms.length());
...
void CNetworkConnection::CGenericMultiPartTemp::set(
    CActionGenericMultiPart *agmp, CNetworkConnection *parent) {
  ...
  if (NbCurrentBlock == NbBlock) {
    ...
    // nldebug("CLMPNET[%p]: Received a TOTAL generic action MP size: number %d
    // nbblock %d", this,  agmp->Number, NbBlock);
    ...
    // nldebug("CLMPNET[%p]: Received a generic action size %d", this,
    // bms.length());
    //  todo interface api, call a user callback

    if (parent->_ImpulseCallback != NULL)
      parent->_ImpulseCallback(bms, parent->_LastReceivedNumber,
                               parent->_ImpulseArg);
  }
}
...
void CNetworkConnection::genericAction(CActionGenericMultiPart *agmp) {
  ...
  // nldebug("CLMPNET[%p]: Received a generic action MP size %d: number %d part
  // %d nbblock %d", this, v.size(), agmp->Number, agmp->Part, agmp->NbBlock);
...
CNetworkConnection::TVPNodeClient::TSlotContext
    CNetworkConnection::TVPNodeClient::SlotContext;

/*
 * Return the average billed upload rate in kbps, including all headers
 * (UDP+IP+Ethernet)
 */
...
/*
 * Return the average billed download rate in kbps, including all headers
 * (UDP+IP+Ethernet)
 */
...
NLMISC_COMMAND(displayPosUpdateGraph, "Display position update interval graph",
               "0|<slot>") {
  ...
  if ((args.size() != 0) && (args[0] != "0")) {
    ...
    PosUpdateIntervalGraph =
        new CSlotGraph("Interval", 350, 2, 100, 200, CRGBA(128, 0, 0, 64), 1000,
                       20, true, slot);
    PosUpdatePredictionGraph =
        new CSlotGraph("                    Prediction", 350, 2, 100, 200,
                       CRGBA(0, 0, 128, 64), 1000, 20, true, slot);
  }
  return true;
}
...
// ***************************************************************************
sint64
CNetworkConnection::convertToSmoothServerTick(NLMISC::TGameCycle t) const {
...
void CNetworkConnection::updateSmoothServerTick() {
  ...
  if (_CurrentSmoothServerTick < actualST + SMOOTH_SERVER_TICK_DIFF_MIN) {
  ...
  } else if (_CurrentSmoothServerTick >=
             actualST + SMOOTH_SERVER_TICK_DIFF_MAX) {
    // Clamp (no reset, no back to past!)
    _CurrentSmoothServerTick = actualST + SMOOTH_SERVER_TICK_DIFF_MAX;
  } else {
    // Compute the Factor of acceleration according to error difference (FIXED
    // 16:16)
    sint64 factor;
    sint64 errorDiff = _CurrentSmoothServerTick - actualST;
    // If the estimation is in the TimeWindow, no acceleration
    if (errorDiff >= -SMOOTH_SERVER_TICK_WINDOW &&
        errorDiff <= SMOOTH_SERVER_TICK_WINDOW) {
      factor = 65536;
    }
    // If the estimation is late, accelerate
    else if (errorDiff < 0) {
      float f = (float)(errorDiff + SMOOTH_SERVER_TICK_WINDOW) /
                (SMOOTH_SERVER_TICK_DIFF_MIN + SMOOTH_SERVER_TICK_WINDOW);
    ...
    // Else the estimation is too early, slowDown
      ...
      float f = (float)(errorDiff - SMOOTH_SERVER_TICK_WINDOW) /
                (SMOOTH_SERVER_TICK_DIFF_MAX - SMOOTH_SERVER_TICK_WINDOW);
      f = 1 - f;
      factor = sint64(f * 65536);
    }
    ...
    _CurrentSmoothServerTick +=
        ((deltaT * SMOOTH_SERVER_TICK_PER_TICK * factor) / getMsPerTick()) >>
        16;
  }
}
