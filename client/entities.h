...
public:
  enum TFlag {
    ...
    NonPlayer = 1 << 6, // ok if entity is a non-player (ie a bot, whaterver Npc
                        // fauna etc...)
...
public:
  CShapeInstanceReference(NL3D::UInstance instance, const string &text,
                          const string &url, bool bbox_active = true) {
    Instance = instance;
    ContextText = text;
    ContextURL = url;
...
private:
  ...
  NLMISC::CRefPtr<NLMISC::CCDBNodeLeaf>
      _MissionTargetTitleDB[MAX_NUM_MISSIONS][MAX_NUM_MISSION_TARGETS];
  NLMISC::CRefPtr<NLMISC::CCDBNodeLeaf>
      _GroupMemberUidDB[8]; // MaxNumPeopleInTeam in people_interaction.h
  NLMISC::CRefPtr<NLMISC::CCDBNodeLeaf>
      _GroupMemberNameDB[8]; // MaxNumPeopleInTeam in people_interaction.h
...
public:
  ...
  CShapeInstanceReference createInstance(const string &shape,
                                         const CVector &pos, const string &text,
                                         const string &url, bool haveCollisions,
                                         sint32 &idx);
  ...
  bool setupInstance(uint32 idx, const std::vector<std::string> &keys,
   ...
   */
  CEntityCL *create(uint slot, uint32 form,
   ...
   */
  bool entitiesNearDoors(float openingDist, const NLMISC::CVector &posDoor1,
                         const NLMISC::CVector &posDoor2);

  /**
   * Get the entity under the (2d) position. Return NULL if no entity under this
   *position. NB: the UserEntity can NOT be returned. But if the code has find
   *it, isPlayerUnderCursor is set to true NB: somewhat slow. should be called
   *ONLY ONCE per frame. NB: unselectable entities are not returned
   */
  CEntityCL *getEntityUnderPos(float x, float y, float distSelection,
                               bool &isPlayerUnderCursor);

  /**
   * Get the entity (not user) in the camera. Return NULL if not entity under
   *this position. \param flags a ORed of CEntityFilterFlag::TFlag \param
   *distSelection don't go beyond \param precEntity is used for Cyclic
   *management. If found in the select list, get the next one, else get the
   *first.
   */
  CEntityCL *getEntityInCamera(uint flags, float distSelection,
   ...
   */
  CEntityCL *getEntityByName(const std::string &name, bool caseSensitive,
                             bool complete) const;
  /**
   ...
   */
  CEntityCL *getEntityByKeywords(const std::vector<std::string> &keywords,
  ...
  void updatePostCamera(uint clippedUpdateTime,
                        const std::vector<NLMISC::CPlane> &clippingPlanes,
  ...
  void updateVisualProperty(const NLMISC::TGameCycle &gameCycle,
                            const uint &slot, const uint &prop,
  ...
  /// Return the reference for all entities in the vision. An entity object, for
  /// a slot, is not allocated if not visible.
   ...
   * who isFriend(), perform releaseInSceneInterfaces() and
   * buildInSceneInterface(). Otherwise, do nothing.
   */
  void refreshInsceneInterfaceOfFriendNPC(uint slot);

  inline NLMISC::CCDBNodeLeaf *getMissionTargetTitleDB(int mission,
                                                       int target) {
    return _MissionTargetTitleDB[mission][target];
  }
  inline NLMISC::CCDBNodeLeaf *getGroupMemberUidDB(int member) {
    return _GroupMemberUidDB[member];
  }
  inline NLMISC::CCDBNodeLeaf *getGroupMemberNameDB(int member) {
    return _GroupMemberNameDB[member];
  }
  inline NLMISC::CCDBNodeLeaf *getBeastUidDB(int beast) {
    return _BeastUidDB[beast];
  }
  inline NLMISC::CCDBNodeLeaf *getBeastStatusDB(int beast) {
    return _BeastStatusDB[beast];
  }
  inline NLMISC::CCDBNodeLeaf *getBeastTypeDB(int beast) {
    return _BeastTypeDB[beast];
  }

private:
  // NB: don't return unselectable entities
  void getEntityListForSelection(std::vector<CEntityCL *> &entities,
                                 uint flags);

  void updateEntitiesIsInTeam();

  /// Log entities Property Stages Change
  void logPropertyChange(CLFECOMMON::TCLEntityId who, const CStage &oldStage,
                         const CStage &newStage,
                         const NLMISC::CVectorD &precOldPos,
                         const NLMISC::CVectorD &precNewPos,
                         sint32 relGameCycle, sint64 relLocalTime);
};
