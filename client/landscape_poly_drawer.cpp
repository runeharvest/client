...
CLandscapePolyDrawer::CLandscapePolyDrawer() {
  // set ILandscapePolyDrawingCallback attribute of current scene to modify
  // stencil operation from NEL classes with callbacks.
...
CLandscapePolyDrawer::~CLandscapePolyDrawer() { delete _InitStencil; }
...
void CLandscapePolyDrawer::beginRenderLandscapePolyPart() {
  ...
  // the eighth bit will be written with a 0 during next render to mark stencil
  // buffer parts which will support Shadow Volume algorithm (landscape). When
  // stencil operation will be modify with "replace" operation, the eighth bit
  // will be written with a 1 (vegeation, shadow...)
  Driver->stencilOp(UDriver::keep, UDriver::keep, UDriver::zero);
}
...
void CLandscapePolyDrawer::setScissor(uint polyId) {
  ...
  vertices[0] = CVector(center.x - halfSize.x, center.y + halfSize.y,
                        center.z + halfSize.z);
  vertices[1] = CVector(center.x + halfSize.x, center.y + halfSize.y,
                        center.z + halfSize.z);
  vertices[2] = CVector(center.x + halfSize.x, center.y - halfSize.y,
                        center.z + halfSize.z);
  vertices[3] = CVector(center.x - halfSize.x, center.y - halfSize.y,
                        center.z + halfSize.z);
  vertices[4] = CVector(center.x - halfSize.x, center.y + halfSize.y,
                        center.z - halfSize.z);
  vertices[5] = CVector(center.x + halfSize.x, center.y + halfSize.y,
                        center.z - halfSize.z);
  vertices[6] = CVector(center.x + halfSize.x, center.y - halfSize.y,
                        center.z - halfSize.z);
  vertices[7] = CVector(center.x - halfSize.x, center.y - halfSize.y,
                        center.z - halfSize.z);
  ...
  // for each face, we research its intersection points with "near" plane (if
  // exist)
  for (uint f = 0; f < 6; f++) {
    uint outNb = nearPlane.clipPolygonFront(faces[f], out, 4);

    // if intersection points exist, we project them in windows coordinates to
    // recover scissor rectangle
    if (outNb != 0) {
  ...
  CScissor scissor(rectMin.x, rectMin.y, rectMax.x - rectMin.x,
                   rectMax.y - rectMin.y);
...
void CLandscapePolyDrawer::renderLandscapePolyPart() {
  ...
  // The eighth bit mustn't be written because it indicates if a part of stencil
  // buffer can support shadow volume algorithm.
...
void CLandscapePolyDrawer::infiniteFrustum() {
...
void CLandscapePolyDrawer::finiteFrustum() {
  // depth range
...
void CLandscapePolyDrawer::buildShadowVolume(uint poly) {
  ...
  // vertices coordinates aren't calculated immediately but in drawShadowVolume
  // method, because they are calculated in camera location.
...
void CLandscapePolyDrawer::drawShadowVolume(uint poly, bool firstPass) {
  ...
  ((CDriverUser *)Driver)
      ->getDriver()
      ->renderTriangles(*GenericMat.getObjectPtr(), 0,
                        4 * (uint32)_Polygons[poly - 1].Vertices.size());
...
void CLandscapePolyDrawer::computeBBoxFromPolygon(
    const NLMISC::CPolygon2D &poly2D, NLMISC::CAABBox &destBBox) {
  ...
  for (it = zoneIds.begin(); it != zoneIds.end(); it++) {
    if (Landscape) {
      ...
      if (zone) {
        // For each zone, we traverse its patchs and check intersection between
        // our polygon bounding rectangle on (x, y) plane and the bounding
        // sphere patch . If intersection, we use the bounding box of the patch
        // to extend the polygon boudning box on z axis. NB : The patch bounding
        // box isn't used immediately because it's more expensive to obtain than
        // bounding sphere.
        sint numPatchs = zone->getNumPatchs();
        for (sint i = 0; i < numPatchs; i++) {
          const CBSphere &bSphere = zone->getPatchBSphere(i);

          const CVector2f sphereMax =
              CVector2f(bSphere.Center.x + bSphere.Radius,
                        bSphere.Center.y + bSphere.Radius);
          const CVector2f sphereMin =
              CVector2f(bSphere.Center.x - bSphere.Radius,
                        bSphere.Center.y - bSphere.Radius);

          // intersection beetween patch bounding sphere and polygon bounding
          // rectangle
          if (sphereMin.x < rectMax.x && sphereMax.x > rectMin.x &&
              sphereMin.y < rectMax.y && sphereMax.y > rectMin.y) {
            ...
            const CVector2f patchMax =
                CVector2f(center.x + halfSize.x, center.y + halfSize.y);
            const CVector2f patchMin =
                CVector2f(center.x - halfSize.x, center.y - halfSize.y);

            // intersection beetween patch bounding box and polygon bounding
            // rectangle
            if (patchMin.x < rectMax.x && patchMax.x > rectMin.x &&
                patchMin.y < rectMax.y && patchMax.y > rectMin.y) {
              if (firstExtend) {
                destBBox.setCenter(CVector(destBBox.getCenter().x,
                                           destBBox.getCenter().y, center.z));
                destBBox.setHalfSize(CVector(destBBox.getHalfSize().x,
                                             destBBox.getHalfSize().y,
                                             halfSize.z));
                firstExtend = false;
              } else {
