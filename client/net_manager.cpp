...
static const std::string PLAYER_EXCHANGE_INVITATION_DIALOG =
...
void impulseDatabaseInitPlayer(NLMISC::CBitMemStream &impulse) {
  try {
    ...
    BOMB(NLMISC::toString(
             "Problem while decoding a DB_INIT:PLR msg, skipped: %s", e.what()),
         return);
  }
}
...
void impulseDatabaseUpdatePlayer(NLMISC::CBitMemStream &impulse) {
  try {
    ...
    // read delta
    IngameDbMngr.readDelta(
        serverTick, impulse,
        CDBPlayer); // unlike on the server, here there is only one unified
                    // CCDBSynchronized object
  } catch (const Exception &e) {

    BOMB(NLMISC::toString(
             "Problem while decoding a DB_UPDATE_PLR msg, skipped: %s",
             e.what()),
         return);
  }
}
...
void updateInventoryFromStream(NLMISC::CBitMemStream &impulse,
                               const CInventoryCategoryTemplate *templ,
...
void impulseDatabaseUpdateBank(NLMISC::CBitMemStream &impulse) {
  ...
  try {
    ...
    // decode bank
    uint nbits;
    FILL_nbits_WITH_NB_BITS_FOR_CDBBANK impulse.serial(bank, nbits);
    ...
    if (bank == CDBGuild) {
      updateInventoryFromStream(
          impulse, (INVENTORIES::CInventoryCategoryForGuild *)NULL, false);
    }
  } catch (const Exception &e) {
    BOMB(
        NLMISC::toString(
            "Problem while decoding a DB_GROUP:UPDATE_BANK %s msg, skipped: %s",
            CDBBankNames[bank], e.what()),
        return);
  }
}
...
void impulseDatabaseInitBank(NLMISC::CBitMemStream &impulse) {
  ...
  try {
    ...
    BOMB(NLMISC::toString(
             "Problem while decoding a DB_GROUP:INIT_BANK %s msg, skipped: %s",
             CDBBankNames[bank], e.what()),
         return);
  }
}
...
void impulseDatabaseResetBank(NLMISC::CBitMemStream &impulse) {
  ...
  try {
    ...
    BOMB(NLMISC::toString(
             "Problem while decoding a DB_GROUP:RESET_BANK %s msg, skipped: %s",
             CDBBankNames[bank], e.what()),
         return);
  }
}
...
static void readPrivileges(NLMISC::CBitMemStream &impulse) {
  ...
  // nico : temporarily uses a try block here to avoid prb with people having
  // updated client and not the server
  try {
...
void impulseFarTP(NLMISC::CBitMemStream &impulse) {
  ...
  // nlinfo("impulseCallback : Received CONNECTION:FAR_TP %u",
  // sessionId.asInt());
  bool bailOutIfSessionVanished;
  impulse.serial(bailOutIfSessionVanished);
  FarTP.requestFarTPToSession(sessionId, PlayerSelectedSlot,
                              CFarTP::JoinSession, bailOutIfSessionVanished);
}

static std::string lookupSrcKeyFile(const std::string &src) {
  if (CFile::isExists("save/" + src))
    return "save/" + src;
...
void copyKeySet(const std::string &srcPath, const std::string &destPath) {
  ...
  try {
    ...
    nlwarning("Couldn't copy %s to %s to create new character keyset",
...
void impulseUserChars(NLMISC::CBitMemStream &impulse) {
  ...
  /*	CBitMemStream out;
          if(GenericMsgHeaderMngr.pushNameToStream("CONNECTION:SELECT_CHAR",
     out))
          {
                  CSelectCharMsg	SelectCharMsg;
                  SelectCharMsg.c = 0;	//TODO set here the character choosen by
     player out.serial( SelectCharMsg ); NetMngr.push(out);
                  NetMngr.send(NetMngr.getCurrentServerTick());
                  // send CONNECTION:USER_CHARS
                  nldebug("impulseCallBack : CONNECTION:SELECT_CHAR sent");
          }
          else
                  nlwarning("impulseCallBack : unknown message name :
  ...
  if (!NewKeysCharNameValidated.empty()) {
    ...
    for (uint k = 0; k < CharacterSummaries.size(); ++k) {
      if (toLower(CharacterSummaries[k].Name.toUtf8()) ==
          toLower(NewKeysCharNameValidated)) {
        // first, stripes server name
        copyKeySet(lookupSrcKeyFile(GameKeySet),
                   "save/keys_" +
                       buildPlayerNameForSaveFile(NewKeysCharNameValidated) +
                       ".xml");
        copyKeySet(lookupSrcKeyFile(RingEditorKeySet),
                   "save/keys_r2ed_" +
                       buildPlayerNameForSaveFile(NewKeysCharNameValidated) +
                       ".xml");
        break;
...
void impulseUserChar(NLMISC::CBitMemStream &impulse) {
  ...
  uint32 userRole;
  CUserCharMsg::read(impulse, posState, ServerSeasonValue, userRole,
                     IsInRingSession, HighestMainlandSessionId,
                     CharFirstConnectedTime, CharPlayedTime);
  ServerSeasonReceived = true; // set the season that will be used when
                               // selecting the continent from the position

  if (UserEntity) {
    UserEntity->pos(CVectorD((float)posState.X / 1000.0f,
                             (float)posState.Y / 1000.0f,
                             (float)posState.Z / 1000.0f));
    UserEntity->front(CVector((float)cos(posState.Heading),
                              (float)sin(posState.Heading), 0.f));
    UserEntity->dir(UserEntity->front());
    UserEntity->setHeadPitch(0);
    UserControls.resetCameraDeltaYaw();
    // nldebug("<impulseUserChar> pos : %f %f %f  heading :
    // %f",UserEntity->pos().x,UserEntity->pos().y,UserEntity->pos().z,posState.Heading);

    // Update the position for the vision.
    NetMngr.setReferencePosition(UserEntity->pos());
  } else {
    UserEntityInitPos =
        CVectorD((float)posState.X / 1000.0f, (float)posState.Y / 1000.0f,
                 (float)posState.Z / 1000.0f);
    UserEntityInitFront = CVector((float)cos(posState.Heading),
                                  (float)sin(posState.Heading), 0.f);
    // nldebug("<impulseUserChar> pos : %f %f %f  heading :
    // %f",UserEntityInitPos.x,UserEntityInitPos.y,UserEntityInitPos.z,posState.Heading);
  ...
  ClientCfg.R2EDEnabled = IsInRingSession /*&& (UserRoleInSession.getValue() !=
                                             R2::TUserRole::ur_player)*/
      ;
  // !!!Do NOT uncomment the following line  do the  ClientCfg.R2EDEnabled =
  // IsInRingSession && (UserRoleInSession != R2::TUserRole::ur_player); even
  // with UserRoleInSession R2::TUserRole::ur_player the ring features must be
  // activated because if the ring is not activated the dss do not know the
  // existence of the player So we can not kick him, tp to him, tp in to next
  // act ....
  nldebug("EnableR2Ed = %u, IsInRingSession = %u, UserRoleInSession = %u",
          (uint)ClientCfg.R2EDEnabled, (uint)IsInRingSession, userRole);
...
void impulseCharNameValid(NLMISC::CBitMemStream &impulse) {
  ...
  if (CharNameValid)
    NewKeysCharNameValidated = NewKeysCharNameWanted;
}
...
void checkHandshake(NLMISC::CBitMemStream &impulse) {
  ...
  if (itemSlotVersion != INVENTORIES::CItemSlot::getVersion())
    nlerror("Handshake: itemSlotVersion mismatch (S:%hu C:%hu)",
            itemSlotVersion, INVENTORIES::CItemSlot::getVersion());
}
...
void impulseShardId(NLMISC::CBitMemStream &impulse) {
  ...
  nlinfo("WEB: Received SHARD_ID %d, web hosted at '%s', using '%s'", shardId,
         webHost.c_str(), WebServer.c_str());
}
...
void impulsePermanentBan(NLMISC::CBitMemStream &impulse) {
  ...
  if (magicNumber != PermanentBanMSGMagicNumber)
    return; // bad msg
...
void impulsePermanentUnban(NLMISC::CBitMemStream &impulse) {
  uint64 magicNumber;
  impulse.serial(magicNumber);
  if (magicNumber != PermanentUnbanMSGMagicNumber)
    return; // bad msg
...
public:
  virtual void displayChat(TDataSetIndex compressedSenderIndex,
                           const std::string &ucstr,
                           const std::string &rawMessage,
                           CChatGroup::TGroupType mode,
                           NLMISC::CEntityId dynChatId, std::string &senderName,
                           uint bubbleTimer = 0);
  virtual void
  displayTell(/*TDataSetIndex senderIndex, */ const std::string &ucstr,
...
void CInterfaceChatDisplayer::colorizeSender(string &text,
...
void CInterfaceChatDisplayer::displayChat(
    TDataSetIndex compressedSenderIndex, const std::string &ucstr,
    const std::string &rawMessage, CChatGroup::TGroupType mode,
    NLMISC::CEntityId dynChatId, std::string &senderName, uint bubbleTimer) {
  ...
  // search a "{no_bubble}" tag
  {
    ...
    if (index != string::npos) {
      bubbleWanted = false;
      finalString = finalString.substr(0, index) +
                    finalString.substr(index + tokenSize, finalString.size());
    }
  }
  ...
  if (mode != CChatGroup::system) {
    ...
    for (;;) {
      ...
      if (index == string::npos)
        break;
      finalString = finalString.substr(0, index) +
                    finalString.substr(index + 7, finalString.size());
    }
    ...
    case CChatGroup::dyn_chat:
      entry += "DYN:" + NLMISC::toString(dbIndex);
      break;
    case CChatGroup::say:
      entry += "SAY";
      break;
  ...
  if (!stringCategory.empty()) {
    ...
    if (it != ClientCfg.SystemInfoParams.end()) {
      if (!(*it).second.SysInfoFxName.empty()) {
        NL3D::UParticleSystemInstance sysInfoFx =
            FXMngr.instantFX((*it).second.SysInfoFxName);
        if (!sysInfoFx.empty()) {
          sysInfoFx.setClusterSystem(UserEntity->getClusterSystem());
          sysInfoFx.setPos(UserEntity->pos());
        } else {
          nlwarning(
              "<CInterfaceChatDisplayer::displayChat> Can't set chat fx %s",
              (*it).second.SysInfoFxName.c_str());
        }
      }
    }
  }
  ...
  if (stringCategory != "bbl") {
    ...
    if (mode == CChatGroup::system) {
      ...
      PeopleInterraction.ChatInput.Guild.displayMessage(finalString, col, 2,
                                                        &windowVisible);
    } else if (mode == CChatGroup::team) {
      PeopleInterraction.ChatInput.Team.displayMessage(finalString, col, 2,
                                                       &windowVisible);
    } else if (mode == CChatGroup::region) {
      PeopleInterraction.ChatInput.Region.displayMessage(finalString, col, 2,
                                                         &windowVisible);
    } else if (mode == CChatGroup::universe) {
      PeopleInterraction.ChatInput.Universe.displayMessage(finalString, col, 2,
                                                           &windowVisible);
      ...
      if (dbIndex >= 0 && dbIndex < CChatGroup::MaxDynChanPerPlayer) {
        PeopleInterraction.ChatInput.DynamicChat[dbIndex].displayMessage(
            finalString, col, 2, &windowVisible);

        // Add dynchannel info before text so that the chat log will show the
        // correct string.
        CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
            "UI:SAVE:CHAT:SHOW_DYN_CHANNEL_NAME_IN_CHAT_CB", false);
        if (pIM->getLogState()) {
          ...
          // If no ] found or if found but after the colon (so part of the user
          // chat)
          ...
          finalString = finalString.substr(0, pos + 1) + prefix +
                        finalString.substr(pos + 1);

          if (node && node->getValueBool()) {
            uint32 textId = ChatMngr.getDynamicChannelNameFromDbIndex(dbIndex);
            string title;
            STRING_MANAGER::CStringManagerClient::instance()->getDynString(
                textId, title);
            prefix = (title.empty() ? "" : " ") + title;
            pos = finalString.find("] ");
            finalString =
                finalString.substr(0, pos) + prefix + finalString.substr(pos);
          }
        }
      } else {
        nlwarning("Dynamic chat %s not found for message: %s",
                  dynChatId.toString().c_str(), finalString.c_str());
      }
    } else {
      ...
      if (index != string::npos) {
        ...
        if (!senderName.empty()) {
          CEntityCL *senderEntity = EntitiesMngr.getEntityByName(
              CEntityCL::removeTitleAndShardFromName(senderName), true, true);
          if (senderEntity) {
            if (senderEntity->Type != CEntityCL::Player) {
              if (playerName.empty()) {
                ...
                CEntityCL *destEntity = EntitiesMngr.getEntityByName(
                    CEntityCL::removeTitleAndShardFromName(playerName), false,
                    true);
                if (destEntity) {
        ...
        PeopleInterraction.ChatInput.AroundMe.displayMessage(finalString, col,
                                                             2, &windowVisible);
      }
    }
    // if tell, bkup sendername
    if (mode == CChatGroup::tell && windowVisible && !senderName.empty()) {
      PeopleInterraction.LastSenderName =
          CEntityCL::removeTitleAndShardFromName(senderName);
    }
  }

  // received CHAT
  // nldebug("<impulseChat> Received CHAT : %s with category
  // %s",finalString.toString().c_str(),stringCategory.c_str());

  // **** Process chat entry for the bubbles
  // todo hulud : registering a chat callback would be better than calling this
  // hardcoded action handler
  string finalRawMessage;
  // remove color qualifier from raw string
  getStringCategory(rawMessage, finalRawMessage);
  if (bubbleWanted) {
    InSceneBubbleManager.chatOpen(compressedSenderIndex, finalRawMessage,
                                  bubbleTimer);
  }
  ...
  if (!stringCategory.empty() &&
      NLMISC::compareCaseInsensitive(stringCategory.c_str(),
                                     "SYS")) // Not empty and not 'SYS'
  {
...
void CInterfaceChatDisplayer::displayTell(
    /*TDataSetIndex senderIndex, */ const std::string &ucstr,
    const std::string &senderName) {

  string finalString = ucstr;

  // for now, '&' are removed by server so use another format	until a special
  // msg is made
  if (strFindReplace(finalString, "<R2_INVITE>", string())) {
    CLuaManager::getInstance().executeLuaScript(
        "RingAccessPoint:forceRefresh()");
  }
  ...
  // The sender part is up to and including the first ":" after the
  // goodSenderName
  ...
  PeopleInterraction.ChatInput.Tell.displayTellMessage(
      /*senderIndex, */ finalString, goodSenderName, prop.getRGBA(), 2,
      &windowVisible);
  CInterfaceManager::getInstance()->log(
      finalString, CChatGroup::groupTypeToString(CChatGroup::tell));
...
// clear a channel
void CInterfaceChatDisplayer::clearChannel(CChatGroup::TGroupType mode,
                                           uint32 dynChatDbIndex) {
  if (mode == CChatGroup::guild)
    PeopleInterraction.ChatInput.Guild.clearMessages();
  else if (mode == CChatGroup::team)
    PeopleInterraction.ChatInput.Team.clearMessages();
  else if (mode == CChatGroup::region)
    PeopleInterraction.ChatInput.Region.clearMessages();
  else if (mode == CChatGroup::arround)
    PeopleInterraction.ChatInput.AroundMe.clearMessages();
  else if (mode == CChatGroup::universe)
    PeopleInterraction.ChatInput.Universe.clearMessages();
  else if (mode == CChatGroup::dyn_chat) {
...
void impulseDynString(NLMISC::CBitMemStream &impulse) {
  ChatMngr.processChatStringWithNoSender(impulse, CChatGroup::system,
...
//	#else
//		nlwarning( "// TRAP // WE MUST NEVER CALL THIS IMPULE ANYMORE :
//ALL IS HANDLED BY STRING_MANAGER NOW !!!" ); 	#endif
//
//	// received ADD_DYN_STR
//	nlinfo("impulseCallBack : Received ADD_DYN_STR : adding %s at index
//%d",ucstr.toString().c_str(),index);
//}
/*
string getInterfaceNameFromId (sint botType, sint interfaceId)
{
        ...
        case BOTCHATTYPE::FriendlyMainPage: interfaceName += "friendly_main";
break; case BOTCHATTYPE::NeutralMainPage: interfaceName += "neutral_main";
break; case BOTCHATTYPE::NastyMainPage: interfaceName += "nasty_main"; break;
        case BOTCHATTYPE::MoreNewsPage: interfaceName += "more_news"; break;
        case BOTCHATTYPE::Done: nlinfo ("end of bot chat");
interfaceName.clear(); break;
        }
        return interfaceName;
}

static char *shortNews[] = {
        "The wind is sour and brings only bad tidings...", "Kitins have been
sighted near the village!", "", "The tribe of the Black Circle has recently",
"increased its activities in our region.", "", "The Black Circle has made an
incursion", "into our territory!", "", "The Black Circle has been sighted near
one", "of our forward posts, deep in dangerous territory.", "", "The tide has
washed up evil news, friend.", "The Black Circle is active in our region.", "",
        "Our people suffer from a debilitating shortage.", "We are in sore need
of KamiBast.", "", "The economy is slow and our reserve of", "Live Seed low.",
"", "We are in sore need of Live Seed", "If there is a Goo epidemic, we shall
all perish!", "", "Our master mages have gotten wind of", "the growing Kami
discontentment", "",
};

static char *longNews[] = {
        "These powerful predators haven't come this near", "to the village since
their devastating attack", "over 15 seasons ago!", "They are after more
KamiBast", "for their occult practices.", "", "They have captured", "2 of our
fortifications in the bush!", "", "They have taken over one of our richest
sources", "of KamiBast, and are exploiting it", "for their own occult
purposes.", "They now hold an important source", "of Live Seed hostage,", "close
to one of our forward posts.", "We use the magical properties of KamiBast and",
"its unusually rich fibers for all our crafts.", "", "If we don't harvest new
Seed soon,", "we will have no way of purchasing goods", "and resources, beyond
what we produce ourselves", "We use the rich Sap of Live Seed to produce", "an
antidote that counters the disastrous", "effects of the Goo on all Atysian life
forms.", "The Kamis are shaken by the Black Circle's", "presence. If the Circle
continues it's occult", "practices, we will all suffer the Kamic anger.",
};
*/
/*
void setFakeNews ()
{
        ...
        for (uint i = 0; i < sizeof(table)/sizeof(table[0]); i++)
        {
                { // set test for the friendly main
                        ...
                        CInterfaceGroup *inter =
CWidgetManager::getInstance()->getWindowFromId(iname); if (inter == NULL)
                        {
                                nlwarning ("cant find interface 's%'",
iname.c_str()); continue;
                        }

                        CViewText *inter2 = (CViewText
*)inter->getView("title0"); nlassert (inter2 != NULL);
                        inter2->setText(ucstring(shortNews[rnd*3])); // OLD

                        CViewText *inter3 = (CViewText
*)inter->getView("title1"); nlassert (inter3 != NULL);
                        inter3->setText(ucstring(shortNews[rnd*3+1])); // OLD

                        CViewText *inter4 = (CViewText
*)inter->getView("title2"); nlassert (inter4 != NULL);
...
static void setupBotChatBotGift(CInterfaceGroup *botChatGroup)
{
        ...
        CBotChat::setBotGift(botChatGroup, "Thanks to have succeeded the
mission", "Here's your reward", "The bot has taken the object quest from your
inventory");
}
*/
...
void impulseDynChatClose(NLMISC::CBitMemStream &impulse) {
  ...
  if (PermanentlyBanned)
    return;
  // nlinfo("impulseCallback : Received BOTCHAT:DYNCHAT_CLOSE
  // BotUID:"+toString(BotUID));
...
void impulseBeginCast(NLMISC::CBitMemStream &impulse) {
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("UI:VARIABLES:SPELL_CAST")
      ->setValue32(1);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("UI:VARIABLES:CAST_BEGIN")
      ->setValue32(begin);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("UI:VARIABLES:CAST_END")
      ->setValue32(end);
}

//-----------------------------------------------
// impulseCorrectPos :
// Message from the server to correct the user position because he is not at the
// same position on the server..
//-----------------------------------------------
void impulseCorrectPos(NLMISC::CBitMemStream &impulse) {
  ...
  if (UserEntity->mode() != MBEHAV::COMBAT_FLOAT) {
    if (x == 0) // Get SpeedAdjustement
    {
      ...
      CVectorD dest =
          CVectorD((float)x / 1000.0f, (float)y / 1000.0f, (float)z / 1000.0f);
      // Update the position for the vision.
...
// \warning This function remove the current target. Do no use to correct a
// position.
//-----------------------------------------------
...
// note - this method added by Sadge and Hamster to deal with unexplained
// recursive calls to impulseTPCommon these calls are provoked by the net
// manager update which is called during loading
void impulseTPCommon(NLMISC::CBitMemStream &impulse, bool hasSeason) {
  ...
  BOMB_IF(queuedTPs.size() != 1,
          NLMISC::toString("Queueing recursive TPs depth=%u", queuedTPs.size()),
          return);
...
void impulseTPCommon2(NLMISC::CBitMemStream &impulse, bool hasSeason) {
  ...
  if (LoadingBackground != ResurectKamiBackground &&
  ...
  // if resurect but user not dead, choose default. NB: this is a bug, the tp
  // impulse should tell which background to choose. \todo yoyo: this is a temp
  // fix
  ...
  if (SoundMngr) {
    ...
    // start to play
    SoundMngr->playEventMusic(LoadingMusic, CSoundManager::LoadingMusicXFade,
                              true);
  }
  ...
  if (!tpReason.empty()) {
    ...
    case R2::TPContext_Mainland:
      tpIcon = "cancel_tp_main_land.tga";
      break;
    case R2::TPContext_Edit:
      tpIcon = "cancel_tp_edit.tga";
      break;
    case R2::TPContext_IslandOwner:
      tpIcon = "cancel_tp_island_owner.tga";
      break;
    default:
      break;
    }
  ...
  ProgressBar.progress(1.f); // do a last display without the buttons because
                             // first frame may take a while to draw, and the
                             // buttons have no more effect at this point.
  ProgressBar.finish();
...
void impulseTeamInvitation(NLMISC::CBitMemStream &impulse) {
  ...
  CLuaManager::getInstance().executeLuaScript(
      "game:onTeamInvation(" + toString(textID) + ")", 0);
} // impulseTeamInvitation //
...
void impulseTeamShareOpen(NLMISC::CBitMemStream &impulse) {
  if (PermanentlyBanned)
    return;
  CInterfaceManager *im = CInterfaceManager::getInstance();
  CGroupContainer *gc = dynamic_cast<CGroupContainer *>(
      CWidgetManager::getInstance()->getElementFromId(
          "ui:interface:team_share"));
  if (!gc)
    return;
  gc->setActive(true);
...
// invalidate the player validation. If someone has choosen an item/phrase after
// the player has validated the player receive this message to let him know that
// the chance percentage to obtain a specific item has changed and so the player
// can update its own settings to fit better to what he wants. On the client
// side we have just to show the valid button. All the resets are done on the
// server side.
//-----------------------------------------------
void impulseTeamShareInvalid(NLMISC::CBitMemStream &impulse) {
  if (PermanentlyBanned)
    return;
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  CCtrlTextButton *pTB = dynamic_cast<CCtrlTextButton *>(
      CWidgetManager::getInstance()->getElementFromId(
          "ui:interface:team_share:content:ok"));
  if (pTB != NULL)
    pTB->setActive(true);
} // impulseTeamShareInvalid //
...
// The server wants to close the team sharing interface (if the sharing has been
// validated or other reasons)
//-----------------------------------------------
...
void impulseTeamContactInit(NLMISC::CBitMemStream &impulse) {
  ...
  // nlinfo("impulseCallback : Received TEAM:CONTACT_INIT nbfriend:%d
  // nbignore:%d", vFriendListName.size(), vIgnoreListName.size());

  PeopleInterraction.initContactLists(vFriendListName, vFriendListOnline,
                                      vIgnoreListName);
} // impulseTeamContactInit //
...
void impulseTeamContactCreate(NLMISC::CBitMemStream &impulse) {
  ...
  // client patch to resolve bad server response when requesting ignore list
  // contact creation
  ...
  // nlinfo("impulseCallback : Received TEAM:CONTACT_CREATE %d %d %s %d",
  // contactId, nameId, online?"true":"false", nList);
...
void impulseTeamContactStatus(NLMISC::CBitMemStream &impulse) {
  ...
  // nlinfo("impulseCallback : Received TEAM:CONTACT_STATUS %d %s", contactId,
  // online == ccs_online ?"online": online==ccs_offline?"offline" :
  // "foreign_online");
  ...
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  CPeopleList::TSortOrder order = (CPeopleList::TSortOrder)(
      NLGUI::CDBManager::getInstance()
          ->getDbProp("UI:SAVE:CONTACT_LIST:SORT_ORDER")
          ->getValue32());
...
void impulseTeamContactRemove(NLMISC::CBitMemStream &impulse) {
  ...
  // nlinfo("impulseCallback : Received TEAM:CONTACT_REMOVE %d %d", contactId,
  // nList);
...
//	u16 ( member index ) u32 (player name ), u8 ( player grade + last bit
//set if player online ).
//-----------------------------------------------
/*void impulseGuildSetMemberInfo(NLMISC::CBitMemStream &impulse)
{
        ...
        grade = (grade & 0x7F);
        CGuildManager::getInstance()->set(index, guildMemberName, grade,
online);
}*/

//-----------------------------------------------
// vector of  pair( u32 (player name ), u8 ( player grade + last bit set if
// player online ) )
//-----------------------------------------------
...
void impulseGuildJoinProposal(NLMISC::CBitMemStream &impulse) {
  ...
  CInterfaceManager *im = CInterfaceManager::getInstance();
  CGroupContainer *gc = dynamic_cast<CGroupContainer *>(
  CWidgetManager::getInstance()->getElementFromId("ui:interface:join_guild_proposal"));
  if (!gc) return;
...
void impulseEnterCrZoneProposal(NLMISC::CBitMemStream &impulse) {
  ...
  // nlinfo("impulseCallback : Received MISSION:ASK_ENTER_CRITICAL %d",
  // phraseID);
  ...
  CGroupContainer *gc = dynamic_cast<CGroupContainer *>(
      CWidgetManager::getInstance()->getElementFromId(
          "ui:interface:enter_crzone_proposal"));
  if (!gc)
    return;
...
void impulseCloseEnterCrZoneProposal(NLMISC::CBitMemStream &impulse) {
  ...
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  CInterfaceGroup *pIG =
      (CInterfaceGroup *)CWidgetManager::getInstance()->getElementFromId(
          "ui:interface:enter_crzone_proposal");
  if (pIG)
    pIG->setActive(false);
} // impulseCloseEnterCrZoneProposal //
...
void impulseExchangeInvitation(NLMISC::CBitMemStream &impulse) {
  ...
  // show the modal window that allow the player to accept / decline the
  // invitation
  CGroupContainer *wnd = dynamic_cast<CGroupContainer *>(
      CWidgetManager::getInstance()->getElementFromId(
          PLAYER_EXCHANGE_INVITATION_DIALOG));
  if (wnd) {
    wnd->setActive(true);
    wnd->updateCoords();
    wnd->center();
...
void impulseExchangeCloseInvitation(NLMISC::CBitMemStream &impulse) {
  if (PermanentlyBanned)
    return;
  CInterfaceManager *iMngr = CInterfaceManager::getInstance();
  // hide the modal window that allow the player to accept / decline the
  // invitation
  CInterfaceGroup *wnd = dynamic_cast<CInterfaceGroup *>(
      CWidgetManager::getInstance()->getElementFromId(
          PLAYER_EXCHANGE_INVITATION_DIALOG));
  if (wnd)
    wnd->setActive(false);
...
void impulseWho(NLMISC::CBitMemStream &impulse)
{
        ...
        CInterfaceManager::getInstance()->displaySystemInfo("Players currently
in the game :");
        ...
        while( impulse.getPos() < (sint32)impulse.length() )
        {
                ...
                angle -= NLMISC::Pi;
                nlinfo ("name %s uid %u dist %hu dirshort %hu angle %f",
name.toString().c_str(),loginId, dist, (uint16)dirshort, angle); sint direction
=(sint) floor( 0.5 + ( 8.0 * (angle + NLMISC::Pi)/(NLMISC::Pi) ) ); direction =
((direction%16)+16)%16; static const string txts[]=
                {
                        "uiW",
                        "uiWSW",
                        "uiSW",
                ...
                str = toString (" - uid %d - distance %hu meters - direction ",
loginId, dist); CInterfaceManager::getInstance()->displaySystemInfo(name + str +
CI18N::get(txts[direction]));
        }
}// impulseWho //
*/
  ...
  try {
    ...
    if (queueTop - counter + 1 > queue.size()) {
      ...
      while (queue.size() > 128) {
        if (!queue.front()) {
          nlwarning("COUNTER: counter %d not received !",
                    queueTop - queue.size() - 1);
        }
...
void impulseStringResp(NLMISC::CBitMemStream &impulse) {
  ...
  STRING_MANAGER::CStringManagerClient::instance()->receiveString(stringId,
                                                                  str);
}
...
CWidgetManager::getInstance()->getElementFromId(MC_M_CONTAINER); CGroupContainer
*pGCM = dynamic_cast<CGroupContainer*>(pIE); if (pGCM == NULL) return NULL;

        CGroupList *pList = pGCM->getList();
        CGroupContainer *pGCS =
dynamic_cast<CGroupContainer*>(pList->getGroup(MC_S_CONTAINER)); return pGCS;
}
...
void addMission(uint32 titleID)
{
        ...
        vArgs.push_back(pair<string,string>("id",
"mc"+NLMISC::toString(nNbMission))); vArgs.push_back(pair<string,string>("mcid",
NLMISC::toString(titleID)));
        ...
        CInterfaceGroup *pIG = pIM->createGroupInstance(MC_TEMPLATE,
pContent->getId(), vArgs); if (pIG == NULL)
        {
...
void impulseJournalCantAbandon(NLMISC::CBitMemStream &impulse) {
  ...
  CCDBNodeLeaf *pNL = NLGUI::CDBManager::getInstance()->getDbProp(
      "UI:TEMP:MISSION_ABANDON_BUTTON", false);
  if (pNL != NULL)
    pNL->setValue64(1);
}
...
void impulseJournalAddCompass(NLMISC::CBitMemStream &impulse) {
  ...
  if (PermanentlyBanned)
    return;
  // nlinfo("impulseCallback : Received JOURNAL:ADD_COMPASS %d %d %d", x, y,
  // text);
...
void impulseRemoteAdmin(NLMISC::CBitMemStream &impulse) {
  ...
  // remove the 2 first rc character if exists, only there to say to the EGS
  // that is a remote command
  if (cmd.size() > 2 && tolower(cmd[0]) == 'r' &&
      tolower(cmd[1]) == 'c') // FIXME: toLowerAscii
    cmd = cmd.substr(2);
  ...
  // nlinfo("impulseCallback : Received COMMAND:REMOTE_ADMIN : Server asked me
  // to execute '%s', result is '%s'", cmd.c_str(), str.c_str());

  CBitMemStream out;
  if (GenericMsgHeaderMngr.pushNameToStream("COMMAND:REMOTE_ADMIN_ANSWER",
                                            out)) {
    out.serial(rid);
    out.serial(cmd);
    out.serial(str);
    NetMngr.push(out);
    // nlinfo("impulseCallback : COMMAND:REMOTE_ADMIN_ANSWER %d %s %s sent",
    // rid, cmd.c_str(), str.c_str());
  }
}
...
void impulseGuildAbortCreation(NLMISC::CBitMemStream &impulse) {
  ...
  CBotChatPageCreateGuild *pPageCG =
      dynamic_cast<CBotChatPageCreateGuild *>(pPage);
...
void impulseGuildOpenInventory(NLMISC::CBitMemStream &impulse) {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  NLGUI::CDBManager::getInstance()
      ->getDbProp("UI:TEMP:INVENTORY_GUILD_OPENED")
      ->setValue32(1);
}
...
void impulsePhraseDownLoad(NLMISC::CBitMemStream &impulse) {
  ...
  for (uint i = 0; i < memorizedPhrases.size(); i++) {
    pPM->memorizePhrase(memorizedPhrases[i].MemoryLineId,
...
#ifdef FINAL_VERSION
#define SKIP_IF(condition, msg, skipAction)                                    \
  if (!(condition))                                                            \
    ;                                                                          \
  else                                                                         \
    skipAction;
...
void updateInventoryFromStream(NLMISC::CBitMemStream &impulse,
  ...
  try {
    ...
    // For All inventories
    for (uint invId = 0; invId != CInventoryCategoryTemplate::NbInventoryIds;
         ++invId) {
      // Presence bit
      bool hasContent;
      ...
      const string invBranchStr = CInventoryCategoryTemplate::getDbStr(
          (typename CInventoryCategoryTemplate::TInventoryId)invId);
      ICDBNode::CTextId textId(invBranchStr);
      ICDBNode *inventoryNode =
          IngameDbMngr.getNodePtr()->getNode(textId, false);
      ...
      for (uint c = 0; c != nbChanges; ++c) {
        ...
        if (iuInfoVersion == 1) {
          ...
          CCDBNodeBranch *slotNode = safe_cast<CCDBNodeBranch *>(
              inventoryNode->getNode((uint16)slotIndex));
          CCDBNodeLeaf *leafNode = type_cast<CCDBNodeLeaf *>(
              slotNode->find(INVENTORIES::InfoVersionStr));
          BOMB_IF(!leafNode, "Inventory slot property missing in database",
                  continue);
          ...
          if (CInventoryCategoryTemplate::needPlainInfoVersionTransfer()) {
            ...
            // NB: don't need to check GC on a info version upgrade, since this
            // is always a delta of +1 the order of received of this impulse is
            // not important
          ...
          if (iuAll == 1) {
            ...
            // nldebug( "Inv %s Update %u",
            // CInventoryCategoryTemplate::InventoryStr[invId],
            // itemSlot.getSlotIndex() );

            // Apply all properties to database
            CCDBNodeBranch *slotNode = safe_cast<CCDBNodeBranch *>(
                inventoryNode->getNode((uint16)itemSlot.getSlotIndex()));
            for (uint i = 0; i != INVENTORIES::NbItemPropId; ++i) {
              CCDBNodeLeaf *leafNode = type_cast<CCDBNodeLeaf *>(slotNode->find(
                  string(INVENTORIES::CItemSlot::ItemPropStr[i])));
              SKIP_IF(!leafNode, "Inventory slot property missing in database",
                      continue);
              leafNode->setPropCheckGC(
                  serverTick,
                  (sint64)itemSlot.getItemProp((INVENTORIES::TItemPropId)i));
            }
          } else {
            uint32 iuOneProp;
            ...
            if (iuOneProp == 1) {
              ...
              // nldebug( "Inv %s Prop %u %s",
              // CInventoryCategoryTemplate::InventoryStr[invId],
              // itemSlot.getSlotIndex(),
              // INVENTORIES::CItemSlot::ItemPropStr[itemSlot.getOneProp().ItemPropId]
              // );

              // Apply property to database
              CCDBNodeBranch *slotNode = safe_cast<CCDBNodeBranch *>(
                  inventoryNode->getNode((uint16)itemSlot.getSlotIndex()));
              CCDBNodeLeaf *leafNode =
                  type_cast<CCDBNodeLeaf *>(slotNode->find(string(
                      INVENTORIES::CItemSlot::ItemPropStr[itemSlot.getOneProp()
                                                              .ItemPropId])));
              SKIP_IF(!leafNode, "Inventory slot property missing in database",
                      continue);
              leafNode->setPropCheckGC(
                  serverTick, (sint64)itemSlot.getOneProp().ItemPropValue);

            } else // iuReset
            {
              uint32 slotIndex;
              impulse.serial(slotIndex,
                             CInventoryCategoryTemplate::SlotBitSize);
              // nldebug( "Inv %s Reset %u",
              // CInventoryCategoryTemplate::InventoryStr[invId], slotIndex );

              // Reset all properties in database
              CCDBNodeBranch *slotNode = safe_cast<CCDBNodeBranch *>(
                  inventoryNode->getNode((uint16)slotIndex));
              for (uint i = 0; i != INVENTORIES::NbItemPropId; ++i) {
                // Instead of clearing all leaves (by index), we must find and
                // clear only the properties in TItemPropId, because the actual
                // database leaves may have less properties, and because we must
                // not clear the leaf INFO_VERSION. NOTE: For example, only
                // player BAG inventory has WORNED leaf.
                CCDBNodeLeaf *leafNode =
                    type_cast<CCDBNodeLeaf *>(slotNode->find(
                        string(INVENTORIES::CItemSlot::ItemPropStr[i])));
                SKIP_IF(!leafNode,
                        "Inventory slot property missing in database",
                        continue);
...
void impulseUpdateInventory(NLMISC::CBitMemStream &impulse) {
  updateInventoryFromStream(
      impulse, (INVENTORIES::CInventoryCategoryForCharacter *)NULL, true);
};
...
void impulseDeathRespawnPoint(NLMISC::CBitMemStream &impulse) {
  ...
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  CGroupMap *pMap =
      dynamic_cast<CGroupMap *>(CWidgetManager::getInstance()->getElementFromId(
          "ui:interface:respawn_map:content:map_content:actual_map"));
  if (pMap == NULL) {
    nlwarning("problem cannot find "
              "ui:interface:respawn_map:content:map_content:actual_map");
    return;
  }
  ...
  if (pMap == NULL) {
    nlwarning(
        "problem cannot find ui:interface:map:content:map_content:actual_map");
    return;
  }
  pMap->addRespawnPoints(msg);
}
...
void impulseDuelInvitation(NLMISC::CBitMemStream &impulse) {
  ...
  CGroupContainer *pGC = dynamic_cast<CGroupContainer *>(
      CWidgetManager::getInstance()->getElementFromId(
          "ui:interface:join_duel_proposal"));
  if (pGC == NULL)
    return;
  CViewTextID *pVTID =
      dynamic_cast<CViewTextID *>(pGC->getView("invitor_name"));
  if (pVTID == NULL)
    return;
...
void impulsePVPChallengeInvitation(NLMISC::CBitMemStream &impulse) {
  ...
  CGroupContainer *pGC = dynamic_cast<CGroupContainer *>(
      CWidgetManager::getInstance()->getElementFromId(
          "ui:interface:join_pvp_challenge_proposal"));
  if (pGC == NULL)
    return;
  CViewTextID *pVTID =
      dynamic_cast<CViewTextID *>(pGC->getView("invitor_name"));
...
void impulsePVPFactionFactionWars(NLMISC::CBitMemStream &impulse) {
  ...
  for (uint i = 0; i < factionWars.FactionWarOccurs.size(); ++i) {
    CFactionWarManager::getInstance()->addFactionWar(
        factionWars.FactionWarOccurs[i]);
  }
}
...
void impulsePVPChooseClan(NLMISC::CBitMemStream &impulse)
{
        ...
        EGSPD::CPeople::TPeople clan1= EGSPD::CPeople::Unknown, clan2=
EGSPD::CPeople::Unknown; impulse.serialEnum( clan1 ); impulse.serialEnum( clan2
);
        ...
        CInterfaceManager *pIM = CInterfaceManager::getInstance();
        CGroupContainer *pGC =
dynamic_cast<CGroupContainer*>(CWidgetManager::getInstance()->getElementFromId("ui:interface:join_pvp_clan_proposal"));
        if (pGC == NULL) return;
        pGC->setActive(true);

        CCtrlTextButton * butClan1 =
dynamic_cast<CCtrlTextButton*>(CWidgetManager::getInstance()->getElementFromId("ui:interface:join_pvp_clan_proposal:content:clan1"));
        if( butClan1 == NULL )
                return;
        butClan1->setText( EGSPD::CPeople::toString( clan1 ) );

        CCtrlTextButton * butClan2 =
dynamic_cast<CCtrlTextButton*>(CWidgetManager::getInstance()->getElementFromId("ui:interface:join_pvp_clan_proposal:content:clan2"));
        if( butClan2 == NULL )
                return;
...
void impulseItemOpenRoomInventory(NLMISC::CBitMemStream &impulse) {
  ...
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  NLGUI::CDBManager::getInstance()
      ->getDbProp("UI:TEMP:INVENTORY_ROOM_OPENED")
      ->setValue32(1);
}
...
void impulseOutpostChooseSide(NLMISC::CBitMemStream &impulse) {
  ...
  // start
  OutpostManager.startPvpJoinProposal(playerGuildInConflict,
                                      playerGuildIsAttacker, ownerGuildNameId,
                                      attackerGuildNameId, declTimer);
}
...
void impulseOutpostDeclareWarAck(NLMISC::CBitMemStream &impulse) {
  ...
  // ack reception
  CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
      "UI:TEMP:OUTPOST:DECLARE_WAR_ACK_RECEIVED");
  if (node)
    node->setValueBool(true);
  // set result of ACK
  node = NLGUI::CDBManager::getInstance()->getDbProp(
      "UI:TEMP:OUTPOST:DECLARE_WAR_ACK_OK");
  if (node)
    node->setValueBool(canValidate);
  node = NLGUI::CDBManager::getInstance()->getDbProp(
      "UI:TEMP:OUTPOST:DECLARE_WAR_ACK_TEXTID");
  if (node)
    node->setValue32(docTextId);
  node = NLGUI::CDBManager::getInstance()->getDbProp(
      "UI:TEMP:OUTPOST:DECLARE_WAR_ACK_TIME_RANGE_ATT");
  if (node)
...
//-----------------------------------------------
//-----------------------------------------------
class CServerMessageBoxOnReceiveTextId
...
private:
  ...
  void activateMsgBoxWindow() {
    STRING_MANAGER::CStringManagerClient *pSMC =
    ...
    // if the string start with a @{Wxxxx} code, remove it and get the wanted
    // window size
    sint w = 256; // default size to 256 !!
    if (contentStr.size() >= 5 && contentStr[0] == '@' &&
        contentStr[1] == '{' && contentStr[2] == 'W') {
      uint i;
    ...
    // open the message box window
    {
      ...
      CGroupContainer *pGC = dynamic_cast<CGroupContainer *>(
          CWidgetManager::getInstance()->getElementFromId(
              "ui:interface:server_message_box"));
      if (pGC) {
        ...
        CViewText *vt = dynamic_cast<CViewText *>(
            CWidgetManager::getInstance()->getElementFromId(
                CWidgetManager::getInstance()->getParser()->getDefine(
                    "server_message_box_content_view_text")));
        if (vt)
        ...
        // Yoyo: because of buggued group container, I found that 6 times is a
        // good number....
        for (uint i = 0; i < 6; i++)
          pGC->updateCoords();
        pGC->center();
...
public:
  ...
  void startWaitTexts(uint32 titleTextId, uint32 docTextId) {
    ...
    // start to wait receive of those string (NB: they may be already here, but
    // waitDynStrings calls directly the callback in this case)
    STRING_MANAGER::CStringManagerClient *pSMC =
        STRING_MANAGER::CStringManagerClient::instance();
...
void impulseCombatFlyingHpDelta(NLMISC::CBitMemStream &impulse) {
  ...
  CRGBA color((uint8)(rgba >> 24 & 255), (uint8)(rgba >> 16 & 255),
              (uint8)(rgba >> 8 & 255), (uint8)(rgba & 255));
  CEntityCL *entity = EntitiesMngr.getEntityByCompressedIndex(entityID);
  if (entity)
    entity->addHPOutput(toString("%d", hpDelta), color);
}

void impulseCombatFlyingTextItemSpecialEffectProc(
    NLMISC::CBitMemStream &impulse) {
  uint32 entityID;
  uint32 rgba;
  uint8 effect;
  ...
  CRGBA color((uint8)(rgba >> 24 & 255), (uint8)(rgba >> 16 & 255),
              (uint8)(rgba >> 8 & 255), (uint8)(rgba & 255));
  string text =
      CI18N::get(toString("uiItemSpecialEffectFlyingText%s",
                          ITEM_SPECIAL_EFFECT::toString(
                              (ITEM_SPECIAL_EFFECT::TItemSpecialEffect)effect)
                              .c_str()));
...
void impulseCombatFlyingText(NLMISC::CBitMemStream &impulse) {
  ...
  impulse.serial(entityID);
  impulse.serial(tmp);
  COMBAT_FLYING_TEXT::TCombatFlyingText type =
      (COMBAT_FLYING_TEXT::TCombatFlyingText)tmp;
...
void impulseDssDown(NLMISC::CBitMemStream &impulse) { FarTP.onDssDown(); }
...
void impulseSetNpcIconDesc(NLMISC::CBitMemStream &impulse) {
  ...
  for (uint i = 0; i != (uint)nb8; ++i) {
    ...
    hasChanged =
        CNPCIconCache::getInstance().onReceiveMissionAvailabilityForThisChar(
            npcIconCacheKey, (NPC_ICON::TNPCMissionGiverState)state) ||
        hasChanged; // mind the order to avoid partial evaluation
  }
  if (hasChanged)
...
void initializeNetwork() {
  ...
  GenericMsgHeaderMngr.setCallback("DB_GROUP:UPDATE_BANK",
                                   impulseDatabaseUpdateBank);
  GenericMsgHeaderMngr.setCallback("DB_GROUP:INIT_BANK",
                                   impulseDatabaseInitBank);
  GenericMsgHeaderMngr.setCallback("DB_GROUP:RESET_BANK",
                                   impulseDatabaseResetBank);
  GenericMsgHeaderMngr.setCallback("CONNECTION:NO_USER_CHAR",
                                   impulseNoUserChar);
  ...
  GenericMsgHeaderMngr.setCallback("CONNECTION:VALID_NAME",
                                   impulseCharNameValid);
  ...
  GenericMsgHeaderMngr.setCallback("CONNECTION:SERVER_QUIT_ABORT",
                                   impulseServerQuitAbort);
  GenericMsgHeaderMngr.setCallback("CONNECTION:MAIL_AVAILABLE",
                                   impulseMailNotification);
  GenericMsgHeaderMngr.setCallback("CONNECTION:GUILD_MESSAGE_AVAILABLE",
                                   impulseForumNotification);
  GenericMsgHeaderMngr.setCallback("CONNECTION:PERMANENT_BAN",
                                   impulsePermanentBan);
  ...
  GenericMsgHeaderMngr.setCallback("STRING:DYN_STRING_GROUP",
                                   inpulseDynStringInChatGroup);
  GenericMsgHeaderMngr.setCallback("STRING:TELL2", impulseTell2);
  //	GenericMsgHeaderMngr.setCallback("STRING:ADD_DYN_STR",
  //impulseAddDynStr);
  ...
  GenericMsgHeaderMngr.setCallback("COMBAT:ENGAGE_FAILED",
                                   impulseCombatEngageFailed);
  GenericMsgHeaderMngr.setCallback("BOTCHAT:DYNCHAT_OPEN", impulseDynChatOpen);
  GenericMsgHeaderMngr.setCallback("BOTCHAT:DYNCHAT_CLOSE",
                                   impulseDynChatClose);
  ...
  GenericMsgHeaderMngr.setCallback("TEAM:SHARE_INVALID",
                                   impulseTeamShareInvalid);
  ...
  GenericMsgHeaderMngr.setCallback("TEAM:CONTACT_CREATE",
                                   impulseTeamContactCreate);
  GenericMsgHeaderMngr.setCallback("TEAM:CONTACT_STATUS",
                                   impulseTeamContactStatus);
  GenericMsgHeaderMngr.setCallback("TEAM:CONTACT_REMOVE",
                                   impulseTeamContactRemove);

  GenericMsgHeaderMngr.setCallback("EXCHANGE:INVITATION",
                                   impulseExchangeInvitation);
  GenericMsgHeaderMngr.setCallback("EXCHANGE:CLOSE_INVITATION",
                                   impulseExchangeCloseInvitation);
  ...
  //
  GenericMsgHeaderMngr.setCallback("STRING_MANAGER:PHRASE_SEND",
                                   impulsePhraseSend);
  GenericMsgHeaderMngr.setCallback("STRING_MANAGER:STRING_RESP",
                                   impulseStringResp);
  GenericMsgHeaderMngr.setCallback("STRING_MANAGER:RELOAD_CACHE",
                                   impulseReloadCache);
  //
  GenericMsgHeaderMngr.setCallback("BOTCHAT:FORCE_END", impulseBotChatForceEnd);

  GenericMsgHeaderMngr.setCallback("JOURNAL:INIT_COMPLETED_MISSIONS",
                                   impulseJournalInitCompletedMissions);
  GenericMsgHeaderMngr.setCallback("JOURNAL:UPDATE_COMPLETED_MISSIONS",
                                   impulseJournalUpdateCompletedMissions);
  //	GenericMsgHeaderMngr.setCallback("JOURNAL:CANT_ABANDON",
  //impulseJournalCantAbandon);

  GenericMsgHeaderMngr.setCallback("JOURNAL:ADD_COMPASS",
                                   impulseJournalAddCompass);
  GenericMsgHeaderMngr.setCallback("JOURNAL:REMOVE_COMPASS",
                                   impulseJournalRemoveCompass);

  // GenericMsgHeaderMngr.setCallback("GUILD:SET_MEMBER_INFO",
  // impulseGuildSetMemberInfo);
  // GenericMsgHeaderMngr.setCallback("GUILD:INIT_MEMBER_INFO",
  // impulseGuildInitMemberInfo);

  GenericMsgHeaderMngr.setCallback("GUILD:JOIN_PROPOSAL",
                                   impulseGuildJoinProposal);

  GenericMsgHeaderMngr.setCallback("GUILD:ASCENSOR", impulseGuildAscensor);
  GenericMsgHeaderMngr.setCallback("GUILD:LEAVE_ASCENSOR",
                                   impulseGuildLeaveAscensor);
  GenericMsgHeaderMngr.setCallback("GUILD:ABORT_CREATION",
                                   impulseGuildAbortCreation);
  GenericMsgHeaderMngr.setCallback("GUILD:OPEN_GUILD_WINDOW",
                                   impulseGuildOpenGuildWindow);

  GenericMsgHeaderMngr.setCallback("GUILD:OPEN_INVENTORY",
                                   impulseGuildOpenInventory);
  GenericMsgHeaderMngr.setCallback("GUILD:CLOSE_INVENTORY",
                                   impulseGuildCloseInventory);

  GenericMsgHeaderMngr.setCallback("GUILD:UPDATE_PLAYER_TITLE",
                                   impulseGuildUpdatePlayerTitle);
  GenericMsgHeaderMngr.setCallback("GUILD:USE_FEMALE_TITLES",
                                   impulseGuildUseFemaleTitles);
  // GenericMsgHeaderMngr.setCallback("GUILD:INVITATION",
  // impulseGuildInvitation);

  GenericMsgHeaderMngr.setCallback("HARVEST:CLOSE_TEMP_INVENTORY",
                                   impulseCloseTempInv);
  ...
  GenericMsgHeaderMngr.setCallback("PHRASE:CONFIRM_BUY",
                                   impulsePhraseConfirmBuy);
  GenericMsgHeaderMngr.setCallback("PHRASE:EXEC_CYCLIC_ACK",
                                   impulsePhraseAckExecuteCyclic);
  GenericMsgHeaderMngr.setCallback("PHRASE:EXEC_NEXT_ACK",
                                   impulsePhraseAckExecuteNext);

  GenericMsgHeaderMngr.setCallback("ITEM_INFO:SET", impulseItemInfoSet);
  GenericMsgHeaderMngr.setCallback("ITEM_INFO:REFRESH_VERSION",
                                   impulseItemInfoRefreshVersion);
  GenericMsgHeaderMngr.setCallback("MISSION_PREREQ:SET", impulsePrereqInfoSet);
  GenericMsgHeaderMngr.setCallback("ITEM:OPEN_ROOM_INVENTORY",
                                   impulseItemOpenRoomInventory);
  GenericMsgHeaderMngr.setCallback("ITEM:CLOSE_ROOM_INVENTORY",
                                   impulseItemCloseRoomInventory);

  GenericMsgHeaderMngr.setCallback("DEATH:RESPAWN_POINT",
                                   impulseDeathRespawnPoint);
  ...
  GenericMsgHeaderMngr.setCallback("DUEL:CANCEL_INVITATION",
                                   impulseDuelCancelInvitation);

  GenericMsgHeaderMngr.setCallback("PVP_CHALLENGE:INVITATION",
                                   impulsePVPChallengeInvitation);
  GenericMsgHeaderMngr.setCallback("PVP_CHALLENGE:CANCEL_INVITATION",
                                   impulsePVPChallengeCancelInvitation);

  GenericMsgHeaderMngr.setCallback("PVP_FACTION:PUSH_FACTION_WAR",
                                   impulsePVPFactionPushFactionWar);
  GenericMsgHeaderMngr.setCallback("PVP_FACTION:POP_FACTION_WAR",
                                   impulsePVPFactionPopFactionWar);
  GenericMsgHeaderMngr.setCallback("PVP_FACTION:FACTION_WARS",
                                   impulsePVPFactionFactionWars);

  //	GenericMsgHeaderMngr.setCallback("PVP_VERSUS:CHOOSE_CLAN",
  //impulsePVPChooseClan);

  GenericMsgHeaderMngr.setCallback("ENCYCLOPEDIA:UPDATE",
                                   impulseEncyclopediaUpdate);
  GenericMsgHeaderMngr.setCallback("ENCYCLOPEDIA:INIT",
                                   impulseEncyclopediaInit);

  GenericMsgHeaderMngr.setCallback("USER:BARS", impulseUserBars);
  GenericMsgHeaderMngr.setCallback("USER:POPUP", impulseUserPopup);

  GenericMsgHeaderMngr.setCallback("MISSION:ASK_ENTER_CRITICAL",
                                   impulseEnterCrZoneProposal);
  GenericMsgHeaderMngr.setCallback("MISSION:CLOSE_ENTER_CRITICAL",
                                   impulseCloseEnterCrZoneProposal);

  // Module gateway message
  GenericMsgHeaderMngr.setCallback("MODULE_GATEWAY:FEOPEN",
                                   cbImpulsionGatewayOpen);
  GenericMsgHeaderMngr.setCallback("MODULE_GATEWAY:GATEWAY_MSG",
                                   cbImpulsionGatewayMessage);
  GenericMsgHeaderMngr.setCallback("MODULE_GATEWAY:FECLOSE",
                                   cbImpulsionGatewayClose);

  GenericMsgHeaderMngr.setCallback("OUTPOST:CHOOSE_SIDE",
                                   impulseOutpostChooseSide);
  GenericMsgHeaderMngr.setCallback("OUTPOST:DECLARE_WAR_ACK",
                                   impulseOutpostDeclareWarAck);

  GenericMsgHeaderMngr.setCallback("COMBAT:FLYING_HP_DELTA",
                                   impulseCombatFlyingHpDelta);
  GenericMsgHeaderMngr.setCallback(
      "COMBAT:FLYING_TEXT_ISE", impulseCombatFlyingTextItemSpecialEffectProc);
  GenericMsgHeaderMngr.setCallback("COMBAT:FLYING_TEXT",
                                   impulseCombatFlyingText);
  ...
  GenericMsgHeaderMngr.setCallback("NPC_ICON:SVR_EVENT_MIS_AVL",
                                   impulseServerEventForMissionAvailability);
  GenericMsgHeaderMngr.setCallback("NPC_ICON:SET_TIMER",
                                   impulseSetNpcIconTimer);
}
...
bool CNetManager::update() {
  ...
  if (ClientCfg.Local) {
    ...
    if ((T1 - _MachineTimeAtTick) >= _MsPerTick) {
      NLMISC::TGameCycle nbTick =
          (NLMISC::TGameCycle)((T1 - _MachineTimeAtTick) / _MsPerTick);
  ...
  for (it = changes.begin(); it < changes.end(); ++it) {
    ...
    if (change.Property < AddNewEntity) {
      if (!IgnoreEntityDbUpdates || change.ShortId == 0) {
        ...
        EntitiesMngr.updateVisualProperty(
            change.GameCycle, change.ShortId, change.Property,
            change.PositionInfo.PredictedInterval);
      } else {
        nlwarning("CNetManager::update : Skipping "
                  "EntitiesMngr.updateVisualProperty() because "
                  "IgnoreEntityDbUpdates=%s and change.ShortId=%d",
                  (IgnoreEntityDbUpdates ? "true" : "false"), change.ShortId);
      }
    }
    // Add New Entity (and remove the old one in the slot).
      ...
      if (!IgnoreEntityDbUpdates || change.ShortId == 0) {
        ...
        if (EntitiesMngr.create(change.ShortId, get(change.ShortId),
                                change.NewEntityInfo) == 0)
          nlwarning("CNetManager::update : entity in the slot '%u' has not "
                    "been created.",
                    change.ShortId);
      } else {
        nlwarning("CNetManager::update : Skipping EntitiesMngr.create() "
                  "because IgnoreEntityDbUpdates=%s and change.ShortId=%d",
                  (IgnoreEntityDbUpdates ? "true" : "false"), change.ShortId);
      }
    }
    // Delete an entity
      ...
      if (!IgnoreEntityDbUpdates || change.ShortId == 0) {
        ...
        nlwarning("CNetManager::update : Skipping EntitiesMngr.remove() "
                  "because IgnoreEntityDbUpdates=%s and change.ShortId=%d",
                  (IgnoreEntityDbUpdates ? "true" : "false"), change.ShortId);
      }
    }
    ...
    // Property unknown.
      ...
      nlwarning("CNetManager::update : The property '%d' is unknown.",
                change.Property);
  }
  ...
  if (IngameDbMngr.getNodePtr()) {
    ...
    if (im) {
      ...
      if (!m_PingLeaf)
        m_PingLeaf = NLGUI::CDBManager::getInstance()->getDbProp(
            "UI:VARIABLES:PING", false);
      ...
      if (!m_UploadLeaf)
        m_UploadLeaf = NLGUI::CDBManager::getInstance()->getDbProp(
            "UI:VARIABLES:UPLOAD", false);
      ...
      if (!m_DownloadLeaf)
        m_DownloadLeaf = NLGUI::CDBManager::getInstance()->getDbProp(
            "UI:VARIABLES:DOWNLOAD", false);
      ...
      if (!m_PacketLostLeaf)
        m_PacketLostLeaf = NLGUI::CDBManager::getInstance()->getDbProp(
            "UI:VARIABLES:PACKETLOST", false);
      ...
      if (!m_ServerStateLeaf)
        m_ServerStateLeaf = NLGUI::CDBManager::getInstance()->getDbProp(
            "UI:VARIABLES:SERVERSTATE", false);
      ...
      if (!m_ConnectionQualityLeaf)
        m_ConnectionQualityLeaf = NLGUI::CDBManager::getInstance()->getDbProp(
            "UI:VARIABLES:CONNECTION_QUALITY", false);
...
/**
 * Buffers a bitmemstream, that will be converted into a generic action, to be
 * sent later to the server (at next update).
 */
 ...
 */
void CNetManager::pushPickup(CLFECOMMON::TCLEntityId slot,
...
void CNetManager::send(NLMISC::TGameCycle gameCycle) {
  ...
  if (_LastSentCycle >= gameCycle) {
    // nlinfo ("Try to CNetManager::send(%d) _LastSentCycle=%d more than one
    // time with the same game cycle, so we wait new game cycle to send",
    // gameCycle, _LastSentCycle);
 ...
 */
void CNetManagerMulti::init(const std::string &cookie,
                            const std::string &addr) {
  uint nb, baseCookie;
  NLMISC::CConfigFile::CVar *var =
      ClientCfg.ConfigFile.getVarPtr("NbConnections");
  if (var)
    nb = var->asInt();
  else
    nb = 1;
  ...
  nlinfo("CNetManagerMulti: Creating %u connections to %u front-ends, "
         "baseCookie=%u...",
         nb, fsAddrs.size(), baseCookie);
...
NLMISC_COMMAND(localTellTeam, "Temp : simulate a tell in local mode",
  ...
  InterfaceChatDisplayer.displayChat(dsi, msg, msg, CChatGroup::team,
                                     NLMISC::CEntityId::Unknown, player);
  return true;
}

// temp : simulate a tell in local mode
NLMISC_COMMAND(localTell, "Temp : simulate a tell in local mode",
               "<people_name> <msg>") {
  if (args.empty())
    return false;
  string player = args[0];
...
NLMISC_COMMAND(testExchangeInvitation,
...
//	ChatMngr.getDynamicDB().add( index, ucstr, code );
// #else
//	// TRAP // WE MUST NEVER CALL THIS COMMAND ANYMORE : ALL IS HANDLED BY
//STRING_MANAGER NOW !!! 	nlstop; #endif
//
//	return true;
// }
