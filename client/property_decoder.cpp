...
void CPropertyDecoder::receive(TPacketNumber /* packetNumber */,
                               CAction *action) {
...
/*
                CPropertyEntry			&px =
   _Entities[act->Slot].Properties[PROPERTY_POSX]; CPropertyEntry
   &py = _Entities[act->Slot].Properties[PROPERTY_POSY]; CPropertyEntry
   &pz = _Entities[act->Slot].Properties[PROPERTY_POSZ];
*/
    ...
    if (act->IsRelative) {
      ...
      nlinfo("Relative Pos: %d %d %d, Pos16: %hd %hd %hd, Date %u",
             (sint32)act->Position[0], (sint32)act->Position[1],
             (sint32)act->Position[2], act->Position16[0], act->Position16[1],
             act->Position16[2], act->GameCycle);
    } else {
      // Absolute position
      // nlinfo( "RefPos: %d %d %d RefBits: %hd %hd %hd", _RefPosX, _RefPosY,
      // _RefPosZ, _RefBitsX, _RefBitsY, _RefBitsZ );
      decodeAbsPos2D(act->Position[0], act->Position[1], act->Position16[0],
                     act->Position16[1]);
      act->Position[2] = ((sint32)((sint16)act->Position16[2])) << 4;
      if (act->Interior)
        act->Position[2] += 2;
      // act->Position[2] = _RefPosZ + (((sint32)((sint16)(act->Position16[2] -
      // _RefBitsZ))) << 4); nlinfo( "Pos16: %hd %hd %hd => Pos: %d %d %d",
      // act->Position16[0], act->Position16[1], act->Position16[2],
      // (sint32)px.LastReceived, (sint32)py.LastReceived,
      // (sint32)pz.LastReceived );
      _Entities[act->Slot].PosIsRelative = false;
      _Entities[act->Slot].PosIsInterior = act->Interior;
      // nlinfo( "Slot %hu: Absolute, Pos: %d %d %d, Pos16: %hu %hu %hu, Date
      // %u", (uint16)act->CLEntityId, (sint32)act->Position[0],
      // (sint32)act->Position[1], (sint32)act->Position[2], act->Position16[0],
      // act->Position16[1], act->Position16[2], act->GameCycle ); nlinfo( "
      // RefPos: %d %d %d, RefBits: %hu %hu %hu", _RefPosX, _RefPosY, _RefPosZ,
      // _RefBitsX, _RefBitsY, _RefBitsZ );
    }
/*
                px.LastReceived = act->Position[0];
                py.LastReceived = act->Position[1];
                pz.LastReceived = act->Position[2];
                //nldebug("CLPROPD: Received position (Id=%d) %d,%d (Pck=%d)",
   act->CLEntityId, act->Position[0], act->Position[1], packetNumber);
*/
#ifdef TEST_POSITION_CORRECTNESS
    // #pragma message ("TEST_POSITION_CORRECTNESS")
    if (!(act->IsRelative)) {
      // Check if the compression algo for positions worked
      nlassertex(abs(posx - (TCoord)act->Position[0]) < 2000,
                 ("Computed posX: %d Real posX: %d PosX16: %hu RefPosX: %d "
                  "RefBitsX: %hu",
                  (TCoord)(act->Position[0]), posx, act->Position16[0],
                  _RefPosX, _RefBitsX));
      nlassertex(abs(posy - (TCoord)act->Position[1]) < 2000,
                 ("Computed posY: %d Real posY: %d PosY16: %hu RefPosY: %d "
                  "RefBitsY: %hu",
                  (TCoord)(act->Position[1]), posy, act->Position16[1],
                  _RefPosY, _RefBitsY));
    }
    nlinfo("Receiving position %d %d %d m", posx / 1000, posy / 1000,
           (TCoord)act->Position[2] / 1000);
#endif
  }
  /*
          else
     ...
     _Entities[act->Slot].Properties[act->Code];

                  property.LastReceived = act->getValue();

                  nldebug("CLPROPD: Received (Id=%d,Act=%d)=(%" NL_I64
     "d)(Pck=%d)", act->Slot, act->Code, property.LastReceived, packetNumber);
          }
  */
}
...
void CPropertyDecoder::setReferencePosition(const NLMISC::CVectorD &position) {
  _RefPosX = (sint32)(position.x * 1000.0) &
             ~0xf; // correction here by Sadge: clear low-order to prevent
                   // flickering depending on player's reference position
...
//
/*
const CAction::TValue	&CPropertyDecoder::getProperty(TCLEntityId entity,
TProperty property) const
{
...
void
CPropertyDecoder::getPosition(TCLEntityId entity, CAction::TValue &posx,
CAction::TValue &posy, CAction::TValue &posz) const
{
        nlassert(entity < _Entities.size() && _Entities[entity].EntryUsed);
...
bool CPropertyDecoder::removeEntity(TCLEntityId entity) {
  nlassertex(entity < _Entities.size(),
             ("entity=%hu size=%u", (uint16)entity, _Entities.size()));
