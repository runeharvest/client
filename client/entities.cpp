...
public:
  ...
  virtual void update(ICDBNode *node) {
    ...
    if (leaf) {
      ...
      // if new target title is not NULL, then show the compass and make it
      // blink to indicate new location please note that the first time the
      // player login, a target has not been saved in his config file, so we
      // permit the first (and only one) mission that is received to become the
      // new compass direction (chiang the strong ...)
  ...
  void update() {
    ...
    while (it != _PendingMissionTitle.end()) {
      ...
      // If the previous title is not empty we probably have to clear the
      // compass
      if (leaf->getOldValue32() != 0) {
        CInterfaceManager *pIM = CInterfaceManager::getInstance();
        CGroupCompas *pGC = dynamic_cast<CGroupCompas *>(
            CWidgetManager::getInstance()->getElementFromId(
                "ui:interface:compass"));
        if (pGC == NULL) {
      ...
      if ((*tmpIt)->getValue32() == 0) {
        ...
        // TODO : maybe the following code could be include in
        // CGroupMap::checkCoords, but it is not called when the map is not
        // visible...
        STRING_MANAGER::CStringManagerClient *pSMC =
            STRING_MANAGER::CStringManagerClient::instance();
        string name;
        if (pSMC->getDynString((*tmpIt)->getValue32(), name)) {
          //					if (_AlreadyReceived.count(name)
          //== 0)
          //					{
          ...
          if (!gc) {
            ...
            if (parent) {
              CCDBNodeLeaf *x = dynamic_cast<CCDBNodeLeaf *>(
                  parent->getNode(ICDBNode::CTextId("X")));
              CCDBNodeLeaf *y = dynamic_cast<CCDBNodeLeaf *>(
                  parent->getNode(ICDBNode::CTextId("Y")));
              if (x && y) {
                CSmartPtr<CNamedEntityPositionState> tracker =
...
public:
  ...
  virtual void update(ICDBNode *node) {
    ...
    if (leaf) {
      ...
      CCDBNodeBranch *parent = leaf->getParent();
      if (parent) {
        leaf = dynamic_cast<CCDBNodeLeaf *>(
            parent->getNode(ICDBNode::CTextId("UID"), false));
        // Get the name id
...
// Destructor.
//-----------------------------------------------
CEntityManager::~CEntityManager() { release(); } // ~CEntityManager //
...
void CEntityManager::initialize(uint nbMaxEntity) {
  ...
  for (i = 0; i < MAX_NUM_MISSIONS; i++)
    for (j = 0; j < MAX_NUM_MISSION_TARGETS; j++) {
      ...
      NLGUI::CDBManager::getInstance()->getDB()->addObserver(
          &MissionTargetObserver, textId);
      _MissionTargetTitleDB[i][j] =
          NLGUI::CDBManager::getInstance()->getDbProp(text, false);
  ...
  for (i = 0; i < MaxNumPeopleInTeam; i++) {
    ...
    textId = ICDBNode::CTextId(text);
    NLGUI::CDBManager::getInstance()->getDB()->addObserver(&TeamUIDObserver,
                                                           textId);
    _GroupMemberUidDB[i] =
        NLGUI::CDBManager::getInstance()->getDbProp(text, false);
    ...
    textId = ICDBNode::CTextId(text);
    NLGUI::CDBManager::getInstance()->getDB()->addObserver(&TeamPresentObserver,
                                                           textId);
    _GroupMemberNameDB[i] =
        NLGUI::CDBManager::getInstance()->getDbProp(text, false);
  ...
  for (i = 0; i < MAX_INVENTORY_ANIMAL; i++) {
    ...
    textId = ICDBNode::CTextId(text);
    NLGUI::CDBManager::getInstance()->getDB()->addObserver(&AnimalUIDObserver,
                                                           textId);
...
CEntityManager::createInstance(const string &shape, const CVector &pos,
                               const string &text, const string &url,
                               bool haveCollisions, sint32 &idx) {
  idx = -1;
  ...
  if (!instance.empty()) {
    ...
    if (_LastRemovedInstance != -1) {
      ...
      CShapeInstanceReference instref = CShapeInstanceReference(
          instance, text, url, !text.empty() || !url.empty());
...
bool CEntityManager::setInstancePos(uint32 idx, CVector pos) {
  ...
  UMovePrimitive *primitive = _ShapeInstances[idx].Primitive;
  if (primitive) {
    primitive->setGlobalPosition(_ShapeInstances[idx].PrimRelativePos + pos,
                                 dynamicWI);
  }
...
bool CEntityManager::setupInstance(uint32 idx, const vector<string> &keys,
  ...
  for (uint32 i = 0; i < keys.size(); i++) {
    ...
    if (param == "col size x" || param == "col size y" ||
        param == "col size z") {
      float width, depth;
      primitive->getSize(width, depth);
      float height = primitive->getHeight();
    ...
    } else if (param == "col pos x" || param == "col pos y" ||
               param == "col pos z") {
      ...
      primitive->setGlobalPosition(pos + _ShapeInstances[idx].PrimRelativePos,
                                   dynamicWI);
...
CShapeInstanceReference
CEntityManager::getShapeInstanceUnderPos(float x, float y, sint32 &idx) {
  ...
  for (uint i = 0; i < _ShapeInstances.size(); i++) {
    if (!_ShapeInstances[i].Deleted && _ShapeInstances[i].BboxActive) {
      ...
      if (!_ShapeInstances[i].Instance.empty()) {
        ...
        bbox.setMinMax(bbox_min + _ShapeInstances[i].Instance.getPos(),
                       bbox_max + _ShapeInstances[i].Instance.getPos());
...
//-----------------------------------------------
CEntityCL *CEntityManager::create(uint slot, uint32 form,
                                  const TNewEntityInfo &newEntityInfo) {
  // DEBUG
  if (verboseVP(NULL, form))
    nlinfo("(%05d,%03d) EM:create: slot '%u': %s", sint32(T1 % 100000),
           NetMngr.getCurrentServerTick(), slot,
           CSheetId(form).toString().c_str());
  // Check parameter : slot.
  if (slot >= _NbMaxEntity) {
    nlwarning("EM:create: Cannot create the entity, the slot '%u' is invalid.",
              slot);
    return 0;
  } else {
    // Slot 0 is for the user and so should be allocated only once (at beginning
    // of main loop).
    if (slot == 0 && _Entities[0]) {
      if (newEntityInfo.DataSetIndex !=
          CLFECOMMON::INVALID_CLIENT_DATASET_INDEX) {
  ...
  if (_Entities[slot]) {
    nlwarning("EM:create: There is already an entity in the slot '%u' ! Old "
  ...
  if (entitySheet == 0) {
    nlwarning("EM:create: Attempt on create an entity with a bad form number "
              "%d (%s) for the slot '%d' trying to compute the default one.",
              form, ((CSheetId)form).toString().c_str(), slot);
    CSheetId defaultEntity;
    if (defaultEntity.buildSheetId(ClientCfg.DefaultEntity) == false) {
      nlwarning("EM:create: The default entity (%s) is not in the sheetid.bin.",
                ClientCfg.DefaultEntity.c_str());
      return 0;
    }
    entitySheet = SheetMngr.get(defaultEntity);
    if (entitySheet == 0) {
      nlwarning(
          "EM:create: The default entity (%s) is not in the sheet manager.",
          ClientCfg.DefaultEntity.c_str());
      return 0;
    }
  }
  ...
  case CEntitySheet::FAUNA: {
    ...
    if (!sheet->R2Npc)
      ...
      _Entities[slot] = new CPlayerR2CL;
  } break;
  ...
  default:
    pushDebugStr(NLMISC::toString(
        "Unknown Form Type '%d' -> entity not created.", entitySheet->type()));
    break;
  }
  ...
  // Entity Not Allocated
    ...
    pushDebugStr(NLMISC::toString(
        "Cannot Allocated the Entity in the slot '%d'.", slot));
  // Log problems about the entity creation.
  flushDebugStack(
      NLMISC::toString("Create Entity in slot '%d' with Form '%s' :", slot,
                       ((CSheetId)form).toString().c_str()));
...
bool CEntityManager::remove(uint slot, bool warning) {
  ...
  // Check parameter : slot.
  if (slot >= _NbMaxEntity) {
    nlwarning("CEntityManager::remove : Attempt on delete a bad slot (slot %d)",
              slot);
    return false;
  }
  ...
  if (slot == 0) {
    nlwarning("CEntityManager::remove : Cannot remove the entity in the slot 0 "
              "(user slot).");
    return false;
  }
  ...
  if (_Entities[slot] == 0) {
    if (warning) {
      nlwarning("CEntityManager::remove : Attempt on delete the slot '%d' that "
                "is not allocated.",
                slot);
      return false;
    }
...
//-----------------------------------------------
bool CEntityManager::entitiesNearDoors(float openingDist,
                                       const CVector &posDoor1,
                                       const CVector &posDoor2) {
...
//-----------------------------------------------
void CEntityManager::getEntityListForSelection(
    std::vector<CEntityCL *> &entities, uint flags) {
  // According to the view (first or third person), the user can or cannot be
  // selected.
  entities.clear();
...
//-----------------------------------------------
// getEntityUnderPos :
// Get the entity under the (2d) position. Return NULL if not entity under this
// position.
//-----------------------------------------------
struct CSortEntity {
  CEntityCL *Entity;
  ...
  bool operator<(const CSortEntity &o) const { return Depth < o.Depth; }
};
CEntityCL *CEntityManager::getEntityUnderPos(float x, float y,
                                             float distSelection,
  ...
  for (i = 0; i < intersectedEntities.size(); i++) {
    ...
    if (!ClientCfg.Light) {
      if (skeleton) {
        if (skeleton->supportFastIntersect() &&
            skeleton->fastIntersect(pos, dir, dist2D, distZ, false))
      ...
      // get the intersection with the instance (bot object)
        ...
        if (inst.supportFastIntersect() &&
            inst.fastIntersect(pos, dir, dist2D, distZ, false))
          trueIntersectComputed = true;
      }
    }
    ...
    if (!trueIntersectComputed) {
      /*
              this happens especially for Forage Source. but could happens for
         anyhting else In this case, estimate face-instersection, with box:
      ...
      */
      ...
      if (!bbox.clipSegment(a, b))
        continue;
      // take the middle of the clipped segment. suppose that this middle is the
      // "nearest ray point to center" This is false, but gives better results.
      CVector m = (a + b) / 2;
      ...
      if (itToCenter.x <= smallBoxHS.x && itToCenter.y <= smallBoxHS.y &&
          itToCenter.z <= smallBoxHS.z) {
        dist2D = 0;
        distZ = (m - pos).norm();
      } else {
        // no intersection
...
//-----------------------------------------------
CEntityCL *
CEntityManager::getEntityInCamera(uint flags, float distSelection,
  ...
  // prefer take the direction of the camera (can select backward for instance
  ...
  if (precEntity != CLFECOMMON::INVALID_SLOT) {
    ...
    if (entitySelected >= screenEntities.size() ||
        entitySelected >= ClientCfg.SpaceSelectionMaxCycle)
      entitySelected = 0;
  }
...
// This update the entites position. Evaluate collisions. Compte final world
// position.
//-----------------------------------------------
...
//-----------------------------------------------
void CEntityManager::updatePostCamera(uint clippedUpdateTime,
                                      const std::vector<CPlane> &clippingPlanes,
  ...
  for (i = 0; i < _EntitiesAllocated; ++i) {
    ...
    if (!activeEntity.Entity->clipped(clippingPlanes, camPos) ||
        (R2::getEditor().getSelectedInstance() &&
         R2::getEditor().getSelectedInstance()->getEntity() ==
             activeEntity.Entity)) {
      ...
      if ((activeEntity.Slot & RZ_CLIPPED_UPDATE_TIME_MASK) ==
          clippedUpdateTime) {
...
void CEntityManager::updatePostRender() {
  ...
  // Flush any no more used Flying text. Must do it before interface display (to
  // be sure texts are hid)
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
...
//-----------------------------------------------
void CEntityManager::updateVisualProperty(
    const NLMISC::TGameCycle &gameCycle, const uint &slot, const uint &prop,
  ...
  // Check parameter : slot.
  if (slot >= _NbMaxEntity) {
    nlwarning("CEntityManager::updateVisualProperty : Slot '%d' is not valid.",
              slot);
    return;
  }
  ...
  if (_Entities[slot] == 0) {
    // INFO : log some debug information about visual properties.
    if (verboseVP(NULL))
      nlinfo("EM:updateVP: backup the property as long as the entity is not "
             "allocated.",
             prop, slot);
    ...
    // Entity already have some changes backuped.
      ...
      // There is already a backuped value
        ...
        nlwarning("EM:updateVP:%d: property '%d' already backuped.", slot,
                  prop);
...
void CEntityManager::applyBackupedProperties(uint slot) {
  ...
  if (it != _BackupedChanges.end()) {
    ...
    while (itProp != properties.end()) {
      _Entities[slot]->updateVisualProperty((*itProp).second.GC,
                                            (*itProp).first, 0);
      ++itProp;
    }
...
void CEntityManager::writeEntities() {
  ...
  for (uint i = 1; i < nb; ++i) {
    if (_Entities[i]) {
      strTmp = toString(
          "\"%s\",\t\"%f\", \"%f\", \"%f\", \"%f\", \"%f\", \"%f\",\t// %3d\n",
          _Entities[i]->sheetId().toString().c_str(), _Entities[i]->pos().x,
          _Entities[i]->pos().y, _Entities[i]->pos().z, _Entities[i]->front().x,
          _Entities[i]->front().y, _Entities[i]->front().z, i);
      f.serialBuffer((uint8 *)strTmp.c_str(), (uint)strTmp.size());
    }
  }
...
void CEntityManager::serial(NLMISC::IStream &f) {
  ...
  //	f.serial(_EntitiesAllocated);	no need to serialize this one except
  //maybe to check.
...
void CEntityManager::dumpXML(class NLMISC::IStream &f) {
  ...
  for (uint i = 0; i < nb; ++i) {
    // Add a comment
    //			f.xmlComment();//toString("Describ the entity in the
    //slot %d.", i).c_str());
    // Start the opening of a new node named Identity
...
//-----------------------------------------------
CEntityCL *
CEntityManager::getEntityByKeywords(const std::vector<string> &keywords,
  ...
  for (uint i = 0; i < count; ++i) {
    ...
    if (onlySelectable && !_Entities[i]->properties().selectable())
      continue;
...
//-----------------------------------------------
CEntityCL *CEntityManager::getEntityByName(const string &name,
                                           bool caseSensitive,
                                           bool complete) const {
  string source;
  ...
  for (i = 0; i < count; i++) {
    if (_Entities[i]) {
      string value =
          caseSensitive
              ? _Entities[i]->getDisplayName()
              : toLower(
                    _Entities[i]->getDisplayName()); // TODO: toLowerInsensitive
...
CEntityCL *CEntityManager::getEntityByCompressedIndex(
    TDataSetIndex compressedIndex) const {
...
//-----------------------------------------------
CEntityCL *
CEntityManager::getEntityBySheetName(const std::string &sheet) const {
  if (!sheet.empty()) {
    uint i;
...
void CEntityManager::managePACSTriggers() {
  ...
  for (i = 0; i < nNbTrig; ++i) {
    ...
    // Detect collisions between user and other entities, to not be block (only
    // the user is a trigger so no need to check).
  ...
  for (i = 1; i < count; i++) {
    if (_Entities[i]) {
      if (_Entities[i]->getPrimitive()) {
        // remove collision only if the entity is Traversable (bot objects may
        // not)
...
void CEntityManager::removeAllAttachedFX() {
  for (TEntities::iterator it = _Entities.begin(); it != _Entities.end();
       ++it) {
    if (*it)
      (*it)->removeAllAttachedFX();
  }
}
...
// ***************************************************************************
void CEntityManager::startLogStageChange(sint32 currentGameCycle,
...
void CEntityManager::logStageChange(sint64 currentLocalTime) {
  ...
  if (ent) {
    ...
    if (WatchedEntitySlot != _LogStageChange.LastEntityLoged) {
      ...
      nldebugraw("*** Start Loging Stage changes for Entity %d",
                 WatchedEntitySlot);
    } else {
      // can log it
      ...
      while (itOld != oldStageSet.end() || itNew != newStageSet.end()) {
        ...
        // if signNewMinusOld= +1, it means an old exist, without a new (=> the
        // stage has been removed)
        if (signNewMinusOld == +1) {
          logPropertyChange(WatchedEntitySlot, itOld->second, CStage(),
                            precOldPos, precNewPos,
                            (sint32)itOld->first - recGcRef,
                            currentLocalTime - recTimeRef);
          // new prec pos (if any)
          itOld->second.getPos(precOldPos);
          itOld++;
        }
        // if signNewMinusOld= -1, it means an new exist, without an old (=> the
        // stage has been added)
        else if (signNewMinusOld == -1) {
          logPropertyChange(WatchedEntitySlot, CStage(), itNew->second,
                            precOldPos, precNewPos,
                            (sint32)itNew->first - recGcRef,
                            currentLocalTime - recTimeRef);
          // new prec pos (if any)
          itNew->second.getPos(precNewPos);
          itNew++;
        }
        // if ==0, means the stage exist in both, but properties set may be
        // different
        else {
          logPropertyChange(WatchedEntitySlot, itOld->second, itNew->second,
                            precOldPos, precNewPos,
                            (sint32)itNew->first - recGcRef,
                            currentLocalTime - recTimeRef);
          // new prec pos (if any)
...
// ***************************************************************************
void CEntityManager::logPropertyChange(
    CLFECOMMON::TCLEntityId who, const CStage &oldStage, const CStage &newStage,
    const CVectorD &precOldPos, const CVectorD &precNewPos, sint32 relGameCycle,
    sint64 relLocalTime) {
  ...
  for (uint i = 0; i < numProps; i++) {
    ...
    if ((oldProp.first || newProp.first) && oldProp != newProp) {
      ...
      // Position
        ...
        if (newProp.first) {
          if (newStage.getPos(pos))
            dist = float(
                CVectorD(pos.x - precNewPos.x, pos.y - precNewPos.y, 0).norm());
          valStr =
              toString("dst=%.1f pi=%d", dist, newStage.predictedInterval());
        } else {
          if (oldStage.getPos(pos))
            dist = float(
                CVectorD(pos.x - precOldPos.x, pos.y - precOldPos.y, 0).norm());
          valStr =
              toString("dst=%.1f pi=%d", dist, oldStage.predictedInterval());
        }
      }
      // Orientation
      ...
      // display log
      nldebugraw("** Entity %d: (gc=%3d,t=%3d) %s: %s %s", (sint32)who,
                 relGameCycle, (sint32)relLocalTime, reason.c_str(),
                 CLFECOMMON::getPropShortText(propLoged[i]), valStr.c_str());
    }
  }
}
