...
struct CPrecipitationDesc {
  ...
  bool UseBBoxSize; // The size of each block of the grid is taken from the x &
                    // y coordinates of the bbox. When set to false, 'Size' is
                    // used instead
  float Size; // Size of the blocks, it is used only if UseBBoxSize is set to
              // false.
  bool ReleasableModel; // Models are allocated only if particles are needed.
                        // This avoid useless models traversal during the render
                        // when thare are no precipitations.
  CPrecipitationDesc() {
    GridSize = 7;
    UseBBoxSize = true;
    Size = 0;
...
public:
  ...
  void update(const NLMISC::CMatrix &camMat,
              NLPACS::UGlobalRetriever *retriever);
  /** Set strenght for rain. Should go from 0 to 1 (clamped).
   * 0 stops the rains
   */
  void setStrenght(float strenght);
  /// After strenght has been set to 0, it may need some time before there are
  /// no more particle in the fx. This allow to now if the fx i still running
