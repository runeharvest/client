...
void updateFromClientCfg() {
  ...
  if ((ClientCfg.VREnable != LastClientCfg.VREnable) ||
    ...
    if (!StereoDisplayAttached)
      setVideoMode(UDriver::CMode(
          ClientCfg.Width, ClientCfg.Height, (uint8)ClientCfg.Depth,
  ...
  if (ClientCfg.InterfaceScale != LastClientCfg.InterfaceScale ||
      ClientCfg.InterfaceScaleAuto != LastClientCfg.InterfaceScaleAuto)
  ...
  if (ClientCfg.LandscapeThreshold != LastClientCfg.LandscapeThreshold) {
    if (Landscape)
      Landscape->setThreshold(ClientCfg.getActualLandscapeThreshold());
  }

  //---------------------------------------------------
  if (ClientCfg.LandscapeTileNear != LastClientCfg.LandscapeTileNear) {
    if (Landscape)
      Landscape->setTileNear(ClientCfg.LandscapeTileNear);
  }
      ...
      if (!ClientCfg.Light) {
        ...
        if (ContinentMngr.cur() && !ContinentMngr.cur()->Indoor) {
          ...
          Landscape->refreshAllZonesAround(
              ...
              zonesAdded, zonesRemoved, ProgressBar,
              ci ? &(ci->ZoneIDs) : NULL);
  ...
  //---------------------------------------------------
  if (ClientCfg.Vision != LastClientCfg.Vision ||
      ClientCfg.FoV != LastClientCfg.FoV ||
      ClientCfg.Windowed != LastClientCfg.Windowed ||
      ClientCfg.ScreenAspectRatio != LastClientCfg.ScreenAspectRatio) {
  ...
  if (Landscape) {
    if (ClientCfg.MicroVeget != LastClientCfg.MicroVeget) {
      if (ClientCfg.MicroVeget) {
        ...
        // Default setup. should work well for night/day transition in 30
        // minutes. Because all vegetables will be updated every 20 seconds =>
        // 90 steps.
  ...
  if (ClientCfg.SoundOn != LastClientCfg.SoundOn ||
      ClientCfg.DriverSound != LastClientCfg.DriverSound) {
  ...
  if (SoundMngr && LastClientCfg.SoundOn &&
      (ClientCfg.SoundForceSoftwareBuffer !=
       LastClientCfg.SoundForceSoftwareBuffer)) {
    SoundMngr->reset();
  ...
  if (SoundMngr &&
      ClientCfg.SoundGameMusicVolume != LastClientCfg.SoundGameMusicVolume) {
    SoundMngr->setGameMusicVolume(ClientCfg.SoundGameMusicVolume);
  }

  // reload only if active and reseted
  if (mustReloadSoundMngrContinent && SoundMngr && ContinentMngr.cur() &&
      !ContinentMngr.cur()->Indoor && UserEntity) {
    SoundMngr->loadContinent(ContinentMngr.getCurrentContinentSelectName(),
                             UserEntity->pos());
  }
