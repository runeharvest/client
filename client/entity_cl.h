 ...
 * \date 2001
 */
class CEntityCL : public NLMISC::IStreamable,
...
public:
  struct SInstanceCL {
    ...
    // Create the loading instance. return false if shapeName is not empty while
    // still fails to load. else return true.
    bool createLoading(const std::string &shapeName,
                       const std::string &stickPoint = std::string(""),
                       sint texture = -1, bool clearIfFail = true);

    // Apply Colors
    void setColors(sint skin, sint user, sint hair, sint eyes);

    // Create the loading instance from the current instance and copy info from
    // current instance to it If the loading instance already exist do not copy
    // anything from current instance
    NL3D::UInstance createLoadingFromCurrent();

    // Update current instance from loading instance (set loading instance to
    // null) Bind the loading instance to the skeleton
...
public:
  ...
  /// Return the entity Id (persistent as long as the entity is connected)
  /// (CLFECOMMON::INVALID_CLIENT_DATASET_INDEX for an invalid one).
  const CLFECOMMON::TClientDataSetIndex &dataSetId() const {
    return _DataSetId;
  }
  /// Set the entity Id (persistent as long as the entity is connected)
  /// (CLFECOMMON::INVALID_CLIENT_DATASET_INDEX for an invalid one).
   ...
   *	clipRadius/clipHeight, specify them if you want special clip values
   *(different from collision ones)
   */
  bool initPrimitive(float radius, float height, float length, float width,
                     NLPACS::UMovePrimitive::TReaction reactionType,
                     NLPACS::UMovePrimitive::TTrigger triggerType,
                     NLPACS::UMovePrimitive::TCollisionMask occlusionMask,
                     NLPACS::UMovePrimitive::TCollisionMask collisionMask,
   ...
   */
  void updateVisualProperty(const NLMISC::TGameCycle &gameCycle,
                            const uint &prop,
                            const NLMISC::TGameCycle &predictedInterval = 0);
  ...
  /** Method called each frame to manage the entity before the collisions
   * detection. \param time : current time of the frame. \parem target : pointer
   * on the current entity target.
   */
  virtual void updatePreCollision(const NLMISC::TTime & /* time */,
                                  CEntityCL * /* target */) {}
  /** Method called each frame to manage the entity after the collisions
   * detection. \param time : current time of the frame. \parem target : pointer
   * on the current entity target.
   */
  virtual void updatePostCollision(const NLMISC::TTime & /* time */,
                                   CEntityCL * /* target */) {}
  /** Method called each frame to manage the entity after the clipping test if
   * the primitive is visible. \param time : current time of the frame. \parem
   * target : pointer on the current entity target.
   */
  virtual void updateVisible(const NLMISC::TTime &time, CEntityCL *target);
  /** Method called to manage the entity after the clipping test if the
   * primitive is clipped. This method is called regulary but not at each frame.
  ...
  /** Method called to manage the entity after the clipping test if the
   * primitive is clipped. This method is called at each frame. \param time :
   * current time of the frame. \parem target : pointer on the current entity
   * target.
   */
   ...
   */
  virtual void updatePos(const NLMISC::TTime & /* time */,
                         CEntityCL * /* target */) {}
  /** Update the entity after the render like for the head offset.
   */
  virtual void updateVisiblePostPos(const NLMISC::TTime &time,
                                    CEntityCL *target);
  /** Update the entity after the render like for the head offset.
   */
   ...
   * \param texture : texture to use (in multi texture) or -1 for default
   * texture. \param instIdx : if not CEntityCL::BadIndex, the instance will
   * replace the one at this index. \return uint32 : index of the instance
   * created, or CEntityCL::BadIndex.
   */
  uint32 addInstance(const std::string &shapeName,
                     const std::string &stickPoint = "", sint texture = -1,
                     uint32 instIdx = BadIndex);

  // Return true if the primitive is clipped by the camera
  bool clipped(const std::vector<NLMISC::CPlane> &clippingPlanes,
               const NLMISC::CVector &camPos);

  /** \name NAME
   * Functions to manipulate the Name.
   */
  //@{
  /// Return the Name of the entity. There may be a specification in it (guard,
  /// trader, etc ...). It is then surrounded by '$'
  const std::string &getEntityName() const { return _EntityName; }
  /// Return the title from a name. The specification is surrounded by '$', and
  /// tells the title of the entity (guard, matis merchant, etc ..)
  static std::string getTitleFromName(const std::string &name);
  /// Remove the specification from a name. The specification is surrounded by
  /// '$', and tells the title of the entity (guard, matis merchant, etc ..)
  static std::string removeTitleFromName(const std::string &name);
  /// Remove the shard from a name (if player from the same shard). The shard is
  /// surrounded by (), and tells the incoming shard of the entity (aniro,
  /// leanon etc...)
  ...
  const std::string &getPermanentStatutIcon() const {
    return _PermanentStatutIcon;
  }
  /// Change the entity permanent content texture name.
  void setPermanentStatutIcon(const std::string &name) {
    _PermanentStatutIcon = name;
  }
   ...
   * string to remove a skeleton. \return USkeleton * : pointer on the skeleton
   * handle or NULL.
   */
  ...
  /** To Inform about another (or self) entity removal (to remove from selection
   * for example). \param slot : Slot of the entity that will be removed.
   */
  virtual void slotRemoved(const CLFECOMMON::TCLEntityId & /* slot */) {}

  /// Return the current slot for the entity or CLFECOMMON::INVALID_SLOT if the
  /// entity is not in any slot.
  const CLFECOMMON::TCLEntityId &slot() const { return _Slot; }
  /// Set the current slot for the entity (CLFECOMMON::INVALID_SLOT for no
  /// slot).
  ...
  /// Set the current target of the entity (CLFECOMMON::INVALID_SLOT for no
  /// target).
  void targetSlot(const CLFECOMMON::TCLEntityId &t) { _TargetSlot = t; }
  /// get the most recent TargeSlot received in updateVisualProperty() (ie
  /// wihtout any LCT delay due to _Stages)
  CLFECOMMON::TCLEntityId getTargetSlotNoLag() const {
    return _TargetSlotNoLag;
  }
  /// dir to target
  NLMISC::CVector dirToTarget() const;
  //@}

  /// Set the mode for the entity. return false if it will be impossible to set
  /// this mode.
  ...
  /// Change the PACS position and the entity position too. This is slow, prefer
  /// pacsMove if you can.
  void
  pacsPos(const NLMISC::CVectorD &vect,
          const NLPACS::UGlobalPosition &globPos = NLPACS::UGlobalPosition());
  /// Move the PACS position and the entity position too. This is fast. The PACS
  /// position will be available after the PACS::evalCollsion call.
  void pacsMove(const NLMISC::CVectorD &vect);
  /**	Update the PACS position after the evalCollision. The entity position is
   *set too. This is fast. If the entity position is too far from its PACS
   *position, setGlobalPosition is called. After this call, the position.z is
   *valid.
   */
   ...
   * valid to be the front (vector Null) even with compute=true. \param
   * forceTurn : set front even if the entity cannot turn \return bool : 'true'
   * if the front has been filled, else 'false'.
   */
  bool front(const NLMISC::CVector &vect, bool compute = true,
             bool check = true, bool forceTurn = false);
   ...
   * direction unchanged if impossible. \param check : warning if the param
   * 'vect' is not valid to be the direction (vector Null) even with
   * compute=true. \return bool : 'true' if the direction has been filled, else
   * 'false'.
   */
   ...
   */
  NL3D::UVisualCollisionEntity *getCollisionEntity() {
    return _CollisionEntity;
  }
  /** Set the collision entity (collision with the landscape)
   * \param pointer on the collision entity
   */
  void setCollisionEntity(NL3D::UVisualCollisionEntity *collisionEntity) {
    _CollisionEntity = collisionEntity;
  }
  //@}
   ...
   * attack angle. \param ang : 0 = the front, >0 and <Pi = left side, <0 and
   * >-Pi = right side.
   */
  virtual NLMISC::CVectorD getAttackerPos(double ang, double dist) const;
  /// Return true if the opponent is well placed.
  virtual bool isPlacedToFight(const NLMISC::CVectorD & /* posAtk */,
                               const NLMISC::CVector & /* dirAtk */,
                               double /* attackerRadius */) const {
    return false;
  }
   ...
   */
  virtual void impact(uint /* impactType */, uint /* type */, uint /* id */,
                      uint /* intensity */) {}
  ...
  virtual void changeColors(sint /* userColor */, sint /* hair */,
                            sint /* eyes */, sint /* part */) {}
  ...
  /// Return true if the character is really dead. With no lag because of anim
  /// or LCT
  virtual bool isReallyDead() const { return false; }

  // Add hit points gain/lost by this entity.
  void addHPOutput(sint16 hp, NLMISC::CRGBA color, float dt = 0.0f) {
    if (_HPModifiers.size() < 20)
      _HPModifiers.push_back(CHPModifier(hp, color, dt));
  }
  void addHPOutput(const std::string &text, NLMISC::CRGBA color,
                   float dt = 0.0f) {
    if (_HPModifiers.size() < 20 && !text.empty())
      _HPModifiers.push_back(CHPModifier(text, color, dt));
  }
  ...
  // If entity is a mesh, then returns its default scale (value from export).
  // Returns (1.f, 1.f, 1.f) of not a mesh or skinned
  ...
  // return vector of ground fxs sorted by ground type, or NULL is ground fxs
  // are not supported for the entity
  ...
  // Was the entity clipped during the last frame ? This is updated during the
  // call CEntityManager::updatePostCamera
  ...
  /// Return true if this player has the viewing properties of a friend (inscene
  /// bars...)
  virtual bool isViewedAsFriend() const {
    return isNeutral() || isFriend() || isInTeam() || isInSameGuild() ||
           isInSameLeague();
  }
  ...
  // Return true if this entity can have missions icons (humanoid NPCs
  // (including Karavan), Kami or Bot Object)
  bool canHaveMissionIcon() const {
    return isNPC() || isKami() || isUnknownRace();
  }
  ...
  // Return true if this entity is the current user selection. NB: actually test
  // UserEntity->selection() (ie not laggy!)
  bool isTarget() const;

  // Return true if this entity is under the cursor
  bool isUnderCursor() const { return _Slot == SlotUnderCursor; }
  ...
  virtual OUTPOSTENUMS::TPVPSide getOutpostSide() const {
  ...
  const std::string &getTitleRaw() const { return _TitleRaw; }
  ...
  // enable display of in scene interface. This flag is 'anded' with the final
  // value when calling 'mustShowInsceneInterface'
  void enableInSceneInterface(bool enabled) {
    _InSceneInterfaceEnabled = enabled;
  }
  ...
  // get bone name from a part of the body (base implementation provides the
  // base bones names)
  virtual const char *getBoneNameFromBodyPart(BODY::TBodyPart part,
                                              BODY::TSide side) const;
  // ...
  virtual bool getBoneHeight(BODY::TBodyPart /* localisation */,
                             BODY::TSide /* side */,
                             float & /* height */) const {
    return false;
  }

  // true if the entity position is valid (related to network lag at creation
  // time)
  ...
  // true if the entity allow to cast a shadowMap (eg: false for userentity in
  // some case). Default to ClientCFG value
  ...
  // even if ClientCfg.ShowNameUnderCursor==false, force OSD to display when
  // under cursor (not if _Sheet->DisplayOSD=false)
...
protected:
  ...
  // Flags to know what is possible to do with the entity (selectable, liftable,
  // etc.).
  ...
  float _ClipDeltaZ; // DeltaZ of the sphere center to ground (different from
                     // clipradius)
  ...
  public:
    CHPModifier() {}
    virtual ~CHPModifier() {}
    CHPModifier(sint16 value, NLMISC::CRGBA color, float dt)
        : Value(value), Color(color), DeltaT(dt) {}
    CHPModifier(const std::string &text, NLMISC::CRGBA color, float dt)
        : Text(text), Color(color), DeltaT(dt) {}
  ...
  static NLMISC::CRGBA _GMTitleColor[CHARACTER_TITLE::EndGmTitle -
  ...
  bool _Displayable : 1;
  // Was the entity visible during the last frame ? This is updated during the
  // call to CEntityManager::updatePostCamera
  bool _Clipped : 1;
...
protected:
  ...
  virtual void
  updateVisualPropertyPos(const NLMISC::TGameCycle & /* gameCycle */,
                          const sint64 & /* prop */,
                          const NLMISC::TGameCycle & /* pI */) {}
  /// Update Entity Orientation.
  virtual void
  updateVisualPropertyOrient(const NLMISC::TGameCycle & /* gameCycle */,
                             const sint64 & /* prop */) {}
  /// Update Entity Behaviour.
  virtual void
  updateVisualPropertyBehaviour(const NLMISC::TGameCycle & /* gameCycle */,
                                const sint64 & /* prop */) {}
  /// Update Entity Name.
  virtual void
  updateVisualPropertyName(const NLMISC::TGameCycle & /* gameCycle */,
                           const sint64 & /* prop */) {}
  /// Update Entity Target.
  virtual void
  updateVisualPropertyTarget(const NLMISC::TGameCycle & /* gameCycle */,
                             const sint64 & /* prop */) {}
  /// Update Entity Mode.
  virtual void
  updateVisualPropertyMode(const NLMISC::TGameCycle & /* gameCycle */,
                           const sint64 & /* prop */) {}
  /// Update Entity Visual Property A
  virtual void
  updateVisualPropertyVpa(const NLMISC::TGameCycle & /* gameCycle */,
                          const sint64 & /* prop */) {}
  /// Update Entity Visual Property B
  virtual void
  updateVisualPropertyVpb(const NLMISC::TGameCycle & /* gameCycle */,
                          const sint64 & /* prop */) {}
  /// Update Entity Visual Property C
  virtual void
  updateVisualPropertyVpc(const NLMISC::TGameCycle & /* gameCycle */,
                          const sint64 & /* prop */) {}
  /// Update Entity Mount
  virtual void
  updateVisualPropertyEntityMounted(const NLMISC::TGameCycle & /* gameCycle */,
                                    const sint64 & /* prop */) {}
  /// Update Entity Rider
  virtual void
  updateVisualPropertyRiderEntity(const NLMISC::TGameCycle & /* gameCycle */,
                                  const sint64 & /* prop */) {}
  /// Update Entity Bars
  virtual void
  updateVisualPropertyBars(const NLMISC::TGameCycle & /* gameCycle */,
                           const sint64 & /* prop */) {}
  virtual void
  updateVisualPropertyGuildSymbol(const NLMISC::TGameCycle & /* gameCycle */,
                                  const sint64 & /* prop */) {}
  virtual void
  updateVisualPropertyGuildNameID(const NLMISC::TGameCycle & /* gameCycle */,
                                  const sint64 & /* prop */) {}
  virtual void
  updateVisualPropertyEventFactionID(const NLMISC::TGameCycle & /* gameCycle */,
                                     const sint64 & /* prop */) {}
  virtual void
  updateVisualPropertyPvpMode(const NLMISC::TGameCycle & /* gameCycle */,
                              const sint64 & /* prop */) {}
  virtual void
  updateVisualPropertyPvpClan(const NLMISC::TGameCycle & /* gameCycle */,
                              const sint64 & /* prop */) {}
  virtual void
  updateVisualPropertyOwnerPeople(const NLMISC::TGameCycle & /* gameCycle */,
                                  const sint64 & /* prop */) {}
  virtual void
  updateVisualPropertyOutpostInfos(const NLMISC::TGameCycle & /* gameCycle */,
                                   const sint64 & /* prop */) {}

  /// Update Entity State
  virtual void
  updateVisualPropertyStatus(const NLMISC::TGameCycle & /* gameCycle */,
                             const sint64 & /* prop */) {}
  /// Target lists
  virtual void
  updateVisualPropertyTargetList(const NLMISC::TGameCycle & /* gameCycle */,
                                 const sint64 & /* prop */,
                                 uint /* listIndex */) {}
  /// Visual FX
  virtual void
  updateVisualPropertyVisualFX(const NLMISC::TGameCycle & /* gameCycle */,
                               const sint64 & /* prop */) {}
  /// Update vprop contextual attributes
  virtual void
  updateVisualPropertyContextual(const NLMISC::TGameCycle &gameCycle,
                                 const sint64 &prop);
   ...
   * \return SInstanceCL * : pointer on the instance associated to the index or
   * 0 if idx has no instance.
   */
  ...
  // called each frame to update some shadowmap properties according to position
  // of player
...
public:
  ...
  void setVisualSelectionBlink(bool bOnOff, NLMISC::CRGBA emitColor) {
    ...
    // NB: cannot do this if both true, since emitColor may not be the same
    // (emitColor caching not interesting...)
   ...
   */
  //@{
  /** update the display of the AsyncTexture of the entity. called in
   *updateDisplay() Deriver: See CPlayerCL implementation \return distance from
   *entity to camera computed (helper for deriver)
   */
