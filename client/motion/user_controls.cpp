...
// Constructor.
//---------------------------------------------------
CUserControls::CUserControls() { init(); } // CUserControls //
...
void CUserControls::update() {
  ...
  NLMISC::CCDBNodeLeaf *node =
      _UiVarMkMoveDB
          ? &*_UiVarMkMoveDB
          : &*(_UiVarMkMoveDB = NLGUI::CDBManager::getInstance()->getDbProp(
                   "UI:VARIABLES:MK_MOVE"));
...
// Acceleration helper
//-----------------------------------------------
void CUserControls::updateVelocity(float deltaTime, float acceleration,
                                   float brake, float speedMax, float &speed) {
  ...
  if (left || right) {
    ...
    if (left) {
      ...
      _RotateUserLRVelocity =
          std::max(_RotateUserLRVelocity, ClientCfg.RotKeySpeedMin);
    }

    if (right) {
      accel -= ClientCfg.RotAccel;
      _RotateUserLRVelocity =
          std::min(_RotateUserLRVelocity, -ClientCfg.RotKeySpeedMin);
    }
    updateVelocity(DT, accel, ClientCfg.RotAccel, ClientCfg.RotKeySpeedMax,
                   _RotateUserLRVelocity);
  ...
  if (up || down) {
    ...
    if (up) {
      ...
      _RotateUserUDVelocity =
          std::max(_RotateUserUDVelocity, ClientCfg.RotKeySpeedMin);
    }

    if (down) {
      accel -= ClientCfg.RotAccel;
      _RotateUserUDVelocity =
          std::min(_RotateUserUDVelocity, -ClientCfg.RotKeySpeedMin);
    }
    updateVelocity(DT, accel, ClientCfg.RotAccel, ClientCfg.RotKeySpeedMax,
                   _RotateUserUDVelocity);
  ...
  if (left || right) {
    ...
    if (left) {
      ...
      _RotateCameraLRVelocity =
          std::max(_RotateCameraLRVelocity, ClientCfg.RotKeySpeedMin);
    }

    if (right) {
      accel -= ClientCfg.RotAccel;
      _RotateCameraLRVelocity =
          std::min(_RotateCameraLRVelocity, -ClientCfg.RotKeySpeedMin);
    }
    updateVelocity(DT, accel, ClientCfg.RotAccel, ClientCfg.RotKeySpeedMax,
                   _RotateCameraLRVelocity);
...
void CUserControls::freeLook(bool fullMode) {
  ...
  if (fullMode) {
    ...
    if (isSwimming() || isRiding())
      ...
      keyboardRotationLR(Actions.valide("strafe_left"),
                         Actions.valide("strafe_right"));
    // Strafe Left
    ...
    // but still important to test it here, in case of FreeLook mode (no right
    // button down needed)
...
void CUserControls::cameraLook(bool fullMode) {
  ...
  if (dx != 0.f)
    appendCameraDeltaYaw(dx);
  // If the mouse move on the axis Y, cahnge the actual user pitch (not the
  // delta camera)
  if (dy != 0.f) {
    UserEntity->rotHeadVertically(dy);
    UserEntity->stopForceHeadPitchInFollow();
...
// \todo GUIGUI : manage control before entities update, but view only after
// move
//-----------------------------------------------
void CUserControls::commonMove() {
  ...
  /////////////////
  // MOUSE WHEEL //
  CEventsListener::TWheelState wheelState =
      EventsListener.getWheelState(); // Done all the time, to reset the state
  View.changeCameraDist((wheelState == CEventsListener::foreward),
                        (wheelState == CEventsListener::backward));
  View.changeCameraDist(Actions.valide("camera_foreward"),
                        Actions.valide("camera_backward"));
  // Camera Up/Down.
  View.changeCameraHeight(Actions.valide("camera_up"),
                          Actions.valide("camera_down"));
  //////////////////
  // LOOK UP/DOWN //
  ...
  if (Actions.valide("turn_left") || Actions.valide("turn_right")) {
    if (_NeedReleaseForward) {
      _NeedReleaseForward = false;
      _NextForwardCancelMoveTo = false;
      // if free cam mode reset the view because move if target turned around
      // camera may be
  ...
  if (Actions.valide("toggle_auto_walk")) {
    ...
    if (!_LastFrameAutowalk) {
      // Leave autowalk if forward and backward not pushed and only if autowalk
      // just pushed.
      if ((_DirectionMove & autowalk) && !Actions.valide("forward") &&
          !Actions.valide("backward"))
        _DirectionMove &= ~(autowalk | forward | backward);
      else {
        // Start Autowalk
        _DirectionMove |= autowalk;
  ...
  if (EventsListener.isMouseButtonReleased(leftButton)) {
    if (T1 <=
        _LeftClickEnd + CWidgetManager::getInstance()->getUserDblClickDelay()) {
      dblClickLeft = true;
    }
    _LeftClickEnd = T1;
  }
  ...
  if (_FreeLook) {
    ...
    if (!_MouseCaptured) {
      ...
      if (EventsListener.isMouseButtonReleased(rightButton) &&
          EventsListener.isMouseButtonUp(leftButton)) {
        // Cursor mode.
        SetMouseCursor();
      ...
      if (EventsListener.isMouseButtonReleased(leftButton) &&
          EventsListener.isMouseButtonUp(rightButton)) {
        // Cursor mode.
        SetMouseCursor();
      ...
      if (EventsListener.isMouseButtonDown(rightButton)) {
        ...
        if ((T1 - _RightClickStart) > _TimeBeforeMouseSlide) {
          // if we are in follow mode, do a cameraLook on RightSlide, else do a
          // freeLook
          if (UserEntity->follow() || UserEntity->moveTo() ||
              UserEntity->isSit() || UserEntity->isAFK())
            ...
            if (_NeedReleaseForward) {
              _NeedReleaseForward = false;
              _NextForwardCancelMoveTo = false;
              // if free cam mode reset the view because move if target turned
              // around camera may be
              if (!ClientCfg.AutomaticCamera) {
                resetSmoothCameraDeltaYaw();
              }
            }
      ...
      // if Right Mouse button is up
        ...
        // mouse camera look: when the user hold down the left button for a
        // time, while the right button is up
        if (EventsListener.isMouseButtonDown(leftButton)) {
          // NB: start the freeLook sooner than a longclick
        ...
        if (!UserEntity->isSit() && !UserEntity->isAFK()) {
          // Test rotation keys
          if (UserEntity->canChangeFront()) {
            keyboardRotationLR(Actions.valide("turn_left"),
                               Actions.valide("turn_right"));
          } else {
            keyboardRotationCameraLR(Actions.valide("turn_left"),
                                     Actions.valide("turn_right"));
          }
        }
      }
  ...
  if (!_MouseCaptured) {
    // If both button are pressed in any case (whatever the
    // _TimeBeforeMouseSlide time), treat as "forward"
    static bool bothButtonPressed = false;
    if (EventsListener.isMouseButtonDown(leftButton) &&
        EventsListener.isMouseButtonDown(rightButton)) {
      // Forward.
      _DirectionMove |= forward;
      _DirectionMove &= ~backward;
  ...
  // Manage here the Release of Forward in any of third case (keyUP, leftButton
  // in freeLook, and both buttons down)
  if ((_DirectionMove & forward) && (_DirectionMove & autowalk) == 0) {
    // freeLook() called and leftButton down => walk
    bool walkInFreeLook = _CurrentFrameFreeLookCalled &&
                          EventsListener.isMouseButtonDown(leftButton);
    // both button down => walk too
    bool walkWithBothButton = (EventsListener.isMouseButtonDown(leftButton) &&
                               EventsListener.isMouseButtonDown(rightButton));
  ...
  if (!_MouseCaptured) {
    ...
    if (UserEntity->isSit() || UserEntity->isAFK()) {
      cameraLeft =
          Actions.valide("camera_turn_left") || Actions.valide("turn_left");
      cameraRight =
          Actions.valide("camera_turn_right") || Actions.valide("turn_right");
    } else {
...
void CUserControls::move() {
  ...
  if (!(isSwimming() || isRiding())) {
    ...
    // Check if the mount is able to run, and force walking mode if not, because
    // the player can always switch the mode
...
void CUserControls::testApplyCameraYawToUser() {
  ...
  if (!EventsListener.isMouseButtonDown(leftButton)) {
    ...
    if ((!_LastFrameForward && Actions.valide("forward") &&
         !_NeedReleaseForward) ||
        (!_LastFrameBackward && Actions.valide("backward")) ||
        (!_LastFrameAutowalk && Actions.valide("toggle_auto_walk")))
    /*( !_LastFrameStrafeLeft && Actions.valide("strafe_left") ) ||
  ...
  // This is for the case where right button is pressed while left button was
  // down for cameraLook()
  if (EventsListener.isMouseButtonDown(leftButton) &&
      EventsListener.isMouseButtonDown(rightButton)) {
    applyCameraDeltaYawToUser();
...
// startFreeLook()
//-----------------------------------------------
void CUserControls::startFreeLook() { _FreeLook = true; }
...
void CUserControls::cancelActionsWhenMoving() {
  if (UserEntity->behaviour() == MBEHAV::EXTRACTING ||
      (UserEntity->behaviour() >= MBEHAV::MAGIC_CASTING_BEHAVIOUR_BEGIN &&
       UserEntity->behaviour() <= MBEHAV::MAGIC_CASTING_BEHAVIOUR_END)) {
