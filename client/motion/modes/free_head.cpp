...
void CUserControls::findDestination(float x, float y) {
  ...
  // Teta : abs value of angle between future view heading and current body
  // heading
  _UpdateView = true;
  _Teta = (float)fabs(acos(UserEntity->front() * UserEntity->head()));
  _PrevTeta = _Teta;
...
//-----------------------------------------------
void CUserControls::freeHeadModeStop() {} // freeHeadModeStop //
...
void CUserControls::freeHeadMode() {
  ...
  if (up || down) {
    if (up) {
      ...
      _RotateVerticalVelocity =
          std::max(_RotateVerticalVelocity, ClientCfg.RotKeySpeedMin);
    }

    if (down) {
      accel -= ClientCfg.RotAccel;
      _RotateVerticalVelocity =
          std::min(_RotateVerticalVelocity, -ClientCfg.RotKeySpeedMin);
    }
    updateVelocity(DT, accel, ClientCfg.RotAccel, ClientCfg.RotKeySpeedMax,
                   _RotateVerticalVelocity);
  ...
  if (_ClickMove == false) {
    ...
    if (_Dist >= _PrevDist && _PrevDist != -1) {
      ...
      if (_T0 != 0) {
        if (_Acc == 0) {
          _Acc = accelerationFromLengthCoeff(_V0, remainingDist,
                                             2); // length is twice
        } else                                   // _Acc!=0
        {
          _T = (float)T1 * 0.001f;
      ...
      // amplitude is 5 centimeters at velocity 3.33 (sin+1 is between 0 and
      // 2(->2m))
      _ZOscil = (float)(sin(2 * Pi * 2 * (time - _StartTime) - Pi / 2) + 1) *
                (0.05f * _TransSpeed / (3.333333f * 2));
  ...
  // Set the view.
  View.viewPos(UserEntity->pos() + CVector(0, 0, UserEntity->eyesHeight()) +
               CVector(0, 0, _ZOscil));
