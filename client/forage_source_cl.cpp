...
CForageSourceCL::CForageSourceCL()
    : CFxCL(), _InSceneUserInterface(NULL), _IconFilename(NULL),
      _IsExtractionInProgress(false), _SafeSource(false), _KnowledgeLevel(0),
      _LastExplosionSwitch(0), _ProspectorSlot(255), _ExtraTime(0),
      _InclBonusExtraTime(0), _InitialQuantity(BarNotInit),
      _CurrentQuantity(0) {
...
bool CForageSourceCL::build(const CEntitySheet *sheet) {
  ...
  const CForageSourceSheet *forageSourceSheet =
      dynamic_cast<const CForageSourceSheet *>(sheet);
  if (!forageSourceSheet) {
    nlwarning("Bad sheet %s for forage source",
              sheet ? sheet->Id.toString().c_str() : "NULL");
    return false;
  }
...
void CForageSourceCL::resetVP() {
  ...
  nlinfo("FG: Source: %s FX: %s", _Position.asVector().asString().c_str(),
         fxInst.getPos().toString().c_str());
}
 ...
 */
void CForageSourceCL::updateClipped(const NLMISC::TTime &currentTimeInMs,
                                    CEntityCL *target) {
...
/*
 * Method called each frame to manage the entity after the clipping test if the
 * primitive is visible.
 */
void CForageSourceCL::updateVisible(const NLMISC::TTime &time,
                                    CEntityCL *target) {
  // Update Modifiers
 ...
 */
inline void setBarValue(float &currentValue, uint8 &displayedValue,
                        uint8 &newValue) {
 ...
 */
inline void updateBarValueTime(uint8 &destValue, float &currentValue,
                               uint8 &displayedValue) {
  sint8 diff = (sint8)(destValue - displayedValue);
  if (diff < 0) {
    currentValue =
        std::max((float)destValue, currentValue - DeltaTimeBarPerSec * DT);
 ...
 * Helper for updateVisiblePostPos()
 */
inline void updateBarValue(uint8 &destValue, float &currentValue,
                           uint8 &displayedValue) {
  sint8 diff = (sint8)(destValue - displayedValue);
  if (diff < 0) {
    currentValue =
        std::max((float)destValue, currentValue - DeltaMoveBarPerSec * DT);
    displayedValue = (uint8)currentValue;
  } else if (diff > 0) {
 ...
 */
void CForageSourceCL::updateVisiblePostPos(const NLMISC::TTime &time,
                                           CEntityCL *target) {
  // Update in scene interface
  if (_InSceneUserInterface) {
    float dist =
        (_Position.asVector() - View.currentViewPos() /*UserEntity->pos()*/)
            .norm();

    // Draw the interface is bars received
    bool showIS = (dist < (CLFECOMMON::THRESHOLD_BARS / 1000 -
                           5.0f)); // mustShowInsceneInterface( true );
  ...
  updateBarValueTime(_BarDestValues[FSBTime], _BarCurrentValues[FSBTime],
                     _TimeBar); // Time (slower transition)
  updateBarValue(_BarDestValues[FSBQuantiy], _BarCurrentValues[FSBQuantiy],
                 _QuantityBar);                                         // Qtty
...
void CForageSourceCL::displayInscenePos() {
  nlinfo("FG: Source: %s Inscene: %s % slot: %u",
         _Position.asVector().asString().c_str(),
         _InSceneUserInterface->Position.asString().c_str(),
         (slot() == UserEntity->selection()) ? "SELECTED" : "Unselected",
         slot());
}
 ...
 */
/*void CForageSourceCL::updateVisualPropertyVpb(const NLMISC::TGameCycle
&gameCycle, const sint64 &prop)
{
 ...
 * Update Entity Bars
 */
void CForageSourceCL::updateVisualPropertyBars(
    const NLMISC::TGameCycle & /* gameCycle */, const sint64 &prop) {
  // NB: forage don't use CBarManager for 2 reasons: useless (forage bars exist
  // only through VP), and complicated since updated at each frame on client
  // (because of smooth transition code below)
  ...
  if (_BarDestValues[FSBE] == 0) // Safe source
  {
    ...
    /* // This is disabled coz setFx don't work after entity creation (:TODO:
    try to see why) if (!_SafeSource)
    {
            CEntitySheet *entitySheet = SheetMngr.get(sheetId());
            const CForageSourceSheet *forageSourceSheet = dynamic_cast<const
    CForageSourceSheet*>(entitySheet); if ( forageSourceSheet )
            {
    ...
    */
    ...
    try to see why) if (_SafeSource)
    {
            CEntitySheet *entitySheet = SheetMngr.get(sheetId());
            const CForageSourceSheet *forageSourceSheet = dynamic_cast<const
    CForageSourceSheet*>(entitySheet); if ( forageSourceSheet ) setFx(
    forageSourceSheet->FxFilename );
    }
    */
    _SafeSource = false;
  }
  if (setBarsNow) {
    ...
    setBarValue(_BarCurrentValues[FSBQuantiy], _QuantityBar,
                _BarDestValues[FSBQuantiy]);
    ...
    _BarDestValues[1] = (_InitialQuantity != 0)
                            ? (_CurrentQuantity * 127 / _InitialQuantity)
                            : 0; // Quantity
  }
  ...
  if (!_Instance.empty()) {
    ...
    if (ForageSourceUseUP) {
      ...
      // Link Time to live to user param 1 (127-> 0=bright; 0->
      // 0.9=dark&transparent)
      fxInst.setUserParam(
          0, 1.0f - (((float)_BarDestValues[0]) * (0.9f / 127.0f) + 0.1f));

      // Link E to user param 1 (glow)
      fxInst.setUserParam(1, ((float)(127 - _BarDestValues[3])) / 127.0f);
      // :TODO: Find a way to change fx or fx color. setUserColor does a
      // modulate, so it's not suitable.
      //	if (_SafeSource)
      //		fxInst.setUserColor(CRGBA(255,0,0));

      // Link Quantity to user param 2 (particle quantity)
      fxInst.setUserParam(2,
                          std::min(1.0f, ((float)_CurrentQuantity) /
                                             50.0f)); // map 100% to quantity 50
    }
  }
}
 ...
 */
void CForageSourceCL::updateVisualPropertyOrient(
    const NLMISC::TGameCycle & /* gameCycle */, const sint64 &prop) {
  uint u = (uint)prop;
 ...
 */
void CForageSourceCL::updateVisualPropertyVisualFX(
    const NLMISC::TGameCycle & /* gameCycle */, const sint64 &prop) {
  // Display explosion FX if the switch flag tells us to do it
  uint8 receivedExplosionSwitch = (uint8)((prop & 0x400) >> 10);
  if (receivedExplosionSwitch &&
      (receivedExplosionSwitch != _LastExplosionSwitch)) // bit 10
  {
  ...
  if ((_KnowledgeLevel & 0x80) != 0) {
    ...
    case 1:
      _EntityName = RM_GROUP::toLocalString((RM_GROUP::TRMGroup)index);
      break; // display group as title
    case 2:
      _EntityName = RM_FAMILY::toLocalString((RM_FAMILY::TRMFamily)index);
    ...
    // Set icon (2 and 3: the family index is transmitted (for knowledge 3, used
    // only as icon index))
...
/*
 * Update Entity Name.
 * Interpret the property Name as the sheet id (it's not the usual string id!)
 * of the raw material, when the knowledge is 3.
 */
void CForageSourceCL::updateVisualPropertyName(
    const NLMISC::TGameCycle & /* gameCycle */, const sint64 &prop) {
 ...
 */
void CForageSourceCL::updateVisualPropertyTarget(
    const NLMISC::TGameCycle & /* gameCycle */, const sint64 &prop) {
  sint slot = (sint)prop;
  if (slot != CLFECOMMON::INVALID_SLOT) {
    _ProspectorSlot = slot;
    CEntityCL *prospector =
...
void CForageSourceCL::displayModifiers() {
  ...
  while (it != _HPDisplayed.end()) {
    ...
    if (TimeInSec > (mod.Time + totalDuration)) {
      ...
      string hpModifier = toString("%u ", qttyDelta) +
                          CI18N::get("uittQualityAbbrev") +
                          toString(" %u", qlty);
      double t = TimeInSec - mod.Time;
      // Compute the position for the Modifier.
      CVector pos =
          namePos + CVector(0.0f, 0.0f, 0.3f + (float)t * 1.0f / totalDuration);
      // get the color
      CRGBA color;
      if (mod.Value < 0)
        color = CRGBA(220, 0, 0);
 ...
 * Destructor
 */
CForageSourceCL::~CForageSourceCL() { releaseInSceneInterfaces(); }
