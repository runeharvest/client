...
#define NL_BEGIN_CLASS_STRING_CONVERSION_TABLE(__class, __type)                \
  static const NLMISC::CStringConversion<__class::__type>::CPair               \
      __type##_nl_string_conversion_table[] = {
#define NL_END_CLASS_STRING_CONVERSION_TABLE(__class, __type, __tableName,     \
                                             __defaultValue)                   \
  }                                                                            \
  ;                                                                            \
  NLMISC::CStringConversion<__class::__type> __tableName(                      \
      __type##_nl_string_conversion_table,                                     \
      sizeof(__type##_nl_string_conversion_table) /                            \
          sizeof(__type##_nl_string_conversion_table[0]),                      \
      __class::__defaultValue);
#define NL_CLASS_STRING_CONVERSION_TABLE_ENTRY(__class, val)                   \
  {#val, __class::val},
...
NL_CLASS_STRING_CONVERSION_TABLE_ENTRY(CLoginStateMachine,
...
NL_END_CLASS_STRING_CONVERSION_TABLE(CLoginStateMachine, TState,
                                     StateConversion, st_unknown)
...
NL_END_CLASS_STRING_CONVERSION_TABLE(CLoginStateMachine, TEvent,
                                     EventConversion, ev_unknown)
...
#define SM_EVENT(eventId, stateId)		\
                ...
                { \
                        ...
                        { \
                                COFile outputF;				\
                                string sLog = NLMISC::toString("[%s] %s ->
%s\n", CLoginStateMachine::toString(ev).c_str(),
CLoginStateMachine::toString(_CurrentState).c_str(),
CLoginStateMachine::toString(stateId).c_str()); \
                                if ( outputF.open( getLogDirectory() +
"error_join.log", true, true ) ) \
                                {
\
                                        outputF.serialBuffer(
(uint8*)(&sLog[0]), (uint)sLog.size() ); \
                                        outputF.close();		\
                                }
\
                        } \
...
void CLoginStateMachine::run() {
  ...
  while (_CurrentState != st_end && !isTerminationRequested()) {
    ...
    case st_start:
      ...
      //				// browser test mode
      //				SM_BEGIN_EVENT_TABLE
      //					SM_EVENT(ev_init_done,
      //st_browser_screen); 					SM_EVENT(ev_quit, st_end); 				SM_END_EVENT_TABLE
      //			}
    ...
    case st_auto_login:
      ...
      //					// legacy mode
      //					SM_BEGIN_EVENT_TABLE
      //						SM_EVENT(ev_login_ok,
      //st_check_patch); 						SM_EVENT(ev_quit, st_end); 					SM_END_EVENT_TABLE
      //				}
      break;
    case st_alt_login:
      initAltLogin();
    ...
    case st_check_patch:
      ...
      CLoginProgressPostThread::getInstance().step(
          CLoginStep(LoginStep_PostLogin, "login_step_post_login"));
...
#ifdef RYZOM_BG_DOWNLOADER
      if (isBGDownloadEnabled()) {
        SM_EVENT(ev_patch_needed, st_patch); // no choice for patch content when
                                             // background downloader is used
      } else
#endif
      {
    ...
    case st_connect:
      ...
      //			if (!ClientCfg.TestBrowser)
      //			{
      //				// in test browser mode, the browser is
      //already init 				initWebBrowser();
      //			}
      //
      //			SM_BEGIN_EVENT_TABLE
    ...
    case st_ingame:
      SM_BEGIN_EVENT_TABLE
      // SM_EVENT(ev_chars_received, st_ingame); // ignored for normal login
      // procedure SM_EVENT(ev_no_user_char, st_ingame);   // "
    ...
    case st_leave_shard:
      ...
      // Server Hop part 1: We are not in main loop but still at character
      // selection
      //   => make a hop to the specified server without doing anything with
      //   interface
      // Far TP part 1.1: From the ingame main loop, the admin html box gives us
      // an event ev_connect for the destination shard.
      //   Note: the admin html box is run by
      //   CInputHandlerManager::getInstance()->pumpEvents() in the main loop.
      //   Tip: to see where a co-task is resumed from, just add a breakpoint on
      //   the end of CCoTask::resume().
      CAHManager::getInstance()->runActionHandler("quit_ryzom", NULL, "");

      if (!FarTP.isIngame()) // assumes there is no Far TP starting between char
                             // selection and main loop, see below
      {
        crashLogAddServerHopEvent();

        FarTP.onServerQuitOk(); // don't wait for onServerQuitOK() because the
                                // EGS will no longer send it if it requests a
                                // Far TP during "select char" (not sending
                                // USER_CHAR)
    ...
    case st_enter_far_tp_main_loop:
      ...
      // This is called when CONNECTION:SERVER_QUIT_OK is received (from
      // NetMngr.update() in main loop).
      ...
      break;
    case st_disconnect:
      // Far TP part 2: disconnect from the FS and unload shard-specific data
      // (called from farTPmainLoop())
    ...
    case st_reconnect_fs:
      // Server Hop part 3.1: connect to the new shard (called from
      // globalMenu()) Far TP part 3.1: connect to the new shard (called from
      // farTPmainLoop())
    ...
    case st_reconnect_select_char:
      // Server Hop part 3.2: bypass character selection ui & select the same
      // character. Far TP part 3.2: bypass character selection ui & select the
      // same character.
      //   This is called from farTPmainloop(), when CONNECTION:USER_CHARS is
      //   received.
      if (!FarTP.isReselectingChar()) {
    ...
    case st_exit_global_menu:
      ...
      SM_BEGIN_EVENT_TABLE
      SM_EVENT(ev_global_menu_exited,
               FarTP.isServerHopEmbedded() ? st_reconnect_ready : st_ingame);
      SM_END_EVENT_TABLE
      break;
    case st_reconnect_ready:
      // Far TP part 3.3: send ready.
      //   This is called from farTPmainloop(), when CONNECTION:READY is
      //   received.
 ...
 */
bool CFarTP::requestFarTPToSession(TSessionId sessionId, uint8 charSlot,
  ...
  if (_HookedForEditor) {
    ...
    return FarTP.requestFarTPToSession(
        (TSessionId)0, charSlot, CFarTP::LaunchEditor,
        true); // allow bailing out in case of edition session not reachable
  }

  // call the join session using the session browser interface
  uint32 charId =
      (NetMngr.getLoginCookie().getUserId() << 4) + (0xf & charSlot);
  ...
  try {
    ...
    retryJoinEdit:
      ...
      bool ret = sb.waitOneMessage(
          CSessionBrowserImpl::getMessageName("on_joinSessionResult"));
      ...
      if (sb._LastJoinSessionResult == 2) {
        ...
        sb.scheduleSession(
            charId, TSessionType::st_edit, "", "", TSessionLevel::sl_a,
            /*TAccessType::at_private, */ TRuleType::rt_strict,
            TEstimatedDuration::et_long, 0, TAnimMode(), TRaceFilter(),
            TReligionFilter(), TGuildFilter(), TShardFilter(), TLevelFilter(),
            std::string(), TSessionOrientation(), true, true);
        ret = sb.waitOneMessage(
            CSessionBrowserImpl::getMessageName("on_scheduleSessionResult"));
        if (!ret || sb._LastScheduleSessionResult != 0)
          throw "schedule error";
        sessionId = sb._LastScheduleSessionId;

        // invite the char in the session
        sb.inviteCharacter(charId, sessionId, charId,
                           TSessionPartStatus::sps_edit_invited);
        ret = sb.waitOneMessage(
            CSessionBrowserImpl::getMessageName("on_invokeResult"));
        if (!ret || sb._LastInvokeResult != 0)
        ...
        ret = sb.waitOneMessage(
            CSessionBrowserImpl::getMessageName("on_invokeResult"));
        if (!ret || sb._LastInvokeResult != 0)
          throw "start session error (2), no DSS available";
        // retry to join
        goto retryJoinEdit;
      ...
      // ok, the join is accepted !, the other far TP work is done in the
      // callback
    } break;
    case JoinSession: {
      ...
      if (sb._LastJoinSessionResult == 16) {
        //	#pragma message (NL_LOC_WRN "inform the player that he is banned
        //from the ring")
  ...
  //			if (!HttpClient.sendGetWithCookie(url, "ryzomId",
  //CurrentCookie, toString("charSlot=%u", charSlot))) 				throw 2; 			break; 		case
  //CFarTP::JoinSession: 			if (!HttpClient.sendPostWithCookie(url, "ryzomId",
  //CurrentCookie, toString("sessionId=%u&charSlot=%u", sessionId, charSlot)))
  //				throw 2;
  //			break;
  //		case CFarTP::JoinMainland:
  //			if (!HttpClient.sendPostWithCookie(url, "ryzomId",
  //CurrentCookie, "ml=")) 				throw 2; 			break;
  //		}
  ...
  //		nlinfo("Found server for %ssession %u",
  //joinMode==CFarTP::LaunchEditor ? "editing " : "", sessionId);
  //
  //		// Begin Far TP
  ...
  //	}
    ...
    //); 				if ( httpErrorStr.find( "200 OK" ) == string::npos ) 					httpErrorStr = "
    //(" + httpErrorStr + ")"; 				else 					httpErrorStr.clear();
    //			}
    //		}
    bool requestRetToMainland =
        /*httpErrorStr.empty() &&*/ bailOutIfSessionVanished;
    bool isAttemptingEmbeddedServerHop =
        isReselectingChar() && (joinMode != CFarTP::JoinSession);
    bool letReturnToCharSelect = (!isIngame()) || isAttemptingEmbeddedServerHop;

    // User-friendly message
    CInterfaceManager *pIM = CInterfaceManager::getInstance();
    if (letReturnToCharSelect) {
      //			// Hide all buttons except Quit. If
      //!requestRetToMainland, we will show them back at the end of
      //connectToNewShard(). 			CAHManager::getInstance()->runActionHandler(
      //"proc", NULL, "charsel_disable_buttons" );
      //			CAHManager::getInstance()->runActionHandler(
      //"set", NULL,
      //"target_property=ui:outgame:charsel:quit_but:active|value=1" );

      CInterfaceElement *btnOk =
          CWidgetManager::getInstance()->getElementFromId(
              "ui:outgame:charsel:message_box:ok");
      if (btnOk)
        btnOk->setActive(!requestRetToMainland);

      // Hide the black screen i.e. force showing the interface
      CInterfaceElement *charSelBlackScreen =
          CWidgetManager::getInstance()->getElementFromId(
              "ui:outgame:charsel:black_screen");
      if (charSelBlackScreen) {
        CViewBase *charSelBlackScreenBitmap =
            dynamic_cast<CViewBase *>(charSelBlackScreen);
        if (charSelBlackScreenBitmap)
          charSelBlackScreenBitmap->setAlpha(0);
      }
    }
    pIM->messageBoxWithHelp(
        CI18N::get(requestRetToMainland ? "uiSessionVanishedFarTP"
                                        : "uiSessionUnreachable") +
            errorMsg,
    ...
    string outErrorMsg =
        toString("Could not join %ssession %u: '%s' error\n",
                 joinMode == CFarTP::LaunchEditor ? "editing " : "",
                 sessionId.asInt(), errorMsg);
    ...
    //		do
    //		{
    //			WarningLog->displayRaw( res.substr( pos, 200 ).c_str()
    //); // truncates long strings 			pos += 200;
    //		}
    //		while ( pos < res.size() );
...
asctime(localtime(&currentTime)), outErrorMsg.c_str() ); outputF.serialBuffer(
(uint8*)(&headerS[0]), (uint)headerS.size() );
//				outputF.serialBuffer( (uint8*)(&res[0]),
res.size() ); outputF.close();
            }
    }
    catch (const Exception &)
    {}
    */
    ...
    // If the session is not a permanent session and has vanished, pop the
    // position
    if (requestRetToMainland) {
      ...
      // Show all buttons except 'New character' so that the character can retry
      // entering game or choose another character.
      ...
      CInterfaceGroup *charselGroup = dynamic_cast<CInterfaceGroup *>(
          CWidgetManager::getInstance()->getElementFromId(
              "ui:outgame:charsel"));
      if (charselGroup)
        CAHManager::getInstance()->runActionHandler("proc", charselGroup,
 ...
 */
void CFarTP::hookNextFarTPForEditor() { _HookedForEditor = true; }
...
void CFarTP::requestReconnection() {
  ...
  if (!requestFarTPToSession(TSessionId(std::numeric_limits<uint16>::max()),
                             std::numeric_limits<uint8>::max(),
                             CFarTP::JoinMainland, false))
    _ReselectingChar = false;
}

const char *CFarTP::_URLTable[CFarTP::NbJoinModes] = {
    "edit_session.php", "join_session.php", "join_shard.php"};
...
bool CFarTP::isServerHopInProgress() const {
  ...
  return (state == CLoginStateMachine::st_leave_shard ||
          ...
          state ==
              CLoginStateMachine::st_reconnect_error); // TODO: error handling
}
...
void CFarTP::disconnectFromPreviousShard() {
  ...
  if (isIngame()) {
    ...
    if (SoundMngr) {
      ...
      // SoundMngr->playEventMusic(LoadingMusic,
      // CSoundManager::LoadingMusicXFade, true);
  ...
  if (isIngame()) {
    // Save the R2EDEnabled flag to know if we are switching it when we
    // reconnect
  ...
  if (isReselectingChar()) {
    ...
    // (if some interface stuff has not received its string yet, its remover
    // will get useless)
  ...
  /*
          ...
          When reinit will reset server counters to 0 in the server database,
     onServerChange() will be called and data will be copied since
     CInterfaceManager::_LocalSyncActionCounter==0 (done in
     resetShardSpecificData() above) If we do the resetShardSpecificData()
     after, scenari could arise where Local database could not be reseted
  */
  NetMngr.reinit();
...
void CFarTP::connectToNewShard() {
  ...
  NLNET::CInetHost sbsAddress(
      CSessionBrowserImpl::getInstance().getFrontEndAddress());
  ...
  // The next step will be triggered by the CONNECTION:USER_CHARS msg from the
  // server
}
...
void CFarTP::selectCharAndEnter() {
  ...
  SelectCharMsg.c =
      (uint8)PlayerSelectedSlot; // PlayerSelectedSlot has not been reset
  ...
  */
  ...
  // if (!isIngame())
  //   globalMenu will exit when WaitServerAnswer and serverReceivedReady
  //   (triggered by the CONNECTION:READY msg) are true
  // else
...
void CFarTP::sendReady() {
  if (isReselectingChar()) {
    ...
    // Reset all fx (no need to CTimedFXManager::getInstance().reset() and
    // EntitiesMngr.getGroundFXManager().reset() because the entities have been
    // removed)
    CProjectileManager::getInstance().reset();
    FXMngr.reset(); // (must be done after EntitiesMngr.release())
    EntitiesMngr.getGroundFXManager().init(Scene, ClientCfg.GroundFXMaxDist,
                                           ClientCfg.GroundFXMaxNB,
                                           ClientCfg.GroundFXCacheSize);
    ...
    if (ClientCfg.R2EDEnabled != _PreviousR2EdEnabled) {
      // Reload textures, keys and interface config if we are switch between
      // playing and r2 mode Must be done after receiving the current
      // R2EDEnabled flag (USER_CHAR)
      pIM->loadIngameInterfaceTextures();

      // Unload (and save if leaving normal playing mode) keys and interface
      // config Instead of doing it in disconnectFromPreviousShard(), we do it
      // here, only when it's needed
      ...
      if (!ClientCfg.R2EDEnabled) {
        // Remove all existing keys and load them back, and load new interface
        // config
        pIM->loadKeys();
    ...
    pIM->configureQuitDialogBox(); // must be called after
                                   // waitForUserCharReceived() to know the ring
                                   // config

    ContinentMngr.select(
        UserEntity->pos(),
        ProgressBar); // IMPORTANT : must select continent after ui init,
                      // because ui init also load landmarks (located in the
                      // icfg file) landmarks would be invisible else (RT 12239)
    ...
    ConnectionReadySent = true; // must be called before
...
void CFarTP::onFailure() {
  ...
  Driver->systemMessageBox(("Unable to join shard" + reason).c_str(), "Error",
                           UDriver::okType, UDriver::exclamationIcon);
...
void CFarTP::onDssDown(bool forceReturn) {
  if (!forceReturn) {
    ...
    if (isJoiningShard()) {
      _DSSDown = true; // note: still, some cases will make the client hang or
                       // abort, e.g. DSS down before the client receives the
                       // start pos (in ring shard)
      return;
    }
  }
...
void CFarTP::joinSessionResult(uint32 /* userId */, TSessionId /* sessionId */,
                               uint32 /* result */,
...
void CFarTP::setJoinSessionResult(TSessionId sessionId,
...
void CFarTP::writeSecurityCodeForDisconnection(NLMISC::IStream &msgout) {
  CSecurityCheckForFastDisconnection::forwardSecurityCode(
      msgout, _SessionIdToJoinFast, _SecurityCodeForDisconnection);
}
...
void CFarTP::farTPmainLoop() {
  ...
  // Update network until the end of the FarTP process, before resuming the main
  // loop
  while (!ConnectionReadySent) {
    ...
    if (LoginSM.getCurrentState() ==
      ...
      // This is done outside the co-routine because it may need to co-routine
      // to do an embedded server hop
      if (FarTP.isReselectingChar()) {
        ++nbRecoSelectCharReceived;
        if (nbRecoSelectCharReceived <= 1) {
          ClientCfg.SelectCharacter = -1; // turn off character autoselection
          if (!FarTP
                   .reselectCharacter()) // it should not return here in
                                         // farTPmainLoop() in the same state
                                         // otherwise this would be called twice
            return;
        } else
          nlwarning("Received more than one st_reconnect_select_char event");
      }
    } else if (LoginSM.getCurrentState() ==
               CLoginStateMachine::st_reconnect_ready) {
      // Don't call sendReady() within the cotask but within the main loop, as
      // it contains event/network loops that could trigger a global exit().
      sendReady();
