...
// CAnimationFX
//-----------------------------------------------
CAnimationFX::CAnimationFX(const std::string &psName /* = "" */,
...
// init
//-----------------------------------------------
void CAnimationFX::init(const CAnimationFXSheet *sheet,
...
//-----------------------------------------------
NL3D::UParticleSystemInstance CAnimationFX::createMatchingInstance(
    const float *customUserParams /* = NULL */) const {
  NL3D::UParticleSystemInstance fx;
  nlassert(Sheet != NULL);
  if (Sheet->PSName.empty())
    return NULL;
  ...
  if (!inst.empty()) {
    ...
    if (fx.empty()) {
      nlwarning(
          "Bad shape type for a fxsheet shape : must be a particle system");
...
// init
//-----------------------------------------------
void CAnimationFXSet::init(const CAnimationFXSetSheet *sheet,
                           NL3D::UAnimationSet *as) {
  if (!sheet)
    return;
  Sheet = sheet;
