...
void CInterfaceDDX::CParam::DBToWidget() {
  ...
  if (Widget == ColorButton) {
    ...
    CCDBNodeLeaf *pNL =
        NLGUI::CDBManager::getInstance()->getDbProp(Link, false);
    if (pNL != NULL) {
...
void CInterfaceDDX::CParam::CFGToWidget() {
  ...
  if (Widget == BoolButton) {
    ...
    sint32 step =
        (sint32)(10000 * ClientCfg.readDoubleNoWarning(Link + "_step"));
    CCtrlScroll *pCS = dynamic_cast<CCtrlScroll *>(Elt.getPtr());
    if (pCS != NULL) {
      pCS->setMinMax(nMin, nMax);
      pCS->setStepValue(step);
...
void CInterfaceDDX::CParam::restoreCFGPreset(uint presetVal) {
  ...
  if (Widget == BoolButton) {
    ...
    CConfigFile::CVar *varPtr =
        ClientCfg.ConfigFile.getVarPtr(Link + "_ps" + toString(presetVal));
    if (varPtr) {
...
uint32 CInterfaceDDX::CParam::getPresetPossibleBF() {
  ...
  if (Widget == BoolButton) {
    ...
    for (uint i = 0; i < CInterfaceDDX::NumPreset; i++) {
      CConfigFile::CVar *varPtr =
          ClientCfg.ConfigFile.getVarPtr(Link + "_ps" + toString(i));
      if (varPtr) {
        bool bVal = varPtr->asInt() != 0;
...
bool CInterfaceDDX::parse(xmlNodePtr cur, CInterfaceGroup *parentGroup) {
  ...
  while (cur) {
    ...
    if (stricmp((char *)cur->name, "param") == 0) {
      ...
      if (p.Widget == CParam::ScrollBarInt) {
        ...
        if (!ptrMin)
          ...
          fromString((const char *)ptrMin, p.Min);
        CXMLAutoPtr ptrMax((const char *)xmlGetProp(cur, (xmlChar *)"max"));
        if (!ptrMax)
          p.Max = 255;
        else
          fromString((const char *)ptrMax, p.Max);
      }

      CXMLAutoPtr ptrRealtime(
          (const char *)xmlGetProp(cur, (xmlChar *)"realtime"));
      if (!ptrRealtime)
      ...
      CXMLAutoPtr ptrUIView(
          (const char *)xmlGetProp(cur, (xmlChar *)"ui_view"));
      if (ptrUIView)
        p.ResultView = _Parent->getId() + ":" + string((const char *)ptrUIView);
      CXMLAutoPtr ptrUIUnit(
          (const char *)xmlGetProp(cur, (xmlChar *)"ui_unit"));
      if (ptrUIUnit && ((const char *)ptrUIUnit)[0] != 0)
        p.ResultUnit = CI18N::get((const char *)ptrUIUnit);
      CXMLAutoPtr ptrUIDecimal(
          (const char *)xmlGetProp(cur, (xmlChar *)"ui_decimal"));
      if (ptrUIDecimal)
        fromString((const char *)ptrUIDecimal, p.ResultDecimal);

      // try to get the preset db entry
      if (p.Type == CParam::ConfigFile && bOK) {
        CXMLAutoPtr ptrPreset(
            (const char *)xmlGetProp(cur, (xmlChar *)"preset"));
        if (ptrPreset) {
          p.PresetDB = NLGUI::CDBManager::getInstance()->getDbProp(
              (const char *)ptrPreset, false);
          if (p.PresetDB) {
    ...
    // apply node?
      ...
      CXMLAutoPtr ptrUI((const char *)xmlGetProp(cur, (xmlChar *)"ui"));
      _ApplyButton = dynamic_cast<CCtrlBaseButton *>(
          CWidgetManager::getInstance()->getElementFromId(_Parent->getId(),
                                                          (const char *)ptrUI));
    }
...
void CInterfaceDDX::cancel() {
  ...
  for (i = 0; i < _Parameters.size(); ++i) {
    ...
    if (p.RealTimeMode == CParam::RTModeFalse)
      continue;
