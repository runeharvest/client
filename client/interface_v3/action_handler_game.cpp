...
void beastOrder(const std::string &orderStr, const std::string &beastIndexStr,
...
public:
  void execute(CCtrlBase *pCaller, const std::string & /* sParams */) {
    ...
    static const char *sIdNames[] = {"talk",     "use",    "lift",
                                     "look",     "attack", "invit",
                                     "exchange", "mount",  "train"};
    ...
    for (i = 0; i < numOptions; ++i) {
      pVTM = dynamic_cast<CViewTextMenu *>(
          pGM->getElement(sMenuPath + ":" + sIdNames[i]));
      if (pVTM != NULL)
        pVTM->setGrayed(true);
    }
  }
};
REGISTER_ACTION_HANDLER(CHandlerActiveGameContextMenu,
...
public:
  void execute(CCtrlBase *pCaller, const std::string & /* sParams */) {
    ...
    if (selection && selection->Type == CEntityCL::Player) {
      string name =
          CEntityCL::removeTitleAndShardFromName(selection->getEntityName());
      if (name.empty())
        return;
      CAHManager::getInstance()->runActionHandler("enter_tell", pCaller,
                                                  "player=" + name);
    }
  }
...
// ***************************************************************************
// sendBotChatStart
// helper increment session id and send session id to the server with specified
// message
// ***************************************************************************
...
class CHandlerContextRingSessions : public IActionHandler {
  void execute(CCtrlBase * /* pCaller */, const std::string & /* sParams */) {
    //	do not send start message to server here (not really a bot chage page,
    //but should dissapear
    // when player go away)

    CInterfaceManager *pIM = CInterfaceManager::getInstance();
    CInterfaceElement *pIE = CWidgetManager::getInstance()->getElementFromId(
        "ui:interface:ring_sessions");

    // check if selection is a Ring terminal
    CEntityCL *selection = EntitiesMngr.entity(UserEntity->selection());
    if (selection &&
        selection->sheetId() ==
            NLMISC::CSheetId("object_karavan_device_neutrl_sel.creature")) {
      CVectorD vect1 = selection->pos();
      CVectorD vect2 = UserEntity->pos();

      double distanceSquare =
          pow(vect1.x - vect2.x, 2) + pow(vect1.y - vect2.y, 2);
      ...
      if (distanceSquare > MaxTalkingDistSquare) {
        ...
        if (!pIE->getActive()) {
          CBotChatManager::getInstance()->setCurrPage(
              BotChatPageAll->RingSessions);
        }
      }
    } else {
      if (pIE != NULL)
...
public:
  void execute(CCtrlBase * /* pCaller */, const std::string & /* sParams */) {
    ...
    BotChatPageAll->Trade->setTitle(
        NLMISC::CI18N::get("uiBotChatGuildOptions"));
    CBotChatManager::getInstance()->setCurrPage(BotChatPageAll->Trade);
    // Context help
    contextHelp("inventory");
  }
};
REGISTER_ACTION_HANDLER(CHandlerContextTradeGuildOptions,
...
REGISTER_ACTION_HANDLER(CHandlerContextTradeOutpostBuilding,
...
public:
  void execute(CCtrlBase * /* pCaller */, const std::string & /* sParams */) {
    ...
    BotChatPageAll->Trade->setTitle(
        NLMISC::CI18N::get("uiBotChatGuildRoleMaster"));
    CBotChatManager::getInstance()->setCurrPage(BotChatPageAll->Trade);
    // Context help
    contextHelp("inventory");
  }
};
REGISTER_ACTION_HANDLER(CHandlerContextTradeGuildRoleMaster,
...
public:
  void execute(CCtrlBase * /* pCaller */, const std::string &sParams) {
    ...
    CInterfaceManager *im = CInterfaceManager::getInstance();
    // get flags
    std::string playerGiftNeededDbPath = toString(
        "LOCAL:TARGET:CONTEXT_MENU:MISSIONS_OPTIONS:%d:PLAYER_GIFT_NEEDED",
        intId);
    CCDBNodeLeaf *playerGiftNeeded =
        NLGUI::CDBManager::getInstance()->getDbProp(playerGiftNeededDbPath,
                                                    false);
    if (!playerGiftNeeded)
      return;
    //
    // CBotChatManager::getInstance()->setChosenMissionFlags((uint)
    // missionFlags->getValue8());
...
public:
        ...
        {
                CInterfaceManager *im = CInterfaceManager::getInstance();
                // if there is only a single page available for missions, go to
that page directly CCDBNodeLeaf *progs =
NLGUI::CDBManager::getInstance()->getDbProp("SERVER:TARGET:PROGRAMMES", false);
                if (!progs)
                {
                        nlwarning("<CHandlerContextMissions::execute> can't
retrieve programs."); return;
                }
                bool missionsToDo = (progs->getValue32() & (1 <<
BOTCHATTYPE::AvailableMissionFamily)) != 0; bool missionsToContinue =
(progs->getValue32() & (1 << BOTCHATTYPE::CompletedMissionFamily)) != 0; bool
missionsTofinish = (progs->getValue32() & (1 <<
BOTCHATTYPE::ContinuedMissionFamily)) != 0; if (missionsToDo &&
!missionsToContinue && !missionsTofinish)
                {
                        startBotChatAtPage(BOTCHATTYPE::AvailableMissions);
                }
                else if (!missionsToDo && missionsToContinue &&
!missionsTofinish)
                {
...
public:
        void execute (CCtrlBase *pCaller, const std::string &sParams)
        {
                startBotChatAtPage(BOTCHATTYPE::AvailableMissions); // missions
list page. If only one option, go to this page ?
        }
};
REGISTER_ACTION_HANDLER(CHandlerContextAvailableMissions,
"context_available_missions");
...
public:
        void execute (CCtrlBase *pCaller, const std::string &sParams)
        {
                startBotChatAtPage(BOTCHATTYPE::MissionProgress); // missions
list page. If only one option, go to this page ?
        }
};
REGISTER_ACTION_HANDLER(CHandlerContextContinueMissions,
"context_continue_missions");
...
public:
        void execute (CCtrlBase *pCaller, const std::string &sParams)
        {
                startBotChatAtPage(BOTCHATTYPE::CompletedMissions); // missions
list page. If only one option, go to this page ?
        }
};
REGISTER_ACTION_HANDLER(CHandlerContextFinishMissions,
"context_finish_missions");
*/
...
REGISTER_ACTION_HANDLER(CHandlerContextUnPVPChallenge,
  ...
  void execute(CCtrlBase * /* pCaller */, const std::string & /* sParams */) {
    ...
    if (trader != CLFECOMMON::INVALID_SLOT) {
      ...
      if (entity) {
        ...
        if (!playerName.empty()) {
          PeopleInterraction.askAddContact(playerName,
                                           &PeopleInterraction.FriendList);
        }
      }
    }
  }
};
REGISTER_ACTION_HANDLER(CHandlerContextAddToFriendList,
...
static void chooseSheath (ITEMFAMILY::EItemFamily eIF, string sAllSkills)
{
        // Choose right sheath
        CInterfaceManager *pIM = CInterfaceManager::getInstance();
        CCDBNodeLeaf *pNLwrite =
NLGUI::CDBManager::getInstance()->getDbProp(CWidgetManager::getInstance()->getParser()->getDefine("ui_set_active"));
        CCDBNodeLeaf *pNLread =
NLGUI::CDBManager::getInstance()->getDbProp(CWidgetManager::getInstance()->getParser()->getDefine("set_nb"));
        sint32 nNbSheath = (sint32)pNLread->getValue64();
        if (nNbSheath == 0) return;
        pNLread =
NLGUI::CDBManager::getInstance()->getDbProp(CWidgetManager::getInstance()->getParser()->getDefine("set_active"));
        ...
        for (sint32 i = 0; i < ((nNbSheath/2)+1); ++i)
        {
                for (sint32 j = 0; j < 2; ++j)
                {
                        ...
                        if (j == 0)
                                ...
                                nSheathToTest = nActiveSheath - i;
                        while (nSheathToTest < INVENTORIES::sheath)
nSheathToTest += nNbSheath; while (nSheathToTest >=
(INVENTORIES::sheath+nNbSheath)) nSheathToTest -= nNbSheath;

                        string sPath;
                        sPath =
CWidgetManager::getInstance()->getParser()->getDefine("set_base") + ":" +
NLMISC::toString(nSheathToTest) + ":" +
CWidgetManager::getInstance()->getParser()->getDefine("set_r") + ":SHEET";
                        pNLread =
NLGUI::CDBManager::getInstance()->getDbProp(sPath); sint32 sheetid =
(sint32)pNLread->getValue64(); CItemSheet *pIS = dynamic_cast<CItemSheet
*>(SheetMngr.get(CSheetId(sheetid))); if (pIS != NULL)
                        {
                                if (pIS->Family == eIF)
                                {
                                        // YOYO: Fast search. important because
skill_list is too big for "training" buton case string skillList=
IActionHandler::getParam (sAllSkills, "skill_list"); if( !skillList.empty() )
                                        {
                                                std::vector<string>	strList;
                                                NLMISC::splitString(skillList,
",", strList); for(uint k=0;k<strList.size();k++)
                                                {
                                                        static string
sSkillToTrain; sSkillToTrain= strList[k];
                                                        // remove \n,\t and
spaces while( !sSkillToTrain.empty() && (sSkillToTrain[0]==' ' ||
sSkillToTrain[0]=='\n' || sSkillToTrain[0]=='\t') ) sSkillToTrain.erase(0, 1);
                                                        // If the tool skill
match the skill filter, and if the player know this skill => OK! if
(pIS->Tool.Skill == SKILLS::toSkill(sSkillToTrain) &&
playerKnowSkill(pIS->Tool.Skill) )
                                                        {
                                                                bFound = true;
    ...
    if (UserEntity->selection()) {
      ...
      if (pSel != NULL)
        if (pSel->isForageSource())
          UserEntity->moveToExtractionPhrase(
              UserEntity->selection(), 2.0f, std::numeric_limits<uint>::max(),
              std::numeric_limits<uint>::max(), true);
    }
  }
};
...
public:
  void execute(CCtrlBase * /* pCaller */, const std::string & /* sParams */) {
    ...
    if (PeopleInterraction.TheUserChat.Filter.getTargetGroup() ==
        CChatGroup::guild)
...
public:
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    ...
    for (uint32 i = 0; i < MAX_INVENTORY_ANIMAL; i++) {
      CCDBNodeLeaf *uidProp = NLGUI::CDBManager::getInstance()->getDbProp(
          toString("SERVER:PACK_ANIMAL:BEAST%d:UID", i), false);
      CCDBNodeLeaf *typeProp = NLGUI::CDBManager::getInstance()->getDbProp(
          toString("SERVER:PACK_ANIMAL:BEAST%d:TYPE", i), false);
      if ((uidProp != NULL) &&
          (uidProp->getValue32() == (sint32)sel->dataSetId()) &&
          (typeProp != NULL) &&
          (typeProp->getValue32() == ANIMAL_TYPE::Mount)) {
        beastOrder(
            "mount",
            toString(i + 1)); // why +1 ? : dixit sendAnimalCommand in EGS :
                              // index 0 = all animals, 1 = animal 0 etc
        CAHManager::getInstance()->runActionHandler("animal_target", NULL,
                                                    toString(i + 1));
...
public:
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    ...
    if (ClientCfg.Local) {
      ...
      IngameDbMngr.setProp("Entities:E" + toString(UserEntity->slot()) + ":P" +
                               toString(CLFECOMMON::PROPERTY_MODE),
                           MBEHAV::NORMAL);
      IngameDbMngr.setProp("Entities:E" + toString(UserEntity->slot()) + ":P" +
                               toString(CLFECOMMON::PROPERTY_ENTITY_MOUNTED_ID),
                           CLFECOMMON::INVALID_SLOT);
      IngameDbMngr.setProp("Entities:E" + toString(UserEntity->mount()) + ":P" +
                               toString(CLFECOMMON::PROPERTY_MODE),
                           MBEHAV::NORMAL);
      IngameDbMngr.setProp("Entities:E" + toString(UserEntity->mount()) + ":P" +
                               toString(CLFECOMMON::PROPERTY_RIDER_ENTITY_ID),
                           CLFECOMMON::INVALID_SLOT);
      // Read the database to unseat.
      CEntityCL *mount = EntitiesMngr.entity(UserEntity->mount());
      if (mount) {
        mount->updateVisualProperty(NetMngr.getCurrentServerTick() + 10,
                                    CLFECOMMON::PROPERTY_MODE);
        mount->updateVisualProperty(NetMngr.getCurrentServerTick() + 10,
                                    CLFECOMMON::PROPERTY_RIDER_ENTITY_ID);
      }
      UserEntity->updateVisualProperty(NetMngr.getCurrentServerTick() + 10,
                                       CLFECOMMON::PROPERTY_MODE);
      UserEntity->updateVisualProperty(NetMngr.getCurrentServerTick() + 10,
...
public:
  void execute(CCtrlBase * /* pCaller */, const std::string &sParams) {
    std::string id = getParam(sParams, "id");
    sint idInDb;
    if (!fromString(id, idInDb))
      return;

    CInterfaceManager *pIM = CInterfaceManager::getInstance();

    // get ID
    CCDBNodeLeaf *dbMissionId = NLGUI::CDBManager::getInstance()->getDbProp(
        toString("LOCAL:TARGET:CONTEXT_MENU:MISSION_RING:%d:ID", idInDb),
        false);
    if (!dbMissionId)
      return;
...
public:
  void execute(CCtrlBase * /* pCaller */, const std::string & /* sParams */) {
    ...
    if ((!ConnectionReadySent) && (!FarTP.isLeavingEGS())) {
      ...
      bool bypassDisconnectionTimer =
          FarTP.isFastDisconnectGranted() &&
          (!IsInRingSession); // no need on a ring shard, as it's very short
      out.serial(bypassDisconnectionTimer);
      FarTP.writeSecurityCodeForDisconnection(
          out); // must always be written because of msg.xml (or could have a
                // special handler in the FS)
      NetMngr.push(out);
...
REGISTER_ACTION_HANDLER(CAHCloseFreeTrialQuitting,
...
#define SKILL_PATH "SERVER:CHARACTER_INFO:SKILLS:"
            ...
            CCDBNodeLeaf *nl =
                NLGUI::CDBManager::getInstance()->getDbProp(path, false);
            if (nl) {
              if (nl->getValue32() == 0) {
                canUse = false;
              }
  ...
  /** On the trade windows, display or not : can use item, can build item, can
   * use built item
   */
  void showItemFlags(CInterfaceManager *im, bool canUse, bool canBuild,
                     bool canUseBuiltItem);
};
REGISTER_ACTION_HANDLER(CSelectItemSheet, "select_item");

void CSelectItemSheet::showItemFlags(CInterfaceManager *im, bool canUse,
                                     bool canBuild, bool canUseBuiltItem) {
  if (!im)
    return;
  CInterfaceGroup *gr;
#define BOT_CHAT_TRADE_PATH                                                    \
  "ui:interface:bot_chat_trade:header_opened:trade_content:"
  gr = dynamic_cast<CInterfaceGroup *>(
  ...
  gr = dynamic_cast<CInterfaceGroup *>(
      CWidgetManager::getInstance()->getElementFromId(BOT_CHAT_TRADE_PATH
                                                      "cant_use_built_item"));
  if (gr)
    gr->setActive(!canUseBuiltItem);
  gr = dynamic_cast<CInterfaceGroup *>(
      CWidgetManager::getInstance()->getElementFromId(BOT_CHAT_TRADE_PATH
                                                      "cant_build_item"));
  if (gr)
    gr->setActive(!canBuild);
}
...
class CSetPriceInDB : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string &Params) {
    ...
    if (ls.empty() || ms.empty() || bs.empty() || vbs.empty() ||
        value.empty()) {
      nlwarning("<CSetItemPriceInDB::execute> can't get db address for all "
                "kind of seeds, or can't get value");
      return;
    }
    ...
    if (price.getInteger() >= 0) {
      ...
      NLGUI::CDBManager::getInstance()->getDbProp(ls)->setValue64(
          money.getLS());
      NLGUI::CDBManager::getInstance()->getDbProp(ms)->setValue64(
          money.getMS());
      NLGUI::CDBManager::getInstance()->getDbProp(bs)->setValue64(
          money.getBS());
      NLGUI::CDBManager::getInstance()->getDbProp(vbs)->setValue64(
          money.getVBS());
    } else {
      // undefined price
...
void beastOrder(const std::string &orderStr, const std::string &beastIndexStr,
  ...
  if (beastIndex > MAX_INVENTORY_ANIMAL) {
    nlwarning("<beastOrder> : invalid animal index %d, maximum is %d",
              beastIndex, (int)MAX_INVENTORY_ANIMAL);
    return;
  }
  ...
  if (order == ANIMALS_ORDERS::FREE && confirmFree) {
    ...
    if (beastIndex == 0)
      ...
      pIM->validMessageBox(CInterfaceManager::QuestionIconMsg,
                           CI18N::get("uiQConfirmFreeAnimal"), "do_beast_free",
                           toString(beastIndex));
  }
  // else launch the command
    ...
    if (GenericMsgHeaderMngr.pushNameToStream(msgName, out)) {
      ...
      out.serial(
          u8BeastIndex); // to activate on server side
...
public:
  virtual void execute(CCtrlBase * /* pCaller */, const string &Params) {
    ...
    if (orderStr == "mount") {
      ...
      CInterfaceManager *pIM = CInterfaceManager::getInstance();
      string beastIndex;
      if (CInterfaceExpr::evalAsString(getParam(Params, "beast_index"),
                                       beastIndex)) {
        CAHManager::getInstance()->runActionHandler("animal_target", NULL,
                                                    beastIndex);
      }
      ...
      beastOrder(getParam(Params, "order"), getParam(Params, "beast_index"),
                 true);
  }
};
...
public:
  virtual void execute(CCtrlBase *pCaller, const string & /* Params */) {
    ...
    CViewTextMenu *pFollow =
        dynamic_cast<CViewTextMenu *>(pMenu->getView("follow"));
    CViewTextMenu *pStop =
        dynamic_cast<CViewTextMenu *>(pMenu->getView("stop"));
    CViewTextMenu *pFree =
        dynamic_cast<CViewTextMenu *>(pMenu->getView("free"));
    CViewTextMenu *pEnterStable =
        dynamic_cast<CViewTextMenu *>(pMenu->getView("enter_stable"));
    CViewTextMenu *pLeaveStable =
        dynamic_cast<CViewTextMenu *>(pMenu->getView("leave_stable"));
    CViewTextMenu *pMount =
        dynamic_cast<CViewTextMenu *>(pMenu->getView("mount"));
    CViewTextMenu *pUnseat =
        dynamic_cast<CViewTextMenu *>(pMenu->getView("unseat"));
    ...
    // All Mode
    if (selected == 0) {
      // Test all menu option ('OR': display even if *all* animals don't comply
      // with the commands)
      for (uint i = 0; i < MAX_INVENTORY_ANIMAL; i++) {
        ...
        if (uidProp) {
          ...
          selectedAnimalInVision =
              EntitiesMngr.getEntityByCompressedIndex(datasetIndex);
        }

        // Enable menu items
        testMenuOptionForPackAnimal(selectedAnimalInVision, i, (i == 0),
                                    pFollow, pStop, pFree, pEnterStable,
                                    pLeaveStable, pMount, pUnseat);
      }
    } else if (selected >= 1 && selected <= MAX_INVENTORY_ANIMAL) {
      // Get the entity if it is in vision
      CEntityCL *selectedAnimalInVision = NULL;
      CCDBNodeLeaf *uidProp = NLGUI::CDBManager::getInstance()->getDbProp(
          toString("SERVER:PACK_ANIMAL:BEAST%d:UID", selected - 1), false);
      if (uidProp) {
        CLFECOMMON::TClientDataSetIndex datasetIndex = uidProp->getValue32();
        selectedAnimalInVision =
            EntitiesMngr.getEntityByCompressedIndex(datasetIndex);
...
class CHandlerAnimalTarget : public IActionHandler {
  void execute(CCtrlBase * /* pCaller */, const std::string &sParams) {
    ...
    if (ok) {
      // Get the animal id.
      CLFECOMMON::TClientDataSetIndex entityUid =
          CLFECOMMON::INVALID_CLIENT_DATASET_INDEX;

      if (animalIndex >= 1 && animalIndex <= MAX_INVENTORY_ANIMAL) {
        CCDBNodeLeaf *prop = NLGUI::CDBManager::getInstance()->getDbProp(
            toString("SERVER:PACK_ANIMAL:BEAST%d:UID", animalIndex - 1), false);
        if (prop)
          entityUid = prop->getValue32();
      }
      ...
      if (entityUid != CLFECOMMON::INVALID_CLIENT_DATASET_INDEX) {
        ...
        if (entity)
          ...
          pIM->displaySystemInfo(CI18N::get("uiAnimalSelectNotInVision"),
                                 "CHK");
        }
      }
    }
  ...
  void execute(CCtrlBase * /* pCaller */, const std::string &sParams) {
    ...
    if (ok) {
      if (animalIndex >= 1 && animalIndex <= MAX_INVENTORY_ANIMAL) {
        ...
        CInterfaceElement *group =
            CWidgetManager::getInstance()->getElementFromId(
                toString("ui:interface:inv_pa%d", animalIndex - 1));
...
REGISTER_ACTION_HANDLER(CHandlerAcceptGuildInvitation,
...
REGISTER_ACTION_HANDLER(CHandlerRefuseGuildInvitation,
...
REGISTER_ACTION_HANDLER(CHandlerAcceptPVPChallengeInvitation,
...
REGISTER_ACTION_HANDLER(CHandlerRefusePVPChallengeInvitation,
...
public:
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    if (ClientCfg.Light) {
      vector<string> v;
      CWidgetManager::getInstance()->runProcedure("proc_reset_interface", NULL,
                                                  v);
...
public:
  virtual void execute(CCtrlBase * /* pCaller */, const string &Params) {
    ...
    CAHManager::getInstance()->runActionHandler(
        "browse", NULL, "name=" + helpContainer + ":content:html|url=" + url);
  }
};
...
static bool findInterfacePath(string &sPath, CCtrlBase *pCaller) {
  if (sPath.rfind(':') == string::npos) {
    ...
    if (pCaller != NULL)
      pIE = CWidgetManager::getInstance()->getElementFromId(pCaller->getId(),
                                                            elt);
    else
    ...
    if (pIE == NULL)
      return false;
    sPath =
        pIE->getId() + ":" + sPath.substr(sPath.rfind(':') + 1, sPath.size());
  }
  return true;
}
        ...
        if (inout.empty()) {
          ...
          copyInout =
              STRING_MANAGER::CStringManagerClient::getTitleLocalizedName(
                  CEntityCL::getTitleFromName(copyInout), womanTitle);
        ...
        if (pChar != NULL) {
          ...
          if (pSheet != NULL) {
            ...
            if (strnicmp(sFame.c_str(), "tribe_", 6) == 0) {
              inout =
                  STRING_MANAGER::CStringManagerClient::getFactionLocalizedName(
                      sFame);
...
class CActionHandlerSetTargetName : public IActionHandler {
  virtual void execute(CCtrlBase *pCaller, const string &Params) {
    ...
    if (!findInterfacePath(sNameTarget, pCaller))
      return;
    ...
    if (CInterfaceExpr::eval(sSlot, evValue, NULL)) {
      ...
      if (nSlot > -1) {
        CInterfaceManager *pIM = CInterfaceManager::getInstance();
        //				uint32 nDBid =
        //NLGUI::CDBManager::getInstance()->getDbProp("SERVER:Entities:E"+toString(nSlot)+":P6")->getValue32();
        uint32 nDBid = 0;
        if (nSlot < sint32(EntitiesMngr.entities().size()) &&
            EntitiesMngr.entities()[nSlot] != NULL) {
          nDBid = EntitiesMngr.entities()[nSlot]->getNameId();
        }

        if (nDBid != 0) {
          CSPPRemoveTitleAndCheckSelection *pSPPRT =
              new CSPPRemoveTitleAndCheckSelection;
          pSPPRT->Slot = nSlot;
          CSPPRemoveNameAndCheckSelection *pSPPRN =
...
class CActionHandlerSetTargetForceRegionLevel : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string &Params) {
    ...
    if (sSlot.empty())
      return;
    CInterfaceManager *pIM = CInterfaceManager::getInstance();
    CViewBitmap *pVBR = dynamic_cast<CViewBitmap *>(
        CWidgetManager::getInstance()->getElementFromId(sTargetRegion));
    if (pVBR == NULL)
      return;
    CViewBitmap *pVBL = dynamic_cast<CViewBitmap *>(
    ...
    if (!CInterfaceExpr::eval(sSlot, evValue, NULL))
      return;
    sint32 nSlot = (sint32)evValue.getInteger();
    CCtrlBase *pTooltip = dynamic_cast<CCtrlBase *>(
        CWidgetManager::getInstance()->getElementFromId(
            "ui:interface:target:header_opened:force"));
    ...
    if (pE->isPlayer()) {
      ...
      CCDBNodeLeaf *pDbTargetUid = NLGUI::CDBManager::getInstance()->getDbProp(
          CWidgetManager::getInstance()->getParser()->getDefine("target_uid"));
      if (!pDbTargetUid)
        return;
      ...
      if (!UserEntity ||
          ...
          pE->dataSetId() !=
              (CLFECOMMON::TClientDataSetIndex)pDbTargetUid->getValue32()) {
        pVBL->setTexture(string());
        pVBR->setColor(CRGBA(0, 0, 0, 0));
      ...
      CCharacterSheet *pCS =
          dynamic_cast<CCharacterSheet *>(SheetMngr.get(pE->sheetId()));
    ...
    if (nForceRegion < 1)
      nForceRegion = 1;
    CRGBA col = CInterfaceElement::convertColor(
        CWidgetManager::getInstance()
            ->getParser()
            ->getDefine("region_force_" + toString(nForceRegion))
            .c_str());
    pVBR->setColor(col);
    ...
    if (nLevelForce < 1)
      nLevelForce = 1;
    string sTexture = CWidgetManager::getInstance()->getParser()->getDefine(
        "force_level_" + toString(nLevelForce));
    pVBL->setTexture(sTexture);

    // Set tooltip
    CCtrlBase *tooltip = dynamic_cast<CCtrlBase *>(
        CWidgetManager::getInstance()->getElementFromId(
            "ui:interface:target:header_opened:force"));
    if (tooltip) {
      string str;
...
REGISTER_ACTION_HANDLER(CActionHandlerSetTargetForceRegionLevel,
...
class CAHUpdateCurrentMode : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string &Params) {
    ...
    CInterfaceManager *pIM = CInterfaceManager::getInstance();
    CCDBNodeLeaf *pNL =
        NLGUI::CDBManager::getInstance()->getDbProp(sDBLink, false);
    if (pNL == NULL)
      return;

    CInterfaceExprValue eVal;
    if (!CInterfaceExpr::eval(sValue, eVal, NULL))
      return;

    sint32 nNewMode = (sint32)eVal.getInteger();

    sint32 nModeMinInf, nModeMaxInf, nModeMinLab, nModeMaxLab, nModeMinKey,
        nModeMaxKey;
    fromString(
        CWidgetManager::getInstance()->getParser()->getDefine("mode_min_info"),
        nModeMinInf);
    fromString(
        CWidgetManager::getInstance()->getParser()->getDefine("mode_max_info"),
        nModeMaxInf);
    fromString(
        CWidgetManager::getInstance()->getParser()->getDefine("mode_min_lab"),
        nModeMinLab);
    fromString(
        CWidgetManager::getInstance()->getParser()->getDefine("mode_max_lab"),
        nModeMaxLab);
    fromString(
        CWidgetManager::getInstance()->getParser()->getDefine("mode_min_keys"),
        nModeMinKey);
    fromString(
        CWidgetManager::getInstance()->getParser()->getDefine("mode_max_keys"),
        nModeMaxKey);
    ...
    if (nNewMode != 0)
      if (nNewMode == pNL->getValue32()) {
        ...
        nNewMode++;
        if (nMode == 1) {
          if (nNewMode > nModeMaxInf)
            nNewMode = nModeMinInf;
        } else if (nMode == 2) {
          if (nNewMode > nModeMaxLab)
            nNewMode = nModeMinLab;
        } else if (nMode == 3) {
          if (nNewMode > nModeMaxKey)
            nNewMode = nModeMinKey;
        }
      }
    ...
    if (nMode == 2) {
      bool bFound = false;
      while (!bFound) {
        CCDBNodeLeaf *pIntFlags = NLGUI::CDBManager::getInstance()->getDbProp(
            "SERVER:INTERFACES:FLAGS", false);
        if (pIntFlags == NULL)
          return;
        ...
        fromString(
            CWidgetManager::getInstance()->getParser()->getDefine("mode_magic"),
            tmpMode);
        if (nNewMode == tmpMode)
...
class CAHTarget : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string &Params) {
    ...
    if (!keywords.empty() && keywords[0].size() > 0 && keywords[0][0] == '"') {
      ...
      // search for optional second parameter from old command for
      // prefer_complete_match param
      keywords.clear();
      ...
      // override the value only when there is no 'prefer_complete_match'
      // parameter set
    ...
    if (entity && entity->properties().selectable() &&
        !entity->getDisplayName().empty()) {
      UserEntity->selection(entity->slot());
      if (ClientCfg.TargetChangeCompass) {
        CGroupCompas *gc = dynamic_cast<CGroupCompas *>(
            CWidgetManager::getInstance()->getElementFromId(
                "ui:interface:compass"));
        if (gc) {
          CCompassTarget ct;
...
class CAHTargetLandmark : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string &Params) {
    ...
    if (search.size() > 0 && (search[0] == '\'' || search[0] == '"') &&
        search[0] == search[search.size() - 1]) {
      startsWith = true;
      search = trimQuotes(search);
    }

    const std::string mapid = "ui:interface:map:content:map_content:actual_map";
    CGroupMap *cgMap = dynamic_cast<CGroupMap *>(
...
class CAHAddShape : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string &Params) {
    ...
    while (have_shapes) {
      ...
      sint32 idx;
      CShapeInstanceReference instref = EntitiesMngr.createInstance(
          shape, CVector((float)x, (float)y, (float)z), c, u, false, idx);
      ...
      if (!instance.empty()) {
          ...
          if (!texture_name.empty()) {
            ...
            for (sint l = 0; l < numStages; l++) {
              if (instance.getMaterial(j).isTextureFile((uint)l)) {
                instance.getMaterial(j).setTextureFileName(texture_name,
                                                           (uint)l);
              }
            }
          }
        ...
        instance.setClusterSystem(
            UserEntity
                ->getClusterSystem()); // for simplicity, assume it is in the
        ...
        // if the shape is a particle system, additionnal parameters are user
        // params
        ...
        nlwarning("Command 'add_shape': cannot find the shape %s.",
                  sShape.c_str());
    }
...
class CAHTargetTeammateShortcut : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string &Params) {
    ...
    CCDBNodeLeaf *pNL = NLGUI::CDBManager::getInstance()->getDbProp(
        NLMISC::toString(TEAM_DB_PATH ":%hu:NAME", indexInTeam), false);
    if (!pNL)
      return;
    if (pNL->getValueBool()) {
      // There is a character corresponding to this index
      pNL = NLGUI::CDBManager::getInstance()->getDbProp(
          NLMISC::toString(TEAM_DB_PATH ":%hu:UID", indexInTeam), false);
      if (!pNL)
        return;
      ...
      CEntityCL *entity =
          EntitiesMngr.getEntityByCompressedIndex(compressedIndex);
      if (entity) {
        UserEntity->selection(entity->slot());
      } else {
        CInterfaceManager *pIM = CInterfaceManager::getInstance();
...
class CAHAssist : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string &Params) {
    ...
    if (!entityName.empty()) {
      // Get the entity
      CEntityCL *entity =
          EntitiesMngr.getEntityByName(entityName, false, false);
      if (entity) {
        // Select the entity
...
public:
  ...
  virtual void execute(CCtrlBase * /* pCaller */, const string &Params) {
    ...
    CInterfaceElement *pIE = CWidgetManager::getInstance()->getElementFromId(
        "ui:interface:gestion_windows:close");
    if (pIE != NULL)
      pIE->setActive(false);
    ...
    } else // Key is down
    {
      ...
      } else // Not the first time
      {
        // Show the container
        pGC->setActive(true);
        // Yoyo: important to setTopWindow ONLY if needed, else save of the
        // TopWindow doesn't work when you switch it.
    ...
    if (switchDesktop) {
      ...
      CWidgetManager::getInstance()->runProcedure("tb_setdesktop", NULL,
                                                  vecStr);
    }
  }
...
class CHandlerCloseAllLabosBut : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string &Params) {
    ...
    const char *laboWindows[] = {
        "ui:interface:faber_create", "ui:interface:faber_repair",
        "ui:interface:faber_refine", "ui:interface:combat",
        "ui:interface:magic",        "ui:interface:special_labo",
        "ui:interface:commerce",     "ui:interface:tracking",
    };
    ...
    for (uint i = 0; i < numLabos; i++) {
      ...
      if (Params != laboWindows[i]) {
        CInterfaceElement *pElt =
            CWidgetManager::getInstance()->getElementFromId(laboWindows[i]);
        if (pElt)
...
class CHandlerToggleInventory : public IActionHandler
{
        virtual void execute (CCtrlBase *pCaller, const string &Params)
        {
                ...
                uint	numWins=
sizeof(inventoryWindows)/sizeof(inventoryWindows[0]);

                // For all labos
                bool state = false;
                CInterfaceElement	*pElt=
CWidgetManager::getInstance()->getElementFromId(inventoryWindows[0]); if (pElt)
                        state = !pElt->getActive();
                for(uint i=0;i<numWins;i++)
                {
                        pElt=
CWidgetManager::getInstance()->getElementFromId(inventoryWindows[i]); if(pElt)
...
#define GAME_CONFIG_TREE_LIST                                                  \
  "ui:interface:game_config:content:sbtree:tree_list"

static vector<UDriver::CMode> VideoModes;
#define GAME_CONFIG_VIDEO_MODES_COMBO                                          \
  "ui:interface:game_config:content:general:video_modes"
#define GAME_CONFIG_VIDEO_FREQS_COMBO                                          \
  "ui:interface:game_config:content:general:video_freqs"
#define GAME_CONFIG_VIDEO_FULLSCREEN_BUTTON                                    \
  "ui:interface:game_config:content:general:fullscreen:c"
...
// VR_CONFIG
#define GAME_CONFIG_VR_ENABLE_BUTTON                                           \
  "ui:interface:game_config:content:vr:enabler:c"
#define GAME_CONFIG_VR_DEVICES_COMBO                                           \
  "ui:interface:game_config:content:vr:vr_devices"
#define GAME_CONFIG_VR_DEVICE_DB "UI:TEMP:VR_DEVICE"

// The combo for Texture Mode selected
#define GAME_CONFIG_TEXTURE_MODE_COMBO                                         \
  "ui:interface:game_config:content:general:texture_mode:combo"
...
#define GAME_CONFIG_ANISOTROPIC_COMBO                                          \
  "ui:interface:game_config:content:fx:anisotropic_gr:anisotropic"
#define GAME_CONFIG_ANISOTROPIC_DB "UI:TEMP:ANISOTROPIC"

// Sound driver
#define GAME_CONFIG_SOUND_DRIVER_COMBO                                         \
  "ui:interface:game_config:content:sound:driver_gr:driver"
#define GAME_CONFIG_SOUND_DRIVER_DB "UI:TEMP:SOUND_DRIVER"

// The 3 possible modes editable (NB: do not allow client.cfg HDEntityTexture==1
// and DivideTextureSizeBy2=2
enum TTextureMode {
  LowTextureMode = 0,
  NormalTextureMode = 1,
  HighTextureMode = 2
  ...
  if (pCB) {
    ...
    if (enable) {
      ...
      for (uint i = 0; i < VRDeviceCache.size(); ++i) {
        ...
        displayname << std::string("[") << VRDeviceCache[i].first << "] ["
                    << VRDeviceCache[i].second << "]";
        pCB->addText(displayname.str());
        if (ClientCfg.VRDisplayDevice == VRDeviceCache[i].first) {
          if (selectedDevice == -1 ||
              ClientCfg.VRDisplayDeviceId == VRDeviceCache[i].second) {
            selectedDevice = i;
          }
        }
      }
      if (selectedDevice == -1) {
        ...
        std::stringstream displayname;
        displayname << std::string("[") << ClientCfg.VRDisplayDevice << "] ["
                    << ClientCfg.VRDisplayDeviceId << "] [DEVICE NOT FOUND]";
        pCB->addText(displayname.str());
        selectedDevice = VRDeviceCache.size();
      }
      NLGUI::CDBManager::getInstance()
          ->getDbProp(GAME_CONFIG_VR_DEVICE_DB)
          ->setValue32(-1);
      NLGUI::CDBManager::getInstance()
          ->getDbProp(GAME_CONFIG_VR_DEVICE_DB)
...
void updateSoundDriverComboUI() {
  CDBGroupComboBox *pCB = dynamic_cast<CDBGroupComboBox *>(
      CWidgetManager::getInstance()->getElementFromId(
          GAME_CONFIG_SOUND_DRIVER_COMBO));
  if (!pCB) {
    nlwarning("Sound driver UI element '%s' not found",
              GAME_CONFIG_SOUND_DRIVER_COMBO);
    return;
  }
  ...
  std::vector<NLSOUND::UAudioMixer::TDriverInfo> drivers =
      NLSOUND::UAudioMixer::getDrivers();
  ...
  if (!found)
    drivers.insert(drivers.begin(), 1,
                   NLSOUND::UAudioMixer::TDriverInfo(
                       NLSOUND::UAudioMixer::DriverAuto, "Auto"));
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp(GAME_CONFIG_SOUND_DRIVER_DB)
      ->setValue32(-1);
  NLGUI::CDBManager::getInstance()
      ->getDbProp(GAME_CONFIG_SOUND_DRIVER_DB)
      ->setValue32(selected);
...
public:
  ...
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    ...
    getRyzomModes(VideoModes, stringModeList, stringFreqList, nFoundMode,
                  nFoundFreq);
    ...
    // frequencies
    pCB = dynamic_cast<CDBGroupComboBox *>(
        CWidgetManager::getInstance()->getElementFromId(
            GAME_CONFIG_VIDEO_FREQS_COMBO));
    if (pCB) {
      pCB->resetTexts();
    ...
    NLGUI::CDBManager::getInstance()
        ->getDbProp(GAME_CONFIG_VIDEO_MODE_DB)
        ->setValue32(-1);
    NLGUI::CDBManager::getInstance()
        ->getDbProp(GAME_CONFIG_VIDEO_MODE_DB)
    ...
    NLGUI::CDBManager::getInstance()
        ->getDbProp(GAME_CONFIG_VIDEO_FREQ_DB)
        ->setValue32(-1);
    NLGUI::CDBManager::getInstance()
        ->getDbProp(GAME_CONFIG_VIDEO_FREQ_DB)
        ->setValue32(nFoundFreq);

    CCtrlBaseButton *pBut = dynamic_cast<CCtrlBaseButton *>(
        CWidgetManager::getInstance()->getElementFromId(
            GAME_CONFIG_VIDEO_FULLSCREEN_BUTTON));
    if (pBut) {
      pBut->setPushed(!ClientCfg.Windowed);
    }
    CAHManager::getInstance()->runActionHandler(
        "game_config_change_vid_fullscreen", NULL);
    ...
    pCB = dynamic_cast<CDBGroupComboBox *>(
        CWidgetManager::getInstance()->getElementFromId(
            GAME_CONFIG_TEXTURE_MODE_COMBO));
    if (pCB) {
      pCB->resetTexts();
    ...
    // -1 is important to indicate we set this value in edit mode
    NLGUI::CDBManager::getInstance()
        ->getDbProp(GAME_CONFIG_ANISOTROPIC_DB)
        ->setValue32(-1);
    NLGUI::CDBManager::getInstance()
        ->getDbProp(GAME_CONFIG_ANISOTROPIC_DB)
    ...
    // VR_CONFIG
    pBut = dynamic_cast<CCtrlBaseButton *>(
        CWidgetManager::getInstance()->getElementFromId(
            GAME_CONFIG_VR_ENABLE_BUTTON));
    if (pBut) {
    ...
    // -1 is important to indicate we set this value in edit mode
    NLGUI::CDBManager::getInstance()
        ->getDbProp(GAME_CONFIG_TEXTURE_MODE_DB)
        ->setValue32(-1);
    NLGUI::CDBManager::getInstance()
        ->getDbProp(GAME_CONFIG_TEXTURE_MODE_DB)
        ->setValue32(texMode);
    ...
    pCB = dynamic_cast<CDBGroupComboBox *>(
        CWidgetManager::getInstance()->getElementFromDefine(
            "game_config_screen_ratio_cb"));
    if (pCB) {
      // Bkup for cancel
      ...
      NLGUI::CDBManager::getInstance()
          ->getDbProp("UI:TEMP:SCREEN_RATIO_MODE")
          ->setValue32(-1);
    ...
    NLGUI::CGroupTree *configTree = dynamic_cast<CGroupTree *>(
...
class CHandlerGameConfigMode : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    ...
    sint oldVideoMode = NLGUI::CDBManager::getInstance()
                            ->getDbProp(GAME_CONFIG_VIDEO_MODE_DB)
                            ->getOldValue32();
    sint nVideModeNb = NLGUI::CDBManager::getInstance()
                           ->getDbProp(GAME_CONFIG_VIDEO_MODE_DB)
                           ->getValue32();
    if (nVideModeNb == -1 || oldVideoMode == -1)
      return;
    ...
    for (i = 0; i < (sint)VideoModes.size(); ++i) {
      if ((VideoModes[i].Width == w) && (VideoModes[i].Height == h) &&
          (VideoModes[i].Depth == GAME_CONFIG_VIDEO_DEPTH_REQ)) {
        bool bFound = false;
...
class CHandlerGameConfigLanguage : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    ...
    sint old = NLGUI::CDBManager::getInstance()
                   ->getDbProp(GAME_CONFIG_LANGUAGE)
                   ->getOldValue32();
    sint newOne = NLGUI::CDBManager::getInstance()
                      ->getDbProp(GAME_CONFIG_LANGUAGE)
                      ->getValue32();
    if ((old != -1) && (newOne != -1)) {
...
REGISTER_ACTION_HANDLER(CHandlerGameConfigLanguage,
...
class CHandlerGameConfigFreq : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    CInterfaceManager *pIM = CInterfaceManager::getInstance();
    sint oldFreq = NLGUI::CDBManager::getInstance()
                       ->getDbProp(GAME_CONFIG_VIDEO_FREQ_DB)
                       ->getOldValue32();
    sint newFreq = NLGUI::CDBManager::getInstance()
                       ->getDbProp(GAME_CONFIG_VIDEO_FREQ_DB)
                       ->getValue32();
...
public:
  virtual void execute(CCtrlBase * /* pCalller */,
                       const string & /* Params */) {
    CInterfaceManager *pIM = CInterfaceManager::getInstance();
    sint oldTMode = NLGUI::CDBManager::getInstance()
                        ->getDbProp(GAME_CONFIG_TEXTURE_MODE_DB)
                        ->getOldValue32();
    sint newTMode = NLGUI::CDBManager::getInstance()
                        ->getDbProp(GAME_CONFIG_TEXTURE_MODE_DB)
                        ->getValue32();
...
REGISTER_ACTION_HANDLER(CHandlerGameConfigTextureMode,
...
REGISTER_ACTION_HANDLER(CHandlerGameConfigFullscreen,
...
REGISTER_ACTION_HANDLER(CHandlerGameConfigVREnable,
...
class CHandlerGameConfigVRDevice : public IActionHandler {
  virtual void execute(CCtrlBase *pCaller, const string & /* Params */) {
    ...
    sint oldDevice = NLGUI::CDBManager::getInstance()
                         ->getDbProp(GAME_CONFIG_VR_DEVICE_DB)
                         ->getOldValue32();
    sint newDevice = NLGUI::CDBManager::getInstance()
                         ->getDbProp(GAME_CONFIG_VR_DEVICE_DB)
                         ->getValue32();
...
REGISTER_ACTION_HANDLER(CHandlerGameConfigVRDevice,
                        "game_config_change_vr_device");

// ***************************************************************************
class CHandlerGameConfigSoundDriver : public IActionHandler {
  virtual void execute(CCtrlBase *pCaller, const string & /* Params */) {
    sint oldDriver = NLGUI::CDBManager::getInstance()
                         ->getDbProp(GAME_CONFIG_SOUND_DRIVER_DB)
                         ->getOldValue32();
    sint newDriver = NLGUI::CDBManager::getInstance()
                         ->getDbProp(GAME_CONFIG_SOUND_DRIVER_DB)
                         ->getValue32();
    ...
    if (oldDriver != -1 && newDriver != oldDriver) {
      CDBGroupComboBox *pCB = dynamic_cast<CDBGroupComboBox *>(
          CWidgetManager::getInstance()->getElementFromId(
              GAME_CONFIG_SOUND_DRIVER_COMBO));
      if (!pCB) {
        nlwarning("Unable to change sound driver. UI '%s' not found.",
                  GAME_CONFIG_SOUND_DRIVER_COMBO);
        return;
      }
...
REGISTER_ACTION_HANDLER(CHandlerGameConfigSoundDriver,
                        "game_config_change_sound_driver");
...
class CHandlerGameConfigApply : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    ...
    sint nVideModeNb = NLGUI::CDBManager::getInstance()
                           ...
                           ->getValue32();
    if (nVideModeNb != -1) {
      sint nVideoFreqNb = NLGUI::CDBManager::getInstance()
                              ->getDbProp(GAME_CONFIG_VIDEO_FREQ_DB)
                              ->getValue32();
      if (nVideoFreqNb != -1) {
        // Get W, H
        ...
        string name;
        {
          ...
          if (pCB != NULL) {
            ...
            fromString(tmp, w);
            tmp =
                vidModeStr.substr(vidModeStr.find('x') + 2, vidModeStr.size());
            fromString(tmp, h);
    ...
    if (ClientCfg.VREnable) {
      // store the new config variables
      sint deviceIdx = NLGUI::CDBManager::getInstance()
                           ->getDbProp(GAME_CONFIG_VR_DEVICE_DB)
                           ->getValue32();
      ...
      ClientCfg.writeString("VRDisplayDeviceId",
                            VRDeviceCache[deviceIdx].second);
    }

    sint sndDriver = NLGUI::CDBManager::getInstance()
                         ->getDbProp(GAME_CONFIG_SOUND_DRIVER_DB)
                         ->getValue32();
    if (sndDriver != -1) {
      CDBGroupComboBox *pCB = dynamic_cast<CDBGroupComboBox *>(
    ...
    sint nTexMode = NLGUI::CDBManager::getInstance()
                        ->getDbProp(GAME_CONFIG_TEXTURE_MODE_DB)
                        ->getValue32();
    if (nTexMode >= 0 && nTexMode <= HighTextureMode) {
      if ((ClientCfg.DivideTextureSizeBy2 != (nTexMode == LowTextureMode)) ||
        ...
        ClientCfg.HDEntityTexture = nTexMode == HighTextureMode;
        ClientCfg.writeInt("DivideTextureSizeBy2",
                           ClientCfg.DivideTextureSizeBy2);
    ...
    sint nAnisotropic = NLGUI::CDBManager::getInstance()
                            ->getDbProp(GAME_CONFIG_ANISOTROPIC_DB)
                            ->getValue32();
    ...
    // *** Apply the Screen AR
    // since already set in the config file, need only to bkup the current
    // version
    ...
    CNPCIconCache::getInstance().setEnabled(
        !ClientCfg.R2EDEnabled &&
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:SAVE:INSCENE:FRIEND:MISSION_ICON")
            ->getValueBool());
...
class CHandlerGameConfigCancel : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    ...
    //	- Video Modes changes are only really validated at apply times so cancel
    //=> noop
    //	- All other standard changes are handled upper by ddx_cancel AH called

    // restore the bkuped Screen AR and invalidate clientcfg (NB: don't need to
    // save, since canceled)
...
class CHandlerGameConfigChangeScreenRatioMode : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    if (CInterfaceLink::isUpdatingAllLinks())
      return; // don't want to trash the value in client.cfg at init, due to
              // 'updateAllLinks' being called

    CInterfaceManager *pIM = CInterfaceManager::getInstance();

    // get the current mode
    sint oldMode = NLGUI::CDBManager::getInstance()
                       ->getDbProp("UI:TEMP:SCREEN_RATIO_MODE")
                       ->getOldValue32();
    sint mode = NLGUI::CDBManager::getInstance()
                    ->getDbProp("UI:TEMP:SCREEN_RATIO_MODE")
                    ->getValue32();
    if (mode < 0 || mode > 3)
      return;
    ...
    if (mode != 2) {
      switch (mode) {
      case 0:
        ClientCfg.ScreenAspectRatio = 1.33333f;
        break;
      case 1:
        ClientCfg.ScreenAspectRatio = 1.77777f;
        break;
      case 3:
        ClientCfg.ScreenAspectRatio = 0.f;
        break;
      }
      ClientCfg.writeDouble("ScreenAspectRatio", ClientCfg.ScreenAspectRatio);
...
REGISTER_ACTION_HANDLER(CHandlerGameConfigChangeScreenRatioMode,
...
class CHandlerGameConfigChangeAnisotropic : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    ...
    sint oldAnisotropic = NLGUI::CDBManager::getInstance()
                              ->getDbProp(GAME_CONFIG_ANISOTROPIC_DB)
                              ->getOldValue32();
    sint anisotropic = NLGUI::CDBManager::getInstance()
                           ->getDbProp(GAME_CONFIG_ANISOTROPIC_DB)
                           ->getValue32();

    // dirt the apply button of the DDX.
    // don't do it at init!
    if (oldAnisotropic != anisotropic && oldAnisotropic != -1 &&
        anisotropic != -1) {
      CDDXManager *pDM = CDDXManager::getInstance();
      CInterfaceDDX *pDDX = pDM->get(GAME_CONFIG_DDX);
      if (pDDX)
...
REGISTER_ACTION_HANDLER(CHandlerGameConfigChangeAnisotropic,
...
REGISTER_ACTION_HANDLER(CHandlerGameConfigChangeScreenRatioCustom,
...
class CHandlerSetInterfaceScale : public IActionHandler {
  virtual void execute(CCtrlBase *pCaller, const string &Params) {
    ...
    if (!s.empty()) {
      ...
      if (nlstricmp(s, "auto") == 0 || s == "0") {
        ...
        if (fromString(s, scale)) {
          if (scale >= ClientCfg.InterfaceScale_min &&
              scale <= ClientCfg.InterfaceScale_max) {
            valid = true;
    ...
    string help =
        "/setuiscale " + toString("%.1f .. %.1f", ClientCfg.InterfaceScale_min,
                                  ClientCfg.InterfaceScale_max);
...
REGISTER_ACTION_HANDLER(CHandlerGameGroupMissionAbandon,
...
REGISTER_ACTION_HANDLER(CHandlerContextChooseZoneCharge,
...
REGISTER_ACTION_HANDLER(CHandlerContextChooseBuilding,
...
public:
  void execute(CCtrlBase * /* pCaller */, const std::string & /* sParams */) {
    ...
    sendBotChatStart("START_CHOOSE_MISSION");
    BotChatPageAll->ChooseMission->setMissionClientType(
        MISSION_DESC::RMUpgrade);
    CBotChatManager::getInstance()->setCurrPage(BotChatPageAll->ChooseMission);
  }
};
...
REGISTER_ACTION_HANDLER(CHandlerContextCancelZoneCharge,
...
REGISTER_ACTION_HANDLER(CHandlerContextDestroyBuilding,
...
// ***************************************************************************
// Common code
static void fillPlayerBarText(std::string &str, const string &dbScore,
                              SCORES::TScores score, const string &ttFormat) {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  CCDBNodeLeaf *node;
  ...
  node = NLGUI::CDBManager::getInstance()->getDbProp(
      "UI:VARIABLES:USER:" + dbScore, false);
  if (node)
    val = node->getValue32();
  // less accurate/speed Max transferred by DB
  node = NLGUI::CDBManager::getInstance()->getDbProp(
      toString("SERVER:CHARACTER_INFO:SCORES%d:", score), false);
  if (node)
...
public:
  void execute(CCtrlBase * /* pCaller */, const std::string &sParams) {
    ...
    sint32 maxVal = 0;
    CCDBNodeLeaf *node =
        NLGUI::CDBManager::getInstance()->getDbProp(dbMax, false);
    if (node)
      maxVal = node->getValue32();
...
uint32 getMissionTitle(sint32 nSelected) {
  ...
  fromString(
      CWidgetManager::getInstance()->getParser()->getDefine("ipj_nb_mission"),
      nNbMission);
  fromString(CWidgetManager::getInstance()->getParser()->getDefine(
                 "ipj_nb_group_mission"),
             nNbGroupMission);
  ...
  if (nSelected < 0)
    ...
    return NLGUI::CDBManager::getInstance()
        ->getDbProp("SERVER:MISSIONS:" + toString(nSelected) + ":TITLE")
        ->getValue32();
  else if (nSelected < (nNbMission + nNbGroupMission))
    return NLGUI::CDBManager::getInstance()
        ->getDbProp("SERVER:GROUP:MISSIONS:" +
                    toString(nSelected - nNbMission) + ":TITLE")
        ->getValue32();
  return 0;
}
...
void runMissionProc(sint32 nSelected) {
  ...
  if (nSelected < 0)
    ...
    CCtrlButton *pCB = dynamic_cast<CCtrlButton *>(
        CWidgetManager::getInstance()->getElementFromId(sButtonPath));
    CAHManager::getInstance()->runActionHandler(
        "proc", pCB, "mission_proc_title|" + toString(nSelected));
  } else if (nSelected < (nNbMission + nNbGroupMission)) {
    string sButtonPath =
        UI_MISSION_LIST ":b_group_title" + toString(nSelected - nNbMission);
    CCtrlButton *pCB = dynamic_cast<CCtrlButton *>(
        CWidgetManager::getInstance()->getElementFromId(sButtonPath));
    CAHManager::getInstance()->runActionHandler(
        "proc", pCB,
        "group_mission_proc_title|" + toString(nSelected - nNbMission));
  }
  return;
}
...
public:
  void execute(CCtrlBase * /* pCaller */, const std::string & /* sParams */) {
    CInterfaceManager *pIM = CInterfaceManager::getInstance();
    sint32 nSelected = NLGUI::CDBManager::getInstance()
    ...
    // If no mission selected or title selected becomes invalid -> search a new
    // title to select
    if ((nSelected == -1) || (getMissionTitle(nSelected) == 0)) {
      ...
      // Ok not found a mission in backward mode try now in forward mode until
...
REGISTER_ACTION_HANDLER(CHandlerMissionChooseNextValid,
...
public:
  void execute(CCtrlBase * /* pCaller */, const std::string &sParams) {
    ...
    CCDBNodeLeaf *pNL = NLGUI::CDBManager::getInstance()->getDbProp(
        "UI:TEMP:MISSION_WAKE_BUTTON", false);
    if (pNL != NULL)
      pNL->setValue64(0);
...
public:
  void execute(CCtrlBase * /* pCaller */, const std::string & /* sParams */) {
    if (UserEntity->selection()) {
      CEntityCL *pSel = EntitiesMngr.entity(UserEntity->selection());
      if (pSel != NULL)
        UserEntity->moveToTotemBuildingPhrase(
            UserEntity->selection(), 2.0f, std::numeric_limits<uint>::max(),
            std::numeric_limits<uint>::max(), true);
    }
  }
...
public:
  void execute(CCtrlBase * /* pCaller */, const std::string &sParams) {
    ...
    bool isCiv = getParam(sParams, "type") == "civ" ||
                 getParam(sParams, "type") == "civ_guild";
    bool isGuild = getParam(sParams, "type") == "civ_guild" ||
                   getParam(sParams, "type") == "cult_guild";
    ...
    if (force) {
      ...
      if (isGuild) {
        pIM->validMessageBox(
            CInterfaceManager::WarningIconMsg,
...
REGISTER_ACTION_HANDLER(CHandlerConfigureQuitDialogBox,
...
public:
  void execute(CCtrlBase * /* pCaller */, const std::string &sParams) {
    ...
    MBEHAV::EBehaviour behavToSend =
        (MBEHAV::EBehaviour)(MBEHAV::EMOTE_BEGIN + behaviour);
    uint16 phraseNbToSend = (uint16)phraseNb;

    if (EAM) {
      const uint nbBehav = EAM->getNbEmots(); // Miscalled: this is the number
                                              // of behaviour for all emotes
    ...
    if (sCustomPhrase.empty()) {
      ...
      if (GenericMsgHeaderMngr.pushNameToStream(msgName, out)) {
        ...
        NetMngr.push(out);
        // nlinfo("impulseCallBack : %s %d %d sent", msgName.c_str(),
        // (uint32)behavToSend, phraseNbToSend);
      } else
      ...
      if (GenericMsgHeaderMngr.pushNameToStream(msgName, out)) {
        ...
        if (sCustomPhrase == "none") {
          ...
          ucstr = ucstring("&EMT&") + UserEntity->getDisplayName() +
                  ucstring(" ") + ucstr; // FIXME: UTF-8 (serial)
        }
        ...
        NetMngr.push(out);
        // nlinfo("impulseCallBack : %s %d %s sent", msgName.c_str(),
        // (uint32)behavToSend, sCustomPhrase.c_str());
      } else
        nlwarning("command 'emote': unknown message named '%s'.", msgName);
    }
  }
...
public:
  void execute(CCtrlBase * /* pCaller */, const std::string & /* sParams */) {
    CGroupList *list = dynamic_cast<CGroupList *>(
        CWidgetManager::getInstance()->getElementFromId(
            "ui:interface:fame:content:tribes:list"));
    if (list && list->getNumChildren() > 1) {
      ...
      for (uint i = 0; i < nbChilds; ++i) {
        CInterfaceGroup *pIG =
            dynamic_cast<CInterfaceGroup *>(list->getChild(i));
        if (!pIG)
          break;
      ...
      std::locale loc("");
      const std::collate<char> &coll = std::use_facet<std::collate<char>>(
          loc); // FIXME: Probably does not work
      ...
      for (uint i = 0; i < nbChilds - 1; ++i) {
        ...
        for (uint j = i; j < nbChilds; j++) {
          ...
          if (coll.compare(names[j].c_str(), names[j].c_str() + names[j].size(),
                           names[imin].c_str(),
                           names[imin].c_str() + names[imin].size()) < 0) {
            imin = j;
          }
        }
        if (imin != i) {
