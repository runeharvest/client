...
void CSBrickManager::init() {
  ...
  for (CSheetManager::TEntitySheetMap::const_iterator it(sheetMap.begin()),
       end(sheetMap.end());
       it != end; ++it) {
    // it's a brick?
    CSBrickSheet *br = dynamic_cast<CSBrickSheet *>(
        it->second
            .EntitySheet); // TODO: Avoid dynamic_cast, depend on getSheetType
    if (br) {
      // ok, add it
  ...
  // Process Bricks
  if (_BrickVector.empty())
    return;

  // Build the vector of family bit fields, and the vector of existing bricks
  for (std::vector<CSBrickSheet *>::iterator itb(_BrickVector.begin()),
       endb(_BrickVector.end());
       itb != endb; ++itb) {
  ...
  // Since _SheetsByFamilies is a vector of vector, avoid long reallocation by
  // building it in 2 pass
  sint32 shidc = -1;
  for (std::vector<CSBrickSheet *>::iterator itb(_BrickVector.begin()),
       endb(_BrickVector.end());
       itb != endb; ++itb) {
    ...
    // Resize our vectors if necessary
    // The index in familly must be decremented because the values start at 1 in
    // the sheets
    if (brickSheet->IndexInFamily < 1) {
      nlwarning("CSBrickManager::CSBrickManager(): Reading file: %s: "
                "IndexInFamily==0 but should be >=1 - entry ignored",
                brickSheet->Id.toString().c_str());
      continue;
    }
    if (_NbBricksPerFamily[brickSheet->BrickFamily] <
        (uint)(brickSheet->IndexInFamily)) {
      _SheetsByFamilies[brickSheet->BrickFamily].resize(
          brickSheet->IndexInFamily);
      _NbBricksPerFamily[brickSheet->BrickFamily] = brickSheet->IndexInFamily;
    }
    _SheetsByFamilies[brickSheet->BrickFamily][brickSheet->IndexInFamily - 1] =
        brickSheet->Id;
  }
...
void CSBrickManager::initInGame() {
  ...
  NLGUI::CDBManager::getInstance()->addBranchObserver("SERVER:BRICK_FAMILY",
                                                      &_BrickFamilyObs);
}
...
void CSBrickManager::makeRoots() {
  ...
  for (std::vector<CSBrickSheet *>::size_type ib = 0; ib < _BrickVector.size();
       ++ib) {
...
// ***************************************************************************
const std::vector<NLMISC::CSheetId> &
CSBrickManager::getFamilyBricks(uint family) const {
...
void CSBrickManager::checkBricks() {
  for (std::vector<CSBrickSheet *>::size_type ib = 0; ib < _BrickVector.size();
    ...
    if (brickSheet->ParameterFamilies.size() > CDBGroupBuildPhrase::MaxParam) {
      nlwarning(
          "The Sheet %s has too many parameters for Client Composition: %d/%d",
          brickSheet->Id.toString().c_str(),
          brickSheet->ParameterFamilies.size(), CDBGroupBuildPhrase::MaxParam);
...
// ***************************************************************************
sint32
CSBrickManager::CBrickContainer::getSabrinaCost(NLMISC::CSheetId id) const {
...
// ***************************************************************************
float CSBrickManager::CBrickContainer::getSabrinaRelativeCost(
    NLMISC::CSheetId id) const {
  // get the true container
  CSBrickManager *pBM = CSBrickManager::getInstance();
  CSBrickSheet *brick = pBM->getBrick(id);
  if (brick) {
...
// ***************************************************************************
sint32
CSBrickManager::CBrickContainer::getNumParameters(NLMISC::CSheetId id) const {
  CSBrickManager *pBM = CSBrickManager::getInstance();
  CSBrickSheet *brick = pBM->getBrick(id);
  if (brick) {
...
// ***************************************************************************
BRICK_TYPE::EBrickType
CSBrickManager::CBrickContainer::getBrickType(NLMISC::CSheetId id) const {
  CSBrickManager *pBM = CSBrickManager::getInstance();
  CSBrickSheet *brick = pBM->getBrick(id);
  if (brick) {
...
// ***************************************************************************
TOOL_TYPE::TCraftingToolType
CSBrickManager::CBrickContainer::getFaberPlanToolType(
    NLMISC::CSheetId id) const {
  CSBrickManager *pBM = CSBrickManager::getInstance();
  CSBrickSheet *brick = pBM->getBrick(id);
  if (brick) {
...
void CSBrickManager::compileBrickProperties() {
  ...
  for (std::vector<CSBrickSheet *>::size_type ib = 0; ib < _BrickVector.size();
    ...
    // Get the Brick texts
    std::string texts[3];
    texts[0] = STRING_MANAGER::CStringManagerClient::getSBrickLocalizedName(
        brickSheet->Id);
    texts[1] =
        STRING_MANAGER::CStringManagerClient::getSBrickLocalizedDescription(
            brickSheet->Id);
    texts[2] = STRING_MANAGER::CStringManagerClient::
        getSBrickLocalizedCompositionDescription(brickSheet->Id);
        ...
        if (text[j] == '$') {
          ...
          // else this is a key desc.
            ...
            for (;;) {
              ...
              // Param Id modifier? (ie read not the 0th value, but the 1th
              // etc... up to 9)
              else if (k < textSize && (uint8)text[k] < (uint8)'\x80' &&
                       isdigit(text[k])) {
                char tp[2];
                tp[0] = (char)text[k];
                tp[1] = 0;
            ...
            if (!key.empty()) {
              ...
              if (propId) {
                for (uint p = 0; p < brickSheet->Properties.size(); p++) {
                  const CSBrickSheet::CProperty &prop =
                      brickSheet->Properties[p];
    ...
    // reset
    STRING_MANAGER::CStringManagerClient::replaceSBrickName(
        brickSheet->Id, texts[0], texts[1], texts[2]);
  }
}
...
void CSBrickManager::CBrickFamilyObs::update(ICDBNode * /* node */) {
  ...
  for (it = Owner->_BrickLearnedCallbackSet.begin();
       it != Owner->_BrickLearnedCallbackSet.end(); it++) {
    (*it)->onBrickLearned();
  }
}
