...
const std::string PHRASE_DB_PROGRESSION[2] = {"UI:PHRASE:PROGRESS_ACTIONS",
                                              "UI:PHRASE:PROGRESS_UPGRADES"};
...
const std::string PHRASE_DB_EXECUTE_NEXT_IS_CYCLIC =
    "UI:PHRASE:EXECUTE_NEXT:ISCYCLIC";
...
const std::string PHRASE_DB_COUNTER_CYCLE =
    "SERVER:EXECUTE_PHRASE:CYCLE_COUNTER";
...
/** Special helper class for lua export : enclose a phrase into an object
 * accessible to lua This way we can setup the phrase tooltip in lua code
 * instead of hardcoded C++ stuff letting the possibility to the user to
...
public:
  ...
  // The Slot 1 indicate the Edited Phrase for phrase composition, and is not
  // "really" a part of the book.
  enum { EditionSlot = 1, BookStartSlot = 2 };

  /// To call when The DB inGame is setup. Else, no write is made to it before.
  /// (NB: DB is updated here)
  void initInGame();
   ...
   *	\param lock special for BotChat phrase buy. when true, the phrase is
   *never written in the book DB the server has to unlock it (or delete if not
   *confirmed) NO MSG SENT
   */
  ...
  /// update Action Book Database. You need only to use it in conjunction with
  /// setPhraseNoUpdateDB()
  void updateBookDB();
   ...
   *	NB: the slot is not still correctly filled(), and getPhrase() will
   *return Empty. use setPhrase() just after \return 0 if not possible (eg too
   *big)
   */
  ...
  // Receive a BotChat phrase Buy confirmation: NB: memorize it (and server
  // send) if possible
   ...
   *	 il filterSkill is unknown, then not filtered (all match). This is the
   *default.
   */
  void setBookFilter(BRICK_TYPE::EBrickType brickTypeFilter,
  ...
  /// This is a clean full delete: forget all phrase before, erase the phrase,
  /// send msg to server
  ...
  /// Memorize a macro (no MSG send). if was a phrase, forget first (client and
  /// server)
  ...
  /// return true if it is a memorized macro. false if empty or if it is a
  /// phrase
  ...
  void sendMemorizeToServer(uint32 memoryLine, uint32 memorySlot,
                            uint32 phraseId);
  ...
  /// Forget all Memories that use this phrase slot, AND send appropriate
  /// message to server. if sendMsgOnly==true, only the msg is sent to server
  /// (no modification to local)
  ...
  /// Update all Ctrl State of the action Bar, according to
  /// macro/itemInRightHand/available states
  ...
  /** return getMemorizedPhrase(memoryLine, memoryIndex), only if this phrase is
   *only used in this memory slot else return allocatePhraseSlot()
   */
  uint32 getMemorizedPhraseIfLastOrNewSlot(uint32 memoryLine,
                                           uint32 memoryIndex);
  ...
  // @{
  // The phrase edited/created. If 0, it may be a phrase newly created, but not
  // still assigned.
  uint32 CompositionPhraseId;
  // For phrase created only. If not -1, then at creation, will be
  // auto-memorized to this slot
  ...
  // @{
  /// Start execution from Client. Only the bitmaps are displayed. A
  /// sendExecuteToServer() or a cancelClientExecution() must follow for each
  /// clientExecute
  ...
  /// Cancel the client execution (case when used with Move in combat for
  /// instance)
  void cancelClientExecute(bool cyclic);
  /// Execute a craft action (after selecting the plan and MPs), on both client
  /// and server side
  ...
  /// Search the default attack like on client. false if not found. NB: find
  /// preferly in current memoryLine
  ...
  uint getPhraseCycleExecuteCounter() const {
    return _PhraseCycleExecuteCounter;
  }
  ...
  uint32 getCycleExecutePhraseId() const {
    return _CurrentExecutePhraseIdCycle;
  }
  ...
  // \name Misc
  // @{
  /** Build the Text description of a phrase (spell). NB: this is a TagFormated
   *text \phraseSheetId not 0 if comes from a .sphrase Sheet => used to show
   *progression info, and display phrase description \specialPhraseFormat if
   *empty, format is auto selected. if "composition", same but the text is cut
   *under the %compostart tag. else take directly this format.
   */
  void buildPhraseDesc(std::string &text, const CSPhraseCom &phrase,
                       uint32 phraseSheetId, bool wantRequirement,
  ...
  // Get the Phrase Success Rate %. Manually gives the Skill to do the
  // comparison (for craft)
  sint getCraftPhraseSuccessRate(const CSPhraseCom &phrase,
                                 SKILLS::ESkills skill, uint minMpLevel,
                                 sint successModifier);
  // Get the Phrase Success Rate %. Manually gives the Skill to do the
  // comparison (for Forage Extraction)
  sint getForageExtractionPhraseSuccessRate(const CSPhraseCom &phrase,
  ...
  void getPhraseSapCost(const CSPhraseCom &phrase, uint32 totalActionMalus,
                        sint &cost, sint &costMalus);
  // Get the Phrase Sta Cost
  void getPhraseStaCost(const CSPhraseCom &phrase, uint32 totalActionMalus,
                        sint &cost, sint &costMalus);
  // Get the Phrase Focus Cost
  void getPhraseFocusCost(const CSPhraseCom &phrase, uint32 totalActionMalus,
                          sint &cost, sint &costMalus);
  // Get the Phrase Hp Cost
  void getPhraseHpCost(const CSPhraseCom &phrase, uint32 totalActionMalus,
                       sint &cost, sint &costMalus);
  // Get the Phrase Cast Time
  void getPhraseCastTime(const CSPhraseCom &phrase, uint32 totalActionMalus,
                         float &castTime, float &castTimeMalus);
  // Get the Phrase Range
  void getPhraseMagicRange(const CSPhraseCom &phrase, uint32 totalActionMalus,
                           sint &range, sint &rangeMalus);
  // For all bricks, get the sum of the propId property
  float getPhraseSumBrickProp(const CSPhraseCom &phrase, uint propId,
                              bool doAbs = false);
  /// If the temporary inventory is opened so we cant execute action
  bool isExecutionInvalidCauseTempInv() const;
  /// build a list of bricks that are needed in order to learn this phrase. NB:
  /// any brick already contained in the phrase are removed
  void buildPhraseBrickRequirement(uint32 phraseSheetId,
  ...
  void getCombatWeaponRestriction(std::string &text, const CSPhraseCom &phrase,
                                  bool &usableWithMelee, bool &usableWithRange);
  void getCombatWeaponRestriction(std::string &text, sint32 phraseSheetId,
                                  bool &usableWithMelee, bool &usableWithRange);
  // return true if any of the Bricks contains AvoidCyclic==true (the phrase
  // cannot be cyclic)
  bool avoidCyclicForPhrase(const CSPhraseCom &phrase) const;
  bool avoidCyclicForPhrase(sint32 phraseSheetId) const;
  // Get total regeneration time for a power/aura phrase, in tick (0 if not a
  // power/aura)
  NLMISC::TGameCycle getPowerDisableTime(const CSPhraseCom &phrase) const;
  // Get current time left for a power/aura phrase, in ticks (0 if not a
  // power/aura)
  NLMISC::TGameCycle getRegenTime(const CSPhraseCom &phrase) const;
  // Get total time span necessary for a power/aura phrase to be available again
  // (in ticks)
  ...
  sint getSuccessRate(TSuccessTable st, sint level,
  ...
  // get the base skill point price of a phrase sheet (ie don't check if bricks
  // learned or not)
  ...
  // inverse
  void getPhraseLevelFromSection(uint32 section, uint32 &minLevel,
                                 uint32 &maxLevel);
  // For book display of section
  void getPhraseSectionBoundFromSkillFilter(sint &minSectionId,
                                            sint &maxSectionId);

  // Client Side Rolemaster phrase generation. return the number of db node
  // filled
  uint fillRoleMasterGenericPhrase(uint32 rmFlags, uint32 rmRace);

  // @}

  /// true if the compareSkill is compatible with this combat phrase, eg more
  /// restrictive for instance
  bool skillCompatibleWithCombatPhrase(
      SKILLS::ESkills compareSkill,
      const std::vector<NLMISC::CSheetId> &phraseBricks) const;
  bool skillCompatibleWithSpecialPowerPhrase(
      SKILLS::ESkills compareSkill,
      const std::vector<NLMISC::CSheetId> &phraseBricks) const;

  /// true if phrase is usable with empty hands
  bool phraseUsableWithEmptyHands(
      const std::vector<NLMISC::CSheetId> &phraseBricks) const;
...
private:
  ...
  typedef std::map<CSPhraseCom, uint32>
      TPhraseToSheet; // map each phrase to its sheet id
  // Map of All Phrase. Contains the Book + some system phrase (1: the Edition
  // Phrase)
  ...
  sint getPhraseSuccessRate(TSuccessTable st, const CSPhraseCom &phrase,
                            sint skillValue, uint minMpLevel);

  // real stuff
  void setPhraseInternal(uint32 slot, const CSPhraseCom &phrase, bool lock,
                         bool updateDB);
  ...
  void updateMemoryCtrlState(uint memorySlot, class CDBCtrlSheet *ctrl,
  ...
  public:
    // All the phrase that can be learned at this level, anbd the required level
    // of the skill
  ...
  class CProgressionUpdate : public IBrickLearnedCallback,
  ...
  void
  insertProgressionSkillRecurs(SKILLS::ESkills skill, uint32 value,
                               sint *skillReqLevel,
  ...
  SKILLS::ESkills
  getPhraseRootSkill(const std::vector<NLMISC::CSheetId> &phraseBricks) const;
  ...
  // NB: Use a ReqSkillFormula, but don't use the value part, cause no
  // requirement is made on it
  CReqSkillFormula buildCombatPhraseSkillFormula(
      const std::vector<NLMISC::CSheetId> &phraseBricks) const;
  ...
  // For Magic
  void getResistMagic(bool resistMagic[RESISTANCE_TYPE::NB_RESISTANCE_TYPE],
