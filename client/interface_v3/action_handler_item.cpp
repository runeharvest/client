...
CInterfaceItemEdition *CInterfaceItemEdition::getInstance() {
  if (!_Instance)
    _Instance = new CInterfaceItemEdition;
  return _Instance;
}
...
// ********************************************************************************************
void CInterfaceItemEdition::setCurrWindow(CDBCtrlSheet *ctrlSheet,
...
// ********************************************************************************************
void CInterfaceItemEdition::update() { _CurrWindow.update(); }
// ********************************************************************************************
...
void CInterfaceItemEdition::CItemEditionWindow::infoReceived() {
  if (_CurrItemSheet && !WindowName.empty()) {
    ...
    if ((pIS != NULL) && ITEMFAMILY::isTextCustomizable(pIS->Family)) {
      ...
      CInterfaceGroup *editShort = dynamic_cast<CInterfaceGroup *>(
          CWidgetManager::getInstance()->getElementFromId(
              CWidgetManager::getInstance()->getParser()->getDefine(
                  "edit_custom_edit_short")));
      CGroupEditBox *editBoxShort = dynamic_cast<CGroupEditBox *>(
          CWidgetManager::getInstance()->getElementFromId(
              CWidgetManager::getInstance()->getParser()->getDefine(
                  "edit_custom_edit_box_short")));
      CInterfaceGroup *editLarge = dynamic_cast<CInterfaceGroup *>(
          CWidgetManager::getInstance()->getElementFromId(
              CWidgetManager::getInstance()->getParser()->getDefine(
                  "edit_custom_edit_large")));
      CGroupEditBox *editBoxLarge = dynamic_cast<CGroupEditBox *>(
          CWidgetManager::getInstance()->getElementFromId(
              CWidgetManager::getInstance()->getParser()->getDefine(
                  "edit_custom_edit_box_large")));
      CViewText *display = dynamic_cast<CViewText *>(
          CWidgetManager::getInstance()->getElementFromId(
              CWidgetManager::getInstance()->getParser()->getDefine(
                  "edit_custom_display")));
      if (group && editShort && editBoxShort && editLarge && editBoxLarge &&
          display) {
        CClientItemInfo const &itemInfo =
            getInventory().getItemInfo(ItemSlotId);
        if (IsInEditionMode) {
          if (pIS->Family == ITEMFAMILY::SCROLL) {
            editBoxLarge->setInputString(
                itemInfo.CustomText); // TODO: UTF-8 (serial)
            editLarge->setActive(true);
            editBoxLarge->setActive(true);

            // Set the Keyboard focus to the editbox (after the
            // enableModalWindow())
          ...
          const char *localDesc =
              STRING_MANAGER::CStringManagerClient::getItemLocalizedDescription(
                  pIS->Id);
          if (itemInfo.CustomText.empty())
            ...
            if (NLMISC::startsWith(text, "@WEB")) {
              CGroupHTML *pGH = dynamic_cast<CGroupHTML *>(
                  CWidgetManager::getInstance()->getElementFromId(
                      "ui:interface:web_transactions:content:html"));
              if (pGH)
                pGH->browse(text.substr(4, delimiter - 4).c_str());
...
void CInterfaceItemEdition::CItemEditionWindow::begin() {
  if (_CurrItemSheet && !WindowName.empty()) {
    ...
    if ((pIS != NULL) && ITEMFAMILY::isTextCustomizable(pIS->Family)) {
      CInterfaceManager *pIM = CInterfaceManager::getInstance();
      // get the dialog stack
      CGroupContainer *group = dynamic_cast<CGroupContainer *>(
          CWidgetManager::getInstance()->getElementFromId(WindowName));
      ...
      CViewText *display = dynamic_cast<CViewText *>(
          CWidgetManager::getInstance()->getElementFromId(
              CWidgetManager::getInstance()->getParser()->getDefine(
                  "edit_custom_display")));
      CInterfaceGroup *editButtons = dynamic_cast<CInterfaceGroup *>(
          CWidgetManager::getInstance()->getElementFromId(
              CWidgetManager::getInstance()->getParser()->getDefine(
                  "edit_custom_edit_buttons")));
      CCtrlBaseButton *closeButton = dynamic_cast<CCtrlBaseButton *>(
          CWidgetManager::getInstance()->getElementFromId(
              CWidgetManager::getInstance()->getParser()->getDefine(
                  "edit_custom_close_button")));
      CViewBitmap *background = dynamic_cast<CViewBitmap *>(
          CWidgetManager::getInstance()->getElementFromId(
              CWidgetManager::getInstance()->getParser()->getDefine(
                  "edit_custom_background")));
      if (group && editShort && editBoxShort && editLarge && editBoxLarge &&
          display && editButtons && closeButton && background) {
        ...
        if (IsInEditionMode) {
          ...
          if (getInventory().isItemInfoUpToDate(ItemSlotId)) {
            // If we already have item info
            const char *localDesc = STRING_MANAGER::CStringManagerClient::
                getItemLocalizedDescription(pIS->Id);
            if (itemInfo.CustomText.empty())
              display->setTextFormatTaged(localDesc);
            else {
              string text = itemInfo.CustomText;
      ...
      if (textValid) {
        ...
        if (!GenericMsgHeaderMngr.pushNameToStream(msgName, out)) {
          ...
          INVENTORIES::TInventory inventory =
              (INVENTORIES::TInventory)pCSItem->getInventoryIndex();
          out.serialShortEnum(inventory);
          uint16 slot = (uint16)pCSItem->getIndexInDB();
          out.serial(slot);
          out.serial(text);
          NetMngr.push(out);
          // nlinfo("impulseCallBack : %s %s %d \"%s\" sent", msgName.c_str(),
          // INVENTORIES::toString((INVENTORIES::TInventory)pCSItem->getInventoryIndex()).c_str(),
          // pCSItem->getIndexInDB(), text.toUtf8().c_str());
        }
      }
    }
  }
...
static void validateStackItem(CDBCtrlSheet *src, CDBCtrlSheet *dest,
                              sint32 quantity, TStackMode stackMode);

//=====================================================================================================================
/** Send a swap item msg to the server
 */
static void sendSwapItemMsg(const CDBCtrlSheet *pCSSrc,
                            const CDBCtrlSheet *pCSDst, sint32 quantitySrc) {
  CBitMemStream out;
  ...
  if (GenericMsgHeaderMngr.pushNameToStream(sMsg, out)) {
    ...
    // Special case for guilds that are not on the same counter as the other
    // inventories The guild counter is global on the server so it can changes
    // without this client
    uint16 nGuildSessionCounter = 0;
    if ((srcInvId == (uint16)INVENTORIES::guild) ||
        (dstInvId == (uint16)INVENTORIES::guild)) {
      CInterfaceManager *pIM = CInterfaceManager::getInstance();
      nGuildSessionCounter = NLGUI::CDBManager::getInstance()
                                 ->getDbProp("SERVER:GUILD:INVENTORY:SESSION")
                                 ->getValue16();
    }
    out.serial(nGuildSessionCounter); // always serial to use the fixed-sized
                                      // msg.xml scheme
    ...
    // nlinfo("impulseCallBack : %s %d %d %d %d %d sent", sMsg.c_str(),
    // srcInvId, srcSlotId, dstInvId, dstSlotId, quantity);
  } else
 ...
 */
//=====================================================================================================================
static void displayQuantityPopup(CCtrlBase *pCaller, CDBCtrlSheet *pCSSrc,
                                 CDBCtrlSheet *pCSDst, sint32 availableStack,
                                 TStackMode stackMode) {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  // get the dialog stack
  CInterfaceGroup *group = dynamic_cast<CInterfaceGroup *>(
      CWidgetManager::getInstance()->getElementFromId(
          "ui:interface:stack_dialog"));
  CGroupEditBox *editBox = dynamic_cast<CGroupEditBox *>(
      CWidgetManager::getInstance()->getElementFromId(
          "ui:interface:stack_dialog:edit:eb"));
  if (group && editBox) {
    ...
    NLGUI::CDBManager::getInstance()
        ->getDbProp("UI:VARIABLES:STACK_SELECTED:CUR_QUANTITY")
        ->setValue32(availableStack);
 ...
 */
//=====================================================================================================================
static void openStackItem(CCtrlBase *pCaller, CDBCtrlSheet *pCSSrc,
                          CDBCtrlSheet *pCSDst, sint32 availableStack,
                          TStackMode stackMode) {
...
//=====================================================================================================================
static void sendExchangeAddToServer(uint16 srcSlotIndex, uint16 destSlotIndex,
                                    uint16 quantitySrc) {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  ...
  if (GenericMsgHeaderMngr.pushNameToStream(sMsg, out)) {
    ...
    pIM->incLocalSyncActionCounter();
    // nlinfo("impulseCallBack : %s %d %d %d sent", sMsg.c_str(), srcSlotIndex,
    // destSlotIndex, quantitySrc);
  } else
    nlwarning(" unknown message name '%s'", sMsg);
}
 ...
 * This put the items back in its start place, and unlock them. There may be
 * several items This put the destination item in the exchange slot, unless it
 * is the same item than src
 */
static void putInventoryItemToExchange(CDBCtrlSheet *src, CDBCtrlSheet *dest) {
  ...
  if (!canStack) {
    ...
    NLGUI::CDBManager::getInstance()
        ->getDbProp("LOCAL:EXCHANGE:ACCEPTED")
        ->setValue32(0);

    // send msg to server
    sendExchangeAddToServer((uint16)src->getIndexInDB(),
                            (uint8)dest->getIndexInDB(), (uint16)quantitySrc);
  } else {
    openStackItem(NULL, src, dest, min(quantitySrc, stackCapacity),
                  StackModeExchange);
  }
}
...
// ***************************************************************************
static void validateStackItem(CDBCtrlSheet *pCSSrc, CDBCtrlSheet *pCSDst,
                              sint32 val, TStackMode stackMode) {
  ...
  if (pCSSrc && pCSDst && val > 0) {
    ...
    // **** Exchange
      ...
      if (val > 0) {
        ...
        // send msg to server
        sendExchangeAddToServer((uint16)pCSSrc->getIndexInDB(),
                                (uint8)pCSDst->getIndexInDB(), (uint16)val);
      }
    }
  }
}
 ...
 *	eg: place in sword in the set, or place an armor in the kit, or swap 2
 *items in bags
 */
  ...
  virtual void execute(CCtrlBase *pCaller, const string &Params) {
    ...
    if (pCSSrc->getType() == CCtrlSheetInfo::SheetType_Item)
      if (pCSDst->getType() == CCtrlSheetInfo::SheetType_Item) {
        ...
        if (!canStack) {
          if ((pCSDst->getInventoryIndex() != INVENTORIES::guild) &&
              (pCSSrc->getInventoryIndex() !=
               INVENTORIES::guild)) // ? why not always skip it now with the new
                                    // itemSwap?
          {
            // write to Local database for minimum effective Lag
          ...
          openStackItem(pCaller, pCSSrc, pCSDst,
                        min(quantitySrc, stackCapacity - quantityDst),
                        StackModeSwap);
        }
      }
  }
...
// ***************************************************************************
/** When an item swap open a "StackDialog" for items with quantities, this is
 * called to validate the item drop
 */
...
public:
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    ...
    sint32 val = NLGUI::CDBManager::getInstance()
                     ->getDbProp("UI:VARIABLES:STACK_SELECTED:CUR_QUANTITY")
                     ->getValue32();
...
public:
  virtual void execute(CCtrlBase *pCaller, const string & /* Params */) {
    ...
    CDBCtrlSheet *dest = dynamic_cast<CDBCtrlSheet *>(
...
REGISTER_ACTION_HANDLER(CPlayerTradePutBagItemToExchange,
...
public:
  virtual void execute(CCtrlBase *pCaller, const string & /* Params */) {
    CDBCtrlSheet *src = dynamic_cast<CDBCtrlSheet *>(pCaller);
    if (src)
      putExchangedItemToInventory(src);
  }
};
REGISTER_ACTION_HANDLER(CPlayerTradePutExchangeItemtoBag,
                        "put_exchange_item_to_bag");
...
public:
  virtual void execute(CCtrlBase *pCaller, const string & /* Params */) {
    ...
    if (cs) {
      CDBCtrlSheet *pCSDst = dynamic_cast<CDBCtrlSheet *>(pCaller);
      if (pCSDst)
        pCSDst->setCanDrop(cs->getSelectionGroupAsString() ==
                           "inventory_selection");
    }
  }
};
...
// **********************************************************************************************************
/** drag'n'drop: true if the ctrlSheet is an item of a special slot ("chest",
 *"legs", etc...) example: Used to kwon if can drop a "chest" on the "chest"
 *Kit.
 */
  ...
  virtual void execute(CCtrlBase *pCaller, const string &Params) {
    ...
    if (pCSSrc && pCSSrc->getType() == CCtrlSheetInfo::SheetType_Item)
      if (pCSDst && pCSDst->getType() == CCtrlSheetInfo::SheetType_Item)
        if (!testEmpty || pCSDst->getSheetId() ==
                              0) // the destination must be empty if testEmpty
        {
          if (pCSSrc->getInventoryIndex() == INVENTORIES::exchange) {
...
bool checkCanExchangeItem(CDBCtrlSheet *pCSSrc) {
  ...
  if ((pIS != NULL) && (pIS->Family == ITEMFAMILY::PET_ANIMAL_TICKET)) {
    // If we are not giving something to a bot (so we are exchanging with a
    // player)
...
public:
  void execute(CCtrlBase * /* pCaller */, const std::string &sParams) {
    ...
    if (GenericMsgHeaderMngr.pushNameToStream("EXCHANGE:VALIDATE", out)) {
      ...
      nlwarning("<CHandlerAcceptExchange::execute> unknown message name "
                "'EXCHANGE:VALIDATE");
  }
...
public:
  void execute(CCtrlBase * /* pCaller */, const std::string &sParams) {
    ...
    if (!CInterfaceExpr::evalAsInt(getParam(sParams, "counter"), counter)) {
      nlwarning(
          "<CHandlerInvalidateExchange::execute> Can't retrieve counter.");
      return;
    }
    CBitMemStream out;
...
public:
  void execute(CCtrlBase * /* pCaller */, const std::string & /* sParams */) {
    ...
    CInterfaceManager *im = CInterfaceManager::getInstance();
    NLGUI::CDBManager::getInstance()
        ->getDbProp("LOCAL:EXCHANGE:BEGUN")
        ->setValue32(0);
    ...
    if (!ClientCfg.Local) {
      ...
      if (GenericMsgHeaderMngr.pushNameToStream("EXCHANGE:END", out)) {
        ...
        nlwarning("<CHandlerEndExchange::execute> unknown message name "
                  "'EXCHANGE:END");
    }
  }
};
...
public:
  void execute(CCtrlBase * /* pCaller */, const std::string &sParams) {
    ...
    if (!CInterfaceExpr::evalAsInt(getParam(sParams, "quantity"), quantity)) {
      nlwarning("<CHandlerValidateSeedSel::execute> Can't retrieve quantity of "
                "seeds");
      return;
    }
    // user changed the proposal => reset the local ACCEPTED db
...
public:
  void execute(CCtrlBase * /* pCaller */, const std::string & /* sParams */) {
    CBitMemStream out;
    if (GenericMsgHeaderMngr.pushNameToStream("EXCHANGE:ACCEPT_INVITATION",
                                              out)) {
      NetMngr.push(out);
      // nlinfo("impulseCallBack : EXCHANGE:ACCEPT_INVITATION sent");
    } else
      nlwarning("<CHandlerAcceptExchangeInvite::execute> unknown message name "
                "'EXCHANGE:ACCEPT_INVITATION");
  }
};
REGISTER_ACTION_HANDLER(CHandlerAcceptExchangeInvite,
                        "accept_exchange_invitation");
...
public:
  void execute(CCtrlBase * /* pCaller */, const std::string & /* sParams */) {
    CBitMemStream out;
    if (GenericMsgHeaderMngr.pushNameToStream("EXCHANGE:DECLINE_INVITATION",
                                              out)) {
      NetMngr.push(out);
      // nlinfo("impulseCallBack : EXCHANGE:DECLINE_INVITATION sent");
    } else
      nlwarning("<CHandlerDeclineExchangeInvite::execute> unknown message name "
                "'EXCHANGE:DECLINE_INVITATION");
  }
};
REGISTER_ACTION_HANDLER(CHandlerDeclineExchangeInvite,
                        "decline_exchange_invitation");

// tool fct to destroy or drop an item
static void dropOrDestroyItem(CDBCtrlSheet *item, CBitMemStream &out,
                              uint16 quantity) {
  ...
  void execute(CCtrlBase * /* pCaller */, const std::string &sParams) {
    ...
    if (!ClientCfg.Local) {
      ...
      if (!GenericMsgHeaderMngr.pushNameToStream("ITEM:DESTROY", out)) {
        nlwarning("<CHandlerDestroyItem::execute> unknown message name "
                  "'ITEM:DESTROY'");
        return;
      }
...
class CHandlerActiveSheath : public IActionHandler
{
        void execute (CCtrlBase *pCaller, const std::string &sParams)
        {
                ...
                uint8 activeSheath =
(uint8)NLGUI::CDBManager::getInstance()->getDbProp(
CWidgetManager::getInstance()->getParser()->getDefine("ui_set_active")
)->getValue32();

                // Write to the Local Database.
                NLGUI::CDBManager::getInstance()->getDbProp(
"LOCAL:INVENTORY:ACTIVE_SHEATH" )->setValue32(activeSheath);

                // Send to server
                if(!ClientCfg.Local)
                {
                        CBitMemStream out;
                        if(!GenericMsgHeaderMngr.pushNameToStream(
"ITEM:ACTIVATE_SHEATH", out) )
                        {
                                nlwarning("<CHandlerServerActiveSheath::execute>
unknown message name 'ITEM:ACTIVATE_SHEATH'"); return;
                        }
...
class CHandlerReceiveActiveSheath : public IActionHandler
{
        void execute (CCtrlBase *pCaller, const std::string &sParams)
        {
                // NB: the System does not recurs between active_sheath and
receive_active_sheath since
                //	at a time, the 2 values 'ui_set_active' and
LOCAL:INVENTORY:ACTIVE_SHEATH
                //	will be the same (an observer is called only if the
value change)
                ...
                uint8 activeSheath =
(uint8)NLGUI::CDBManager::getInstance()->getDbProp(
"LOCAL:INVENTORY:ACTIVE_SHEATH" )->getValue32();

                // Write to the Local Database.
                NLGUI::CDBManager::getInstance()->getDbProp(
CWidgetManager::getInstance()->getParser()->getDefine("ui_set_active")
)->setValue32(activeSheath);
        }
};
  ...
  void execute(CCtrlBase * /* pCaller */, const std::string &sParams) {
    ...
    if (sParams == string("loot")) {
      if (!GenericMsgHeaderMngr.pushNameToStream("ITEM:PICK_UP_CLOSE", out)) {
        nlwarning("<CHandlerEndHarvest::execute> unknown message name "
                  "'PICK_UP_CLOSE'");
        return;
      }
    } else {
...
class CHandlerHarvestItem : public IActionHandler {
  void execute(CCtrlBase *pCaller, const std::string & /* sParams */) {
    ...
    if (!GenericMsgHeaderMngr.pushNameToStream("ITEM:HARVEST", out)) {
      nlwarning(
          "<CHandlerEndHarvest::execute> unknown message name 'PICK_UP_CLOSE'");
      return;
    }
  ...
  void execute(CCtrlBase *pCaller, const std::string &sParams) {
    ...
    if (sDest == "lists") {
      ...
      if (nNbList > 0) {
        ...
        while (!sListId.empty()) {
          IListSheetBase *pLS = dynamic_cast<IListSheetBase *>(
              CWidgetManager::getInstance()->getElementFromId(sListId));
          if (pLS == NULL)
            return;
          ...
          for (sint32 i = 0; i < nbelt; ++i) {
            if (pLS->getSheet(i)->getSheetId() == 0) {
              ...
              CAHManager::getInstance()->runActionHandler(
                  "swap_item", pLS->getSheet(i),
                  "src=" + toString(pCaller->getId()));
              return;
            }
          }
          // ok try next list
    ...
    /*else if (sDest == "groups")
    {
            ...
            while (!sGrpId.empty())
            {
                    uint i;
                    CInterfaceGroup *pIG =
    dynamic_cast<CInterfaceGroup*>(CWidgetManager::getInstance()->getElementFromId(sGrpId));
                    if (pIG != NULL)
                    {
                            ...
                            for (i = 0; i < vDBCS.size(); ++i)
                            {
                                    ...
                                    if ((pCS != NULL) && (pCS->getType() ==
    CCtrlSheetInfo::SheetType_Item) && (pCS->getSheetId() == 0) &&
    pCS->canDropItem(item) && (!pCS->getGrayed()))
                                    {
                                            // Send swap_item
                                            CAHManager::getInstance()->runActionHandler("swap_item",
    pCS, "src="+toString(pCaller->getId())); return;
                                    }
                            }
                    }
                    // Next Group
    ...
    dynamic_cast<CInterfaceGroup*>(CWidgetManager::getInstance()->getElementFromId(sGrpId));
                    if (pIG != NULL)
                    {
                            ...
                            for (i = 0; i < vDBCS.size(); ++i)
                            {
                                    CDBCtrlSheet *pCS = vDBCS[i];
                                    if ((pCS != NULL) && (pCS->getType() ==
    CCtrlSheetInfo::SheetType_Item))
                                    {
                                            if ((pCS->canDropItem(item)) &&
    (!pCS->getGrayed()))
                                            {
                                                    // Send swap_item
                                                    CAHManager::getInstance()->runActionHandler("swap_item",
...
class CHandlerDragNDrop : public IActionHandler {
  void execute(CCtrlBase * /* pCaller */, const std::string &sParams) {
    ...
    CDBCtrlSheet *pCSsrc = dynamic_cast<CDBCtrlSheet *>(
        CWidgetManager::getInstance()->getElementFromId(sSrc));
    CDBCtrlSheet *pCSdst = dynamic_cast<CDBCtrlSheet *>(
        CWidgetManager::getInstance()->getElementFromId(sDst));
    if ((pCSdst == NULL) || (pCSsrc == NULL) || sAH.empty())
      return;
    CAHManager::getInstance()->runActionHandler(sAH, pCSdst,
                                                "src=" + pCSsrc->getId());
  }
};
...
class CHandlerItemCristalEnchant : public IActionHandler {
  void execute(CCtrlBase *pCaller, const std::string &sParams) {
    CInterfaceManager *pIM = CInterfaceManager::getInstance();
    CAHManager::getInstance()->runActionHandler("item_cristal_reload", pCaller,
                                                sParams);
  }
};
...
class CHandlerItemCristalReload : public IActionHandler {
  void execute(CCtrlBase * /* pCaller */, const std::string & /* sParams */) {
    ...
    sendToServerEnchantMessage((uint8)pCS->getInventoryIndex(),
                               (uint16)pCS->getIndexInDB());
  }
};
REGISTER_ACTION_HANDLER(CHandlerItemCristalReload, "item_cristal_reload");

// **********************************************************************************************************
class CItemMenuInBagInfoWaiter : public IItemInfoWaiter {
public:
...
void CItemMenuInBagInfoWaiter::infoValidated(CDBCtrlSheet *ctrlSheet) {
  ...
  // get the dialog stack
  CInterfaceGroup *pMenu = dynamic_cast<CInterfaceGroup *>(
      CWidgetManager::getInstance()->getElementFromId(
          "ui:interface:item_menu_in_bag"));
  if (!pMenu)
    return;

  const CItemSheet *pIS = ctrlSheet->asItemSheet();
  CViewTextMenu *pItemTextEdition =
      dynamic_cast<CViewTextMenu *>(pMenu->getView("item_text_edition"));
  if (pIS != NULL && pItemTextEdition &&
      ITEMFAMILY::isTextCustomizable(pIS->Family)) {
    CClientItemInfo const &itemInfo =
        getInventory().getItemInfo(getInventory().getItemSlotId(ctrlSheet));

    // get the CreatorTextID
    bool isCraftedByUserEntity = false;
    string creatorNameString;
    if (STRING_MANAGER::CStringManagerClient::instance()->getString(
            itemInfo.CreatorName, creatorNameString)) {
      std::string userNameString = UserEntity->getEntityName() +
                                   PlayerSelectedHomeShardNameWithParenthesis;
      if (NLMISC::compareCaseInsensitive(userNameString, creatorNameString) ==
          0)
...
class CHandlerItemMenuCheck : public IActionHandler {
  void execute(CCtrlBase *pCaller, const std::string & /* sParams */) {
    ...
    INVENTORIES::TInventory invId =
        (INVENTORIES::TInventory)pCS->getInventoryIndex();
    ...
    CViewTextMenu *pEquip =
        dynamic_cast<CViewTextMenu *>(pMenu->getView("equip"));
    CViewTextMenu *pCrisEnchant =
        dynamic_cast<CViewTextMenu *>(pMenu->getView("cris_enchant"));
    CViewTextMenu *pCrisReload =
        dynamic_cast<CViewTextMenu *>(pMenu->getView("cris_reload"));
    CViewTextMenu *pTeleportUse =
        dynamic_cast<CViewTextMenu *>(pMenu->getView("teleport_use"));
    CViewTextMenu *pItemConsume =
        dynamic_cast<CViewTextMenu *>(pMenu->getView("item_consume"));
    CViewTextMenu *pXpCatalyserUse =
        dynamic_cast<CViewTextMenu *>(pMenu->getView("xp_catalyser_use"));
    CViewTextMenu *pDrop =
        dynamic_cast<CViewTextMenu *>(pMenu->getView("drop"));
    CViewTextMenu *pDestroy =
        dynamic_cast<CViewTextMenu *>(pMenu->getView("destroy"));
    CViewTextMenu *pLockUnlock =
        dynamic_cast<CViewTextMenu *>(pMenu->getView("lockunlock"));
    CViewTextMenu *pMoveSubMenu =
        dynamic_cast<CViewTextMenu *>(pMenu->getView("move"));
    CViewTextMenu *pMoveToBag =
        dynamic_cast<CViewTextMenu *>(pMenu->getView("bag"));
    CViewTextMenu *pMoveToGuild =
        dynamic_cast<CViewTextMenu *>(pMenu->getView("guild"));
    CViewTextMenu *pMoveToRoom =
        dynamic_cast<CViewTextMenu *>(pMenu->getView("room"));
    ...
    for (i = 0; i < MAX_INVENTORY_ANIMAL; i++) {
      pMoveToPa[i] =
          dynamic_cast<CViewTextMenu *>(pMenu->getView(toString("pa%d", i)));
    }
    CViewTextMenu *pItemInfos =
        dynamic_cast<CViewTextMenu *>(pMenu->getView("infos"));
    CViewTextMenu *pItemTextDisplay =
        dynamic_cast<CViewTextMenu *>(pMenu->getView("item_text_display"));
    CViewTextMenu *pItemTextEdition =
        dynamic_cast<CViewTextMenu *>(pMenu->getView("item_text_edition"));
    ...
    if (pItemTextEdition)
      pItemTextEdition->setActive(false);

    if (pLockUnlock)
      pLockUnlock->setActive(true);
    ...
    if (invId != INVENTORIES::guild)
      if (pIS != NULL) {
        ...
        // teleport can be used only from bag (NB: should not exist in
        // mektoub....)
        if (pTeleportUse && pIS->Family == ITEMFAMILY::TELEPORT &&
            pCS->getInventoryIndex() == INVENTORIES::bag && !bIsLockedByOwner) {
          pTeleportUse->setActive(true);
        }
        if (pItemConsume && pIS->IsConsumable == true &&
            pCS->getInventoryIndex() == INVENTORIES::bag && !bIsLockedByOwner) {
          pItemConsume->setActive(true);
        }
        if (pXpCatalyserUse && pIS->Family == ITEMFAMILY::XP_CATALYSER &&
            pCS->getInventoryIndex() == INVENTORIES::bag && !bIsLockedByOwner) {
        ...
        if (pItemTextEdition && ITEMFAMILY::isTextCustomizable(pIS->Family)) {
          if (pCS->getInventoryIndex() == INVENTORIES::bag) {
            ...
            if (!UserPrivileges.empty() &&
                itemTextEditionPriv.find(UserPrivileges) != std::string::npos) {
              isTextEditionActive = true;
            } else {
              // Finish setting the isTextEditionActive boolean or add the
              // waiter
              if (getInventory().isItemInfoUpToDate(
                      getInventory().getItemSlotId(pCS))) {
                // get the CreatorTextID
                string creatorNameString;
                if (STRING_MANAGER::CStringManagerClient::instance()->getString(
                        getInventory()
                            .getItemInfo(getInventory().getItemSlotId(pCS))
                            .CreatorName,
                        creatorNameString)) {
                  string userNameString =
                      UserEntity->getEntityName() +
                      PlayerSelectedHomeShardNameWithParenthesis;
                  if (NLMISC::compareCaseInsensitive(userNameString,
                                                     creatorNameString) == 0)
                    isTextEditionActive = true;
                }
              } else {
                ...
                ItemMenuInBagUpdater.ItemSlotId =
                    getInventory().getItemSlotId(pCS);
    ...
    if (pIS != NULL && ((pIS->Family == ITEMFAMILY::PET_ANIMAL_TICKET) ||
                        (pIS->Id.toString().substr(0, 6) == "rpjob_"))) {
      // cannot move to other animals! :)
      if (pMoveToBag)
        pMoveToBag->setActive(false);
      for (i = 0; i < MAX_INVENTORY_ANIMAL; i++)
        if (pMoveToPa[i])
          pMoveToPa[i]->setActive(false);

      // additionnaly, cannot drop/destroy/lock an animal item.
      if (pDrop)
        pDrop->setActive(false);
      if (pDestroy)
        pDestroy->setActive(false);
      if (pLockUnlock)
        pLockUnlock->setActive(false);
    } else {
      // Disable Text entries not supported by this menu (eg: if I am the Bag, i
      // cannot drop to myself!) Also Disable Text entries if the inventory is
      // not present
      if (pMoveToBag)
        pMoveToBag->setActive(invId != INVENTORIES::bag &&
                              invMngr.isInventoryPresent(INVENTORIES::bag) &&
                              (invId != INVENTORIES::guild ||
                               invMngr.isInventoryPresent(INVENTORIES::guild)));

      for (i = 0; i < MAX_INVENTORY_ANIMAL; i++) {
        if (pMoveToPa[i])
          pMoveToPa[i]->setActive(
              (uint)invId != INVENTORIES::pet_animal + i &&
              invMngr.isInventoryPresent(
                  (INVENTORIES::TInventory)(INVENTORIES::pet_animal + i)));
      }

      if (pMoveToGuild)
        pMoveToGuild->setActive(invId != INVENTORIES::guild &&
                                invMngr.isInventoryPresent(INVENTORIES::guild));

      if (pMoveToRoom)
        pMoveToRoom->setActive(
            invId != INVENTORIES::player_room &&
            invMngr.isInventoryPresent(INVENTORIES::player_room));

      // std case: can drop / destroy
      if (pDrop)
        pDrop->setActive(invId != INVENTORIES::guild);
      if (pDestroy)
        pDestroy->setActive(invId != INVENTORIES::guild);
      if (pLockUnlock)
        pLockUnlock->setActive(invId != INVENTORIES::guild);
    }
    ...
    if (pMoveSubMenu) {
      if (pMoveToBag)
        someMovePossible = someMovePossible || pMoveToBag->getActive();
      for (i = 0; i < MAX_INVENTORY_ANIMAL; i++) {
        if (pMoveToPa[i])
          someMovePossible = someMovePossible || pMoveToPa[i]->getActive();
      }
      if (pMoveToGuild)
        someMovePossible = someMovePossible || pMoveToGuild->getActive();
      if (pMoveToRoom)
        someMovePossible = someMovePossible || pMoveToRoom->getActive();
    ...
    if (pEquip != NULL) {
      ...
      /* gray if crafting window is opened (cannot equip another tool while
         crafting), and the item is a HAND item
      */
      if (valid) {
        pEquip->setGrayed(NLGUI::CDBManager::getInstance()
                              ->getDbProp("UI:VARIABLES:ISACTIVE:PHRASE_FABER")
                              ->getValueBool() &&
                          pIS->Family != ITEMFAMILY::ARMOR &&
                          pIS->Family != ITEMFAMILY::JEWELRY);
      }
    ...
    if (pItemTextEdition != NULL && pItemTextDisplay != NULL) {
      pItemTextEdition->setGrayed(
          NLGUI::CDBManager::getInstance()
              ->getDbProp("UI:VARIABLES:ISACTIVE:PHRASE_EDIT_CUSTOM")
              ->getValueBool());
      pItemTextDisplay->setGrayed(
          NLGUI::CDBManager::getInstance()
              ->getDbProp("UI:VARIABLES:ISACTIVE:PHRASE_EDIT_CUSTOM")
              ->getValueBool());
    }
    ...
    if (pLockUnlock)
      pLockUnlock->setActive(pCS->canOwnerLock());

    // **** Gray Entries
    // If ourselves are not available, then gray all
    if (!invMngr.isInventoryAvailable(invId)) {
      if (pCrisEnchant)
        pCrisEnchant->setGrayed(true);
      if (pCrisReload)
        pCrisReload->setGrayed(true);
      if (pTeleportUse)
        pTeleportUse->setGrayed(true);
      if (pItemConsume)
        pItemConsume->setGrayed(true);
      if (pXpCatalyserUse)
        pXpCatalyserUse->setGrayed(true);
      if (pDrop)
        pDrop->setGrayed(true);
      if (pDestroy)
        pDestroy->setGrayed(true);
      if (pLockUnlock)
        pLockUnlock->setGrayed(true);
      if (pMoveSubMenu)
        pMoveSubMenu->setGrayed(true);
      if (pMoveToBag)
        pMoveToBag->setGrayed(true);
      for (i = 0; i < MAX_INVENTORY_ANIMAL; i++) {
        if (pMoveToPa[i])
          pMoveToPa[i]->setGrayed(true);
      }
    }
    // Gray Text entries according to Availables Destinations
      ...
      // ungray basics
      if (pCrisEnchant)
        pCrisEnchant->setGrayed(false);
      if (pCrisReload)
        pCrisReload->setGrayed(false);
      if (pTeleportUse)
        pTeleportUse->setGrayed(false);
      if (pItemConsume)
        pItemConsume->setGrayed(false);
      if (pXpCatalyserUse)
        pXpCatalyserUse->setGrayed(false);
      if (pDrop)
        pDrop->setGrayed(false);
      if (pDestroy)
        pDestroy->setGrayed(false);
      if (pLockUnlock)
        pLockUnlock->setGrayed(false);
      if (pMoveSubMenu)
        pMoveSubMenu->setGrayed(false);

      // check each inventory dest if available
      if (pMoveToBag)
        pMoveToBag->setGrayed(!invMngr.isInventoryAvailable(INVENTORIES::bag));
      for (i = 0; i < MAX_INVENTORY_ANIMAL; i++) {
        if (pMoveToPa[i])
          pMoveToPa[i]->setGrayed(!invMngr.isInventoryAvailable(
              (INVENTORIES::TInventory)(INVENTORIES::pet_animal + i)));
      }
    }
    ...
    // Item GROUP logic
#ifdef RYZOM_FORGE
    CGroupMenu *pGroupRootMenu = dynamic_cast<CGroupMenu *>(
        CWidgetManager::getInstance()->getElementFromId(
            "ui:interface:item_menu_in_bag:item_group_menu"));
    if (pGroupRootMenu) {
      CGroupSubMenu *pGroupMenu = pGroupRootMenu->getRootMenu();
      std::vector<std::string> groupNames =
          CItemGroupManager::getInstance()->getGroupNames(pCS);
      CViewText *pGroup =
          dynamic_cast<CViewText *>(pMenu->getView("item_group"));
      // First, hide/show the menu if pertinent (we need to hide the action due
      // to it beeing a submenu)
      if (pGroup) {
        if (groupNames.empty()) {
      ...
      //  We do it the lazy way : active/gray options matching regular options
      //  (when you do things on a single item) Same for translated name of
      //  interface
      pGroupMenu->reset();
      for (i = 0; i < groupNames.size(); i++) {
        std::string name = groupNames[i];
        std::string ahParams = "name=" + name;
        // Use utf-8 string because group name can contain accentued characters
        // (and stuff like that)
        ...
        if (pNewSubMenu) {
          if (pEquip)
            pNewSubMenu->addLine(pEquip->getHardText(), "item_group_equip",
                                 ahParams, name + "_equip");
          // Add move sub menu
          if (pMoveSubMenu) {
            pNewSubMenu->addLine(pMoveSubMenu->getHardText(), "", "",
                                 name + "_move");
          ...
          if (pMoveToBag && pMoveToBag->getActive()) {
            CViewTextMenu *tmp = pNewSubMenu->addLine(
                pMoveToBag->getHardText(), "item_group_move",
                "destination=bag|" + ahParams, name + "_bag");
            if (tmp)
              tmp->setGrayed(pMoveToBag->getGrayed());
          }
          for (int j = 0; j < MAX_INVENTORY_ANIMAL; j++) {
            if (pMoveToPa[j] && pMoveToPa[j]->getActive()) {
              // there is an offset of 1 because TInventory names are
              // pet_animal1/2/3/4
              std::string dst = toString("destination=pet_animal%d|", j + 1);
              CViewTextMenu *tmp = pNewSubMenu->addLine(
                  pMoveToPa[j]->getHardText(), "item_group_move",
                  dst + ahParams, name + toString("_pa%d", j + 1));
              if (tmp)
                tmp->setGrayed(pMoveToPa[j]->getGrayed());
            }
          }
          if (pMoveToRoom && pMoveToRoom->getActive()) {
            CViewTextMenu *tmp = pNewSubMenu->addLine(
                pMoveToRoom->getHardText(), "item_group_move",
                "destination=player_room|" + ahParams, name + "_room");
            if (tmp)
              tmp->setGrayed(pMoveToRoom->getGrayed());
          }
          if (pMoveToGuild && pMoveToGuild->getActive() &&
              ClientCfg.ItemGroupAllowGuild) {
            CViewTextMenu *tmp = pNewSubMenu->addLine(
                pMoveToGuild->getHardText(), "item_group_move",
                "destination=guild|" + ahParams, name + "_guild");
            if (tmp)
              tmp->setGrayed(pMoveToRoom->getGrayed());
          }
        }
...
REGISTER_ACTION_HANDLER(CHandlerValidateItemTextEdition,
                        "validate_edit_custom");
...
class CHandlerItemTextDisplay : public IActionHandler {
  void execute(CCtrlBase * /* pCaller */, const std::string &sParams) {
    ...
    CInterfaceManager *pIM = CInterfaceManager::getInstance();
    CDBCtrlSheet *pCSItem = dynamic_cast<CDBCtrlSheet *>(
        CWidgetManager::getInstance()->getCtrlLaunchingModal());
    if (pCSItem == NULL || windowName.empty())
      return;

    CInterfaceItemEdition::getInstance()->setCurrWindow(pCSItem, windowName,
                                                        false);
  }
};
...
REGISTER_ACTION_HANDLER(CHandlerRingXpCatalyserStopUse,
...
class CHandlerItemGroupMove : public IActionHandler {
  void execute(CCtrlBase * /* pCaller */, const std::string &sParams) {
    ...
    CItemGroupManager::getInstance()->moveGroup(
        name, INVENTORIES::toInventory(destination));
  }
};
