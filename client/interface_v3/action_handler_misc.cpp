...
// static //
////////////
// static CCDBNodeLeaf *MenuColorWidgetValue = NULL; // db entry for the color
// menu widget (Red)
...
// ***************************************************************************
void CActionHandlerShowOne::execute(CCtrlBase * /* pCaller */,
  ...
  for (uint i = 0; i < wndList.size(); i++) {
    CInterfaceElement *wnd =
        CWidgetManager::getInstance()->getElementFromId(wndList[i]);
    if (wnd)
      wnd->setActive(false);
  }
...
// ***************************************************************************
void CActionHandlerActive::execute(CCtrlBase * /* pCaller */,
                                   const std::string &params) {
  ...
  if (CInterfaceExpr::eval(active, activeValue, NULL)) {
    if (!activeValue.toBool()) {
      nlwarning("<CActionHandlerActive::execute> The 'active' param must be "
    ...
    if (!wnd) {
      nlwarning("<CActionHandlerActive::execute> Can't get window %s",
                target.c_str());
      return;
    }
...
// ***************************************************************************
void CActionHandlerSetOpen::execute(CCtrlBase * /* pCaller */,
                                    const std::string &params) {
...
// ***************************************************************************
void CActionHandlerHideClose::execute(CCtrlBase * /* pCaller */,
                                      const std::string &params) {
  CInterfaceManager *mngr = CInterfaceManager::getInstance();
  ...
  for (i = 0; i < hideList.size(); i++) {
    CInterfaceElement *wnd =
        CWidgetManager::getInstance()->getElementFromId(hideList[i]);
    if (wnd)
      wnd->setActive(false);
  }
  ...
  for (i = 0; i < closeList.size(); i++) {
    ...
    CInterfaceElement *wnd =
        CWidgetManager::getInstance()->getElementFromId(closeList[i]);
...
// ***************************************************************************
void CActionHandlerEnterModal::execute(CCtrlBase *pCaller,
                                       const std::string &params) {
  ...
  if (group) {
    ...
    nlwarning("<CActionHandlerEnterModal::execute> Couldn't find group %s",
              groupName.c_str());
  }
}

// ***************************************************************************
void CActionHandlerPushModal::execute(CCtrlBase *pCaller,
                                      const std::string &params) {
  CInterfaceManager *mngr = CInterfaceManager::getInstance();
  ...
  if (group) {
    ...
    nlwarning("<CActionHandlerPushModal::execute> Couldn't find group %s",
              groupName.c_str());
  }
}

// ***************************************************************************
void CActionHandlerLeaveModal::execute(CCtrlBase * /* pCaller */,
                                       const std::string & /* params */) {
  CInterfaceManager *mngr = CInterfaceManager::getInstance();
...
public:
  virtual void execute(CCtrlBase *pCaller, const std::string &params) {
    ...
    CWidgetManager::getInstance()->runProcedure(paramList[0], pCaller,
                                                paramList);
  }
};
...
REGISTER_ACTION_HANDLER(CActionHandlerConfirmCanDeactivate,
...
REGISTER_ACTION_HANDLER(CActionHandlerCancelCanDeactivate,
  ...
  virtual void execute(CCtrlBase *pCaller, const std::string &params) {
    ...
    if (!valueStr.empty()) {
      ...
      if (!CInterfaceExpr::eval(valueStr, maxValue) || !maxValue.toInteger()) {
        nlwarning("<CActionHandlerEditBoxNumber::execute> Can't eval maxValue, "
                  "or can't convert to integer : %s",
                  valueStr.c_str());
        return;
      }
    ...
    if (!valueStr.empty()) {
      ...
      if (!CInterfaceExpr::eval(valueStr, minValue) || !minValue.toInteger()) {
        nlwarning("<CActionHandlerEditBoxNumber::execute> Can't eval minValue, "
                  "or can't convert to integer : %s",
                  valueStr.c_str());
        return;
      }
    ...
    bool mustUpdate = updateText.empty()
                          ? true
                          : CInterfaceElement::convertBool(updateText.c_str());
...
// ***************************************************************************
void CActionHandlerEvalExpr::execute(CCtrlBase * /* pCaller */,
                                     const std::string &params) {
  std::string expr = getParam(params, "expr");
  if (expr.empty()) {
    nlwarning("<CActionHandlerEvalExpr::execute> 'expr' parameter not found or "
              "empty.");
    return;
  }
...
// *********************************************************
CCameraBackup setupCameraForScreenshot(UScene &scene, uint left, uint right,
  ...
  frustumPart.Left =
      cb.Frustum.Left + (cb.Frustum.Right - cb.Frustum.Left) *
                            ((float)left / (float)screenShotWidth);
  frustumPart.Right =
      cb.Frustum.Left + (cb.Frustum.Right - cb.Frustum.Left) *
                            ((float)right / (float)screenShotWidth);
  frustumPart.Top = cb.Frustum.Top + (cb.Frustum.Bottom - cb.Frustum.Top) *
                                         ((float)top / (float)screenShotHeight);
  frustumPart.Bottom =
      cb.Frustum.Top + (cb.Frustum.Bottom - cb.Frustum.Top) *
                           ((float)bottom / (float)screenShotHeight);
  ...
  viewport.init(0, 0, (float)(right - left) / Driver->getWindowWidth(),
                (float)(bottom - top) / Driver->getWindowHeight());
...
// ***************************************************************************
void renderSceneScreenShot(uint left, uint right, uint top, uint bottom,
                           uint screenShotWidth, uint screenShotHeight) {
  CCameraBackup cbScene = setupCameraForScreenshot(
      *Scene, left, right, top, bottom, screenShotWidth, screenShotHeight);
  CCameraBackup cbCanopy = setupCameraForScreenshot(
      *SceneRoot, left, right, top, bottom, screenShotWidth, screenShotHeight);
  commitCamera();
...
void getBuffer(CBitmap &btm) {
  ...
  if (ClientCfg.ScreenShotWidth && ClientCfg.ScreenShotHeight) {
    // Destination image
    CBitmap temp;
    btm.resize(ClientCfg.ScreenShotWidth, ClientCfg.ScreenShotHeight,
               CBitmap::RGBA);

    uint top;
    uint bottom =
        std::min(Driver->getWindowHeight(), ClientCfg.ScreenShotHeight);
    for (top = 0; top < ClientCfg.ScreenShotHeight;
         top += Driver->getWindowHeight()) {
      uint left;
      uint right =
          std::min(Driver->getWindowWidth(), ClientCfg.ScreenShotWidth);
      for (left = 0; left < ClientCfg.ScreenShotWidth;
           left += Driver->getWindowWidth()) {
        Driver->clearBuffers(CRGBA::Black);
        renderSceneScreenShot(left, right, top, bottom,
                              ClientCfg.ScreenShotWidth,
                              ClientCfg.ScreenShotHeight);
        // Get the bitmap
        Driver->getBuffer(temp);
        Driver->swapBuffers();
        btm.blit(temp, 0, Driver->getWindowHeight() - (bottom - top),
                 right - left, bottom - top, left, top);

        // Next
        right = std::min(right + Driver->getWindowWidth(),
                         ClientCfg.ScreenShotWidth);
      }

      // Next
      bottom = std::min(bottom + Driver->getWindowHeight(),
                        ClientCfg.ScreenShotHeight);
    }
  } else {
...
bool screenshotZBuffer(const std::string &filename) {
  ...
  std::string filename_z =
      filename.substr(0, pos) + "_z" + filename.substr(pos);
...
static std::string findNewScreenShotFileName(std::string filename) {
  ...
  if (tms) {
    ...
    if (pos == std::string::npos)
      ...
      filename =
          filename.substr(0, pos) + "_" + cstime + "_" + filename.substr(pos);
  }
...
void screenShotTGA() {
  ...
  nlinfo("Screenshot '%s' saved in tga format (%ux%u)", filename.c_str(),
         ClientCfg.ScreenShotWidth, ClientCfg.ScreenShotHeight);
...
void screenShotPNG() {
  ...
  nlinfo("Screenshot '%s' saved in png format (%ux%u)", filename.c_str(),
         ClientCfg.ScreenShotWidth, ClientCfg.ScreenShotHeight);
  displayScreenShotSavedInfo(filename);
}
...
void screenShotJPG() {
  ...
  nlinfo("Screenshot '%s' saved in jpg format (%ux%u)", filename.c_str(),
         ClientCfg.ScreenShotWidth, ClientCfg.ScreenShotHeight);
  displayScreenShotSavedInfo(filename);
}
...
// ***************************************************************************
// Reply to the last people who talked in the chat -> this change the target of
// the main chat to the name of the last teller
  ...
  void execute(CCtrlBase * /* pCaller */, const std::string & /* params */) {
    if (!PeopleInterraction.LastSenderName.empty()) {
      ...
      if (w) {
        w->setKeyboardFocus();
        w->enableBlink(1);
        PeopleInterraction.ChatGroup.Filter.setTargetPlayer(
            CEntityCL::removeTitleAndShardFromName(
...
// ***************************************************************************
// Reply to the last people who talked in the chat only once (display '/tell
// name' in the last activated chat window)
 ...
 * Focus must be in a window with a target (main chat or user chat), otherwise
 * the main chat is used
 */
...
bool CStringPostProcessRemoveName::cbIDStringReceived(string &inOut) {
  ...
  if (!strNewTitle.empty()) {
    inOut = STRING_MANAGER::CStringManagerClient::getTitleLocalizedName(
        strNewTitle, Woman);
    {
      ...
      if (pos != string::npos) {
        inOut = STRING_MANAGER::CStringManagerClient::getTitleLocalizedName(
            CEntityCL::getTitleFromName(inOut), Woman);
      }
    }
  } else
    inOut.clear();
