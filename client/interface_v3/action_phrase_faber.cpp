...
const std::string ItemResultSheetColor =
    "UI:PHRASE:FABER:RESULT_ITEM:USER_COLOR";
const std::string ItemResultSheetClassType =
    "UI:PHRASE:FABER:RESULT_ITEM:RM_CLASS_TYPE";
const std::string ItemResultSheetStatType =
    "UI:PHRASE:FABER:RESULT_ITEM:RM_FABER_STAT_TYPE";
const std::string FaberPhraseWindow = "ui:interface:phrase_faber_execution";
const std::string FaberPhraseItemReqLine =
    FaberPhraseWindow + ":header_opened:item_reqs:item_req_%d";
const std::string FaberPhraseList = "list";
const std::string FaberPhraseText = "text";
const std::string FaberPhraseIcon = "icon";
const std::string FaberPhraseValidButton =
    FaberPhraseWindow + ":header_opened:ok_cancel:ok";
const std::string FaberPhraseFpCtrl =
    FaberPhraseWindow + ":header_opened:faber_plan";
const std::string FaberPhraseFpSuccessText =
    FaberPhraseWindow + ":header_opened:success_text";
const std::string FaberPhraseMpListModal =
    "ui:interface:phrase_faber_mp_selection";
const std::string FaberPhraseMpQuantityModal =
    "ui:interface:phrase_faber_mp_quantity";
const std::string FaberPhraseItemResultGroup =
    FaberPhraseWindow + ":header_opened:item_result";

#define MAX_MP_SELECTION_ENTRIES 256

// ***************************************************************************
CActionPhraseFaber::CActionPhraseFaber() {
  uint size = MAX_PLAYER_INV_ENTRIES +
              (MAX_ANIMALINV_ENTRIES * MAX_INVENTORY_ANIMAL) +
...
// ***************************************************************************
void CActionPhraseFaber::fillDBWithMP(const std::string &sheetBase,
                                      const CItem &item) {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();

  NLGUI::CDBManager::getInstance()
      ->getDbProp(sheetBase + ":SHEET")
      ->setValue32(item.Sheet.asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp(sheetBase + ":QUALITY")
      ->setValue32(item.Quality);
  NLGUI::CDBManager::getInstance()
      ->getDbProp(sheetBase + ":QUANTITY")
      ->setValue32(item.Quantity);
  NLGUI::CDBManager::getInstance()
      ->getDbProp(sheetBase + ":USER_COLOR")
      ->setValue32(item.UserColor);
  NLGUI::CDBManager::getInstance()
      ->getDbProp(sheetBase + ":CHARAC_BUFFS")
      ->setValue32(item.CharacBuffs);
  NLGUI::CDBManager::getInstance()
      ->getDbProp(sheetBase + ":WEIGHT")
      ->setValue32(item.Weight);
}

// ***************************************************************************
void CActionPhraseFaber::launchFaberCastWindow(sint32 memoryLine,
                                               uint memoryIndex,
  ...
  // get the family of item plan (for selection) from the rootBrick. It is
  // stored in the Property0.
  ...
  if (!rootBrick->Properties.empty()) {
    ...
    if (strList.size() >= 2 && strList[0] == "FPLAN:") {
      for (uint i = 1; i < strList.size(); i++) {
        BRICK_FAMILIES::TBrickFamily bfam =
            BRICK_FAMILIES::toSBrickFamily(strList[i]);
  ...
  if (_FaberPlanBrickFamilies.empty()) {
    nlwarning("ERROR: The Craft Root %s does not contain a valid FPLAN: "
              "property -> Can't select plan to craft",
              rootBrick->Id.toString().c_str());
    return;
  }

  // **** Hide all widgets, MP Ctrls, and reset DB, until the Plan is not
  // selected
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  // Hide the valid button
  CCtrlBaseButton *validButton = dynamic_cast<CCtrlBaseButton *>(
  ...
  for (itemReqLine = 0; itemReqLine < MAX_ITEM_REQ_LINE; itemReqLine++) {
    ...
    for (uint mpSlot = 0; mpSlot < MAX_MP_SLOT; mpSlot++) {
      CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
          toString("%s:%d:%d:SHEET", MPFaberDB.c_str(), itemReqLine, mpSlot),
          false);
      if (node)
        node->setValue32(0);
    }
    ...
    CInterfaceGroup *itemReqLineGroup = dynamic_cast<CInterfaceGroup *>(
        CWidgetManager::getInstance()->getElementFromId(
            toString(FaberPhraseItemReqLine.c_str(), itemReqLine)));
    if (itemReqLineGroup)
  ...
  CCDBNodeLeaf *node =
      NLGUI::CDBManager::getInstance()->getDbProp(FaberPlanDB, false);
  if (node)
    node->setValue32(0);
  ...
  CInterfaceGroup *groupMp = dynamic_cast<CInterfaceGroup *>(
      CWidgetManager::getInstance()->getElementFromId(
          FaberPhraseItemResultGroup));
  if (groupMp)
    groupMp->setActive(false);

  // **** Open the window!
  CGroupContainer *window = dynamic_cast<CGroupContainer *>(
  ...
  if (branch)
    NLGUI::CDBManager::getInstance()->removeBranchObserver(
        "LOCAL:INVENTORY:BAG", &_DBInventoryObs);
  if (branch)
    NLGUI::CDBManager::getInstance()->addBranchObserver("LOCAL:INVENTORY:BAG",
                                                        &_DBInventoryObs);

  // and for all pack animals
  uint i;
  for (i = 0; i < MAX_INVENTORY_ANIMAL; i++) {
    branch = NLGUI::CDBManager::getInstance()->getDbBranch(
        toString("LOCAL:INVENTORY:PACK_ANIMAL%d", i));
    if (branch)
      NLGUI::CDBManager::getInstance()->removeBranchObserver(
          toString("LOCAL:INVENTORY:PACK_ANIMAL%d", i).c_str(),
          &_DBInventoryObs);
    if (branch)
      NLGUI::CDBManager::getInstance()->addBranchObserver(
          toString("LOCAL:INVENTORY:PACK_ANIMAL%d", i).c_str(),
          &_DBInventoryObs);
  }

  // Add observers on animal status, cause inventory may become unavailabe
  // during the process
  for (i = 0; i < MAX_INVENTORY_ANIMAL; i++) {
...
void CActionPhraseFaber::onCloseFaberCastWindow() {
  ...
  branch = NLGUI::CDBManager::getInstance()->getDbBranch("LOCAL:INVENTORY:BAG");
  if (branch)
    branch->removeBranchObserver(&_DBInventoryObs);
  // and for all pack animals
  ...
  // remove observers on animal status, cause inventory may become unavailabe
  // during the process
  for (uint i = 0; i < MAX_INVENTORY_ANIMAL; i++) {
    CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
        toString("SERVER:PACK_ANIMAL:BEAST%d:STATUS", i), false);
...
// ***************************************************************************
void CActionPhraseFaber::fillFaberPlanSelection(const std::string &brickDB,
                                                uint maxSelection) {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  ...
  for (i = 0; i < _FaberPlanBrickFamilies.size(); i++) {
    const std::vector<NLMISC::CSheetId> &famBricks =
        pBM->getFamilyBricks(_FaberPlanBrickFamilies[i]);
  ...
  for (i = 0; i < maxSelection; i++) {
    if (i < num)
      NLGUI::CDBManager::getInstance()
          ->getDbProp(brickDB + ":" + toString(i) + ":SHEET")
          ->setValue32(bricks[i].asInt());
    else
      NLGUI::CDBManager::getInstance()
          ->getDbProp(brickDB + ":" + toString(i) + ":SHEET")
          ->setValue32(0);
...
CItemImage *CActionPhraseFaber::getInvMirrorItemImage(uint slotIndex,
    ...
    if (getInventory().isInventoryAvailable(INVENTORIES::guild)) {
      ...
      CCDBNodeBranch *itemBranch =
          NLGUI::CDBManager::getInstance()->getDbBranch(
              SERVER_INVENTORY ":GUILD:" + toString(slotIndex));
  ...
  if (slotIndex < MAX_ROOMINV_ENTRIES) {
    if (getInventory().isInventoryAvailable(INVENTORIES::player_room)) {
      ...
      CCDBNodeBranch *itemBranch =
          NLGUI::CDBManager::getInstance()->getDbBranch(
              SERVER_INVENTORY ":ROOM:" + toString(slotIndex));
      static CItemImage image;
      image.build(itemBranch);
...
// ***************************************************************************
bool CActionPhraseFaber::isMpAvailable(CItemSheet *mpSheet, uint invId,
                                       uint slotIndex) const {
  return mpSheet && mpSheet->Family == ITEMFAMILY::RAW_MATERIAL &&
         getInventory().isInventoryAvailable((INVENTORIES::TInventory)invId);
}

// ***************************************************************************
void CActionPhraseFaber::validateFaberPlanSelection(
    CSBrickSheet *itemPlanBrick) {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  ...
  // TestYoyo
  /*for(uint
  tam=0;tam<_ExecuteFromItemPlanBrick->FaberPlan.ItemPartMps.size();tam++)
  {
          _ExecuteFromItemPlanBrick->FaberPlan.ItemPartMps[tam].Quantity= 20;
  }
  _ExecuteFromItemPlanBrick->FaberPlan.FormulaMps.resize(2);
  _ExecuteFromItemPlanBrick->FaberPlan.FormulaMps[0].ItemRequired=
  CSheetId("m0152chdca01.sitem");
  _ExecuteFromItemPlanBrick->FaberPlan.FormulaMps[0].Quantity= 13;
  _ExecuteFromItemPlanBrick->FaberPlan.FormulaMps[1].ItemRequired=
  CSheetId("m0691chdca01.sitem");
  ...
  */
  ...
  _MPBuildNumItemPartReq =
      min((uint)MAX_ITEM_REQ_LINE,
          (uint)_ExecuteFromItemPlanBrick->FaberPlan.ItemPartMps.size());
  _MPBuildNumSpecificItemReq =
      min(((uint)MAX_ITEM_REQ_LINE - _MPBuildNumItemPartReq),
          (uint)_ExecuteFromItemPlanBrick->FaberPlan.FormulaMps.size());
  ...
  node =
      NLGUI::CDBManager::getInstance()->getDbProp(ItemResultQuantityDB, false);
  if (node)
  ...
  for (itemReqLine = 0; itemReqLine < _MPBuildNumTotalItemReq; itemReqLine++) {
    ...
    if (itemReqLine < _MPBuildNumItemPartReq) {
      ...
      mpBuild.FaberTypeRequired =
          _ExecuteFromItemPlanBrick->FaberPlan.ItemPartMps[itemPartId]
              .FaberTypeFilter;
      mpBuild.QuantityReq =
          _ExecuteFromItemPlanBrick->FaberPlan.ItemPartMps[itemPartId].Quantity;
    }
    // Then go the Specific item reqs
      ...
      mpBuild.SpecificItemRequired =
          _ExecuteFromItemPlanBrick->FaberPlan.FormulaMps[itemSpecificId]
              .ItemRequired;
      mpBuild.QuantityReq =
          _ExecuteFromItemPlanBrick->FaberPlan.FormulaMps[itemSpecificId]
              .Quantity;
    ...
    if (itemReqLineGroup) {
      if (itemReqLine < _MPBuildNumTotalItemReq) {
        ...
        CViewText *viewText = dynamic_cast<CViewText *>(
            itemReqLineGroup->getView(FaberPhraseText));
        if (viewText) {
          string text;
          if (mpBuild.RequirementType == CMPBuild::ItemPartReq) {
            ...
            strFindReplace(
                text, "%f",
                RM_FABER_TYPE::toLocalString(mpBuild.FaberTypeRequired));
          } else if (mpBuild.RequirementType == CMPBuild::SpecificItemReq) {
            text = STRING_MANAGER::CStringManagerClient::getItemLocalizedName(
                mpBuild.SpecificItemRequired);
          } else {
            nlstop;
          }
        ...
        // Set as Icon the required MP FaberType / or Sheet Texture
        // (directly...)
        CViewBitmap *viewBmp = dynamic_cast<CViewBitmap *>(
            itemReqLineGroup->getView(FaberPhraseIcon));
        if (viewBmp) {
          if (mpBuild.RequirementType == CMPBuild::ItemPartReq) {
            ...
            // NB: the texture is scaled, so it's ok to put the item 40x40
            // texture
            const CItemSheet *itemSheet = dynamic_cast<const CItemSheet *>(
                SheetMngr.get(mpBuild.SpecificItemRequired));
            if (itemSheet)
  ...
  if (window) {
    ...
    strFindReplace(title, "%item",
                   STRING_MANAGER::CStringManagerClient::getItemLocalizedName(
                       _ExecuteFromItemPlanBrick->FaberPlan.ItemBuilt));
    window->setTitle(title);
  }
...
void CActionPhraseFaber::resetSelection() {
  ...
  for (uint i = 0; i < MAX_MP_SELECTION_ENTRIES; i++) {
    NLGUI::CDBManager::getInstance()
        ->getDbProp(MPSelectionDB + ":" + toString(i) + ":SHEET")
        ->setValue32(0);
  }
}
...
// ***************************************************************************
void CActionPhraseFaber::filterSelectionItemPart(
    std::vector<uint> &mps, RM_FABER_TYPE::TRMFType itemPartFilter,
  ...
  for (uint i = 0; i < mps.size(); i++) {
    // get the item sheet
    const CItemSheet *itemSheet = dynamic_cast<const CItemSheet *>(
        SheetMngr.get(_InventoryMirror[mps[i]].Sheet));
...
// ***************************************************************************
void CActionPhraseFaber::filterSelectionItemSpecific(
    std::vector<uint> &mps, NLMISC::CSheetId specificItemWanted) {
  std::vector<uint> res;
  res.reserve(mps.size());
...
void CActionPhraseFaber::startMpSelection(uint itemReqLine, uint mpSlot) {
  ...
  CInterfaceGroup *itemReqLineGroup = dynamic_cast<CInterfaceGroup *>(
      CWidgetManager::getInstance()->getElementFromId(
          toString(FaberPhraseItemReqLine.c_str(), itemReqLine)));
  if (itemReqLineGroup) {
    CDBGroupListSheet *listSheet = dynamic_cast<CDBGroupListSheet *>(
        itemReqLineGroup->getGroup(FaberPhraseList));
    if (listSheet)
  ...
  // If the slot selected is already filled, Launch the MP Quantity selection
  // modal
  if (mpSlot < mpBuild.NumMpSlot) {
    ...
    CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
        MPQuantitySelectDb + ":CUR_QUANTITY", false);
    if (node)
      node->setValue32(maxQuantity);
    node = NLGUI::CDBManager::getInstance()->getDbProp(
        MPQuantitySelectDb + ":MAX_QUANTITY", false);
    if (node)
      node->setValue32(maxQuantity);
    ...
    CInterfaceGroup *quantityModal = dynamic_cast<CInterfaceGroup *>(
        CWidgetManager::getInstance()->getElementFromId(
            FaberPhraseMpQuantityModal));
    if (quantityModal) {
      CGroupEditBox *eb =
          dynamic_cast<CGroupEditBox *>(quantityModal->getGroup("eb"));
      if (eb) {
  ...
  // else select new MP
    ...
    for (uint i = 0; i < _InventoryMirror.size(); i++) {
      // If still some MP on this stack, and if not already selected, add to
      // selection
      if (_InventoryMirror[i].Quantity > 0 &&
          (_InventoryMirror[i].Selected & (1 << itemReqLine)) == 0) {
        selectMps.push_back(i);
      }
    }

    // Filter the selection whether it is an itemPart or specificItem
    // reqiurement
    if (mpBuild.RequirementType == CMPBuild::ItemPartReq) {
      CItemSheet *itemBuilt = dynamic_cast<CItemSheet *>(
          SheetMngr.get(_ExecuteFromItemPlanBrick->FaberPlan.ItemBuilt));
      ITEM_ORIGIN::EItemOrigin itemOrigin =
          itemBuilt ? itemBuilt->ItemOrigin : ITEM_ORIGIN::UNKNOWN;
    ...
    CWidgetManager::getInstance()->enableModalWindow(ctrlSlot,
...
void CActionPhraseFaber::validateMpSelection(uint selectId) {
  ...
  // it may be possible (by update DB and error) that selected slot is no more
  // usable => just quit
  if (quantity <= 0)
    return;
  ...
  fillDBWithMP(toString("%s:%d:%d", MPFaberDB.c_str(), itemReqLine,
                        mpBuild.NumMpSlot - 1),
               item);
  ...
  // must hide the modal window which had open us. NB: must be done here because
  // next, we'll open the MP quantity selection
  CWidgetManager::getInstance()->disableModalWindow();
...
void CActionPhraseFaber::validateMpSelectQuantity() {
  ...
  CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
      MPQuantitySelectDb + ":CUR_QUANTITY", false);
  if (node)
    quantitySelected = node->getValue32();

  // maximize (if error)
  quantitySelected =
      min(quantitySelected, getMaxQuantityChange(itemReqLine, mpSlot));
  ...
  if (quantitySelected == 0) {
    ...
    fillDBWithMP(toString("%s:%d:%d", MPFaberDB.c_str(), itemReqLine, mpSlot),
                 item);
  }
...
void CActionPhraseFaber::validateExecution() {
  ...
  for (uint itemReqLine = 0; itemReqLine < _MPBuildNumTotalItemReq;
       itemReqLine++) {
    CMPBuild &mpBuild = _MPBuild[itemReqLine];
    ...
    for (uint mpSlot = 0; mpSlot < mpBuild.NumMpSlot; mpSlot++) {
      ...
      nlassert(invSlot < _InventoryMirror.size());
      item.setInvId(
          INVENTORIES::TInventory(_InventoryMirror[invSlot].InventoryId));
  ...
  pPM->executeCraft(_ExecuteFromMemoryLine, _ExecuteFromMemoryIndex,
                    _ExecuteFromItemPlanBrick->Id.asInt(), mpItemPartList,
                    specificItemList);
  ...
  /*CInterfaceManager	*pIM= CInterfaceManager::getInstance();
  CInterfaceElement	*window=
  CWidgetManager::getInstance()->getElementFromId(FaberPhraseWindow); if(window)
...
// ***************************************************************************
uint CActionPhraseFaber::getMaxQuantityChange(uint itemReqLine,
                                              uint mpSlot) const {
  nlassert(itemReqLine < MAX_ITEM_REQ_LINE);
  ...
  // This is the quantity already selected for this itemReqLine, + rest in
  // inventory
  uint maxQuantity =
      _MPBuild[itemReqLine].QuantitySelected[mpSlot] + item.Quantity;
  // maximize with the rest of quantity with have to setup (remove us btw)
  maxQuantity =
      min(maxQuantity, _MPBuild[itemReqLine].QuantityReq -
                           (getTotalQuantitySetuped(itemReqLine) -
                            _MPBuild[itemReqLine].QuantitySelected[mpSlot]));

  return maxQuantity;
}

// ***************************************************************************
void CActionPhraseFaber::updateEmptySlot(uint itemReqLine,
                                         CInterfaceGroup *itemReqLineGroup) {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();

  if (!itemReqLineGroup)
    itemReqLineGroup = dynamic_cast<CInterfaceGroup *>(
        CWidgetManager::getInstance()->getElementFromId(
            toString(FaberPhraseItemReqLine.c_str(), itemReqLine)));
  if (!itemReqLineGroup)
    return;
  ...
  // NB: forceValidity calls invalidateCoords() if state change => dont "clear
  // then set".

  // All Setuped?
  bool allSetuped =
      getTotalQuantitySetuped(itemReqLine) >= _MPBuild[itemReqLine].QuantityReq;
  ...
  // Special for Specific Item requirement. Setup grayed item for the last empty
  // slot
  for (uint i = 0; i < MAX_MP_SLOT; i++) {
    CMPBuild &mpBuild = _MPBuild[itemReqLine];
    ...
    if (i >= mpBuild.NumMpSlot) {
      ...
      if (!allSetuped && i == mpBuild.NumMpSlot &&
          mpBuild.RequirementType == CMPBuild::SpecificItemReq) {
        item.Sheet = mpBuild.SpecificItemRequired;
      }
      fillDBWithMP(toString("%s:%d:%d", MPFaberDB.c_str(), itemReqLine, i),
                   item);
    }

    // *** Grayed,NoQuantity,NoQuality for the last slot of a specific
    // requirement
...
void CActionPhraseFaber::updateQuantityView(uint itemReqLine) {
  ...
  CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
      toString("%s:%d:SELECTED", MPQuantityDb.c_str(), itemReqLine), false);
  if (node)
    node->setValue32(getTotalQuantitySetuped(itemReqLine));
  node = NLGUI::CDBManager::getInstance()->getDbProp(
      toString("%s:%d:REQUIRED", MPQuantityDb.c_str(), itemReqLine), false);
  if (node)
...
void CActionPhraseFaber::updateValidButton() {
  ...
  if (_ExecuteFromItemPlanBrick) {
    ...
    for (uint itemReqLine = 0; itemReqLine < _MPBuildNumTotalItemReq;
         itemReqLine++) {
      canValid = canValid && getTotalQuantitySetuped(itemReqLine) ==
                                 _MPBuild[itemReqLine].QuantityReq;
    }
  } else
    canValid = false;
  ...
  CCtrlBaseButton *validButton = dynamic_cast<CCtrlBaseButton *>(
      CWidgetManager::getInstance()->getElementFromId(FaberPhraseValidButton));
  if (validButton)
    validButton->setFrozen(!canValid);
}
...
void CActionPhraseFaber::deleteMpSlot(uint itemReqLine, uint mpSlot) {
  ...
  nlassert(invSlot < _InventoryMirror.size());
  nlassert(_InventoryMirror[invSlot].Selected & (1 << itemReqLine));
  // NB: possible that mpBuild.QuantitySelected[mpSlot]==0 (call from
  // removeMpSlotThatUseInvSlot())
  ...
  CItem item;
  fillDBWithMP(
      toString("%s:%d:%d", MPFaberDB.c_str(), itemReqLine, mpBuild.NumMpSlot),
      item);
}
...
public:
  virtual void execute(CCtrlBase *pCaller, const string &Params) {
    ...
    if (ActionPhraseFaber == NULL)
      ActionPhraseFaber = new CActionPhraseFaber;
...
REGISTER_ACTION_HANDLER(CHandlerPhraseFaberValidateMP,
...
public:
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    ...
    CCtrlBaseButton *button = dynamic_cast<CCtrlBaseButton *>(
        CWidgetManager::getInstance()->getElementFromId(
            FaberPhraseValidButton));

    // Ok, button found. test if active.
    if (button && !button->getFrozen()) {
      // Act as if the player click on this button
      CAHManager::getInstance()->runActionHandler("phrase_faber_validate",
                                                  button);
    }
  }
};
REGISTER_ACTION_HANDLER(CHandlerPhraseFaberValidateOnEnter,
...
REGISTER_ACTION_HANDLER(CHandlerPhraseFaberSelectMpQuantity,
                        "phrase_faber_select_mp_quantity");

// ***************************************************************************
void launchFaberCastWindow(sint32 memoryLine, uint memoryIndex,
                           CSBrickSheet *rootBrick) {
  if (ActionPhraseFaber == NULL)
    ActionPhraseFaber = new CActionPhraseFaber;
...
// ***************************************************************************
void CActionPhraseFaber::removeMpSlotThatUseInvSlot(uint invSlot,
  ...
  for (uint itemReqLine = 0; itemReqLine < _MPBuildNumTotalItemReq;
    ...
    for (uint mpSlot = 0; mpSlot < mpBuild.NumMpSlot;) {
      ...
      if (mpBuild.Id[mpSlot] == invSlot) {
        ...
        removeSlotQuantity =
            min(quantityToRemove, mpBuild.QuantitySelected[mpSlot]);
        ...
        if (mpBuild.QuantitySelected[mpSlot] == 0) {
          // NB: nothing is restored to inventory since
          // mpBuild.QuantitySelected[mpSlot] is reseted before
          deleteMpSlot(itemReqLine, mpSlot);
        } else {
          ...
          item.Quantity = mpBuild.QuantitySelected[mpSlot];
          fillDBWithMP(
              toString("%s:%d:%d", MPFaberDB.c_str(), itemReqLine, mpSlot),
              item);
...
void CActionPhraseFaber::onInventoryChange() {
  ...
  // If the Faber Plan has not yet been selected, then must not check
  // _InventoryMirror, since not initialized
    ...
    if (itemImage) {
      ...
      */
      ...
      // else must test change or remove
        ...
        // if the Mp was deleted from this slot, delete it from all faber
        // execution
        if (!sameMp) {
        ...
        // test change of quantity
          ...
          if (curInvItem.OriginalQuantity != newInvItem.OriginalQuantity) {
            ...
            if (newInvItem.OriginalQuantity > curInvItem.OriginalQuantity) {
              // just add the difference to the original and current setuped
              // quantity
              uint32 diff =
                  newInvItem.OriginalQuantity - curInvItem.OriginalQuantity;
              ...
              // The remaining quantity is not enough, must also remove from
              // mpSlot that use it!
              else {
  ...
  if (displayChange) {
    ...
    if (quantityModal &&
      ...
      CWidgetManager::getInstance()->disableModalWindow();
    CInterfaceGroup *listModal = dynamic_cast<CInterfaceGroup *>(
        CWidgetManager::getInstance()->getElementFromId(
            FaberPhraseMpListModal));
    if (listModal &&
...
void CActionPhraseFaber::updateItemResult() {
  ...
  uint phraseSlot =
      pPM->getMemorizedPhrase(_ExecuteFromMemoryLine, _ExecuteFromMemoryIndex);
  ...
  for (i = 0; i < _MPBuildNumTotalItemReq; i++) {
    ...
    if (mpBuild.RequirementType == CMPBuild::ItemPartReq) {
      ...
      for (uint j = 0; j < nSlot; j++) {
        ...
        CItemSheet *mp = dynamic_cast<CItemSheet *>(
            SheetMngr.get(_InventoryMirror[mpBuild.Id[j]].Sheet));
        if (mp && mp->canBuildItemPart(mpBuild.FaberTypeRequired)) {
          ...
          const CItemSheet::CMpItemPart &mpIP =
              mp->getItemPart(mpBuild.FaberTypeRequired);

          // append to the stats
          uint numMps = mpBuild.QuantitySelected[j];
          for (uint k = 0; k < RM_FABER_STAT_TYPE::NumRMStatType; k++) {
            if (RM_FABER_STAT_TYPE::isStatRelevant(
                    mpBuild.FaberTypeRequired,
                    (RM_FABER_STAT_TYPE::TRMStatType)k)) {
              // %age to 0-1.
  ...
  RM_FABER_STAT_TYPE::stretchItemStats(
      statArray, itemStatBF, totalItemPartMPSetuped >= totalItemPartMPReq);

  // **** setup Level
  if (minLevel == INT_MAX)
    minLevel = 0;
  CCDBNodeLeaf *node =
      NLGUI::CDBManager::getInstance()->getDbProp(ItemResultSheetLevel, false);
  if (node)
    node->setValue32(minLevel);

  // **** change success rate too
  CViewText *successView =
      dynamic_cast<CViewText *>(CWidgetManager::getInstance()->getElementFromId(
          FaberPhraseFpSuccessText));
  if (successView) {
    string text = CI18N::get("uiPhraseFaberSuccessRate");
    // Get the success rate of the related phrase
    uint phraseSlot = pPM->getMemorizedPhrase(_ExecuteFromMemoryLine,
                                              _ExecuteFromMemoryIndex);

    sint32 craftSuccessModifier = 0;
    CCDBNodeLeaf *nodeCSM = NLGUI::CDBManager::getInstance()->getDbProp(
        "SERVER:CHARACTER_INFO:SUCCESS_MODIFIER:CRAFT", false);
    if (nodeCSM) {
      craftSuccessModifier = nodeCSM->getValue32();
    }
    // With the faber plan skill
    sint success = pPM->getCraftPhraseSuccessRate(
        pPM->getPhrase(phraseSlot), _ExecuteFromItemPlanBrick->getSkill(),
        minLevel, 0);
    sint bonus =
        pPM->getCraftPhraseSuccessRate(pPM->getPhrase(phraseSlot),
                                       _ExecuteFromItemPlanBrick->getSkill(),
                                       minLevel, (sint)craftSuccessModifier) -
        success;
    string successStr;
    if (bonus == 0) {
    ...
    {
      successStr = "@{0F0F}" + toString(success + bonus) + "@{FFFF}(" +
                   toString(success) + "@{0F0F} + " + toString(bonus) +
                   "@{FFFF})";
    } else {
      successStr = "@{E42F}" + toString(success + bonus) + "@{FFFF}(" +
                   toString(success) + "@{E42F} - " + toString(abs(bonus)) +
                   "@{FFFF})";
    }
  ...
  node =
      NLGUI::CDBManager::getInstance()->getDbProp(ItemResultSheetColor, false);
  if (node)
    node->setValue32(dominanteColor);

  // **** Get Stat validity
  uint64 itemStatFinalUsageBF = 0;
  // Some stat (magic protection and magic resist) are finaly used in the item
  // only for the best ones
  itemStatFinalUsageBF =
      RM_FABER_STAT_TYPE::getStatFinalValidity(statArray, itemStatBF);
  ...
  if (groupMp) {
    ...
    for (i = 0; i < RM_FABER_STAT_TYPE::NumRMStatType; i++) {
      ...
      if (groupStat) {
        ...
        CDBViewBar *statValueBar = dynamic_cast<CDBViewBar *>(
            groupStat->getElement(groupStat->getId() + ":bar"));
        CViewText *statValueText = dynamic_cast<CViewText *>(
            groupStat->getElement(groupStat->getId() + ":textstat"));
        CCtrlBase *statToolTip = dynamic_cast<CCtrlBase *>(
            groupStat->getElement(groupStat->getId() + ":tt"));
        ...
        if (statToolTip) {
          if (finalyUsed) {
            ...
            if (RM_FABER_STAT_TYPE::isMagicResistStat(
                    RM_FABER_STAT_TYPE::TRMStatType(i)) ||
                RM_FABER_STAT_TYPE::isMagicProtectStat(
                    RM_FABER_STAT_TYPE::TRMStatType(i)))
              statToolTip->setDefaultContextHelp(
                  CI18N::get("uiFaberStatActive"));
            else
  ...
  node = NLGUI::CDBManager::getInstance()->getDbProp(ItemResultSheetStatType,
                                                     false);
  if (node) {
    float bestStatValue = -1.0f;
    RM_FABER_STAT_TYPE::TRMStatType bestStat =
  ...
  node = NLGUI::CDBManager::getInstance()->getDbProp(ItemResultSheetClassType,
                                                     false);
  if (node) {
    // Setup DB
    node->setValue32(
        RM_CLASS_TYPE::getItemClass((uint32)(100.0f * statEnergy)));
  }
}
