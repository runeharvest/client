...
public:
  ...
  // Tells if this group actually want guildName or guildSymbol => must rebuild
  // interface if changes
...
protected:
  enum TBar {
    ...
    HP = 0,
    Time = HP, // If making changes to this enum, please update
    SAP,
    Amount = SAP, // CForageSourceCL::updateVisualPropertyBars() and
    STA,
    Life = STA, // updateVisiblePostPos() because the HP/SAP/STA
    Focus,
    Danger = Focus, // properties are *not* in an array in CEntityCL.
    Action,
    Spawn = Action,
    NumBars
  };
  ...
  static void getBarSettings(CInterfaceManager *pIM, bool isUser, bool isPlayer,
                             bool isFriend, int &dbEntry, bool *bars);
  ...
  static CGroupInSceneUserInfo *newGroupInScene(const std::string &templateName,
                                                const std::string &id);
};
