...
void CItemConsumableEffectHelper::getItemConsumableEffectText(
    const CItemSheet *pIS, string &itemText, sint32 itemQuality) {
  ...
  for (i = 0; i < pIS->Consumable.Properties.size(); ++i) {
    ...
    if (name == "SP_CHG_CHARAC") {
      CHARACTERISTICS::TCharacteristics charac =
          CHARACTERISTICS::toCharacteristic(params[0]);
    ...
    if (name == "SP_MOD_DEFENSE") {
      ...
      else if (toLowerAscii(params[0]) == "parry")
    ...
    if (!result.empty()) {
      ...
      if (modifier < 0)
        strFindReplace(result, "%modifier",
                       "@{E42F}-" + toString(modifier) + "@{FFFF}");
      else
        strFindReplace(result, "%modifier",
                       "@{0F0F}" + toString(modifier) + "@{FFFF}");
