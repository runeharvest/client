...
// ***************************************************************************
const std::string CDBGroupBuildPhrase::BrickSelectionModal =
...
NLMISC_REGISTER_OBJECT(CViewBase, CDBGroupBuildPhrase, std::string,
                       "build_phrase");
...
bool CDBGroupBuildPhrase::parse(xmlNodePtr cur, CInterfaceGroup *parentGroup) {
  ...
  _TextureIdSlotDisabled =
      rVR.getTextureIdFromName("w_slot_brick_disabled.tga");
  ...
  for (i = 0; i < MaxBricks; i++) {
    NLGUI::CDBManager::getInstance()->getDbProp(
        BrickBuildDB + ":MAIN:" + toString(i) + ":SHEET");
    for (uint j = 0; j < MaxParam; j++)
      NLGUI::CDBManager::getInstance()->getDbProp(BrickBuildDB +
                                                  ":PARAM:" + toString(i) +
                                                  ":" + toString(j) + ":SHEET");
  }
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp(BrickBuildDB + ":EDITION_PHRASE:PHRASE")
      ->setValue32(CSPhraseManager::EditionSlot);
...
void CDBGroupBuildPhrase::setupBuildSentence() {
  ...
  _ValidateButton =
      dynamic_cast<CCtrlBaseButton *>(CInterfaceGroup::getCtrl("ok_cancel:ok"));
  if (_ValidateButton)
    _ValidateButton->setFrozen(true);

  _TextPhraseDesc =
      dynamic_cast<CViewText *>(CInterfaceGroup::getView("infos:phrase_desc"));
  ...
  for (;;) {
    ...
    if (ctrl && back && cost && info) {
      ...
      for (uint i = 0; i < MaxParam; i++) {
        idCtrl =
            string("bricks:param_brick") + toString(index) + "_" + toString(i);
        idBack =
            string("bricks:param_back") + toString(index) + "_" + toString(i);
        ctrl = dynamic_cast<CDBCtrlSheet *>(CInterfaceGroup::getCtrl(idCtrl));
        back = dynamic_cast<CViewBitmap *>(CInterfaceGroup::getView(idBack));
        if (ctrl && back) {
  ...
  if (_GroupValid) {
    ...
    fromString(CWidgetManager::getInstance()->getParser()->getDefine(
                   "phrase_build_root_info_maxw"),
               rootTextMaxw);
    _MainWords[0].InfoView->setLineMaxW(rootTextMaxw);
  }

  // Get the sentence texts
  _UserSentenceName = dynamic_cast<CGroupEditBox *>(
      CInterfaceGroup::getGroup("eb_spell_name:eb"));

  // Get the SpellView
  _SpellView =
      dynamic_cast<CDBCtrlSheet *>(CInterfaceGroup::getCtrl("spell_view"));
}

// ***************************************************************************
void CDBGroupBuildPhrase::draw() { CInterfaceGroup::draw(); }
...
void CDBGroupBuildPhrase::fillSelectionMain(uint index) {
  ...
  if (!brick) {
    ...
    // fill selection with all bricks of the same family (whatever the main
    // brick)
  ...
  if (brick && brick->isCombat() && brick->isOptional()) {
    // Ensure not same bricks are setuped. Also don't insert me since I am
    // already here...
    filterBrickSetuped(bricks);
    // Ensure only optional of compatible Skill are inserted. Don't test with
    // me, since i may be removed!
    filterSkillSetuped(bricks, true, false, index);
  }
  // for power optional, must filter by brick
  else if (brick && brick->isSpecialPower() &&
           (brick->isOptional() || brick->isMandatory())) {
    // Ensure not same bricks are setuped. Also don't insert me since I am
    // already here...
    filterBrickSetuped(bricks);
  }
...
void CDBGroupBuildPhrase::validateMain(uint index, const CSBrickSheet *sheet) {
  ...
  // If the brick chosen is actually the "Remove Brick", then call the correct
  // method
...
// ***************************************************************************
void CDBGroupBuildPhrase::validateParam(uint index, uint paramIndex,
                                        const CSBrickSheet *sheet) {
  // the root must be OK.
  if (!_GroupValid)
    return;
  const CSBrickSheet *rootBrick = getRootBrick();
  ...
  if (sheet)
    newParamSons = sheet->ParameterFamilies;
  mustResetParamHrc =
      newParamSons != _MainWords[index].ParamSlot[paramIndex].ViewParamFamilies;
...
void CDBGroupBuildPhrase::deleteOpCredit(uint index) {
  ...
  if (index >= 1 + _NumMandatories &&
      index < 1 + _NumMandatories + _NumOptionals)
    isCredit = false;
  else if (index >= 1 + _NumMandatories + _NumOptionals &&
           index < getNumMainBricks())
    isCredit = true;
  else
    return;
...
// ***************************************************************************
static void getDefaultSheetForFamily(uint familyId, sint32 &sheet,
                                     bool &valid) {
  CSBrickManager *pBM = CSBrickManager::getInstance();
...
void CDBGroupBuildPhrase::updateParams(uint index) {
  ...
  // if param families of the new brick different from what setuped in view,
  // reset all param hierachy
...
public:
  CParamTreeNode(CParamTreeNode *parent)
      : Brick(NULL), Valid(true), Parent(parent) {}
  ~CParamTreeNode() {
    Parent = NULL;
    Brick = NULL;
    deleteSons();
  ...
  // rebuild default param sons (recurs) if BrickParamFamilies and
  // ViewParamFamilies differ. return false if error
  bool synchronizeParams(uint maxDepth) {
    ...
    // compare with the current View one. if equals then OK! just recurs test
    // sons
  ...
  // return false if error
  bool buildRawParamList(CDBGroupBuildPhrase::CWord &word,
                         uint &rawParamIndex) {
    // If this is the root
    if (!Parent) {
    ...
    // else, add this parameter to list
      ...
      if (rawParamIndex >= CDBGroupBuildPhrase::MaxParam) {
        nlwarning(
            "BRICK ERROR: Not enough param Space (%d) to add all son parameter",
...
void CDBGroupBuildPhrase::updateParamHrc(uint index) {
  /*
          When we are here, we may have some Parameter Hierarchy inconsitency
     between the ViewParameterFamilies and the Brick parameter Families. we must
     fix them and rebuild all the raw param list
  */
  // get the Word
  CWord *word;
  if (index < _MainWords.size())
  ...
  // **** From the current View setup of 'word', build the Parameter Hierarchy
  // (in simple tree form)
  CParamTreeNode rootNode(NULL);
  // NB: here rootNode represent the Main (ie mandatory, optional or credit)
  // brick. Therefore, it is not a real parameter.
  ...
  while (curNode) {
    // if this node still need sons, then the next raw index param must be a son
    // of curNode
  ...
  // **** Parse the Parameter tree, and invalidate each branch where View
  // families and Brick families differs. Avoid .sbrick data erros: allow only a
  // max recurs level of 10
...
void CDBGroupBuildPhrase::updateAllDisplay(const CSPhraseCom &phrase) {
  ...
  // update database
  NLGUI::CDBManager::getInstance()
      ->getDbProp("UI:PHRASE:BUILD:TOTAL_COST")
      ->setValue32(totalCost);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("UI:PHRASE:BUILD:TOTAL_CREDIT")
      ->setValue32(totalCredit);
  ...
  if (phrase.Bricks.size()) {
    ...
    for (uint i = 0; i < phrase.Bricks.size();) {
      ...
      if (!brick) {
        ...
        float relative_cost;
        cost =
            pBM->getSabrinaCom().getPhraseBrickAndParamCost(phrase.Bricks, i);
        relative_cost = pBM->getSabrinaCom().getPhraseBrickAndParamRelativeCost(
            phrase.Bricks, i);
        std::string costText;
  ...
  for (uint i = 0; i < 1 + _NumMandatories; i++) {
    ...
    if (word.Slot.Brick->asSBrickSheet())
      ...
      if (i == 0)
        ...
        word.InfoView->setText(CI18N::get("uiTextHelpSelectEffectBrick") +
                               toString(i));
    }
  }
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("UI:PHRASE:BUILD:ROOT_EFFECT_VALID")
      ->setValue32(mandatOk);
  ...
  if (rootBrick && rootBrick->isCombat()) {
    ...
    NLGUI::CDBManager::getInstance()
        ->getDbProp("UI:PHRASE:BUILD:RESTRICT_COMBAT:ENABLED")
        ->setValue32(1);

    // If not already done, retrieve the weapon skills, and fill the sbricks
    // SHEET
    if (_WeaponSkills.empty()) {
      // get define, and verify data
      uint numWeaponSkill;
      fromString(CWidgetManager::getInstance()->getParser()->getDefine(
                     "phrase_max_restrict_combat"),
                 numWeaponSkill);
      string strWeaponSkill =
          CWidgetManager::getInstance()->getParser()->getDefine(
              "phrase_def_skill_restrict_combat");
      vector<string> weaponSkillList;
      splitString(strWeaponSkill, " ", weaponSkillList);
      nlassert(weaponSkillList.size() == numWeaponSkill);

      // NOTE TO CODER WHO CHANGE SKILLS::ESkill. If you change combat skills,
      // ask yoyo for modification or search "phrase_def_skill_restrict_combat"
      // and "phrase_max_restrict_combat" in XML.
      nlctassert(SKILLS::SFM1SSM && SKILLS::SFM1SAM && SKILLS::SFM1BMM &&
                 SKILLS::SFM1BSM && SKILLS::SFM1PSM && SKILLS::SFM2SSM &&
                 SKILLS::SFM2SAM && SKILLS::SFM2BMM && SKILLS::SFM2PPM &&
                 SKILLS::SFMCADM && SKILLS::SFMCAHM && SKILLS::SFR1APM &&
      ...
      for (uint i = 0; i < numWeaponSkill; i++) {
        ...
        uint32 viewBrickCombatSheetId =
            pBM->getVisualBrickForSkill(_WeaponSkills[i]).asInt();

        // And fill in DB
        NLGUI::CDBManager::getInstance()
            ->getDbProp(toString("UI:PHRASE:BUILD:RESTRICT_COMBAT:%d:SHEET", i))
    ...
    for (uint i = 0; i < _WeaponSkills.size(); i++) {
      bool ok =
          pPM->skillCompatibleWithCombatPhrase(_WeaponSkills[i], phrase.Bricks);
      NLGUI::CDBManager::getInstance()
          ->getDbProp(toString("UI:PHRASE:BUILD:RESTRICT_COMBAT:%d:LOCKED", i))
          ->setValue32(!ok);
    }
  } else {
...
void CDBGroupBuildPhrase::resetSelection() {
  ...
  for (uint i = 0; i < MaxSelection; i++) {
    NLGUI::CDBManager::getInstance()
        ->getDbProp(BrickSelectionDB + ":" + toString(i) + ":SHEET")
        ->setValue32(0);
  }
}
...
// ***************************************************************************
void CDBGroupBuildPhrase::filterBrickExclusion(std::vector<CSheetId> &bricks,
                                               uint16 indexToSkip) {
  CSBrickManager *pBM = CSBrickManager::getInstance();
  ...
  // test for all bricks
  for (uint j = 1 /*+_NumMandatories*/;
       j < 1 + _NumMandatories + _NumOptionals + _NumCredits; j++) {
    // skip brick at index indexToSkip (if replacing a brick for example, don't
    // consider it's forbiden flags)
    if (j == indexToSkip)
      continue;
  ...
  for (uint i = 0; i < bricks.size(); i++) {
    ...
    if (brick) {
      // For all define words, search if excluded from the current set of
      // optional setup
      forbidWords.clear();
...
// ***************************************************************************
void CDBGroupBuildPhrase::filterBrickSetuped(
    std::vector<NLMISC::CSheetId> &bricks) {
  std::vector<CSheetId> res;
  res.reserve(bricks.size());
...
// ***************************************************************************
void CDBGroupBuildPhrase::filterSkillSetuped(
    std::vector<NLMISC::CSheetId> &bricks, bool checkOptional, bool checkCredit,
  ...
  uint checkStart =
      checkOptional ? 1 + _NumMandatories : 1 + _NumMandatories + _NumOptionals;
  uint checkEnd =
      checkCredit ? 1 + _NumMandatories + _NumOptionals : getNumMainBricks();

  // test for all optionals and/or Credits
  uint i;
  for (uint i = checkStart; i < checkEnd; i++) {
    // For Replacement of brick, don't test with the brick replaced (suppose
    // will be removed!)
  ...
  for (i = 0; i < bricks.size(); i++) {
    const CSBrickSheet *brick = pBM->getBrick(bricks[i]);
    if (brick && brick->getSkill() != SKILLS::unknown) {
      // simulate a choose of this brick, ie AND its skill formula with the
      // current phrase formula
      CReqSkillFormula brickFormula;
      for (uint j = 0; j < brick->UsedSkills.size(); j++) {
        brickFormula.orV(CSkillValue(brick->UsedSkills[j]));
      }
      ...
      // Nb: the following test works if testFormula is empty(), because in this
      // case tempFormula.and(brickFormula)==brickFormula and hence is of form
      // SFR | SFM | ....
...
// ***************************************************************************
void CDBGroupBuildPhrase::filterRootBrickType(
    std::vector<NLMISC::CSheetId> &bricks) {
  // no filter => ok.
    ...
    if (brick0) {
      ...
      for (uint j = 0; j < MaxRootBrickTypeFilter; j++) {
        if (_RootBrickTypeFilter[j] != BRICK_TYPE::UNKNOWN &&
            _RootBrickTypeFilter[j] ==
                BRICK_FAMILIES::brickType(brick0->BrickFamily)) {
          res.push_back(bricks[i]);
          break;
        }
      }
...
// ***************************************************************************
void CDBGroupBuildPhrase::filterRootPossibles(
    std::vector<NLMISC::CSheetId> &bricks) {
  CSBrickManager *pBM = CSBrickManager::getInstance();
  std::vector<CSheetId> res;
  res.reserve(bricks.size());
...
void CDBGroupBuildPhrase::buildCurrentPhrase(CSPhraseCom &newPhrase) {
  ...
  if (_UserSentenceName) {
    newPhrase.Name =
        ucstring::makeFromUtf8(_UserSentenceName->getInputString());
  }
}

// ***************************************************************************
void CDBGroupBuildPhrase::updateNewButtons() {
  // TODO_BRICK: have I to hide button and their text if not possible to add any
  // op/credit?
...
// ***************************************************************************
void CDBGroupBuildPhrase::fillNewOptionalBricks(
    std::vector<NLMISC::CSheetId> &bricks) {
  if (!_GroupValid)
    return;
  const CSBrickSheet *rootBrick = getRootBrick();
  ...
  // Don't filter Combat and Power optional families (Use Brick exclusion system
  // instead)
  ...
  for (uint i = 0; i < optionalFamilies.size(); i++) {
    const vector<CSheetId> &famBricks =
        pBM->getFamilyBricks(optionalFamilies[i]);
...
// ***************************************************************************
void CDBGroupBuildPhrase::fillNewCreditBricks(
    std::vector<NLMISC::CSheetId> &bricks) {
  if (!_GroupValid)
    return;
  const CSBrickSheet *rootBrick = getRootBrick();
...
// ***************************************************************************
void CDBGroupBuildPhrase::setRootBrickTypeFilter(
    BRICK_TYPE::EBrickType rootBtFilter, BRICK_TYPE::EBrickType rootBtFilter2,
