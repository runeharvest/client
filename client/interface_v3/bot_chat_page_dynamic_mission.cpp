...
static const char *WIN_BOT_CHAT_PAGE_DYNAMIC_MISSION =
    "ui:interface:bot_chat_dynamic_mission";
...
CBotChatPageDynamicMission::CBotChatPageDynamicMission() {
  std::fill(_ChoiceCB, _ChoiceCB + DYNAMIC_MISSION_NUM_CHOICES,
            (CDBGroupComboBox *)NULL);
...
void CBotChatPageDynamicMission::invalidateMission() {
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp(DM_DESCRIPTION_DB_PATH)
      ->setValue32(0);
...
void CBotChatPageDynamicMission::begin() {
  ...
  for (uint k = 0; k < DYNAMIC_MISSION_NUM_CHOICES; ++k) {
    NLGUI::CDBManager::getInstance()
        ->getDbProp(toString(DM_CHOICE "%d:TITLE", (int)k))
        ->setValue32(0);
    for (uint l = 0; l < DYNAMIC_MISSION_MAX_NUM_OPTIONS; ++l) {
      NLGUI::CDBManager::getInstance()
          ->getDbProp(toString(DM_CHOICE "%d:%d:TEXT", (int)k, (int)l))
          ->setValue32(0);
    }
  }
  ...
  CInterfaceGroup *ig = dynamic_cast<CInterfaceGroup *>(
      CWidgetManager::getInstance()->getElementFromId(
          WIN_BOT_CHAT_PAGE_DYNAMIC_MISSION));
  if (!ig) {
    std::fill(_ChoiceCB, _ChoiceCB + DYNAMIC_MISSION_NUM_CHOICES,
              (CDBGroupComboBox *)NULL);
  } else {
    for (uint k = 0; k < DYNAMIC_MISSION_NUM_CHOICES; ++k) {
      ...
      _ChoiceCB[k] = dynamic_cast<CDBGroupComboBox *>(
          ig->getGroup(toString("choice%d", (int)k)));
  ...
  for (uint k = 0; k < DYNAMIC_MISSION_NUM_CHOICES; ++k) {
    NLGUI::CDBManager::getInstance()
        ->getDbProp(toString("UI:TEMP:DYNAMIC_MISSION:CHOICE%d", (int)k))
        ->setValue32(-1);
  }
}
...
void CBotChatPageDynamicMission::update() {
    ...
    if (_ChoiceCB[k]) {
      ...
      for (l = 0; l < _ChoiceCB[k]->getNumTexts(); ++l) {
        if (!_TextReceived[k][l]) {
          if (ClientCfg.Local) {
            _ChoiceCB[k]->setText(
                l, toString("Dynamic mission %d:%d", (int)k, (int)l));
            _TextReceived[k][l] = true;
          } else {
            uint32 textID = (uint32)NLGUI::CDBManager::getInstance()
                                ->getDbProp(toString(DM_CHOICE "%d:%d:TEXT",
                                                     (int)k, (int)l))
                                ->getValue32();
            // see if text has been receive
            string result;
            bool received =
                CStringManagerClient::instance()->getDynString(textID, result);
            if (received) {
      ...
      for (l = _ChoiceCB[k]->getNumTexts(); l < DYNAMIC_MISSION_NUM_CHOICES;
           ++l) {
        ...
        if (received) {
          ...
          _ChoiceCB[k]->addText(
              NLMISC::CI18N::get("uiWaitingChoiceFromServer"));
        }
      }
    }
  }
  if (ClientCfg.Local) {
    NLGUI::CDBManager::getInstance()
        ->getDbProp(DM_VALID_DB_PATH)
        ->setValue32(1);
  } else {
    if (!_MissionValid) {
      // activate 'regen' and 'accept' button when the description has been
      // received
      uint32 textID = NLGUI::CDBManager::getInstance()
                          ->getDbProp(DM_TITLE_DB_PATH)
                          ->getValue32();
      if (textID != 0) {
        string result;
        if (CStringManagerClient::instance()->getDynString(textID, result)) {
          textID = NLGUI::CDBManager::getInstance()
                       ->getDbProp(DM_DESCRIPTION_DB_PATH)
                       ->getValue32();
          if (textID != 0) {
            if (CStringManagerClient::instance()->getDynString(textID,
                                                               result)) {
              NLGUI::CDBManager::getInstance()
                  ->getDbProp(DM_VALID_DB_PATH)
                  ->setValue32(1);
