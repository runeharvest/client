...
CChatTextManager::CChatTextManager()
    : _TextFontSize(NULL), _TextMultilineSpace(NULL), _TextShadowed(NULL),
...
uint CChatTextManager::getTextFontSize() const {
  if (!_TextFontSize) {
    ...
    _TextFontSize = NLGUI::CDBManager::getInstance()->getDbProp(
        "UI:SAVE:CHAT:FONT_SIZE", false);
    if (!_TextFontSize)
      return 12;
  }
...
uint CChatTextManager::getTextMultiLineSpace() const {
  if (!_TextMultilineSpace) {
    CInterfaceManager *im = CInterfaceManager::getInstance();
    _TextMultilineSpace = NLGUI::CDBManager::getInstance()->getDbProp(
        "UI:SAVE:CHAT:MULTI_LINE_SPACE", false);
    if (!_TextMultilineSpace)
      return 1;
  }
...
bool CChatTextManager::isTextShadowed() const {
  if (!_TextShadowed) {
    CInterfaceManager *im = CInterfaceManager::getInstance();
    _TextShadowed = NLGUI::CDBManager::getInstance()->getDbProp(
        "UI:SAVE:CHAT:SHADOWED_TEXT", false);
    if (!_TextShadowed)
      return false;
  }
...
bool CChatTextManager::showTimestamps() const {
  if (!_ShowTimestamps) {
    CInterfaceManager *im = CInterfaceManager::getInstance();
    _ShowTimestamps = CDBManager::getInstance()->getDbProp(
        "UI:SAVE:CHAT:SHOW_TIMES_IN_CHAT_CB", false);
    if (!_ShowTimestamps)
      return false;
  }
...
static CInterfaceGroup *parseCommandTag(string &line) {
  ...
  if (params.size() != 4) {
    nlwarning(
        "4 parameters wanted for command tag : template|caption|ah|ah_params");
    return NULL;
  }
  //
  ...
  return CWidgetManager::getInstance()->getParser()->createGroupInstance(
      params[0], "", uiTemplateParams, 4);
}

static CInterfaceGroup *buildLineWithCommand(CInterfaceGroup *commandGroup,
                                             CViewText *text) {
...
static inline bool isUrlTag(const string &s, string::size_type index,
...
// ***************************************************************************
// isUrlTag must match
static inline void getUrlTag(const string &s, string::size_type &index,
  ...
  // fallback to full string length as we did match http:// already and url
  // spans to the end probably
  if (pos == string::npos) {
    pos = textSize;
  }
...
static void prependTimestamp(string &msg) {
  ...
  if (codePos != string::npos) {
    // Prepend the current time (do it after the color if the color at first
    // position.
    if (codePos == 0) {
      codePos = msg.find(string("}"));
      msg = msg.substr(0, codePos + 1) + cur_time +
            msg.substr(codePos + 1, msg.length() - codePos);
    } else {
      msg = cur_time + msg;
    }
  } else {
...
//=================================================================================
CViewBase *CChatTextManager::createMsgText(const string &cstMsg,
                                           NLMISC::CRGBA col,
...
//=================================================================================
CViewBase *
CChatTextManager::createMsgTextSimple(const string &msg, NLMISC::CRGBA col,
                                      bool justified,
                                      CInterfaceGroup *commandGroup) {
...
//=================================================================================
CViewBase *
CChatTextManager::createMsgTextComplex(const string &msg, NLMISC::CRGBA col,
                                       bool justified, bool plaintext,
                                       CInterfaceGroup *commandGroup) {
  ...
  for (string::size_type i = 0; i < textSize;) {
    if (hasUrl && isUrlTag(msg, i, textSize)) {
      if (pos != i) {
        CViewBase *vt =
            createMsgTextSimple(msg.substr(pos, i - pos), col, justified, NULL);
        para->addChild(vt);
      }
      ...
      if (url.size() > 0) {
        ...
        vt->setFontSize(getTextFontSize());
        vt->setMultiLine(true);
        vt->setTextMode(justified ? CViewText::Justified
  ...
  if (pos < textSize) {
    CViewBase *vt = createMsgTextSimple(msg.substr(pos, textSize - pos), col,
                                        justified, NULL);
    vt->setId("text");
    para->addChild(vt);
  }
...
public:
  virtual void execute(CCtrlBase *pCaller, const string &params) {
    ...
    CWidgetManager::getInstance()->enableModalWindow(
        pCaller, "ui:interface:chat_copy_action_menu");
  }
};
...
public:
  virtual void execute(CCtrlBase *pCaller, const string &params) {
    ...
    CAHManager::getInstance()->runActionHandler("copy_to_clipboard", NULL,
                                                LastSelectedChat);
