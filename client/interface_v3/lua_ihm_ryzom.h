...
private:
  static int getUI(
      CLuaState &ls); // params: "ui:interface:...". return: CInterfaceElement*
                      // (nil if error), an additionnal boolean parameter
  // LUA exported Functions with standard lua (because use ui object, use
  // variable param number, or return dynamic-typed object)
  static int getUICaller(
      CLuaState
          &ls); // params: none. return: CInterfaceElement*  (nil if error)
                //		   can specify verbose display when the element
                //is note found (default is true)
  static int createGroupInstance(
      CLuaState &ls); // params : param 1 = template name,
                      // param 2 = id of parent where the instance will be
                      // inserted param 3 = table with ("template_param",
                      // "template_param_value") key/value pairs such as {
                      // id="foo", x="10" } etc. -> returns a new instance of
                      // the template, or nil on fail
  static int createRootGroupInstance(
      CLuaState &ls); // params : param 1 = template name,
                      // param 2 = id of parent where the instance will be
                      // inserted param 3 = table with ("template_param",
                      // "template_param_value") key/value pairs such as {
                      // id="foo", x="10" } etc. -> returns a new instance of
                      // the template, or nil on fail
  static int createUIElement(
      CLuaState &ls); // params : param 1 = template name,
                      // param 2 = id of parent where the instance will be
                      // inserted param 3 = table with ("template_param",
                      // "template_param_value") key/value pairs
  // such as { id="foo", x="10" } etc. -> returns a new instance of the
  // template, or nil on fail

  static int displayBubble(
      CLuaState &ls); // params : param 1 = bot id
                      // param 2 = text
                      // param 3 = table with all strings and urls
                      // {"main text"="http:///", "text option 1"="http:///",
                      // "text option 2"="http:///") etc...
  static int getIndexInDB(
      CLuaState &ls); // params: CDBCtrlSheet*.... return: index, or 0 if error
  static int formatUI(CLuaState &ls); // params: "expr", param1, param2....
                                      // return: string with # and % parsed
  static int formatDB(CLuaState &ls); // params: param1, param2.... return:
                                      // string with @ and , added
  static int launchContextMenuInGame(CLuaState &ls); // params : menu name
  static int
  parseInterfaceFromString(CLuaState &ls); // params : intreface script
  ...
  static int setLuaBreakPoint(
      CLuaState &ls); // set a breakpoint in lua external debugger (file, line)
  ...
  static int setWeatherValue(
      CLuaState &ls); // first value is a boolean to say automatic, second value
                      // ranges from of to 1 and gives the weather
  static int getWeatherValue(
      CLuaState
          &ls); // get current real weather value (blend between server driven
                // value & predicted value). Manual weather value is ignored
  static int disableContextHelpForControl(
      CLuaState &ls); // params: CCtrlBase*. return: none
  static int disableContextHelp(CLuaState &ls);
  static int
  getServerSeason(CLuaState
                      &ls); // get the last season sent by the server
                            // 0->auto, computed locally from the current day
                            // (or not received from server yet) 1->server force
                            // spring 2->' '		 ' summer 3->' '
                            // ' autumn 4->' '		 ' winter
  static int computeCurrSeason(
      CLuaState &ls); // compute current displayed season (1->spring, etc .)
  static int
  getAutoSeason(CLuaState &ls); // compute automatic season that would be at
                                // this time (1->spring, etc .)
  ...
  static int
  dumpUI(CLuaState &ls); // params: CInterfaceElement*.... return: none
  ...
  static sint64
  getDbProp64(const std::string &dbProp); // return 0 if not found.
  static void
  setDbProp(const std::string &dbProp,
            sint32 value); // Nb: the db prop is not created if not present.
  static void
  setDbProp64(const std::string &dbProp,
              sint64 value); // Nb: the db prop is not created if not present.
  static void
  addDbProp(const std::string &dbProp,
            sint32 value); // Nb: the db prop is created if not present.
  static void delDbProp(const std::string &dbProp);
  static void setDbRGBA(
      const std::string &dbProp,
      const NLMISC::CRGBA &color); // the db prop is created if not present
  static std::string
  getDbRGBA(const std::string &dbProp); // empty string if not found
...
public:
  ...
  // a button will be created near the line to allow to goto the lua line that
  // issued the message by using an external editor
  ...
  /** execute function that is currently on the stack, possibly outputing error
   * messages to the log \return true if execution succeeded
   */
...
private:
  ...
  // Additionnally, if ClientCfg.LuaDebugInfoGotoButtonEnabled is set, then
  // buttons will be created in front of each line to allow to go to the lua
  // line that issued the message by using an external editor
...
#ifdef RYZOM_LUA_UCSTRING
  static ucstring replacePvpEffectParam(const ucstring &str,
                                        sint32 parameter); // TODO: UTF-8 Lua
#else
  static std::string replacePvpEffectParam(const std::string &str,
                                           sint32 parameter); // TODO: UTF-8 Lua
#endif
  ...
  static bool isPlayerSlotNewbieLand(
      uint32 slot); // test if one of the player slot is a newbieland one, if
                    // not so, client must be patched in order to continue
  ...
  static sint32 getMagicResistChance(bool elementalSpell, sint32 casterSpellLvl,
                                     sint32 victimResistLvl);
  static sint32 getDodgeParryChance(sint32 attLvl, sint32 defLvl);
  static void browseNpcWebPage(const std::string &htmlId,
                               const std::string &url, bool addParameters,
                               double timeout);
  ...
  static sint32
  getNbTribeFameIndex(); // number of tribe fame index (which are contiguous)
  ...
  // sheet access
  // TODO nico : using the reflection system on sheets would allow to export
  // them to lua without these functions ...
  static std::string getCharacterSheetSkel(const std::string &sheet,
                                           bool isMale);
  ...
  // open the window to do a tell to 'player', if 'msg' is not empty, then the
  // message will be sent immediatly
  // else, current command of the chat window will be replaced with tell
  // 'player'
#ifdef RYZOM_LUA_UCSTRING
  ...
  static sint32 getPlayerLevel(); // get max level among player skills (magi,
                                  // combat, crafting ,foraging)
  static sint64 getPlayerVpa();
  static sint64 getPlayerVpb();
  static sint64 getPlayerVpc();
  static sint32
  getTargetLevel(); // get current, precise level of the selected target, or -1
                    // if there's no such selected target
  static sint32
  getTargetForceRegion(); // get 'force region' for current target, or -1 if
                          // there's no selected target
  static sint32 getTargetLevelForce(); // get 'level force' for current target,
                                       // or -1 if there's no selected target
#ifdef RYZOM_LUA_UCSTRING
  static ucstring getTargetSheet(); // get the name of the target sheet (like
                                    // 'zoha2old.creature')
#else
  static std::string getTargetSheet(); // get the name of the target sheet (like
                                       // 'zoha2old.creature')
#endif
...
public:
  // Create a special tag that will add a 'goto' button for the given file and
  // line The tag should be appended in front of a string to use with
  // 'rawDebugInfo'. when the final string will be printed, a button will be
  // created in front of it Requires that
  // 'ClientCfg.LuaDebugInfoGotoButtonEnabled' is set to 1, else a, empty tag is
  // returned
