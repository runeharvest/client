...
NLMISC_REGISTER_OBJECT(CViewBase, CDBGroupListSheetMission, std::string,
                       "list_sheet_mission");

// **********************************************************************************
void CDBGroupListSheetMission::CSheetChildMission::init(
...
// **********************************************************************************
CViewText *
CDBGroupListSheetMission::CSheetChildMission::createViewText() const {
  ...
  if (Ctrl)
    vti->setDBLeaf(dynamic_cast<CCDBNodeLeaf *>(
        Ctrl->getRootBranch()->getNode(ICDBNode::CTextId("TEXT"))));
  else
    vti->setDBLeaf(NULL);
...
// **********************************************************************************
bool CDBGroupListSheetMission::CSheetChildMission::isInvalidated(
...
// ***************************************************************************
void CDBGroupListSheetMission::CSheetChildMission::update(
    CDBGroupListSheetText *pFather) {
  uint8 newPreReqState = (uint8)CurrentPreReqState.getSInt32();
...
// **********************************************************************************
void CDBGroupListSheetMission::CSheetChildMission::updateViewText(
    CDBGroupListSheetText *pFather) {
  CSheetChild::updateViewText(pFather);

  if ((Ctrl == NULL) || (Text == NULL))
    return;
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  ...
  if (CachePreReqState == MISSION_DESC::PreReqFail) {
    ...
    color = CWidgetManager::getInstance()
                ->getSystemOption(CWidgetManager::OptionCtrlTextRedifyColor)
                .getValColor();
    Text->setColor(color);
  } else if (CachePreReqState == MISSION_DESC::PreReqFailAlreadyDone) {
    // If mission prerequesits failed because mission is already done : COLOR
    // GREEN
    CRGBA color =
        CWidgetManager::getInstance()
            ->getSystemOption(CWidgetManager::OptionCtrlSheetGreenifyColor)
            .getValColor();
    ...
    color = CWidgetManager::getInstance()
                ->getSystemOption(CWidgetManager::OptionCtrlTextGreenifyColor)
                .getValColor();
    Text->setColor(color);
  } else if (CachePreReqState == MISSION_DESC::PreReqFailRunning) {
    // If mission prerequesits failed because the mission is in progress : COLOR
    // GRAY
    CRGBA color =
        CWidgetManager::getInstance()
...
// **********************************************************************************
bool CDBGroupListSheetMission::CSheetChildMission::isSheetValid(
    CDBGroupListSheetText * /* pFather */) {
  if (!Ctrl)
    return false;
