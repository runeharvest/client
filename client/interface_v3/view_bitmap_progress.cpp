...
bool CDBViewBar::parse(xmlNodePtr cur, CInterfaceGroup *parentGroup) {
  ...
  if (prop) {
    ...
    string tmp = "cannot getprop:range, view:" + getId() +
  ...
  if (prop) {
    ...
    string tmp = "cannot getprop:rangemax, view:" + getId() +
                 ", parent:" + parentGroup->getId();
    nlinfo(tmp.c_str());
    return false;
  }
...
void CDBViewBar::draw() {
  ...
  if (_Vertical.getBool()) {
    if (_RangeMax.getSInt32())
      hBar =
          _HReal * ((float)_Range.getSInt32() / (float)_RangeMax.getSInt32());
    else
      hBar = 0.0f;
  } else {
    if (_RangeMax.getSInt32())
      wBar =
          _WReal * ((float)_Range.getSInt32() / (float)_RangeMax.getSInt32());
    else
      wBar = 0.0f;
  }
  ...
  scisHeight =
      std::min(scisHeight + oldScissorY, (sint32)(yabs + hBar)) - scisY;
  ...
  rVR.drawRotFlipBitmap(_RenderLayer, _XReal, _YReal, _WReal, _HReal,
                        (uint8)_Rot, _Flip, _TextureId, _Color);
