...
public:
  ...
  /// Called from server (if MemberName==0 and MemberGrade==Unknown then remove
  /// the entry)
  // void set (uint32 indexMember, uint32 MemberName, uint8 MemberGrade, bool
  // bOnline);
  ...
  /// If the player is in a guild get the amount of money the guild owns else
  /// return zero
  uint64 getMoney();
  ...
  // Launch join proposal interface (when a guild member player wants to invite
  // you in its guild)
  ...
  static uint64 iconMake(uint8 back, uint8 symb, bool inv, NLMISC::CRGBA col1,
                         NLMISC::CRGBA col2) {
    return ((uint64)(back & 15)) | (((uint64)(symb & 63)) << 4) |
           (((uint64)(inv & 1)) << 10) | (((uint64)(col1.R & 255)) << 11) |
           (((uint64)(col1.G & 255)) << 19) | (((uint64)(col1.B & 255)) << 27) |
           (((uint64)(col2.R & 255)) << 35) | (((uint64)(col2.G & 255)) << 43) |
           (((uint64)(col2.B & 255)) << 51);
  }

  static uint8 iconGetBack(uint64 icon) { return (uint8)(icon & 15); }
  static uint8 iconGetSymbol(uint64 icon) { return (uint8)(icon >> 4) & 63; }
  static bool iconGetInvertSymbol(uint64 icon) {
    return ((uint8)(icon >> 10) & 1) == 0 ? false : true;
  }
  static NLMISC::CRGBA iconGetColor1(uint64 icon) {
    return NLMISC::CRGBA((uint8)(icon >> 11) & 255, (uint8)(icon >> 19) & 255,
                         (uint8)(icon >> 27) & 255);
  }
  static NLMISC::CRGBA iconGetColor2(uint64 icon) {
    return NLMISC::CRGBA((uint8)(icon >> 35) & 255, (uint8)(icon >> 43) & 255,
                         (uint8)(icon >> 51) & 255);
  }

  static void iconSetBack(uint64 &icon, uint8 n) {
    icon &= ~((uint64)15);
    icon |= ((uint64)n & 15);
  }
  static void iconSetSymbol(uint64 &icon, uint8 n) {
    icon &= ~((uint64)63 << 4);
    icon |= (((uint64)n & 63) << 4);
  }
  static void iconSetInvertSymbol(uint64 &icon, bool inv) {
    icon &= ~((uint64)1 << 10);
    uint64 n = inv ? 1 : 0;
    icon |= n << 10;
  }
  static void iconSetColor1(uint64 &icon, NLMISC::CRGBA col) {
    icon &= ~((uint64)255 << 11);
    icon &= ~((uint64)255 << 19);
    icon &= ~((uint64)255 << 27);
    icon |= ((uint64)col.R << 11);
    icon |= ((uint64)col.G << 19);
    icon |= ((uint64)col.B << 27);
  }
  static void iconSetColor2(uint64 &icon, NLMISC::CRGBA col) {
    icon &= ~((uint64)255 << 35);
    icon &= ~((uint64)255 << 43);
    icon &= ~((uint64)255 << 51);
    icon |= ((uint64)col.R << 35);
    icon |= ((uint64)col.G << 43);
    icon |= ((uint64)col.B << 51);
  }
...
private:
  ...
  // Need to rebuild Members at next update (+all). this is called internally by
  // the observer on DB
...
private:
  ...
  // Leader : bear | invite | xp | (kick/set rank) members, bearer, recruiter,
  // officer, high officer or Leader High Officer : bear | invite | xp |
  // (kick/set rank) members, bearer, recruiter or officer Officer : bear |
  // invite | kick members, recruiter or bearer Recruiter : invite Bearer : bear
  // Member : nothing
  // bear=can bear the banner, invite=can invite other players to the guild, xp=
  // can spend XP guild on role masters kick=can kick a player out of the guild,
  // set rank=set the rank of a guild member
