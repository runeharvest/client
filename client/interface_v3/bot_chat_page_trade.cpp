...
static const char *WIN_BOT_CHAT_SELL_BUY_ITEM =
    "ui:interface:bot_chat_buy_sell_item";
static const char *WIN_BOT_CHAT_CHANGE_BUY_FILTER =
    "ui:interface:bot_chat_change_buy_filter";
static const char *WIN_BOT_CHAT_CHANGE_BUY_FILTER_MP =
    "ui:interface:bot_chat_change_buy_filter_mp";
static const char *WIN_BOT_CHAT_CHANGE_BUY_FILTER_CLASS =
...
static const char *WIN_BOT_CHAT_DESTROY_ITEM =
    "ui:interface:bot_chat_destroy_confirm_modal";

#define DB_BOT_CHAT_BASE_BUY_FILTER "UI:SAVE:BOTCHAT_FILTER"

// fill the help setup infos for the trade modal
static void fillHelpSetupInfosForTrade(CSheetHelpSetup &dest,
                                       CDBCtrlSheet *sheet,
                                       CInterfaceGroup *helpGroup) {
  dest.HelpWindow = helpGroup;
  dest.SrcSheet = sheet;
  dest.DestSheet =
      dynamic_cast<CDBCtrlSheet *>(helpGroup->getCtrl("buy_sell_slot"));
...
public:
  virtual void infoReceived() {
    ...
    CGroupContainer *ig = dynamic_cast<CGroupContainer *>(
        CWidgetManager::getInstance()->getElementFromId(
            WIN_BOT_CHAT_SELL_BUY_ITEM));
...
bool CBotChatPageTrade::CItemCheck::test(CDBCtrlSheet *ctrl) const {
  nlassert(ctrl);
  return _SheetId == ctrl->getSheetId() && _Quality == ctrl->getQuality() &&
         _Quantity == ctrl->getQuantity() && _Price == ctrl->getItemPrice() &&
         _ResaleFlag == ctrl->getItemResaleFlag();
}
...
void CBotChatPageTrade::init() {
  ...
  if (NLGUI::CDBManager::getInstance()->getDbBranch("SERVER:TRADING")) {
    NLGUI::CDBManager::getInstance()->addBranchObserver("SERVER:TRADING",
                                                        &_TradePagesObs);
  }

  _FamePriceFactorLeaf = NLGUI::CDBManager::getInstance()->getDbProp(
      "SERVER:TRADING:FAME_PRICE_FACTOR");
}
...
void CBotChatPageTrade::invalidateCoords() {
  ...
  CGroupContainer *gc = dynamic_cast<CGroupContainer *>(
      CWidgetManager::getInstance()->getElementFromId(
          _BuyOnly ? WIN_BOT_CHAT_PAGE_BUY : WIN_BOT_CHAT_PAGE_TRADE));
  if (!gc)
    return;
...
void CBotChatPageTrade::begin() {
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp(BOT_CHAT_BASE_DB_PATH ":TRADE")
      ->setValue32(0);

  // at each new bot chat trade, we must reset filter for ItemPart and ItemType
  // (NB: server should do the same)
  resetItemPartAndTypeFilters();

  // reset also the filters at each open if user wants to
  CCDBNodeLeaf *dbResetFilterOnOpen =
      NLGUI::CDBManager::getInstance()->getDbProp(
          DB_BOT_CHAT_BASE_BUY_FILTER ":RESET_ON_OPEN", false);
  if (dbResetFilterOnOpen && dbResetFilterOnOpen->getValueBool() &&
      (_BuyMean == Money)) {
    // temporary value for conversions
    sint32 value;

    // Reset Price
    CCDBNodeLeaf *dbPriceMin = NLGUI::CDBManager::getInstance()->getDbProp(
        string(DB_BOT_CHAT_BASE_BUY_FILTER ":MIN_PRICE"), false);
    CCDBNodeLeaf *dbPriceMax = NLGUI::CDBManager::getInstance()->getDbProp(
        string(DB_BOT_CHAT_BASE_BUY_FILTER ":MAX_PRICE"), false);
    if (dbPriceMin)
      dbPriceMin->setValue32(0);
    if (dbPriceMax) {
      fromString(CWidgetManager::getInstance()->getParser()->getDefine(
                     "bot_chat_filter_max_price"),
                 value);
      dbPriceMax->setValue32(value);
    }

    // Reset Quality
    CCDBNodeLeaf *dbQualityMin = NLGUI::CDBManager::getInstance()->getDbProp(
        string(DB_BOT_CHAT_BASE_BUY_FILTER ":MIN_QUALITY"), false);
    CCDBNodeLeaf *dbQualityMax = NLGUI::CDBManager::getInstance()->getDbProp(
        string(DB_BOT_CHAT_BASE_BUY_FILTER ":MAX_QUALITY"), false);
    if (dbQualityMin)
      dbQualityMin->setValue32(0);
    if (dbQualityMax) {
      fromString(CWidgetManager::getInstance()->getParser()->getDefine(
                     "bot_chat_filter_max_quality"),
                 value);
      dbQualityMax->setValue32(value);
    }

    // Reset Class
    CCDBNodeLeaf *dbClassMin = NLGUI::CDBManager::getInstance()->getDbProp(
        DB_BOT_CHAT_BASE_BUY_FILTER ":MIN_CLASS", false);
    CCDBNodeLeaf *dbClassMax = NLGUI::CDBManager::getInstance()->getDbProp(
        DB_BOT_CHAT_BASE_BUY_FILTER ":MAX_CLASS", false);
    if (dbClassMin)
      dbClassMin->setValue32(0);
    if (dbClassMax)
      dbClassMax->setValue32(RM_CLASS_TYPE::NumTRMClassType - 1);
  ...
  // If BuyMean is skillpoints, means its phrase selling => allow phrase price
  // update (and possible client fill)
  ...
  if (gc) {
    ...
    gc->setTitle(_Title);
    // show the buy mean
    CInterfaceGroup *money =
        dynamic_cast<CInterfaceGroup *>(gc->getGroup("money"));
    if (money)
      money->setActive((_BuyMean == Money) || (_BuyMean == MoneyFactionPoints));
    CInterfaceGroup *factionPoints =
        dynamic_cast<CInterfaceGroup *>(gc->getGroup("fp"));
    if (factionPoints)
      factionPoints->setActive(_BuyMean == MoneyFactionPoints);
    CInterfaceGroup *skillPoints =
        dynamic_cast<CInterfaceGroup *>(gc->getGroup("sp"));
    if (skillPoints)
      skillPoints->setActive(_BuyMean == SkillPoints);
    CInterfaceGroup *pMoneyGuildXP =
        dynamic_cast<CInterfaceGroup *>(gc->getGroup("money_guild_xp"));
    if (pMoneyGuildXP)
      pMoneyGuildXP->setActive((_BuyMean == MoneyGuildXP) ||
                               (_BuyMean == GuildXP));
    CInterfaceGroup *pMoney =
        dynamic_cast<CInterfaceGroup *>(gc->getGroup("money_guild_xp:money"));
    if (pMoney)
      pMoney->setActive(_BuyMean == MoneyGuildXP);

    CInterfaceGroup *pGuildMoneyGuildXP =
        dynamic_cast<CInterfaceGroup *>(gc->getGroup("guild_money_guild_xp"));
    if (pGuildMoneyGuildXP)
      pGuildMoneyGuildXP->setActive((_BuyMean == GuildMoneyGuildXP) ||
                                    (_BuyMean == GuildMoney));
    CViewText *pGuildXPText =
        dynamic_cast<CViewText *>(gc->getView("guild_money_guild_xp:xp_text"));
    if (pGuildXPText)
      pGuildXPText->setActive(_BuyMean == GuildMoneyGuildXP);
    CViewText *pGuildXPValue =
        dynamic_cast<CViewText *>(gc->getView("guild_money_guild_xp:xp_value"));
    if (pGuildXPValue)
      pGuildXPValue->setActive(_BuyMean == GuildMoneyGuildXP);
...
// *****************************************************************************************
void CBotChatPageTrade::update() { updateTradeModal(); }
...
uint32 CBotChatPageTrade::getCurrItemQuantity() const {
  ...
  if (_CurrItemSheet->getType() == CCtrlSheetInfo::SheetType_Item) {
    ...
    if (itemSheet && itemSheet->Family != ITEMFAMILY::TELEPORT &&
      ...
      CGroupEditBox *ed = dynamic_cast<CGroupEditBox *>(
          ig->getGroup("header_opened:standard_price:quantity:edit:eb"));
      if (!ed)
        return std::numeric_limits<uint32>::max();
      uint32 intQuantity;
...
uint32 CBotChatPageTrade::getCurrItemPriceResale() const {
  ...
  if (_CurrItemSheet->getType() == CCtrlSheetInfo::SheetType_Item) {
    ...
    if (!ig)
      return 0;
    CGroupEditBox *resaleMarginBox = dynamic_cast<CGroupEditBox *>(ig->getGroup(
        "header_opened:resell_group:can_resell:choose_resell:edit:eb"));
    if (!resaleMarginBox)
      return 0;

    // Calculate the resale price from the resale margin
    uint32 margin = (uint32)resaleMarginBox->getInputStringAsInt();
    // temp uint64 because we add case where basePrice= 50000, and margin=
    // 99999% => 50000*99999 > 4gig
    uint64 basePrice =
        getCurrItemPrice(false); // base Price without fame margin
...
uint64 CBotChatPageTrade::getCurrItemPrice(bool mulByFame) const {
  ...
  if (_BuyDlgOn && (_BuyMean == MoneyGuildXP || _BuyMean == GuildMoneyGuildXP ||
                    _BuyMean == GuildMoney)) {
    ...
    if ((_BuyMean == GuildMoneyGuildXP) || (_BuyMean == GuildMoney)) {
      ...
      const COutpostBuildingSheet *pOB =
          _CurrItemSheet->asOutpostBuildingSheet();
      if (pOB != NULL)
    ...
    if (_CurrItemSheet->getRootBranch()) {
      ...
      ICDBNode *node;
      // take the PRICE_RETIRE if buying an item which actually belongs to the
      // user
      if (_UsePriceRetire)
        node = _CurrItemSheet->getRootBranch()->getNode(
            ICDBNode::CTextId("PRICE_RETIRE"), false);
      else
        node = _CurrItemSheet->getRootBranch()->getNode(
            ICDBNode::CTextId("PRICE"), false);
      ...
      if (priceLeaf) {
        ...
        if (_BuyDlgOn || !mulByFame)
          ...
          return (uint64)(price * _FamePriceFactorLeaf->getValue16() /
                          10000.0f);
      }
    }
  }
...
// ***************************************************************************
void CBotChatPageTrade::getItemFactionTypePoints(CDBCtrlSheet *sheet,
  ...
  // CCDBNodeLeaf *currencyLeaf = dynamic_cast<CCDBNodeLeaf
  // *>(sheet->getRootBranch()->getNode(ICDBNode::CTextId("CURRENCY"), false));
  // if (currencyLeaf == NULL)
  //	return;

  // RYMSG::TTradeCurrency currency((RYMSG::TTradeCurrency::TValues)
  // currencyLeaf->getValue32()); if (currency !=
  // RYMSG::TTradeCurrency::tc_faction_points) 	return;

  // bad DB => abort
  CCDBNodeLeaf *ftLeaf =
      dynamic_cast<CCDBNodeLeaf *>(sheet->getRootBranch()->getNode(
          ICDBNode::CTextId("FACTION_TYPE"), false));
  CCDBNodeLeaf *fppLeaf = dynamic_cast<CCDBNodeLeaf *>(
      sheet->getRootBranch()->getNode(ICDBNode::CTextId("PRICE"), false));
...
// ***************************************************************************
void CBotChatPageTrade::getCurrItemFactionTypePoints(
    PVP_CLAN::TPVPClan &pvpClan, uint32 &points) const {
...
uint32 CBotChatPageTrade::getUserFactionPoints(PVP_CLAN::TPVPClan clan) const {
  ...
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  CCDBNodeLeaf *pLeaf = NLGUI::CDBManager::getInstance()->getDbProp(
      toString("LOCAL:USER:FACTION_POINTS_%d:VALUE", nClan), false);
  if (pLeaf == NULL)
    return 0;
...
void CBotChatPageTrade::notifyDownloadComplete(bool completed) {
  ...
  public:
    ...
    virtual void visitGroup(CInterfaceGroup *group) {
      CDBGroupListSheetText *dbGroup =
          dynamic_cast<CDBGroupListSheetText *>(group);
      if (dbGroup) {
...
void CBotChatPageTrade::updateTradeModal() {
  // if loading is finished, then signal it to all sheet list, so they can
  // display an help message if they are empty
  ...
  if (_CurrItemSheet) {
    ...
    if (_BuyDlgOn || _SellDlgOn) {
      // check that curr item hasn't been modified (so that the index remains
      // valid) this could happen if there was a db correction from the server
      ...
      CCtrlTextButton *cantTradeButton =
          dynamic_cast<CCtrlTextButton *>(ig->getCtrl("cant_trade_button"));
        ...
        if ((_BuyMean == MoneyGuildXP) || (_BuyMean == GuildMoneyGuildXP)) {
          if (totalXP > pGM->getXP()) {
            ...
            if (cantTradeGroup)
              cantTradeGroup->setActive(true);
            if (cantTradeButton)
              cantTradeButton->setText(CI18N::get("uiNotEnoughGuildXP"));
            if (cantTradeButton)
              cantTradeButton->setDefaultContextHelp(
                  CI18N::get("uittNotEnoughGuildXP"));
            return;
          }
        }
      ...
      bool validSetup =
          priceWithoutFame != std::numeric_limits<uint64>::max() &&
          quantity != std::numeric_limits<uint32>::max() && quantity != 0;
      ...
      if (validSetup) {
        // basic price
        uint64 totalPriceWithoutFame =
            (uint64)priceWithoutFame * (uint64)quantity;
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:TEMP:TRADE_ITEM:UNIT_PRICE")
            ->setValue64(priceWithoutFame);
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:TEMP:TRADE_ITEM:PRICE")
            ->setValue64(totalPriceWithoutFame);
        // price with fame
        uint64 totalPriceWithFame = (uint64)priceWithFame * (uint64)quantity;
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:TEMP:TRADE_ITEM:UNIT_PRICE_WITH_FAME")
            ->setValue64(priceWithFame);
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:TEMP:TRADE_ITEM:PRICE_WITH_FAME")
            ->setValue64(totalPriceWithFame);
        // resale price
        uint64 totalResalePrice = (uint64)resalePrice * (uint64)quantity;
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:TEMP:TRADE_ITEM:UNIT_PRICE_RESALE")
            ->setValue64(resalePrice);
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:TEMP:TRADE_ITEM:PRICE_RESALE")
            ->setValue64(totalResalePrice);
        // quantity edit box
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:TEMP:TRADE_ITEM:EDIT_QUANTITY")
            ->setValue64(quantity);
        // Faction Points
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:TEMP:TRADE_ITEM:UNIT_PRICE_FACTION")
            ->setValue64(fpCost);
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:TEMP:TRADE_ITEM:PRICE_FACTION")
            ->setValue64(fpCost * quantity);
        ...
        if (_CannotValidateBecauseRetireNotAvailable) {
          if (confirmTradeGroup)
            confirmTradeGroup->setActive(false);
          if (confirmResellGroup)
            confirmResellGroup->setActive(false);
          if (cantTradeGroup)
            cantTradeGroup->setActive(true);
          // can't sell more than what is in inventory
          if (cantTradeButton)
            cantTradeButton->setText(CI18N::get("uiBCNotAvailable"));
          if (cantTradeButton)
            cantTradeButton->setDefaultContextHelp(
                CI18N::get("uittBCNotAvailable"));
          ok = false;
        }
        ...
        if (ok && _QuantityCheck) {
          ...
          if ((sint32)quantity > _QuantityCheck) {
            ...
            // can't sell more than what is in inventory
            if (cantTradeButton)
              cantTradeButton->setText(CI18N::get("uiBadQuantity"));
            if (cantTradeButton)
              cantTradeButton->setDefaultContextHelp(
                  CI18N::get("uittBadQuantity"));
            ok = false;
          }
        }
          ...
          if (_BuyDlgOn) {
            if ((_BuyMean == Money) || (_BuyMean == MoneyFactionPoints)) {
              if (totalPriceWithoutFame > getInventory().getMoney()) {
                ...
                // Check if the player has enough faction point for the object
                // selected
                if ((confirmTradeGroup != NULL) && (cantTradeButton != NULL) &&
                    (cantTradeGroup != NULL)) {
                  if ((fpCost * quantity) > userFactionPoints) {
                    confirmTradeGroup->setActive(false);
                    cantTradeGroup->setActive(true);
                    cantTradeButton->setText(CI18N::get(
                        "uiNotEnoughFP_" + PVP_CLAN::toString(fpType)));
                    cantTradeButton->setDefaultContextHelp(CI18N::get(
                        "uittNotEnoughFP_" + PVP_CLAN::toString(fpType)));
                  } else {
                    confirmTradeGroup->setActive(true);
                    cantTradeGroup->setActive(false);
                  }
              ...
              if (totalPriceWithoutFame > numSkillPoints) {
                ...
                if (cantTradeGroup)
                  cantTradeGroup->setActive(true);
                if (cantTradeButton)
                  cantTradeButton->setText(
                      CI18N::get("uiNotEnoughSkillPoints"));
                if (cantTradeButton)
                  cantTradeButton->setDefaultContextHelp(
            ...
            if (_ResaleEdit && resalePrice < priceWithoutFame) {
              ...
              if (cantTradeButton)
                cantTradeButton->setText(CI18N::get("uiBadResalePrice"));
              if (cantTradeButton)
                cantTradeButton->setDefaultContextHelp(
                    CI18N::get("uittBadResalePrice"));
            }
            // else ok, can resell
            else {
              if (confirmTradeGroup)
                confirmTradeGroup->setActive(true);
              if (_ResaleEdit && confirmResellGroup)
      ...
      // else price is not valid
        ...
        if (_SellDlgOn && priceWithoutFame == 0) {
          if (cantTradeButton)
            cantTradeButton->setText(CI18N::get("uiWontBuyThis"));
          if (cantTradeButton)
            cantTradeButton->setDefaultContextHelp(
                CI18N::get("uittWontBuyThis"));
                   ...
                   priceWithoutFame == std::numeric_limits<uint64>::max()) {
          if (cantTradeButton)
            cantTradeButton->setText(CI18N::get("uiPriceNotReceived"));
        } else if (quantity == 0 ||
                   quantity == std::numeric_limits<uint32>::max()) {
          if (cantTradeButton)
            cantTradeButton->setText(CI18N::get("uiBadQuantity"));
          if (cantTradeButton)
            cantTradeButton->setDefaultContextHelp(
...
void CBotChatPageTrade::setupBuyMeanInModal(CInterfaceGroup *modal) {
  ...
  CInterfaceGroup *money =
      modal->getGroup("standard_price:total_price:item_price");
  CInterfaceGroup *skillPoints =
      modal->getGroup("standard_price:total_price:sp");
  if (equal)
    equal->setActive(_BuyMean == Money);
  if (money)
    money->setActive(_BuyMean == Money);
  if (skillPoints)
    skillPoints->setActive(_BuyMean == SkillPoints);
}

// ***************************************************************************************
void CBotChatPageTrade::startBuyDialog(CDBCtrlSheet *sheet,
                                       CCtrlBase * /* pCaller */) {
  if (!sheet)
    return;
  CInterfaceManager *im = CInterfaceManager::getInstance();
  ...
  // If this item is a User Item already sold (-1), cancel the RETIRE (in list
  // only for information)
  if (sheet->getType() == CCtrlSheetInfo::SheetType_Item &&
      (sheet->getItemSellerType() == BOTCHATTYPE::User ||
       sheet->getItemSellerType() == BOTCHATTYPE::UserRetirable) &&
      sheet->getItemPrice() == -1)
    return;

  // Set price label
  CViewText *priceLabel = dynamic_cast<CViewText *>(
      ig->getView("standard_price:total_price_header"));
  ...
  CCtrlTextButton *confirmButton =
      dynamic_cast<CCtrlTextButton *>(ig->getCtrl("ok"));
  if (confirmButton) {
    confirmButton->setActive(true);
    // no need any context help because too simple
    confirmButton->setDefaultContextHelp(std::string());
    if (isItem) {
      CItemSheet *itemSheet = dynamic_cast<CItemSheet *>(
          SheetMngr.get(CSheetId(sheet->getSheetId())));
      if (itemSheet && itemSheet->Family == ITEMFAMILY::COSMETIC) {
        EGSPD::CPeople::TPeople people =
            ITEM_ORIGIN::itemOriginStringToPeopleEnum(
                ITEM_ORIGIN::enumToString(itemSheet->ItemOrigin));
        if (UserEntity->getGender() != itemSheet->Cosmetic.Gender ||
            UserEntity->people() != people)
          confirmButton->setActive(false);
        else
          confirmButton->setText(
              CI18N::get(_UsePriceRetire ? "uiRetire" : "uiBuy"));
      } else
        confirmButton->setText(
            CI18N::get(_UsePriceRetire ? "uiRetire" : "uiBuy"));
    } else
  ...
  ig->updateCoords();
  ig->center();
  ig->setModalParentList(_BuyOnly ? WIN_BOT_CHAT_PAGE_BUY
                                  : WIN_BOT_CHAT_PAGE_TRADE);
  ...
  _BuyDlgOn = true;
  NLGUI::CDBManager::getInstance()
      ->getDbProp("UI:TEMP:TRADE_ITEM:IS_SELL_DLG")
      ->setValueBool(false);
  // setup trade mean
...
// ***************************************************************************************
void CBotChatPageTrade::startSellDialog(CDBCtrlSheet *sheet,
                                        CCtrlBase * /* pCaller */) {
  nlassert(!_BuyOnly);
  if (!sheet)
    return;
  ...
  // If this sheet is grayed cause of an "Animal Inventory unavailable" flag,
  // quit
  if (sheet->getItemBeastGrayed())
    return;
  ...
  CViewText *itemNameView =
      dynamic_cast<CViewText *>(ig->getView("object_name"));
  if (itemNameView) {
    string itemName;
...
void CBotChatPageTrade::endTradeModal() {
  ...
  // Hide any confirmation dialog related to trade (important for building
...
bool CBotChatPageTrade::isTradeValid(bool enableResale) const {
  ...
  if ((_BuyMean == MoneyGuildXP) || (_BuyMean == GuildMoney) ||
    ...
    uint64 totalXP = (uint64)getCurrItemXP() * (uint64)quantity;
    CGuildManager *pGM = CGuildManager::getInstance();
    uint64 moneyOwned = (_BuyMean == MoneyGuildXP) ? getInventory().getMoney()
                                                   : pGM->getMoney();
    if ((_BuyMean == MoneyGuildXP) || (_BuyMean == GuildMoneyGuildXP))
  ...
  // if price/quantity is valid
  bool validSetup = priceWithoutFame != std::numeric_limits<uint64>::max() &&
                    quantity != std::numeric_limits<uint32>::max() &&
                    quantity != 0;
  if (validSetup && _BuyMean == MoneyFactionPoints) {
    // valid if at least one price type is not 0
  ...
  if (validSetup) {
    ...
    // Sell dlg
      ...
      return (!_ResaleEdit || !enableResale ||
              (sint32)priceWithoutFame <= resalePrice);
    }
  }
  return false;
}
...
void CBotChatPageTrade::confirmTrade(bool enableResale) {
  ...
  if (_BuyDlgOn || _SellDlgOn) {
    uint32 quantity = getCurrItemQuantity();
    sint32 resalePrice =
        resaleEnabled ? getCurrItemPriceResale()
                      : 0; // getCurrItemPriceResale() returns 0 is !_ResaleEdit
    if (quantity != std::numeric_limits<uint32>::max()) {
      ...
      if (_CurrItemSheet->isSPhrase()) {
        ...
        extern void phraseBuyBotChat(CDBCtrlSheet * ctrl, uint8 index,
                                     bool useBuySheetMsg);
        // client Side fill Mode? => use PHRASE:BUY_SHEET
        phraseBuyBotChat(_CurrItemSheet, (uint8)_CurrItemIndex,
                         _TradePagesObs.isPhraseClientFill());
      ...
      // Else standard buy.
        ...
        if (_BuyDlgOn) {
          ...
          if ((_BuyMean == MoneyGuildXP) || (_BuyMean == GuildMoney) ||
            ...
            // bug #212: Server should check if player has enough room in his
            // inventory before removing items in trade window Temp fix on
            // client side : check room in player's bag before removing items
            ...
            bool isSpaceInBag =
                pInv->isSpaceInBagForItem(_CurrItemSheet, quantity, 0);
            ...
            if (GenericMsgHeaderMngr.pushNameToStream(msg, out)) {
              ...
              NetMngr.push(out);
              // nlinfo("impulseCallBack sent: %s %d %d", msg, u16Index,
              // u16Quantity);
            } else
              nlwarning(" unknown message name '%s'", msg);
            ...
            if (isSpaceInBag) {
              // If the Buy is from a Player item or User item (retire) localy
              // delete it
              if (_BuyMean == Money &&
                  _CurrItemSheet->getItemSellerType() != BOTCHATTYPE::NPC) {
                // if item stackable, the player was allowed to buy only a
                // subset
                if (_QuantityEdit) {
          ...
          if (GenericMsgHeaderMngr.pushNameToStream(msg, out)) {
            ...
            NetMngr.push(out);
            // nlinfo("impulseCallBack sent: %s %d %d %d %d", msg, u8Inv,
            // u16Index, u16Quantity, u32resalePrice);
          } else
            nlwarning(" unknown message name '%s'", msg);
          ...
          if (_ResaleEdit) {
            ...
            if (ig) {
              CGroupEditBox *eb = dynamic_cast<CGroupEditBox *>(
                  ig->getGroup("header_opened:resell_group:can_resell:choose_"
                               "resell:edit:eb"));
              if (eb) {
                NLGUI::CDBManager::getInstance()
                    ->getDbProp("UI:SAVE:TRADE_ITEM:RESALE_MARGIN")
                    ->setValue32(eb->getInputStringAsInt());
              }
            }
          }
        }
...
sint32 CBotChatPageTrade::getSkillPointUsedForCurrentRoleMaster() const {
  ...
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  uint rmt = NLGUI::CDBManager::getInstance()
                 ->getDbProp("SERVER:BOTCHAT:ROLEMASTER_TYPE")
                 ->getValue32();
  // get the prop (use local for less lag)
  CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
      toString("LOCAL:USER:SKILL_POINTS_%d:VALUE", rmt), false);
  if (node)
...
// ***************************************************************************
void CBotChatPageTrade::addSkillPointForCurrentRoleMaster(
    sint32 addValue) const {
  // get the current phrase rolemaster type
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  uint rmt = NLGUI::CDBManager::getInstance()
                 ->getDbProp("SERVER:BOTCHAT:ROLEMASTER_TYPE")
...
uint CBotChatPageTrade::initQuantityMode(bool sellMode, CDBCtrlSheet *sheet) {
  ...
  if (pIS &&
      (pIS->Stackable > 1 || pIS->Family == ITEMFAMILY::ITEM_SAP_RECHARGE) &&
      pIS->Family != ITEMFAMILY::TELEPORT) {
...
// ***************************************************************************
void CBotChatPageTrade::setupPriceGroupQuantity(CInterfaceGroup *priceGroup,
                                                sint32 defaultQuantity) {
  ...
  if (!_QuantityEdit) {
    ...
    if (quantityGroup) {
      ...
      CGroupEditBox *eb =
          dynamic_cast<CGroupEditBox *>(quantityGroup->getGroup("edit:eb"));
      if (eb) {
...
// ***************************************************************************
void CBotChatPageTrade::setupResellGroup(bool sellMode, uint defaultQuantity,
  ...
  // must be a sell, must be an item, and must be in a standard BuyMean, also
  // must be a ressellable item
  ...
  const CItemSheet *pIS = sheet->asItemSheet();
  bool resellable = sellMode && pIS && _BuyMean == Money &&
                    ITEMFAMILY::isResellable(pIS->Family);
  // also cannot resell Raw Material that don't have any item part (formula
  // materials, because cannot filter them)
  ...
  if (!resellable) {
    ...
    CCtrlBase *confirmResellGroup =
        parentGroup->getCtrl("confirm_trade_resell");
    if (confirmResellGroup)
      confirmResellGroup->setActive(false);
    _ResaleEdit = false;
  } else {
    ...
    CInterfaceGroup *chooseResellGroup =
        resellGroup->getGroup("can_resell:choose_resell");
    CCtrlBase *confirmResellGroup =
        parentGroup->getCtrl("confirm_trade_resell");
    if (!canResellGroup || !cantResellGroup || !chooseResellGroup ||
        !confirmResellGroup) {
    ...
    BOTCHATTYPE::TBotChatResaleFlag resaleFlag =
        (BOTCHATTYPE::TBotChatResaleFlag)sheet->getItemResaleFlag();
    //		sint	basePrice = sint(sheet->getItemPrice() *
    //_FamePriceFactorLeaf->getValue16()/10000.0f);
    ...
    if (resaleFlag != BOTCHATTYPE::ResaleOk) {
      ...
      // setup the reason
      CViewText *vt =
          dynamic_cast<CViewText *>(cantResellGroup->getView("reason"));
      if (vt) {
    ...
    // else setup "can_resell:choose_resell group"
      ...
      CGroupEditBox *eb =
          dynamic_cast<CGroupEditBox *>(chooseResellGroup->getGroup("edit:eb"));
      if (eb) {
        sint32 resaleMargin =
            NLGUI::CDBManager::getInstance()
                ->getDbProp("UI:SAVE:TRADE_ITEM:RESALE_MARGIN")
...
void CBotChatPageTrade::setupEditFocus(CInterfaceGroup *parentGroup) {
  ...
  if (_QuantityEdit) {
    ...
    CGroupEditBox *eb = dynamic_cast<CGroupEditBox *>(
        parentGroup->getGroup("standard_price:quantity:edit:eb"));
    if (eb) {
      // set focus
...
// ***************************************************************************
void CBotChatPageTrade::setupPriceOrPriceRetire(bool sellMode,
                                                CDBCtrlSheet *sheet) {
  nlassert(sheet);
  ...
  if (!sellMode && pIS && _BuyMean == Money) {
    ...
    if (st == BOTCHATTYPE::UserRetirable ||
        st == BOTCHATTYPE::ResaleAndUserRetirable || st == BOTCHATTYPE::User ||
        st == BOTCHATTYPE::ResaleAndUser) {
    ...
    if (pIS->Family == ITEMFAMILY::COSMETIC) {
      EGSPD::CPeople::TPeople people =
          ITEM_ORIGIN::itemOriginStringToPeopleEnum(
              ITEM_ORIGIN::enumToString(pIS->ItemOrigin));
      if (UserEntity->getGender() != pIS->Cosmetic.Gender ||
          UserEntity->people() != people)
        _CannotValidateBecauseRetireNotAvailable = true;
    }
  }
...
// ***************************************************************************
void CBotChatPageTrade::setupFactionPointPrice(bool /* sellMode */,
  ...
  if (fpCost == 0 || fpType == PVP_CLAN::None) {
    ...
    CViewText *vt =
        dynamic_cast<CViewText *>(fpGroup->getView("unit_price_header"));
    if (vt) {
    ...
    string factionIcon = CWidgetManager::getInstance()->getParser()->getDefine(
        toString("faction_icon_%s", factionName.c_str()));
    CViewBitmap *vBmp = dynamic_cast<CViewBitmap *>(
        fpGroup->getView("unit_price:item_price:icone"));
    if (vBmp)
      vBmp->setTexture(factionIcon);
    vBmp = dynamic_cast<CViewBitmap *>(
        fpGroup->getView("total_price:item_price:icone"));
    if (vBmp)
      vBmp->setTexture(factionIcon);
  }
}
...
void CBotChatPageTrade::startChangeBuyFilterDialog(const std::string &dbext,
  ...
  CInterfaceGroup *ig = dynamic_cast<CInterfaceGroup *>(
      CWidgetManager::getInstance()->getElementFromId(
          WIN_BOT_CHAT_CHANGE_BUY_FILTER));
  if (!ig)
    return;
  ...
  CGroupEditBox *edMin =
      dynamic_cast<CGroupEditBox *>(ig->getGroup("edit_min:eb"));
  CGroupEditBox *edMax =
      dynamic_cast<CGroupEditBox *>(ig->getGroup("edit_max:eb"));
  CCDBNodeLeaf *dbRangeMin = NLGUI::CDBManager::getInstance()->getDbProp(
      string(DB_BOT_CHAT_BASE_BUY_FILTER ":MIN_") + _FilterBuyDlgDBExt, false);
  CCDBNodeLeaf *dbRangeMax = NLGUI::CDBManager::getInstance()->getDbProp(
      string(DB_BOT_CHAT_BASE_BUY_FILTER ":MAX_") + _FilterBuyDlgDBExt, false);
  sint rangeMin = 0;
  sint rangeMax = 0;
  if (dbRangeMin)
    rangeMin = dbRangeMin->getValue32();
  if (dbRangeMax)
    rangeMax = dbRangeMax->getValue32();
  if (edMin)
    edMin->setInputString(toString(rangeMin));
  if (edMax)
    edMax->setInputString(toString(rangeMax));
  if (edMin)
    edMin->setPositiveIntegerMaxValue(maxValue);
  if (edMax)
    edMax->setPositiveIntegerMaxValue(maxValue);
...
void CBotChatPageTrade::resetBuyFilterDialog() {
  ...
  if (edMax)
    edMax->setInputString(toString(rangeMax));
  if (dbRangeMin)
    dbRangeMin->setValue32(rangeMin);
  if (dbRangeMax)
    dbRangeMax->setValue32(rangeMax);
...
void CBotChatPageTrade::confirmChangeBuyFilterDialog() {
  ...
  sint rangeMax = 0;
  if (edMin)
    rangeMin = edMin->getInputStringAsInt();
  if (edMax)
    rangeMax = edMax->getInputStringAsInt();
...
void CBotChatPageTrade::startChangeBuyFilterMPDialog() {
  ...
  CInterfaceGroup *ig = dynamic_cast<CInterfaceGroup *>(
      CWidgetManager::getInstance()->getElementFromId(
          WIN_BOT_CHAT_CHANGE_BUY_FILTER_MP));
  if (!ig)
    return;
...
void CBotChatPageTrade::confirmChangeBuyFilterMPDialog(uint ft) {
  ...
  // set ItemPart DB
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  CCDBNodeLeaf *dbItemPart = NLGUI::CDBManager::getInstance()->getDbProp(
      string(DB_BOT_CHAT_BASE_BUY_FILTER ":MP_ITEM_PART"), false);
  if (dbItemPart)
    dbItemPart->setValue32(ft);
...
void CBotChatPageTrade::resetItemPartAndTypeFilters() {
  ...
  CCDBNodeLeaf *dbItemPart = NLGUI::CDBManager::getInstance()->getDbProp(
      DB_BOT_CHAT_BASE_BUY_FILTER ":MP_ITEM_PART", false);
  CCDBNodeLeaf *dbItemType = NLGUI::CDBManager::getInstance()->getDbProp(
      DB_BOT_CHAT_BASE_BUY_FILTER ":ITEM_TYPE", false);

  if (dbItemPart)
    dbItemPart->setValue32(RM_FABER_TYPE::Unknown);
  if (dbItemType)
    dbItemType->setValue32(ITEM_TYPE::UNDEFINED);
}
...
void CBotChatPageTrade::sendCurrentBuyFilterToServer(bool resetBuyList) {
  ...
  CCDBNodeLeaf *dbQualityMin = NLGUI::CDBManager::getInstance()->getDbProp(
      DB_BOT_CHAT_BASE_BUY_FILTER ":MIN_QUALITY", false);
  CCDBNodeLeaf *dbQualityMax = NLGUI::CDBManager::getInstance()->getDbProp(
      DB_BOT_CHAT_BASE_BUY_FILTER ":MAX_QUALITY", false);
  CCDBNodeLeaf *dbPriceMin = NLGUI::CDBManager::getInstance()->getDbProp(
      DB_BOT_CHAT_BASE_BUY_FILTER ":MIN_PRICE", false);
  CCDBNodeLeaf *dbPriceMax = NLGUI::CDBManager::getInstance()->getDbProp(
      DB_BOT_CHAT_BASE_BUY_FILTER ":MAX_PRICE", false);
  CCDBNodeLeaf *dbClassMin = NLGUI::CDBManager::getInstance()->getDbProp(
      DB_BOT_CHAT_BASE_BUY_FILTER ":MIN_CLASS", false);
  CCDBNodeLeaf *dbClassMax = NLGUI::CDBManager::getInstance()->getDbProp(
      DB_BOT_CHAT_BASE_BUY_FILTER ":MAX_CLASS", false);
  ...
  uint8 classMin = 0,
        classMax = RM_CLASS_TYPE::NumTRMClassType - 1; // max class is 4
  ...
  if (dbQualityMin)
    qualityMin = dbQualityMin->getValue32();
  if (dbQualityMax)
    qualityMax = dbQualityMax->getValue32();
  if (dbPriceMin)
    priceMin = dbPriceMin->getValue32();
  if (dbPriceMax)
    priceMax = dbPriceMax->getValue32();
  if (dbClassMin)
    classMin = (uint8)dbClassMin->getValue32();
  if (dbClassMax)
    classMax = (uint8)dbClassMax->getValue32();
  if (dbItemPart)
    itemPart = (uint8)dbItemPart->getValue32();
  if (dbItemType)
    itemType = (uint8)dbItemType->getValue32();
  ...
  if (GenericMsgHeaderMngr.pushNameToStream(msg, out)) {
    ...
    out.serial(itemType);
    NetMngr.push(out);
    // nlinfo("impulseCallBack sent: %s %d %d %d %d %d %d %d %d", msg,
    // qualityMin, qualityMax, priceMin, priceMax, classMin, classMax, itemPart,
    // itemType);
  } else
    nlwarning(" unknown message name '%s'", msg);
...
void CBotChatPageTrade::unselectBuyList(CGroupContainer *gc) {
  ...
  if (buyGroup) {
    ...
    CDBGroupListSheetText *buyListSheet = dynamic_cast<CDBGroupListSheetText *>(
        buyGroup->getGroup("general_buy"));
    if (buyListSheet)
      buyListSheet->unselect();
    // unselect Player group
    buyListSheet =
        dynamic_cast<CDBGroupListSheetText *>(buyGroup->getGroup("player_buy"));
    if (buyListSheet)
      buyListSheet->unselect();
  }
}
...
void CBotChatPageTrade::unselectSellList(CGroupContainer *gc) {
  ...
  if (sellGroup) {
    // unselect the bag
    CDBGroupListSheetText *sellListSheet =
        dynamic_cast<CDBGroupListSheetText *>(sellGroup->getGroup("bag_sell"));
    if (sellListSheet)
      sellListSheet->unselect();
    // unselect all pack animals
    for (uint i = 0; i < MAX_INVENTORY_ANIMAL; i++) {
      sellListSheet = dynamic_cast<CDBGroupListSheetText *>(
          sellGroup->getGroup(toString("beast%d_sell", i)));
      if (sellListSheet)
        sellListSheet->unselect();
    }
  }
...
void CBotChatPageTrade::startChangeBuyFilterClassDialog() {
  ...
  CCDBNodeLeaf *dbClassMax = NLGUI::CDBManager::getInstance()->getDbProp(
      DB_BOT_CHAT_BASE_BUY_FILTER ":MAX_CLASS", false);
  CCDBNodeLeaf *tempClassMin = NLGUI::CDBManager::getInstance()->getDbProp(
      "UI:VARIABLES:BOTCHAT:TEMP_FILTER_MIN_CLASS");
  CCDBNodeLeaf *tempClassMax = NLGUI::CDBManager::getInstance()->getDbProp(
      "UI:VARIABLES:BOTCHAT:TEMP_FILTER_MAX_CLASS");
  if (dbClassMin)
    tempClassMin->setValue32(dbClassMin->getValue32());
  if (dbClassMax)
    tempClassMax->setValue32(dbClassMax->getValue32());

  // show the modal
  CInterfaceGroup *ig = dynamic_cast<CInterfaceGroup *>(
      CWidgetManager::getInstance()->getElementFromId(
          WIN_BOT_CHAT_CHANGE_BUY_FILTER_CLASS));
  if (!ig)
    return;
...
void CBotChatPageTrade::confirmChangeBuyFilterClassDialog() {
  ...
  if (dbClassMin)
    dbClassMin->setValue32(minClass);
  if (dbClassMax)
    dbClassMax->setValue32(maxClass);
...
// ***************************************************************************
std::string
CBotChatPageTrade::getItemSheetNameForItemType(ITEM_TYPE::TItemType it) {
...
void CBotChatPageTrade::startChangeBuyFilterItemTypeDialog() {
  ...
  // the list of possible item type to select is filled when the DB of bitfield
  // change

  // show the modal
  CInterfaceGroup *ig = dynamic_cast<CInterfaceGroup *>(
      CWidgetManager::getInstance()->getElementFromId(
          WIN_BOT_CHAT_CHANGE_BUY_FILTER_ITEM_TYPE));
  if (!ig)
    return;
...
// ***************************************************************************
void CBotChatPageTrade::confirmChangeBuyFilterItemTypeDialog(
  ...
  CCDBNodeLeaf *dbItemType = NLGUI::CDBManager::getInstance()->getDbProp(
      DB_BOT_CHAT_BASE_BUY_FILTER ":ITEM_TYPE", false);
  if (dbItemType)
    dbItemType->setValue32(itemType);
...
void CBotChatPageTrade::startDestroyItemDialog() {
  ...
  // set view
  NLGUI::CDBManager::getInstance()
      ->getDbProp("UI:TEMP:TRADE_ITEM:DESTROY_QUANTITY")
      ->setValue32(quantity);

  // show the modal
  CInterfaceGroup *ig = dynamic_cast<CInterfaceGroup *>(
      CWidgetManager::getInstance()->getElementFromId(
          WIN_BOT_CHAT_DESTROY_ITEM));
  if (!ig)
    return;
  CWidgetManager::getInstance()->enableModalWindow(NULL, ig);
}
...
class CAHConfirmTrade : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string &params) {
    ...
    case 1:
      enableResale = true;
      break;
    default: // 2: comes only from Enter of Shift+Enter key from an edit box or
             // in the modal window
    {
      const NLGUI::CEventDescriptorKey &keyEvent =
          CWidgetManager::getInstance()->getLastKeyEvent();
    ...
    if (canTestConfirmation) {
      // Building:
      if (NLGUI::CDBManager::getInstance()->getDbValue32(
              "SERVER:TRADING:BUILDING_LOSS_WARNING") == 1)
        mustConfirm = true;
    }
    ...
    if (!mustConfirm) {
      ...
      // need to ask user first. Important unexecpected consequence, so use a
      // warning icon
      pIM->validMessageBox(
          CInterfaceManager::WarningIconMsg, CI18N::get("uiQReplaceAppartment"),
          "confirm_trade",
          toString("resale=%d|noconfirm=1", (uint)enableResale));
    }
  }
};
...
REGISTER_ACTION_HANDLER(CAHEnterChangeBotChatBuyFilter,
                        "enter_change_botchat_buy_filter");
...
REGISTER_ACTION_HANDLER(CAHConfirmChangeBotChatBuyFilter,
                        "confirm_change_botchat_buy_filter");
...
REGISTER_ACTION_HANDLER(CAHChangeBotChatBuyFilterGiveFocusToMax,
                        "change_botchat_buy_filter_give_focus_to_max");
...
REGISTER_ACTION_HANDLER(CAHEnterChangeBotChatBuyFilterMP,
                        "enter_change_botchat_buy_filter_mp");

// ***************************************************************************
// Validate Change the Min/Max of Price or quantity
class CAHConfirmChangeBotChatBuyFilterMP : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string &params) {
    BotChatPageAll->Trade->confirmChangeBuyFilterMPDialog(
        RM_FABER_TYPE::toFaberType(params));
  }
};
REGISTER_ACTION_HANDLER(CAHConfirmChangeBotChatBuyFilterMP,
...
REGISTER_ACTION_HANDLER(CAHEnterChangeBotChatBuyFilterClass,
                        "enter_change_botchat_buy_filter_class");
...
REGISTER_ACTION_HANDLER(CAHConfirmChangeBotChatBuyFilterClass,
                        "confirm_change_botchat_buy_filter_class");
...
REGISTER_ACTION_HANDLER(CAHResetBotChatBuyFilterClass,
                        "reset_botchat_buy_filter_class");
...
REGISTER_ACTION_HANDLER(CAHEnterChangeBotchatBuyFilterItemType,
                        "enter_change_botchat_buy_filter_item_type");
...
REGISTER_ACTION_HANDLER(CAHConfirmChangeBotchatBuyFilterItemType,
                        "confirm_change_botchat_buy_filter_item_type");
...
class CAHBotChatRefilItemTypeChoiceList : public IActionHandler {
  void execute(CCtrlBase * /* pCaller */, const std::string &sParams) {
    ...
    /* If ItemType grows too big, must change SERVER and CLIENT!!! because of
       ItemType bitfield system: use
       SERVER:TRADING:ITEM_TYPE_SELLER_BITFILED_0_63 which is a 64 bits
                    SERVER:TRADING:ITEM_TYPE_SELLER_BITFILED_64_127 which is a
       64 bits (param offset gives which part of enum to use)
    */
    nlctassert(ITEM_TYPE::UNDEFINED <= 128);

    // get the src bitfield
    CCDBNodeLeaf *nodeSrc =
        NLGUI::CDBManager::getInstance()->getDbProp(srcDB, false);
    if (!nodeSrc)
      return;
    ...
    for (uint i = 0; i < 64; i++) {
      ...
      // get the dest node
      CCDBNodeLeaf *nodeDst = NLGUI::CDBManager::getInstance()->getDbProp(
          destDB + toString(":%d:SHEET", i + offset), false);
      if (nodeDst) {
        if (present) {
          // get the item sheet id associated to this item type
          CSheetId sheetId;
          sheetId.buildSheetId(CBotChatPageTrade::getItemSheetNameForItemType(
              (ITEM_TYPE::TItemType)(i + offset)));
          // and set it
...
REGISTER_ACTION_HANDLER(CAHBotChatRefilItemTypeChoiceList,
                        "botchat_refill_item_type_choice_list");
...
static DECLARE_INTERFACE_USER_FCT(getPriceWithFame) {
  ...
  if (value == -1)
    ...
    result.setString(NLMISC::formatThousands(toString(valueFame)) + " (" +
...
NLMISC_COMMAND(testColorItems, "Temp : test some items to trade", "") {
  ...
  //
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:0:SHEET")
      ->setValue32(CSheetId(itemName).asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:0:PRICE")
      ->setValue32(100);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:0:QUALITY")
      ->setValue32(0);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:0:SLOT_TYPE")
      ->setValue32(0);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:0:USER_COLOR")
      ->setValue32(0);
  //
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:1:SHEET")
      ->setValue32(CSheetId(itemName).asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:1:PRICE")
      ->setValue32(200);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:1:QUALITY")
      ->setValue32(25);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:1:SLOT_TYPE")
      ->setValue32(0);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:1:USER_COLOR")
      ->setValue32(1);
  //
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:2:SHEET")
      ->setValue32(CSheetId(itemName).asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:2:PRICE")
      ->setValue32(10);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:2:QUALITY")
      ->setValue32(0);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:2:SLOT_TYPE")
      ->setValue32(0);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:2:USER_COLOR")
      ->setValue32(2);
  //
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:3:SHEET")
      ->setValue32(CSheetId(itemName).asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:3:PRICE")
      ->setValue32(10);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:3:QUALITY")
      ->setValue32(0);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:3:SLOT_TYPE")
      ->setValue32(0); //
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:3:USER_COLOR")
      ->setValue32(3);
  //
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:4:SHEET")
      ->setValue32(CSheetId(itemName).asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:4:PRICE")
      ->setValue32(100);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:4:QUALITY")
      ->setValue32(0);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:4:SLOT_TYPE")
      ->setValue32(0);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:4:USER_COLOR")
      ->setValue32(4);
  //
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:5:SHEET")
      ->setValue32(CSheetId(itemName).asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:5:PRICE")
      ->setValue32(200);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:5:QUALITY")
      ->setValue32(25);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:5:SLOT_TYPE")
      ->setValue32(0);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:5:USER_COLOR")
      ->setValue32(5);
  //
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:6:SHEET")
      ->setValue32(CSheetId(itemName).asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:6:PRICE")
      ->setValue32(10);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:6:QUALITY")
      ->setValue32(0);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:6:SLOT_TYPE")
      ->setValue32(0);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:6:USER_COLOR")
      ->setValue32(6);
  //
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:7:SHEET")
      ->setValue32(CSheetId(itemName).asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:7:PRICE")
      ->setValue32(10);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:7:QUALITY")
      ->setValue32(0);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:7:SLOT_TYPE")
      ->setValue32(0);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:7:USER_COLOR")
      ->setValue32(7);
  //
  sint32 value;
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:SESSION")
      ->setValue32(CBotChatManager::getInstance()->getSessionID());
  fromString(args[0], value);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:PAGE_ID")
      ->setValue32(value);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:HAS_NEXT")
      ->setValue32(1);
  //
  return true;
}
...
NLMISC_COMMAND(testTradeItems, "Temp : test some items to trade", "") {
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:0:SHEET")
      ->setValue32(CSheetId("guild_main_building.sitem").asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:0:PRICE")
      ->setValue32(100);
  NLGUI::CDBManager::getInstance()
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:1:SHEET")
      ->setValue32(CSheetId("guild_rm_craft.sitem").asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:1:PRICE")
      ->setValue32(200);
  NLGUI::CDBManager::getInstance()
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:2:SHEET")
      ->setValue32(CSheetId("guild_rm_fight.sitem").asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:2:PRICE")
      ->setValue32(10);
  NLGUI::CDBManager::getInstance()
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:4:SHEET")
      ->setValue32(CSheetId("guild_rm_magic.sitem").asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:4:PRICE")
      ->setValue32(100);
  NLGUI::CDBManager::getInstance()
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:5:SHEET")
      ->setValue32(CSheetId("icfp1pb.sitem").asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:5:PRICE")
      ->setValue32(200);
  NLGUI::CDBManager::getInstance()
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:6:SHEET")
      ->setValue32(CSheetId("icragt.sitem").asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:6:PRICE")
      ->setValue32(10);
  NLGUI::CDBManager::getInstance()
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:7:SHEET")
      ->setValue32(CSheetId("icmss.sitem").asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:7:PRICE")
      ->setValue32(10);
  NLGUI::CDBManager::getInstance()
...
NLMISC_COMMAND(testTradeItems2, "Temp : test some items to trade (2)", "") {
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:0:SHEET")
      ->setValue32(CSheetId("icfm1bm.sitem").asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:0:PRICE")
      ->setValue32(100);
  NLGUI::CDBManager::getInstance()
  ...
  NLGUI::CDBManager::getInstance()
      ...
      ->setValue32(0);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:0:FACTION_TYPE")
      ->setValue32(0);
  //	NLGUI::CDBManager::getInstance()->getDbProp("SERVER:TRADING:0:PRICE")->setValue32(130);
  //// should result to none
  //
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:1:SHEET")
      ->setValue32(CSheetId("icfm1bs.sitem").asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:1:PRICE")
      ->setValue32(200);
  NLGUI::CDBManager::getInstance()
  ...
  NLGUI::CDBManager::getInstance()
      ...
      ->setValue32(0);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:1:FACTION_TYPE")
      ->setValue32(PVP_CLAN::Kami);
  //	NLGUI::CDBManager::getInstance()->getDbProp("SERVER:TRADING:1:FACTION_POINT_PRICE")->setValue32(0);
  //// should result to none
  //
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:2:SHEET")
      ->setValue32(CSheetId("icfm1pd.sitem").asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:2:PRICE")
      ->setValue32(10);
  NLGUI::CDBManager::getInstance()
  ...
  NLGUI::CDBManager::getInstance()
      ...
      ->setValue32(0); //
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:3:FACTION_TYPE")
      ->setValue32(PVP_CLAN::Tryker);
  //	NLGUI::CDBManager::getInstance()->getDbProp("SERVER:TRADING:3:FACTION_POINT_PRICE")->setValue32(10);
  //
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:4:SHEET")
      ->setValue32(CSheetId("ictr2b.sitem").asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:4:PRICE")
      ->setValue32(0);
  NLGUI::CDBManager::getInstance()
  ...
  NLGUI::CDBManager::getInstance()
      ...
      ->setValue32(0);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:4:FACTION_TYPE")
  ...
  NLGUI::CDBManager::getInstance()
      ...
      ->setValue32(0);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:5:FACTION_TYPE")
      ->setValue32(PVP_CLAN::Tryker);
  ...
  NLGUI::CDBManager::getInstance()
      ...
      ->setValue32(0);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:6:FACTION_TYPE")
      ->setValue32(PVP_CLAN::Karavan);
  ...
  NLGUI::CDBManager::getInstance()
      ...
      ->setValue32(0);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:7:FACTION_TYPE")
      ->setValue32(PVP_CLAN::Kami);
...
NLMISC_COMMAND(testTradeItems3, "Temp : test some items to trade (3)", "") {
  ...
  CInterfaceManager *im = CInterfaceManager::getInstance();
  NLGUI::CDBManager::getInstance()
      ->getDbProp("LOCAL:TRADING:0:SHEET")
      ->setValue32(CSheetId("fyros_buckler_lvl_01_05.item").asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("LOCAL:TRADING:0:PRICE")
      ->setValue32(200);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("LOCAL:TRADING:0:QUALITY")
      ->setValue32(25);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("LOCAL:TRADING:0:SLOT_TYPE")
      ->setValue32(0);
  //
  NLGUI::CDBManager::getInstance()
      ->getDbProp("LOCAL:TRADING:1:SHEET")
      ->setValue32(CSheetId("fyros_buckler_lvl_01_05.item").asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("LOCAL:TRADING:1:PRICE")
      ->setValue32(200);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("LOCAL:TRADING:1:QUALITY")
      ->setValue32(25);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("LOCAL:TRADING:1:SLOT_TYPE")
      ->setValue32(0);
  //
  NLGUI::CDBManager::getInstance()
      ->getDbProp("LOCAL:TRADING:2:SHEET")
      ->setValue32(CSheetId("fyros_buckler_lvl_01_05.item").asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("LOCAL:TRADING:2:PRICE")
      ->setValue32(200);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("LOCAL:TRADING:2:QUALITY")
      ->setValue32(25);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("LOCAL:TRADING:2:SLOT_TYPE")
      ->setValue32(0);
  //
  NLGUI::CDBManager::getInstance()
      ->getDbProp("LOCAL:TRADING:3:SHEET")
      ->setValue32(CSheetId("fyros_buckler_lvl_01_05.item").asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("LOCAL:TRADING:3:PRICE")
      ->setValue32(200);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("LOCAL:TRADING:3:QUALITY")
      ->setValue32(25);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("LOCAL:TRADING:3:SLOT_TYPE")
      ->setValue32(0);
...
NLMISC_COMMAND(testTradeItems4, "Temp : test some items to trade (4)", "") {
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("LOCAL:TRADING:0:SHEET")
      ->setValue32(CSheetId("pacts.death_impact").asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("LOCAL:TRADING:0:QUALITY")
      ->setValue32(0);
  NLGUI::CDBManager::getInstance()
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("LOCAL:TRADING:1:SHEET")
      ->setValue32(CSheetId("kami_pactes.death_impact").asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("LOCAL:TRADING:1:QUALITY")
      ->setValue32(0);
  NLGUI::CDBManager::getInstance()
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("LOCAL:TRADING:2:SHEET")
      ->setValue32(CSheetId("karavan_pactes.death_impact").asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("LOCAL:TRADING:2:QUALITY")
      ->setValue32(0);
  NLGUI::CDBManager::getInstance()
...
NLMISC_COMMAND(testPhrases, "Temp : test some items to trade", "") {
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:0:SHEET")
      ->setValue32(CSheetId("abfaimhame09.sphrase").asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:0:PRICE")
      ->setValue32(11);
  NLGUI::CDBManager::getInstance()
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:1:SHEET")
      ->setValue32(CSheetId("abfaimhcme07.sphrase").asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:1:PRICE")
      ->setValue32(22);
  NLGUI::CDBManager::getInstance()
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:2:SHEET")
      ->setValue32(CSheetId("abfaimhfme04.sphrase").asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:2:PRICE")
      ->setValue32(33);
  NLGUI::CDBManager::getInstance()
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:3:SHEET")
      ->setValue32(CSheetId("abfaimhfme08.sphrase").asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:3:PRICE")
      ->setValue32(44);
  NLGUI::CDBManager::getInstance()
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:4:SHEET")
      ->setValue32(CSheetId("abfma05.sphrase").asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:4:PRICE")
      ->setValue32(55);
  NLGUI::CDBManager::getInstance()
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:5:SHEET")
      ->setValue32(CSheetId("abm_mt_ae_acid_00055.sphrase").asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:5:PRICE")
      ->setValue32(66);
  NLGUI::CDBManager::getInstance()
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:6:SHEET")
      ->setValue32(CSheetId("abm_mt_heal_00140.sphrase").asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:6:PRICE")
      ->setValue32(77);
  NLGUI::CDBManager::getInstance()
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:7:SHEET")
      ->setValue32(CSheetId("abm_mt_cannibalism_00020.sphrase").asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:7:PRICE")
      ->setValue32(88);
  NLGUI::CDBManager::getInstance()
  ...
  NLGUI::CDBManager::getInstance()
      ...
      ->setValue32(1);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:ROLEMASTER_FLAGS")
      ->setValue32(0);
...
NLMISC_COMMAND(testResaleItems, "Temp : test resale", "") {
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:0:SHEET")
      ->setValue32(CSheetId("m0006dxajf01.sitem").asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:0:PRICE")
      ->setValue32(0);
  NLGUI::CDBManager::getInstance()
      ...
      ->setValue32(1);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:0:SELLER_TYPE")
      ->setValue32(0);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:0:PRICE_RETIRE")
      ->setValue32(111);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:0:QUANTITY")
      ->setValue32(62);
  //
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:1:SHEET")
  ...
  NLGUI::CDBManager::getInstance()
      ...
      ->setValue32(11);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:1:SELLER_TYPE")
      ->setValue32(1);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:1:PRICE_RETIRE")
      ->setValue32(222);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:1:QUANTITY")
      ->setValue32(1);
  //
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:2:SHEET")
  ...
  NLGUI::CDBManager::getInstance()
      ...
      ->setValue32(8);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:2:SELLER_TYPE")
      ->setValue32(2);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:2:PRICE_RETIRE")
      ->setValue32(333);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:2:QUANTITY")
      ->setValue32(1);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:2:RESALE_TIME_LEFT")
      ->setValue32(10);
  //
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:3:SHEET")
  ...
  NLGUI::CDBManager::getInstance()
      ...
      ->setValue32(33);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:3:SELLER_TYPE")
      ->setValue32(3);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:3:PRICE_RETIRE")
      ->setValue32(444);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:3:QUANTITY")
      ->setValue32(1);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:3:RESALE_TIME_LEFT")
      ->setValue32(100);
  //
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:4:SHEET")
  ...
  NLGUI::CDBManager::getInstance()
      ...
      ->setValue32(44);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:4:SELLER_TYPE")
      ->setValue32(4);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:4:PRICE_RETIRE")
      ->setValue32(555);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:4:QUANTITY")
      ->setValue32(1);
  //
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:5:SHEET")
  ...
  NLGUI::CDBManager::getInstance()
      ...
      ->setValue32(55);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:5:SELLER_TYPE")
      ->setValue32(5);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:5:PRICE_RETIRE")
      ->setValue32(666);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:5:QUANTITY")
      ->setValue32(23);
  //
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:6:SHEET")
      ->setValue32(CSheetId("m0119dxajd01.sitem").asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:6:PRICE")
      ->setValue32(6 * pf);
  NLGUI::CDBManager::getInstance()
      ...
      ->setValue32(66);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:6:SELLER_TYPE")
      ->setValue32(2);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:6:PRICE_RETIRE")
      ->setValue32(777);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:6:QUANTITY")
      ->setValue32(19);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:6:RESALE_TIME_LEFT")
      ->setValue32(48);
  //
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:7:SHEET")
  ...
  NLGUI::CDBManager::getInstance()
      ...
      ->setValue32(77);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:7:SELLER_TYPE")
      ->setValue32(5);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:7:PRICE_RETIRE")
      ->setValue32(888);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:7:QUANTITY")
      ->setValue32(1);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:7:RESALE_TIME_LEFT")
      ->setValue32(10);
...
NLMISC_COMMAND(testClientPhrases, "", "") {
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:ROLEMASTER_RACE")
      ->setValue32(value);
...
NLMISC_COMMAND(testOutpostBuildings, "Temp : test some items to trade", "") {
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:0:SHEET")
      ->setValue32(
          CSheetId("driller_bountybeaches_kami_u1_100a.outpost_building")
              .asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:0:PRICE")
      ->setValue32(0);
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:1:SHEET")
      ->setValue32(
          CSheetId("driller_citiesofintuition_kami_u2_50a.outpost_building")
              .asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:1:PRICE")
      ->setValue32(0);
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:2:SHEET")
      ->setValue32(
          CSheetId("driller_couloirbrule_kami_u2_150b.outpost_building")
              .asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:2:PRICE")
      ->setValue32(0);
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:3:SHEET")
      ->setValue32(
          CSheetId("driller_bountybeaches_karavan_u1_100a.outpost_building")
              .asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:3:PRICE")
      ->setValue32(0);
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:4:SHEET")
      ->setValue32(
          CSheetId("driller_bountybeaches_karavan_u1_150a.outpost_building")
              .asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:4:PRICE")
      ->setValue32(0);
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:5:SHEET")
      ->setValue32(
          CSheetId("driller_bountybeaches_karavan_u1_200a.outpost_building")
              .asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:5:PRICE")
      ->setValue32(0);
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:6:SHEET")
      ->setValue32(
          CSheetId("driller_bountybeaches_karavan_u2_100a.outpost_building")
              .asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:6:PRICE")
      ->setValue32(0);
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:7:SHEET")
      ->setValue32(
          CSheetId("driller_bountybeaches_karavan_u2_150a.outpost_building")
              .asInt());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:TRADING:7:PRICE")
      ->setValue32(0);
