...
#define FOREACH(__itvar, __conttype, __contvar)                                \
  for (__conttype::iterator __itvar(__contvar.begin()),                        \
       __itvar##end(__contvar.end());                                          \
       __itvar != __itvar##end; ++__itvar)
#endif
...
void CInterfaceManager::DebugTrackGroupsDump() {
  // dump groups
  nldebug("AJM DEBUG: %d Interface Groups remaining",
          _DebugTrackGroupCreateCount - _DebugTrackGroupDestroyCount);
  ...
  void process() {
    if (CWidgetManager::getInstance()->getPointer()->show()) {
      ...
      if ((pCS != NULL) && (pCS->isDragged())) {
        sint x = CWidgetManager::getInstance()->getPointer()->getX() -
                 pCS->getDeltaDragX();
        sint y = CWidgetManager::getInstance()->getPointer()->getY() -
                 pCS->getDeltaDragY();
        pCS->drawSheet(x, y, false, false);

        // if the control support CopyDrag, and if copy key pressed, display a
        // tiny "+"
        if (pCS->canDragCopy() &&
            CInterfaceManager::getInstance()->testDragCopyKey()) {
          ...
          rVR.draw11RotFlipBitmap(
              pCS->getRenderLayer() + 1, x - w / 2, y - h / 2, 0, false,
...
class CStringManagerTextProvider : public CViewTextID::IViewTextProvider {
  bool getString(uint32 stringId, string &result) {
    return STRING_MANAGER::CStringManagerClient::instance()->getString(stringId,
                                                                       result);
  }

  bool getDynString(uint32 dynStringId, string &result) {
    return STRING_MANAGER::CStringManagerClient::instance()->getDynString(
        dynStringId, result);
  }
};
...
public:
  std::string formatString(const std::string &inputString,
                           const std::string &paramString) {
    std::string formatedResult;

    // Apply the format
    for (std::string::const_iterator it = inputString.begin();
         it != inputString.end();) {
      if (*it == '$') {
        ++it;
...
CInterfaceManager::CInterfaceManager() {
  CWidgetManager::getInstance()->registerNewScreenSizeHandler(
      new CDesktopUpdater());
  CWidgetManager::getInstance()->registerOnWidgetsDrawnHandler(
      new CDrawDraggedSheet());

  CInterfaceParser *parser = dynamic_cast<CInterfaceParser *>(
...
void CInterfaceManager::initLUA() {
  ...
  CLuaIHMRyzom::RegisterRyzomFunctions(
...
void CInterfaceManager::initLogin() {
  ...
  for (vector<string>::iterator it = ClientCfg.TexturesLoginInterface.begin(),
                                end = ClientCfg.TexturesLoginInterface.end();
       it != end; ++it) {
  ...
  for (vector<string>::iterator
           it = ClientCfg.TexturesLoginInterfaceDXTC.begin(),
           end = ClientCfg.TexturesLoginInterfaceDXTC.end();
       it != end; ++it) {
...
void CInterfaceManager::initOutGame() {
  ...
  for (vector<string>::iterator it = ClientCfg.TexturesOutGameInterface.begin(),
                                end = ClientCfg.TexturesOutGameInterface.end();
       it != end; ++it) {
  ...
  for (vector<string>::iterator
           it = ClientCfg.TexturesOutGameInterfaceDXTC.begin(),
           end = ClientCfg.TexturesOutGameInterfaceDXTC.end();
       it != end; ++it) {
...
void CInterfaceManager::uninitOutGame() {
  ...
  // nlinfo ("%d seconds for uninitOutGame", (uint32)(ryzomGetLocalTime
  // ()-initStart)/1000);
  ...
  CInterfaceParser *parser = dynamic_cast<CInterfaceParser *>(
      CWidgetManager::getInstance()->getParser());
  // nlinfo ("%d seconds for activateMasterGroup", (uint32)(ryzomGetLocalTime
  // ()-initStart)/1000);
  //	initStart = ryzomGetLocalTime ();
  parser->removeAll();
  // nlinfo ("%d seconds for removeAll", (uint32)(ryzomGetLocalTime
  // ()-initStart)/1000);
  //	initStart = ryzomGetLocalTime ();
  reset();
  // nlinfo ("%d seconds for reset", (uint32)(ryzomGetLocalTime
  // ()-initStart)/1000);
  //  reset the mouse pointer to avoid invalid pointer access
  CWidgetManager::getInstance()->setPointer(NULL);
  //	initStart = ryzomGetLocalTime ();
  CInterfaceLink::removeAllLinks();
  // nlinfo ("%d seconds for removeAllLinks", (uint32)(ryzomGetLocalTime
  // ()-initStart)/1000);
  //	initStart = ryzomGetLocalTime ();
  ICDBNode::CTextId textId("UI");
  NLGUI::CDBManager::getInstance()->getDB()->removeNode(textId);
  // nlinfo ("%d seconds for removeNode", (uint32)(ryzomGetLocalTime
  // ()-initStart)/1000);
  ...
  // Init the action manager
  {
    ...
    //		initStart = ryzomGetLocalTime ();
    uninitActions();
    //	nlinfo ("%d seconds for uninitActions", (uint32)(ryzomGetLocalTime
    //()-initStart)/1000);
  }
...
void badXMLParseMessageBox() {
  ...
  NL3D::UDriver::TMessageBoxId ret = driver->systemMessageBox(
      ...
      "XML reading failed!", NL3D::UDriver::yesNoType,
...
void CInterfaceManager::initInGame() {
  ...
  // SPhrase Manager DB Init (BEFORE loading). Must be init AFTER skill and
  // brick init
  CSPhraseManager *pPM = CSPhraseManager::getInstance();
  pPM->initInGame();
  ...
  // Initialize inventory manager : link to DB and to interface element so must
  // be here
  getInventory().init();
  ...
  for (uint i = 0; i < PeopleInterraction.SystemMessageBuffer.size(); ++i) {
    displaySystemInfo(PeopleInterraction.SystemMessageBuffer[i].Str,
                      PeopleInterraction.SystemMessageBuffer[i].Cat);
  }
  ...
  {
    // Before loadInterfaceConfig(), CViewRenderer needs to know correct scale,
    // because CWidgetManager asks it. Usually set in
    // CInterfaceManager::drawViews(), but then its too late for
    // loadInterfaceConfig()
  ...
  // Whole initInGame profile
  nlinfo("%d seconds for initInGame",
         (uint32)(ryzomGetLocalTime() - initStart) / 1000);
  ...
  // rebuild mp3 player playlist if user reselected a char (songs are already
  // scanned)
  CAHManager::getInstance()->runActionHandler("music_player", NULL,
                                              "update_playlist");
...
void CInterfaceManager::loadIngameInterfaceTextures() {
  ...
  for (vector<string>::iterator it = ClientCfg.TexturesInterface.begin(),
                                end = ClientCfg.TexturesInterface.end();
       it != end; ++it) {
    nldebug("Textures Ingame Interface: %s", (*it).c_str());
    loadTextures(*it + ".tga", *it + ".txt", false);
  }

  for (vector<string>::iterator it = ClientCfg.TexturesInterfaceDXTC.begin(),
                                end = ClientCfg.TexturesInterfaceDXTC.end();
       it != end; ++it) {
...
void CInterfaceManager::configureQuitDialogBox() {
  ...
  CInterfaceGroup *quitDlg = dynamic_cast<CInterfaceGroup *>(
  ...
  if (quitDlg) {
    ...
    CInterfaceElement *eltCancel =
        quitDlg->getElement(quitDialogStr + ":cancel");
    CInterfaceElement *eltEdit =
        quitDlg->getElement(quitDialogStr + ":launch_editor");
    if (eltEdit) {
      if (UserRoleInSession != R2::TUserRole::ur_editor && !sessionOwner) {
        ...
        if (eltCancel)
          (safe_cast<CCtrlTextButton *>(eltCancel))
              ->setText(CI18N::get("uittQuitCancel"));
      } else {
        eltEdit->setY(
            0); // prevent from displaying a gap between two shown buttons
        eltEdit->setActive(false);

        if (eltCancel)
          (safe_cast<CCtrlTextButton *>(eltCancel))
              ->setText(R2::getEditor().getMode() != R2::CEditor::EditionMode
                            ? CI18N::get("uittQuitCancel")
                            : CI18N::get("uittQuitCancelEditor"));
      }
    }

    // Other buttons
    if (IsInRingSession || (ClientCfg.Local && ClientCfg.R2EDEnabled)) {
      // display "return to mainland", unless we are the scenario owner (player
      // or 'aventure master')
      if (eltRet) {
        if (!sessionOwner ||
          ...
          const char *textLabel =
              (UserRoleInSession == R2::TUserRole::ur_editor)
                  ? "uittLeaveEditor"
                  : "uittReturnToMainland";
          (safe_cast<CCtrlTextButton *>(eltRet))
  ...
  CInterfaceGroup *quitDlgMain = dynamic_cast<CInterfaceGroup *>(
  ...
  for (std::vector<CCtrlBase *>::const_iterator ic = controls.begin();
       ic != controls.end(); ++ic) {
...
// ------------------------------------------------------------------------------------------------
//
std::string CInterfaceManager::getSaveFileName(const std::string &module,
                                               const std::string &ext,
                                               bool useShared) const {
...
void CInterfaceManager::uninitInGame1() {
  ...
  CSPhraseManager *pPM = CSPhraseManager::getInstance();
  pPM->reset();
  CSPhraseManager::releaseInstance(); // must release before BrickManager,
                                      // SkillManager
  ...
  // AJM don't release SkillManager, else impulse msg update guild title will
  // crash :(
...
static float roundWeatherValue(float weatherValue) {
  ...
  uint weatherIndex =
      min((uint)(weatherValue * NB_WEATHER_SETUPS), NB_WEATHER_SETUPS - 1);
  uint floorIndex =
      min((uint)(floorValue * NB_WEATHER_SETUPS), NB_WEATHER_SETUPS - 1);
  ...
  if (_InGame) {
    ...
    if ((T0 - _UpdateWeatherTime) > (1 * 3 * 1000)) {
      ...
      string str =
          ...
          toString(", %d",
                   (uint)(roundWeatherValue(WeatherManager.getWeatherValue()) *
                          100.f)) +
      ...
      CCtrlBase *pTooltip = dynamic_cast<CCtrlBase *>(
          CWidgetManager::getInstance()->getElementFromId(
              "ui:interface:map:content:map_content:weather_tt"));
      if (pTooltip != NULL) {
        string tt =
            toString("%02d", WeatherManager.getNextWeatherHour()) +
            CI18N::get("uiMissionTimerHour") + " - " +
            CI18N::get("uiHumidity") + " " +
            toString("%d", (uint)(roundWeatherValue(
                                      WeatherManager.getNextWeatherValue()) *
                                  100.f)) +
            "%";
      ...
      // str += toString("%02d", RT.getRyzomDayOfMonth()+1) + " - "; // Start at
      // 1 for January str += toString("%02d", (sint)RT.getRyzomTime()) + " " +
      // CI18N::get("uiMissionTimerHour");

      // literal version
      // str = CI18N::get("uiDate");
      uint minutes = ((RT.getRyzomTime() - (sint)RT.getRyzomTime()) *
                      (float)RYZOM_HOUR_IN_MINUTES);
      str += toString("%02d:%02d", (sint)RT.getRyzomTime(), minutes) + " - ";
      str += CI18N::get("ui" + WEEKDAY::toString(
                                   (WEEKDAY::EWeekDay)RT.getRyzomDayOfWeek())) +
             ", ";
      str += CI18N::get("ui" +
                        MONTH::toString(
                            (MONTH::EMonth)RT.getRyzomMonthInCurrentCycle())) +
             " ";
      str += toString("%02d", RT.getRyzomDayOfMonth() + 1) + ", ";
      str += CI18N::get("uiAtysianCycle" + toString(RT.getRyzomCycle() + 1) +
                        "Ordinal") +
             " " + CI18N::get("uiAtysianCycle") + " ";
      str += toString("%04d", RT.getRyzomYear());

      pVT = dynamic_cast<CViewText *>(
          CWidgetManager::getInstance()->getElementFromId(
      ...
      pVT = dynamic_cast<CViewText *>(
          CWidgetManager::getInstance()->getElementFromId(
              "ui:interface:compass:clock:time"));
      if (pVT != NULL) {
        if (pVT->getActive()) {
          if (use12hClock())
...
// ------------------------------------------------------------------------------------------------
bool CInterfaceManager::parseInterface(
    const std::vector<std::string> &xmlFileNames, bool reload,
    bool isFilename) {
  ...
  _DB_UI_DUMMY_PREREQUISIT_VALID = NLGUI::CDBManager::getInstance()->getDbProp(
      "UI:DUMMY:PREREQUISIT_VALID", true);
  _DB_UI_DUMMY_FACTION_TYPE = NLGUI::CDBManager::getInstance()->getDbProp(
      "UI:DUMMY:FACTION_TYPE", true);
  ...
  return CWidgetManager::getInstance()->getParser()->parseInterface(
      xmlFileNames, reload, isFilename);
}

// ------------------------------------------------------------------------------------------------
void CInterfaceManager::loadTextures(const string &textFileName,
                                     const string &uvFileName,
                                     bool uploadDXTC) {
  CViewRenderer::getInstance()->loadTextures(textFileName, uvFileName,
                                             uploadDXTC);
}
...
bool CInterfaceManager::loadConfig(const string &filename) {
  ...
  if (ClientCfg.R2EDEnabled) {
    CWidgetManager::getInstance()->runProcedure("proc_reset_r2ed_interface",
                                                NULL, v);
  } else {
  ...
  // By default, consider the reset interface has been set with the current
  // resolution
  {
    uint32 w, h;
    // NB: even if minimzed, getScreenSize() no more return 0 values (return the
    // last setuped screen size)
    CViewRenderer::getInstance()->getScreenSize(w, h);
    // Windows are positioned according to resolution, and we must backup W/H
    // for the system that move windows when the resolution change
  ...
  try {
    ...
    // serial user chats info (serial it before position of windows so that they
    ...
    for (uint32 i = 0; i < nNbMode; ++i) {
      // must create a tmp mem stream because desktop image expect its datas to
      // occupy the whole stream This is because of old system that manipulated
      // desktop image direclty as a mem stream
      CMemStream ms;
      if (!ms.isReading())
        ms.invert();
      uint32 length;
      f.serial(length);
      if (length > 0) {
        // HACK. if the version is <=2, then the CInterfaceConfig has no
        // serialVersion. append here a 0
        if (ver <= 2) {
    ...
    // special for in game: backup last mission because of delayed update
    {
      CCDBNodeLeaf *pNL = NLGUI::CDBManager::getInstance()->getDbProp(
          "UI:SAVE:MISSION_SELECTED", false);
      if (pNL) {
        CCDBNodeLeaf *pSelectedMissionBackup =
            NLGUI::CDBManager::getInstance()->getDbProp(
                "UI:VARIABLES:MISSION_SELECTED_PREV_SESSION", true);
    ...
    CCDBNodeLeaf *pNL = NLGUI::CDBManager::getInstance()->getDbProp(
        "SERVER:INTERFACES:NB_BONUS_LANDMARKS");
    if (pNL) {
  ...
  if (nbLandmarks > 0) {
    ...
    // if icfg is interface_player.icfg, then landmarks must also be
    // loaded/saved to player file
  ...
  // *** If saved resolution is different from the current one setuped, must fix
  // positions in _Modes
  if (lastInGameScreenResLoaded) {
    // Temporarily set screen to saved size so that positions are correctly
    // calculated
    CWidgetManager::getInstance()->setScreenWH(_LastInGameScreenW,
                                               _LastInGameScreenH);
    // NB: we are typically InGame here (even though the _InGame flag is not yet
    // set) Use the screen size of the config file. Don't update current UI,
    // just _Modes
    //
    uint32 width, height;
    ...
    CWidgetManager::getInstance()->moveAllWindowsToNewScreenSize(
  ...
  CNPCIconCache::getInstance().init(
...
bool CInterfaceManager::saveLandmarks(const std::string &filename) const {
  ...
  try {
    ...
    if (f.open(filename, false, false, true)) {
      ...
      xmlNodePtr node =
          xmlNewDocNode(doc, NULL, (const xmlChar *)"interface_config", NULL);
    ...
    nlwarning("Error while writing the file %s : %s.", filename.c_str(),
              e.what());
  }
...
bool CInterfaceManager::saveConfig(const string &filename) {
  ...
  if (nlstricmp(NLMISC::CFile::getFilename(filename).substr(0, 5), "r2ed_") ==
      0) {
  ...
  if (quitVisitor.BadWindowFound) {
    ...
    nlassert(0);
#endif
    // tmp patch : when trying to overwrite the r2ed_ config, if a bad window is
...
void CInterfaceManager::drawViews(NL3D::UCamera camera) {
  ...
  for (uint i = 0; i < CHARACTERISTICS::NUM_CHARACTERISTICS; ++i) {
    if (!_CurrentPlayerCharacLeaf[i])
      _CurrentPlayerCharacLeaf[i] = NLGUI::CDBManager::getInstance()->getDbProp(
          toString("SERVER:CHARACTER_INFO:CHARACTERISTICS%d:VALUE", i), false);
...
bool CInterfaceManager::handleEvent(const NLGUI::CEventDescriptor &event) {
  ...
  if (event.getType() == NLGUI::CEventDescriptor::mouse) {
    NLGUI::CEventDescriptorMouse &eventDesc =
        (NLGUI::CEventDescriptorMouse &)event;
    ...
    if ((eventDesc.getEventTypeExtended() ==
      ...
      // directly later in the main loop (not through message queue), so it has
      // no way of knowing that the event was handled...
...
void CInterfaceManager::updateDesktops(uint32 newScreenW, uint32 newScreenH) {
  ...
  for (uint md = 0; md < _Modes.size(); md++) {
    ...
    for (uint gc = 0; gc < mode.GCImages.size(); gc++) {
      ...
      CWidgetManager::getInstance()->getNewWindowCoordToNewScreenSize(
          gcCont.X, gcCont.Y, gcCont.W, gcCont.H, newScreenW, newScreenH);
    }
  }
...
public:
  InvalidateTextVisitor(bool reset) { this->reset = reset; }

  void visitGroup(CInterfaceGroup *group) {
    const std::vector<CViewBase *> &vs = group->getViews();
    for (std::vector<CViewBase *>::const_iterator itr = vs.begin();
         itr != vs.end(); ++itr) {
...
// ------------------------------------------------------------------------------------------------
void CInterfaceManager::addServerString(const std::string &sTarget, uint32 id,
                                        IStringProcess *cb) {
  if (id == 0) {
...
// ------------------------------------------------------------------------------------------------
void CInterfaceManager::addServerID(const std::string &sTarget, uint32 id,
                                    IStringProcess *cb) {
  if (id == 0) {
    CInterfaceExprValue val;
    val.setString(std::string());
...
// ------------------------------------------------------------------------------------------------
void CInterfaceManager::messageBoxInternal(const string &msgBoxGroup,
                                           const string &text,
                                           const string &masterGroup,
                                           TCaseMode caseMode) {
  CInterfaceGroup *group = dynamic_cast<CInterfaceGroup *>(
      CWidgetManager::getInstance()->getElementFromId(masterGroup + ":" +
                                                      msgBoxGroup));
  CViewText *viewText =
      dynamic_cast<CViewText *>(CWidgetManager::getInstance()->getElementFromId(
          masterGroup + ":" + msgBoxGroup + ":text"));
...
// ------------------------------------------------------------------------------------------------
void CInterfaceManager::messageBox(const string &text,
                                   const string &masterGroup,
                                   TCaseMode caseMode) {
...
// ------------------------------------------------------------------------------------------------
void CInterfaceManager::messageBoxWithHelp(const std::string &text,
                                           const std::string &masterGroup,
                                           const std::string &ahOnOk,
                                           const std::string &paramsOnOk,
                                           TCaseMode caseMode) {
  // replace the procedure "proc_valid_message_box_ok" action
  CWidgetManager::getInstance()->setProcedureAction(
      "proc_message_box_with_help_ok", 1, ahOnOk, paramsOnOk);
  const char *mbName = "message_box_with_help";
  // if no action handler is wanted, then assume that
  // clicking 'ok' do not have any consequence, so allow exiting the message box
  // by clicking outside of it (this behavior is wanted on the login page, to
  // allow to reclick on 'login' without having to click 'ok' in the message box
  // each time)
  CInterfaceGroup *group = dynamic_cast<CInterfaceGroup *>(
      CWidgetManager::getInstance()->getElementFromId(masterGroup + ":" +
                                                      mbName));
...
// ------------------------------------------------------------------------------------------------
void CInterfaceManager::validMessageBox(
    TValidMessageIcon icon, const std::string &text, const std::string &ahOnOk,
    const std::string &paramsOnOk, const std::string &ahOnCancel,
    const std::string &paramsOnCancel, const string &masterGroup) {
  CInterfaceGroup *group = dynamic_cast<CInterfaceGroup *>(
      CWidgetManager::getInstance()->getElementFromId(masterGroup +
                                                      ":valid_message_box"));
  CViewText *viewText =
      dynamic_cast<CViewText *>(CWidgetManager::getInstance()->getElementFromId(
          masterGroup + ":valid_message_box:text"));
  CViewBitmap *viewBitmap = dynamic_cast<CViewBitmap *>(
      CWidgetManager::getInstance()->getElementFromId(
          masterGroup + ":valid_message_box:icon_group:icon"));

  if (group && viewText) {
    // replace the procedure "proc_valid_message_box_ok" action
    CWidgetManager::getInstance()->setProcedureAction(
        "proc_valid_message_box_ok", 1, ahOnOk, paramsOnOk);
    // replace the procedure "proc_valid_message_box_cancel" action
    CWidgetManager::getInstance()->setProcedureAction(
        "proc_valid_message_box_cancel", 1, ahOnCancel, paramsOnCancel);
...
// ------------------------------------------------------------------------------------------------
bool CInterfaceManager::getCurrentValidMessageBoxOnOk(
  ...
  if (mw == group) {
    ...
    string dummyParams;
    if (CWidgetManager::getInstance()->getParser()->getProcedureAction(
            "proc_valid_message_box_ok", 1, ahOnOk, dummyParams))
      return true;
  }
...
// ***************************************************************************
void CInterfaceManager::displayDebugInfo(const string &str,
                                         TSystemInfoMode mode /*=InfoMsg*/) {
  if (PeopleInterraction.DebugInfo)
    PeopleInterraction.ChatInput.DebugInfo.displayMessage(
        str, getDebugInfoColor(mode), 2);
}
...
#define SYSTEM_INFO_COLOR_DB_PATH "UI:VARIABLES:SYSTEM_INFOS:COLORS"
    _NeutralColor = NLGUI::CDBManager::getInstance()->getDbProp(
        SYSTEM_INFO_COLOR_DB_PATH ":NEUTRAL");
    _WarningColor = NLGUI::CDBManager::getInstance()->getDbProp(
        SYSTEM_INFO_COLOR_DB_PATH ":WARNING");
    _ErrorColor = NLGUI::CDBManager::getInstance()->getDbProp(
        SYSTEM_INFO_COLOR_DB_PATH ":ERROR");
  }
  NLMISC::CRGBA color;
  switch (mode) {
  case InfoMsg:
    color.setPacked(_NeutralColor->getValue32());
    break;
  case WarningMsg:
    color.setPacked(_WarningColor->getValue32());
    break;
  case ErrorMsg:
    color.setPacked(_ErrorColor->getValue32());
    break;
  default:
    color = CRGBA::White;
    break;
...
// ***************************************************************************
void CInterfaceManager::displaySystemInfo(const string &str,
                                          const string &cat) {
  CClientConfig::SSysInfoParam::TMode mode =
      CClientConfig::SSysInfoParam::Normal;
  CRGBA color = CRGBA::White;

  map<string, CClientConfig::SSysInfoParam>::const_iterator it =
      ClientCfg.SystemInfoParams.find(toLowerAscii(cat));
  if (it != ClientCfg.SystemInfoParams.end()) {
    mode = it->second.Mode;
    color = it->second.Color;
  }

  if (mode != CClientConfig::SSysInfoParam::OverOnly &&
      mode != CClientConfig::SSysInfoParam::Around) {
  ...
  if (mode == CClientConfig::SSysInfoParam::Center ||
      mode == CClientConfig::SSysInfoParam::CenterAround)
    InSceneBubbleManager.addMessagePopupCenter(str, color);

  // If over popup a string at the bottom of the screen
  if ((mode == CClientConfig::SSysInfoParam::Over) ||
      (mode == CClientConfig::SSysInfoParam::OverOnly))
    InSceneBubbleManager.addMessagePopup(str, color);
  else if ((mode == CClientConfig::SSysInfoParam::Around ||
            mode == CClientConfig::SSysInfoParam::CenterAround) &&
...
// ***************************************************************************
void CInterfaceManager::updateGroupContainerImage(CGroupContainer &gc,
                                                  uint8 mode) {
  if (mode >= _Modes.size()) {
...
// ***************************************************************************
void CInterfaceManager::removeGroupContainerImage(const std::string &groupName,
                                                  uint8 mode) {
  if (mode >= _Modes.size()) {
    nlwarning("wrong desktop");
    return;
...
// ***************************************************************************
void CInterfaceManager::removeGroupContainerImageFromDesktops(
...
void CInterfaceManager::setMode(uint8 newMode) {
  ...
  std::vector<CWidgetManager::SMasterGroup> &_MasterGroups =
      CWidgetManager::getInstance()->getAllMasterGroup();

  // Check if we can change vdesk !
  for (uint32 nMasterGroup = 0; nMasterGroup < _MasterGroups.size();
       nMasterGroup++) {
      ...
      for (uint8 nPriority = 0; nPriority < WIN_PRIORITY_MAX; ++nPriority) {
        ...
        for (itw = rList.begin(); itw != rList.end(); itw++) {
          ...
          if ((pGC != NULL) && (pGC->getActive())) {
            // if this GC is a Full modal window, or if it is a modal son of
            // another GC,
            if (pGC->isModal() || pGC->isModalSon()) {
              CWidgetManager::getInstance()->setTopWindow(pGC);
              pGC->enableBlink(2);
              return;
            } else if (pGC->isGrayed()) {
  ...
  // check if there's a special behaviour with current captured ctrl that
  // prevent from changing desktop
  if (CWidgetManager::getInstance()->getCapturePointerLeft() != NULL) {
    if (!CWidgetManager::getInstance()
             ->getCapturePointerLeft()
             ->canChangeVirtualDesktop())
      return;
  }
  if (CWidgetManager::getInstance()->getCapturePointerRight() != NULL) {
    if (!CWidgetManager::getInstance()
  ...
  while (!left.empty()) {
    ...
    if (ig) {
      ...
      for (uint k = 0; k < ig->getViews().size(); ++k) {
        std::string info(currDepth * 4, ' ');
        info +=
            toString("View %d / %d : ", (int)k + 1, (int)ig->getViews().size());
        if (ig->getViews()[k]) {
          info += id;
          NLGUI::CViewBase *view = ig->getViews()[k];
          info +=
              toString(", type = %s, address=0x%p", typeid(*view).name(), view);
        } else {
          info += "<NULL>";
        }
        nlinfo(info.c_str());
      ...
      for (uint k = 0; k < ig->getControls().size(); ++k) {
        std::string info(currDepth * 4, ' ');
        info += toString("Ctrl %d / %d : ", (int)k + 1,
                         (int)ig->getControls().size());
        if (ig->getControls()[k]) {
          info += id;
          NLGUI::CCtrlBase *control = ig->getControls()[k];
          info += toString(", type = %s, address=0x%p", typeid(*control).name(),
                           control);
        } else {
          info += "<NULL>";
        }
  ...
  for (uint32 nMasterGroup = 0; nMasterGroup < _MasterGroups.size();
    ...
    for (uint8 nPriority = 0; nPriority < WIN_PRIORITY_MAX; ++nPriority) {
      ...
      for (it = rList.begin(); it != rList.end(); ++it) {
        if (*it)
          (*it)->renderWiredQuads(CInterfaceElement::RenderView, uiFilter);
      }
    }
  }
}
...
void CInterfaceManager::displayUICtrlBBoxs(const std::string &uiFilter) {
  ...
  for (uint32 nMasterGroup = 0; nMasterGroup < _MasterGroups.size();
    ...
    for (uint8 nPriority = 0; nPriority < WIN_PRIORITY_MAX; ++nPriority) {
      ...
      for (it = rList.begin(); it != rList.end(); ++it) {
        if (*it)
          (*it)->renderWiredQuads(CInterfaceElement::RenderCtrl, uiFilter);
      }
    }
  }
}
...
void CInterfaceManager::displayUIGroupBBoxs(const std::string &uiFilter) {
  ...
  for (uint32 nMasterGroup = 0; nMasterGroup < _MasterGroups.size();
    ...
    for (uint8 nPriority = 0; nPriority < WIN_PRIORITY_MAX; ++nPriority) {
      ...
      for (it = rList.begin(); it != rList.end(); ++it) {
        if (*it)
          (*it)->renderWiredQuads(CInterfaceElement::RenderGroup, uiFilter);
      }
    }
  }
}
...
// ***************************************************************************
void writeComboActionMap(const CActionsManager &actions, xmlNodePtr node,
  ...
  while (ite != combos.end()) {
    ...
    // Props
    xmlSetProp(
        keyNode, (const xmlChar *)"name",
        (const xmlChar *)CEventKey::getStringFromKey(ite->first.Key).c_str());
    ...
    xmlSetProp(keyNode, (const xmlChar *)"action",
               (const xmlChar *)ite->second.Name.c_str());
    if (!(const xmlChar *)ite->second.Argu.empty())
      xmlSetProp(keyNode, (const xmlChar *)"params",
                 (const xmlChar *)ite->second.Argu.c_str());

    // Context
    if (!context.empty())
      xmlSetProp(keyNode, (const xmlChar *)"context",
                 (const xmlChar *)context.c_str());
...
void writeMacros(xmlNodePtr node) {
  const std::vector<CMacroCmd> &macros =
  ...
  if (_LogState) {
    ...
    if (f != NULL) {
      const string finalString =
          string(NLMISC::IDisplayer::dateToHumanString()) + " (" +
          NLMISC::toUpperAscii(cat) + ") * " + str;
      fprintf(f, "%s\n", finalString.c_str());
      fclose(f);
    }
  }
...
void CInterfaceManager::restoreAllContainersBackupPosition() {
  ...
  for (uint32 nMasterGroup = 0; nMasterGroup < _MasterGroups.size();
    ...
    for (uint8 nPriority = 0; nPriority < WIN_PRIORITY_MAX; ++nPriority) {
      ...
      for (it = rList.begin(); it != rList.end(); ++it) {
        if (*it)
          (*it)->restoreAllContainersBackupPosition();
      }
    }
  }
}
...
void CInterfaceManager::visit(CInterfaceElementVisitor *visitor) {
  ...
  std::vector<CWidgetManager::SMasterGroup> &_MasterGroups =
      CWidgetManager::getInstance()->getAllMasterGroup();
  for (uint nMasterGroup = 0; nMasterGroup < _MasterGroups.size();
       nMasterGroup++) {
...
NLMISC_COMMAND(loadui, "Load an interface file",
               "<loadui [all]/interface.xml>") {
  if (args.size() != 1)
    return false;
  ...
  if (result)
    CInterfaceManager::getInstance()->displaySystemInfo(
        "File " + xmlFileNames.back() + " loaded successfully.");
  else
    CInterfaceManager::getInstance()->displaySystemInfo(
        "File " + xmlFileNames.back() + " NOT loaded successully.");
#endif
...
// ***************************************************************************
void CInterfaceManager::displayWebWindow(const string &name,
                                         const string &url) {
  ...
  CAHManager::getInstance()->runActionHandler(
      "browse", NULL, "name=" + name + ":content:html|url=" + url);
}
...
class CHandlerDispWebOnQuit : public IActionHandler
{
        ...
        {
                if (ClientCfg.Local)
                        CAHManager::getInstance()->runActionHandler("enter_modal",
pCaller, "group=ui:interface:quit_dialog"); else
                        CInterfaceManager::getInstance()->displayWebWindow("ui:interface:web_on_quit",
"http://213.208.119.190/igpoll/poll_form.php");
        }
};
...
class CHandlerExitWebOnQuit : public IActionHandler
{
        virtual void execute (CCtrlBase *pCaller, const string &Params)
        {
                CAHManager::getInstance()->runActionHandler("quit_ryzom",
pCaller);
        }
};
...
static bool translateEmote(const std::string &id, std::string &translatedName,
  ...
  if (CI18N::hasTranslation(id)) {
    ...
    if (commandNameAlt == commandName)
      commandNameAlt.clear();
...
void CInterfaceManager::initEmotes() {
  ...
  CTextEmotListSheet *pTELS = dynamic_cast<CTextEmotListSheet *>(
      SheetMngr.get(CSheetId("list.text_emotes")));
  if (pTELS == NULL)
    return;
  ...
  CEmotListSheet *pEmotList =
      dynamic_cast<CEmotListSheet *>(SheetMngr.get(CSheetId("list.emot")));
  ...
  for (list<CEmoteEntry>::const_iterator it = entries.begin();
       it != entries.end(); it++) {
    ...
    for (i = 0; i < pEmotList->Emots.size(); ++i)
      if (CAnimationStateSheet::getAnimationStateName(pEmotList->Emots[i]) ==
          sState) {
        nBehav = (uint8)i;
        break;
      }
    ...
    CGroupMenu *pRootMenu = dynamic_cast<CGroupMenu *>(
        CWidgetManager::getInstance()->getElementFromId(
            "ui:interface:user_chat_emote_menu"));
    nlassert(pRootMenu);
    ...
    for (i = 0; i < nbToken; ++i) {
      ...
      if (!bFound) // Create it
      {
        if (i != (nbToken - 1)) {
          ...
          // Create a line
          pMenu->addLine(
              sTranslatedName + " (/" + sCommandName + ")", "emote",
    ...
    if (!sTranslatedName.empty()) {
      if (ICommand::exists(sCommandName)) {
        nlwarning("Translation for emote %s already exist: '%s' exist twice",
                  sName.c_str(), sCommandName.c_str());
      } else {
        ...
        if (!sCommandNameAlt.empty()) {
          if (ICommand::exists(sCommandNameAlt)) {
            nlwarning(
                "Translation for emote %s already exist: '%s' exist twice",
                sName.c_str(), sCommandName.c_str());
          } else {
        ...
        for (i = 0; i < pMenu->getNumLine(); i++) {
          if (sName == pMenu->getLineId(i)) {
            ...
            pMenu->addLineAtIndex(i, "@{FFFF}/" + sCommandName, "emote",
                                  "nb=" + toString(nEmoteNb) +
...
bool CInterfaceManager::CEmoteCmd::execute(
    const std::string & /* rawCommandString */, const vector<string> &args,
    CLog & /* log */, bool /* quiet */, bool /* human */) {
  string customPhrase;
  if (!args.empty()) {
    customPhrase = args[0];
  }
  ...
  CAHManager::getInstance()->runActionHandler(
      "emote", NULL,
      "nb=" + toString(EmoteNb) + "|behav=" + toString(Behaviour) +
...
std::vector<std::string> CInterfaceManager::getInGameXMLInterfaceFiles() {
  ...
  // But still preserve order given in XMLInterfaceFiles (important for
  // config.xml for instance)
  set<string> fileSet;
  for (uint i = 0; i < ret.size(); i++) {
    fileSet.insert(ret[i]);
  }

  // Add R2 Editor .xml. This is removed as it will not be done when
  // initializing CEditor
  //	if (ClientCfg.R2EDEnabled)
  //	{
...
void CInterfaceManager::getLuaValueInfo(std::string &str, sint index) {
  ...
  if (type == LUA_TNIL) {
    ...
    str = NLMISC::toString(ls.isInteger(index) ? ls.toInteger(index)
                                               : ls.toNumber(index));
...
// ***************************************************************************
void CInterfaceManager::dumpLuaKeyValueInfo(uint recursTableLevel,
                                            uint tabLevel) {
  CLuaState &ls = *(CLuaManager::getInstance().getLuaState());
...
void CInterfaceManager::dumpLuaState(uint detail) {
  ...
  dumpLuaString(
      NLMISC::toString("Memory Used : %d Kb", _LuaState->getGCCount()));
  dumpLuaString(
      NLMISC::toString("GC Threshold: %d Kb", _LuaState->getGCThreshold()));
  ...
  if (detail > 0) {
    ...
    dumpLuaString(
        NLMISC::toString("Number of EnvTable for ui groups: %d", count));
  }
}

// ------------------------------------------------------------------------------------------------
void CInterfaceManager::createLocalBranch(
    const std::string &fileName, NLMISC::IProgressCallback &progressCallBack) {
  try {
    CIFile file;
    if (file.open(fileName)) {
      ...
      NLGUI::CDBManager::getInstance()->getDB()->attachChild(localNode,
                                                             "LOCAL");
...
#endif
CInterfaceManager::CServerToLocalAutoCopy::CServerToLocalAutoCopy()
    : _LocalObserver(*this), _ServerObserver(*this) {
  _ServerCounter = NULL;
...
// ------------------------------------------------------------------------------------------------
void CInterfaceManager::CServerToLocalAutoCopy::buildRecursLocalLeaves(
    CCDBNodeBranch *branch, std::vector<CCDBNodeLeaf *> &leaves) {
...
// ------------------------------------------------------------------------------------------------
void CInterfaceManager::CServerToLocalAutoCopy::init(
    const std::string &dbPath) {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();

  // Get the synchronisation Counter in Server DB
  _ServerCounter = NLGUI::CDBManager::getInstance()->getDbProp(
      string("SERVER:") + dbPath + ":COUNTER", false);
  ...
  if (_ServerCounter) {
    ...
    NLGUI::CDBManager::getInstance()->getDB()->addObserver(&_ServerObserver,
                                                           textId);

    // add the observers when local node change
    textId = ICDBNode::CTextId(string("LOCAL:") + dbPath);
    NLGUI::CDBManager::getInstance()->getDB()->addObserver(&_LocalObserver,
                                                           textId);

    // **** Init the Nodes shortcut
    // Parse all Local Nodes
    CCDBNodeBranch *localBranch = NLGUI::CDBManager::getInstance()->getDbBranch(
        string("LOCAL:") + dbPath);
    if (localBranch) {
      uint i;
      ...
      for (i = 0; i < leaves.size(); i++) {
        ...
        CCDBNodeLeaf *serverLeaf =
            NLGUI::CDBManager::getInstance()->getDbProp(serverLeafStr, false);
        if (serverLeaf) {
...
// ------------------------------------------------------------------------------------------------
void CInterfaceManager::CServerToLocalAutoCopy::onServerChange(
    ICDBNode *serverNode) {
  if (_Nodes.empty())
    return;
  ...
  if (ClientCfg.Local ||
      pIM->localActionCounterSynchronizedWith(_ServerCounter)) {
    // update all leaves
...
// ------------------------------------------------------------------------------------------------
void CInterfaceManager::CServerToLocalAutoCopy::onLocalChange(
    ICDBNode *localNode) {
  if (_Nodes.empty())
    return;

  // if the local changes because of localLeaf->setValue64() in
  // onServerChange(), no-op !!!
  if (_LocalUpdating)
    return;
...
// ------------------------------------------------------------------------------------------------
char *
CInterfaceManager::getTimestampHuman(const char *format /* "[%H:%M:%S] " */) {
  ...
  while ((start_pos < str.length() - 1) &&
    ...
    if (token_param.substr(0, 3) != "gs(") {
      if (param_vector.size() == 2) {
        ...
        while ((token_replacement_pos =
                    token_replacement.find(string("\\d"))) != string::npos) {
    ...
    if (pTokenSubjectEntity != NULL) {
      ...
      if (token_param == "name") {
        ...
        if (pC) {
          ...
          if (race >= EGSPD::CPeople::Playable &&
              race <= EGSPD::CPeople::EndPlayable) {
            string srace =
        ...
        STRING_MANAGER::CStringManagerClient *pSMC =
            STRING_MANAGER::CStringManagerClient::instance();
        string ucGuildName;
        if (pSMC->getString(pTokenSubjectEntity->getGuildNameID(),
                            ucGuildName)) {
          token_replacement =
              ucGuildName.empty() ? token_replacement : ucGuildName;
        }
        ...
        if (pTokenSubjectEntity->isUser() || pTokenSubjectEntity->isPlayer() ||
            pTokenSubjectEntity->isNPC()) {
          CCharacterCL *pC = dynamic_cast<CCharacterCL *>(pTokenSubjectEntity);
          if (pC) {
            gender = pC->getGender();
          }
        ...
        GSGENDER::EGender g =
            ((uint)gender >= strList.size()) ? GSGENDER::male : gender;
    ...
    extra_replacement =
        (token_replacement == token_default) ? extra_replacement : 0;
    if (str.find(token_whole, start_pos) != string::npos) {
      str =
          str.replace(token_whole_pos, token_whole.length() + extra_replacement,
                      token_replacement);
...
std::string CInterfaceManager::getNextBackupName(std::string filename) {
  ...
  if (!ts.empty()) {
    ...
    if (pos == std::string::npos)
      ...
      filename =
          filename.substr(0, pos) + "_" + ts + "_" + filename.substr(pos);
  }
...
void CInterfaceManager::createFileBackup(const std::string &message,
