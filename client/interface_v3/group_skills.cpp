...
NLMISC_REGISTER_OBJECT(CViewBase, CGroupSkills, std::string,
                       "skills_displayer");
...
bool CGroupSkills::parse(xmlNodePtr cur, CInterfaceGroup *parentGroup) {
  ...
  // Add observer on the skill parameter because if the skill is coming to be
  // not-zero, a new skill must be displayed
...
// ***************************************************************************
void CGroupSkills::clearGroups() { CInterfaceGroup::clearGroups(); }
...
void CGroupSkills::rebuild() {
  ...
  // **** first time bind?
  if (!_Tree) {
    _Tree = dynamic_cast<CGroupTree *>(
        CWidgetManager::getInstance()->getElementFromId(getId(),
                                                        WIN_TREE_LIST));
    if (_Tree == NULL) {
  ...
  for (uint32 i = 0; i < SKILLS::NUM_SKILLS; ++i) {
    ...
    if (!pSM->isUnknown((SKILLS::ESkills)i)) {
      // Show level 0 and level 1 skills + skills that have a PARENT with value
      // > 0
...
void CGroupSkills::CSkillsObs::update(ICDBNode *node) {
  ...
  if ((leaf->getOldValue32() != 0) &&
      (leaf->getValue32() !=
       0)) // prevent displaying FX when resetData() is called during a Far TP
  {
      ...
      if (skillParent) {
        ...
        if (skillParent->getNodeIndex(skill, skillId)) {
          CAHManager::getInstance()->runActionHandler(
              "skill_popup", NULL,
              "skillId=" + toString(skillId) + "|delta=" +
                  toString(leaf->getValue32() - leaf->getOldValue32()));
    ...
    if (!CGroupSkills::InhibitSkillUpFX) // TODO: couldn't this be replaced by
                                         // IngameDbMngr.initInProgress()?
    {
      UserEntity->skillUp();
    }
  }
...
public:
  ...
  bool operator<(const CSortNode &o) const { return Value < o.Value; }
};
// constructor
CGroupSkills::CGroupSkills(const TCtorParam &param) : CInterfaceGroup(param) {
  _MustRebuild = false;
  _Tree = NULL;
  _TreeRoot = NULL;
}
...
CGroupSkills::~CGroupSkills() {
  ...
  for (uint k = 0; k < SKILLS::NUM_SKILLS; ++k) {
    sTmp = string(DB_SKILLS) + ":" + NLMISC::toString((sint32)k) + ":BaseSKILL";
    textId = ICDBNode::CTextId(sTmp);
    NLGUI::CDBManager::getInstance()->getDB()->removeObserver(&_SkillsObs,
                                                              textId);
  }
  ...
  // template nodes not linked to hierarchy will memory leak, we must remove
  // them also
  for (sint i = 0; i < SKILLS::NUM_SKILLS; i++) {
    // is the refptr still allocated?
    if (_AllNodes[i]) {
      // NB: delete call makeOrphan, and delete children that may still be in
      // the array but it's OK, because tested with refptr
      delete _AllNodes[i];
      _AllNodes[i] = NULL;
    }
  }
    ...
    for (uint32 i = 0; i < SKILLS::NUM_SKILLS; ++i)
      if (_AllNodes[i] == NULL) // not already created
      {
        ...
        if (_TemplateSkill.empty()) {
          ...
          // create the template
          tempVec[0].first = "id";
          tempVec[0].second = pNode->Id;
          tempVec[1].first = "skillid";
          tempVec[1].second = NLMISC::toString(i);
          CInterfaceGroup *pIG =
              CWidgetManager::getInstance()->getParser()->createGroupInstance(
                  _TemplateSkill, getId() + ":" + WIN_TREE_LIST, tempVec);
          if (pIG == NULL)
            nlwarning("error");
          // Set Skill Name
          CViewText *pViewSkillName =
              dynamic_cast<CViewText *>(pIG->getView("name"));
          if (pViewSkillName != NULL)
            pViewSkillName->setText(sSkillName);
          // Set Skill Max Value
          CViewText *pViewSkillMax =
              dynamic_cast<CViewText *>(pIG->getView("max"));
          if (pViewSkillMax != NULL)
            pViewSkillMax->setText(
                toString(pSM->getMaxSkillValue((SKILLS::ESkills)i)));
          pNode->Template = pIG;
        }
