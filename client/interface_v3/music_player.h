...
public:
  ...
  class CSongs {
  public:
    CSongs(std::string file = std::string(), std::string title = std::string(),
           float length = 0.f)
  ...
  void play(sint index = -1); // Play the song at current position, if playing,
                              // restart. If paused, resume.
  void pause();
  void stop();
  void previous();
  void next();
  ...
  void updateSong(const std::string filename, const std::string title,
                  float length);
