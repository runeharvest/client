...
NLMISC_REGISTER_OBJECT(CViewBase, CDBGroupListSheetTextPhrase, std::string,
                       "list_sheet_text_phrase");

CDBGroupListSheetTextPhrase::CDBGroupListSheetTextPhrase(
...
// ***************************************************************************
void CDBGroupListSheetTextPhrase::CSheetChildPhrase::init(
  ...
  if (root) {
    LevelDB = dynamic_cast<CCDBNodeLeaf *>(
        root->getNode(ICDBNode::CTextId("LEVEL"), false));
  }
}

// ***************************************************************************
void CDBGroupListSheetTextPhrase::CSheetChildPhrase::updateViewText(
...
// ***************************************************************************
bool CDBGroupListSheetTextPhrase::CSheetChildPhrase::isInvalidated(
    CDBGroupListSheetText * /* pFather */) {
  if (Ctrl->getType() != CCtrlSheetInfo::SheetType_SPhrase)
    return false;
...
// ***************************************************************************
void CDBGroupListSheetTextPhrase::CSheetChildPhrase::update(
    CDBGroupListSheetText * /* pFather */) {
  if (Ctrl->getType() != CCtrlSheetInfo::SheetType_SPhrase)
    return;
...
// ***************************************************************************
CInterfaceGroup *
CDBGroupListSheetTextPhrase::createSectionGroup(const std::string &igName) {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  vector<pair<string, string>> tempVec(1);
  tempVec[0].first = "id";
  tempVec[0].second = igName;

  // make parent to the list, not me
  CInterfaceGroup *pIG =
      CWidgetManager::getInstance()->getParser()->createGroupInstance(
          LSP_SECTION_TEMPLATE, _List->getId(), tempVec);
  pIG->setParent(_List);
  _List->addGroup(pIG);
...
// ***************************************************************************
void CDBGroupListSheetTextPhrase::setSectionGroupId(CInterfaceGroup *pIG,
                                                    uint sectionId) {
  nlassert(pIG);
...
// ***************************************************************************
void CDBGroupListSheetTextPhrase::getCurrentBoundSectionId(sint &minSectionId,
                                                           sint &maxSectionId) {
  // get the actual bound from book filter. NB: this work because the book
  // filter is updated at same time the database is modified, hence updateCoords
  // is called, then this skill filter is already up to date.
  CSPhraseManager *pPM = CSPhraseManager::getInstance();
