...
public:
  ...
  virtual ~CDBGroupListSheetText(); // AJM: make base class destructors virtual
  ...
  // @{
  // Get the index of a sheet inserted in this list. Returns -1 if it is not an
  // index of that list
  ...
  // Helper : if this sheet db list is updated from the server, then it can know
  // when the download is finished If, because of a filter, the list is empty, a
  // replacement message can be shown to inform the player
  ...
  struct CSheetChild {
    ...
    sint32 SheetId;
    // For Item only, cache for NameId, updated in updateCoords, and checked in
    // checkCoords()
    sint32 NameId;
    ...
    // if _CheckCoordAccelerated is true (default), called at each DB modif,
    // else called each frame NB: if CDBGroupListSheetText::needCheckAll()
    // return true, all are tested too
    ...
    // create a CViewText object that is displayed next to the item. This is the
    // opportunnity to create a derived class such as CViewTextID
    virtual CViewText *createViewText() const {
      return new CViewText(CViewBase::TCtorParam());
    }
    ...
    // return the Child X delta (does not affect button, just CtrlSheet and
    // Text)
    virtual sint getDeltaX(CDBGroupListSheetText * /* pFather */) const {
      return 0;
    }

    // for listsheet sectionable, return the sectionId associated with this
    // sheet
  ...
  protected:
    // tool method to setup an common item text. used by ListSheetBag and
    // ListSheetTextShare for instance
  ...
  // true if this list is able to swap(). if yes, then the list support "scroll
  // when drag"
  virtual bool swapable() const { return false; }
  // Called when we drag'n'drop a control sheet of this list on another of this
  // list. Default is no op
  virtual void onSwap(sint /* nDraggedSheet */, sint /* nDroppedSheet */) {}

  // Called when the list changed for a reason or another and should be
  // reconstructed to possibly sort items
  virtual void sort() {}
  void needToSort() {
    _NeedToSort = true;
    invalidateCoords();
...
protected:
  ...
  // section
  void insertSectionGroupList(sint firstSectionId, sint lastSectionId,
                              uint &groupSectionIndex, sint &yItem);
};
