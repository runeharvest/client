...
static const char *WIN_BOT_CHAT_PAGE_MISSION_END =
    "ui:interface:bot_chat_mission_end";
...
void CBotChatPageMissionEnd::begin() {
  ...
  *>(CWidgetManager::getInstance()->getElementFromId(WIN_BOT_CHAT_PAGE_MISSION_END));
  if (!ig) return;
  ...
  if (rewardText)
  {
          if ((flags & 1) || ClientCfg.Local) // is reward text needed
          {
                  ...
                  NLGUI::CDBManager::getInstance()->getDbProp(BOT_CHAT_BASE_DB_PATH
  ":MISSION_END_REWARD")->setValue32(0);
          }
          else
          {
