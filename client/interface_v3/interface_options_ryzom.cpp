...
NLMISC_REGISTER_OBJECT(CInterfaceOptions, CMissionIconList, std::string,
                       "mission_icons");
...
bool CMissionIconList::parse(xmlNodePtr cur) {
  ...
  for (std::map<std::string, CInterfaceOptionValue>::iterator it =
           _ParamValue.begin();
       it != _ParamValue.end(); ++it) {
    int index;
    if (fromString(it->first, index)) {
      if (index > 255) {
        ...
        if (texID != -1) {
          IconBackTexID.resize(std::max((int)IconBackTexID.size(), index + 1),
                               -1);
...
// ***************************************************************************
NLMISC_REGISTER_OBJECT(CInterfaceOptions, COptionsAnimationSet, std::string,
                       "animation_set");
...
COptionsAnimationSet::~COptionsAnimationSet() {
  if (AnimationSet) {
    /* Important Note: this CInterfaceOptions is released BEFORE any
       CCharacter3d is released himself BUT this is OK, since the actual
       animationSet is kept by SmartPtr through UPlayList (see
       deleteAnimationSet() doc)
     */
    ...
    do {
      ...
      if (!sTmp.empty()) {
        ...
        if (params.empty() || params[0].empty()) {
          ...
          animName += ".anim";
          uint animID =
              AnimationSet->addAnimation(animName.c_str(), animName.c_str());
          if (animID == UAnimationSet::NotFound)
            nlwarning("Character3D : not found anim : %s", animName.c_str());
          // try to add the Face animation for this one (not important if
          // failed)
          string faceAnimName = getFaceAnimName(animName);
          AnimationSet->addAnimation(faceAnimName.c_str(),
                                     faceAnimName.c_str());
