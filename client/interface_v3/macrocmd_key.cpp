...
/*
#define WIN_EDITKEY
"ui:interface:editkey" #define VIEW_EDITKEY_TEXT_CATEGORY
"ui:interface:editkey:content:category" #define VIEW_EDITKEY_TEXT_ACTION
"ui:interface:editkey:content:action" #define VIEW_EDITKEY_TEXT_FIRST_PARAM
"ui:interface:editkey:content:param1" #define VIEW_EDITKEY_TEXT_SECOND_PARAM
"ui:interface:editkey:content:param2" #define VIEW_EDITKEY_TEXT_FIRST_PARAM_NAME
"ui:interface:editkey:content:param1_name" #define
VIEW_EDITKEY_TEXT_SECOND_PARAM_NAME
"ui:interface:editkey:content:param2_name" #define VIEW_EDITKEY_TEXT_KEY
"ui:interface:editkey:content:txtkey"
*/
// To define a key
...
void addKeyLine(CGroupList *pParent, const string &keyName,
                const string &shortcutName, bool grayed) {
  ...
  pKeysLine = CWidgetManager::getInstance()->getParser()->createGroupInstance(
      TEMPLATE_KEYS_GROUP, pParent->getId(), vParams);
  if (pKeysLine == NULL)
    return;

  // Put name
  CViewText *pViewKeyName =
      dynamic_cast<CViewText *>(pKeysLine->getView(TEMPLATE_KEYS_KEY_NAME));
  if (pViewKeyName != NULL) {
    pViewKeyName->setText(keyName);
    pViewKeyName->setColor(
        grayed ? CWidgetManager::getInstance()
                     ->getSystemOption(CWidgetManager::OptionCtrlTextGrayColor)
                     .getValColor()
               : CRGBA::White);
  }

  CViewText *pViewShortcutName = dynamic_cast<CViewText *>(
      pKeysLine->getView(TEMPLATE_KEYS_SHORTCUT_NAME));
  if (pViewShortcutName != NULL) {
    pViewShortcutName->setText(shortcutName);
    pViewShortcutName->setColor(
        grayed ? CWidgetManager::getInstance()
                     ->getSystemOption(CWidgetManager::OptionCtrlTextGrayColor)
                     .getValColor()
               : CRGBA::White);
...
void buildActionToComboMap(uint8 nAM, CGroupList * /* pList */, string catName,
                           map<string, CComboActionName> &remaped) {
  CMacroCmdManager *pMCM = CMacroCmdManager::getInstance();
  CActionsManager *pAM = pMCM->ActionManagers[nAM];
  const CActionsManager::TComboActionMap &keyShortcut =
      pAM->getComboActionMap();
    ...
    if (pCat != NULL)
      if (pCat->Name == catName) {
        ...
        if (pAM->isActionPresentInContext(it->second)) {
          ...
          // Don't take any risk: avoid any bug if the localisation is buggy and
          // give same text for 2 differents CAction::CName Use the localized
          // text first, to have correct sort according to language
          value.first =
              pAM->getActionLocalizedText(rName) + rName.Name + rName.Argu;
  ...
  const CActionsManager::TActionsForceDisplaySet &actionFD =
      pAM->getActionsForceDisplaySet();
  CActionsManager::TActionsForceDisplaySet::const_iterator itFD =
      actionFD.begin();
...
// Get all the couple (combo,action) from the action manager nAM and insert them
// into pList (with the template)
void getAllComboAction(uint8 nAM, CGroupList *pList,
                       const map<string, CComboActionName> &remaped) {
  CMacroCmdManager *pMCM = CMacroCmdManager::getInstance();
  CActionsManager *pAM = pMCM->ActionManagers[nAM];
  // *** Fill Actions
  ...
  while (remapIT != remaped.end()) {
    ...
    const CBaseAction *baseAction =
        pAM->getBaseAction(remapIT->second.ActionName);
    if (baseAction) {
      string shortcutName =
          baseAction->getActionLocalizedText(remapIT->second.ActionName);

      addKeyLine(pList, keyName, shortcutName,
                 remapIT->second.Combo.Key == KeyCount);
  ...
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    ...
    for (uint nAM = 0; nAM < pMCM->ActionManagers.size(); ++nAM) {
      ...
      for (uint i = 0; i < rCats.size(); ++i) {
        ...
        pGC = dynamic_cast<CGroupContainer *>(
    ...
    for (uint nAM = 0; nAM < pMCM->ActionManagers.size(); ++nAM) {
      ...
      for (uint i = 0; i < rCats.size(); ++i) {
        string contName = string(WIN_KEYS_GAME) + rCats[i].Name;
        CInterfaceGroup *pCategory = dynamic_cast<CInterfaceGroup *>(
            CWidgetManager::getInstance()->getElementFromId(contName));
        CGroupList *pList = dynamic_cast<CGroupList *>(
            CWidgetManager::getInstance()->getElementFromId(contName +
                                                            ":content"));
...
public:
  virtual void execute(CCtrlBase *pCaller, const string & /* Params */) {
    ...
    if ((nLineNb < 0) || (nLineNb >= (sint32)pMCM->NewKey->AllLines.size()))
      return;

    pMCM->NewKey->CurrentEditCmdLine = pMCM->NewKey->AllLines[nLineNb];
    pMCM->NewKey->CurAM =
        pMCM->ActionManagers[pMCM->NewKey->AllLines[nLineNb].ComboAM];
    CGroupModalGetKey *pGetKey = dynamic_cast<CGroupModalGetKey *>(
...
// ***************************************************************************
// Called when we right click on a shortcut line and choose edit from context
// menu
...
public:
  virtual void execute(CCtrlBase *pCaller, const string & /* Params */) {
    ...
    CActionsManager::TComboActionMap::const_iterator it =
        pMCM->NewKey->CurAM->getComboActionMap().find(
...
// ***************************************************************************
void CModalContainerEditCmd::create(const std::string &name, bool bDefKey,
  ...
  Win = dynamic_cast<CGroupContainer *>(
      CWidgetManager::getInstance()->getParser()->createGroupInstance(
          TEMPLATE_EDITCMD, "ui:interface", vArgs));
  if (Win == NULL) {
...
void CModalContainerEditCmd::activate() {
  ...
  CDBGroupComboBox *pCB = dynamic_cast<CDBGroupComboBox *>(
      CWidgetManager::getInstance()->getElementFromId(
          WinName + WIN_EDITCMD_COMBO_CATEGORY));
  if (pCB != NULL) {
    CurrentEditCmdCategories.clear();
    pCB->resetTexts();
    for (uint am = 0; am < pMCM->ActionManagers.size(); am++) {
      const vector<CCategory> &rVCat =
          pMCM->ActionManagers[am]->getCategories();
      for (uint i = 0; i < rVCat.size(); i++) {
        ...
        if (_AllowAllActions || cat.Macroisable) {
          // category should have at least one action that is possible in
          // currenyt context
          bool found = false;
  ...
  pCB = dynamic_cast<CDBGroupComboBox *>(
      CWidgetManager::getInstance()->getElementFromId(
          WinName + WIN_EDITCMD_COMBO_ACTION));
  if (pCB != NULL)
    pCB->resetTexts();
  // Clean up
  CurAM = NULL;
  ...
  CViewText *pViewParamName =
      dynamic_cast<CViewText *>(CWidgetManager::getInstance()->getElementFromId(
          WinName + VIEW_EDITCMD_FIRST_PARAM_NAME));
  if (pViewParamName != NULL)
    pViewParamName->setText(string());
  pViewParamName =
      dynamic_cast<CViewText *>(CWidgetManager::getInstance()->getElementFromId(
          WinName + VIEW_EDITCMD_SECOND_PARAM_NAME));
  if (pViewParamName != NULL)
    pViewParamName->setText(string());
...
// ***************************************************************************
void CModalContainerEditCmd::activateFrom(const std::string &cmdName,
                                          const std::string &cmdParams,
                                          sint nRef) {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  CMacroCmdManager *pMCM = CMacroCmdManager::getInstance();
  ...
  for (i = 0; i < pBA->Parameters.size(); ++i) {
    ...
    // Get ith param (params are nameOfParam=argumentOfAction strings separated
    // by |) except for the last real param (which can then contains | chars) if
    // it is the last param

    string::size_type pos = curStr.find('|');

    if ((pos == string::npos) ||
        (((noParam == nbRealParam - 1) &&
          (rP.Type != CBaseAction::CParameter::Hidden)) &&
         (i == (pBA->Parameters.size() - 1)))) {
      sTmp = curStr;
      curStr.clear();
    } else {
    ...
    if (noParam >= 2)
      break; // NO MORE THAN 2 REAL PARAMS !
    ...
    if (rP.Type == CBaseAction::CParameter::Hidden) {
      ...
      if (noParam == 0)
        NLGUI::CDBManager::getInstance()
            ->getDbProp(DbComboDisp1P)
            ->setValue32(0);
      else
        NLGUI::CDBManager::getInstance()
            ->getDbProp(DbComboDisp2P)
            ->setValue32(0);
      ...
      if (bValFound) {
        string sCombo;
        if (noParam == 0)
          sCombo = WinName + WIN_EDITCMD_COMBO_FIRST_PARAM_LIST;
        else
          sCombo = WinName + WIN_EDITCMD_COMBO_SECOND_PARAM_LIST;
        pCB = dynamic_cast<CDBGroupComboBox *>(
            CWidgetManager::getInstance()->getElementFromId(sCombo));
        pCB->setSelection(nPinCB);
      }
    } else if ((rP.Type == CBaseAction::CParameter::User) ||
               (rP.Type == CBaseAction::CParameter::UserName)) {
      if (noParam == 0)
        NLGUI::CDBManager::getInstance()
            ->getDbProp(DbComboDisp1P)
            ->setValue32(1);
      ...
      string sEditBox;
      if (noParam == 0)
        sEditBox = WinName + WIN_EDITCMD_COMBO_FIRST_PARAM_EDITBOX;
      else
        sEditBox = WinName + WIN_EDITCMD_COMBO_SECOND_PARAM_EDITBOX;
      CGroupEditBox *pEB = dynamic_cast<CGroupEditBox *>(
    ...
    if (rP.Type != CBaseAction::CParameter::Hidden) {
      string sText;
      if (noParam == 0)
        sText = WinName + VIEW_EDITCMD_FIRST_PARAM_NAME;
      else
        sText = WinName + VIEW_EDITCMD_SECOND_PARAM_NAME;
      CViewText *pVT = dynamic_cast<CViewText *>(
          CWidgetManager::getInstance()->getElementFromId(sText));
      if (pVT != NULL)
        pVT->setText(CI18N::get(pBA->Parameters[i].LocalizedName));
      noParam++;
    }
  }
  validCurrentCommand();
  ...
  // Update the key name
  const CActionsManager::TActionComboMap &actionCombo =
      pAM->getActionComboMap();
  CActionsManager::TActionComboMap::const_iterator it =
      actionCombo.find(CurrentEditCmdLine.ActionName);
  ...
  if (it != actionCombo.end()) {
    ...
    pBut = dynamic_cast<CCtrlBaseButton *>(
        CWidgetManager::getInstance()->getElementFromId(
            WinName + CTRL_EDITCMD_BUTTON_OK));
    if (pBut != NULL)
      pBut->setFrozen(false);
  }
}

// ***************************************************************************
// Given the cat index in comboBox, find action manager and the real cat index
// depending on the AM / macrosiable categories
bool CModalContainerEditCmd::getCatIndexAndAM(sint32 nInCatIndex,
                                              sint32 &nOutCatIndex,
                                              CActionsManager *&nOutAM) {
  CMacroCmdManager *pMCM = CMacroCmdManager::getInstance();
...
// ***************************************************************************
// Give the action index in Combo box, find the action index in the category
// (skip not macroisable actions)
bool CModalContainerEditCmd::getBaseActionIndex(const CCategory &cat,
                                                sint32 nInActIndex,
                                                sint32 &nOutActIndex) {
  if (nInActIndex < 0)
    return false;
  for (nOutActIndex = 0; nOutActIndex < (sint32)cat.BaseActions.size();
       nOutActIndex++) {
...
// ***************************************************************************
// The current command is not valid (so we cant drag it into a macro nor define
// a key for it)
void CModalContainerEditCmd::invalidCurrentCommand() {
  ...
  CCtrlBaseButton *pCB = dynamic_cast<CCtrlBaseButton *>(
      CWidgetManager::getInstance()->getElementFromId(WinName +
                                                      CTRL_EDITCMD_BUTTON_KEY));
  if (pCB != NULL)
    pCB->setActive(false);
  ...
  pCB = dynamic_cast<CCtrlBaseButton *>(
      CWidgetManager::getInstance()->getElementFromId(WinName +
...
void CModalContainerEditCmd::validCurrentCommand() {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  sint32 catIndex =
      NLGUI::CDBManager::getInstance()->getDbProp(DbComboSelCat)->getValue32();
  if (catIndex < 0)
    return;
  sint32 actIndex =
      NLGUI::CDBManager::getInstance()->getDbProp(DbComboSelAct)->getValue32();
  if (actIndex < 0)
    return;

  CActionsManager *pAM;
  // From ComboBox to index in category/action
  if (!getCatIndexAndAM(catIndex, catIndex, pAM))
    return;
  if (!getBaseActionIndex(pAM->getCategories()[catIndex], actIndex, actIndex))
    return;
  ...
  for (uint i = 0; i < rBA.Parameters.size(); ++i) {
    ...
    if (noParam >= 2)
      break; // NO MORE THAN 2 REAL PARAMS !
    if ((i >= 1) && (!CurrentEditCmdLine.ActionName.Argu.empty()))
    ...
    if (rP.Type == CBaseAction::CParameter::Hidden) {
      ...
      // cannot use index directly because some options may be disabled in
      // current context
      sint32 paramIndex;
      const std::vector<CBaseAction::CParameter::CValue> &rVVal = rP.Values;
      if (noParam == 0)
        paramIndex = NLGUI::CDBManager::getInstance()
                         ->getDbProp(DbComboSel1P)
                         ->getValue32();
      else
        paramIndex = NLGUI::CDBManager::getInstance()
                         ->getDbProp(DbComboSel2P)
                         ->getValue32();
      ...
      string sWin;
      if (noParam == 0)
        sWin = WinName + WIN_EDITCMD_COMBO_FIRST_PARAM_EDITBOX;
      else
        sWin = WinName + WIN_EDITCMD_COMBO_SECOND_PARAM_EDITBOX;
      CGroupEditBox *pEB = dynamic_cast<CGroupEditBox *>(
  ...
  if (WinName == "ui:interface:newkey") {
    if (CurrentEditCmdNb == -1) {
      ...
      CCtrlBaseButton *pCB = dynamic_cast<CCtrlBaseButton *>(
          CWidgetManager::getInstance()->getElementFromId(
              WinName + CTRL_EDITCMD_BUTTON_KEY));
      if (pCB != NULL)
        pCB->setActive(true);
      // Activate the key definer text
      CViewText *pVT = dynamic_cast<CViewText *>(
          CWidgetManager::getInstance()->getElementFromId(
              WinName + VIEW_EDITCMD_TEXT_KEY));
      if (pVT != NULL)
        pVT->setActive(true);
...
bool CModalContainerEditCmd::isParamValid(sint32 nParamIndex) {
  ...
  sint32 paramType;
  if (nParamIndex == 0)
    paramType = NLGUI::CDBManager::getInstance()
                    ->getDbProp(DbComboDisp1P)
                    ->getValue32();
  else
    paramType = NLGUI::CDBManager::getInstance()
  ...
  if (paramType == 0) // combo box list
  {
    ...
    if (nParamIndex == 0)
      paramListIndex = NLGUI::CDBManager::getInstance()
                           ->getDbProp(DbComboSel1P)
                           ->getValue32();
    else
      paramListIndex = NLGUI::CDBManager::getInstance()
    ...
    CGroupEditBox *pEB;
    if (nParamIndex == 0)
      pEB = dynamic_cast<CGroupEditBox *>(
          CWidgetManager::getInstance()->getElementFromId(
              WinName + WIN_EDITCMD_COMBO_FIRST_PARAM_EDITBOX));
    else
      pEB = dynamic_cast<CGroupEditBox *>(
          CWidgetManager::getInstance()->getElementFromId(
              WinName + WIN_EDITCMD_COMBO_SECOND_PARAM_EDITBOX));
    if (pEB == NULL)
      return false;

    // no need to translate utf8 or not here
    if (pEB->getInputStringRef().empty())
      return false;
    else
      return true;
  }
...
void CModalContainerEditCmd::onChangeCategory() {
  ...
  if (pCB) {
    ...
    if (!getCatIndexAndAM(catIndex, catIndex, CurAM))
      return;

    const vector<CBaseAction> &rVBA =
        CurAM->getCategories()[catIndex].BaseActions;
...
void CModalContainerEditCmd::onChangeAction() {
  ...
  if (!getCatIndexAndAM(catIndex, catIndex, CurAM))
    return;
  if (!getBaseActionIndex(CurAM->getCategories()[catIndex], actIndex, actIndex))
    return;
  ...
  for (uint i = 0; i < rVParams.size(); ++i) {
    ...
    // Set the parameter name
    {
      string sViewText;
      if (noParam == 0)
        sViewText = WinName + VIEW_EDITCMD_FIRST_PARAM_NAME;
      else
        sViewText = WinName + VIEW_EDITCMD_SECOND_PARAM_NAME;

      pViewParamName = dynamic_cast<CViewText *>(
    ...
    if (rP.Type == CBaseAction::CParameter::Constant) {
      string sWin;
      if (noParam == 0)
        sWin = WinName + WIN_EDITCMD_COMBO_FIRST_PARAM_LIST;
      else
        sWin = WinName + WIN_EDITCMD_COMBO_SECOND_PARAM_LIST;
      CDBGroupComboBox *pCB = dynamic_cast<CDBGroupComboBox *>(
          CWidgetManager::getInstance()->getElementFromId(sWin));
      if (pCB) {
        pCB->resetTexts();
...
public:
  virtual void execute(CCtrlBase * /* pCaller */, const string &Params) {
    ...
    if (Params == "newkey") {
      ...
      if (pGC != NULL)
        pGC->setModalParentList(WIN_KEYS);
    ...
    if (Params == "editcmd") {
      ...
      if (pGC != NULL)
        pGC->setModalParentList(WIN_NEWMACRO);
...
REGISTER_ACTION_HANDLER(CHandlerEditCmdChangeCategory,
...
REGISTER_ACTION_HANDLER(CHandlerEditCmdChangeFirstParam,
...
REGISTER_ACTION_HANDLER(CHandlerEditCmdChangeSecondParam,
  ...
  virtual void execute(CCtrlBase *pCaller, const string &Params) {
    ...
    if (Params == "editcmd") {
      if (pMCM->EditCmd->CurrentEditCmdNb != -1) // Edit mode ?
      {
        ...
        pMCM->CurrentEditMacro.addCommand(
            pMCM->EditCmd->CurrentEditCmdLine.ActionName.Name,
            pMCM->EditCmd->CurrentEditCmdLine.ActionName.Argu,
...
// ***************************************************************************
// Called when the key defined is validated (with ok button on the assign_key
// modal)
...
public:
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    ...
    if (pGetKey->Caller == "newkey") {
      ...
      CViewText *pVT = dynamic_cast<CViewText *>(
          CWidgetManager::getInstance()->getElementFromId(
              pMCM->NewKey->WinName + VIEW_EDITCMD_TEXT_KEY));
      if (pVT != NULL)
        pVT->setText(pMCM->NewKey->CurrentEditCmdLine.Combo.toString());

      CCtrlBaseButton *pCB = dynamic_cast<CCtrlBaseButton *>(
          CWidgetManager::getInstance()->getElementFromId(
              pMCM->NewKey->WinName + CTRL_EDITCMD_BUTTON_OK));
      if (pCB != NULL)
        pCB->setFrozen(false);
    }
    ...
    if (pGetKey->Caller == "editcmd") {
      ...
      if (pVT != NULL)
        pVT->setText(pMCM->EditCmd->CurrentEditCmdLine.Combo.toString());
    }
  }
};
...
static DECLARE_INTERFACE_USER_FCT(canDeleteKey) {
  ...
  if (lineNb < 0 || lineNb >= (sint)pMCM->NewKey->AllLines.size())
    ...
    CActionsManager *pCurAM = pMCM->ActionManagers[line.ComboAM];
    nlassert(pCurAM);
    // cannot delete the action if the action name is "Force Display" AND
    // NotAssigned (since already deleted)
    result.setBool(!(pCurAM->isActionDisplayForced(line.ActionName) &&
                     line.Combo.Key == KeyCount));
  }
