...
const uint MAX_PLAYER_INV_ENTRIES =
    std::max(MAX_BAGINV_ENTRIES, MAX_ANIMALINV_ENTRIES);
...
public:
  ...
  uint32 getSheetID() const {
    return (uint32)(Sheet ? Sheet->getValue32() : 0);
  }
  uint16 getQuality() const {
    return (uint16)(Quality ? Quality->getValue16() : 0);
  }
  uint16 getQuantity() const {
    return (uint16)(Quantity ? Quantity->getValue16() : 0);
  }
  uint8 getUserColor() const {
    return (uint8)(UserColor ? UserColor->getValue8() : 0);
  }
  uint8 getCharacBuffs() const {
    return (uint8)(CharacBuffs ? CharacBuffs->getValue8() : 0);
  }
  uint32 getPrice() const { return (uint32)(Price ? Price->getValue32() : 0); }
  uint32 getWeight() const {
    return (uint32)(Weight ? Weight->getValue32() : 0);
  }
  uint32 getNameId() const {
    return (uint32)(NameId ? NameId->getValue32() : 0);
  }
  uint8 getInfoVersion() const {
    return (uint8)(InfoVersion ? (uint8)InfoVersion->getValue8() : 0);
  }
  uint8 getResaleFlag() const {
    return (uint8)(ResaleFlag ? (uint8)ResaleFlag->getValue8() : 0);
  }
  bool getLockedByOwner() const {
    return (bool)(ResaleFlag ? (ResaleFlag->getValue8() ==
                                BOTCHATTYPE::ResaleKOLockedByOwner)
                             : false);
  }
  //
  void setSheetID(uint32 si) {
    if (Sheet)
      Sheet->setValue32((sint32)si);
  }
  void setQuality(uint16 quality) {
    if (Quality)
      Quality->setValue16((sint16)quality);
  }
  void setQuantity(uint16 quantity) {
    if (Quantity)
      Quantity->setValue16((sint16)quantity);
  }
  void setUserColor(uint8 uc) {
    if (UserColor)
      UserColor->setValue8((sint8)uc);
  }
  void setCharacBuffs(uint8 uc) {
    if (CharacBuffs)
      CharacBuffs->setValue8((sint8)uc);
  }
  void setPrice(uint32 price) {
    if (Price)
      Price->setValue32((sint32)price);
  }
  void setWeight(uint32 wgt) {
    if (Weight)
      Weight->setValue32((sint32)wgt);
  }
  void setNameId(uint32 nid) {
    if (NameId)
      NameId->setValue32((sint32)nid);
  }
  void setInfoVersion(uint8 iv) {
    if (InfoVersion)
      InfoVersion->setValue8((sint8)iv);
  }
  void setResaleFlag(uint8 resale) {
    if (ResaleFlag)
      ResaleFlag->setValue8(resale);
  }
};
...
public:
  ...
  /// Set InfoVersion from a refresh message (to prevent missing a change when
  /// InfoVersion in the database has reached a complete cycle)
  void refreshInfoVersion(uint8 infoVersion) {
    InfoVersionFromMsg = infoVersion;
  }
};
...
// ***************************************************************************
/** This manager gives direct access to inventory slots (bag, temporary
 * inventory, hands, and equip inventory) This also give access to player money
 * \author Nicolas Vizerie
 * \author Nevrax France
 * \date September 2003
 */
...
public:
  ...
  /** Init the manager. This should be done after the local database has been
   * created, because shortcuts pointers are made to the db leaf that represents
   * the inventory. Interface manager should also have been created
   */
  void init();
  // LOCAL INVENTORY
  ...
  // get item of pack animal (local inventory). beastIndex ranges from 0 to
  // MAX_INVENTORY_ANIMAL-1
  ...
  // get item of pack animal (server inventory). beastIndex ranges from 0 to
  // MAX_INVENTORY_ANIMAL-1
  ...
  // Get from an interface object the corresponding database entry (ie:
  // LOCAL:INVENTORY:EQUIP:4)
  ...
  bool isLeftHandItemCompatibleWithRightHandItem(uint32 leftHandSheet,
                                                 uint32 rightHandSheet,
                                                 uint32 lastRightHandSheet);

  // Bag wearing
  // lock an item in the bag because it is weared (done on interface elements of
  // the 2 lists (text and icons))
  ...
  // Equip a part of the player with a bag entry (the params are path in the
  // database) ie : bagPath = LOCAL:INVENTORY:BAG:0 and invPath =
  ...
  void dropOrDestroyItem(CDBCtrlSheet *item, NLMISC::CBitMemStream &out,
                         uint16 quantity);
  ...
  static double getBranchBulk(const std::string &basePath,
                              uint16 startItemIndex, uint16 numItems);
  // Get the number of used and max slots
  static void getBranchSlotCounts(const std::string &basePath,
                                  uint &nbUsedSlots, uint &nbMaxSlots);
  ...
  const CClientItemInfo *getItemInfoCache(uint32 serial,
  ...
  // Add a Waiter on ItemInfo (ItemHelp opening). no-op if here, but reorder
  // (returns true if the version already matches or if waiter is NULL)
  void addItemInfoWaiter(IItemInfoWaiter *waiter);
  // remove a Waiter on ItemInfo (ItemHelp closing). no-op if not here. NB: no
  // delete
  ...
  // Animal Inventories
  // Is the inventory present? if true, it may still not be available for
  // modification (eg: animal too far)
...
private:
  ...
  void initItemArray(const std::string &dbBranchName, CItemImage *dest,
                     uint numItems);
  // init array of int that represents indirection to the bag
  void initIndirection(const std::string &dbbranch, sint32 *indices,
                       sint32 nbIndex, bool putObs = false);
...
inline CInventoryManager &getInventory() {
...
private:
  ...
  public:
    CDBForageQQObs()
        : NLMISC::ICDBNode::IPropertyObserver(), WhichOne(~0), FullValue(0.0f) {
    }
    virtual void update(NLMISC::ICDBNode *node);
    uint WhichOne;
    float FullValue;
...
void initStructForItemSort(std::vector<SSortStruct> &vTemp, sint32 sheetId,
...
struct SBagOptions {
  ...
  SBagOptions() {
    ...
    DbFilterArmor = DbFilterWeapon = DbFilterTool = DbFilterPet = DbFilterMP =
        DbFilterMissMP = DbFilterTP = NULL;
    LastDbFilterArmor = LastDbFilterWeapon = LastDbFilterTool =
        LastDbFilterPet = LastDbFilterMP = LastDbFilterMissMP = LastDbFilterTP =
            false;
...
public:
  ...
  struct CSheetChildBag : public CDBGroupListSheetText::CSheetChild {
    ...
    CSheetChildBag() : CSheetChild() { LastQuality = -1; }
  };
...
public:
  CDBGroupIconListBag(const TCtorParam &param) : CDBGroupListSheet(param) {}
...
// ***************************************************************************
/**
 * Special list for filtering items according to the CLM (control launching
 * modal) \author Lionel Berenguier
 */
...
#define WIN_TEMPINV_TITLE_MISSIONREWARD                                        \
  "uiTIMissionItem" // in fact not only for rewards
...
#define CTRL_JEWEL_EARING_LEFT                                                 \
  "ui:interface:inv_equip:content:equip:jewelry:earing_l"
#define CTRL_JEWEL_BRACELET_LEFT                                               \
  "ui:interface:inv_equip:content:equip:jewelry:bracelet_l"
#define CTRL_JEWEL_RING_LEFT                                                   \
  "ui:interface:inv_equip:content:equip:jewelry:ring_l"
#define CTRL_JEWEL_ANKLET_LEFT                                                 \
  "ui:interface:inv_equip:content:equip:jewelry:anklet_l"
#define CTRL_JEWEL_EARING_RIGHT                                                \
  "ui:interface:inv_equip:content:equip:jewelry:earing_r"
#define CTRL_JEWEL_BRACELET_RIGHT                                              \
  "ui:interface:inv_equip:content:equip:jewelry:bracelet_r"
#define CTRL_JEWEL_RING_RIGHT                                                  \
  "ui:interface:inv_equip:content:equip:jewelry:ring_r"
#define CTRL_JEWEL_ANKLET_RIGHT                                                \
  "ui:interface:inv_equip:content:equip:jewelry:anklet_r"
#define CTRL_JEWEL_HEADDRESS                                                   \
  "ui:interface:inv_equip:content:equip:jewelry:headdress"
...
#define CTRL_JEWL2_EARING_LEFT                                                 \
  "ui:interface:inventory:content:equip:jewelry:earing_l"
#define CTRL_JEWL2_BRACELET_LEFT                                               \
  "ui:interface:inventory:content:equip:jewelry:bracelet_l"
#define CTRL_JEWL2_RING_LEFT                                                   \
  "ui:interface:inventory:content:equip:jewelry:ring_l"
#define CTRL_JEWL2_ANKLET_LEFT                                                 \
  "ui:interface:inventory:content:equip:jewelry:anklet_l"
#define CTRL_JEWL2_EARING_RIGHT                                                \
  "ui:interface:inventory:content:equip:jewelry:earing_r"
#define CTRL_JEWL2_BRACELET_RIGHT                                              \
  "ui:interface:inventory:content:equip:jewelry:bracelet_r"
#define CTRL_JEWL2_RING_RIGHT                                                  \
  "ui:interface:inventory:content:equip:jewelry:ring_r"
#define CTRL_JEWL2_ANKLET_RIGHT                                                \
  "ui:interface:inventory:content:equip:jewelry:anklet_r"
#define CTRL_JEWL2_HEADDRESS                                                   \
  "ui:interface:inventory:content:equip:jewelry:headdress"
