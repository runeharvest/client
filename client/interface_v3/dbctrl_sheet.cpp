...
string CControlSheetInfoWaiter::infoValidated() const {
  ...
  if (CtrlSheet && !LuaMethodName.empty()) {
    ...
    CLuaState *ls = CLuaManager::getInstance().getLuaState();
    {
      ...
      CLuaIHM::pushReflectableOnStack(*ls,
                                      (CReflectableRefPtrTarget *)CtrlSheet);
      ls->pushGlobalTable();
      CLuaObject game(*ls);
      game = game["game"];
      ...
      if (!ls->empty()) {
        ...
        nlwarning(toString("Ucstring result expected when calling '%s', "
...
int CDBCtrlSheet::luaGetDraggedSheet(CLuaState &ls) {
  CLuaIHM::pushUIOnStack(
      ls, dynamic_cast<CInterfaceElement *>(
              dynamic_cast<CDBCtrlSheet *>(CCtrlDraggable::getDraggedSheet())));
  return 1;
}
...
int CDBCtrlSheet::luaGetCreatorName(CLuaState &ls) {
  ...
  CClientItemInfo itemInfo = getInventory().getItemInfo(itemSlotId);
  string creatorName;
  STRING_MANAGER::CStringManagerClient::instance()->getString(
      itemInfo.CreatorName, creatorName);
#ifdef RYZOM_LUA_UCSTRING
...
int CDBCtrlSheet::luaBuildCrystallizedSpellListBrick(CLuaState &ls) {
  ...
  for (i = 0; i < itemInfo.Enchantment.Bricks.size(); i++) {
    // if ( ! (pBM->getBrick(itemInfo.Enchantment.Bricks[i])->isCredit() ||
    // pBM->getBrick(itemInfo.Enchantment.Bricks[i])->isParameter()))
    {
      CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
          toString("UI:VARIABLES:CRYSTALBRICKS:%d:SHEET", currentBrick++));
      if (node)
  ...
  for (;; currentBrick++) {
    CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
        toString("UI:VARIABLES:CRYSTALBRICKS:%d:SHEET", currentBrick), false);
    if (node)
      node->setValue32(0);
    else
      break;
...
// ----------------------------------------------------------------------------
bool CCtrlSheetInfo::parseCtrlInfo(xmlNodePtr cur,
  ...
  CAHManager::getInstance()->parseAH(cur, "onclick_l", "params_l",
                                     _AHOnLeftClick, _AHLeftClickParams);
  CAHManager::getInstance()->parseAH(cur, "onclick_r", "params_r",
                                     _AHOnRightClick, _AHRightClickParams);
  CAHManager::getInstance()->parseAH(cur, "oncandrop", "params_candrop",
                                     _AHOnCanDrop, _AHCanDropParams);
  CAHManager::getInstance()->parseAH(cur, "ondrop", "params_drop", _AHOnDrop,
                                     _AHDropParams);
  CAHManager::getInstance()->parseAH(cur, "oncannotdrop", "params_cannotdrop",
                                     _AHOnCannotDrop, _AHCannotDropParams);
  CAHManager::getInstance()->parseAH(cur, "oncandrag", "params_candrag",
                                     _AHOnCanDrag, _AHCanDragParams);
  CAHManager::getInstance()->parseAH(cur, "ondrag", "params_drag", _AHOnDrag,
                                     _AHDragParams);

  prop = (char *)xmlGetProp(cur, (xmlChar *)"selection_group");
  if (prop) {
    const CCtrlSheetSelection &css =
    ...
    if (_SheetSelectionGroup == -1) {
      nlwarning(
          "<CCtrlSheetInfo::parseCtrlInfo> %s if not a sheet selection group",
          (const char *)prop);
    }
  } else
    _SheetSelectionGroup = -1;
  ...
  if (prop)
    _UseQuantity = CInterfaceElement::convertBool(prop);
  prop = (char *)xmlGetProp(cur, (xmlChar *)"read_quantity_from_sheet");
  if (prop)
    _ReadQuantityFromSheet = CInterfaceElement::convertBool(prop);
  prop = (char *)xmlGetProp(cur, (xmlChar *)"use_quality");
  if (prop)
    _UseQuality = CInterfaceElement::convertBool(prop);
  prop = (char *)xmlGetProp(cur, (xmlChar *)"duplicate_on_drag");
  if (prop)
    _DuplicateOnDrag = CInterfaceElement::convertBool(prop);
  ...
  if (prop) {
    ...
    for (uint i = 0; i < strList.size(); i++) {
      ...
      if (brickType == BRICK_TYPE::UNKNOWN) {
        nlwarning(
            "<CCtrlSheetInfo::parseCtrlInfo> %s has an unvalid Brick Type",
            (const char *)prop);
      } else {
  ...
  if (prop)
    _AutoGrayed = CInterfaceElement::convertBool(prop);

  // _BrickOverable
  prop = (char *)xmlGetProp(cur, (xmlChar *)"brick_over");
  if (prop)
    _BrickOverable = CInterfaceElement::convertBool(prop);

  // Drag Copy support?
  prop = (char *)xmlGetProp(cur, (xmlChar *)"drag_copy");
  if (prop)
    _DragCopy = CInterfaceElement::convertBool(prop);

  // _ForceItemBackGroundGeneric
  prop = (char *)xmlGetProp(cur, (xmlChar *)"force_item_background_generic");
  if (prop)
    _ForceItemBackGroundGeneric = CInterfaceElement::convertBool(prop);
...
// ----------------------------------------------------------------------------
CDBCtrlSheet::CDBCtrlSheet(const TCtorParam &param) : CCtrlDraggable(param) {
  _LastSheetId = 0;
  _DispSlotBmpId = -1;
  _DispBackBmpId = -1;
  _DispSheetBmpId = -1;
...
bool CDBCtrlSheet::parse(xmlNodePtr cur, CInterfaceGroup *parentGroup) {
  ...
  if (!CCtrlBase::parse(cur, parentGroup)) {
    string tmp =
        "cannot parse view:" + getId() + ", parent:" + parentGroup->getId();
    nlinfo(tmp.c_str());
    return false;
  }
  ...
  if (_Type != SheetType_Macro) {
    ...
    if (prop)
      ...
      string tmp = "cannot get property value, view:" + getId() +
  ...
  if (_Type == SheetType_SBrick || _Type == SheetType_SPhraseId ||
      _Type == SheetType_SPhrase || _Type == SheetType_Macro) {
    prop = (char *)xmlGetProp(cur, (xmlChar *)"is_shortcut");
    if (prop)
      _ShortCut = convertBool(prop);
  }
  ...
  if (prop)
    _Grayed = CInterfaceElement::convertBool(prop);

  // Draw the slot ?
  _DrawSlot = true;
  prop = (char *)xmlGetProp(cur, (xmlChar *)"slot");
  if (prop)
    _DrawSlot = CInterfaceElement::convertBool(prop);
  ...
  if (prop)
    _HpBuffIcon = string((const char *)prop);

  _SapBuffIcon = "ico_sap.tga";
  prop = (char *)xmlGetProp(cur, (xmlChar *)"sap_buff_icon");
  if (prop)
    _SapBuffIcon = string((const char *)prop);

  _StaBuffIcon = "ico_stamina.tga";
  prop = (char *)xmlGetProp(cur, (xmlChar *)"sta_buff_icon");
  if (prop)
    _StaBuffIcon = string((const char *)prop);

  _FocusBuffIcon = "ico_focus.tga";
  prop = (char *)xmlGetProp(cur, (xmlChar *)"focus_buff_icon");
  if (prop)
    _FocusBuffIcon = string((const char *)prop);
#endif
  ...
  XML_READ_COLOR(cur, "regen_text_color", _RegenTextColor,
                 NLMISC::CRGBA::White);
  XML_READ_COLOR(cur, "regen_text_shadow_color", _RegenTextShadowColor,
                 NLMISC::CRGBA::Black);
  XML_READ_COLOR(cur, "regen_text_outline_color", _RegenTextOutlineColor,
                 NLMISC::CRGBA::Black);
...
// ----------------------------------------------------------------------------
void CDBCtrlSheet::initSheet(const std::string &dbBranchId,
  ...
  *static_cast<CCtrlSheetInfo *>(this) = ctrlInfo;
  ...
  if (ctrlInfo._Type != SheetType_Macro) {
    ...
    // get over for spell
    CInterfaceManager *pIM = CInterfaceManager::getInstance();
    _TextureIdOver = CViewRenderer::getInstance()->getTextureIdFromName(
        "w_slot_spell_over.tga");
  }
...
// ----------------------------------------------------------------------------
void CDBCtrlSheet::initSheetFast(const std::string &dbParentBranchId,
                                 int sheetNum, int slotNum,
                                 const CCtrlSheetInfo &ctrlInfo) {
...
// ----------------------------------------------------------------------------
std::string CDBCtrlSheet::getSheet() const { return _DbBranchName; }
...
// ***************************************************************************
void CDBCtrlSheet::setSheetFast(const std::string &dbParentBranchId,
                                int sheetNum, int slotNum) {
...
void CDBCtrlSheet::setupSheetDbLinks() {
  ...
  CCDBNodeBranch *dbBranch =
      NLGUI::CDBManager::getInstance()->getDbBranch(_DbBranchName);
  ...
  case SheetType_Mission: {
    // LastSheetId is used as last mission text (mission text which is a server
    // string id)
    _LastSheetId = 0;

    // Mission icon use _SheetId -> gives the sheet that contains the name of
    // the icon texture
    if (!_SheetId.link(dbBranch, _DbBranchName + ":ICON",
                       pIM->_DB_UI_DUMMY_SHEET))
  ...
  case SheetType_SPhraseId: {
    _SheetId.link(dbBranch, _DbBranchName + ":PHRASE",
                  pIM->_DB_UI_DUMMY_PHRASE);
    break;
  }
  case SheetType_GuildFlag:
  case SheetType_ElevatorDestination: {
    // LastSheetId is used as last guild name (guild name which is a server
    // string id)
    _LastSheetId = 0;

    // Guild name use _SheetId
    if (!_SheetId.link(dbBranch, _DbBranchName + ":NAME",
                       pIM->_DB_UI_DUMMY_SHEET))
      nlwarning("ERROR !!! Cannot find guild name : %s",
                (_DbBranchName + ":NAME").c_str());

    // Guild icon use _Quality
    if (!_Quality.link(dbBranch, _DbBranchName + ":ICON",
                       pIM->_DB_UI_DUMMY_QUALITY))
      nlwarning("ERROR !!! Cannot find guild icon: %s",
                (_DbBranchName + ":ICON").c_str());
  ...
  default: {
    ...
    if (_HasTradeSlotType)
      _TradeSlotType.link(dbBranch, _DbBranchName + ":SLOT_TYPE",
                          pIM->_DB_UI_DUMMY_SLOT_TYPE);
    ...
    if (_ActualType == SheetType_Item || _ActualType == SheetType_Pact) {
      // quality
      if (_UseQuality)
        _Quality.link(dbBranch, _DbBranchName + ":QUALITY",
                      pIM->_DB_UI_DUMMY_QUALITY);
      else
        _Quality.link(pIM->_DB_UI_DUMMY_QUALITY);
      ...
      if (_ActualType == SheetType_Item) {
        if (_UseQuantity)
          _Quantity.link(dbBranch, _DbBranchName + ":QUANTITY",
                         pIM->_DB_UI_DUMMY_QUANTITY);
        else
      ...
      if (_ActualType == SheetType_Item) {
        ...
        _Worned.link(dbBranch, _DbBranchName + ":WORNED",
                     pIM->_DB_UI_DUMMY_WORNED);

        // Link to _PrerequisitValid state if possible
        _PrerequisitValid.link(dbBranch, _DbBranchName + ":PREREQUISIT_VALID",
                               pIM->_DB_UI_DUMMY_PREREQUISIT_VALID);
      }
    }
  ...
  _Enchant.link(dbBranch, _DbBranchName + ":ENCHANT",
                pIM->_DB_UI_DUMMY_ENCHANT);
  ...
  if (dbBranch) {
    ...
    _ItemRMClassType =
        dbBranch->getLeaf(_DbBranchName + ":RM_CLASS_TYPE", false);
    _ItemRMFaberStatType =
        dbBranch->getLeaf(_DbBranchName + ":RM_FABER_STAT_TYPE", false);
...
void CDBCtrlSheet::updateCharacBuffs() {
  ...
  _BuffIcons.clear();
  if (characBuffs & (1 << CHARACTERISTICS::constitution))
    _BuffIcons.push_back(SBuffIcon(rVR.getTextureIdFromName("ico_heal.tga")));
  if (characBuffs & (1 << CHARACTERISTICS::intelligence))
    _BuffIcons.push_back(SBuffIcon(rVR.getTextureIdFromName("ico_sap.tga")));
  if (characBuffs & (1 << CHARACTERISTICS::strength))
    _BuffIcons.push_back(
        SBuffIcon(rVR.getTextureIdFromName("ico_stamina.tga")));
  if (characBuffs & (1 << CHARACTERISTICS::dexterity))
    _BuffIcons.push_back(SBuffIcon(rVR.getTextureIdFromName("ico_focus.tga")));

  // update sizes
  for (uint i = 0; i < _BuffIcons.size(); ++i) {
    rVR.getTextureSizeFromId(_BuffIcons[i].TextureId, _BuffIcons[i].IconW,
                             _BuffIcons[i].IconH);
  }
}
...
void CDBCtrlSheet::infoReceived() {
  ...
  const CClientItemInfo *itemInfo =
      getInventory().getItemInfoCache(getItemSerial(), getItemCreateTime());
  if (itemInfo == NULL) {
  ...
  // crystallized spell
  {
    ...
    for (uint i = 0; i < itemInfo->Enchantment.Bricks.size(); ++i) {
      const CSBrickSheet *brick =
          pBM->getBrick(itemInfo->Enchantment.Bricks[i]);
      if (brick) {
        if (!brick->isRoot() && !brick->isCredit() && !brick->isParameter()) {
          _EnchantIcons.push_back(SBuffIcon(
              rVR.getTextureIdFromName(brick->getIcon()), brick->IconColor));
          rVR.getTextureSizeFromId(_EnchantIcons.back().TextureId,
                                   _EnchantIcons.back().IconW,
                                   _EnchantIcons.back().IconH);
        } else if (brick->isRoot()) {
          // there should be single root icon and it should be first one
          _EnchantIcons.push_back(
              SBuffIcon(rVR.getTextureIdFromName(brick->getIconBack()),
                        brick->IconBackColor));
          rVR.getTextureSizeFromId(_EnchantIcons.back().TextureId,
                                   _EnchantIcons.back().IconW,
                                   _EnchantIcons.back().IconH);
...
bool CDBCtrlSheet::useItemInfoForFamily(ITEMFAMILY::EItemFamily family) const {
  return family == ITEMFAMILY::CRYSTALLIZED_SPELL ||
         family == ITEMFAMILY::JEWELRY || family == ITEMFAMILY::ARMOR ||
...
void CDBCtrlSheet::setupItem() {
  ...
  if (_LastSheetId != sheet || _NeedSetup) {
    ...
    if ((pES != NULL) && (pES->type() == CEntitySheet::ITEM)) {
      ...
      if (_DispSheetBmpId == -1)
        _DispSheetBmpId =
            rVR.getSystemTextureId(CViewRenderer::DefaultItemTexture);
  ...
  // must update dispNumber for good quantity/quality.
    ...
    // update quality. NB: if quality change, the must
    // updateItemCharacRequirement
    if (_DisplayItemQuality) {
  ...
  // at each frame, must test for Redifyed (player carac does not met item
  // requirement)
  _Useable = _PrerequisitValid.getBool();
  /*
          _Useable= pIM->isItemCaracRequirementMet(_ItemCaracReqType,
     _ItemCaracReqValue);

          if (_Useable && _ItemSheet != NULL)
          {
                  if (_ItemSheet->RequiredCharac != CHARACTERISTICS::Unknown &&
     _ItemSheet->RequiredCharacLevel>0) _Useable=
     pIM->isItemCaracRequirementMet(_ItemSheet->RequiredCharac,
     _ItemSheet->RequiredCharacLevel);

                  if (_Useable && _ItemSheet->RequiredSkill != SKILLS::unknown
     && _ItemSheet->RequiredSkillLevel > 0) _Useable =
     CSkillManager::getInstance()->checkBaseSkillMetRequirement(_ItemSheet->RequiredSkill,
     _ItemSheet->RequiredSkillLevel);
          }
  */
...
void CDBCtrlSheet::updateItemCharacRequirement(sint32 sheet) {
  ...
  if ((pES != NULL) && (pES->type() == CEntitySheet::ITEM) &&
    ...
    _ItemCaracReqValue = 0;
    float tmpVal;
    if (pIS->hasCharacRequirement(_Quality.getSInt32(), _ItemCaracReqType,
                                  tmpVal))
...
void CDBCtrlSheet::setupMission() {
  ...
  if (_LastSheetId != currIcon || _NeedSetup) {
    ...
    if (pES == NULL)
      return;
    if (pES->Type != CEntitySheet::MISSION_ICON)
      return;
...
void CDBCtrlSheet::setupGuildFlag() {
  ...
  if (_UseGuildIcon) {
    ...
    uint8 nGuildColBackR =
        (uint8)((nGuildIcon >> 11) & 255); // 8 bits en pos 11
    uint8 nGuildColBackG =
        (uint8)((nGuildIcon >> 19) & 255); // 8 bits en pos 19
    uint8 nGuildColBackB =
        (uint8)((nGuildIcon >> 27) & 255); // 8 bits en pos 27
    uint8 nGuildColSymbR =
        (uint8)((nGuildIcon >> 35) & 255); // 8 bits en pos 35
    uint8 nGuildColSymbG =
        (uint8)((nGuildIcon >> 43) & 255); // 8 bits en pos 43
    uint8 nGuildColSymbB =
        (uint8)((nGuildIcon >> 51) & 255); // 8 bits en pos 51
...
void CDBCtrlSheet::setupDisplayAsSBrick(sint32 sheet, sint32 optSheet) {
  ...
  if (pBR != NULL) {
    ...
    if (pBROpt) {
      ...
      if (pBM->getSabrinaCom().isMainDisplayIconInOverSlot(
              CSheetId(optSheet), iconOver2NotSuitableForActionDisplay)) {
    ...
    if (_DispSheetBmpId == -1)
      _DispSheetBmpId =
          rVR.getSystemTextureId(CViewRenderer::DefaultBrickTexture);
    updateIconSize();
...
// ***************************************************************************
void CDBCtrlSheet::setupDisplayAsPhrase(
    const std::vector<NLMISC::CSheetId> &bricks, const string &phraseName) {
  CSBrickManager *pBM = CSBrickManager::getInstance();

  // Get the best SBrick to display.
  CSheetId rootBrickSheetId = bricks[0];
  {
    CSheetId bestBrickSheetId =
        pBM->getSabrinaCom().getPhraseBestDisplayBrick(bricks);
    setupDisplayAsSBrick(rootBrickSheetId.asInt(), bestBrickSheetId.asInt());
  }

  // Override background if type is forace extraction/prospection and ecosystem
  // brick is used
  {
    BRICK_FAMILIES::TBrickFamily family =
        pBM->getSabrinaCom().getPhraseForageFamily(bricks);
    std::string icon;
    switch (family) {
...
void CDBCtrlSheet::setupSPhrase() {
  ...
  if (_LastSheetId != sheet || _NeedSetup) {
    _LastSheetId = sheet;
    CSPhraseSheet *pSPS =
        dynamic_cast<CSPhraseSheet *>(SheetMngr.get(CSheetId(sheet)));
    if (pSPS && !pSPS->Bricks.empty()) {
      const char *phraseName =
          STRING_MANAGER::CStringManagerClient::getSPhraseLocalizedName(
              CSheetId(sheet));
...
void CDBCtrlSheet::setupSPhraseId() {
  ...
  if (_NeedSetup || _LastSheetId != phraseId ||
    ...
    if (phraseId == 0) {
      ...
      if (phrase.empty()) {
        ...
        setupDisplayAsPhrase(phrase.Bricks,
                             phrase.Name.toUtf8()); // FIXME: UTF-8 (serial)
      }
    }
...
// ***************************************************************************
void CDBCtrlSheet::setupCharBitmaps(sint32 maxW, sint32 maxLine,
                                    sint32 maxWChar, bool topDown) {
...
// ***************************************************************************
void CDBCtrlSheet::displayCharBitmaps(sint32 rdrLayer, sint32 x, sint32 y,
                                      CRGBA color) {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  CViewRenderer &rVR = *CViewRenderer::getInstance();

  for (uint i = 0; i < _CharBitmaps.size(); ++i) {
    rVR.draw11RotFlipBitmap(rdrLayer, x + _CharBitmaps[i].X,
                            y + _CharBitmaps[i].Y, 0, false, _CharBitmaps[i].Id,
                            color);
  }
}
...
void CDBCtrlSheet::draw() {
  ...
  // Display slot
  if (_DrawSlot)
    rVR.draw11RotFlipBitmap(
        _RenderLayer, _XReal, _YReal, 0, false, _DispSlotBmpId,
        CWidgetManager::getInstance()->getGlobalColorForContent());

  // Drag'N'Drop : display the selected slot bitmap if this slot accept the
    ...
    if ((CWidgetManager::getInstance()->getCapturePointerLeft() != NULL) &&
      ...
      if ((CWidgetManager::getInstance()->getPointer()->getX() >= _XReal) &&
          ...
          (CWidgetManager::getInstance()->getPointer()->getY() <=
           (_YReal + _HReal)))
        if (CWidgetManager::getInstance()->getCurrentWindowUnder() ==
            CWidgetManager::getInstance()->getWindow(this)) {
          CDBCtrlSheet *pCSSrc = dynamic_cast<CDBCtrlSheet *>(
          ...
          if ((pCSSrc != NULL) && pCSSrc->isDragged()) {
            ...
            CAHManager::getInstance()->runActionHandler(_AHOnCanDrop, this,
                                                        params);
          }
        }
    }
  ...
  // Draw the selection after the sheet. Important for spells because selection
  // border is same size as spell square
  if (_CanDrop) {
    // decal layer because must drawn after Items/Brick in DXTC
    rVR.draw11RotFlipBitmap(
        _RenderLayer + 1, _XReal, _YReal, 0, false, _DispSelSlotId,
        CWidgetManager::getInstance()->getGlobalColorForContent());
  }
  ...
  if (_RegenTickRange.EndTick != _RegenTickRange.StartTick) {
    if (!_LastSheetId) {
      ...
      float amount = (LastGameCycle - _RegenTickRange.StartTick) /
                     float(_RegenTickRange.EndTick - _RegenTickRange.StartTick);
      ...
      for (uint tri = 0; tri < numTris; ++tri) {
        rVR.drawQuad(_RenderLayer + 1, regenTris[tri], frontTex, CRGBA::White,
                     false);
      }
      numTris = buildPie(regenTris, amount, 1.f, texWidth);
      nlassert(numTris <= sizeofarray(regenTris));
      for (uint tri = 0; tri < numTris; ++tri) {
        rVR.drawQuad(_RenderLayer + 1, regenTris[tri], backTex, CRGBA::White,
                     false);
      }
  ...
  if (_NotifyAnimEndTime > T1) {
    if (!_LastSheetId) {
      ...
      float animProgress =
          1.f - float(_NotifyAnimEndTime - T1) / float(NOTIFY_ANIM_MS_DURATION);
      ...
      if (texId != -1) {
        ...
        drawRotatedQuad(rVR, float(NLMISC::Pi) * animProgress * freq0 + phase0,
                        scale, _RenderLayer + 3, (uint32)texId, texWidth,
                        texHeight);
        drawRotatedQuad(rVR, float(NLMISC::Pi) * animProgress * freq1 + phase1,
                        scale, _RenderLayer + 3, (uint32)texId, texWidth,
                        texHeight);
      }
    }
...
void CDBCtrlSheet::drawRegenText() {
  if (!_RegenText) {
    ...
    // do not set shadow if outline is set to avoid clearing it on draw (would
    // call invalidate)
  ...
  if (_RegenTextValue != nextValue) {
    ...
    if (_RegenTextFct.empty()) {
      ...
      if (_RegenTextValue > 600) {
        ...
        if (_RegenTextValue < 60)
          ...
          _RegenText->setText(
              toString("%d'%02d", _RegenTextValue / 60, _RegenTextValue % 60));
      } else {
        _RegenText->setText("");
      }
    } else {
      ...
      if (_RegenTextFctLua) {
        ...
        if (root)
          fct = toString("%s(%d, '%s')", _RegenTextFct.c_str(), _RegenTextValue,
                         root->getFullName().c_str());
        else
      ...
      // if using color tags in format, then RegenText color should be set to
      // CRGBA::White as tag color is modulated with main color
      std::string result;
    ...
    _RegenText->updateTextContext();
    // todo: posref
    // note: if x,y is moved outside icon area it might get cliped and not be
    // visible (wreal/hreal == 0)
  ...
  if (_RegenTextOutline) {
    ...
    _RegenText->setXReal(x - 1);
    _RegenText->setYReal(y + 0);
    _RegenText->draw();
    _RegenText->setXReal(x + 1);
    _RegenText->setYReal(y + 0);
    _RegenText->draw();
    _RegenText->setXReal(x + 0);
    _RegenText->setYReal(y - 1);
    _RegenText->draw();
    _RegenText->setXReal(x + 0);
    _RegenText->setYReal(y + 1);
    _RegenText->draw();
    ...
    _RegenText->setColor(_RegenTextColor);
    _RegenText->setXReal(x);
    _RegenText->setYReal(y);
    _RegenText->draw();
    _RegenText->draw();
  } else {
    _RegenText->draw();
...
// ----------------------------------------------------------------------------
void CDBCtrlSheet::drawRotatedQuad(CViewRenderer &vr, float angle, float scale,
                                   uint renderLayer, uint32 texId,
                                   sint32 texWidth, sint32 texHeight) {
  NLMISC::CQuadUV quv;
  float cosA = cosf(angle);
  float sinA = sinf(angle);
  //
  quv.V0.set(_XReal + 0.5f * _WReal + 0.5f * scale * texWidth * (-cosA + sinA),
             _YReal + 0.5f * _HReal + 0.5f * scale * texHeight * (-sinA - cosA),
             0.5f);
  //
...
// ----------------------------------------------------------------------------
inline void CDBCtrlSheet::uvToScreen(float x, float y, CVector &screenPos,
                                     uint texSize) const {
  screenPos.set(_XReal + texSize * x, _YReal + texSize * (1.f - y), 0.5f);
}

// ----------------------------------------------------------------------------
void CDBCtrlSheet::buildPieCorner(float angle, CUV &uv, CVector &pos,
                                  uint texSize) const {
...
// ----------------------------------------------------------------------------
uint CDBCtrlSheet::buildPie(CQuadUV *triPtr, float startAngle, float endAngle,
                            uint texSize) {
  ...
  while (iCurr != iEnd) {
    ...
    // well, not really a quad, but we don't have yet simple triangles rendering
    // in the ui
...
static inline CRGBA fastMulRGB(CRGBA sheetColor, CRGBA iconColor) {
  // NB: the test here is more to avoid loss of precision when icon value is
  // 255,255,255
...
// ----------------------------------------------------------------------------
void CDBCtrlSheet::drawSheet(sint32 x, sint32 y, bool draging,
  ...
  curSheetColor.A =
      ((CWidgetManager::getInstance()->getGlobalColorForContent().A + 1) *
       _SheetColor.A) >>
      8;
  ...
  CRGBA grayColor =
      CWidgetManager::getInstance()
          ->getSystemOption(CWidgetManager::OptionCtrlSheetGrayColor)
          .getValColor();
  CRGBA redifyColor =
      CWidgetManager::getInstance()
          ->getSystemOption(CWidgetManager::OptionCtrlSheetRedifyColor)
          .getValColor();
  ...
  if (!_Useable) {
    ...
    numberColor.A =
        (uint8)(((uint32)redifyColor.A *
                 (CWidgetManager::getInstance()->getGlobalColorForContent().A +
                  1)) >>
                8);
  } else if (_Grayed)
    numberColor.modulateFromColor(
        grayColor, CWidgetManager::getInstance()->getGlobalColorForContent());
  else
  ...
  case CCtrlSheetInfo::SheetType_Pact:
    setupPact();
    rVR.draw11RotFlipBitmap(_RenderLayer, x, y, 0, false, _DispBackBmpId,
                            curSheetColor);
    rVR.draw11RotFlipBitmap(_RenderLayer, x, y, 0, false, _DispSheetBmpId,
                            curSheetColor);
    if (_DispQuality != -1) {
  ...
  case CCtrlSheetInfo::SheetType_Skill:
    rVR.draw11RotFlipBitmap(_RenderLayer, x, y, 0, false,
                            rVR.getSystemTextureId(CViewRenderer::SkillTexture),
                            curSheetColor);
    break;
  case CCtrlSheetInfo::SheetType_Item:
    setupItem();
    ...
    if (_Worned.getBool()) {
      rVR.draw11RotFlipBitmap(
          _RenderLayer + 1, x, y, 0, false,
          rVR.getSystemTextureId(CViewRenderer::ItemWornedTexture),
          curSheetColor);
    }
    ...
    if ((_DispSheetBmpId == -1) || (draging && !_DuplicateOnDrag)) {
      if (_DispNoSheetBmpId != -1) {
        rVR.draw11RotFlipBitmap(_RenderLayer, x, y, 0, false, _DispNoSheetBmpId,
                                curNoSheetColor);
      }
    } else {
      // Background and icon
      rVR.draw11RotFlipBitmap(_RenderLayer, x, y, 0, false, _DispBackBmpId,
                              fastMulRGB(curSheetColor, _IconBackColor));
      rVR.draw11RotFlipBitmap(_RenderLayer, x, y, 0, false, _DispSheetBmpId,
                              fastMulRGB(curSheetColor, _IconColor));
      ...
      if (_PackedArmourColor != 0) {
        ...
        rVR.draw11RotFlipBitmap(_RenderLayer, x, y, 0, false, _DispOverBmpId,
                                armourCol);
        // decal layer because must drawn after Items/Brick in DXTC
        // NB: use OverColor, not Over2Color here. Because of hack in
        // updateArmourColor()
        rVR.draw11RotFlipBitmap(_RenderLayer + 1, x, y, 0, false,
                                _DispOver2BmpId,
                                fastMulRGB(curSheetColor, _IconOverColor));
      } else {
        // decal layer because must drawn after Items/Brick in DXTC
        rVR.draw11RotFlipBitmap(_RenderLayer + 1, x, y, 0, false,
                                _DispOverBmpId,
        ...
        rVR.draw11RotFlipBitmap(_RenderLayer + 1, x, y, 0, false,
                                _DispOver2BmpId,
                                fastMulRGB(curSheetColor, _IconOver2Color));
      }
      ...
      if (
        ...
        for (uint i = 0; i < _BuffIcons.size(); ++i) {
          ...
          rVR.drawRotFlipBitmap(_RenderLayer + 1, xIcon, yIcon, wIcon, hIcon, 0,
                                false, _BuffIcons[i].TextureId,
                                fastMulRGB(curSheetColor, _BuffIcons[i].Color));
          xIcon += wIcon / 2;
      ...
      if (enchant > 0) {
        // Yes draw the additionnal bitmap and the charge (number of enchanted
        // spell we can launch with the enchanted item)
        enchant--;
        rVR.draw11RotFlipBitmap(
            _RenderLayer + 1, x, y, 0, false,
            rVR.getSystemTextureId(CViewRenderer::ItemEnchantedTexture),
            curSheetColor);
        drawNumber(x + 1, y - 2 + hSheet - rVR.getFigurTextureH(), wSheet,
                   hSheet, numberColor, enchant, false);
      }
      ...
      if (
        ...
        for (uint i = 1; i < _EnchantIcons.size(); ++i) {
          ...
          rVR.drawRotFlipBitmap(
              _RenderLayer + 1, xIcon, yIcon, wIcon, hIcon, 0, false,
              _EnchantIcons[0].TextureId,
              fastMulRGB(curSheetColor, _EnchantIcons[0].Color));
          rVR.drawRotFlipBitmap(
              _RenderLayer + 1, xIcon, yIcon, wIcon, hIcon, 0, false,
              _EnchantIcons[i].TextureId,
              fastMulRGB(curSheetColor, _EnchantIcons[i].Color));
        }
      }
      ...
      if (_UseQuantity && _DispQuantity > -1) {
        sint32 crossId =
            rVR.getSystemTextureId(CViewRenderer::QuantityCrossTexture);
        sint32 crossW =
            rVR.getSystemTextureW(CViewRenderer::QuantityCrossTexture);
        // +1 for lookandfeel
        // draw the "x" bitmap. decal layer because must drawn after Items/Brick
        // in DXTC
        rVR.draw11RotFlipBitmap(_RenderLayer + 2, x + 1, y + 1, 0, false,
                                crossId, curSheetColor);
        ...
        drawNumber(x + 1 + crossW, y + 1, wSheet, hSheet, curSheetColor,
                   quantity, false);
      }
      ...
      if (getLockedByOwner()) {
        rVR.draw11RotFlipBitmap(
            _RenderLayer + 1, x - 2, y + 8, 0, false,
            rVR.getSystemTextureId(CViewRenderer::ItemLockedByOwnerTexture),
            curSheetColor);
      }
    }
    break;
  ...
  case CCtrlSheetInfo::SheetType_Macro:
    setupMacro();
    if (((_DispBackBmpId == -1) && (_DispOverBmpId == -1) &&
         (_DispOver2BmpId == -1)) ||
        (draging && isShortCut())) {
      if (_DispNoSheetBmpId != -1)
        rVR.draw11RotFlipBitmap(_RenderLayer, x, y, 0, false, _DispNoSheetBmpId,
      ...
      if (_DispOver2BmpId != -1)
        rVR.draw11RotFlipBitmap(_RenderLayer, x, y, 0, false, _DispOver2BmpId,
                                curSheetColor);

      // if the pointer is over the button
      if (_Over)
        // Draw -1,-1 because the slot over is 26x26
        rVR.draw11RotFlipBitmap(_RenderLayer + 1, x - 1, y - 1, 0, false,
                                _TextureIdOver, curSheetColor);
    }
    // Display Macro Text
    { displayCharBitmaps(_RenderLayer + 2, x, y, curSheetColor); }
    break;
  case CCtrlSheetInfo::SheetType_GuildFlag:
    ...
    if (_UseGuildIcon) {
      ...
      if (_GuildIcon != NLMISC::CSheetId::Unknown)
        guildIconSheet =
            dynamic_cast<CFactionSheet *>(SheetMngr.get(_GuildIcon));
      if (guildIconSheet != NULL && !guildIconSheet->Icon.empty())
        guildFlagBmpId = rVR.getTextureIdFromName(guildIconSheet->Icon);
      //	else
      //		guildFlagBmpId =
      //rVR.getTextureIdFromName("asc_unknown.tga");
      if (guildFlagBmpId != -1)
        rVR.draw11RotFlipBitmap(_RenderLayer, x, y, 0, false, guildFlagBmpId,
                                curSheetColor);
    } else if (_MacroID == 0) {
      if (_DispNoSheetBmpId != -1)
        rVR.draw11RotFlipBitmap(_RenderLayer, x, y, 0, false, _DispNoSheetBmpId,
      ...
      rVR.drawCustom(x, y, 32, 32, CUV(0, 0), CUV(0.5f, 1), CUV(0, 0),
                     CUV(1, 1), col, _GuildMat);

      col = _IconColor;
      col.modulateFromui(col, curSheetColor.A);
      col.A = curSheetColor.A;

      rVR.drawCustom(x, y, 32, 32, CUV(0.5f, 0), CUV(1, 1), CUV(0, 0),
                     CUV(1, 1), col, _GuildMat);
  ...
  case CCtrlSheetInfo::SheetType_SPhrase: {
    ...
    // for phrases, display special icon if a target is selected and if it is
    // too far
    if (_ActualType == CCtrlSheetInfo::SheetType_SPhraseId) {
      ...
      if (phraseId) {
          ...
          if (target && UserEntity) {
            ...
            if (rootBrick && rootBrick->isMagic()) {
              ...
              if (!forceGrayed) {
                sint phraseRange;
                sint rangeMalus;
                pPM->getPhraseMagicRange(phrase, totalActionMalus, phraseRange,
                                         rangeMalus);
                double rangeDist = (float)(phraseRange + rangeMalus);
                if (phraseRange !=
                    0) // if range is '0' then it is a 'self' action
                {
                  rangeDist +=
                      0.5 + target->getSheetScale() *
                                target->getSheetColRadius(); // player radius
        ...
        // in every cases, test against current gauges to see if the action is
        // possible
        if (UserEntity) {
          sint cost;
          sint costMalus;
    ...
    if (_DispSheetBmpId == -1 || (draging && isShortCut())) {
      ...
      // center the brick (because may be in an item slot when sheetType is
      // auto)
      sint32 px = x + ((_W - 2) / 2) - (BrickSheetWidth / 2);
      sint32 py = y + ((_H - 2) / 2) - (BrickSheetHeight / 2);

      // The brick must drawn in 24x24 even if bitmap is an item-like (MP
      // knowledge bricks for example)
      rVR.drawRotFlipBitmap(_RenderLayer, px, py, BrickSheetWidth,
                            BrickSheetHeight, 0, false, _DispBackBmpId,
                            fastMulRGB(curSheetColor, _IconBackColor));
      rVR.drawRotFlipBitmap(_RenderLayer, px, py, BrickSheetWidth,
                            BrickSheetHeight, 0, false, _DispSheetBmpId,
                            fastMulRGB(curSheetColor, _IconColor));
      // decal layer because must drawn after Items/Brick in DXTC
      rVR.drawRotFlipBitmap(_RenderLayer + 1, px, py, BrickSheetWidth,
                            BrickSheetHeight, 0, false, _DispOverBmpId,
                            fastMulRGB(curSheetColor, _IconOverColor));
      rVR.drawRotFlipBitmap(_RenderLayer + 1, px, py, BrickSheetWidth,
                            BrickSheetHeight, 0, false, _DispOver2BmpId,
                            fastMulRGB(curSheetColor, _IconOver2Color));
      // Draw Quality. -1 for lookandfeel.
      if (_ActualType == CCtrlSheetInfo::SheetType_SBrick) {
        if (_UseQuality && _MustDisplayLevel)
          drawNumber(px - 1, py + 1, BrickSheetWidth, BrickSheetHeight,
                     curSheetColor, _DispLevel);
      } else {
    ...
    if (showOutOfRangeSymbol) {
      rVR.draw11RotFlipBitmap(
          _RenderLayer + 1, x, y, 0, false,
          rVR.getSystemTextureId(CViewRenderer::OutOfRangeTexture),
          CRGBA::White);
    }
  } break;
  case CCtrlSheetInfo::SheetType_OutpostBuilding:
        ...
        if (ssg) {
          ...
          if (ssg->isActive() && _CurrSelection == this) {
            ...
            if (ssg->isGlobalColorEnabled()) {
              color.modulateFromColor(
                  color,
                  CWidgetManager::getInstance()->getGlobalColorForContent());
            }
            // decal layer because must drawn after Items/Brick in DXTC
            rVR.draw11RotFlipBitmap(_RenderLayer + 1, middleX - (tw >> 1),
                                    middleY - (th >> 1), 0, false,
                                    ssg->getTextureIndex(), color);
          }
        }
      }
    }
...
// ----------------------------------------------------------------------------
sint32 CDBCtrlSheet::drawNumber(sint32 x, sint32 y, sint32 wSheet,
                                sint32 /* hSheet */, CRGBA color, sint32 value,
                                bool rightAlign) {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  CViewRenderer &rVR = *CViewRenderer::getInstance();
  ...
  if (value > -1) {
    ...
    do {
      ...
      // decal layer because must drawn after Items/Brick in DXTC
      rVR.drawRotFlipBitmap(_RenderLayer + 2, x + pos, y, wDigit, hDigit, 0,
                            false, unitsID, color);
      units = units / 10;
      pos -= wDigit;
      totalWidth += wDigit;
  ...
  if (event.getType() == NLGUI::CEventDescriptor::mouse) {
    ...
    if (CWidgetManager::getInstance()->getCapturePointerLeft() == this) {
      ...
      if (_DispSheetBmpId != -1) {
        ...
        if (asItemSheet() && asItemSheet()->Stackable > 1 && _UseQuantity) {
          ...
          validClic = isDraggable() && !isDragged() &&
                      ((!getItemWeared() && !getGrayed()) || isShortCut());
        }
      }
      if (_Type == SheetType_Macro) {
      ...
      if (validClic && _AHOnCanDrag != NULL) {
        _TempCanDrag = true;
        CAHManager::getInstance()->runActionHandler(_AHOnCanDrag, this,
                                                    _AHCanDragParams);
        validClic = _TempCanDrag;
      }

      if (validClic) {
        if ((abs(_DragX - eventDesc.getX()) > 5) ||
            (abs(_DragY - eventDesc.getY()) > 5)) {
          _DeltaDragX = _DragX - (_XReal + 1);
          _DeltaDragY = _DragY - (_YReal + 1);
          if (_DeltaDragX > _WReal)
            _DeltaDragX = _WReal;
          if (_DeltaDragY > _HReal)
            _DeltaDragY = _HReal;
          setDragged(true);
          setDraggedSheet(this);

          if (_AHOnDrag != NULL) {
            CAHManager::getInstance()->runActionHandler(_AHOnDrag, this,
                                                        _AHDragParams);
          }
        }
      }
      ...
      if (isDragged()) {
        ...
        if (eventDesc.getEventTypeExtended() ==
          ...
          if (pCSdest != NULL) {
            ...
            if (pCSdest->_AHOnDrop != NULL && pCSdest->_AHOnCanDrop != NULL) {
              ...
              if (!pCSdest->_AHCanDropParams.empty()) {
                if (CAHManager::getInstance()->getAHName(
                        pCSdest->_AHOnCanDrop) == "lua") {
              ...
              CAHManager::getInstance()->runActionHandler(pCSdest->_AHOnCanDrop,
                                                          pCSdest, params);
              ...
              if (pCSdest->_CanDrop) {
                ...
                if (!pCSdest->_AHDropParams.empty()) {
                  if (CAHManager::getInstance()->getAHName(
                          pCSdest->_AHOnDrop) == "lua") {
                    params = pCSdest->_AHDropParams;
                    strFindReplace(params, "%src", _Id);
                  } else {
                    string sTmp = pCSdest->_AHDropParams;
                    params = sTmp + "|" +
                             params; // must copy 'drop' params at start because
                                     // it could be the name of a procedure
                  }
                }
                // call action
                CAHManager::getInstance()->runActionHandler(pCSdest->_AHOnDrop,
                                                            pCSdest, params);
                handled = true;
              }
            }
          } else // If slot not found try to drop on a list
          {
            ...
            const vector<CInterfaceGroup *> &rGUP =
            ...
            if ((pList != NULL) || (pTextList != NULL)) {
              if (pList != NULL)
                if (pList->getCtrlSheetInfo()._AHOnDrop != NULL &&
                    pList->getCtrlSheetInfo()._AHOnCanDrop != NULL) {
                  pList->setCanDrop(false);
                  string params = string("src=") + _Id;
                  if (!pList->getCtrlSheetInfo()._AHCanDropParams.empty()) {
                    if (CAHManager::getInstance()->getAHName(
                            pList->getCtrlSheetInfo()._AHOnCanDrop) == "lua") {
                  ...
                  CAHManager::getInstance()->runActionHandler(
                      pList->getCtrlSheetInfo()._AHOnCanDrop, pList, params);
                  ...
                  if (pList->getCanDrop()) {
                    ...
                    if (!pList->getCtrlSheetInfo()._AHDropParams.empty()) {
                      if (CAHManager::getInstance()->getAHName(
                              pList->getCtrlSheetInfo()._AHOnDrop) == "lua") {
                    ...
                    // call action
                    CAHManager::getInstance()->runActionHandler(
                        pList->getCtrlSheetInfo()._AHOnDrop, pList, params);
                    handled = true;
                  }
                }

              if (pTextList != NULL)
                if (pTextList->getCtrlSheetInfo()._AHOnDrop != NULL &&
                    pTextList->getCtrlSheetInfo()._AHOnCanDrop != NULL) {
                  pTextList->setCanDrop(false);
                  string params = string("src=") + _Id;
                  if (!pTextList->getCtrlSheetInfo()._AHCanDropParams.empty()) {
                    string sTmp =
                        pTextList->getCtrlSheetInfo()._AHCanDropParams;
                    params = sTmp + "|" + params;
                  }
                  CAHManager::getInstance()->runActionHandler(
                      pTextList->getCtrlSheetInfo()._AHOnCanDrop, pTextList,
                      params);
                  ...
                  if (pTextList->getCanDrop()) {
                    ...
                    if (!pTextList->getCtrlSheetInfo()._AHDropParams.empty()) {
                      ...
                      params =
                          sTmp + "|" +
                          params; // must copy 'drop' params at start because it
                                  // could be the name of a procedure
                    }
                    // call action
                    CAHManager::getInstance()->runActionHandler(
                        pTextList->getCtrlSheetInfo()._AHOnDrop, pTextList,
                        params);
                    handled = true;
                  }
                }
          ...
          if (!handled && _AHOnCannotDrop != NULL) {
            CAHManager::getInstance()->runActionHandler(_AHOnCannotDrop, this,
                                                        _AHCannotDropParams);
            handled = true;
          }
          ...
          setDragged(false);
          setDraggedSheet(NULL);
          // In call case, end of drag => consider handled to not call another
          // action
          return true;
        }
      }
    ...
    if (eventDesc.getEventTypeExtended() ==
      ...
      if (_AHOnLeftClick != NULL)
        CAHManager::getInstance()->runActionHandler(_AHOnLeftClick, this,
                                                    _AHLeftClickParams);
    ...
    if (eventDesc.getEventTypeExtended() ==
      ...
      if (_AHOnRightClick != NULL) {
        handled = true;
        CAHManager::getInstance()->runActionHandler(_AHOnRightClick, this,
                                                    _AHRightClickParams);
      }
      // Run Menu (if item is not being dragged)
      if (!_ListMenuRight.empty() || !_ListMenuRightEmptySlot.empty()) {
        ...
        if (dynamic_cast<CDBCtrlSheet *>(CCtrlDraggable::getDraggedSheet()) ==
            NULL) {
          // if a macro, don't test if Sheet==0
          if (isMacro() || getSheetId() != 0) {
            if (!_ListMenuRight.empty()) {
              _CurrMenuSheet = this;
              CWidgetManager::getInstance()->enableModalWindow(this,
                                                               _ListMenuRight);
            }
          }
          // if sheetId==0, then may open the other menu
            ...
            if (!_ListMenuRightEmptySlot.empty()) {
              _CurrMenuSheet = this;
              CWidgetManager::getInstance()->enableModalWindow(
                  this, _ListMenuRightEmptySlot);
            }
          }
        }
      }
...
void CDBCtrlSheet::swapSheet(CDBCtrlSheet *other) {
  ...
  if (_ActualType == SheetType_Item) {
    ...
    swapDBProps(getItemRMFaberStatTypePtr(),
                other->getItemRMFaberStatTypePtr());
    swapDBProps(getItemPrerequisitValidPtr(),
                other->getItemPrerequisitValidPtr());
#ifdef RYZOM_FORGE
...
void CDBCtrlSheet::setCurrSelection(CDBCtrlSheet *selected) {
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("UI:SELECTED_ITEM_SHEET_ID:SHEET")
      ->setValue64(selected ? selected->getSheetId() : 0);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("UI:SELECTED_ITEM_SHEET_ID:QUALITY")
      ->setValue64(selected ? selected->getQuality() : 0);
  NLGUI::CDBManager::getInstance()
      ->getDbProp("UI:SELECTED_ITEM_SHEET_ID:SLOT_TYPE")
      ->setValue64(selected ? selected->getBehaviour() : 0);
  // set the selection group in the db
  NLGUI::CDBManager::getInstance()
      ->getDbProp("UI:SELECTED_ITEM_SELECTION_GROUP")
      ->setValue64(selected ? selected->getSelectionGroup() : -1);
}
...
void CDBCtrlSheet::setupItemInfoWaiter() {
  ...
  string luaMethodName = ((item->Family == ITEMFAMILY::CRYSTALLIZED_SPELL)
                              ? "updateCrystallizedSpellTooltip"
                              : "updateBuffItemTooltip");
  CDBCtrlSheet *ctrlSheet = const_cast<CDBCtrlSheet *>(this);
  ...
  if (itemInfo) {
    ...
    // Using isInventoryPresent/Available() will fail for packers when out of
    // range Getting server item however will work correctly for
    // packer/room/guild
    const CItemImage *itemImage = getInventory().getServerItem(itemSlotId);
    if (itemImage) {
      _ItemInfoWaiter.sendRequest();
    } else {
      // schedule for next draw() - if inventory should not be available (ie
      // guild), but user opens it anyway, then this will loop back here on
      // every draw()
      _ItemInfoChanged = true;
    }
  }
...
void CDBCtrlSheet::getContextHelp(std::string &help) const {
  if (getType() == CCtrlSheetInfo::SheetType_Skill) {
    ...
    help = STRING_MANAGER::CStringManagerClient::getSkillLocalizedName(
        (SKILLS::ESkills)_SheetId.getSInt32());
  } else if (getType() == CCtrlSheetInfo::SheetType_Macro) {
    help = _ContextHelp;
    const CMacroCmd *macro =
        CMacroCmdManager::getInstance()->getMacroFromMacroID(getMacroId());
    if (!macro)
      return;
    ...
    for (uint i = 0; i < macro->Commands.size(); ++i) {
      ...
      for (uint j = 0; j < pMCM->ActionManagers.size(); ++j) {
        CAction::CName c(macro->Commands[i].Name.c_str(),
                         macro->Commands[i].Params.c_str());
        if (pMCM->ActionManagers[j]->getBaseAction(c) != NULL) {
          commandName =
              pMCM->ActionManagers[j]->getBaseAction(c)->getActionLocalizedText(
                  c);
    ...
    if (brick)
      help = STRING_MANAGER::CStringManagerClient::getSBrickLocalizedName(
          brick->Id);
    else
      help = _ContextHelp;
    ...
    if (phraseId == 0) {
      ...
      CLuaState *ls = CLuaManager::getInstance().getLuaState();
      {
        ...
        if (!ls->empty()) {
          ...
          nlwarning("Ucstring result expected when calling "
                    "'game:updatePhraseTooltip', possible script error");
        }
      }
    }
    /*
    ...
    */
    ...
    CSPhraseSheet *phrase =
        dynamic_cast<CSPhraseSheet *>(SheetMngr.get(CSheetId(getSheetId())));
    if (phrase)
      help = STRING_MANAGER::CStringManagerClient::getSPhraseLocalizedName(
          phrase->Id);
    else
      help = _ContextHelp;
    ...
    if (outpost)
      help = CStringManagerClient::getOutpostBuildingLocalizedName(
          CSheetId(_SheetId.getSInt32()));
    else
      help = _ContextHelp;
  }
}
...
bool CDBCtrlSheet::canDropItem(CDBCtrlSheet *src) const {
  ...
  if ((pES != NULL) && (pES->type() == CEntitySheet::ITEM)) {
    ...
    if (_ItemSlot == SLOTTYPE::RIGHT_HAND) {
      // Can put an object in right hand also if it is TWO_HANDS, or
      // RIGHT_HAND_EXCLUSIVE
    ...
    if (pIS->SlotBF & bf) {
      ...
      // Can put an object in left or right hand is dependent of other hand
      // content
      if (_OtherHandItemFilter && (_ItemSlot == SLOTTYPE::LEFT_HAND ||
                                   _ItemSlot == SLOTTYPE::RIGHT_HAND)) {
        ...
        if (_ItemSlot == SLOTTYPE::LEFT_HAND) {
          ...
          // if item present: must check if the right has a TWO_HANDS or
          // RIGHT_HAND_EXCLUSIVE
          if (pRightHandES != NULL &&
              pRightHandES->type() == CEntitySheet::ITEM) {
            ...
            // if the current item we wants to drop is a dagger, check if right
            // hand is a sword or a dagger
        ...
        // special for right hand
          ...
          // if item present: must check if the right has a TWO_HANDS or
          RIGHT_HAND_EXCLUSIVE if ( pOwnES != NULL && pOwnES->type() ==
          CEntitySheet::ITEM )
          {
                  CItemSheet *pOwnIS = (CItemSheet*)pOwnES;
                  if( pOwnIS->hasSlot( SLOTTYPE::TWO_HANDS ) || pOwnIS->hasSlot(
          SLOTTYPE::RIGHT_HAND_EXCLUSIVE ) ) bRightHandContainsTwoHandItem =
          true;
          }

          // if the LeftHand is not empty, and If the item we want to drop is a
          2Hands item, CANNOT drop if (!bRightHandContainsTwoHandItem) if(
          _OtherHandItemFilter->getSheetId()!=0 ) if( pIS->hasSlot(
          SLOTTYPE::TWO_HANDS ) || pIS->hasSlot( SLOTTYPE::RIGHT_HAND_EXCLUSIVE
          ) )
          {
                  return false;
          }
...
void CDBCtrlSheet::updateActualType() const {
  if (_Type == SheetType_Auto) {
    ...
    if (isMission()) {
      ...
      if (!es) {
        ...
        case CEntitySheet::PACT:
          newType = SheetType_Pact;
          break;
        case CEntitySheet::SBRICK:
          newType = SheetType_SBrick;
          break;
        case CEntitySheet::SPHRASE:
          newType = SheetType_SPhrase;
          break;
        case CEntitySheet::OUTPOST_BUILDING:
          newType = SheetType_OutpostBuilding;
          break;
        default:
          newType = SheetType_Item;
          break;
    ...
    // if type changed, must reset some ctrl state (important for list sheet
    // trade for instance else new SPhrase can get aspect of old SItem (eg:
    // Upper-Left MP Text and Redified color...)
...
CCtrlSheetInfo::TSheetType CDBCtrlSheet::getType() const {
  if (_Type == SheetType_GuildFlag)
    return SheetType_GuildFlag;
...
void CDBCtrlSheet::copyAspect(CDBCtrlSheet *dest) {
  ...
  if (isSBrick() || isSPhrase() || isSPhraseId()) {
    // must reset dest Useable/Redifyed and slotType (in case precedent type was
    // Item!)
  ...
  if (dest->_HasTradeSlotType)
    dest->setBehaviour(getBehaviour());
...
bool CDBCtrlSheet::sameAspect(CDBCtrlSheet *dest) const {
  ...
  if (!isSBrickOrSPhraseId() && !isSPhrase()) {
    ...
    if( dest->getRMFaberStatType() != getRMFaberStatType() )
            return false;
    */
    return false; // nico v : Items can be different because of their item info.
                  // So can't conclude about equality. default to false for
                  // safety
  }
  if (dest->_HasTradeSlotType) {
...
CDBCtrlSheet::TSheetCategory CDBCtrlSheet::getSheetCategory() const {
  if (isSkill())
    return Skill;
  if (getType() == SheetType_Pact)
    return Pact;
  if (getType() == SheetType_GuildFlag)
    return GuildFlag;
  if (getType() == SheetType_Mission)
    return Mission;
  if (isSPhrase())
    return Phrase;
  return Item;
...
bool CDBCtrlSheet::isMission() const {
  ...
  if (!root)
    return false;
  CCDBNodeLeaf *node = dynamic_cast<CCDBNodeLeaf *>(
      root->getNode(ICDBNode::CTextId("ICON"), false));
  return node != NULL;
}
...
void CDBCtrlSheet::setupInit() {
  ...
  if (!_OptString.empty()) {
    ...
    CInterfaceElement *pElt =
        CWidgetManager::getInstance()->getElementFromId(_Id, _OptString);
    CDBCtrlSheet *pOtherHand = dynamic_cast<CDBCtrlSheet *>(pElt);
    if (!pOtherHand ||
        pOtherHand->getType() != CCtrlSheetInfo::SheetType_Item) {
...
void CDBCtrlSheet::readFromMacro(const CMacroCmd &mc) {
  ...
  _DispBackBmpId =
      (mc.BitmapBack == 0xFF) ? -1 : pMCM->getTexIdBack(mc.BitmapBack);

  _DispOverBmpId =
      (mc.BitmapIcon == 0xFF) ? -1 : pMCM->getTexIdIcon(mc.BitmapIcon);

  _DispOver2BmpId =
      (mc.BitmapOver == 0xFF) ? -1 : pMCM->getTexIdOver(mc.BitmapOver);
...
void CDBCtrlSheet::writeToMacro(CMacroCmd &mc) {
  ...
  if (_DispBackBmpId == -1) {
    ...
    i = 0;
    while (i != 255) {
      if (pMCM->getTexIdBack(i) == _DispBackBmpId)
        break;
      i++;
    }
    if (i == 255)
      nlwarning("cannot find back id for macro");
    mc.BitmapBack = i;
  }
  ...
  if (_DispOverBmpId == -1) {
    ...
    i = 0;
    while (i != 255) {
      if (pMCM->getTexIdIcon(i) == _DispOverBmpId)
        break;
      i++;
    }
    if (i == 255)
      nlwarning("cannot find icon id for macro");
    mc.BitmapIcon = i;
  }
  ...
  if (_DispOver2BmpId == -1) {
    ...
    i = 0;
    while (i != 255) {
      if (pMCM->getTexIdOver(i) == _DispOver2BmpId)
        break;
      i++;
    }
    if (i == 255)
      nlwarning("cannot find over id for macro");
    mc.BitmapOver = i;
  }
...
bool CDBCtrlSheet::isSheetValid() const {
  ...
  case SheetType_SBrick:
    validSheet = validSheet &&
                 CSBrickManager::getInstance()->getBrick(CSheetId((uint32)sid));
    break;
  case SheetType_SPhraseId:
...
CCDBNodeLeaf *CDBCtrlSheet::getItemInfoVersionPtr() const {
  ...
  if (!root)
    return NULL;
  return dynamic_cast<CCDBNodeLeaf *>(
      root->getNode(ICDBNode::CTextId("INFO_VERSION"), false));
}
...
void CDBCtrlSheet::setItemInfoVersion(uint8 infoVersion) {
  CCDBNodeLeaf *node = getItemInfoVersionPtr();
  if (node)
    node->setValue8((sint8)infoVersion);
}
...
CCDBNodeLeaf *CDBCtrlSheet::getItemWeightPtr() const {
  ...
  if (!root)
    return NULL;
  return dynamic_cast<CCDBNodeLeaf *>(
      root->getNode(ICDBNode::CTextId("WEIGHT"), false));
}
...
CCDBNodeLeaf *CDBCtrlSheet::getItemLockedPtr() const {
  ...
  if (!root)
    return NULL;
  return dynamic_cast<CCDBNodeLeaf *>(
      root->getNode(ICDBNode::CTextId("LOCKED"), false));
}
...
CCDBNodeLeaf *CDBCtrlSheet::getItemPricePtr() const {
  ...
  if (!root)
    return NULL;
  return dynamic_cast<CCDBNodeLeaf *>(
      root->getNode(ICDBNode::CTextId("PRICE"), false));
}
...
CCDBNodeLeaf *CDBCtrlSheet::getItemResaleFlagPtr() const {
  ...
  if (!root)
    return NULL;
  return dynamic_cast<CCDBNodeLeaf *>(
      root->getNode(ICDBNode::CTextId("RESALE_FLAG"), false));
}
...
CCDBNodeLeaf *CDBCtrlSheet::getItemCreateTimePtr() const {
  ...
  if (!root)
    return NULL;
  return dynamic_cast<CCDBNodeLeaf *>(
      root->getNode(ICDBNode::CTextId("CREATE_TIME"), false));
}
...
CCDBNodeLeaf *CDBCtrlSheet::getItemSerialPtr() const {
  ...
  if (!root)
    return NULL;
  return dynamic_cast<CCDBNodeLeaf *>(
      root->getNode(ICDBNode::CTextId("SERIAL"), false));
}
...
CCDBNodeLeaf *CDBCtrlSheet::getItemSellerTypePtr() const {
  ...
  if (!root)
    return NULL;
  return dynamic_cast<CCDBNodeLeaf *>(
      root->getNode(ICDBNode::CTextId("SELLER_TYPE"), false));
}
...
CCDBNodeLeaf *CDBCtrlSheet::getItemPrerequisitValidPtr() const {
  ...
  if (!root)
    return NULL;
  return dynamic_cast<CCDBNodeLeaf *>(
      root->getNode(ICDBNode::CTextId("PREREQUISIT_VALID"), false));
}
...
CCDBNodeLeaf *CDBCtrlSheet::getItemCharacBuffsPtr() const {
  ...
  if (!root)
    return NULL;
  return dynamic_cast<CCDBNodeLeaf *>(
      root->getNode(ICDBNode::CTextId("CHARAC_BUFFS"), false));
}
...
CCDBNodeLeaf *CDBCtrlSheet::getItemAccessPtr() const {
  ...
  if (!root)
    return NULL;
  return dynamic_cast<CCDBNodeLeaf *>(
      root->getNode(ICDBNode::CTextId("ACCESS"), false));
}
...
void CDBCtrlSheet::initArmourColors() {
  ...
  for (uint col = 0; col < RM_COLOR::NumColors; ++col) {
    ...
    std::string colVal =
        CWidgetManager::getInstance()->getParser()->getDefine(defineName);
    if (!colVal.empty())
  ...
  if (!pIS)
    ...
    if (pIS->Family == ITEMFAMILY::SCROLL_R2) {
      const R2::TMissionItem *mi =
          R2::getEditor().getPlotItemInfos(getSheetId());
      if (mi)
        return mi->Name.toUtf8();
    }
    ...
    if (!nameId &&
        (pIS->Family == ITEMFAMILY::ARMOR ||
         ...
         pIS->Family == ITEMFAMILY::AMMO || pIS->Family == ITEMFAMILY::SHIELD ||
         pIS->Family == ITEMFAMILY::JEWELRY)) {
      // get description string for item format
      std::string formatID =
          getItemRMFaberStatType() != RM_FABER_STAT_TYPE::Unknown
              ? "uihelpItemFaberPrefixAndSuffix"
              : "uihelpItemFaberPrefix";
      string format;
      ...
      // suffix
      strFindReplace(format, "%p",
                     RM_CLASS_TYPE::toLocalString(getItemRMClassType()));
      // name
      strFindReplace(format, "%n", ret);
      // prefix
      strFindReplace(format, "%s",
                     CI18N::get(toString("mpstatItemQualifier%d",
                                         (int)getItemRMFaberStatType())
                                    .c_str()));
...
void CDBCtrlSheet::updateArmourColor(sint8 col) {
  ...
  if (_ArmourColorIndex >= 0 && _ArmourColorIndex <= 7) {
    ...
    if (!_ArmourColorBmpOk) {
      ...
      std::string maskName = CFile::getFilenameWithoutExtension(iconName) +
                             "_mask." + CFile::getExtension(iconName);
...
bool CDBCtrlSheet::checkItemRequirement() {
  ...
  // NB: we cannot test directly _Useable, because callers typically call this
  // method BEFORE updateCoords()

  // If this is not the same sheet/quality, need to resetup charac requirement
  /*
          sint32 sheet = _SheetId.getSInt32();
          if (_LastSheetId != sheet || _NeedSetup ||
     _Quality.getSInt32()!=_DispQuality)
          {
                  // NB: don't update cache, leave this feature to
     updateCoords()
                  // Special Item requirement
                  updateItemCharacRequirement(sheet);
          }

          // at each frame, must test for Redifyed (player carac does not met
     item requirement) CInterfaceManager	*pIM=
     CInterfaceManager::getInstance();

          bool retVal = pIM->isItemCaracRequirementMet(_ItemCaracReqType,
     _ItemCaracReqValue);

          if (retVal && _ItemSheet)
          {
                  if (_ItemSheet->RequiredCharac != CHARACTERISTICS::Unknown &&
     _ItemSheet->RequiredCharacLevel>0) retVal =
     pIM->isItemCaracRequirementMet(_ItemSheet->RequiredCharac,
     _ItemSheet->RequiredCharacLevel);

                  if (retVal && _ItemSheet->RequiredSkillLevel > 0 &&
     _ItemSheet->RequiredSkill != SKILLS::unknown) _Useable =
     CSkillManager::getInstance()->checkBaseSkillMetRequirement(_ItemSheet->RequiredSkill,
     _ItemSheet->RequiredSkillLevel);
          }
          return retVal;
...
bool NoOpForCCtrlSheetInfo_Serial =
    false; // Prevent an assert in CDBCtrlSheet::serial (set externally) for a
           // very specific case. The only case where an empty implementation is
           // ok is for r2 in edition mode when building in scene interface for
           // entities (the CDBCtrlSheet are hidden then ...!)
           // /

// ***************************************************************************
void CDBCtrlSheet::serial(NLMISC::IStream &f) {
  CCtrlBase::serial(f);
  if (NoOpForCCtrlSheetInfo_Serial)
