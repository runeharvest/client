...
void CInputHandlerManager::operator()(const NLMISC::CEvent &event) {
  ...
  if (event == EventSetFocusId) {
    ...
    if (!pEvent->Get) {
      ...
      if (!_SkipInterfaceManager) {
        // if there was some control capturing the mouse, warn them that they
        // lost the focus
        inputHandler.handleSetFocusEvent(event);
        UserControls.stopFreeLook();
      }
      // be nice with other app : let the mouse reappear (useful in direct 3D
      // mode with no hardware cursor)
      ...
      CViewRenderer::getInstance()->getDriver()->showCursor(
    ...
    if (!_SkipInterfaceManager) {
      ...
      if (R2::getEditor().isInitialized() &&
        ...
        R2::getEditor().handleEvent(
            NLGUI::CEventDescriptorSetFocus(pEvent->Get));
      }
    }
  ...
  if (_SkipInterfaceManager) {
    ...
    FilteredEventServer.postEvent(event.clone());
    /*	Invalidate Actions managers by simulate keyUp on all. Else can have this
       scenario:
  ...
  // **** Event Keyboard
  if (event == EventKeyDownId || event == EventKeyUpId ||
      event == EventCharId || event == EventStringId) {
    ...
    if (!inputHandler.handleKeyboardEvent(event)) {
      ...
      if (R2::getEditor().isInitialized() &&
          (ClientCfg.R2EDEnabled || R2::getEditor().getCurrentTool())) {
        handled = R2::getEditor().handleEvent(
            NLGUI::CEventDescriptorKey((const CEventKey &)event));
      }
      if (!handled) {
        // Event from the Keyboard (DOWN KEYS)
        if (event == EventKeyDownId) {
          CEventKeyDown *downEvent = (CEventKeyDown *)&event;
          if (!CWidgetManager::getInstance()->getCaptureKeyboard() ||
              !EditActions.keyPushed(*downEvent))
  ...
  // **** Event Mouse
      ...
      _Focus /* && CWidgetManager::getInstance()->isMouseHandlingEnabled() */ &&
      (event == EventMouseMoveId || event == EventMouseDownId ||
       event == EventMouseUpId || event == EventMouseWheelId ||
    ...
    if (!doTest ||
        (doTest && CWidgetManager::getInstance()->isMouseHandlingEnabled())) {
      if (event == EventMouseDownId) {
        if (_RecoverFocusLost) {
          handled |= updateMousePos(
              (CEventMouse &)
                  event); // must update mouse pos here,
                          // because when app window focus is gained by a mouse
                          // click, this is the only place where we can retrieve
                          // mouse pos before a mouse move
          _RecoverFocusLost = false;
        }
        if (!handled) {
          if (R2::getEditor().isInitialized() &&
              (R2::isEditionCurrent() || R2::getEditor().getCurrentTool())) {
            const NLMISC::CEventMouseDown *mouseDownEvent =
                static_cast<const NLMISC::CEventMouseDown *>(&event);
            if (mouseDownEvent->Button & NLMISC::leftButton) {
              eventDesc.setEventTypeExtended(
                  CEventDescriptorMouse::mouseleftdown);
              handled |= R2::getEditor().handleEvent(eventDesc);
            }
            if (mouseDownEvent->Button & NLMISC::rightButton) {
              eventDesc.setEventTypeExtended(
                  CEventDescriptorMouse::mouserightdown);
              handled |= R2::getEditor().handleEvent(eventDesc);
            }
          }
        }
      ...
      // db click ?
        ...
        // TODO: yoyo make it work if needed (for now, seems preferable to
        // manage in each ActionHandler)
    ...
    if (!handled) {
      if (R2::getEditor().isInitialized() &&
        ...
        if (event == EventMouseDownId) {
          ...
          const NLMISC::CEventMouseUp *mouseUpEvent =
              static_cast<const NLMISC::CEventMouseUp *>(&event);
          ...
          const NLMISC::CEventMouseDblClk *mouseDblClkEvent =
              static_cast<const NLMISC::CEventMouseDblClk *>(&event);
          ...
          if (mouseDblClkEvent->Button & NLMISC::rightButton) {
            eventDesc.setEventTypeExtended(
                CEventDescriptorMouse::mouserightdblclk);
            handled |= R2::getEditor().handleEvent(eventDesc);
          }
        } else {
          if (event == EventMouseWheelId) {
            const NLMISC::CEventMouseWheel *wheelEvent =
                static_cast<const NLMISC::CEventMouseWheel *>(&event);
...
// ***************************************************************************
void CInputHandlerManager::CComboKey::init(
    const NLGUI::CEventDescriptorKey &rDK) {
  Key = rDK.getKey();
  CtrlFlags = 0;
  if (rDK.getKeyCtrl())
    CtrlFlags |= CtrlKey;
...
// ***************************************************************************
bool CInputHandlerManager::isComboKeyChat(
    const NLGUI::CEventDescriptorKey &edk) const {
  CComboKey ckey;
  ckey.init(edk);
...
bool CInputHandlerManager::readInputConfigFile(const std::string &fileName) {
  ...
  try {
    ...
    // Output error
    root = NULL;
    nlwarning("CFormLoader: Error while loading the xml input file : %s",
              e.what());
  }
  if (!root) {
...
// ***************************************************************************
void CInputHandlerManager::parseKey(xmlNodePtr cur,
                                    std::vector<CComboKey> &out) {
  // read it
  CXMLAutoPtr prop;
  string keyStr;
  string flagStr;
