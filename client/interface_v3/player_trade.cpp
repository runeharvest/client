...
void CPlayerTrade::putItemInExchange(CDBCtrlSheet *src, CDBCtrlSheet *dest,
                                     uint quantityMove) {
  if (dest && src) {
    restoreItem(dest);
    src->copyAspect(dest);
...
void CPlayerTrade::restoreItem(CDBCtrlSheet *exchangeSlot) {
  ...
  // NB: cannot auto stack ammo because their full props (dammage etc...) are
  // not sent to client
  if (itemSheet) {
    canAutoStack =
        itemSheet->Stackable > 1 && itemSheet->Family != ITEMFAMILY::AMMO;
    autoStackSheetId = itemSheet->Id.asInt();
    autoStackQuality = exchangeSlot->getQuality();
    autoStackMaxQuantity = itemSheet->Stackable;
  }

  // *** find an empty place in the inventory, or if autoStack, distribute over
  // existing slot
  sint32 emptySlot = -1;
  ...
  // if some quantity not distributed, Move all the remaining exchange slot to
  // the empty slot detected (if any)
  if (quantityLeft && emptySlot >= 0) {
    ...
    im.getBagItem(emptySlot).setCharacBuffs(
        (uint8)exchangeSlot->getItemCharacBuffs());
...
CDBCtrlSheet *CPlayerTrade::getExchangeItem(uint index) {
  ...
  return dynamic_cast<CDBCtrlSheet *>(
      CWidgetManager::getInstance()->getElementFromId(NLMISC::toString(
          "ui:interface:player_trade:header_opened:give:given_items:slot%d",
          (int)index)));
}
  ...
  virtual void execute(CCtrlBase *pCaller, const string & /* Params */) {
    ...
    if (sheet->getSheetId() == 0) {
      ...
      if (!isThereObjectNotWorn) {
        // every object are worn, so there's no use to display an empty list ->
        // no-op
        return;
      }
    }
    CInterfaceManager *im = CInterfaceManager::getInstance();
    CWidgetManager::getInstance()->pushModalWindow(
        pCaller, "ui:interface:exchange_choose_in_bag");
  }
};
