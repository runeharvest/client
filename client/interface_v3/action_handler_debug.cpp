...
class CAHDisplayInfos : public IActionHandler {
  void execute(CCtrlBase * /* pCaller */, const std::string & /* params */) {
    ...
    if (ClientCfg.Local || hasPrivilegeDEV() || hasPrivilegeSGM() ||
        hasPrivilegeGM() || hasPrivilegeSG() || hasPrivilegeEM() ||
        hasPrivilegeVG())
#endif
    {
  ...
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    if (Driver) {
      ...
      CFileDisplayer fileDisplayer(
          NLMISC::CFile::findNewFile(getLogDirectory() + "texture.log"));
  ...
  virtual void execute(CCtrlBase * /* pCaller */, const string &Params) {
    ...
    if (filter <= RYZOM_MAX_FILTER_3D) {
      ...
      if (filter == FilterVegetable) {
        if (Landscape)
          Landscape->enableVegetable(Filter3D[FilterVegetable]);
      }
    }
#endif // _PROFILE_ON_
  }
...
class CAHToggleSeason : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    if (ClientCfg.ManualWeatherSetup) {
      ...
      if (ManualSeasonValue == EGSPD::CSeason::Invalid)
        ManualSeasonValue = EGSPD::CSeason::Spring;
