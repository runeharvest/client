 ...
 * This is done through a small window in the server database of 8 (for the
 *moment) elements Each elements can be composed of several leaves This is done
 *through an intermediairy structure to prevent updating local database with
 *page that should not be here (ie 1,2 are received and we receive 4. If we
 *update directly we will have pop and displacement which can be confusing for
 *the player, for example if he has selected an item in page 4.
 *
 * For the moment there are 4 differents lists
 *
 * MISSIONS (in the screen to choose the mission, with an icon for each mission
 *category) ICON TEXT DETAIL_TEXT PREREQ_STATE
 *
...
public:
  ...
  // Init the category. This must be called (no more that once) before any other
  // method (or the ctor must provide the category)
  void setListType(ListType listType);

  // Set the current mission type, for special msg. Call before start
  void setMissionClientType(MISSION_DESC::TClientMissionType mType) {
    _MType = mType;
  }

  // Used For Phrase Trading only, true to allow phrase update and client fill
  // (only if server agree)
  void allowPhrasePriceUpdateAndClientFill(bool state) {
    _PhrasePriceUpdateAndMaybeClientFill = state;
  }
  ...
  uint16 getSessionID() {
    nlassert((uint)_Category < ListTypeCount);
    return _CurrentSessionNb;
  }
...
private:
  ...
  public:
    ...
    //
    NLMISC::CCDBNodeLeaf
        *LogicTextID; // valid if the item is to be obtained for a mission
    NLMISC::CCDBNodeLeaf
        *DescTextID; // valid if the item is to be obtained for a mission
    //
    NLMISC::CCDBNodeLeaf
        *Price; // valid if the item is to be obtained with money
    //
  ...
  public:
    CItemDBLeaves()
        : GuildIcon(NULL), GuildName(NULL), SlotType(NULL), Quality(NULL),
          SheetIDOrSkill(NULL), LogicTextID(NULL), DescTextID(NULL),
          Price(NULL), MissionText(NULL), MissionDetailText(NULL),
          MissionIcon(NULL), MissionPreReqState(NULL), Weight(NULL),
          NameId(NULL), InfoVersion(NULL), UserColor(NULL), CharacBuffs(NULL),
          Enchant(NULL), RMClassType(NULL), RMFaberStatType(NULL),
          Quantity(NULL), PriceRetire(NULL), ResaleTimeLeft(NULL),
          VendorNameId(NULL), FactionType(NULL), FactionPointPrice(NULL),
          SellerType(NULL) {}
  };
  //
  ...
  // tool fct
  void setPriceInDB(uint32 value, const std::string &ls, const std::string &ms,
                    const std::string &bs, const std::string &vbs);
...
private:
  ...
  struct CItem {
    ...
    uint32
        LogicTextID; // Valid if the item is to be obtained as a mission reward
    uint32
        DescTextID; // Valid if the item is to be obtained as a mission reward
    uint32
        Price; // Valid for standard items (not to be obtained with a mission)
