 ...
 *
 * "getSelectedItemPrice" : Get the price of the current selected item (should
 * come from bot inventory)
 * ====
 * "getDraggedSheet"      : Get ptr object on the current dragged sheet
 * "getSelectedSheet"     : Get ptr object on the current selected sheet
 * "getMenuItemSheet"     : Get ptr object on the last sheet that trigerred
 * menu.
 * ====
 * "getSheetID"               : From a pointer on a sheet, get its sheetID
 * "getSheetSelectionGroup"   : From a pointer on a sheet, get its selection
 * group name "getSheetName"             : From a pointer on a sheet, or a sheet
 * ID, get its name "getSheetIndex"            : From a pointer on a sheet, get
 * its index in the parent sheet list "getSheetQuantity"         : From a
 * pointer on a sheet, retrieve its quantity "getSheetQuality"          : From a
 * pointer on a sheet, retrieve its quality
 * ====
 * "isStackable"              : From a pointer on a sheet, get its index in the
 * parent sheet list "isTeleport"               : From a pointer on a sheet or a
 * sheet ID, test if an item is a teleport item "isPlainItem               :
 * From a pointer on a sheet or a sheet ID, test if an item is a plain item (not
 * a teleport, not a brick, not a pact) "isBrick	               : From a
 * sheet ID, test if an item is a brick "isPact		               : From a
 * sheet ID, test if an item is a pact "isRangeWeapon"            : From a
 * pointer on a sheet or a sheet ID, test if an item is a ran,ged weapon
 * "isRoomLeftFor"            : From a sheet ID, test if there's room for it in
 * the inventory
 * ===
 ...
 * ===
 * "getItemsWeight(basePath, firstItem, numItem)" : Get the sum of all item's
 * weight at the given db path "getItemBranchsWeight(baseBranch, firstBranch,
 * numBranch)" : Get the sum of all items located in several branchs
 * "getSeedsWeight()" : Returns the weight of the seeds owned by the player
 * "getArmoursWeight()" : Returns the weight of all armours pieces worn by the
 * player "getInventoryWeight()" : Returns the weight of the whole inventory,
 * not including beasts'inventory of course "getSheathWeight()" : Returns the
 * weight of the sheaths
 * ===
 * "getBranchSheetCategory() : return the types of items that are contained in a
 * branch (with the enum CDBCtrlSheet::TSheetCategory
 * ===
 * "copySheet" : param 0 = pointer on a sheet, param 1 = dbAdress. This copy
 * characteristics of a sheet (quantity, quality, sheet id) at the given
 * database adress
 *
 * "canAfford(price, ls, ms, bs, vbs)" : test if the given seeds can afford the
 * given price
 */
...
static DECLARE_INTERFACE_USER_FCT(canAfford) {
  ...
  CSeeds rhs(
      (CSeeds::TUInt)args[1].getInteger(), (CSeeds::TUInt)args[2].getInteger(),
      (CSeeds::TUInt)args[3].getInteger(), (CSeeds::TUInt)args[4].getInteger());
...
static DECLARE_INTERFACE_USER_FCT(getDraggedSheet) {
  result.setUserType(new CDBCtrlSheetPtrUserType(
      dynamic_cast<CDBCtrlSheet *>(CDBCtrlSheet::getDraggedSheet())));
  return true;
}
...
static DECLARE_INTERFACE_USER_FCT(getSelectedSheet) {
  result.setUserType(
      new CDBCtrlSheetPtrUserType(CDBCtrlSheet::getCurrSelection()));
  return true;
}
...
static DECLARE_INTERFACE_USER_FCT(getSheetFromId) {
  ...
  CDBCtrlSheet *sheet = dynamic_cast<CDBCtrlSheet *>(
      CWidgetManager::getInstance()->getElementFromId(args[0].getString()));
  if (!sheet) {
...
static DECLARE_INTERFACE_USER_FCT(getMenuItemSheet) {
  result.setUserType(
      new CDBCtrlSheetPtrUserType(CDBCtrlSheet::getCurrSelSheet()));
  return true;
}
REGISTER_INTERFACE_USER_FCT("getMenuItemSheet", getMenuItemSheet)

//=========================================================================================
/** Tool function : convert an interface expr value to a pointer on a
 * CDBCtrlSheet
 */
static CDBCtrlSheet *asCtrlSheet(CInterfaceExprValue &arg) {
  ...
  CDBCtrlSheetPtrUserType *sheetPtr =
      dynamic_cast<CDBCtrlSheetPtrUserType *>(arg.getUserType());
  if (!sheetPtr) {
    nlwarning("<asCtrlSheet> Arg 0 has wrong type!");
    return NULL;
  }
...
//=========================================================================================
/** Tool function : convert a list of arguments to a pointer on a CDBCtrlSheet
 * (check that the list as one single element of the right type)
 */
...
//=========================================================================================
/** Tool function : convert a list of arguments to a pointer on a CItemSheet
 * (check that the list as one single element of the right type)
 */
...
static DECLARE_INTERFACE_USER_FCT(getSheetName) {
  CDBCtrlSheet *ctrlSheet = asCtrlSheet(args);
  // if from ctrlSheet, then take the correct ACTUAL name (ie from NAMEID if not
  // 0)
  if (ctrlSheet) {
  ...
  // Standard (but less accurate) way
    ...
    const char *tmp = "";
    if (itemSheet != NULL)
      tmp = STRING_MANAGER::CStringManagerClient::getItemLocalizedName(
          itemSheet->Id);
...
////////////////////////////////////////////////////////////////////////////////////////
// From a pointer on a sheet or a sheet ID, test if an item is a ticket for an
// animal //
////////////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// From a pointer on a sheet or a sheet ID, test if an item is a plain item (not
// a teleport, not a brick, not a pact) //
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
static DECLARE_INTERFACE_USER_FCT(isRoomLeftFor) {
  ...
  std::string nbBagPath =
      CWidgetManager::getInstance()->getParser()->getDefine("bag_nb");
  ...
  std::string bagsPath =
      CWidgetManager::getInstance()->getParser()->getDefine("bag");
  if (bagsPath.empty())
    return false;
  uint k = 0;
  for (k = 0; k < nbItemInBags; ++k) {
    std::string bagPath = bagsPath + ":" + toString(k) + ":SHEET";
    CCDBNodeLeaf *bagProp =
        NLGUI::CDBManager::getInstance()->getDbProp(bagPath);
    if (!bagProp)
      return false;
  ...
  static const CArmourInfo armourInfos[] = {
      {"head", SLOTTYPE::HEAD}, {"chest", SLOTTYPE::CHEST},
      {"arms", SLOTTYPE::ARMS}, {"hands", SLOTTYPE::HANDS},
      {"legs", SLOTTYPE::LEGS}, {"feet", SLOTTYPE::FEET}};
  uint numArmorSlots = sizeof(armourInfos) / sizeof(armourInfos[0]);

  for (k = 0; k < numArmorSlots; ++k) {
    std::string dbPath = CWidgetManager::getInstance()->getParser()->getDefine(
        armourInfos[k].DefineName);
    if (dbPath.empty())
      return false;
    CCDBNodeLeaf *armorProp =
        NLGUI::CDBManager::getInstance()->getDbProp(dbPath + ":SHEET");
    if (!armorProp)
      return false;
...
REGISTER_INTERFACE_USER_FCT("getSelectionGroupIdFromName",
...
static double getItemsWeight(CCDBNodeBranch *branch, uint16 startItemIndex,
                             uint16 numItems) {
  ...
  for (uint16 currItem = startItemIndex; currItem < lastIndexItem; ++currItem) {
    ...
    if (node) {
      ...
      CCDBNodeLeaf *wgtNode = dynamic_cast<CCDBNodeLeaf *>(
          node->getNode(ICDBNode::CTextId("WEIGHT")));
      // must not be empty
      if (wgtNode && sheetNode && quantityNode &&
          sheetNode->getValue32() != 0) {
        totalWeight += std::max((sint32)1, quantityNode->getValue32()) *
                       wgtNode->getValue32();
      }
    }
  }
 ...
 */
static double getItemsWeight(const std::string &basePath, uint16 startItemIndex,
                             uint16 numItems) {
  CInterfaceManager *im = CInterfaceManager::getInstance();
...
////////////////////////////////////////////////////////////////////////////////////////
//   getItemsWeight(basePath)" : Get the sum of all item weight in the given db
//   path. //
////////////////////////////////////////////////////////////////////////////////////////

static DECLARE_INTERFACE_USER_FCT(getItemsWeightUserFct) {
  if (args.size() != 3 || !args[0].toString() || !args[1].toInteger() ||
      !args[2].toInteger()) {
    nlwarning("<getItemWeight> 3 arguments expected : (dbPath, startItemIndex, "
              "numItems)");
    return false;
  }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// getItemBranchsWeight(baseBranch, firstBranch, numBranch) : Get the sum of all
// items located in several branchs //
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
static double getItemBranchsWeight(const std::string &basePath,
                                   uint startBranchIndex, uint numBranchs) {
  //
  CInterfaceManager *im = CInterfaceManager::getInstance();
  //
  double totalWeight = 0;
  // look for all branchs
  for (uint branchIndex = startBranchIndex;
       branchIndex < startBranchIndex + numBranchs; ++branchIndex) {
    std::string branchName = toString("%s:%d", basePath.c_str(), branchIndex);
    CCDBNodeBranch *currBranch =
        NLGUI::CDBManager::getInstance()->getDbBranch(branchName);
    if (!currBranch) {
      nlwarning(
          "<getItemBranchsWeight> can't get branch %s:%d, or this is a leaf",
          basePath.c_str(), branchIndex);
    } else {
...
static DECLARE_INTERFACE_USER_FCT(getItemBranchsWeightUserFct) {
  if (args.size() != 3 || !args[0].toString() || !args[1].toInteger() ||
      !args[2].toInteger()) {
    nlwarning("<getItemBranchsWeight> 3 arguments expected : (dbBranchPath, "
  ...
  result.setDouble(
      getItemBranchsWeight(basePath, startBranchIndex, numBranchs));
  return true;
}
...
static double
getWeightOfEquipmentPieces(const SLOT_EQUIPMENT::TSlotEquipment * /* slots */,
                           uint /* numSlots */) {
  ...
  {
          std::string dbPath = NLMISC::toString("LOCAL:INVENTORY:8:%d:SHEET",
  (int) slots[k]); uint32 sheetID =
  NLGUI::CDBManager::getInstance()->getDbProp(dbPath)->getValue32(); if
  (sheetID)
          {
...
static double getArmoursWeight() {
  static const SLOT_EQUIPMENT::TSlotEquipment armourPieces[] = {
      SLOT_EQUIPMENT::HEAD,  SLOT_EQUIPMENT::CHEST, SLOT_EQUIPMENT::ARMS,
      SLOT_EQUIPMENT::HANDS, SLOT_EQUIPMENT::LEGS,  SLOT_EQUIPMENT::FEET};
  return getWeightOfEquipmentPieces(armourPieces, sizeof(armourPieces) /
                                                      sizeof(armourPieces[0]));
}
...
static double getJewelryWeight() {
  static const SLOT_EQUIPMENT::TSlotEquipment jewelryPieces[] = {
      SLOT_EQUIPMENT::HEADDRESS, SLOT_EQUIPMENT::EARL,
      SLOT_EQUIPMENT::EARR,      SLOT_EQUIPMENT::NECKLACE,
      SLOT_EQUIPMENT::WRISTL,    SLOT_EQUIPMENT::WRISTR,
      SLOT_EQUIPMENT::FINGERL,   SLOT_EQUIPMENT::FINGERR,
      SLOT_EQUIPMENT::ANKLEL,    SLOT_EQUIPMENT::ANKLER,
  };
  return getWeightOfEquipmentPieces(
      jewelryPieces, sizeof(jewelryPieces) / sizeof(jewelryPieces[0]));
}
...
{
        CInterfaceManager *im = CInterfaceManager::getInstance();
        uint numSheaths = (uint)
NLGUI::CDBManager::getInstance()->getDbProp("LOCAL:INVENTORY:NB_SHEATH")->getValue32();
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// "getInventoryWeight()" : Returns the weight of the whole inventory, not
// including beasts'inventory of course  //
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/*
...
static double getBagsWeight()
{
        CInterfaceManager *im = CInterfaceManager::getInstance();
        uint numBags = (uint)
NLGUI::CDBManager::getInstance()->getDbProp("LOCAL:INVENTORY:NB_BAG")->getValue32();
...
{
        result.setDouble(getJewelryWeight() + getArmoursWeight() +
getSeedsWeight() + getBagsWeight() + getSheathsWeight()); return true;
}
...
static DECLARE_INTERFACE_USER_FCT(getBranchSheetCategory) {
  ...
  CInterfaceManager *im = CInterfaceManager::getInstance();
  CCDBNodeBranch *branch =
      NLGUI::CDBManager::getInstance()->getDbBranch(args[0].getString());
  if (!branch) {
  ...
  for (uint index = (uint)args[1].getInteger();
       index < (uint)args[1].getInteger() + (uint)args[2].getInteger();
       ++index) {
    ICDBNode *node = branch->getNode(index);
    if (node) {
      CCDBNodeLeaf *sheetNode = dynamic_cast<CCDBNodeLeaf *>(
          node->getNode(ICDBNode::CTextId("SHEET")));
      CCDBNodeLeaf *slotTypeNode = dynamic_cast<CCDBNodeLeaf *>(
          node->getNode(ICDBNode::CTextId("SLOT_TYPE")));
      if (sheetNode) {
...
static DECLARE_INTERFACE_USER_FCT(getItemsBulkUserFct) {
  if (args.size() != 3 || !args[0].toString() || !args[1].toInteger() ||
      !args[2].toInteger()) {
    nlwarning("<getItemBulk> 3 arguments expected : (dbPath, startItemIndex, "
              "numItems)");
    return false;
  }
  const std::string &basePath = args[0].getString();
  ...
  uint16 numItems = (uint16)args[2].getInteger();
  result.setDouble(
      CInventoryManager::getBranchBulk(basePath, startItemIndex, numItems));
  return true;
}
