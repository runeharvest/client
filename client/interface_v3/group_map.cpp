...
////////////
NLMISC::CRGBA
    CUserLandMark::_LandMarksColor[CUserLandMark::UserLandMarkTypeCount];

const uint32 ISLAND_PIXEL_PER_METER = 2;

static void setupFromZoom(CViewBase *pVB, CContLandMark::TContLMType t,
...
static void popupLandMarkNameDialog() {
  ...
  CDBGroupComboBox *cb =
      dynamic_cast<CDBGroupComboBox *>(gc->getGroup("landmarktypes"));
  cb->sortText();

  if (LastSelectedLandMark) {
    CGroupMap *map =
        dynamic_cast<CGroupMap *>(LastSelectedLandMark->getParent());
    if (!map)
      return;
    ...
    NLGUI::CDBManager::getInstance()
        ->getDbProp("UI:TEMP:LANDMARKTYPE")
        ->setValue8(cb->getTextPos(userLM.Type));
    eb->setInputString(userLM.Title.toUtf8());
  } else {
    NLGUI::CDBManager::getInstance()
        ->getDbProp("UI:TEMP:LANDMARKTYPE")
        ->setValue8(cb->getTextPos(CUserLandMark::Misc));
...
//============================================================================================================
CGroupMap::CPolyButton::CPolyButton() : CCtrlBase(TCtorParam()) {
  if (Driver != NULL) {
...
bool CGroupMap::CPolyButton::handleEvent(const NLGUI::CEventDescriptor &event) {
  ...
  if (event.getType() == NLGUI::CEventDescriptor::mouse) {
    CInterfaceManager *im = CInterfaceManager::getInstance();
    const NLGUI::CEventDescriptorMouse &eventDesc =
        (const NLGUI::CEventDescriptorMouse &)event;

    if (eventDesc.getEventTypeExtended() ==
        NLGUI::CEventDescriptorMouse::mouseleftup) {
      if (CWidgetManager::getInstance()->getCapturePointerLeft() != this)
        return false;

      // Set the map !!!
      if (contains(
              CVector2f((float)eventDesc.getX(), (float)eventDesc.getY()))) {
        bool bFound = false;
        uint32 i;
    ...
    if (eventDesc.getEventTypeExtended() ==
        NLGUI::CEventDescriptorMouse::mouseleftdown) {
      if (contains(
              CVector2f((float)eventDesc.getX(), (float)eventDesc.getY()))) {
...
void CGroupMap::CPolyButton::drawPolyButton() {
  ...
  bool bOver = false;
  const std::vector<CCtrlBase *> &rCUP =
  ...
  */
  ...
  if (bOver)
    LineMat.setColor(
        CRGBA(255, 255, 255, 64)); // LineMat.setColor(CRGBA(165,38,6,128));
  else
...
//============================================================================================================
bool CGroupMap::CPolyButton::build(const NLLIGO::CPrimZone &concavePoly,
                                   CGroupMap *pMap, const string &sID) {
  static int gStat = 0;
  ...
  for (uint32 i = 0; i < concavePoly.VPoints.size(); ++i)
    p.Vertices.push_back(
        CVector(concavePoly.VPoints[i].x, concavePoly.VPoints[i].y, 0));
    ...
    while (it != outputPolygons.end()) {
      ...
      for (uint32 i = 0; i < it->Vertices.size(); ++i)
        poly.Vertices.push_back(
            CVector2f(it->Vertices[i].x, it->Vertices[i].y));
...
CGroupMap::CGroupMap(const TCtorParam &param) : CInterfaceGroup(param) {
  _Offset.x = 0.f;
  _Offset.y = 0.f;
  _WorldOffset.x = 0.f;
  _WorldOffset.y = 0.f;
  ...
  _WorldSheet =
      dynamic_cast<CWorldSheet *>(SheetMngr.get(CSheetId("ryzom.world")));
  _CurMap = NULL;
  _CurContinent = NULL;
  _Panning = false;
  _HasMoved = false;
  ...
  _MissionTargetTextIDs.resize(2 * MAX_NUM_MISSIONS * MAX_NUM_MISSION_TARGETS,
                               0);
  _MissionTargetTextReceived.resize(
      2 * MAX_NUM_MISSIONS * MAX_NUM_MISSION_TARGETS, false);
 ...
 */
static void loadLandmarkInfo(xmlNodePtr cur, const std::string &prefix,
                             CLandMarkOptions &dest) {
  CXMLAutoPtr ptr;
  ptr = (char *)xmlGetProp(
      cur, (xmlChar *)(prefix + "_landmark_tex_normal").c_str());
  if (ptr)
    dest.LandMarkTexNormal = (const char *)ptr;
  ptr = (char *)xmlGetProp(cur,
                           (xmlChar *)(prefix + "_landmark_tex_over").c_str());
  if (ptr)
    dest.LandMarkTexOver = (const char *)ptr;
  ptr = (char *)xmlGetProp(
      cur, (xmlChar *)(prefix + "_landmark_tex_pushed").c_str());
  if (ptr)
    dest.LandMarkTexPushed = (const char *)ptr;
  ptr = (char *)xmlGetProp(cur, (xmlChar *)(prefix + "_landmark_menu").c_str());
  if (ptr)
    dest.LandMarkMenu = (const char *)ptr;
  ptr = (char *)xmlGetProp(
      cur, (xmlChar *)(prefix + "_landmark_color_normal").c_str());
  if (ptr)
    dest.ColorNormal = CCtrlBase::convertColor(ptr);
  ptr = (char *)xmlGetProp(
      cur, (xmlChar *)(prefix + "_landmark_color_over").c_str());
  if (ptr)
    dest.ColorOver = CCtrlBase::convertColor(ptr);
  ptr = (char *)xmlGetProp(
      cur, (xmlChar *)(prefix + "_landmark_color_pushed").c_str());
  if (ptr)
    dest.ColorPushed = CCtrlBase::convertColor(ptr);
}
...
bool CGroupMap::parse(xmlNodePtr cur, CInterfaceGroup *parentGroup) {
  ...
  if (ptr)
    fromString((const char *)ptr, _MinH);
  ptr = (char *)xmlGetProp(cur, (xmlChar *)"max_h");
  if (ptr)
    fromString((const char *)ptr, _MaxH);
  ...
  if (Driver) {
    if (_MapMaterial.empty()) {
      ...
      if (!_MapMaterial.empty()) {
        ...
        _MapMaterial.texEnvArg0RGB(0, NL3D::UMaterial::Texture,
                                   NL3D::UMaterial::SrcColor);
        _MapMaterial.texEnvArg1RGB(0, NL3D::UMaterial::Diffuse,
                                   NL3D::UMaterial::SrcColor);
        _MapMaterial.texEnvOpAlpha(0, NL3D::UMaterial::Modulate);
        _MapMaterial.texEnvArg0Alpha(0, NL3D::UMaterial::Texture,
                                     NL3D::UMaterial::SrcAlpha);
        _MapMaterial.texEnvArg1Alpha(0, NL3D::UMaterial::Diffuse,
                                     NL3D::UMaterial::SrcAlpha);
        // Setup stage 1
        _MapMaterial.texEnvOpRGB(1, NL3D::UMaterial::Modulate);
        _MapMaterial.texEnvArg0RGB(1, NL3D::UMaterial::Previous,
                                   NL3D::UMaterial::SrcColor);
        _MapMaterial.texEnvArg1RGB(1, NL3D::UMaterial::Texture,
                                   NL3D::UMaterial::SrcAlpha);
        _MapMaterial.texEnvOpAlpha(1, NL3D::UMaterial::Replace);
        _MapMaterial.texEnvArg0Alpha(1, NL3D::UMaterial::Previous,
                                     NL3D::UMaterial::SrcAlpha);
      }
    }
    if (_PlayerPosMaterial.empty()) {
  ...
  case EGSPD::CPeople::Fyros:
    loadLandmarkInfo(cur, "home_fyros", _HomeLMOptions);
    break;
  case EGSPD::CPeople::Matis:
    loadLandmarkInfo(cur, "home_matis", _HomeLMOptions);
    break;
  case EGSPD::CPeople::Zorai:
    loadLandmarkInfo(cur, "home_zorai", _HomeLMOptions);
    break;
  case EGSPD::CPeople::Tryker:
    loadLandmarkInfo(cur, "home_tryker", _HomeLMOptions);
    break;
  default:
    break;
  }
  ...
  if (_MapMode == MapMode_Normal) {
    ...
    for (uint i = 0; i < _AnimalLM.size(); i++) {
      ...
      if (_AnimalLM[i]) {
        ...
        addCtrl(_AnimalLM[i]);
        _AnimalLM[i]->setDefaultContextHelp(
            NLMISC::CI18N::get(NLMISC::toString("uiPATitleMount%d", i + 1)));
      }
    }
    ...
    for (uint i = 0; i < MaxNumPeopleInTeam; i++) {
      ...
      if (_TeammateLM[i]) {
        ...
        _TeammateLM[i]->setDefaultContextHelp(
            NLMISC::CI18N::get(NLMISC::toString("uittLMTeam%d", i)));
      }
    }
  }
  ...
  if ((_MapMode == MapMode_Death) || (_MapMode == MapMode_SpawnSquad)) {
    ...
    // CCtrlBaseButton *pCB =
    // dynamic_cast<CCtrlBaseButton*>(CWidgetManager::getInstance()->getElementFromId(_RespawnButton));
  ...
  try {
    ...
    const R2::CScenarioEntryPoints::TCompleteIslands &completeIslands =
...
//============================================================================================================
void CGroupMap::setSelectionAxis(
    bool active, const NLMISC::CVector2f &pos /*=NLMISC::CVector2f::Null*/) {
...
//============================================================================================================
bool CGroupMap::getCtrlsUnder(sint32 x, sint32 y, sint32 clipX, sint32 clipY,
                              sint32 clipW, sint32 clipH,
                              std::vector<CCtrlBase *> &vICL) {
  if (!((x >= _XReal) && (x < (_XReal + _WReal)) && (y > _YReal) &&
        (y <= (_YReal + _HReal))))
    return false;
  // test against current clip
  computeCurrentClipContribution(clipX, clipY, clipW, clipH, clipX, clipY,
                                 clipW, clipH);
...
void CGroupMap::updateCoords() {
  updateSelectionAxisSize();
  //
  CGroupContainer *enclosingContainer =
      static_cast<CGroupContainer *>(getEnclosingContainer());

  if (!enclosingContainer || !enclosingContainer->getActive())
    return;
  ...
  if (_MapMode == MapMode_Normal) {
    ...
    for (i = 0; i < _ContinentText.size(); ++i) {
      if (_ContinentText[i]->SearchMatch)
        ...
        setupFromZoom(_ContinentText[i], _ContinentText[i]->Type,
                      _MeterPerPixel);
    }
    //
    ...
    // target
    if (_TargetLM && _TargetLM->getActive())
      updateButtonPos(*_TargetLM);
    // home
    if (_HomeLM && _HomeLM->getActive())
      updateButtonPos(*_HomeLM);
    // animals
  ...
  //
  CInterfaceGroup::updateCoords(); // must update coords twice : first is to get
                                   // map size and meter per pixel -> this allow
                                   // to know which landmark are visible
                                   // second is to update (possibly newly
                                   // visible) landmarks position
}

//============================================================================================================
void CGroupMap::worldToMap(NLMISC::CVector2f &dest,
                           const NLMISC::CVector2f &src) const {
  dest.x = _MapMinCorner.x != _MapMaxCorner.x
               ? (src.x - _MapMinCorner.x) / (_MapMaxCorner.x - _MapMinCorner.x)
               : _MapMinCorner.x;
  dest.y = _MapMinCorner.y != _MapMaxCorner.y
               ? 1.f - (src.y - _MapMinCorner.y) /
                           (_MapMaxCorner.y - _MapMinCorner.y)
               : 1.f - _MapMinCorner.y;
}

//============================================================================================================
void CGroupMap::mapToWorld(NLMISC::CVector2f &dest,
                           const NLMISC::CVector2f &src) const {
  dest.x = src.x * (_MapMaxCorner.x - _MapMinCorner.x) + _MapMinCorner.x;
  dest.y =
      (1.f - src.y) * (_MapMaxCorner.y - _MapMinCorner.y) + _MapMinCorner.y;
}

/*
//============================================================================================================
void CGroupMap::mapToScreen(sint32 &px, sint32 &py, const NLMISC::CVector2f
&src) const
{
        px = (sint32) (_XReal + (src.x - (_PlayerPos.x + _Offset.x)) * _MapTexW
* _Scale); py = (sint32) (_YReal + _HReal - (src.y - (_PlayerPos.y + _Offset.y))
* _MapTexH * _Scale);
}

//============================================================================================================
void CGroupMap::mapToWindow(sint32 &px, sint32 &py, const NLMISC::CVector2f
&src) const
{
        px = (sint32) ((src.x - (_PlayerPos.x + _Offset.x)) * _MapTexW *
_Scale); py = (sint32) (_HReal - (src.y - (_PlayerPos.y + _Offset.y)) * _MapTexH
* _Scale);
}
*/

//============================================================================================================
void CGroupMap::worldToWindow(NLMISC::CVector2f &dest,
                              const NLMISC::CVector2f &src) const {
  worldToMap(dest, src);
...
//============================================================================================================
void CGroupMap::worldToWindowSnapped(sint32 &px, sint32 &py,
                                     const NLMISC::CVector2f &src) const {
  NLMISC::CVector2f snappedPos;
  snappedPos.x = _WorldToMapDeltaX != 0.f
                     ? (src.x - fmodf(src.x, _WorldToMapDeltaX))
                     : src.x;
  snappedPos.y = _WorldToMapDeltaY != 0.f
                     ? (src.y - fmodf(src.y, _WorldToMapDeltaY))
                     : src.y;
...
//============================================================================================================
void CGroupMap::mapToWindowSnapped(sint32 &px, sint32 &py,
                                   const NLMISC::CVector2f &src) const {
  NLMISC::CVector2f snappedPos;
  snappedPos.x = _WorldToMapDeltaX != 0.f
                     ? (src.x - fmodf(src.x, _WorldToMapDeltaX))
...
//============================================================================================================
void CGroupMap::mapToScreen(sint32 &px, sint32 &py,
                            const NLMISC::CVector2f &src) const {
  px = (sint32)(_XReal + _MapX +
                (src.x - (_PlayerPos.x + _Offset.x)) * _MapTexW * _Scale);
  py = (sint32)(_YReal + _MapY + _HReal -
                (src.y - (_PlayerPos.y + _Offset.y)) * _MapTexH * _Scale);
}

//============================================================================================================
void CGroupMap::windowToScreen(sint32 &destX, sint32 &destY, sint32 srcX,
                               sint32 srcY) const {
  destX = srcX + _XReal;
  destY = srcY + _YReal;
}
...
//============================================================================================================
void CGroupMap::mapToWindow(sint32 &px, sint32 &py,
                            const NLMISC::CVector2f &src) const {
...
//============================================================================================================
void CGroupMap::mapToWindow(NLMISC::CVector2f &dest,
                            const NLMISC::CVector2f &src) const {
  dest.x =
      ((src.x - (_PlayerPos.x + _Offset.x)) * _MapTexW * _Scale) + (float)_MapX;
  dest.y = (sint32)(_HReal -
                    (src.y - (_PlayerPos.y + _Offset.y)) * _MapTexH * _Scale) +
           (float)_MapY;
}

//============================================================================================================
void CGroupMap::screenToMap(NLMISC::CVector2f &dest, sint32 px,
                            sint32 py) const {
  ...
  dest.x =
      (px - (_XReal + _MapX)) / (_MapTexW * _Scale) + _PlayerPos.x + _Offset.x;
  dest.y = (_YReal + _HReal + _MapY - py) / (_MapTexH * _Scale) + _PlayerPos.y +
           _Offset.y;
}

//============================================================================================================
void CGroupMap::windowToMap(NLMISC::CVector2f &dest, sint32 px,
                            sint32 py) const {
  if (_MapTexW == 0 || _MapTexH == 0) {
    dest.set(0.f, 0.f);
    return;
  ...
  dest.x =
      ((px + 0.5f) - _MapX) / (_MapTexW * _Scale) + _PlayerPos.x + _Offset.x;
  dest.y = (_HReal + _MapY - (py - 0.5f)) / (_MapTexH * _Scale) + _PlayerPos.y +
           _Offset.y;
}

//============================================================================================================
void CGroupMap::updateLMPosFromDBPos(CLandMarkButton *dest, sint32 px,
                                     sint32 py) {
  nlassert(dest);
  if (px == 0 && py == 0) {
...
static CSmartPtr<CNamedEntityPositionState>
buildMissionPositionState(CInterfaceManager *im, const std::string &baseDBpath,
                          uint missionIndex, uint targetIndex) {
  nlassert(im);
  CCDBNodeLeaf *name = NLGUI::CDBManager::getInstance()->getDbProp(
      baseDBpath + NLMISC::toString(":%d:TARGET%d:TITLE", (int)missionIndex,
                                    (int)targetIndex));
  CCDBNodeLeaf *x = NLGUI::CDBManager::getInstance()->getDbProp(
      baseDBpath +
      NLMISC::toString(":%d:TARGET%d:X", (int)missionIndex, (int)targetIndex));
  CCDBNodeLeaf *y = NLGUI::CDBManager::getInstance()->getDbProp(
      baseDBpath +
      NLMISC::toString(":%d:TARGET%d:Y", (int)missionIndex, (int)targetIndex));
...
void CGroupMap::checkCoords() {
  ...
  {
    _MissionPosStates.resize(
        2 * MAX_NUM_MISSIONS *
        MAX_NUM_MISSION_TARGETS); // 2 is because of group missions
    uint targetIndex = 0;
    for (uint k = 0; k < MAX_NUM_MISSIONS; ++k) {
      for (uint l = 0; l < MAX_NUM_MISSION_TARGETS; ++l) {
        _MissionPosStates[targetIndex++] =
            buildMissionPositionState(pIM, MISSIONS_DB_PATH, k, l);
        _MissionPosStates[targetIndex++] =
            buildMissionPositionState(pIM, GROUP_MISSIONS_DB_PATH, k, l);
      }
    }
    // also fill ptr for special landmarks (target, home & respawn)
    _TargetPos =
        NLGUI::CDBManager::getInstance()->getDbProp(COMPASS_DB_PATH ":TARGET");
    _HomePos = NLGUI::CDBManager::getInstance()->getDbProp(COMPASS_DB_PATH
                                                           ":HOME_POINT");
  }
  //
  ...
  for (uint k = 0; k < 2 * MAX_NUM_MISSIONS * MAX_NUM_MISSION_TARGETS; ++k) {
    ...
    if (_MissionLM[k]) {
      ...
      if (!_MissionTargetTextReceived[k]) {
        string result;
        if (STRING_MANAGER::CStringManagerClient::instance()->getDynString(
                _MissionTargetTextIDs[k], result)) {
  ...
  if (_TargetLM) {
    ...
    if (_TargetLM->getActive()) {
      if (UserEntity) {
        if (UserEntity->selection() != CLFECOMMON::INVALID_SLOT &&
            UserEntity->selection() != 0) {
          CEntityCL *sel = EntitiesMngr.entity(UserEntity->selection());
          if (sel) {
            _TargetLM->setDefaultContextHelp(
                NLMISC::CI18N::get("uiTargetTwoPoint") +
                sel->removeTitleAndShardFromName(sel->getEntityName()));
          }
        }
      }
    }
  ...
  for (i = 0; i < _RespawnPos.size(); ++i) {
    if (_RespawnLM[i] == NULL) {
      ...
      if (_MapMode == MapMode_SpawnSquad)
        _RespawnLM[i]->setDefaultContextHelp(
            NLMISC::CI18N::get("uiSquadSpawnPoint") +
            NLMISC::toString(" %u", i + 1));
      else {
        if (isIsland()) {
          _RespawnLM[i]->setDefaultContextHelp(
              NLMISC::CI18N::get("uiR2EntryPoint"));
        } else {
          _RespawnLM[i]->setDefaultContextHelp(
              NLMISC::CI18N::get("uiRespawnPoint"));
        }
        _RespawnLM[i]->HandleEvents =
  ...
  if ((_MapMode == MapMode_Death) || (_MapMode == MapMode_SpawnSquad)) {
    if (_RespawnPosReseted) {
      ...
      if (_MapMode == MapMode_Death)
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:SAVE:RESPAWN_PT")
            ->setValue32(_RespawnSelected);
      else if (_MapMode == MapMode_SpawnSquad)
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:TEMP:OUTPOST:SQUAD_RESPAWN_PT")
            ->setValue32(_RespawnSelected);
      if (!_RespawnLM.empty())
  ...
  if (_AnimalPosStates.empty() && !_AnimalLM.empty()) {
    ...
    for (uint i = 0; i < MAX_INVENTORY_ANIMAL; i++) {
      ...
      _AnimalPosStates[i]->build(
          NLMISC::toString("SERVER:PACK_ANIMAL:BEAST%d", i));
    }
  }
  ...
  for (i = 0; i < _AnimalLM.size(); i++) {
    if (_AnimalLM[i]) {
      ...
      if (_IsIsland) {
        ...
        CCDBNodeLeaf *statusNode = NLGUI::CDBManager::getInstance()->getDbProp(
            toString("SERVER:PACK_ANIMAL:BEAST%d", i) + ":STATUS", false);
        if (statusNode &&
            ANIMAL_STATUS::isInStable(
                (ANIMAL_STATUS::EAnimalStatus)statusNode->getValue32())) {
          _AnimalLM[i]->setTexture(_AnimalStableLMOptions.LandMarkTexNormal);
          _AnimalLM[i]->setScale(
              true); // hardcoded because chosen icon is too big (bad...)
        } else if (statusNode &&
                   ANIMAL_STATUS::isDead((ANIMAL_STATUS::EAnimalStatus)
                                             statusNode->getValue32())) {
        ...
        at = (ANIMAL_TYPE::EAnimalType)NLGUI::CDBManager::getInstance()
                 ->getDbProp("SERVER:PACK_ANIMAL:BEAST" + toString(i) + ":TYPE")
                 ->getValue32();
        string sPrefix;
        switch (at) {
        default:
        case ANIMAL_TYPE::All:
          sPrefix = "uiPATitleMount";
          break;
        case ANIMAL_TYPE::Mount:
          sPrefix = "uiPATitleMount";
          break;
        case ANIMAL_TYPE::Packer:
          sPrefix = "uiPATitlePacker";
          break;
        case ANIMAL_TYPE::Demon:
          sPrefix = "uiPATitleDemon";
          break;
        }
        _AnimalLM[i]->setDefaultContextHelp(
            NLMISC::CI18N::get(sPrefix + toString(i + 1)));
      }
    }
  }
  ...
  for (i = 0; i < _TeammateLM.size(); i++) {
    ...
    if (_TeammateLM[i]) {
      ...
      if (_TeammatePosStates[i]->getPos(px, py)) {
        ...
        uint32 val =
            NLGUI::CDBManager::getInstance()
                ->getDbProp(NLMISC::toString("SERVER:GROUP:%d:NAME", i))
                ->getValue32();
  ...
  // TODO : try to factor behaviour between anim / treamate / missions (they are
  // all dynamic)
...
static void setupFromZoom(CViewBase *pVB, CContLandMark::TContLMType t,
  ...
  bool active = false;
  // Icon
  if ((t == CContLandMark::Capital) && (fMeterPerPixel <= 5.0f))
    active = true;
  if ((t == CContLandMark::Village) && (fMeterPerPixel <= 4.0f))
    active = true;
  if ((t == CContLandMark::Outpost) && (fMeterPerPixel <= 4.0f))
    active = true;
  if ((t == CContLandMark::Stable) && (fMeterPerPixel <= 3.5f))
    active = true;
  // Just a text
  if ((t == CContLandMark::Region) && (fMeterPerPixel <= 50.0f))
    active = true;
  if ((t == CContLandMark::Place) && (fMeterPerPixel <= 6.0f))
    active = true;
  if ((t == CContLandMark::Street) && (fMeterPerPixel <= 2.0f))
    active = true;
  if (t == CContLandMark::Unknown)
    active = true;
  pVB->setActive(active);
}

//============================================================================================================
void CGroupMap::computeUVRect(float &minU, float &minV, float &maxU,
                              float &maxV) const {
  ...
  // ensure that UV displacement results in moves pixel by pixel on screen
  // (landmarks move pixel by pixel, so the map may appear not to follow them
  // because it moves smoothly)
...
void CGroupMap::draw() {
  ...
  if (!_MapLoadFailure) {
    if (_MapTexW != 0 && _MapTexH != 0) {
      ...
      if (_CurContinent != NULL) {
        ...
        if (rFOW.getMaxX() != rFOW.getMinX())
          fowMin.x =
              (worldMin.x - rFOW.getMinX()) / (rFOW.getMaxX() - rFOW.getMinX());
        if (rFOW.getMaxY() != rFOW.getMinY())
          fowMin.y =
              (worldMin.y - rFOW.getMinY()) / (rFOW.getMaxY() - rFOW.getMinY());
        if (rFOW.getMaxX() != rFOW.getMinX())
          fowMax.x =
              (worldMax.x - rFOW.getMinX()) / (rFOW.getMaxX() - rFOW.getMinX());
        if (rFOW.getMaxY() != rFOW.getMinY())
          fowMax.y =
              (worldMax.y - rFOW.getMinY()) / (rFOW.getMaxY() - rFOW.getMinY());
        if ((fowMin.x >= 0) && (fowMin.x <= 1) && (fowMin.y >= 0) &&
            (fowMin.y <= 1) && (fowMax.x >= 0) && (fowMax.x <= 1) &&
            (fowMax.y >= 0) && (fowMax.y <= 1)) {
          fowURatio =
              (float)rFOW.getMapWidth() / favoid0((float)rFOW.getRealWidth());
          fowVRatio =
              (float)rFOW.getMapHeight() / favoid0((float)rFOW.getRealHeight());
        }
      }

      vr.drawCustom(_XReal + std::max(_MapX, (sint32)0),
                    _YReal - std::min(_MapY, (sint32)0), mapW, mapH,
                    CUV(minU * _URatio, minV * _VRatio),
                    CUV(maxU * _URatio, maxV * _VRatio),
                    CUV(fowMin.x * fowURatio, fowMin.y * fowVRatio),
                    CUV(fowMax.x * fowURatio, fowMax.y * fowVRatio),
                    CRGBA(255, 255, 255, alpha), _MapMaterial);

      // Look if we want to display some details info
      // Show / Hide details with the ratio indicating the meter per pixel of
      // screen
      /*
      {
              ...
              uint32 i;
              for (i = 0; i < _ContinentLM.size(); ++i)
                      setupFromZoom(_ContinentLM[i], _ContinentLM[i]->Type,
      fMeterPerPixel); for (i = 0; i < _ContinentText.size(); ++i)
                      setupFromZoom(_ContinentText[i], _ContinentText[i]->Type,
      fMeterPerPixel);
      }
      */
      ...
      /* if (_MapW < _WReal || _MapH < _HReal)
              vr.drawWiredQuad(_XReal + _MapX, _YReal - _MapY, _MapW, _MapH,
         CRGBA(0, 0, 0, alpha));*/
    }
  }

  // if editor is in edition mode, draw the frustum instead (for better
  // visibility)
  CQuad fruQuad;
  CTriangle fruTri;
    ...
    if (R2::getEditor().getMode() == R2::CEditor::EditionMode) {
      ...
      if (wantFrustum) {
        ...
        if (CWidgetManager::getInstance()->getPointer()) {
          ...
          CVector mousePos(
              (float)CWidgetManager::getInstance()->getPointer()->getXReal(),
              (float)CWidgetManager::getInstance()->getPointer()->getYReal(),
              0.f);
          mousePos -= delta;
          CVector dummyHit;
          float deltaBlend = DT / (0.001f * (float)_FrustumViewBlendTimeInMs);
          bool hit =
              fruTri.intersect(mousePos + CVector::K, mousePos - CVector::K,
                               dummyHit, CPlane(0.f, 0.f, 0.f, 1.f));
          NLMISC::incrementalBlend(_FrustumOverBlendFactor, hit ? 1.f : 0.f,
                                   deltaBlend);
          fruColor.blendFromui(_FrustumViewColor, _FrustumViewColorOver,
                               (uint8)(255 * _FrustumOverBlendFactor));
        } else {
  ...
  if (R2::getEditor().getMode() != R2::CEditor::EditionMode) {
    ...
    if (!_PlayerPosLoadFailure) {
      ...
      quv.V0 = center - 0.5f * (float)_PlayerPosTexW * right -
               0.5f * (float)_PlayerPosTexH * front;
      quv.V1 = center + 0.5f * (float)_PlayerPosTexW * right -
               0.5f * (float)_PlayerPosTexH * front;
      quv.V2 = center + 0.5f * (float)_PlayerPosTexW * right +
               0.5f * (float)_PlayerPosTexH * front;
      quv.V3 = center - 0.5f * (float)_PlayerPosTexW * right +
               0.5f * (float)_PlayerPosTexH * front;
      quv.Uv0.set(0.f, 1.f);
      quv.Uv1.set(1.f, 1.f);
      quv.Uv2.set(1.f, 0.f);
      quv.Uv3.set(0.f, 0.f);
      quv.Color0 = quv.Color1 = quv.Color2 = quv.Color3 =
    ...
    if (R2::getEditor().getMode() == R2::CEditor::EditionMode) {
      if (_FrustumMaterial.empty()) {
        ...
        if (!_FrustumMaterial.empty()) {
          ...
          _FrustumMaterial.setBlendFunc(NL3D::UMaterial::srcalpha,
      ...
      if (!_FrustumMaterial.empty()) {
        ...
        //
        CTriangle fruTri;
        fruTri.V0.set((fruQuad.V0.x + _XReal) / sw,
                      (fruQuad.V0.y + _YReal) / sh, 0.f);
        fruTri.V1.set((fruQuad.V1.x + _XReal) / sw,
                      (fruQuad.V1.y + _YReal) / sh, 0.f);
        fruTri.V2.set((fruQuad.V3.x + _XReal) / sw,
                      (fruQuad.V3.y + _YReal) / sh, 0.f);
...
bool CGroupMap::handleEvent(const NLGUI::CEventDescriptor &event) {
  ...
  if (ClientCfg.R2EDEnabled && R2::getEditor().getCurrentTool()) {
    ...
    if (event.getType() == NLGUI::CEventDescriptor::mouse) {
      const NLGUI::CEventDescriptorMouse &eventDesc =
          (const NLGUI::CEventDescriptorMouse &)event;
      panEnd = eventDesc.getEventTypeExtended() ==
                   NLGUI::CEventDescriptorMouse::mouseleftup &&
               _Panning && _HasMoved;
      if (eventDesc.getEventTypeExtended() ==
              NLGUI::CEventDescriptorMouse::mouseleftup &&
        ...
        //  example : clicking in the palette window and doing a drag-and-drop
        //  to the minimap
        {
        ...
        if (!handled && R2::getEditor().getMode() == R2::CEditor::EditionMode) {
          if (R2::getEditor().getCurrentTool()) {
            if (!R2::getEditor()
                     .getCurrentTool()
                     ->getPreviousToolClickEndFlag()) {
              if (CWidgetManager::getInstance()->getCapturePointerLeft() ==
                  this) {
                // unselected unless tool has been changed before last mouse
                // left up (happens when one's finish a route using double click
                // -> should not unselect then)
                R2::getEditor().setSelectedInstance(NULL);
              }
            }
          }
        }
      } else if (eventDesc.getEventTypeExtended() ==
                 NLGUI::CEventDescriptorMouse::mouserightup) {
        if (CWidgetManager::getInstance()->getCapturePointerRight() == this) {
          if (isIn(eventDesc.getX(), eventDesc.getY())) {
            ...
            if (clickPos.x >= 0.f && clickPos.y >= 0.f && clickPos.x <= 1.f &&
                clickPos.y <= 1.f) {
              handled =
  ...
  if (event.getType() == NLGUI::CEventDescriptor::mouse) {
    ...
    if (eventDesc.getEventTypeExtended() ==
        NLGUI::CEventDescriptorMouse::mouserightdown) {
    ...
    if (eventDesc.getEventTypeExtended() ==
      ...
      if (isIn(eventDesc.getX(), eventDesc.getY())) {
        ...
        // Clamp the start WorldOffset on the Move start. NB: not clamped on a
        // scale
        computeOffsets();
        _StartWorldOffsetForPaning.x =
            _Offset.x * (_MapMaxCorner.x - _MapMinCorner.x);
        _StartWorldOffsetForPaning.y =
            _Offset.y * (_MapMaxCorner.y - _MapMinCorner.y);

        _PanStartDateInMs = T1;
        /** If in editor, and current tool is a creation tool, only handle
         * panning after a small delta pos / delta time for better ergonomy
         */
        if (ClientCfg.R2EDEnabled && R2::getEditor().getCurrentTool() &&
    ...
    if (eventDesc.getEventTypeExtended() ==
        NLGUI::CEventDescriptorMouse::mousemove) {
      if (CWidgetManager::getInstance()->getCapturePointerLeft() != this ||
          !_Panning)
      ...
      if (_MapTexW != 0 && _MapTexH != 0) {
        if (_HasMoved || (T1 - _PanStartDateInMs) > _DeltaTimeBeforePanInMs ||
            (uint)abs(eventDesc.getX() - _StartXForPaning) >
                _DeltaPosBeforePan ||
            (uint)abs(eventDesc.getY() - _StartYForPaning) >
                _DeltaPosBeforePan) {
          _HasMoved = true;
          NLMISC::CVector2f dWorld;
          dWorld.x = -(_MapMaxCorner.x - _MapMinCorner.x) *
                     (eventDesc.getX() - _StartXForPaning) /
                     (_MapTexW * _Scale);
          dWorld.y = +(_MapMaxCorner.y - _MapMinCorner.y) *
                     (eventDesc.getY() - _StartYForPaning) /
                     (_MapTexH * _Scale);
    ...
    if (eventDesc.getEventTypeExtended() ==
            NLGUI::CEventDescriptorMouse::mousewheel &&
        !_Panning) {
  ...
  if (event.getType() == NLGUI::CEventDescriptor::system) {
    ...
    if (es.getEventTypeExtended() ==
        NLGUI::CEventDescriptorSystem::activecalledonparent) {
...
void CGroupMap::pan(sint32 dx, sint32 dy) {
  if (_MapTexW * _Scale != 0.f)
    _WorldOffset.x +=
        dx * (_MapMaxCorner.x - _MapMinCorner.x) / (_MapTexW * _Scale);
  if (_MapTexH * _Scale != 0.f)
    _WorldOffset.y -=
        dy * (_MapMaxCorner.y - _MapMinCorner.y) / (_MapTexH * _Scale);
  computeOffsets();
  _WorldOffset.x = _Offset.x * (_MapMaxCorner.x - _MapMinCorner.x);
  _WorldOffset.y = _Offset.y * (_MapMaxCorner.y - _MapMinCorner.y);
  invalidateCoords();
...
void CGroupMap::loadMap() {
      ...
      if (game.callMethodByNameNoThrow("onLoadMap", numArg, numResult)) {
        if (ls->isString(1)) {
          if (!NLMISC::CPath::lookup(ls->toString(1), false, false).empty())
            ...
            nlwarning("Custom map texture not found '%s' for map '%s'",
                      ls->toString(1), _MapTexture.c_str());
        }
      }
    }
  ...
  if (_MapTF) {
    ...
    if (_IsIsland) {
      ...
      _MapTexW =
          (uint32)(ISLAND_PIXEL_PER_METER * (_CurMap->MaxX - _CurMap->MinX));
      _MapTexH =
          (uint32)(ISLAND_PIXEL_PER_METER * (_CurMap->MaxY - _CurMap->MinY));
      if (_MapTexW > w) {
        nlwarning("Island real width > texture width, snapshot not up-to-date "
                  "? (island = %s)",
                  _CurMap->Name.c_str());
        _MapTexW = w;
      }
      if (_MapTexH > h) {
        nlwarning("Island real height > texture height, snapshot not "
                  "up-to-date ? (island = %s)",
                  _CurMap->Name.c_str());
        _MapTexH = h;
      }
...
void CGroupMap::unloadMap() {
  ...
  if (_MapTF)
    Driver->deleteTextureFile(_MapTF);
  _MapTF = NULL;
  if (_PlayerPosTF)
    Driver->deleteTextureFile(_PlayerPosTF);
  _PlayerPosTF = NULL;
...
//============================================================================================================
void CGroupMap::updateContinentInfo() { return; }

//============================================================================================================
void CGroupMap::setMap(SMap *map) {
  nlassert(map);
  if ((_CurMap != NULL) && (_CurMap->Name == map->Name))
    return;
  ...
  if (_MapMinCorner.x > _MapMaxCorner.x)
    std::swap(_MapMinCorner.x, _MapMaxCorner.x);
  if (_MapMinCorner.y > _MapMaxCorner.y)
    std::swap(_MapMinCorner.y, _MapMaxCorner.y);
  ...
  // disable the map_back button for islands (islands can't be seen on the world
  // map)
  CInterfaceGroup *gc = getParentContainer();
  if (gc) {
    CCtrlBase *mapBack = gc->getCtrl("map_back");
    if (mapBack)
      mapBack->setActive(!_IsIsland);
  }
...
void CGroupMap::setMap(const string &mapName) {
  if ((_CurMap != NULL) && (_CurMap->Name == mapName))
    return;
...
void CGroupMap::centerOnPlayer() {
  ...
  // Here, in some case (init, if the map is not displayed), scale is 0. Avoid
  // div by 0.
  ...
  if (ly == 0.f)
    ly = 0.01f;
  _WorldOffset.x =
      (_MapMaxCorner.x - _MapMinCorner.x) * (_MapX - _WReal * 0.5f) / lx;
  _WorldOffset.y =
      (_MapMaxCorner.y - _MapMinCorner.y) * (-_MapY - _HReal * 0.5f) / ly;
...
//============================================================================================================
void CGroupMap::setScale(float newUserScale,
                         const NLMISC::CVector2f & /* center */) {
  NLMISC::clamp(newUserScale, 1.f,
...
void CGroupMap::updateMatchedLandmarks() {
  ...
  for (uint k = 0; k < _MatchedLandmarks.size(); ++k) {
    ...
    params.push_back(std::pair<string, string>(
        "tooltip", "u:" + _MatchedLandmarks[k].Title.toUtf8()));
    params.push_back(std::pair<string, string>("index", toString(k)));

    CInterfaceGroup *g =
        CWidgetManager::getInstance()->getParser()->createGroupInstance(
            "lm_search_result", pL->getId(), params);
    if (g) {
      pL->addChild(g);
...
void CGroupMap::createLMWidgets(const std::vector<CContLandMark> &lms) {
  ...
  for (uint32 k = 0; k < lms.size(); ++k) {
    ...
    const char *ucsTmp =
        CStringManagerClient::getPlaceLocalizedName(rCLM.TitleTextID);
    const std::string lcTitle = toLower(ucsTmp);

    bool searchMatch =
        notWorldMode && _LandmarkFilter.size() > 0 && filterLandmark(lcTitle);
    if (searchMatch)
      _MatchedLandmarks.push_back(SMatchedLandmark(
          rCLM.Pos, ucstring::makeFromUtf8(ucsTmp), _ContinentLMOptions));

    // Add button if not a region nor a place
    if ((rCLM.Type != CContLandMark::Region) &&
        (rCLM.Type != CContLandMark::Place) &&
        (rCLM.Type != CContLandMark::Street)) {
      if (rCLM.Type != CContLandMark::Stable)
        addLandMark(_ContinentLM, mapPos, ucstring::makeFromUtf8(ucsTmp),
                    _ContinentLMOptions);
      else
        addLandMark(_ContinentLM, mapPos, CI18N::get("uiStable"),
                    _ContinentLMOptions);
    ...
    // If the name of the landmark is used as a click zone name of the current
    // map, create a polybutton
    bool bFound = false;
...
void CGroupMap::createContinentLandMarks() {
  ...
  if (_CurMap->Name == "world") {
    ...
    for (k = 0; k < _CurContinent->UserLandMarks.size(); ++k) {
      ...
      addLandMark(_UserLM, mapPos, _CurContinent->UserLandMarks[k].Title,
                  options);

      if (_LandmarkFilter.size() > 0) {
        if (filterLandmark(_CurContinent->UserLandMarks[k].Title)) {
          _MatchedLandmarks.push_back(
              SMatchedLandmark(_CurContinent->UserLandMarks[k].Pos,
                               _CurContinent->UserLandMarks[k].Title, options));
        } else {
...
static void hideTeleportButtonsInPopupMenuIfNotEnoughPriv() {
  bool showTeleport =
      (hasPrivilegeDEV() || hasPrivilegeSGM() || hasPrivilegeGM() ||
       hasPrivilegeVG() || hasPrivilegeSG() || hasPrivilegeEM() ||
       hasPrivilegeEG());
  CInterfaceManager *im = CInterfaceManager::getInstance();

  CInterfaceElement *ie = CWidgetManager::getInstance()->getElementFromId(
      "ui:interface:map_menu:teleport");
  if (ie)
    ie->setActive(showTeleport);

  ie = CWidgetManager::getInstance()->getElementFromId(
      "ui:interface:land_mark_menu:lmteleport");
  if (ie)
    ie->setActive(showTeleport);

  ie = CWidgetManager::getInstance()->getElementFromId(
      "ui:interface:user_land_mark_menu:lmteleport");
  if (ie)
    ie->setActive(showTeleport);

  ie = CWidgetManager::getInstance()->getElementFromId(
      "ui:interface:user_land_mark_menu_base:lmteleport");
  if (ie)
    ie->setActive(showTeleport);
}
...
void CGroupMap::updateUserLandMarks() {
  ...
  if (_MapMode != MapMode_Normal)
    return;
  if (_CurMap == NULL || _CurMap->Name == "world" || _CurContinent == NULL)
    return;
...
//============================================================================================================
CGroupMap::CLandMarkButton *
CGroupMap::createLandMarkButton(const CLandMarkOptions &options) {
  ...
  if (!options.LandMarkMenu.empty()) {
    ...
    lmb->setParamsOnRightClick(NLMISC::toString("map=%s|menu=%s", _Id.c_str(),
                                                options.LandMarkMenu.c_str()));
  }
  lmb->setPosRef(Hotspot_MM);
  return lmb;
}
//============================================================================================================
void CGroupMap::updateLandMarkButton(CLandMarkButton *lmb,
                                     const CLandMarkOptions &options) {
  lmb->setTexture(options.LandMarkTexNormal);
  lmb->setTextureOver(options.LandMarkTexOver);
  lmb->setTexturePushed(options.LandMarkTexPushed);
...
//============================================================================================================
bool CGroupMap::filterLandmark(const ucstring &title,
                               const std::vector<ucstring> filter,
...
//============================================================================================================
void CGroupMap::addLandMark(TLandMarkButtonVect &destList,
                            const NLMISC::CVector2f &pos, const ucstring &title,
                            const CLandMarkOptions &options) {
...
//============================================================================================================
CCtrlButton *
CGroupMap::addUserLandMark(const NLMISC::CVector2f &pos, const ucstring &title,
  ...
  addLandMark(
      _UserLM, pos, title,
      getUserLandMarkOptions((uint32)_CurContinent->UserLandMarks.size() - 1));
...
//============================================================================================================
CCtrlButton *CGroupMap::getLandmarkCtrl(const std::string &lmType,
                                        uint lmIndex) const {
  ...
  if (lmType == "user") {
    if (lmIndex < _UserLM.size()) {
      ...
      nlwarning("_UserLM index out of bounds (size:%u, index:%u)",
                (uint)_UserLM.size(), lmIndex);
  } else
    nlwarning("unsupported landmark type '%s'", lmType.c_str());
...
void CGroupMap::removeUserLandMark(CCtrlButton *button) {
  ...
  for (uint k = 0; k < _UserLM.size(); ++k) {
    if (_UserLM[k] == button) {
      ...
      _CurContinent->UserLandMarks.erase(_CurContinent->UserLandMarks.begin() +
                                         k);
...
//============================================================================================================
void CGroupMap::updateUserLandMark(
    CCtrlButton *button, const ucstring &newTitle,
    const CUserLandMark::EUserLandMarkType lmType) {
  if (_CurContinent == NULL)
    return;
  nlassert(_CurContinent->UserLandMarks.size() >= _UserLM.size());
...
CLandMarkOptions CGroupMap::getUserLandMarkOptions(uint32 lmindex) const {
  ...
  clmo.ColorNormal = clmo.ColorOver = clmo.ColorPushed =
      _CurContinent->UserLandMarks[lmindex].getColor();
...
void CGroupMap::updatePlayerPos() {
  if (_MapMode != MapMode_SpawnSquad) {
    ...
    NLMISC::CVector pos((float)playerPosD.x, (float)playerPosD.y,
                        (float)playerPosD.z);
    ...
    if (!isInX || !isInY) {
      ...
      for (uint k = 0; k < _Islands.size(); ++k) {
        if (pos.x >= _Islands[k].MinX && pos.y >= _Islands[k].MinY &&
            pos.x <= _Islands[k].MaxX && pos.y <= _Islands[k].MaxY) {
          _IsIsland = true;
          setMap(&_Islands[k]);
          break;
        }
...
void CGroupMap::fitWindow()
{
        ...
        const NLMISC::CVectorD &playerPosD = EntitiesMngr.entity(0)->pos();
        NLMISC::CVector pos((float) playerPosD.x, (float) playerPosD.y, (float)
playerPosD.z);
        // convert player pos into pos in map
        worldToMap(_PlayerPos, pos);
        // x coord
        if (_MapTexW != 0)
        {
                ...
                if ((float)_WReal >= mapWidthOnScreen)
                {
                        ...
                        if ((float)_WReal > mapWidthOnScreen)
                        {
                                parentCont->setW((sint32) mapWidthOnScreen +
diffWReal);
                        }
                }
                else
                {
                        NLMISC::clamp(_Offset.x, - _PlayerPos.x, - _PlayerPos.x
+ 1.f - _WReal / ((float) _MapTexW * _Scale));
                }
        ...
        if (_MapTexH != 0)
        {
                ...
                // parentCont->setPopupMaxH((sint32) mapHeightOnScreen +
diffHReal);
                // parentCont->setPopupMinH((sint32) std::min(_MinH, (sint32)
mapHeightOnScreen) + diffHReal);
                ...
                {
                        _H = std::max(_H, std::min(_MinH, (sint32)
mapHeightOnScreen)); NLMISC::clamp(_Offset.y, - _PlayerPos.y, - _PlayerPos.y
+ 1.f - _H / ((float) mapHeightOnScreen));
                }
        }
}
*/
...
//============================================================================================================
void CGroupMap::evalMapOffset(float userScale, float &scale, sint32 &x,
                              sint32 &y) const {
  if (_MapTexW == 0 || _MapTexH == 0) {
    x = 0;
    y = 0;
...
void CGroupMap::updateScale() {
  if (_MapTexW == 0 || _MapTexH == 0) {
    ...
    _Scale = std::min(getActualMaxScale(),
                      computeRealScaleFromUserScale(_UserScale));
...
//=========================================================================================================
void CGroupMap::computeMapRectInsideGroup(sint32 &x, sint32 &y, sint32 &w,
                                          sint32 &h) const {
...
void CGroupMap::computeOffsets() {
  ...
  if (_MapX < 0)
    startCornerX -= _MapX / (_MapTexW * _Scale);
  float startCornerY = _Offset.y + _PlayerPos.y;
  if (_MapY > 0)
    startCornerY += _MapY / (_MapTexH * _Scale);
  float endCornerX = startCornerX;
  float endCornerY = startCornerY;
  NLMISC::clamp(endCornerX, 0.f,
                1.f - std::min(_MapW, _WReal) / ((float)_MapTexW * _Scale));
  NLMISC::clamp(endCornerY, 0.f,
                1.f - std::min(_MapH, _HReal) / ((float)_MapTexH * _Scale));
...
void CGroupMap::targetLandmark(CCtrlButton *lm) {
  ...
  CCompassTarget ct;
  bool found = false;
  TLandMarkButtonVect::iterator it =
      std::find(_ContinentLM.begin(), _ContinentLM.end(), lm);
  {
  ...
  if (found) {
    CInterfaceManager *im = CInterfaceManager::getInstance();
    CGroupCompas *gc = dynamic_cast<CGroupCompas *>(
...
void CGroupMap::targetLandmarkResult(uint32 index) {
  if (index > _MatchedLandmarks.size())
    return;
...
//=========================================================================================================
CGroupMap::CLandMarkButton *CGroupMap::findClosestLandmark(
    const CVector2f &center, const ucstring &search, bool startsWith,
    const TLandMarkButtonVect &landmarks, float &closest) const {
  ...
  for (TLandMarkButtonVect::const_iterator it = landmarks.begin();
       it != landmarks.end(); ++it) {
    std::string lc;
...
//=========================================================================================================
CGroupMap::CLandMarkText *CGroupMap::findClosestLandmark(
    const CVector2f &center, const ucstring &search, bool startsWith,
    const TLandMarkTextVect &landmarks, float &closest) const {
  ...
  closest = std::numeric_limits<float>::max();
  for (TLandMarkTextVect::const_iterator it = landmarks.begin();
       it != landmarks.end(); ++it) {
    ucstring lc;
...
bool CGroupMap::targetLandmarkByName(const ucstring &search,
                                     bool startsWith) const {
  CCompassTarget ct;
  CLandMarkButton *lm;
  float dist;
...
//=========================================================================================================
void CGroupMap::getLandmarkPosition(const CCtrlButton *lm,
                                    NLMISC::CVector2f &worldPos) {
  if (!lm)
    return;
...
void CGroupMap::addRespawnPoints(const CRespawnPointsMsg &rpm) {
  ...
  // Choose the good map ! (select the first respawn point and check for first
  // matching bounding box map
  if (_MapMode != MapMode_Death)
    return;
  if (_RespawnPos.empty())
    return;
  ...
  NLMISC::CVector2f rpWorldPos(_RespawnPos[0].x * 0.001f,
                               _RespawnPos[0].y * 0.001f);
  ...
  for (uint32 i = 0; i < pWS->Maps.size(); ++i) {
    ...
    if ((rpWorldPos.x >= rMap.MinX) && (rpWorldPos.x <= rMap.MaxX) &&
        (rpWorldPos.y >= rMap.MinY) && (rpWorldPos.y <= rMap.MaxY)) {
      setMap(rMap.Name);
      break;
    }
  }
...
sint32 CGroupMap::getRespawnSelected() const {
  ...
  if (_MapMode == MapMode_Death)
    pNL = NLGUI::CDBManager::getInstance()->getDbProp("UI:SAVE:RESPAWN_PT",
                                                      false);
  else if (_MapMode == MapMode_SpawnSquad)
    pNL = NLGUI::CDBManager::getInstance()->getDbProp(
        "UI:TEMP:OUTPOST:SQUAD_RESPAWN_PT", false);
  if (pNL != NULL)
...
void CGroupMap::setRespawnSelected(sint32 nSpawnPointIndex) {
  if (_RespawnPos.empty())
    return;
  if (nSpawnPointIndex < 0)
    return;
  if ((uint32)nSpawnPointIndex >= _RespawnPos.size())
    return;
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  CCDBNodeLeaf *pNL = NULL;
  if (_MapMode == MapMode_Death)
...
//=========================================================================================================
void CGroupMap::updateClosestLandMarkMenu(const std::string &menu,
                                          const NLMISC::CVector2f &pos) const {
  static uint nbShowLandmarks = 5;
  static uint nbShowLandmarksMax = 5;

  const CGroupMenu *pMenu = dynamic_cast<CGroupMenu *>(
    ...
    if (sortedIndices.empty()) {
      ...
      for (uint j = 0; j < sortedIndices.size(); j++) {
        if (dist < sortedIndices[j].second) {
          sortedIndices.insert(sortedIndices.begin() + j,
                               TSortedDistPair(i, dist));
          found = true;
  ...
  for (uint i = 0; i < sortedIndices.size(); ++i) {
    ...
    ucstring name =
        ucstring(toString("%.2fm ", sqrt(sortedIndices[i].second))) +
        _CurContinent->UserLandMarks[index].Title;
    std::string lineId = toString("%s:lmcosest%d", menu.c_str(), i);
    std::string ahParams =
        toString("type=user|map=%s|index=%d", _Id.c_str(), index);

    CViewTextMenu *vt =
        rootMenu->addLine(std::string(), "map_landmark_by_index", ahParams,
                          lineId.c_str(), "", "", false, false, false);
    if (!vt)
      break;
    ...
    vparams.push_back(
        TTmplParams("icon_color", options.ColorNormal.toString()));

    CInterfaceGroup *pUGLeft =
        CWidgetManager::getInstance()->getParser()->createGroupInstance(
            "landmark_row_icon", lineId, vparams);
    if (pUGLeft)
      rootMenu->setUserGroupLeft(lineIndex, pUGLeft, true);

    rootMenu->setRightClickHandler(lineIndex, "map_landmark_by_index");
    rootMenu->setRightClickHandlerParam(
        lineIndex, toString("%s|menu=%s_base", ahParams.c_str(),
                            options.LandMarkMenu.c_str()));
    lineIndex++;
  }
}
...
//=========================================================================================================
// remap caller with landmark using index/type and call popup menu or set
// compass target if no menu is set
class CAHMapLandmarkByIndex : public IActionHandler {
  virtual void execute(CCtrlBase *pCaller, const string &params) {
    ...
    uint index;
    if (!fromString(getParam(params, "index"), index))
      return;

    CGroupMap *map = dynamic_cast<CGroupMap *>(
    ...
    if (menuId.empty())
      ...
      CAHManager::getInstance()->runActionHandler(
          "active_menu", pButton,
          toString("pushmodal=true|popmodal=false|menu=%s", menuId.c_str()));
  }
};
...
class CAHValidateUserLandMarkName : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* params */) {
    ...
    if (!gc)
      return;
    // Retrieve ComboBox to get the position(ordered landmark type) of the
    // selected item
    CDBGroupComboBox *cb =
        dynamic_cast<CDBGroupComboBox *>(gc->getGroup("landmarktypes"));

    CUserLandMark::EUserLandMarkType landMarkType = CUserLandMark::Misc;
    sint8 nLandMarkType = cb->getTextId(CDBManager::getInstance()
                                            ->getDbProp("UI:TEMP:LANDMARKTYPE")
                                            ->getValue8());
    if (nLandMarkType >= 0 &&
        nLandMarkType <= CUserLandMark::UserLandMarkTypeCount) {
    ...
    if (LastSelectedLandMark) {
      ...
      map->updateUserLandMark(LastSelectedLandMark,
      ...
      if (UseUserPositionForLandMark) {
        LastClickedMap->addUserLandMark(
            LastClickedMap->getPlayerPos(),
            ucstring::makeFromUtf8(eb->getInputString()), landMarkType);
      } else {
        LastClickedMap->addUserLandMark(
            LastClickedMap->getRightClickLastPos(),
            ucstring::makeFromUtf8(eb->getInputString()), landMarkType);
      }
...
REGISTER_ACTION_HANDLER(CAHValidateUserLandMarkName,
                        "validate_user_landmark_name");

//=========================================================================================================
void createUserLandMark(CCtrlBase * /* pCaller */,
                        const string & /* params */) {
  CInterfaceManager *im = CInterfaceManager::getInstance();
  // pop the rename dialog
  LastClickedMap = dynamic_cast<CGroupMap *>(
      CWidgetManager::getInstance()->getCtrlLaunchingModal());
  if (LastClickedMap->isInDeathMode())
    return;
  LastSelectedLandMark = NULL;
  popupLandMarkNameDialog();
}
...
REGISTER_ACTION_HANDLER(CAHCreateUserLandMarkAtUserPos,
...
class CAHMapZoomIn : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string &params) {
    ...
    CInterfaceManager *im = CInterfaceManager::getInstance();
    CGroupMap *gm = dynamic_cast<CGroupMap *>(
...
class CAHMapBack : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string &params) {
    std::string map = getParam(params, "map");
    CInterfaceManager *im = CInterfaceManager::getInstance();
    CGroupMap *pGM = dynamic_cast<CGroupMap *>(
        CWidgetManager::getInstance()->getElementFromId(map));
    if (pGM == NULL)
      return;
...
class CAHRespawnMapValid : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string &params) {
    ...
    /*
            Yoyo: NO!!! don't do this!! leave the DB Player MBEHAV Mode drive
       this (laggy but correct).... Else Errors arise if the client MBEHAV Mode
       never reset to Normal==1 (for any lag reason) The following scenario else
       could cause a bug:
                    - the player is dead, the respawn map is activated
                    - the player click "respawn"
                    - in the buggy version we close the window immediatly (no
       lag)
                    - the server receive the ASK respawn, resapwn the player,
       and change the mode to NORMAL==1
                    - the server decide that the PLAYER DIES AUTOMATICALLY in
       the same server tick (gingo in town for instance! :)  ) NB: even if the
       DIE arise in following frame, lag and packet loss etc... can still cause
       problems
                    - the server then reset the mode to DEATH
                    - the client never receive the change Mode: 0-->1-->0
       (always 0), hence the window is not reopened
    ...
    */
    NLGUI::CDBManager::getInstance()
        ->getDbProp("UI:VARIABLES:RESPAWN:MSG_SENT")
        ->setValue64(1);
  }
};
...
class CAHWorldMapRightClick : public IActionHandler {
  virtual void execute(CCtrlBase *pCaller, const string &params) {
    ...
    if (gm->isIsland()) {
      ...
      CAHManager::getInstance()->runActionHandler(
          "active_menu", pCaller, "menu=ui:interface:map_menu_island");
    }

    CAHManager::getInstance()->runActionHandler("active_menu", pCaller,
                                                "menu=" + menu);
  }
};
...
class CAHMapTeleport : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* params */) {
    CInterfaceManager *im = CInterfaceManager::getInstance();
    CGroupMap *clickedMap = dynamic_cast<CGroupMap *>(
        CWidgetManager::getInstance()->getCtrlLaunchingModal());
    closeLandMarkNameDialog();
...
class CUpdateLandMarksColor : public IActionHandler {
public:
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    CInterfaceManager *pIM = CInterfaceManager::getInstance();

    CUserLandMark::_LandMarksColor[CUserLandMark::Misc] =
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:SAVE:LANDMARK:COLORS:MISC")
            ->getValueRGBA();
    CUserLandMark::_LandMarksColor[CUserLandMark::Tribe] =
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:SAVE:LANDMARK:COLORS:TRIBE")
            ->getValueRGBA();
    CUserLandMark::_LandMarksColor[CUserLandMark::Bandit] =
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:SAVE:LANDMARK:COLORS:BANDIT")
            ->getValueRGBA();
    CUserLandMark::_LandMarksColor[CUserLandMark::Citizen] =
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:SAVE:LANDMARK:COLORS:CITIZEN")
            ->getValueRGBA();
    CUserLandMark::_LandMarksColor[CUserLandMark::Fauna] =
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:SAVE:LANDMARK:COLORS:FAUNA")
            ->getValueRGBA();
    CUserLandMark::_LandMarksColor[CUserLandMark::FaunaExcel] =
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:SAVE:LANDMARK:COLORS:FAUNAEXCEL")
            ->getValueRGBA();
    CUserLandMark::_LandMarksColor[CUserLandMark::FaunaSup] =
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:SAVE:LANDMARK:COLORS:FAUNASUP")
            ->getValueRGBA();
    CUserLandMark::_LandMarksColor[CUserLandMark::Forage] =
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:SAVE:LANDMARK:COLORS:FORAGE")
            ->getValueRGBA();
    CUserLandMark::_LandMarksColor[CUserLandMark::ForageExcel] =
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:SAVE:LANDMARK:COLORS:FORAGEEXCEL")
            ->getValueRGBA();
    CUserLandMark::_LandMarksColor[CUserLandMark::ForageSup] =
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:SAVE:LANDMARK:COLORS:FORAGESUP")
            ->getValueRGBA();
    CUserLandMark::_LandMarksColor[CUserLandMark::Sap] =
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:SAVE:LANDMARK:COLORS:SAP")
            ->getValueRGBA();
    CUserLandMark::_LandMarksColor[CUserLandMark::Amber] =
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:SAVE:LANDMARK:COLORS:AMBER")
            ->getValueRGBA();
    CUserLandMark::_LandMarksColor[CUserLandMark::Node] =
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:SAVE:LANDMARK:COLORS:NODE")
            ->getValueRGBA();
    CUserLandMark::_LandMarksColor[CUserLandMark::Fiber] =
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:SAVE:LANDMARK:COLORS:FIBER")
            ->getValueRGBA();
    CUserLandMark::_LandMarksColor[CUserLandMark::Bark] =
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:SAVE:LANDMARK:COLORS:BARK")
            ->getValueRGBA();
    CUserLandMark::_LandMarksColor[CUserLandMark::Seed] =
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:SAVE:LANDMARK:COLORS:SEED")
            ->getValueRGBA();
    CUserLandMark::_LandMarksColor[CUserLandMark::Shell] =
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:SAVE:LANDMARK:COLORS:SHELL")
            ->getValueRGBA();
    CUserLandMark::_LandMarksColor[CUserLandMark::Resin] =
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:SAVE:LANDMARK:COLORS:RESIN")
            ->getValueRGBA();
    CUserLandMark::_LandMarksColor[CUserLandMark::Wood] =
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:SAVE:LANDMARK:COLORS:WOOD")
            ->getValueRGBA();
    CUserLandMark::_LandMarksColor[CUserLandMark::Oil] =
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:SAVE:LANDMARK:COLORS:OIL")
            ->getValueRGBA();
    CUserLandMark::_LandMarksColor[CUserLandMark::Mission] =
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:SAVE:LANDMARK:COLORS:MISSION")
            ->getValueRGBA();
    CUserLandMark::_LandMarksColor[CUserLandMark::Food] =
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:SAVE:LANDMARK:COLORS:FOOD")
            ->getValueRGBA();
    CUserLandMark::_LandMarksColor[CUserLandMark::Construction] =
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:SAVE:LANDMARK:COLORS:CONSTRUCTION")
            ->getValueRGBA();
    CUserLandMark::_LandMarksColor[CUserLandMark::Goo] =
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:SAVE:LANDMARK:COLORS:GOO")
            ->getValueRGBA();
    CUserLandMark::_LandMarksColor[CUserLandMark::Insect] =
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:SAVE:LANDMARK:COLORS:INSECT")
            ->getValueRGBA();
    CUserLandMark::_LandMarksColor[CUserLandMark::Kitin] =
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:SAVE:LANDMARK:COLORS:KITIN")
            ->getValueRGBA();
    CUserLandMark::_LandMarksColor[CUserLandMark::Nocive] =
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:SAVE:LANDMARK:COLORS:NOCIVE")
            ->getValueRGBA();
    CUserLandMark::_LandMarksColor[CUserLandMark::Preservative] =
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:SAVE:LANDMARK:COLORS:PRESERVATIVE")
            ->getValueRGBA();
    CUserLandMark::_LandMarksColor[CUserLandMark::Passage] =
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:SAVE:LANDMARK:COLORS:PASSAGE")
            ->getValueRGBA();
    CUserLandMark::_LandMarksColor[CUserLandMark::Teleporter] =
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:SAVE:LANDMARK:COLORS:TELEPORTER")
            ->getValueRGBA();
...
NLMISC_COMMAND(testMapHome, "Debug : test display of home on map", "") {
  ...
  CInterfaceManager *im = CInterfaceManager::getInstance();
  NLGUI::CDBManager::getInstance()
      ->getDbProp(COMPASS_DB_PATH ":HOME_POINT")
      ->setValue64((((sint64)4787 * 1000) << 32) |
                   (sint64)(uint32)((sint64)-3661 * 1000));
  return true;
}
/*
...
":BIND_POINT")->setValue64((((sint64) 4687 * 1000) << 32 )| (sint64) (uint32)
((sint64) -3561 * 1000)); return true;
}
*/
NLMISC_COMMAND(testRespawn, "Debug : test respawn map", "") {
  ...
  rpm.RespawnPoints.push_back(
      CRespawnPointsMsg::SRespawnPoint(4150 * 1000, -4350 * 1000));
  rpm.RespawnPoints.push_back(
      CRespawnPointsMsg::SRespawnPoint(4640 * 1000, -4320 * 1000));
  rpm.RespawnPoints.push_back(
      CRespawnPointsMsg::SRespawnPoint(4100 * 1000, -4120 * 1000));
  rpm.RespawnPoints.push_back(
      CRespawnPointsMsg::SRespawnPoint(4050 * 1000, -4200 * 1000));
  rpm.RespawnPoints.push_back(
      CRespawnPointsMsg::SRespawnPoint(4200 * 1000, -4150 * 1000));
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  CGroupMap *pMap =
  ...
  /*
          CInterfaceManager *im = CInterfaceManager::getInstance();
          NLGUI::CDBManager::getInstance()->getDbProp(COMPASS_DB_PATH
     ":BIND_POINT")->setValue64((((sint64) 4687 * 1000) << 32 )| (sint64)
     (uint32) ((sint64) -3561 * 1000));
  */
  return true;
}
