...
// Values for float (scrollbar float) must not exceed 200,00 this is due to
// storing on an integer value with a precision of 10,000 For the moment
// realtime parameters are only valid on scrollbar
// ***************************************************************************
...
public:
  ...
  // Update all parameters to obey their preset (no op if no preset or if preset
  // is Custom)
...
private:
  ...
  public:
    ...
    // From the current CFG value, compare to each preset value, and init the
    // bitfield
