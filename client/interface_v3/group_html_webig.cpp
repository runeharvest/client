...
static string getWebAuthKey() {
  if (!UserEntity || !NetMngr.getLoginCookie().isValid())
    return "";
  ...
  string rawKey = toString(CharacterHomeSessionId) +
                  UserEntity->getLoginName() + toString(cid) +
                  NetMngr.getLoginCookie().toString();
  string key =
      getMD5((const uint8 *)rawKey.c_str(), (uint32)rawKey.size()).toString();
...
void addWebIGParams(string &url, bool trustedDomain) {
  ...
  uint32 cid = NetMngr.getLoginCookie().getUserId() * 16 + PlayerSelectedSlot;
  url += ((url.find('?') != string::npos) ? "&" : "?") + string("shardid=") +
         toString(CharacterHomeSessionId) + string("&name=") +
         UserEntity->getLoginName() + // FIXME: UrlEncode
         string("&lang=") + CI18N::getCurrentLanguageCode() +
         string("&datasetid=") + toString(UserEntity->dataSetId()) +
         string("&ig=1");
  if (trustedDomain) {
    url +=
        string("&cid=") + toString(cid) + string("&authkey=") + getWebAuthKey();

    if (url.find('$') != string::npos) {
      strFindReplace(url, "$gender$",
                     GSGENDER::toString(UserEntity->getGender()));
      strFindReplace(url, "$displayName$",
                     UserEntity->getDisplayName()); // FIXME: UrlEncode...
      ...
      strFindReplace(
          url, "$post$",
          toString(atan2(UserEntity->front().y, UserEntity->front().x)));
      ...
      if (node &&
          (uint8)node->getValue32() != (uint8)CLFECOMMON::INVALID_SLOT) {
        ...
        if (target) {
          ...
          strFindReplace(url, "$tdisplayName$",
                         target->getDisplayName()); // FIXME: UrlEncode...
          ...
          strFindReplace(url, "$tpost$",
                         toString(atan2(target->front().y, target->front().x)));
...
CGroupHTMLAuth::CGroupHTMLAuth(const TCtorParam &param) : CGroupHTML(param) {}
...
void CGroupHTMLAuth::addHTTPPostParams(SFormFields &formfields,
                                       bool trustedDomain) {
  // no extras parameters added to url if not in trusted domains list
  if (!trustedDomain)
    return;
...
string CGroupHTMLAuth::home() const { return Home; }

// ***************************************************************************

void CGroupHTMLAuth::handle() { CGroupHTML::handle(); }
...
void CGroupHTMLWebIG::addHTTPPostParams(SFormFields &formfields,
                                        bool trustedDomain) {
...
string CGroupHTMLWebIG::home() const { return Home; }

// ***************************************************************************

void CGroupHTMLWebIG::handle() { CGroupHTMLAuth::handle(); }
