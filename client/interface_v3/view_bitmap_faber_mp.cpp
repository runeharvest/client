...
NLMISC_REGISTER_OBJECT(CViewBase, CViewBitmapFaberMp, std::string,
                       "bitmap_faber_mp");

/*
 * parse an xml node and initialize the base view mambers. Must call
...
bool CViewBitmapFaberMp::parse(xmlNodePtr cur, CInterfaceGroup *parentGroup) {
  ...
  /*	_SheetText = new CViewText(_Id + string(":sheettext"),"const 9","const
     255 255 255 255","const false","const 0"); _QualityText = new CViewText(_Id
     + string(":qualitytext"),"const 9","const 255 255 255 255","const
     false","const 0"); _QuantityText = new CViewText(_Id +
     string(":quantitytext"),"const 9","const 255 255 255 255","const
     false","const 0");
  ...
  if ((pES != NULL) && (pES->type() == CEntitySheet::ITEM)) {
    ...
    if (!pIS->getIconBack().empty()) {
      ...
      rVR.drawRotFlipBitmap(_RenderLayer, _XReal, _YReal, _WReal, _HReal,
                            (uint8)_Rot, _Flip, _AccIconBackId);
    }
    ...
    if (!pIS->getIconMain().empty()) {
      ...
      rVR.drawRotFlipBitmap(_RenderLayer, _XReal, _YReal, _WReal, _HReal,
                            (uint8)_Rot, _Flip, _AccIconMainId);
    }
    ...
    if (!pIS->getIconOver().empty()) {
      ...
      rVR.drawRotFlipBitmap(_RenderLayer, _XReal, _YReal, _WReal, _HReal,
                            (uint8)_Rot, _Flip, _AccIconOverId);
    }
    ...
    /*		// draw wanted quantity
                    ...
                    rVR.drawRotFlipBitmap (_RenderLayer, _XReal+xOffset+13,
       _YReal+yOffset, 8, 8, (uint8)_Rot.getSInt32(), _Flip.getBool(),
                                    rVR.getFigurTextureId(units),
       _Color.getRGBA());
                    ...
                    rVR.drawRotFlipBitmap (_RenderLayer, _XReal+xOffset+8,
       _YReal+yOffset, 8, 8, (uint8)_Rot.getSInt32(), _Flip.getBool(), tensId,
       _Color.getRGBA());
                    }
                    ...
                    if (_Quantity.getSInt32() > 1)
                    {
                            ...
                            rVR.drawRotFlipBitmap (_RenderLayer,
       _XReal+xOffset+_WReal-8, _YReal+yOffset, 8, 8, (uint8)_Rot.getSInt32(),
       _Flip.getBool(), rVR.getFigurTextureId(units), _Color.getRGBA());
                            ...
                            if (tens > 9 || tens == 0)
                                    ...
                                    tensId = rVR.getFigurTextureId(tens);
                            rVR.drawRotFlipBitmap (_RenderLayer,
       _XReal+xOffset+_WReal-13, _YReal+yOffset, 8, 8, (uint8)_Rot.getSInt32(),
       _Flip.getBool(), tensId, _Color.getRGBA());
                    }
                    */
    ...
    if (qty > 1) {
      // draw the units
      uint units = qty % 10;
      rVR.drawRotFlipBitmap(_RenderLayer, _XReal + _WReal - 8, _YReal, 8, 8,
                            (uint8)_Rot, _Flip, rVR.getFigurTextureId(units),
                            _Color);
      ...
      if (tens > 9 || tens == 0)
        ...
        tensId = rVR.getFigurTextureId(tens);
      rVR.drawRotFlipBitmap(_RenderLayer, _XReal + _WReal - 13, _YReal, 8, 8,
                            (uint8)_Rot, _Flip, tensId, _Color);
    }
  } else {
    if (_TextureNoItemId == -2)
      _TextureNoItemId = rVR.getTextureIdFromName(_TextureNoItemName);
    rVR.drawRotFlipBitmap(_RenderLayer, _XReal, _YReal, _WReal, _HReal,
                          (uint8)_Rot, _Flip, _TextureNoItemId,
                          _ColorNoItem.getRGBA());
  }
}
