...
// ***************************************************************************
void CAddOnManager::addSearchFiles(
    const std::string &path, const std::string &posFilterList,
    const std::string &negFilterList,
    NLMISC::IProgressCallback *progressCallBack) {
  /*
          NB: this is mainly a copy of CPath::addSearchPath(), with filter
     added.
  */
  ...
  for (uint f = 0; f < filesToProcess.size(); f++) {
    ...
    if (progressCallBack) {
      ...
      progressCallBack->pushCropedValues(
          (float)f / (float)filesToProcess.size(),
          (float)(f + 1) / (float)filesToProcess.size());
    }
...
// ***************************************************************************
void CAddOnManager::getFiles(const std::string &filterList,
