...
NLMISC_REGISTER_OBJECT(CViewBase, CGroupPhraseSkillFilter, std::string,
                       "phrase_skill_filter");
...
// ***************************************************************************
bool CGroupPhraseSkillFilter::parse(xmlNodePtr cur,
  ...
  // Add observer on each Brick Families because if some brick is learned, a new
...
// ***************************************************************************
void CGroupPhraseSkillFilter::bkupSkillOpenedStateRecurs(
    bool *skillOpened, CGroupTree::SNode *node) {
  if (!node)
    return;
  // the rootNode has an empty Id, dont test it
  // Also don't test SpecialPower Node.
  if (!node->Id.empty() && node->Id != NodeIdSpecialPower &&
...
void CGroupPhraseSkillFilter::rebuild() {
  ...
  // Add a special Fitler at root for Special Powers Phrases (because they don't
  // use skill)
  {
    // Ok lets create it
    CGroupTree::SNode *pNode = new CGroupTree::SNode;
