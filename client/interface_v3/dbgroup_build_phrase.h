 ...
 *	A Main Brick is a Root/Mandatory/Optional/Credit brick. ie everything
 *but a Param \author Lionel Berenguier \author Nevrax France \date 2003
 */
...
public:
  ...
  public:
    ...
    CViewBitmap *Back;
    bool Valid;
    // The Parameter families setuped for this slot (ie if != from
    // Brick->ParameterFamilies, must update display)
   ...
   *	NB: A Word can exist at runtime only if all his slots can exist (Brick
   *and Back are setuped.) But for Credits...
   */
  class CWord {
  public:
    CSlot Slot;
...
public:
  ...
  // call after clearBuildingPhrase(), to copy a phrase, or empty phrase to init
  // new Name.
  ...
  /** For edition, can set a brickType filter for the root. if !Unknown, only
   *root of this type will match Second and more filter are ORed with the root
   *brick, if unknown, not ored.
   */
  void setRootBrickTypeFilter(
      BRICK_TYPE::EBrickType rootBtFilter,
  ...
  // if index==0, you must call fillSelectionRoot(). It can be a
  // Mandatory/Optional/Credit
  ...
  /// Call this when the name or the root brick change, to update the view of
  /// the spell edited only.
...
protected:
  ...
  void filterBrickExclusion(std::vector<NLMISC::CSheetId> &bricks,
                            uint16 indexToSkip = 0xffff);
  void filterFamilySetuped(std::vector<uint16> &families);
  void filterBrickSetuped(std::vector<NLMISC::CSheetId> &bricks);
  /// Filter bricks so only one that are compatible with skill of the current
  /// setuped one are filtered.
  void filterSkillSetuped(std::vector<NLMISC::CSheetId> &bricks,
                          bool checkOptional, bool checkCredit,
                          sint avoidCheckIndex = -1);
  ...
  uint getNumMainBricks() const {
    return 1 + _NumMandatories + _NumOptionals + _NumCredits;
  }
