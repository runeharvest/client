...
void CMacroCmd::writeTo(xmlNodePtr node) const {
  ...
  xmlNodePtr macroNode =
      xmlNewChild(node, NULL, (const xmlChar *)"macro", NULL);

  // Props
  xmlSetProp(macroNode, (const xmlChar *)"name", (const xmlChar *)Name.c_str());
  xmlSetProp(macroNode, (const xmlChar *)"id",
             (const xmlChar *)toString(ID).c_str());
  xmlSetProp(macroNode, (const xmlChar *)"back",
             (const xmlChar *)toString(BitmapBack).c_str());
  xmlSetProp(macroNode, (const xmlChar *)"icon",
             (const xmlChar *)toString(BitmapIcon).c_str());
  xmlSetProp(macroNode, (const xmlChar *)"over",
             (const xmlChar *)toString(BitmapOver).c_str());
  xmlSetProp(macroNode, (const xmlChar *)"text",
             (const xmlChar *)DispText.c_str());

  for (uint i = 0; i < Commands.size(); ++i) {
    xmlNodePtr cmdNode =
        xmlNewChild(macroNode, NULL, (const xmlChar *)"command", NULL);
    xmlSetProp(cmdNode, (const xmlChar *)"name",
               (const xmlChar *)Commands[i].Name.c_str());
    xmlSetProp(cmdNode, (const xmlChar *)"params",
               (const xmlChar *)Commands[i].Params.c_str());
  }
}
...
bool CMacroCmd::readFrom(xmlNodePtr node) {
  ...
  while (node) {
    if (stricmp((char *)node->name, "command") == 0) {
      ...
      ptrName = (char *)xmlGetProp(node, (xmlChar *)"name");
      if (ptrName)
        c.Name = (const char *)ptrName;

      ptrName = (char *)xmlGetProp(node, (xmlChar *)"params");
      if (ptrName)
        c.Params = (const char *)ptrName;
...
// ------------------------------------------------------------------------------------------------
void CMacroCmd::addCommand(const string &sName, const string &sParam,
                           sint32 nPos) {
...
void CMacroCmdManager::initInGame() {
  ...
  CInterfaceOptions *pIO =
      CWidgetManager::getInstance()->getOptions("macro_custom_icon");
  if (pIO != NULL) {
    string sTmp;
    uint i, nOpt;
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:USER:ACT_NUMBER")
      ->setValue64(0);

  // Create the NewKey container. Can edit key, and can assign all actions (not
  // only macroisable ones)
  NewKey = new CModalContainerEditCmd;
  NewKey->create("newkey", true, true);

  // Create the EditCmd container. Cannot edit key, and can assign only
  // macroisable actions
...
// ------------------------------------------------------------------------------------------------
void CMacroCmdManager::removeAllMacros() { _Macros.clear(); }
...
// ------------------------------------------------------------------------------------------------
// Refresh key association that can be changed in another place than in macro
// container
void CMacroCmdManager::refreshMacroCombo() {
  ...
  for (uint i = 0; i < _Macros.size(); ++i) {
    ...
    CActionsManager::TActionComboMap::const_iterator it =
        rACmap.find(CAction::CName(AH_MACRO_EXEC, toString(i).c_str()));
    if (it != rACmap.end())
...
void CMacroCmdManager::addActionManagerEntries() {
  ...
  for (i = 0; i < _Macros.size(); ++i) {
    if (_Macros[i].Combo.Key != KeyCount) {
      pAM->addCombo(CAction::CName(AH_MACRO_EXEC, toString(i).c_str()),
                    _Macros[i].Combo);
    }
  }
}
...
void CMacroCmdManager::execute(uint nMacronb) {
  if (_CurExecMac != -1)
    return; // Cannot execute a macro while last macro is not finished
...
void CMacroCmdManager::updateMacroExecution() {
  if (_CurExecMac != -1) {
    ...
    for (;;) {
      ...
      // Flush interface links (else bug with Macro "Select ShortCutBar/Run
      // Shortcut"
...
void CMacroCmdManager::refreshAllKeyDisplays() {
  ...
  CAHManager::getInstance()->runActionHandler(
      "gestion_windows_update_key_binding", NULL);
}
...
public:
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    CInterfaceManager *pIM = CInterfaceManager::getInstance();
    CMacroCmdManager *pMCM = CMacroCmdManager::getInstance();
    pMCM->receiveActionEnd((uint8)NLGUI::CDBManager::getInstance()
                               ->getDbProp("SERVER:USER:ACT_NUMBER")
                               ->getValue64());
  }
};
...
public:
  virtual void execute(CCtrlBase * /* pCaller */, const string &Params) {
    ...
    CDBCtrlSheet *pCS = dynamic_cast<CDBCtrlSheet *>(
        CWidgetManager::getInstance()->getElementFromId(sTP));
    sint32 nb;
    fromString(getParam(Params, "value"), nb);
    if (pCS != NULL)
      pCS->setMacroBack((uint8)nb);
  }
};
  ...
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    ...
    CDBCtrlSheet *pCS = dynamic_cast<CDBCtrlSheet *>(
        CWidgetManager::getInstance()->getElementFromId(
            CTRL_MACROICONCREATION_ICON));
    if (pCS != NULL)
      pCS->writeToMacro(pMCM->CurrentEditMacro);

    pCS = dynamic_cast<CDBCtrlSheet *>(
        CWidgetManager::getInstance()->getElementFromId(CTRL_NEWMACRO_ICON));
    if (pCS != NULL)
      pCS->readFromMacro(pMCM->CurrentEditMacro);
...
public:
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    ...
    CAHManager::getInstance()->runActionHandler(
        "reset_pushed", NULL, string("dblink=") + GROUP_MACROICONCREATION_BACK);
    CAHManager::getInstance()->runActionHandler(
        "reset_pushed", NULL, string("dblink=") + GROUP_MACROICONCREATION_ICON);
    CAHManager::getInstance()->runActionHandler(
        "reset_pushed", NULL, string("dblink=") + GROUP_MACROICONCREATION_OVER);

    uint8 back = pMCM->CurrentEditMacro.BitmapBack;
    if (back != 0xff) {
      string sButton = string(GROUP_MACROICONCREATION_BACK) +
                       CTRL_MACROICONCREATION_BUTTON + toString(back + 1);
      CCtrlBaseButton *pCB = dynamic_cast<CCtrlBaseButton *>(
          CWidgetManager::getInstance()->getElementFromId(sButton));
      if (pCB != NULL)
        pCB->setPushed(true);
    }
    ...
    if (icon != 0xff) {
      string sButton = string(GROUP_MACROICONCREATION_ICON) +
                       CTRL_MACROICONCREATION_BUTTON + toString(icon + 1);
      CCtrlBaseButton *pCB = dynamic_cast<CCtrlBaseButton *>(
          CWidgetManager::getInstance()->getElementFromId(sButton));
      if (pCB != NULL)
        pCB->setPushed(true);
    ...
    if (over != 0xff) {
      string sButton = string(GROUP_MACROICONCREATION_OVER) +
                       CTRL_MACROICONCREATION_BUTTON + toString(over + 1);
      CCtrlBaseButton *pCB = dynamic_cast<CCtrlBaseButton *>(
          CWidgetManager::getInstance()->getElementFromId(sButton));
      if (pCB != NULL)
        pCB->setPushed(true);
    ...
    CGroupEditBox *pEB = dynamic_cast<CGroupEditBox *>(
        CWidgetManager::getInstance()->getElementFromId(
            CTRL_MACROICONCREATION_EDITTEXT));
    if (pEB != NULL) {
    ...
    CAHManager::getInstance()->runActionHandler(
        "set_macro_back", NULL,
        string("target=") + CTRL_MACROICONCREATION_ICON +
            "|value=" + toString(back));
    CAHManager::getInstance()->runActionHandler(
        "set_macro_icon", NULL,
        string("target=") + CTRL_MACROICONCREATION_ICON +
...
REGISTER_ACTION_HANDLER(CHandlerMacroIconCreationOpen,
                        "macro_icon_creation_open");
...
// ***************************************************************************
sint32 getCmdNbFromId(
    string id) // copy the string ! (do not change for a const string &)
{
  if (id.rfind(":c") == string::npos)
    return -1;
...
// ***************************************************************************
// Called from context menu when we right click on a command of the new_macro
// container
...
public:
  virtual void execute(CCtrlBase *pCaller, const string & /* Params */) {
    ...
    pMCM->CurrentEditMacro.addCommand(
        pMCM->CurrentEditMacro.Commands[nCmdNb].Name,
        pMCM->CurrentEditMacro.Commands[nCmdNb].Params, nCmdNb);
    CInterfaceManager *pIM = CInterfaceManager::getInstance();
    CMacroCmdManager::getInstance()->EditCmd->deactivate();
    CAHManager::getInstance()->runActionHandler("new_macro_open", NULL);
  }
...
void addCommandLine(CGroupList *pParent, uint cmdNb, const string &cmdName) {
  ...
  CInterfaceGroup *pNewCmd =
      CWidgetManager::getInstance()->getParser()->createGroupInstance(
          TEMPLATE_NEWMACRO_COMMAND, pParent->getId(), vParams);
  if (pNewCmd == NULL)
    return;

  CViewText *pVT = dynamic_cast<CViewText *>(
      pNewCmd->getView(TEMPLATE_NEWMACRO_COMMAND_TEXT));
  if (pVT != NULL)
    pVT->setText(cmdName);
...
public:
  virtual void execute(CCtrlBase *pCaller, const string & /* Params */) {
    CInterfaceManager *pIM = CInterfaceManager::getInstance();
    if (pCaller == NULL)
      pCaller = dynamic_cast<CCtrlBase *>(
          CWidgetManager::getInstance()->getElementFromId(
              "ui:interface:new_macro:content:edit_name"));
    CGroupEditBox *pEB = dynamic_cast<CGroupEditBox *>(pCaller);
    if (pEB == NULL)
      return;
    ...
    if ((pMCM->CurrentEditMacro.Name.size() >= 2) &&
        ...
        (pMCM->CurrentEditMacro.Name[1] == 'i'))
      pMCM->CurrentEditMacro.Name[0] = 'U';
...
public:
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    // Init 'new_macro' container from the global current macro
    // (gCurrentEditMacro)
    CInterfaceManager *pIM = CInterfaceManager::getInstance();
    CMacroCmdManager *pMCM = CMacroCmdManager::getInstance();
    // Icon
    ...
    CGroupEditBox *pEB = dynamic_cast<CGroupEditBox *>(
        CWidgetManager::getInstance()->getElementFromId(
            GROUP_NEWMACRO_EDIT_NAME));
    if (pEB != NULL)
      pEB->setInputString(pMCM->CurrentEditMacro.Name);
    // Commands
    CGroupList *pList = dynamic_cast<CGroupList *>(
        CWidgetManager::getInstance()->getElementFromId(
            GROUP_NEWMACRO_COMMANDS));
    if (pList == NULL)
      return;
    // Key Shortcut
    CViewText *pVT = dynamic_cast<CViewText *>(
    ...
    for (uint i = 0; i < pMCM->CurrentEditMacro.Commands.size(); ++i) {
      ...
      for (uint j = 0; j < pMCM->ActionManagers.size(); ++j) {
        CAction::CName c(pMCM->CurrentEditMacro.Commands[i].Name.c_str(),
                         pMCM->CurrentEditMacro.Commands[i].Params.c_str());
...
public:
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    ...
    if (pMCM->CurrentEditMacro.Commands.empty())
      return;

    // Add a macro
    if (pMCM->CurrentEditMacroNb != -1) {
      // retrieve the Id of the edited macro, to keep the same id in addMacro()
      pMCM->CurrentEditMacro.ID =
          pMCM->getMacros()[pMCM->CurrentEditMacroNb].ID;
    ...
    CGroupContainer *pGC = dynamic_cast<CGroupContainer *>(
        CWidgetManager::getInstance()->getElementFromId(WIN_NEWMACRO));
    if (pGC != NULL)
      pGC->setActive(false);
...
void addMacroLine(CGroupList *pParent, uint macNb, const CMacroCmd &macro) {
  ...
  CInterfaceGroup *pNewMacro =
      CWidgetManager::getInstance()->getParser()->createGroupInstance(
          TEMPLATE_MACRO_ELT, pParent->getId(), vParams);
  if (pNewMacro == NULL)
    return;

  CViewText *pVT =
      dynamic_cast<CViewText *>(pNewMacro->getView(TEMPLATE_MACRO_ELT_TEXT));
  if (pVT != NULL)
    pVT->setText(macro.Name);

  CDBCtrlSheet *pCS =
      dynamic_cast<CDBCtrlSheet *>(pNewMacro->getCtrl(TEMPLATE_MACRO_ELT_ICON));
  if (pCS != NULL)
    pCS->readFromMacro(macro);

  pVT =
      dynamic_cast<CViewText *>(pNewMacro->getView(TEMPLATE_MACRO_ELT_KEYTEXT));
  if (pVT != NULL) {
...
public:
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    ...
    CGroupList *pList = dynamic_cast<CGroupList *>(
        CWidgetManager::getInstance()->getElementFromId(WIN_MACRO_CONTENT));
    if (pList == NULL)
      return;
...
public:
  virtual void execute(CCtrlBase *pCaller, const string & /* Params */) {
    ...
    CAHManager::getInstance()->runActionHandler(AH_MACRO_EXEC, pCaller,
                                                toString(nMacNb));
  }
};
...
public:
  virtual void execute(CCtrlBase *pCaller, const string & /* Params */) {
    ...
    pIM->validMessageBox(CInterfaceManager::QuestionIconMsg,
                         CI18N::get("uiQDeleteMacro"), "macros_do_del", Params);
  }
};
