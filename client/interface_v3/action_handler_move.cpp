...
public:
  virtual void execute(CCtrlBase * /* pCaller */, const string &Params) {
    ...
    uint flags;
    if (Params == "friend")
      // for friend: select both dead and alive (for heal). But Select only
      // Players (not bot)
      flags = CEntityFilterFlag::NotUser | CEntityFilterFlag::Player |
      ...
      flags = CEntityFilterFlag::NotUser | CEntityFilterFlag::Enemy |
              CEntityFilterFlag::Alive;

    // If there is an entity selected -> Set as the Target. Cycle with last
    // target
    CEntityCL *entity = EntitiesMngr.getEntityInCamera(
        flags, ClientCfg.SpaceSelectionDist, UserEntity->targetSlot());
    if (entity) {
      // Select this entity.
...
class CAHFreeMouse : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    ...
    if (im) {
      ...
      if (ig && ig->getActive()) {
        CInterfaceGroup *igHard =
            dynamic_cast<CInterfaceGroup *>(ig->getGroup("hard"));
        if (igHard) {
          CCtrlBaseButton *cbb =
              dynamic_cast<CCtrlBaseButton *>(igHard->getCtrl("c"));
          if (cbb) {
  ...
  virtual void execute(CCtrlBase * /* pCaller */, const string &Params) {
    ...
    if (pAM->valide(CAction::CName("rear_view", Params.c_str()))) {
      PreviousShowInterface =
          ShowInterface; // save previous show interface value
      ShowInterface = false;
      View.rearView(true);
    }
    // Key Up
      ...
      ShowInterface =
          PreviousShowInterface; // restore previous show interface value
      View.rearView(false);
    }
  }
};
...
class CAHToggleDodgeParry : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    ...
    if (NLGUI::CDBManager::getInstance()
            ->getDbProp("SERVER:DEFENSE:DEFENSE_MODE")
            ->getValue32() == 0) {
