...
uint8 getOutpostSelection() {
  ...
  CCDBNodeLeaf *pNL = NLGUI::CDBManager::getInstance()->getDbProp(
      "UI:TEMP:OUTPOST:SELECTION", false);
  if (pNL != NULL)
...
uint32 getOutpostSheet() {
  ...
  CCDBNodeLeaf *pNL = NLGUI::CDBManager::getInstance()->getDbProp(
      toString("SERVER:GUILD:OUTPOST:O%d:SHEET", outpostSel), false);
  if (pNL != NULL)
...
public:
  void execute(CCtrlBase * /* pCaller */, const std::string &sParams) {
    ...
    NLGUI::CDBManager::getInstance()
        ->getDbProp("UI:TEMP:OUTPOST:CONFIRM_DEL_OUTPOST")
        ->setValueBool(true);
...
public:
  void execute(CCtrlBase * /* pCaller */, const std::string &sParams) {
    ...
    uint8 nSquadSlot = NLGUI::CDBManager::getInstance()
...
public:
  void execute(CCtrlBase *pCaller, const std::string &sParams) {
    ...
    CAHManager::getInstance()->runActionHandler("outpost_set_squad", pCaller,
                                                sParams);
  }
};
...
public:
  void execute(CCtrlBase * /* pCaller */, const std::string &sParams) {
    ...
    // init map with all xy that are in
    // SERVER:GUILD:OUTPOST:O#x:SQUAD_SPAWN_ZONE (if 0,0 not present)
    string sDBPathSZ = "SERVER:GUILD:OUTPOST:O" +
                       toString(getOutpostSelection()) + ":SQUAD_SPAWN_ZONE:";
    sint32 xMin = 0, xMax = 0, yMin = 0, yMax = 0;
    bool bInit = false;
    uint32 i;
    for (i = 0; i < OUTPOSTENUMS::OUTPOST_MAX_SPAWN_ZONE; ++i) {
      sint32 x = NLGUI::CDBManager::getInstance()
                     ->getDbProp(sDBPathSZ + toString(i) + ":X")
                     ->getValue32();
      sint32 y = NLGUI::CDBManager::getInstance()
                     ->getDbProp(sDBPathSZ + toString(i) + ":Y")
                     ->getValue32();
      if ((x != 0) || (y != 0)) {
        if (!bInit) {
          xMin = xMax = x;
    ...
    // *** Find the finest map that contains the 2 points xMin,yMin and
    // xMax,yMax
    CWorldSheet *pWorldSheet =
    ...
    for (i = 0; i < pWorldSheet->Maps.size(); ++i) {
      ...
      if (!rMap.ContinentName.empty()) {
        ...
        if ((rMap.MinX < xMin) && (rMap.MinY < yMin) && (rMap.MaxX > xMax) &&
            (rMap.MaxY > yMax)) {
          // Is there a map already found ?
          if (nMapFound == -1) {
            nMapFound = i; // No, first map found
          } else // Yes we have to compare with the previous map to keep the
                 // finest
          {
    ...
    if (nMapFound != -1) {
      ...
      pMap->setScale((rMapFound.MaxX - rMapFound.MinX) /
                     max(sint32(1), (xMax - xMin)));
...
public:
  void execute(CCtrlBase * /* pCaller */, const std::string &sParams) {
    ...
    if (nSpawnIndex != nSpawnSelected) {
      ...
      sendMsgToServer("OUTPOST:SET_SQUAD_SPAWN", getOutpostSheet(), nSquadSlot,
                      nSpawnSelected);
    }
  }
};
...
REGISTER_ACTION_HANDLER(COutpostSelectSquadCapital,
...
static sint localToGmt(sint hour) {
  ...
  CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
      "UI:SAVE:OUTPOST:TIME_ZONE", false);
  if (node)
...
public:
  void execute(CCtrlBase * /* pCaller */, const std::string &sParams) {
    ...
    sendMsgToServer("OUTPOST:SET_DEF_PERIOD", getOutpostSheet(),
                    uint8(localToGmt(defPeriod)));
  }
};
...
public:
  void execute(CCtrlBase *pCaller, const std::string &sParams) {
    ...
    CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
        "UI:TEMP:OUTPOST:DECLARE_WAR_ATTACK_PERIOD", false);
    if (node)
      node->setValue32(localToGmt(attPeriod));

    // Nead to resend a Declare War Start (because wanted Att Hour changed)
    CAHManager::getInstance()->runActionHandler("outpost_declare_war_start",
                                                pCaller);
  }
};
...
public:
  void execute(CCtrlBase * /* pCaller */, const std::string & /* sParams */) {
    ...
    CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
        "SERVER:OUTPOST_SELECTED:SHEET", false);
    if (node)
...
public:
  void execute(CCtrlBase * /* pCaller */, const std::string & /* sParams */) {
    ...
    // read result Att Period Time (NB: for final server check: ensure that the
    // user will get what it sees)
    uint32 startAttackTime = 0;
    node = NLGUI::CDBManager::getInstance()->getDbProp(
        "UI:TEMP:OUTPOST:DECLARE_WAR_ACK_TIME_RANGE_ATT", false);
    if (node)
    ...
    if (GenericMsgHeaderMngr.pushNameToStream(sMsg, out)) {
      // nlinfo("impulseCallBack : %s %d %d %d sent", sMsg.c_str(),
      // outpostSheet, wantedAttHour, startAttackTime);
...
REGISTER_ACTION_HANDLER(CAHOutpostDeclareWarValidate,
...
public:
  void execute(CCtrlBase * /* pCaller */, const std::string & /* sParams */) {
    ...
    uint32 outpostSheet = 0;
    CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
        "LOCAL:TARGET:CONTEXT_MENU:OUTPOST");
    if (node)
      outpostSheet = node->getValue32();
    node = NLGUI::CDBManager::getInstance()->getDbProp(
        "UI:TEMP:OUTPOST:BOT_SELECTION");
    if (node)
      node->setValue32(outpostSheet);
...
public:
  void execute(CCtrlBase * /* pCaller */, const std::string & /* sParams */) {
    // Called when the Outpost State window (the one opened from BotChat) is
    // closed
...
void outpostTestUserCanBan(CCDBNodeLeaf *dbBanRight, bool &okForBanPlayer,
                           bool &okForBanGuild) {
  okForBanPlayer = false;
  okForBanGuild = false;

  /** A player can ban a player in a PVP outpost if:
   *	- The player has right to ban AND his guild attack/owns the outpost the
   *player attacks (all this is in RIGHT_BANNISH)
   *	- The player and the target are in the same war of the outpost, and in
   *same alliance.
...
public:
  void execute(CCtrlBase * /* pCaller */, const std::string & /* sParams */) {
    CInterfaceManager *pIM = CInterfaceManager::getInstance();
    CCDBNodeLeaf *dbBan = NLGUI::CDBManager::getInstance()->getDbProp(
...
public:
  void execute(CCtrlBase * /* pCaller */, const std::string & /* sParams */) {
    CInterfaceManager *pIM = CInterfaceManager::getInstance();
    CCDBNodeLeaf *dbTZ = NLGUI::CDBManager::getInstance()->getDbProp(
        "UI:SAVE:OUTPOST:TIME_ZONE", false);
    if (!dbTZ)
      return;
...
REGISTER_ACTION_HANDLER(CAHOutpostUpdateTimeZoneAuto,
