...
NLMISC_REGISTER_OBJECT(CViewBase, CDBGroupListSheetBag, std::string,
                       "list_sheet_bag");
NLMISC_REGISTER_OBJECT(CViewBase, CDBGroupIconListBag, std::string,
                       "list_icon_bag");
NLMISC_REGISTER_OBJECT(CViewBase, CDBGroupListSheetFilterCLMSlot, std::string,
                       "list_sheet_filter_clm_slot");
NLMISC_REGISTER_OBJECT(CViewBase, CDBGroupListSheetFilterExchangeable,
...
const uint CInventoryManager::NumInventories =
    sizeof(CInventoryManager::InventoryDBs) /
    sizeof(CInventoryManager::InventoryDBs[0]);
...
void CItemImage::build(CCDBNodeBranch *branch) {
  if (!branch)
    return;
  Sheet = dynamic_cast<CCDBNodeLeaf *>(
      branch->getNode(ICDBNode::CTextId("SHEET"), false));
  Quality = dynamic_cast<CCDBNodeLeaf *>(
      branch->getNode(ICDBNode::CTextId("QUALITY"), false));
  Quantity = dynamic_cast<CCDBNodeLeaf *>(
      branch->getNode(ICDBNode::CTextId("QUANTITY"), false));
  UserColor = dynamic_cast<CCDBNodeLeaf *>(
      branch->getNode(ICDBNode::CTextId("USER_COLOR"), false));
  CharacBuffs = dynamic_cast<CCDBNodeLeaf *>(
      branch->getNode(ICDBNode::CTextId("CHARAC_BUFFS"), false));
  Price = dynamic_cast<CCDBNodeLeaf *>(
      branch->getNode(ICDBNode::CTextId("PRICE"), false));
  Weight = dynamic_cast<CCDBNodeLeaf *>(
      branch->getNode(ICDBNode::CTextId("WEIGHT"), false));
  NameId = dynamic_cast<CCDBNodeLeaf *>(
      branch->getNode(ICDBNode::CTextId("NAMEID"), false));
  InfoVersion = dynamic_cast<CCDBNodeLeaf *>(
      branch->getNode(ICDBNode::CTextId("INFO_VERSION"), false));
  ResaleFlag = dynamic_cast<CCDBNodeLeaf *>(
      branch->getNode(ICDBNode::CTextId("RESALE_FLAG"), false));

  // Should always have at least those one:(ie all but Price)
  nlassert(Sheet && Quality && Quantity && UserColor && Weight && NameId &&
           InfoVersion);
}
...
const CClientItemInfo *CItemInfoCache::getItemInfo(uint32 serial,
...
void CItemInfoCache::debugItemInfoCache() const {
  ...
  uint count = 0;
  for (TItemInfoCacheMap::const_iterator it = _ItemInfoCacheMap.begin();
       it != _ItemInfoCacheMap.end(); ++it) {
    uint32 serial = (it->first >> 32) & 0xFFFFFFFF;
    uint32 created = it->first & 0xFFFFFFFF;
    nlinfo("[%-4d] cacheCycle:%d, serial:%d, createTime:%d", count++,
           it->second.CacheCycle, serial, created);
  }
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  pIM->displaySystemInfo(
      toString("ItemInfoCache: %d entries written to client.log",
               _ItemInfoCacheMap.size()));
}
...
CInventoryManager::CInventoryManager() {
  ...
  _ItemInfoCacheFilename =
      toString("save/item_infos_%d.cache", CharacterHomeSessionId.asInt());
  _ItemInfoCache.load(_ItemInfoCacheFilename);

#endif

  nlctassert(NumInventories ==
             sizeof(InventoryIndexes) / sizeof(InventoryIndexes[0]));
}
...
void CInventoryManager::init() {
  ...
  // Init observers for auto equipment
  {
    for (uint i = 0; i < MAX_BAGINV_ENTRIES; ++i) {
      CCDBNodeLeaf *pNL = NLGUI::CDBManager::getInstance()->getDbProp(
          LOCAL_INVENTORY ":BAG:" + toString(i) + ":SHEET");
  ...
  for (uint i = 0; i < MAX_INVENTORY_ANIMAL; i++)
    initItemArray(toString(LOCAL_INVENTORY ":PACK_ANIMAL%d", i), PAInv[i],
  ...
  ServerMoney =
      NLGUI::CDBManager::getInstance()->getDbProp(SERVER_INVENTORY ":MONEY");
  // Init Animals
  for (uint i = 0; i < MAX_INVENTORY_ANIMAL; i++)
    initItemArray(toString(SERVER_INVENTORY ":PACK_ANIMAL%d", i),
                  ServerPAInv[i], MAX_ANIMALINV_ENTRIES);

  // Drag'n'Drop
  DNDCurrentItem = NULL;
  DNDFrom = Nowhere;
  // Initialize interface part
  UIHands[0] = dynamic_cast<CDBCtrlSheet *>(
      CWidgetManager::getInstance()->getElementFromId(CTRL_HAND_RIGHT));
  UIHands[1] = dynamic_cast<CDBCtrlSheet *>(
      CWidgetManager::getInstance()->getElementFromId(CTRL_HAND_LEFT));

  UIEquip[SLOT_EQUIPMENT::HEADDRESS] = dynamic_cast<CDBCtrlSheet *>(
      CWidgetManager::getInstance()->getElementFromId(CTRL_JEWEL_HEADDRESS));
  UIEquip[SLOT_EQUIPMENT::EARL] = dynamic_cast<CDBCtrlSheet *>(
      CWidgetManager::getInstance()->getElementFromId(CTRL_JEWEL_EARING_LEFT));
  UIEquip[SLOT_EQUIPMENT::EARR] = dynamic_cast<CDBCtrlSheet *>(
      CWidgetManager::getInstance()->getElementFromId(CTRL_JEWEL_EARING_RIGHT));
  UIEquip[SLOT_EQUIPMENT::NECKLACE] = dynamic_cast<CDBCtrlSheet *>(
      CWidgetManager::getInstance()->getElementFromId(CTRL_JEWEL_NECK));
  UIEquip[SLOT_EQUIPMENT::WRISTL] = dynamic_cast<CDBCtrlSheet *>(
      CWidgetManager::getInstance()->getElementFromId(
          CTRL_JEWEL_BRACELET_LEFT));
  UIEquip[SLOT_EQUIPMENT::WRISTR] = dynamic_cast<CDBCtrlSheet *>(
      CWidgetManager::getInstance()->getElementFromId(
          CTRL_JEWEL_BRACELET_RIGHT));
  UIEquip[SLOT_EQUIPMENT::FINGERL] = dynamic_cast<CDBCtrlSheet *>(
      CWidgetManager::getInstance()->getElementFromId(CTRL_JEWEL_RING_LEFT));
  UIEquip[SLOT_EQUIPMENT::FINGERR] = dynamic_cast<CDBCtrlSheet *>(
      CWidgetManager::getInstance()->getElementFromId(CTRL_JEWEL_RING_RIGHT));
  UIEquip[SLOT_EQUIPMENT::ANKLEL] = dynamic_cast<CDBCtrlSheet *>(
      CWidgetManager::getInstance()->getElementFromId(CTRL_JEWEL_ANKLET_LEFT));
  UIEquip[SLOT_EQUIPMENT::ANKLER] = dynamic_cast<CDBCtrlSheet *>(
      CWidgetManager::getInstance()->getElementFromId(CTRL_JEWEL_ANKLET_RIGHT));

  UIEquip[SLOT_EQUIPMENT::HEAD] = dynamic_cast<CDBCtrlSheet *>(
      CWidgetManager::getInstance()->getElementFromId(CTRL_ARMOR_HEAD));
  UIEquip[SLOT_EQUIPMENT::CHEST] = dynamic_cast<CDBCtrlSheet *>(
      CWidgetManager::getInstance()->getElementFromId(CTRL_ARMOR_CHEST));
  UIEquip[SLOT_EQUIPMENT::ARMS] = dynamic_cast<CDBCtrlSheet *>(
      CWidgetManager::getInstance()->getElementFromId(CTRL_ARMOR_ARMS));
  UIEquip[SLOT_EQUIPMENT::FEET] = dynamic_cast<CDBCtrlSheet *>(
      CWidgetManager::getInstance()->getElementFromId(CTRL_ARMOR_FEET));
  UIEquip[SLOT_EQUIPMENT::LEGS] = dynamic_cast<CDBCtrlSheet *>(
      CWidgetManager::getInstance()->getElementFromId(CTRL_ARMOR_LEGS));
  UIEquip[SLOT_EQUIPMENT::HANDS] = dynamic_cast<CDBCtrlSheet *>(
      CWidgetManager::getInstance()->getElementFromId(CTRL_ARMOR_HANDS));

  UIEquip2[SLOT_EQUIPMENT::HEADDRESS] = dynamic_cast<CDBCtrlSheet *>(
      CWidgetManager::getInstance()->getElementFromId(CTRL_JEWL2_HEADDRESS));
  UIEquip2[SLOT_EQUIPMENT::EARL] = dynamic_cast<CDBCtrlSheet *>(
      CWidgetManager::getInstance()->getElementFromId(CTRL_JEWL2_EARING_LEFT));
  UIEquip2[SLOT_EQUIPMENT::EARR] = dynamic_cast<CDBCtrlSheet *>(
      CWidgetManager::getInstance()->getElementFromId(CTRL_JEWL2_EARING_RIGHT));
  UIEquip2[SLOT_EQUIPMENT::NECKLACE] = dynamic_cast<CDBCtrlSheet *>(
      CWidgetManager::getInstance()->getElementFromId(CTRL_JEWL2_NECK));
  UIEquip2[SLOT_EQUIPMENT::WRISTL] = dynamic_cast<CDBCtrlSheet *>(
      CWidgetManager::getInstance()->getElementFromId(
          CTRL_JEWL2_BRACELET_LEFT));
  UIEquip2[SLOT_EQUIPMENT::WRISTR] = dynamic_cast<CDBCtrlSheet *>(
      CWidgetManager::getInstance()->getElementFromId(
          CTRL_JEWL2_BRACELET_RIGHT));
  UIEquip2[SLOT_EQUIPMENT::FINGERL] = dynamic_cast<CDBCtrlSheet *>(
      CWidgetManager::getInstance()->getElementFromId(CTRL_JEWL2_RING_LEFT));
  UIEquip2[SLOT_EQUIPMENT::FINGERR] = dynamic_cast<CDBCtrlSheet *>(
      CWidgetManager::getInstance()->getElementFromId(CTRL_JEWL2_RING_RIGHT));
  UIEquip2[SLOT_EQUIPMENT::ANKLEL] = dynamic_cast<CDBCtrlSheet *>(
      CWidgetManager::getInstance()->getElementFromId(CTRL_JEWL2_ANKLET_LEFT));
  UIEquip2[SLOT_EQUIPMENT::ANKLER] = dynamic_cast<CDBCtrlSheet *>(
      CWidgetManager::getInstance()->getElementFromId(CTRL_JEWL2_ANKLET_RIGHT));

  UIEquip2[SLOT_EQUIPMENT::HEAD] = dynamic_cast<CDBCtrlSheet *>(
      CWidgetManager::getInstance()->getElementFromId(CTRL_ARMR2_HEAD));
  UIEquip2[SLOT_EQUIPMENT::CHEST] = dynamic_cast<CDBCtrlSheet *>(
      CWidgetManager::getInstance()->getElementFromId(CTRL_ARMR2_CHEST));
  UIEquip2[SLOT_EQUIPMENT::ARMS] = dynamic_cast<CDBCtrlSheet *>(
      CWidgetManager::getInstance()->getElementFromId(CTRL_ARMR2_ARMS));
  UIEquip2[SLOT_EQUIPMENT::FEET] = dynamic_cast<CDBCtrlSheet *>(
      CWidgetManager::getInstance()->getElementFromId(CTRL_ARMR2_FEET));
  UIEquip2[SLOT_EQUIPMENT::LEGS] = dynamic_cast<CDBCtrlSheet *>(
      CWidgetManager::getInstance()->getElementFromId(CTRL_ARMR2_LEGS));
  UIEquip2[SLOT_EQUIPMENT::HANDS] = dynamic_cast<CDBCtrlSheet *>(
      CWidgetManager::getInstance()->getElementFromId(CTRL_ARMR2_HANDS));

  // Init ItemInfoObservers
  {
    CCDBNodeLeaf *nodeTS;

    nodeTS = NLGUI::CDBManager::getInstance()->getDbProp(
        "SERVER:TRADING:SESSION", false);
    if (nodeTS) {
      ICDBNode::CTextId textId;
    ...
    for (uint i = 0; i < NumInventories; i++) {
      ...
      if (InventoryIndexes[i] != INVENTORIES::trading) {
        ...
        for (uint j = 0;; j++) {
          CCDBNodeLeaf *nodeIV = NLGUI::CDBManager::getInstance()->getDbProp(
              "SERVER:" + InventoryDBs[i] + ":" + toString(j) + ":INFO_VERSION",
              false);
          CCDBNodeLeaf *nodeSH = NLGUI::CDBManager::getInstance()->getDbProp(
              "SERVER:" + InventoryDBs[i] + ":" + toString(j) + ":SHEET",
              false);
          if (nodeIV && nodeSH) {
...
// *************************************************************************************************
void CInventoryManager::initItemArray(const std::string &dbBranchName,
                                      CItemImage *dest, uint numItems) {
  nlassert(dest);
  CInterfaceManager *im = CInterfaceManager::getInstance();
  CCDBNodeBranch *branch =
      NLGUI::CDBManager::getInstance()->getDbBranch(dbBranchName);
  if (!branch) {
    nlwarning("Can't init inventory image from branch %s.",
              dbBranchName.c_str());
    return;
  }
  for (uint k = 0; k < numItems; ++k) {
    CCDBNodeBranch *itemBranch =
        dynamic_cast<CCDBNodeBranch *>(branch->getNode((uint16)k));
    if (!itemBranch) {
      nlwarning("Can't retrieve item %d of branch %s", (int)k,
                dbBranchName.c_str());
    } else {
...
// ***************************************************************************
void CInventoryManager::initIndirection(const std::string &dbbranch,
                                        sint32 *indices, sint32 nbIndex,
                                        bool putObs) {
  CInterfaceManager *im = CInterfaceManager::getInstance();
  for (uint i = 0; i < (uint)nbIndex; ++i) {
    CCDBNodeLeaf *pNL = NLGUI::CDBManager::getInstance()->getDbProp(
        dbbranch + toString(i) + ":INDEX_IN_BAG");
    if (putObs) {
      ICDBNode::CTextId textId;
...
// ***************************************************************************
// Used for interface objects which are reference in bag (the getSheet() returns
// INDEX_IN_BAG)
...
bool CInventoryManager::is2HandItem(uint32 sheetID) {
  ...
  if (sheet && sheet->type() == CEntitySheet::ITEM) {
    ...
    if (item->hasSlot(SLOTTYPE::TWO_HANDS) ||
        item->hasSlot(SLOTTYPE::RIGHT_HAND_EXCLUSIVE))
      result = true;
  }
  return result;
}
...
uint32 CInventoryManager::getHandItemSheet(bool rightHand) const {
  ...
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  string dbPropPath =
      toString("LOCAL:INVENTORY:HAND:%d:INDEX_IN_BAG", rightHand ? 0 : 1);
  // get the RightHand bag index
  sint32 itemSlot =
      NLGUI::CDBManager::getInstance()->getDbProp(dbPropPath)->getValue32();
  // if something in hand
  if (itemSlot > 0) {
    CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
        "LOCAL:INVENTORY:BAG:" + toString(itemSlot - 1) + ":SHEET", false);
    if (node)
...
// ***************************************************************************
bool CInventoryManager::isLeftHandItemCompatibleWithRightHandItem(
    uint32 leftHandSheet, uint32 rightHandSheet, uint32 lastRightHandSheet) {
  CEntitySheet *pLastRight = SheetMngr.get(CSheetId(lastRightHandSheet));
  if (pLastRight != NULL) {
    if (pLastRight->type() != CEntitySheet::ITEM)
      return false;
    CItemSheet *pIsLastRight = (CItemSheet *)pLastRight;

    // Last item in right hand is a 2 hand item and the new item is nothing
    // (unequip)
    if (pIsLastRight->hasSlot(SLOTTYPE::TWO_HANDS) ||
        pIsLastRight->hasSlot(SLOTTYPE::RIGHT_HAND_EXCLUSIVE))
      return false;
  }
  ...
  if (pLeft == NULL)
    return false;
  if (pLeft->type() != CEntitySheet::ITEM)
    return false;
  ...
  if (pRight == NULL)
    return true;
  if (pRight->type() != CEntitySheet::ITEM)
    return true;
  ...
  if (pIsLeft->ItemType == ITEM_TYPE::DAGGER) {
    if ((pIsRight->ItemType == ITEM_TYPE::SWORD) ||
        (pIsRight->ItemType == ITEM_TYPE::DAGGER))
      return true;
    else
      return false;
  }

  if (!pIsRight->hasSlot(SLOTTYPE::TWO_HANDS) &&
      !pIsRight->hasSlot(SLOTTYPE::RIGHT_HAND_EXCLUSIVE)) {
    return true;
  }
...
// ***************************************************************************
static void grayItem(const std::string &listname, sint32 bagEntryIndex,
                     bool gray) {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();

  IListSheetBase *pList = dynamic_cast<IListSheetBase *>(
      CWidgetManager::getInstance()->getElementFromId(listname));
...
static bool isSwimming() {
  if (UserEntity != NULL)
    return (UserEntity->mode() == MBEHAV::SWIM ||
            UserEntity->mode() == MBEHAV::MOUNT_SWIM);
  else
    return false;
}
...
//
// ----------------------------------------------
void CInventoryManager::equip(const std::string &bagPath,
  ...
  // Hands management : check if we have to unequip left hand because of
  // incompatibility with right hand item
  sint16 oldRightIndexInBag = NLGUI::CDBManager::getInstance()
                                  ->getDbProp(invPath + ":INDEX_IN_BAG")
                                  ->getValue16();
  if (inventory == INVENTORIES::handling && invSlot == 0) {
    CDBCtrlSheet *pCSLeftHand = dynamic_cast<CDBCtrlSheet *>(
        CWidgetManager::getInstance()->getElementFromId(CTRL_HAND_LEFT));
    if (pCSLeftHand == NULL) {
      return;
    }
    ...
    uint32 leftSheet = 0;
    CCDBNodeLeaf *pNL = NLGUI::CDBManager::getInstance()->getDbProp(
        LOCAL_INVENTORY ":HAND:1:INDEX_IN_BAG", false);
    if (pNL == NULL) {
      return;
    }
    if (pNL->getValue32() > 0) {
      CCDBNodeLeaf *pNL2 = NLGUI::CDBManager::getInstance()->getDbProp(
          LOCAL_INVENTORY ":BAG:" + toString(pNL->getValue32() - 1) + ":SHEET",
          false);
      if (pNL2 == NULL) {
        return;
      }
    ...
    if (oldRightIndexInBag > 0) {
      pNL = NLGUI::CDBManager::getInstance()->getDbProp(
          LOCAL_INVENTORY ":BAG:" + toString(oldRightIndexInBag - 1) + ":SHEET",
          false);
      if (pNL == NULL) {
        return;
      }
    ...
    if (indexInBag + 1 > 0) {
      pNL = NLGUI::CDBManager::getInstance()->getDbProp(
          LOCAL_INVENTORY ":BAG:" + toString(indexInBag) + ":SHEET", false);
      if (pNL == NULL) {
        return;
      }
    ...
    if (!getInventory().isLeftHandItemCompatibleWithRightHandItem(
            leftSheet, rightSheet, lastRightSheet)) {
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp(invPath + ":INDEX_IN_BAG")
      ->setValue16(indexInBag + 1);

  // Yoyo add: when the user equip an item, the action are invalid during some
  // time
  if (indexInBag < MAX_BAGINV_ENTRIES) {
    CItemSheet *pIS = dynamic_cast<CItemSheet *>(
        SheetMngr.get(CSheetId(getBagItem(indexInBag).getSheetID())));
    if (pIS) {
  ...
  if (inventory != INVENTORIES::UNDEFINED) {
    ...
    if (GenericMsgHeaderMngr.pushNameToStream(sMsg, out)) {
      // Fill the message (equipped inventory, equipped inventory slot, bag
      // slot)
      ...
      // nlinfo("impulseCallBack : %s %d %d %d sent", sMsg.c_str(), inventory,
      // invSlot, indexInBag);
    } else {
...
void CInventoryManager::unequip(const std::string &invPath) {
  if (isSwimming() || isStunned() || isDead())
    return;

  CInterfaceManager *pIM = CInterfaceManager::getInstance();

  sint16 oldIndexInBag = NLGUI::CDBManager::getInstance()
                             ->getDbProp(invPath + ":INDEX_IN_BAG")
                             ->getValue16();
  if (oldIndexInBag == 0) {
    return;
  ...
  if (inventory == INVENTORIES::handling && invSlot == 0) {
    ...
    if (oldIndexInBag > 0) {
      pNL = NLGUI::CDBManager::getInstance()->getDbProp(
          LOCAL_INVENTORY ":BAG:" + toString(oldIndexInBag - 1) + ":SHEET",
          false);
      if (pNL == NULL) {
        return;
      }
  ...
  if (inventory != INVENTORIES::UNDEFINED) {
    ...
    if (GenericMsgHeaderMngr.pushNameToStream(sMsg, out)) {
      ...
      // nlinfo("impulseCallBack : %s %d %d sent", sMsg.c_str(), inventory,
      // invSlot);
    } else {
      nlwarning("don't know message name %s", sMsg.c_str());
    }
...
void CInventoryManager::CDBEquipObs::update(ICDBNode *node) {
  ...
  if (strnicmp(sTmp.c_str(), "LOCAL:INVENTORY:HAND", 20) == 0) {
    ...
    case SLOT_EQUIPMENT::HEADDRESS:
      sIE = CTRL_JEWEL_HEADDRESS;
      sIE2 = CTRL_JEWL2_HEADDRESS;
      break;
    case SLOT_EQUIPMENT::EARL:
      sIE = CTRL_JEWEL_EARING_LEFT;
      sIE2 = CTRL_JEWL2_EARING_LEFT;
      break;
    case SLOT_EQUIPMENT::EARR:
      sIE = CTRL_JEWEL_EARING_RIGHT;
      sIE2 = CTRL_JEWL2_EARING_RIGHT;
      break;
    case SLOT_EQUIPMENT::NECKLACE:
      sIE = CTRL_JEWEL_NECK;
      sIE2 = CTRL_JEWL2_NECK;
      break;
    case SLOT_EQUIPMENT::WRISTL:
      sIE = CTRL_JEWEL_BRACELET_LEFT;
      sIE2 = CTRL_JEWL2_BRACELET_LEFT;
      break;
    case SLOT_EQUIPMENT::WRISTR:
      sIE = CTRL_JEWEL_BRACELET_RIGHT;
      sIE2 = CTRL_JEWL2_BRACELET_RIGHT;
      break;
    case SLOT_EQUIPMENT::FINGERL:
      sIE = CTRL_JEWEL_RING_LEFT;
      sIE2 = CTRL_JEWL2_RING_LEFT;
      break;
    case SLOT_EQUIPMENT::FINGERR:
      sIE = CTRL_JEWEL_RING_RIGHT;
      sIE2 = CTRL_JEWL2_RING_RIGHT;
      break;
    case SLOT_EQUIPMENT::ANKLEL:
      sIE = CTRL_JEWEL_ANKLET_LEFT;
      sIE2 = CTRL_JEWL2_ANKLET_LEFT;
      break;
    case SLOT_EQUIPMENT::ANKLER:
      sIE = CTRL_JEWEL_ANKLET_RIGHT;
      sIE2 = CTRL_JEWL2_ANKLET_RIGHT;
      break;

    case SLOT_EQUIPMENT::HEAD:
      sIE = CTRL_ARMOR_HEAD;
      sIE2 = CTRL_ARMR2_HEAD;
      break;
    case SLOT_EQUIPMENT::CHEST:
      sIE = CTRL_ARMOR_CHEST;
      sIE2 = CTRL_ARMR2_CHEST;
      break;
    ...
    case SLOT_EQUIPMENT::FEET:
      sIE = CTRL_ARMOR_FEET;
      sIE2 = CTRL_ARMR2_FEET;
      break;
    case SLOT_EQUIPMENT::LEGS:
      sIE = CTRL_ARMOR_LEGS;
      sIE2 = CTRL_ARMR2_LEGS;
      break;
    case SLOT_EQUIPMENT::HANDS:
      sIE = CTRL_ARMOR_HANDS;
      sIE2 = CTRL_ARMR2_HANDS;
      break;
  ...
  CDBCtrlSheet *pCS = dynamic_cast<CDBCtrlSheet *>(
      CWidgetManager::getInstance()->getElementFromId(sIE));
  CDBCtrlSheet *pCS2 = dynamic_cast<CDBCtrlSheet *>(
      CWidgetManager::getInstance()->getElementFromId(sIE2));
  ...
  // Update Display
  if (newVal == 0) {
    // in some case left sheet is same than right sheet so don't clear it now
    // (ex: 2 hands item, right hand exclusive)
    ...
    if (pCS != NULL)
      pCS->setSheet(LOCAL_INVENTORY ":BAG:" + toString(newVal - 1));
    if (pCS2 != NULL)
      pCS2->setSheet(LOCAL_INVENTORY ":BAG:" + toString(newVal - 1));
  }
  ...
  if (sIE == CTRL_HAND_RIGHT) {
    ...
    {
      uint32 leftSheet = 0;
      CCDBNodeLeaf *pNL3 = NLGUI::CDBManager::getInstance()->getDbProp(
          LOCAL_INVENTORY ":HAND:1:INDEX_IN_BAG", false);
      if (pNL3 == NULL)
        return;
      if (pNL3->getValue32() > 0) {
        CCDBNodeLeaf *pNL4 = NLGUI::CDBManager::getInstance()->getDbProp(
            LOCAL_INVENTORY ":BAG:" + toString(pNL3->getValue32() - 1) +
                ":SHEET",
            false);
        if (pNL4 == NULL)
          return;
      ...
      uint32 rightSheet = 0;
      if (newVal > 0) {
        pNL3 = NLGUI::CDBManager::getInstance()->getDbProp(
            LOCAL_INVENTORY ":BAG:" + toString(newVal - 1) + ":SHEET", false);
        if (pNL3 == NULL)
          return;
      ...
      if (oldVal > 0) {
        pNL3 = NLGUI::CDBManager::getInstance()->getDbProp(
            LOCAL_INVENTORY ":BAG:" + toString(oldVal - 1) + ":SHEET", false);
        if (pNL3 == NULL)
          return;
      ...
      // WORKAROUND: useful when an item is destroyed before it is unequipped
      // (clean the left hand)
    ...
    if (newVal > 0) {
      ...
      if (getInventory().is2HandItem(pNL2->getValue32())) {
        if (getInventory().isRangeWeaponItem(pNL2->getValue32())) {
          ...
          pCSLeftHand->setTextureNoItem(
              rVR.getTextureIdFromName("W_AM_logo.tga"));
        } else {
  ...
  if (sIE == CTRL_HAND_LEFT) {
    ...
    if (pCSLeftHand) {
      ...
      if (newVal == 0) {
        // check if we clear display (have to manage 2 hands weapons for
        // instance)
        bool clearLeftHandDisplay = true;
        CDBCtrlSheet *pCSRightHand = dynamic_cast<CDBCtrlSheet *>(
            CWidgetManager::getInstance()->getElementFromId(CTRL_HAND_RIGHT));
        if (pCSRightHand && pCSRightHand->getSheetId()) {
          CCDBNodeLeaf *pNL3 = NLGUI::CDBManager::getInstance()->getDbProp(
              LOCAL_INVENTORY ":HAND:0:INDEX_IN_BAG", false);
          if (pNL3) {
            if (pNL3->getValue32() > 0) {
              CCDBNodeLeaf *pNL4 = NLGUI::CDBManager::getInstance()->getDbProp(
                  LOCAL_INVENTORY ":BAG:" + toString(pNL3->getValue32() - 1) +
              ...
              if (pNL4) {
                ...
                if (getInventory().is2HandItem(rightSheet)) {
                  if (getInventory().isRangeWeaponItem(rightSheet)) {
                    pCSLeftHand->setItemSlot(
                        SLOTTYPE::stringToSlotType("AMMO"));
                    pCSLeftHand->setTextureNoItem(
                        rVR.getTextureIdFromName("W_AM_logo.tga"));
                  } else {
                    pCSLeftHand->setItemSlot(
                        SLOTTYPE::stringToSlotType("LEFT_HAND"));
                    pCSLeftHand->setTextureNoItem(
                        rVR.getTextureIdFromName("hand_left.tga"));
...
void CInventoryManager::CDBBagObs::update(ICDBNode * /* node */) {
  ...
  // AUTO EQUIP the player with incoming item if we can put this item in an
  // equipment slot
...
bool CInventoryManager::autoEquip(sint bagEntryIndex, bool allowReplace) {
  ...
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  IListSheetBase *pList = dynamic_cast<IListSheetBase *>(
      CWidgetManager::getInstance()->getElementFromId(LIST_BAG_TEXT));
  ...
  for (i = 0; i < MAX_HANDINV_ENTRIES; ++i) {
    ...
    sint32 indexDstPath = NLGUI::CDBManager::getInstance()
                              ->getDbProp(dstPath + ":INDEX_IN_BAG")
                              ->getValue16();
...
// ***************************************************************************
void CInventoryManager::dropOrDestroyItem(CDBCtrlSheet *item,
                                          CBitMemStream &out, uint16 quantity) {
  if (!item)
    return;
...
static void checkEquipmentIntegrity(const string &equipVal) {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  CCDBNodeLeaf *pNL = NLGUI::CDBManager::getInstance()->getDbProp(
      equipVal + ":INDEX_IN_BAG", false);
  if (pNL != NULL) {
    uint32 indexInBag = pNL->getValue16();
    if (indexInBag != 0) {
      string sTmp = string(LOCAL_INVENTORY) +
                    ":BAG:" + toString(indexInBag - 1) + ":SHEET";
      CCDBNodeLeaf *pNLBag =
          NLGUI::CDBManager::getInstance()->getDbProp(sTmp, false);
      if (pNLBag != NULL) {
        if (pNLBag->getValue32() == 0) // If no more item in this slot bag
        {
          //					if (!
          //IngameDbMngr.initInProgress())			// Commented
          //because init is end when we received equipment 						pNL->setValue16(0);
          //// Reset INDEX_IN_BAG
        } else {
          // If the slot was previously empty check that there is no reference
          // on it else update the reference
...
// ***************************************************************************
double CInventoryManager::getBranchBulk(const string &basePath,
                                        uint16 startItemIndex,
                                        uint16 numItems) {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  CCDBNodeBranch *branch =
      NLGUI::CDBManager::getInstance()->getDbBranch(basePath);
  if (!branch) {
  ...
  double totalBulk = 0;
  //
  uint16 lastIndexItem = std::min((uint16)(startItemIndex + numItems),
                                  (uint16)branch->getNbNodes());
  for (uint16 currItem = startItemIndex; currItem < lastIndexItem; ++currItem) {
    ICDBNode *node = branch->getNode(currItem);
    if (node) {
      CCDBNodeLeaf *sheetNode = dynamic_cast<CCDBNodeLeaf *>(
          node->getNode(ICDBNode::CTextId("SHEET")));
      CCDBNodeLeaf *quantityNode = dynamic_cast<CCDBNodeLeaf *>(
          node->getNode(ICDBNode::CTextId("QUANTITY")));
      if (sheetNode && quantityNode) {
        // get the Sheet
        CSheetId sheetId = CSheetId(sheetNode->getValue32());
        if (sheetId != CSheetId::Unknown) {
          CItemSheet *itemSheet =
              dynamic_cast<CItemSheet *>(SheetMngr.get(sheetId));
          if (itemSheet) {
            totalBulk += std::max((sint32)1, quantityNode->getValue32()) *
                         itemSheet->Bulk;
          }
        }
      }
 ...
 */
void CInventoryManager::getBranchSlotCounts(const std::string &basePath,
                                            uint &nbUsedSlots,
                                            uint &nbMaxSlots) {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  CCDBNodeBranch *branch =
  ...
  nbMaxSlots = 0; // different from nbNodes, because there can be non-slots
                  // leaves (e.g. guild money...)
  nbUsedSlots = 0;
...
double CInventoryManager::getBagBulk(uint32 inventoryIndex) {
  ...
  if (inventoryIndex == 0)
    ...
    return getBranchBulk(LOCAL_INVENTORY ":PACK_ANIMAL0", 0,
                         MAX_ANIMALINV_ENTRIES);
  else if (inventoryIndex == 2)
    return getBranchBulk(LOCAL_INVENTORY ":PACK_ANIMAL1", 0,
                         MAX_ANIMALINV_ENTRIES);
  else if (inventoryIndex == 3)
    return getBranchBulk(LOCAL_INVENTORY ":PACK_ANIMAL2", 0,
                         MAX_ANIMALINV_ENTRIES);
  else if (inventoryIndex == 4)
    return getBranchBulk(LOCAL_INVENTORY ":PACK_ANIMAL3", 0,
                         MAX_ANIMALINV_ENTRIES);
...
double CInventoryManager::getItemBulk(uint32 sheetID) {
  CItemSheet *itemSheet =
      dynamic_cast<CItemSheet *>(SheetMngr.get(CSheetId(sheetID)));
  if (itemSheet)
...
double CInventoryManager::getMaxBagBulk(uint32 inventoryIndex) {
  ...
  if (inventoryIndex == 0)
    pNL = NLGUI::CDBManager::getInstance()->getDbProp(
        "SERVER:STATIC_DATA:BAG_BULK_MAX");
  else if (inventoryIndex == 1)
    pNL = NLGUI::CDBManager::getInstance()->getDbProp(
        "SERVER:PACK_ANIMAL:BEAST0:BULK_MAX");
  else if (inventoryIndex == 2)
    pNL = NLGUI::CDBManager::getInstance()->getDbProp(
        "SERVER:PACK_ANIMAL:BEAST1:BULK_MAX");
  else if (inventoryIndex == 3)
    pNL = NLGUI::CDBManager::getInstance()->getDbProp(
        "SERVER:PACK_ANIMAL:BEAST2:BULK_MAX");
  else if (inventoryIndex == 4)
    pNL = NLGUI::CDBManager::getInstance()->getDbProp(
        "SERVER:PACK_ANIMAL:BEAST3:BULK_MAX");
  if (pNL != NULL)
...
bool CInventoryManager::isSpaceInAllBagsForItem(CDBCtrlSheet *item) {
  ...
  if (!pCSDst->isSheetValid())
    return false;
  string sTmp = pCSDst->getSheet();
  CCDBNodeLeaf *pNL =
      NLGUI::CDBManager::getInstance()->getDbProp(sTmp + ":SHEET", false);
  CCDBNodeLeaf *pNLquantity =
      NLGUI::CDBManager::getInstance()->getDbProp(sTmp + ":QUANTITY", false);
  if (pNL == NULL)
    return false;
  if (pNLquantity == NULL)
    return false;
  ...
  for (uint32 i = 0; i < 7; ++i) {
    if (pInv->isInventoryAvailable(
            (INVENTORIES::TInventory)CInventoryManager::InventoryIndexes[i]))
...
// ***************************************************************************
bool CInventoryManager::isSpaceInBagForItem(CDBCtrlSheet *item, uint32 quantity,
                                            uint32 bagId) {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  CDBCtrlSheet *pCSDst = item;
  if (!pCSDst->isSheetValid())
  ...
  double totalBulk = quantity * pInv->getItemBulk(pNL->getValue32());
  if (pInv->isInventoryAvailable(
          (INVENTORIES::TInventory)CInventoryManager::InventoryIndexes[bagId]))
...
CTempInvManager::CTempInvManager() {
  ...
  CCDBNodeLeaf *pNL =
      NLGUI::CDBManager::getInstance()->getDbProp(sPath + ":TYPE", false);
  if (pNL) {
    ICDBNode::CTextId textId;
...
void CTempInvManager::update() {
  ...
  _Mode = (TEMP_INV_MODE::TInventoryMode)NLGUI::CDBManager::getInstance()
              ->getDbProp("LOCAL:INVENTORY:TEMP:TYPE")
              ->getValue8();
  ...
  if (_Mode == TEMP_INV_MODE::Forage) {
    ...
    bool disableTake = (NLGUI::CDBManager::getInstance()
                            ->getDbProp(sPath + ":ENABLE_TAKE")
                            ->getValue32() == 0);
    NLGUI::CDBManager::getInstance()
        ->getDbProp("UI:TEMP_INV:ALL_EMPTY")
        ->setValue32(disableTake);
    if (disableTake) {
  ...
  if (bAllEmpty) {
    ...
    CAHManager::getInstance()->runActionHandler(
        "phrase_update_all_memory_ctrl_regen_tick_range", NULL);
  } else {
    pGC->setActive(true);
    CAHManager::getInstance()->runActionHandler(
        "phrase_update_all_memory_ctrl_regen_tick_range", NULL);
    ...
    case TEMP_INV_MODE::Loot:
      pGC->setTitle(WIN_TEMPINV_TITLE_LOOT);
      break;
    case TEMP_INV_MODE::Quarter:
      pGC->setTitle(WIN_TEMPINV_TITLE_QUARTERING);
      break;
    case TEMP_INV_MODE::Forage: /* see above */
      break;
    case TEMP_INV_MODE::BagFull:
      pGC->setTitle(WIN_TEMPINV_TITLE_BAGFULL);
      break;
    case TEMP_INV_MODE::Craft:
      pGC->setTitle(WIN_TEMPINV_TITLE_CRAFT);
      break;
    case TEMP_INV_MODE::MissionReward:
      pGC->setTitle(WIN_TEMPINV_TITLE_MISSIONREWARD);
      break;
    case TEMP_INV_MODE::Crystallize:
      pGC->setTitle(WIN_TEMPINV_TITLE_CRYSTALLIZE);
      break;

    case TEMP_INV_MODE::Unknown:
    default:
      pGC->setTitle(WIN_TEMPINV_TITLE_ERROR);
      break;
    }
  }
}
...
void CTempInvManager::updateType() {
  ...
  case TEMP_INV_MODE::Unknown:
  default: /*pGC->setTitle(WIN_TEMPINV_TITLE_ERROR);*/ // do not overwrite a
                                                       // locally-set title with
                                                       // the default one when
                                                       // ServerAutoCopy syncs
                                                       // the local db during
                                                       // 'make item' (craft)
    break;
  }
...
void CTempInvManager::updateForageQQ(uint whichOne) {
  ...
  // Avoid recursion, because we can't call CCDBNodeLeaf::setValue16() without
  // calling observers!
  ...
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  bool disableTake = (NLGUI::CDBManager::getInstance()
                          ->getDbProp("LOCAL:INVENTORY:TEMP:ENABLE_TAKE")
                          ->getValue32() == 0);
  if (disableTake) {
    float qt = 0.f, ql = 0.f;
    switch (whichOne) {
    case 0: {
      CCDBNodeLeaf *leafQt = NLGUI::CDBManager::getInstance()->getDbProp(
          "LOCAL:INVENTORY:TEMP:0:QUANTITY");
    ...
    case 1: {
      CCDBNodeLeaf *leafQl = NLGUI::CDBManager::getInstance()->getDbProp(
          "LOCAL:INVENTORY:TEMP:0:QUALITY");
...
void CTempInvManager::open(TEMP_INV_MODE::TInventoryMode m) {
  ...
  // In Foraging mode, we can call open() on the inventory with the same
  // contents (e.g. when changing Forage action)
  if (_Mode != TEMP_INV_MODE::Forage) {
    string sPath = string("LOCAL:INVENTORY:TEMP");
    for (uint i = 0; i < MAX_TEMPINV_ENTRIES; i++) {
      string sTmp = sPath + ":" + toString(i) + ":SHEET";
  ...
  if (pGC != NULL) {
    switch (_Mode) {
    case TEMP_INV_MODE::Loot:
      pGC->setTitle(WIN_TEMPINV_TITLE_WAIT_LOOT);
      break;
    case TEMP_INV_MODE::Quarter:
      pGC->setTitle(WIN_TEMPINV_TITLE_WAIT_QUARTERING);
      break;
    case TEMP_INV_MODE::Forage:
      pGC->setTitle(WIN_TEMPINV_TITLE_WAIT_FORAGING);
      break;
    case TEMP_INV_MODE::BagFull:
      pGC->setTitle(WIN_TEMPINV_TITLE_WAIT_BAGFULL);
      break;
    case TEMP_INV_MODE::Craft:
      pGC->setTitle(WIN_TEMPINV_TITLE_WAIT_CRAFT);
      break;
    case TEMP_INV_MODE::MissionReward:
      pGC->setTitle(WIN_TEMPINV_TITLE_WAIT_MISSIONREWARD);
      break;
    case TEMP_INV_MODE::Crystallize:
      pGC->setTitle(WIN_TEMPINV_TITLE_WAIT_CRYSTALLIZE);
      break;

    case TEMP_INV_MODE::Unknown:
    default:
      pGC->setTitle(WIN_TEMPINV_TITLE_WAIT_ERROR);
      break;
    };
...
void CTempInvManager::close() {
  ...
  CInterfaceGroup *pIG = dynamic_cast<CInterfaceGroup *>(
      CWidgetManager::getInstance()->getElementFromId(WIN_TEMPINV));
  if (pIG != NULL) {
    pIG->setActive(false);
    CAHManager::getInstance()->runActionHandler(
...
// Used for sorting
void initStructForItemSort(vector<SSortStruct> &vTemp, sint32 sheetId,
                           sint32 quality, sint32 indexInList,
                           sint32 indexInDB) {
  ...
  if (sheetId != 0) {
    ...
    if ((pItem != NULL) && (pItem->Type == CEntitySheet::ITEM)) {
      ...
      if (pIS->Family == ITEMFAMILY::TELEPORT)
        vTemp[indexInList].Pos +=
            toString("%02d%02d", pIS->ItemOrigin, pIS->Teleport.Type);
      else
      ...
      // add at last the index in DB. to avoid resort for items that are exaclty
      // the same
...
bool SBagOptions::parse(xmlNodePtr cur, CInterfaceGroup * /* parentGroup */) {
  ...
  if (prop)
    DbFilterArmor = NLGUI::CDBManager::getInstance()->getDbProp(prop.str());

  prop = xmlGetProp(cur, (xmlChar *)"filter_weapon");
  if (prop)
    DbFilterWeapon = NLGUI::CDBManager::getInstance()->getDbProp(prop.str());

  prop = xmlGetProp(cur, (xmlChar *)"filter_tool");
  if (prop)
    DbFilterTool = NLGUI::CDBManager::getInstance()->getDbProp(prop.str());

  prop = xmlGetProp(cur, (xmlChar *)"filter_pet");
  if (prop)
    DbFilterPet = NLGUI::CDBManager::getInstance()->getDbProp(prop.str());

  prop = xmlGetProp(cur, (xmlChar *)"filter_mp");
  if (prop)
    DbFilterMP = NLGUI::CDBManager::getInstance()->getDbProp(prop.str());

  prop = xmlGetProp(cur, (xmlChar *)"filter_missmp");
  if (prop)
    DbFilterMissMP = NLGUI::CDBManager::getInstance()->getDbProp(prop.str());

  prop = xmlGetProp(cur, (xmlChar *)"filter_tp");
  if (prop)
    DbFilterTP = NLGUI::CDBManager::getInstance()->getDbProp(prop.str());
...
bool SBagOptions::canDisplay(CDBCtrlSheet *pCS) const {
  ...
  if (pIS != NULL) {
    ...
    // Quality range
    if (SearchQualityMin > pCS->getQuality() ||
    ...
    if ((pIS->Family == ITEMFAMILY::MISSION_ITEM) ||
        ...
        ((pIS->Family == ITEMFAMILY::RAW_MATERIAL) &&
         !pIS->canBuildSomeItemPart()))
      if (!bFilterMissMP)
        bDisplay = false;
...
// ***************************************************************************
void CDBGroupListSheetBag::CSheetChildBag::updateViewText(
...
// ***************************************************************************
bool CDBGroupListSheetBag::CSheetChildBag::isSheetValid(
    CDBGroupListSheetText *pFather) {
...
// ***************************************************************************
void CDBGroupListSheetBag::CSheetChildBag::init(CDBGroupListSheetText *pFather,
                                                uint index) {
  // init my parent
...
// ***************************************************************************
bool CDBGroupListSheetBag::CSheetChildBag::isInvalidated(
    CDBGroupListSheetText * /* pFather */) {
  // quality change
...
// ***************************************************************************
void CDBGroupListSheetBag::CSheetChildBag::update(
    CDBGroupListSheetText * /* pFather */) {
  LastQuality = CurrentQuality.getSInt32();
}

// ***************************************************************************
void CDBGroupListSheetBag::onSwap(sint /* nDraggedSheet */,
                                  sint /* nDroppedSheet */) {
...
void CDBGroupListSheetBag::sort() {
  ...
  for (i = 0; i < _MaxItems; ++i) {
    ...
    initStructForItemSort(vTemp, ctrl->getSheetId(), ctrl->getQuality(), i,
                          ctrl->getIndexInDB());
  }
...
// ***************************************************************************
bool CDBGroupIconListBag::CSheetChildBag::isSheetValid(
    CDBGroupListSheet *pFather) {
  if (CSheetChild::isSheetValid(pFather)) {
    // Check if the control match the filters !
...
// ***************************************************************************
bool CDBGroupListSheetFilterCLMSlot::CSheetChildFilter::isSheetValid(
    CDBGroupListSheet *pFather) {
  if (CSheetChild::isSheetValid(pFather)) {
    /* This filter look the ctrl who launch the modal where this list is
       displayed. If we can drop this ChildCtrl, on the CLM control, then ok,
       filtered Plus the ChildControl must not be locked
    */
    CInterfaceManager *pIM = CInterfaceManager::getInstance();
    CDBCtrlSheet *clmCtrl = dynamic_cast<CDBCtrlSheet *>(
...
// ***************************************************************************
bool CDBGroupListSheetFilterExchangeable::CSheetChildFilter::isSheetValid(
    CDBGroupListSheet *pFather) {
  if (CSheetChild::isSheetValid(pFather)) {
    if (!Ctrl)
      return false;
...
// ***************************************************************************
bool CDBGroupListSheetFilterExchangeable::parse(xmlNodePtr cur,
...
class CHandlerInvCanDrag : public IActionHandler {
  virtual void execute(CCtrlBase *pCaller, const string & /* Params */) {
    ...
    // If the item comes from an animal list which is not available -> cannot
    // move
    if (strnicmp(pCSSrc->getSheet().c_str(), "LOCAL:INVENTORY:PACK_ANIMAL",
                 27) == 0) {
      string sTmp = pCSSrc->getSheet().substr(0, pCSSrc->getSheet().rfind(':'));
      sTmp = sTmp.substr(27, sTmp.size());
      uint32 nAni;
      fromString(sTmp, nAni);
      INVENTORIES::TInventory inv =
          (INVENTORIES::TInventory)(INVENTORIES::pet_animal + nAni);
...
class CHandlerInvSearchButton : public IActionHandler {
  virtual void execute(CCtrlBase *pCaller, const string &sParams) {
    ...
    if (!btn) {
      nlwarning("inv_search_button pCaller == NULL, caller must be "
                "CCtrlBaseButton with 'toggle_button' type");
      return;
    }
    ...
    CDBGroupListSheetBag *pList = dynamic_cast<CDBGroupListSheetBag *>(
        CWidgetManager::getInstance()->getElementFromId(
            btn->getParent()->getId() + ":bag_list"));
    if (pList != NULL)
      pList->setSearchFilter(filter);

    CDBGroupIconListBag *pIcons = dynamic_cast<CDBGroupIconListBag *>(
        CWidgetManager::getInstance()->getElementFromId(
            btn->getParent()->getId() + ":bag_icons"));
    if (pIcons != NULL)
      pIcons->setSearchFilter(filter);
  }
};
...
class CHandlerInvSetSearch : public IActionHandler {
  void execute(CCtrlBase *pCaller, const std::string &sParams) {
    ...
    if (pList != NULL)
      pList->setSearchFilter(eb->getInputString());

    CDBGroupIconListBag *pIcons = dynamic_cast<CDBGroupIconListBag *>(
        CWidgetManager::getInstance()->getElementFromId(id + ":bag_icons"));
    if (pIcons != NULL)
      pIcons->setSearchFilter(eb->getInputString());
  }
};
...
class CHandlerInvCanDropTo : public IActionHandler {
  virtual void execute(CCtrlBase *pCaller, const string &Params) {
    ...
    CInventoryManager *pInv = CInventoryManager::getInstance();
    if (!pInv->isDragging())
      return; // To prevent other things to happens
    ...
    if (pCSSrc == NULL)
      return; // Cannot do anything if the incoming sheet is not a sheet
    ...
    // -------------------------------------------------------
    {
      ...
      if ((pIS != NULL) && (pIS->Family == ITEMFAMILY::PET_ANIMAL_TICKET)) {
        ...
        if (pCSDst != NULL) {
          ...
          if (strnicmp(pCSDst->getSheet().c_str(), "LOCAL:EXCHANGE:GIVE", 19) ==
              0) {
            // The animal bag must be empty before giving it
            if (pIS->Pet.Slot > 0)
              if (pInv->isInventoryEmpty((INVENTORIES::TInventory)(
                      INVENTORIES::pet_animal + (pIS->Pet.Slot - 1))))
                pCSDst->setCanDrop(true);
          }
          // Or moving in the bag (user sort)
          if (strnicmp(pCSDst->getSheet().c_str(), "LOCAL:INVENTORY:BAG", 19) ==
              0)
            pCSDst->setCanDrop(true);
        }
        return;
        ...
        if (pCSSrc && pCSSrc->getType() == CCtrlSheetInfo::SheetType_Item)
          if (pCSDst && pCSDst->getType() == CCtrlSheetInfo::SheetType_Item) {
            if (!pInv->getDBIndexPath(pCSSrc).empty()) {
              // The item dragged comes from a slot check if this is the good
              // type
              if (pCSDst->canDropItem(pCSSrc))
                pCSDst->setCanDrop(true);
            } else {
              // The source must be from the bag
              if (strnicmp(pCSSrc->getSheet().c_str(), "LOCAL:INVENTORY:BAG",
                           19) == 0)
                pCSDst->setCanDrop(true);
            }
          }
    ...
    // ------------------------------------
    CDBGroupListSheetBag *pListDstText =
        dynamic_cast<CDBGroupListSheetBag *>(pCaller);
    CDBGroupIconListBag *pListDstIcon =
        dynamic_cast<CDBGroupIconListBag *>(pCaller);
    if ((pListDstText != NULL) || (pListDstIcon != NULL)) {
      ...
      if (pListDstIcon != NULL) {
        if (pListDstIcon->getInvType() == CInventoryManager::InvGuild)
          if (strnicmp(pCSSrc->getSheet().c_str(), "LOCAL:INVENTORY:BAG", 19) !=
              0)
            bCanDrop = false;
      }
      ...
      if (pListDstIcon != NULL) {
        if (pListDstIcon->getInvType() != CInventoryManager::InvBag)
          if (strnicmp(pCSSrc->getSheet().c_str(), "SERVER:GUILD:INVENTORY",
                       19) == 0)
            bCanDrop = false;
      }
      if (pListDstText != NULL) {
      ...
      if (pListDstIcon != NULL) {
        if ((pListDstIcon->getInvType() == CInventoryManager::InvPA0) ||
          ...
          INVENTORIES::TInventory e = (INVENTORIES::TInventory)(
              INVENTORIES::pet_animal1 +
              (pListDstIcon->getInvType() - CInventoryManager::InvPA0));
      ...
      if (pListDstText != NULL) {
        if ((pListDstText->getInvType() == CInventoryManager::InvPA0) ||
          ...
          INVENTORIES::TInventory e = (INVENTORIES::TInventory)(
              INVENTORIES::pet_animal1 +
              (pListDstText->getInvType() - CInventoryManager::InvPA0));
          if (!pInv->isInventoryAvailable(e))
            bCanDrop = false;
        }
      }
      ...
      if (pListDstText != NULL)
        pListDstText->setCanDrop(bCanDrop);
      if (pListDstIcon != NULL)
        pListDstIcon->setCanDrop(bCanDrop);
    }
  }
};
...
class CHandlerInvDropTo : public IActionHandler {
  virtual void execute(CCtrlBase *pCaller, const string &Params) {
    ...
    if (!getInventory().isDragging()) {
      ...
      if (pIG == NULL)
        return;
      if (pIG->getId() != "ui:interface:bag_choose")
        return;
    ...
    if (getInventory().isDraggingFromTextList() ||
        getInventory().isDraggingFromIconList()) {
      // If the destination is an equipment slot ?
      if (pCSDst != NULL) {
        string invPath = getInventory().getDBIndexPath(
            pCSDst); // Get the index in the equipment
        if (!invPath.empty()) {
          // Drop to the slot ie write the database with the index of the slot
          string bagPath =
              pCSSrc
                  ->getSheet(); // Get the database branch of the dragged sheet
          ...
          if (pCSSrc && pCSSrc->getType() == CCtrlSheetInfo::SheetType_Item)
            if (pCSDst && pCSDst->getType() == CCtrlSheetInfo::SheetType_Item) {
              ...
              if (pCSDst->canDropItem(pCSSrc)) {
                ...
                const string sTmp =
                    bagPath.substr(bagPath.rfind(':') + 1, bagPath.size());
                sint index;
                fromString(sTmp, index);
      ...
      if ((pListDst != NULL) && (pListSrc != NULL)) {
        ...
        {
          // Move the item to the destination list using the procedure
          // move_to_xxx
          CDBGroupListSheetBag *pListDstText =
              dynamic_cast<CDBGroupListSheetBag *>(pListDst);
          CDBGroupIconListBag *pListDstIcon =
              dynamic_cast<CDBGroupIconListBag *>(pListDst);

          if (((pListDstText != NULL) &&
               (pListDstText->getInvType() == CInventoryManager::InvBag)) ||
              ((pListDstIcon != NULL) &&
               (pListDstIcon->getInvType() == CInventoryManager::InvBag))) {
            CAHManager::getInstance()->runActionHandler("proc", pCSSrc,
                                                        "move_to_bag");
          } else if (((pListDstText != NULL) &&
                      ((pListDstText->getInvType() ==
                        CInventoryManager::InvPA0) ||
                       (pListDstText->getInvType() ==
                        CInventoryManager::InvPA1) ||
                       (pListDstText->getInvType() ==
                        CInventoryManager::InvPA2) ||
                       (pListDstText->getInvType() ==
                        CInventoryManager::InvPA3))) ||
                     ((pListDstIcon != NULL) &&
                      ((pListDstIcon->getInvType() ==
                        CInventoryManager::InvPA0) ||
                       (pListDstIcon->getInvType() ==
                        CInventoryManager::InvPA1) ||
                       (pListDstIcon->getInvType() ==
            ...
            string sTmp;
            if (pListDstText != NULL)
              sTmp = toString("%d", pListDstText->getInvType() -
                                        CInventoryManager::InvPA0);
            if (pListDstIcon != NULL)
              sTmp = toString("%d", pListDstIcon->getInvType() -
                                        CInventoryManager::InvPA0);
            CAHManager::getInstance()->runActionHandler("proc", pCSSrc,
                                                        "move_to_pa|" + sTmp);
          } else if (((pListDstText != NULL) &&
                      (pListDstText->getInvType() ==
                       CInventoryManager::InvGuild)) ||
                     ((pListDstIcon != NULL) &&
                      (pListDstIcon->getInvType() ==
                       CInventoryManager::InvGuild))) {
            if (strnicmp(pCSSrc->getSheet().c_str(), "LOCAL:INVENTORY:BAG",
              ...
              CAHManager::getInstance()->runActionHandler("proc", pCSSrc,
                                                          "move_to_guild");
          } else if (((pListDstText != NULL) && (pListDstText->getInvType() ==
                                                 CInventoryManager::InvRoom)) ||
                     ((pListDstIcon != NULL) && (pListDstIcon->getInvType() ==
                                                 CInventoryManager::InvRoom))) {
            CAHManager::getInstance()->runActionHandler("proc", pCSSrc,
                                                        "move_to_room");
          }
    ...
    if (pCSDst != NULL)
      if (getInventory().isDraggingFromSlot()) {
        ...
        sint32 i1 = NLGUI::CDBManager::getInstance()
                        ->getDbProp(invPath1 + ":INDEX_IN_BAG")
                        ->getValue16();
        sint32 i2 = NLGUI::CDBManager::getInstance()
                        ->getDbProp(invPath2 + ":INDEX_IN_BAG")
                        ->getValue16();
        ...
        if (i2 != 0)
          getInventory().equip(sBag + toString(i2 - 1), invPath1);
        if (i1 != 0)
          getInventory().equip(sBag + toString(i1 - 1), invPath2);
      }
...
class CHandlerLockInvItem : public IActionHandler {
  void execute(CCtrlBase *pCaller, const std::string &sParams) {
    ...
    if (lock)
      item->setItemResaleFlag(BOTCHATTYPE::ResaleKOLockedByOwner);
...
class CHandlerInvTempToBag : public IActionHandler {
  virtual void execute(CCtrlBase *pCaller, const string & /* Params */) {
    ...
    // If we cant find place display a message and dont send the request to the
    // server
...
class CHandlerInvTempAll : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    if (!CTempInvManager::getInstance()->isOpened())
      return;
    if (NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:TEMP_INV:ALL_EMPTY")
            ->getValue32() != 0)
      return;
    ...
    // Try to put all items in the DB order in all the bags of the player :
    // (bag, pa0-3, steed)
    vector<pair<double, double>> BagsBulk;
    BagsBulk.push_back(
        pair<double, double>(pInv->getBagBulk(0), pInv->getMaxBagBulk(0)));

    nlctassert(MAX_INVENTORY_ANIMAL == 4);
    if (pInv->isInventoryAvailable(INVENTORIES::pet_animal1))
      BagsBulk.push_back(
          pair<double, double>(pInv->getBagBulk(1), pInv->getMaxBagBulk(1)));
    if (pInv->isInventoryAvailable(INVENTORIES::pet_animal2))
      BagsBulk.push_back(
          pair<double, double>(pInv->getBagBulk(2), pInv->getMaxBagBulk(2)));
    if (pInv->isInventoryAvailable(INVENTORIES::pet_animal3))
      BagsBulk.push_back(
          pair<double, double>(pInv->getBagBulk(3), pInv->getMaxBagBulk(3)));
    if (pInv->isInventoryAvailable(INVENTORIES::pet_animal4))
      BagsBulk.push_back(
          pair<double, double>(pInv->getBagBulk(4), pInv->getMaxBagBulk(4)));

    bool bPlaceFound = true;

    for (uint32 itemNb = 0; itemNb < MAX_TEMPINV_ENTRIES; ++itemNb) {
      CCDBNodeLeaf *pNL = NLGUI::CDBManager::getInstance()->getDbProp(
          LOCAL_INVENTORY ":TEMP:" + toString(itemNb) + ":SHEET");
      CCDBNodeLeaf *pNLquantity = NLGUI::CDBManager::getInstance()->getDbProp(
          LOCAL_INVENTORY ":TEMP:" + toString(itemNb) + ":QUANTITY");
      if (pNL == NULL || pNLquantity == NULL)
        continue;
      if (pNL->getValue32() == 0 || pNLquantity->getValue32() == 0)
        continue;
      double itemBulk =
          pNLquantity->getValue32() * pInv->getItemBulk(pNL->getValue32());
...
// ***************************************************************************
uint16 CInventoryManager::getItemSlotId(const std::string &itemDb,
                                        uint slotIndex) {
...
uint CInventoryManager::getItemSheetForSlotId(uint slotId) const {
  ...
  for (uint i = 0; i < NumInventories; i++) {
    if (InventoryIndexes[i] == (slotId >> CItemInfos::SlotIdIndexBitSize)) {
      CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
          toString("SERVER:%s:%d:SHEET", InventoryDBs[i].c_str(),
                   (slotId & CItemInfos::SlotIdIndexBitMask)),
          false);
      if (node)
...
#ifdef RYZOM_FORGE
// ***************************************************************************
const CClientItemInfo *
CInventoryManager::getItemInfoCache(uint32 serial, uint32 createTime) const {
...
void CInventoryManager::updateItemInfoWaiters(uint itemSlotId) {
  // First verify if the versions matches. If differ, no need to update waiters
  // since not good.
  if (!isItemInfoUpToDate(itemSlotId)) {
    return;
  }

  bool isItemFromTrading =
      (itemSlotId >> CItemInfos::SlotIdIndexBitSize) == INVENTORIES::trading;
  ...
  for (it = _ItemInfoWaiters.begin(); it != _ItemInfoWaiters.end();) {
    ...
    if (waiter->ItemSlotId == itemSlotId &&
        (isItemFromTrading ||
         waiter->ItemSheet == getItemSheetForSlotId(itemSlotId))) {
      waiter->infoReceived();
    }
...
void CInventoryManager::onReceiveItemInfoSlotVersion(ICDBNode *node) {
  ...
  if (!dynamic_cast<CCDBNodeLeaf *>(node) || !node->getParent() ||
  ...
  _ItemInfoMap[itemSlotId].InfoVersionFromSlot =
      ((CCDBNodeLeaf *)node)->getValue8();
...
void CInventoryManager::onReceiveItemSheet(ICDBNode *node) {
  // When a SHEET change, it may unblock some ITEM_INFO:GET calls or some info
  // waiters update.
  uint itemSlotId;
...
// ***************************************************************************
void CInventoryManager::onRefreshItemInfoVersion(uint16 slotId,
                                                 uint8 infoVersion) {
...
void CInventoryManager::onTradeChangeSession() {
  // Dummy set a 255 InfoVersionMsg for all items in trade. Because Server
  // cannot do the code.
  for (uint i = 0; i < TRADE_MAX_ENTRIES; i++) {
    uint itemSlotId = getItemSlotId("TRADING", i);
    // Since InfoVersionFromSlot always == 0, a msg will be sent to have new
    // version
...
void CInventoryManager::updateItemInfoQueue() {
  if (!ConnectionReadySent) // CONNECTION:READY not yet sent, so we cannot send
                            // requests yet!
  {
    // Caused by CNetworkConnection::reinit, and
    // NLMISC::CCDBNodeBranch::resetData
    // TODO: Item sheets are effectively being set to 0, any way to detect this
    // in particular?
    //       Slots with sheet 0 won't have any info to request anyway!
    //       Remove this check in favour of the assert lower down when properly
    //       implemented.
    nlwarning("Update item info queue (%i), but connection not ready",
              (int)_ItemInfoWaiters.size());
    return;
  }
  ...
  for (it = _ItemInfoWaiters.begin(); it != _ItemInfoWaiters.end(); it++) {
    ...
    if (it != _ItemInfoMap.end()) {
      ...
      // If versions differ, and if a msg was not already sent to get the new
      // version, then ask server SheetId must also match
      if (itemInfo.InfoVersionFromSlot != itemInfo.InfoVersionFromMsg &&
          itemInfo.InfoVersionFromSlot !=
              itemInfo.InfoVersionSlotServerWaiting &&
          (isItemFromTrading ||
           waiter->ItemSheet == getItemSheetForSlotId(itemSlotId))) {
        ...
        // Debug
          ...
          CInterfaceManager *pIM = CInterfaceManager::getInstance();
          pIM->displaySystemInfo(
              toString("ITEM_INFO:GET, slotId: %d, slotVersion: %d", itemSlotId,
                       itemInfo.InfoVersionFromSlot));
        }
...
void CInventoryManager::debugItemInfoWaiters() {
  ...
  for (it = _ItemInfoWaiters.begin(); it != _ItemInfoWaiters.end(); it++) {
    ...
    if (itMap != _ItemInfoMap.end()) {
      ...
      pIM->displaySystemInfo(toString(
          "ItemInfoWaiter: slotId: %d, slotVersion: %d, msgVersion: %d, "
          "msgVersionWait: %d",
          itemSlotId, itemInfo.InfoVersionFromSlot, itemInfo.InfoVersionFromMsg,
          itemInfo.InfoVersionSlotServerWaiting));
    }
  }
}
...
void CInventoryManager::sortBag() {
  ...
  pIconList = dynamic_cast<CDBGroupIconListBag *>(
      CWidgetManager::getInstance()->getElementFromId(LIST_BAG_ICONS));
  if (pIconList != NULL)
    pIconList->needToSort();
  pList = dynamic_cast<CDBGroupListSheetBag *>(
      CWidgetManager::getInstance()->getElementFromId(LIST_BAG_TEXT));
  if (pList != NULL)
    pList->needToSort();

  pIconList = dynamic_cast<CDBGroupIconListBag *>(
      CWidgetManager::getInstance()->getElementFromId(LIST_ROOM_ICONS));
  if (pIconList != NULL)
    pIconList->needToSort();
  pList = dynamic_cast<CDBGroupListSheetBag *>(
      CWidgetManager::getInstance()->getElementFromId(LIST_ROOM_TEXT));
  if (pList != NULL)
    pList->needToSort();

  pIconList = dynamic_cast<CDBGroupIconListBag *>(
      CWidgetManager::getInstance()->getElementFromId(LIST_GUILD_ICONS));
  if (pIconList != NULL)
    pIconList->needToSort();
  pList = dynamic_cast<CDBGroupListSheetBag *>(
      CWidgetManager::getInstance()->getElementFromId(LIST_GUILD_TEXT));
  if (pList != NULL)
    pList->needToSort();

  pIconList = dynamic_cast<CDBGroupIconListBag *>(
      CWidgetManager::getInstance()->getElementFromId(LIST_PA0_ICONS));
  if (pIconList != NULL)
    pIconList->needToSort();
  pList = dynamic_cast<CDBGroupListSheetBag *>(
      CWidgetManager::getInstance()->getElementFromId(LIST_PA0_TEXT));
  if (pList != NULL)
    pList->needToSort();

  pIconList = dynamic_cast<CDBGroupIconListBag *>(
      CWidgetManager::getInstance()->getElementFromId(LIST_PA1_ICONS));
  if (pIconList != NULL)
    pIconList->needToSort();
  pList = dynamic_cast<CDBGroupListSheetBag *>(
  ...
  pIconList = dynamic_cast<CDBGroupIconListBag *>(
      CWidgetManager::getInstance()->getElementFromId(LIST_PA2_ICONS));
  if (pIconList != NULL)
    pIconList->needToSort();
  pList = dynamic_cast<CDBGroupListSheetBag *>(
      CWidgetManager::getInstance()->getElementFromId(LIST_PA2_TEXT));
  if (pList != NULL)
    pList->needToSort();

  pIconList = dynamic_cast<CDBGroupIconListBag *>(
      CWidgetManager::getInstance()->getElementFromId(LIST_PA3_ICONS));
  if (pIconList != NULL)
    pIconList->needToSort();
  pList = dynamic_cast<CDBGroupListSheetBag *>(
      CWidgetManager::getInstance()->getElementFromId(LIST_PA3_TEXT));
  if (pList != NULL)
    pList->needToSort();
}
...
bool CInventoryManager::isInventoryPresent(INVENTORIES::TInventory invId) {
  ...
  // PA present?
  if (invId >= INVENTORIES::pet_animal &&
      invId < INVENTORIES::pet_animal + MAX_INVENTORY_ANIMAL) {
    CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
        toString("SERVER:PACK_ANIMAL:BEAST%d:STATUS",
                 invId - INVENTORIES::pet_animal),
        false);
    if (!node)
      return false;
    ...
    return (NLGUI::CDBManager::getInstance()
                ->getDbProp("UI:TEMP:INVENTORY_GUILD_OPENED", true)
                ->getValue8() != 0);
  } else if (invId == INVENTORIES::player_room) {
    return (NLGUI::CDBManager::getInstance()
                ->getDbProp("UI:TEMP:INVENTORY_ROOM_OPENED", true)
                ->getValue8() != 0);
  }
...
bool CInventoryManager::isInventoryAvailable(INVENTORIES::TInventory invId) {
  ...
  if (invId >= INVENTORIES::pet_animal &&
    ...
    return ANIMAL_STATUS::isInventoryAvailable(
        (ANIMAL_STATUS::EAnimalStatus)status);
  } else if (invId == INVENTORIES::guild) {
    return (NLGUI::CDBManager::getInstance()
                ->getDbProp("UI:TEMP:INVENTORY_GUILD_OPENED", true)
                ->getValue8() != 0);
...
bool CInventoryManager::isInventoryEmpty(INVENTORIES::TInventory invId) {
  ...
  for (uint32 i = 0; (sint)i < nNbEntries; ++i) {
    CCDBNodeLeaf *pNL;
    pNL = NLGUI::CDBManager::getInstance()->getDbProp(
        sPath + toString(":%d:SHEET", i), false);
    if (pNL == NULL)
      return true;
    if (pNL->getValue32() != 0)
      return false;
...
const CItemImage &CInventoryManager::getServerPAItem(uint beastIndex,
                                                     uint index) const {
  nlassert(beastIndex < MAX_INVENTORY_ANIMAL);
  nlassert(index < MAX_ANIMALINV_ENTRIES);
  return ServerPAInv[beastIndex][index];
...
CItemImage &CInventoryManager::getLocalItem(uint inv, uint index) {
  ...
  if (inv >= INVENTORIES::pet_animal &&
      inv < INVENTORIES::pet_animal + MAX_INVENTORY_ANIMAL)
...
const CItemImage *CInventoryManager::getServerItem(uint slotId) const {
  ...
  if (inv == INVENTORIES::bag) {
    return &getServerBagItem(index);
  } else if (inv >= INVENTORIES::pet_animal &&
             inv < INVENTORIES::pet_animal + MAX_INVENTORY_ANIMAL) {
    return &getServerPAItem(inv - INVENTORIES::pet_animal, index);
    ...
    if (getInventory().isInventoryAvailable(INVENTORIES::guild)) {
      CCDBNodeBranch *itemBranch =
          NLGUI::CDBManager::getInstance()->getDbBranch(
              "SERVER:GUILD:INVENTORY:" + toString(index));
      if (itemBranch) {
    ...
    if (getInventory().isInventoryAvailable(INVENTORIES::player_room)) {
      CCDBNodeBranch *itemBranch =
          NLGUI::CDBManager::getInstance()->getDbBranch(
              SERVER_INVENTORY ":ROOM:" + toString(index));
      if (itemBranch) {
        static CItemImage image;
        image.build(itemBranch);
        return &image;
    ...
    CCDBNodeBranch *itemBranch = NLGUI::CDBManager::getInstance()->getDbBranch(
        "LOCAL:TRADING:" + toString(index));
    if (itemBranch) {
      static CItemImage image;
      image.build(itemBranch);
      return &image;
    ...
    CCDBNodeBranch *itemBranch = NLGUI::CDBManager::getInstance()->getDbBranch(
        "LOCAL:EXCHANGE:GIVE:" + toString(index));
    if (itemBranch) {
      static CItemImage image;
      image.build(itemBranch);
      return &image;
    ...
    CCDBNodeBranch *itemBranch = NLGUI::CDBManager::getInstance()->getDbBranch(
        "LOCAL:EXCHANGE:RECEIVE:" + toString(index));
    if (itemBranch) {
      static CItemImage image;
      image.build(itemBranch);
      return &image;
...
// ***************************************************************************
CInventoryManager::TInvType
CInventoryManager::invTypeFromString(const string &str) {
...
// ***************************************************************************
void CInventoryManager::getSlotInvIndex(uint slotId, uint &inv,
                                        uint &index) const {
