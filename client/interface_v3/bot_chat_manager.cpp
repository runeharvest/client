...
CBotChatManager::~CBotChatManager() {
  // Destruct
  nlassert(_CurrPage ==
           NULL); // should have called setCurrPage(NULL) before quitting (and
...
// ***************************************************************************
void CBotChatManager::removeMissionInfoWaiter(
    IMissionPrereqInfosWaiter *waiter) {
...
// ***************************************************************************
void CBotChatManager::onReceiveMissionInfo(uint16 missionSlotId,
...
// ********************************************************************************************
/*void CBotChatManager::processMissionHelpInfos(uint8 index, CPrerequisitInfos
&infos)
{
        std::map<uint8,CInterfaceGroup*>::iterator it =
_MissionHelpWindowsWaiting.find(index); if (it ==
_MissionHelpWindowsWaiting.end())
        {
                nlwarning("Error, mission help window for mission index %u not
found", index); return;
        }
...
NLMISC::toString("text_id_prereq_%u",i+1);
                ...
                CViewTextID	*viewTextID = dynamic_cast<CViewTextID
*>(help->getView(textId)); if(viewTextID)
                {
...
help->HelpWindow->getGroup(help->ScrollTextGroup); if (viewTextGroup)
                                viewTextGroup->setActive(false);
                }
                CInterfaceGroup *viewTextGroup =
help->HelpWindow->getGroup(help->ScrollTextIdGroup); if (viewTextGroup)
