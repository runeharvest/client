...
public:
  ...
  void addMessagePopup(const std::string &message,
                       NLMISC::CRGBA col = CRGBA::White, uint time = 0);

  // The same as previous but centered in the screen. if 0, get the
  // OptionTimeoutMessages
  void addMessagePopupCenter(const std::string &message,
                             NLMISC::CRGBA col = CRGBA::White, uint time = 0);
  ...
  void addContextHelp(const std::string &message, const std::string &target,
                      uint time);

  // Add a context help
  void addContextHelpHTML(const std::string &filename,
                          const std::string &target, uint time);
  ...
  // Open a Dynamic Chat
  void dynChatOpen(uint32 nBotUID, uint32 nBotName,
                   const std::vector<uint32> &DynStrs);

  // Open a Dynamic Chat from webig
  void webIgChatOpen(uint32 nBotUID, std::string sBotName,
...
private:
  ...
  public:
    ...
    // Timeout ?
    bool timeOut() const { return TimeInSec >= TimeOut; }
  };
  ...
  CPopupContext *buildContextHelp(const std::string &templateName,
                                  const std::string &targetName,
                                  CInterfaceElement *&target, uint time);
...
public:
  ...
  bool isOnLastPage() const {
    return _TextParts.empty() || _CurrentPart == (_TextParts.size() - 1);
  }

  // Tell the bubble's user if the bubble can be activated.
  bool canBeShown() const { return _CanBeShown; }
