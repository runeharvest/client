...
static const string MAIN_CHAT_SOURCE_MENU =
    "ui:interface:main_chat_source_menu";
static const string USER_CHAT_SOURCE_MENU =
    "ui:interface:user_chat_source_menu";
static const string STD_CHAT_SOURCE_MENU = "ui:interface:std_chat_source_menu";
static const string NEW_PARTY_CHAT_WINDOW =
    "ui:interface:create_new_party_chat";

NLMISC::CRefPtr<CChatWindow> ChatWindowForFilter;

static const sint PARTY_CHAT_SPAWN_DELTA =
    20; // to avoid that all party chat appear at the same position, a random
        // value is added
...
/** Display an error msg in the system info window, and also in the last window
 * that triggered the command (so that the user is sure to see it)
 */
static void displayVisibleSystemMsg(const std::string &msg,
                                    const string &cat = "CHK");
...
public:
  uint DbIndex;
  CDynamicChatEntryHandler() { DbIndex = 0; }

  virtual void msgEntered(const string &msg, CChatWindow *chatWindow) {
    if (ClientCfg.Local) {
      chatWindow->displayMessage(msg, CRGBA::White, CChatGroup::dyn_chat,
                                 DbIndex);
    } else {
      ChatMngr.setChatMode(CChatGroup::dyn_chat,
                           ChatMngr.getDynamicChannelIdFromDbIndex(DbIndex));
      ChatMngr.chat(msg);
    }
  }
};
...
static CDynamicChatEntryHandler
    DynamicChatEntryHandler[CChatGroup::MaxDynChanPerPlayer];
...
//===========================================================================================================
CPeopleInterraction::CPeopleInterraction()
    : Region(NULL), Universe(NULL), TeamChat(NULL), GuildChat(NULL),
      SystemInfo(NULL), TellWindow(NULL), DebugInfo(NULL), CurrPartyChatID(0) {
...
void CPeopleInterraction::release() {
  ...
  AroundMe.release();
  if (Region)
    cwm.removeChatWindow(Region);
  if (Universe)
    cwm.removeChatWindow(Universe);
  if (TeamChat)
    cwm.removeChatWindow(TeamChat);
  if (GuildChat)
    cwm.removeChatWindow(GuildChat);
  if (SystemInfo)
    cwm.removeChatWindow(SystemInfo);
  TheUserChat.release();
  if (DebugInfo)
    cwm.removeChatWindow(DebugInfo);
  ...
  for (uint i = 0; i < CChatGroup::MaxDynChanPerPlayer; i++) {
    if (DynamicChat[i])
      cwm.removeChatWindow(DynamicChat[i]);
    DynamicChat[i] = NULL;
  }
  ...
  if (numCW != 0) {
    ...
    for (uint k = 0; k < numCW; ++k) {
      nlwarning("Window %d : %s", (int)k,
                (cwm.getChatWindowByIndex(k)->getTitle()).c_str());
    }
  }
}
...
void CPeopleInterraction::removeAllPartyChat() {
  CChatWindowManager &cwm = getChatWndMgr();
  for (std::vector<CPartyChatInfo>::iterator it = PartyChats.begin();
       it != PartyChats.end(); ++it) {
    if (it->Window)
      cwm.removeChatWindow(it->Window);
    it->Window = NULL;
  }
  PartyChats.clear();
...
bool CPeopleInterraction::isUserChat(CChatWindow *cw) const {
  ...
  if (cw == ChatGroup.Window)
    return true;
  for (uint k = 0; k < MaxNumUserChats; ++k) {
    if (UserChat[k].Window == cw)
      return true;
  }
  return false;
}
...
void CPeopleInterraction::init() {
  ...
  // main chat should be created after other windows, because it relies on them
...
void CPeopleInterraction::initAfterLoad() {
  /* activate the USER chat per default.
          Important: we must do it after ChatGroup.Window var init, DB color
     init etc... because the latest are used in chat_group_filter ActionHandler
  */
...
void CPeopleInterraction::createTeamList() {
  ...
  peopleListDesc.ContactType =
      CPeopleListDesc::Team /* CPeopleListDesc::Contact*/;
  ...
  TeamList.create(
      peopleListDesc,
      NULL); // &chatDesc // create the team list with a chat box in it
  TeamList.setMenu("ui:interface:sort_menu");
  // Special case for team : each entry is connected to the database so we
  // create all team member at once
  ...
  // NB: use an intermediate temp var, to avoid show of the window each time a
  // new team member enters
  string sExpr = "@UI:VARIABLES:IS_TEAM_PRESENT";
  string sAction = "set";
  string sCond;
  string sParams = "target_property=ui:interface:team_list:active|value=@UI:"
                   "VARIABLES:IS_TEAM_PRESENT";
  ...
  if (TeamChat) {
    ...
    il->init(targets, cdbTargets, sExpr, sAction, sParams, sCond,
             TeamChat->getContainer());
  }
}
...
void CPeopleInterraction::createFriendList() {
  ...
  peopleListDesc.BaseContainerTemplateName =
  ...
  FriendList.setPeopleMenuEx(
      "ui:interface:friend_list_menu_offline_unblocked",
...
void CPeopleInterraction::createAroundMeWindow() {
  ...
  // Configure filter for the main chat. By default, it listen to everything (no
  // party chats : none have been created yet)
...
void CPeopleInterraction::createDynamicChats() {
  for (uint i = 0; i < CChatGroup::MaxDynChanPerPlayer; i++) {
    CChatWindowDesc chatDesc;
    chatDesc.FatherContainer = "ui:interface";
    chatDesc.Title = toString("dummyDynChatTitle%d",
                              i); // title not used, but still important because
                                  // create() test uniqueness
    ...
    chatDesc.ChatTemplateParams.push_back(
        make_pair(string("dyn_chat_nb"), toString(i)));
    chatDesc.Id = string("dynamic_chat") + toString(i);
    // no active proc because active state is driven by database
    chatDesc.AHOnCloseButton = "proc";
    chatDesc.AHOnCloseButtonParams =
...
void CPeopleInterraction::createTheUserChat() {
  ...
  // Configure filter for the new chat (by default, listen to everything but
  // party chats)
...
class CHandlerUserChatActive : public IActionHandler {
  void execute(CCtrlBase * /* pCaller */, const std::string & /* sParams */) {
    CChatWindow *pCGW =
        dynamic_cast<CChatWindow *>(PeopleInterraction.TheUserChat.Window);
    if (pCGW == NULL)
      return;
    CCtrlTextButton *pUserBut = dynamic_cast<CCtrlTextButton *>(
        pCGW->getContainer()->getCtrl("content:target_button"));
    CInterfaceGroup *pEditBox = dynamic_cast<CInterfaceGroup *>(
        pCGW->getContainer()->getGroup("content:ebw"));

    if (!pUserBut)
      return;

    CChatGroup::TGroupType m =
        PeopleInterraction.TheUserChat.Filter.getTargetGroup();
    switch (m) {
    default:
    ...
    case CChatGroup::region:
      pUserBut->setHardText("uiFilterRegion");
      break;
    case CChatGroup::universe:
      pUserBut->setHardText("uiFilterUniverse");
      break;
    case CChatGroup::team:
      pUserBut->setHardText("uiFilterTeam");
      break;
    case CChatGroup::guild:
      pUserBut->setHardText("uiFilterGuild");
      break;
    }
    ...
    if (pEditBox != NULL)
      pEditBox->setW(-pUserBut->getWReal() - 4);
  }
};
...
void CPeopleInterraction::createChatGroup() {
  ...
  chatDesc.FatherContainer = "ui:interface";
  chatDesc.Listener = NULL;
  chatDesc.Title.clear(); // NB: the chatgroup is the only one that can be not
                          // named (because of uniqueness title test)
  chatDesc.Localize = true;
  chatDesc.Savable = true;
...
class CHandlerChatGroupFilter : public IActionHandler {
  void execute(CCtrlBase *pCaller, const std::string &sParams) {
    ...
    if (sParams == "around")
      rCTF.setTargetGroup(CChatGroup::say);
    else if (sParams == "region")
      rCTF.setTargetGroup(CChatGroup::region);
    else if (sParams == "universe")
      rCTF.setTargetGroup(CChatGroup::universe);
    else if (sParams == "team")
      rCTF.setTargetGroup(CChatGroup::team);
    else if (sParams == "guild")
      rCTF.setTargetGroup(CChatGroup::guild);
      ...
      dynChatDbIndex =
          min(dynChatDbIndex, (uint32)(CChatGroup::MaxDynChanPerPlayer - 1));
      rCTF.setTargetGroup(CChatGroup::dyn_chat, dynChatDbIndex);
    }

    // inform DB for write right.
    NLGUI::CDBManager::getInstance()
        ->getDbProp("UI:VARIABLES:MAIN_CHAT:WRITE_RIGHT")
        ->setValue32(writeRight);
    NLGUI::CDBManager::getInstance()
        ->getDbProp("UI:VARIABLES:MAIN_CHAT:IS_DYN_CHAT")
        ->setValue32(isDynChat);
    NLGUI::CDBManager::getInstance()
        ->getDbProp("UI:VARIABLES:MAIN_CHAT:INDEX_DYN_CHAT")
        ->setValue32(dynChatDbIndex);
    ...
    if (!pCGW)
      return;
    CCtrlTextButton *pUserBut = dynamic_cast<CCtrlTextButton *>(
        pCGW->getContainer()->getCtrl("content:but_user"));
    CCtrlTextButton *pEmoteBut = dynamic_cast<CCtrlTextButton *>(
        pCGW->getContainer()->getCtrl("content:but_emote"));
    CInterfaceGroup *pEditBox = dynamic_cast<CInterfaceGroup *>(
        pCGW->getContainer()->getGroup("content:ebw"));
    CInterfaceGroup *pTextList = dynamic_cast<CInterfaceGroup *>(
        pCGW->getContainer()->getGroup("content:cb"));
    ...
    if (sParams == "user") {
      if (pUserBut != NULL) {
        ...
        case CChatGroup::dyn_chat:
          uint32 index = PeopleInterraction.TheUserChat.Filter
                             .getTargetDynamicChannelDbIndex();
          uint32 textId = NLGUI::CDBManager::getInstance()
                              ->getDbProp("SERVER:DYN_CHAT:CHANNEL" +
                                          toString(index) + ":NAME")
                              ->getValue32();
          string title;
          ...
          if (title.empty()) {
            ...
            PeopleInterraction.TheUserChat.Filter.setTargetGroup(
                CChatGroup::arround);
            pUserBut->setHardText("uiFilterAround");
          } else {
      ...
      rCTF.setTargetGroup(
          PeopleInterraction.TheUserChat.Filter.getTargetGroup(),
          PeopleInterraction.TheUserChat.Filter
              .getTargetDynamicChannelDbIndex());
    } else {
      if (pUserBut != NULL)
        pUserBut->setActive(false);
    ...
    // if called from a tab button => force the tab ctrl button to have standard
    // color
    CCtrlTabButton *pTabButton = dynamic_cast<CCtrlTabButton *>(pCaller);
    if (pTabButton) {
      CRGBA stdColor =
          CRGBA::stringToRGBA(CWidgetManager::getInstance()
                                  ->getParser()
                                  ->getDefine("chat_group_tab_color_normal")
                                  .c_str());
...
class CHandlerChatGroupUpdatePrompt : public IActionHandler {
  void execute(CCtrlBase * /* pCaller */, const std::string & /* sParams */) {
    // re set the target group will automatically reset the prompt and prompt
    // color
    CChatTargetFilter &rCTF = PeopleInterraction.ChatGroup.Filter;
    rCTF.setTargetGroup(rCTF.getTargetGroup(),
                        rCTF.getTargetDynamicChannelDbIndex());
  }
};
REGISTER_ACTION_HANDLER(CHandlerChatGroupUpdatePrompt,
                        "chat_group_update_prompt");

//===========================================================================================================
CPeopleList *
CPeopleInterraction::getPeopleListFromContainerID(const std::string &id) {
  if (TeamList.getContainerID() == id)
    return &TeamList;
  else if (FriendList.getContainerID() == id)
    return &FriendList;
  else if (IgnoreList.getContainerID() == id)
    return &IgnoreList;
  return NULL;
}

//===========================================================================================================
bool CPeopleInterraction::getPeopleFromContainerID(const std::string &id,
...
//===========================================================================================================
bool CPeopleInterraction::getPeopleFromCurrentMenu(CPeopleList *&peopleList,
                                                   uint &index) {
  CInterfaceManager *im = CInterfaceManager::getInstance();
  // the group that launched the modal window (the menu) must be the header of
  // the group container that represent a people entry
  CInterfaceGroup *header = dynamic_cast<CInterfaceGroup *>(
...
//===========================================================================================================
CFilteredChat *
CPeopleInterraction::getFilteredChatFromChatWindow(CChatWindow *cw) {
  // if (cw == MainChat.Window) return &MainChat;
  if (cw == ChatGroup.Window)
    return &ChatGroup;
  if (cw == AroundMe.Window)
    return &AroundMe;
  if (cw == TheUserChat.Window)
    return &TheUserChat;
  for (uint k = 0; k < MaxNumUserChats; ++k) {
    if (UserChat[k].Window == cw)
      return &UserChat[k];
...
//===========================================================================================================
void CPeopleInterraction::askAddContact(const string &contactName,
                                        CPeopleList *pl) {
  if (pl == NULL)
    return;
  ...
  if ((pl != &IgnoreList) && (pl != &FriendList)) {
    nlwarning(
        "<askAddContact> For now, only support friend list & ignore list");
    return;
  }
  ...
  if (pl->getIndexFromName(contactName) != -1) {
    ...
    CInterfaceManager::getInstance()->displaySystemInfo(
        CI18N::get("uiContactAlreadyInList"));
    return;
  }
  ...
  if (GenericMsgHeaderMngr.pushNameToStream(sMsg, out)) {
    ...
    out.serial(list);
    NetMngr.push(out);
    // nlinfo("impulseCallBack : %s %s %d sent", sMsg.c_str(),
    // contactName.toString().c_str(), list);
  } else
...
//=================================================================================================================
void CPeopleInterraction::askMoveContact(uint peopleIndexInSrc,
                                         CPeopleList *plSRC,
                                         CPeopleList *plDST) {
  if ((plSRC == NULL) || (plDST == NULL))
    return;
  if ((plSRC != &IgnoreList) && (plSRC != &FriendList)) {
    nlwarning(
        "<askMoveContact> For now, only support friend list & ignore list");
    return;
  }
  ...
  if (peopleIndexInSrc >= plSRC->getNumPeople())
    return;
  ...
  if (GenericMsgHeaderMngr.pushNameToStream(sMsg, out)) {
    ...
    out.serial(nListSRC);
    NetMngr.push(out);
    // nlinfo("impulseCallBack : %s %d %d sent", sMsg.c_str(), contactId,
    // nListSRC);
  } else
...
//=================================================================================================================
void CPeopleInterraction::initContactLists(
    const std::vector<uint32> &vFriendListName,
...

{
  ...
  for (uint i = 0; i < vFriendListName.size(); ++i)
    addContactInList(contactIdPool++, vFriendListName[i], vFriendListOnline[i],
                     0);
  for (uint i = 0; i < vIgnoreListName.size(); ++i)
    addContactInList(contactIdPool++, vIgnoreListName[i].toUtf8(), ccs_offline,
                     1);
  updateAllFreeTellerHeaders();
}

//=================================================================================================================
void CPeopleInterraction::addContactInList(uint32 contactId,
                                           const string &nameIn,
...
//=================================================================================================================
void CPeopleInterraction::addContactInList(uint32 contactId, uint32 nameID,
                                           TCharConnectionState online,
                                           uint8 nList) {
  string name;
...
//=================================================================================================================
bool CPeopleInterraction::isContactInList(const string &nameIn,
                                          uint8 nList) const {
  // select correct people list
...
//=================================================================================================================
void CPeopleInterraction::updateContactInList(uint32 contactId,
                                              TCharConnectionState online,
                                              uint nList) {
  if (nList == 0) {
    ...
    if (index != -1) {
      ...
      if (FriendList.getOnline(index) != online) {
        CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
            "UI:SAVE:CHAT:SHOW_ONLINE_OFFLINE_NOTIFICATIONS_CB", false);
        if (node && node->getValueBool()) {
          // Only show the message if this player is not in my guild (because
          // then the guild manager will show a message)
          std::vector<SGuildMember> GuildMembers =
          ...
          bool showMsg = bOnlyFriend &&
                         (prevState == ccs_offline || online == ccs_offline);

          // Player is not in my guild, and the status change is from offline to
          // online/abroad online or vice versa.
          if (showMsg) {
            string msg = (online != ccs_offline)
            ...
            bool dummy;
            PeopleInterraction.ChatInput.AroundMe.displayMessage(msg, col, 2,
                                                                 &dummy);
          }
        }
...
bool CPeopleInterraction::testValidPartyChatName(const string &title) {
  ...
  if (NLMISC::startsWith(title, userChatStr))
    return false;
  // can't match a translation identifier
  if (CI18N::hasTranslation(title))
    return false;
  for (uint k = 0; k < PartyChats.size();
       ++k) // there shouldn't be that much party chat simultaneously so a
            // linear search is ok
  {
    if (PartyChats[k].Window->getTitle() == title)
      return false;
  }
  // check for other chat window names (local only ?)
  if (SystemInfo && title == SystemInfo->getTitle())
    return false;
  if (AroundMe.Window && title == AroundMe.Window->getTitle())
    return false;
  if (GuildChat && title == GuildChat->getTitle())
    return false;
  if (TeamChat && title == TeamChat->getTitle())
    return false;
  sint index;
  ...
  if (index != -1)
    return false;
  // TODO_GAMEDEV server test for the name (not only local), & modify callers of
  // this function
...
bool CPeopleInterraction::removePartyChat(CChatWindow *window) {
  ...
  if (it != PeopleInterraction.PartyChats.end()) {
    ...
    // TODO GAMEDEV : send msg to server to tell that the player has deleted
    // this party chat.
    return true;
  } else {
    return false;
...
bool CPeopleInterraction::createNewPartyChat(const string &title) {
  // now there are no party chat windows, party chat phrases must be filtered
  // from the main chat
  ...
  // if (newPartyChat)
  {
    ...
    newPartyChat->getContainer()->setX(newPartyChat->getContainer()->getX() +
    (sint32) (rand() % PARTY_CHAT_SPAWN_DELTA));
    newPartyChat->getContainer()->setY(newPartyChat->getContainer()->getY() +
    (sint32) (rand() % PARTY_CHAT_SPAWN_DELTA));
...
//=================================================================================================================
void CPeopleInterraction::buildFilteredChatSummary(const CFilteredChat &src,
  ...
  // fill target infos
  if (src.Filter.getTargetPartyChat() != NULL ||
      !src.Filter.getTargetPlayer().empty()) {
...
//=================================================================================================================
void CPeopleInterraction::buildFilteredDynChatSummary(
    const CFilteredChat &src, CFilteredDynChatSummary &fcs) {
...
//=================================================================================================================
void CPeopleInterraction::saveFilteredChat(NLMISC::IStream &f,
                                           const CFilteredChat &src) {
  bool present;
  if (src.Window == NULL) {
    present = false;
    f.serial(present);
...
//=================================================================================================================
void CPeopleInterraction::saveFilteredDynChat(NLMISC::IStream &f,
                                              const CFilteredChat &src) {
  bool present;
  if (src.Window == NULL) {
    present = false;
...
bool CPeopleInterraction::saveUserChatsInfos(NLMISC::IStream &f) {
  ...
  try {
    ...
    // Save the free tellers only if they belongs to friend list to avoid the
...
bool CPeopleInterraction::loadUserChatsInfos(NLMISC::IStream &f) {
  ...
  try {
    ...
    f.serialCheck(NELID("TAHC"));
    if (ver >= 1) {
      //			CChatGroupWindow *pCGW =
      //PeopleInterraction.getChatGroupWindow();
      sint32 index;
      f.serial(index);
      /* Yoyo: decide to always start with the default channel (user) activated
              because complex (at this time, the buttons are not all active,
         must wait guild loading, UI:SAVE loading etc...) Hence this doesn't
         work for anything but User and Sysinfo (if it is activated....) NB:
         must still load the index for file format reason
...
//=================================================================================================================
void CPeopleInterraction::setupUserChatFromSummary(
    const CFilteredChatSummary &summary, CFilteredChat &dest) {
  // User Dest. Do not allow Universe Warning, because do not want a warning
  // open at load (moreover, the UNIVERSE tab should not be activated)
...
//=================================================================================================================
void CPeopleInterraction::setupUserDynChatFromSummary(
    const CFilteredDynChatSummary &summary, CFilteredChat &dest) {
  // src
  for (uint8 i = 0; i < CChatGroup::MaxDynChanPerPlayer; i++) {
...
//=================================================================================================================
void CPeopleInterraction::talkInDynamicChannel(uint32 channelNb,
                                               string sentence) {
  if (channelNb < CChatGroup::MaxDynChanPerPlayer) {
    DynamicChatEntryHandler[channelNb].msgEntered(sentence,
                                                  DynamicChat[channelNb]);
  }
}
...
void CPeopleInterraction::displayTellInMainChat(const string &playerName) {
  ...
  // display a new command '/name' in the chat. The player must enter a new
  // unique name for the party chat.
    ...
    if (PeopleInterraction.getPeopleFromCurrentMenu(list, peopleIndex)) {
      if (list == &PeopleInterraction.TeamList) // check for good list
      {
        ...
        if (GenericMsgHeaderMngr.pushNameToStream(msgName, out)) {
          ...
          NetMngr.push(out);
          // nlinfo("impulseCallBack : %s %d sent", msgName.c_str(),
          // teamMember);
        } else
          nlwarning("command 'dismiss_member': unknown message named '%s'.",
                    msgName.c_str());
      }
    }
  }
    ...
    if (PeopleInterraction.getPeopleFromCurrentMenu(list, peopleIndex)) {
      if (list == &PeopleInterraction.TeamList) // check for good list
      {
        ...
        teamMember);
                ...
                nlwarning("command 'set_leader': unknown message named '%s'.",
        msgName.c_str());
        */
        NLMISC::ICommand::execute("a setTeamLeader " + toString(peopleIndex),
                                  g_log);
      }
    }
  }
  ...
  void execute(CCtrlBase * /* pCaller */, const std::string & /* sParams */) {
    ...
    if (PeopleInterraction.getPeopleFromCurrentMenu(list, peopleIndex)) {
      if (list == &PeopleInterraction.TeamList) // check for good list
      {
        if (ClientCfg.Local) {
          NLGUI::CDBManager::getInstance()
              ->getDbProp(TEAM_DB_PATH ":SUCCESSOR_INDEX")
              ->setValue32(peopleIndex);
        } else {
          ...
          if (GenericMsgHeaderMngr.pushNameToStream(msgName, out)) {
            ...
            nlwarning("command 'set_successor': unknown message named '%s'.",
                      msgName.c_str());
        }
      }
    }
...
public:
  void execute(CCtrlBase * /* pCaller */, const std::string & /* sParams */) {
    ...
    if (GenericMsgHeaderMngr.pushNameToStream(msgName, out)) {
      ...
      nlwarning(
          "CHandlerContextQuitTeam::execute: unknown message name : '%s'.",
          msgName.c_str());
  }
};
...
class CHandlerTellContact : public IActionHandler {
  void execute(CCtrlBase *pCaller, const std::string & /* sParams */) {
    ...
    if (!ig)
      return;
    CGroupContainer *gc =
        static_cast<CGroupContainer *>(ig->getEnclosingContainer());
    if (!gc)
      return;
    CPeopleList *list;
    uint peopleIndex;
    if (PeopleInterraction.getPeopleFromContainerID(gc->getId(), list,
                                                    peopleIndex)) {
      CPeopleInterraction::displayTellInMainChat(list->getName(peopleIndex));
    }
  }
...
public:
  void execute(CCtrlBase *pCaller, const std::string &sParams) {
    /** This msg may have been triggered from valid button or from the edit box
     * itself, so retrieve the edit box from the enclosing group
     */
      ...
      if (group) {
        ...
        if (group) {
          ...
          if (!LastFatherAddContactId.empty()) {
            ...
            CInterfaceGroup *gc = dynamic_cast<CInterfaceGroup *>(
                CWidgetManager::getInstance()->getElementFromId(groupName));
            if (gc) {
              CGroupEditBox *geb = dynamic_cast<CGroupEditBox *>(
                  gc->getGroup("add_contact_eb:eb"));
              geb->setInputString(std::string());
            }
            CAHManager::getInstance()->runActionHandler("enter_modal", pCaller,
                                                        sParams);
          }
        }
      }
      ...
      if (fatherGC) {
        ...
        if (geb && !geb->getInputString().empty()) {
          ...
          if (lastIndex != std::string::npos) {
            ...
            CPeopleList *peopleList =
                PeopleInterraction.getPeopleListFromContainerID(
                    LastFatherAddContactId.substr(lastIndex + 1));
            if (peopleList) {
              // don't add if it is the player name
              if (!ClientCfg.Local &&
                  (UserEntity->getEntityName() == geb->getInputString())) {
                displayVisibleSystemMsg(
                    CI18N::get("uiCantAddYourSelfInContactList"));
              } else {
                PeopleInterraction.askAddContact(geb->getInputString(),
                                                 peopleList);
                geb->setInputString(std::string());
              }
            }
...
public:
  void execute(CCtrlBase * /* pCaller */, const std::string & /* sParams */) {
    CInterfaceManager *pIM = CInterfaceManager::getInstance();
    nlinfo("Load Order : %d", NLGUI::CDBManager::getInstance()
                                  ->getDbProp("UI:SAVE:CONTACT_LIST:SORT_ORDER")
                                  ->getValue32());
    CPeopleList::TSortOrder order = (CPeopleList::TSortOrder)(
        NLGUI::CDBManager::getInstance()
...
class CHandlerValidatePartyChatName : public IActionHandler {
  void execute(CCtrlBase * /* pCaller */, const std::string & /* sParams */) {
    ...
    // TODO GAMEDEV : create (or join ?) a new channel. Each channel (party
    // chat) should have a unique name in the game moreover, it should not have
    // the name of another available chat window (for example, it shouldn't be
    // named 'Around Me') It shouldn't have the name of an existing player,
    // either.. Maybe this last test can be done in local only

    if (!PeopleInterraction.testValidPartyChatName(title)) {
      displayVisibleSystemMsg(title + " : " +
...
REGISTER_ACTION_HANDLER(CHandlerValidatePartyChatName,
...
class CHandlerRemovePartyChat : public IActionHandler {
  void execute(CCtrlBase * /* pCaller */, const std::string & /* sParams */) {
    CChatWindow *chat = getChatWndMgr().getChatWindowFromCaller(
        CWidgetManager::getInstance()->getCtrlLaunchingModal());
    if (chat)
      PeopleInterraction.removePartyChat(chat);
  }
};
...
REGISTER_ACTION_HANDLER(CHandlerAddAllTeamMembersToPartyChat,
...
REGISTER_ACTION_HANDLER(CHandlerRemoveAllTeamMembersToPartyChat,
...
REGISTER_ACTION_HANDLER(CHandlerAddAllGuildMembersToPartyChat,
...
REGISTER_ACTION_HANDLER(CHandlerRemoveAllGuildMembersToPartyChat,
 ...
 * This create a menu with the standard window (team, around me ...) + the party
 * chat windows
 */
...
public:
  void execute(CCtrlBase *pCaller, const std::string &sParams) {
    ...
    uint insertionIndex =
        lastTargetSelectedIndex + 1; // insert after standard options
    if (partyChats) {
      for (uint l = 0; l < pl.PartyChats.size(); ++l) {
        menu->addLineAtIndex(
            insertionIndex, pl.PartyChats[l].Window->getTitle(),
            "chat_target_selected", toString(pl.PartyChats[l].ID));
        ++insertionIndex;
      }
    }
    ...
    if ((cw == PeopleInterraction.ChatGroup.Window) ||
      ...
      cw = PeopleInterraction.TheUserChat.Window;
      //			CChatStdInput &ci =
      //PeopleInterraction.ChatInput;
      CGroupMenu *pMenu = dynamic_cast<CGroupMenu *>(
          CWidgetManager::getInstance()->getElementFromId(
              "ui:interface:user_chat_target_menu"));
      CViewTextMenu *pMenuAround = dynamic_cast<CViewTextMenu *>(
          pMenu->getElement("ui:interface:user_chat_target_menu:around"));
      CViewTextMenu *pMenuRegion = dynamic_cast<CViewTextMenu *>(
          pMenu->getElement("ui:interface:user_chat_target_menu:region"));
      CViewTextMenu *pMenuUniverse = dynamic_cast<CViewTextMenu *>(
          pMenu->getElement("ui:interface:user_chat_target_menu:universe"));
      CViewTextMenu *pMenuTeam = dynamic_cast<CViewTextMenu *>(
          pMenu->getElement("ui:interface:user_chat_target_menu:team"));
      CViewTextMenu *pMenuGuild = dynamic_cast<CViewTextMenu *>(
          pMenu->getElement("ui:interface:user_chat_target_menu:guild"));
      const bool teamActive = NLGUI::CDBManager::getInstance()
                                  ->getDbProp("SERVER:GROUP:0:PRESENT")
                                  ->getValueBool();
      const bool guildActive = NLGUI::CDBManager::getInstance()
                                   ->getDbProp("SERVER:GUILD:NAME")
                                   ->getValueBool();
      if (pMenuAround)
        pMenuAround->setGrayed(false);
      if (pMenuRegion)
        pMenuRegion->setGrayed(false);
      if (pMenuUniverse)
        pMenuUniverse->setGrayed(false);
      if (pMenuTeam)
        pMenuTeam->setGrayed(!teamActive);
      if (pMenuGuild)
        pMenuGuild->setGrayed(!guildActive);
      ...
      for (uint i = 0; i < CChatGroup::MaxDynChanPerPlayer; i++) {
        ...
        if (active) {
          uint32 canWrite =
              NLGUI::CDBManager::getInstance()
                  ->getDbProp("SERVER:DYN_CHAT:CHANNEL" + s + ":WRITE_RIGHT")
                  ->getValue32();
          if (canWrite != 0) {
            string title;
            ...
            pMenu->addLineAtIndex(5 + insertion_index, res,
                                  "chat_target_selected", "dyn" + s, "dyn" + s);
            insertion_index++;
          }
        }
      }
...
class CHandlerChatTargetSelected : public IActionHandler {
  void execute(CCtrlBase * /* pCaller */, const std::string &sParams) {
    ...
    if (cw == PeopleInterraction.ChatGroup.Window) {
      PeopleInterraction.TheUserChat.Filter.setTargetGroup(
          cf.getTargetGroup(), cf.getTargetDynamicChannelDbIndex());
      CAHManager::getInstance()->runActionHandler("chat_group_filter", NULL,
                                                  "user");
    }
    ...
    if (fromString(sParams, partyChatID)) {
      ...
      // The party chat has been deleted while the menu was displayed it seems..
      // -> no-op
      return;
    }
  }
    ...
    if (PeopleInterraction.TheUserChat.Filter.getTargetGroup() ==
      ...
      if (im) {
        if (!NLGUI::CDBManager::getInstance()
                 ->getDbProp("UI:VARIABLES:IS_TEAM_PRESENT")
                 ->getValueBool()) {
 ...
 */
static CInterfaceGroup *createMenuCheckBox(const std::string &onclickL,
                                           const std::string &paramsL,
                                           bool checked) {
  ...
  CInterfaceGroup *ig =
      CWidgetManager::getInstance()->getParser()->createGroupInstance(
          "menu_checkbox", "", params, sizeof(params) / sizeof(params[0]));
  if (!ig)
    return NULL;
...
class CHandlerSelectChatSource : public IActionHandler {
  void execute(CCtrlBase *pCaller, const std::string & /* sParams */) {
    ...
    if (cw == pi.ChatGroup.Window) {
      ...
      menu = dynamic_cast<CGroupMenu *>(
          CWidgetManager::getInstance()->getElementFromId(
              MAIN_CHAT_SOURCE_MENU));

      // Remove all unused dynamic channels and set the names
      for (uint i = 0; i < CChatGroup::MaxDynChanPerPlayer; i++) {
        string s = toString(i);
        CViewTextMenu *pVTM = dynamic_cast<CViewTextMenu *>(
            CWidgetManager::getInstance()->getElementFromId(
                MAIN_CHAT_SOURCE_MENU + ":tab:dyn" + s));
        if (pVTM) {
          uint32 textId = ChatMngr.getDynamicChannelNameFromDbIndex(i);
          bool active = (textId != 0);
      ...
      // Simple menu
        ...
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:VARIABLES:GC_POPUP")
            ->setValue64(cw->getContainer()->isPopuped() ||
                                 cw->getContainer()->getLayerSetup() == 0
                             ? 1
                             : 0);
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:VARIABLES:GC_HAS_HELP")
            ->setValue64(!cw->getContainer()->getHelpPage().empty());
        CWidgetManager::getInstance()->enableModalWindow(pCaller,
    ...
    if (addUserChatEntries) {
      ...
      // AROUND ME
      menu->addLineAtIndex(insertionIndex, CI18N::get("uiAroundMe"),
                           FILTER_TOGGLE, "am");
      // add a checkbox
      menu->setUserGroupLeft(
          insertionIndex,
          createMenuCheckBox(FILTER_TOGGLE, "am",
                             pi.ChatInput.AroundMe.isListeningWindow(cw)));
      ++insertionIndex;

      // REGION
      menu->addLineAtIndex(insertionIndex, CI18N::get("uiREGION"),
                           FILTER_TOGGLE, "region");
      // add a checkbox
      menu->setUserGroupLeft(
          insertionIndex,
          createMenuCheckBox(FILTER_TOGGLE, "region",
                             pi.ChatInput.Region.isListeningWindow(cw)));
      ++insertionIndex;

      // UNIVERSE
      menu->addLineAtIndex(insertionIndex, CI18N::get("uiUNIVERSE"),
                           FILTER_TOGGLE, "universe");
      // add a checkbox
      menu->setUserGroupLeft(
          insertionIndex,
          createMenuCheckBox(FILTER_TOGGLE, "universe",
                             pi.ChatInput.Universe.isListeningWindow(cw)));
      ++insertionIndex;

      // TEAM
      menu->addLineAtIndex(insertionIndex, CI18N::get("uiTeam"), FILTER_TOGGLE,
                           "team");
      // add a checkbox
      menu->setUserGroupLeft(
          insertionIndex,
          createMenuCheckBox(FILTER_TOGGLE, "team",
                             pi.ChatInput.Team.isListeningWindow(cw)));
      ++insertionIndex;

      // GUILD
      menu->addLineAtIndex(insertionIndex, CI18N::get("uimGuild"),
                           FILTER_TOGGLE, "guild");
      // add a checkbox
      menu->setUserGroupLeft(
          insertionIndex,
          createMenuCheckBox(FILTER_TOGGLE, "guild",
                             pi.ChatInput.Guild.isListeningWindow(cw)));
      ++insertionIndex;

      // TELL
      // menu->addLineAtIndex(insertionIndex, CI18N::get("uiTell"),
      // FILTER_TOGGLE, "tell");
      // add a checkbox
      // menu->setUserGroupLeft(insertionIndex,
      // createMenuCheckBox(FILTER_TOGGLE, "tell",
      // pi.ChatInput.Tell.isListeningWindow(cw)));
      //++ insertionIndex;

      // SYSTEM INFOS
      menu->addLineAtIndex(insertionIndex, CI18N::get("uiSystemInfo"),
                           FILTER_TOGGLE, "si");
      // add a checkbox
      menu->setUserGroupLeft(
          insertionIndex,
          createMenuCheckBox(FILTER_TOGGLE, "si",
                             pi.ChatInput.SystemInfo.isListeningWindow(cw)));
      ++insertionIndex;
      ...
      for (uint l = 0; l < pc.size(); ++l) {
        if (pc[l].Filter != NULL) {
          menu->addLineAtIndex(insertionIndex, pc[l].Window->getTitle(),
                               FILTER_TOGGLE, toString(pc[l].ID));
          menu->setUserGroupLeft(
              insertionIndex,
              createMenuCheckBox(FILTER_TOGGLE, toString(pc[l].ID),
                                 pc[l].Filter->isListeningWindow(cw)));
          ++insertionIndex;
        }
      }
      ...
      for (uint8 i = 0; i < CChatGroup::MaxDynChanPerPlayer; i++) {
        ...
        if (active) {
          ...
          STRING_MANAGER::CStringManagerClient::instance()->getDynString(textId,
                                                                         title);
          menu->addLineAtIndex(insertionIndex, "[" + s + "] " + title,
                               FILTER_TOGGLE, "dyn" + s);
          menu->setUserGroupLeft(
              insertionIndex,
              createMenuCheckBox(
                  FILTER_TOGGLE, "dyn" + s,
                  pi.ChatInput.DynamicChat[i].isListeningWindow(cw)));
          ++insertionIndex;
        }
      }
    }
...
class CHandlerChatSourceSelected : public IActionHandler {
  void execute(CCtrlBase * /* pCaller */, const std::string &sParams) {
    ...
    if (nlstricmp(sParams, "guild") == 0) {
      if (ci.Guild.isListeningWindow(cw))
        ci.Guild.removeListeningWindow(cw);
      else
        ci.Guild.addListeningWindow(cw);
    } else
      // TEAM
      if (nlstricmp(sParams, "team") == 0) {
        if (ci.Team.isListeningWindow(cw))
          ci.Team.removeListeningWindow(cw);
        else
          ci.Team.addListeningWindow(cw);
      } else
        // AROUND ME
        if (nlstricmp(sParams, "am") == 0) {
          if (ci.AroundMe.isListeningWindow(cw))
            ci.AroundMe.removeListeningWindow(cw);
          else
            ci.AroundMe.addListeningWindow(cw);
        } else
          // REGION
          if (nlstricmp(sParams, "region") == 0) {
            if (ci.Region.isListeningWindow(cw))
              ci.Region.removeListeningWindow(cw);
            else
              ci.Region.addListeningWindow(cw);
          } else
            // UNIVERSE
            if (nlstricmp(sParams, "universe") == 0) {
              if (ci.Universe.isListeningWindow(cw))
                ci.Universe.removeListeningWindow(cw);
              else
                ci.Universe.addListeningWindow(cw);
            } else
              // TELL
              if (nlstricmp(sParams, "tell") == 0) {
                if (ci.Tell.isListeningWindow(cw))
                  ci.Tell.removeListeningWindow(cw);
                else
                  ci.Tell.addListeningWindow(cw);
              } else
                // SYSTEM INFOS
                if (nlstricmp(sParams, "si") == 0) {
                  if (ci.SystemInfo.isListeningWindow(cw))
                    ci.SystemInfo.removeListeningWindow(cw);
                  else
                    ci.SystemInfo.addListeningWindow(cw);
                } else
                  // PARTY CHAT
                  if (fromString(sParams, partyChatID)) {
                    std::vector<CPartyChatInfo> &partyChats =
                        PeopleInterraction.PartyChats;
                    for (uint k = 0; k < partyChats.size(); ++k) {
                      if (partyChats[k].ID == (uint)partyChatID) {
                        if (partyChats[k].Filter != NULL) {
                          if (partyChats[k].Filter->isListeningWindow(cw))
                            partyChats[k].Filter->removeListeningWindow(
                                partyChats[k].Window);
                          else
                            partyChats[k].Filter->addListeningWindow(cw);
                        }
                      }
                    }
                    ...
                    if (ci.DynamicChat[i].isListeningWindow(cw))
                      ci.DynamicChat[i].removeListeningWindow(cw);
                    else
                      ci.DynamicChat[i].addListeningWindow(cw);
                  }
  }
};
...
// 'ignore'
//-----------------------------------------------
NLMISC_COMMAND(ignore, "add or remove a player from the ignore list",
               "<player name>") {
  // Check parameters.
  if (args.size() < 1) {
    return false;
...
NLMISC_COMMAND(remove_party_chat, "Remove a party chat", "<party_chat_name>")
{
        ...
        {
                displayVisibleSystemMsg(title + " : " +
CI18N::get("uiBadPartyChatName")); return true;
        }
        if (!PeopleInterraction.removePartyChat(chat))
        {
                displayVisibleSystemMsg(title + " : " +
CI18N::get("uiCantRemovePartyChat")); return true;
        }
        return true;
}
...
NLMISC_COMMAND(add_to_party_chat, "Join the given party chat",
"<party_chat_name>")
{
        if (args.size() != 1)
        {
...
NLMISC_COMMAND(invite, "Invite someone to a party chat", "<people_name>
<party_chat_name>")
{
        if (args.size() != 2)
        {
        ...
        //                The people being invited should receive a popup to
announce that he is being invited return true;
}
...
static void displayVisibleSystemMsg(const string &msg, const string &cat) {
  ...
  if (CChatWindow::getChatWindowLaunchingCommand()) {
    CChatWindow::getChatWindowLaunchingCommand()->displayMessage(
        msg, im->getSystemInfoColor(cat), CChatGroup::system, 0, 2);
  }
}
