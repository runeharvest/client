...
// ***************************************************************************
bool CDBGroupListSheet::CSheetChild::isSheetValid(
...
bool CDBGroupListSheet::parse(xmlNodePtr cur, CInterfaceGroup *parentGroup) {
  ...
  if (prop)
    fromString((const char *)prop, _LeftMargin);
  prop = (char *)xmlGetProp(cur, (xmlChar *)"rmargin");
  if (prop)
    fromString((const char *)prop, _RightMargin);
  prop = (char *)xmlGetProp(cur, (xmlChar *)"tmargin");
  if (prop)
    fromString((const char *)prop, _TopMargin);
  prop = (char *)xmlGetProp(cur, (xmlChar *)"bmargin");
  if (prop)
    fromString((const char *)prop, _BottomMargin);

  // read row setup
  prop = (char *)xmlGetProp(cur, (xmlChar *)"rowmin");
  if (prop)
    fromString((const char *)prop, _MinRows);
  prop = (char *)xmlGetProp(cur, (xmlChar *)"rowmax");
  if (prop)
    fromString((const char *)prop, _MaxRows);
...
sint CDBGroupListSheet::getCurScrollValue() {
  ...
  if (_CurWantedRows > _MaxRows) {
    ...
    sint sv =
        (sint)floor(bias + _CurWantedRows * (float)_DummyGroup->getOfsY() /
                               favoid0((float)(_DummyGroup->getH() - _HSlot)));
...
void CDBGroupListSheet::updateCoords() {
  ...
  if (_Parent != NULL) {
    if (!_Parent->getActive())
      return;
    if ((_Parent->getParent() != NULL) && (!_Parent->getParent()->getActive()))
      return;
  }
      ...
      if (_Sectionable && !_SectionableError) {
        ...
        if (ctrlSectionId != curSectionId) {
          ...
          insertSectionGroupList(curSectionId + 1, ctrlSectionId + 1,
                                 groupSectionIndex, yItem, xStart, xItem);
          // new active section
...
// ***************************************************************************
void CDBGroupListSheet::insertSectionGroupList(sint firstSectionId,
                                               sint lastSectionId,
                                               uint &groupSectionIndex,
                                               sint &yItem, sint xStart,
                                               sint &xItem) {
...
// ***************************************************************************
void CDBGroupListSheet::clearViews() { CInterfaceGroup::clearViews(); }
...
void CDBGroupListSheet::setup() {
  ...
  _ButtonAddRow =
      dynamic_cast<CCtrlBaseButton *>(CInterfaceGroup::getCtrl("add_row"));
  _ButtonSubRow =
      dynamic_cast<CCtrlBaseButton *>(CInterfaceGroup::getCtrl("sub_row"));
  _ScrollBar =
  ...
  if (_ScrollBar) {
    // Create a dummy group JUST to be the target of the scrollBar. Not linked
    // to this, not displayed etc...
  ...
  for (i = 0; i < nbNodes; i++) {
    ...
    if (_DisplayEmptySlot) {
      if (i == 0)
        ...
        //				ctrl->initSheet(_DbBranchName+":"+toString(i-1+_StartDbIdx),
        //_CtrlInfo);
        ctrl->initSheetFast(_DbBranchName, (i - 1 + _StartDbIdx), slotNum,
                            _CtrlInfo);
    } else
...
public:
  virtual void execute(CCtrlBase *pCaller, const std::string & /* Params */) {
    CDBGroupListSheet *pLS =
        dynamic_cast<CDBGroupListSheet *>(pCaller->getParent());
    if (pLS == NULL)
      return;
    pLS->changeNbRow(+1);
  }
