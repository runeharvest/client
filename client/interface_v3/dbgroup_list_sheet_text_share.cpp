...
// ***************************************************************************
NLMISC_REGISTER_OBJECT(CViewBase, CDBGroupListSheetTextShare, std::string,
                       "list_sheet_share");
...
// ***************************************************************************
bool CDBGroupListSheetTextShare::parse(xmlNodePtr cur,
...
// ***************************************************************************
void CDBGroupListSheetTextShare::CSheetChildShare::init(
...
// ***************************************************************************
void CDBGroupListSheetTextShare::CSheetChildShare::updateViewText(
...
// ***************************************************************************
void CDBGroupListSheetTextShare::CSheetChildShare::hide(
    CDBGroupListSheetText *pFather) {
  CSheetChild::hide(pFather);

  // hide additional views
  if (NbMember != NULL)
    NbMember->setActive(false);
  if (Chance != NULL)
...
// ***************************************************************************
bool CDBGroupListSheetTextShare::CSheetChildShare::isInvalidated(
    CDBGroupListSheetText * /* pFather */) {
  ...
  if ((newNbMember != CacheNbMember) || (newChance != CacheChance) ||
...
// ***************************************************************************
void CDBGroupListSheetTextShare::CSheetChildShare::update(
    CDBGroupListSheetText *pFather) {
  uint8 newNbMember = (uint8)CurrentNbMember.getSInt32();
  uint8 newChance = (uint8)CurrentChance.getSInt32();
  bool newWanted = (CurrentWanted.getSInt32() != 0);
...
class CHandlerTeamShareChoose : public IActionHandler {
  virtual void execute(CCtrlBase *pCaller, const string & /* Params */) {
    ...
    // Retrieve index
    if (NLGUI::CDBManager::getInstance()->getDbProp(localDB)->getValue8() ==
        0) {
      strMsgName = "TEAM:SHARE_VALID_ITEM";
      NLGUI::CDBManager::getInstance()->getDbProp(localDB)->setValue8(
          1); // We can do it because it is in local
    } else {
      strMsgName = "TEAM:SHARE_INVALID_ITEM";
      NLGUI::CDBManager::getInstance()->getDbProp(localDB)->setValue8(
          0); // We can do it because it is in local
    }
    ...
    if (!GenericMsgHeaderMngr.pushNameToStream(strMsgName, out)) {
      ...
      uint8 sessionid = NLGUI::CDBManager::getInstance()
                            ->getDbProp("LOCAL:INVENTORY:SHARE:SESSION")
                            ->getValue8();
      out.serial(sessionid);
      NetMngr.push(out);
      // nlinfo("impulseCallBack : %s %d %d sent", strMsgName.c_str(), index,
      // sessionid);
    }
  }
};
