 ...
 * \date 2003
 */
class CChatInputFilter : public NLMISC::CRefCount,
                         public CChatWindow::IObserver {
public:
  CChatInputFilter() {
    FilterType = CChatGroup::say;
    DynamicChatDbIndex = 0;
  }
  virtual ~CChatInputFilter();
  /** Display a msg in the chat. The msg will be forwarded to all the listening
   * windows Listening windows will blick only if there isnt a single visible
   * listening window, so that the player can know if there's a message \param
   * windowVisible is not NULL, points a bool that will be filled with true if
   * one of the window on the which the msg was displayed is visible.
   */
  void displayMessage(const std::string &msg, NLMISC::CRGBA col,
                      uint numBlinks = 0, bool *windowVisible = NULL);
  /** The same as displayMessage, but with sender name, so that the msg will be
   * displayed in attached people lists as well
   */
  void
  displayTellMessage(/*TDataSetIndex &senderIndex, */ const std::string &msg,
                     const std::string &sender, NLMISC::CRGBA col,
                     uint numBlinks = 0, bool *windowVisible = NULL);
   ...
   * NB : Because of observer system, a chat window will call
   * removeListeningWindow automatically when it is destroyed
   */
  //@{
  ...
  // helpers : depending on the 'listening' flag value, remove or add the window
  // from the list
...
private:
  ...
  //
  // copy not supported
  CChatInputFilter(const CChatInputFilter & /* other */) : NLMISC::CRefCount() {
    nlassert(0);
  }
...
public:
   ...
   *	\param dynamicChannelId: valid only if group is 'dyn_chat' (for server
   *dynamic channels)
   */
  void setTargetGroup(CChatGroup::TGroupType groupType,
                      uint32 dynamicChannelDbIndex = 0,
                      bool allowUniverseWarning = true);
  CChatGroup::TGroupType getTargetGroup() const { return _TargetGroup; }
  uint32 getTargetDynamicChannelDbIndex() const {
    return _TargetDynamicChannelDbIndex;
  }
