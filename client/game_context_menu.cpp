...
void CGameContextMenu::init(const std::string &srcMenuId) {
  std::string menuId =
      srcMenuId.empty() ? std::string("game_context_menu") : srcMenuId;
  CInterfaceManager *pIM = CInterfaceManager::getInstance();

  _GroupMenu =
      dynamic_cast<CGroupMenu *>(CWidgetManager::getInstance()->getWindowFromId(
          "ui:interface:" + menuId + ""));
  _ContextVal = NLGUI::CDBManager::getInstance()->getDbProp(
      "SERVER:TARGET:CONTEXT_VAL", false);

  if (_GroupMenu == NULL) {
    nlwarning(("gamecontextmenu:init: 'ui:interface:" + menuId +
  ...
  if (_GroupMilkoPad != NULL) {
    _MilkoAttackDisengage = NLGUI::CDBManager::getInstance()->getDbProp(
        "UI:VARIABLES:MK_ATTACK", false);
    _MilkoAttDisBut1 = "ui:interface:milko_pad:content:mode1:mode1_content:"
                       "milko_actions:action5";
    _MilkoAttDisBut2 = "ui:interface:milko_pad:content:mode2:action5";
  } else {
    nlwarning("gamecontextmenu:init: 'ui:interface:milko_pad' window does not "
              "exist.");
  }

  if (_ContextVal == NULL) {
    nlwarning("gamecontextmenu:init: 'SERVER:TARGET:CONTEXT_VAL' node does not "
              "exist.");
    return;
  }

  // Some DB links
  _AvailablePrograms = NLGUI::CDBManager::getInstance()->getDbProp(
      "LOCAL:TARGET:CONTEXT_MENU:PROGRAMMES");
  _ServerTeamPresent = NLGUI::CDBManager::getInstance()->getDbProp(
      TEAM_DB_PATH ":0:PRESENT", false);
  _ServerInDuel =
      NLGUI::CDBManager::getInstance()->getDbProp("SERVER:USER:IN_DUEL", false);
  _ServerInPvpChallenge = NLGUI::CDBManager::getInstance()->getDbProp(
      "SERVER:USER:IN_PVP_CHALLENGE", false);
  _WebPageTitle = NLGUI::CDBManager::getInstance()->getDbProp(
      "LOCAL:TARGET:CONTEXT_MENU:WEB_PAGE_TITLE", false);
  _OutpostSheet = NLGUI::CDBManager::getInstance()->getDbProp(
      "LOCAL:TARGET:CONTEXT_MENU:OUTPOST", false);
  _OutpostRightToBannish = NLGUI::CDBManager::getInstance()->getDbProp(
      "SERVER:CHARACTER_INFO:PVP_OUTPOST:RIGHT_TO_BANISH", false);
  ...
  _TextTradeOutpostBuilding =
      "ui:interface:" + menuId + ":trade_outpost_building";
  _TextTradeGuildRoleMaster =
      "ui:interface:" + menuId + ":trade_guild_role_master";
  ...
  _TextOutpostBanishPlayer =
      "ui:interface:" + menuId + ":outpost_bannish_player";
...
void CGameContextMenu::update() {
  ...
  // botChatPrograms
  uint32 availablePrograms =
      filterAvailablePrograms((uint32)_AvailablePrograms->getValue32());
  ...
  if (UserEntity->isFighting()) {
    ...
    } else if (selection && selection->Type == CEntityCL::Player) {
      setupContextMenuTalkWithPlayer();
    }
  ...
  // Not in combat or riding mode.
    ...
    if (_TextLootAction) {
      _TextLootAction->setActive(false);
      // Action possible only if the client is not already busy and the
      // selection is able to do this with you..
      if (selection && selection->isDead() &&
          (selection->properties().lootable() ||
           selection->properties().liftable()) &&
    ...
    if (_TextQuartering) {
      ...
      // selection is able to do this with you..
      if (selection && selection->isDead() &&
          selection->properties().harvestable() && !R2::getEditor().isDMing())
    ...
    if (ForceTalkWithBot) // tmp
    {
      ...
      setupContextMenuTalkWithPlayer();
    } else if (UserEntity->selection() !=
               UserEntity->slot()) // Talking again to yourself ?
    {
      if (selection) {
        if (selection->Type == CEntityCL::Player) {
          setupContextMenuTalkWithPlayer();
        } else if (selection->properties().talkableTo()) {
          if (!CBotChatManager::getInstance()->getCurrPage()) // Not talking
          {
            ...
            if (distanceSquare <= MaxTalkingOutpostBuildingDistSquare)
              _OkTextTradeOutpostBuilding =
                  ((availablePrograms &
                    (1 << BOTCHATTYPE::TradeOutpostBuilding)) != 0);
          }
        }
      }
    }
  ...
  if (_TextPvpChallenge && _TextUnPvpChallenge) {
    if ((!UserEntity->isRiding()) &&
        (_ServerInPvpChallenge->getValue8() != 0)) {
  ...
  // Follow
  if (_TextFollow) {
    // Action possible only if the client is not already busy and the selection
    // is able to do this with you..
    _TextFollow->setActive(
        selection && (selection->slot() != UserEntity->slot()) &&
        (!selection->isForageSource()) && (selection->isDead() == false) &&
  ...
  if (_TextAssist) {
    // Action possible only if the client is not already busy and the selection
    // is able to do this with you..
    _TextAssist->setActive(!R2::getEditor().isDMing() && selection &&
  ...
  if (_TextInvit) {
    ...
    // User should not be flagged as invitable by himself, so no need to check
    // that selection is not the user
    if (selection && selection->properties().invitable() &&
        propValidation.invitable()) {
      invitable = true;
    }
  ...
  if (_TextGuildInvit) {
    // if server OK.
    if (filterAvailablePrograms(_AvailablePrograms->getValue32()) &
        (1 << BOTCHATTYPE::GuildInviteFlag))
  ...
  // Invite to friend list (only if target is a player, and not a friend
  // already)
  if (_TextAddToFriendList) {
    ...
    if (selection && selection->Type == CEntityCL::Player) {
      if (!selection->getEntityName().empty()) {
        sint peopleIndex = PeopleInterraction.FriendList.getIndexFromName(
  ...
  if (_TextMount)
    _TextMount->setActive(false);
  if (selection &&
      (selection->isUserPackAnimal() || (selection->isUserMount()))) {
    ...
    bool
        ok =
            testMenuOptionForPackAnimal(
                selection, animalIndex, true, _TextPAFollow, _TextPAStop,
                _TextPAFree, _TextPAEnterStable,
                NULL /*no 'leave stable' in context menu*/, pTextMount, NULL /*unmount always active in context menu when the character is riding*/);
...
bool CGameContextMenu::canAttack() {
  ...
  if (!CBotChatManager::getInstance()->getCurrPage()) // Not talking
  {
    ...
    if (selection && selection->isDead() == false) {
      uint32 availablePrograms =
          (uint32)filterAvailablePrograms(_AvailablePrograms->getValue32());

      if (selection->properties().attackable() ||
          (availablePrograms & (1 << BOTCHATTYPE::Attackable)))
        ret = true;
    }
  }
  return ret;
...
void CGameContextMenu::updateContextMenuWebPage(uint options) {
  ...
  // apply the active
  if (_TextWebPage)
    _TextWebPage->setActive(_OkTextWebPage);
}
...
void CGameContextMenu::updateContextMenuOutpostState(uint options) {
  ...
  if (_OkTextOutpostState) {
    ...
    if (pVTM)
      pVTM->setText(
          STRING_MANAGER::CStringManagerClient::getOutpostLocalizedName(
              outpostSheet));
  }

  // apply the active
  if (_TextOutpostState)
    _TextOutpostState->setActive(_OkTextOutpostState);
}
...
void CGameContextMenu::updateContextMenuOutpostBanish() {
  ...
  // validate text
  if (_TextOutpostBanishPlayer)
    _TextOutpostBanishPlayer->setActive(okForBanPlayer);
  if (_TextOutpostBanishGuild)
    _TextOutpostBanishGuild->setActive(okForBanGuild);
}
...
void CGameContextMenu::updateContextMenuTalkEntries(uint options) {
  if (ClientCfg.Local) {
    options =
        std::numeric_limits<uint>::max(); // in local mode, force all options to
                                          // be shown (for debug)
  }
  // news
  ...
  _OkTextTradeTeleport =
      ((options & (1 << BOTCHATTYPE::TradeTeleportFlag)) != 0);
  _OkTextTradeFaction = ((options & (1 << BOTCHATTYPE::TradeFactionFlag)) != 0);
  _OkTextTradeCosmetic =
      ((options & (1 << BOTCHATTYPE::TradeCosmeticFlag)) != 0);
  _OkTextTradeGuildOptions =
      ((options & (1 << BOTCHATTYPE::TradeBuildingOptions)) != 0);
  _OkTextTradeOutpostBuilding =
      ((options & (1 << BOTCHATTYPE::TradeOutpostBuilding)) != 0);
  _OkTextTradeGuildRoleMaster =
      ((options & (1 << BOTCHATTYPE::GuildRoleMaster)) != 0);
  _OkTextTradePact = ((options & (1 << BOTCHATTYPE::TradePactFlag)) != 0);
  _OkTextTradePhrase = ((options & (1 << BOTCHATTYPE::TradePhraseFlag)) != 0);
  // choose mission
  _OkTextChooseMission =
      ((options & (1 << BOTCHATTYPE::ChooseMissionFlag)) != 0);
  // create guild
  _OkTextCreateGuild = ((options & (1 << BOTCHATTYPE::CreateGuildFlag)) != 0);
  // New Yoyo BotChat
  _OkTextChooseZCCharge =
      ((options & (1 << BOTCHATTYPE::AskGuildDutyFlag)) != 0);
  _OkTextCancelZCCharge =
      ((options & (1 << BOTCHATTYPE::CancelGuildDutyFlag)) != 0);
...
// ***************************************************************************
void CGameContextMenu::setupContextMenuTalkWithPlayer() { _OkTextTalk = true; }

// ***************************************************************************
void CGameContextMenu::applyTextTalk() {
  if (_TextNews)
    _TextNews->setActive(_OkTextNews);
  if (_TextTradeItem)
    _TextTradeItem->setActive(_OkTextTradeItem);
  if (_TextTradeTeleport)
    _TextTradeTeleport->setActive(_OkTextTradeTeleport);
  if (_TextTradeFaction)
    _TextTradeFaction->setActive(_OkTextTradeFaction);
  if (_TextTradeCosmetic)
    _TextTradeCosmetic->setActive(_OkTextTradeCosmetic);
  if (_TextTradeGuildOptions)
    _TextTradeGuildOptions->setActive(_OkTextTradeGuildOptions);
  if (_TextTradeOutpostBuilding)
    _TextTradeOutpostBuilding->setActive(_OkTextTradeOutpostBuilding);
  if (_TextTradeGuildRoleMaster)
    _TextTradeGuildRoleMaster->setActive(_OkTextTradeGuildRoleMaster);
  if (_TextTradePact)
    _TextTradePact->setActive(_OkTextTradePact);
  if (_TextTradePhrase)
    _TextTradePhrase->setActive(_OkTextTradePhrase);
  if (_TextChooseMission)
    _TextChooseMission->setActive(_OkTextChooseMission);
  if (_TextCreateGuild) {
  ...
  if (_TextDynamicMission)
    _TextDynamicMission->setActive(_OkTextDynamicMission);
  if (_TextTalk)
    _TextTalk->setActive(_OkTextTalk);
  if (_TextChooseZCCharge)
    _TextChooseZCCharge->setActive(_OkTextChooseZCCharge);
  if (_TextChooseBuilding)
    _TextChooseBuilding->setActive(_OkTextChooseBuilding);
  if (_TextBuyRM)
    _TextBuyRM->setActive(_OkTextBuyRM);
  if (_TextUpgradeRM)
    _TextUpgradeRM->setActive(_OkTextUpgradeRM);
  if (_TextCancelZCCharge)
    _TextCancelZCCharge->setActive(_OkTextCancelZCCharge);
  if (_TextDestroyBuilding)
    _TextDestroyBuilding->setActive(_OkTextDestroyBuilding);
}
...
bool testMenuOptionForPackAnimal(CEntityCL *selectedAnimalInVision, uint index,
  ...
  if (clearAll) {
    ...
    if (pFollow) {
      ...
      pFollow->setGrayed(true);
    }
    if (pStop) {
      pStop->setActive(false);
      pStop->setGrayed(true);
    }
    if (pFree) {
      pFree->setActive(false);
      pFree->setGrayed(true);
    }
    if (pEnterStable) {
      pEnterStable->setActive(false);
      pEnterStable->setGrayed(true);
    }
    if (pLeaveStable) {
      pLeaveStable->setActive(false);
      pLeaveStable->setGrayed(true);
    }
    if (pMount) {
      pMount->setActive(false);
      pMount->setGrayed(true);
    }
    if (pUnmount) {
      pUnmount->setActive(false);
      pUnmount->setGrayed(true);
    }
  }

  // Don't enable anything if index not found (e.g. the character is not the
  // owner of the animal)
  ...
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
      toString("SERVER:PACK_ANIMAL:BEAST%d:STATUS", index), false);
  if (!node)
    return false;
  ANIMAL_STATUS::EAnimalStatus status =
      (ANIMAL_STATUS::EAnimalStatus)node->getValue32();
  node = NLGUI::CDBManager::getInstance()->getDbProp(
      toString("SERVER:PACK_ANIMAL:BEAST%d:TYPE", index), false);
  if (!node)
    return false;
  ANIMAL_TYPE::EAnimalType anitype =
  ...
  if (selectedAnimalInVision && (anitype == ANIMAL_TYPE::Mount) &&
      UserEntity->isRiding()) {
    uint8 mountSlot = (uint8)IngameDbMngr.getProp(
        "Entities:E" + toString(UserEntity->slot()) + ":P" +
  ...
  if (ANIMAL_STATUS::isSpawned(status)) {
    ...
    if (!userIsMountedOnIt) {
      if (pFollow && onLandscape)
        pFollow->setActive(true);
      if (pStop && onLandscape)
        pStop->setActive(true);

      // Enter/leave stable are only displayed when near a stable
      // ('canEnterLeaveStable')
      if (pEnterStable && onLandscape && canEnterLeaveStable)
        pEnterStable->setActive(true);
      if (pLeaveStable && ANIMAL_STATUS::isInStable(status) &&
          canEnterLeaveStable)
        pLeaveStable->setActive(true);
    ...
    if (ANIMAL_STATUS::isAlive(status)) {
      if (onLandscape && selectedAnimalInVision &&
          (distanceSquare <= MaxAnimalCommandDistSquare)) {
        if (pFollow)
          pFollow->setGrayed(false);
        if (pStop)
          pStop->setGrayed(false);
        if (pEnterStable)
          pEnterStable->setGrayed(false);
      }
      if (pLeaveStable)
  ...
  if ((anitype == ANIMAL_TYPE::Mount) && (ANIMAL_STATUS::isSpawned(status)) &&
      onLandscape) {
    if (userIsMountedOnIt) {
      // Always allow to unmount the mounted mount
      if (pUnmount)
        pUnmount->setActive(true);
      if (pUnmount)
        pUnmount->setGrayed(false);
    } else {
      if (pMount && ((!selectedAnimalInVision) ||
                     selectedAnimalInVision->properties().mountable())) {
        // Display 'Mount' if the entity is of mountable type
        pMount->setActive(true);

        // Enable 'Mount' if the character is not busy and the entity is alive
        // and close enough
