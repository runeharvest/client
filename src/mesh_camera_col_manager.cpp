...
// ***************************************************************************
void CMeshCameraColManager::instanceGroupLoaded(
...
void CMeshCameraColManager::instanceGroupAdded(NL3D::UInstanceGroup *ig) {
      ...
      if (!shape.empty()) {
        ...
        if (!colMesh.empty()) {
          ...
          avoidCollisionOutside =
              avoidCollisionOutside ||
              toLowerAscii(shapeName) == "ma_serre_interieur.shape";

          // then send the result to the collision manager, and keep the mesh
          // col id if succeed
          uint32 meshId = CollisionManager->addMeshInstanceCollision(
              colMesh, mat, avoidCollisionInside, avoidCollisionOutside);
          if (meshId)
  ...
  nlinfo("IG Camera Added: {%x, %d}: %.3f ms", (uint32)ig, ig->getNumInstance(),
  msPerTick * clock.getNumTicks());*/
}
