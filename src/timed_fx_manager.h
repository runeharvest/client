...
#if !FINAL_VERSION
  bool FromIG; // true if the fx comes from an ig, or false if it was generated
               // dynamically
#endif
public:
...
private:
  ...
  struct CTimeStampedFX {
    ...
    bool operator==(const CTimeStampedFX &rhs) const {
      return Date == rhs.Date && FX == rhs.FX;
    }
  ...
  /** A managed FX. Unless it is flagged as 'always intanciated', or is in a
   * group that is shutting down, such a fx can only be in one set : _FXToAdd if
   * it is not currently instanciated (or is shutting down), and _FXToRemove if
   * it is instanciated and is not being shut down. and will be removed in the
   * future.
   */
  ...
  public:
    ...
    NL3D::UParticleSystemInstance
        Instance; // Pointer to the actual FX. NULL if failed to instanciate or
                  // if not instanciated
    TState State;
    TTimeStampedFXPtrSet::iterator
        SetHandle; // if the fx is in a list (see its state), then it is an
                   // iterator into that list (for removal)
    //
    CManagedFX *
        *_PrevCandidateFX; // if the fx has asked to be instanciated, it is
                           // inserted in a list of candidate fxs. points the
                           // previous "Next" pointer even if the fx is
                           // currently instanciated, it remains in that list
                           // (those are the potnetially instanciated fxs)
    CManagedFX *_NextCandidateFX; // next candidate FX
    //
    CManagedFX **_PrevInstanciatedFX; // link into instanciated fx list, prev
                                      // (is also a candidate)
...
// Tmp for debug
  ...
  public:
    ...
    void computeStartHour(sint32 cycle, CClientDate &resultDate,
                          float cycleLength, float dayLength,
                          const NLMISC::CNoiseValue &nv) const;
    // compute end hour of that fx for the given day
    void computeEndHour(sint32 cycle, CClientDate &resultDate,
                        float cycleLength, float dayLength,
                        const NLMISC::CNoiseValue &nv) const;
  ...
  private:
    void computeHour(sint32 cycle, float bias, CClientDate &resultDate,
                     float cycleLength, float dayLength,
                     const NLMISC::CNoiseValue &nv, float minHour,
                     float maxHour) const;
  };
  // A group of managed fx. We never resize these vectors after creation so we
  // can keep pointers in them.
...
public:
   ...
   * \param noiseFrequency        Frequency of noise used to compute pseudo
   * random spwaning dates of fxs (depends on their spawn position and on the
   * date) \param maxNumberOfFXInstance Max number of fx instances that are
   * allowed to be instanciated at a given time. Nearest fx are instanciated
   * first \param sortDistanceInterval  precision with which fx are sorted in
   * distance. For example, with 1meter precision, fx at 0.2 & 0.5 from the user
   * are considered to be at the same distance \param maxDist               Max
   * dist at which fxs are sorted. For efficiency, it is important to keep
   * maxDist / sortDistanceInterval
   */
  void init(NL3D::UScene *scene, const CClientDate &startDate, float dayLength,
            float noiseFrequency, uint maxNumberOfFXInstances,
   ...
   */
  TFXGroupHandle add(const std::vector<CTimedFX> &fxs,
                     EGSPD::CSeason::TSeason season);
  ...
  // Update the manager state to match the new date. This add / removes FX as
  // necessary
  void update(const CClientDate &date, EGSPD::CSeason::TSeason currSeason,
              const NLMISC::CVector &camPos);
  // Set a new date, but do not update current fx. The new hour will be taken in
  // account during the next call to 'add'
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
private:
  // Set of fx to be instanciated in the future. It is managed as a priority
  // queue (sorted by date of instanciation).
  TTimeStampedFXPtrSet _FXToAdd;
  // Set of fx to remove in the future, and that are currently instanciated (and
  // are not shutting done). It is managed as a priority queue (sorted by date
  // of removal).
  ...
  // List of group that are shutting down (that is, shutDown(TFXGroupHandle
  // handle) was called on that group)
  ...
  //@{
  // FX to be displayed can be limited in number -> the nearest fx are
  // instanciated in priority To achieve this, we keep a list of candidate fx
  // for each interval of distance (thus achieving linear approximate sorting)

  typedef std::vector<CManagedFX *> TCandidateFXListSortedByDist;
  TCandidateFXListSortedByDist
      _CandidateFXListSortedByDist; // roughly sorted list of candidate fx.
                                    // Should be rebuilt completely when players
                                    // moves (done in linear time)
  TCandidateFXListSortedByDist
      _CandidateFXListSortedByDistTmp; // already allocated vect for fast
                                       // resorting of list when player moves

  // NB : this vector nevers grows
  bool _CandidateFXListTouched; // the list of candidate has been modified, so
                                // the manager should see which fxs should be
                                // instanciated, and which fxs should be removed
  float _SortDistance;          // length of each distance interval, in meters
  uint _MaxNumberOfFXInstances; // max number of instances that can be 'alive'
                                // at a time
  //@}
...
private:
  ...
  // insert a fx in list of candidate for instanciation (also unlink from a
  // previous list)
  void linkCandidateFX(TCandidateFXListSortedByDist &targetList, float dist,
                       CManagedFX *fx);
  ...
  // re-sort list of candidate fx by distance (this is necessary when player
  // moves)
  void updateCandidateFXListSorting();

public:
  // convert a cycle and an hour to a date
  static void cycleToDate(sint32 cycle, float hour, float cycleLength,
                          float dayLength, CClientDate &result);
  // give the matching cycle for a date
  static sint32 dateToCycle(const CClientDate &date, float cycleLength,
                            float dayLength);
};
