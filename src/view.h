...
public:
  ...
  float _CurrentCameraDistanceMax; // equal ClientCfg.CameraDistMax in Ryzom or
                                   // ring(play/test)mode or equal
                                   // ClientCfg.DMCameraDistMax in ring dm mode
...
protected:
  ...
  /// const. move back camera target to avoid problem of camera target too near
  /// a wall
  float _CameraCollisionDecal;
  // const the threshold is to avoid problem because of imprecision and camera
  // near clip
...
public:
  ...
  // Return the current Camera Target (for 3rd person only. 1st person: return
  // currentViewPos())
  CVector currentCameraTarget() const;

  // For debug only
  void getCamera3rdPersonSetup(CVector &cameraStart, CVector &cameraEnd,
   ...
   *	For optimisation, it should be the UserEntity pos, and not the view pos
   *(else 3rd person=> slower)
   */
  void refinePos(const CVector &pos) { _RefinePos = pos; }
  const CVector &refinePos() const { return _RefinePos; }

  // update the camera collision, must be called once per frame, after changing
  // viewPos() and/or view()
  void updateCameraCollision();

  NL3D::UInstanceGroup *getThirdPersonClusterSystem() const {
    return _ThirPersonClusterSystem;
  }
  ...
  // update the max distance of the camera for player that want to play as
  // player (camera must be near)
...
protected:
  ...
  void getCamera3rdPersonSetupInternal(CVector &cameraStart, CVector &cameraEnd,
