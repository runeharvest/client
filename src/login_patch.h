...
// Useful for using an external downloader (BitTorrent) use of interface with
// CGameDownloader from client_background_rd.exe using as install program
// Forward all messages to CRyzomControl that will forward to CGameDownloader
// (The engine of the install software)
class IAsyncDownloader {
public:
  virtual ~IAsyncDownloader() {}
  virtual void addToDownloadList(const std::string & /* patchName */,
                                 const std::string & /* sourceName */,
                                 uint32 /* timestamp */,
                                 const std::string & /* extractPath */,
                                 uint32 /* sevenZipSize */, uint32 /* size */) {
  }
  virtual void onFileInstall(const std::string & /* filename */,
                             uint32 /* index */, uint32 /* filecount */) {}
  virtual void onFileInstallFinished() {}
  virtual void onFileDownloading(const std::string & /* sourceName */,
                                 uint32 /* rate */, uint32 /* fileIndex */,
                                 uint32 /* fileCount */, uint64 /* fileSize */,
                                 uint64 /* fullSize */) {}
  virtual void onFileInstalling(const std::string & /* sourceName */,
                                uint32 /* rate */, uint32 /* fileIndex */,
                                uint32 /* fileCount */, uint64 /* fileSize */,
                                uint64 /* fullSize */) {}
  virtual void onFileDownloadFinished() {}
  virtual void fatalError(const std::string & /* errorId */,
...
public:
  virtual void setState(bool /* bOutputToLog */,
                        const std::string & /* ucsNewState */) {}
};
...
public:
  ...
  CInstallThreadEntry(const char *patchName, const char *sourceName,
                      uint32 timestamp, const char *extractPath, uint32 size,
                      uint32 sZipFileSize)
      : PatchName(patchName), SourceName(sourceName), Timestamp(timestamp),
        Size(size), SZipFileSize(sZipFileSize), ExtractPath(extractPath) {}
...
public:
  struct SFileToPatch {
    ...
    std::string
        SrcFileName; // Optional src file to which apply patch (empty unless
                     // some version of the file, possibly with all patchs
                     // applied, was found in the /unpack directory due to a
                     // download by the background downloader)
    std::vector<uint32> Patches;
    std::vector<uint32> PatcheSizes;
    uint32 LastFileDate; // Used during patching to set the date of the file to
                         // the Date of the last patch
    bool LocalFileToDelete; // Used during patching to know if we have to delete
                            // local file prior to apply patches
    bool LocalFileExists;
    bool Incremental; // Used during patching to know if we have to xdelta
                      // against a ref file
    std::string ExtractPath; // Used during patching to extract a bnp file to a
                             // specific directory
...
public:
  ...
  // serverPath is the name where to find all patches, files description and so
  // on serverVersion is a string describing the version of the description file
  void init(const std::vector<std::string> &patchURIs,
            const std::string &serverPath, const std::string &serverVersion);
  ...
  // start the check of bnp (download description file DescFilename, look at the
  // version of bnps) init must have been done
   ...
   * \param commitPatch When true, the patched version of the file will replace
   *the original file, else the patched version will be left in the /unpack
   *directory and the previous version in /data will remain unchanged. The
   *background downloader should set this to false.
   *
   */
  ...
  // Ok is set to true if the patch is ended and the patch process is ok (if the
  // patch is already ended ok is false)
  bool isPatchThreadEnded(bool &ok);

  void forceStopPatchThread();

  bool mustLaunchBatFile() const { return MustLaunchBatFile; }
  void createBatchFile(CProductDescriptionForClient &descFile,
                       bool wantRyzomRestart = true, bool useBatchFile = true);
  ...
  // ask to stop the Scan Data thread. NB: for security, the thread will
  // continue to the current scanned file, then stop. Hence, you must still wait
  // isScanDataThreadEnded() return true
  void askForStopScanDataThread();

  // If the scan thread is not running, touch the files with error, and return
  // the number of corrupted files
  uint applyScanDataResult();

  // get the current info Log for data Scan (true if some change from last get,
  // else text is not filled)
  ...
  // set an external downloader (useful for installer using BitTorrent
  // protocole)
  void setAsyncDownloader(IAsyncDownloader *asyncDownloader);
  // get the external downloader (useful for installer using BitTorrent
  // protocole or NULL)
  ...
  void setRyzomFilename(const std::string &ryzomFilename) {
    RyzomFilename = ryzomFilename;
  }
  ...
  static bool download(const std::string &patchPath,
  ...
  static bool extract(const std::string &patchPath,
                      ...
                      const std::string &exectName, void (*stopFun)());
  // Installation software must be able to indicate if ryzom must be stated at
  // end of installation
  void setStartRyzomAtEnd(bool startAtEnd) { _StartRyzomAtEnd = startAtEnd; }
  // Forward message to installation software if needed
  void fatalError(const std::string &errorId, const std::string &param1,
                  const std::string &param2);
...
private:
  ...
  std::string deleteFile(const std::string &filename,
                         bool bThrowException = true, bool bWarning = true);
  ...
  void getServerFile(const std::string &name, bool bZipped = false,
  ...
  void getPatchFromDesc(SFileToPatch &ftpOut, const CBNPFile &fIn,
                        bool forceCheckSumTest);

  bool bnpUnpack(const std::string &srcBigfile, const std::string &dstPath,
  ...
  static void getCorruptedFileInfo(const SFileToPatch &ftp,
                                   std::string &sTranslate);

  static bool downloadAndUnpack(const std::string &patchPath,
  ...
  void onFileDownloading(const std::string &sourceName, uint32 rate,
                         uint32 fileIndex, uint32 fileCount, uint64 fileSize,
                         uint64 fullSize);
  void onFileInstalling(const std::string &sourceName, uint32 rate,
                        uint32 fileIndex, uint32 fileCount, uint64 fileSize,
                        uint64 fullSize);

private:
  static int downloadProgressFunc(void *foo, double t, double d, double ultotal,
                                  double ulnow);
  static int validateProgress(void *foo, double t, double d, double ultotal,
                              double ulnow);
...
private:
  ...
  struct CDataScanState {
    ...
    bool Changed;
    CDataScanState() { Changed = false; }
  };
  typedef NLMISC::CSynchronized<CDataScanState> TSyncDataScanState;
  TSyncDataScanState DataScanState;
  // The date to apply when file is buggy: use default of year 2001, just to
  // have a coherent date
...
public:
  ...
  float getCurrentFileProgress() const {
    return fmodf(CurrentFilePatched, 1.f);
  }
...
private:
  ...
  void xDeltaPatch(const std::string &patch, const std::string &src,
                   const std::string &out);
};
/**
 * This thread peforms the installation of file that have been downloaded via
 * asynchrous download (BitTorrent) It unzip, delete obsolete file, extract file
 * from bnp
 */

class CInstallThread : public NLMISC::IRunnable {
public:
  CInstallThread(const std::vector<CInstallThreadEntry> entries)
      : _Entries(entries) {}
  void run();
...
public:
  CDownloadThread(const std::vector<CInstallThreadEntry> entries)
      : _Entries(entries) {}
  void run();
  std::vector<CInstallThreadEntry> _Entries;
};
...
/**
 *	This thread perform a checksum check on all existing files registered in
 *the local .idx \author Lionel Berenguier \author Nevrax France \date July 2004
 */
...
public:
  ...
  NLMISC::CAtomicBool
      AskForCancel; // true if the main thread ask to cancel the task
