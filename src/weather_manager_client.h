...
public:
  ...
  /// From CWeatherManager : Release all datas (fx models..). Should be called
  /// before deleting the scene
  ...
  void update(uint64 day, float hour, const CWeatherContext &wc,
              const NLMISC::CMatrix &camMat, const class CContinent &continent);
  ...
  /** Does the same than 'update', but let the user choose the weather value.
   * The weather value ranges from 0 to 1. The day and hour are needed only to
   * manage phenomena like thunder (need a clock to know when there are thunder
   * strikes) Small update, only to update current weather state
   */
  void manualUpdate(uint64 day, float hour, const CWeatherContext &wc,
                    float weatherValue, EGSPD::CSeason::TSeason season);
  /** Does the same than 'update', but let the user choose the weather value.
   * The weather value ranges from 0 to 1. The day and hour are needed only to
   * manage phenomena like thunder (need a clock to know when there are thunder
   * strikes) Full update, with 3d FX thunder update
   */
  void manualUpdate(uint64 day, float hour, const CWeatherContext &wc,
                    float weatherValue, EGSPD::CSeason::TSeason season,
                    const NLMISC::CMatrix &camMat,
                    const class CContinent &continent);
  ...
  void computeCloudState(uint64 day, float hour, const CWeatherContext &wc,
                         CCloudState &dest) const;
  // Compute the cloud state for the given weather value.
  void
  computeCloudState(float weatherValue, EGSPD::CSeason::TSeason season,
                    CCloudState &dest,
  ...
  // Get the 'thunder level', that is how bright the thunder lighting is. It
  // ranges from 0 (no thunder) to 1 (thunder impact)
  float getThunderLevel() const { return _ThunderLevel; }
  // To avoid rain in town, There's a Noprecipitation map that give a factor to
  // stop rain at some places
  float getLocalPrecipitationFactor() const {
    return _LocalPrecipitationFactor;
  }
  /////////////////////////////////////////////////////////////////
protected:
...
private:
  void manualUpdateImpl(uint64 day, float hour, const CWeatherContext &wc,
                        float weatherValue, EGSPD::CSeason::TSeason season);
  void manualUpdateImpl(uint64 day, float hour, const CWeatherContext &wc,
                        float weatherValue, EGSPD::CSeason::TSeason season,
                        const NLMISC::CMatrix &camMat,
...
private:
  ...
  void setupFXs(const NLMISC::CMatrix &camMat, NLPACS::UGlobalRetriever *gr,
                const class CContinent &continent);
  void setupWind(const CWeatherFunction *wf);

public:
  /** A time measurment for thunder function. Each cycle of thunder last a
   * period like 0.5 s. One thunder strike can happen only each 2 cycles PRIVATE
...
private:
  ...
  void
  updateThunder(uint64 day, float hour, const CWeatherContext &wc,
                bool manual = false, float manualWeatherValue = 0.f,
                EGSPD::CSeason::TSeason manualSeason = EGSPD::CSeason::Spring);
  // The thunder intensity at the given day and hour (value of the
  // 'ThunderIntensity' field in the weather setups)
  float getThunderIntensity(uint64 day, float hour, const CWeatherContext &wc);
  // Compute the minimum threshold that must be reached to create a thunder
  // strike
  float getThunderThreshold(uint64 thunderCycle, const CWeatherContext &wc);
  // update the thunder state between 2 times between which the thunder function
  // is linear
  bool updateThunderState(CThunderTimeMeasure &t0, CThunderTimeMeasure &t1,
