...
static std::vector<UInstance>
    ShapeAddedByCommand; // list of shapes added with the 'shape' command
...
NLMISC_COMMAND(where, "Ask information on the position", "") {
  // Check parameters.
  if (args.empty()) { // Create the message and send.
...
NLMISC_COMMAND(equipGroup, "equip group <name>", "name") {
  ...
  if (CItemGroupManager::getInstance()->equipGroup(args[0])) {
    ...
    // Use utf-8 string because group name can contain accentued characters (and
    // stuff like that)
...
NLMISC_COMMAND(moveGroup, "move group <name> to <dst>", "name dst") {
  ...
  if (CItemGroupManager::getInstance()->moveGroup(
          args[0], INVENTORIES::toInventory(args[1]))) {
...
NLMISC_COMMAND(
    createGroup,
    "create group <name> [true](create a <remove> for every unequiped item)",
    "name [removeUnequiped]") {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  if (args.empty()) {
...
bool randomFromString(std::string const &str, sint16 &val, sint16 min = -32768,
                      sint16 max = 32767) {
  bool negative = str[0] == '-';
...
NLMISC_COMMAND(dumpShapePos, "Dump Last Added Shape Pos.", "") {
#if FINAL_VERSION
  if (!hasPrivilegeDEV() && !hasPrivilegeSGM() && !hasPrivilegeGM() &&
      !hasPrivilegeVG() && !hasPrivilegeSG() && !hasPrivilegeG() &&
      !hasPrivilegeEM() && !hasPrivilegeEG())
    return true;
#endif // FINAL_VERSION
  ...
  IM->displaySystemInfo(
      toString("Shape Pos = %f, %f, %f", pos.x, pos.y, pos.z));
  return true;
}
//-----------------------------------------------
...
NLMISC_COMMAND(shape, "Add a shape in the scene.", "<shape file>") {
  ...
  if (args.size() < 1) {
    nlwarning("Command 'shape': need at least 1 parameter, try '/help shape' "
              "for more details.");
    return false;
  }
  if (!Scene) {
  ...
  if (!instance.empty()) {
    ...
    instance.setClusterSystem(
        UserEntity->getClusterSystem()); // for simplicity, assume it is in the
                                         // same cluster system than the user
...
NLMISC_COMMAND(bugReport, "Call the bug report tool with dump",
               "<AddScreenshot>") {
  const char *brname[] = {"bug_report.exe", "bug_report_r.exe",
                          "bug_report_rd.exe", "bug_report_df.exe",
                          "bug_report_d.exe"};
  ...
  if (fp != NULL) {
    ...
    //UserEntity->getName().toString().c_str()); 		fprintf (fp, "UserPosition:
    //%.2f %.2f %.2f\r\n", UserEntity->pos().x, UserEntity->pos().y,
    //UserEntity->pos().z); 		fprintf (fp, "ViewPosition: %.2f %.2f %.2f\r\n",
    //View.viewPos().x,   View.viewPos().y,   View.viewPos().z); 		time_t ts;
    //time( &ts ); 		fprintf (fp, "LocalTime: %s\r\n",
    //NLMISC::IDisplayer::dateToHumanString( ts ) ); 		fprintf (fp, "ServerTick:
    //%u\r\n", NetMngr.getCurrentServerTick()); 		fprintf (fp, "ConnectState:
    //%s\r\n", NetMngr.getConnectionStateCStr()); 		fprintf (fp, "LocalAddress:
...
//
// For example: "/c charName God 1" will set god mod on character if it's
// online, or keep command for wait character login
//

NLMISC_COMMAND(c, "Execute an admin command on character name",
               "<Character Name> <cmd> <arg>") {
  if (args.size() < 2)
    return false;
...
NLMISC_COMMAND(boxes, "Show/Hide selection boxes",
               "[<state> : 0 to Hide, anything else to Show. Invert the "
               "current state if nothing specified.]") {
#if FINAL_VERSION
...
NLMISC_COMMAND(dump,
               "Command to create a file with the current state of the client",
               "[<dump name>]") {
  if (args.size() > 1)
    return false;
...
NLMISC_COMMAND(verbose, "Enable/Disable some Debug Information",
               "none or magic") {
  // Check parameters.
  if (args.size() != 1) {
    // Help
...
NLMISC_COMMAND(verboseAnimSelection,
...
NLMISC_COMMAND(logEntities,
               "Write the position and orientation af all entities in the "
               "vision in the file 'entities.txt'",
               "") {
  // Check parameters
  if (args.size() != 0)
    return false;
...
NLMISC_COMMAND(log, "Add/Del Positive/Negative Filters for logs",
               "Log System <debug, info, warning, assert>, Type "
               "<pos/neg/del/reset>, Filter <string>") {
...
NLMISC_COMMAND(db, "Modify Database", "<Property> <Value>") {
  ...
  if (size == 2) {
    ...
    // Set the property.
    CCDBNodeLeaf *node =
        NLGUI::CDBManager::getInstance()->getDbProp(args[0], false);
    if (node)
      node->setValue64(value);
    else
      pIM->displaySystemInfo(
          toString("DB '%s' does not exist.", args[0].c_str()));
#else
...
public:
  ...
  virtual void progress(float value) {
    ...
    Driver->drawQuad(PROGRESS_BAR_LEFT, 0.5f - PROGRESS_BAR_HEIGHT / 2.0f,
                     PROGRESS_BAR_LEFT + PROGRESS_BAR_WIDTH,
                     0.5f + PROGRESS_BAR_HEIGHT / 2.0f, PROGRESS_BAR_BG_COLOR);
    // Display a progress bar
    Driver->drawQuad(PROGRESS_BAR_LEFT, 0.5f - PROGRESS_BAR_HEIGHT / 2.0f,
                     PROGRESS_BAR_LEFT + value * PROGRESS_BAR_WIDTH,
                     0.5f + PROGRESS_BAR_HEIGHT / 2.0f, PROGRESS_BAR_COLOR);
    if (TextContext != NULL) {
      // Init the Pen.
...
NLMISC_COMMAND(reloadAnim, "reload animations", "") {
  ...
  if (ClientCfg.UpdatePackedSheet) {
    for (uint i = 0; i < ClientCfg.UpdatePackedSheetPath.size(); i++) {
      ...
      dummy.pushCropedValues(
          (float)i / (float)ClientCfg.UpdatePackedSheetPath.size(),
          (float)(i + 1) / (float)ClientCfg.UpdatePackedSheetPath.size());
      CPath::addSearchPath(ClientCfg.UpdatePackedSheetPath[i], true, false,
                           &dummy);
...
NLMISC_COMMAND(reloadAttack, "reload attack", "") {
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  // replace attack list of creature in place (so pointers on character sheets
  // remains valid)
  ...
  for (CSheetManager::TEntitySheetMap::const_iterator it = sm.begin();
       it != sm.end(); ++it) {
    if (it->second.EntitySheet &&
        it->second.EntitySheet->Type == CEntitySheet::FAUNA) {
      // find matching sheet in new sheetManager
      const CEntitySheet *other = sheetManager.get(it->first);
      if (other) {
        // replace data in place
        ((CCharacterSheet &)*it->second.EntitySheet).AttackLists =
            ((const CCharacterSheet &)*other).AttackLists;
      }
    } else if (it->second.EntitySheet &&
               it->second.EntitySheet->Type == CEntitySheet::RACE_STATS) {
      // find matching sheet in new sheetManager
      const CEntitySheet *other = sheetManager.get(it->first);
      if (other) {
        // replace data in place
        ((CRaceStatsSheet &)*it->second.EntitySheet).AttackLists =
            ((const CRaceStatsSheet &)*other).AttackLists;
      }
    }
  }
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
NLMISC_COMMAND(missionReward, "debug", " ") {
  if (args.size() == 1) {
    ...
    CBitMemStream out;
    if (GenericMsgHeaderMngr.pushNameToStream("BOTCHAT:COMPLETE_MISSION",
                                              out)) {
      out.serial(index);
      NetMngr.push(out);
    } else
      nlwarning("<CBotChat acceptMission> : unknown message name : "
                "BOTCHAT:COMPLETE_MISSION");
    return true;
  }
  return false;
}
...
NLMISC_COMMAND(missionProgress, "debug", " ") {
  if (args.size() == 1) {
    ...
    fromString(args[0], index);
    CBitMemStream out;
    if (GenericMsgHeaderMngr.pushNameToStream("BOTCHAT:PROGRESS_MISSION",
                                              out)) {
      out.serial(index);
      NetMngr.push(out);
    } else
      nlwarning("<CBotChat acceptMission> : unknown message name : "
                "BOTCHAT:PROGRESS_MISSION");
    return true;
  }
  return false;
}
...
/*
NLMISC_COMMAND( displayDBModifs, "display server database modification in the
chat window"," ")
{
        if ( VerboseDatabase )
                CInterfaceManager::getInstance()->getChatOutput()->addTextChild("the
database is already in verbose mode",CRGBA(255,255,255,255)); else
        {
                CInterfaceManager::getInstance()->getChatOutput()->addTextChild("database
is now in verbose mode",CRGBA(255,255,255,255)); VerboseDatabase = true;
        }
        return true;
}

NLMISC_COMMAND( hideDBModifs, "stop displaying server database modification in
the chat window"," ")
{
        if ( !VerboseDatabase )
                CInterfaceManager::getInstance()->getChatOutput()->addTextChild("the
database is already not in verbose mode",CRGBA(255,255,255,255)); else
        {
                CInterfaceManager::getInstance()->getChatOutput()->addTextChild("database
is not in verbose mode anymore",CRGBA(255,255,255,255)); VerboseDatabase =
false;
        }
        return true;
}
...
NLMISC_COMMAND(pos, "Change the position of the user (in local only)",
               "<x, y, (z)> OR 1 name of 'tp.teleport_list'. or a bot name") {
  CVectorD newPos;
...
NLMISC_COMMAND(
    ...
    "<Slot> <Form> [posx posy posz] [relativeToPlayer]") {
  CInterfaceManager *IM = CInterfaceManager::getInstance();
  ...
  // Debug Infos
  nldebug("Command 'entity' : AddNewEntity with form %s in the slot %d",
          args[1].c_str(), slot);
  ...
  if (entity) {
    ...
    node = NLGUI::CDBManager::getInstance()->getDbProp(
        "SERVER:Entities:E" + toString("%d", slot) + ":P" +
            toString("%d", CLFECOMMON::PROPERTY_POSX),
        false);
    if (node) {
    ...
    if (dynamic_cast<CForageSourceCL *>(entity)) {
      ...
      NLGUI::CDBManager::getInstance()
          ->getDbProp("SERVER:Entities:E" + toString("%d", slot) + ":P" +
                      toString("%d", CLFECOMMON::PROPERTY_BARS))
          ->setValue64(barVal);
    ...
    } else if (dynamic_cast<CFxCL *>(entity)) // FX cl special
    {
      // must also update position, else don't work
      EntitiesMngr.updateVisualProperty(0, slot, CLFECOMMON::PROPERTY_POSITION);
    }
    //
    nlinfo("entity: slot: %d \"%s\", \"%f\", \"%f\", \"%f\" ", slot,
           args[1].c_str(), entityPos.x, entityPos.y, entityPos.z);
  } else
...
NLMISC_COMMAND(sheet2idx, "Return the index of a sheet",
               "<sheet name> <visual slot number>") {
  CInterfaceManager *IM = CInterfaceManager::getInstance();
  ...
  if (sheetId.buildSheetId(args[0])) {
    ...
    uint32 idx = CVisualSlotManager::getInstance()->sheet2Index(
        sheetId, (SLOTTYPE::EVisualSlot)slot);
...
NLMISC_COMMAND(serverstr, "display a server string value",
               "<serverstr string_id>") {
  if (args.size() != 1)
    return false;
...
NLMISC_COMMAND(cmde, "Send a command to a server with entityid",
               "<service name> <cmd>") {
  // Check parameters.
  if (args.size() < 2)
    return false;
...
NLMISC_COMMAND(mode, "Change the mode for an entity in a slot",
               "<Slot> <Mode> [dt(tick)]") {
  // Check parameters.
  if (args.size() < 2) {
    // Help
    CInterfaceManager::getInstance()->displaySystemInfo(
        "This command need 2 paramters :");
    CInterfaceManager::getInstance()->displaySystemInfo(
        "  <Slot> : the slot number of the entity to change");
    CInterfaceManager::getInstance()->displaySystemInfo(
        "  <Mode> : the mode wanted for the entity, one of the following "
        "number :");
    for (uint i = 0; i < MBEHAV::NUMBER_OF_MODES; ++i)
      CInterfaceManager::getInstance()->displaySystemInfo(NLMISC::toString(
          "    %d - %s", i, MBEHAV::modeToString((MBEHAV::EMode)i).c_str()));
  }
  // Right parameters number
    ...
    if (entity) {
      ...
      IngameDbMngr.setProp("Entities:E" + toString(slot) + ":P" +
                               toString(CLFECOMMON::PROPERTY_MODE),
                           mod);
      // Update the behaviour.
      sint32 dt = 10;
      if (args.size() > 2)
        fromString(args[2], dt);
      entity->updateVisualProperty(NetMngr.getCurrentServerTick() + dt,
                                   CLFECOMMON::PROPERTY_MODE);
    }
    // Invalid slot.
    else
      CInterfaceManager::getInstance()->displaySystemInfo(
          "There is no entity in the given slot");
  }
...
NLMISC_COMMAND(behaviour, "Change the behaviour for an entity in a slot",
               "<Slot> <Behaviour> [<Attack Intensity>] [<Impact Intensity>] "
               "[<delta HP>] [dt(tick)]") {
  // Check parameters.
  if (args.size() < 2 || args.size() > 6) {
    // Help
    CInterfaceManager::getInstance()->displaySystemInfo(
        "This command need 2 to 6 paramters :");
    CInterfaceManager::getInstance()->displaySystemInfo(
        "  <Slot> : the slot number of the entity to change");
    CInterfaceManager::getInstance()->displaySystemInfo(
        "  <Behaviour> : the behaviour to play for the entity, one of the "
        "following number :");
    for (uint i = 0; i < MBEHAV::EMOTE_BEGIN; ++i)
      CInterfaceManager::getInstance()->displaySystemInfo(NLMISC::toString(
          "    %d - %s", i,
          MBEHAV::behaviourToString((MBEHAV::EBehaviour)i).c_str()));
    CInterfaceManager::getInstance()->displaySystemInfo(NLMISC::toString(
        "    %d-%d - Emotes", MBEHAV::EMOTE_BEGIN, MBEHAV::EMOTE_END));
  } else {
    // Compute parameters
    CLFECOMMON::TCLEntityId slot;
    fromString(args[0], slot);
    ...
    if (entity) {
      // Write the behaviour in the DB.
      IngameDbMngr.setProp("Entities:E" + toString(slot) + ":P" +
                               toString(CLFECOMMON::PROPERTY_BEHAVIOUR),
                           behaviour);
      // Update the behaviour.
      entity->updateVisualProperty(NetMngr.getCurrentServerTick() + dt,
...
"<resist> : 0->not resisted, any other->resisted.\n")
{
        ...
        {
                // Help
//		CInterfaceManager::getInstance()->displaySystemInfo("This
command need 2 or 3 paramters :");
//		CInterfaceManager::getInstance()->displaySystemInfo("  <Slot> :
the slot number of the entity to change");
//		CInterfaceManager::getInstance()->displaySystemInfo("
<Behaviour> : the behaviour to play for the entity, one of the following number
:");
//		for(uint i = 0; i<MBEHAV::EMOTE_BEGIN; ++i)
//
CInterfaceManager::getInstance()->displaySystemInfo(NLMISC::toString("    %d -
%s", i, MBEHAV::behaviourToString((MBEHAV::EBehaviour)i)));
//
CInterfaceManager::getInstance()->displaySystemInfo(NLMISC::toString("    %d-%d
- Emotes", MBEHAV::EMOTE_BEGIN, MBEHAV::EMOTE_END));
        }
        else
        {
                // Compute parameters
                ...
                type %= 3;
                MBEHAV::EBehaviour behTmp =
(MBEHAV::EBehaviour)(MBEHAV::CASTING_GOOD+type); MBEHAV::CBehaviour
castingBeh(behTmp);
                // Result
                ...
                if     (type==0)
                        behaviour.Behaviour =
(MBEHAV::EBehaviour)(MBEHAV::END_CASTING_GOOD_SUCCESS    + result); else
if(type==1) behaviour.Behaviour =
(MBEHAV::EBehaviour)(MBEHAV::END_CASTING_BAD_SUCCESS     + result); else
                        behaviour.Behaviour =
(MBEHAV::EBehaviour)(MBEHAV::END_CASTING_NEUTRAL_SUCCESS + result); uint16
spellPower, impactIntensity;
                // Spell Power
                ...
                {
                        // Write the behaviour in the DB.
                        IngameDbMngr.setProp("Entities:E" + toString(slot) +
":P" + toString(CLFECOMMON::PROPERTY_BEHAVIOUR), castingBeh);
                        // Update the behaviour.
                        entity->updateVisualProperty(NetMngr.getCurrentServerTick()+10,
CLFECOMMON::PROPERTY_BEHAVIOUR);
                        // Write the behaviour in the DB.
                        IngameDbMngr.setProp("Entities:E" + toString(slot) +
":P" + toString(CLFECOMMON::PROPERTY_BEHAVIOUR), behaviour);
                        // Update the behaviour.
                        entity->updateVisualProperty(NetMngr.getCurrentServerTick()+50,
CLFECOMMON::PROPERTY_BEHAVIOUR);
                }
                else
                        CInterfaceManager::getInstance()->displaySystemInfo("There
is no entity in the given slot");
        }
...
NLMISC_COMMAND(spell, "Cast a spell",
  ...
  if (args.size() != 6) {
    ...
    //the slot number of the entity to change");
    //		CInterfaceManager::getInstance()->displaySystemInfo("
    //<Behaviour> : the behaviour to play for the entity, one of the following
    //number :"); 		for(uint i = 0; i<MBEHAV::EMOTE_BEGIN; ++i)
    //			CInterfaceManager::getInstance()->displaySystemInfo(NLMISC::toString("
    //%d - %s", i, MBEHAV::behaviourToString((MBEHAV::EBehaviour)i)));
    //		CInterfaceManager::getInstance()->displaySystemInfo(NLMISC::toString("
    //%d-%d - Emotes", MBEHAV::EMOTE_BEGIN, MBEHAV::EMOTE_END));
  } else {
    // Compute parameters
    ...
    result %= 4;
    behaviour.Behaviour =
        (MBEHAV::EBehaviour)(MBEHAV::CAST_OFF_FAIL + type * 4 + result);
    // Spell Power
    uint16 spellMode;
...
NLMISC_COMMAND(settarget,
               "Set a target for an entity. Do not set the target slot to "
               "remove the target",
               "<Slot> [<Target Slot>]") {
  ...
  case 1: {
    ...
    if (entity)
      ...
      nlwarning("command 'settarget': there is no entity in the slot %d",
                entitySlot);
  } break;
...
NLMISC_COMMAND(particle,
               "Create a particule at the user position (play "
               "FireWorkA_with_sound.ps by default)",
               "[<filename.ps>]") {
  string fn;
...
NLMISC_COMMAND(move, "Move an entity", "Slot: [1-254]") {
  ...
  if (entity) {
    sint64 x =
        (sint64)((entity->pos().x + UserEntity->front().x * 10.0) * 1000.0);
    sint64 y =
        (sint64)((entity->pos().y + UserEntity->front().y * 10.0) * 1000.0);
    sint64 z =
        (sint64)((entity->pos().z + UserEntity->front().z * 10.0) * 1000.0);
    ...
    // Update the position.
    EntitiesMngr.updateVisualProperty(NetMngr.getCurrentServerTick() + 30, slot,
                                      0);
    ...
    IngameDbMngr.setProp("Entities:E" + toString(slot) + ":P2", z);
    // Update the position.
    EntitiesMngr.updateVisualProperty(NetMngr.getCurrentServerTick() + 60, slot,
                                      0);
  } else
...
NLMISC_COMMAND(moveRel, "Move an entity, specifying delta pos from current",
               "Slot: [1-254] dx(m) dy(m) [dt(tick)] [predictedIV(tick)]") {
  // Check parameters.
  if (args.size() < 3)
    return false;
  ...
  if (entity) {
    ...
    IngameDbMngr.setProp("Entities:E" + toString(slot) + ":P2", z);
    // Update the position.
    EntitiesMngr.updateVisualProperty(NetMngr.getCurrentServerTick() + dt, slot,
                                      0, pi);
  } else
    nlwarning("command 'move' : there is no entity allocated in slot %d", slot);
...
NLMISC_COMMAND(orient, "Orient an entity",
               "Slot: [1-254] orient(degree) [dt(tick)]") {
  // Check parameters.
  if (args.size() < 2)
    return false;
  ...
  if (entity) {
    ...
    IngameDbMngr.setProp("Entities:E" + toString(slot) + ":P" +
                             toString(CLFECOMMON::PROPERTY_ORIENTATION),
                         r.u32[0]);
    // Update the position.
    EntitiesMngr.updateVisualProperty(NetMngr.getCurrentServerTick() + dt, slot,
...
NLMISC_COMMAND(moveTo, "Move an entity to another one",
               "<slot(from)>:[1-254], <slot(to)>:[0-254] default 0") {
  sint64 x, y, z;
  ...
  if (args.size() == 1) {
    ...
    if (entity) {
      ...
      nlwarning("command 'move_to' : there is no entity allocated for the dest "
                "in slot %d",
                slotDest);
      return true;
    }
...
NLMISC_COMMAND(setMode,
               "Set The Mode for an Entity without to add a stage for it",
               "<slot> <mode>") {
  // Check parameters.
  if (args.size() != 2)
    return false;
  ...
  if (entity)
    ...
    nlwarning("command 'setMode' : there is no entity allocated in slot '%d'",
              slot);
...
NLMISC_COMMAND(paintTarget, "Modify the target color",
  ...
  SPropVisualB vB;
  SPropVisualC vC;
  const string propNameA =
      toString("SERVER:Entities:E%d:P%d", slot, CLFECOMMON::PROPERTY_VPA);
  const string propNameB =
      toString("SERVER:Entities:E%d:P%d", slot, CLFECOMMON::PROPERTY_VPB);
  const string propNameC =
      toString("SERVER:Entities:E%d:P%d", slot, CLFECOMMON::PROPERTY_VPC);
  vA.PropertyA =
      NLGUI::CDBManager::getInstance()->getDbProp(propNameA)->getValue64();
  vB.PropertyB =
      NLGUI::CDBManager::getInstance()->getDbProp(propNameB)->getValue64();
  vC.PropertyC =
  ...
  // Set the database.
  NLGUI::CDBManager::getInstance()->getDbProp(propNameA)->setValue64(
      (sint64)vA.PropertyA);
  NLGUI::CDBManager::getInstance()->getDbProp(propNameB)->setValue64(
      (sint64)vB.PropertyB);
  NLGUI::CDBManager::getInstance()->getDbProp(propNameC)->setValue64(
      (sint64)vC.PropertyC);
...
NLMISC_COMMAND(playAnim, "Try to play the animation to the target",
               "<anim name>") {
  // Check parameters
  if (args.size() != 1)
    return false;
  ...
  if (character)
    character->setAnim(CAnimationStateSheet::Idle,
                       (TAnimStateKey)CAnimation::UnknownAnim, animId);
  return true;
}
...
NLMISC_COMMAND(vP, "Modify the Visual Property",
  ...
  if (type <= 7) {
    ...
    switch (type) {
    case 0:
      vslot = SLOTTYPE::CHEST_SLOT;
      break;
    case 1:
      vslot = SLOTTYPE::LEGS_SLOT;
      break;
    case 2:
      vslot = SLOTTYPE::ARMS_SLOT;
      break;
    case 3:
      vslot = SLOTTYPE::HEAD_SLOT;
      break;
    case 4:
      vslot = SLOTTYPE::RIGHT_HAND_SLOT;
      break;
    case 5:
      vslot = SLOTTYPE::LEFT_HAND_SLOT;
      break;
    case 6:
      vslot = SLOTTYPE::FEET_SLOT;
      break;
    case 7:
      vslot = SLOTTYPE::HANDS_SLOT;
      break;
    default:
      break;
    }
...
NLMISC_COMMAND(altLook, "Modify the Alternative Look Property",
  ...
  altLookProp.Summary =
      NLGUI::CDBManager::getInstance()->getDbProp(propName)->getValue64();
  uint32 colorTop, colorBot, weaponRightHand, weaponLeftHand, seed, colorHair,
      hat;
  ...
  // Set the database.
  NLGUI::CDBManager::getInstance()->getDbProp(propName)->setValue64(
      (sint64)altLookProp.Summary);
  // Force to update properties.
  EntitiesMngr.updateVisualProperty(0, slot, CLFECOMMON::PROPERTY_VPA);
...
NLMISC_COMMAND(
  ...
  if (entity) {
    ...
    nlwarning(
        "command 'changeColors': there is no entity allocated in slot '%d'",
        slot);
...
NLMISC_COMMAND(harvestDeposit, "harvest a deposit", "") {
  ...
  /*	CBitMemStream out;
     ...
     CWidgetManager::getInstance()->getWindowFromId("ui:interface:harvest")->setActive(true);
                  ...
                  nlwarning("command : unknown message name :
     'HARVEST:DEPOSIT'");
  */
  return true;
}
...
NLMISC_COMMAND(mount, "Set the entity to mount", "<Slot> [<Mount>]") {
  ...
  // Set the database.
  string propName = toString("SERVER:Entities:E%d:P%d", slot,
                             CLFECOMMON::PROPERTY_ENTITY_MOUNTED_ID);
  NLGUI::CDBManager::getInstance()->getDbProp(propName)->setValue64(mount);
  // Force to update properties.
  EntitiesMngr.updateVisualProperty(0, slot,
...
NLMISC_COMMAND(learnAllBrick, "learn all bricks (only in local mode)", "") {
  ...
  for (;;) {
    CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
        toString("SERVER:BRICK_FAMILY:%d:BRICKS", i), false);
    if (node)
...
NLMISC_COMMAND(learnBrick, "learn a specified brick (only in local mode)",
               "<brick number or name>") {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
...
NLMISC_COMMAND(learnPhrase,
               "learn all bricks of a specified phrase (only in local mode)",
               "<phrase sheetId or name>") {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  CSBrickManager *pBM = CSBrickManager::getInstance();
...
/*NLMISC_COMMAND(xp, "To gain XP in a given Skill","<Amount Xp> <Skill>
[<Speciality>]")
{
        // Check parameters.
        if( args.size() < 2 || args.size() > 3 )
...
NLMISC_COMMAND(money, "To earn Money (only in local mode)",
               "<very big seed> [<big seed>] [<medium seed>] [<small seed>]") {
  if (args.size() != 1)
    return false;
  uint64 money;
  ...
  CInterfaceManager *im = CInterfaceManager::getInstance();
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:INVENTORY:MONEY")
      ->setValue64(money);
  return true;
  /*
          sint32 a = 0;
          ...
          NLGUI::CDBManager::getInstance()->getDbProp(ls +
     ":QUANTITY")->setValue32(a); NLGUI::CDBManager::getInstance()->getDbProp(ms
     + ":QUANTITY")->setValue32(b);
          NLGUI::CDBManager::getInstance()->getDbProp(bs +
     ":QUANTITY")->setValue32(c);
          NLGUI::CDBManager::getInstance()->getDbProp(vbs +
     ":QUANTITY")->setValue32(d); return true;
  */
}
/*
NLMISC_COMMAND( createPerso, "create a new character", "Parameters:\n-Character
name\n-Race( Fyros, Tryker...)\n-gender(Male, Female)\n-Role( MeleeFighter,
RangeFighter, AttackCaster, BufferCaster, HealerCaster...)\n-Level (1-25 (but
more accepted)>" )
{
        // Check parameters.
        ...
        if( GenericMsgHeaderMngr.pushNameToStream(msgType,bms) )
        {
                ...
                bms.serial( level );
                NetMngr.push( bms );
                nldebug("<create_perso> sending 'CHEAT:CREATE_CHARACTER' message
to server");
        }
        else
        {
                nlwarning("<create_perso> unknown message name :
CHEAT:CREATE_CHARACTER");
        }
        return true;
}
*/
/*
NLMISC_COMMAND( add_role, "add role to character", "<Role( MeleeFighter,
RangeFighter, AttackCaster, BufferCaster, HealerCaster...), Level (1-25 (but
more accepted))>" )
{
        // Check parameters.
        ...
        if( GenericMsgHeaderMngr.pushNameToStream(msgType,bms) )
        {
                ...
                bms.serial( level );
                NetMngr.push( bms );
                nldebug("<add_role> sending 'CHEAT:ADD_ROLE' message to
server");
        }
        else
        {
  ...
  if (entPtr) {
    if (entPtr->skeleton()) {
      if (entPtr->skeleton()->getLastClippedState()) {
        ...
        IngameDbMngr.setProp("Entities:E" + toString("%d", entSlot) + ":P" +
                                 toString("%d", CLFECOMMON::PROPERTY_POSX),
                             x);
        IngameDbMngr.setProp("Entities:E" + toString("%d", entSlot) + ":P" +
                                 toString("%d", CLFECOMMON::PROPERTY_POSY),
        ...
        IngameDbMngr.setProp("Entities:E" + toString("%d", entSlot) + ":P" +
                                 toString("%d", CLFECOMMON::PROPERTY_POSZ),
                             z);
        entPtr->updateVisualProperty(NetMngr.getCurrentServerTick(),
        ...
        IngameDbMngr.setProp("Entities:E" + toString("%d", entSlot) + ":P" +
                                 toString("%d", CLFECOMMON::PROPERTY_POSZ),
                             z);
        entPtr->updateVisualProperty(NetMngr.getCurrentServerTick() + 5,
                                     CLFECOMMON::PROPERTY_POSITION);
      }
    }
  }
...
NLMISC_COMMAND(testLongBubble, "To display a bubble with a long text",
               "<entity>") {
  if (args.size() != 1)
    return false;
  uint entityId;
  ...
  string text("test\ntest\ntest\ntest\ntest\ntest\ntest\ntest\ntest\ntest\ntest"
              "\ntest\ntest\ntest\ntest\ntest\ntest\ntest\ntest\ntest\ntest\nte"
              "st\ntest\ntest\ntest\ntest\ntest\ntest\ntest\n");
  uint duration = CWidgetManager::getInstance()
                      ->getSystemOption(CWidgetManager::OptionTimeoutBubbles)
                      .getValSInt32();
...
#define DIST_TO_COMMAND(commandName, variableName)                             \
  /* Check Parameters */                                                       \
  if (args.size() != 2) {                                                      \
    nlwarning("Command '" #commandName                                         \
              "': need 2 parameters, try '/help " #commandName                 \
              "' for more details.");                                          \
    return false;                                                              \
  }                                                                            \
                                                                               \
  ...
  if (!sheetId.buildSheetId(args[0])) {                                        \
    ...
    sheetId = CSheetId(nSheetId);                                              \
    if (sheetId == CSheetId::Unknown) {                                        \
      nlwarning("Command '" #commandName "': '%s' is not a valid form.",       \
                args[0].c_str());                                              \
      return false;                                                            \
    }                                                                          \
  }                                                                            \
  ...
  if (dist < 0) {                                                              \
    ...
    return false;                                                              \
                                                                               ...
                                                                               \
  CCharacterSheet *ch =                                                        \
      dynamic_cast<CCharacterSheet *>(SheetMngr.get(sheetId));                 \
  if (ch == 0) {                                                               \
    nlwarning("Command '" #commandName                                         \
              "': cannot find the character for the given sheet.");            \
    return false;                                                              \
  }                                                                            \
                                                                               \
...
//-----------------------------------------------
NLMISC_COMMAND(dist2front,
               "Change the distance to the front for a given sheet.",
               "<form> <dist>") {
...
//-----------------------------------------------
NLMISC_COMMAND(dist2back, "Change the distance to the back for a given sheet.",
               "<form> <dist>") {
...
//-----------------------------------------------
NLMISC_COMMAND(dist2side, "Change the distance to the side for a given sheet.",
               "<form> <dist>") {
  DIST_TO_COMMAND(dist2side, DistToSide);
}

// Change the parent of an entity. 'parent slot' not defined remove the current
// parent.
NLMISC_COMMAND(parent, "Change the parent of an entity.",
               "<slot> [<parent slot>]") {
  ...
  case 1: {
    ...
    if (entity) {
      ...
      nlwarning("command 'parent': there is no entity in the slot %d",
                entitySlot);
  } break;
...
NLMISC_COMMAND(displayInventoryCounter,
               "display the Inventory counter to compare with db counter", "") {

  CInterfaceManager *pIM = CInterfaceManager::getInstance();

  uint srvVal = NLGUI::CDBManager::getInstance()
                    ->getDbProp("SERVER:INVENTORY:COUNTER")
                    ->getValue32();
  ...
  pIM->displaySystemInfo("ServerCounter: " + toString(srvVal) +
...
NLMISC_COMMAND(displayActionCounter, "display the action counters", "") {
  ...
  // next
  uint srvVal = NLGUI::CDBManager::getInstance()
                    ->getDbProp(PHRASE_DB_COUNTER_NEXT)
                    ->getValue32();
  uint locVal = pPM->getPhraseNextExecuteCounter();
  srvVal &= PHRASE_EXECUTE_COUNTER_MASK;
  locVal &= PHRASE_EXECUTE_COUNTER_MASK;

  pIM->displaySystemInfo("NextCounter: " + toString(srvVal) +
                         "/ LocalCounter: " + toString(locVal));

  // cycle
  srvVal = NLGUI::CDBManager::getInstance()
               ->getDbProp(PHRASE_DB_COUNTER_CYCLE)
               ->getValue32();
  locVal = pPM->getPhraseCycleExecuteCounter();
  srvVal &= PHRASE_EXECUTE_COUNTER_MASK;
  locVal &= PHRASE_EXECUTE_COUNTER_MASK;

  pIM->displaySystemInfo("CycleCounter: " + toString(srvVal) +
                         "/ LocalCounter: " + toString(locVal));
...
NLMISC_COMMAND(reconnect, "Reconnect to the same shard (self Far TP)", "") {
  // If the server is up, the egs will begin the quit sequence (shortened only
  // if we are in edition or animation mode). If the server is down or frozen, a
  // second /reconnect will be necessary to make the client reconnect but if you
  // reconnect before letting the EGS save the character file, the previous
  ...
  case CLoginStateMachine::st_leave_shard:
    ...
    break;
  default:
    log.displayNL("Can't reconnect from LoginSM state %u",
                  (uint)LoginSM.getCurrentState());
  }
...
static inline bool operator<(const CItemSheetSort &lhs,
                             const CItemSheetSort &rhs) {
...
NLMISC_COMMAND(dumpItems, "Sort items by category & display their sheet ids",
  ...
  //
  for (std::vector<CItemSheetSort>::iterator itemIt = isVect.begin();
       itemIt != isVect.end(); ++itemIt) {
    std::string info;
    info = "FAMILY: ";
...
NLMISC_COMMAND(browse,
  ...
  CAHManager::getInstance()->runActionHandler(
      "browse", NULL,
      "name=ui:interface:help_browser:content:html|url=" + args[0]);
  return true;
}
...
NLMISC_COMMAND(openRingWindow, "Browse the main page in the ring web browser.",
  ...
  CAHManager::getInstance()->runActionHandler(
      "browse", NULL,
      "name=ui:interface:r2ed_web_admin:content:admin_web_page|url=" +
          RingMainURL);
  return true;
}

NLMISC_COMMAND(browseRingAdmin,
  ...
  CAHManager::getInstance()->runActionHandler(
      "browse", NULL,
      "name=ui:interface:r2ed_web_admin:content:admin_web_page|url=" + args[0]);
  return true;
}
...
NLMISC_COMMAND(GUFriend, "invite a player to become a friend of the guild",
               "<player name>") {
  if (args.size() != 1)
    return false;
...
NLMISC_COMMAND(GUSetSuccessor, "set the successor of the guild leader",
               "<player name>") {
  if (args.size() != 1)
    return false;
...
NLMISC_COMMAND(logFaberMpCompatibles,
  ...
  if (brick)
    itemBuilt =
        dynamic_cast<CItemSheet *>(SheetMngr.get(brick->FaberPlan.ItemBuilt));
  if (brick && itemBuilt) {
    ...
    for (it = sheetMap.begin(); it != sheetMap.end(); it++) {
      CItemSheet *mp = const_cast<CItemSheet *>(
          dynamic_cast<const CItemSheet *>(it->second.EntitySheet));
    ...
    nlinfo("**********  FABERLOG  **********");
    nlinfo("  ItemBuilt Origin: %s",
           ITEM_ORIGIN::enumToString(itemBuilt->ItemOrigin).c_str());
    nlinfo("  NumMPSlot: %d", numMpSlots);

    // Parse All Slots.
    for (uint i = 0; i < numMpSlots; i++) {
      CSBrickSheet::CFaberPlan::CItemPartMP &mpSlot =
          brick->FaberPlan.ItemPartMps[i];
      nlinfo("  MPSlot %d", i);
      nlinfo("  Quantity: %d", mpSlot.Quantity);
      nlinfo("  TypeReq: %s",
             RM_FABER_TYPE::toString(mpSlot.FaberTypeFilter).c_str());
      ...
      for (uint i = 0; i < mps.size(); i++) {
        ...
        if (mpSlot.FaberTypeFilter != RM_FABER_TYPE::Unknown &&
            !itemSheet->canBuildItemPart(mpSlot.FaberTypeFilter,
                                         itemBuilt->ItemOrigin))
          ok = false;
...
NLMISC_COMMAND(debugItemInfo, "simulate a ItemInfo received from server",
               "itemSlotId version [enchant]") {
  CItemInfos itemInfo;
...
NLMISC_COMMAND(getSkillValue, "get a skill value by its name", "skill_name") {
  ...
  CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
      toString("SERVER:CHARACTER_INFO:SKILLS:%d:SKILL", skillId), false);
  if (node) {
...
NLMISC_COMMAND(setSkillValue, "set a skill value by its name",
               "skill_name value") {
  if (args.size() != 2)
    return false;
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
...
NLMISC_COMMAND(getBaseSkillValue, "get a baseskill value by its name",
               "skill_name") {
  if (args.size() != 1)
    return false;
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  uint skillId = (uint)SKILLS::toSkill(args[0]);
  CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
      toString("SERVER:CHARACTER_INFO:SKILLS:%d:BaseSKILL", skillId), false);
  if (node) {
    pIM->displaySystemInfo(toString(node->getValue32()));
  }
...
NLMISC_COMMAND(setBaseSkillValue, "set a baseskill value by its name",
               "skill_name value") {
  if (args.size() != 2)
    return false;
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
...
NLMISC_COMMAND(setAllSkillValue,
  ...
  for (uint i = 0; i < SKILLS::NUM_SKILLS; i++) {
    CCDBNodeLeaf *node;
    node = NLGUI::CDBManager::getInstance()->getDbProp(
        toString("SERVER:CHARACTER_INFO:SKILLS:%d:BaseSKILL", i), false);
    if (node)
      node->setValue32(value);
    node = NLGUI::CDBManager::getInstance()->getDbProp(
        toString("SERVER:CHARACTER_INFO:SKILLS:%d:SKILL", i), false);
    if (node)
      node->setValue32(value);
  }
...
template <class T> bool reloadSheets(string filter, string wildcardFilter) {
  CSheetManager sheetManager;
  ...
  ClientSheetsStrings.memoryCompress();
  // copy sheets into current sheet manager (because numerous ptr are kept on
  // the previous sheets in various places)
...
CMD_RELOAD_SHEET(reloadSucces_chances_table, "succes_chances_table",
CSuccessTableSheet) CMD_RELOAD_SHEET(reloadAutomaton_list, "automaton_list",
CAutomatonListSheet) CMD_RELOAD_SHEET(reloadAnimset_list, "animset_list",
CAnimationSetListSheet) CMD_RELOAD_SHEET(reloadAnimation_fx, "animation_fx",
CAnimationFXSheet) CMD_RELOAD_SHEET(reloadEmot, "emot", CEmotListSheet)
...
NLMISC_COMMAND(vprop,
               "Flush the Visual Property (local only). you must write to the "
               "DB before (but if you give the value in the 3rd arg)",
               "slot propId [val]") {
  if (args.size() != 2 && args.size() != 3)
    return false;
  uint slot;
  ...
  if (args.size() == 3) {
    ...
    CInterfaceManager *pIM = CInterfaceManager::getInstance();
    CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
        toString("SERVER:Entities:E%d:P%d", slot, propId), false);
    if (node)
      node->setValue64(val);
  }
...
NLMISC_COMMAND(forceDisplayFXBBoxes, "Force to display bboxes of all fxs",
               "0=off, 1=off") {
  if (args.size() != 1)
    return false;
  bool on;
...
NLMISC_COMMAND(dumpVillages, "Dump villages loading zones in a bitmap",
               "filename>") {
  if (args.size() != 1)
    return false;
...
NLMISC_COMMAND(dumpRainMap, "Dump fog rain map", "filename>") {
  ...
  ContinentMngr.cur()->dumpFogMap(CFogMapBuild::NoPrecipitation, args[0],
                                  CContinent::ChannelR, colorLookup);
  return true;
}
...
NLMISC_COMMAND(stick_log, "", "<slot>") {
  ...
  if (skel) {
    ...
    for (uint i = 0; i < sticks.size(); i++) {
      ...
      inst.cast(sticks[i]);
      if (!inst.empty()) {
        string str = toString("  %d: %X. %s", i, inst.getObjectPtr(),
                              inst.getShapeName().c_str());
        nlinfo(str.c_str());
        pIM->displaySystemInfo(str);
      } else {
        string str = toString("  %d: %X. NOT a TransformShape", i,
                              sticks[i].getObjectPtr());
        nlinfo(str.c_str());
        pIM->displaySystemInfo(str);
      }
    }
...
NLMISC_COMMAND(fillAllInfoVersion, "", "<version>") {
  ...
  for (i = 0; i < CPlayerTrade::NumTradeSlot; i++) {
    CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
        toString("SERVER:EXCHANGE:GIVE:%d:INFO_VERSION", i), false);
    if (node)
      node->setValue32(ver);
    node = NLGUI::CDBManager::getInstance()->getDbProp(
        toString("SERVER:EXCHANGE:RECEIVE:%d:INFO_VERSION", i), false);
    if (node)
      node->setValue32(ver);
  }
...
NLMISC_COMMAND(fullFillInventory, "", "dbstring sheetName") {
  ...
  for (uint i = 0; i < num; i++) {
    CCDBNodeLeaf *nl;
    nl = NLGUI::CDBManager::getInstance()->getDbProp(
        args[0] + ":" + toString(i) + ":SHEET", false);
    if (nl) {
      nl->setValue64(value);
      nl = NLGUI::CDBManager::getInstance()->getDbProp(
          args[0] + ":" + toString(i) + ":QUALITY", false);
      if (nl)
        nl->setValue64(i);
      nl = NLGUI::CDBManager::getInstance()->getDbProp(
          args[0] + ":" + toString(i) + ":PREREQUISIT_VALID", false);
      if (nl)
        nl->setValue64(1);
    }
  }
...
NLMISC_COMMAND(fillAllItemPreReq, "", "dbstring value") {
  ...
  for (;;) {
    CInterfaceManager *pIM = CInterfaceManager::getInstance();
    CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
        toString("%s:%d:PREREQUISIT_VALID", dbBase.c_str(), index), false);
    if (!node)
      break;
    node->setValue32(value);
    index++;
...
NLMISC_COMMAND(setLightHour,
               "force the light hour, (negative value to reset to auto)",
               "<hour>") {
  if (args.size() != 1)
    return false;
...
NLMISC_COMMAND(jobAnim, "set the job anim specialisation of an entity",
               "eid number") {
  if (args.size() != 2)
    return false;
...
NLMISC_COMMAND(startLogStageChange,
...
NLMISC_COMMAND(stopLogStageChange,
               "stop to log the change of watched entity stages", "") {
  EntitiesMngr.stopLogStageChange();
  return true;
}

NLMISC_COMMAND(testReceiveMissionInfo,
...
NLMISC_COMMAND(dumpUICoords, "Debug only : dump all coords info of an UI",
               "uiid") {
  if (args.size() != 1)
    return false;

  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  CInterfaceElement *el =
      CWidgetManager::getInstance()->getElementFromId(args[0]);
  if (!el) {
    pIM->displaySystemInfo(
        toString("dumpUICoords: '%s' does not exist", args[0].c_str()));
  } else {
    pIM->displaySystemInfo(
        toString("dumpUICoords: **** '%s'", args[0].c_str()));
    ...
    pIM->displaySystemInfo(
        toString("    parent= '%s'",
                 el->getParent() ? el->getParent()->getId().c_str() : ""));
    pIM->displaySystemInfo(toString(
        "    parentpos= '%s'",
        el->getParentPos() ? el->getParentPos()->getId().c_str() : ""));
    pIM->displaySystemInfo(toString(
        "    parentsize= '%s'",
        el->getParentSize() ? el->getParentSize()->getId().c_str() : ""));
    ...
    // parent
    if (hsParent & Hotspot_Bx)
      pr += "B";
    else if (hsParent & Hotspot_Mx)
      pr += "M";
    else if (hsParent & Hotspot_Tx)
      pr += "T";
    else
      pr += "?";
    if (hsParent & Hotspot_xL)
      pr += "L";
    else if (hsParent & Hotspot_xM)
      pr += "M";
    else if (hsParent & Hotspot_xR)
      pr += "R";
    else
      pr += "?";
    pr += " ";
    // self
    if (hsSelf & Hotspot_Bx)
      pr += "B";
    else if (hsSelf & Hotspot_Mx)
      pr += "M";
    else if (hsSelf & Hotspot_Tx)
      pr += "T";
    else
      pr += "?";
    if (hsSelf & Hotspot_xL)
      pr += "L";
    else if (hsSelf & Hotspot_xM)
      pr += "M";
    else if (hsSelf & Hotspot_xR)
      pr += "R";
    else
      pr += "?";
...
// Command to clear the dump of done Files opened and Async File Manager done
// files (for debug)
NLMISC_COMMAND(
    clearDumpFiles,
...
// LUA
// ***************************************************************************
const char *LUADebugNotEnabledMsg =
    "Lua Commands are available only if you add 'AllowDebugLua= 1;' in your "
    "client.cfg";
...
NLMISC_COMMAND(luaReload, "reload all .lua script files", "") {
  ...
  if (ClientCfg.AllowDebugLua) {
    ...
    pIM->displaySystemInfo(
        LuaHelperStuff::formatLuaErrorSysInfo(LUADebugNotEnabledMsg));
    return false;
  }
}
...
NLMISC_COMMAND(luaInfo, "Dump some information on LUA state",
               "detaillevel from 0 to 2") {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  if (ClientCfg.AllowDebugLua) {
    if (args.size() != 1)
      return false;
...
NLMISC_COMMAND(luaObject, "Dump the content of a lua object",
               "<table name> [maxDepth = 20, 0 for no limits]") {
  if (args.empty())
    return false;
  if (args.size() > 2)
    return false;
  ...
  try {
    // make a reference to the table to be inspected (is this this a primitive
    // type, just make a copy)
...
CUserCommand::CUserCommand(const string &commandName, const string &help,
                           const string &argsHelp)
...
void CUserCommand::addMode(const string &action, uint numArg, bool infiniteAgr,
...
bool CUserCommand::execute(const std::string & /* rawCommandString */,
                           const std::vector<std::string> &args,
                           NLMISC::CLog & /* log */, bool /* quiet */,
                           bool /* human */) {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  ...
  else if (!InfiniteMode.Keywords.empty() &&
           (args.size() >= InfiniteMode.KeywordsCount))
    mode = &InfiniteMode;
  ...
  if (mode) {
    ...
    for (i = 0; i < keywords.size(); i++) {
      if ((keywords[i] == "$") || (keywords[i] == "+")) {
        if ((uint)index >= args.size()) {
          ...
          pIM->displaySystemInfo(CommandName + " : " +
    ...
    CAHManager::getInstance()->runActionHandler(
        mode->Action, CWidgetManager::getInstance()->getOldCaptureKeyboard(),
        finalArgs);
  } else {
    // Not enough argument
    pIM->displaySystemInfo(CommandName + " : " +
...
void CUserCommand::createCommand(const char *name, const char *action,
  ...
  // Find action name
  string help;
  const CBaseAction *ab =
      Actions.getBaseAction(::CAction::CName(action, ptrParams));
  if (ab)
          ...
          for (uint j = 0; j < ab->Parameters.size(); j++) {
            ...
            if (ab->Parameters[j].Name == keywords[i - 2]) {
              ...
              if (!argsHelp.empty())
                argsHelp += " ";
              argsHelp +=
                  "<" + CI18N::get(ab->Parameters[j].LocalizedName) + ">";
              bFound = true;
            }
          }
          // Not found ? Warning
          if (!bFound) {
            nlwarning("Argument %s not found in command %s using action %s",
                      keywords[i - 2].c_str(), name, action);
          }
        }
      }
    }
...
NLMISC_COMMAND(
  ...
  std::string newPrim =
      NLMISC::toString("<CHILD TYPE=\"CPrimPoint\">            \n\
                       ...
                       (float)EntitiesMngr.entity(0)->pos().x,
                       (float)EntitiesMngr.entity(0)->pos().y,
                       (float)EntitiesMngr.entity(0)->pos().z, comment.c_str());
...
NLMISC_COMMAND(clear, "clear content of current char window",
               "<chat window caller id>") {
  if (args.size() > 1)
    return false;
  CInterfaceManager *im = CInterfaceManager::getInstance();
  im->flushDebugWindow();
  CChatWindow *cw;
  if (args.size() == 1) {
    cw = getChatWndMgr().getChatWindowFromCaller(dynamic_cast<CCtrlBase *>(
        CWidgetManager::getInstance()->getElementFromId(args[0])));
  } else {
    // if no chat window (or enclosed control) id is given then see if a chat
    // window called this command
    cw = CChatWindow::getChatWindowLaunchingCommand();
  }
  if (cw && cw->getContainer()) {
    CGroupList *gl =
        dynamic_cast<CGroupList *>(cw->getContainer()->getGroup("text_list"));
    if (gl) {
...
NLMISC_COMMAND(tickToDate, "convert a tick value into a readable ryzom time",
  ...
  std::string readableDate =
      toString("Day = %d, hour = %d:%d", rt.getRyzomDay(), (int)floorf(ryTime),
               (int)floorf(60.f * fmodf(ryTime, 1.f)));
...
NLMISC_COMMAND(dumpShapeMaxDist, "dump max dist for shapes", "") {
  /*
          ...
          for (CSheetManager::TEntitySheetMap::const_iterator it =
     sheets.begin(); it != sheets.end(); ++it)
          {
                  CCharacterSheet *cs = dynamic_cast<CCharacterSheet
     *>(SheetMngr.get(it->first)); if (cs)
                  {
                          equipList.clear();
                          ...
                          {
                                  std::string item =
     toLowerAscii(equipList[k]->getItem()); if (!item.empty())
                                  {

                                          string ext =
     CFile::getExtension(item); if (ext == "shape")
                                          {
                                                  if (!shapeSet.count(item))
                                                  {
                                                          UInstance inst =
     Scene->createInstance(item); if (!inst.empty())
                                                          {
                                                                  shapes.insert(make_pair(inst.getDistMax(),
     item)); Scene->deleteInstance(inst);
                                                          }
          ...
          for (TShapeMap::iterator it = shapes.begin(); it != shapes.end();
     ++it)
          {
                  nlwarning("Dist = %f, shape = %s", it->first,
     it->second.c_str());
          }
  */
  return true;
...
NLMISC_COMMAND(dumpContinentCorners, "dump max dist for shapes", "") {
  ...
  if (posMin.x > posMax.x)
    std::swap(posMin.x, posMax.x);
  if (posMin.y > posMax.y)
    std::swap(posMin.y, posMax.y);
  posMax.x += 160.f;
  posMax.y += 160.f;
  nlwarning("min = (%f, %f), max = (%f, %f)", posMin.x, posMin.y, posMax.x,
            posMax.y);
  return true;
}

#if !FINAL_VERSION
NLMISC_COMMAND(setMission, "locally set a mission text for test",
               "<mission index><text>") {
  if (!ClientCfg.Local)
    return false;
  if (args.size() != 2)
  ...
  if (index < 15) {
    NLGUI::CDBManager::getInstance()
        ->getDbProp(toString("SERVER:MISSIONS:%d:TITLE", (int)index))
        ->setValue32(strID);
    NLGUI::CDBManager::getInstance()
        ->getDbProp(toString("SERVER:MISSIONS:%d:FINISHED", (int)index))
        ->setValue32(0);
  } else {
    NLGUI::CDBManager::getInstance()
        ->getDbProp(toString("SERVER:GROUP:MISSIONS:%d:TITLE", (int)index - 15))
        ->setValue32(strID);
    NLGUI::CDBManager::getInstance()
        ->getDbProp(
            toString("SERVER:GROUP:MISSIONS:%d:FINISHED", (int)index - 15))
        ->setValue32(0);
  }
  setDynString(strID++, args[1]);
  return true;
...
static bool setMissionStep(uint missionIndex, uint stepIndex, uint32 strID) {
  ...
  if (missionIndex < 15) {
    NLGUI::CDBManager::getInstance()
        ->getDbProp(toString("SERVER:MISSIONS:%d:GOALS:%d:TEXT",
                             (int)missionIndex, (int)stepIndex))
        ->setValue32(strID);
  } else {
    NLGUI::CDBManager::getInstance()
        ->getDbProp(toString("SERVER:GROUP:MISSIONS:%d:GOALS:%d:TEXT",
                             (int)(missionIndex - 15), (int)stepIndex))
        ->setValue32(strID);
  }
  return true;
}
...
NLMISC_COMMAND(setMissionStep, "locally set a mission step for test",
               "<mission index><step index><text>") {
  if (!ClientCfg.Local)
    return false;
  if (args.size() != 3)
    return false;
  ...
  if (!setMissionStep(missionIndex, stepIndex, strID))
    return false;
...
NLMISC_COMMAND(clearMissionStep, "locally set a mission step for test",
               "<mission index><step index><text>") {
  if (!ClientCfg.Local)
    return false;
  if (args.size() != 2)
...
#if !FINAL_VERSION
NLMISC_COMMAND(clearMission, "clear the content of a mission",
               "<mission index>") {
  if (!ClientCfg.Local)
    return false;
  if (args.size() != 1)
...
#if !FINAL_VERSION
NLMISC_COMMAND(finishMission, "clear the content of a mission",
               "<mission index>") {
  if (!ClientCfg.Local)
    return false;
  if (args.size() != 1)
...
#if !FINAL_VERSION
NLMISC_COMMAND(failMission, "clear the content of a mission",
               "<mission index>") {
  if (!ClientCfg.Local)
    return false;
  if (args.size() != 1)
...
NLMISC_COMMAND(em, "emote command", "<emote phrase>") {
  ...
  if (pIM) {
    ...
    CAHManager::getInstance()->runActionHandler(
        "emote", NULL, "nb=0|behav=255|custom_phrase=" + emotePhrase);
    return true;
  }
  return false;
}
