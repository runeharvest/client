...
void buildLightCycleDesc(CLightCycleDesc &dest,
  ...
  float intervalDayNight =
      slc.NightHour >= slc.DayToDuskHour
          ? slc.NightHour - slc.DayToDuskHour
          : WorldLightCycle.NumHours - slc.DayToDuskHour + slc.NightHour;
  float intervalDayDusk =
      slc.DuskToNightHour >= slc.DayToDuskHour
          ? slc.DuskToNightHour - slc.DayToDuskHour
          : WorldLightCycle.NumHours - slc.DayToDuskHour + slc.DuskToNightHour;

  if (intervalDayDusk != 0.f)
    dest.DuskRatio = intervalDayDusk / intervalDayNight;
  else
    dest.DuskRatio = 0.5f;
}
...
void loadWeatherFunctionParams() {
  ...
  if (ClientCfg.ManualWeatherSetup) {
    WeatherFunctionParams->CWeatherFunctionParamsSheetBase::build(
        "ryzom.weather_function_params");
  } else {
    ...
    if (result.size() != 1) {
      nlwarning(
          "Multiple .weather_function_params sheets available, taking first");
    }
...
void updateWeatherManager(CContinent *continent) {
  ...
  // NB nico : even when light hour is forced, we use the real time to compute
  // the weather (so that the weather will reflect the real one, even if hour &
  // light is fixed)
  if (ClientCfg.ManualWeatherSetup && !ForceTrueWeatherValue) {
    WeatherManager.manualUpdate(RT.getRyzomDay(), (float)RT.getRyzomTime(), wc,
                                ManualWeatherValue, CurrSeason);
  } else {
...
void updateWeatherManager(const NLMISC::CMatrix &camMatrix,
  ...
  if (continent) {
    if (ClientCfg.ManualWeatherSetup && !ForceTrueWeatherValue) {
      WeatherManager.manualUpdate(RT.getRyzomDay(), (float)RT.getRyzomTime(),
                                  wc, ManualWeatherValue, CurrSeason, camMatrix,
                                  *continent);
    } else {
      // NB nico : even when light hour is forced, we use the real time to
      // compute the weather (so that the weather will reflect the real one,
      // even if hour & light is fixed)
      WeatherManager.update(RT.getRyzomDay(), RT.getRyzomTime(), wc, camMatrix,
                            *continent);
    }
  }
}
...
float LocalServerWeather =
    0.f; // 'local' server driven weather, trying to reach the weather value
         // given in the database as time passes (for smooth transition)
float ServerWeatherBlendFactor =
    0.f; // blend factor between server driven weather & local computed pseudo
         // random weather (based on the current date, gives same result for all
         // clients)
bool ServerDrivenWeather = false;

const float WEATHER_BLEND_SPEED =
    1.f / 8.f; // number of seconds to blend betwen weather states

static NLMISC::CRefPtr<NLMISC::CCDBNodeLeaf> s_ServerWeatherValueDB;

// ***************************************************************************
static uint16 getServerWeather() {
  CCDBNodeLeaf *node = s_ServerWeatherValueDB
...
void updateDBDrivenWeatherValue() {
  ...
  if (dbWeather == 0) {
    ...
    NLMISC::incrementalBlend(ServerWeatherBlendFactor, 0.f,
                             WEATHER_BLEND_SPEED * DT);
  } else {
    ...
    NLMISC::incrementalBlend(ServerWeatherBlendFactor, 1.f,
                             WEATHER_BLEND_SPEED * DT);

    // slowly blend between server driven weathers as values in the db changes
    NLMISC::incrementalBlend(LocalServerWeather, targetWeather,
                             WEATHER_BLEND_SPEED * DT);
  }
}

// ***************************************************************************
float getBlendedWeather(uint64 day, float hour,
                        const CWeatherFunctionParamsSheetBase &wfp,
                        const CWeatherFunction wf[EGSPD::CSeason::Invalid]) {
  return NLMISC::blend(CPredictWeather::predictWeather(day, hour, wfp, wf),
                       LocalServerWeather, ServerWeatherBlendFactor);
}

uint8 getSeasonDBValue() {
  CCDBNodeLeaf *serverSeason =
...
EGSPD::CSeason::TSeason computeCurrSeason() {
  ...
  // This mean that when the season change due to time, it won't be updated on
  // the client This is a deliberate choice to avoid possibly disadvantaging
  // freeze on the client while in combat To have this done correctly, the
  // server should be aware of the change, and freeze action accordingly. When
  // entering a new area, the 'StartupSeason' variable is updated however,
  // because the player is safe at that time. (see Continent::select)
  ...
  if (!ClientCfg.R2EDEnabled || !ClientCfg.Local) // ... Standard client
  {
    ...
    if (ServerSeasonValue != 0) {
      ...
      LastSeason =
          ClientCfg.ManualWeatherSetup ? ManualSeasonValue : StartupSeason;
    }
    return LastSeason;
  }

  // When we change act( different location) the Server TP the player at the
  // good location So the manual setting of season is useless
  // (R2::getEditor().getSeason()) because there is a huge risk to (Change to 2
  // times season) eg Act1 Season1, Island1, Act2 Season2, Island2 If server tp
  // occurs before, client received change of Season: there will be a unecessary
  // TP

  // When in edition mode, the editor tell what is the season (it is local and
  // given by each act of the current scenario, so ignore server value here...)
  if (R2::getEditor().getMode() == R2::CEditor::EditionMode) {
    OldR2EDMode = R2::getEditor().getMode();
    switch (R2::getEditor().getSeason()) {
    case R2::CEditor::Automatic:
      LastSeason = StartupSeason;
      break;
    case R2::CEditor::Spring:
      LastSeason = EGSPD::CSeason::Spring;
      break;
    case R2::CEditor::Summer:
      LastSeason = EGSPD::CSeason::Summer;
      break;
    case R2::CEditor::Autumn:
      LastSeason = EGSPD::CSeason::Autumn;
      break;
    case R2::CEditor::Winter:
      LastSeason = EGSPD::CSeason::Winter;
      break;
  ...
  if (OldR2EDMode == R2::CEditor::EditionMode) {
    ...
    // At some point the server will impulse the current season, be it the same
    // than the current one Until this occur, we just retain the last season
    // that was set in the editor, because season change are VERY costly
    // (several seconds freeze) and we don't want the season to change twice in
    // a row just because we don't know its real value at this point.
  ...
  // At this point we ignore the 'ClientCfg.ManualWeatherSetup' flag, however,
  // because it may be set when the "change weather" dialog is shown, but we
  // only want to preview weather change, not the season change.
  if (ServerSeasonValue != 0) {
    // server driven season
    LastSeason = (EGSPD::CSeason::TSeason)((ServerSeasonValue - 1) & 3);
  } else {
