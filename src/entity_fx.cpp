 ...
 * Constructor
 */
CEntityFx::CEntityFx(const std::string &FxName, const std::string &InstanceName,
...
CEntityFx::~CEntityFx() { deleteInstance(); }
...
void newFx(const string &FxName, const string &InstanceName,
           const CVector &Position, const CQuat &Rotation) {
  if (EntityFx.find(FxName) != EntityFx.end()) {
    nlerror("Fx %s already exist", FxName.c_str());
  } else {
    CEntityFx *pEntityFx =
        new CEntityFx(FxName, InstanceName, Position, Rotation);
...
void manageFxEntities(void) {
  ...
  for (TMapEntityFx::iterator it = EntityFx.begin(); it != EntityFx.end();
       ++it) {
