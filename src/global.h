...
// ***************************************************************************
// Data
extern NLWEB::CHttpPackageProvider
    *HttpPackageProvider; // Http provider from on-the-fly downloaded game data
...
extern bool
    StereoDisplayAttached;       // Is stereo display handling the display mode
extern CSoundManager *SoundMngr; // the sound manager
extern NL3D::UMaterial GenericMat; // Generic Material
extern NL3D::UTextContext
    *TextContext; // Context for all the text in the client.
...
// Main scene matrices.
extern NLMISC::CMatrix
    MainSceneViewMatrix; // Matrix to transform from world space to camera space
extern NLMISC::CMatrix InvMainSceneViewMatrix; // Matrix to transform from
...
// VR
extern std::vector<std::pair<std::string, std::string>> VRDeviceCache;
