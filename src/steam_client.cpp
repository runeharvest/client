...
typedef void(__cdecl *SteamAPI_RegisterCallbackFuncPtr)(
    class CCallbackBase *pCallback, int iCallback);
typedef void(__cdecl *SteamAPI_UnregisterCallbackFuncPtr)(
...
// taken from steam_api.h, we needed to change it to use our dynamically loaded
// functions
...
// and can be ignored.
#define NL_STEAM_CALLBACK(thisclass, func,                                     \
                          ... /*callback_type, [deprecated] var*/)             \
  _NL_STEAM_CALLBACK_SELECT((__VA_ARGS__, 4, 3),                               \
...
#define _NL_STEAM_CALLBACK_HELPER(_1, _2, SELECTED, ...)                       \
  _NL_STEAM_CALLBACK_##SELECTED
...
#define _NL_STEAM_CALLBACK_3(extra_code, thisclass, func, param)               \
  struct CCallbackInternal_##func                                              \
      : private CSteamCallbackImpl<sizeof(param)> {                            \
    CCallbackInternal_##func() {                                               \
      extra_code nlSteamAPI_RegisterCallback(this, param::k_iCallback);        \
    }                                                                          \
  ...
  private:                                                                     \
    virtual void Run(void *pvParam) {                                          \
      _NL_STEAM_CALLBACK_AUTO_HOOK(thisclass, func, param)                     \
...
#define _NL_STEAM_CALLBACK_4(_, thisclass, func, param, var)                   \
  CSteamCallback<thisclass, param> var;                                        \
  void func(param *pParam)

//-----------------------------------------------------------------------------
// Purpose: templated base for callbacks - internal implementation detail
//-----------------------------------------------------------------------------
template <int sizeof_P> class CSteamCallbackImpl : protected CCallbackBase {
public:
  ~CSteamCallbackImpl() {
...
//			bGameserver = listen for gameserver callbacks instead of
//client callbacks
//-----------------------------------------------------------------------------
...
public:
  CAuthSessionTicketListener()
      : _AuthSessionTicketResponse(
            this, &CAuthSessionTicketListener::OnAuthSessionTicketResponse) {
  ...
  // wait until a ticket is available or return if no ticket received after
  // specified ms
  bool waitTicket(uint32 ms) {
    // call Steam method
    _AuthSessionTicketHandle = s_SteamUser->GetAuthSessionTicket(
        _AuthSessionTicketData, sizeof(_AuthSessionTicketData),
        &_AuthSessionTicketSize);

    nldebug("GetAuthSessionTicket returned %u bytes, handle %u",
            _AuthSessionTicketSize, _AuthSessionTicketHandle);
    ...
    while (!_AuthSessionTicketCallbackCalled &&
  ...
  std::string getTicket() const {
    ...
    if (!_AuthSessionTicketCallbackCalled || _AuthSessionTicketCallbackError ||
        _AuthSessionTicketCallbackTimeout)
      return "";
...
private:
  ...
  NL_STEAM_CALLBACK(CAuthSessionTicketListener, OnAuthSessionTicketResponse,
                    GetAuthSessionTicketResponse_t, _AuthSessionTicketResponse);
};

// method called by Steam
void CAuthSessionTicketListener::OnAuthSessionTicketResponse(
...
CSteamClient::~CSteamClient() { release(); }
...
bool CSteamClient::init() {
  ...
  s_SteamClient = (ISteamClient *)nlSteamInternal_CreateInterface(
      STEAMCLIENT_INTERFACE_VERSION);
  if (!s_SteamClient)
    return false;

  s_SteamUser = s_SteamClient->GetISteamUser(hSteamUser, hSteamPipe,
                                             STEAMUSER_INTERFACE_VERSION);
  if (!s_SteamUser)
    return false;

  s_SteamApps = s_SteamClient->GetISteamApps(hSteamUser, hSteamPipe,
                                             STEAMAPPS_INTERFACE_VERSION);
  if (!s_SteamApps)
    return false;

  s_SteamFriends = s_SteamClient->GetISteamFriends(
      hSteamUser, hSteamPipe, STEAMFRIENDS_INTERFACE_VERSION);
  if (!s_SteamFriends)
    return false;

  s_SteamUtils =
      s_SteamClient->GetISteamUtils(hSteamPipe, STEAMUTILS_INTERFACE_VERSION);
  if (!s_SteamUtils)
    return false;
