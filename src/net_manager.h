...
public:
   ...
   */
  void pushPickup(CLFECOMMON::TCLEntityId slot,
...
#define ALL_MANAGERS                                                           \
  std::vector<CNetManager *>::iterator inm;                                    \
  for (inm = _NetManagers.begin(); inm != _NetManagers.end(); ++inm)           \
  (*inm)
...
public:
  ...
  void pushPickup(CLFECOMMON::TCLEntityId slot,
                  LHSTATE::TLHState lootOrHarvest) {
    ALL_MANAGERS->pushPickup(slot, lootOrHarvest);
  }
  ...
  sint64 getCurrentSmoothServerTick() const {
    return FIRST_MANAGER->getCurrentSmoothServerTick();
  }
  // Convert a GameCycle to a SmoothServerTick
  sint32 convertToSmoothServerTick(NLMISC::TGameCycle t) const {
    return (sint32)(FIRST_MANAGER->convertToSmoothServerTick(t));
  }
  // @}
  ...
  const NLNET::CInetAddress &getAddress() {
    return FIRST_MANAGER->getAddress();
  }

  /// Set player's reference position
  void setReferencePosition(const NLMISC::CVectorD &position) {
    ALL_MANAGERS->setReferencePosition(position);
  }

  const std::vector<CNetworkConnection::CChange> &getChanges() {
    return FIRST_MANAGER->getChanges();
  }

  // void
  // clearChanges() { ALL_MANAGERS->clearChanges(); }

  void setImpulseCallback(CNetworkConnection::TImpulseCallback callback,
                          void *argument = NULL) {
    FIRST_MANAGER->setImpulseCallback(callback, argument);
  }
  void setDataBase(NLMISC::CCDBNodeBranch *database) {
    ALL_MANAGERS->setDataBase(database);
  }
  bool connect(std::string &result) {
    bool res = false;
    std::vector<CNetManager *>::iterator inm;
    for (inm = _NetManagers.begin(); inm != _NetManagers.end(); ++inm)
      if ((*inm)->connect(result))
        res = true;
    return res;
  }
  CNetworkConnection::TConnectionState getConnectionState() const {
    return FIRST_MANAGER->getConnectionState();
  }
  ...
  const NLNET::CLoginCookie &getLoginCookie() const {
    return FIRST_MANAGER->getLoginCookie();
  }
