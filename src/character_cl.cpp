 ...
 *				PACS::move
 *
 *	The characters know where they want to be in 2d (pos().x and pos().y
 *valid).
 *
 *	PACS::evalCollision
 *
...
static const double AURA_SHUTDOWN_TIME =
    1.f; // number of seconds for auras and links to shutdown
...
#define CHECK(param)                                                           \
  if ((param) == false) {                                                      \
    ...
    nlwarning("entity:%d: Last Method called '%s'", _Slot,                     \
              LastMethod.c_str());                                             \
    nlstop;                                                                    \
  }
...
#else
#define ADD_METHOD(header) header {
#define CHECK(param)
...
// Constructor.
//-----------------------------------------------
CCharacterCL::CCharacterCL() : CEntityCL() {
  Type = NPC;

  _FirstPos = INVALID_POS;   // Initialize the first with a bad position.
  _FirstTime = INVALID_TIME; // Initialize the time for the first position with
                             // a bad one.
  dist2FirstPos(INVALID_DIST); // Initialize the distance to the first position
                               // with a bad value.
  ...
  // Initialize the time for the last loop with the current time when entity
  // created.
  _LastFrameTime = 0.0;
  ...
  _NameBoneId = -1;
  // No UTransform for the name needed if there is no name so not allocated for
  // the time.
  _NameTransform = 0;
...
void CCharacterCL::computePrimitive() {
  ...
  if (_Sheet) {
    initPrimitive(_Sheet->ColRadius * getScale(),
                  _Sheet->ColHeight * getScale(), _Sheet->ColLength,
                  _Sheet->ColWidth, UMovePrimitive::DoNothing,
                  UMovePrimitive::NotATrigger, MaskColNpc, MaskColNone,
                  _Sheet->ClipRadius, _Sheet->ClipHeight);
  } else {
    initPrimitive(0.5f, 2.0f, 0.0f, 0.0f, UMovePrimitive::DoNothing,
                  UMovePrimitive::NotATrigger, MaskColNpc, MaskColNone);
  }
  if (_Primitive)
...
void CCharacterCL::stopAttachedFXForCurrrentAnim(bool stopLoopingFX) {
  ...
  std::list<CAttachedFX::TSmartPtr>::iterator itAttachedFx =
      _AttachedFXListForCurrentAnim.begin();
  while (itAttachedFx != _AttachedFXListForCurrentAnim.end()) {
    std::list<CAttachedFX::TSmartPtr>::iterator tmpItAttached = itAttachedFx;
    ++itAttachedFx;
    if (!(!stopLoopingFX && (*tmpItAttached)->AniFX &&
          (*tmpItAttached)->AniFX->Sheet->RepeatMode ==
              CAnimationFXSheet::Loop)) // dont remove looping fx if it is
                                        // requested
    {
      ...
      if ((*tmpItAttached)->AniFX &&
        ...
        if (!(*tmpItAttached)->FX.empty()) {
          if (!(*tmpItAttached)->FX.removeByID(NELID("STOP")) &&
              !(*tmpItAttached)->FX.removeByID(NELID("main"))) {
            (*tmpItAttached)->FX.activateEmitters(false);
          }
        }
      }
      _AttachedFXListToRemove.splice(_AttachedFXListToRemove.begin(),
                                     _AttachedFXListForCurrentAnim,
                                     tmpItAttached);
      _AttachedFXListToRemove.front()->TimeOutDate +=
          TimeInSec; // compute absolute timeout date
    }
  }
  ...
  while (itAttachedFx != _AttachedFXListToRemove.end()) {
    ...
    if ((*tmpItAttachedFX)->AniFX) {
      if ((*tmpItAttachedFX)->MaxAnimCount !=
          0) // if there a limit to the number of animation during which the fx
             // can live ?
      {
...
// applyColorSlot :
//-----------------------------------------------
void CCharacterCL::applyColorSlot(SInstanceCL &instance, sint skin,
                                  sint userColor, sint hair, sint eyes) {
  ...
  array[2].second = (uint)hair;

  // Eyes Color
  array[3].first = (uint)3;
  array[3].second = (uint)eyes;
...
// changeColors :
//-----------------------------------------------
void CCharacterCL::changeColors(sint userColor, sint hair, sint eyes,
                                sint part) // virtual
{
...
// addColoredInstance :
//-----------------------------------------------
uint32 CCharacterCL::addColoredInstance(const std::string &shapeName,
                                        const std::string &stickPoint,
                                        sint texture, uint32 instIdx,
                                        sint color) {
  // Get the instance
  ...
  if (instance)
    ...
    nlwarning("CH::addColoredInstance: cannot create the instance for the "
              "shape '%s'.",
              shapeName.c_str());
...
//-----------------------------------------------
uint32 CCharacterCL::buildEquipment(const CCharacterSheet::CEquipment &slot,
                                    SLOTTYPE::EVisualSlot visualSlot,
                                    sint color, uint32 instIdx) {
  ...
  // This is a reference on an item (the file is store with an UPPER case so
  // check in UPPER CASE).
    ...
    if (itemId.buildSheetId(NLMISC::toLowerAscii(slot.getItem()))) {
      ...
      if (entitySheet) {
        ...
        if (_Items[visualSlot].Sheet) {
          ...
          idx = createItemInstance(itemSheet, instIdx, visualSlot, bindBone,
                                   slot.Texture, slotColor);
        } else
          nlwarning("CH::buildEquipment: the sheet '%s' is not an item one.",
                    slot.getItem().c_str());
      } else
        nlwarning(
            "CH::buildEquipment: the sheet '%s' is not stored in the database.",
            slot.getItem().c_str());
    } else
      nlwarning("CH::buildEquipment: item '%s' is not in the Sheet Id list.",
                slot.getItem().c_str());
  }
  // This is a shape.
  else {
    ...
    // Get the instance
    idx = addColoredInstance(slot.getItem(), slot.getBindPoint(), slot.Texture,
                             instIdx, slotColor);
  }
...
bool CCharacterCL::build(const CEntitySheet *sheet) // virtual
{
  ...
  if (Type == Fauna) {
    ...
    const char *creatureName =
        STRING_MANAGER::CStringManagerClient::getCreatureLocalizedName(
            _Sheet->Id);
  ...
  if (IngameDbMngr.getNodePtr()) {
    CCDBNodeBranch *nodeRoot =
        dynamic_cast<CCDBNodeBranch *>(IngameDbMngr.getNodePtr()->getNode(0));
    if (nodeRoot) {
  ...
  // Set the skeleton.
  if (!ClientCfg.Light && !_Sheet->getSkelFilename().empty() &&
      skeleton(_Sheet->getSkelFilename())) {
    ...
    if (isNPC() == false) {
      ...
      if (!_Sheet->Head.getItem().empty()) {
        // Create the Helm.
        _HeadIdx =
            buildEquipment(_Sheet->Head, SLOTTYPE::HEAD_SLOT, -1, _HeadIdx);
        // Hide the face
      ...
      // Display the Hair.
        ...
        if (_HairIndex != _BadHairIndex)
          _HeadIdx = buildEquipment(_Sheet->HairItemList[_HairIndex],
                                    SLOTTYPE::HEAD_SLOT, -1, _HeadIdx);
        // Display the face.
        SInstanceCL *pInstFace = getFace();
        if (pInstFace)
          if (!pInstFace->Current.empty())
      ...
      // Objects in Hands
      _RHandInstIdx =
          buildEquipment(_Sheet->ObjectInRightHand, SLOTTYPE::RIGHT_HAND_SLOT,
                         -1, _RHandInstIdx); // In The Right Hand
      _LHandInstIdx =
          buildEquipment(_Sheet->ObjectInLeftHand, SLOTTYPE::LEFT_HAND_SLOT, -1,
                         _LHandInstIdx); // In The Left Hand

      // Look is now ready.
      _LookRdy = true;
    }
    // Cannot build as long as the alternative look property not received, so
    // hide the entity.
    else
      skeleton()->hide();

    // Compute the animation set (after weapons are set to choose the right
    // animation set).
    computeAnimSet();
  ...
  initPrimitive(_Sheet->ColRadius * getScale(), _Sheet->ColHeight * getScale(),
                _Sheet->ColLength, _Sheet->ColWidth, UMovePrimitive::DoNothing,
                UMovePrimitive::NotATrigger, MaskColNpc, MaskColNone,
                _Sheet->ClipRadius, _Sheet->ClipHeight);
...
//-----------------------------------------------
CCharacterCL::TAtkHeight
CCharacterCL::getAttackHeight(CEntityCL *target, BODY::TBodyPart localisation,
                              BODY::TSide side) const {
...
// getBoneHeight :
//-----------------------------------------------
bool CCharacterCL::getBoneHeight(BODY::TBodyPart localisation, BODY::TSide side,
...
bool CCharacterCL::modeWithHiddenItems() const {
  return ((ClientCfg.PutBackItems && !isFighting()) || isSit() ||
          _Mode == MBEHAV::SWIM || isRiding() || _Mode == MBEHAV::SWIM_DEATH ||
          _Mode == MBEHAV::REST);
} // lookAtItemsInHands //
...
void CCharacterCL::computeAutomaton() {
  _CurrentAutomaton = automatonType() + "_" +
                      NLMISC::toLowerAscii(MBEHAV::modeToString(_Mode)) +
                      ".automaton";
} // computeAutomaton //
...
void CCharacterCL::computeAnimSet() {
  ...
  if (!::computeAnimSet(
          _CurrentAnimSet[MOVE], _Mode, _Sheet->getAnimSetBaseName(),
          _Items[SLOTTYPE::LEFT_HAND_SLOT].Sheet,
          _Items[SLOTTYPE::RIGHT_HAND_SLOT].Sheet, !modeWithHiddenItems())) {
    nlwarning("CH:computeAnimSet:%d: pb when trying to compute the animset. "
              "Sheet Id '%u(%s)'.",
              _Slot, _SheetId.asInt(), _SheetId.toString().c_str());
  }
...
//-----------------------------------------------
NLMISC::TGameCycle CCharacterCL::adjustPI(float x, float y, float /* z */,
                                          const NLMISC::TGameCycle &pI) {
  NLMISC::TGameCycle adjustedPI = pI;
  if (ClientCfg.RestrainPI && adjustedPI > 0) {
    double dist = (x - UserEntity->pos().x) * (x - UserEntity->pos().x) +
    ...
    if (dist < 50 * 50) {
      ...
      if (adjustedPI > maxPi) {
        ...
        if (VerboseAnimSelection && _Slot == UserEntity->selection())
          nlinfo("CH:updtVPPos:%d: dist'%f' PI'%d' newPI'%d'.", _Slot,
                 sqrt(dist), pI, adjustedPI);
      }
    }
  }
  return adjustedPI;
...
//-----------------------------------------------
void CCharacterCL::updateVisualPropertyPos(const NLMISC::TGameCycle &gameCycle,
                                           const sint64 &prop,
                                           const NLMISC::TGameCycle &pI) {
  ...
  if (_DBEntry == 0)
    return;
  // Get The property 'Y'.
  CCDBNodeLeaf *nodeY = dynamic_cast<CCDBNodeLeaf *>(
      _DBEntry->getNode(CLFECOMMON::PROPERTY_POSY));
  if (nodeY == 0) {
    nlwarning("CH::updtVPPos:%d: Cannot find the property 'PROPERTY_POSY(%d)'.",
              _Slot, CLFECOMMON::PROPERTY_POSY);
    return;
  }
  // Get The property 'Z'.
  CCDBNodeLeaf *nodeZ = dynamic_cast<CCDBNodeLeaf *>(
      _DBEntry->getNode(CLFECOMMON::PROPERTY_POSZ));
  if (nodeZ == 0) {
    nlwarning("CH::updtVPPos:%d: Cannot find the property 'PROPERTY_POSZ(%d)'.",
              _Slot, CLFECOMMON::PROPERTY_POSZ);
    return;
  }
  ...
  // Stock the position (except if this is the user mount because it's the user
  // that control him not the server)
...
//-----------------------------------------------
void CCharacterCL::updateVisualPropertyOrient(
    const NLMISC::TGameCycle &gameCycle, const sint64 &prop) {
#ifdef TMP_DEBUG_GUIGUI
  ...
  if (verboseVP(this)) {
    ...
    nlinfo("(%05d,%03d) CH::updateVPOri:%d: '%f' received.",
           sint32(T1 % 100000), NetMngr.getCurrentServerTick(), _Slot, ori);
  }

  // if no skeleton we set the orientation
  if (_Skeleton.empty()) {
    // server forces the entity orientation even if it cannot turn
    front(CVector((float)cos(_TheoreticalOrientation),
                  (float)sin(_TheoreticalOrientation), 0.f),
          true, true, true);
...
// New mode received.
// \warning For the first mode, we must have received the position and
// orientation (but this should be the case). \warning Read the position or
// orientation from the database when reading the mode (no more updated in
// updateVisualPropertyPos and updateVisualPropertyOrient).
//-----------------------------------------------
void CCharacterCL::updateVisualPropertyMode(const NLMISC::TGameCycle &gameCycle,
                                            const sint64 &prop) {
  if (verboseVP(this))
    nlinfo("(%05d,%03d) CH:updtVPMode:%d: '%s(%d)' received.",
           sint32(T1 % 100000), NetMngr.getCurrentServerTick(), _Slot,
           modeToString((MBEHAV::EMode)prop).c_str(), (MBEHAV::EMode)prop);
  ...
  if (_TheoreticalMode != (MBEHAV::EMode)(prop & 0xffff))
    ...
    nlwarning("CH:updtVPMode:%d: The mode '%s(%d)' sent is the same as the "
              "current one.",
              _Slot, modeToString(_TheoreticalMode).c_str(), _TheoreticalMode);
    return;
  }
  ...
  if (_Mode == MBEHAV::UNKNOWN_MODE) {
    ...
    CCDBNodeLeaf *nodeX = dynamic_cast<CCDBNodeLeaf *>(
        _DBEntry->getNode(CLFECOMMON::PROPERTY_POSX));
    if (nodeX == 0) {
      nlwarning(
          "CH::updtVPMode:%d: Cannot find the property 'PROPERTY_POSX(%d)'.",
          _Slot, CLFECOMMON::PROPERTY_POSX);
      return;
    }
    ...
    if (nodeY == 0) {
      nlwarning(
          "CH::updtVPMode:%d: Cannot find the property 'PROPERTY_POSY(%d)'.",
          _Slot, CLFECOMMON::PROPERTY_POSY);
      return;
    }
    ...
    if (nodeZ == 0) {
      nlwarning(
          "CH::updtVPMode:%d: Cannot find the property 'PROPERTY_POSZ(%d)'.",
          _Slot, CLFECOMMON::PROPERTY_POSZ);
      return;
    }
    ...
    CCDBNodeLeaf *nodeOri = dynamic_cast<CCDBNodeLeaf *>(
        _DBEntry->getNode(CLFECOMMON::PROPERTY_ORIENTATION));
    if (nodeOri == 0) {
      nlwarning("CH::updtVPMode:%d: Cannot find the property "
                "'PROPERTY_ORIENTATION(%d)'.",
                _Slot, CLFECOMMON::PROPERTY_ORIENTATION);
      return;
    }
    C64BitsParts parts;
    ...
    // server forces the entity orientation even if it cannot turn
    front(CVector((float)cos(angleZ), (float)sin(angleZ), 0.f), true, true,
          true);
    dir(front(), false, false);
    _TargetAngle = angleZ;
    if (_Primitive)
    ...
    if ((_Mode == MBEHAV::MOUNT_NORMAL) &&
        (_Rider == CLFECOMMON::INVALID_SLOT)) {
      _Mode = MBEHAV::NORMAL;
      _ModeWanted = MBEHAV::MOUNT_NORMAL;
      // See also updateVisualPropertyRiderEntity() for the case when _Rider is
      // received after the mode
      computeAutomaton();
      computeAnimSet();
      setAnim(CAnimationStateSheet::Idle);
  ...
  // Not the first mode -> Add to a stage.
    ...
    if (_TheoreticalMode == MBEHAV::COMBAT_FLOAT) {
      ...
      _Stages.addStage(gameCycle, CLFECOMMON::PROPERTY_ORIENTATION,
                       nodeOri->getValue64());
    }
    // Any other mode push the position
      ...
      if (_TheoreticalMode != MBEHAV::MOUNT_NORMAL) {
        ...
        // Add Stage.
        _Stages.addStage(gameCycle, CLFECOMMON::PROPERTY_POSX,
                         nodeX->getValue64());
        _Stages.addStage(gameCycle, CLFECOMMON::PROPERTY_POSY,
                         nodeY->getValue64());
        _Stages.addStage(gameCycle, CLFECOMMON::PROPERTY_POSZ,
                         nodeZ->getValue64());
      }
    }
  }
...
//-----------------------------------------------
void CCharacterCL::updateVisualPropertyBehaviour(
    const NLMISC::TGameCycle &gameCycle, const sint64 &prop) {
  // New Behaviour Received.
  CBehaviour beh(prop);
  if (verboseVP(this))
    nlinfo("(%05d,%03d) CH::updateVPBeha:%d: '%s(%d)' received.",
           sint32(T1 % 100000), NetMngr.getCurrentServerTick(), _Slot,
           behaviourToString((EBehaviour)beh.Behaviour).c_str(),
           (sint)beh.Behaviour);
...
void CCharacterCL::updateVisualPropertyTargetList(
    const NLMISC::TGameCycle &gameCycle, const sint64 &prop, uint listIndex) {
  // Add in right stage.
  _Stages.addStage(gameCycle, PROPERTY_TARGET_LIST_0 + listIndex, prop);
}

void CCharacterCL::updateVisualPropertyVisualFX(
    const NLMISC::TGameCycle &gameCycle, const sint64 &prop) {
...
//-----------------------------------------------
void CCharacterCL::updateVisualPropertyName(
    const NLMISC::TGameCycle & /* gameCycle */, const sint64 &prop) {
  ...
  //	STRING_MANAGER::CStringManagerClient::instance()->waitString(nameId,
  //this, &_Name);
  STRING_MANAGER::CStringManagerClient::instance()->waitString(nameId, this);

  // if(!getEntityName().empty())
  //	nlwarning("CH::updateVPName:%d: name Id '%d' received but no name
  //allocated.", _Slot, nameId); else if(verboseVP(this)) 	nlinfo("(%05d,%03d)
  //CH::updateVPName:%d: name '%s(%d)' received.", sint32(T1%100000),
  //NetMngr.getCurrentServerTick(), _Slot, getEntityName().toString().c_str(),
  //nameId);
...
//-----------------------------------------------
void CCharacterCL::updateVisualPropertyTarget(
    const NLMISC::TGameCycle &gameCycle, const sint64 &prop) {
  // New target Received.
  sint targ = (sint)prop;

  if (verboseVP(this))
    nlinfo("(%05d,%03d) CH::updateVPTarget:%d: '%d' received.",
           sint32(T1 % 100000), NetMngr.getCurrentServerTick(), _Slot, targ);
...
// \todo GUIGUI : should be added in a stage.
//-----------------------------------------------
void CCharacterCL::updateVisualPropertyVpa(
    const NLMISC::TGameCycle & /* gameCycle */, const sint64 &prop) {
  // VPA only useful for NPC
  if (isNPC() == false) {
    // nlwarning("CH:updtVPVpa:%d: VPA received but NOT an NPC. Sheet Id
    // '%u(%s)'.", _Slot, _SheetId.asInt(), _SheetId.toString().c_str());
    return;
  }
  ...
  if (verboseVP(this)) {
    nlinfo(
        "(%05d,%03d) CH:updtVPVpa:%d: TopColor(%d) BotColor(%d) RH(%d) LH(%d) "
        "Hat(%d) Seed(%d) HairColor(%d)",
        sint32(T1 % 100000), NetMngr.getCurrentServerTick(), _Slot,
        (uint)altLookProp.Element.ColorTop, (uint)altLookProp.Element.ColorBot,
        (uint)altLookProp.Element.WeaponRightHand,
        (uint)altLookProp.Element.WeaponLeftHand, (uint)altLookProp.Element.Hat,
        (uint)altLookProp.Element.Seed, (uint)altLookProp.Element.ColorHair);
  }
  ...
  if (!_LookRdy) {
    ...
    if (SheetMngr.nbHairColor() == 0)
      ...
      _HairColor =
          (sint8)altLookProp.Element.ColorHair % SheetMngr.nbHairColor();
    // Hair Index
     ...
     */
    buildEquipment(_ClothesSheet->Body, SLOTTYPE::CHEST_SLOT,
                   altLookProp.Element.ColorTop); // Chest
    buildEquipment(_ClothesSheet->Arms, SLOTTYPE::ARMS_SLOT,
                   altLookProp.Element.ColorArm); // Arms
    buildEquipment(_ClothesSheet->Hands, SLOTTYPE::HANDS_SLOT,
                   altLookProp.Element.ColorGlove); // Gloves
    buildEquipment(_ClothesSheet->Legs, SLOTTYPE::LEGS_SLOT,
                   altLookProp.Element.ColorBot); // Legs
    buildEquipment(_ClothesSheet->Feet, SLOTTYPE::FEET_SLOT,
                   altLookProp.Element.ColorBoot); // Boots
    // Face
    _FaceIdx = buildEquipment(_Sheet->Face, SLOTTYPE::FACE_SLOT);
  ...
  if (altLookProp.Element.Hat != 0 && !_ClothesSheet->Head.getItem().empty()) {
    // Create the Helm.
    _HeadIdx = buildEquipment(_ClothesSheet->Head, SLOTTYPE::HEAD_SLOT,
                              altLookProp.Element.ColorHair, _HeadIdx);
    // Hide the face.
    SInstanceCL *pInstFace = getFace();
    if (pInstFace) {
  ...
  // Display the Hair.
    ...
    if (_HairIndex != _BadHairIndex)
      _HeadIdx =
          buildEquipment(_Sheet->HairItemList[_HairIndex], SLOTTYPE::HEAD_SLOT,
                         altLookProp.Element.ColorHair, _HeadIdx);
    // Display the face.
    SInstanceCL *pInstFace = getFace();
    if (pInstFace)
  ...
  // Right Hand
  const CItemSheet *newRightHand = SheetMngr.getItem(
      SLOTTYPE::RIGHT_HAND_SLOT, (uint)altLookProp.Element.WeaponRightHand);
  if (newRightHand != _Items[SLOTTYPE::RIGHT_HAND_SLOT].Sheet) // item changed ?
  {
    ...
    if (newRightHand) {
      if (newRightHand->ItemType != ITEM_TYPE::MAGICIAN_STAFF)
        _RHandInstIdx =
            createItemInstance(*newRightHand, _RHandInstIdx,
                               SLOTTYPE::RIGHT_HAND_SLOT, "box_arme", -1, -1);
      else
        _RHandInstIdx =
            createItemInstance(*newRightHand, _RHandInstIdx,
                               SLOTTYPE::RIGHT_HAND_SLOT, "", -1, -1);
    }
  }
  ...
  if (newRightHand) {
    ...
    if (instCLRH) {
      NL3D::UInstance itemInstance =
          (!instCLRH->Loading.empty()) ? instCLRH->Loading : instCLRH->Current;
      if (!itemInstance.empty()) {
        ...
        if (_CurrentBehaviour.Behaviour != MBEHAV::EXTRACTING)
          _Items[SLOTTYPE::RIGHT_HAND_SLOT].setTrailSize(
              altLookProp.Element.RTrail);
      }
    }
  }

  // Left Hand
  const CItemSheet *newLeftHand = SheetMngr.getItem(
      SLOTTYPE::LEFT_HAND_SLOT, (uint)altLookProp.Element.WeaponLeftHand);
  if (newLeftHand != _Items[SLOTTYPE::LEFT_HAND_SLOT].Sheet) // item changed ?
  {
    ...
    if (newLeftHand) {
      ...
      if (newLeftHand->getAnimSet() == "s")
        ...
        bindBone = "box_arme_gauche";
      _LHandInstIdx =
          createItemInstance(*newLeftHand, _LHandInstIdx,
                             SLOTTYPE::LEFT_HAND_SLOT, bindBone, -1, -1);
    }
  }
  ...
  if (newLeftHand) {
    ...
    if (instCLLH) {
      NL3D::UInstance itemInstance =
          (!instCLLH->Loading.empty()) ? instCLLH->Loading : instCLLH->Current;
      if (!itemInstance.empty()) {
        // update fxs
        _Items[SLOTTYPE::LEFT_HAND_SLOT].enableAdvantageFX(itemInstance);
        _Items[SLOTTYPE::LEFT_HAND_SLOT].setTrailSize(
            (uint)(2 * altLookProp.Element.LTrail));
      }
    }
  }
...
// updateVisualPropertyVpb :
//-----------------------------------------------
void CCharacterCL::updateVisualPropertyVpb(
    const NLMISC::TGameCycle & /* gameCycle */, const sint64 &prop) {
  // Get the alternative look property.
  SAltLookProp2 altLookProp = *(SAltLookProp2 *)(&prop);
  // Display debug infos
  if (verboseVP(this)) {
    nlinfo("(%05d,%03d) CH:updtVPVpb:%d: Scale(%d)", sint32(T1 % 100000),
           NetMngr.getCurrentServerTick(), _Slot,
  ...
  if (_Primitive) {
    ...
    _Primitive->setSize((width / oldCustomScale) * _CustomScale,
                        (depth / oldCustomScale) * _CustomScale);
...
// Update Entity Mount
//-----------------------------------------------
void CCharacterCL::updateVisualPropertyEntityMounted(
    const NLMISC::TGameCycle &gameCycle, const sint64 &prop) {
  // New target Received.
  sint mountSlot = (sint)prop;
  _TheoreticalMount = (CLFECOMMON::TCLEntityId)mountSlot;
  if (verboseVP(this))
    nlinfo("(%05d,%03d) CH::updateVPMount:%d: '%d' received.",
           sint32(T1 % 100000), NetMngr.getCurrentServerTick(), _Slot,
           mountSlot);
...
// Update Entity Rider
//-----------------------------------------------
void CCharacterCL::updateVisualPropertyRiderEntity(
    const NLMISC::TGameCycle &gameCycle, const sint64 &prop) {
  // New target Received.
  sint riderSlot = (sint)prop;
  _TheoreticalRider = (CLFECOMMON::TCLEntityId)riderSlot;
  if (verboseVP(this))
    nlinfo("(%05d,%03d) CH::updateVPRider:%d: '%d' received.",
           sint32(T1 % 100000), NetMngr.getCurrentServerTick(), _Slot,
           riderSlot);
...
// Update Entity Bars
//-----------------------------------------------
void CCharacterCL::updateVisualPropertyBars(const NLMISC::TGameCycle &gameCycle,
                                            const sint64 &prop) // virtual
{
  ...
  /* ***********
    WHY gameCycle+1 ?????  (yoyo)
          It's because sometimes I have a bug With target DB update and VP
  update. This is the scenario where I suppose the problem rises: tick=320:
  EGS::tickUpdate(): player.DBTargetHP.setProp(49) tick=321:	EGS::combat
  update, target ennemy receives a Hit, VPHp=10 => transmitted to client with
  timestamp=321 EGS::databaseUpdate(), DB updated, with timestamp=321!!!

          Thus I receives on client:
                  first the VP with Hp=10, timestamp=321
                  second the DB with Hp=49, timestamp=321 too => replaced => BUG
          NB: DB is typically sent at low frequency by FrontEnd, thus received
  later on client.

          Since databaseUpdate() is called every 2 ticks, adding +1 to VP
  timestamps solve easily the problem.

          NB: the problem occurs because tickUpdate() and databaseUpdate() are
  called typically with 1 tick shift (tickUpdate() on even ticks, and
  databaseUpdate() on odd ticks for instance).

          NB: moreover, tickupdate() is called every 8 (or 16) ticks, and
  databaseUpdate() every 2 ticks. So there is one more possible bug: 318:
  EGS::tickUpdate(): player.DBTargetHP.setProp(49) 319:	EGS::combat update,
  target ennemy receives a Hit, VPHp=10 => transmitted to client with
  timestamp=319 EGS::databaseUpdate(), BUT decide to send only a small subset of
  DB (because lot of things to send)
                                          => our TargetHP is not updated
                  320:	nothing. tickupdate() is not called, since every 8 ticks
                  321:	EGS::databaseUpdate(), update TargetHP, with
  timestamp=321 !!!!! => Bug

          (remind that we cannot store a timestamp for each DB property, else
  would be too big to store and to send...)

          BTW, this last bug should be very rare, so don't care.
  *********** */
  pBM->updateBars(dataSetId(), barInfo, gameCycle + 1,
                  CBarManager::HpFlag | CBarManager::StaFlag |
                      CBarManager::SapFlag | CBarManager::FocusFlag);
...
//
//-----------------------------------------------
void CCharacterCL::updateVisualPropertyGuildSymbol(
    const NLMISC::TGameCycle & /* gameCycle */, const sint64 &prop) {
  _GuildSymbol = prop;
...
//
//-----------------------------------------------
void CCharacterCL::updateVisualPropertyGuildNameID(
    const NLMISC::TGameCycle & /* gameCycle */, const sint64 &prop) {
...
//
//-----------------------------------------------
void CCharacterCL::updateVisualPropertyEventFactionID(
    const NLMISC::TGameCycle & /* gameCycle */, const sint64 &prop) {
  // ODD Hack by Ulukyn
...
//
//-----------------------------------------------
void CCharacterCL::updateVisualPropertyPvpMode(
    const NLMISC::TGameCycle & /* gameCycle */, const sint64 &prop) {
  //_PvpMode = uint32(prop);
  // buildInSceneInterface();
...
//
//-----------------------------------------------
void CCharacterCL::updateVisualPropertyPvpClan(
    const NLMISC::TGameCycle & /* gameCycle */, const sint64 &prop) {
...
//-----------------------------------------------
void CCharacterCL::updateVisualPropertyStatus(
    const NLMISC::TGameCycle & /* gameCycle */,
    const sint64 & /* prop */) // virtual
{
...
// Update Entity Status
//-----------------------------------------------
void CCharacterCL::updateVisualPropertyContextual(
    const NLMISC::TGameCycle &gameCycle, const sint64 &prop) {
...
//
//-----------------------------------------------
void CCharacterCL::updateVisualPropertyOwnerPeople(
    const NLMISC::TGameCycle & /* gameCycle */, const sint64 &prop) {
  if (_OwnerPeople != MOUNT_PEOPLE::TMountPeople(prop)) {
    // reset scale pos
    float oldPeopleScaleFactor;
    switch (_OwnerPeople) {
    case MOUNT_PEOPLE::Fyros:
      oldPeopleScaleFactor = ClientCfg.FyrosScale;
      break;
    case MOUNT_PEOPLE::Matis:
      oldPeopleScaleFactor = ClientCfg.MatisScale;
      break;
    case MOUNT_PEOPLE::Tryker:
      oldPeopleScaleFactor = ClientCfg.TrykerScale;
      break;
    case MOUNT_PEOPLE::Zorai:
      oldPeopleScaleFactor = ClientCfg.ZoraiScale;
      break;
    default:
    ...
    switch (_OwnerPeople) {
    case MOUNT_PEOPLE::Fyros:
      newPeopleScaleFactor = ClientCfg.FyrosScale;
      break;
    case MOUNT_PEOPLE::Matis:
      newPeopleScaleFactor = ClientCfg.MatisScale;
      break;
    case MOUNT_PEOPLE::Tryker:
      newPeopleScaleFactor = ClientCfg.TrykerScale;
      break;
    case MOUNT_PEOPLE::Zorai:
      newPeopleScaleFactor = ClientCfg.ZoraiScale;
      break;
    default:
    ...
    if (skel) {
      ...
      if (boneID != -1) {
        UBone bone = skel->getBone(boneID);
        CVector newBoneScale =
            bone.getScale() * oldPeopleScaleFactor / newPeopleScaleFactor;
        bone.setScale(newBoneScale);
      }
    }
  }
...
//
//-----------------------------------------------
void CCharacterCL::updateVisualPropertyOutpostInfos(
    const NLMISC::TGameCycle & /* gameCycle */, const sint64 &prop) {
  _OutpostId = ((uint16)prop) & 0x7FFF;
  uint16 side = (((uint16)prop) & 0x8000) >> 15;
  _OutpostSide = (OUTPOSTENUMS::TPVPSide)side;

  nldebug("<CCharacterCL::updateVisualPropertyOutpostInfos> prop = %d, id=%d "
          "side=%d",
          (uint16)prop, _OutpostId, _OutpostSide);
...
double CCharacterCL::computeTimeStep(const double &currentTime) {
  ...
  if (_LastFrameTime > currentTime) {
    nlwarning("CCharacterCL::computeTimeStep : Time since last frame is "
              "negative (%f). Set _LastFrameTime with currentTime",
...
//-----------------------------------------------
// computeSpeed :
// \todo GUIGUI : to do an average speed, there is a problem if time become very
// small because small frame or _LastFrameTime increase when looping
//-----------------------------------------------
...
double CCharacterCL::computeSpeedFactor(double speedToDest) {
      ...
      if (_CurrentState) {
        ...
        if (_CurrentState->Move) {
          ...
          if (speedToDest != -1) {
            ...
            double animSpeed = EAM->getAnimationAverageSpeed(anim->id()) *
                               getSheetScale() * _CustomScalePos *
                               _CharacterScalePos;
            if (animSpeed > 0.0)
              speedFactor = speedToDest / animSpeed;
            else
              nlwarning("The animation is a move but animation speed is %f !",
                        animSpeed);
          }
        ...
        // The current animation is a rotation so adjust the rotation speed
        ...
        // This animation must be play in a maximum time when there is someting
        // to do after. (must be done after the rotation speed adjustment)
        if (_CurrentState->MaxAnimDuration > 0.00f) {
          if (dist2Dest() >= 0.0) {
            ...
            // If the animation speed should have been greater, let it play
            // faster.
        ...
        if (!_CurrentState->Move && _ImportantStepTime != 0.0) {
          const float beginPanic =
              1.5f; // start panic when too late of 1.5 seconds
          const float endPanic =
              5.f; // max panic factor at 5 seconds of interval
        ...
        // But still allow acceleration for mode animation transition
        // (_Mode!=_ModeWanted)
        if (!_CurrentState->Move && _RunStartTimeNoPop != INVALID_TIME &&
          ...
          float remainAnimTime =
              float(EAM->getAnimationLength(anim->id()) - animOffset(MOVE));
          ...
          // else target the animation speed so it ends at estimated start of
          // move
          else {
        ...
        if(_Slot==WatchedEntitySlot && EntitiesMngr.isLogingStageChange())
        {
                ...
                NLMISC::createDebug();
                NLMISC::DebugLog->displayRawNL("** Entity %d: (t=%3d) animState
        %s: %.2f/%.2f. move: %d. rstnp: %d. sf: %.2f", (sint32)_Slot,
        sint32(T1-refLT),
                        CAnimationState::getAnimationStateName(animStatePtr->state()).c_str(),
                        float(animOffset(MOVE)),
        float(EAM->getAnimationLength(anim->id())), uint(_CurrentState->Move),
        _RunStartTimeNoPop==INVALID_TIME?-1:sint32(sint64(_RunStartTimeNoPop*1000)-refLT),
                        speedFactor
                        );
        }
        */
  ...
  if (speedFactor <= 0.0) {
    nlwarning("CCharacterCL::computeSpeedFactor: speedFactor = %f and this "
              "should never be <= 0.",
              speedFactor);
    speedFactor = 1.0;
  }
...
void CCharacterCL::endAnimTransition() {
  ...
  // If the animation is a rotation, set the character direction he should have
  // at the end of the animation.
  if (_CurrentState->Rotation) {
    if (isUser()) {
      nldebug("<CCharacterCL::endAnimTransition> rotation : set dir as end "
              "anim dir");
    }
  ...
  if (isUser()) {
    if (_CurrentBehaviour.Behaviour == MBEHAV::RANGE_ATTACK &&
        _Mode == MBEHAV::COMBAT && !UserControls.isMoving()) {
      dir(dirToTarget());
  ...
  if (_CurrentState->NextMode != _Mode) {
    ...
    case MBEHAV::COMBAT_FLOAT:
      if (ClientCfg.UsePACSForAll && _Primitive)
        _Primitive->setCollisionMask(
            MaskColPlayer | MaskColNpc |
            MaskColDoor); // Collision with player and npc.
      break;
    // Mount
    ...
    // else
    //	nlinfo( "NO mode change from %u to %u, %s S%hu", _Mode,
    //_CurrentState->NextMode, _SheetId.toString().c_str(), _Slot );
...
// onMove :
//-----------------------------------------------
CCharacterCL::TOnMove
CCharacterCL::onMove(const CAutomatonStateSheet &curAnimState) {
  // Animation is breakable if the distance to destination is long enough (at
  // least > 0).
  if (curAnimState.BreakableOnMove && dist2Dest() > 0.0) {
    // \todo GUIGUI : take the next position to current one (it could be
    // possible this position was the same as the first).
    ...
    if (dirToFirstPos != CVectorD::Null)
      ...
      nlwarning(
          "CH:onMove:%d: First pos == pos -> take the dir(%f,%f,%f) instead.",
          _Slot, dir().x, dir().y, dir().z);
      dirToFirstPos = dir();
    }

    // Compute Angle between the front and the first position.
    double angToDest = computeShortestAngle(
        atan2(front().y, front().x), atan2(dirToFirstPos.y, dirToFirstPos.x));
    if (!_MaxLoop) {
      // Strafe Left
    ...
    if (curAnimState.OnMoveForward != CAnimationStateSheet::UnknownState) {
      ...
      nlwarning("CH:onMove:%d: the current state is breakable on Move and the "
                "dist to dest is not Null, but there is no animation to move.",
                _Slot);
  }
  // No Move
  return OnMoveNone;
...
//-----------------------------------------------
CCharacterCL::TOnRot
CCharacterCL::onRotation(const CAutomatonStateSheet &curAnimState,
                         CVector &dirEndA) {
  // Turn and About Face
  if (curAnimState.BreakableOnRotation) {
    ...
    if (isUser() && _CurrentBehaviour.Behaviour == MBEHAV::RANGE_ATTACK &&
        _Mode == MBEHAV::COMBAT && !UserControls.isMoving()) {
      destDir = dirToTarget();
    }

    // Compute the angle between the current heading and computed heading.
    double angToDest = computeShortestAngle(atan2(bodyDir.y, bodyDir.x),
                                            atan2(destDir.y, destDir.x));
...
// onBigBend :
//-----------------------------------------------
CCharacterCL::TOnBigBend
CCharacterCL::onBigBend(const CAutomatonStateSheet &curAnimState,
                        CVector &dirEndA) {
  // If the current direction is too different of the direction to the first
  // destination -> play a rotation.
  if (curAnimState.BrkOnBigBend) {
    ...
    if (setVect(dirToFirstPos, _FirstPos - pos(), true, false)) {
      ...
      double angToDest = computeShortestAngle(
          atan2(dir().y, dir().x), atan2(dirToFirstPos.y, dirToFirstPos.x));
      // Rotation to the left.
      if (angToDest >= 1.5) {
  ...
  if (curAnimState.BreakableOnBadHeading && !_MaxLoop) {
    ...
    if (setVect(dirToFirstPos, _FirstPos - pos(), true, false)) {
      ...
      if (curAnimState.BadHeadingMin <= curAnimState.BadHeadingMax) {
        if ((angToDest < curAnimState.BadHeadingMin) ||
            (angToDest > curAnimState.BadHeadingMax))
          return true;
      } else {
        if ((angToDest < curAnimState.BadHeadingMin) &&
            (angToDest > curAnimState.BadHeadingMax))
...
//-----------------------------------------------
ADD_METHOD(void CCharacterCL::setAnim(TAnimStateKey newKey,
                                      TAnimStateKey subKey, uint animID))
if (ClientCfg.Light)
  return;

// Debug Animation for the target
if (VerboseAnimSelection && _Slot == UserEntity->selection())
  nlinfo("CH:setAnim:%d: state '%s'.", _Slot,
         CAnimationState::getAnimationStateName(newKey).c_str());

// RELEASE THE OLD ANIMATION
TAnimStateId lastAnimStateId =
    animState(MOVE); // Bkup the current anim state Id.

// \todo GUIGUI : Faire une fonction pour savoir si on a changer de type
// d'animation. Reset loop counter
...
animState(
    MOVE,
    CAnimationStateSheet::UnknownState); // Reset the current animation state
animOffset(MOVE, 0.0);                   // Reset the current animation offset
animIndex(MOVE_BLEND_OUT,
          CAnimation::UnknownAnim); // Reset the current animation Index
animState(
    MOVE_BLEND_OUT,
    CAnimationStateSheet::UnknownState); // Reset the current animation state
...
_SubStateKey =
    CAnimationStateSheet::UnknownState; // No SubStateKey for the time.
dirEndAnim(dir()); // For the time the direction at the end of the animation is
...
if (!animationStateKey(MOVE, newKey)) {
  nlwarning("CH:setAnim:%d: automaton '%s': animation state key '%s' asked is "
            "not valid -> trying Idle.",
            _Slot, _CurrentAutomaton.c_str(),
            CAnimationState::getAnimationStateName(newKey).c_str());
...
// Compute the direction to the first position (direction = front if there is
// not first pos)
CVector dirToFirstPos;
...
KeyChosen :
    ...
    const CAutomatonStateSheet *state =
        EAM->mState(_CurrentAutomaton, animState(MOVE));
if (state == 0) {
  nlwarning(
      "CH:setAnim:%d: State '%s' not in the automaton '%s'.", _Slot,
      CAnimationStateSheet::getAnimationStateName(animState(MOVE)).c_str(),
      _CurrentAutomaton.c_str());
...
if (antiFreezeCounter > 10) {
  /*
                  nlwarning("CH:setAnim:anitFreeze:%d: Automaton '%s'", _Slot,
     _CurrentAutomaton.c_str()); nlwarning("CH:setAnim:anitFreeze:%d:
     _IsThereAMode '%s'",       _Slot, _IsThereAMode?"true":"false");
                  nlwarning("CH:setAnim:anitFreeze:%d: dist2Dest '%f'", _Slot,
     dist2Dest()); nlwarning("CH:setAnim:anitFreeze:%d: Mode '%s(%d)'", _Slot,
     modeToString(_Mode).c_str(), _Mode); nlwarning("CH:setAnim:anitFreeze:%d:
     Mode Wanted '%s(%d)'",     _Slot, modeToString(_ModeWanted).c_str(),
     _ModeWanted); nlwarning("CH:setAnim:anitFreeze:%d: Anim State Move
     '%s(%d)'", _Slot,
     CAnimationStateSheet::getAnimationStateName(animState(MOVE)).c_str(),
     animState(MOVE));
  */
...
//	if(_IsThereAMode && (dist2Dest()==INVALID_DIST))
{
  ...
  if ((_Mode != _ModeWanted) && (dist2Dest() == INVALID_DIST)) {
    ...
    if (curAnimState.getModeConnection(_ModeWanted, transition)) {
      ...
      if (_ModeWanted == MBEHAV::MOUNT_NORMAL ||
          _ModeWanted == MBEHAV::MOUNT_SWIM) {
        // The Mount
        if (_Rider != CLFECOMMON::INVALID_SLOT) {
          if (animationStateKey(MOVE, transition))
            ...
            nlwarning(
                "CH:setAnim:%d: Mode Wanted '%d' : automaton '%s': state '%s': "
                "Transition '%s' is not valid.",
                _Slot, _ModeWanted, _CurrentAutomaton.c_str(),
                CAnimationState::getAnimationStateName(curAnimState.MoveState)
                    .c_str(),
                CAnimationState::getAnimationStateName(transition).c_str());
        }
        // The Rider
          ...
          if (mount) {
            if (mountTmp->mode() == _ModeWanted) {
              if (animationStateKey(MOVE, transition))
                ...
                nlwarning(
                    "CH:setAnim:%d: automaton '%s': state '%s': "
                    "MountDefaultModeTransition '%s' is not valid.",
                    _Slot, _CurrentAutomaton.c_str(),
                    CAnimationState::getAnimationStateName(
                        curAnimState.MoveState)
                        .c_str(),
                    CAnimationState::getAnimationStateName(transition).c_str());
              }
            }
            ...
            nlwarning(
                "CH:setAnim:%d: Mode Wanted '%s' but the mount does not exist.",
                _Slot, MBEHAV::modeToString(_ModeWanted).c_str());
          }
        }
      }
...
if (curAnimState.BrkAtDest && (dist2Dest() <= ClientCfg.DestThreshold)) {
  ...
  if (animationStateKey(MOVE, CAnimationStateSheet::Idle))
    ...
    nlwarning(
        "CH:setAnim:%d: automaton '%s': state '%s': BrkAtDest 'idle' is not "
        "valid.",
        _Slot, _CurrentAutomaton.c_str(),
        CAnimationState::getAnimationStateName(curAnimState.MoveState).c_str());
}
...
case OnMoveForward:
  if (animationStateKey(MOVE, curAnimState.OnMoveForward))
    ...
    nlwarning(
        "CH::setAnim:%d: automaton '%s': state '%s': OnMoveForeward '%s' is "
        "not valid.",
        _Slot, _CurrentAutomaton.c_str(),
        CAnimationState::getAnimationStateName(curAnimState.MoveState).c_str(),
        CAnimationState::getAnimationStateName(curAnimState.OnMoveForward)
            .c_str());
  break;
// On Move Backward
case OnMoveBackward:
  if (animationStateKey(MOVE, curAnimState.OnMoveBackward))
    ...
    nlwarning(
        "CH::setAnim:%d: automaton '%s': state '%s': OnMoveBackward '%s' is "
        "not valid.",
        _Slot, _CurrentAutomaton.c_str(),
        CAnimationState::getAnimationStateName(curAnimState.MoveState).c_str(),
        CAnimationState::getAnimationStateName(curAnimState.OnMoveBackward)
            .c_str());
  break;
// On Move Left
case OnMoveLeft:
  if (animationStateKey(MOVE, curAnimState.OnMoveLeft))
    ...
    nlwarning(
        "CH::setAnim:%d: automaton '%s': state '%s': OnMoveLeft '%s' is not "
        "valid.",
        _Slot, _CurrentAutomaton.c_str(),
        CAnimationState::getAnimationStateName(curAnimState.MoveState).c_str(),
        CAnimationState::getAnimationStateName(curAnimState.OnMoveLeft)
            .c_str());
  break;
// On Move Right
case OnMoveRight:
  if (animationStateKey(MOVE, curAnimState.OnMoveRight))
    ...
    nlwarning(
        "CH::setAnim:%d: automaton '%s': state '%s': OnMoveRight '%s' is not "
        "valid.",
        _Slot, _CurrentAutomaton.c_str(),
        CAnimationState::getAnimationStateName(curAnimState.MoveState).c_str(),
        CAnimationState::getAnimationStateName(curAnimState.OnMoveRight)
            .c_str());
  break;
default:
  break;
...
case OnRotLeft:
  if (animationStateKey(MOVE, curAnimState.OnLeftRotation)) {
    ...
    nlwarning(
        "CH::setAnim:%d: automaton '%s': state '%s': OnLeftRotation '%s' is "
        "not valid.",
        _Slot, _CurrentAutomaton.c_str(),
        CAnimationState::getAnimationStateName(curAnimState.MoveState).c_str(),
        CAnimationState::getAnimationStateName(curAnimState.OnLeftRotation)
            .c_str());
  break;
// On Rot Right
case OnRotRight:
  if (animationStateKey(MOVE, curAnimState.OnRightRotation)) {
    ...
    nlwarning(
        "CH::setAnim:%d: automaton '%s': state '%s': OnRightRotation '%s' is "
        "not valid.",
        _Slot, _CurrentAutomaton.c_str(),
        CAnimationState::getAnimationStateName(curAnimState.MoveState).c_str(),
        CAnimationState::getAnimationStateName(curAnimState.OnRightRotation)
            .c_str());
  break;
default:
  break;
...
if (onBadHeading(curAnimState)) {
  if (animationStateKey(MOVE, CAnimationStateSheet::Idle))
    ...
    nlwarning(
        "CH::setAnim:%d: automaton '%s': state '%s': 'Idle' is not valid.",
        _Slot, _CurrentAutomaton.c_str(),
        CAnimationState::getAnimationStateName(curAnimState.MoveState).c_str());
}
...
case OnBendLeft:
  if (animationStateKey(MOVE, curAnimState.OnBigBendLeft)) {
    ...
    nlwarning(
        "CH::setAnim:%d: automaton '%s': state '%s': OnBigBendLeft '%s' is not "
        "valid.",
        _Slot, _CurrentAutomaton.c_str(),
        CAnimationState::getAnimationStateName(curAnimState.MoveState).c_str(),
        CAnimationState::getAnimationStateName(curAnimState.OnBigBendLeft)
            .c_str());
  break;
  // On Bend Right
case OnBendRight:
  if (animationStateKey(MOVE, curAnimState.OnBigBendRight)) {
    ...
    nlwarning(
        "CH::setAnim:%d: automaton '%s': state '%s': OnBigBendRight '%s' is "
        "not valid.",
        _Slot, _CurrentAutomaton.c_str(),
        CAnimationState::getAnimationStateName(curAnimState.MoveState).c_str(),
        CAnimationState::getAnimationStateName(curAnimState.OnBigBendRight)
            .c_str());
  break;
default:
  break;
...
// If the animation change according to a high speed and speed high enough or
// oo.
if (ClientCfg.BlendForward == false) {
  if (curAnimState.OnMaxSpeed.Breakable) {
    if (speedToDest >= _CurrentAnimSet[MOVE]->speedToRun() ||
        speedToDest == -1.0) {
      if (animationStateKey(MOVE, curAnimState.OnMaxSpeed.NextStateKey))
        goto KeyChosen;
      else
        nlwarning("CH::setAnim: Char '%d': automaton '%s': state '%s': "
                  "OnMaxSpeed.NextStateKey '%s' is not valid.",
                  _Slot, _CurrentAutomaton.c_str(),
                  CAnimationState::getAnimationStateName(curAnimState.MoveState)
                      .c_str(),
                  CAnimationState::getAnimationStateName(
                      curAnimState.OnMaxSpeed.NextStateKey)
                      .c_str());
    }
  }
}
...
if (curAnimState.OnMinSpeed.Breakable) {
  // If the speed is low enough (check this is not -1 (infinite speed)) ->
  // update the animation to play.
  if (speedToDest != -1.0 &&
      speedToDest <= _CurrentAnimSet[MOVE]->speedToWalk()) {
    if (animationStateKey(MOVE, curAnimState.OnMinSpeed.NextStateKey))
      goto KeyChosen;
    else
      nlwarning("CH::setAnim: Char '%d': automaton '%s': state '%s': "
                "OnMinSpeed.NextStateKey '%s' is not valid.",
                _Slot, _CurrentAutomaton.c_str(),
                CAnimationState::getAnimationStateName(curAnimState.MoveState)
                    .c_str(),
                CAnimationState::getAnimationStateName(
                    curAnimState.OnMinSpeed.NextStateKey)
                    .c_str());
  }
}
//// END LOOP /////
...
if (!sameAnim) {
  ...
  stopItemAttackFXs();
  stopAttachedFXForCurrrentAnim(
      true); // stop all anim fx, including looping fxs
} else {
  stopAttachedFXForCurrrentAnim(
      false); // stop all anim fxs, but do not stop looping fxs (because the
              // same anim is repeating)
}
...
// The state is valid.
  ...
  if (_CurrentState->Rotation)
    angToDest = computeShortestAngle(atan2(dir().y, dir().x),
                                     atan2(dirEndAnim().y, dirEndAnim().x));
  // Get the right animation state and choose an animation.
  {
    ...
    if (animState(MOVE) == CAnimationStateSheet::Emote) {
      ...
      animationState =
          _CurrentAnimSet[MOVE]->getAnimationState(animState(MOVE));
    if (animationState) {
      ...
      animIndex(MOVE,
                animationState->chooseAnim(_AnimJobSpecialisation, people(),
                                           getGender(), angToDest, index));
  ...
  if (animIndex(MOVE) != CAnimation::UnknownAnim) {
    ...
    if (_CurrentState->Rotation) {
      ...
      double animAngle = CAnimationMisc::getAnimationRotation(
          EAM->getAnimationSet(), animId(MOVE));
      // Compute the rotation factor.
      if (animAngle != 0.0)
        _RotationFactor = fabs(angToDest / animAngle);
      else
        _RotationFactor =
            -1.0; // \todo GUIGUI : see which value we should use if we have a
                  // rot anim without rot and which should rotate character
    }
    ...
    if (_CurrentBehaviour.Behaviour == MBEHAV::EXTRACTING) {
      ...
      if (animState(MOVE) == CAnimationStateSheet::UseLoop) {
        _Items[SLOTTYPE::RIGHT_HAND_SLOT].setTrailSize(
            _CurrentBehaviour.ForageExtraction.Level);
        startItemAttackFXs(true, 1);
      }
    } else if (_CurrentState->Attack) {
      if (_CurrentBehaviour.Behaviour == MBEHAV::RANGE_ATTACK) {
        startItemAttackFXs(_CurrentBehaviour.Range.ImpactIntensity != 0,
                           _CurrentBehaviour.Range.ImpactIntensity);
      } else {
        startItemAttackFXs(_CurrentBehaviour.Combat.ImpactIntensity != 0 &&
                               _CurrentBehaviour.Combat.HitType !=
                                   HITTYPE::Failed,
                           _CurrentBehaviour.Combat.ImpactIntensity);
      }
    }
    ...
    if (_PlayList) {
      ...
      if (ClientCfg.BlendFrameNumber && _BlendRemaining <= 0 // Blend On ?
          ...
          && ((lastAnimStateId != animState(MOVE)) ||
              (_OldAutomaton != _CurrentAutomaton) ||
              (animState(MOVE) ==
               CAnimationStateSheet::Emote)) // Last anim != or automaton != ?
          && !isRiding()) // No Blend on a mount. \todo GUIGUI trouver un
                          // meilleur moyen.
      {
      ...
      if (ClientCfg.BlendForward &&
          (animState(MOVE) == CAnimationStateSheet::Walk)) {
        _CurrentAnimSet[MOVE_BLEND_OUT] = _CurrentAnimSet[MOVE];
        animState(MOVE_BLEND_OUT, CAnimationStateSheet::Run);
        const CAnimationState *animationBlendState =
            _CurrentAnimSet[MOVE_BLEND_OUT]->getAnimationState(
                animState(MOVE_BLEND_OUT));
        if (animationBlendState) {
          animIndex(MOVE_BLEND_OUT,
                    animationBlendState->chooseAnim(_AnimJobSpecialisation,
                                                    people(), getGender()));
          _PlayList->setAnimation(MOVE_BLEND_OUT, animId(MOVE_BLEND_OUT));
          _PlayList->setWeight(MOVE_BLEND_OUT,
                               1.0f); // \todo GUIGUI : verify what is happening
                                      // if animId is "empty".
        } else
      ...
      while (it != _Children.end()) {
        if (*it) {
          ...
          if (child) {
            ...
            child->currentAutomaton() =
                _CurrentAutomaton; // \todo GUIGUI : CA VA PAS MARCHER A CAUSE
                                   // DU TYPE !!!
            ...
            const CAnimationState *animStatePtr =
                child->currentAnimSet()[MOVE]->getAnimationState(
                    child->animState(MOVE));
            if (animStatePtr) {
              child->animIndex(MOVE, animStatePtr->chooseAnim(
                                         _AnimJobSpecialisation, people(),
                                         getGender(), angToDest));
              child->playList()->setAnimation(MOVE, child->animId(MOVE));
            }
            // TEMP : \todo GUIGUI : Pour le moment on enlever le Blend sur les
            // montures.
            {
    ...
    // Get the animation.
    const CAnimationState *animStatePtr =
        _CurrentAnimSet[MOVE]->getAnimationState(
            (animState(MOVE) == CAnimationStateSheet::Emote) ? subKey
                                                             : animState(MOVE));
    if (animStatePtr) {
      const CAnimation *anim = animStatePtr->getAnimation(animIndex(MOVE));
      if (anim) {
        ...
        if (_CurrentAttack) {
          if (_CurrentAttackInfo.Intensity >= 1 &&
              _CurrentAttackInfo.Intensity <= MAGICFX::NUM_SPELL_POWER) {
            ...
            switch (attackStage) {
            case MAGICFX::CastBegin:
              afs = &_CurrentAttack->AttackBeginFX;
              break;
            case MAGICFX::CastLoop:
              afs = &_CurrentAttack->AttackLoopFX;
              break;
            case MAGICFX::CastEnd:
              afs = &_CurrentAttack->AttackEndFX;
              break;
            case MAGICFX::CastFail:
              afs = &_CurrentAttack->AttackFailFX;
              break;
            default:
              break;
            }
        ...
        if ((behaviour() == MBEHAV::PROSPECTING) ||
          ...
          for (uint k = 0; k < fxSet.FX.size(); ++k) {
            ...
            if (!fx->FX.empty()) {
              ...
              if (k == 0) {
                ...
                {
                  fx->FX.setUserParam(
                      0, ((float)(_CurrentBehaviour.ForageProspection.Range)) /
                             127.0f);
                  float up[3];
                  ...
                  // nlinfo( "Prospection %s %u %u",
                  // behaviour()==MBEHAV::PROSPECTING?"PROSPTG":(behaviour()==MBEHAV::PROSPECTING_END?"PRO_END":"OTHER"),
                  // _CurrentBehaviour.ForageProspection.Range,
                  // _CurrentBehaviour.ForageProspection.Angle );
                }
              } else if (k == 1) {
        ...
        // start standard anim fx
          ...
          for (uint k = 0; k < anim->getFXSet().FX.size(); ++k) {
            // if the fx is in looping mode, & the anim has already done a loop,
            // then don't recreate it
            if (anim->getFXSet().FX[k].Sheet->RepeatMode ==
      ...
      nlwarning("CH:setAnim:%d: cannot get the pointer on the animation.",
                _Slot);
  }
  // No animation found -> check if it is just a transition or is there really
  // an animation missing
  else {
    // INFO : Verbose mode for Animations of the selection.
    if (VerboseAnimSelection && _Slot == UserEntity->selection())
      nlinfo("CH:setAnim:%d: Anim Not Found, state '%s'.", _Slot,
             CAnimationState::getAnimationStateName(animState(MOVE)).c_str());

    // If the next animation or automaton is not the same -> this is a
    // transition -> no animation needed.
    if (_CurrentState->MoveState != _CurrentState->NextState ||
        (_CurrentState->NextMode != _Mode)) {
...
setAnimLOD(((lastAnimStateId != animState(MOVE)) ||
            (_OldAutomaton != _CurrentAutomaton)));
} // setAnim //
...
void CCharacterCL::showOrHideBodyParts(bool objectsVisible) {
  ...
  if (_Items[SLOTTYPE::HEAD_SLOT].Sheet &&
      _Items[SLOTTYPE::HEAD_SLOT].Sheet->Family == ITEMFAMILY::ARMOR) {
    // Get the face
  ...
  if (isPlayer() || isUser()) {
    ...
    if (_Items[SLOTTYPE::RIGHT_HAND_SLOT].Sheet &&
        (_Items[SLOTTYPE::RIGHT_HAND_SLOT].Sheet->ItemType ==
  ...
  if (!objectsVisible) {
    // Right Hand
    nlassert(SLOTTYPE::RIGHT_HAND_SLOT < _Items.size() &&
             SLOTTYPE::LEFT_HAND_SLOT < _Items.size());
    if (rHandInstIdx < _Instances.size())
      if (!(_Items[SLOTTYPE::RIGHT_HAND_SLOT].Sheet &&
            _Items[SLOTTYPE::RIGHT_HAND_SLOT].Sheet->NeverHideWhenEquipped))
    ...
    if (lHandInstIdx < _Instances.size())
      if (!(_Items[SLOTTYPE::LEFT_HAND_SLOT].Sheet &&
            _Items[SLOTTYPE::LEFT_HAND_SLOT].Sheet->NeverHideWhenEquipped))
...
// \todo GUIGUI : improve, we are setting often 'idle' to recompute orientation
// at the end of animation instead of doing directly the right one.
//-----------------------------------------------
...
// Last animation not finished check for some chances to break current
// animation.
else {
  // Check Modes.
  //		if(_IsThereAMode && (dist2Dest()==INVALID_DIST))
  {
    ...
    if ((_Mode != _ModeWanted) && (dist2Dest() == INVALID_DIST)) {
      if ((_ModeWanted != MBEHAV::MOUNT_NORMAL &&
        ...
        // InfoLog->displayRawNL("(%d)_Rider=%d  _Mount=%d  _ModeWanted=%s
        // _Mode=%s  offset=%f
        // length=%f",_Slot,_Rider,_Mount,MBEHAV::modeToString(_ModeWanted).c_str(),MBEHAV::modeToString(_Mode).c_str(),animOffset(MOVE),animLength);
        //  anti-bug : sometimes _rider is not set yet and we stay in an
        //  infinite move
  ...
  if (!(isUser() && ClientCfg.AutomaticCamera == false)) {
    if (_CurrentState->AdjustOri) {
      ...
      if (animOffset(MOVE) > animL)
        ...
        double ang = computeShortestAngle(atan2(dir().y, dir().x),
...
// computeMotion :
//-----------------------------------------------
ADD_METHOD(double CCharacterCL::computeMotion(const double &oldMovingTimeOffset,
                                              TAnimationType channel) const)
...
if (_CurrentState->Move) {
  ...
  if (EAM->interpolate(animID, oldMovingTimeOffset, oldPos)) {
    if (EAM->interpolate(animID, animOffset(channel), newPos)) {
      ...
      // Scale it by the CharacterScalePos, if needed, according to the
      // animation.
      bool mustApplyCharacterScalePosFactor = true;
      const CAnimationState *animStatePtr =
          _CurrentAnimSet[channel]->getAnimationState(animState(channel));
      if (animStatePtr) {
        const CAnimation *anim = animStatePtr->getAnimation(animIndex(channel));
        if (anim)
          mustApplyCharacterScalePosFactor =
  ...
  nlwarning("CCharacterCL::computeMotion : Animation should have a track for "
            "the position.");
  return 0.0;
}
...
// endCast :
//-----------------------------------------------
void CCharacterCL::endCast(const MBEHAV::CBehaviour &behaviour,
                           const MBEHAV::CBehaviour &lastBehaviour) {
  if (!(isUser() && isSit())) {
...
void CCharacterCL::updateCurrentAttack() {
  ...
  if (_CurrentBehaviour.isMagic()) {
    ...
    _CurrentAttackID.SpellInfo.Mode =
        (MAGICFX::TSpellMode)_CurrentBehaviour.Spell.SpellMode;
    _CurrentAttackID.SpellInfo.ID =
        (MAGICFX::TMagicFx)_CurrentBehaviour.Spell.SpellId;
  }
  // This is a behaviour for the combat.
    ...
    _CurrentAttackID.RangeWeaponType =
        (RANGE_WEAPON_TYPE::TRangeWeaponType)_CurrentBehaviour.Range.WeaponType;
    ...
    _CurrentAttackID.CreatureAttackIndex =
        _CurrentBehaviour.Behaviour == MBEHAV::CREATURE_ATTACK_0 ? 0 : 1;
  } else {
  ...
  // This is a behaviour for the combat.
    ...
    _CurrentAttackInfo.DamageType = DMGTYPE::UNDEFINED;
    _CurrentAttackInfo.HitType = HITTYPE::Undefined;
    _CurrentAttackInfo.Localisation =
        (BODY::TBodyPart)_CurrentBehaviour.Range.Localisation;
    _CurrentAttackInfo.PhysicalImpactIntensity = 0;
  } else if (_CurrentBehaviour.isCombat()) {
    _CurrentAttackInfo.Intensity = _CurrentBehaviour.Combat.ImpactIntensity;
    _CurrentAttackInfo.DamageType =
        (DMGTYPE::EDamageType)_CurrentBehaviour.Combat2.DamageType;
    _CurrentAttackInfo.HitType =
        (HITTYPE::THitType)_CurrentBehaviour.Combat.HitType;
    _CurrentAttackInfo.Localisation =
        (BODY::TBodyPart)_CurrentBehaviour.Combat.Localisation;
    _CurrentAttackInfo.PhysicalImpactIntensity =
        _CurrentBehaviour.Combat.ImpactIntensity;
  } else if (_CurrentBehaviour.isCreatureAttack()) {
    // creature attack is the most general form
    _CurrentAttackInfo.Intensity =
        _CurrentBehaviour.CreatureAttack.MagicImpactIntensity;
    _CurrentAttackInfo.DamageType =
        (DMGTYPE::EDamageType)_CurrentBehaviour.CreatureAttack2.DamageType;
    _CurrentAttackInfo.HitType =
        (HITTYPE::THitType)_CurrentBehaviour.CreatureAttack2.HitType;
    _CurrentAttackInfo.Localisation =
        (BODY::TBodyPart)_CurrentBehaviour.CreatureAttack.Localisation;
    _CurrentAttackInfo.PhysicalImpactIntensity =
        _CurrentBehaviour.CreatureAttack.ImpactIntensity;
  }
  _CurrentAttackInfo.Side = (rand() & 1) ? BODY::Left : BODY::Right;
}

// utility function for performCurrentAttackEnd
inline static void getResistAndDistance(uint8 packedInfo, bool isDirectAttack,
                                        bool isCombat, bool &resist,
                                        uint &distance) {
...
// *********************************************************************************************
void CCharacterCL::performCurrentAttackEnd(const CBehaviourContext &bc,
                                           bool directOffensifSpell,
                                           vector<double> &targetHitDates,
                                           TAnimStateKey animForCombat) {
  if (!_CurrentAttack)
    return;
  if (!_CurrentAttack->Sheet)
    return;
  ...
  // Combat (range or melee) => no resist (yoyo: legacy code. Actually the
  // server should set the resist bit to 0 in Target.Info)
  bool isCombat = _CurrentBehaviour.isCombat();

  // An attack is said 'direct' when no projectile is casted (Generally those
  // are melee attack, but also pistol range attack for instance) In this case,
  // all targets are hit at the same time, no matter what date is supplied for
  // them.
  ...
  if (isDirectAttack) {
    ...
    // DIRECT ATTACK //
    ///////////////////
    // compute impact date. It is given by the trigger on right hand in the
    // animation Get the current animation id.
    if (sheet.ForceUseProjectileDelay) {
      delay = sheet.ProjectileDelay;
    } else if (!directOffensifSpell) {
      if ((_CurrentAttackID.Type != CAttackIDSheet::Range) &&
          (_PlayList != NULL)) {
        // default
        delay = 0.5f;
        // if the animation has been correctly chosen
        if (animForCombat != CAnimationStateSheet::UnknownState) {
          // try to get the MeleeImpactDelay that is stored in the sheet (given
          // by graphists)
          const CAnimationSet *animSet = currentAnimSet()[MOVE];
          if (animSet) {
            const CAnimationState *animState =
                animSet->getAnimationState(animForCombat);
            if (animState)
              delay = animState->getMeleeImpactDelay();
          }
        }
      } else {
        // see if weapon is known, and in this case, delay the impact by the
        // same amount
        const CItemSheet *sheet = _Items[SLOTTYPE::RIGHT_HAND_SLOT].Sheet;
        if (sheet)
          delay = sheet->FX.ImpactFXDelay;
      }
    }
    ...
    if (usesStaticCastFX) {
      ...
      if (!directOffensifSpell) {
        // if object has a list of cast rays, then we assume it is a static
        // object (like guard towers) projectile cast from a character
        delay = sheet.ProjectileDelay;
      }
    }
  }
  ...
  // Special for direct attack. because of lag sometimes behavTime<TimeInSec,
  // which results on too early played hits regarding animation played
  if (isDirectAttack) {
    // start at max(behavTime, TimeInSec), but with a limit of bc.BehavTime+0.5
    // sec
    ...
    // In case of magic or range (with projectile), this is not required because
    // the projectilemanager should take cares of cast start time and current
    // time (and therfore advance the projectile if required)
  ...
  case MAGICFX::Bomb: {
    getResistAndDistance(bc.Targets.Targets[0].Info, isDirectAttack, isCombat,
                         resist, distance);
    double mainStartDate = castStartTime;
    double mainEndDate =
        mainStartDate +
        timeFactor * double(distance * MULTI_TARGET_DISTANCE_UNIT /
                            MAGICFX::PROJECTILE_SPEED);
    CCharacterCL *mainTarget = dynamic_cast<CCharacterCL *>(
        EntitiesMngr.entity(bc.Targets.Targets[0].TargetSlot));
    if (mainTarget) {
      computeTargetStickMode(*_CurrentAttack->Sheet, attackInfo,
                             pb.ProjectileAimingPoint, *mainTarget);
      if (usesStaticCastFX) {
        // compute the cast pos
        computeBestCastRay(*mainTarget, pb.ProjectileAimingPoint,
                           castWorldOrigin, castWorldPos, additionnalOffset);
        castRayValid = true;
      }
      ...
      if (createCurrentAttackEndPart(
              pb, _CurrentAttack, *mainTarget, NULL, mainStartDate, mainEndDate,
              true, sheet.PlayImpactAnim, resist, sheet.IsImpactLocalised,
              attackInfo, additionnalOffset)) {
        CProjectileManager::getInstance().addProjectileToQueue(pb);
      }
      attackInfo.Intensity = attackInfo.Intensity != 0 ? 1 : 0;
      attackInfo.PhysicalImpactIntensity =
          attackInfo.PhysicalImpactIntensity != 0 ? 1 : 0;
      // all subsequent projectiles are casted from the secondary target, from
      // the first impact point, and have level 1
      for (uint k = 1; k < bc.Targets.Targets.size(); ++k) {
        getResistAndDistance(bc.Targets.Targets[k].Info, isDirectAttack,
                             isCombat, resist, distance);
        double secondaryEndDate =
            mainEndDate +
            timeFactor * ((double)(distance * MULTI_TARGET_DISTANCE_UNIT /
                                   MAGICFX::PROJECTILE_SPEED));
        CCharacterCL *secondaryTarget = dynamic_cast<CCharacterCL *>(
            EntitiesMngr.entity(bc.Targets.Targets[k].TargetSlot));
        if (secondaryTarget) {
          // the target hit date
          targetHitDates[k] = secondaryEndDate;

          // Add the projectile to queue
          if (mainTarget->createCurrentAttackEndPart(
                  pb, _CurrentAttack, *secondaryTarget,
                  &pb.ProjectileAimingPoint, mainEndDate, secondaryEndDate,
                  k != 0 ? !sheet.PlayImpactFXOnlyOnMainTarget : true,
                  sheet.PlayImpactAnim, resist, sheet.IsImpactLocalised,
                  attackInfo)) {
            computeTargetStickMode(*_CurrentAttack->Sheet, attackInfo,
                                   pb.ProjectileAimingPoint, *secondaryTarget);
            CProjectileManager::getInstance().addProjectileToQueue(pb);
          }
        }
      }
  ...
  case MAGICFX::Chain: {
    ...
    const CFXStickMode *projectileStartPoint =
        NULL; // by default, start at caster hand
    ...
    for (uint k = 0; k < bc.Targets.Targets.size(); ++k) {
      ...
      getResistAndDistance(bc.Targets.Targets[k].Info, isDirectAttack, isCombat,
                           resist, distance);
      double nextDate =
          currDate +
          timeFactor * ((double)(distance * MULTI_TARGET_DISTANCE_UNIT /
                                 MAGICFX::PROJECTILE_SPEED));
      CCharacterCL *currTarget = dynamic_cast<CCharacterCL *>(
          EntitiesMngr.entity(bc.Targets.Targets[k].TargetSlot));
      if (!currTarget)
        break;

      computeTargetStickMode(*_CurrentAttack->Sheet, attackInfo,
                             pb.ProjectileAimingPoint, *currTarget);

      if (k == 0 && usesStaticCastFX) {
        // compute the initial cast pos for objects with multiple possible cast
        // positions
        computeBestCastRay(*currTarget, pb.ProjectileAimingPoint,
                           castWorldOrigin, castWorldPos, additionnalOffset);
        castRayValid = true;
      }
      ...
      // Add the projectile to queue
      if (currCaster->createCurrentAttackEndPart(
              pb, _CurrentAttack, *currTarget, projectileStartPoint, currDate,
              nextDate, k != 0 ? !sheet.PlayImpactFXOnlyOnMainTarget : true,
              sheet.PlayImpactAnim, resist, sheet.IsImpactLocalised, attackInfo,
    ...
    for (uint k = 0; k < bc.Targets.Targets.size(); ++k) {
      ...
      if (currTarget) {
        computeTargetStickMode(*_CurrentAttack->Sheet, attackInfo,
                               pb.ProjectileAimingPoint, *currTarget);
        if (k == 0 && usesStaticCastFX) {
          // compute the initial cast pos for objects with multiple possible
          // cast positions
          computeBestCastRay(*currTarget, pb.ProjectileAimingPoint,
                             castWorldOrigin, castWorldPos, additionnalOffset);
          castRayValid = true;
        ...
        if (createCurrentAttackEndPart(
                pb, _CurrentAttack, *currTarget, NULL, startDate, endDate,
                k != 0 ? !sheet.PlayImpactFXOnlyOnMainTarget : true,
                sheet.PlayImpactAnim, resist, sheet.IsImpactLocalised,
                attackInfo, k == 0 ? additionnalOffset : CVector::Null)) {
          CProjectileManager::getInstance().addProjectileToQueue(pb);
        }
      }
    }
  ...
  // if object has a list of cast rays, then we assume it is a static object
  // (like guard towers)
  if (usesStaticCastFX && castRayValid) {
    buildStaticObjectCastFX(castWorldOrigin, castWorldPos,
                            *_CurrentAttack->Sheet, mainCastFXIntensity);
  }

  // *** Play damage shields when melee attack is done
  /*
  // TODO: to finalize (server code not done).
  if (ClientCfg.DamageShieldEnabled && _CurrentBehaviour.isCombat() &&
  _CurrentBehaviour.todoNotRange())
  {
          for(uint k = 0; k < bc.Targets.Targets.size(); ++k)
          {
                  ...
                  if (power)
                  {
                          ...
                          CCharacterCL *currTarget = dynamic_cast<CCharacterCL
  *>(EntitiesMngr.entity(bc.Targets.Targets[k].TargetSlot)); if (!currTarget)
  continue; const CAttack *damageShieldReaction =
  currTarget->getAttack(damageShieldID); if (!damageShieldReaction) continue;
                          ...
                          // no physical part
                          const CAttackSheet &damageShieldSheet =
  *damageShieldReaction->Sheet; computeTargetStickMode(damageShieldSheet,
  attackInfo, pb.ProjectileAimingPoint, *this); if
  (currTarget->createCurrentAttackEndPart(pb, damageShieldReaction, *this, NULL,
                                                                                                     castStartTime,
                                                                                                     castStartTime,
                                                                                                     true,
  ...
  ...) pb.CastAspect = &damageShieldReaction->AttackEndFX; pb.CastPower = power;
...
// *********************************************************************************************
void CCharacterCL::buildStaticObjectCastFX(
    const NLMISC::CVector &castWorldOrigin, NLMISC::CVector &castWorldPos,
    const CAttackSheet & /* sheet */, uint intensity) {
  if (intensity == 0)
    return;
  ...
  // Build lookat matrix (with respect to axis) looking from castWorldOrigin to
  // castWorldPos
  ...
  for (uint k = 0; k < afs.FX.size(); ++k) {
    // if the fx is in looping mode, & the anim has already done a loop, then
    // don't recreate it
    CAttachedFX::TSmartPtr fx = new CAttachedFX;
    bi.Sheet = &afs.FX[k];
    fx->create(*this, bi, ti);
...
// *********************************************************************************************
void CCharacterCL::computeTargetStickMode(const CAttackSheet &sheet,
  ...
  if (sheet.IsImpactLocalised || hasPhysicalImpact) {
    ...
    // & generate stick mode
    const char *targetBoneName = target.getBoneNameFromBodyPart(
        attackInfo.Localisation, attackInfo.Side);
    if (targetBoneName) {
      if (sheet.DefaultAimingPoint.Mode ==
...
// *********************************************************************************************
bool CCharacterCL::createCurrentAttackEndPart(
    CProjectileBuild &destPB, const CAttack *currentAttack,
    const CCharacterCL &target, const CFXStickMode *sm, double spawnDate,
    double hitDate, bool playImpactFX, bool playImpactAnim, bool magicResist,
    bool /* mainImpactIsLocalised */, const CAttackInfo &attackInfo,
    const NLMISC::CVector &additionnalOffset /*= NLMISC::CVector::Null*/
) {
  if (!currentAttack)
    return false;
  if (!currentAttack->Sheet)
    return false;
  ...
  if (attackInfo.Intensity >= 1 &&
      attackInfo.Intensity <= MAGICFX::NUM_SPELL_POWER) {
  ...
  if (!playImpactFX) {
    ...
    if (attackInfo.Intensity >= 1 &&
        attackInfo.Intensity <=
            MAGICFX::NUM_SPELL_POWER) // impact has same intensity than
                                      // projectile
    {
  ...
  // If this is a secondary projectile, it may start from another location,
  // which is the impact point of the previous projectile (so it doesn't start
  // from the caster hand, or any around settings that is read from the spell
  // sheet)
  if (sm) // start stickmode wanted ?
  {
    destPB.TargeterInfo.StickMode = *sm;
  } else {
    // if no stick mode is not forced, then use the one given in the  projectile
    // sheet
    if (destPB.ProjectileAspect && !destPB.ProjectileAspect->FX.empty()) {
      destPB.TargeterInfo.StickMode =
          destPB.ProjectileAspect->FX[0].Sheet->StickMode;
    } else {
  ...
  // offset if projectile is launched from a range weapon and projectile is
  // sticked to box_arme
  if (destPB.TargeterInfo.StickMode.Mode == CFXStickMode::UserBone &&
      sheet.ApplyItemOffsetToWeaponBone) {
    // should be fired from the 'box_arme' bone, which means it is fired from a
    // weapon
    if (CStringMapper::unmap(destPB.TargeterInfo.StickMode.UserBoneName) ==
        "box_arme") {
...
// *********************************************************************************************
void CCharacterCL::computeBestCastRay(
    CEntityCL &targetEntity, const CFXStickMode &targetStickMode,
    NLMISC::CVector &castWorldOrigin, NLMISC::CVector &castWorldPos,
    NLMISC::CVector &worldOffsetToCasterPivot) const {
  // additionnal offset taken from sheet. Useful for towers that have no bones,
  // but can fire projectiles anyway
  nlassert(_Sheet && !_Sheet->ProjectileCastRay.empty());
  // if several offsets are provided, then choose the one that has the smallest
  ...
  for (uint k = 0; k < _Sheet->ProjectileCastRay.size(); ++k) {
    ...
    CVector currCastWorldOrigin =
        casterMatrix * _Sheet->ProjectileCastRay[k].Origin;
    float dp3 = (target - currCastWorldPos).normed() *
                (currCastWorldPos - currCastWorldOrigin).normed();
    if (dp3 > maxDP3) {
      maxDP3 = dp3;
      worldOffsetToCasterPivot =
          casterMatrix.mulVector(_Sheet->ProjectileCastRay[k].Pos);
...
// ***************************************************************************
void CCharacterCL::applyBehaviourFlyingHPs(
    const CBehaviourContext &bc, const MBEHAV::CBehaviour &behaviour,
  ...
  if (!bc.Targets.Targets.empty()) {
    if (behaviour.DeltaHP != 0) {
      ...
      for (size_t i = 0; i < bc.Targets.Targets.size(); ++i) {
        CEntityCL *target2 =
            EntitiesMngr.entity(bc.Targets.Targets[i].TargetSlot);
        if (target2)
          target2->addHPOutput(behaviour.DeltaHP, deltaHPColor,
                               float(targetHitDates[i] - TimeInSec));
      }
    }
  }
}
...
void CCharacterCL::applyBehaviour(const CBehaviourContext &bc) // virtual
{
  ...
  if (!behaviour.isMagic() &&
      (behaviour.isCombat() || behaviour.isCreatureAttack())) {
    // Atk Animation when moving
    if (_CurrentState && _CurrentState->Move &&
        (_CurrentState->OnAtk != CAnimationStateSheet::UnknownState)) {
    ...
    // Atk Animation when NOT moving
      ...
      // Default Animation
      case DEFAULT_ATTACK:
        switch (getAttackHeight(target,
                                (BODY::TBodyPart)behaviour.Combat.Localisation,
                                BODY::Right)) {
        // LOW
  ...
  if (isCurrentBehaviourAttackEnd()) {
    ...
    performCurrentAttackEnd(bc, selfSpell && isOffensif, targetHitDates,
                            combatAnimState);
  }

  // INFO : display some debug information.
  if ((VerboseAnimUser && _Slot == 0) ||
      (VerboseAnimSelection && _Slot == UserEntity->selection()))
    nlinfo("CH:applyBeh:%d: '%d(%s)'", _Slot, (sint)behaviour.Behaviour,
           behaviourToString((EBehaviour)behaviour.Behaviour).c_str());
  ...
  // Emote
    ...
    if (ClientCfg.Light == false && ClientCfg.EAMEnabled) {
      ...
      if (EAM) {
        ...
        CTextEmotListSheet *pTELS =
        dynamic_cast<CTextEmotListSheet*>(SheetMngr.get(CSheetId("list.text_emotes")));
        if (pTELS)
        {
                if (emoteIndex < pTELS->TextEmotList.size())
                {
                        const CTextEmotListSheet::STextEmot &emot =
        pTELS->TextEmotList[emoteIndex]; if (!emot.FXToSpawn.empty())
                        {
                                // Compute the direction Matrix
                                CMatrix fxMatrix;
                                CVector vi = dir() ^ CVector::K;
                                CVector vk = vi ^ dir();
                                fxMatrix.setRot(vi, UserEntity->dir(), vk,
        true); fxMatrix.setPos(pos().asVector() + fxMatrix.getJ() *
        emot.FXSpawnDist); FXMngr.deferFX(emot.FXToSpawn, fxMatrix,
        emot.FXSpawnDelay);
                        }
                }
        }
        */
        if (EAM->getEmot(behaviour.Behaviour - EMOTE_BEGIN, emot))
          ...
          nlwarning("CH:applyBeh:%d: Emot '%d' unknown.", _Slot,
                    behaviour.Behaviour - EMOTE_BEGIN);
      }
    }
  }
  // Others
    ...
    case MBEHAV::EXTRACTING:
      ...
      // If receiving a new DeltaHP in the current extraction, don't reset the
      // animation
    ...
    default:
      nlwarning("CH::computeBehaviour : Entity in slot %d has an unknown "
                "behaviour %d to manage.",
                _Slot, (sint)behaviour.Behaviour);
      break;
    }
  }
} // computeBehaviour //
...
//-----------------------------------------------
void CCharacterCL::impact(uint /* impactType */, uint type, uint id,
                          uint intensity) // virtual
{
  // Display Magic Debug Infos
  if (Verbose & VerboseMagic)
    nlinfo("CH:impact:%d: type: %d, id: %d, intensity: %d", _Slot, type, id,
           intensity);
  ...
  if (!impact.empty()) {
    ...
    if (!impactFX.empty()) {
      ...
      if (!instFX.empty()) {
        // UserParam  |  Intensity 1  |  Intensity 2  |  Intensity 3  |
...
void CCharacterCL::meleeImpact(const CAttackInfo &attack) {
  if (_Skeleton.empty())
    return;
  if (attack.PhysicalImpactIntensity < 1 || attack.PhysicalImpactIntensity > 5)
    return;
  if (attack.HitType == HITTYPE::Failed)
    return;
  const char *boneName =
      getBoneNameFromBodyPart(attack.Localisation, attack.Side);
  if (!boneName)
    return;
  sint boneId = _Skeleton.getBoneIdByName(std::string(boneName));
  if (boneId == -1)
  ...
  case DMGTYPE::BLUNT:
    instance = Scene->createInstance("mel_impactblunt.ps");
    break;
  case DMGTYPE::SLASHING:
    instance = Scene->createInstance("mel_impactslashing.ps");
    break;
  case DMGTYPE::PIERCING:
    instance = Scene->createInstance("mel_impactpiercing.ps");
    break;
  default:
  ...
  static const float intensityUP[5][2] = {
      {0.f, 0.f}, {0.f, 0.5f}, {0.5f, 0.5f}, {0.5f, 1.f}, {1.f, 1.f}};
  ...
  impact.setUserParam(2, (attack.HitType == HITTYPE::CriticalHit ||
                          attack.HitType == HITTYPE::CriticalHitResidual)
                             ? 1.f
                             : 0.f);
  impact.setUserParam(3, (attack.HitType == HITTYPE::HitResidual ||
                          attack.HitType == HITTYPE::CriticalHitResidual)
                             ? 1.f
                             : 0.f);
  //
...
bool CCharacterCL::mode(MBEHAV::EMode m) {
  ...
  if (m == MBEHAV::UNKNOWN_MODE || m >= MBEHAV::NUMBER_OF_MODES) {
    nlwarning("CH::mode:%d: Invalid Mode Wanted '%s(%d)' -> keep '%s(%d)'.",
              _Slot, MBEHAV::modeToString(m).c_str(), m,
              MBEHAV::modeToString(_Mode).c_str(), _Mode);
    return false;
  }
  // Set the mode wanted.
...
bool CCharacterCL::applyStage(CStage &stage) {
  ...
  if (resultBehaviour.first) {
    ...
    // See if there's a list of target associated with that behaviour (for
    // multitarget spells)
  ...
  // The Mount
  pair<bool, sint64> resultParent =
      stage.property(CLFECOMMON::PROPERTY_ENTITY_MOUNTED_ID);
  ...
  // The Rider
  pair<bool, sint64> resultRider =
      stage.property(CLFECOMMON::PROPERTY_RIDER_ENTITY_ID);
  ...
  pair<bool, sint64> resultVisualFX =
      stage.property(CLFECOMMON::PROPERTY_VISUAL_FX);
...
void CCharacterCL::applyAllStagesToFirstPos() {
  ...
  if (it != _Stages._StageSet.end()) {
    ...
    nlwarning(
        "CH:applyAllStagesToFirstPos:%d: There is no stage with a position.",
        _Slot);
...
//-----------------------------------------------
// playToEndAnim :
// Play the time step for the loop and truncate to End Anim if Time Step too
// big.
//-----------------------------------------------
ADD_METHOD(void CCharacterCL::playToEndAnim(const double &startTimeOffset,
                                            double &length))
// Average Speed
...
if (ClientCfg.BlendForward && (animState(MOVE) == CAnimationStateSheet::Walk)) {
  ...
  double animWalkSpeed = EAM->getAnimationAverageSpeed(animWalkId) *
                         getSheetScale() * _CustomScalePos * _CharacterScalePos;
  double animRunSpeed = EAM->getAnimationAverageSpeed(animRunId) *
                        getSheetScale() * _CustomScalePos * _CharacterScalePos;
  if (animWalkSpeed <= animRunSpeed) {
    ...
    if (speed() <= animWalkSpeed) {
      ...
      // Adjust Time Offset for the Run Channel
        ...
        animOffset(MOVE_BLEND_OUT,
                   animRunLength * (animTimeOffWalk / animWalkLength));
      }
    }
    ...
    // Current Speed > Walk Speed & < Run Speed, so mix Walk and Run animation.
      ...
      runFactor(t2 / t1);
      double mixLength =
          runFactor() * animRunLength + (1.0 - runFactor()) * animWalkLength;
      double animTimeOffWalk =
          animOffset(MOVE) + animWalkLength / mixLength * length;
      if (animTimeOffWalk > animWalkLength) {
        animOffset(MOVE, animWalkLength);
        animOffset(MOVE_BLEND_OUT, animRunLength);
        length =
            (animWalkLength - startTimeOffset) / (animWalkLength / mixLength);
      } else {
        animOffset(MOVE, animTimeOffWalk);
        animOffset(MOVE_BLEND_OUT,
                   animRunLength * animTimeOffWalk /
                       animWalkLength); // Same percentage in the animation than
                                        // the Walk one.
      }
    }
    return;
...
// Truncate animation time offset if it over-runs end of animation and change
// the loopTimeOffset too.
...
//-----------------------------------------------
// updateStages :
// Call this method to give a time for each stage, compute distance to
// destination and some more information. \todo GUIGUI : clean up
//-----------------------------------------------
void CCharacterCL::updateStages() {
  ...
  // ***** update predicted interval: if a new pos B is found after a pos A,
  // then the interval B-A is known!
  CStageSet::TStageSet::iterator it = _Stages._StageSet.begin();
  ...
  while (it != _Stages._StageSet.end()) {
    ...
    if (it->second.isPresent(PROPERTY_POSITION)) {
      ...
      // then it's cool we can set the new accurate interval to the prec stage
      // which has a pos
  ...
  // ***** Compute Stages to give them a time and get some information from
  // those stages. yoyo: use any stage with no LCT, until it is to be played
  // AFTER a position
  ...
  while (it != _Stages._StageSet.end()) {
    ...
    if (hasPos && dist2Dest() == INVALID_DIST) {
      ...
      double distToFirst = (CVectorD(posInStage.x, posInStage.y, 0.0) -
                            CVectorD(posTmp.x, posTmp.y, 0.0))
                               .norm();
      double distToLimiter =
          (CVectorD(posInStage.x, posInStage.y, 0.0) -
           CVectorD(_PositionLimiter.x, _PositionLimiter.y, 0.0))
              .norm();
      ...
      if ((distToFirst < ClientCfg.DestThreshold) ||
        ...
        // The FIRST POSITION is the SAME as the CURRENT entity POSITION ->
        // REMOVE POSITION in the stage
        ...
        //
        if (VerboseAnimSelection && _Slot == UserEntity->selection())
          nlinfo("CH:updateStages:%d: Bad First, distToFirst(%f), "
                 "ClientCfg.DestThreshold(%f), distToLimiter(%f), "
                 "ClientCfg.PositionLimiterRadius(%f)",
                 _Slot, distToFirst, ClientCfg.DestThreshold, distToLimiter,
                 ClientCfg.PositionLimiterRadius);
      }
    }
    ...
    // Compute difference in Game Cycle Between the current Game Cycle and the
    // current Stage.
    sint32 t;
    if (ClientCfg.StageLCTUsage == CClientConfig::StageUseAllLCT)
      ...
      if (stageForceLCTFound) {
        ...
        /* minimize the LCT impact with the current char one
                Hence, if the stage had a lct impact lowered, but LCT decrease
           was canceled, the stage will keep its LCT impact
        */
    ...
    // Compute the estimated Time for the Stage.
    stage.time(
        (double)(NetMngr.getMachineTimeAtTick() + t * NetMngr.getMsPerTick()) /
        1000.0);

    // *** Important step is used for "Panic mode" animation acceleration. skip
    // the first stage
    if (_ImportantStepTime == 0.0 && it != _Stages._StageSet.begin()) {
      // Important steps are ones that takes times (pos, orientation, mode,
      // animation etc....)
    ...
    if ((_IsThereAMode == false) && hasPos) {
      ...
      _DestTime =
          stage.time() +
          (double)(stage.predictedInterval() * NetMngr.getMsPerTick()) / 1000.0;
      // Update First Pos.
      if (dist2Dest() == INVALID_DIST) {
        _FirstPos = _DestPos;
        _FirstTime = stage.time();
        // Compute the distance to the first position
        double distToFirst = (CVectorD(_DestPos.x, _DestPos.y, 0.0) -
                              CVectorD(posTmp.x, posTmp.y, 0.0))
                                 .norm();
        // Set the Distance to the Destination as the distance to the first
        // position.
      ...
      // Increase distance to destination.
        ...
        dist2Dest(dist2Dest() + (CVectorD(_DestPos.x, _DestPos.y, 0.0) -
                                 CVectorD(posTmp.x, posTmp.y, 0.0))
                                    .norm());
      // Backup the last pos.
      posTmp = _DestPos;
    ...
    itTmp = it;
    ++it;
    // REMOVE EMPTY STAGE (because only position and the same as the current
    // one).
    if (stage.empty())
      _Stages._StageSet.erase(itTmp);
  }
  ...
  // It must usually be the theorical one except for some mode used only by the
  // client like SWIM
  if (!_IsThereAMode)
    _ModeWanted = _Mode;
  ...
  if (somePosFoundEarly) {
    ...
    // else try to compute the first pos, WIHTOUT regarding if there is a mode
    // or not (because even mode anim can be accelerated....)
    else {
      it = _Stages._StageSet.begin();
      while (it != _Stages._StageSet.end()) {
        CStage &stage = (*it).second;
        ...
        if (stage.getPos(firstPos)) {
          fpTime = stage.time() + (double)(stage.predictedInterval() *
                                           NetMngr.getMsPerTick()) /
                                      1000.0;
          d2fp =
              CVectorD(firstPos.x - pos().x, firstPos.y - pos().y, 0.0).norm();
          break;
        }
        it++;
      }
    ...
    // with d2fp, fpTime, and maxSpeed, we can estimate the moment where the run
    // should start
    if (fpTime != INVALID_TIME) {
      float maxSpeed = (float)getMaxSpeed();
      if (maxSpeed > 0) {
        // compute at which time the first move should begin so it doesn't have
        // to accelerate
...
// \param timeFactor : when to activate the impact if there is no track (value
// has to be between 0 and 1 to be valid). \return bool : true if the trigger is
// valid. \warning This method does not check if the animation is Null.
//-----------------------------------------------
ADD_METHOD(bool CCharacterCL::beginImpact(NL3D::UAnimation *anim,
                                          NL3D::TAnimationTime currentTime,
                                          const std::string &triggerName,
                                          bool &isActive, float timeFactor))
...
// \param startTime : time to start processing events from the current
// animation. \param stopTime : time to stop processing events from the current
// animation. \todo GUIGUI : Optimize FXs launch when we would have time
//-----------------------------------------------
void CCharacterCL::animEventsProcessing(double startTime, double stopTime) {
  if (_CurrentState == 0)
    return;

  // \todo Vianney : temp le temps de savoir comment on joue les son pour le
  // propre joueur No sound for the player.
  if (_Slot != 0 || _Mode != MBEHAV::NORMAL) {
    ...
    if (sndMngr && (_SoundId[MOVE] != CSoundAnimationNoId)) {
      ...
      sndMngr->playAnimation(_SoundId[MOVE], (float)startTime, (float)stopTime,
                             pcluster, _SoundContext);
    }
  }
...
//-----------------------------------------------
void CCharacterCL::updatePreCollision(const TTime &currentTimeInMs,
                                      CEntityCL *target) // virtual
{
  ...
  if (_Mode == MBEHAV::COMBAT_FLOAT) {
    ...
    if (targetSlot() != CLFECOMMON::INVALID_SLOT && targetSlot() != slot() &&
        target)
...
//-----------------------------------------------
inline void CCharacterCL::updateFX() { updateAttachedFX(); } // updateFX //
...
void CCharacterCL::updateAttachedFX() {
  ...
  std::list<CAttachedFX::CBuildInfo>::iterator itAttachedFxToStart =
      _AttachedFXListToStart.begin();
  while (itAttachedFxToStart != _AttachedFXListToStart.end()) {
    if ((*itAttachedFxToStart).DelayBeforeStart <
        (float)(TimeInSec - (*itAttachedFxToStart).StartTime)) {
  ...
  while (itAttachedFx != _AttachedFXListForCurrentAnim.end()) {
    ...
    if (!attachedFX.FX.empty())
      attachedFX.FX.setUserMatrix(alignMatrix);
    ++itAttachedFx;
  }
  ...
  while (itAttachedFx != _AttachedFXListToRemove.end()) {
    ...
    if (attachedFX.SpawnTime != TimeInSec) {
      if (attachedFX.FX.empty() || !attachedFX.FX.isSystemPresent() ||
          !attachedFX.FX.isValid()) {
        mustDelete = true;
      }
    }
  ...
  for (uint k = 0; k < MaxNumAura; ++k) {
    if (_AuraFX[k]) {
      if (_AuraFX[k]->TimeOutDate !=
          0.f) // we use that flag to mark the aura as 'shutting down'
      {
        if (TimeInSec >= _AuraFX[k]->TimeOutDate) {
          _AuraFX[k] = NULL;
        } else {
          float lifeRatio = (float)((_AuraFX[k]->TimeOutDate - TimeInSec) /
                                    AURA_SHUTDOWN_TIME);
          if (!_AuraFX[k]->FX.empty())
            _AuraFX[k]->FX.setUserParam(0, 1.f - lifeRatio);
        }
      }
...
// updateVisible :
//-----------------------------------------------
void CCharacterCL::updateVisible(const TTime &currentTimeInMs,
                                 CEntityCL *target) {
  ...
  // Apply the new entity position to the visual of the entity (apply x and z
  // movement due to animation).
...
// updateSomeClipped :
//-----------------------------------------------
void CCharacterCL::updateSomeClipped(const TTime &currentTimeInMs,
                                     CEntityCL *target) {
  // Snap the entity to the ground.
  {
    H_AUTO(RZ_Client_Entity_CL_Update_Snap_To_Ground)
...
// updateClipped :
//-----------------------------------------------
void CCharacterCL::updateClipped(const TTime &currentTimeInMs,
                                 CEntityCL *target) {
...
//-----------------------------------------------
void CCharacterCL::updateVisiblePostPos(const NLMISC::TTime &currentTimeInMs,
                                        CEntityCL *target) // virtual
{
    ...
    if (_InSceneUserInterface) {
      ...
      if (showIS) {
        ...
        if (getNamePos(pos)) {
          ...
          if ((isValidDouble(pos.x) && isValidDouble(pos.y) &&
               isValidDouble(pos.z)) == false) {
            nlwarning("CH:updateVisiblePostPos:%d: invalid pos %f %f %f", _Slot,
                      pos.x, pos.y, pos.z);
            nlstop;
          }
          ...
          pos.z = box().getMax().z;
          nlassert(isValidDouble(pos.x) && isValidDouble(pos.y) &&
                   isValidDouble(pos.z));
          _InSceneUserInterface->Position = pos;
        }
      }
...
// \parem target : pointer on the current entity target.
//-----------------------------------------------
void CCharacterCL::updatePostCollision(const TTime & /* currentTimeInMs */,
                                       CEntityCL * /* target */) // virtual
{
  ...
  // Finalize PACS position
  {
    ...
    pacsFinalizeMove();
    // \todo GUIGUI : fait rapidement pour voir les autres se baigner pour la
    // video, faire mieux.

    // changed : Malkav , also do this for mektoub (as they can swim)
    if (PACS && _Primitive &&
        (isPlayer() || isNPC() ||
         (_Sheet && (_Sheet->Race == EGSPD::CPeople::MektoubMount ||
                     _Sheet->Race == EGSPD::CPeople::MektoubPacker)))) {
      // Is in water ?
      if (GR) {
        ...
        if (GR->isWaterPosition(gPos, waterHeight)) {
          if (isSwimming() == false) {
            if (isDead()) {
              ...
              if (_Mount != CLFECOMMON::INVALID_SLOT) {
                CCharacterCL *mount =
                    dynamic_cast<CCharacterCL *>(EntitiesMngr.entity(_Mount));
                if (mount) {
                  // Set the mount.
...
// getTheMove :
//-----------------------------------------------
double CCharacterCL::getTheMove(double loopTimeStep, double oldMovingTimeOffset,
                                double oldMovingTimeOffsetRun) const {
  double move;
  if (_CurrentState) {
    // A Real Move State
    if (_CurrentState->Move) {
      ...
      if (runFactor() > 0.0) {
        ...
        move =
            move * (1.0 - runFactor()) +
            getTheMove(loopTimeStep, oldMovingTimeOffsetRun, MOVE_BLEND_OUT) *
                runFactor();
...
double CCharacterCL::getTheMove(double loopTimeStep, double oldMovingTimeOffset,
  ...
  if (animIndex(channel) == CAnimation::UnknownAnim) {
    ...
    if (offsetT <= 0.0) {
      // \todo GUIGUI : in this case, 'loopTimeStep' should not decrease so FIX
      // IT.
      move = dist2Dest();
    } else {
...
//-----------------------------------------------
void CCharacterCL::updatePosCombatFloat(double /* frameTimeRemaining */,
                                        CEntityCL *target) // virtual
{
  ...
  // The target is valid
  if (target) {
    // Get the position where the attacker should go to attack his target
    ...
    if (ClientCfg.Local ||
      ...
      _DestPos = target->getAttackerPos(angToTarget,
                                        attackRadius() + ClientCfg.AttackDist);
    } else
      _DestPos = target->getAttackerPos(_TargetAngle,
                                        attackRadius() + ClientCfg.AttackDist);
    ...
    if (vectToDest.norm() > ClientCfg.DestThreshold) {
      ...
      _FirstPos = _DestPos;
      _DestTime =
          _LastFrameTime + lengthOfTimeToDest + ClientCfg.ChaseReactionTime;
      _FirstTime = _DestTime;
      /*
                              // The time remaining will be enough to reach the
         destination if(frameTimeRemaining >= lengthOfTimeToDest)
                              {
                                      _FirstPos	= _DestPos;
                                      _DestTime	= _LastFrameTime +
         lengthOfTimeToDest + ClientCfg.ChaseReactionTime; _FirstTime	=
         _DestTime;
                              }
                              // The time remaining is not enough to reach the
         destination at max speed -> compute a first pos possible to reach. else
                              {
                                      _FirstPos	= pos() +
         vectToDest*frameTimeRemaining/lengthOfTimeToDest; _DestTime	=
         _LastFrameTime + lengthOfTimeToDest + ClientCfg.ChaseReactionTime;
                                      _FirstTime	= _LastFrameTime +
         frameTimeRemaining + ClientCfg.ChaseReactionTime;
                              }
      */
      // Compute the distance to the first position.
...
// \todo GUIGUI : compute it when receiving a new stage instead of every frame
// (should be faster). \todo GUIGUI: recompute distance to destination even if
// the Stage not reached.
//-----------------------------------------------
ADD_METHOD(void CCharacterCL::updatePos(const TTime &currentTimeInMs,
                                        CEntityCL *target))
_OldAutomaton = _CurrentAutomaton;
...
if (_LodCharacterAnimEnabled) {
  ...
  _LodCharacterAnimTimeOffset +=
      DT * _PlayList->getSpeedFactor(_LodCharacterMasterAnimSlot);
}
// BLEND
if (_PlayList) {
  ...
  if (isRiding() || ClientCfg.BlendFrameNumber == 0 || _BlendRemaining <= 0) {
    ...
    if (runFactor() < 0.5 || (_CurrentAnimSet[MOVE_BLEND_OUT] == 0)) {
      if (_CurrentAnimSet[MOVE]) {
        ...
        animIndex(ACTION,
                  animIndex(MOVE)); // This also call "animId" and set it.
        ...
        animIndex(
            ACTION,
            CAnimation::UnknownAnim); // This also call "animId" and set it.
      ...
      animIndex(
          ACTION,
          animIndex(MOVE_BLEND_OUT)); // This also call "animId" and set it.
...
while (frameTimeRemaining > 0) {
  ...
  if (antiFreezeCounter > 50) {
    /*
                            nlwarning("CH:updatePos:antiFreeze:%d:
       frameTimeRemaining '%f'",  _Slot, frameTimeRemaining);
                            nlwarning("CH:updatePos:antiFreeze:%d: Automaton
       '%s'",           _Slot, _CurrentAutomaton.c_str());
                            nlwarning("CH:updatePos:antiFreeze:%d: _IsThereAMode
       '%s'",       _Slot, _IsThereAMode?"true":"false");
                            nlwarning("CH:updatePos:antiFreeze:%d: dist2Dest
       '%f'",           _Slot, dist2Dest());
                            nlwarning("CH:updatePos:antiFreeze:%d: Mode
       '%s(%d)'",            _Slot, modeToString(_Mode).c_str(), _Mode);
                            nlwarning("CH:updatePos:antiFreeze:%d: Mode Wanted
       '%s(%d)'",     _Slot, modeToString(_ModeWanted).c_str(), _ModeWanted);
                            nlwarning("CH:updatePos:antiFreeze:%d: Anim State
       Move '%s(%d)'", _Slot,
       CAnimationState::getAnimationStateName(animState(MOVE)).c_str(),
       animState(MOVE));
    */
  ...
  // Update Stages
  updateStages();
  // \todo GUIGUI : Bug with _TargetAngle in fight float, we overwrite here
  // angle sent by the server ? If the entity is too far (orientation not
  // received yet), set the front vector as the moving direction.
  ...
  if (loopTimeStep > checkLoopTimeStep) {
    nlwarning("CH:updtPos:%d: loopTimeStep(%f) > checkLoopTimeStep(%f).", _Slot,
  ...
  // -- END CHECK -- //
  /////////////////////
  // (DEBUG) : Backup the Animation Time Offset after the adjustment with end
  // anim to make some checks.
  ...
  // of the code in updateStage).
  if (dist2FirstPos() > 0.0) {
    H_AUTO(RZ_Client_Entity_CL_Update_Pos_dist2FirstPos_gt_0);

    // Get the covered distance from the animation.
    double move =
        getTheMove(loopTimeStep, oldMovingTimeOffset, oldMovingTimeOffsetRun);
    ...
    if (move >= dist2FirstPos()) // dist2FirstPos() > 0 -> move > 0.
    {
      ...
      if (loopTimeStep >
          checkLoopTimeStep) // prevent bugs because of the double's precision
        loopTimeStep = checkLoopTimeStep;
      if (loopTimeStep < 0.0)
        loopTimeStep = 0.0;
      // Update Animation Time Offset (move greater than the dist to next stage;
      // update animation time to get them equal).
      animOffset(MOVE, oldMovingTimeOffset +
                           (animOffset(MOVE) - oldMovingTimeOffset) * percent);
      animOffset(MOVE_BLEND_OUT,
                 oldMovingTimeOffsetRun +
                     (animOffset(MOVE_BLEND_OUT) - oldMovingTimeOffsetRun) *
                         percent);
      // \todo GUIGUI : check if the following line is necessary
      buLoopTimeStep = loopTimeStep;
      // First Position Reached
      pos(_FirstPos);
      dist2FirstPos(0.0); // Current entity position is now the same as the
                          // First position so dis is Null.
      // Complete the Stage.
      if (_Mode != MBEHAV::COMBAT_FLOAT || _IsThereAMode) {
        if (posInStage)
          stageReach = true;
    ...
    // Even if the movement is not enough to reach the first position, move the
    // entity to this position.
    else if (move > 0.0) {
  ...
  // If there is no position in the next stage and the stage should be done
  // already.
  if (!_Stages._StageSet.empty() && !posInStage && !stageReach &&
      !allToFirstPos && ((_LastFrameTime + loopTimeStep) >= stageTime)) {
  ...
  if (animOffset(MOVE) > backupAnimTimeOff) {
    nlwarning("CH:updtPos:%d: backupAnimTimeOff(%f) < AnimationsTimeOffset(%f) "
              "animLen(%f) -> animOffset(MOVE) = backupAnimTimeOff",
              _Slot, backupAnimTimeOff, animOffset(MOVE),
              EAM->getAnimationLength(animId(MOVE)));
    if (ClientCfg.Check)
      nlstop;
  ...
  if (loopTimeStep < 0.0) {
    nlwarning("CH:updtPos:%d: loopTimeStep(%f) < 0 -> loopTimeStep=0.0.", _Slot,
              loopTimeStep);
    if (ClientCfg.Check)
      nlstop;
    loopTimeStep = 0.0;
  ...
  if (loopTimeStep > frameTimeRemaining) {
    nlwarning("CH:updtPos:%d: loopTimeStep(%f) > frameTimeRemaining(%f) -> "
...
if (frameTimeRemaining < 0.0)
  nlwarning("CCharacterCL::updatePos : frameTimeRemaining(%f) < 0 ! This "
            "should never happen.",
            frameTimeRemaining);
/// (END DEBUG) ///
...
// Update the children display.
{
  ...
  while (it != _Children.end()) {
    itTmp = it;
    // Next Child (done before just in case child is detached during his
    // processFrame).
    it++;
      ...
      if (child) {
        if (!ClientCfg.Light) {
          ...
          if (animLength > 0.0) {
            ...
            if (factor > 1.0)
              factor = 1.0;
            double childTimeOffset =
                factor * EAM->getAnimationLength(child->animId(MOVE));
        ...
        child->pacsMove(
            pos()); // Move the child at the same position than the parent.
      } else
...
void CCharacterCL::updateVisiblePostRender() // virtual
{
  ...
  if (_HeadBoneId != -1 && !_Skeleton.empty()) {
    if (_Skeleton.getLastClippedState() &&
        _Skeleton.isBoneComputed(_HeadBoneId)) {
    ...
    if (mode != _Mode) {
      ...
      if ((_Mode == MBEHAV::MOUNT_NORMAL || _Mode == MBEHAV::MOUNT_SWIM) &&
        ...
        if (stage.getPos(_OldPos)) {
          ...
          nlwarning("CH:processStage:%d: The stage should have a position with "
                    "the mode.",
                    _Slot);
      }
...
void CCharacterCL::updateBlink(const TTime &currentTimeInMs) {
  ...
  if ((gender == GSGENDER::male) || (gender == GSGENDER::female)) {
    ...
    bool validTime = (_NextBlinkTime + blinkTime >= currentTimeInMs) &&
                     (_NextBlinkTime <= (currentTimeInMs + maxBlinkLength));
    ...
    if (blinkEnd || !validTime) {
      ...
      // Compute next time
      _NextBlinkTime = (TTime)(((double)rand() / (double)RAND_MAX) *
                                   (maxBlinkLength - minBlinkLength) +
                               minBlinkLength + (double)currentTimeInMs);
    } else {
      // Blink time ?
...
// \todo GUIGUI : put this method 'virtual' to have a different code for the
// user (no playlist). \todo GUIGUI : manage the parent better.
//---------------------------------------------------
ADD_METHOD(void CCharacterCL::updateDisplay(CEntityCL *parent))
// Animable ?
if (_PlayList) {
  // Reverse the animation if needed.
  const double animOffsetMOV =
      _AnimReversed[MOVE]
          ? EAM->getAnimationLength(animId(MOVE)) - animOffset(MOVE)
          : animOffset(MOVE);
  const double animOffsetACT =
      _AnimReversed[ACTION]
          ? EAM->getAnimationLength(animId(ACTION)) - animOffset(ACTION)
          : animOffset(ACTION);
  const double animOffsetBLE =
      _AnimReversed[MOVE_BLEND_OUT]
          ? EAM->getAnimationLength(animId(MOVE_BLEND_OUT)) -
                animOffset(MOVE_BLEND_OUT)
          : animOffset(MOVE_BLEND_OUT);
  // Update Speed Factor
  ...
  if (animIndex(MOVE) != CAnimation::UnknownAnim) {
    // POSITION //
    // Get the 3D position for the current time in the animation (Vector Null if
    // animation has no move).
    CVector currentAnimPos;
    if (!EAM->interpolate(animId(MOVE), animOffsetMOV, currentAnimPos))
      currentAnimPos = CVector::Null;
    else {
      // If the current animation state is a move, do not take the Y move in the
      // animation because it's the code that compute the move.
      if (_CurrentState && _CurrentState->Move) {
        CVector currentAnimPosStart;
        if (!EAM->interpolate(animId(MOVE), 0.0, currentAnimPosStart))
          currentAnimPosStart = CVector::Null;
        if (_CurrentState->XFactor)
          currentAnimPos.x = currentAnimPosStart.x;
        if (_CurrentState->YFactor)
          currentAnimPos.y = currentAnimPosStart.y;
        if (_CurrentState->ZFactor)
          currentAnimPos.z = currentAnimPosStart.z;
      }
      // Scale the animation with the Character Scale Pos if the animation need
      // it.
      {
    ...
    if (runFactor() > 0.0) {
      ...
      if (!EAM->interpolate(animId(MOVE_BLEND_OUT), animOffsetBLE,
                            currentAnimPosRun))
        currentAnimPosRun = CVector::Null;
      else {
        // If the current animation state is a move, do not take the Y move in
        // the animation because it's the code that compute the move.
        if (_CurrentState && _CurrentState->Move) {
          CVector currentAnimPosStart;
          if (!EAM->interpolate(animId(MOVE_BLEND_OUT), 0.0,
                                currentAnimPosStart))
            currentAnimPosStart = CVector::Null;
          if (_CurrentState->XFactor)
            currentAnimPosRun.x = currentAnimPosStart.x;
          if (_CurrentState->YFactor)
            currentAnimPosRun.y = currentAnimPosStart.y;
          if (_CurrentState->ZFactor)
            currentAnimPosRun.z = currentAnimPosStart.z;
        }
        // Scale it by the CharacterScalePos, if needed, according to the
        // animation.
        bool applyCharacterScalePosFactor = true;
        ...
        if (animStatePtr) {
          const CAnimation *anim =
              animStatePtr->getAnimation(animIndex(MOVE_BLEND_OUT));
          if (anim)
            applyCharacterScalePosFactor = anim->applyCharacterScalePosFactor();
        }
        // scale it.
      ...
      currentAnimPos = currentAnimPos * (float)(1.0 - runFactor()) +
                       currentAnimPosRun * (float)runFactor();
    }
    // ROTATION //
    // Get the rotation for the current time in the animation (Rotation Null if
    // animation has no rotation).
    ...
    if (!EAM->interpolate(animId(MOVE), animOffsetMOV, currentAnimRot))
      ...
      if (parent == 0 && _CurrentState && _CurrentState->Rotation &&
          _RotationFactor != -1.0) {
        // Get the Rotation at the beginning of the animation.
        CQuat currentAnimRotStart;
        if (!EAM->interpolate(_PlayList->getAnimation(MOVE), 0.0,
                              currentAnimRotStart))
        ...
        if (!EAM->interpolate(_PlayList->getAnimation(MOVE), animLength,
                              currentAnimRotEnd))
        ...
        currentAnimRot = applyRotationFactor(
            currentAnimRot, (float)_RotationFactor, currentAnimRotStart,
            currentAnimRotEnd, (float)(animOffsetMOV / animLength));
        ...
        //// OLD ////
        /*
                                                // Get the Rotation at the
        beginning of the animation. CQuat currentAnimRotStart;
                                                if(!EAM->interpolate(_PlayList->getAnimation(MOVE),
        0.0, currentAnimRotStart)) currentAnimRotStart = CQuat::Identity;

                                                // Find the closest quat.
        //
        currentAnimRotStart.makeClosest(currentAnimRot);

                                                // Get the angle done by the
        animation from the beginning CQuat rotStartTmp = currentAnimRotStart;
                                                rotStartTmp.invert();
                                                CQuat rotTmp =  rotStartTmp *
        currentAnimRot; float ang = rotTmp.getAngle();

                                                // Get the Rotation scaled.
                                                currentAnimRot =
        CQuat::slerp(currentAnimRotStart, currentAnimRot,
        (float)_RotationFactor);

                                                // Get the angle done once
        scaled. rotTmp =  rotStartTmp * currentAnimRot; CMatrix rotMat;
                                                rotMat.identity();
                                                rotMat.rotateZ(_CurrentState->RotFactor*(ang-rotTmp.getAngle()));

                                                // Apply the scaled rotation to
        the position. currentAnimPos = rotMat*currentAnimPos;
        */
        //// FIN OLD ////
      }
    }
    // Blend Walk/Run
    if (runFactor() > 0.0) {
      // Get the rotation for the current time in the animation (Rotation Null
      // if animation has no rotation).
      ...
      currentAnimRot =
          CQuat::slerp(currentAnimRot, currentAnimRotRun, (float)runFactor());
    }
    ...
    if (ClientCfg.BlendFrameNumber && _BlendRemaining > 0) {
      ...
      tmpQuat = CQuat::slerp(
          current.getRot(), _OldRotQuat,
          ((float)_BlendRemaining / (float)(ClientCfg.BlendFrameNumber + 1)));
    ...
    // Set the instance position and rotation.
      ...
      if (!once) {
        nlwarning(
            "CH::updtDisp:%d: no instance nor skeleton. Sheet Id '%d(%s)'.",
            _Slot, _SheetId.asInt(), _SheetId.toString().c_str());
        once = true;
      }
    }
...
void CCharacterCL::updateHeadDirection(CEntityCL *target) {
  ...
  if (_TargetSlot != CLFECOMMON::INVALID_SLOT && _TargetSlot != _Slot) {
    ...
    if (target != 0) {
      ...
      if ((fabs(vectDist.x) + fabs(vectDist.y)) <=
              ClientCfg.MaxHeadTargetDist &&
          vectDist != CVectorD::Null) {
...
void CCharacterCL::displayName() {
  ...
  if (!_Skeleton.empty() && !ClientCfg.Light) {
    ...
    if (_Skeleton.isDisplayedAsLodCharacter())
      return;
    // If the entity was not displayed last frame (clipped) -> do not display
    // the name.
    if (!_Skeleton.getLastClippedState())
      return;
    ...
    if (_NameBoneId != -1)
      namePos =
          _Skeleton.getBone(_NameBoneId).getLastWorldMatrixComputed().getPos();
...
void CCharacterCL::drawName(const NLMISC::CMatrix &mat) // virtual
{
  ...
  if (!getEntityName().empty()) {
    ...
    // If there is an extended name, display the extended name at the name place
    // and the name above.
    else {
      ...
      CVector v = mat.getPos() + mat.getK().normed() * ClientCfg.NamePos *
                                     mat.getScaleUniform();
      mat2.setPos(v);
...
void CCharacterCL::displayModifiers() // virtual
{
  ...
  while (it != _HPDisplayed.end()) {
    ...
    if (TimeInSec > (mod.Time + totalDuration)) {
      ...
      // for character, keep the deltaZ the first time it is displayed, and
      // apply the same each frame (avoid Z movement of the flying text because
      // of animation)
      ...
      float dynT =
          sqrtf((float)t /
                totalDuration); // a sqrt just so it looks much more "jumpy"
      ...
      if (t < noFadeDuration)
        ...
        mod.Color.A =
            255 - (uint8)((t - noFadeDuration) * 255.0 / fadeDuration);

      // Display the hp modifier. display with a X offset according if user or
      // not, for more readability
      sint deltaX = -pIM->FlyingTextManager.getOffsetXForCharacter();
      if (UserEntity && UserEntity->slot() == slot())
        deltaX *= -1;
      pIM->FlyingTextManager.addFlyingText(&mod, hpModifier, pos, mod.Color,
                                           scale, deltaX);
...
void CCharacterCL::drawBox() // virtual
{
  ...
  if (_Primitive)
    _Primitive->getGlobalPosition(gPos, dynamicWI);
  ::drawBox(PACSBox.getMin(), PACSBox.getMax(),
            ((gPos.InstanceId == -1) && (T1 % 1000) > 500)
                ? CRGBA(255, 0, 0)
                : CRGBA(0, 250, 250));
  ...
  if (!ClientCfg.Light) {
    ...
    clipPos.z +=
        _ClipDeltaZ -
        _Aabbox.getHalfSize().z; // _ClipDeltaZ is relative to pos on ground
...
const NLMISC::CAABBox &CCharacterCL::selectBox() // virtual
{
  ...
  if (_LastSelectBoxComputeTime < T1) {
    ...
    if (!_Skeleton.empty()) {
      // Don't compute if in LOD form (else flick because sometimes valid
      // because of shadow animation)
      if (!_Skeleton.isDisplayedAsLodCharacter() &&
    ...
    // else compute from static bot object
      ...
      if (!inst.empty()) {
        ...
        if (bbox.getHalfSize() != CVector::Null) {
          ...
          _SelectBox =
              CAABBox::transformAABBox(inst.getLastWorldMatrixComputed(), bbox);
          found = true;
        }
      }
    }
...
//---------------------------------------------------
void CCharacterCL::updateAttachedFXListForSlotRemoved(
    std::list<CAttachedFX::TSmartPtr> &fxList,
    const CLFECOMMON::TCLEntityId &slotRemoved) {
...
void CCharacterCL::slotRemoved(const CLFECOMMON::TCLEntityId &slotRemoved) {
  ...
  updateAttachedFXListForSlotRemoved(_AttachedFXListForCurrentAnim,
                                     slotRemoved);
...
//---------------------------------------------------
// attackRadius :
// Method to return the attack radius of an entity (take the scale into
// account).
//---------------------------------------------------
...
//---------------------------------------------------
// getAttackerPos :
// Return the position the attacker should have to combat according to the
// attack angle. \param ang : 0 = the front, >0 and <Pi = left side, <0 and >-Pi
// = right side. \todo : GUIGUI precalculate entity matrix
//---------------------------------------------------
CVectorD CCharacterCL::getAttackerPos(double ang, double dist) const {
  ...
  p.x = getScale() * distToSide * sin(-ang) +
        dist * sin(-ang); // or: pos.x = _Sheet->DistToSide*cos(ang) +
                          // dist*cos(ang); but 0 should be right side.
  p.y = dist * cos(ang);
  if (fabs(ang) <= Pi / 2.0)
...
//---------------------------------------------------
bool CCharacterCL::isPlacedToFight(const NLMISC::CVectorD &posAtk,
                                   const NLMISC::CVector &dirAtk,
                                   double attackerRadius) const // virtual
{
...
bool CCharacterCL::getNamePos(CVectorD &pos) // virtual
{
  ...
  // If the entity was not displayed last frame (clipped) -> do not display the
  // name.
  if (!_Skeleton.getLastClippedState())
    return false;
  ...
  if (namePosZ == 0.f) {
    ...
    if (_Skeleton.isDisplayedAsLodCharacter()) {
      ...
      if (_NameCLodDeltaZ == NameCLodDeltaZNotComputed) {
        ...
        float boneZ = _Skeleton.getBone(_NameBoneId)
  ...
  const float boneZ =
      _Skeleton.getBone(_NameBoneId).getLastWorldMatrixComputed().getPos().z;
  ...
  if (deltaNamePosZ != 0.f) {
    ...
    if (deltaBonePosZ != 0.f &&
        (deltaBonePosZ > 0.f) == (deltaNamePosZ > 0.f)) {
...
bool CCharacterCL::getChestPos(CVectorD &pos) const // virtual
{
  ...
  pos.y = this->pos().y;
  pos.z =
      _Skeleton.getBone(_ChestBoneId).getLastWorldMatrixComputed().getPos().z;
  return true;
} // getChestPos //
...
float CCharacterCL::getScale() const // virtual
{
  switch (_OwnerPeople) {
  case MOUNT_PEOPLE::Fyros:
    return getSheetScale() * ClientCfg.FyrosScale * _CustomScale;
  case MOUNT_PEOPLE::Matis:
    return getSheetScale() * ClientCfg.MatisScale * _CustomScale;
  case MOUNT_PEOPLE::Tryker:
    return getSheetScale() * ClientCfg.TrykerScale * _CustomScale;
  case MOUNT_PEOPLE::Zorai:
    return getSheetScale() * ClientCfg.ZoraiScale * _CustomScale;
  default:
...
//---------------------------------------------------
std::string
CCharacterCL::currentAnimationSetName(TAnimationType animType) const {
...
//---------------------------------------------------
uint32 CCharacterCL::createItemInstance(const CItemSheet &itemSheet,
                                        uint32 instIdx,
                                        SLOTTYPE::EVisualSlot visualSlot,
                                        const string &bindBone, sint8 texture,
                                        sint color) {
  uint32 idx = CEntityCL::BadIndex;
  ...
  if (!shape.empty()) {
    ...
    if (shape != "none.shape") {
      ...
      if (!instance.empty()) {
        ...
        nlwarning("CH:createItemInstance: cannot create the instance for the "
                  "shape '%s'.",
                  shape.c_str());
    }
  } else
...
//---------------------------------------------------
ADD_METHOD(void CCharacterCL::displayDebug(float x, float &y,
                                           float lineStep)) // virtual
CInterfaceManager *IM = CInterfaceManager::getInstance();

CEntityCL::displayDebug(x, y, lineStep);
// Mode Wanted
// Display the Target Mode.
TextContext->printfAt(x, y, "Mode Wanted: %d(%s)", (sint)_ModeWanted,
                      MBEHAV::modeToString(_ModeWanted).c_str());
y += lineStep;
// Stage Remaining
...
y += lineStep;
// Current Speed
TextContext->printfAt(x, y,
                      "Speed: %f (_DestTime(%f) - _LastFrameTime(%f)) = %f",
                      speed(), _DestTime, _LastFrameTime,
                      _DestTime - _LastFrameTime);
y += lineStep;
...
TextContext->printfAt(x, y, "Current Animation: %s(%u)(%lf)(%u loops)",
                      animId(MOVE) == std::numeric_limits<uint>::max()
                          ? "[NONE]"
                          : currentAnimationName().c_str(),
                      animId(MOVE), animOffset(MOVE), _NbLoopAnim);
y += lineStep;
// First Pos
if (_First_Pos)
...
if (_Primitive) {
  ...
  TextContext->printfAt(x, y, "Prim Pos: %f %f %f", primFinalPos.x,
                        primFinalPos.y, primFinalPos.z);
  y += lineStep;
}
// Skeleton Ptr, Animset Ptr and Current State Ptr
TextContext->printfAt(x, y, "Skel Ptr: %p - AnimSet Ptr: %p - State Ptr: %p",
                      &_Skeleton, _CurrentAnimSet[MOVE], _CurrentState);
y += lineStep;
// Display the target mount and rider.
TextContext->printfAt(
    x, y, "Mount: %3u(Theoretical: %3u) Rider: %3u(Theoretical: %3u)", mount(),
    _TheoreticalMount, rider(), _TheoreticalRider);
y += lineStep;
// VPA
sint64 prop = NLGUI::CDBManager::getInstance()
                  ->getDbProp("SERVER:Entities:E" + toString("%d", _Slot) +
...
if (isPlayer() || isUser()) {
  ...
  TextContext->printfAt(
      x, y,
      "VPA: %" NL_I64
      "X : Chest(%d,%d) Legs(%d,%d) Arms(%d,%d) Hat(%d,%d) RH(%d) LH(%d)",
      prop, (uint)visualA.PropertySubData.JacketModel,
      (uint)visualA.PropertySubData.JacketColor,
      (uint)visualA.PropertySubData.TrouserModel,
      (uint)visualA.PropertySubData.TrouserColor,
      (uint)visualA.PropertySubData.ArmModel,
      (uint)visualA.PropertySubData.ArmColor,
      (uint)visualA.PropertySubData.HatModel,
      (uint)visualA.PropertySubData.HatColor,
...
y += lineStep;
// VPB
prop = NLGUI::CDBManager::getInstance()
           ->getDbProp("SERVER:Entities:E" + toString("%d", _Slot) + ":P" +
                       toString("%d", CLFECOMMON::PROPERTY_VPB))
           ->getValue64();
if (isPlayer() || isUser()) {
  SPropVisualB visualB = *(SPropVisualB *)(&prop);
  TextContext->printfAt(x, y, "VPB: %" NL_I64 "X : Hands(%d,%d) Feet(%d,%d).",
                        prop, (uint)visualB.PropertySubData.HandsModel,
                        (uint)visualB.PropertySubData.HandsColor,
                        (uint)visualB.PropertySubData.FeetModel,
                        (uint)visualB.PropertySubData.FeetColor);
} else
...
prop = NLGUI::CDBManager::getInstance()
           ->getDbProp("SERVER:Entities:E" + toString("%d", _Slot) + ":P" +
                       toString("%d", CLFECOMMON::PROPERTY_VPC))
           ->getValue64();
if (isPlayer() || isUser()) {
  SPropVisualC visualC = *(SPropVisualC *)(&prop);
  TextContext->printfAt(x, y, "VPC: %" NL_I64 "X : EyesColor(%d) Tattoo(%d).",
                        prop, visualC.PropertySubData.EyesColor,
                        visualC.PropertySubData.Tattoo);
} else
...
//-----------------------------------------------
void CCharacterCL::displayDebugPropertyStages(float x, float &y,
                                              float lineStep) {
  CStageSet::TStageSet::iterator it = _Stages._StageSet.begin();
  for (; it != _Stages._StageSet.end(); it++) {
    ...
    for (uint i = 0; i < CLFECOMMON::NB_VISUAL_PROPERTIES; i++) {
      if (i != CLFECOMMON::PROPERTY_POSY && i != CLFECOMMON::PROPERTY_POSZ &&
          stage.isPresent(i))
...
void CCharacterCL::readWrite(NLMISC::IStream &f) {
  ...
  //_SoundContext; 	std::vector<double>
  //_AnimationsTimeOffset; 	std::vector<CAnimationState::TAnimStateId>
  //_AnimationsStateKey; 	const CAnimationSet
  //*_CurrentAnimSet;
  f.serial(_LastFrameTime);
  f.serial(_LodCharacterAnimEnabled);
  f.serial(_LodCharacterAnimTimeOffset);
  //	uint
  //_LodCharacterMasterAnimSlot;
  ...
  //	GSGENDER::EGender
  //_Gender; 	sint
  //_NameBoneId; 	NL3D::UTransform
  //_NameTransform; 	std::vector<CItemSheet *>
  //_Items;
  f.serial(_HeadIdx);
  f.serial(_FaceIdx);
  //	sint	_HeadBoneId;
  ...
  //	const CAutomatonStateSheet
  //*_CurrentState; 	MBEHAV::EMode
  //_ModeWanted; 	sint
  //_BlendRemaining;
  ...
  //	std::list<UParticleSystemInstance>
  //_CurrentAnimFXList; 	std::list<UParticleSystemInstance>
  //_RemoveAnimFXList; 	NL3D::UParticleSystemInstance
  //_CurrentAnimFX;
  ...
  if (_LookRdy) {
    ...
    sint64 prop =
        NLGUI::CDBManager::getInstance()
            ->getDbProp("SERVER:Entities:E" + toString("%d", _Slot) + ":P" +
                        toString("%d", CLFECOMMON::PROPERTY_VPA))
            ->getValue64();
...
void CCharacterCL::buildPlaylist() {
  ...
  // Get the right animation state and choose an animation.
  {
    ...
    if (animationState) {
      // Choose the animation
      animIndex(MOVE, animationState->chooseAnim(_AnimJobSpecialisation,
...
void CCharacterCL::dist2FirstPos(double d2FP) {
  CHECK((d2FP == INVALID_DIST) || (d2FP == 0.0) ||
        (d2FP >= ClientCfg.DestThreshold));
  _DistToFirst = d2FP;
} // dist2FirstPos //
//-----------------------------------------------
// dist2FirstPos :
...
double CCharacterCL::dist2FirstPos() const {
  CHECK((_DistToFirst == INVALID_DIST) || (_DistToFirst == 0.0) ||
        (_DistToFirst >= ClientCfg.DestThreshold));
  return _DistToFirst;
} // dist2FirstPos //
...
void CCharacterCL::dist2Dest(double d2D) {
  CHECK((d2D == INVALID_DIST) || (d2D == 0.0) ||
        (d2D >= ClientCfg.DestThreshold));
  _DistToDest = d2D;
} // dist2Dest //
...
double CCharacterCL::dist2Dest() const {
  CHECK((_DistToDest == INVALID_DIST) || (_DistToDest == 0.0) ||
        (_DistToDest >= ClientCfg.DestThreshold));
  return _DistToDest;
} // dist2Dest //
...
{
  CHECK((_Speed == -1.0) || (_Speed == 0.0) ||
        ((_Speed >= 0.001) && (_Speed <= 1000.0)));
  return _Speed;
} // getSpeed //
...
//---------------------------------------------------
bool CCharacterCL::animationStateKey(TAnimationType channel,
                                     TAnimStateId value) {
  // Is the new key valid ?
  if (value == CAnimationStateSheet::UnknownState) {
    nlwarning("CH::animationStateKey: Char '%d': new state key is Null.",
              _Slot);
    return false;
  }
  // Set the new key.
  ...
  // Debug Animation for the selection
  if (VerboseAnimSelection && _Slot == UserEntity->selection())
    nlinfo("CH:animationStateKey:%d: state '%s'.", _Slot,
           CAnimationState::getAnimationStateName(value).c_str());
  //
  return true;
} // animationStateKey //
...
//-----------------------------------------------
void CCharacterCL::animIndex(TAnimationType channel,
                             CAnimation::TAnimId index) {
  // Check the channel
  CHECK((uint)channel < _AnimState.size());
  ...
  if (_AnimIndex[channel] == CAnimation::UnknownAnim)
    ...
    const CAnimationState *animStatePtr =
        _CurrentAnimSet[channel]->getAnimationState(
            (animState(channel) == CAnimationStateSheet::Emote)
                ? _SubStateKey
                : animState(channel));
    if (animStatePtr == 0)
      animId(channel, NL3D::UPlayList::empty);
...
//-----------------------------------------------
void CCharacterCL::alignFX(
    UParticleSystemInstance instance, float scale /* = 1.f */,
    const NLMISC::CVector &localOffset /*= NLMISC::CVector::Null*/) const {
...
void CCharacterCL::alignFX(
    UParticleSystemInstance instance, const CMatrix &matrix,
    float scale /*=1.f*/,
    const NLMISC::CVector &localOffset /*=NLMISC::CVector::Null*/) const {
  if (instance.empty())
    return;
...
void CCharacterCL::buildAlignMatrix(NLMISC::CMatrix &dest) const {
  // if not in clod, pelvis bone has been computed -> use it to get the current
  // orientation use the dir matrix otherwise
  /*CVector forward;
...
// attachFXInternal
//-----------------------------------------------
void CCharacterCL::attachFXInternal(const CAttachedFX::TSmartPtr fx,
...
void CCharacterCL::setAuraFX(uint index, const CAnimationFX *sheet) {
  ...
  // no-op if same aura
  if (_AuraFX[index] && _AuraFX[index]->AniFX == sheet)
    return;
  ...
  if (sheet == NULL) {
    ...
    // if there's already an aura attached, and if it is not already shutting
    // down
...
// ***********************************************************************************************************************
void CCharacterCL::setLinkFX(const CAnimationFX *fx,
                             const CAnimationFX *dispell) {
  // no-op if same link
  if (_LinkFX && _LinkFX->AniFX == fx)
    return;
  if (_LinkFX) {
    if (dispell) {
      CAttachedFX::TSmartPtr fx = new CAttachedFX;
...
void CCharacterCL::startItemAttackFXs(bool activateTrails, uint intensity) {
  ...
  for (uint k = 0; k < numItems; ++k) {
    _Items[k].startAttackFX(_Skeleton, intensity, (SLOTTYPE::EVisualSlot)k,
                            activateTrails);
  }
}
...
// isNeutral :
//-----------------------------------------------
bool CCharacterCL::isNeutral() const { return !isEnemy(); }

//-----------------------------------------------
// isFriend :
//-----------------------------------------------
bool CCharacterCL::isFriend() const { return !isEnemy(); }
...
// isAlly :
//-----------------------------------------------
bool CCharacterCL::isAlly() const { return this->isAnOutpostAlly(); }
...
// *********************************************************************************************
const char *CCharacterCL::getBoneNameFromBodyPart(BODY::TBodyPart part,
                                                  BODY::TSide side) const {
  if (!_Sheet)
    return CEntityCL::getBoneNameFromBodyPart(part, side);
...
// ***********************************************************************************************************************
void CCharacterCL::CWornItem::startAttackFX(NL3D::USkeleton skeleton,
                                            uint intensity,
                                            SLOTTYPE::EVisualSlot visualSlot,
                                            bool activateTrail) {
      ...
      if (stickPoint) {
        ...
        if (boneId != -1) {
          ...
          if (!instance.empty()) {
            ...
            if (!atk.empty()) {
              // set the user params
              static const float up[5][4] = {{0.f, 0.f, 0.f, 0.f},
                                             {1.f, 0.f, 0.f, 0.f},
                                             {1.f, 1.f, 0.f, 0.f},
                                             {1.f, 1.f, 1.f, 0.f},
                                             {1.f, 1.f, 1.f, 1.f}};
              ...
              atk.setRotEuler(fxSheet.AttackFXRot.x * degreeToRad,
                              fxSheet.AttackFXRot.y * degreeToRad,
                              fxSheet.AttackFXRot.z * degreeToRad);
...
// ***********************************************************************************************************************
void CCharacterCL::CWornItem::initFXs(SLOTTYPE::EVisualSlot /* visualSlot */,
  ...
  if (!shapeName.empty()) {
    ...
    // Initialize the remanence. Must substract the object matrix since
    // parent(parent)
...
void CCharacterCL::CWornItem::enableAdvantageFX(NL3D::UInstance parent) {
  ...
  if (!enabled) {
    // well, it is unlikely that player will loses its ability to master an item
    // after he gained it, but manage the case anyway.
    if (!AdvantageFX.removeByID(NELID("STOP")) &&
        !AdvantageFX.removeByID(NELID("main"))) {
...
void CCharacterCL::CWornItem::setTrailSize(uint size) {
  ...
  if (size == 0) {
    ...
    Trail.setSliceTime(ratio * Sheet->FX.TrailMaxSliceTime +
                       (1.f - ratio) * Sheet->FX.TrailMinSliceTime);
  }
}
...
// ***********************************************************************************************************************
const CAttack *CCharacterCL::getAttack(
    const CAttackIDSheet &id,
    const std::vector<NLMISC::TSStringId> &attackList) const {
  for (std::vector<NLMISC::TSStringId>::const_reverse_iterator it =
           attackList.rbegin();
       it != attackList.rend(); ++it) {
    const CAttackList *al = CAttackListManager::getInstance().getAttackList(
        ClientSheetsStrings.get(*it));
    if (al) {
...
NLMISC_COMMAND(beginCast, "Start spell cast",
               "<spell_id> <strenght> [<mode> <caster slot> <behaviour>]") {
  if (args.size() < 2)
    return false;
  if (args.size() > 5)
    return false;
  ...
  CCharacterCL *ch =
      dynamic_cast<CCharacterCL *>(EntitiesMngr.entity(casterSlot));
  if (!ch)
    return false;
  CBehaviourContext bc;
...
NLMISC_COMMAND(failCast, "Simulate failure of a spell cast",
               "<spell_id> <strenght> [<mode>]") {
  if (args.size() < 2)
    return false;
  if (args.size() > 3)
    return false;
...
NLMISC_COMMAND(
    projectile, "Cast a projectile on another entity",
    "<spellID> <strenght> [<mode> <target entity> <resist> <source entity>]") {
  if (args.size() < 2)
    return false;
  if (args.size() > 6)
    return false;
  ...
  if (args.size() > 3) {
    ...
    bool resist = false;
    if (args.size() > 4)
      fromString(args[4], resist);
    bc.Targets.Targets.push_back(CMultiTarget::CTarget(
        targetSlot, resist,
        (uint8)ceilf((float)(dist / MULTI_TARGET_DISTANCE_UNIT))));
  }
...
NLMISC_COMMAND(mtProjectile, "Cast a projectile on one or several entities",
               "<caster> <spellID> <strenght> <mode> <target0> [<target n>]*") {
  if (args.size() < 5)
    return false;
  uint8 casterSlot;
...
NLMISC_COMMAND(auraReceipt, "enable / disable aura receipt on an entity",
               "<slot> <aura> <0=on/1=off>") {
  if (args.size() != 2)
    return false;
  uint slot;
  fromString(args[0], slot);
  ...
  if (enableAura) {
    fx = CAttackListManager::getInstance().getAuras().getFX(
        0); // 0 is special for aura receipt
    if (!fx)
      return false;
  }
  ch->setAuraFX(1, fx);
...
NLMISC_COMMAND(weapon, "change the weapon in hand", "<slot> <hand> <weapon>") {
  ...
  uint slot;
  fromString(args[0], slot);
  CCDBNodeLeaf *propA = NLGUI::CDBManager::getInstance()->getDbProp(
      toString("SERVER:Entities:E%d:P%d", (int)slot, (int)PROPERTY_VPA), false);
  if (!propA)
    return false;
  sint64 valueA = propA->getValue64();

  CCDBNodeLeaf *propB = NLGUI::CDBManager::getInstance()->getDbProp(
      toString("SERVER:Entities:E%d:P%d", (int)slot, (int)PROPERTY_VPB), false);
  if (!propB)
    return false;
...
NLMISC_COMMAND(advantageFX,
               "turn on / off the advantage fx for an item in hand",
               "<slot> <hand> <on = 1/ off = 0>") {
  if (args.size() != 3)
    return false;
  CInterfaceManager *im = CInterfaceManager::getInstance();
  uint slot;
  ...
  /*
  CCDBNodeLeaf *prop =
  NLGUI::CDBManager::getInstance()->getDbProp(toString("SERVER:Entities:E%d:P%d",
  (int) slot, (int) PROPERTY_VPA), false); if (!prop) return false; sint64 value
  = prop->getValue64(); uint hand; fromString(args[1], hand);
  // the VP is dependent of Entity actual type
  if(dynamic_cast<CPlayerCL*>(EntitiesMngr.entity(slot)))
  {
...
NLMISC_COMMAND(trailLength, "set length of trail for one weapon in hand",
               "<slot> <hand> <power = 0..15>") {
  if (args.size() != 3)
    return false;
  CInterfaceManager *im = CInterfaceManager::getInstance();
  uint slot;
...
NLMISC_COMMAND(attack, "simulate an attack",
               "<slot> <intensity> <hit_type> <localisation> <dmg_type> "
               "<damage_shield_power> <damage_shield_id>") {
  if (args.size() < 2)
    return false;
  if (args.size() > 7)
    return false;
  ...
  fromString(args[0], slot);
  CCharacterCL *entity =
      dynamic_cast<CCharacterCL *>(EntitiesMngr.entity(slot));
  if (!entity)
    return false;
...
NLMISC_COMMAND(
    rangeAttack, "simulate a range attack",
    "<slot> [intensity] [localisation] [range_weapon_type_if_unequipped]") {
  if (args.size() < 1)
    return false;
  if (args.size() > 4)
    return false;
  ...
  // if not a generic range weapon, add a single target (this is the current
  // selection)
...
NLMISC_COMMAND(
    creatureAttack, "simulate a creature attack (2 attaques per creature)",
    "<casterSlot> <targetSlot> [attk=0/1] [magicIntensity] [physicalIntensity] "
    "[localisation] [damageType] [hitType] [resist=1/0]") {
  if (args.size() < 2)
    return false;
  if (args.size() > 9)
    return false;
  ...
  if (args.size() > 2) {
    ...
    bc.Behav.Behaviour =
        attk == 0 ? MBEHAV::CREATURE_ATTACK_0 : MBEHAV::CREATURE_ATTACK_1;
  }
  uint8 casterSlot;
  fromString(args[0], casterSlot);
  CCharacterCL *caster =
  ...
  CCharacterCL *target =
      dynamic_cast<CCharacterCL *>(EntitiesMngr.entity(targetSlot));
  if (!target)
    return false;
...
NLMISC_COMMAND(setNamePosZ, "", "<low/high/normal> <value>") {
  ...
  if (target->Type == CEntityCL::Player) {
    ...
    if (playerTarget) {
      CRaceStatsSheet *sheet =
          const_cast<CRaceStatsSheet *>(playerTarget->playerSheet());
      if (sheet) {
        if (toLowerAscii(args[0]) == "low")
          ...
          namePosZ =
              &sheet->GenderInfos[playerTarget->getGender()].NamePosZNormal;
        else if (toLowerAscii(args[0]) == "high")
          namePosZ =
              &sheet->GenderInfos[playerTarget->getGender()].NamePosZHigh;
    ...
    if (creatureTarget) {
      CCharacterSheet *sheet =
          const_cast<CCharacterSheet *>(creatureTarget->getSheet());
      if (sheet) {
        if (toLowerAscii(args[0]) == "low")
  ...
  if (namePosZ) {
    ...
    nlinfo("NAMEPOSZ: sheet: %s, skel: %s, NamePosZ%s = %g", sheetName.c_str(),
           skelName.c_str(), args[0].c_str(), *namePosZ);
  }
...
NLMISC_COMMAND(setMyNamePosZ, "", "<low/high/normal> <value>") {
  ...
  string sheetName, skelName;
  CRaceStatsSheet *sheet =
      const_cast<CRaceStatsSheet *>(UserEntity->playerSheet());
  if (sheet) {
    if (toLowerAscii(args[0]) == "low")
  ...
  if (args.empty()) {
    ...
    if (state) {
      ...
      IM->displaySystemInfo(
          toString("<pvpMode> adding pvp mode %s", args[0].c_str()));
    } else {
      uint16 currentPVPMode = playerTarget->getPvpMode();
      currentPVPMode &= ~pvpMode;
      playerTarget->setPvpMode(currentPVPMode);
      IM->displaySystemInfo(
          toString("<pvpMode> removing pvp mode %s", args[0].c_str()));
    }
  }
...
NLMISC_COMMAND(pvpClan, "modify pvp clan", "<pvp clan>")
{
        ...
        if (target->Type != CEntityCL::Player && target->Type !=
CEntityCL::User)
        {
                IM->displaySystemInfo(toString("<pvpMode> target is not a
player")); return false;
        }
        CPlayerCL *playerTarget = dynamic_cast<CPlayerCL*>(target);
        if (!playerTarget)
