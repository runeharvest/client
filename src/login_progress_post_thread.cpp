...
//===========================================================================================
static std::string extractToken(const std::string &res,
  ...
  std::string::size_type end =
      begin != std::string::npos && res.size() > tokenBeginLen + tokenEndLen
          ? res.find(tokenEnd, begin + tokenBeginLen)
          : std::string::npos;
...
public:
  //============================================================================================
  CLoginProgressTask() : StepQueue("") { StopWanted = false; }
  ...
  virtual void run() {
    try {
      ...
      if (userId.empty()) {
        res = sendMsg(std::string("init") + "&install_id=" + productInstallId +
                      buildSysInfoString());
      ...
      for (;;) {
        ...
        bool newStep = false;
        {
          ...
          if (newStep) {
            ...
            // during install program some messages can be launch multiple times
            // (eg "install_update&percent=17" that indicates that 17 percent of
            // the installation is already done)
            bool isInInstallation =
                InstallStep_StartDownload <= loginStep.Step &&
                loginStep.Step <= InstallStep_StopInstall;
            bool hasBeenInstalled = currentStep > InstallStep_StopInstall;
            if ((isInInstallation && !hasBeenInstalled) ||
                (loginStep.Step > CLoginRegistry::getLoginStep())) {
              std::string ret =
                  sendMsg(loginStep.PostString + "&user_id=" + userId);
              // If we reached this point, server has correctly received the
              // step -> commit current progress in registry Only send msg for
              // player that have never installed the software
              CLoginRegistry::setLoginStep(loginStep.Step);
              // Async Ret enable the player to Know via pulling the return of
              // the sendMsg
  ...
  std::string sendMsg(const std::string &msg) {
    ...
    // std::string postString = "http://" + StartupHost + StartupPage +
    // "?cmd=log&msg=" + msg; nlwarning("POST STRING = %s", postString.c_str());
...
//===========================================================================================
CLoginProgressPostThread::~CLoginProgressPostThread() { release(); }

//===========================================================================================
void CLoginProgressPostThread::init(const std::string &startupHost,
                                    const std::string &startupPage) {
  nlassert(!_Thread);
  CLoginProgressTask *lpt = new CLoginProgressTask;
  // Its not the real Startup Host this values comes from the InstallStatsUrl
...
void CLoginProgressPostThread::init(NLMISC::CConfigFile &configFile) {
  ...
  if (configFile.getVarPtr("InstallStatsUrl")) {
    ...
    installStartupPage = tmp.substr(hostPageSeparator); // keep the leading
                                                        // slash
    installStartupHost =
        tmp.substr(httpStrSize,
                   hostPageSeparator - httpStrSize); // dont keep the last slah

    init(installStartupHost, installStartupPage);
  } else {
    //		nlwarning("Error the InstallStatsUrl is not in the
    //client_default.cfg.");
  }
}
