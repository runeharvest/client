...
static const float DEFAUT_PRECIPITATION_TIMEOUT =
    10.f; // at most 10 second before precipitations are removed

//============================================================
CPrecipitation::CPrecipitation()
    : _ClipGrid(NULL), _Strenght(0), _TimeOut(0.f), _XSize(0), _YSize(0),
      _OldX(0), _OldY(0), _Touched(false) {
...
//============================================================
void CPrecipitation::update(const NLMISC::CMatrix &camMat,
                            NLPACS::UGlobalRetriever * /* retriever */) {
  H_AUTO_USE(RZ_Precipitation)
  if (_TimeOut != 0.f) {
  ...
  if (_Desc.ReleasableModel) {
    if (_Strenght == 0.f &&
        areFXsAllocated()) // no more precipitations & FXs are still allocated ?
                           // This means that there are particles left
    {
      if (!isRunning()) {
        deallocateFXs();
        return;
  ...
  if (_Touched || worldX != _OldX || worldY != _OldY) {
    ...
    */
    ...
    // snap the user pos
    //
    NLMISC::CVector snappedPos(fsnapf(userPos.x, _XSize),
                               fsnapf(userPos.y, _YSize), userPos.z);
    snappedPos -=
        NLMISC::CVector((float(_Desc.GridSize >> 1) - 0.5f) * _XSize,
                        (float(_Desc.GridSize >> 1) - 0.5f) * _YSize, 0);
    ...
    for (uint k = 0; k < _Desc.GridSize; ++k) {
      ...
      for (uint l = 0; l < _Desc.GridSize; ++l) {
        ...
        else // use the clip grid to see if that block should be activated, and
        at what height it should be put
        {
                typedef CPrecipitationClipGrid::CGridPoint CGridPoint; // alias
        CGridPoint const CGridPoint *p0 = _ClipGrid->get(l + gridCornerX, k +
        gridCornerY); const CGridPoint *p1 = _ClipGrid->get(l + gridCornerX + 1,
        k + gridCornerY); const CGridPoint *p2 = _ClipGrid->get(l + gridCornerX
        + 1, k + gridCornerY + 1); const CGridPoint *p3 = _ClipGrid->get(l +
        gridCornerX, k + gridCornerY + 1); if (p0 && p1 && p2 && p3)
                {
                        // if one of the 4 corners is in an interior, don't
        display the precipiations if (p0->Clipped || p1->Clipped || p2->Clipped
        || p3->Clipped)
                        {
                                getBlock(l, k).hide();
                        }
                        ...
                        {
                                NL3D::UParticleSystemInstance psi = getBlock(l,
        k); psi.show();
                                // take the mean height
                                hPos.z = 0.25f * (p0->MeanHeight +
        p1->MeanHeight + p2->MeanHeight + p3->MeanHeight); psi.setPos(hPos);
                        }
                }
                else
                {
