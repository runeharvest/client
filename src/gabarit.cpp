...
//===================================================================================
void CSkeletonGabarit::buildFromSkeleton(NL3D::USkeleton src,
  ...
  for (uint k = 0; k < NumBones; ++k) {
    ...
    if (boneID == -1) {
      // If the skeleton is male and we try to get breast its normal that we
      // dont found it
      if (!((boneIndexToCategory(k) == CSkeletonGabarit::Breast) && (bMale)))
        nlwarning("CSkeletonGabarit : can't get bone %s for skeleton %s",
                  boneIndexToName(k), skelName.c_str());
...
void CGabaritSet::loadGabarits(NLMISC::IProgressCallback &progress) {
  ...
  if (!scene) {
    nlwarning(
        "CGabaritSet::loadGabarits : can't create scene to load skeletons");
  }

  for (uint g = 0; g < NumGender; ++g) {
    // Progress bar
    progress.progress((float)g / (float)NumGender);
    progress.pushCropedValues((float)g / (float)NumGender,
                              (float)(g + 1) / (float)NumGender);

    for (uint r = 0; r < NumRace; ++r) {
      // Progress bar
      progress.progress((float)r / (float)NumRace);
      progress.pushCropedValues((float)r / (float)NumRace,
                                (float)(r + 1) / (float)NumRace);

      for (uint h = 0; h < NumHeights; ++h) {
        // Progress bar
        progress.progress((float)h / (float)NumHeights);
        progress.pushCropedValues((float)h / (float)NumHeights,
                                  (float)(h + 1) / (float)NumHeights);
        ...
        for (uint w = 0; w < NumWidths; ++w) {
          ...
          if (skel.empty()) {
            nlwarning("CGabaritSet::loadGabarits : can't load skeleton %s",
                      skelName.c_str());
          } else {
            _Gabarit[g][r][h][w].buildFromSkeleton(skel, skelName,
                                                   g == GSGENDER::male);
...
//===================================================================================
void CGabaritSet::applyGabarit(NL3D::USkeleton dest, uint gender,
                               EGSPD::CPeople::TPeople people, float height,
                               float torsoWidth, float armsWidth,
                               float legsWidth, float breastSize,
                               float *finalHeightScale) {
  H_AUTO_USE(RZ_CSkeletonGabarit)
  if (dest.empty()) {
    nlwarning("<CGabaritSet::applyGabarit> Skeleton NULL (gender=%d People=%d)",
              gender, people);
    return;
  }
  if (gender >= NumGender)
  ...
  float *widthsTab[CSkeletonGabarit::NumBoneCategory] = {
      &armsWidth, &torsoWidth, &legsWidth, &breastSize};
  ...
  for (k = 0; k < CSkeletonGabarit::NumBoneCategory; ++k) {
    ...
    gbTab[k].BL =
        &_Gabarit[gender][race][height >= 0 ? 1 : 0][width >= 0 ? 1 : 0];
    gbTab[k].BR =
        &_Gabarit[gender][race][height >= 0 ? 1 : 0][width >= 0 ? 2 : 1];
    gbTab[k].TL =
        &_Gabarit[gender][race][height >= 0 ? 2 : 1][width >= 0 ? 1 : 0];
    gbTab[k].TR =
        &_Gabarit[gender][race][height >= 0 ? 2 : 1][width >= 0 ? 2 : 1];
    //
  ...
  for (k = 0; k < CSkeletonGabarit::NumBones; ++k) {
    ...
    if (boneID != -1) {
      ...
      CSkeletonGabarit::EBoneCategory boneCategory =
          CSkeletonGabarit::boneIndexToCategory(k);
      if (boneCategory != CSkeletonGabarit::Other) {
        const CGabaritBlend &gb =
            gbTab[boneCategory]; // takes the gabarit blend matching the bone
                                 // category
        bone.setTransformMode(UTransform::RotQuat);

        // standard scale
        CVector scale = NLMISC::computeBilinear(
            gb.BL->BoneScale[k], gb.BR->BoneScale[k], gb.TR->BoneScale[k],
            gb.TL->BoneScale[k], blendTab[boneCategory], heightBlend);
        bone.setScale(scale);

        // skin scale
        CVector skinScale = NLMISC::computeBilinear(
            gb.BL->BoneSkinScale[k], gb.BR->BoneSkinScale[k],
            gb.TR->BoneSkinScale[k], gb.TL->BoneSkinScale[k],
            blendTab[boneCategory], heightBlend);
  ...
  if (finalHeightScale) {
    *finalHeightScale = NLMISC::computeBilinear(
        gbTab[0].BL->HeightScale, gbTab[0].BR->HeightScale,
        gbTab[0].TR->HeightScale, gbTab[0].TL->HeightScale, 0.f, heightBlend);
  }
}

//===================================================================================
float CGabaritSet::getRefHeightScale(uint gender,
