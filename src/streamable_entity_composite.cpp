...
CStreamableEntityComposite::~CStreamableEntityComposite() {
  ...
  for (TStreambleEntities::iterator it = _Entities.begin();
       it != _Entities.end(); ++it) {
    delete *it;
  }
}
...
void CStreamableEntityComposite::add(IStreamableEntity *entity) {
  H_AUTO_USE(RZ_StremableEntityComposite)
  nlassert(entity);
  nlassert(std::find(_Entities.begin(), _Entities.end(), entity) ==
           _Entities.end());
...
void CStreamableEntityComposite::remove(IStreamableEntity *entity) {
  H_AUTO_USE(RZ_StremableEntityComposite)
  TStreambleEntities::iterator it =
...
//===============================================================================
/*virtual*/ bool CStreamableEntityComposite::needCompleteLoading(
    const NLMISC::CVector &pos) const {
  H_AUTO_USE(RZ_StremableEntityComposite)
  for (TStreambleEntities::const_iterator it = _Entities.begin();
       it != _Entities.end(); ++it) {
    if ((*it)->needCompleteLoading(pos))
      return true;
  }
  return false;
}
...
//===============================================================================
/*virtual*/ void
CStreamableEntityComposite::update(const NLMISC::CVector &pos) {
  H_AUTO_USE(RZ_StremableEntityComposite)
  for (TStreambleEntities::iterator it = _Entities.begin();
       it != _Entities.end(); ++it) {
    (*it)->update(pos);
...
//===============================================================================
/*virtual*/ void
CStreamableEntityComposite::forceUpdate(const NLMISC::CVector &pos,
  ...
  for (TStreambleEntities::iterator it = _Entities.begin();
    ...
    progress.pushCropedValues((float)count / (float)size,
                              (float)(count + 1) / (float)size);
