...
// ***************************************************************************
COutpostManager::COutpostManager() { _EndTickForPvpJoinProposal = 0; }

// ***************************************************************************
void COutpostManager::startPvpJoinProposal(bool playerGuildInConflict,
  ...
  CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
      "UI:TEMP:OUTPOST:PVP_PROPOSAL_PLAYER_GUILD_IN_CONFLICT");
  if (node)
    node->setValueBool(playerGuildInConflict);
  node = NLGUI::CDBManager::getInstance()->getDbProp(
      "UI:TEMP:OUTPOST:PVP_PROPOSAL_PLAYER_GUILD_IS_ATTACKER");
  if (node)
    node->setValueBool(playerGuildIsAttacker);
  node = NLGUI::CDBManager::getInstance()->getDbProp(
      "UI:TEMP:OUTPOST:PVP_PROPOSAL_ATTACKER");
  if (node)
    node->setValue32(attackerGuildNameId);
  node = NLGUI::CDBManager::getInstance()->getDbProp(
      "UI:TEMP:OUTPOST:PVP_PROPOSAL_DEFENDER");
  if (node)
    node->setValue32(ownerGuildNameId);
  node = NLGUI::CDBManager::getInstance()->getDbProp(
      "UI:TEMP:OUTPOST:PVP_PROPOSAL_TICK_END");
  if (node)
    node->setValue32(_EndTickForPvpJoinProposal);

  // open Popup
  CGroupContainer *pGC = dynamic_cast<CGroupContainer *>(
      CWidgetManager::getInstance()->getElementFromId(
          "ui:interface:join_pvp_outpost_proposal"));
  if (pGC) {
    pGC->setActive(true);
...
// ***************************************************************************
void COutpostManager::endPvpJoinProposal(bool bNeutral,
                                         OUTPOSTENUMS::TPVPSide pvpSide) {
  // send msg
  string sMsg = "OUTPOST:SIDE_CHOSEN";
  CBitMemStream out;
  if (GenericMsgHeaderMngr.pushNameToStream(sMsg, out)) {
    // nlinfo("impulseCallBack : %s %d %s sent", sMsg.c_str(), bNeutral,
    // OUTPOSTENUMS::toString(pvpSide).c_str());
    out.serial(bNeutral);
...
void COutpostManager::update() {
  ...
  if (_EndTickForPvpJoinProposal != 0) {
    if (_EndTickForPvpJoinProposal < NetMngr.getCurrentServerTick()) {
      ...
      CAHManager::getInstance()->runActionHandler("outpost_pvp_join", NULL,
                                                  "neutral");
