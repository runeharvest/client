...
void CSceneParser::load(const string &filename) {
  ...
  if (file.is_open()) {
    ...
    if (speedFile.is_open()) {
      ...
      nlwarning("CSceneParser::load : 'capture_speed.txt' can't be open, "
                "default frame rate is %f.",
                _FrameRate);
    ...
    nlwarning("CSceneParser::load : File \"%s\" can't be open.",
              sceneFile.c_str());
    return;
  }
} // load //
...
void CSceneParser::parseParticle(ifstream &file, char *buff) {
  ...
  if (_CurrentParticle.Id >= 0) {
    map<uint, CParticle>::iterator it =
        _Particles.find((uint)_CurrentParticle.Id);
    // Id already exists.
    if (it != _Particles.end())
      nlwarning("Particle before line %d has the ID %d that already exists -> "
                "Particle Not Created.",
                _Line, _CurrentParticle.Id);
    // Insert the new Particle.
    else
      _Particles.insert(make_pair((uint)_CurrentParticle.Id, _CurrentParticle));
  } else
    nlwarning("Particle before line %d has no ID or a Bad one -> Particle Not "
              "Created.",
              _Line);
} // parseParticle //
...
void CSceneParser::parseActor(ifstream &file, char *buff) {
  ...
  if (_CurrentActor.Id >= 0) {
    ...
    // Id already exists.
    if (it != _Actors.end())
      nlwarning("Actor before line %d has the ID %d that already exists -> "
                "Actor Not Created.",
                _Line, _CurrentActor.Id);
    // Insert the new actor.
    else
      _Actors.insert(make_pair((uint)_CurrentActor.Id, _CurrentActor));
  } else
    nlwarning(
        "Actor before line %d has no ID or a Bad one -> Actor Not Created.",
        _Line);
} // parseActor //
...
void CSceneParser::parseSequence(ifstream &file, char *buff) {
  ...
  if (_CurrentSequence.Id >= 0) {
    map<uint, CSequence>::iterator itSeq =
        _Sequences.find((uint)_CurrentSequence.Id);
    // Id already exists.
    if (itSeq != _Sequences.end())
      nlwarning("Sequence before line %d has the ID %d that already exists -> "
                "Sequence Not Created.",
                _Line, _CurrentSequence.Id);
    // Insert the new actor.
    else
      _Sequences.insert(make_pair((uint)_CurrentSequence.Id, _CurrentSequence));
  } else
    nlwarning("Sequence before line %d has no ID or a Bad one -> Sequence Not "
              "Created.",
              _Line);
} // parseSequence //
...
void CSceneParser::parseSeqParticle(ifstream &file, char *buff) {
  ...
  if (_CurParticleSeq.Id >= 0) {
    map<uint, CParticleSeq>::iterator itPartSeq =
        _CurrentSequence.ParticlesSeq.find((uint)_CurParticleSeq.Id);
    // Id already exists.
    if (itPartSeq != _CurrentSequence.ParticlesSeq.end())
      nlwarning("Particle in Sequence before line %d has the ID %d that "
                "already exists -> Particle in the sequence Not Created.",
                _Line, _CurParticleSeq.Id);
    // Insert the new particle in the sequence.
    else
      _CurrentSequence.ParticlesSeq.insert(
          make_pair((uint)_CurParticleSeq.Id, _CurParticleSeq));
  } else
    nlwarning("Particle in Sequence before line %d has no ID or a Bad one -> "
              "Particle in the sequence Not Created.",
              _Line);
} // parseSeqParticle //
...
void CSceneParser::parseSeqActor(ifstream &file, char *buff) {
  ...
  if (_CurrentActorSeq.Id >= 0) {
    map<uint, CActorSeq>::iterator itActSeq =
        _CurrentSequence.ActorsSeq.find((uint)_CurrentActorSeq.Id);
    // Id already exists.
    if (itActSeq != _CurrentSequence.ActorsSeq.end())
      nlwarning("Actor in Sequence before line %d has the ID %d that already "
                "exists -> Actor in the sequence Not Created.",
                _Line, _CurrentActorSeq.Id);
    // Insert the new actor.
    else
      _CurrentSequence.ActorsSeq.insert(
          make_pair((uint)_CurrentActorSeq.Id, _CurrentActorSeq));
  } else
    nlwarning("Actor in Sequence before line %d has no ID or a Bad one -> "
              "Actor in the sequence Not Created.",
              _Line);
} // parseSeqActor //
  ...
  if (IGInit) {
    ...
    for (int i = 0; i < nbInst; ++i) {
      ...
      if (itActor != _Actors.end()) {
        ...
        nlwarning("CSceneParser::initActors : Actor %d is in the IG \"%s\" but "
    ...
    nlwarning("CSceneParser::initActors : Can't create the Instance group to "
              "initialize actors.");
} // initActors ///
...
void CSceneParser::apply() {
  ...
  // Initialize IG for the scenery (IG present from start to end like trees or
  // torch.
  applyIG();
...
void CSceneParser::resetParticles() {
  ...
  for (itPart = _Particles.begin(); itPart != _Particles.end(); ++itPart) {
    ...
    if (particle.IGPtr && _IGInScene.find(particle.IGPtr) != _IGInScene.end()) {
      ...
      try {
        particle.IGPtr =
            UInstanceGroup::createInstanceGroup((*itPart).second.IG);
      } catch (Exception &e) {
        nlwarning("CSceneParser::resetParticles : %s", e.what());
      }
    }
  }
} // resetParticles //
...
//-----------------------------------------------
// applyParticles :
// Initialize the scene with the parameters loaded from the script for
// particles.
//-----------------------------------------------
void CSceneParser::applyParticles() {
  ...
  for (itPart = _Particles.begin(); itPart != _Particles.end(); ++itPart) {
    ...
    for (itPartAnim = particle.Anims.begin();
         itPartAnim != particle.Anims.end(); ++itPartAnim) {
      uint idAnim = UAnimationSet::NotFound;
      try {
        idAnim = particle.AnimationSet->addAnimation(
            (*itPartAnim + string(".anim")).c_str(), (*itPartAnim).c_str());
      } catch (Exception &e) {
        nlwarning("%s", e.what());
      }
      ...
      if (idAnim != UAnimationSet::NotFound) {
        ...
        nlwarning("CSceneParser::applyParticles : Anim %s cannot be add in the "
                  "animation set",
                  (*itPartAnim).c_str());
    }
...
void CSceneParser::applyCamera() {
  ...
  if (_AnimationSet) {
    ...
    for (itCamAnims = _CamAnims.begin(); itCamAnims != _CamAnims.end();
         ++itCamAnims) {
      uint idAnim = UAnimationSet::NotFound;
      try {
        idAnim = _AnimationSet->addAnimation(
            (*itCamAnims + string(".anim")).c_str(), (*itCamAnims).c_str());
      } catch (Exception &e) {
        nlwarning("%s", e.what());
      }
      ...
      if (idAnim != UAnimationSet::NotFound)
        ...
        nlwarning("CSceneParser::apply : Camera Anim %s cannot be add in the "
                  "animation set",
                  (*itCamAnims).c_str());
    }
...
void CSceneParser::applyIG() {
  ...
  for (list<string>::iterator itIG = _IG.begin(); itIG != _IG.end(); ++itIG) {
    ...
    try {
      ...
      nlwarning("CSceneParser::applyIG : %s", e.what());
    }
...
void CSceneParser::applyActors() {
  ...
  for (itActor = _Actors.begin(); itActor != _Actors.end(); ++itActor) {
    ...
    if (entityTmp) {
      ...
      for (itMeshes = actor.Meshes.begin(); itMeshes != actor.Meshes.end();
           ++itMeshes) {
        // If there are too many meshes.
        if (count >= (uint)CEntityCL::NB_SLOT) {
          nlwarning("CSceneParser::applyActors : Too many meshes for Actor %d.",
                    actor.Id);
          break;
        }
  ...
  if (itSeq != _Sequences.end()) {
    ...
    if (!sequence.CamAnims.empty()) {
      if (_PlayList) {
        ...
        for (itAnimCamId = _AnimCamToId.begin();
             itAnimCamId != _AnimCamToId.end(); ++itAnimCamId) {
    ...
    map<uint, CParticleSeq>::iterator itPartSeq;
    for (itPartSeq = sequence.ParticlesSeq.begin();
         itPartSeq != sequence.ParticlesSeq.end(); ++itPartSeq) {
      ...
      if (itPart != _Particles.end()) {
        ...
        if (particle.IGPtr) {
          ...
          for (itAnimId = particle.AnimToId.begin();
               itAnimId != particle.AnimToId.end(); ++itAnimId) {
    ...
    map<uint, CActorSeq>::iterator itActSeq;
    for (itActSeq = sequence.ActorsSeq.begin();
         itActSeq != sequence.ActorsSeq.end(); ++itActSeq) {
      CActorSeq &actorSeq = (*itActSeq).second;
      updateAnimationSequence(Sid(Sid::npc, (uint64)actorSeq.Id),
                              actorSeq.Anims, 0);
    }
  }
} // playSeq //
  ...
  if (_PlayList) {
    ...
    if (idAnim != UPlayList::empty) {
      ...
      if (animation) {
        ...
        UTrack *trackPosTarget =
            animation->getTrackByName("Camera.Target.PathPos");
...
void CSceneParser::updateActors() {
  ...
  for (map<uint, CActor>::iterator itActor = _Actors.begin();
       itActor != _Actors.end(); ++itActor) {
    // Get a reference on the actor.
    CActor &actor = (*itActor).second;
    ...
    if (!entity) {
      nlwarning("CSceneParser::updateActors : Cannot get the actor %d.",
                actor.Id);
      continue;
    }
    ...
    if (!entityTarget) {
      nlwarning(
          "CSceneParser::updateActors : Cannot get the targeted actor %d.",
          actor.Actor);
      continue;
    }
...
void CSceneParser::updateParticles(double timeInSec) {
  for (map<uint, CParticle>::iterator itPart = _Particles.begin();
       itPart != _Particles.end(); ++itPart) {
    // Get a reference on the particule.
    CParticle &particle = (*itPart).second;
...
//
//-----------------------------------------------
void CSceneParser::updateParticlesNoActor(float difTime, CParticle &particle,
  ...
  for (uint i = 0; i < particle.IGPtr->getNumInstance(); ++i) {
    ...
    if (!trackPos)
      trackPos =
          animation.getTrackByName(string(iName + "." + "PathPos").c_str());
    if (trackPos) {
      CVector pos;
    ...
    if (!trackRot)
      trackRot =
          animation.getTrackByName(string(iName + "." + "PathRotQuat").c_str());
    if (trackRot) {
      CQuat rot;
...
//
//-----------------------------------------------
void CSceneParser::updateParticlesActor(float difTime, CParticle &particle,
                                        UAnimation &animation) {
  // Get the entity pointer.
  CEntityCL *entity = getEntity(Sid(Sid::npc, (uint64)particle.Actor));
  if (!entity) {
    nlwarning("CSceneParser::updateParticlesActor : cannot get the actor %d.",
              (uint64)particle.Actor);
    return;
  }

  // If the entity has no skeleton -> Next particle.
  if (!entity->skeleton()) {
    nlwarning("The particle follow an entity %d without a skeleton.",
              (uint64)particle.Actor);
    return;
  }
...
void CSceneParser::loadScene(const string &filename) {
  ...
  string sceneFile = CPath::lookup(filename, false);
  if (sceneFile.empty()) {
    nlwarning("CSceneParser::loadScene : Can't find file \"%s\".",
              filename.c_str());
    return;
  }
  ...
  if (file.is_open()) {
    ...
    nlwarning("CSceneParser::loadScene : File \"%s\" can't be open.",
              sceneFile.c_str());
    return;
  }
} // loadScene //
