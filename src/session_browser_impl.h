...
public:
   ...
   *
   * game.getSessionList() -> send a request to server to get the list of
   * session As a result : On success, RingAccessPoint:fill() will be called
   * with a table containing the result of the request On failure : -
   * RingAccessPoint:onDisconnection()
  ...
  virtual void
  on_CRingSessionManagerWebClient_Disconnection(NLNET::TSockId from);
  virtual void on_invokeResult(NLNET::TSockId from, uint32 userId,
                               uint32 resultCode,
                               const std::string &resultString);
  virtual void on_scheduleSessionResult(NLNET::TSockId from, uint32 charId,
                                        TSessionId sessionId, uint8 result,
                                        const std::string &resultString);
  virtual void
  on_sessionInfoResult(NLNET::TSockId from, uint32 charId, TSessionId sessionId,
                       ...
                       const RSMGR::TLevelFilter &levelFilter,
                       bool subscriptionClosed, bool autoInvite,
                       const std::string &language,
                       const RSMGR::TSessionOrientation &orientation,
                       const std::string &description);
  virtual void
  on_joinSessionResult(NLNET::TSockId from, uint32 userId, TSessionId sessionId,
                       uint32 result, const std::string &shardAddr,
                       const RSMGR::TSessionPartStatus &participantStatus);
  virtual void
  on_joinSessionResultExt(NLNET::TSockId from, uint32 userId,
                          TSessionId sessionId, uint32 result,
                          const std::string &shardAddr,
                          const RSMGR::TSessionPartStatus &participantStatus,
                          const CSecurityCode &securityCode);
  virtual void on_getShardsResult(NLNET::TSockId from, uint32 userId,
                                  const std::string &result);
  virtual void
  on_CSessionBrowserServerWebClient_Disconnection(NLNET::TSockId from);
  virtual void on_sessionList(NLNET::TSockId from, uint32 charId,
                              const std::vector<RSMGR::TSessionDesc> &sessions);
  virtual void on_charList(NLNET::TSockId from, uint32 charId,
                           TSessionId sessionId,
                           const std::vector<RSMGR::TCharDesc> &charDescs);
  virtual void on_playerRatings(NLNET::TSockId from, uint32 charId,
                                bool scenarioRated, uint32 rateFun,
                                uint32 rateDifficulty, uint32 rateAccessibility,
                                uint32 rateOriginality, uint32 rateDirection);
  virtual void on_ringRatings(NLNET::TSockId from, uint32 charId,
                              uint32 authorRating, uint32 AMRating,
                              uint32 masterlessRating);
  virtual void on_ringPoints(NLNET::TSockId from, uint32 charId,
                             const std::string &ringPoints,
                             const std::string &maxRingPoints);
  // Return average scores of a session
  virtual void on_sessionAverageScores(NLNET::TSockId from, bool scenarioRated,
                                       uint32 rateFun, uint32 rateDifficulty,
                                       uint32 rateAccessibility,
                                       uint32 rateOriginality,
                                       uint32 rateDirection, uint32 rrpTotal);
  virtual void on_scenarioAverageScores(NLNET::TSockId from, bool scenarioRated,
                                        uint32 rateFun, uint32 rateDifficulty,
                                        uint32 rateAccessibility,
                                        uint32 rateOriginality,
                                        uint32 rateDirection, uint32 rrpTotal);
...
private:
  ...
  void ringStatsFill();
  void playerRatingFill(bool scenarioRated, uint32 rateFun,
                        uint32 rateDifficulty, uint32 rateAccessibility,
                        uint32 rateOriginality, uint32 rateDirection);
  void averageScoresFill(bool scenarioRated, uint32 rateFun,
                         uint32 rateDifficulty, uint32 rateAccessibility,
                         uint32 rateOriginality, uint32 rateDirection,
                         uint32 rrpTotal);
