...
void checkLocalTimeStep(const CPacketInfos &packetInfos) {
  ...
  // If the predicted time is too small according to the min time computed for
  // the packet -> increase 'LocalTimeStep'.
  if (predictedTime < packetInfos.Min) {
    ...
    //		nlwarning("packetInfos.Min - CurrentPacketTime : %f",
    //(double)((double)packetInfos.Min - (double)CurrentPacketTime));

    LocalTimeStep = (packetInfos.Min - CurrentPacketTime) / nbPacket;

    //		nlwarning("LocalTimeStep : %f", (double)LocalTimeStep);
  }
  // If the predicted time is too big according to the max time computed for the
  // packet -> increase 'LocalTimeStep'.
  else if (predictedTime > packetInfos.Max) {
    //		nlwarning("packetInfos.Max : %f", (double)packetInfos.Max);
    //		nlwarning("CurrentPacketTime : %f", (double)CurrentPacketTime);
    //		nlwarning("packetInfos.Max - CurrentPacketTime : %f",
    //(double)((double)packetInfos.Max - (double)CurrentPacketTime));
  ...
  if (LocalTimeStep == 0) {
    ...
    //		nlwarning("LocalTimeStep (corrected): %f",
...
void ackPacketTimeBefore(uint8 ackPacketNumber) {
  // If the acknowledged packet is not in the list -> player do not have send
  // any packet for the time.
  map<uint8, TTime>::iterator itEnd = PacketTime.find(ackPacketNumber);
  if (itEnd != PacketTime.end()) {
    // If the packet cannot be found -> First valid packet the server is
    // acknowledging
    ...
    if (itBegin != PacketTime.end()) {
      while (itBegin != itEnd) {
        ...
        itBegin++;
        // Destroy packets between the last acknowledged packet and the recent
        // one.
...
void initClientTime() {
  ...
  NLGUI::CDBManager::getInstance()->getDbProp(
      "UI:VARIABLES:CURRENT_SERVER_TICK");
  ...
  NLGUI::CDBManager::getInstance()->getDbProp(
      "UI:VARIABLES:DAY_BEFORE_ZC_DISTRIB");
...
void updateClientTime() {
  ...
  if (ClientCfg.ForceDeltaTime == 0) {
    ...
    if (T1 <= T0) {
      // This case is known to occurs if the framerate is very fast (e.g. during
      // the loading stage) or if the machine has got a heavy load (e.g. lots of
      // AGP data) delaying the timer updates.
...
#define REAL_SECONDS_TO_RYZOM_HOUR(s)                                          \
  (double(s) * 10 / double(RYZOM_HOURS_IN_TICKS))
  //
  ...
  if (SmoothedClientDate.Hour > WorldLightCycle.NumHours) {
    ...
    SmoothedClientDate.Hour =
        (float)fmod(SmoothedClientDate.Hour, WorldLightCycle.NumHours);
  }
  double smoothedClientDateInHour =
      (double)SmoothedClientDate.Day * 24 + SmoothedClientDate.Hour;
  double ryzomDateInHour =
      (double)RT.getRyzomDay() * 24 + (double)RT.getRyzomTime();
  const double SMOOTHED_DATE_MAX_ERROR_IN_RYZOM_HOUR =
  ...
  if (hourError > SMOOTHED_DATE_MAX_ERROR_IN_RYZOM_HOUR) {
    if (smoothedClientDateInHour < ryzomDateInHour) {
      ...
      if (hourError > (2 * SMOOTHED_DATE_MAX_ERROR_IN_RYZOM_HOUR)) {
        ...
        // cancel time increment (wait until server time is up with smoothed
        // client time)
