 ...
 * Must be initialized from a .id_to_string_array sheet, which gives the id /
 * sheet_name pairs The sheet name must be a .animation_fx one
 *
...
public:
   ...
   * \param mustDeleteAnimSet true if ownerShip of animset must be given to that
   * object (e.g it is deleted by that object at release())
   */
  void init(const CIDToStringArraySheet &sheet, NL3D::UAnimationSet *animSet,
   ...
   * object (e.g it is deleted by that object at release())
   */
  void init(const std::string &sheetName, NL3D::UAnimationSet *animSet,
            bool mustDeleteAnimSet = false);
