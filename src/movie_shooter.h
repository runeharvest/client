...
public:
  ...
  /// Add a frame. return false if error. NB: if the movie has looped, erase
  /// some images from begin, but still return true
  ...
  /** Save the movie to a directory: shoot_00000.tga, .... Throw Exception if
   *error. Frames are saved at a constant rate. if linearInterp is true, frames
   *are interpolated to give the best view result. NB: stop if ESC is pressed
   */
  void saveMovie(UDriver *drv, UTextContext *textContext, const char *path,
                 float framePeriod, bool allowLinearInterp = true,
                 const char *prefix = "shot_");
