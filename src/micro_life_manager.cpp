...
// ********************************************************************************************
void CMicroLifeManager::init(const NLMISC::CVector2f &minCorner,
                             const NLMISC::CVector2f &maxCorner,
                             float cellSize /* = 30.f*/) {
  FPU_CHECKER
  H_AUTO_USE(RZ_MicroLifeManager)
  release();
  ...
  if (minCornerFinal.x > maxCornerFinal.x)
    std::swap(minCornerFinal.x, maxCornerFinal.x);
  if (minCornerFinal.y > maxCornerFinal.y)
    std::swap(minCornerFinal.y, maxCornerFinal.y);
  //
...
// To quickly know if a tile is inside a zone of micro-life, we subdivide the
// world into a grid. Each grid cell may be overlapped by [0, n] polygons of a
// micro-life zone. We don't want to store a full list for each cell of the
// grid, because such lists are repeated several times (it would end up wasting
// too much space). What we just need to keep is a set of possible polygon lists
// that can overlap a grid cell. Afterward, we just need a grid of index into
// the list of possible polygon lists.
// To do that : - we first store complete lists in each grid cells : this is
// done by several calls to drawPolyInBuildGrid
//              - we search for redundant lists and store the result as index in
//              the final grid. This is done in packBuildGrid

// *********************************************************************************************************************************
bool CMicroLifeManager::CGridOverlapPolyInfo::operator<(
    const CMicroLifeManager::CGridOverlapPolyInfo &other) const {
  FPU_CHECKER
  H_AUTO_USE(RZ_MicroLifeManager)
  if (PrimitiveIndex != other.PrimitiveIndex)
    return PrimitiveIndex < other.PrimitiveIndex;
  if (Sheet != other.Sheet)
    return Sheet < other.Sheet;
  if (IsExcludePoly != other.IsExcludePoly)
    return !other.IsExcludePoly; // want to test exclude polys first
  if (IsFullyCovered != other.IsFullyCovered)
    return other.IsFullyCovered;
...
bool CMicroLifeManager::CGridOverlapPolyInfo::operator==(
  ...
  FPU_CHECKER
  H_AUTO_USE(RZ_MicroLifeManager)
  return PrimitiveIndex == other.PrimitiveIndex && Sheet == other.Sheet &&
         IsExcludePoly == other.IsExcludePoly &&
         IsFullyCovered == other.IsFullyCovered && Poly == other.Poly;
}

// *********************************************************************************************************************************
bool CMicroLifeManager::CGridOverlapPolyInfoVector::operator<(
    const CMicroLifeManager::CGridOverlapPolyInfoVector &other) const {
  FPU_CHECKER
  H_AUTO_USE(RZ_MicroLifeManager)
...
// ***************************************************************************************************************************
void CMicroLifeManager::drawPolyInBuildGrid(
    const std::vector<NLLIGO::CPrimVector> &primPoly, uint primitiveIndex,
    CMicroLifeManager::TBuildGrid &buildGrid, const CFloraSheet *sheet,
    bool isExcludeTri) {
  FPU_CHECKER
  H_AUTO_USE(RZ_MicroLifeManager)
  ...
  for (std::list<NLMISC::CPolygon>::iterator it = convexPolys.begin();
       it != convexPolys.end(); ++it) {
    ...
    for (uint k = 0; k < it->Vertices.size(); ++k) {
      ...
      // nlinfo("poly2D.Vertices[k].x = %.1f, poly2D.Vertices[k].y = %.1f",
      // poly2D.Vertices[k].x, poly2D.Vertices[k].y);
    }
    // rasterize poly
    ...
    if (outerBorders.empty())
      continue; // no contribution for that poly
    ...
    sint maxY =
        std::min(outerMinY + (sint)outerBorders.size(), (sint)_GridHeight);
    sint startY = std::max(0, outerMinY);
    for (sint y = startY; y < maxY; ++y) {
      sint maxX =
          std::min(outerBorders[y - startY].second, (sint)(_GridWidth - 1));
      for (sint x = std::max((sint)0, outerBorders[y - startY].first);
           x <= maxX; ++x) {
        nlassert(x >= 0);
        nlassert(y >= 0);
        ...
        if (y >= innerMinY && y < innerMinY + (sint)innerBorders.size()) {
          if (x >= innerBorders[y - innerMinY].first &&
              x <= innerBorders[y - innerMinY].second) {
...
// ***************************************************************************************************************************
// pack the build grid so that each cell only contains index into a small set of
// poly list, instead of a full poly list (a lot of cells share the same poly
// list)
void CMicroLifeManager::packBuildGrid(
  ...
  // for each cell :  - remove all polys of the same zone if there's an exclude
  // tri that covers the whole grid
  ...
  for (uint k = 0; k < buildGrid.size(); ++k) {
    ...
    do {
      restart = false;
      for (std::vector<CGridOverlapPolyInfo>::iterator it = gopiv.V.begin();
           it != gopiv.V.end(); ++it) {
        if (it->IsExcludePoly && it->IsFullyCovered) {
          ...
          gopiv.V.erase(std::remove_if(gopiv.V.begin(), gopiv.V.end(), pred),
                        gopiv.V.end());
          restart = true; // must restart scan from start of list because
  ...
  // second pass : flatten the set of possible lists to build an index for each
  // grid cell
  ...
  for (uint k = 0; k < buildGrid.size(); ++k) {
    TPossibleOverlapingPolyLists::const_iterator it =
        std::lower_bound(flatSet.begin(), flatSet.end(), buildGrid[k]);
...
void CMicroLifeManager::addPrimitiveToBuildGrid(
    const std::string &fileName, uint &primitiveIndex,
  ...
  for (uint k = 0; k < prims.RootNode->getNumChildren(); ++k) {
    ...
    if (!(prims.RootNode->getChild(child, k) && child))
      continue;
    //
    std::string className;
    // make sure it is a 'micro_life' primitive
    if (!child->getPropertyByName("class", className)) {
      nlwarning("Can't get class for child %d of primitive %s", (int)k,
                path.c_str());
      continue;
    }
    if (NLMISC::nlstricmp(className.c_str(), "micro_life") != 0)
      continue; // only deals with micro life
    // read flora sheet in the primitive
    std::string formName;
    if (!child->getPropertyByName("form", formName)) {
      nlwarning("Can't get form name for child %d of primitive %s", (int)k,
                path.c_str());
      continue;
    }
    ...
    for (uint m = 0; m < child->getNumChildren(); ++m) {
      ...
      if (!(child->getChild(mlZone, m) && mlZone))
        continue;
      // make sure it is a 'micro_life_zone'
      if (!mlZone->getPropertyByName("class", className)) {
        nlwarning("Can't get class for child %d of primitive %s", (int)m,
                  path.c_str());
        continue;
      }
      if (NLMISC::nlstricmp("micro_life_zone", className) != 0)
        continue;

      NLLIGO::CPrimZone *zone = dynamic_cast<NLLIGO::CPrimZone *>(mlZone);
      if (!zone) {
        nlwarning("Child %d of primitive %s is not a zone", (int)k,
                  path.c_str());
        continue;
      }
      drawPolyInBuildGrid(zone->VPoints, primitiveIndex, buildGrid, floraSheet,
                          false);
      ...
      for (uint l = 0; l < zone->getNumChildren(); ++l) {
        ...
        if (zone->getChild(exclPrim, l) && exclPrim) {
          if (!exclPrim->getPropertyByName("class", className)) {
            nlwarning(
                "Can't get class of sub-zone %d for child %d of primitive %s",
                (int)l, (int)m, path.c_str());
            continue;
          }
        }
        if (NLMISC::nlstricmp("micro_life_exclude_zone", className) == 0) {
          NLLIGO::CPrimZone *exclZone =
              dynamic_cast<NLLIGO::CPrimZone *>(exclPrim);
          if (!zone) {
            nlwarning("Child %d of child %d of primitive %s is not a zone",
                      (int)l, (int)m, path.c_str());
            continue;
          }
          // draw exclude polygon
          drawPolyInBuildGrid(exclZone->VPoints, primitiveIndex, buildGrid,
                              floraSheet, true);
        }
      }
      ++primitiveIndex;
...
void CMicroLifeManager::release() {
  ...
  for (TTileIDToFX::iterator it = _ActiveFXs.begin(); it != _ActiveFXs.end();
       ++it) {
...
void CMicroLifeManager::tileAdded(const NL3D::CTileAddedInfo &infos) {
  ...
  if (gridCoordX < 0 || gridCoordX >= (sint)_GridWidth)
    return;
  sint gridCoordY = (sint)floorf((infos.Center.y - _MinCorner.y) / _CellSize);
  if (gridCoordY < 0 || gridCoordY >= (sint)_GridHeight)
    return;
  ...
  const CGridOverlapPolyInfoVector &iv =
      _PossibleOverlapPolyLists[_Grid[gridCoordX + gridCoordY * _GridWidth]];
  if (iv.V.empty())
    return;
  ...
  do {
    ...
    do {
      if (iv.V[k].IsExcludePoly) {
        ...
        if (iv.V[k].Poly.contains(
                NLMISC::CVector2f(infos.Center.x, infos.Center.y))) {
          inside = false;
          // jump to next primitive
          for (;;) {
            if (k == iv.V.size())
              break;
            if (iv.V[k].PrimitiveIndex != currPrimIndex)
              break;
            ++k;
          }
          break;
    ...
    // Compute noise at center of the tile, and if it is over the micro-life
    // threshold, then spawn a fx To avoid that each kind of primitive be
    // created at the same time, add an abitrary bias to the position for each
    // primitive type
    float noise = computeUniformNoise(
        _Noise, infos.Center + (float)(currPrimIndex + 1) * _CellSize *
                                   1.2564f * NLMISC::CVector::K);
    if (noise > fs->MicroLifeThreshold) {
      // choose a category of plant/microlife to instanciate by computing some
      // noise around
      float mlCategory = computeUniformNoise(
          _Noise, infos.Center +
                      (float)(k + 1) * _CellSize * 1.254f * NLMISC::CVector::J +
                      1.421f * NLMISC::CVector::I);
      uint64 plantWeightedIndex =
          (uint64)((double)mlCategory * fs->getPlantInfoTotalWeight());
      const CPlantInfo *pi =
          fs->getPlantInfoFromWeightedIndex(plantWeightedIndex);
      if (!pi)
        continue;
      ...
      if (infos.Normal.z < cosMin)
        continue; // not valid
      if (infos.Normal.z > cosMax)
        continue; // not valid
      ...
      weight[0] = computeUniformNoise(
          _Noise, infos.Center + (float)(currPrimIndex + 1) * _CellSize *
                                     1.415f * NLMISC::CVector::I);
      weight[1] = computeUniformNoise(
          _Noise, infos.Center - (float)(currPrimIndex + 1) * _CellSize *
                                     1.568f * NLMISC::CVector::I);
      weight[2] = computeUniformNoise(
          _Noise, infos.Center - (float)(currPrimIndex + 1) * _CellSize *
                                     1.512f * NLMISC::CVector::J);
      bool tri = computeUniformNoise(_Noise,
                                     infos.Center + (float)(currPrimIndex + 1) *
                                                        _CellSize * 1.898f *
                                                        NLMISC::CVector::I) >
                 0.5f; // choose a tri to use
      // if by extraordinary...
      ...
      // choose one of the tri to spawn fx (the 2 tri of the quad are not
      // planar)
      if (tri) {
        newFX.SpawnPosition = invTotalWeight * (weight[0] * infos.Corners[0] +
                                                weight[1] * infos.Corners[1] +
                                                weight[2] * infos.Corners[2]);
      } else {
        newFX.SpawnPosition = invTotalWeight * (weight[0] * infos.Corners[1] +
                                                weight[1] * infos.Corners[2] +
                                                weight[2] * infos.Corners[3]);
      }
      ...
      if (sfs.AlignOnWater) {
        float height;
        bool splashEnabled;
        bool hasWater = ContinentMngr.cur()->WaterMap.getWaterHeight(
            NLMISC::CVector2f(newFX.SpawnPosition.x, newFX.SpawnPosition.y),
            height, splashEnabled);
        if (hasWater) {
      ...
      // fit K axis of model with the normal, by doing a rotation around K ^
      // Normal by an angle alpha whose cos(alpha) is K * Normal = Normal.Z K ^
      // Normal resolves to [-y x 0] if z is near from 1.f or -1.f then no
      // rotation is performed (because [-y x 0] tends to 0 and can't be
      // normalized)
      ...
      if (!sfs.DontRotate) {
        if (1.f - infos.Normal.z < 10e-4) {
          rotAxis =
              NLMISC::CVector::I; // any axis in the (x, y) plane will be ok
          angle = 0.f;
      ...
      if (!sfs.DontRotateAroundLocalZ) {
        // Once instance is positionned, rotate it around its normal for more
        // variety
        angleAroundNormal = computeUniformNoise(
            _Noise, infos.Center + (float)(currPrimIndex + 1) * _CellSize *
                                       1.1213f * NLMISC::CVector::K);
      }
      // build final rot
      if (!sfs.DontRotate && !sfs.DontRotateAroundLocalZ) {
        newFX.Rot = NLMISC::CQuat(infos.Normal, angleAroundNormal) *
                    NLMISC::CQuat(rotAxis, angle);
      ...
      if (!sfs.WantScaling) {
        ...
        if (sfs.UniformScale) {
          float scaleBlend = computeUniformNoise(
              _Noise, infos.Center + (float)currPrimIndex * _CellSize *
                                         3.2371f * NLMISC::CVector::J);
          newFX.Scale =
              scaleBlend * sfs.ScaleMax + (1.f - scaleBlend) * sfs.ScaleMin;
        } else {
          // compute a different blend factor for each axis
          CVector scaleBlend(
              computeUniformNoise(_Noise, infos.Center +
                                              (float)currPrimIndex * _CellSize *
                                                  3.2371f * NLMISC::CVector::J),
              computeUniformNoise(_Noise, infos.Center +
                                              (float)currPrimIndex * _CellSize *
                                                  2.9784f * NLMISC::CVector::J),
              computeUniformNoise(
                  _Noise, infos.Center + (float)currPrimIndex * _CellSize *
                                             1.1782f * NLMISC::CVector::J));
          newFX.Scale.set(scaleBlend.x * sfs.ScaleMax.x +
                              (1.f - scaleBlend.x) * sfs.ScaleMin.x,
                          scaleBlend.y * sfs.ScaleMax.y +
                              (1.f - scaleBlend.y) * sfs.ScaleMin.y,
                          scaleBlend.z * sfs.ScaleMax.z +
                              (1.f - scaleBlend.z) * sfs.ScaleMin.z);
        }
      }
      newFX.FXSheet = &sfs;
  ...
  if (!fxToSpawn.empty()) {
    CTimedFXManager::TFXGroupHandle fxsHandle =
        CTimedFXManager::getInstance().add(fxToSpawn, CurrSeason);
#ifdef NL_DEBUG
...
void CMicroLifeManager::tileRemoved(uint64 id) {
  ...
  H_AUTO_USE(RZ_MicroLifeManager)
#ifdef NL_DEBUG
  CHashMap<uint64, NL3D::CTileAddedInfo>::iterator tileIt =
      _ActiveTiles.find(id);
...
static const NLMISC::CRGBA DebugCols[] = {
    NLMISC::CRGBA(255, 32, 32),  NLMISC::CRGBA(32, 255, 32),
    NLMISC::CRGBA(255, 255, 32), NLMISC::CRGBA(32, 255, 255),
    NLMISC::CRGBA(255, 32, 255), NLMISC::CRGBA(255, 127, 32),
  ...
  for (uint x = 0; x < _GridWidth; ++x) {
    for (uint y = 0; y < _GridHeight; ++y) {
      if (_Grid[x + y * _GridWidth] == 0) {
        ...
        pix[x + y * _GridWidth] =
            DebugCols[_Grid[x + y * _GridWidth] % NumDebugCols];
      }
    }
  }
  NLMISC::COFile f;
...
// ********************************************************************************************
void CMicroLifeManager::renderMLZones(const NLMISC::CVector2f &camPos,
  ...
  NL3D::CFrustum fr;
  Scene->getCam().getFrustum(fr.Left, fr.Right, fr.Bottom, fr.Top, fr.Near,
                             fr.Far);
  fr.Perspective = true;
  ...
  for (uint k = 0; k < _PossibleOverlapPolyLists.size(); ++k) {
    const CGridOverlapPolyInfoVector &currPolyList =
        _PossibleOverlapPolyLists[k];
    for (uint l = 0; l < currPolyList.V.size(); ++l) {
      const std::vector<NLMISC::CVector2f> &verts =
          currPolyList.V[l].Poly.Vertices;
      ...
      for (uint m = 0; m < verts.size(); ++m) {
        ...
        line.V1.set(verts[(m + 1) % verts.size()].x,
                    verts[(m + 1) % verts.size()].y, userZ);
  ...
  for (uint x = 0; x < _GridWidth; ++x) {
    for (uint y = 0; y < _GridHeight; ++y) {
      const CGridOverlapPolyInfoVector &currPolyList =
          _PossibleOverlapPolyLists[_Grid[x + _GridWidth * y]];
      if (currPolyList.V.empty())
        continue;
      // see if cell not too far
      NLMISC::CVector2f pos(x * _CellSize + _MinCorner.x,
                            y * _CellSize + _MinCorner.y);
      if ((camPos - pos).norm() > maxDist)
        continue; // too far, don't display
      // display box for each primitive type
      NLMISC::CVector cornerMin(pos.x, pos.y, userZ - 5.f);
      NLMISC::CVector cornerMax(pos.x + _CellSize, pos.y + _CellSize,
                                userZ + 5.f);
      for (uint l = 0; l < currPolyList.V.size(); ++l) {
        // add a bias each time to see when several primitives are overlapped
        NLMISC::CVector bias = (float)currPolyList.V[l].PrimitiveIndex *
                               NLMISC::CVector(0.01f, 0.f, 0.1f);
...
void CMicroLifeManager::renderActiveTiles(){
    ...
    NL3D::CFrustum fr;
    Scene->getCam().getFrustum(fr.Left, fr.Right, fr.Bottom, fr.Top, fr.Near,
    fr.Far); fr.Perspective = true; Driver->setFrustum(fr);
    Driver->setModelMatrix(NLMISC::CMatrix::Identity);
    ...
    for(std::hash_map<uint64, NL3D::CTileAddedInfo>::iterator it =
    _ActiveTiles.begin(); it != _ActiveTiles.end(); ++it)
    {
            Driver->drawLine(NLMISC::CLine(it->second.Corners[0],
    it->second.Corners[1]), *mat);
            Driver->drawLine(NLMISC::CLine(it->second.Corners[1],
    it->second.Corners[2]), *mat);
            Driver->drawLine(NLMISC::CLine(it->second.Corners[2],
    it->second.Corners[3]), *mat);
            Driver->drawLine(NLMISC::CLine(it->second.Corners[3],
    it->second.Corners[0]), *mat);
    }
    mat->setColor(CRGBA::Red);
    for(std::hash_map<uint64, NL3D::CTileAddedInfo>::iterator it =
    _ActiveTilesWithFX.begin(); it != _ActiveTilesWithFX.end(); ++it)
    {
            Driver->drawLine(NLMISC::CLine(it->second.Corners[0],
    it->second.Corners[1]), *mat);
            Driver->drawLine(NLMISC::CLine(it->second.Corners[1],
...
NLMISC_COMMAND(showMLActiveTiles, "display micro-life active tiles",
               "<0 = off, 1 = on>") {
  if (args.size() != 1)
    return false;
...
NLMISC_COMMAND(reloadMLZones, "reload micro-life zones", "") {
  ...
  for (CSheetManager::TEntitySheetMap::const_iterator it = sm.begin();
    ...
    if (it->second.EntitySheet &&
        it->second.EntitySheet->Type == CEntitySheet::PLANT) {
