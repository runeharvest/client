...
void CTeleport::load(const std::string & /* filename */) {
  /*
  // TRAP : no more teleport list to load from georges file if you want this
  feature again ask me
          ...
          if(formLoader)
          {
                  ...
                  if(!form)
                  {
                          nlwarning("CTeleport::load: can't load form '%s'.",
                  ...
                  for(uint i = 0; i<arrawSize; ++i)
                  {
                          ...
                          if(tpElmt)
                          {
                                  ...
                                  if(tpElmt->getValueByName(destName, "name"))
                                  {
                                          // Get the position
                                          CVector pos;
                                          if(tpElmt->getValueByName(pos.x,
  "position.X")
                                          && tpElmt->getValueByName(pos.y,
  "position.Y")
                                          && tpElmt->getValueByName(pos.z,
  "position.Z"))
                                          {
                                                  // All in UPPER CASE to not be
  CASE SENSITIVE. _Destinations.insert(make_pair(NLMISC::toLowerAscii(destName),
  pos));
                                          }
                                          else
                                                  nlwarning("CTeleport::load:
  Cannot find the one of the key 'position.X or Y or Z' for the element %d.",
  i);
                                  }
                                  else
                                          nlwarning("CTeleport::load: Cannot
  find the key 'name' for the element %d.", i);
                          }
                          else
                                  nlwarning("CTeleport::load: element (%d)
  should be here in 'list'.", i);
                  }
          }
          else
