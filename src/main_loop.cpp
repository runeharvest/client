...
bool bZeroCpu = false; // For no Cpu use if application is minimize  TODO:
                       // intercept minimize message, called by CTRL + Z at this
...
// temp
float DelayBeforeCloudUpdate =
    0.f; // delay in s before cloud state must be recomputed
...
// CGraph				FpsGraph ("frame rate (fps)", 10.0f,
// 110.0f, 100.0f, 100.0f, CRGBA(128,0,0,128), 1000, 60.0f); CGraph
// SpfGraph ("mspf", 10.0f, 10.0f, 100.0f, 100.0f, CRGBA(0,128,0,128), 0,
// 800.0f);
//  TestYoyo
// CGraph CameraThirPersonGraph ("Camera Thir Person", 300.0f, 460.0f, 200.0f,
// 200.0f, CRGBA(0,64,128,128), 0, 5.0f, 1);
...
7 : Cloud
8 : CoarseMesh*/
bool Filter3D[RYZOM_MAX_FILTER_3D] = {true, true, true, true, true,
...
// validate current dialogs (end them if the player is too far from its
// interlocutor)
...
void preRenderNewSky() {
  ...
  // setup fog, lighting & sky object. We use the same light direction than with
  // the main scene
  CClientDate cd = SmoothedClientDate;
  if (ClientCfg.R2EDEnabled && R2::getEditor().getFixedLighting()) {
    cd.Hour = 12.f;
  }
  sky.setup(cd, SmoothedClientDate, WeatherManager.getWeatherValue(),
            MainFogState.FogColor, Scene->getSunDirection(), false);
}
...
void beginRenderCanopyPart() { SceneRoot->beginPartRender(); }
void endRenderCanopyPart() { SceneRoot->endPartRender(false); }

void beginRenderMainScenePart() { Scene->beginPartRender(); }
...
static void renderCanopyPart(UScene::TRenderPart renderPart) {
  ...
  ContinentMngr.getFogState(CanopyFog, LightCycleManager.getLightLevel(),
                            LightCycleManager.getLightDesc().DuskRatio,
                            LightCycleManager.getState(), View.viewPos(),
                            RootFogState);
...
static void renderMainScenePart(UScene::TRenderPart renderPart,
                                bool wantTraversals, bool keepTraversals) {
...
void clearBuffers() {
  if (Render) {
    ...
    // Sky is used to clear the frame buffer now, but if in line or point
    // polygon mode, we should draw it
  ...
  if (WaterEnvMapRefCount > 0) // water env map needed
  {
    if (!WaterEnvMap) {
      ...
      if (sky.getScene()) {
        ...
        WaterEnvMapSkyCam =
            sky.getScene()->createCamera(); // deleted in unselect
...
void updateWeather() {
  ...
  ThunderColor.modulateFromui(WeatherManager.getCurrWeatherState().ThunderColor,
                              (uint)(256.f * WeatherManager.getThunderLevel()));
  ...
  ContinentMngr.getFogState(MainFog, LightCycleManager.getLightLevel(),
                            LightCycleManager.getLightDesc().DuskRatio,
                            LightCycleManager.getState(), View.viewPos(),
                            MainFogState);

  // TODO: ZBuffer clear was originally before this, but should not be necessary
  // normally. The anim function renders new clouds. Ensure this does not break.
...
void beginRenderScene() {
  // Update Filter Flags
  Scene->enableElementRender(UScene::FilterAllMeshNoVP,
                             Filter3D[FilterMeshNoVP]);
  ...
  Scene->enableElementRender(UScene::FilterLandscape,
                             Filter3D[FilterLandscape]);
  ...
  Scene->enableElementRender(UScene::FilterCoarseMesh,
                             Filter3D[FilterCoarseMesh]);
...
void drawRenderScene(bool wantTraversals, bool keepTraversals) {
  ...
  if (s_SkyMode != NoSky)
    renderSkyPart((UScene::TRenderPart)(UScene::RenderOpaque |
                                        UScene::RenderTransparent));
  renderCanopyPart(
      (UScene::TRenderPart)(UScene::RenderTransparent | UScene::RenderFlare));
  renderMainScenePart(
      (UScene::TRenderPart)(UScene::RenderTransparent | UScene::RenderFlare),
...
void updateGameQuitting() {
  ...
  // Yoyo: prefer now leave the user press "Quit Now" if don't want to wait
  // server
  /*
  ...
  CInterfaceGroup *group = dynamic_cast<CInterfaceGroup *>(
      CWidgetManager::getInstance()->getElementFromId(
          "ui:interface:free_trial_game_quitting"));
  if (group) {
    // if Free trial
  ...
  group = dynamic_cast<CInterfaceGroup *>(
      CWidgetManager::getInstance()->getElementFromId(
          "ui:interface:game_quitting"));
  if (group) {
    // if exit request
    if (game_exit_request && !paying_account_request) {
      ...
      if (group != CWidgetManager::getInstance()->getModalWindow()) {
        ...
        // Far TP: skipping not allowed (because we can't duplicate the
        // avatar...), anyway the quit button would quit the game (no far tp)
        CInterfaceElement *quitNowButton =
            group->getElement(group->getId() + ":indent_middle:ryzom");
        if (quitNowButton)
          quitNowButton->setActive(!farTPing);
        // From ring session or from mainland's ring access point: cancelling
        // not allowed (would lead to inconsistencies with the SU & DSS)
        CInterfaceElement *backToGame =
            group->getElement(group->getId() + ":indent_middle:cancel");
        if (backToGame)
          backToGame->setActive(
              !(farTPing &&
                (IsInRingSession || FarTP.isFastDisconnectGranted())));
        CInterfaceElement *quittingRules =
            group->getElement(group->getId() + ":text_mean");
        if (quittingRules)
          quittingRules->setActive(!farTPing);
        CInterfaceElement *quittingText =
            group->getElement(group->getId() + ":text");
        if (quittingText)
...
bool mainLoop() {
  ...
  nlinfo("Starting main loop...", (uint32)(initCurrent - initLast) / 1000,
         (uint32)(initCurrent - initStart) / 1000);
  ...
  T1 = ryzomGetLocalTime(); //(sint64)CGDTime::getTime ();	// \todo GUIGUI
                            //: VOIR COMMENT MANAGER LE TEMPS
  ...
  CNetworkConnection::TConnectionState lastConnectionState =
      CNetworkConnection::Connected;
  CNetworkConnection::TConnectionState connectionState =
      NetMngr.getConnectionState();
  ...
  // For stoping the outgame music, start after 30 frames, and duration of 3
  // seconds
  CMusicFader outgameFader(60, 3);

  // check for banned player
  if (testPermanentBanMarkers()) {
    setPermanentBanMarkers(true); // re-set any marker that could have been
                                  // removed by the trouble maker
...

  {
    ...
    nlinfo("PROFILE: %d seconds (%d total) for Starting main loop",
           (uint32)(initCurrent - initLast) / 1000,
           (uint32)(initCurrent - initStart) / 1000);
    ...
    CSessionBrowserImpl::getInstance().init(
  ...
  if (StartInitTime != 0) {
    CLoginProgressPostThread::getInstance().step(CLoginStep(
        LoginStep_InGameEntry,
        "login_step_game_entry&load_time=" +
            toString((NLMISC::CTime::getLocalTime() - StartInitTime) / 1000)));
    StartInitTime = 0;
  }
  ...
  while (!UserEntity->permanentDeath() && !game_exit) {
    // If an action handler execute. NB: MUST BE DONE BEFORE ANY THING ELSE
    // PROFILE CRASH!!!!!!!!!!!!!!!!!
    testLaunchProfile();
    ...
    if (isBGDownloadEnabled()) {
      ...
      // if the ui is frozen, wait for a few frame to allow client to do first
      // frame load, without having to compete with the downloader frame
      if (!FirstFrame && bgDownloader.isDownloaderUIFrozen() &&
          SkipFrame == 0) {
        unpauseBGDownloader();
      }
    }
...
#define BAR_STEP_TP 2
...

    {
      ...
      if (ClientCfg.R2EDEnabled) {
        ...
        if (UserEntity) {
          UserEntity->updateNpcContolSpeed();
        }
      }
    ...
    CInterfaceManager *pIMinstance;
    {
      ...
      EventsListener.updateMouseSmoothing(); // IMPORTANT: this should be called
                                             // before updateClientTime &&
                                             // events handling in the frame
      ...
      // NB: must update frame events, even if ShowInterface==0, else may
      // OutOfMemory (cause vector<> never cleared).
      pIMinstance->updateFrameEvents();

      if ((ContinentMngr.cur()) && (UserEntity != NULL))
        ContinentMngr.cur()->FoW.explore((float)UserEntity->pos().x,
                                         (float)UserEntity->pos().y);
    ...
    if (pIMinstance) {
      ...
      if (!CViewRenderer::getInstance()->isMinimized()) {
        ...
        CViewPointer *cursor = static_cast<CViewPointer *>(
...

    {
      ...
      if ((!newDatabaseInitStatus) && prevDatabaseInitStatus) {
        // When database received, activate allegiance buttons (for neutral
        // state) in fame window
        CInterfaceManager *pIM = CInterfaceManager::getInstance();
        CInterfaceGroup *group = dynamic_cast<CInterfaceGroup *>(
            CWidgetManager::getInstance()->getElementFromId(
                "ui:interface:fame:content:you"));
        if (group)
          group->updateAllLinks();
        // send a msg to lua for specific ui update
        CLuaManager::getInstance().executeLuaScript(
            "game:onInGameDbInitialized()");
      }
    }
    ...
    // Update Ryzom Time.
    {
      ...
      if (!ClientCfg.Local) {
        ...
        SimulatedServerDate =
            (float)((double)SimulatedServerDate - (double)numTicks * 0.1);
    ...
    if (Landscape)
      Landscape->setRefineCenterUser(View.refinePos());
    ...
    if (StereoHMD) {
      ...
      NLMISC::CMatrix
          posMatrix; // minimal head modeling, will be changed in the future
    ...
    if (ContinentMngr.cur()) {
      float waterHeight;
      bool splashEnabled;
      if (ContinentMngr.cur()->WaterMap.getWaterHeight(
              CVector2f(currViewPos.x, currViewPos.y), waterHeight,
              splashEnabled)) {
    ...
    if (Landscape) {
      if (!ClientCfg.Light) {
        // Not in an indoor ?
        if (ContinentMngr.cur() && !ContinentMngr.cur()->Indoor) {
          // Load Zone in streaming according to the refine position (not
          // necessarily the User Position);
          string zoneAdded, zoneRemoved;
          const R2::CScenarioEntryPoints::CCompleteIsland *ci =
              R2::CScenarioEntryPoints::getInstance()
                  .getCompleteIslandFromCoords(CVector2f(
                      (float)UserEntity->pos().x, (float)UserEntity->pos().y));
          Landscape->refreshZonesAround(
              View.refinePos(), ClientCfg.Vision + ExtraZoneLoadingVision,
              zoneAdded, zoneRemoved, ci ? &(ci->ZoneIDs) : NULL);
    ...
    if (!ClientCfg.Light) {
      ...
      if (newDate < CTimedFXManager::getInstance().getDate() ||
          abs((sint32)RT.getRyzomDay() -
              CTimedFXManager::getInstance().getDate().Day) > 1) {
        // The manager make the assumption that no more than one day can occurs
        // between 2 ticks This only happens when date is changed manually
        if (IGCallbacks) {
          IGCallbacks->changeSeason(); // the season doesn't change, but this
                                       // force fxs to be recreated
        }
      }
      CTimedFXManager::getInstance().update(newDate, CurrSeason,
                                            Scene->getCam().getPos());
    ...
    if (GR) {
      ...
      if (UserControls.mode() != CUserControls::ThirdMode) {
        ...
        // important to update this each frame, for shadow map consideration
        // against the "matis serre bug"
      ...
      // Camera 3rd person complex mode
        ...
        LastDebugClusterCameraThirdPersonPelvisPos =
            View.viewPos() + CVector(0.f, 0.f, 1.f);
    ...
    if (haveEffects) {
      ...
      if (ClientCfg.Bloom) {
        CBloomEffect::getInstance().setSquareBloom(ClientCfg.SquareBloom);
        CBloomEffect::getInstance().setDensityBloom(
            (uint8)ClientCfg.DensityBloom);
      }
    }
    bool fullDetail = false;
    while ((!StereoDisplay && i == 0) ||
           (StereoDisplay && StereoDisplay->nextPass())) {
      ++i;
      ///////////////////
      // SETUP CAMERAS //
      ///////////////////
      ...
      bool stereoRenderTarget =
          (StereoDisplay != NULL) && StereoDisplay->beginRenderTarget();
      ...
      if (!StereoDisplay || StereoDisplay->wantScene()) {
        if (!ClientCfg.Light && Render) {
          if (!StereoDisplay || StereoDisplay->isSceneFirst()) {
            ...
            if (!fullDetail) {
              fullDetail = ScreenshotRequest != ScreenshotRequestNone &&
                           ClientCfg.ScreenShotFullDetail;
              if (fullDetail) {
                s_ForceFullDetail.backup();
                s_ForceFullDetail.set();
              }
      ...
      if (!StereoDisplay || StereoDisplay->wantInterface3D()) {
        if (!ClientCfg.Light) {
          ...
          //				backgroundBitmap =
          //Driver->createTextureFile("temp_background.tga");
          //			Driver->setMatrixMode2D11();
          //			Driver->drawBitmap (0.f, 0.f, 1024.f/1024.f,
          //1024.f/768.f, (UTexture&)*backgroundBitmap);
          //			Driver->setMatrixMode3D(MainCam);
        ...
        // Display some things not in the scene like the name, the entity path,
        // etc.
      ...
      if (!StereoDisplay || StereoDisplay->wantInterface2D()) {
        ...
        // Update the contextual menu
        {
          ...
          // special case in OpenGL : all scene has been display in render
          // target, now, final texture is display with a quad
          /*if (!ClientCfg.Light && ClientCfg.Bloom && Render && bloomStage ==
          2) // NO VR BLOOMZ
          {
                  // End bloom effect system after drawing the 3d interface (z
          buffer related). if (StereoDisplay)
          Driver->setViewport(NL3D::CViewport());
                  CBloomEffect::instance().endInterfacesDisplayBloom();
                  if (StereoDisplay)
          Driver->setViewport(StereoDisplay->getCurrentViewport()); bloomStage =
          0;
          }*/
        }

        {
          ...
          if (!Driver->isLost()) {
            ...
            for (uint i = 0; i < ClientCfg.Logos.size(); i++) {
              ...
              if (res.size() == 9 && i < LogoBitmaps.size() &&
                  LogoBitmaps[i] != NULL) {
                fromString(res[5], x);
                fromString(res[6], y);
                ...
                Driver->drawBitmap(
                    x / (float)ClientCfg.Width, y / (float)ClientCfg.Height,
                    width / (float)ClientCfg.Width,
                    height / (float)ClientCfg.Height, *LogoBitmaps[i]);
              }
            }
          }
        }
        ...
        if (ClientCfg.R2EDEnabled) {
          // IMPORTANT : this should be called after
          // CEntitiesMngr::updatePostRender() because entity may be added /
          // removed there !
        ...
        // and what he was doing. He can't move because the connection quality
        // returns false.

        if ((connectionState == CNetworkConnection::Disconnect) &&
            (lastConnectionState != CNetworkConnection::Disconnect) &&
            (!FarTP.isFarTPInProgress())) {
          UserControls.stopFreeLook(); // let the player click on Exit
          pIMinstance->messageBoxWithHelp(CI18N::get("uiDisconnected"));

          // If we have started a Far TP sequence and are waiting for
          // onServerQuitOK() from the EGS, resume the sequence because the EGS
          // is down and won't reply.
        ...
        if (ClientCfg.ManualWeatherSetup) {
          H_AUTO_USE(RZ_Client_Main_Loop_Debug)
          static float displayHourDelta =
              0.04f; // static for edition during debug..
          ...
          if (DisplayWeatherFunction) {
            ...
            for (uint x = 0; x < wndWidth; ++x) {
              float weatherValue;
              if (ContinentMngr.cur())
                weatherValue = ::getBlendedWeather(
                    currDay, currHour, *WeatherFunctionParams,
                    ContinentMngr.cur()->WeatherFunction);
              else
                weatherValue = ::getBlendedWeather(currDay, currHour,
                                                   *WeatherFunctionParams, 0);

              NLMISC::clamp(weatherValue, 0.f, 1.f);
              CRGBA seasonToColor[EGSPD::CSeason::Invalid] = {
                  CRGBA::Green, CRGBA::Yellow, CRGBA::Red, CRGBA::Blue};
              Driver->drawLine(
                  (float)x, 0.f, (float)x, lineHeight * weatherValue,
                  seasonToColor[CRyzomTime::getSeasonByDay((uint32)currDay)]);
            ...
            if (ContinentMngr.cur()) {
              ...
              uint numWeatherSetups = ContinentMngr.cur()
                                          ->WeatherFunction[CurrSeason]
                                          .getNumWeatherSetups();
              ...
              0) // if time is forced then can't change it manually ...
          {
            ...
            if (Actions.valide("dec_hour")) {
              ...
              RT.updateRyzomClock(NetMngr.getCurrentServerTick());
              CTimedFXManager::getInstance().setDate(
                  CClientDate(RT.getRyzomDay(), (float)RT.getRyzomTime()));
              if (IGCallbacks) {
                IGCallbacks->changeSeason(); // the season doesn't change, but
                                             // this force fxs to be recreated
              }
            }
          }
  ...
  if (Actions.valide ("weather_stats"))
  {
          ...
          if(ContinentMngr.cur())
                  CPredictWeather::generateWeatherStats("weather_stats.csv",
  WeatherFunctionParams, ContinentMngr.cur()->WeatherFunction);
  }*/
...
#if !FINAL_VERSION
        CVector2f camPos(Scene->getCam().getPos().x,
                         Scene->getCam().getPos().y);
        if (!ClientCfg.Light) {
    ...
    if (SkipFrame == 0) {
      ...
      // Start background sound play now !  (nb: restarted if load just ended,
      // or if sound re-enabled)
      if (SoundMngr) {
        H_AUTO_USE(RZ_Client_Main_Loop_Sound)

        // fade out loading music
        if (SoundMngr->getEventMusicPlayed() == LoadingMusic) {
          SoundMngr->stopEventMusic(LoadingMusic,
      ...
      // If the device is lost then no rendering will occur, so let some time to
      // other applications
      if (Driver->isLost()) {
        nlSleep(50);
        nldebug("lost device");
      ...
      //			UTransformable::TTransformMode m =
      //MainCam.getTransformMode();
      if (SkipFrame == 0) {
        MainCam.setRotQuat(MainCamOri);
      } else {
        CMatrix mat = CMatrix::Identity;
        mat.setRot(MainCamOri);
        mat.rotateZ(2 * (float)Pi *
                    ((float)(SkipFrame) / (float)RYZOM_FIRST_FRAME_TO_SKIP));
        CQuat qTmp;
        mat.getRot(qTmp);
    ...
    if (NetMngr.getCurrentServerTick() > LastGameCycle) {
      ...
      // Put here things you have to send to the server only once per tick like
      // user position. UPDATE COMPASS
      NLMISC::CCDBNodeLeaf *node =
          s_UiDirectionLeaf
              ? (&*s_UiDirectionLeaf)
              : &*(s_UiDirectionLeaf =
                       NLGUI::CDBManager::getInstance()->getDbProp(
                           "UI:VARIABLES:DIRECTION"));
      ...
      // Give information to the server about the combat position (ability to
      // strike).
      {
        CBitMemStream out;
      ...
      // Create the message for the server to move the user (except in combat
      // mode).
      if (Ping.rdyToPing()) {
        CBitMemStream out;
    ...
    // FAR_TP -> //
    ///////////////
    // Enter a network loop during the FarTP process, without doing the whole
    // real main loop. This code must remain at the very end of the main loop.
    if (LoginSM.getCurrentState() ==
      ...
      if (FarTP.isReselectingChar()) {
        ...
        // check for banned player
        if (testPermanentBanMarkers()) {
          setPermanentBanMarkers(true); // re-set any marker that could have
                                        // been removed by the trouble maker
          applyPermanentBanPunishment();
          PermanentlyBanned = true;
        }
      }
      ...
      //			TODO: temporary commented, CEditor must be
      //initialized before to call next lines 			if (!IsInRingSession)
      //				R2::getEditor().registerLuaFunc();

      CurrSeason = computeCurrSeason();

      // Get the Connection State (must be done after any Far TP to prevent the
      // uiDisconnected box to be displayed)
  ...
  if (!FarTP.isReselectingChar()) // skip some parts if the user wants to quit
                                  // in the middle of a char reselect
  {
    ...
    //	TTime endTime = ryzomGetLocalTime () +
    //(TTime)(ClientCfg.EndScreenTimeOut*1000.f); 	do
    //	{
    //		// Grab Inputs.
    //		CInputHandlerManager::getInstance()->pumpEventsNoIM();
...
void endMovieShooting() { MovieShooterSaving = false; }
    ...
    if (!MovieShooter.enabled()) {
      ...
      if (!MovieShooterSaving) {
        try {
          ...
          // Save the movie.
          MovieShooter.saveMovie(Driver, TextContext, theDir.c_str(),
                                 ClientCfg.MovieShooterFramePeriod,
                                 ClientCfg.MovieShooterBlend,
                                 ClientCfg.MovieShooterPrefix.c_str());
        } catch (const Exception &e) {
          Driver->systemMessageBox(e.what(), "MovieShooter");
        }
      }
...
//===================================================================================================
static void updateCloudScape(const CCloudState &desc, const CWeatherContext &wc,
                             float /* wind */, float dayNight,
                             float updateDelay, bool mustInit) {
  if (!CloudScape)
    return;
  ...
  case CLightCycleManager::DayToNight:
    if (dayNight < duskRatio) // day->dusk
    {
      ...
      css.Ambient.blendFromui(desc.AmbientDay, desc.AmbientDusk,
                              (uint)(256.f * blendFactor));
      css.Diffuse.blendFromui(desc.DiffuseDay, desc.DiffuseDusk,
                              (uint)(256.f * blendFactor));
    } else // dusk->night
    {
      float blendFactor =
          duskRatio != 1 ? (dayNight - duskRatio) / (1.f - duskRatio) : 0.f;
      css.Ambient.blendFromui(desc.AmbientDusk, desc.AmbientNight,
                              (uint)(256.f * blendFactor));
      css.Diffuse.blendFromui(desc.DiffuseDusk, desc.DiffuseNight,
                              (uint)(256.f * blendFactor));
    }
    break;
  default: // not a day->night transition, so no step for dusk
    css.Ambient.blendFromui(desc.AmbientDay, desc.AmbientNight,
                            (uint)(256.f * dayNight));
    css.Diffuse.blendFromui(desc.DiffuseDay, desc.DiffuseNight,
                            (uint)(256.f * dayNight));
    break;
  }
  css.NbCloud = desc.NumClouds;
  css.WindSpeed = WeatherManager.getCurrWeatherState().WindIntensity *
                      wc.WFP->CloudWindSpeedFactor +
                  wc.WFP->CloudMinSpeed;
  if (mustInit) {
    css.TimeToChange = 0;
    CloudScape->init(&css);
...
void updateClouds() {
  ...
  if (ClientCfg.ManualWeatherSetup && !ForceTrueWeatherValue) {
    ...
    if (CloudScape) {
      CCloudState cs;
      WeatherManager.computeCloudState(ManualWeatherValue, CurrSeason, cs,
                                       wc.WF);
      updateCloudScape(cs, wc,
                       WeatherManager.getCurrWeatherState().WindIntensity,
                       LightCycleManager.getLightLevel(), 1.f, InitCloudScape);
    ...
    if (InitCloudScape) {
      ...
      // 1 ) set current state
      CCloudState cs;
      WeatherManager.computeCloudState(RT.getRyzomDay(), DayNightCycleHour, wc,
                                       cs);
      updateCloudScape(cs, wc,
                       WeatherManager.getCurrWeatherState().WindIntensity,
                       LightCycleManager.getLightLevel(), 0.f, true);
      // 2 )set next state
      ...
      WeatherManager.computeCloudState(RT.getRyzomDay(),
                                       DayNightCycleHour + updateDelay, wc, cs);
      updateCloudScape(cs, wc,
                       WeatherManager.getCurrWeatherState().WindIntensity,
                       LightCycleManager.getLightLevel(), 0.f, true);
      ...
      if (DelayBeforeCloudUpdate <= 0.f) {
        ...
        if (lcd.RealDayLength != 0.f) {
          updateDelay =
              DelayBeforeCloudUpdate / lcd.RealDayLength * lcd.NumHours;
        }
        CCloudState cs;
        WeatherManager.computeCloudState(
            RT.getRyzomDay(), DayNightCycleHour + updateDelay, wc, cs);
        updateCloudScape(cs, wc,
                         WeatherManager.getCurrWeatherState().WindIntensity,
                         LightCycleManager.getLightLevel(), updateDelay, false);
      }
    }
  }
}
...
CVector PacsBox[PacsBoxPointCount] = {
    CVector(-0.5f, -0.5f, 0), CVector(0.5f, -0.5f, 0),  CVector(0.5f, -0.5f, 0),
    CVector(0.5f, 0.5f, 0),   CVector(0.5f, 0.5f, 0),   CVector(-0.5f, 0.5f, 0),
    CVector(-0.5f, 0.5f, 0),  CVector(-0.5f, -0.5f, 0),

    CVector(-0.5f, -0.5f, 1), CVector(0.5f, -0.5f, 1),  CVector(0.5f, -0.5f, 1),
    CVector(0.5f, 0.5f, 1),   CVector(0.5f, 0.5f, 1),   CVector(-0.5f, 0.5f, 1),
    CVector(-0.5f, 0.5f, 1),  CVector(-0.5f, -0.5f, 1),

    CVector(-0.5f, -0.5f, 0), CVector(-0.5f, -0.5f, 1), CVector(0.5f, -0.5f, 0),
    CVector(0.5f, -0.5f, 1),  CVector(0.5f, 0.5f, 0),   CVector(0.5f, 0.5f, 1),
...
CVector PacsCyl[PacsCylPointCount] = {
    CVector(0, 1, 0),
    CVector(0.7071067f, 0.7071067f, 0),
    CVector(0.7071067f, 0.7071067f, 0),
    CVector(1, 0, 0),
    CVector(1, 0, 0),
    CVector(0.7071067f, -0.7071067f, 0),
    ...
    CVector(0, 1, 1),
    CVector(0.7071067f, 0.7071067f, 1),
    CVector(0.7071067f, 0.7071067f, 1),
    CVector(1, 0, 1),
    CVector(1, 0, 1),
    CVector(0.7071067f, -0.7071067f, 1),
...
void displaySceneProfiles() {
  ...
  for (it = res.MeshProfileTriVBFormat.begin();
       it != res.MeshProfileTriVBFormat.end(); it++) {
    // build the format
  ...
  for (it = res.MeshMRMProfileTriVBFormat.begin();
       it != res.MeshMRMProfileTriVBFormat.end(); it++) {
    // build the format
    string format = buildStrVBFormat(it->first);
    nlinfo("   NumTris: %5d. Format: %s", it->second, format.c_str());
  }
  ...
  nlinfo("   NumMeshRdrBlockWithVBHeap TriCount: %d",
         res.NumMeshTriRdrBlockWithVBHeap);
  ...
  nlinfo("   NumMeshMRMRdrBlockWithVBHeap: %d",
         res.NumMeshMRMRdrBlockWithVBHeap);
  ...
  nlinfo("   NumMeshMRMRdrBlockWithVBHeap TriCount: %d",
         res.NumMeshMRMTriRdrBlockWithVBHeap);
...
//-----------------------------------------------
// validateDialogs();
// validate current dialogs by checking the distance between the two talking
// entities. End dialogs if they are too far
//-----------------------------------------------
void validateDialogs(const CGameContextMenu &gcm) {
...
NLMISC_DYNVARIABLE(float, FPS, "The second smoothed frame rate per second") {
  if (get)
    *pointer = 1.0f / smoothFPS.getSmoothValue();
}

// show hide all the debuging of ui
NLMISC_COMMAND(debugUI, "Debug the ui : show/hide quads of bboxs and hotspots",
               "debugUI 1 or 0") {
  if (args.size() > 1)
    return false;
  bool on = true;
...
NLMISC_COMMAND(setWeatherValue, "Set weather value", "") {
  ...
  ManualWeatherValue =
      value /
      (ContinentMngr.cur()->WeatherFunction[CurrSeason].getNumWeatherSetups() -
       1);
  return true;
}
...
class CAHShowTimedFX : public IActionHandler {
  void execute(CCtrlBase * /* pCaller */, const std::string & /* params */) {
    if (ShowTimedFX) {
      ShowTimedFXMode =
          (CTimedFXManager::TDebugDisplayMode)(ShowTimedFXMode + 1);
...
void inGamePatchUncompleteWarning() {
  ...
  *>(CWidgetManager::getInstance()->getElementFromId("ui:interface:bg_downloader"));
  if (gc)
  {
          gc->setActive(true);
