...
public:
  ...
  CFEClientRoute(IGatewayTransport *transport, CNetworkConnection *netConn)
      : CGatewayRoute(transport), NetConn(netConn) {
  ...
  void sendMessage(const CMessage &message) const {
    ...
    GenericMsgHeaderMngr.pushNameToStream("MODULE_GATEWAY:GATEWAY_MSG",
                                          wrapper);
    //		wrapper.serial(message.getName());
    //		wrapper.serialBufferWithSize(const_cast<uint8*>(message.buffer())+message.getHeaderSize(),
    //message.length()-message.getHeaderSize());
    wrapper.serialBufferWithSize(const_cast<uint8 *>(message.buffer()),
                                 message.length());
  ...
  void sendRawMessage(CBitMemStream &message) const { NetConn->push(message); }
};
...
public:
  ...
  bool _Open;
  /// A flag that is false after opening the transport until we receive one
  /// message
...
#if !defined(RZ_CLIENT_DRONE)
  // store the unique active transport (only one transport of this type can be
  // activated at a time)
  ...
  CGatewayFEClientTransport(const IGatewayTransport::TCtorParam &param)
      : IGatewayTransport(param), _Route(NULL), _Open(false),
        _FirstMessageReceived(false), _NextAwaitedMessage(0) {
#if defined(RZ_CLIENT_DRONE)
  ...
  virtual uint32 getRouteCount() const { return _Route != NULL ? 1 : 0; }
  ...
  void dump(NLMISC::CLog &log) const {
    ...
    if (!_Open) {
      ...
      if (_Route == NULL) {
        ...
        log.displayNL(
            "    The route has %u entries in the proxy translation table :",
            route->ForeignToLocalIdx.getAToBMap().size());
        {
          CGatewayRoute::TForeignToLocalIdx::TAToBMap::const_iterator first(
              route->ForeignToLocalIdx.getAToBMap().begin()),
              last(route->ForeignToLocalIdx.getAToBMap().end());
          for (; first != last; ++first) {
            IModuleProxy *modProx = mm.getModuleProxy(first->second);

            log.displayNL("      - Proxy '%s' : local proxy id %u => foreign "
                          "module id %u",
                          modProx != NULL ? modProx->getModuleName().c_str()
  ...
  bool onCommand(const TParsedCommandLine &command) {
    ...
    if (commandName == "open") {
      ...
      if (OpenTransport() != NULL) {
        nlwarning("A transport is already open, only one transport can be open "
                  "at a time");
        return false;
      }
#endif
  ...
  /// Close the server, this will close the listing socket and any active
  /// connection
  void close() {
    if (!_Open) {
  ...
  void onDispatchMessage(NLMISC::CBitMemStream &bms) {
    if (_Route == NULL) {
      nlwarning(
          "onDispatchMessage : no route created, message will be discarded");
      return;
    }
  ...
  void processPendingMessage() {
    ...
    while (it != _WaitingMessages.end()) {
      ...
      // advance to next message, _NextAwaitedMessage is incremented by
      // impulsion handler
  ...
  void impulsionGatewayMessage(NLMISC::CBitMemStream &bms,
  ...
  // impulsion gateway open callback handler
  void impulsionGatewayClose(NLMISC::CBitMemStream &bms,
                             bool readSerialNumber) {
    uint8 serialNumber;
    if (readSerialNumber)
      bms.serial(serialNumber);
  ...
  static CGatewayFEClientTransport *getCurrentTransport() {
    ...
    return static_cast<CGatewayFEClientTransport *>(
...
void cbImpulsionGatewayOpen(NLMISC::CBitMemStream &bms) {
  if (CGatewayFEClientTransport::getCurrentTransport() != NULL)
    CGatewayFEClientTransport::getCurrentTransport()->impulsionGatewayOpen(
        bms, true);
}
...
void cbImpulsionGatewayMessage(NLMISC::CBitMemStream &bms) {
  if (CGatewayFEClientTransport::getCurrentTransport() != NULL)
    CGatewayFEClientTransport::getCurrentTransport()->impulsionGatewayMessage(
        bms, true);
}
...
void cbImpulsionGatewayClose(NLMISC::CBitMemStream &bms) {
  if (CGatewayFEClientTransport::getCurrentTransport() != NULL)
    CGatewayFEClientTransport::getCurrentTransport()->impulsionGatewayClose(
        bms, true);
}

// register this class in the transport factory
NLMISC_REGISTER_OBJECT(IGatewayTransport, CGatewayFEClientTransport,
                       std::string, string(FE_CLIENT_CLASS_NAME));
