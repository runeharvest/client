...
// *************************************************************************************************
void CSky::init(UDriver *drv, const CSkySheet &sheet,
                bool forceFallbackVersion /*= false*/,
                float numHourInDay /*= 24.f*/,
  ...
  if (!sheet.AnimationName.empty()) {
    ...
    if (_PlayListManager) {
      ...
      if (_AnimationSet && _PlayList) {
        uint animationID = _AnimationSet->addAnimation(
            sheet.AnimationName.c_str(), sheet.AnimationName.c_str());
  ...
  for (uint k = 0; k < sheet.Objects.size(); ++k) {
    ...
    if (!instance.empty() &&
        !instance.supportMaterialRendering(*drv, forceFallbackVersion)) {
      if (unsupportedObjects) {
      ...
      // build fallbacks
      for (uint l = 0; l < 2; ++l) {
        UInstance fallbackInstance =
            _IG->getByName(sheet.Objects[k].FallbackPass[l].ShapeName);
        if (!fallbackInstance.empty()) {
          fallbackInstance.hide();
          CSkyObject so;
          so.init(sheet.Objects[k].FallbackPass[l], fallbackInstance,
                  buildShareBitmapByName, _Bitmaps,
                  sheet.Objects[k].VisibleInMainScene,
                  sheet.Objects[k].VisibleInEnvMap);
          _Objects.push_back(so);
          if (_PlayList) {
            _PlayList->registerTransform(
                fallbackInstance,
                (sheet.Objects[k].FallbackPass[l].ShapeName + ".").c_str());
          }
        }
      }
      ...
      CSkyObject so;
      so.init(sheet.Objects[k].Std, instance, buildShareBitmapByName, _Bitmaps,
              sheet.Objects[k].VisibleInMainScene,
              sheet.Objects[k].VisibleInEnvMap);
      _Objects.push_back(so);
      for (uint l = 0; l < 2; ++l) {
      ...
      if (_PlayList) {
        _PlayList->registerTransform(
            instance, (sheet.Objects[k].Std.ShapeName + ".").c_str());
      }
    } else {
      nlwarning("Object not found in scene : %s",
                sheet.Objects[k].Std.ShapeName.c_str());
    }
  }
  // get gradient that gives sun light color
  bool alreadyBuilt;
  _AmbientSunLight =
      buildSharedBitmap(sheet.AmbientSunLightBitmap, buildShareBitmapByName,
                        _Bitmaps, alreadyBuilt);
  _DiffuseSunLight =
      buildSharedBitmap(sheet.DiffuseSunLightBitmap, buildShareBitmapByName,
                        _Bitmaps, alreadyBuilt);
  //
  _FogColor = buildSharedBitmap(sheet.FogColorBitmap, buildShareBitmapByName,
                                _Bitmaps, alreadyBuilt);
  //
...
// *************************************************************************************************
uint CSky::setup(const CClientDate &date, const CClientDate &animationDate,
  ...
  for (uint k = 0; k < numObjects; ++k) {
    if (_Objects[k].setup(date, animationDate, _NumHourInDay, weatherLevel,
                          fogColor, envMapScene))
      ++numVisibleObjects;
  }
  // animate objects
  if (_PlayListManager) {
  ...
  if (_AmbientSunLight) {
    _Scene->setSunAmbient(
        _AmbientSunLight->getColor(dayPart, weatherLevel, true, false));
  } else {
    _Scene->setSunAmbient(CRGBA::White);
  }
  if (_DiffuseSunLight) {
    _Scene->setSunDiffuse(
        _DiffuseSunLight->getColor(dayPart, weatherLevel, true, false));
  } else {
...
// *************************************************************************************************
NLMISC::CRGBA CSky::computeFogColor(const CClientDate &date,
...
CBitmap *buildSharedBitmap(const std::string &filename,
  ...
  if (filename.empty())
    return NULL;
  std::string lcBMFilename =
      toLowerAscii(CFile::getFilenameWithoutExtension(filename));
  std::map<std::string, CBitmap *>::iterator it =
      bitmapByName.find(lcBMFilename);
