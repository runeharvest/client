...
void CContinentManager::reset() {
  ...
  // clear continents DB
  for (TContinents::iterator it = _Continents.begin(); it != _Continents.end();
       ++it) {
    delete it->second;
  }
  _Continents.clear();
...
SContInit vAllContinents[NBCONTINENT] = {
        { "fyros",		"fyros",				15767,
20385,	-27098,	-23769	}, { "tryker",		"tryker",
15285,	18638,	-34485,	-30641	}, { "matis",		"lesfalaises",
235,	6316,	-7920,	-256	}, { "zorai",		"lepaysmalade",
6805,	12225,	-5680,	-1235	}, { "bagne",		"lebagne",
434,	1632,	-11230,	-9715	}, { "route",		"laroutedesombres",
5415,	7400,	-17000, -9575	}, { "sources",	"sources",
2520,	3875,	-11400, -9720	}, { "terres",		"lesterres",
100,	3075,	-15900, -13000	}
};
*/
  ...
  for (i = 0; i < ws->ContLocs.size(); ++i) {
    ...
    if (sheet) {
      if (sheet->type() == CEntitySheet::CONTINENT) {
        ...
        nlwarning("Bad type for continent form %s.",
                  continentSheetName.c_str());
      }
    } else {
...
//-----------------------------------------------
void CContinentManager::select(const string &name, const CVectorD &pos,
  ...
  if (itCont == _Continents.end()) {
    nlwarning("CContinentManager::select: Continent '%s' is Unknown. Cannot "
              "Select it.",
              name.c_str());
    return;
  }
  ...
  // Modify this season according to the continent reached. eg: newbieland
  // continent force the winter to be autumn
  if (StartupSeason < EGSPD::CSeason::Invalid)
    StartupSeason = (*itCont).second->ForceDisplayedSeason[StartupSeason];

  // Compute the current season according to StartupSeason, Server driver
  // season, R2 Editor season or manual debug season
  CurrSeason = computeCurrSeason();

  // Is it the same continent than the old one.
  {
    H_AUTO(InitRZWorldSelectCont)
    if (((*itCont).second != _Current) ||
        ((*itCont).second->Season != CurrSeason)) {
      ...
      if (!(*itCont).second->Indoor && _Current) {
        ...
        // Remove the primitive for all entitites (new PACS coming soon and need
        // new primitives).
...
//-----------------------------------------------
void CContinentManager::select(const CVectorD &pos,
                               NLMISC::IProgressCallback &progress) {
  ...
  while (it != _Continents.end()) {
    ...
    if (!pCont->Zone.VPoints
             .empty()) // Patch because some continent have not been done yet
    {
      if (pCont->Zone.contains(fPos)) {
        ...
        /*
        nlwarning("**********************************************");
        nlwarning("Start position (%s) not found in continent %s",
        NLMISC::toString(pos.asVector()).c_str(), it->first.c_str()); for(uint k
        = 0; k < pCont->Zone.VPoints.size(); ++k)
        {
                nlwarning("zone point %d = %s", (int)k,
        NLMISC::toString(pCont->Zone.VPoints[k]).c_str());
        }
        */
      }
    }
  ...
  /* *****************
          ...
          While this is a bug, it is a data bug. Crashing is not a good solution
     in this case. If you put an assert, it can happens this scenario for
     example:
          - A levelDesigner put a bad Teleporter which teleport to an invalid
     position
          - The player teleport, but its position is invalid => crash
          - the next time he logs, it crashes at start, AND HENCE CANNOT ASK A
     GM TO TELEPORT HIM AWAY

          Other scenarios can happens like Data change, Continent change =>
     Player no more at valid position etc... HENCE MUST NOT CRASH, but display a
     "Lost In Space screen" leaving the player the possibility to ask HELP.
     *****************
  */
...
bool CContinentManager::isLoadingforced(
    const NLMISC::CVector &playerPos) const {
  if (_Current == 0)
    return false;
...
void CContinentManager::forceUpdateStreamable(
    const NLMISC::CVector &playerPos, NLMISC::IProgressCallback &progress) {
  H_AUTO_USE(RZ_Client_Continent_Mngr_Update_Streamable)
...
void CContinentManager::getFogState(TFogType fogType, float dayNight,
                                    float duskRatio,
                                    CLightCycleManager::TLightState lightState,
                                    const NLMISC::CVectorD &pos,
                                    CFogState &result) {
  if (_Current)
    _Current->getFogState(fogType, dayNight, duskRatio, lightState, pos,
                          result);
}
...
void CContinentManager::writeTo(xmlNodePtr node) const {
  ...
  //    ...
  // </landmarks>
  for (TContinents::const_iterator it = _Continents.begin();
       it != _Continents.end(); ++it) {
    std::string name = it->first;
    xmlNodePtr contNode =
        xmlNewChild(node, NULL, (const xmlChar *)"landmarks", NULL);
    xmlSetProp(contNode, (const xmlChar *)"continent",
    ...
    if (it->second && it->second->UserLandMarks.size() > 0) {
      for (uint i = 0; i < it->second->UserLandMarks.size(); ++i) {
        ...
        xmlNodePtr lmNode =
            xmlNewChild(contNode, NULL, (const xmlChar *)"landmark", NULL);
        xmlSetProp(lmNode, (const xmlChar *)"type",
                   (const xmlChar *)toString("%d", (uint32)lm.Type).c_str());
        xmlSetProp(lmNode, (const xmlChar *)"x",
                   (const xmlChar *)toString("%.2f", lm.Pos.x).c_str());
        xmlSetProp(lmNode, (const xmlChar *)"y",
                   (const xmlChar *)toString("%.2f", lm.Pos.y).c_str());
        ...
        xmlSetProp(lmNode, (const xmlChar *)"title",
...
void CContinentManager::readFrom(xmlNodePtr node) {
  ...
  if (itContinent == _Continents.end() || !itContinent->second) {
    nlwarning("Ignore landmarks group with unknown 'continent' '%s'",
              continent.c_str());
    return;
  }
  ...
  if (lmtype != "user") {
    nlwarning("Ignore landmarks group with type '%s', expected 'user'.",
              lmtype.c_str());
    return;
  }
  ...
  while (node) {
    if (stricmp((char *)node->name, "landmark") != 0) {
      nlwarning("Ignore invalid node '%s' under landmarks group",
    ...
    if (add) {
      ...
      if (add) {
        ...
        nlwarning("Ignore landmark with duplicate pos (continent:'%s', x:%.2f, "
                  "y:%.2f, type:%d, title:'%s')",
                  continent.c_str(), lm.Pos.x, lm.Pos.y, (uint8)lm.Type,
                  lm.Title.toUtf8().c_str());
      }
    } else {
...
uint32 CContinentManager::serialUserLandMarks(NLMISC::IStream &f) {
  ...
  if (!f.isReading()) {
    ...
    f.serial(numCont);
    for (TContinents::iterator it = _Continents.begin();
         it != _Continents.end(); ++it) {
      std::string name = it->first;
      f.serial(name);
      if (it->second) {
...
void CContinentManager::serialFOWMaps() {
  for (TContinents::iterator it = _Continents.begin(); it != _Continents.end();
       ++it) {
    CContinent *pCont = it->second;
    // nlinfo("Saving fow continent %s of name %s", it->first.c_str(),
    // pCont->Name.c_str());
...
void CContinentManager::reloadWeather() {
  ...
  for (TContinents::iterator it = _Continents.begin(); it != _Continents.end();
    ...
    CContinentSheet *cs =
        dynamic_cast<CContinentSheet *>(SheetMngr.get(contSI));
    if (cs) {
      // update continent weather part
      for (uint l = 0; l < EGSPD::CSeason::Invalid; ++l) {
        it->second->WeatherFunction[l].buildFromSheet(cs->WeatherFunction[l],
                                                      WeatherManager);
      }
      // update misc params
  ...
  for (CSheetManager::TEntitySheetMap::const_iterator it = sm.begin();
    ...
    if (it->second.EntitySheet) {
      ...
      if (type == CEntitySheet::CONTINENT) {
        ...
        if (other) {
          const CContinentParameters &cp =
              static_cast<const CContinentSheet *>(other)->Continent;
          // find matching continent in manager
          for (TContinents::iterator it = _Continents.begin();
               it != _Continents.end(); ++it) {
            if (it->second && nlstricmp(it->second->Name, cp.Name) == 0) {
              std::copy(cp.SkySheet, cp.SkySheet + EGSPD::CSeason::Invalid,
                        it->second->SkySheet);
              break;
            }
          }
...
void CContinentManager::loadContinentLandMarks() {
  ...
  if (ClientCfg.UpdatePackedSheet == false) {
    ...
    buildLMConts("ryzom.world", "../../common/data_leveldesign/primitives",
                 dataPath);
    readLMConts(dataPath);
  }
}
