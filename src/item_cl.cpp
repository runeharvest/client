...
//
//-----------------------------------------------
CItemCL::CItemCL() : CEntityCL() {} // CItemCL //
...
bool CItemCL::build(const CEntitySheet *sheet) // virtual
{
  ...
  if (sh == 0) {
    nlwarning("Item:build: the sheet is not an item sheet -> entity not "
              "initialized.");
    return false;
  }
  // Get the DB Entry
...
void CItemCL::initShape(const string &fileName) {
  ...
  if (!filePath.empty())
    ...
    nlwarning("Item:initShape:%d: file '%s' not found.", _Slot,
              fileName.c_str());
} // initShape //
...
//-----------------------------------------------
void CItemCL::updateVisualPropertyPos(
