...
public:
  ...
  void startTask(const BGDownloader::CTaskDesc &taskDesc,
                 const std::string &commandLine, bool showDownloader);
  bool isTaskEnded(BGDownloader::TTaskResult &result,
                   ucstring &errorMsg) const; // OLD
  // The following flag will be true after a 'patch' task has been completed
  // successfully
  ...
  void reboot();
  // Valid after a task has ended. If this was a check task, then contain a
  // bitfield indexed by BGDownloader::TDownloadID that indicate the part that
  // need to be patched
  ...
  // download thread
  BGDownloader::TThreadPriority getDownloadThreadPriority() const {
    return _DownloadThreadPriority;
  }
  bool isDownloaderUIFrozen() const { return _FrozenUI; }
  void requestDownloadThreadPriority(BGDownloader::TThreadPriority newPriority,
                                     bool freezeUI);
...
private:
  ...
  public:
    ...
    void waitMsg(BGDownloader::TMsgType wantedMsgType, NLMISC::CMemStream &msg,
                 NLMISC::TTime timeOutInMs = 6000000);
    void sendMsg(NLMISC::CMemStream &msg);
    void sendSimpleMsg(BGDownloader::TMsgType msgType);
    bool defaultMessageHandling(BGDownloader::TMsgType msgType,
    ...
    void
    doTask(const BGDownloader::CTaskDesc
               &taskDesc); // return value may indicate 'idle' or 'want reboot'
    void stopTask();
    void startTask(const BGDownloader::CTaskDesc &taskDesc);
    void waitIdle();
    void checkDownloaderAlive();
    void yieldDownload(); // same as yield, but also handle the
                          // isTerminationRequested() case
    void shutdownDownloader();
    void getTaskResult(BGDownloader::TTaskResult &result,
                       uint32 &availablePatchs, bool &mustLaunchBatFile,
