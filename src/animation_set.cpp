...
// init
//-----------------------------------------------
void CAnimationSet::init(CAnimationSetSheet *sheet,
  ...
  if (animState) {
    // Compute the distance to break the idle (done according to the walk
    // state).
    CAnimation::TAnimId walkId =
        animState->chooseAnim(0, EGSPD::CPeople::Unknown, GSGENDER::male);
    ...
    if (walkId != CAnimation::UnknownAnim) {
      ...
      _WalkLength =
          CAnimationMisc::getAnimationLength(animationSet, walkAnim->id());

      animState = getAnimationState(CAnimationStateSheet::Run);
      if (animState) {
        // Get the run animation ID.
        CAnimation::TAnimId runId =
            animState->chooseAnim(0, EGSPD::CPeople::Unknown, GSGENDER::male);
        if (runId != CAnimation::UnknownAnim) {
          ...
          CVector mov;
          if (CAnimationMisc::getAnimationMove(animationSet, runAnim->id(),
                                               mov))
            _RunDist = fabs(mov.y);
          _RunLength =
              CAnimationMisc::getAnimationLength(animationSet, runAnim->id());

          // Get the walk average speed.
          double aveWalk = CAnimationMisc::getAnimationAverageSpeed(
              animationSet, walkAnim->id());
          // Get the run average speed.
          double aveRun = CAnimationMisc::getAnimationAverageSpeed(
              animationSet, runAnim->id());
          ...
          if (aveRun < aveWalk)
            pushDebugStr(NLMISC::toString("Walk speed(%f) > Run speed(%f) !",
                                          aveWalk, aveRun));
        ...
        else if (_Sheet->IsRunEssential)
          pushDebugStr("No animation found to run: speedToRun and speedToWalk "
                       "will return the default value.");
      }
    }
    // No animation found to walk.
    else if (_Sheet->IsWalkEssential)
      pushDebugStr("No animation found to walk: maxDist, speedToRun and "
                   "speedToWalk will return the default value.");
  }
  ...
  if (animState) {
    ...
    if (turnLeftId != CAnimation::UnknownAnim) {
      ...
      if (anim) {
        ...
        if (CAnimationMisc::interpolate(animationSet, anim->id(), 0.0,
                                        currentAnimRotStart)) {
          double animLength =
              CAnimationMisc::getAnimationLength(animationSet, turnLeftId);
          if (CAnimationMisc::interpolate(animationSet, anim->id(), animLength,
                                          currentAnimRotEnd)) {
...
CAnimationState *CAnimationSet::getAnimationStateByIndex(uint index) {
  if (index >= _AnimationStates.size())
    return NULL;
