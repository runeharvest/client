...
struct CLightCycleDesc {
  ...
  float DuskRatio; // ratio for dusk transition. e.g. when set to 0.5f, that
                   // means that the dusk colors are taken when the day->night
                   // transition is at its half
...
public:
   ...
   * NB : the weather manager is needed if the lighting needs to be overriden in
   * case of bad weather
   */
  void setHour(float hour, const CWeatherManagerClient &wm,
               NLMISC::CRGBA lightningColor);
  //
  float getHour() const { return _Hour; }
  // Get the last computed light level in a call to setHour (from 0 fro day to 1
  // for night)
   ...
   */
  void touch();
  /// Get needed landscape update freq (or 0 if the setupped lightDesc was not
  /// valid)
  ...
  TLightState getState() const { return _State; }
...
private:
  ...
  static bool isInDayInterval(float startHour, float endHour, float dayDuration,
                              float hour, float &ratio);
  void setDirLight(const CDirLightSetup &setup0, const CDirLightSetup &setup1,
                   float level, float intensity, NL3D::UScene &scene);
  ...
  void setupDayToNightLight(NL3D::UScene &scene, const CDirLightSetup &dayLight,
