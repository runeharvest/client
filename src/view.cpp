...
void CView::update() {
  if (UserEntity->viewMode() == CUserEntity::FirstPV) {
    ...
    if (ClientCfg.CameraDistance > _CurrentCameraDist) {
      ...
      // Speed Limit.
      clamp(_CurrentCameraSpeed, ClientCfg.CameraSpeedMin,
            ClientCfg.CameraSpeedMax);
      ...
      _CurrentCameraSpeed -= ClientCfg.CameraAccel * DT;
      // Speed Limit.
      clamp(_CurrentCameraSpeed, -ClientCfg.CameraSpeedMax,
            -ClientCfg.CameraSpeedMin);
      // Adjust Camera Distance
      _CurrentCameraDist += _CurrentCameraSpeed * DT;
      if (_CurrentCameraDist < ClientCfg.CameraDistance)
        _CurrentCameraDist = ClientCfg.CameraDistance;
...
// update the max distance of the camera for player that want to play as dm
// (camera can be far)
void CView::setCameraDistanceMaxForDm() {
  _CurrentCameraDistanceMax = ClientCfg.DmCameraDistMax;
  if (!ClientCfg.FPV) {
    cameraDistance(
        std::min(ClientCfg.CameraDistance, _CurrentCameraDistanceMax));
  }
}

// update the max distance of the camera for player that want to play as player
// (camera must be near)
...
//-----------------------------------------------
void CView::getCamera3rdPersonSetup(CVector &cameraStart, CVector &cameraEnd,
                                    CVector &cameraTestStart) const {
  float testStartDecal;
  getCamera3rdPersonSetupInternal(cameraStart, cameraEnd, cameraTestStart,
                                  testStartDecal);
}
...
//-----------------------------------------------
void CView::getCamera3rdPersonSetupInternal(CVector &cameraStart,
                                            CVector &cameraEnd,
                                            CVector &cameraTestStart,
                                            float &testStartDecal) const {
  // get the camera path
  if (_RearView) {
    CVector v;
    v.x = -UserEntity->front().x;
    ...
    cameraEnd =
        cameraStart - v * (_CurrentCameraDist + _CameraCollisionThreshold);
  } else {
    cameraStart = _ViewPos + CVector(0.0f, 0.0f, _CurrentCameraHeight);
    // add the threshold, to avoid discontinuity at f=1, when col start
    cameraEnd =
        cameraStart - _View * (_CurrentCameraDist + _CameraCollisionThreshold);
  }
...
void CView::updateCameraCollision() {
  ...
  if (UserEntity->viewMode() != CUserEntity::FirstPV &&
      UserControls.mode() != CUserControls::AIMode && CollisionManager) {
    ...
    // For "matis serre bug", suppose the player is "inside" if walk on a
    // cluster system
    ...
    // Use an approximate "pelvis", instead of getHeadPos(), because sometimes,
    // even the head of the player can enter in landscape. Thus this case will
    // be tested, but the player will still see inside the landscape
    CVector pelvisPos;
    ...
    if (CollisionManager->getRayCollision(pelvisPos, cameraTestStart, true)) {
      ...
      float f = CollisionManager->getCameraCollision(cameraTestStart, cameraEnd,
                                                     colRadius, true);
      ...
      if (f < 1) {
        ...
        _CollisionCameraDist =
            testStartDecal + (cameraEnd - cameraTestStart).norm() * f;
      ...
      _ThirPersonClusterSystem = Scene->findCameraClusterSystemFromRay(
          pPlayerClusterSystem, cameraStart, cameraEnd);
