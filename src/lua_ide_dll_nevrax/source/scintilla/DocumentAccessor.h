...
public:
  DocumentAccessor(Document *pdoc_, PropSet &props_, WindowID id_ = 0)
      : Accessor(), pdoc(pdoc_), props(props_), id(id_), lenDoc(-1),
        validLen(0), chFlags(0), chWhile(0), startSeg(0), startPosStyling(0) {}
  ~DocumentAccessor();
  ...
  char *GetProperties() { return props.ToString(); }
  ...
  int IndentAmount(int line, int *flags,
                   PFNIsCommentLeader pfnIsCommentLeader = 0);
};
