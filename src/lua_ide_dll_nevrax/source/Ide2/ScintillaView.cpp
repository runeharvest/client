...
#ifdef _DEBUG
void CScintillaView::AssertValid() const { CView::AssertValid(); }

void CScintillaView::Dump(CDumpContext &dc) const { CView::Dump(dc); }
#endif //_DEBUG
...
BOOL CScintillaView::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT *pResult) {
  ...
  if (lpnmhdr->hwndFrom == m_view.m_hWnd) {
    *pResult =
        ((CScintillaBar *)GetParent())->OnSci(this, (SCNotification *)lParam);
    return TRUE;
  }
