...
CLuaEditor::CLuaEditor() { m_bShowCalltips = TRUE; }
...
BOOL CLuaEditor::Create(CWnd *pParentWnd, UINT nCtrlId) {
  BOOL bCreated =
      CreateEx(0, "Scintilla", "", WS_CHILD | WS_VISIBLE | WS_TABSTOP,
               CRect(0, 0, 0, 0), pParentWnd, nCtrlId);

  if (!bCreated)
    return FALSE;

  m_fnScintilla = (int(__cdecl *)(void *, int, int, int))SendMessage(
...
int CLuaEditor::LineFromPoint(CPoint &pt) {
  return 1 +
         Sci(SCI_LINEFROMPOSITION, Sci(SCI_POSITIONFROMPOINT, pt.x, pt.y), 0);
}
...
void CLuaEditor::ClearAllBreakpoints() { Sci(SCI_MARKERDELETEALL, 0); }

void CLuaEditor::SetBreakpoint(int nLine) { Sci(SCI_MARKERADD, nLine - 1, 0); }

void CLuaEditor::SetCursorLine(int nLine) { Sci(SCI_GOTOLINE, nLine - 1); }

void CLuaEditor::SetCursorPos(int pos) { Sci(SCI_SETCURRENTPOS, pos); }
...
BOOL CLuaEditor::CanUndo() { return Sci(SCI_CANUNDO); }

void CLuaEditor::Undo() { Sci(SCI_UNDO); }

BOOL CLuaEditor::CanRedo() { return Sci(SCI_CANREDO); }

void CLuaEditor::Redo() { Sci(SCI_REDO); }

void CLuaEditor::SelectAll() { Sci(SCI_SELECTALL); }
...
void CLuaEditor::Cut() { Sci(SCI_CUT); }

void CLuaEditor::Clear() { Sci(SCI_CLEAR); }

BOOL CLuaEditor::CanPaste() { return Sci(SCI_CANPASTE); }

void CLuaEditor::Paste() { Sci(SCI_PASTE); }

void CLuaEditor::Copy() { Sci(SCI_COPY); }

void CLuaEditor::GrabFocus() { Sci(SCI_GRABFOCUS); }
...
void CLuaEditor::ClearAll() { Sci(SCI_CLEARALL); }
...
void CLuaEditor::FindText(const char *txt, BOOL matchCase, BOOL wholeWord,
                          BOOL regExp, BOOL forward, int startPos, int &first,
                          int &last) {
  TextToFind ttf;
  ...
  int flags = (wholeWord ? SCFIND_MATCHCASE : 0) |
              (matchCase ? SCFIND_WHOLEWORD : 0) | (regExp ? SCFIND_REGEXP : 0);
...
int CLuaEditor::GetCursorPos() { return Sci(SCI_GETCURRENTPOS, 0, 0); }
int CLuaEditor::GetTextLength() { return Sci(SCI_GETTEXTLENGTH, 0, 0); }
void CLuaEditor::GotoLastLine() {
  int nLine = Sci(SCI_GETLINECOUNT);
  Sci(SCI_GOTOLINE, nLine);
}

void CLuaEditor::WordLeft() { Sci(SCI_WORDLEFT); }

void CLuaEditor::WordRight() { Sci(SCI_WORDRIGHT); }

void CLuaEditor::WordLeftExtend() { Sci(SCI_WORDLEFTEXTEND); }

void CLuaEditor::WordRightExtend() { Sci(SCI_WORDRIGHTEXTEND); }

void CLuaEditor::WordPartLeft() { Sci(SCI_WORDPARTLEFT); }

void CLuaEditor::WordPartRight() { Sci(SCI_WORDPARTRIGHT); }

void CLuaEditor::WordPartLeftExtend() { Sci(SCI_WORDPARTLEFTEXTEND); }

void CLuaEditor::WordPartRightExtend() { Sci(SCI_WORDPARTRIGHTEXTEND); }

int CLuaEditor::GetCurrentLine() {
  return Sci(SCI_LINEFROMPOSITION, Sci(SCI_GETCURRENTPOS)) + 1;
}

int CLuaEditor::GetLineCount() { return Sci(SCI_GETLINECOUNT); }
...
BOOL CLuaEditor::PreparePrint(CDC *pDC, CPrintInfo *pInfo) {
  ...
  while (lengthPrinted < lengthDoc) {
    ...
    lengthPrinted =
        Sci(SCI_FORMATRANGE, FALSE, reinterpret_cast<LPARAM>(&frPrint));
  }
...
void CLuaEditor::SetLuaLexer() {
  ...
  const char keywords[] =
      "and break do else elseif end false for function global if in local nil "
  ...
  // now set up the styles (remember you have to set up font name for each
  // style; if you fail to do so, bold/italics will not work (only color will
  // work)
  ...
  // style 12: end of line where string is not closed (black on violet,
  // eol-filled)
