...
BOOL CLuaHelper::ErrorStringToFileLine(CString strError, CString &strPathName,
                                       int &nLine) {
...
BOOL CLuaHelper::PrepareDebugger(lua_realloc_t reallocfunc,
                                 lua_free_t freefunc) {
  ASSERT(L == NULL);
...
void CLuaHelper::func_hook(lua_State *L, lua_Debug *ar) {
  ...
  CDebugger::GetDebugger()->FunctionHook(szSource, ar->currentline,
...
const char *CLuaHelper::GetSource() { return m_pAr->source + 1; };
...
void CLuaHelper::DrawStackTrace() {
  ...
  while (lua_getstack(L, nLevel, &ar)) {
    ...
    if (ar.source[0] == '@') {
      ...
      CDebugger::GetDebugger()->AddStackTrace(szDesc, ar.source + 1,
                                              ar.currentline);
    }
...
struct CHelper {
  static const char *luaChunkReaderFromString(lua_State *L, void *ud,
                                              size_t *sz) {
