...
void CBreakPointWnd::UpdateContent() {
  ...
  for (int k = 0; k < (int)m_BP.size(); ++k) {
    ...
    CString bpDesc;
    bpDesc.Format("%s, line = %d", (LPCTSTR)m_BP[k].m_File->GetNameExt(),
                  m_BP[k].m_BP.m_Line);
...
void CBreakPointWnd::OnSelchangeBpList() { UpdateButtons(); }
  ...
  if (bpc.DoModal() == IDOK) {
    ...
    if (!bpc.m_Condition.IsEmpty()) {
      if (!theApp.GetMainFrame()->GetDebugger()->GetLuaHelper().CheckSyntax(
              "return (" + bpc.m_Condition + ")", errors)) {
        CString caption;
        caption.LoadString(AFX_IDS_APP_TITLE);
        MessageBox((LPCTSTR)errors, (LPCTSTR)caption,
...
void CBreakPointWnd::OnRemove() { EraseSel(); }
