...
BOOL CExecutor::Execute(CString strCmdLine, CMemFile *pInput) {
  ...
  DWORD ThreadId;
  m_hThread =
      CreateThread(NULL, 0, ReadAndHandleOutput, (LPVOID)this, 0, &ThreadId);
  if (m_hThread == NULL)
...
void CExecutor::Close() {
  ...
  if (!CloseHandle(m_hInputWrite))
    theApp.FormatMessage("CExecutor::Close::CloseHandle");
}

void CExecutor::PrepAndLaunchRedirectedChild(HANDLE hChildStdOut,
                                             HANDLE hChildStdIn,
                                             HANDLE hChildStdErr,
                                             CString strCmdLine) {
  ...
  // confusion.
  if (!CreateProcess(NULL, strCmdLine.GetBuffer(0), NULL, NULL, TRUE, 0,
                     _environ, NULL, &si, &pi))
    theApp.FormatMessage(
        "CExecutor::PrepAndLaunchRedirectedChild::CreateProcess");

  // Close any unnecessary handles.
  if (!CloseHandle(pi.hThread))
    theApp.FormatMessage(
        "CExecutor::PrepAndLaunchRedirectedChild::CloseHandle");
}
...
DWORD WINAPI CExecutor::ReadAndHandleOutput(LPVOID lpvThreadParam) {
  ...
  while (TRUE) {
    if (!ReadFile(pExe->m_hOutputRead, lpBuffer, sizeof(lpBuffer), &nBytesRead,
                  NULL) ||
        !nBytesRead) {
      if (GetLastError() == ERROR_BROKEN_PIPE)
        ...
        theApp.FormatMessage(
            "CExecutor::ReadAndHandleOutput::ReadFile"); // Something bad
                                                         // happened.
    } else
      pExe->m_output.Write(lpBuffer, nBytesRead);
  }

  if (!CloseHandle(pExe->m_hOutputRead))
    theApp.FormatMessage("CExecutor::ReadAndHandleOutput::CloseHandle");
...
CString CExecutor::GetOutputString() {
  ...
  m_output.Read(strOutput.GetBuffer(m_output.GetLength()),
                m_output.GetLength());
