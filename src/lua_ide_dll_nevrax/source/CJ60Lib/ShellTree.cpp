 ...
 *enumerating items for.  In other words, this is the PIDL to the item
 ...
 *
 ****************************************************************************/
void CShellTree::FillTreeView(LPSHELLFOLDER lpsf, LPITEMIDLIST lpifq,
                              HTREEITEM hParent) {
...
/****************************************************************************
 *
 *    FUNCTION: GetNormalAndSelectedIcons(LPITEMIDLIST lpifq, LPTV_ITEM
 *lptvitem)
 *
 *    PURPOSE:  Gets the index for the normal and selected icons for the current
 *item.
 *
 *    PARAMETERS:
...
void CShellTree::GetNormalAndSelectedIcons(LPITEMIDLIST lpifq,
  ...
  lptvitem->iImage =
      GetItemIcon(lpifq, SHGFI_PIDL | SHGFI_SYSICONINDEX | SHGFI_SMALLICON);
 ...
 *
 ****************************************************************************/
int CALLBACK CShellTree::TreeViewCompareProc(LPARAM lparam1, LPARAM lparam2,
                                             LPARAM lparamSort) {
 ...
 *
 *	PURPOSE:	Reponds to an TVN_ITEMEXPANDING message in order to fill
 *up subdirectories. Pass the parameters from OnItemExpanding() to this
 *function. You need to do that or your folders won't expand.
 *
 *	OTHER:		It can also be used to update a corresponding listview.
 *Seem MFCENUM
 *
...
void CShellTree::OnFolderExpanding(NMHDR *pNMHDR, LRESULT *pResult) {
  ...
  if (lptvid) {
    hr = lptvid->lpsfParent->BindToObject(lptvid->lpi, 0, IID_IShellFolder,
                                          (LPVOID *)&lpsf2);
...
/****************************************************************************
 *
 *	FUNCTION:	OnFolderSelected(NMHDR* pNMHDR, LRESULT* pResult,
 *CString &szFolderPath)
 *
 *	PURPOSE:	Call this function if for example you want to put the
 *path of the folder selected inside a combobox or an edit window. You would
 *pass the parameters from OnSelChanged() to this function along with a CString
 *object that will hold the folder path. If the path is not in the filesystem(eg
 *MyComputer) it returns false.
 *
 *	MESSAGEMAP:	TVN_SELCHANGED
 *
 ****************************************************************************/
BOOL CShellTree::OnFolderSelected(NMHDR *pNMHDR, LRESULT *pResult,
                                  CString &szFolderPath) {
  // TODO: Add your control notification handler code here
  LPTVITEMDATA lptvid; // Long pointer to TreeView item data
  LPSHELLFOLDER lpsf2 = NULL;
  ...
  if ((hItem = GetSelectedItem())) {
    ...
    if (lptvid && lptvid->lpsfParent && lptvid->lpi) {
      ...
      if (SUCCEEDED(hr)) {
        ...
        // Determine what type of object we have.
        lptvid->lpsfParent->GetAttributesOf(
            1, (const struct _ITEMIDLIST **)&lptvid->lpi, &ulAttrs);
 ...
 *
 *	MISC:		failure to call this function will result in a memory
 *leak
 *
 ****************************************************************************/
 ...
 *
 *	PURPOSE:	Obtains a handle to the system image list and attaches
 *it to the tree control. DO NOT DELETE the imagelist
 *
 *	MESSAGEMAP:	NONE
 *
 ****************************************************************************/
void CShellTree::EnableImages() {
  ...
  hImageList =
      (HIMAGELIST)SHGetFileInfo((LPCSTR) "C:\\", 0, &sfi, sizeof(SHFILEINFO),
 ...
 *				Pass a CString object that will hold the folder
 *path. If the path is not in the filesystem(eg MyComputer) or none is selected
 *it returns false.
 *
 *	MESSAGEMAP:	NONE
 *
 ...
 *				identifies the subfolder relative to its parent
 *folder. see GetParentShellFolder();
 *
 *	MESSAGEMAP:	NONE
 *
 ****************************************************************************/
 ...
 *				structure that identifies the subfolder relative
 *to the desktop. This is a fully qualified Item Identifier
 *
 *	MESSAGEMAP:	NONE
 *
 ****************************************************************************/
 ...
 *				version of windows. Use with caution. Pretty
 *much ok if you're using on local drives
 *
 ****************************************************************************/
bool CShellTree::SearchTree(HTREEITEM treeNode, CString szSearchName,
                            FindAttribs attr) {
  LPTVITEMDATA lptvid; // Long pointer to TreeView item data
  LPSHELLFOLDER lpsf2 = NULL;
