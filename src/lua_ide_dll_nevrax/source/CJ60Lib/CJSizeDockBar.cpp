...
// HISTORY:
// ==========================================================================
//			1.00	12 Jan 1999	- Initial release to add
...
void CCJSizeDockBar::DoPaint(CDC *pDC) { CDockBar::DoPaint(pDC); }

void CCJSizeDockBar::OnNcPaint() { EraseNonClient(); }
...
void CCJSizeDockBar::DrawBorders(CDC *pDC, CRect &rect) {
  ...
  if (m_dwStyle & CBRS_BORDER_TOP) {
    pDC->FillSolidRect(rc.left, rc.top, rc.Width(), 1, clrBtnShadow);
    pDC->FillSolidRect(rc.left, rc.top + 1, rc.Width(), 1, clrBtnHilite);
    cyBorderTop += 2;
  }

  if (m_dwStyle & CBRS_BORDER_BOTTOM) {
    pDC->FillSolidRect(rc.left, rc.bottom - 1, rc.Width(), 1, clrBtnHilite);
    pDC->FillSolidRect(rc.left, rc.bottom - 2, rc.Width(), 1, clrBtnShadow);
    cyBorderBottom += 2;
  }

  if (m_dwStyle & CBRS_BORDER_LEFT) {
    pDC->FillSolidRect(rc.left, rc.top, 1, rc.Height(), clrBtnShadow);
    pDC->FillSolidRect(rc.left + 1, rc.top, 1, rc.Height(), clrBtnHilite);
    cxBorderLeft += 2;
  }

  if (m_dwStyle & CBRS_BORDER_RIGHT) {
    pDC->FillSolidRect(rc.right - 2, rc.top, 1, rc.Height(), clrBtnShadow);
    pDC->FillSolidRect(rc.right - 1, rc.top, 1, rc.Height(), clrBtnHilite);
    cxBorderRight += 2;
  }
  ...
  if (nID == AFX_IDW_SIZEBAR_LEFT) {
    ...
    rcTrack.right += 1;
    pDC->FillSolidRect(rcTrack, clrBtnFace);
    pDC->Draw3dRect(rcTrack, clrBtnHilite, clrBtnShadow);
...
void CCJSizeDockBar::OnNcCalcSize(BOOL bCalcValidRects,
...
BOOL CCJSizeDockBar::OnSetCursor(CWnd *pWnd, UINT nHitTest, UINT message) {
  return (nHitTest == HTCLIENT) ? CDockBar::OnSetCursor(pWnd, nHitTest, message)
                                : FALSE;
}
...
void CCJSizeDockBar::OnMouseMove(UINT nFlags, CPoint point) {
  if (m_bDragging) {
    ...
    if (nID == AFX_IDW_SIZEBAR_LEFT) {
      ...
      point.x = point.x > rectAvail.right + m_iActualSize - m_iTrackBorderSize
                    ? rectAvail.right - m_iTrackBorderSize + m_iActualSize
                    : point.x;
    }

    else if (nID == AFX_IDW_SIZEBAR_BOTTOM) {
      point.y = point.y < rectAvail.top ? rectAvail.top : point.y;
      point.y = point.y > rectAvail.bottom + m_iActualSize - m_iTrackBorderSize
                    ? rectAvail.bottom - m_iTrackBorderSize + m_iActualSize
                    : point.y;
    }
    ...
    if (nID == AFX_IDW_SIZEBAR_LEFT || nID == AFX_IDW_SIZEBAR_RIGHT && deltaX) {
      ...
      OnInvertTracker(m_rcTrack);
             ...
             nID == AFX_IDW_SIZEBAR_BOTTOM && deltaY) {
      OnInvertTracker(m_rcTrack);
...
void CCJSizeDockBar::GetAvailableRect(CRect &rect) {
  ...
  GetDockingFrame()->RepositionBars(0xffff, 0xffff, AFX_IDW_PANE_FIRST,
                                    reposQuery, &rect, &rect, TRUE);
}

void CCJSizeDockBar::SetActualSize(int iSize) { m_iActualSize = iSize; }
...
void CCJSizeDockBar::CalcSizeBarLayout() {
  ...
  for (int i = 0; i < m_arrBars.GetSize(); ++i) {
    ...
    if (pBar != NULL && pBar->IsVisible()) {
      ...
      pBar->SetWindowPos(NULL, rectBar.left, rectBar.top, rectBar.Width(),
...
void CCJSizeDockBar::Normalize(CCJControlBar *pBar) {
  for (int i = 0; i < m_arrBars.GetSize(); ++i) {
    CCJControlBar *pBarDock = (CCJControlBar *)GetDockedControlBar(i);
...
void CCJSizeDockBar::RecalcAllExcept(CCJSizeDockBar *pBar) {
  ...
  for (int i = 0; i < 4; i++) {
    CCJSizeDockBar *pDock =
        (CCJSizeDockBar *)pFrame->GetControlBar(dwSizeBarMap[i][0]);
...
CControlBar *CCJSizeDockBar::GetDockedSizeBar(int nPos) {
  return GetDockedControlBar(nPos);
}

void CCJSizeDockBar::OnLButtonDblClk(UINT nFlags, CPoint point) {
  // TODO: Add your message handler code here and/or call default

  CCJSizeDockBar::OnLButtonDblClk(nFlags, point);
}
