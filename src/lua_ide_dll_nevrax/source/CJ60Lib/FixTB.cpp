...
CFixMFCToolBar::CFixMFCToolBar() { m_bShowDropdownArrowWhenVertical = FALSE; }
  ...
  for (int i = 0; i < nCount; i++) {
    ...
    if (x + dx > nWidth) {
      ...
      for (int j = i; j >= 0 && !(pData[j].fsState & TBSTATE_WRAP); j--) {
        ...
        if ((pData[j].fsStyle & TBSTYLE_SEP) && (pData[j].idCommand == 0) &&
      ...
      if (!bFound) {
        for (int j = i - 1; j >= 0 && !(pData[j].fsState & TBSTATE_WRAP); j--) {
          ...
          if ((pData[j].fsState & TBSTATE_HIDDEN) ||
              ((pData[j].fsStyle & TBSTYLE_SEP) && (pData[j].idCommand != 0)))
            continue;
...
void CFixMFCToolBar::SizeToolBar(TBBUTTON *pData, int nCount, int nLength,
                                 BOOL bVert) {
  ASSERT(pData != NULL && nCount > 0);
...
CSize CFixMFCToolBar::CalcLayout(DWORD dwMode, int nLength) {
  ...
  // BLOCK: Adjust Margins
  {
    ...
    CSize size =
        CControlBar::CalcFixedLayout((dwMode & LM_STRETCH), (dwMode & LM_HORZ));
  ...
  if (memcmp(pButton, &button, sizeof(TBBUTTON)) != 0) {
    ...
    if (((pButton->fsStyle ^ button.fsStyle) & TBSTYLE_SEP) ||
        ((pButton->fsStyle & TBSTYLE_SEP) &&
         pButton->iBitmap != button.iBitmap)) {
