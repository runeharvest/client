...
//      'Class to select directory' which is where the 'BrowseForFolder()'
//      method came from.
...
//		'Class for Browsing shell namespace with your own dialog', which
//is 		where the inspiration for the rest of this class came from, as well as 		the
//CShellTree() and CShellPidl() classes came from.
...
///////////////////////////////////////////////////////////////////
CString CSHFileInfo::GetFileTitle() { return m_szFname; }
...
CString CSHFileInfo::GetLastWriteTime() {
  ...
  strTime.Format("%.2d/%.2d/%d %.2d:%.2d", time.GetMonth(), time.GetDay(),
...
///////////////////////////////////////////////////////////////////
void CSHFileInfo::GetSystemImageList(CImageList *pSmallList,
  ...
  HIMAGELIST hSystemSmallImageList = (HIMAGELIST)::SHGetFileInfo(
      (LPCTSTR) _T("C:\\"), 0, &ssfi, sizeof(SHFILEINFO),
      SHGFI_SYSICONINDEX | SHGFI_SMALLICON);
  ...
  HIMAGELIST hSystemLargeImageList = (HIMAGELIST)::SHGetFileInfo(
      (LPCTSTR) _T("C:\\"), 0, &lsfi, sizeof(SHFILEINFO),
...
//////////////////////////////////////////////////////////////////////
static int __stdcall BrowseCtrlCallback(HWND hwnd, UINT uMsg, LPARAM lParam,
                                        LPARAM lpData) {
  CSHFileInfo *pShellFileInfo = (CSHFileInfo *)lpData;

  if (uMsg == BFFM_INITIALIZED && !pShellFileInfo->m_strSelDir.IsEmpty()) {
    ::SendMessage(hwnd, BFFM_SETSELECTION, TRUE,
                  (LPARAM)(LPCTSTR)(pShellFileInfo->m_strSelDir));
  ...
  if (!m_strInitDir.IsEmpty()) {
    ...
    if (SUCCEEDED(SHGetDesktopFolder(&pDesktopFolder))) {
      ...
      MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED,
                          m_strInitDir.GetBuffer(MAX_PATH), -1, olePath,
                          MAX_PATH);
