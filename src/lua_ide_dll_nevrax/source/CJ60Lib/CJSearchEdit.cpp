...
BOOL CEditButton::Create(CWnd *pWnd, SEARCH_TYPE eType, UINT nMenu) {
  ...
  m_pWnd->SetWindowPos(NULL, 0, 0, rc.Width() - 23, rc.Height(),
  ...
  case SEARCH_FILE: {
    if (CButton::Create(_T("..."), dwStyle, rc, pWndParent,
                        GetNextID(pWndParent))) {
  ...
  case SEARCH_POPUP: {
    if (CButton::Create(_T("..."), dwStyle | BS_ICON, rc, pWndParent,
                        GetNextID(pWndParent))) {
      SetWindowPos(m_pWnd, 0, 0, 0, 0, SWP_NOSIZE | SWP_NOMOVE);
      EnableWindow(m_pWnd->IsWindowEnabled());
      SetFont(&m_Font);
