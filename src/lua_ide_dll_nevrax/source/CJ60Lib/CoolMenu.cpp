...
// HISTORY:
// ==========================================================================
//		1.01	13 Aug 1998	- Peter Tewkesbury - Added
//AddSingleBitmap(...) 							  method for adding a single bitmap to a pulldown 							  menu
//item. 		1.02	13 Aug 1998 - Omar L Francisco - Fixed bug with lpds->CtlType
//							   and lpds->itemData
//item checking. 		1.03	12 Nov 1998	- Fixes debug assert in system menu. -
//Wang Jun 		1.04	17 Nov 1998 - Fixes debug assert when you maximize a view - Wang
//Jun 							  window, then try to use the system menu for the view. 		1.05	09 Jan
//1998 - Fix for virtual key names - Seain Conover
// ==========================================================================
//
/////////////////////////////////////////////////////////////////////////////
...
void PLDrawEmbossed(CDC &dc, CImageList &il, int i, CPoint p,
                    BOOL bColor = FALSE);
...
CCoolMenuManager::~CCoolMenuManager() { Destroy(); }
  ...
  if (pmd->fType & MFT_SEPARATOR) {
    ...
    if (iButton >= 0) {
      ...
      if (!bDisabled) {
        ...
        PLFillRect(
            dc, rcButn,
            GetSysColor((bChecked && !bSelected) ? COLOR_3DLIGHT : COLOR_MENU));
      ...
      if (bChecked || info.hbmpUnchecked) {
        bHaveButn =
            Draw3DCheckmark(dc, rcButn, bSelected,
...
//		hbmCheck		Checkmark bitmap to use, or NULL for
//default
//
BOOL CCoolMenuManager::Draw3DCheckmark(CDC &dc, const CRect &rc, BOOL bSelected,
  ...
  dc.BitBlt(rcDest.left, rcDest.top, rcDest.Width(), rcDest.Height(), &memdc,
            p.x, p.y, SRCCOPY);
...
//
void CCoolMenuManager::OnInitMenuPopup(CMenu *pMenu, UINT nIndex,
                                       BOOL bSysMenu) {
  if (m_bAutoAccel) {
...
void CCoolMenuManager::OnMenuSelect(UINT nItemID, UINT nFlags, HMENU hSysMenu) {
  if (hSysMenu == NULL && nFlags == 0xFFFF) {
    ...
    while (!m_menuList.IsEmpty()) {
      ConvertMenu(CMenu::FromHandle((HMENU)m_menuList.RemoveHead()), 0, FALSE,
                  FALSE);
    }
  }
}
...
CString CCoolMenuManager::GetVirtualKeyName(const CString strVirtKey) const {
  ...
  if (strVirtKey == "Num 7") {
    strResult = "Home";
  } else if (strVirtKey == "Num 1") {
    strResult = "End";
  } else if (strVirtKey == "Num 9") {
    strResult = "PgUp";
  } else if (strVirtKey == "Num 3") {
    strResult = "PgDn";
  } else if (strVirtKey == "Num 0") {
    strResult = "Ins";
  } else if (strVirtKey == "Num Del") {
    strResult = "Del";
  } else {
...
BOOL CCoolMenuManager::AddSingleBitmap(UINT nBitmapID, UINT n, UINT *nID) {
  // load bitmap
  HBITMAP hbmBitmap =
      (HBITMAP)::LoadImage(AfxGetInstanceHandle(), MAKEINTRESOURCE(nBitmapID),
  ...
  for (UINT i = 0; i < n; i++) {
    if (nID[i] > 0) {
      if (GetButtonIndex(nID[i]) >= 0) {
        ...
        m_mapIDtoImage.SetAt(nID[i], (void *)iNextImage++);
    }
  }
