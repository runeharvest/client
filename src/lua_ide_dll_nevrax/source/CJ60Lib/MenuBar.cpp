...
CMenuBar::CMenuBar() {
  if (iVerComCtl32 <= 470)
    AfxMessageBox(_T("Warning: This program requires comctl32.dll version ")
                  _T("4.71 or greater."));
...
void CMenuBar::ToggleTrackButtonMode() {
  ...
  if (m_iTrackingState == TRACK_NONE || m_iTrackingState == TRACK_BUTTON) {
    SetTrackingState(m_iTrackingState == TRACK_NONE ? TRACK_BUTTON : TRACK_NONE,
                     0);
  }
}
...
void CMenuBar::TrackPopup(int iButton) {
  ...
  while (iButton >= 0) { // while user selects another menu
    ...
    g_hMsgHook = SetWindowsHookEx(WH_MSGFILTER, MenuInputFilter, NULL,
                                  ::GetCurrentThreadId());
...
BOOL CMenuBar::TranslateFrameMessage(MSG *pMsg) {
  ...
  if (WM_LBUTTONDOWN <= msg && msg <= WM_MOUSELAST) {
    if (pMsg->hwnd != m_hWnd && m_iTrackingState > 0) {
      ...
      MBTRACE(_T("CMenuBar::TranslateFrameMessage: user clicked outside menu ")
              _T("bar: end tracking\n"));
    ...
    if (vkey == VK_MENU ||
      ...
      if (m_iTrackingState > 0) {
        MBTRACE(_T("CMenuBar::TranslateFrameMessage: unknown key, stop ")
                _T("tracking\n"));
        SetTrackingState(TRACK_NONE);
      }
    }
...
void CMenuBar::Dump(CDumpContext &dc) const { CFlatToolBar::Dump(dc); }
#endif
