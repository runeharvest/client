...
DWORD CCJDockContext::CanDockDockBar() {
  ...
  if (bStyleHorz && HORZF(m_dwDockStyle)) {
    dwDock = CanDockDockBar(m_rectDragHorz, m_dwDockStyle & ~CBRS_ORIENT_VERT);
  }

  else if (VERTF(m_dwDockStyle)) {
    dwDock = CanDockDockBar(m_rectDragVert, m_dwDockStyle & ~CBRS_ORIENT_HORZ);
  }

  if (!m_bFlip) {
    if (dwDock == 0 && HORZF(m_dwDockStyle)) {
      dwCurr =
          CanDockDockBar(m_rectDragVert, m_dwDockStyle & ~CBRS_ORIENT_VERT);
      dwDock =
          CanDockDockBar(m_rectDragHorz, m_dwDockStyle & ~CBRS_ORIENT_VERT);
      dwDock = (dwDock == dwCurr) ? dwDock : 0;
    }

    if (dwDock == 0 && VERTF(m_dwDockStyle)) {
      dwCurr =
          CanDockDockBar(m_rectDragHorz, m_dwDockStyle & ~CBRS_ORIENT_HORZ);
      dwDock =
          CanDockDockBar(m_rectDragVert, m_dwDockStyle & ~CBRS_ORIENT_HORZ);
      dwDock = (dwDock == dwCurr) ? dwDock : 0;
    }
  }
...
DWORD CCJDockContext::CanDockDockBar(CRect rect, DWORD dwDockStyle,
                                     CDockBar **ppDockBar) {
  ...
  while (pos != NULL) {
    CCJSizeDockBar *pDockBar =
        (CCJSizeDockBar *)pFrame->m_listControlBars.GetNext(pos);
...
void CCJDockContext::EndDragDockBar() {
  ...
  if (m_dwOverDockStyle != 0) {
    ...
    CRect rect = (m_dwOverDockStyle & CBRS_ORIENT_VERT) ? m_rectDragVert
                                                        : m_rectDragHorz;
...
CCJSizeDockBar *CCJDockContext::GetDockBar(DWORD dwOverDockStyle) {
  ...
  if (HORZF(dwOverDockStyle)) {
    ASSERT(m_pDockSite->IsKindOf(RUNTIME_CLASS(CFrameWnd)));
    dw = CanDockDockBar(m_rectDragHorz, dwOverDockStyle & ~CBRS_ORIENT_VERT,
                        &pBar);
    ASSERT(dw != 0);
    ASSERT(pBar != NULL);
  ...
  if (VERTF(dwOverDockStyle)) {
    dw = CanDockDockBar(m_rectDragVert, dwOverDockStyle & ~CBRS_ORIENT_HORZ,
                        &pBar);
    ASSERT(dw != 0);
    ASSERT(pBar != NULL);
    return (CCJSizeDockBar *)pBar;
...
void CCJDockContext::DockSizeBar(CControlBar *pBar, CCJSizeDockBar *pDockBar,
                                 LPRECT lpRect) {
  CFrameWnd *pFrame = (CFrameWnd *)AfxGetApp()->m_pMainWnd;
  ASSERT_VALID(pFrame);
