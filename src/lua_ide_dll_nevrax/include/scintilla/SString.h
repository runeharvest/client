...
// These functions are implemented because each platform calls them something
// different.
...
// While it would be 'better' to use std::string, that doubles the executable
 ...
 * can have a buffer bigger than the string to avoid too many memory allocations
 *and copies. May have embedded zeroes as a result of @a substitute, but relies
 *too heavily on C string functions to allow reliable manipulations of these
 *strings, other than simple appends, etc.
 **/
class SString {
public:
  /** Type of string lengths (sizes) and positions (indexes). */
  typedef size_t lenpos_t;
  /** Out of bounds value indicating that the string argument should be
   * measured. */
  enum { measure_length = 0xffffffffU };

private:
  char *s;        ///< The C string
  lenpos_t sSize; ///< The size of the buffer, less 1: ie. the maximum size of
                  ///< the string
  ...
  SString &assign(const char *sOther, lenpos_t sSize_ = measure_length) {
    ...
    if (sSize > 0 && sSize_ <= sSize) { // Does not allocate new buffer if the
      ...
      if (s) {
        sSize = sSize_; // Allow buffer bigger than real string, thus providing
                        // space to grow
        sLen = strlen(s);
      } else {
        sSize = sLen = 0;
...
public:
  ...
  SString(const char *s_, lenpos_t first, lenpos_t last)
      : sizeGrowth(sizeGrowthDefault) {
    // note: expects the "last" argument to point one beyond the range end (a la
    // STL iterators)
  ...
  lenpos_t length() const { return sLen; }
  SString &operator=(const char *source) { return assign(source); }
  ...
  bool operator!=(const SString &sOther) const { return !operator==(sOther); }
  ...
  bool operator!=(const char *sOther) const { return !operator==(sOther); }
  ...
  void setsizegrowth(lenpos_t sizeGrowth_) { sizeGrowth = sizeGrowth_; }
  ...
  SString &append(const char *sOther, lenpos_t sLenOther = measure_length,
                  char sep = '\0') {
    if (!sOther) {
      return *this;
    }
  ...
  SString &operator+=(char ch) { return append(&ch, 1); }
  SString &appendwithseparator(const char *sOther, char sep) {
    return append(sOther, strlen(sOther), sep);
  }
  SString &insert(lenpos_t pos, const char *sOther,
                  lenpos_t sLenOther = measure_length) {
    if (!sOther) {
      return *this;
    }
    if (sLenOther == measure_length) {
  ...
  bool contains(const char *sFind) { return search(sFind) >= 0; }
  ...
  int remove(const char *sFind) { return substitute(sFind, ""); }
  /**
   * Duplicate a C string.
   * Allocate memory of the given size, or big enough to fit the string if
   * length isn't given; then copy the given string in the allocated memory.
   * @return the pointer to the new string
   */
  static char *StringAllocate(
      const char *s, ///< The string to duplicate
      lenpos_t len =
          measure_length) ///< The length of memory to allocate. Optional.
  {
    if (s == 0) {
      return 0;
    }
...
/**
 * Duplicate a C string.
 * Allocate memory of the given size, or big enough to fit the string if length
 * isn't given; then copy the given string in the allocated memory.
 * @return the pointer to the new string
 */
inline char *StringDup(
    const char *s, ///< The string to duplicate
    SString::lenpos_t len = SString::measure_length) ///< The length of memory
