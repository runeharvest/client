...
//-----------------------------------------------
CQuat applyRotationFactor(CQuat in, float rotFactor, CQuat qStart, CQuat qEnd,
                          float time) {
...
// readStringArray :
//-----------------------------------------------
void readStringArray(const std::string &filename,
                     NLGEORGES::UFormLoader *formLoader,
  ...
  if (formLoader) {
    ...
    if (form) {
          ...
          for (uint i = 0; i < arraySize; ++i) {
            ...
            if (elmt->getArrayNodeName(nodeName, i)) {
              if (elmt->getArrayValue(stringName, i))
                ...
                nlwarning("readStringArray: no string associated to the node "
                          "'%u(%s)'.",
                          i, nodeName.c_str());
            } else
      ...
      nlwarning("readStringArray: cannot create the form from file '%s'.",
                filename.c_str());
  } else
  ...
  if (!init) {
    ...
    for (uint i = 0; i < MBEHAV::NUMBER_OF_MODES; ++i) {
      ...
      std::map<std::string, std::string>::const_iterator it =
          mode2Animset.find(modeName);
      if (it != mode2Animset.end()) {
        mode2AnimArray[i] = (*it).second;
        if (mode2AnimArray[i].empty())
          nlwarning(
              "mode2Anim: The mode '%d(%s)' has an empty animset associated.",
              i, modeName.c_str());
      }
      // No animset for the mode.
      else {
        mode2AnimArray[i].clear();
        nlwarning("mode2Anim: no animset associated to the mode %d'%s'.", i,
                  modeName.c_str());
      }
    }
    // Init Done now.
...
// \param mode : the mode.
// \param animSetBaseName : basic name to construc the complet name of the
// animSet. \param leftHand : animSet name for the left hand. \param rightHand :
// animSet name for the right hand. \param lookAtItemsInHands : compute animset
// according to items in hands or not. \return bool : 'true' if the new
// animation set is the right one. 'false' if the one choosen is not the right
// one.
//-----------------------------------------------
bool computeAnimSet(const CAnimationSet *&animSet, MBEHAV::EMode mode,
                    const string &animSetBaseName,
                    const CItemSheet *itemLeftHand,
                    const CItemSheet *itemRightHand, bool lookAtItemsInHands) {
  static std::set<std::string> UnknownAnimSet;

  if (EAM == 0) {
    if (!ClientCfg.Light) {
      nlwarning(
          "computeAnimSet: EAM not allocated -> cannot compute the anim set.");
    }
    return false;
  }
  ...
  if (lookAtItemsInHands)
    result =
        animSetBaseName + "_" + result + "_" + animSetRight + "_" + animSetLeft;
  else
  ...
  // Bad one try something else.
    ...
    if (UnknownAnimSet.size() < 100) {
      if (UnknownAnimSet.insert(result).second)
        nlwarning("computeAnimSet: unknown Anim Set '%s' ('%u' unkowns).",
                  result.c_str(), UnknownAnimSet.size());
    }
...
void dump(const std::string &name) {
  ...
  if (fStart.open(name + "_start.rec", false, false)) {
    ...
    nlwarning("dump: cannot open/create the file '%s_start.rec'.",
              name.c_str());
...
//-----------------------------------------------
// getLodCharacterId
// Get the lod character id from the scene, according to LodCharacterName.
// Cached. -1 if id not found.
//-----------------------------------------------
  ...
  if (lodCharacterId == -1 && !lodCharacterName.empty()) {
    ...
    if (LodCharactersNotFound.size() < 100) {
      ...
      if (LodCharactersNotFound.insert(lodCharacterName).second)
        nlwarning("getLodCharacterId: Not found A Character LodCharacter in "
                  "the Manager: %s",
...
CItemSheet *getItem(const CGenderInfo &genderInfo, SLOTTYPE::EVisualSlot slot) {
  CEntitySheet *faceItem =
      SheetMngr.get(CSheetId(genderInfo.getItemName(slot)));
...
SPropVisualA buildPropVisualA(const CGenderInfo &genderInfo) {
  ...
  dest.PropertyA = 0;
  // setup items
  dest.PropertySubData.JacketModel = SheetMngr.getVSIndex(
      genderInfo.Items[SLOTTYPE::CHEST_SLOT], SLOTTYPE::CHEST_SLOT);
  dest.PropertySubData.TrouserModel = SheetMngr.getVSIndex(
      genderInfo.Items[SLOTTYPE::LEGS_SLOT], SLOTTYPE::LEGS_SLOT);
  dest.PropertySubData.ArmModel = SheetMngr.getVSIndex(
      genderInfo.Items[SLOTTYPE::ARMS_SLOT], SLOTTYPE::ARMS_SLOT);
  dest.PropertySubData.HatModel = SheetMngr.getVSIndex(
      genderInfo.Items[SLOTTYPE::HEAD_SLOT], SLOTTYPE::HEAD_SLOT);
...
SPropVisualB buildPropVisualB(const CGenderInfo &genderInfo) {
  ...
  dest.PropertyB = 0;
  // setup items
  dest.PropertySubData.HandsModel = SheetMngr.getVSIndex(
      genderInfo.Items[SLOTTYPE::HANDS_SLOT], SLOTTYPE::HANDS_SLOT);
  dest.PropertySubData.FeetModel = SheetMngr.getVSIndex(
      genderInfo.Items[SLOTTYPE::FEET_SLOT], SLOTTYPE::FEET_SLOT);
...
//-----------------------------------------------
bool isUserColorSupported(const CPlayerSheet &playerSheet,
                          SLOTTYPE::EVisualSlot vs) {
  switch (vs) {
  case SLOTTYPE::CHEST_SLOT:
    return isUserColorSupported(playerSheet.Body);
  case SLOTTYPE::LEGS_SLOT:
    return isUserColorSupported(playerSheet.Legs);
  case SLOTTYPE::HEAD_SLOT:
    return isUserColorSupported(playerSheet.Head);
  case SLOTTYPE::ARMS_SLOT:
    return isUserColorSupported(playerSheet.Arms);
  case SLOTTYPE::HANDS_SLOT:
    return isUserColorSupported(playerSheet.Hands);
  case SLOTTYPE::FEET_SLOT:
    return isUserColorSupported(playerSheet.Feet);
  case SLOTTYPE::RIGHT_HAND_SLOT:
    return isUserColorSupported(playerSheet.ObjectInRightHand);
  case SLOTTYPE::LEFT_HAND_SLOT:
    return isUserColorSupported(playerSheet.ObjectInLeftHand);
  default:
    break;
  }
  return false;
...
SPropVisualA buildPropVisualA(const CPlayerSheet &playerSheet) {
  SPropVisualA vp;
  // setup items
  vp.PropertySubData.JacketModel =
      SheetMngr.getVSIndex(playerSheet.Body.Item, SLOTTYPE::CHEST_SLOT);
  vp.PropertySubData.TrouserModel =
      SheetMngr.getVSIndex(playerSheet.Legs.Item, SLOTTYPE::LEGS_SLOT);
  vp.PropertySubData.ArmModel =
      SheetMngr.getVSIndex(playerSheet.Arms.Item, SLOTTYPE::ARMS_SLOT);
  vp.PropertySubData.HatModel =
      SheetMngr.getVSIndex(playerSheet.Head.Item, SLOTTYPE::HEAD_SLOT);
  vp.PropertySubData.WeaponRightHand = SheetMngr.getVSIndex(
      playerSheet.ObjectInRightHand.Item, SLOTTYPE::RIGHT_HAND_SLOT);
  vp.PropertySubData.WeaponLeftHand = SheetMngr.getVSIndex(
      playerSheet.ObjectInLeftHand.Item, SLOTTYPE::LEFT_HAND_SLOT);
...
SPropVisualB buildPropVisualB(const CPlayerSheet &playerSheet) {
  ...
  vp.PropertySubData.HandsModel =
      SheetMngr.getVSIndex(playerSheet.Hands.Item, SLOTTYPE::HANDS_SLOT);
  vp.PropertySubData.FeetModel =
      SheetMngr.getVSIndex(playerSheet.Feet.Item, SLOTTYPE::FEET_SLOT);
...
void drawBox(const CVector &vMin, const CVector &vMax, const CRGBA &color) {
  ...
  // Bottom quad
  line =
      CLine(CVector(vMin.x, vMin.y, vMin.z), CVector(vMax.x, vMin.y, vMin.z));
  Driver->drawLine(line, GenericMat);
  line =
      CLine(CVector(vMax.x, vMin.y, vMin.z), CVector(vMax.x, vMax.y, vMin.z));
  Driver->drawLine(line, GenericMat);
  line =
      CLine(CVector(vMax.x, vMax.y, vMin.z), CVector(vMin.x, vMax.y, vMin.z));
  Driver->drawLine(line, GenericMat);
  line =
      CLine(CVector(vMin.x, vMax.y, vMin.z), CVector(vMin.x, vMin.y, vMin.z));
  Driver->drawLine(line, GenericMat);
  // Top quad
  line =
      CLine(CVector(vMin.x, vMin.y, vMax.z), CVector(vMax.x, vMin.y, vMax.z));
  Driver->drawLine(line, GenericMat);
  line =
      CLine(CVector(vMax.x, vMin.y, vMax.z), CVector(vMax.x, vMax.y, vMax.z));
  Driver->drawLine(line, GenericMat);
  line =
      CLine(CVector(vMax.x, vMax.y, vMax.z), CVector(vMin.x, vMax.y, vMax.z));
  Driver->drawLine(line, GenericMat);
  line =
      CLine(CVector(vMin.x, vMax.y, vMax.z), CVector(vMin.x, vMin.y, vMax.z));
  Driver->drawLine(line, GenericMat);
  // Sides Quad
  line =
      CLine(CVector(vMin.x, vMin.y, vMin.z), CVector(vMin.x, vMin.y, vMax.z));
  Driver->drawLine(line, GenericMat);
  line =
      CLine(CVector(vMax.x, vMin.y, vMin.z), CVector(vMax.x, vMin.y, vMax.z));
  Driver->drawLine(line, GenericMat);
  line =
      CLine(CVector(vMax.x, vMax.y, vMin.z), CVector(vMax.x, vMax.y, vMax.z));
  Driver->drawLine(line, GenericMat);
  line =
      CLine(CVector(vMin.x, vMax.y, vMin.z), CVector(vMin.x, vMax.y, vMax.z));
  Driver->drawLine(line, GenericMat);
} // drawBox //
...
// Draw a Sphere
//-----------------------------------------------
void drawSphere(const NLMISC::CVector &center, float radius,
...
void getSeedsFromDB(CSeeds &dest) {
  ...
  nlctassert(sizeof(CSeeds::TUInt) ==
             4); // excpect that the number of each seed type is encoded on 32
                 // bits if this assert at compile, change the following code
...
// \param compute : adjust the param 'vect' to be valid or leave the old one
// unchanged if impossible. \param check : warning if the param 'vect' is not
// valid (vector Null) even with compute=true. \return bool : 'true' if the
// vectToChange has been filled, else 'false'.
//-----------------------------------------------
bool setVect(CVector &vectToChange, const CVector &vect, bool compute,
             bool check) {
  // Compute the vector.
  if (compute) {
  ...
  if (vect == CVector::Null) {
    ...
    if (check)
      nlwarning(
          "setVect: param is a vector Null, vectToChange remains the same.");
    return false;
  }
  ...
  if (src.size() >= 3) {
    if (src[0] == '&') {
      ...
      if (nextPos != string::npos) {
        ...
        std::map<std::string, CClientConfig::SSysInfoParam>::const_iterator it =
            ClientCfg.SystemInfoParams.find(colorCode);
...
std::string getStringCategory(const string &src, string &dest,
    ...
    if (src[startPos] == '&') {
      ...
      if (nextPos != string::npos) {
        ...
        for (ptrdiff_t k = 0; k < (ptrdiff_t)codeSize; ++k) {
          colorCode[k] =
              tolower((char)src[k + startPos + 1]); // TODO: toLowerAscii
        }
        string destTmp;
        if (startPos != 0)
...
void computeCurrentFovAspectRatio(float &fov, float &ar) {
  ...
  // if windowed, must modulate aspect ratio by (WindowResolution /
  // ScreenResolution)
...
// ***************************************************************************
void drawDisc(CBitmap &dest, float x, float y, float radius, const CRGBA &color,
              bool additif /*= false*/, uint numSegs /*= 127*/) {
  CPolygon2D poly;
  poly.Vertices.resize(numSegs);
  for (uint k = 0; k < numSegs; ++k) {
    poly.Vertices[k].set(x + radius * (float)cos(k / (float)numSegs * 2 * Pi),
                         y + radius * (float)sin(k / (float)numSegs * 2 * Pi));
  }
...
// *************************************************************************************************
void setDiffuse(NL3D::UInstance instance, bool onOff,
                const NLMISC::CRGBA &color) {
  if (instance.empty())
    return;
  for (uint k = 0; k < instance.getNumMaterials(); ++k) {
...
// *************************************************************************************************
void makeInstanceTransparent(UInstance &inst, uint8 opacity,
                             bool disableZWrite) {
  ...
  if (opacity == 255) {
    ...
    inst.setBypassLODOpacityFlag(true); // these flags prevails over the current
                                        // lods flags for multi-lod objects
  }
  ...
  for (uint32 j = 0; j < numMats; ++j) {
    ...
    if (opacity == 255) {
      ...
      matInst.setBlendFunc(
          (NL3D::UInstanceMaterial::TBlend)matShape.getSrcBlend(),
          (NL3D::UInstanceMaterial::TBlend)matShape.getDstBlend());
      // if orginal material is opaque or additif and has no alpha test, then
      // ensure restore last tex env if needed
      ...
      if (!matShape.getBlend() && !matShape.getAlphaTest()) {
        if (destInternalMat->getShader() == CMaterial::Normal) {
          ...
          uint numTex = 0;
          for (; numTex < 4 && srcInternalMat->getTexture(numTex) != NULL;
               ++numTex) {
          }
          if (numTex > 0) {
            if (srcInternalMat->getTexEnvMode(numTex - 1) !=
                destInternalMat->getTexEnvMode(numTex - 1)) {
              destInternalMat->setTexEnvMode(
                  numTex - 1, srcInternalMat->getTexEnvMode(numTex - 1));
            }
          }
        }
      }
      ...
      // If default is ???/one or , then use a srcalpha/one  (eg: for
      // Diamond-like weapons)
      if (matShape.getBlend() && (sint32)matShape.getDstBlend() ==
                                     (sint32)NL3D::UInstanceMaterial::one)
        matInst.setBlendFunc(NL3D::UInstanceMaterial::srcalpha,
      ...
      // if orginal material is opaque or additif and has no alpha test, then
      // ensure that the alpha output is 'diffuse'
      CMaterial *internalMat = matInst.getObjectPtr();
      if (!matShape.getBlend() && !matShape.getAlphaTest()) {
        if (internalMat->getShader() == CMaterial::Normal) {
          uint numTex = 0;
          for (; numTex < 4 && internalMat->getTexture(numTex) != NULL;
               ++numTex) {
          }
          if (numTex > 0) {
            internalMat->texEnvOpAlpha(numTex - 1, CMaterial::Replace);
            // if material is unlighted, then use the constant at this stage to
            // set the alpha
            internalMat->texEnvArg0Alpha(numTex - 1, CMaterial::Diffuse,
                                         CMaterial::SrcAlpha);
          }
        }
      }
    ...
    matInst.setAlphaTestThreshold(matShape.getAlphaTestThreshold() *
...
void setVideoMode(const UDriver::CMode &mode) {
  ...
  } else if ((!oldMode.Windowed || wasMaximized) &&
             (newMode.Windowed && !isMaximized)) // leaving fullscreen ?
  {
...
bool getRyzomModes(std::vector<NL3D::UDriver::CMode> &videoModes,
                   std::vector<std::string> &stringModeList,
                   std::vector<std::string> &stringFreqList,
                   sint &nFoundStringMode, sint &nFoundStringFreq) {
  // default values
  nFoundStringMode = -1;
  nFoundStringFreq = -1;
  ...
  // TODO: for resolutions below 1024x768, could use automatic UI scaling like
  // in login/outgame Remove modes under 1024x768 (outgame ui limitation) and
  ...
  for (i = 0; i < (sint)videoModes.size(); ++i) {
    if ((videoModes[i].Width < 1024) || (videoModes[i].Height < 768)) {
      ...
      string res = toString(videoModes[i].Width) + " x " +
                   toString(videoModes[i].Height);
      ...
      if (!bFound) {
        ...
        if ((videoModes[i].Width <= ClientCfg.Width) &&
            (videoModes[i].Height <= ClientCfg.Height)) {
        ...
        } else if (videoModes[i].Width == currentMode.Width &&
  ...
  if (nFoundStringMode == -1) {
    nlwarning("Mode %ux%u not found, fall back to windowed",
              (uint)ClientCfg.Width, (uint)ClientCfg.Height);
    ClientCfg.Windowed = true;
    ClientCfg.writeInt("FullScreen", 0);
  } else {
    ...
    for (i = 0; i < (sint)videoModes.size(); ++i) {
      ...
      if (videoModes[i].Width == videoModes[nFoundMode].Width &&
        ...
        if (!bFound) {
          ...
          if (nFoundStringFreq == -1 &&
              ((ClientCfg.Frequency == 0) || (freq == ClientCfg.Frequency))) {
...
// Get float value from string. Return true if the value is relatif ( src =
// "+15.5" for example )
...
void updateVector(const string part, CVector &dst, float value,
                  bool add /* = false */) {
  string p = part;
  if (part.size() > 1)
