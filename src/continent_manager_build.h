...
public:
  ...
  std::string TitleTextID; // should be converted with
                           // CStringManagerClient::getPlaceLocalizedName() to
                           // get the actual title in utf-8 string
  ...
  static NLMISC::CVector2f getZoneCenter(const NLLIGO::CPrimZone &z) {
    ...
    for (uint32 i = 1; i < z.VPoints.size(); ++i) {
      if (z.VPoints[i].x < vMin.x)
        vMin.x = z.VPoints[i].x;
      if (z.VPoints[i].y < vMin.y)
        vMin.y = z.VPoints[i].y;
      if (z.VPoints[i].x > vMax.x)
        vMax.x = z.VPoints[i].x;
      if (z.VPoints[i].y > vMax.y)
        vMax.y = z.VPoints[i].y;
    }
    return (vMax + vMin) / 2;
  }

  NLMISC::CVector2f getZoneCenter() { return getZoneCenter(Zone); }
};

extern bool buildLMConts(const std::string &worldSheet,
