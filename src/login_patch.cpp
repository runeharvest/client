...
CPatchManager::CPatchManager()
    : State("t_state"), DataScanState("t_data_scan_state") {
...
void CPatchManager::setClientRootPath(const std::string &clientRootPath) {
  ...
  if (CFile::isDirectory(ReadableClientDataPath))
    ReadableClientDataPath.clear();
#else
  ReadableClientDataPath.clear();
#endif

  if (ReadableClientDataPath.empty())
...
// ****************************************************************************
void CPatchManager::forceStopCheckThread() { PatchThread->StopAsked = true; }

// ****************************************************************************
void CPatchManager::forceStopPatchThread() { CheckThread->StopAsked = true; }

// ****************************************************************************
void CPatchManager::init(const std::vector<std::string> &patchURIs,
                         const std::string &sServerPath,
                         const std::string &sServerVersion) {
  uint i;
  PatchServers.clear();
  ...
  UsedServer = (sint)(((double)rand() / ((double)RAND_MAX + 1.0)) *
                      (double)PatchServers.size());
  ...
  // try to read the version file from the server (that will replace the version
  // number)
  try {
    CConfigFile *cf;
    ...
    // Use the version specified in this file, if the file does not contain an
    // asterisk
    if (line[0] != '*') {
...
#ifdef __CLIENT_INSTALL_EXE__
    {
      // The install program load a the url of the mini web site in the patch
      // directory
      ...
      try {
        ...
        std::string url =
            toString("%05u/%s", nServerVersion, clientLauncherUrl.c_str());
        // The client version is different from the server version : download
        // new description file
        getServerFile(url.c_str(),
                      false); // For the moment description file is not zipped
      } catch (...) {
        // fallback to patch root directory
        try {
          getServerFile(clientLauncherUrl.c_str(),
                        false); // For the moment description file is not zipped
        } catch (...) {
          ok = false;
        }
  ...
  try {
    ...
    for (uint32 i = 0; i < vFiles.size(); ++i) {
      ...
      if ((sExt == "idx") && (sBase == "ryzom")) {
        ...
        if (fromString(val, nNewVersion) &&
            ((nNewVersion > nVersion) || (nVersion == 0xFFFFFFFF)))
    ...
    nlwarning("EXCEPTION CATCH: readClientVersionAndDescFile() failed - not "
              "important");
...
sint32 updateCat(vector<CPatchManager::SPatchInfo::SCat> &rOutVec,
                 CPatchManager::SPatchInfo::SCat &rInCat) {
  uint32 i;
...
void CPatchManager::getInfoToDisp(SPatchInfo &piOut) {
  ...
  // Convert FilesToPatch vector that must be initialized into something human
  // readable
  uint32 i;
  ...
  for (i = 0; i < OptionalCat.size(); ++i) {
    const CBNPCategory *pCat =
        DescFile.getCategories().getCategory(OptionalCat[i]);
    nlassert(pCat != NULL);
...
void CPatchManager::startPatchThread(const vector<string> &CategoriesSelected,
  ...
  // Select all the files we have to patch depending on non-optional categories
        ...
        if (pFile != NULL) {
          ...
          if (k < FilesToPatch.size()) {
            ...
            if (!pCat->getUnpackTo().empty())
              FilesToPatch[k].ExtractPath =
                  CPath::standardizePath(pCat->getUnpackTo());
...
bool CPatchManager::getThreadState(std::string &stateOut,
                                   vector<string> &stateLogOut) {
  if ((PatchThread == NULL) && (CheckThread == NULL) &&
      (ScanDataThread == NULL))
    return false;
...
// ****************************************************************************
void CPatchManager::createBatchFile(CProductDescriptionForClient &descFile,
                                    bool wantRyzomRestart, bool useBatchFile) {
  uint nblab = 0;
      ...
      for (uint32 j = 0; j < rCat.fileCount(); ++j) {
        ...
        if (!result) {
          ...
          for (uint32 fff = 0; fff < vFilenames.size(); fff++) {
            ...
            string fullDstPath = CPath::standardizePath(
                rCat.getUnpackTo()); // to be sure there is a / at the end
            ...
            if (!useBatchFile) {
              // don't check result, because it's possible the olk file doesn't
              // exist
              CFile::deleteFile(fullDstPath + FileName);

              // try to move it, if fails move it later in a script
              if (CFile::moveFile(fullDstPath + FileName,
                                  ClientPatchPath + FileName))
                succeeded = true;
            }

            // if we didn't succeed to delete or move the file, create a batch
            // file anyway
            if (!succeeded) {
              string batchRelativeDstPath;

              // should be always true
              if (fullDstPath.compare(0, ClientRootPath.length(),
                                      ClientRootPath) == 0) {
                batchRelativeDstPath =
                    fullDstPath.substr(ClientRootPath.length()) + FileName;
              } else {
              ...
              string realDstPath = toString("\"%%ROOTPATH%%\\%s\"",
                                            batchRelativeDstPath.c_str());
              string realSrcPath =
                  toString("\"%%UNPACKPATH%%\\%s\"", FileName.c_str());
              ...
              // use DSTPATH and SRCPATH variables and append filenames
              string realDstPath =
                  toString("\"$ROOTPATH/%s\"", batchRelativeDstPath.c_str());
              string realSrcPath =
                  toString("\"$UNPACKPATH/%s\"", FileName.c_str());
  ...
  // Finalize batch file
  if (NLMISC::CFile::isExists(patchDirectory) &&
      NLMISC::CFile::isDirectory(patchDirectory)) {
    std::string patchContent;

    vector<string> vFileList;
    CPath::getPathContent(patchDirectory, false, false, true, vFileList, NULL,
                          false);
  ...
  if (!content.empty()) {
    ...
    if (fp == NULL) {
      string err = toString(
          "Can't open file '%s' for writing: code=%d %s (error code 29)",
          batchFilename.c_str(), errno, strerror(errno));
      throw Exception(err);
    } else {
...
#ifdef NL_OS_WINDOWS
    // launch upgrade script if present (it'll execute additional steps like
    ...
    if (wantRyzomRestart) {
      ...
      contentSuffix +=
          toString("start \"\" /D \"%%STARTUPPATH%%\" \"%%RYZOM_CLIENT%%\" %s "
                   "\"%%LOGIN%%\" \"%%PASSWORD%%\" \"%%SHARDID%%\"\n",
    ...
    // launch upgrade script if present (it'll execute additional steps like
    // moving or deleting files)
    contentSuffix += "if [ -e \"$UPGRADE_FILE\" ]; then chmod +x "
                     "\"$UPGRADE_FILE\" && \"$UPGRADE_FILE\"; fi\n\n";
    ...
    if (wantRyzomRestart) {
      ...
      contentSuffix += toString(
          "exec \"$RYZOM_CLIENT\" %s \"$LOGIN\" \"$PASSWORD\" \"$SHARDID\"\n",
          additionalParams.c_str());
#else
      contentSuffix += toString(
          "\"$RYZOM_CLIENT\" %s \"$LOGIN\" \"$PASSWORD\" \"$SHARDID\" &\n",
          additionalParams.c_str());
#endif
    }
#endif
...
void CPatchManager::executeBatchFile() {
  ...
  // 3 first parameters are Ryzom client full path, patch directory full path
  // and client root directory full path
#ifdef NL_OS_WINDOWS
  ...
  if (!LoginLogin.empty()) {
    ...
    if (!LoginPassword.empty()) {
      // encode password in hexadecimal to avoid invalid characters on
      // command-line
  ...
  // launchProgram with array of strings as argument will escape arguments with
  // spaces
  if (!launchProgramArray(batchFilename, arguments, false)) {
    // error occurs during the launch
    string str = toString("Can't execute '%s': code=%d %s (error code 30)",
                          batchFilename.c_str(), errno, strerror(errno));
    throw Exception(str);
  }
}
...
void CPatchManager::reboot() {
  onFileInstallFinished(); // In install program wait for the player to select
                           // in the gui of the install program if we want to
                           // Launch Ryzom or not
...
void CPatchManager::setRWAccess(const string &filename, bool bThrowException) {
  ...
  if (!NLMISC::CFile::setRWAccess(filename) && bThrowException) {
    s = CI18N::get("uiAttribErr") + " " + CFile::getFilename(filename) + " (" +
...
// ****************************************************************************
string CPatchManager::deleteFile(const string &filename, bool bThrowException,
                                 bool bWarning) {
  ...
  if (!NLMISC::CFile::deleteFile(filename)) {
    s = CI18N::get("uiDelErr") + " " + CFile::getFilename(filename) + " (" +
        toString(errno) + "," + strerror(errno) + ")";
    if (bWarning)
      setState(true, s);
    if (bThrowException)
...
void CPatchManager::renameFile(const string &src, const string &dst) {
  ...
  if (!NLMISC::CFile::moveFile(dst, src)) {
    s = CI18N::get("uiRenameErr") + " " + src + " -> " + dst + " (" +
        toString(errno) + "," + strerror(errno) + ")";
    setState(true, s);
    throw Exception(s);
  }
...
void CPatchManager::readDescFile(sint32 nVersion) {
  ...
  if (ClientRootPath != "./") {
    ...
    for (cat = 0; cat < DescFile.getCategories().categoryCount(); ++cat) {
      CBNPCategory &category =
          const_cast<CBNPCategory &>(DescFile.getCategories().getCategory(cat));
  ...
  if (foundPlatformPatchCategory) {
    ...
    for (cat = 0; cat < DescFile.getCategories().categoryCount();) {
      ...
      if (std::find(forceRemovePatchCategories.begin(),
                    forceRemovePatchCategories.end(),
        ...
        const_cast<CBNPCategorySet &>(DescFile.getCategories())
            .deleteCategory(cat);
      } else {
        ++cat;
      }
...
// ****************************************************************************
void CPatchManager::getServerFile(const std::string &name, bool bZipped,
                                  const std::string &specifyDestName,
  ...
  while (!downloadSuccess) {
    ...
    try {
      string s = CI18N::get("uiLoginGetFile") + " " +
                 NLMISC::CFile::getFilename(srcName);
      setState(true, s);
      ...
      // nlwarning("EXCEPTION CATCH: getServerFile() failed - try to find an
      // alternative: %i:
      // %s",UsedServer,PatchServers[UsedServer].DisplayedServerPath.c_str());
      ...
      if (nextServer == UsedServer) {
        ...
        UsedServer = -1;
        nlwarning(
            "EXCEPTION CATCH: getServerFile() failed - no alternative found");
      } else {
        UsedServer = nextServer;
        nlwarning(
            "EXCEPTION CATCH: getServerFile() failed - trying server: %i: %s",
            UsedServer, PatchServers[UsedServer].DisplayedServerPath.c_str());
      }
    }
  }
...
// ****************************************************************************
void CPatchManager::downloadFileWithCurl(const string &source,
                                         const string &dest,
                                         NLMISC::IProgressCallback *progress) {
  ...
  try {
    ...
    if (source.length() > 8 &&
        (source[4] == 's' || source[4] == 'S')) // 01234 https
    {
    ...
    if (fp == NULL) {
      ...
      throw Exception(
          "Can't open file '%s' for writing: code=%d %s (error code 37)",
    ...
    if (CURLE_FTP_COULDNT_RETR_FILE == res) {
      // file not found, delete local file
      NLMISC::CFile::deleteFile(dest.c_str());
      throw EPatchDownloadException(
          NLMISC::toString("curl download failed: (ec %d %d)", res, r));
    }
...
void CPatchManager::downloadFile(const string &source, const string &dest,
  ...
  if (startsWith(sourceLower, "http:") || startsWith(sourceLower, "https:") ||
      startsWith(sourceLower, "ftp:") || startsWith(sourceLower, "file:")) {
    ...
    if (!NLMISC::CFile::copyFile(dest, source, false, progress)) {
      ...
      throw Exception("cannot copy file %s to %s", source.c_str(),
                      dest.c_str());
    }
  }
}
...
// ****************************************************************************
// TODO : Review this uncompress routine to uncompress in a temp file before
// overwriting destination file

void CPatchManager::decompressFile(const string &filename) {
  string sTranslate = CI18N::get("uiDecompressing") + " " +
  ...
  if (gz == NULL) {
    string err =
        toString("Can't open compressed file '%s' : ", filename.c_str());
    if (errno == 0) {
      // gzerror
      int gzerrno;
  ...
  FILE *fp = nlfopen(dest, "wb");
  if (fp == NULL) {
    string err = toString("Can't open file '%s' : code=%d %s, (error code 32)",
                          dest.c_str(), errno, strerror(errno));
  ...
  while (!gzeof(gz)) {
    ...
    if (res == -1) {
      ...
      fclose(fp);
      // deleteFile (filename);
      throw Exception("Can't read compressed file '%s' (after %d bytes) : "
                      "code=%d %s, (error code 33)",
                      filename.c_str(), currentSize, gzerrno, gzerr);
    }
    ...
    if (res2 != res) {
      ...
      string err =
          toString("Can't write file '%s' : code=%d %s (error code 34)",
                   dest.c_str(), errno, strerror(errno));
...
void CPatchManager::applyDate(const string &sFilename, uint32 nDate) {
  ...
  if (nDate != 0) {
    ...
    string s = CI18N::get("uiChangeDate") + " " +
               NLMISC::CFile::getFilename(sFilename) + " " +
               timestampToHumanReadable(
                   NLMISC::CFile::getFileModificationDate(sFilename)) +
               " -> " + timestampToHumanReadable(nDate);
    setState(true, s);

    if (!NLMISC::CFile::setFileModificationDate(sFilename, nDate)) {
      int err = NLMISC::getLastError();
      s = CI18N::get("uiChgDateErr") + " " + CFile::getFilename(sFilename) +
          " (" + toString(err) + ", " + formatErrorMessage(err) + ")";
      setState(true, s);
    }
    s = CI18N::get("uiNowDate") + " " + CFile::getFilename(sFilename) + " " +
        timestampToHumanReadable(
            NLMISC::CFile::getFileModificationDate(sFilename));
    setState(true, s);
  }
...
// ****************************************************************************
// Get all the patches that need to be applied to a file from the description of
// this file given by the server
void CPatchManager::getPatchFromDesc(SFileToPatch &ftpOut, const CBNPFile &fIn,
  ...
  // first see if there's a version that was downloaded from the background
  // downloader
  ...
  const CBNPCategory *cat =
      DescFile.getCategories().getCategoryFromFile(rFilename);
  if (cat) {
  ...
  // Only look in data path if the file should not be unpack (otherwise it
  // should only remains in the "unpack" directory)
  if (!needUnpack) {
    if (sFilePath.empty()) {
      ...
      } else if (NLMISC::CFile::fileExists(ReadableClientDataPath +
                                           rFilename)) {
  ...
  if (sFilePath.empty() &&
      NLMISC::CFile::fileExists(ClientPatchPath + rFilename)) {
  ...
  if (sFilePath.empty()) {
    ...
    // If forceChecksum is wanted (slow), then don't do the test with
    // filesize/date
    if (!forceCheckSumTest) {
      for (j = 0; j < rFile.versionCount(); ++j) {
        ...
        if ((nServerSize == nLocalSize) &&
            (abs((sint32)(nServerTime - nLocalTime)) <= 2)) {
          nVersionFound = rVersion.getVersionNumber();
          // break; // ace -> get the last good version (if more than one
          // version of the same file exists)
        }
      }
    }
    ...
    } else // A version of the file has been found
    {
      ...
      nlassert(j != rFile.versionCount()); // Not normal if we cant find the
                                           // version we found previously
      ...
      ftpOut.FinalFileSize =
          rFile.getVersion(rFile.versionCount() - 1).getFileSize();
    }
  } // end of else local BNP file exists
}

// ****************************************************************************
bool CPatchManager::bnpUnpack(const string &srcBigfile, const string &dstPath,
                              vector<string> &vFilenames) {
  ...
  //	if		(NLMISC::CFile::fileExists(ClientDataPath + srcBigfile))
  //SourceName = ClientDataPath + srcBigfile; 	else if
  //(NLMISC::CFile::fileExists(srcBigfile))
  //SourceName = ClientDataPath + srcBigfile;
  SourceName = srcBigfile;

  // following lines removed by Sadge to ensure that the correct file gets
  // patched
...
// ****************************************************************************
int CPatchManager::downloadProgressFunc(void *foo, double t, double d,
                                        double ultotal, double ulnow) {
  if (d != t) {
    // In the case of progress = 1, don't update because, this will be called in
    // case of error to signal the end of the download, though no download
    // actually occurred. Instead, we set progress to 1.f at the end of
    // downloadWithCurl if everything went fine
...
// ****************************************************************************
int CPatchManager::validateProgress(void *foo, double t, double d,
                                    double /* ultotal */, double /* ulnow */) {
  ...
  string sTranslate =
      CI18N::get("uiLoginGetFile") +
      toString(" %s : %s / %s (%.02f %%)",
               NLMISC::CFile::getFilename(pPM->CurrentFile).c_str(),
               NLMISC::bytesToHumanReadableUnits((uint64)d, units).c_str(),
               NLMISC::bytesToHumanReadableUnits((uint64)t, units).c_str(),
               pour1);
...
void CPatchManager::MyPatchingCB::progress(float f) {
  ...
  double p = 100.0 * f;
  string sTranslate =
      CI18N::get("uiApplyingDelta") +
      toString(" %s (%.02f %%)", CFile::getFilename(patchFilename).c_str(), p);
  pPM->setState(false, sTranslate);
}
...
uint CPatchManager::applyScanDataResult() {
...

  {
    ...
    for (uint i = 0; i < numError; i++) {
      ...
      // get file path
      // following lines added by Sadge to ensure that the correct file gets
      // patched
      string sFilePath;
      if (NLMISC::CFile::fileExists(WritableClientDataPath + ftp.FileName))
        sFilePath = WritableClientDataPath + ftp.FileName;
      if (sFilePath.empty() &&
          NLMISC::CFile::fileExists(ReadableClientDataPath + ftp.FileName))
        sFilePath = ReadableClientDataPath + ftp.FileName;
      if (sFilePath.empty() &&
          NLMISC::CFile::fileExists(ClientPatchPath + ftp.FileName))
        sFilePath = ClientPatchPath + ftp.FileName;

      // following lines removed by Sadge to ensure that the correct file gets
      // patched
      //			string sFilePath = CPath::lookup(ftp.FileName,
      //false, false); 			if (sFilePath.empty())
      //			{
      //				if
      //(NLMISC::CFile::fileExists(ClientPatchPath + ftp.FileName)) 					sFilePath =
      //ClientPatchPath + ftp.FileName;
      //			}

      // Reset to a dummy date, so the patch will be fully/checked/patched next
      // time
      if (!sFilePath.empty())
...
// ***************************************************************************
void CPatchManager::getCorruptedFileInfo(const SFileToPatch &ftp,
                                         string &sTranslate) {
  sTranslate = CI18N::get("uiCorruptedFile") + " " + ftp.FileName + " (" +
               toString("%.1f ", (float)ftp.FinalFileSize / 1000000.f) +
               CI18N::get("uiMb") + ")";
}
...
void CCheckThread::run() {
  ...
  try {
    ...
    if (nClientVersion != nServerVersion) {
      ...
      try {
        pPM->DescFilename =
            toString("%05d/ryzom_%05d.idx", nServerVersion, nServerVersion);
        // The client version is different from the server version : download
        // new description file
        pPM->getServerFile(
            pPM->DescFilename,
    ...
    for (i = 0; i < rDescFiles.fileCount(); ++i) {
      ...
      sTranslate = CI18N::get("uiCheckingFile") + " " +
                   rDescFiles.getFile(i).getFileName();
      pPM->setState(true, sTranslate);
      // get list of patch to apply to this file. don't to a full checksum test
      // if possible
      nlwarning(rDescFiles.getFile(i).getFileName().c_str());
      pPM->getPatchFromDesc(ftp, rDescFiles.getFile(i), false);
      // add the file if there are some patches to apply, or if an already
      // patched version was found in the unpack directory
      if (!ftp.Patches.empty() ||
          (IncludeBackgroundPatch && !ftp.SrcFileName.empty())) {
        pPM->FilesToPatch.push_back(ftp);
        sTranslate =
            CI18N::get("uiNeededPatches") + " " + toString(ftp.Patches.size());
        pPM->setState(true, sTranslate);
      }
      CurrentFileChecked = i;
    }

    // Here we got all the files to patch in FilesToPatch and all the versions
    // that must be obtained Now we have to get the optional categories
      ...
      if (rCat.isOptional())
        for (j = 0; j < rCat.fileCount(); ++j) {
          ...
          for (k = 0; k < pPM->FilesToPatch.size(); ++k) {
            if (stricmp(pPM->FilesToPatch[k].FileName.c_str(),
                        rFilename.c_str()) == 0) {
    ...
    // For all categories that required an optional category if the cat required
    // is present the category that reference it must be present
    for (i = 0; i < rDescCats.categoryCount(); ++i) {
      // For all optional categories check if there is a 'file to patch' in it
      const CBNPCategory &rCat = rDescCats.getCategory(i);
      ...
      if (rCat.isOptional() && rCat.isHidden()) {
        ...
        for (j = 0; j < pPM->OptionalCat.size(); ++j) {
          if (rCat.getName() == pPM->OptionalCat[j]) {
            // remove it
            /*#ifdef RY_BG_DOWNLOADER
                    // fixme : strange stlport link error for the background
            downloader when using erase ... std::copy(pPM->OptionalCat.begin() +
            j + 1, pPM->OptionalCat.end(), pPM->OptionalCat.begin()+j);
    ...
    // Get all extract to category and check files inside the bnp with real
    // files
    for (i = 0; i < rDescCats.categoryCount(); ++i) {
      // For all optional categories check if there is a 'file to patch' in it
      const CBNPCategory &rCat = rDescCats.getCategory(i);
...
// ****************************************************************************
void CPatchThread::clear() { AllFilesToPatch.clear(); }
...
void CPatchThread::run() {
  ...
  // ---------------------------------------
  // To do that we create a batch file that will copy all files (unpacked to
  // patch directory) to the directory we want (the UnpackTo string)
...
public:
  ...
  CPatchThreadDownloadProgress()
      : PatchThread(NULL), Scale(1.f), Bias(0.f), CurrentFilePatched(0) {}
  virtual void progress(float progressValue) {
    clamp(progressValue, 0.f, 1.f);
    nlassert(PatchThread);
    // PatchThread->CurrentFilePatched =
    // std::max(PatchThread->CurrentFilePatched, (float)
    // this->CurrentFilePatched + Bias + Scale * progressValue);
    PatchThread->CurrentFilePatched =
        this->CurrentFilePatched + Bias + Scale * progressValue;
...
void CPatchThread::processFile(CPatchManager::SFileToPatch &rFTP) {
  ...
  if (rFTP.ExtractPath.empty()) {
    ...
    if (rFTP.LocalFileExists) {
      ...
      if (NLMISC::CFile::fileExists(pPM->WritableClientDataPath +
                                    rFTP.FileName)) {
        SourceName = pPM->WritableClientDataPath + rFTP.FileName;
      } else if (NLMISC::CFile::fileExists(pPM->ReadableClientDataPath +
                                           rFTP.FileName)) {
      ...
      if (SourceName.empty())
        throw Exception(std::string("ERROR: Failed to find file: ") +
                        rFTP.FileName);

      // following lines removed by Sadge to ensure that the correct file gets
      // patched
      //			SourceName = CPath::lookup(rFTP.FileName); //
      //exception if file do not exists
    } else {
  ...
  if (pPM->getAsyncDownloader()) {
    ...
    string PatchName =
        toString("%05d/%s%s", rFTP.Patches[rFTP.Patches.size() - 1],
                 rFTP.FileName.c_str(), ".lzma");
    pPM->getAsyncDownloader()->addToDownloadList(
        PatchName, SourceName, rFTP.LastFileDate, rFTP.ExtractPath,
        rFTP.SZFileSize, rFTP.FinalFileSize);
    return;
  }

  std::string tmpSourceName =
      rFTP.SrcFileName.empty()
          ? SourceName
          : rFTP.SrcFileName; // source is same than destination, or possibly
                              // a patch from a previous background downloader
                              // session
  if (rFTP.LocalFileToDelete) {
    // corrupted or invalid file ? ....
    rFTP.LocalFileExists = false;
  ...
  if ((rFTP.Incremental &&
       !rFTP.LocalFileExists) // incremental and no base file, we need a
                              // complete file
      || totalPatchSize >
             rFTP.FinalFileSize / 2 // patch is too big regarding the final
                                    // file, patch applying time will be slow
      || rFTP.SZFileSize < totalPatchSize) // lzma is smaller than patch !
  {
    breakable {
      ...
      try {
        ...
        try {
          ...
          if (!rFTP.Patches.empty()) {
            pPM->getServerFile(toString("%05u/", rFTP.Patches.back()) +
                                   lzmaFile,
                               false, "", &progress);
          }
          // else -> file comes from a previous download (with .tmp extension,
          // and is up to date) the remaining code will just rename it with good
          // name and exit
      ...
      OutFilename =
          pPM->ClientPatchPath + NLMISC::CFile::getFilename(rFTP.FileName);
      // try to unpack the file
      try {
        if (!unpackLZMA(pPM->ClientPatchPath + lzmaFile,
                        OutFilename + ".tmp")) {
        ...
        nlwarning("Failed to unpack lzma file %s",
                  (pPM->ClientPatchPath + lzmaFile).c_str());
        // fallback to standard patch method
        usePatchFile = true;
        break;
      }
      ...
      pPM->deleteFile(pPM->ClientPatchPath +
                      lzmaFile); // delete the archive file
      pPM->deleteFile(SourceName, false,
                      false); // File can exists if bad BNP loading
      if (_CommitPatch) {
  ...
  if (usePatchFile) {
    ...
    for (uint32 j = 0; j < rFTP.Patches.size(); ++j) {
      ...
      string PatchName;
      try {
        PatchName = toString("%05d/", rFTP.Patches[j]) +
                    rFTP.FileName.substr(0, rFTP.FileName.size() - 4) +
                    toString("_%05d", rFTP.Patches[j]) + ".patch";
        sTranslate = CI18N::get("uiLoginGetFile") + " " + PatchName;
        pPM->setState(true, sTranslate);
        progress.Scale = 1.f;
        progress.Bias = totalPatchSize != 0
                            ? (float)currentPatchedSize / totalPatchSize
                            : 0.f;
        progress.Scale = totalPatchSize != 0
                             ? (float)rFTP.PatcheSizes[j] / totalPatchSize
                             : 1.f;
      ...
      if (!rFTP.Incremental) {
        ...
        // Usually, files such as '.exe' or '.dll' do not work well with
        // incremental patch, so it is better to apply a single patch from a
        // refrence version to them. (so here we necessarily got
        // 'rFTP.Patches.size() == 1' !)
        // The reference version itself is incrementally patched, however, and
        // may be rebuilt from time to time when the delta to the reference has
        // become too big (but is most of the time < to the sum of equivallent
        // delta patchs) The non-incremental final patch (from ref file to final
        // bnp), is guaranteed to be done last after the reference file has been
        // updated
        ...
        if (!_CommitPatch) {
          ...
          if (!NLMISC::CFile::fileExists(pPM->ClientPatchPath + tmpRefFile)) {
            // Not found in the patch directory -> version in data directory
            // should be good, or would have been detected by the check thread
            // else.
          } else {
            SourceNameXD =
                tmpRefFile; // good ref file download by bg downloader
          }
        }
        ...
        //			if
        //(NLMISC::CFile::fileExists(pPM->ClientDataPath + SourceNameXD))
        //SourceNameXDFull = pPM->ClientDataPath + SourceNameXD;

        // following lines removed by Sadge to ensure that the correct file gets
        // patched
        //			string SourceNameXDFull =
        //CPath::lookup(SourceNameXD, false, false); 			if
        //(SourceNameXDFull.empty()) 				SourceNameXDFull = pPM->ClientDataPath +
        //SourceNameXD; 			SourceNameXD = SourceNameXDFull;
        if (CFile::fileExists(pPM->WritableClientDataPath + SourceNameXD)) {
          SourceNameXD = pPM->WritableClientDataPath + SourceNameXD;
        } else if (CFile::fileExists(pPM->ReadableClientDataPath +
                                     SourceNameXD)) {
      ...
      string OutFilename =
          pPM->ClientPatchPath + rFTP.FileName + ".tmp__" + toString(j);
      ...
      if (j > 0) {
        pPM->deleteFile(SourceNameXD, false,
                        false); // File can exists if bad BNP loading
      }
    ...
    if (tmpSourceName != DestinationName) {
      ...
      if (!_CommitPatch) {
        ...
        pPM->renameFile(tmpSourceName,
                        pPM->ClientPatchPath + rFTP.FileName + ".tmp");
      } else {
...
// ****************************************************************************
void CPatchThread::xDeltaPatch(const string &patch, const string &src,
                               const string &out) {
  // Internal xdelta
  ...
  std::string errorMsg;
  CXDeltaPatch::TApplyResult ar =
      CXDeltaPatch::apply(patch, src, out, errorMsg, &patchingCB);
  if (ar != CXDeltaPatch::ApplyResult_Ok) {
    ...
    case CXDeltaPatch::ApplyResult_DiskFull:
      ...
      break;
    default: {
      std::string str = toString("Error applying %s to %s giving %s",
                                 patch.c_str(), src.c_str(), out.c_str());
      throw Exception(str);
    } break;
    }
  }
...
void CScanDataThread::run() {
  ...
  try {
    ...
    for (i = 0; i < rDescFiles.fileCount(); ++i) {
      ...
      // get list of file to apply to this patch, performing a full checksum
      // test (slow...)
      CPatchManager::SFileToPatch ftp;
      pPM->getPatchFromDesc(ftp, rDescFiles.getFile(i), false);
      // if the file has been found but don't correspond to any local version
      // (SHA1) or if the file has not been found (or the .bnp is very very
      // buggy so that addSearchFile() failed)
...
uint32 CPatchManager::SPatchInfo::getAvailablePatchsBitfield() const {
  ...
  // Required : stands for 'bnp' required by the Optional bnps !! so ignore only
  // RoS is wanted
...
// **************************************************************************
// ****************************************************************************
void CPatchManager::startInstallThread(
    const std::vector<CInstallThreadEntry> &entries) {
...
void CPatchManager::startDownloadThread(
    const std::vector<CInstallThreadEntry> &entries) {
...
void CPatchManager::onFileDownloading(const std::string &sourceName,
                                      uint32 rate, uint32 fileIndex,
                                      uint32 fileCount, uint64 fileSize,
                                      uint64 fullSize) {
  if (_AsyncDownloader) {
    _AsyncDownloader->onFileDownloading(sourceName, rate, fileIndex, fileCount,
                                        fileSize, fullSize);
  }
}

void CPatchManager::onFileInstalling(const std::string &sourceName, uint32 rate,
                                     uint32 fileIndex, uint32 fileCount,
                                     uint64 fileSize, uint64 fullSize) {
  if (_AsyncDownloader) {
    _AsyncDownloader->onFileInstalling(sourceName, rate, fileIndex, fileCount,
                                       fileSize, fullSize);
  }
}

bool CPatchManager::download(const std::string &patchFullname,
                             const std::string &sourceFullname,
  ...
  // if the file is the same then indicates its not necessary to download it a
  // second time
  if (NLMISC::CFile::fileExists(sourceFullname)) {
    uint32 t = NLMISC::CFile::getFileModificationDate(sourceFullname);
    if (t == timestamp) { // we didn't downl oad
      return true;
    }
  ...
  if (patchFullname.size() >= zsStrLength &&
      patchFullname.substr(patchFullname.size() - zsStrLength) == zsStr) {
    extension = zsStr;
  }
  // remove tmp file if exist
  std::string patchName = tmpDirectory +
                          NLMISC::CFile::getFilename(sourceFullname) +
                          extension + std::string(".tmp");
  ...
  if (patchName.size() >= zsStrLength &&
    ...
    std::string outFilename =
        patchName.substr(0, patchName.size() - zsStrLength);
...
bool CPatchManager::extract(const std::string &patchPath,
                            ...
                            const std::string &execName, void (*stopFun)()) {
  ...
  if (fp == 0) {
    string err =
        toString("Can't open file '%s' for writing: code=%d %s (error code 29)",
                 updateBatchFilename.c_str(), errno, strerror(errno));
    throw Exception(err);
  }
...
void CPatchManager::setStateListener(
    IPatchManagerStateListener *stateListener) {
  _StateListener = stateListener;
}

void CPatchManager::fatalError(const std::string &errorId,
...
void CDownloadThread::run() {
  ...
  std::string patchPath =
      CPath::standardizePath(ClientRootPath + TheTmpInstallDirectory) +
      "patch/";
  ...
  for (uint first = 0, last = (uint)_Entries.size(); first != last; ++first) {
    std::string patchName =
        CPath::standardizePath(_Entries[first].PatchName, false);
    std::string sourceName =
        CPath::standardizePath(_Entries[first].SourceName, false);
    ...
    // update Gui
    pPM->onFileDownloading(sourceName, rate, first + 1, last, _CurrentSize,
                           _FullSize);
    ...
    if (simulateDownload) {
      ...
      // Do not download if file are identicaly (check size and modification
      // date)
      ...
      if (toDownload) {
        ...
        // Try to download, rename, applyDate and send error msg to gui in case
        // of error
        try {
...
void CInstallThread::run() {
  ...
  for (first = 0, last = (uint)_Entries.size(); first != last; ++first) {
    std::string correct =
        CPath::standardizePath(patchPath + _Entries[first].PatchName, false);
    allowed.insert(correct);
  }
  // Delete file from tmp directory that are not "allowed" (torrent protocol can
  // download partial file that are not asked)
  ...
  for (first = 0, last = (uint)_Entries.size(); first != last; ++first) {
    ...
    if (NLMISC::CFile::fileExists(patchName)) {
      ...
      static std::string::size_type zsStrLength = zsStr.size();
      try {
        // if file is compressed unpack the file (decompress, rename tempory
        // file, apply modification date)
        if (patchName.size() >= zsStrLength &&
            patchName.substr(patchName.size() - zsStrLength) == zsStr) {
          std::string outFilename =
              patchName.substr(0, patchName.size() - zsStrLength);
          ...
          // if file is uncompressed rename tempory file and apply modification
          // date)
    ...
    // update gui
    pPM->onFileInstalling(sourceName, rate, first + 1, last, _CurrentSize,
                          _FullSize);
  }
  // extract bnp
