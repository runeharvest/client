...
struct CDynamicStringInfos {
  ...
  /// index in the infos buffer, same as the index in the client dynamic string
  /// known buffer
  uint32 Index;
  ...
  /// Constructor
  CDynamicStringInfos()
      : Associated(false), Index(0xffffffff), IsHuffman(true), Str("???") {}
};
...
public:
  ...
  /// ) \param index the index of the string \param str the string \param
  /// huffCode the Huffman code(may be empty) \return the index of the string
...
public:
  ...
  public:
     ...
     *	\param dynChatId is valid only if mode==dyn_chat. This the Id of channel
     *(not the index in DB!)
     */
    virtual void displayChat(TDataSetIndex compressedSenderIndex,
                             ...
                             std::string &senderName, uint bubbleTimer = 0) = 0;
    /**
    ...
    /**
     *	Clear a channel.
     *	\param dynChatDbIndex is valid only if mode==dyn_chat. Contrary to
     *displayChat, this is the Db Index (0..MaxDynChanPerPlayer)
     */
    virtual void clearChannel(CChatGroup::TGroupType mode,
...
public:
   ...
   * \param receiver is the name of the listening char (truncated to 255 char
   * max) \param str is the chat content (truncated to 255 char max)
   */
  void tell(const std::string &receiver, const std::string &str);

  /** Get the last name of the people with which a 'tell' has been done, then
   * move that name at the start of the list
   */
   ...
   *	Not the db index! Use getDynamicChannelIdFromDbIndex() if you got a
   *dbIndex
   */
  void setChatMode(CChatGroup::TGroupType group,
   ...
   */
  void processTellString(NLMISC::CBitMemStream &bms,
                         IChatDisplayer &chatDisplayer);

  /**
   * Get the string for a far tell. display now if ready or delay in
   * flushBuffer()
   */
  void processFarTellString(NLMISC::CBitMemStream &bms,
                            IChatDisplayer &chatDisplayer);

  /**
   * Extract and decode the chat string from the stream. display now if ready or
   * delay in flushBuffer()
   */
  void processChatString(NLMISC::CBitMemStream &bms,
                         IChatDisplayer &chatDisplayer);

  /**
   * Extract and decode the chat string from the stream. display now if ready or
   *delay in flushBuffer() Difference with processTellString() is that
   *processTellString2() receive a DynamicString for the message
   */
  void processTellString2(NLMISC::CBitMemStream &bms,
                          IChatDisplayer &chatDisplayer);

  /**
   * Extract and decode the chat string from the stream. display now if ready or
   *delay in flushBuffer() Difference with processChatString() is that
   *processChatString2() receive a DynamicString for the message
   */
  void processChatString2(NLMISC::CBitMemStream &bms,
                          IChatDisplayer &chatDisplayer);

  /**
   * Extract and decode the chat string from the stream.
   *	the stream here is only a iunt32 for the id of the dynamic string
   *	\param type where do you want this string to go (dyn_chat is not
   *allowed)
   */
  void processChatStringWithNoSender(NLMISC::CBitMemStream &bms,
   ...
   * \return true if the string is finalize, false if some param are missing
   * from network
   */
  bool getString(std::string &result, std::vector<uint64> &args,
                 const std::string &strbase);

  // build a sentence to be displayed in the chat (e.g add "you say", "you
  // shout", "[user name] says" or "[user name] shout")
  static void buildChatSentence(TDataSetIndex compressedSenderIndex,
                                const std::string &sender,
                                const std::string &msg,
                                CChatGroup::TGroupType type,
                                std::string &result);
  // build a sentence to be displayed in the tell
  static void buildTellSentence(const std::string &sender,
  ...
  // return true if channel exist (getDynamicChannelDbIndexFromId>=0 &&
  // <MaxDynChan)
...
private:
  ...
  // peoples
  std::list<std::string>
      _TellPeople; // the last people on which tells ha been done
  uint _NumTellPeople;
  ...
  NLMISC::CCDBNodeLeaf
      *_DynamicChannelNameLeaf[CChatGroup::MaxDynChanPerPlayer];
...
/** This class enable you to store a static string + arg and evaluate it every
   frame. All args could perhaps not be translated because all dynamic string
   are not received right now. So you can call getString() every time to
 */
#ifdef OLD_STRING_SYSTEM
...
public:
  ...
  void setString(const ucstring &staticStringId,
