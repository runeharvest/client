...
NLMISC::CRGBA CUserLandMark::getColor() const { return _LandMarksColor[Type]; }
...
//===================================================================================
uint8 *CFogOfWar::getData() { return (Tx != NULL) ? Tx->getPointer() : NULL; }
...
void CFogOfWar::load(const std::string & /* contName */) {
  /* // Temporary comment : see this when server ready
          string fileName = "save/fow_" + contName + "_" +
     PlayerSelectedFileName + ".ibmp";
          ...
          else
          {
                  ...
                  // Look in all the maps to know the texture map size and apply
     a ratio to this size CWorldSheet *pWS =
     dynamic_cast<CWorldSheet*>(SheetMngr.get(CSheetId("ryzom.world"))); if (pWS
     == NULL) return; for (uint32 i = 0; i < pWS->Maps.size(); ++i)
                  {
                          SMap &rM = pWS->Maps[i];
                          if (strncmp(rM.Name.c_str(), "continent", 9) == 0)
                          {
                                  if (stricmp(rM.ContinentName.c_str(),
     contName.c_str()) == 0)
                                  {
                                          // get the size of the bitmap
                                          uint32 nWidth = 0, nHeight = 0;
                                          string path =
     CPath::lookup(rM.BitmapName, false); if (!path.empty())
                                                  CBitmap::loadSize(path,
     nWidth, nHeight);
                                          // create the texture fow
                                          if ((nWidth != 0) && (nHeight != 0))
                                          {
                                                  MapWidth = (sint16)nWidth / 4;
                                                  MapHeight = (sint16)nHeight /
     4; createData(MapWidth, MapHeight);
                                          }
                                          MinX = rM.MinX;
                                          MinY = rM.MinY;
                                          MaxX = rM.MaxX;
...
class CChangeTaskPriority : public CTaskManager::IChangeTaskPriority {
  virtual float getTaskPriority(const IRunnable &runnable) {
    const NLMISC::IRunnablePos *_3dRunnable =
        dynamic_cast<const NLMISC::IRunnablePos *>(&runnable);
    if (_3dRunnable) {
...
void CContinent::setup() {
  ...
  for(uint k = 0; k < EGSPD::CSeason::Invalid; ++k)
  {
          nlinfo("Season = %s, num setups = %d",
  EGSPD::CSeason::toString((EGSPD::CSeason::TSeason) k).c_str(), (int)
  WeatherFunction[k].getNumWeatherSetups()); for(uint l = 0; l <
  WeatherFunction[k].getNumWeatherSetups(); ++l)
          {
                  if (WeatherFunction[k].getWeatherSetup(l))
                  {
                          nlinfo("Setup %d for season %s is %s", (int) l,
  EGSPD::CSeason::toString((EGSPD::CSeason::TSeason) k).c_str(),
  NLMISC::CStringMapper::unmap(WeatherFunction[k].getWeatherSetup(l)->SetupName).c_str());
                  }
                  else
                  {
...
//-----------------------------------------------
void CContinent::select(const CVectorD &pos,
                        NLMISC::IProgressCallback &progress, bool complete,
                        bool unhibernate, EGSPD::CSeason::TSeason season) {
  ...
  if (Landscape)
    Landscape->setTileColor(TileColorMono[Season], TileColorFactor[Season]);
  ...
  if (complete) {
      ...
      if (!ClientCfg.Light) {
        if (Landscape) {
          ...
          if (!Indoor) {
            ...
            Landscape->setPointLightDiffuseMaterial(
                LandscapePointLightMaterial);
          }
        }
      }
    }
    // Initialize Landscape IG Callbacks.
    {
      ...
      if (!ClientCfg.Light) {
        // Add weather as an observer (to know when an ig is added, and to set
        // its lightmaps color accordingly)
        registerObserver(&LightCycleManager);
        IGCallbacks->registerObserver(&LightCycleManager);

        // register the LightCycleManager as an observer to Objects that may
        // create IGs
        ...
        if (!Indoor) {
          ...
          try {
            ...
            Scene->setCoarseMeshManagerTexture(
                CPath::lookup(seasonname).c_str());
          } catch (const Exception &e) {
            nlwarning(e.what());
          }
        }
      }
      // Register door handling (when an ig is added check all shapes for doors
      // and add to the door manager)
...

    {
      ...
      if (IGCallbacks)
        IGCallbacks->setMoveContainer(PACS);
    }

    {
      ...
      if (!Indoor) {
        if (!ClientCfg.Light && Landscape) {
          // Init ig
          try {
            LandscapeIGManager.initIG(Scene, CPath::lookup(LandscapeIG).c_str(),
                                      Driver, Season, &progress);
          } catch (const Exception &e) {
            nlwarning(e.what());
          }
        }
        ...
        for (uint i = 0; i < igsWithNames.size(); ++i) {
          string igZone = toLowerAscii(
              CFile::getFilenameWithoutExtension(igsWithNames[i].second));
          ...
          for (uint j = 0; j < ZCList.size(); ++j) {
            ...
            if (outpost) {
              ...
              string outpostZone = toLowerAscii(
                  CFile::getFilenameWithoutExtension(ZCList[j].Name));
              if (igZone == outpostZone) {
                nlctassert(RZ_MAX_BUILDING_PER_OUTPOST == 4);
                // Check there is all information in the IG for a ZC and
                // initialize.
                bool zcOK[RZ_MAX_BUILDING_PER_OUTPOST + 1] = {
                ...
                for (k = 0; k < ig->getNumInstance(); ++k) {
                  ...
                  for (b = 0; b < RZ_MAX_BUILDING_PER_OUTPOST; b++) {
                    // Good one ?
                    if (ig->getInstanceName(k) ==
                        ("bat_zc_0" + toString(b + 1))) {
                      // Set the position
                ...
                if (k != RZ_MAX_BUILDING_PER_OUTPOST + 1)
                  nlwarning("CContinent::select : Zone '%s' should be a ZC but "
                            "some doomy are missing.",
                            igsWithNames[i].second.c_str());
          ...
          if (IGCallbacks)
            IGCallbacks->addIG(igsWithNames[i].first);
        }
        ...
        if (Landscape) {
          if (!Indoor) {
            ...
            if (!SmallBank.empty() && !farBankSeason.empty()) {
              ...
              std::string farBankSeasonPath =
                  CPath::lookup(farBankSeason, false);
              ...
              Landscape->postfixTileFilename(
                  CSeasonFileExt::getExtension(Season));
              Landscape->postfixTileVegetableDesc(
                  CSeasonFileExt::getExtension(Season));
      ...
      if (!Indoor) {
        {
          ...
          try {
            ...
            if (!CanopyIGfileName[CurrSeason].empty() &&
                CurrSeason != EGSPD::CSeason::Invalid) {
              BackgroundIG = UInstanceGroup::createInstanceGroup(
                  CanopyIGfileName[CurrSeason]);
            } else {
              BackgroundIG =
          ...
          if (BackgroundIG) {
            if (SceneRoot) {
              ...
              // See for which objects distance should be overriden (object
              // which use a .PLANT sheet)
    ...
    if (!ClientCfg.Light) {
      if (!Indoor) {
        if (Landscape) {
          ...
          // If any zone already loaded into the landscape, must load them to
          // the IGManager.
          //	Yoyo: this is important for teleportation near a current
          //position, since not so many landscape zones 	will be removed/added.
          ...
          Landscape->refreshAllZonesAround(
              pos, ClientCfg.Vision + ExtraZoneLoadingVision, zonesAdded,
  ...
  if (complete || unhibernate) {
    ...
    CollisionManager->setSunContributionPower(
        EntitySunContributionPower, EntitySunContributionMaxThreshold);
  }

  if (complete) {
    reloadFogMap();
    // temp for debug : create visualisation of current pacs primitives
    // createInstancesFromMoveContainer(Scene, IGCallbacks->getMoveContainer(),
    // NULL);
    //
    // Progress bar
    progress.popCropedValues();
  ...
  if (complete || unhibernate) {
    if (!ClientCfg.Light && ClientCfg.LandscapeEnabled &&
        ClientCfg.MicroLifeEnabled) {
      H_AUTO(InitRZWorldLoadMicroLife)
      loadMicroLife();
    }
  }
  // Register callback on streaming
  CAsyncFileManager::getInstance().registerTaskPriorityCallback(
      &ChangeTaskPriority);
} // select //

//=========================================================================
void CContinent::reloadFogMap() {
  const R2::CScenarioEntryPoints::CCompleteIsland *completeIsland =
      R2::CScenarioEntryPoints::getInstance().getCompleteIslandFromCoords(
  ...
  if (completeIsland) {
    ...
    FogMap.init(fmb,
                CVector((float)completeIsland->XMin + deltaX,
                        (float)completeIsland->YMin + deltaY, 0.f),
                CVector((float)completeIsland->XMax - deltaX,
                        (float)completeIsland->YMax - deltaY, 0.f));
    bool found = false;
    for (uint k = 0; k < CFogMapBuild::NumMap && !found; ++k) {
      if (FogMap.getMap((CFogMap::TMapType)k).getWidth() != 0)
        found = true;
    }
    // as a fallback, assume that new fog maps have not been installed, so
    // fallback on the old ones ...
    if (!found) {
...
#ifdef NL_DEBUG
  nlinfo("fog map : (minX, minY) = (%.1f, %.1f), (maxX, maxY) = (%.1f, %.1f)",
         FogMap.getMinCoord().x, FogMap.getMinCoord().y, FogMap.getMaxCoord().x,
         FogMap.getMaxCoord().y);
#endif
}

//=========================================================================
bool CContinent::getCorners(NLMISC::CVector2f &cornerMin,
  ...
  if (cornerMin.x > cornerMax.x)
    std::swap(cornerMin.x, cornerMax.x);
  if (cornerMin.y > cornerMax.y)
    std::swap(cornerMin.y, cornerMax.y);
  cornerMax.x += 160.f;
  cornerMax.y += 160.f;
  return true;
}
...
void CContinent::initWaterMap() {
  ...
  if (!getCorners(cornerMin, cornerMax))
    return;
...
//=========================================================================
void CContinent::forceUpdateStreamable(const NLMISC::CVector &playerPos,
                                       NLMISC::IProgressCallback &progress) {
  _Villages.forceUpdate(playerPos, progress);
}

//=========================================================================
void CContinent::removeVillages() { _Villages.removeAll(); }
...
void CContinent::unselect() {
  ...
  // Remove the primitive for all entitites (new PACS coming soon and need new
  // primitives).
...
//=========================================================================
/** Override current fog by the fog in the weather system. This is used when
 * there is bad weather or precipitations \param dest the fog state to modify
 * \param fogIndex the fog to modify (0 for main, 1 for canopy)
 */
static void overrideFog(CFogState &dest, TFogType fogType, float dayNight,
                        float duskRatio,
  ...
  // if there's a new style sky with a fog bitmap *(giving fog depending on hour
  // & weather, use it)
  if (ContinentMngr.cur() &&
      ContinentMngr.cur()->CurrentSky.getScene() != NULL &&
      ContinentMngr.cur()->CurrentSky.hasFogColor()) {
    fogColor = ContinentMngr.cur()->CurrentSky.computeFogColor(
        CClientDate(RT.getRyzomDay(), (float)DayNightCycleHour),
    ...
    case CLightCycleManager::DayToNight: {
      if (dayNight < duskRatio) {
        ...
        fogColor.blendFromui(ws.FogColorDay, ws.FogColorDusk,
                             (uint)(blendFactor * 256.f));
      } else {
        float blendFactor =
            duskRatio != 1 ? (dayNight - duskRatio) / (1.f - duskRatio) : 0.f;
        fogColor.blendFromui(ws.FogColorDusk, ws.FogColorNight,
                             (uint)(blendFactor * 256.f));
      }
    } break;
    default: // no dusk transition
      fogColor.blendFromui(ws.FogColorDay, ws.FogColorNight,
                           (uint)(dayNight * 256.f));
      break;
    }
  }
  float finalFogAlpha = (float)dest.FogColor.A * 256.f / 255.f;
  dest.FogColor.blendFromui(dest.FogColor, fogColor,
                            (uint)((256.f - finalFogAlpha) * ws.FogRatio));
...
static inline void makeRangedFog(CFogState &result, UScene &scene) {
  if (result.FogEndDist != 0.f) {
    ...
    result.FogStartDist =
        result.FogStartDist * newEndFogDist / result.FogEndDist;
...
//=========================================================================
void CContinent::getFogState(TFogType fogType, float dayNight, float duskRatio,
                             CLightCycleManager::TLightState lightState,
  ...
  switch (fogType) {
  case MainFog:
    FogMap.getFogParams(FogStart, FogEnd, (float)pos.x, (float)pos.y, dayNight,
                        duskRatio, lightState, result);
    break;

  case CanopyFog:
    FogMap.getFogParams(RootFogStart, RootFogEnd, (float)pos.x, (float)pos.y,
                        dayNight, duskRatio, lightState, result);
    break;
  default:
    nlstop;
    break;
  ...
  overrideFog(result, fogType, dayNight, duskRatio, lightState,
  ...
  switch(fogType)
  {
          ...
          case CanopyFog: if (SceneRoot) makeRangedFog(result, *SceneRoot);
  break;
  }
  */
  /*
...
bool CContinent::enumIGs(IIGEnum *callback) {
  ...
  for (uint k = 0; k < numEntities && continueEnum; ++k) {
    ...
    if (v) // well this should always be the case, but it may change in the
           // future
    {
...
void CContinent::dumpVillagesLoadingZones(const std::string &filename) {
  ...
  for (uint k = 0; k < _Villages.getNumEntities(); ++k) {
    ...
    if (!v.isOutpost()) {
      ...
      drawDisc(outBitmap, (sig.getPos().x - minPos.x) / NUM_METER_PER_PIX,
               (sig.getPos().y - minPos.y) / NUM_METER_PER_PIX,
               sig.getUnloadRadius() / NUM_METER_PER_PIX, CRGBA(0, 0, 100),
               true);
      drawDisc(outBitmap, (sig.getPos().x - minPos.x) / NUM_METER_PER_PIX,
               (sig.getPos().y - minPos.y) / NUM_METER_PER_PIX,
               sig.getLoadRadius() / NUM_METER_PER_PIX, CRGBA(0, 100, 0), true);
      drawDisc(outBitmap, (sig.getPos().x - minPos.x) / NUM_METER_PER_PIX,
               (sig.getPos().y - minPos.y) / NUM_METER_PER_PIX,
               sig.getForceLoadRadius() / NUM_METER_PER_PIX, CRGBA(100, 0, 0),
               true);
    }
  }
  ...
  drawDisc(outBitmap,
           ((float)UserEntity->pos().x - minPos.x) / (float)NUM_METER_PER_PIX,
           ((float)UserEntity->pos().y - minPos.y) / (float)NUM_METER_PER_PIX,
           2, CRGBA::Magenta);
...
//=========================================================================
void CContinent::dumpFogMap(CFogMapBuild::TMapType mapType,
  ...
  drawDisc(outBitmap, mx * outBitmap.getWidth(), my * outBitmap.getHeight(),
           2.f, CRGBA::Magenta);
  COFile outFile;
  if (outFile.open(filename)) {
    try {
...
void CContinent::initSky() {
  if (!SkySheet[CurrSeason].empty()) {
    // get the sky sheet
    CEntitySheet *skySheet =
        SheetMngr.get(NLMISC::CSheetId(SkySheet[CurrSeason]));
    if (skySheet && skySheet->Type == CEntitySheet::SKY) {
      // new-style sky
      CurrentSky.init(Driver, *static_cast<CSkySheet *>(skySheet), false,
                      WorldLightCycle.NumHours);
    }
#ifdef USE_WATER_ENV_MAP
    ...
    Sky2ndPass =
        SkyScene->createInstance(SkyDay); // Sky shape used for second pass. We
                                          // use it to keep pointers on textures
