...
// ***************************************************************************
map<string, CStringManagerClient::CItem>
...
void CStringManagerClient::loadCache(uint32 timestamp, uint32 shardId) {
  ...
  if (_CacheInited) {
    try {
      ...
      _CacheFilename = string("save/") + clientApp + "_" + toString(shardId) +
                       "_" + m_LanguageCode + ".string_cache";
      ...
      while (!file.eof()) {
        ...
        // nldebug("SM : loading string [%6u] as [%s] in cache", id,
        // str.toString().c_str());
...
void CStringManagerClient::waitString(uint32 stringId,
                                      const IStringWaiterRemover *premover,
                                      string *result) {
...
void CStringManagerClient::waitDynString(uint32 stringId,
                                         const IStringWaiterRemover *premover,
                                         string *result) {
...
void CStringManagerClient::removeStringWaiter(
  ...
  {
  restartLoop1:
    TStringWaitersContainer::iterator first(_StringsWaiters.begin()),
        last(_StringsWaiters.end());
  ...
  {
  restartLoop2:
    TStringWaitersContainer::iterator first(_DynStringsWaiters.begin()),
        last(_DynStringsWaiters.end());
    for (; first != last; ++first) {
      if (first->second.Remover == remover) {
...
void CStringManagerClient::removeStringWaiter(
    const IStringWaitCallback *callback) {
  //		H_AUTO( CStringManagerClient_removeStringWaiter2 )

  // search in waiting string
  {
  restartLoop3:
    TStringCallbacksContainer::iterator first(_StringsCallbacks.begin()),
        last(_StringsCallbacks.end());
    for (; first != last; ++first) {
  ...
  {
  restartLoop4:
    TStringCallbacksContainer::iterator first(_DynStringsCallbacks.begin()),
        last(_DynStringsCallbacks.end());
    for (; first != last; ++first) {
      if (first->second == callback) {
  ...
  if (ClientCfg.Local) {
    ...
    if (it == _ReceivedStrings.end()) {
      ...
      if (it == _WaitingStrings.end()) {
        ...
        if (GenericMsgHeaderMngr.pushNameToStream(msgType, bms)) {
          bms.serial(stringId);
          NetMngr.push(bms);
          // nldebug("<CStringManagerClient::getString> sending
          // 'STRING_MANAGER:STRING_RQ' message to server");
        } else {
          nldebug("<CStringManagerClient::getString> unknown message name "
    ...
    if (ClientCfg.DebugStringManager) {
      ...
      if (result.size() > 9 && result.substr(0, 9) == "<missing:") {
        map<string, string>::iterator itds =
            _DynStrings.find(result.substr(9, result.size() - 10));
...
void CStringManagerClient::receiveString(uint32 stringId, const string &str) {
  ...
  if (_ReceivedStrings.find(stringId) != _ReceivedStrings.end()) {
    TStringsContainer::iterator it(_ReceivedStrings.find(stringId));
    nlwarning("Receiving stringID %u (%s), already in received string (%s), "
              "replacing with new value.",
              stringId, str.c_str(), it->second.c_str());
  ...
  {
    std::pair<TStringWaitersContainer::iterator,
              TStringWaitersContainer::iterator>
        range = _StringsWaiters.equal_range(stringId);
  ...
  // callback the waiter
  {
    std::pair<TStringCallbacksContainer::iterator,
              TStringCallbacksContainer::iterator>
        range = _StringsCallbacks.equal_range(stringId);
  ...
  restartLoop:
    ...
    for (; first != last; ++first) {
      ...
      /// Warning: if getDynString() return true, 'first' is erased => don't use
      /// it after in this loop
      if (getDynString(number, value)) {
        // nlinfo("DynString %u available : [%s]", number,
        ...
        //  update the waiting dyn strings
        {
          std::pair<TStringWaitersContainer::iterator,
                    TStringWaitersContainer::iterator>
              range = _DynStringsWaiters.equal_range(number);
        ...
        // callback the waiting dyn strings
        {
          std::pair<TStringCallbacksContainer::iterator,
                    TStringCallbacksContainer::iterator>
              range = _DynStringsCallbacks.equal_range(number);
...
void CStringManagerClient::receiveDynString(NLMISC::CBitMemStream &bms) {
  ...
  if (dynInfo.Status == TDynStringInfo::complete) {
    if (!ClientCfg.Light) {
      // nlinfo("DynString %u available : [%s]", dynId,
      // dynInfo.String.toString().c_str());
    }

    _ReceivedDynStrings.insert(std::make_pair(dynId, dynInfo));
    // security, if dynstring Message received twice, it is possible that the
    // dynstring is still in waiting list
    ...
    // update the waiting dyn strings
    {
      std::pair<TStringWaitersContainer::iterator,
                TStringWaitersContainer::iterator>
          range = _DynStringsWaiters.equal_range(dynId);
    ...
    // callback the waiting dyn strings
    {
      std::pair<TStringCallbacksContainer::iterator,
                TStringCallbacksContainer::iterator>
          range = _DynStringsCallbacks.equal_range(dynId);
...
bool CStringManagerClient::buildDynString(TDynStringInfo &dynInfo) {
  ...
  if (dynInfo.Status == TDynStringInfo::serialized) {
    ...
    std::vector<TParamValue>::iterator first(dynInfo.Params.begin()),
        last(dynInfo.Params.end());
    for (; first != last; ++first) {
      ...
      case string_id: {
        ...
        // If the string is a player name, we may have to remove the shard name
        // (if the string looks like a player name)
        if (!str.empty() &&
            !PlayerSelectedHomeShardNameWithParenthesis.empty()) {
          // fast pre-test
          if (str[str.size() - 1] == ')') {
            // the player name must be at least bigger than the string with ()
            if (str.size() >
                PlayerSelectedHomeShardNameWithParenthesis.size()) {
              // If the shard name is the same as the player home shard name,
              // remove it
              uint len =
                  (uint)PlayerSelectedHomeShardNameWithParenthesis.size();
              uint start = (uint)str.size() - len;
              if (ucstrnicmp(str, start, len,
      ...
      case time: {
        ...
        if (time >= (10 * 60 * 60)) {
          ...
          value = value + toString("%d ", nbMinutes) +
        ...
        value = value + toString("%d", nbSeconds) +
      ...
      case money:
        ///\todo nicoB/Boris : this is a temp patch that display money as
        ///integers
        {
          char value[1024];
        ...
        // TODO
        //					temp.append(move,
        //src+param.ReplacementPoint); 					move =
        //dynInfo.String.begin()+param.ReplacementPoint+2;
        break;
      case dyn_string_id: {
        string dynStr;
...
bool CStringManagerClient::getDynString(uint32 dynStringId,
                                        std::string &result) {
  ...
  if (ClientCfg.Local) {
    ...
    if (it != _ReceivedDynStrings.end()) {
      ...
      if (it != _WaitingDynStrings.end()) {
        nlwarning("CStringManager::getDynString : the string %u is received "
                  "but still in _WaintingDynStrings !",
                  dynStringId);
...
void CLoadProxy::loadStringFile(const string &filename,
                                ucstring &text) // TODO: UTF-8 (serial)
{
...
void CLoadProxy::onEquivalent(uint addIndex, uint /* refIndex */,
                              TStringDiffContext &context) {
  context.Diff.push_back(context.Addition[addIndex]);
}
void CLoadProxy::onAdd(uint addIndex, uint /* refIndex */,
                       TStringDiffContext &context) {
  context.Diff.push_back(context.Addition[addIndex]);
  // nldebug("Adding new string '%s' in CI18N",
  // context.Addition[addIndex].Identifier.c_str());
  if (ClientCfg.DebugStringManager)
    context.Diff.back().Text =
        ucstring("<NEW>") + context.Diff.back().Text; // TODO: UTF-8 (serial)
}
void CLoadProxy::onRemove(uint /* addIndex */, uint /* refIndex */,
                          TStringDiffContext & /* context */) {
  // nothing to do because we don't insert bad value
}
void CLoadProxy::onChanged(uint addIndex, uint /* refIndex */,
                           TStringDiffContext &context) {
  // we use the addition value in this case
  context.Diff.push_back(context.Addition[addIndex]);
  // nldebug("Using changed string '%s' in CI18N",
  // context.Addition[addIndex].Identifier.c_str());
  if (ClientCfg.DebugStringManager)
    context.Diff.back().Text =
        ucstring("<CHG>") + context.Diff.back().Text; // TODO: UTF-8 (serial)
}
void CLoadProxy::onSwap(uint /* newIndex */, uint /* refIndex */,
                        TStringDiffContext & /* context */) {
  // don't swap.
}
...
// ***************************************************************************
/*
        The readed for skill_word_en.txt for example. Do all the job of Diffs
   with work and translated dirs.
*/
class CReadWorkSheetFile : public TWorkSheetDiff::IDiffCallback {
public:
  void readWorkSheetFile(const string &filename,
                         ucstring &text) // TODO: UTF-8 (serial)
  {
    TWorksheet addition;
    ...
    if (reference.ColCount != addition.ColCount) {
      nlwarning("Can't check for difference for file %s, column number is not "
                "the same!",
                filename.c_str());
      doDiff = false;
    }
    if (doDiff) {
      ...
      for (i = 0; i < addition.ColCount; ++i) {
        if (addition.getData(0, i) != reference.getData(0, i)) {
          nlwarning("Can't check difference for file %s, collumn name differ !",
                    filename.c_str());
          doDiff = false;
          break;
        }
  ...
  void onEquivalent(uint /* addIndex */, uint refIndex,
                    TWordsDiffContext &context) {
    context.Diff.push_back(context.Reference[refIndex]);
  }
  void onAdd(uint addIndex, uint /* refIndex */, TWordsDiffContext &context) {
    context.Diff.push_back(context.Addition[addIndex]);
    nlinfo("Using newly sheet row %s",
           context.Diff.getData(context.Diff.size() - 1, 1).toString().c_str());
  }
  void onRemove(uint /* addIndex */, uint /* refIndex */,
                TWordsDiffContext & /* context */) {
    // nothing to do because we don't insert bad value
  }
  void onChanged(uint addIndex, uint /* refIndex */,
                 TWordsDiffContext &context) {
    context.Diff.push_back(context.Addition[addIndex]);
    nlinfo("Using changed sheet row %s",
           context.Diff.getData(context.Diff.size() - 1, 1).toString().c_str());
  }
  void onSwap(uint /* newIndex */, uint /* refIndex */,
              TWordsDiffContext & /* context */) {
    // don't swap.
  }
};
...
// Must Increment this number if change are made to the code (else change not
// taken into account)
const uint StringClientPackedVersion = 0;
bool CStringManagerClient::checkWordFileDates(
    vector<CFileCheck> &fileChecks, const std::vector<string> &fileNames,
  ...
  for (uint i = 0; i < fileChecks.size(); i++) {
    ...
    fileChecks[i].ReferenceDate =
        referenceFile.empty() ? 0
                              : CFile::getFileModificationDate(referenceFile);

    // get then one of the working File (NB: 0 is a valid reponse for Final
    // Client: no working file)
    string workingFile("./translation/work/" +
                       CFile::getFilename(fileNames[i]));
    fileChecks[i].AdditionDate =
        CPath::exists(workingFile) ? CFile::getFileModificationDate(workingFile)
                                   : 0;
  }
...
void CStringManagerClient::initI18NSpecialWords(const string &languageCode) {
  ...
  static const char *specialWords[] = {
      ...
      // 3rd is the extenstion to add to the key for correct matchup (sheet
      // cases)
      "skill",
      "skill ID",
      "",
  ...
  for (uint i = 0; i < numSpecialWords; i++) {
    fileNames[i] =
        string(specialWords[i * 3 + 0]) + "_words_" + languageCode + ".txt";
  }
  ...
  if (mustRebuild) {
    for (uint i = 0; i < numSpecialWords; i++) {
      ...
      if (!ws.findCol(ucstring("name"), nameColIndex)) // TODO: UTF-8 (serial)
        continue;
      if (!ws.findCol(ucstring(specialWords[i * 3 + 1]),
                      keyColIndex)) // TODO: UTF-8 (serial)
        continue;
      ...
      for (uint j = 1; j < ws.size(); j++) {
        ...
        string key = ws.getData(j, keyColIndex)
                         .toUtf8(); // FIXME: const string & when UTF-8
        string name = ws.getData(j, nameColIndex)
                          .toUtf8(); // FIXME: const string & when UTF-8
        // Append to the I18N.
        // avoid case problems
        ...
        if (it != _SpecItem_TempMap.end()) {
          nlwarning("Error in initI18NSpecialWords(), %s already exist (not "
                    "replaced)!",
                    keyStr.c_str());
        } else {
          ...
          if (womenNameColIndex != std::numeric_limits<uint>::max()) {
            const ucstring &womenName =
                ws.getData(j, womenNameColIndex); // TODO: UTF-8 (serial)
          ...
          if (descColIndex != std::numeric_limits<uint>::max()) {
            const ucstring &desc =
                ws.getData(j, descColIndex); // TODO: UTF-8 (serial)
          ...
          if (descColIndex2 != std::numeric_limits<uint>::max()) {
            const ucstring &desc =
                ws.getData(j, descColIndex2); // TODO: UTF-8 (serial)
      ...
      nlinfo("%d seconds for I18N words: Total: %s",
             (uint32)(ryzomGetLocalTime() - profile0 + 500) / 1000,
             fileName.c_str());
    }
  } else {
    CIFile packFile;
...
void CStringManagerClient::specialWordsMemoryCompress() {
  ...
  // No need to sort the vector because the map was already sorted in the good
  // order
...
// ***************************************************************************
const char *CStringManagerClient::getSpecialWord(const string &label,
                                                 bool women) {
  if (label.empty()) {
  ...
  if (_SpecItem_MemoryCompressed) {
    ...
    vector<CItemLight>::iterator it = lower_bound(
        _SpecItems.begin(), _SpecItems.end(), tmp, CItemLightComp());
...
// ***************************************************************************
/*const ucchar
*CStringManagerClient::getBrickLocalizedName(BRICK_FAMILIES::TBrickFamily e)
{
...
// ***************************************************************************
const char *
CStringManagerClient::getPlaceLocalizedName(const string &placeNameID) {
  return getSpecialWord(placeNameID);
}

// ***************************************************************************
const char *
CStringManagerClient::getFactionLocalizedName(const string &factionNameID) {
...
// ***************************************************************************
const char *
CStringManagerClient::getCreatureLocalizedName(NLMISC::CSheetId id) {
  return getSpecialWord(id.toString());
}
...
// ***************************************************************************
/*const char
*CStringManagerClient::getBrickLocalizedDescription(BRICK_FAMILIES::TBrickFamily
e)
{
...
// ***************************************************************************
const char *
CStringManagerClient::getSkillLocalizedDescription(SKILLS::ESkills e) {
...
// ***************************************************************************
const char *
CStringManagerClient::getSBrickLocalizedDescription(NLMISC::CSheetId id) {
  return getSpecialDesc(id.toString());
}

// ***************************************************************************
const char *CStringManagerClient::getSBrickLocalizedCompositionDescription(
    NLMISC::CSheetId id) {
...
// ***************************************************************************
const char *
CStringManagerClient::getSPhraseLocalizedDescription(NLMISC::CSheetId id) {
  return getSpecialDesc(id.toString());
}

// ***************************************************************************
const char *CStringManagerClient::getTitleLocalizedName(const string &titleId,
                                                        bool women) {
...
// ***************************************************************************
vector<string> CStringManagerClient::getTitleInfos(const string &titleId,
                                                   bool women) {
...
// ***************************************************************************
const char *CStringManagerClient::getClassificationTypeLocalizedName(
...
// ***************************************************************************
const char *
CStringManagerClient::getOutpostLocalizedDescription(NLMISC::CSheetId id) {
  return getSpecialDesc(id.toString());
}

// ***************************************************************************
const char *
CStringManagerClient::getOutpostBuildingLocalizedName(NLMISC::CSheetId id) {
  return getSpecialWord(id.toString());
}

// ***************************************************************************
const char *CStringManagerClient::getOutpostBuildingLocalizedDescription(
    NLMISC::CSheetId id) {
  return getSpecialDesc(id.toString());
}
...
// ***************************************************************************
const char *
CStringManagerClient::getSquadLocalizedDescription(NLMISC::CSheetId id) {
  return getSpecialDesc(id.toString());
}

// ***************************************************************************
void CStringManagerClient::replaceDynString(const std::string &name,
                                            const std::string &text) {
  _DynStrings[name] = text;
}

// ***************************************************************************
void CStringManagerClient::replaceSBrickName(NLMISC::CSheetId id,
