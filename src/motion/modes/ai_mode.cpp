...
//-----------------------------------------------
void CUserControls::aiModeStop() {} // aiModeStop //
...
void CUserControls::aiMode() {
  ...
  if (EventsListener.isMouseButtonReleased(rightButton)) {
    ...
    if ((T1 - _RightClickStart) <= _TimeLongClick) {
      ...
      // Launch Context Menu
      if (!R2::isEditionCurrent()) // context menu managed in a different
                                   // fashion for R2ED
      {
  ...
  if (Actions.valide("camera_down"))
    accel -= ClientCfg.FlyAccel;
  updateVelocity(DT, accel, ClientCfg.FlyAccel, ClientCfg.Fly,
  ...
  View.viewPos(View.viewPos() + camMatrix.getI() * (_FlyLateralVelocity * DT) +
               camMatrix.getJ() * (_FlyFrontVelocity * DT) +
               camMatrix.getK() * (_FlyVerticalVelocity * DT));
} // aiMode //
