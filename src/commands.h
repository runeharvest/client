...
/**
 * User command class. This NeL command is binded to an action handler with the
 * commands.xml file.
 */
...
public:
  ...
  CUserCommand(const std::string &commandName, const std::string &help,
               const std::string &argsHelp);

  void addMode(const std::string &action, uint numArg, bool infiniteAgr,
               const std::vector<std::string> &keywords);

  // From ICommand
  virtual bool execute(const std::string &rawCommandString,
                       const std::vector<std::string> &args, NLMISC::CLog &log,
                       bool quiet, bool human);

  // Create a command
  static void createCommand(const char *name, const char *action,
                            const char *params);
