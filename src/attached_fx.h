...
/** An fx linked to a character using a stick mode (replacement to former class
 * CCharacterCL::CAnimFX) \TODO nico The template sheet is named CAnimationFX.
 * Maybe CAttachedFXTemplate would be a better name... \author Nicolas Vizerie
...
public:
  ...
  public:
    ...
    const CFXStickMode
        *StickMode; // NULL to use default stick mode of the sheet
    NLMISC::CVector StickOffset;
    const NLMISC::CMatrix
        *StaticMatrix;    // Useful if stick mode is "StaticMatrix"
    uint MaxNumAnimCount; // Number of frame on which the fx can  overlap when
  ...
  void create(CCharacterCL &parent, const CBuildInfo &buildInfo,
              const CTargeterInfo &targeterInfo);
  // Creation from an externally created FX (sheet is still used to specify
  // track)
  void create(CCharacterCL &parent, NL3D::UParticleSystemInstance instance,
              const CBuildInfo &buildInfo, const CTargeterInfo &targeterInfo);
  // reset that object
  void clear();
...
public:
  ...
  NLMISC::CVector SpawnPos; // spawn pos is in world in 'spawn' mode, or local
                            // for 'sticked' and 'follow' modes
  double TimeOutDate; // timeOut date for FXs to be removed. If the fx is in the
                      // active list, then the date is relative and is added to
                      // current date when the fx is shutdown
  uint MaxAnimCount;  // max number of anim changes during which the fx is still
                      // visible while being shutdown (0 if no limit)
  //
  CFXStickMode::TStickMode
      StickMode; // The stick mode for that fx. If NULL, the default stickmode
                 // of the .animation_fx sheet is used instead
