...
void CProgress::setFontFactor(float temp) { _FontFactor = temp; }

void CProgress::newMessage(const string &message) {
  popCropedValues();
  _CurrentRootStep++;
  pushCropedValues((float)_CurrentRootStep / (float)_RootStepCount,
                   (float)(_CurrentRootStep + 1) / (float)_RootStepCount);
...
void CProgress::internalProgress(float value) {
  ...
  bool stereoHMD = StereoHMD && !MainCam.empty() &&
                   (MainCam.getTransformMode() == UCamera::RotQuat);
  CVector oldPos;
  CQuat oldQuat;
  if (stereoHMD) {
  ...
  while ((!stereoHMD && i == 0) || (stereoHMD && StereoDisplay->nextPass())) {
    ...
    if (stereoHMD) {
      ...
      nldebug("Cam pos: %f, %f, %f", MainCam.getPos().x, MainCam.getPos().y,
              MainCam.getPos().z);
    }
    ...
    if (!stereoHMD || StereoDisplay->wantInterface2D()) {
      ...
      for (uint i = 0; i < ClientCfg.Logos.size(); i++) {
        ...
        if (res.size() == 9 && i < LogoBitmaps.size() &&
          ...
          Driver->drawBitmap(x / (float)ClientCfg.Width,
                             y / (float)ClientCfg.Height,
      ...
      if (TextContext != NULL) {
        ...
        if (!TipsOfTheDay.empty()) {
          while (index < end) {
            ...
            fY = nextLine(TextContext->getFontSize(), Driver->getWindowHeight(),
                          fY);
        ...
        if (!_TPCancelText.empty()) {
          if (ClientCfg.Width != 0 && ClientCfg.Height != 0) {
            ...
            string uc = CI18N::get("uiR2EDTPEscapeToInteruptLoading") + " (" +
                        _TPCancelText + ") - " +
        ...
        if (!ApplyTextCommands && LoadingContinent &&
            !LoadingContinent->Indoor) {
          TextContext->setFontSize((uint)(13.f * fontFactor));

          // Print some more info
          uint32 day = RT.getRyzomDay();
          str = toString(
              CI18N::get("uiTipsTeleport").c_str(),
              CI18N::get(LoadingContinent->LocalizedName).c_str(), day,
              (uint)RT.getRyzomTime(),
              CI18N::get("uiSeason" +
                         toStringEnum(CRyzomTime::getSeasonByDay(day)))
                  .c_str(),
        ...
        if (ApplyTextCommands) {
          std::vector<CClientConfig::SPrintfCommand> printfCommands =
              ClientCfg.PrintfCommands;
          if (FreeTrial)
            printfCommands = ClientCfg.PrintfCommandsFreeTrial;

          if (!printfCommands.empty()) {
            TextContext->setHotSpot(UTextContext::MiddleBottom);

            vector<CClientConfig::SPrintfCommand>::iterator itpc;
            for (itpc = printfCommands.begin(); itpc != printfCommands.end();
                 ++itpc) {
              ...
              uint fontHeight =
                  (uint)si.StringHeight + 2; // we add 2 pixels for the gap
  ...
  if ((UseEscapeDuringLoading &&
       Driver->AsyncListener.isKeyPushed(KeyESCAPE)) ||
      !activeDriver) {
  ...
  if (!_TPCancelText.empty() && Driver->AsyncListener.isKeyPushed(KeySHIFT) &&
      Driver->AsyncListener.isKeyPushed(KeyESCAPE)) {
    _TPCancelFlag = true;
  }
...
void CProgress::setTPMessages(const string &tpReason,
                              const string &tpCancelText,
