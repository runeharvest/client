...
static CAnimationFXSetSheet *
getAnimFXSetSheetFromName(const std::string &name) {
  CEntitySheet *sheet = SheetMngr.get(NLMISC::CSheetId(name));
  if (!sheet)
    return NULL;
  if (sheet->Type == CEntitySheet::ANIMATION_FX_SET)
    return static_cast<CAnimationFXSetSheet *>(sheet);
  return NULL;
}

// build an attack part
static void buildAttackPart(const std::string &sheetName,
                            CAnimationFXSet &fxSet, NL3D::UAnimationSet *as) {
  if (!sheetName.empty()) {
...
void CAttack::init(const CAttackSheet *sheet, NL3D::UAnimationSet *as) {
  ...
  buildAttackPart(Sheet->AttackStaticObjectCastFX, AttackStaticObjectCastFX,
                  as);
...
// ***********************************************************************************************
void CAttackList::init(const CAttackListSheet *attackList,
                       NL3D::UAnimationSet *as) {
...
{
        bool operator()(const CAttackListEntry &lhs, const CAttackIDSheet &rhs)
const
        {
                nlassert(lhs.ID);
                return *lhs.ID < rhs;
...
struct CAttackEntryComp2 {
  bool operator()(const CAttackListEntry &lhs,
                  const CAttackListEntry &rhs) const {
    nlassert(lhs.ID);
    nlassert(rhs.ID);
...
const CAttack *CAttackList::getAttackFromID(const CAttackIDSheet &id) const {
  ...
  //	std::vector<CAttackListEntry>::const_iterator it =
  //std::lower_bound(_Attacks.begin(), _Attacks.end(), id, CAttackEntryComp());

  CAttackListEntry ale(&id);
  std::vector<CAttackListEntry>::const_iterator it = std::lower_bound(
      _Attacks.begin(), _Attacks.end(), ale, CAttackEntryComp2());
  if (it == _Attacks.end())
    return NULL;
  if (*(it->ID) != id)
    return NULL;
...
void CAttackListManager::init() {
  ...
  for (uint k = 0; k < result.size(); ++k) {
    const CAttackListSheet *sheet =
        dynamic_cast<const CAttackListSheet *>(SheetMngr.get(result[k]));
    if (sheet) {
...
// ***********************************************************************************************
const CAttackList *
CAttackListManager::getAttackList(const std::string &name) const {
  H_AUTO_USE(RZ_AttackList)
...
// ***********************************************************************************************
CAttackListManager::CAttackListManager() { _AnimationSet = NULL; }

// *******************************************************************************************
void CAttackListManager::buildAurasFXs() {
  _Auras.init("auras.id_to_string_array", _AnimationSet,
              false /* must not delete animset, owned by this object */);
}

// *******************************************************************************************
void CAttackListManager::buildLinkFXs() {
  _Links.init("links.id_to_string_array", _AnimationSet,
              false /* must not delete animset, owned by this object */);
}
