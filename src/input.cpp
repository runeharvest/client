...
// *********************************************************************************
uint GetMouseButtonsState() { return DownMouseButtons; }
...
bool IsMouseCursorHardware() { return MouseHardware; }
...
// *********************************************************************************
bool IsMouseFreeLook() { return MouseFreeLook; }
  ...
  if (MouseFreeLook) {
    ...
    if (instance) {
      ...
      if (cursor) {
        ...
        if (ix != CViewPointer::InvalidCoord &&
            iy != CViewPointer::InvalidCoord) {
...
void SetMouseSpeed(float speed) { MouseCursorSpeed = speed; }

// *********************************************************************************
// Use this method to set the cursor acceleration
void SetMouseAcceleration(uint accel) { MouseCursorAcceleration = accel; }

// *********************************************************************************
void HandleSystemCursorCapture(const CEvent &event) {
  static bool mouseCaptured = false;

  // capture on first move event after button is held down or free look is
  // activated
  if (event == EventMouseMoveId && !mouseCaptured &&
      (MouseFreeLook || DownMouseButtons != 0)) {
    mouseCaptured = true;
  ...
  if (event == EventMouseUpId) {
    ...
    CEventMouseDown &em = (CEventMouseDown &)event;
    DownMouseButtons &=
        ~(em.Button & (leftButton | middleButton | rightButton));
