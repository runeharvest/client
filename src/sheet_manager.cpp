...
// there can be several instance of CSheetManager (for reload operations) ->
// ctruct the loader here rather than in CSheetManager ctor
...
public:
  ...
  CTypeVersion(std::string type, uint version) {
    Type = type;
    Version = version;
  }
};

// if you change these values please rebuild the packed_sheets with an updated
// sheets_packer binary.
//   This is the only way to have correct version in both client and
//   packed_sheets
...
// ***************************************************************************
void CSheetManagerEntry::readGeorges(
  ...
  if (form) {
    ...
    std::string extension =
        NLMISC::CSheetId::fileExtensionFromType(sheetId.getSheetType());
    ...
    if (extension == "creature")
      ...
      nlwarning("CSheetManager::loadSheet: Do not know how to create the class "
                "from the sheet '%s'.",
  ...
  // Error while loading the form.
    ...
    //		nlwarning("CSheetManager::loadSheet: Cannot load the form
    //'%s'.", filename.c_str());
    EntitySheet = 0;
  }
}
...
// ***************************************************************************
void CSheetManagerEntry::initSheet(CEntitySheet *pES, NLMISC::IStream &s,
...
void CSheetManagerEntry::serial(NLMISC::IStream &s) {
  if (s.isReading()) {
    ...
    default:
      nlwarning("CSheetManager::load: Unknown type '%d' in the packed file. "
                "Rebuild=true and Ignore this sheet.",
                type);
      EntitySheet = NULL;
      break;
    }
...
// ***************************************************************************
uint CSheetManagerEntry::getVersion() { return Version; }
...
// Destructor.
//-----------------------------------------------
CSheetManager::~CSheetManager() { release(); }
...
// Load all sheets.
//-----------------------------------------------
void CSheetManager::load(NLMISC::IProgressCallback &callBack,
                         bool updatePackedSheet, bool needComputeVS,
                         bool dumpVSIndex) {
  // Open The Item Association File
  //	if(!fItemAssoc.open(getLogDirectory() + "item_association.dbg", false,
  //true)) 		nlwarning("CSheetManager::load: Cannot Open the
  //'item_association.txt'."); 	else 		ItemAssocFileOpen = true;
...
// Load all sheets.
//-----------------------------------------------
void CSheetManager::loadAllSheet(
    NLMISC::IProgressCallback &callBack, bool updatePackedSheet,
    bool needComputeVS, bool dumpVSIndex, bool forceRecompute /*= false*/,
    const std::vector<std::string> *userExtensions /*= NULL*/) {
    ...
    for (uint i = 0; i < nb; ++i) {
      ...
      if (userExtensions) {
        for (uint l = 0; l < userExtensions->size(); ++l) {
          if (stricmp((*userExtensions)[l].c_str(),
                      TypeVersion[i].Type.c_str()) == 0) {
            found = true;
          }
        }
      } else {
      ...
      if (found) {
        ...
        string path =
            CPath::lookup(TypeVersion[i].Type + ".packed_sheets", false);
        ...
        if (path.empty())
          path = CPath::standardizePath(_OutputDataPath) + TypeVersion[i].Type +
                 ".packed_sheets";
  ...
  {
    for (uint i = 0; i < SLOTTYPE::NB_SLOT; ++i)
      _VisualSlots[i].resize(
          CVisualSlotManager::getInstance()->getNbIndex(
              (SLOTTYPE::EVisualSlot)i) +
              1,
          0); // Nb Index +1 because index 0 is reserve for empty.
    ...
    while (it != _EntitySheetContainer.end()) {
      ...
      for (uint i = 0; i < result.size(); ++i) {
        if (dynamic_cast<CItemSheet *>((*it).second.EntitySheet)) {
          _SheetToVS[dynamic_cast<CItemSheet *>((*it).second.EntitySheet)]
              .push_back(std::make_pair(result[i].VisualSlot, result[i].Index));
          _VisualSlots[result[i].VisualSlot][result[i].Index] =
              dynamic_cast<CItemSheet *>((*it).second.EntitySheet);
        }
      }
...
// ***************************************************************************
void CSheetManager::loadAllSheetNoPackedSheet(
    NLMISC::IProgressCallback &callBack,
    const std::vector<std::string> &extensions,
    const std::string &wildcardFilter) {
...
void CSheetManager::computeVS() {
  ...
  while (itS != _EntitySheetContainer.end()) {
    ...
    if (item && (*itS).first.getSheetType() ==
                    CSheetId::typeFromFileExtension(std::string("sitem"))) {
      for (uint j = 0; j < SLOTTYPE::NB_SLOT_TYPE; ++j) {
        ...
        if (item->hasSlot(slotType)) {
          ...
          if (visualSlot != SLOTTYPE::HIDDEN_SLOT) {
            ...
            string sheetName =
                toString("%s%d", currentSheet.c_str(), visualSlot);

            // Is the sheet already process (could be process by a sheet with a
            // different lvl)
            ...
            if (it == ProcessedItem.end()) {
              ...
              if (vs[visualSlot].Element.empty())
                ...
                itemNumber = vs[visualSlot]
                                 .Element[vs[visualSlot].Element.size() - 1]
                                 .Index +
                             1;
...
//=======================================================================
const CSheetManager::TVisualSlotItemArray *
CSheetManager::getVSItems(CItemSheet *sheet) const {
...
//=======================================================================
sint CSheetManager::getVSIndex(const std::string &itemName,
                               SLOTTYPE::EVisualSlot slot) {
  NLMISC::CSheetId si;
  if (!si.buildSheetId(itemName)) {
    nlwarning("<CSheetManager::getVSIndex> : cannot build id from item %s for "
              "the slot %d.",
              itemName.c_str(), slot);
    return -1;
  }
  ...
  if (it == _EntitySheetContainer.end()) {
    nlwarning("<CSheetManager::getVSIndex> : cannot find %s for the slot %d.",
              itemName.c_str(), slot);
    return -1;
  }
  if (it->second.EntitySheet == 0 ||
      it->second.EntitySheet->type() != CEntitySheet::ITEM) {
    nlwarning(
        "<CSheetManager::getVSIndex> : %s is not an item for the slot %d.",
        itemName.c_str(), slot);
    return -1;
  }
  ...
  if (ia == NULL) {
    nlwarning("<CSheetManager::getVSIndex> : no items for the slot %d. while "
              "looking for %s",
              slot, itemName.c_str());
    return -1;
  }
...
// \return CEntitySheet * : pointer on the sheet according to the param or 0 if
// any pb.
//-----------------------------------------------
...
CItemSheet *CSheetManager::getItem(SLOTTYPE::EVisualSlot slot, uint index) {
  ...
  if (s < _VisualSlots.size()) {
    ...
    // Bad index.
      ...
      // nlwarning("CSheetManager::getItem : invalid index %d for the slot %d.",
      // index, slot);
      return NULL;
    }
  }
...
void CSheetManager::loadTyp() {
  ...
  NLMISC::CSmartPtr<NLGEORGES::UType> smartPtr =
      FormLoader->loadFormType("_creature_3d_eyes_color.typ");
  if (smartPtr) {
    string maxStr = smartPtr->getMax();
    fromString(maxStr, _NbEyesColor);
    if (_NbEyesColor <= 0)
      nlwarning(
          "CSheetManager::loadTyp: There no is Color available for the eyes.");
  } else
    nlwarning("CSheetManager::loadTyp: Cannot load the "
              "'_creature_3d_eyes_color.typ' file.");
  ...
  if (smartPtr) {
    ...
    if (_NbHairColor <= 0)
      nlwarning(
          "CSheetManager::loadTyp: There is no Color available for the hair.");
  } else
    nlwarning("CSheetManager::loadTyp: Cannot load the "
              "'_creature_3d_hair_color.typ' file.");
} // initTyp //
...
void CSheetManager::dumpVisualSlots() {
  for (uint k = 0; k < _VisualSlots.size(); ++k) {
    ...
    for (uint l = 0; l < iv.size(); ++l) {
      if (iv[l]) {
        nlinfo("Slot %d, item %d = %s", (int)k, (int)l,
               iv[l]->Id.toString().c_str());
      }
    }
  }
}
