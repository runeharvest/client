...
private:
   ...
   *	A Root animation is an animation that is not referenced in the "Graph of
   *Next Anim"
   */
  ...
  void recursMarkTraverseNext(sint idAnim, std::vector<bool> &traversedAnims,
                              bool rootCall);

  // Build list of possible animations, according to job specialisation and race
  void buildAnimFilter(std::vector<uint> &filteredRootAnimList,
                       std::vector<bool> &animFilterStates,
                       uint32 jobSpecialisation, EGSPD::CPeople::TPeople race,
  ...
  uint
  chooseAnimationIndex(const std::vector<uint> &filteredRootAnimList) const;

  /** Backward compatibility. FallBack to old animations if
   *EnableRacialAnimation==false The fallbakc depends on the StateId (run,
   *sit...) and on sex of player: 0 male 1 female
   */
  static EGSPD::CPeople::TPeople
      _FallBackToDefaultRace[2][CAnimationStateSheet::StaticStateCount];
  static bool _FallBackToDefaultRaceInited;
  static void initFallBackToDefaultRace();
  bool isOldRaceAnimationForced(EGSPD::CPeople::TPeople race,
                                GSGENDER::EGender gender) const;
...
public:
  ...
  TAnimStateId state() const { return (TAnimStateId)_Sheet->State; }

  /*
   * Choose an animation in the list.
   * \return TAnimId : Id of the animation.
   * \param jobSpecialisation. 0 if none, else an index of a job specialisation
   * of the character (eg: CraftRolemaster) \param race. unknown => all selected
   * \param gender. Special Hack for EnableRacialAnimation==false....
   */
  CAnimation::TAnimId chooseAnim(
      uint32 jobSpecialisation, EGSPD::CPeople::TPeople race,
      GSGENDER::EGender gender, double angToDest = 0.0,
      CAnimation::TAnimId currentAnimIndex = CAnimation::UnknownAnim) const;
  ...
  const CAnimation *getAnimation(CAnimation::TAnimId animId) const {
    if ((uint)animId >= _Animations.size()) {
      nlwarning("CAnimationState:getAnimation: animId '%d' is invalid, state "
                "animation count = %d",
                animId, _Animations.size());
      if (_Sheet) {
        nlinfo("CAnimationState:getAnimation: animation sheet count = %d, "
               "State = %d, display objects : %d",
               _Sheet->Animations.size(), _Sheet->State,
               _Sheet->DisplayObjects ? 1 : 0);
      }
#if !FINAL_VERSION
      nlstop;
#endif // !FINAL_VERSION
  ...
  void check(CAnimation::TAnimId animId) const {
    if (animId != CAnimation::UnknownAnim)
      nlassertex(((uint)animId < _Animations.size()),
                 ("CAnimationState:check: animId '%d' is invalid.", animId));
  }
  //@}
