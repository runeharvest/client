...
public:
  ...
  class CColorInfo {
  public:
    NLMISC::CBitmap *Map; // color computed from a map depending on hour &
                          // weather (NULL if color is unseted)
    TSkyColorMode Mode;   // how the color is to be used
  public:
    CColorInfo() : Map(NULL) {}
    /** Init color map from its name. Eventually load the bitmap if itsn't found
     * in the map \param bitmapByName already build bitmap, sorted by their name
     * \param buildBitmap list of used bitmap (to be completed if required
     * bitmap id not in "bitmapByName")
     */
    ...
    NLMISC::CRGBA computeColor(float dayPart, float weatherLevel,
  ...
  public:
    sint32 TargetTextureStage; // the texture stage to which the gradient must
                               // be applied.
    /** each bitmap in the following list  gives the gradient depending on light
     * level. The V coordinate gives the gradient values, end U gives the light
     * level each bitmap match a weather state. First bitmap maps to weather
     * value = 0 and last bitmap maps to weather value = 1 for intermediary
     * weather values, the two nearest bitmap are blended to get the value of
     * the gradient
     */
  ...
  public:
    ...
    /** Init from a sheet.
     * Init color gradient from its bitmap names. Eventually load the bitmap if
     * itsn't found in the map \param bitmapByName already build bitmap, sorted
     * by their name \param buildBitmap list of used bitmap (to be completed if
     * required bitmaps are not in "bitmapByName")
     */
    void init(const CSkyObjectSheet::CColorGradientInfoSheet &cgi,
              std::map<std::string, NLMISC::CBitmap *> &bitmapByName,
              std::vector<NLMISC::CBitmap *> &buildBitmap);
    void setup(NL3D::UInstance instance, float dayPart, float weatherLevel,
  ...
  NL3D::UParticleSystemInstance
      PS; // If the object is also a particle system, keep a pointer on it.
  //
  CColorInfo DiffuseColor; // map for diffuse color (or fx color), depending on
                           // light level and weather
  CColorInfo
      ParticleEmitters; // map whose R channel control emitters activation
  CColorInfo
      ConstantColor[SKY_MAX_NUM_STAGE]; // map for constant color at each stage
  CColorGradientInfo ColorGradient;
  TSkyRefColor RefColor; // tells which color info is used to test if object is
                         // visible (when alpha is 0, the object must be hidden)
  // cache last colors
...
public:
  // ctor
  CSkyObject()
      : Instance(NULL), PS(NULL), Active(false), VisibleInMainScene(true),
   ...
   * \param buildBitmap list of used bitmap (to be completed if required bitmap
   * id not in "bitmapByName")
   */
   ...
   */
  bool setup(const CClientDate &date, const CClientDate &animationDate,
             float numHoursInDay, float weatherLevel, NLMISC::CRGBA fogColor,
             bool envMapScene);
};
