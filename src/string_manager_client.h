...
public:
   ...
   *	There is one cache file for each language and for each encountered
   *shard.
   */
  ...
  void waitString(uint32 stringId, const IStringWaiterRemover *premover,
                  std::string *result);
  void waitString(uint32 stringId, IStringWaitCallback *pcallback);
  bool getDynString(uint32 dynStringId, std::string &result);
  void waitDynString(uint32 stringId, const IStringWaiterRemover *premover,
                     std::string *result);
  ...
  // Yoyo: Replace the Brick Name with Filled stats (CSBrickManager work). No-Op
  // if not found
  static void replaceSBrickName(NLMISC::CSheetId id, const std::string &name,
                                const std::string &desc,
                                const std::string &desc2);
  static void replaceDynString(const std::string &name,
  ...
  static const char *
  getSBrickLocalizedCompositionDescription(NLMISC::CSheetId id);
  ...
  static const char *getTitleLocalizedName(const std::string &titleId,
                                           bool women);
  static std::vector<std::string> getTitleInfos(const std::string &titleId,
                                                bool women);

  // Get the Localized name of a classification type
  static const char *getClassificationTypeLocalizedName(
  ...
  static const char *
  getOutpostBuildingLocalizedDescription(NLMISC::CSheetId id);
  // Squad name
...
private:
  ...
  /// A string wait callback  is destoyed, remove any waiting string reference
  /// from it.
  void removeStringWaiter(const IStringWaitCallback *callback);

  enum TParamType { string_id, integer, time, money, dyn_string_id, sheet_id };
  ...
  struct TDynStringInfo {
    enum TStatus { received, serialized, complete };
    TStatus Status;
  ...
  enum { EmptyStringId = 0, EmptyDynStringId = 0xffffffff };
  ...
  static const char *getSpecialWord(const std::string &label,
                                    bool women = false);
  ...
  static bool checkWordFileDates(std::vector<CFileCheck> &fileChecks,
...
public:
  ...
  virtual void onStringAvailable(uint /* stringId */,
 ...
 */
class CLoadProxy : public NLMISC::CI18N::ILoadProxy,
                   public TStringDiff::IDiffCallback {
  void loadStringFile(const std::string &filename,
