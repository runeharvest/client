...
static void stringArrayToVector(const char *strArray[], uint size,
                                CColorSlotManager::TStringVect &dest) {
  dest.resize(size);
...
const uint CColorSlotManager::_NumDefaultTextureExtensions =
    sizeof(CColorSlotManager::_DefaultTextureExtension) / sizeof(const char *);

//=======================================================================
CColorSlotManager::TIntCouple
CColorSlotManager::findFileExtensionInSlot(const std::string &ext) const {
  H_AUTO_USE(RZ_ColorSlotManager)
  for (TSlotVect::const_iterator it = _Slots.begin(); it != _Slots.end();
       ++it) {
    TStringVect::const_iterator extIt = std::find(it->begin(), it->end(), ext);
    if (extIt != it->end()) {
      return std::make_pair((uint)(it - _Slots.begin()),
                            (uint)(extIt - it->begin()));
    }
  }
...
//=======================================================================
bool CColorSlotManager::addTexturesFromPath(
  ...
  if (!validExtensions || numExtensions == 0) {
    nlwarning(
        "CColorSlotManager::addTexturesFromPath : no file extensions provided");
    return false;
  }
  if (path.empty()) {
  ...
  if (!NLMISC::CFile::isExists(path)) {
    nlwarning(
        ("CColorSlotManager::addTexturesFromPath : path not found : " + path)
            .c_str());
    return false;
  }
  ...
  if (files.empty()) {
    nlwarning(("CColorSlotManager::addTexturesFromPath : empty path :" + path)
                  .c_str());
    return false;
  }
  ...
  if (!fileFound) {
    nlwarning(("CColorSlotManager::addTexturesFromPath : the path " + path +
               " contains no texture files")
                  .c_str());
  }
  return everythingOk;
}
...
//=======================================================================
bool CColorSlotManager::parseTexName(
    const char *texName, std::string *texNameWithoutExtensions,
    TIntCoupleVect *destSlotsId /*=NULL*/) const {
  H_AUTO_USE(RZ_ColorSlotManager)
  /** If we got a name such as "armour_red_shiny.tga", and if shiny and red are
   * extensions, we parse shiny ('_' is considered has a separator), and find
   * the slot that has 'shiny' in it (lets says slot 0), then 'red' is parsed
   * (let says it is in the slot 1). The slotId vector will be { 0, 1 } at the
   * end If no slots where found, the parsing is considered to have failed. If
   * an extension is duplicated then it fails, too.
   */
  ...
  nameToParse =
      NLMISC::toUpperAscii(NLMISC::CFile::getFilenameWithoutExtension(texName));
  ...
  for (;;) {
    ...
    currentExt = nameToParse.substr(extPos + _Separator.length(),
                                    currPos - extPos - _Separator.length() + 1);
    ...
    for (uint k = 0; k < slotsId.size(); ++k) {
      if (slotsId[k].first == slotId.first) {
        nlwarning(
            (std::string("Duplicated extension in texture name ") + texName)
                .c_str());
        return false;
      }
    }
  ...
  if (texNameWithoutExtensions) {
    ...
    std::copy(nameToParse.begin(), nameToParse.begin() + currPos + 1,
              texNameWithoutExtensions->begin());
  }
  return true;
}

//=======================================================================
bool CColorSlotManager::addSlotsFromConfigFile(NLMISC::CConfigFile &cf,
                                               uint &startSlotDest,
                                               uint &numSlots) {
  H_AUTO_USE(RZ_ColorSlotManager)
  ...
  for (uint k = 0; k < (uint)mask_extensions->size(); ++k) {
    try {
      NLMISC::CConfigFile::CVar &extensions =
          cf.getVar(mask_extensions->asString(k) + "_color_id");
      ...
      nlwarning(("CColorSlotManager::addSlotsFromConfigFile : invalid config "
                 "file, variable not found :" +
                 mask_extensions->asString(k) + "_color_id")
                    .c_str());
      return false;
    }
  }
...
//=======================================================================
bool CColorSlotManager::changeTexName(std::string &texName, TIntCouple *slotIDs,
                                      uint numValues,
  ...
  if (texIt != _TexMap.end()) {
      ...
      for (n = 0; n < numValues; ++n) {
        if (texIt->second[m] == slotIDs[n].first) {
          if (slotIDs[n].second < _Slots[slotIDs[n].first].size()) {
            texNameNoExt +=
                _Separator + _Slots[slotIDs[n].first][slotIDs[n].second];
          } else {
            nlwarning("Color slot : out of range value found");
            // Use previous extension
            texNameNoExt +=
                _Separator + _Slots[srcSlotIDs[m].first][srcSlotIDs[m].second];
            everythingOk = false;
          }
          break;
        }
    ...
    return true;
  } else // Extensions are already present in the tex name, or simply, there are
         // no slots for this texture
  {
        ...
        for (n = 0; n < numValues; ++n) {
          if (srcSlotIDs[m].first == slotIDs[n].first) {
            if (slotIDs[n].second < _Slots[slotIDs[n].first].size()) {
              ...
              nlwarning("Color slot : out of range value found");
              // Use previous extension
              texNameBuild += _Separator +
                              _Slots[srcSlotIDs[m].first][srcSlotIDs[m].second];
              everythingOk = false;
            }
            break;
          }
...
//=======================================================================
bool CColorSlotManager::setInstanceSlot(NL3D::UInstance instance,
                                        TIntCouple *slotIDs,
  ...
  for (uint k = 0; k < numMat; ++k) {
    ...
    for (sint l = 0; l <= numTex; ++l) {
      if (mat.isTextureFile(l)) // one texture from a file ?
      {
        ...
        if (changeTexName(texName, slotIDs, numValues,
                          everythingOkForCurrent)) {
...
//=======================================================================
bool CColorSlotManager::setInstanceSlot(NL3D::UInstance instance, uint slotID,
                                        uint value) const {
  H_AUTO_USE(RZ_ColorSlotManager)
