...
string CBug_reportDlg::checkEmail(const string &email) {
  ...
  for (uint i = 0; i < email.size(); i++) {
    ...
    if (!isalnum(email[i]) && email[i] != '-' && email[i] != '.' &&
        email[i] != '_' && email[i] != '@') {
      mailerr +=
          toString("character '%c' is not allowed in email\r\n", email[i]);
    }
  }
  if (!at)
...
string getGraphicCardInformation() {
  ...
  HWND tmpHWND = CreateWindow("BRClass", "", WndFlags, CW_USEDEFAULT,
                              CW_USEDEFAULT, WndRect.right, WndRect.bottom,
  ...
  string res = string(vendor) + string(" ") + string(renderer) + string(" ") +
               string(version);
...
BOOL CALLBACK CSoundDriverDSoundEnumCallback(LPGUID guid, LPCSTR description,
                                             PCSTR module, LPVOID context) {
...
HCURSOR CBug_reportDlg::OnQueryDragIcon() { return (HCURSOR)m_hIcon; }

void CBug_reportDlg::sendReport(bool withScreenshot) {
  string body, res, requestor, mailerr;

  requestor = get(IDC_REQUESTOR);
  if (requestor.empty()) {
    MessageBox("Field Requestor must be filled", "Warning");
    return;
  }
  body += "RT-Requestor: " + requestor + "\n";
  ...
  if (!mailerr.empty()) {
    mailerr =
        "Bad email format, please modify it on \"bug_report.cfg\":\r\n\r\n" +
        mailerr;
  ...
  res = get(IDC_LANGUAGE);
  if (res.empty()) {
    MessageBox("Field Language must be filled", "Warning");
    return;
  }
  body += "RT-Language: " + res + "\n";
  res = get(IDC_SUBJECT);
  if (res.empty()) {
    MessageBox("Field Subject must be filled", "Warning");
    return;
  }
  body += "RT-Subject: " + res + "\n";
  res = get(IDC_CLIENTVERSION);
  if (res.empty()) {
    MessageBox("Field ClientVersion must be filled", "Warning");
    return;
  }
  body += "RT-ClientVersion: " + res + "\n";
  res = get(IDC_SHARDNAME);
  if (res.empty()) {
    MessageBox("Field ShardName must be filled", "Warning");
    return;
  }
  body += "RT-ShardName: " + res + "\n";
  res = get(IDC_SHARDVERSION);
  if (res.empty()) {
    MessageBox("Field ShardVersion must be filled", "Warning");
    return;
  }
  body += "RT-ShardVersion: " + res + "\n";
  res = get(IDC_SKU);
  if (res.empty()) {
    MessageBox("Field SKU must be filled", "Warning");
    return;
  }
  body += "RT-SKU: " + res + "\n";
  res = get(IDC_CLASS);
  if (res.empty()) {
    MessageBox("Field Class must be filled", "Warning");
    return;
  }
  body += "RT-Class: " + res + "\n";
  res = get(IDC_CATEGORY);
  if (res.empty()) {
    MessageBox("Field Category must be filled", "Warning");
    return;
  }
  body += "RT-Category: " + res + "\n";
  res = get(IDC_FREQUENCY);
  if (res.empty()) {
    MessageBox("Field Frequency must be filled", "Warning");
    return;
  }
  body += "RT-Frequency: " + res + "\n";
  res = get(IDC_QUEUE);
  if (res.empty()) {
    MessageBox("Field Queue must be filled", "Warning");
    return;
  }
  body += "RT-Queue: " + res + "\n";
  ...
  //	if (sendEmail ("192.168.254.1", "lecroart@nevrax.com",
  //"piuzzi@nevrax.com", "", body, attachedFile, false)) 	if (sendEmail
  //("192.168.254.1", "lecroart@nevrax.com", "lecroart@nevrax.com", "", body,
  //attachedFile, false))
  ...
  if (sendEmail(SMTPServer, requestor, ToEmail, "", body, attachedFile,
                false)) {
    if (MessageBox("Report successfully sent. Thank you for your "
                   "help\r\n\r\nDo you want to quit the bug report tool now?",
                   "Success", MB_YESNO) == IDYES) {
      exit(0);
    } else {
      setFocus(IDC_CLEAR);
    }
  } else {
    MessageBox("An error occurred when sending the report. Report wasn't sent, "
               "look log.log for more info",
               "Error");
  }
}

void CBug_reportDlg::OnSendreport() { sendReport(false); }

void CBug_reportDlg::OnSendreportwithss() { sendReport(true); }
...
void CBug_reportDlg::OnClear() {
  ...
  /////// TEMP pre fill entries
          ...
          set(IDC_ATTACHEDFILE, "C:\\Documents and
...
void CBug_reportDlg::OnNoattach() { set(IDC_ATTACHEDFILE, "", true); }
