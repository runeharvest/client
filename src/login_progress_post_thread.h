...
public:
  // if async is not null the return of the sendMsg will be in *async then
  // *AsyncSent will be passed to true; You add to check via pulling if data
  // have been send (maximum time 1 second)
  CLoginStep(uint step = 0, const std::string &postString = "",
             std::string *asyncRet = 0, bool *asyncSent = 0)
      : Step(step), PostString(postString), AsyncRet(asyncRet),
        AsyncSent(asyncSent) {}
};

/** Notify the http logon server from the various step that have been reached in
 * the client. Php scripts on server side use these information to collect
 * statistics such as :
 ...
 * - etc.
 * The sending is done in a separate thread to avoid freeze of the client
 * (especially at launch
 * -> if the server is unavailable, the user will at least be able to reach the
 * login screen without having to wait 10 seconds or so)
 */
class CLoginProgressPostThread
    : public NLMISC::CSingleton<CLoginProgressPostThread> {
public:
  CLoginProgressPostThread();
  ~CLoginProgressPostThread();
  void init(const std::string &startupHost, const std::string &startupPage);
...
enum {
  ...
  // following are not really part of the login, but are useful information as
  // well (TODO : find a better name for the class ...)
