...
NLMISC_REGISTER_OBJECT(CViewBase, CDBGroupListSheetBonusMalus, std::string,
                       "list_sheet_bonus_malus");

// ***************************************************************************
CDBGroupListSheetBonusMalus::CDBGroupListSheetBonusMalus(
    const TCtorParam &param)
    : CDBGroupListSheet(param), _RegenTextEnabled(true), _RegenTextY(-14),
      _RegenTextFontSize(8), _RegenTextColor(NLMISC::CRGBA::White),
      _RegenTextDisabledColor(NLMISC::CRGBA(127, 127, 127)) {
  // want leave space between controls in the list
  // Yoyo: I think it's better like this, + this is important for space
  // consideration and because of XPCat/PVPOutpost
...
// ***************************************************************************
void CDBGroupListSheetBonusMalus::CSheetChildTimer::init(
  ...
  if (root) {
    TimerDB = dynamic_cast<CCDBNodeLeaf *>(
        root->getNode(ICDBNode::CTextId("DISABLED_TIME"), false));
    DisabledDB = dynamic_cast<CCDBNodeLeaf *>(
        root->getNode(ICDBNode::CTextId("DISABLED"), false));
  }

  if (Ctrl) {
    CDBGroupListSheetBonusMalus *owner =
        dynamic_cast<CDBGroupListSheetBonusMalus *>(pFather);
    if (owner) {
...
// ***************************************************************************
void CDBGroupListSheetBonusMalus::CSheetChildTimer::update(
...
// ***************************************************************************
bool CDBGroupListSheetBonusMalus::parse(xmlNodePtr cur,
  ...
  XML_READ_COLOR(cur, "regen_text_color", _RegenTextColor,
                 NLMISC::CRGBA::White);
  XML_READ_COLOR(cur, "regen_text_disabled_color", _RegenTextDisabledColor,
