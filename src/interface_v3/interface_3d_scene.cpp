...
bool CInterface3DScene::parse(xmlNodePtr cur, CInterfaceGroup *parentGroup) {
  ...
  if (ptr)
    _UserInteraction = convertBool(ptr);
  ...
  if (ptr)
    fromString((const char *)ptr, _RotZFactor);
  ...
  if (ptr)
    fromString((const char *)ptr, _RotYFactor);

  ptr = (char *)xmlGetProp(cur, (xmlChar *)"dist_limit_min");
  if (ptr)
    fromString((const char *)ptr, _DistLimitMin);

  ptr = (char *)xmlGetProp(cur, (xmlChar *)"dist_limit_max");
  if (ptr)
    fromString((const char *)ptr, _DistLimitMax);

  ptr = (char *)xmlGetProp(cur, (xmlChar *)"dist_factor");
  if (ptr)
    fromString((const char *)ptr, _DistFactor);
...
// ----------------------------------------------------------------------------
void CInterface3DScene::updateCoords() { CViewBase::updateCoords(); }
...
void CInterface3DScene::draw() {
  ...
  // false, 						rVR.getBlankTextureId(), CRGBA(255,255,255,255) );
  // TEMP TEMP TEMP
  ...
  newFrustum.initPerspective(pI3DCam->getFOV() * (float)(NLMISC::Pi / 180),
                             (float)_WReal / iavoid0(_HReal), 0.1f, 100.f);
  ...
  cam.lookAt(pos, pI3DCam->getTarget(),
             pI3DCam->getRoll() * (float)(NLMISC::Pi / 180));
...
bool CInterface3DScene::handleEvent(const NLGUI::CEventDescriptor &event) {
  ...
  if (event.getType() == NLGUI::CEventDescriptor::mouse) {
    ...
    if (eventDesc.getEventTypeExtended() ==
      ...
      CInterfaceManager *pIM = CInterfaceManager::getInstance();
      CWidgetManager::getInstance()->setCapturePointerLeft(
          this); // Because we are not just a control
      return true;
    }
    if (eventDesc.getEventTypeExtended() ==
    ...
    if (eventDesc.getEventTypeExtended() ==
      ...
      CInterfaceManager *pIM = CInterfaceManager::getInstance();
      CWidgetManager::getInstance()->setCapturePointerRight(
          this); // Because we are not just a control
      return true;
    }
    if (eventDesc.getEventTypeExtended() ==
...
void CInterface3DScene::setCurrentCamera(const string &name) {
  ...
  CInterfaceElement *pIE =
      CWidgetManager::getInstance()->getElementFromId(pI3DS->getId(), name);
...
void CInterface3DScene::setCurrentClusterSystem(const string &sCSName) {
  ...
  CInterface3DScene *pI3DS = (_Ref3DScene != NULL) ? _Ref3DScene : this;
  CInterfaceElement *pIE =
      CWidgetManager::getInstance()->getElementFromId(pI3DS->getId(), sCSName);
...
// ----------------------------------------------------------------------------
CInterface3DCharacter::CInterface3DCharacter() { _Char3D = NULL; }

// ----------------------------------------------------------------------------
CInterface3DCharacter::~CInterface3DCharacter() { delete _Char3D; }

// ----------------------------------------------------------------------------
bool CInterface3DCharacter::parse(xmlNodePtr cur,
...
// ----------------------------------------------------------------------------
void CInterface3DCharacter::setSex(bool male) { _Char3D->setSex(male); }

// ----------------------------------------------------------------------------
bool CInterface3DCharacter::getSex() const { return _Char3D->getSex(); }
...
bool CInterface3DIG::parse(xmlNodePtr cur, CInterfaceGroup *parentGroup) {
  ...
  if (ptr)
    _Pos = convertVector(ptr);

  ptr = xmlGetProp(cur, (xmlChar *)"rot");
  if (ptr)
    _Rot = convertVector(ptr);

  ptr = xmlGetProp(cur, (xmlChar *)"name");
  if (ptr)
    _Name = toLowerAscii((const char *)ptr);
  ...
  setRotY(_Rot.y);
  setRotZ(_Rot.z);
  _IG->addToScene(*dynamic_cast<CInterface3DScene *>(parentGroup)->getScene(),
                  CViewRenderer::getInstance()->getDriver());
  dynamic_cast<CInterface3DScene *>(parentGroup)
      ->getScene()
      ->setToGlobalInstanceGroup(_IG);

  return true;
}

// ----------------------------------------------------------------------------
float CInterface3DIG::getPosX() const { return _Pos.x; }

// ----------------------------------------------------------------------------
float CInterface3DIG::getPosY() const { return _Pos.y; }

// ----------------------------------------------------------------------------
float CInterface3DIG::getPosZ() const { return _Pos.z; }
...
// ----------------------------------------------------------------------------
float CInterface3DIG::getRotX() const { return _Rot.x; }

// ----------------------------------------------------------------------------
float CInterface3DIG::getRotY() const { return _Rot.y; }

// ----------------------------------------------------------------------------
float CInterface3DIG::getRotZ() const { return _Rot.z; }
...
// ----------------------------------------------------------------------------
std::string CInterface3DIG::getName() const { return _Name; }
...
bool CInterface3DShape::parse(xmlNodePtr cur, CInterfaceGroup *parentGroup) {
  ...
  _Instance = dynamic_cast<CInterface3DScene *>(parentGroup)
                  ->getScene()
...
// ----------------------------------------------------------------------------
float CInterface3DShape::getPosX() const { return _Pos.x; }

// ----------------------------------------------------------------------------
float CInterface3DShape::getPosY() const { return _Pos.y; }

// ----------------------------------------------------------------------------
float CInterface3DShape::getPosZ() const { return _Pos.z; }
...
// ----------------------------------------------------------------------------
std::string CInterface3DShape::getName() const { return _Name; }
...
bool CInterface3DCamera::parse(xmlNodePtr cur, CInterfaceGroup *parentGroup) {
  ...
  if (ptr)
    _Target = convertVector(ptr);
  ptr = xmlGetProp(cur, (xmlChar *)"fov");
  if (ptr)
    fromString((const char *)ptr, _FOV);
  ptr = xmlGetProp(cur, (xmlChar *)"roll");
  if (ptr)
    fromString((const char *)ptr, _Roll);
...
bool CInterface3DLight::parse(xmlNodePtr cur, CInterfaceGroup *parentGroup) {
  ...
  _Light = dynamic_cast<CInterface3DScene *>(parentGroup)
               ->getScene()
               ->createPointLight();
  ...
  if (ptr)
    fromString((const char *)ptr, _Near);
  ptr = xmlGetProp(cur, (xmlChar *)"far");
  if (ptr)
    fromString((const char *)ptr, _Far);
...
// ----------------------------------------------------------------------------
float CInterface3DFX::getPosX() const { return _Pos.x; }

// ----------------------------------------------------------------------------
float CInterface3DFX::getPosY() const { return _Pos.y; }

// ----------------------------------------------------------------------------
float CInterface3DFX::getPosZ() const { return _Pos.z; }
...
// ----------------------------------------------------------------------------
std::string CInterface3DFX::getName() const { return _Name; }
...
// ----------------------------------------------------------------------------
bool CInterface3DFX::getStarted() const { return (!_FX.empty()); }
