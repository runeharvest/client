...
// ------------------------------------------------------------------------------------------------
void SCharacter3DSetup::setupDefault(EGSPD::CPeople::TPeople eRace,
                                     bool bMale) {
  People = eRace;
  Male = bMale;
  ...
  for (sint32 i = 0; i < SLOTTYPE::NB_SLOT; ++i) {
    ...
    if (!ISstr.empty()) {
      CItemSheet *pIS =
          dynamic_cast<CItemSheet *>(SheetMngr.get(CSheetId(ISstr)));
      if (pIS != NULL) {
        sint32 cpIndex =
            convert_VisualSlot_To_Char3DPart((SLOTTYPE::EVisualSlot)i);
...
void SCharacter3DSetup::setupFromCharacterSummary(const CCharacterSummary &cs) {
  ...
  setupFromCS_ModelCol(SLOTTYPE::CHEST_SLOT, rPVA.JacketModel,
                       rPVA.JacketColor);
  setupFromCS_ModelCol(SLOTTYPE::LEGS_SLOT, rPVA.TrouserModel,
                       rPVA.TrouserColor);
  setupFromCS_ModelCol(SLOTTYPE::HEAD_SLOT, rPVA.HatModel, rPVA.HatColor);

  // Should have to hide the face ?
  {
    //		TChar3DPart part = convert_VisualSlot_To_Char3DPart
    //(SLOTTYPE::HEAD_SLOT);
    CItemSheet *item = SheetMngr.getItem(SLOTTYPE::HEAD_SLOT, rPVA.HatModel);
    if ((item != NULL) && ((item->Family == ITEMFAMILY::ARMOR) ||
                           (item->Family == ITEMFAMILY::SHIELD)))
      HideFace = true;
    else
      HideFace = false;
  }
  ...
  // armor gloves are not displayed if character has the 'weapon' magician
  // gloves
  {
    CItemSheet *item =
        SheetMngr.getItem(SLOTTYPE::RIGHT_HAND_SLOT, rPVA.WeaponRightHand);
    if (!((item != NULL) && (item->ItemType == ITEM_TYPE::MAGICIAN_STAFF)))
      setupFromCS_ModelCol(SLOTTYPE::HANDS_SLOT, rPVB.HandsModel,
                           rPVB.HandsColor);
  }
  Tattoo = rPVC.Tattoo;
  HairColor = rPVA.HatColor; // TODO : For the moment no diff between hair color
                             // and head color !!!
  ...
  if (!ClientCfg.BlendShapePatched) {
    MTmin = 0.0f;
    MTmax = 100.0f;
  }
...
// ------------------------------------------------------------------------------------------------
void SCharacter3DSetup::setupDBFromCharacterSummary(
    const string &branchName, const CCharacterSummary &CS) {
  setDB(branchName + ":PEOPLE", CS.People);

  setDB(branchName + ":VPA:SEX", CS.VisualPropA.PropertySubData.Sex);
  setDB(branchName + ":VPA:JACKETMODEL",
        CS.VisualPropA.PropertySubData.JacketModel);
  setDB(branchName + ":VPA:JACKETCOLOR",
        CS.VisualPropA.PropertySubData.JacketColor);
  setDB(branchName + ":VPA:TROUSERMODEL",
        CS.VisualPropA.PropertySubData.TrouserModel);
  setDB(branchName + ":VPA:TROUSERCOLOR",
        CS.VisualPropA.PropertySubData.TrouserColor);
  setDB(branchName + ":VPA:WEAPONRIGHTHAND",
        CS.VisualPropA.PropertySubData.WeaponRightHand);
  setDB(branchName + ":VPA:WEAPONLEFTHAND",
  ...
  setDB(branchName + ":VPB:HANDSMODEL",
        CS.VisualPropB.PropertySubData.HandsModel);
  setDB(branchName + ":VPB:HANDSCOLOR",
        CS.VisualPropB.PropertySubData.HandsColor);
  setDB(branchName + ":VPB:FEETMODEL",
        CS.VisualPropB.PropertySubData.FeetModel);
  setDB(branchName + ":VPB:FEETCOLOR",
        CS.VisualPropB.PropertySubData.FeetColor);

  setDB(branchName + ":VPC:MORPHTARGET1",
        CS.VisualPropC.PropertySubData.MorphTarget1);
  setDB(branchName + ":VPC:MORPHTARGET2",
        CS.VisualPropC.PropertySubData.MorphTarget2);
  setDB(branchName + ":VPC:MORPHTARGET3",
        CS.VisualPropC.PropertySubData.MorphTarget3);
  setDB(branchName + ":VPC:MORPHTARGET4",
        CS.VisualPropC.PropertySubData.MorphTarget4);
  setDB(branchName + ":VPC:MORPHTARGET5",
        CS.VisualPropC.PropertySubData.MorphTarget5);
  setDB(branchName + ":VPC:MORPHTARGET6",
        CS.VisualPropC.PropertySubData.MorphTarget6);
  setDB(branchName + ":VPC:MORPHTARGET7",
        CS.VisualPropC.PropertySubData.MorphTarget7);
  setDB(branchName + ":VPC:MORPHTARGET8",
        CS.VisualPropC.PropertySubData.MorphTarget8);
  setDB(branchName + ":VPC:EYESCOLOR",
        CS.VisualPropC.PropertySubData.EyesColor);
  setDB(branchName + ":VPC:TATTOO", CS.VisualPropC.PropertySubData.Tattoo);
  setDB(branchName + ":VPC:CHARACTERHEIGHT",
        CS.VisualPropC.PropertySubData.CharacterHeight);
  setDB(branchName + ":VPC:TORSOWIDTH",
        CS.VisualPropC.PropertySubData.TorsoWidth);
  setDB(branchName + ":VPC:ARMSWIDTH",
        CS.VisualPropC.PropertySubData.ArmsWidth);
  setDB(branchName + ":VPC:LEGSWIDTH",
        CS.VisualPropC.PropertySubData.LegsWidth);
  setDB(branchName + ":VPC:BREASTSIZE",
        CS.VisualPropC.PropertySubData.BreastSize);
}

// ------------------------------------------------------------------------------------------------
void SCharacter3DSetup::setupCharacterSummaryFromDB(CCharacterSummary &CS,
                                                    const string &branchName) {
  CS.People = (EGSPD::CPeople::TPeople)getDB(branchName + ":PEOPLE");

  CS.VisualPropA.PropertySubData.Sex = getDB(branchName + ":VPA:SEX");
  CS.VisualPropA.PropertySubData.JacketModel =
      getDB(branchName + ":VPA:JACKETMODEL");
  CS.VisualPropA.PropertySubData.JacketColor =
      getDB(branchName + ":VPA:JACKETCOLOR");
  CS.VisualPropA.PropertySubData.TrouserModel =
      getDB(branchName + ":VPA:TROUSERMODEL");
  CS.VisualPropA.PropertySubData.TrouserColor =
      getDB(branchName + ":VPA:TROUSERCOLOR");
  CS.VisualPropA.PropertySubData.WeaponRightHand =
      getDB(branchName + ":VPA:WEAPONRIGHTHAND");
  CS.VisualPropA.PropertySubData.WeaponLeftHand =
      getDB(branchName + ":VPA:WEAPONLEFTHAND");
  ...
  CS.VisualPropB.PropertySubData.HandsModel =
      getDB(branchName + ":VPB:HANDSMODEL");
  CS.VisualPropB.PropertySubData.HandsColor =
      getDB(branchName + ":VPB:HANDSCOLOR");
  CS.VisualPropB.PropertySubData.FeetModel =
      getDB(branchName + ":VPB:FEETMODEL");
  CS.VisualPropB.PropertySubData.FeetColor =
      getDB(branchName + ":VPB:FEETCOLOR");

  CS.VisualPropC.PropertySubData.MorphTarget1 =
      getDB(branchName + ":VPC:MORPHTARGET1");
  CS.VisualPropC.PropertySubData.MorphTarget2 =
      getDB(branchName + ":VPC:MORPHTARGET2");
  CS.VisualPropC.PropertySubData.MorphTarget3 =
      getDB(branchName + ":VPC:MORPHTARGET3");
  CS.VisualPropC.PropertySubData.MorphTarget4 =
      getDB(branchName + ":VPC:MORPHTARGET4");
  CS.VisualPropC.PropertySubData.MorphTarget5 =
      getDB(branchName + ":VPC:MORPHTARGET5");
  CS.VisualPropC.PropertySubData.MorphTarget6 =
      getDB(branchName + ":VPC:MORPHTARGET6");
  CS.VisualPropC.PropertySubData.MorphTarget7 =
      getDB(branchName + ":VPC:MORPHTARGET7");
  CS.VisualPropC.PropertySubData.MorphTarget8 =
      getDB(branchName + ":VPC:MORPHTARGET8");
  CS.VisualPropC.PropertySubData.EyesColor =
      getDB(branchName + ":VPC:EYESCOLOR");
  CS.VisualPropC.PropertySubData.Tattoo = getDB(branchName + ":VPC:TATTOO");
  CS.VisualPropC.PropertySubData.CharacterHeight =
      getDB(branchName + ":VPC:CHARACTERHEIGHT");
  CS.VisualPropC.PropertySubData.TorsoWidth =
      getDB(branchName + ":VPC:TORSOWIDTH");
  CS.VisualPropC.PropertySubData.ArmsWidth =
      getDB(branchName + ":VPC:ARMSWIDTH");
  CS.VisualPropC.PropertySubData.LegsWidth =
      getDB(branchName + ":VPC:LEGSWIDTH");
  CS.VisualPropC.PropertySubData.BreastSize =
      getDB(branchName + ":VPC:BREASTSIZE");
}

// ------------------------------------------------------------------------------------------------

void SCharacter3DSetup::setupCharacterSummaryFromSERVERDB(CCharacterSummary &cs,
                                                          uint8 entityID) {
  ...
  if ((pp = dynamic_cast<CPlayerCL *>(EntitiesMngr.entity(entityID))) == NULL) {
    pp =
        (CPlayerCL *)dynamic_cast<CPlayerR2CL *>(EntitiesMngr.entity(entityID));
  }
  if (pp) {
    cs.People = pp->people();
...
// ------------------------------------------------------------------------------------------------
void SCharacter3DSetup::setupFromCS_ModelCol(SLOTTYPE::EVisualSlot s,
                                             sint32 model, sint32 col) {
  TChar3DPart part = convert_VisualSlot_To_Char3DPart(s);
  if (part == Char3DPart_INVALID)
    return;

  CItemSheet *item = SheetMngr.getItem(s, model);
  if (item != NULL) {
    // magician gloves are a weapon but displayed in hands slot(armor gloves)
    if ((s == SLOTTYPE::RIGHT_HAND_SLOT) &&
        (item->ItemType == ITEM_TYPE::MAGICIAN_STAFF)) {
    ...
    if ((s == SLOTTYPE::FEET_SLOT) &&
        (item->ItemType == ITEM_TYPE::LIGHT_BOOTS ||
         item->ItemType == ITEM_TYPE::MEDIUM_BOOTS ||
         item->ItemType == ITEM_TYPE::HEAVY_BOOTS)) {
      ...
      if (shapeLegs.find("_caster01_") != std::string::npos) {
        ...
        if (posBottes != std::string::npos) {
          std::string orgType = tmpName.substr(
              7, posBottes - 7); // underwear, caster01, armor00 or armor01
          ...
          if (CPath::exists(tmpName)) {
            ...
            // temporary hack because Fyros light boots don't respect
            // conventions
            if (tmpName[0] == 'f' &&
                (item->ItemType == ITEM_TYPE::LIGHT_BOOTS)) {
              if (tmpName[5] == 'f') {
              ...
              if (CPath::exists(tmpName)) {
                ...
                nlwarning("File %s doesn't exist, use %s", tmpName.c_str(),
    ...
    if (part == Char3DPart_HandLeftItem) {
      if ((item->ItemType == ITEM_TYPE::SHIELD) ||
          (item->ItemType == ITEM_TYPE::BUCKLER))
  ...
  for (uint cp = 0; cp < NB_CHARACTER3D_PARTS; ++cp)
    if (cp != Char3DPart_Face) {
      ...
      if (cp == Char3DPart_HandRightItem)
        nNbItems = 1 << 11;
      if ((cp == Char3DPart_Chest) || (cp == Char3DPart_Hands) ||
          (cp == Char3DPart_Feet))
        nNbItems = 1 << 9;
      if ((cp == Char3DPart_Legs) || (cp == Char3DPart_Arms) ||
          (cp == Char3DPart_HandLeftItem))
        nNbItems = 1 << 8;
      ...
      for (uint it = 0; it < nNbItems; ++it) {
        ...
        if (item == NULL) {
          ...
          // nlinfo("  val:%d M[%s] F[%s]", it, item->Shape.c_str(),
          // item->ShapeFemale.c_str());
...
CCharacter3D::CCharacter3D() {
  ...
  _CurrentSetup.ArmsWidth = _CurrentSetup.LegsWidth = _CurrentSetup.BreastSize =
      -20.0f;
...
bool CCharacter3D::init(UScene *pScene) {
  ...
  // ANIMATIONS
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  COptionsAnimationSet *pOAS = dynamic_cast<COptionsAnimationSet *>(
      CWidgetManager::getInstance()->getOptions("character_animations"));
  if (!pOAS || !pOAS->AnimationSet) {
    nlwarning("Not found <options> 'character_animations', or not of type "
              "'animation_set'");
    return false;
  }
...
void CCharacter3D::resetAnimation(UAnimationSet *animSet) {
  ...
  //	if (_FacePlayList != NULL) _PlayListManager->deletePlayList
  //(_FacePlayList);
...
void CCharacter3D::disableFaceMorphAndBlinks() {
  if (_FacePlayList && _AnimationSet) {
    ...
    for (uint i = 0; i < NB_MORPH_TARGETS; i++) {
      ...
      id = _AnimationSet->getChannelIdByName(baseName + toString(i) +
                                             "MorphFactor");
      if (id != UAnimationSet::NotFound)
        _FacePlayList->enableChannel(id, false);
    }
...
void CCharacter3D::setup(const SCharacter3DSetup &c3ds) {
  ...
  for (i = 0; i < NB_CHARACTER3D_PARTS; ++i) {
    ...
    // Create Instance
    if ((c3ds.Parts[i].Name != _CurrentSetup.Parts[i].Name) ||
        (c3ds.Parts[i].AdvFx != _CurrentSetup.Parts[i].AdvFx)) {
      ...
      if (i == Char3DPart_Face && _FacePlayList &&
          !_Instances[Char3DPart_Face].empty()) {
    ...
    if (c3ds.Parts[i].Quality != -1)
      if ((c3ds.Parts[i].Quality != _CurrentSetup.Parts[i].Quality) ||
          bInstanceRebuilt) {
        if (!_Instances[i].empty()) {
    ...
    if (c3ds.Parts[i].Color != -1)
      if ((c3ds.Parts[i].Color != _CurrentSetup.Parts[i].Color) ||
          bInstanceRebuilt || bQualityRebuilt) {
        if (!_Instances[i].empty()) {
    ...
    if (c3ds.People != EGSPD::CPeople::Undefined)
      if ((c3ds.People != _CurrentSetup.People) || bInstanceRebuilt ||
          bQualityRebuilt) {
        if (!_Instances[i].empty() && i != Char3DPart_HandRightItem &&
            i != Char3DPart_HandLeftItem) {
          ColorSlotManager.setInstanceSlot(_Instances[i],
                                           0u, // Slot 0 is for skin
                                           peopleToSkin(c3ds.People));
        }
        // Here we do not update current setup people value to let other
        // instances colorize too
      }
    ...
    case Char3DPart_Face:
      ...
      if (c3ds.EyesColor != -1)
        if ((c3ds.EyesColor != _CurrentSetup.EyesColor) || bInstanceRebuilt) {
          if (!_Instances[Char3DPart_Face].empty())
            ColorSlotManager.setInstanceSlot(
                _Instances[Char3DPart_Face],
      ...
      if ((c3ds.MorphTarget[0] != _CurrentSetup.MorphTarget[0]) ||
        ...
        if (!_Instances[Char3DPart_Face].empty()) {
          for (uint k = 0; k < NB_MORPH_TARGETS; ++k) {
            ...
            _Instances[Char3DPart_Face].setBlendShapeFactor(
                baseName + toString(k), c3ds.MorphTarget[k], true);
  ...
  if ((c3ds.CharHeight != _CurrentSetup.CharHeight) ||
    ...
    GabaritSet.applyGabarit(_Skeleton, gender, _CurrentSetup.People,
                            c3ds.CharHeight, c3ds.ChestWidth, c3ds.ArmsWidth,
                            c3ds.LegsWidth, c3ds.BreastSize, &heightScale);
    float refHeightScale =
        GabaritSet.getRefHeightScale(gender, _CurrentSetup.People);
  ...
  if (c3ds.AnimPlayed != -1)
    if ((c3ds.AnimPlayed != _CurrentSetup.AnimPlayed) || (bSkeletonRebuilt) ||
        _CopyAnim)
  ...
  // If skeleton or gabarit has changed replace correctly the skeleton from feet
  // reference point
...
void CCharacter3D::setAnim(uint animID) {
  ...
  if (!_CopyAnim) {
    ...
    if (_CurrentSetup.Male) {
      if (animID >= _AnimMale.size())
        return;
      animID = _AnimMale[animID].AnimId;
    } else {
      if (animID >= _AnimFemale.size())
        return;
    ...
    if (!(entity &&
          ((character = dynamic_cast<CCharacterCL *>(entity)) != NULL)))
      return;
  ...
  if (_PlayList) {
    if (_CopyAnim) {
      _PlayList->setTimeOrigin(MOVE,
                               character->playList()->getTimeOrigin(MOVE));
      if (character->playList()->getAnimation(MOVE) !=
          _PlayList->getAnimation(MOVE)) {
        _PlayList->setAnimation(MOVE, animID);
        _PlayList->setSpeedFactor(MOVE,
                                  character->playList()->getSpeedFactor(MOVE));
  ...
  if (_FacePlayList) {
    ...
    if (_AnimationSet && animID < _AnimationSet->getNumAnimation()) {
      ...
      string faceAnimName = COptionsAnimationSet::getFaceAnimName(
          _AnimationSet->getAnimationName(animID));
    ...
    if (faceAnimId != UPlayList::empty) {
      if (_CopyAnim) {
        _FacePlayList->setTimeOrigin(
            MOVE, character->facePlayList()->getTimeOrigin(MOVE));
        _FacePlayList->setSpeedFactor(
            MOVE, character->facePlayList()->getSpeedFactor(MOVE));
        _FacePlayList->setWrapMode(
            MOVE, character->facePlayList()->getWrapMode(MOVE));
      } else {
...
void CCharacter3D::animate(double globalTime) {
  ...
  double wrappedTime =
      (globalTime - _PlayList->getTimeOrigin(0)) * _PlayList->getSpeedFactor(0);
  // Mod repeat the time
  {
    float length = pAnim->getEndTime() - pAnim->getBeginTime();
    if (wrappedTime >= 0)
      wrappedTime =
          pAnim->getBeginTime() + (float)fmod((float)wrappedTime, length);
    else
      wrappedTime = pAnim->getBeginTime() +
                    (float)fmod((float)wrappedTime, length) + length;
  }
  pTrack->interpolate((float)wrappedTime, animPos);

  // apply race scale pos only if animation need it
  if (applyRaceScalePos)
    animPos *= getGenderInfo(_CurrentSetup.People, _CurrentSetup.Male)
                   ->CharacterScalePos;
...
void CCharacter3D::setSkeleton(const string &filename) {
  ...
  if (_Skeleton.empty()) {
    nlwarning("CCharacter3D::setSkeleton : Skeleton %s can't be created.",
              filename.c_str());
    return;
  }
...
// ------------------------------------------------------------------------------------------------
void CCharacter3D::createInstance(
    TChar3DPart i, const SCharacter3DSetup::SCharacterPart &part) {
  if (_Scene == NULL) {
  ...
  if (_Instances[i].empty()) {
    if ((i != Char3DPart_HandRightItem) && (i != Char3DPart_HandLeftItem))
      nlwarning(
          "CCharacter3D::createInstance : cannot create the instance : %s.",
          part.Name.c_str());
    return;
  }
  ...
  if ((!part.AdvFx.empty()) && (part.AdvFx != "none.shape")) {
    ...
    if (_InstancesFx[i].AdvantageFx.empty()) {
      nlwarning("CCharacter3D::createInstance : cannot create the fx : %s.",
                part.AdvFx.c_str());
    } else {
      CMatrix mat = _Instances[i].getMatrix();
      mat.invert();
      mat *= _InstancesFx[i].AdvantageFx.getMatrix();
      _InstancesFx[i].AdvantageFx.setTransformMode(
          UTransformable::DirectMatrix);
  ...
  for (fx = 0; fx < part.StatFxNames.size(); ++fx)
    if ((!part.StatFxNames[fx].empty()) &&
        (part.StatFxNames[fx] != "none.shape") &&
        (!part.StatFxBones[fx].empty()) &&
        (part.StatFxBones[fx] != "none.shape")) {
      ...
      if (boneID != -1) {
        ...
        if (!instance.empty()) {
          ...
          nlwarning("Can't create static fx %s sticked on bone %s",
                    part.StatFxNames[fx].c_str(), part.StatFxBones[fx].c_str());
        }
      } else {
        nlwarning("Can't find bone %s for static fx %s",
                  part.StatFxBones[fx].c_str(), part.StatFxNames[fx].c_str());
      }
    }
}
...
void CCharacter3D::bindToSkeleton(TChar3DPart i) {
  ...
  default:
    if (!_Skeleton.bindSkin(_Instances[i])) {
      nlwarning("CCharacter3D::bindToSkeleton: Cannot bind the instance : %d.",
                i);
      return;
    }
    break;
...
void CCharacter3D::animblink(double globalTime) {
  ...
  bool validTime = (_NextBlinkTime + blinkTime >= globalTime) &&
                   (_NextBlinkTime <= (globalTime + maxBlinkLength));
  ...
  if (blinkEnd || !validTime) {
    ...
    // Compute next time
    _NextBlinkTime = (((double)rand() / (double)RAND_MAX) *
                          (maxBlinkLength - minBlinkLength) +
                      minBlinkLength + (double)globalTime);
  } else {
    // Blink time ?
