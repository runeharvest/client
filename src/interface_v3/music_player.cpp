...
bool CMusicPlayer::isRepeatEnabled() const {
  return (NLGUI::CDBManager::getInstance()
              ->getDbProp(MP3_SAVE_REPEAT)
              ->getValue32() == 1);
}

bool CMusicPlayer::isShuffleEnabled() const {
  return (NLGUI::CDBManager::getInstance()
              ->getDbProp(MP3_SAVE_SHUFFLE)
              ->getValue32() == 1);
}
...
void CMusicPlayer::playSongs(const std::vector<std::string> &filenames) {
  ...
  for (uint i = 0; i < filenames.size(); i++) {
    _Songs.push_back(
        CSongs(filenames[i], CFile::getFilename(filenames[i]), 0.f));
  }
...
void CMusicPlayer::updatePlaylist(uint index, bool state) {
  ...
  CInterfaceElement *pIE = dynamic_cast<CInterfaceElement *>(
...
void CMusicPlayer::updateSong(const std::string filename,
                              const std::string title, float length) {
...
void CMusicPlayer::updateSong(const CSongs &song) {
  ...
  pVT =
      dynamic_cast<CViewText *>(pIG->getView(TEMPLATE_PLAYLIST_SONG_DURATION));
  if (pVT) {
    ...
    nlwarning("duration element '%s' not found",
              TEMPLATE_PLAYLIST_SONG_DURATION);
  }
}
...
void CMusicPlayer::rebuildPlaylist() {
  ...
  if (pList) {
    ...
    for (uint i = 0; i < _Songs.size(); ++i) {
      ...
      CInterfaceGroup *pNew =
          CWidgetManager::getInstance()->getParser()->createGroupInstance(
              TEMPLATE_PLAYLIST_SONG, pList->getId(), vParams);
      if (pNew) {
        CViewText *pVT = dynamic_cast<CViewText *>(
            pNew->getView(TEMPLATE_PLAYLIST_SONG_TITLE));
        if (pVT) {
          pVT->setText(_Songs[i].Title);
        }

        pVT = dynamic_cast<CViewText *>(
            pNew->getView(TEMPLATE_PLAYLIST_SONG_DURATION));
        if (pVT) {
...
void CMusicPlayer::play(sint index) {
  ...
  if (!_Songs.empty()) {
    ...
    if (_State == Paused) {
      ...
      SoundMngr->playMusic(_Songs[_CurrentSongIndex].Filename, 0, true, false,
                           false);
      _PauseTime = 0;
    }
...
void CMusicPlayer::pause() {
  ...
  // pause the music only if we are really playing (else risk to pause a
  // background music!)
  if (_State == Playing) {
...
void CMusicPlayer::stop() {
  ...
  // stop the music only if we are really playing (else risk to stop a
  // background music!)
  if (_State != Stopped)
...
void CMusicPlayer::updatePlayingInfo(const std::string info) {
  CViewText *pVT =
      dynamic_cast<CViewText *>(CWidgetManager::getInstance()->getElementFromId(
          "ui:interface:mp3_player:screen:text"));
  if (pVT) {
    pVT->setText(info);
  }
}
...
// ***************************************************************************
static void addFromPlaylist(const std::string &playlist,
  ...
  if (file) {
    ...
    while (fgets(line, 512, file)) {
      ...
      if (!useUtf8 && lineStr.length() >= 3 &&
          memcmp(line, utf8Header, 3) == 0) {
        useUtf8 = true;
      ...
      lineStr =
          CUtfStringView(lineStr).toUtf8(true); // Re-encode external string

      // Not a comment line
      if (lineStr[0] != '#') {
        std::string filename =
            CPath::makePathAbsolute(CFile::getPath(lineStr), basePlaylist) +
...
void CMusicPlayer::createPlaylistFromMusic() {
  ...
  std::string newPath = CPath::makePathAbsolute(
      CPath::standardizePath(ClientCfg.MediaPlayerDirectory),
  ...
  virtual void execute(CCtrlBase * /* pCaller */, const string &Params) {
    if (!SoundMngr) {
      ...
      if (Params.find("volume") == std::string::npos)
        CInterfaceManager::getInstance()->messageBox(
            CI18N::get("uiMP3SoundDisabled"));
