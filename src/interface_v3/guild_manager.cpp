...
NLMISC_REGISTER_OBJECT(CViewBase, CDBGroupListAscensor, std::string,
                       "list_sheet_guild");
...
#define VIEW_TEXT_GUILD_QUIT                                                   \
  "ui:interface:guild:content:tab_guild_info:quit_guild"
#define CTRL_SHEET_GUILD_BLASON                                                \
  "ui:interface:guild:content:tab_guild_info:blason"
#define VIEW_TEXT_GUILD_MEMBER_COUNT                                           \
  "ui:interface:guild:content:tab_guild_info:member_count"

#define LIST_GUILD_MEMBERS                                                     \
...
#define VIEW_JOIN_PROPOSAL_PHRASE                                              \
  "ui:interface:join_guild_proposal:content:inside:phrase"
...
static inline bool lt_member_name(const SGuildMember &m1,
                                  const SGuildMember &m2) {
  return toLower(m1.Name) < toLower(m2.Name);
}

static inline bool lt_member_grade(const SGuildMember &m1,
                                   const SGuildMember &m2) {
  return m1.Grade < m2.Grade;
}

static inline bool lt_member_online(const SGuildMember &m1,
                                    const SGuildMember &m2) {
...
bool CGuildManager::isProxy() {
  ...
  return NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:GUILD:PROXY")
      ->getValueBool();
}

// ***************************************************************************
/*void CGuildManager::init (const std::vector< std::pair<uint32,uint8> >
&NameGrade)
{
        _GuildMembers.clear();
        ...
        for (uint32 i = 0; i < NameGrade.size(); ++i)
        {
                ...
                _GuildMembers[i].Grade =
(EGSPD::CGuildGrade::TGuildGrade)(NameGrade[i].second&0x7f);
                _GuildMembers[i].Online = ((NameGrade[i].second&0x80) != 0);
        }
}*/

// ***************************************************************************
/*void CGuildManager::set (uint32 indexMember, uint32 MemberName, uint8
MemberGrade, bool bOnline)
{
        uint32 i;
...
(uint32)EGSPD::CGuildGrade::EndGuildGrade))
        {
                // Delete entry
        ...
        else
        {
                ...
                _GuildMembers[i].Grade =
(EGSPD::CGuildGrade::TGuildGrade)MemberGrade; _GuildMembers[i].Online = bOnline;
        }
...
bool CGuildManager::canRecruit() {
  if (!_InGuild)
    return false;
  if ((_Grade == EGSPD::CGuildGrade::Leader) ||
      (_Grade == EGSPD::CGuildGrade::HighOfficer) ||
      (_Grade == EGSPD::CGuildGrade::
                     Officer) /*|| (_Grade == EGSPD::CGuildGrade::Recruiter)*/)
    return true;
  return false;
}
...
uint64 CGuildManager::getMoney() {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  return NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:GUILD:INVENTORY:MONEY")
      ->getValue64();
}
...
void CGuildManager::update() {
  ...
  if (_NeedRebuild) {
    ...
    _Guild.NameID = NLGUI::CDBManager::getInstance()
    ...
    if (!_InGuild)
      closeAllInterfaces();
    _Guild.Icon = NLGUI::CDBManager::getInstance()
                      ->getDbProp("SERVER:GUILD:ICON")
                      ->getValue64();
      ...
      for (uint32 i = 0; i < MAX_GUILD_MEMBER; ++i) {
        sint32 name =
            NLGUI::CDBManager::getInstance()
                ->getDbProp("SERVER:GUILD:MEMBERS:" + toString(i) + ":NAME")
                ->getValue32();
        if (name != 0) {
          SGuildMember gm;
          gm.NameID = name;
          gm.Index = i;
          gm.Grade = (EGSPD::CGuildGrade::TGuildGrade)(
              NLGUI::CDBManager::getInstance()
                  ->getDbProp("SERVER:GUILD:MEMBERS:" + toString(i) + ":GRADE")
                  ->getValue32());
          gm.Online =
              (TCharConnectionState)(NLGUI::CDBManager::getInstance()
                                         ->getDbProp("SERVER:GUILD:MEMBERS:" +
                                                     toString(i) + ":ONLINE")
                                         ->getValue32());
          gm.EnterDate = NLGUI::CDBManager::getInstance()
                             ->getDbProp("SERVER:GUILD:MEMBERS:" + toString(i) +
                                         ":ENTER_DATE")
                             ->getValue32();
    ...
    bool playerNewToTheGuild =
        _NewToTheGuild && (oldName != _Guild.NameID) && _InGuild;
    if (playerNewToTheGuild) {
      // reset the flag
      _NewToTheGuild = false;
      // Don't pop the guild window in ring mode.
      // NB nico : this test should not be necessary, as the guild infos should
      // be filled during the init of the db However, there are situation where
      // this db info is filled after init (should only happen at guild creation
      // time ...) Maybe an EGS  bug ?
      if (R2::getEditor().getMode() == R2::CEditor::NotInitialized) {
        ...
        // Browse the forum
        pElt = CWidgetManager::getInstance()->getElementFromId(WIN_GUILD_FORUM
                                                               ":content:html");
        if (pElt != NULL) {
  ...
  if (_NeedUpdate) {
    bool bAllValid = true;
    // Update wait until all the name of members, name of the guild and
    // description are valid

    if (!pSMC->getString(_Guild.NameID, _Guild.Name))
      bAllValid = false;

    for (uint i = 0; i < _GuildMembers.size(); ++i) {
      if (!pSMC->getString(_GuildMembers[i].NameID, _GuildMembers[i].Name))
        bAllValid = false;
      else
        _GuildMembers[i].Name =
            CEntityCL::removeTitleAndShardFromName(_GuildMembers[i].Name);
    }

    // If all is valid no more need update and if guild is opened update the
    // interface
    if (bAllValid) {
      ...
      if (node && node->getValueBool()) {
        ...
        for (uint i = 0; i < _GuildMembers.size(); ++i) {
          map<string, SGuildMember>::const_iterator it =
              CachedGuildMembers.find(_GuildMembers[i].Name);
          if (it != CachedGuildMembers.end()) {
            ...
            if ((*it).second.Online != ccs_offline &&
                _GuildMembers[i].Online != ccs_offline) {
              // Not from offline, or to offline, so don't show anything
              continue;
            }

            string msg = (_GuildMembers[i].Online != ccs_offline)
                             ? onlineMessage
                             : offlineMessage;
            ...
            bool dummy;
            PeopleInterraction.ChatInput.Guild.displayMessage(msg, col, 2,
                                                              &dummy);
            break;
          }
        }
        ...
        for (uint i = 0; i < _GuildMembers.size(); ++i) {
          CachedGuildMembers.insert(
              make_pair(_GuildMembers[i].Name, _GuildMembers[i]));
        }
      }
      ...
      NLGUI::CDBManager::getInstance()
          ->getDbProp("UI:VARIABLES:USER:GUILD_GRADE")
          ->setValue32(_Grade);

      // update the guild display
      CGroupContainer *pGuild = dynamic_cast<CGroupContainer *>(
      ...
      if (pGuild != NULL) {
        ...
        if (pGuild->isOpen() && pGuild->getActive()) {
          ...
          if (CWidgetManager::getInstance()->getModalWindow() != NULL &&
              _NeedUpdateMembers) {
            if (CWidgetManager::getInstance()->getModalWindow()->getId() ==
                MENU_GUILD_MEMBER)
          ...
          CAHManager::getInstance()->runActionHandler(
              "guild_sheet_open", NULL,
              toString("update_members=%d", (uint)_NeedUpdateMembers));
        }
      }
  ...
  if (_JoinPropUpdate) {
    bool bAllValid = true;
    if (!pSMC->getDynString(_JoinPropPhraseID, _JoinPropPhrase))
      bAllValid = false;
    // If all is valid no more need update and update the interface
    if (bAllValid) {
      _JoinPropUpdate = false;
      CGroupContainer *pJoinProp = dynamic_cast<CGroupContainer *>(
          CWidgetManager::getInstance()->getElementFromId(WIN_JOIN_PROPOSAL));
      if (pJoinProp != NULL) {
        CViewText *pJoinPropPhraseView = dynamic_cast<CViewText *>(
            CWidgetManager::getInstance()->getElementFromId(
                VIEW_JOIN_PROPOSAL_PHRASE));
...
void CGuildManager::launchAscensor() {
  ...
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  CGroupContainer *pAC = dynamic_cast<CGroupContainer *>(
...
NLMISC_COMMAND(
  ...
  //
  NLGUI::CDBManager::getInstance()
      ->getDbProp("LOCAL:ASCENSOR:0:NAME")
      ->setValue32(12);
  prop = CGuildManager::iconMake(1, 1, false, CRGBA(255, 255, 0),
                                 CRGBA(0, 255, 0));
  NLGUI::CDBManager::getInstance()
      ->getDbProp("LOCAL:ASCENSOR:0:ICON")
      ->setValue64(prop);
  //
  NLGUI::CDBManager::getInstance()
      ->getDbProp("LOCAL:ASCENSOR:1:NAME")
      ->setValue32(12);
  prop = CGuildManager::iconMake(1, 2, false, CRGBA(0, 255, 255),
                                 CRGBA(255, 0, 255));
  NLGUI::CDBManager::getInstance()
      ->getDbProp("LOCAL:ASCENSOR:1:ICON")
      ->setValue64(prop);
  //
  NLGUI::CDBManager::getInstance()
      ->getDbProp("LOCAL:ASCENSOR:2:NAME")
      ->setValue32(12);
  prop =
      CGuildManager::iconMake(2, 3, false, CRGBA(255, 0, 0), CRGBA(0, 255, 0));
  NLGUI::CDBManager::getInstance()
      ->getDbProp("LOCAL:ASCENSOR:2:ICON")
      ->setValue64(prop);
  //
  NLGUI::CDBManager::getInstance()
      ->getDbProp("LOCAL:ASCENSOR:3:NAME")
      ->setValue32(12);
  prop = CGuildManager::iconMake(2, 4, false, CRGBA(255, 255, 0),
                                 CRGBA(0, 255, 255));
  NLGUI::CDBManager::getInstance()
      ->getDbProp("LOCAL:ASCENSOR:3:ICON")
      ->setValue64(prop);
  //
  NLGUI::CDBManager::getInstance()
      ->getDbProp("LOCAL:ASCENSOR:4:NAME")
      ->setValue32(12);
  prop = CGuildManager::iconMake(3, 5, false, CRGBA(255, 255, 0),
                                 CRGBA(255, 0, 255));
  NLGUI::CDBManager::getInstance()
      ->getDbProp("LOCAL:ASCENSOR:4:ICON")
      ->setValue64(prop);
  //
  NLGUI::CDBManager::getInstance()
      ->getDbProp("LOCAL:ASCENSOR:5:NAME")
      ->setValue32(12);
  prop = CGuildManager::iconMake(3, 6, false, CRGBA(0, 255, 255),
                                 CRGBA(255, 255, 0));
  NLGUI::CDBManager::getInstance()
      ->getDbProp("LOCAL:ASCENSOR:5:ICON")
      ->setValue64(prop);
  //
  NLGUI::CDBManager::getInstance()
      ->getDbProp("LOCAL:ASCENSOR:6:NAME")
      ->setValue32(12);
  prop = CGuildManager::iconMake(4, 7, false, CRGBA(0, 255, 255),
                                 CRGBA(255, 0, 255));
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:ASCENSOR:6:ICON")
      ->setValue64(prop);
  //
  NLGUI::CDBManager::getInstance()
      ->getDbProp("LOCAL:ASCENSOR:7:NAME")
      ->setValue32(12);
  prop = CGuildManager::iconMake(4, 8, false, CRGBA(255, 0, 255),
                                 CRGBA(0, 255, 255));
  NLGUI::CDBManager::getInstance()
      ->getDbProp("LOCAL:ASCENSOR:7:ICON")
      ->setValue64(prop);
...
void CGuildManager::closeAllInterfaces() {
  ...
  CGroupContainer *pGuildForum = dynamic_cast<CGroupContainer *>(
      CWidgetManager::getInstance()->getElementFromId(WIN_GUILD_FORUM));
  if (pGuildForum != NULL)
    pGuildForum->setActive(false);
  CGroupContainer *pGuildChat = dynamic_cast<CGroupContainer *>(
...
void CGuildManager::openGuildWindow() {
  ...
  //
  //	CCDBNodeLeaf	*node=
  //NLGUI::CDBManager::getInstance()->getDbProp("CLIENT:GUILD:HAVE_JOINED",
  //false); 	if(node) 		node->setValue64(1);
  //
  //	CInterfaceElement *pElt;
  //	pElt = CWidgetManager::getInstance()->getElementFromId(WIN_GUILD);
  //	if (pElt != NULL)
  ...
  //	// Browse the forum
  //	pElt =
  //CWidgetManager::getInstance()->getElementFromId(WIN_GUILD_FORUM":content:html");
  //	if (pElt != NULL)
  //	{
  //		CGroupHTML *html = dynamic_cast<CGroupHTML*>(pElt);
  //		if (html)
...
void CGuildManager::initForDebug() {
  ...
  _Guild.Icon = CGuildManager::iconMake(1, 1, false, CRGBA(255, 0, 255),
                                        CRGBA(0, 255, 255));
...
void CGuildManager::initDBObservers() {
  ...
  NLGUI::CDBManager::getInstance()->addBranchObserver("SERVER:GUILD:MEMBERS",
                                                      &_DBObsMembers);

  // observer on ascencors
  Ascensors.setListType(CHugeListObs::Ascensor);
  NLGUI::CDBManager::getInstance()->addBranchObserver("SERVER:ASCENSOR",
                                                      &Ascensors);
}
...
// **********************************************************************************
void CDBGroupListAscensor::CSheetChildAscensor::init(
...
// **********************************************************************************
bool CDBGroupListAscensor::CSheetChildAscensor::isInvalidated(
  ...
  if (!Setuped) {
    ...
    uint32 nameID =
        NLGUI::CDBManager::getInstance()
            ->getDbProp("LOCAL:ASCENSOR:" + toString(Index) + ":NAME")
            ->getValue32();
    string name;
    if (nameID && pSMC->getDynString(nameID, name)) {
      ...
      uint64 icon =
          NLGUI::CDBManager::getInstance()
              ->getDbProp("LOCAL:ASCENSOR:" + toString(Index) + ":ICON")
              ->getValue64();

      // Slot setup
      if ((icon & UINT64_CONSTANT(0x8000000000000000)) != 0) {

        LIFT_ICONS::TLiftIcon li =
            (LIFT_ICONS::TLiftIcon)(icon & UINT64_CONSTANT(0x7FFFFFFFFFFFFFFF));
...
// **********************************************************************************
bool CDBGroupListAscensor::CSheetChildAscensor::isSheetValid(
    CDBGroupListSheetText * /* pFather */) {
  if (!Ctrl)
    return false;
...
class CAHGuildSheetOpen : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string &Params) {
    ...
    CCtrlBaseButton *control = dynamic_cast<CCtrlBaseButton *>(
    ...
    if (updateMembers) {
      CGuildManager::TSortOrder order = (CGuildManager::TSortOrder)(
          NLGUI::CDBManager::getInstance()
              ->getDbProp("UI:SAVE:GUILD_LIST:SORT_ORDER")
              ->getValue32());
      ...
      CViewText *pVT = dynamic_cast<CViewText *>(
          CWidgetManager::getInstance()->getElementFromId(
              VIEW_TEXT_GUILD_MEMBER_COUNT));
      if (pVT)
        pVT->setText(toString(rGuildMembers.size()));

      // rebuild guild member list
      CGroupList *pParent = dynamic_cast<CGroupList *>(
      ...
      for (uint i = 0; i < rGuildMembers.size(); i++) {
        ...
        vParams.push_back(
            vector<pair<string, string>>::value_type("id", templateId));
        CInterfaceGroup *pLine = NULL;
        pLine = CWidgetManager::getInstance()->getParser()->createGroupInstance(
            TEMPLATE_GUILD_MEMBER, LIST_GUILD_MEMBERS, vParams);
        if (pLine == NULL)
          continue;

        // Set name
        CViewText *pViewName = dynamic_cast<CViewText *>(
            pLine->getView(TEMPLATE_GUILD_MEMBER_NAME));
        if (pViewName != NULL)
          pViewName->setText(rGuildMembers[i].Name);

        // Set Grade
        CViewText *pViewGrade = dynamic_cast<CViewText *>(
            pLine->getView(TEMPLATE_GUILD_MEMBER_GRADE));
        if (pViewGrade != NULL) {
        ...
        // online?
        CViewBitmap *onlineView =
            dynamic_cast<CViewBitmap *>(pLine->getView("online"));
        if (onlineView != NULL) {
          ...
          case ccs_online:
            ...
            if (toolTip)
              toolTip->setDefaultContextHelp(
                  CI18N::get("uittGuildMemberOnline"));
            break;
          case ccs_online_abroad:
            onlineView->setTexture("w_online_abroad.tga");
            if (toolTip)
              toolTip->setDefaultContextHelp(
                  CI18N::get("uittGuildMemberOnlineAbroad"));
            break;
          default:
            onlineView->setTexture("w_offline.tga");
            if (toolTip)
              toolTip->setDefaultContextHelp(
                  CI18N::get("uittGuildMemberOffline"));
            break;
          }
        }
        ...
        if (inviteButton != NULL)
          inviteButton->setActive(rGuildMembers[i].Online != ccs_offline &&
                                  rGuildMembers[i].Name !=
                                      UserEntity->getEntityName());

        // Enter Date
        CViewText *pViewEnterDate = dynamic_cast<CViewText *>(
            pLine->getView(TEMPLATE_GUILD_MEMBER_ENTER_DATE));
...
// ***************************************************************************
static void setRights(bool lead, bool hioff, bool offi, bool recr, bool bear,
                      bool memb, bool kick) {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  CViewTextMenu *pVTM;
  pVTM = dynamic_cast<CViewTextMenu *>(
      CWidgetManager::getInstance()->getElementFromId(
          string(MENU_GUILD_MEMBER ":lead")));
  if (pVTM != NULL)
    pVTM->setGrayed(!lead);
  pVTM = dynamic_cast<CViewTextMenu *>(
      CWidgetManager::getInstance()->getElementFromId(
          string(MENU_GUILD_MEMBER ":hiof")));
  if (pVTM != NULL)
    pVTM->setGrayed(!hioff);
  pVTM = dynamic_cast<CViewTextMenu *>(
      CWidgetManager::getInstance()->getElementFromId(
          string(MENU_GUILD_MEMBER ":offi")));
  if (pVTM != NULL)
    pVTM->setGrayed(!offi);
  pVTM = dynamic_cast<CViewTextMenu *>(
      CWidgetManager::getInstance()->getElementFromId(
          string(MENU_GUILD_MEMBER ":recr")));
  if (pVTM != NULL)
    pVTM->setGrayed(!recr);
  pVTM = dynamic_cast<CViewTextMenu *>(
      CWidgetManager::getInstance()->getElementFromId(
          string(MENU_GUILD_MEMBER ":bear")));
  if (pVTM != NULL)
    pVTM->setGrayed(!bear);
  pVTM = dynamic_cast<CViewTextMenu *>(
      CWidgetManager::getInstance()->getElementFromId(
          string(MENU_GUILD_MEMBER ":memb")));
  if (pVTM != NULL)
    pVTM->setGrayed(!memb);
  pVTM = dynamic_cast<CViewTextMenu *>(
      CWidgetManager::getInstance()->getElementFromId(
          string(MENU_GUILD_MEMBER ":kick")));
  if (pVTM != NULL)
    pVTM->setGrayed(!kick);
}
...
class CAHGuildSheetMenuOpen : public IActionHandler {
  virtual void execute(CCtrlBase *pCaller, const string & /* Params */) {
    ...
    // *** Check and retrieve the current member index (index in the member
    // list)
    CCtrlBase *ctrlLaunchingModal =
    ...
    // *** Check with the grade of the local player which types of actions we
    // can do on the player selected
...
public:
  // Current selection
  static sint32
      MemberIndexSelected; // Index of the member selected when right clicked
  static std::string MemberNameSelected; // Name of the member selected when
                                         // right clicked (for extra check)
};
...
static void sendMsgSetGrade(EGSPD::CGuildGrade::TGuildGrade Grade) {
  ...
  if ((MemberNb < 0) || (MemberNb >= (sint32)rGuildMembers.size()))
    return;
  ...
  if (GenericMsgHeaderMngr.pushNameToStream(message, out)) {
    ...
    uint8 u8Grade = Grade;
    out.serial(u8Grade);
    uint8 u8Counter = (uint8)NLGUI::CDBManager::getInstance()
                          ->getDbProp("SERVER:GUILD:COUNTER")
                          ->getValue8();
    out.serial(u8Counter);
    NetMngr.push(out);
    // nlinfo("impulseCallBack : %s %d %d %d sent", message.c_str(),
    // u16MemberNb, u8Grade, u8Counter);
  } else {
...
public:
  void execute(CCtrlBase * /* pCaller */, const std::string & /* sParams */) {
    ...
    CAHManager::getInstance()->runActionHandler("guild_sheet_open", NULL,
...
public:
  ...
  // Current selection
  static sint32
      MemberIndexSelected; // Index of the member selected when left clicked
  static std::string
      MemberNameSelected; // Name of the member selected when lef clicked
};
...
REGISTER_ACTION_HANDLER(CAHGuildSheetSetLeaderConfirm,
                        "guild_member_do_change_leader");
...
REGISTER_ACTION_HANDLER(CAHGuildSheetSetHighOfficer,
...
REGISTER_ACTION_HANDLER(CAHGuildSheetSetRecruiter,
...
class CAHGuildSheetKick : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    ...
    if ((MemberNb < 0) || (MemberNb >= (sint32)rGuildMembers.size()))
      return;
    // double check the name (in case of some change in the guild members list)
    if (rGuildMembers[MemberNb].Name !=
    ...
    if (GenericMsgHeaderMngr.pushNameToStream(message, out)) {
      ...
      out.serial(u8Counter);
      NetMngr.push(out);
      // nlinfo("impulseCallBack : %s %d %d sent", message.c_str(), u16MemberNb,
      // u8Counter);
    } else {
      nlwarning("<CHandlerAcceptExchange::execute> unknown message name '%s'",
                message.c_str());
...
class CHandlerInvGuildToBag : public IActionHandler
{
        virtual void execute (CCtrlBase *pCaller, const string &Params)
        {
                ...
                CCDBNodeLeaf *pNL =
NLGUI::CDBManager::getInstance()->getDbProp(sTmp+":SHEET",false); CCDBNodeLeaf
*pNLquantity =
NLGUI::CDBManager::getInstance()->getDbProp(sTmp+":QUANTITY",false); if (pNL ==
NULL) return; if (pNLquantity == NULL) return;
                ...
                double totalBulk = quantity *
pInv->getItemBulk(pNL->getValue32());

                // Only check with the bag for the moment

                bool bPlaceFound = false;
                if (pInv->isInventoryAvailable(INVENTORIES::bag))
                        if ((pInv->getBagBulk(0) + totalBulk) <=
pInv->getMaxBagBulk(0)) bPlaceFound = true;
...
NLGUI::CDBManager::getInstance()->getDbProp("SERVER:GUILD:INVENTORY:SESSION")->getValue16();
                ...
                else
                {
                        ...
                        out.serial(Session);
                        NetMngr.push (out);
                        nlinfo("impulseCallBack : %s %d %d sent", sMsg.c_str(),
GuildIndex, Session);
                }
        }
};
...
static void sendMoneyServerMessage(const string &sMsg) {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  uint64 nMoney = NLGUI::CDBManager::getInstance()
                      ->getDbProp("UI:VARIABLES:CHOOSE_MONEY")
                      ->getValue64();
  uint16 Session = NLGUI::CDBManager::getInstance()
