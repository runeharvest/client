...
public:
  ...
  //
  void initAfterLoad(); // must call after load time to reset prompt color,
                        // default channel etc...
  //
  void release(); // removes every people list & chat windows
  //
  /** from a group container ID, returns a pointer on the people list and a
   * people index \return false is the id was invalid
   */
  bool getPeopleFromContainerID(const std::string &id, CPeopleList *&peopleList,
                                uint &index);
  ...
  void addContactInList(uint32 contactId, uint32 nameID,
                        TCharConnectionState Online, uint8 nList);
  void addContactInList(uint32 contactId, const std::string &name,
                        TCharConnectionState Online, uint8 nList);
  ...
  void updateContactInList(uint32 contactId, TCharConnectionState online,
                           uint nList);
  // @}
  ...
  // refrech the 'active' state of user chats : useful when a virtual desktop
  // change occurs
...
private:
  ...
  void buildFilteredChatSummary(const CFilteredChat &src,
                                CFilteredChatSummary &dest);
  void buildFilteredDynChatSummary(const CFilteredChat &src,
                                   CFilteredDynChatSummary &dest);
  ...
  void setupUserChatFromSummary(const CFilteredChatSummary &summary,
                                CFilteredChat &dest);
  void setupUserDynChatFromSummary(const CFilteredDynChatSummary &summary,
                                   CFilteredChat &dest);
