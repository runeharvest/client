...
static const char *WIN_BOT_CHAT_PAGE_PLAYER_GIFT =
    "ui:interface:bot_chat_player_gift";
...
void CBotChatPagePlayerGift::begin() {
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp(BOT_CHAT_BASE_DB_PATH ":PLAYER_GIFT")
      ->setValue32(0);

  // clear money proposal value
  CCDBNodeLeaf *moneyProposal = NLGUI::CDBManager::getInstance()->getDbProp(
      CWidgetManager::getInstance()->getParser()->getDefine("money_proposal"));
  if (moneyProposal)
    moneyProposal->setValue64(0);
...
public:
  virtual void execute(CCtrlBase * /* pCaller */,
    ...
    if (GenericMsgHeaderMngr.pushNameToStream("BOTCHAT:VALIDATE_PLAYER_GIFT",
                                              out)) {
      NetMngr.push(out);
    } else {
      nlwarning("bad msg");
    ...
    CBotChatPagePlayerGift *bpGift = dynamic_cast<CBotChatPagePlayerGift *>(
