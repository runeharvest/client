...
public:
  ...
  virtual void update();
  virtual void end();
  // start the 'buy' dialog window for the given sheet (the player has clicked
  // on an item to buy)
  ...
  // should be called when the player has confirmed  the current trade (clicked
  // 'ok' in the modal window for trade)
  void confirmTrade(bool enableResale = false);
  // should be called when the player has canceled the current trade (the modal
  ...
  // update the 'buy' dialog window for the given sphrase sheet (the player has
  // changed an equipped weapon => action stats change)
  void updateSPhraseBuyDialog();
  // invalidate window coords
  void invalidateCoords();

  // Filter for BotChat Buy
  void startChangeBuyFilterDialog(const std::string &dbext,
                                  const std::string &title, uint32 maxValue);
  ...
  // static method to get an item sheet name, according to item type (NULL if
  // not reconized for instance)
  static std::string getItemSheetNameForItemType(ITEM_TYPE::TItemType it);

  ///////////////////////////////////////////////////////////////////////////////////////
private:
  // an observer to update big mission list from littles pages in server
  // database
  ...
  // Cache To verify each frame the properties has not changed (verify approx
  // same item)
  class CItemCheck {
  public:
    CItemCheck() { reset(); }
  ...
  bool _CannotValidateBecauseRetireNotAvailable; // buy item only (if the item
                                                 // belongs to the user)
...
private:
  ...
  // get quantity in modal box for current item, or ~0 if not retrieved, or 1 if
  // _QuantityEdit==false
  uint32 getCurrItemQuantity() const;
  // get resale price in modal box for current item, or 0 if not retrieved, or
  // if resale is impossible
  ...
  void getItemFactionTypePoints(CDBCtrlSheet *sheet,
                                PVP_CLAN::TPVPClan &pvpClan,
                                uint32 &points) const;
  void getCurrItemFactionTypePoints(PVP_CLAN::TPVPClan &pvpClan,
                                    uint32 &points) const;
  ...
  // @{
  // Setup the _QuantityEdit and _QauntityCheck according to current
  // sellMode/sheet. return the default quantity to put in the edit box
  uint initQuantityMode(bool sellMode, CDBCtrlSheet *sheet);
  // Setup a common price group quantity according to _QuantityEdit and default
  // quantity
  void setupPriceGroupQuantity(CInterfaceGroup *priceGroup,
                               sint32 defaultQuantity);
  // Setup the resell group according to DB and price etc...
  void setupResellGroup(bool sellMode, uint defaultQuantity,
                        CInterfaceGroup *parentGroup, CDBCtrlSheet *sheet);
  // according to _QuantityEdit, setup Focus on correct group
  void setupEditFocus(CInterfaceGroup *parentGroup);
  // Setup if must take AMOUNT or PRICE_RETIRE (only on an ITEM buy), and
  // according to SELLER_TYPE
  void setupPriceOrPriceRetire(bool sellMode, CDBCtrlSheet *sheet);
  // Setup the faction point price
  void setupFactionPointPrice(bool sellMode, uint defaultQuantity,
