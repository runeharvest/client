...
NLMISC_REGISTER_OBJECT(CViewBase, CDBGroupListSheetText, std::string,
                       "list_sheet_text");

// ***************************************************************************
CDBGroupListSheetText::CDBGroupListSheetText(const TCtorParam &param)
    : IListSheetBase(param), _TextTemplate(TCtorParam()) {
  _MaxItems = INT_MAX;
  _WSlot = 24;
  _HSlot = 24;
  _HSpace = 0;
...
// ***************************************************************************
bool CDBGroupListSheetText::parse(xmlNodePtr cur,
  ...
  if (prop) {
    ...
    CCDBNodeBranch *branch =
        NLGUI::CDBManager::getInstance()->getDbBranch(prop.str());
    if (!branch) {
  ...
  case CCtrlSheetInfo::SheetType_Item:
    dispSlotBmpId = rVR.getTextureIdFromName("w_slot_item.tga");
    break;
  ...
  case CCtrlSheetInfo::SheetType_GuildFlag:
    dispSlotBmpId = rVR.getTextureIdFromName("w_slot_blason.tga");
    break;
  case CCtrlSheetInfo::SheetType_ElevatorDestination:
    dispSlotBmpId = rVR.getTextureIdFromName("w_slot_blason.tga");
    break;
  default:
    break;
  ...
  // read array. if true, empty slots are displayed, else only correct sheet id
  // are displayed
  ...
  if (prop)
    _GrayTextWithCtrlState = convertBool(prop);
...
void CDBGroupListSheetText::updateCoords() {
  ...
  if (_CtrlInfo._Type != CCtrlSheetInfo::SheetType_Macro) {
    // for all children, test the ones that really need text updates, and update
    // cache
    for (i = 0; i < _SheetChildren.size(); i++) {
      ...
      if (oldNameId != curNameId) {
        if (curNameId == 0)
          ...
          // if not received, must insert in list of pending dynstring to check
          // each frame
          string result;
    ...
    if (_SheetChildren[i]->Valid) {
      ...
      if (_Sectionable && !_SectionableError) {
        ...
        if (ctrlSectionId != curSectionId) {
          // insert all section between curSectionId excluded and ctrlSectionId
          // included
          insertSectionGroupList(curSectionId + 1, ctrlSectionId + 1,
                                 groupSectionIndex, yItem);
          // new active section
  ...
  if (_Sectionable && !_SectionableError &&
    ...
    if (curSectionId + 1 < maxSectionId) {
      ...
      insertSectionGroupList(curSectionId + 1, maxSectionId, groupSectionIndex,
                             yItem);
  ...
  for (i = 0; i < _SheetChildren.size(); i++) {
    ...
    if (child.Valid) {
      ...
      child.Text->setLineMaxW(
          min((sint32)(_List->getWReal() - (_XText + deltaX)),
              (sint32)_TextTemplate.getLineMaxW()),
          false);
  ...
  // Keep the same MaxH
  // must update it before _ScrollBar update (done in
...
// ***************************************************************************
void CDBGroupListSheetText::insertSectionGroupList(sint firstSectionId,
                                                   sint lastSectionId,
                                                   uint &groupSectionIndex,
                                                   sint &yItem) {
  ...
  for (; firstSectionId < lastSectionId; firstSectionId++) {
    if (insertSectionGroup(groupSectionIndex, firstSectionId, _XItem,
                           _YItem - yItem * _HSlot)) {
      groupSectionIndex++;
      yItem++;

      // if it is an empty section, may also add a white space
      if (firstSectionId < lastSectionId - 1 &&
          _SectionEmptyScheme == AllowEmptySectionWithExtraSpace)
        yItem++;
    } else {
      // never retry
...
void CDBGroupListSheetText::checkCoords() {
  ...
  for (set<CSheetChild *>::iterator it = _NameIdToUpdate.begin();
       it != _NameIdToUpdate.end();) {
    CSheetChild *cst = (*it);
    // String result
    string result;
    if (STRING_MANAGER::CStringManagerClient::instance()->getDynString(
  ...
  if (forceCheck || _BranchModified == true) {
    // if the branch is modified, then may check the gray state of each control
    // => gray the text too!
    if (_BranchModified && _GrayTextWithCtrlState) {
      ...
      CRGBA redifyColor =
          CWidgetManager::getInstance()
              ->getSystemOption(CWidgetManager::OptionCtrlTextRedifyColor)
              .getValColor();
      ...
      for (uint i = 0; i < _SheetChildren.size(); i++) {
        if (_SheetChildren[i]->Text) {
          ...
          bool redifiyed;
          if (_SheetChildren[i]->Ctrl->getType() !=
              CCtrlSheetInfo::SheetType_Item)
            redifiyed = lockPtr && lockPtr->getValue32() == 2;
          else
            redifiyed = _SheetChildren[i]
                            ->Ctrl->getItemPrerequisitValidPtr()
                            ->getValueBool();
    ...
    // Must test all SheetId change (even if array), because sheetId change
    // means ViewText name change Must test also the NameId change (if ctrl is
    // item)
    {
      ...
      for (uint i = 0; i < _SheetChildren.size(); i++) {
        ...
        sint32 curRMFaberStatType =
            _SheetChildren[i]->Ctrl->getItemRMFaberStatType();
        if (curId != _SheetChildren[i]->SheetId ||
            curNameId != _SheetChildren[i]->NameId ||
            curRMClassType != _SheetChildren[i]->RMClassType ||
            curRMFaberStatType != _SheetChildren[i]->RMFaberStatType) {
  ...
  // Check each frame if must change the ctrl gray state of all items, according
  // to animal status
  if (_AnimalStatus) {
    if (_CacheAnimalStatus != _AnimalStatus->getValue32()) {
      _CacheAnimalStatus = _AnimalStatus->getValue32();
      bool mustGrayAll = !ANIMAL_STATUS::isInventoryAvailable(
          (ANIMAL_STATUS::EAnimalStatus)_CacheAnimalStatus);
      ...
      if (CWidgetManager::getInstance()->getCurrentWindowUnder() == pGC) {
        if ((CWidgetManager::getInstance()->getPointer()->getX() >= _XReal) &&
          ...
          if ((pCSSrc != NULL) && pCSSrc->isDragged()) {
            ...
            CAHManager::getInstance()->runActionHandler(_CtrlInfo._AHOnCanDrop,
                                                        this, params);
          }
        }
      }
...
// ***************************************************************************
void CDBGroupListSheetText::clearViews() { CInterfaceGroup::clearViews(); }
...
bool CDBGroupListSheetText::handleEvent(const NLGUI::CEventDescriptor &event) {
  if (event.getType() == NLGUI::CEventDescriptor::mouse) {
    ...
    if (isIn(eventDesc.getX(), eventDesc.getY())) {
      ...
      if ((CWidgetManager::getInstance()->getCapturePointerLeft() != NULL) &&
        ...
        CCtrlButton *pCB = dynamic_cast<CCtrlButton *>(
            CWidgetManager::getInstance()->getCapturePointerLeft());
        if (pCB != NULL) {
          // A button has been captured -> Transform the capture to the
          // corresponding ctrlsheet
          sint pos = getIndexOf(pCB);
          if ((pos >= 0) && _SheetChildren[pos]->Ctrl->isDraggable() &&
              (!_SheetChildren[pos]->Ctrl->getGrayed())) {
            ...
            // Send this because not send (the captured button has processed the
            // event mouseleftdown)
            newEv.setEventTypeExtended(
                NLGUI::CEventDescriptorMouse::mouseleftdown);
            pDraggedSheet->handleEvent(newEv);
          }
        } else {
          pDraggedSheet = dynamic_cast<CDBCtrlSheet *>(
              CWidgetManager::getInstance()->getCapturePointerLeft());
          // auto scroll only if swapable
          if (swapable()) {
            // If a ctrlsheet of this list is being dragged and we are on top or
            // bottom : scroll
        ...
        // If we were dragging a sheet from this list so we want to drop it
        // somewhere
        if (swapable()) {
      ...
      if (eventDesc.getEventTypeExtended() ==
        ...
        if (_ScrollBar != NULL && _List != NULL &&
            _List->getH() > _List->getMaxH()) {
          // The listSheetTrade can be very big. So to get better precision,
          // scroll only 1 item for 1 mouseWheel In this case, also ensure that
          // we get no Items clipped by the group.
    ...
    const NLGUI::CEventDescriptorSystem &systemEvent =
        (const NLGUI::CEventDescriptorSystem &)event;
    if (systemEvent.getEventTypeExtended() ==
...
void CDBGroupListSheetText::setup() {
  ...
  _ScrollBar =
      dynamic_cast<CCtrlScroll *>(CInterfaceGroup::getCtrl("scroll_row"));
  ...
  for (i = 0; i < (uint)nbNodes; i++) {
    ...
    //		ctrl->initSheet( _DbBranchName + ":" + toString(i+_StartDbIdx),
    //_CtrlInfo );
    ctrl->initSheetFast(_DbBranchName, i + _StartDbIdx, slotNum, _CtrlInfo);
    // Change the ActionHandler for selection and info. Because must also select
    // the button....
...
sint CDBGroupListSheetText::getIndexOf(const CDBCtrlSheet *sheet) const {
  for (uint k = 0; k < _SheetChildren.size(); ++k) {
    if (_SheetChildren[k]->Ctrl == sheet)
      return k;
  }
  return -1;
}
...
sint CDBGroupListSheetText::getIndexOf(const CCtrlButton *button) const {
  for (uint k = 0; k < _SheetChildren.size(); ++k) {
    if (_SheetChildren[k]->Button == button)
      return k;
  }
  return -1;
}
...
void CDBGroupListSheetText::unselect() {
  ...
  if (_SheetChildren.empty())
    return;
  _SheetChildren[0]->Button->unselect();
}

// ***************************************************************************
bool CDBGroupListSheetText::CSheetChild::isSheetValid(
...
// ***************************************************************************
void CDBGroupListSheetText::CSheetChild::hide(
    CDBGroupListSheetText * /* pFather */) {
...
CGroupContainer *CDBGroupListSheetText::getContainer() {
  return dynamic_cast<CGroupContainer *>(
      CWidgetManager::getInstance()->getWindow(this));
}

// ***************************************************************************
bool CDBGroupListSheetText::needCheckAllItems() {
  if (_CtrlInfo._Type == CCtrlSheetInfo::SheetType_Item ||
      _CtrlInfo._Type == CCtrlSheetInfo::SheetType_Auto) {
    CInterfaceManager *pIM = CInterfaceManager::getInstance();
    // Must test each frame if Player characteristics have changed, because in
    // this case, ItemRequirement and thus TextColor may change....
    bool mustTest = false;
    for (uint i = 0; i < CHARACTERISTICS::NUM_CHARACTERISTICS; i++) {
      sint32 newVal =
          pIM->getCurrentPlayerCarac((CHARACTERISTICS::TCharacteristics)i);
...
// ***************************************************************************
void CDBGroupListSheetText::CSheetChild::init(CDBGroupListSheetText *pFather,
                                              uint /* index */) {
  Owner = pFather;
}
...
class CHandlerListSheetTextSelect : public IActionHandler {
  virtual void execute(CCtrlBase *pCaller, const string &Params) {
    ...
    if (!pCaller->getParent())
      return;
    CDBGroupListSheetText *listSheetTrade =
        dynamic_cast<CDBGroupListSheetText *>(
            pCaller->getParent()->getParent());
    if (!listSheetTrade)
      return;
    ...
    if (!listSheetTrade->_CtrlInfo._ListMenuRight.empty()) {
      if (CDBCtrlSheet::getDraggedSheet() == NULL) {
        CWidgetManager::getInstance()->enableModalWindow(
            ctrlSheet, listSheetTrade->_CtrlInfo._ListMenuRight);
      }
    }
  }
};
...
REGISTER_ACTION_HANDLER(CHandlerListSheetTextResetSelection,
