...
static void fillSabrinaPhraseListBrick(const CSPhraseCom &phrase,
                                       IListSheetBase *listBrick);
...
static void setupSabrinaBrickHelp(CSheetHelpSetup &setup,
                                  bool auraDisabled = false);
void setupSabrinaPhraseHelp(CSheetHelpSetup &setup,
                            const class CSPhraseCom &phrase,
...
CInterfaceHelp::CFittedWeaponWeightObserver
    CInterfaceHelp::_FittedWeaponWeightObserver;
...
// ***************************************************************************
void CInterfaceHelp::CInfoWindow::missionInfoReceived(
...
void CInterfaceHelp::initWindows() {
  ...
  sint maxHelpWindow;
  fromString(
      CWidgetManager::getInstance()->getParser()->getDefine("MAX_HELP_WINDOW"),
      maxHelpWindow);
  ...
  for (sint i = 0; i < maxHelpWindow; i++) {
    CInterfaceGroup *group = dynamic_cast<CInterfaceGroup *>(
        CWidgetManager::getInstance()->getElementFromId(
            "ui:interface:sheet_help" + toString(i)));
  ...
  // add observers for the update of phrase help texts (depends of weight of
  // equipped weapons)
  for (uint i = 0; i < MAX_HANDINV_ENTRIES; ++i) {
    CCDBNodeLeaf *pNodeLeaf = NLGUI::CDBManager::getInstance()->getDbProp(
        string(LOCAL_INVENTORY) + ":HAND:" + toString(i), false);
    if (pNodeLeaf) {
...
// ***************************************************************************
CInterfaceGroup *CInterfaceHelp::activateNextWindow(CDBCtrlSheet *elt,
                                                    sint forceKeepWindow) {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  ...
  for (i = 0; i < _ActiveWindows.size(); i++) {
    ...
    CDBCtrlSheet *ctrlDst =
        dynamic_cast<CDBCtrlSheet *>(group->getCtrl(":ctrl_slot"));
    if (ctrlDst && ctrlSrc) {
      // if same Aspect
      if (ctrlSrc->sameAspect(ctrlDst)) {
        bool ok = true;

        // for items, must also test if they have the same itemSlotId, cause
        // relies also on "ItemInfo system"
        if (elt->getType() == CCtrlSheetInfo::SheetType_Item) {
          ...
          if (!realCtrlDst)
            ok = false;
          else if (getInventory().getItemSlotId(ctrlSrc) !=
                   getInventory().getItemSlotId(realCtrlDst))
            ok = false;
        }
  ...
  for (i = 0; i < _ActiveWindows.size(); i++) {
    ...
    if (!_InfoWindows[_ActiveWindows[i]].KeepMode &&
        forceKeepWindow != (sint)_ActiveWindows[i]) {
  ...
  if (newIndexWindow == -1) {
    if (_ActiveWindows.size() < (uint)maxHelpWindow) {
      ...
      // All the info window are opened (and should not be in KeepMode....),
      // take the last recently opened.
  ...
  CInterfaceElement *ctrl =
      group->getElement(group->getId() + ":content:ctrl_slot");
  if (ctrl)
    ctrl->setActive(showSlotAndCreator);
  ...
  if (elt) {
    if (elt->getType() == CCtrlSheetInfo::SheetType_Item) {
      ...
      _InfoWindows[newIndexWindow].ItemSlotId =
          getInventory().getItemSlotId(elt);
      ...
      CBotChatManager::getInstance()->addMissionInfoWaiter(
          &_InfoWindows[newIndexWindow]);
    }
  }
  ...
  void execute(CCtrlBase *pCaller, const string &sParams) {
    ...
    if (cs != NULL && cs->getSheetId() != 0) {
      ...
      CInterfaceGroup *group =
          CInterfaceHelp::activateNextWindow(cs, forceKeepWindow);
      if (!group)
        return;
      CSheetHelpSetup setup;
      setup.setupDefaultIDs();
      setup.HelpWindow = group;
      setup.SrcSheet = cs;
      setup.DestSheet = dynamic_cast<CDBCtrlSheet *>(
          group->getElement(group->getId() + ":content:ctrl_slot"));
      setupItemHelp(setup);
    }
  }
  ...
  void execute(CCtrlBase *pCaller, const string &sParams) {
    ...
    if (getParam(sParams, "from") == "contact") {
      ...
      CPeopleList *peopleList =
          PeopleInterraction.getPeopleListFromContainerID(str);
      if (peopleList == NULL)
        return;
      ...
      if (!name.empty()) {
        CAHManager::getInstance()->runActionHandler(
            "show_hide", pCaller,
            "profile|pname=" + name +
                "|ptype=" + toString((int)CEntityCL::Player));
      }
      return;
      ...
      // if(selection->isNPC())
      {
          ...
          if (nDBid != 0) {
            ...
            if (name.empty()) {
              ...
              if (!strNewTitle.empty())
                name =
                    STRING_MANAGER::CStringManagerClient::getTitleLocalizedName(
                        strNewTitle, woman);
              else
                name.clear();
            }
        ...
        if (!name.empty())
          CAHManager::getInstance()->runActionHandler(
              "show_hide", pCaller,
              "profile|pname=" + name +
                  "|ptype=" + toString((int)selection->Type));
        return;
      }
    }
#endif
    ...
    if (sFrom == "target") {
      ...
      name =
          CEntityCL::removeTitleAndShardFromName(UserEntity->getEntityName());
    ...
    // -----------------------------------------
    CUnblockTitlesSheet *pUTS = dynamic_cast<CUnblockTitlesSheet *>(
        SheetMngr.get(CSheetId("unblock.titles")));
    if (pUTS == NULL) {
    ...
    uint titleIDnb;
    for (titleIDnb = 0; titleIDnb < CHARACTER_TITLE::NB_CHARACTER_TITLE;
         ++titleIDnb) {
      bool women = UserEntity && UserEntity->getGender() == GSGENDER::female;
      if (CStringManagerClient::getTitleLocalizedName(
              CHARACTER_TITLE::toString(
                  (CHARACTER_TITLE::ECharacterTitle)titleIDnb),
              women) == title)
        break;
    }
        ...
        for (uint i = 0; i < pTU->SkillsNeeded.size(); ++i) {
          for (uint j = 0; j < pTU->SkillsNeeded[i].size(); ++j) {
            ...
            for (skillNb = 0; skillNb < SKILLS::NUM_SKILLS; ++skillNb)
              if (pTU->SkillsNeeded[i][j] ==
                  SKILLS::toString((SKILLS::ESkills)skillNb))
                break;

            if (skillNb != SKILLS::NUM_SKILLS) {
              sSkillsNeeded += CStringManagerClient::getSkillLocalizedName(
                  (SKILLS::ESkills)skillNb);
              sSkillsNeeded +=
                  "  (" + toString(pTU->SkillsLevelNeeded[i][j]) + ")";
              sSkillsNeeded += "\n";
            } else {
              nlwarning("cant find skill : %s",
                        pTU->SkillsNeeded[i][j].c_str());
            }
          }
    ...
    if (pTU != NULL) {
      ...
      if (pTU->BricksNeeded.empty() || reservedTitle) {
        ...
        IListSheetBase *listBrick =
            dynamic_cast<IListSheetBase *>(group->getElement(listID));
        if (listBrick) {
          CSPhraseCom phrase;
...
REGISTER_ACTION_HANDLER(CHandlerOpenSkillToTradeHelp,
                        "open_skill_to_trade_help");
  ...
  void execute(CCtrlBase *pCaller, const string &sParams) {
    ...
    if (strncmp(urls.c_str(), "ah:", 3) == 0) {
      ...
      while (start < end) {
        ...
        // Replace %HH encoding with ASCII values (AFTER '&' replacing, to
        // possibly have reals '&')
        for (uint i = 0; i < params.size(); i++) {
          if (params[i] == '%' && i < params.size() - 2) {
            if (isxdigit(params[i + 1]) &&
                isxdigit(params[i + 2])) // FIXME: Locale dependent
            {
        ...
        CAHManager::getInstance()->runActionHandler(action, elementGroup,
                                                    params);
      ...
      if (element) {
        ...
        if (groupHtml) {
          if (show) {
            ...
            while (parent) {
              if (parent->getParent() &&
                  (parent->getParent()->getId() == "ui:interface")) {
          ...
          // localize if wanted
          if (localizePage)
            strFindReplace(urls, "_wk.",
                           string("_") + ClientCfg.getHtmlLanguageCode() + ".");
...
public:
  void execute(CCtrlBase *pCaller, const string &sParams) {
    CInterfaceManager *pIM = CInterfaceManager::getInstance();
    string container = getParam(sParams, "name");
    CGroupHTML *groupHtml = dynamic_cast<CGroupHTML *>(
        CWidgetManager::getInstance()->getElementFromId(container));
    if (groupHtml) {
...
class CHandlerHTMLSubmitForm : public IActionHandler {
  void execute(CCtrlBase *pCaller, const string &sParams) {
    ...
    if (!fromString(getParam(sParams, "button"), button)) {
      nlwarning("Invalid button index: '%s', expected integer",
                getParam(sParams, "button").c_str());
      return;
    }
...
void setHelpText(CSheetHelpSetup &setup, const string &text) {
  ...
  if (!setup.HelpWindow)
    return;
  CViewText *viewText =
      dynamic_cast<CViewText *>(setup.HelpWindow->getView(setup.ViewText));
  if (viewText) {
    viewText->setTextFormatTaged(copyStr);
  }
  CInterfaceGroup *viewTextGroup =
      setup.HelpWindow->getGroup(setup.ScrollTextGroup);
  if (viewTextGroup)
...
void setHelpTextID(CSheetHelpSetup &setup, sint32 id) {
  if (!setup.HelpWindow)
    return;
  CViewTextID *viewTextID =
      dynamic_cast<CViewTextID *>(setup.HelpWindow->getView(setup.ViewTextID));
  if (viewTextID) {
  ...
  CInterfaceGroup *viewTextGroup =
      setup.HelpWindow->getGroup(setup.ScrollTextIdGroup);
  if (viewTextGroup)
    viewTextGroup->setActive(true);
}
...
static void setupSkillToTradeHelp(CSheetHelpSetup &setup) {
  ...
  //		{
  //			string dbPath =
  //toString("CHARACTER_INFO:CAREER%d:JOB%d:JOB_CAP", (int) career, (int) job);
  //			uint level = (uint)
  //NLGUI::CDBManager::getInstance()->getDbProp(dbPath)->getValue32(); 			if (level
  //!= 0) // has the player this job ?
  //			{
  //				// check if level in this job is enough to get
  //the skills
  //				// TODO finish it
  //				/*
  //				JOBS::TJob job = JOBS::getJobFromDBIndex(career,
  //job); 				if (job != JOBS::Unknown)
  //				{
  //				}*/
  //			}
  //		}
...
void getItemDefenseText(CDBCtrlSheet *item, string &itemText) {
  ...
  const CClientItemInfo &itemInfo =
      getInventory().getItemInfo(getInventory().getItemSlotId(item));
  ...
  strFindReplace(itemText, "%protect",
                 toPercentageText(itemInfo.ProtectionFactor));
  strFindReplace(itemText, "%p_slash",
                 toString(itemInfo.MaxSlashingProtection));
  strFindReplace(itemText, "%p_pierce",
                 toString(itemInfo.MaxPiercingProtection));
  strFindReplace(itemText, "%p_blunt", toString(itemInfo.MaxBluntProtection));
}

void getDamageText(CDBCtrlSheet *item, const CItemSheet *pIS, string &itemText,
                   bool displayAsMod) {
  // retrieve the current itemInfo
  const CClientItemInfo &itemInfo =
      getInventory().getItemInfo(getInventory().getItemSlotId(item));
  ...
  if (pIS->Family != ITEMFAMILY::AMMO) {
    ...
    strFindReplace(itemText, "%adv_dodge",
                   toString(itemInfo.AdversaryDodgeModifier));
    strFindReplace(itemText, "%adv_parry",
                   toString(itemInfo.AdversaryParryModifier));
  }
}
...
void getSpeedText(CDBCtrlSheet *item, string &itemText, bool displayAsMod) {
  ...
  strFindReplace(itemText, "%speed",
                 strMod + toReadableFloat(itemInfo.HitRate));
}
...
void getRangeText(CDBCtrlSheet *item, string &itemText, bool displayAsMod) {
  ...
  strFindReplace(itemText, "%range",
                 strMod + toReadableFloat(itemInfo.Range / 1000.f));
}

void getHPAndSapLoadText(CDBCtrlSheet *item, const CItemSheet *pIS,
                         string &itemText) {
  // retrieve the current itemInfo
  const CClientItemInfo &itemInfo =
      getInventory().getItemInfo(getInventory().getItemSlotId(item));
...
void getBuffText(CDBCtrlSheet *item, string &itemText) {
  ...
  sint32 vals[] = {itemInfo.HpBuff, itemInfo.SapBuff, itemInfo.StaBuff,
                   itemInfo.FocusBuff};
  ...
  for (uint i = 0; i < numVals; i++) {
    ...
    if (modifier != 0) {
      string line = CI18N::get("uihelpItem" + valIds[i] +
...
void getMagicProtection(CDBCtrlSheet *item, string &itemText) {
  ...
  for (uint i = 0; i < CClientItemInfo::MaxMagicProtectionByJewel; i++) {
    if (itemInfo.MagicProtection[i] != PROTECTION_TYPE::None) {
      ...
      strFindReplace(str, "%t",
                     CI18N::get("pt" + PROTECTION_TYPE::toString(
                                           itemInfo.MagicProtection[i])));
  ...
  // add Max damage absorbed
  {
    ...
    CCDBNodeLeaf *nodeFactor = NLGUI::CDBManager::getInstance()->getDbProp(
...
void getMagicResistance(CDBCtrlSheet *item, string &itemText) {
  ...
  for (uint i = 0; i < RESISTANCE_TYPE::NB_RESISTANCE_TYPE; i++) {
    if (resist[i] != 0) {
      ...
      strFindReplace(
          str, "%t",
          CI18N::get("rs" + RESISTANCE_TYPE::toString(
                                (RESISTANCE_TYPE::TResistanceType)i)));
...
void getActionMalus(CDBCtrlSheet *item, string &itemText) {
  ...
  strFindReplace(itemText, "%actmalus",
                 toPercentageText(itemInfo.WearEquipmentMalus));
}

void getBulkText(CDBCtrlSheet *item, const CItemSheet *pIS, string &itemText) {
  // Display direct value: because cannot know where this item will be drop!!
  // (bag, mektoub etc...)
  float slotBulkTotal = max((sint32)1, item->getQuantity()) * pIS->Bulk;

  // If stackable and bulk not 0, display in form "1 (10)". where (10) is the
  // total of quantity*bulk
  if (pIS->Stackable > 1 && pIS->Bulk > 0)
    strFindReplace(itemText, "%bulk",
                   toString("%.2f  (%.2f)", pIS->Bulk, slotBulkTotal));
  // else simple form
  else
    strFindReplace(itemText, "%bulk", toString("%.2f", slotBulkTotal));
}

void getWeightText(CDBCtrlSheet *item, const CItemSheet *pIS,
                   string &itemText) {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();

  CCDBNodeLeaf *pWG = NLGUI::CDBManager::getInstance()->getDbProp(
      item->getSheet() + ":WEIGHT", false);
  if (pWG) {
    ...
    // if stackable and weight not 0, display in form "1 (10)". where (10) is
    // the total of quantity*weight
    if (pIS->Stackable > 1 && slotWeight > 0) {
      string sws;
      string swt;
      if ((slotWeight % DB_WEIGHT_SCALE) == 0)
        sws = toString("%d", slotWeight / DB_WEIGHT_SCALE);
      else
        sws = toString("%.2f", float(slotWeight) / DB_WEIGHT_SCALE);
      if ((slotWeightTotal % DB_WEIGHT_SCALE) == 0)
        swt = toString("%d", slotWeightTotal / DB_WEIGHT_SCALE);
      else
        swt = toString("%.2f", float(slotWeightTotal) / DB_WEIGHT_SCALE);

      // combine
      strFindReplace(itemText, "%weight",
                     toString("%s  (%s)", sws.c_str(), swt.c_str()));
    }
    // else display in simple form
    else {
      if ((slotWeightTotal % DB_WEIGHT_SCALE) == 0)
        strFindReplace(itemText, "%weight",
                       toString("%d", slotWeightTotal / DB_WEIGHT_SCALE));
      else
        strFindReplace(
            itemText, "%weight",
            toString("%.2f", float(slotWeightTotal) / DB_WEIGHT_SCALE));
    }
  } else
    strFindReplace(itemText, "%weight", "???");
}
...
void getMagicBonus(CDBCtrlSheet *item, string &itemText) {
  ...
  const string valIds[CClientItemInfo::NumMagicFactorType] = {
      "OffElemental", "OffAffliction", "DefHeal", "DefAffliction"};
  string mbInfo;
  ...
  for (uint i = 1; i < CClientItemInfo::NumMagicFactorType; i++) {
    ...
    if (cs != allCastSpeedFactor || mp != allMagicPowerFactor) {
      equal = false;
      break;
    }
  }
...
void getItemRequirementText(CDBCtrlSheet *item, const CItemSheet *pIS,
                            string &itemText) {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  ...
  if (itemInfo.RequiredCharac != CHARACTERISTICS::Unknown &&
      itemInfo.RequiredCharacLevel != 0) {
    // Build the req string
    if (pIM->isItemCaracRequirementMet(itemInfo.RequiredCharac,
                                       (sint32)itemInfo.RequiredCharacLevel))
    ...
    strFindReplace(
        fmtc, "%s",
        CI18N::get(toString("uiCaracId%d", (uint)itemInfo.RequiredCharac)));
  ...
  if (itemInfo.RequiredSkillLevel > 0) {
    if (itemInfo.RequiredSkill != SKILLS::unknown) {
      if (CSkillManager::getInstance()->checkBaseSkillMetRequirement(
              itemInfo.RequiredSkill, itemInfo.RequiredSkillLevel))
      ...
      const char *skillName =
          STRING_MANAGER::CStringManagerClient::getSkillLocalizedName(
              itemInfo.RequiredSkill);
      strFindReplace(fmt, "%s", skillName);
    } else {
      if (CSkillManager::getInstance()->checkBaseSkillMetRequirement(
              SKILLS::unknown, itemInfo.RequiredSkillLevel))
  ...
  if (itemInfo.RequiredSkillLevel2 > 0) {
    if (itemInfo.RequiredSkill2 != SKILLS::unknown) {
      if (CSkillManager::getInstance()->checkBaseSkillMetRequirement(
              itemInfo.RequiredSkill2, itemInfo.RequiredSkillLevel2))
        fmt = CI18N::get("uihelpItemSkillReqMetFmt");
      else
        fmt = CI18N::get("uihelpItemSkillReqNotMetFmt");
      ...
      const char *skillName =
          STRING_MANAGER::CStringManagerClient::getSkillLocalizedName(
              itemInfo.RequiredSkill2);
      strFindReplace(fmt, "%s", skillName);
    } else {
      if (CSkillManager::getInstance()->checkBaseSkillMetRequirement(
              SKILLS::unknown, itemInfo.RequiredSkillLevel2))
        fmt = CI18N::get("uihelpItemAnySkillReqMetFmt");
      else
        fmt = CI18N::get("uihelpItemAnySkillReqNotMetFmt");
...
void getSkillModVsType(CDBCtrlSheet *item, const CItemSheet *pIS,
                       string &itemText) {
  // retrieve the current itemInfo
  const CClientItemInfo &itemInfo =
      getInventory().getItemInfo(getInventory().getItemSlotId(item));
  ...
  if (!itemInfo.TypeSkillMods.empty()) {
    for (uint i = 0; i < itemInfo.TypeSkillMods.size(); ++i) {
      EGSPD::CClassificationType::TClassificationType type =
          itemInfo.TypeSkillMods[i].Type;

      sMod = CI18N::get("uihelpSkillModVsType");
      strFindReplace(sMod, "%mod",
                     NLMISC::toString(itemInfo.TypeSkillMods[i].Modifier));
      strFindReplace(
          sMod, "%type",
          CStringManagerClient::getClassificationTypeLocalizedName(type));
      sMod += "\n\n";
...
void getArmorBonus(CDBCtrlSheet *item, string &itemText,
                   const CItemSheet *pIS) {
  string armor_bonus;
  sint32 level = 0;
  ...
  if (pIS->Armor.ArmorType == ARMORTYPE::HEAVY ||
      pIS->Armor.ArmorType == ARMORTYPE::MEDIUM)
    armor_bonus = "@{FFFF}(+@{2F2F}" + toString(level) + " @{FFFF}" +
                  CI18N::get("uiHP") + ")";
...
void getItemText(CDBCtrlSheet *item, string &itemText, const CItemSheet *pIS) {
  ...
  case ITEMFAMILY::ARMOR:
    itemText = CI18N::get("uihelpItemArmorFormat");
    break;
  case ITEMFAMILY::MELEE_WEAPON:
    itemText = CI18N::get("uihelpItemMeleeWeaponFormat");
    break;
  case ITEMFAMILY::RANGE_WEAPON:
    itemText = CI18N::get("uihelpItemRangeWeaponFormat");
    break;
  case ITEMFAMILY::AMMO:
    itemText = CI18N::get("uihelpItemAmmoFormat");
    break;
  case ITEMFAMILY::RAW_MATERIAL:
    itemText = CI18N::get("uihelpItemMPFormat");
    break;
  case ITEMFAMILY::SHIELD:
    itemText = CI18N::get("uihelpItemShieldFormat");
    break;
  case ITEMFAMILY::CRAFTING_TOOL:
    itemText = CI18N::get("uihelpItemCraftingToolFormat");
    break;
  case ITEMFAMILY::HARVEST_TOOL:
    itemText = CI18N::get("uihelpItemHarvestToolFormat");
    break;
  case ITEMFAMILY::TAMING_TOOL:
    itemText = CI18N::get("uihelpItemTamingToolFormat");
    break;
  case ITEMFAMILY::JEWELRY:
    itemText = CI18N::get("uihelpItemJewelFormat");
    break;
  case ITEMFAMILY::CRYSTALLIZED_SPELL:
    itemText = CI18N::get("uihelpItemCrystalSpell");
    break;
  case ITEMFAMILY::ITEM_SAP_RECHARGE:
    itemText = CI18N::get("uihelpItemChargeSpell");
    break;
  case ITEMFAMILY::PET_ANIMAL_TICKET:
    itemText = CI18N::get("uihelpItemAnimal");
    break;
  case ITEMFAMILY::TELEPORT:
    itemText = CI18N::get("uihelpItemTeleport");
    break;
  case ITEMFAMILY::COSMETIC:
    itemText = CI18N::get("uihelpItemCosmetic");
    break;
  case ITEMFAMILY::SCROLL:
    itemText = CI18N::get("uihelpItemScroll");
    break;
  case ITEMFAMILY::SCROLL_R2:
    itemText = CI18N::get("uihelpItemScrollR2");
    break;
  case ITEMFAMILY::CONSUMABLE:
    itemText = CI18N::get("uihelpItemConsumableFormat");
    break;
  default:
    itemText = CI18N::get("uihelpItemDefaultFormat");
  };

  // *** Replace Common part
  strFindReplace(itemText, "%origin",
                 CI18N::get("io" + ITEM_ORIGIN::enumToString(pIS->ItemOrigin)));
  ...
  // Get Item effect
  CItemSpecialEffectHelper::getInstance()->getItemSpecialEffectText(pIS,
                                                                    itemText);

  // Description
  const char *desc = CStringManagerClient::getItemLocalizedDescription(pIS->Id);
  if (*desc) {
    strFindReplace(itemText, "%desc",
                   "@{FFF9}" + CI18N::get("uiMissionDesc") + "\n@{FFFF}" +
                       desc + "\n");
  } else
  ...
  if (pIS->Family == ITEMFAMILY::COSMETIC) {
    EGSPD::CPeople::TPeople people = ITEM_ORIGIN::itemOriginStringToPeopleEnum(
        ITEM_ORIGIN::enumToString(pIS->ItemOrigin));
    if (UserEntity->getGender() != pIS->Cosmetic.Gender ||
        UserEntity->people() != people)
      strFindReplace(itemText, "%cansell",
                     CI18N::get("uihelpItemCosmeticDontFit"));
    else
  ...
  case ITEMFAMILY::ARMOR: {
    strFindReplace(
        itemText, "%armor",
        CI18N::get("at" + ARMORTYPE::toString(pIS->Armor.ArmorType)));
  ...
  case ITEMFAMILY::MELEE_WEAPON: {
    strFindReplace(
        itemText, "%skill",
        CStringManagerClient::getSkillLocalizedName(pIS->MeleeWeapon.Skill));
    strFindReplace(
        itemText, "%cat",
        CI18N::get("wt" + WEAPONTYPE::toString(pIS->MeleeWeapon.WeaponType)));
    strFindReplace(
        itemText, "%dmtype",
        CI18N::get("dt" + DMGTYPE::toString(pIS->MeleeWeapon.DamageType)));
  ...
  case ITEMFAMILY::RANGE_WEAPON: {
    strFindReplace(
        itemText, "%skill",
        CStringManagerClient::getSkillLocalizedName(pIS->RangeWeapon.Skill));
    strFindReplace(
        itemText, "%cat",
        CI18N::get("wt" + WEAPONTYPE::toString(pIS->RangeWeapon.WeaponType)));
    // Damage / Speed / Range
  ...
  case ITEMFAMILY::AMMO: {
    ...
    strFindReplace(
        itemText, "%skill",
        CStringManagerClient::getSkillLocalizedName(pIS->Ammo.Skill));
    strFindReplace(itemText, "%dmtype",
                   CI18N::get("dt" + DMGTYPE::toString(pIS->Ammo.DamageType)));
    // WARNING: here 999 is hardcoded because of the new global limit of stacks
    // (999), Ammo.Magazine should not be used anymore
    if (item->getUseQuantity())
      strFindReplace(itemText, "%magazine",
                     toString(item->getQuantity()) + " / " +
                         toString(999 /*pIS->Ammo.Magazine*/));
    else
      strFindReplace(itemText, "%magazine",
                     toString(999 /*pIS->Ammo.Magazine*/));
  ...
  case ITEMFAMILY::RAW_MATERIAL: {
    ...
    strFindReplace(
        itemText, "%ecosystem",
        CI18N::get("ecosys" + ECOSYSTEM::toString(pIS->Mp.Ecosystem)));
    strFindReplace(itemText, "%family",
                   RM_FAMILY::toLocalString(pIS->Mp.Family));
    strFindReplace(
        itemText, "%skill",
        CStringManagerClient::getSkillLocalizedName(pIS->Mp.HarvestSkill));

    // MpColor
    if (RM_COLOR::validColor(pIS->Mp.MpColor))
      strFindReplace(itemText, "%mpcolor",
                     RM_COLOR::toLocalString(pIS->Mp.MpColor));
    else
      strFindReplace(itemText, "%mpcolor", "???");
    ...
    // Craft Mp requirement?
      ...
      strFindReplace(itemText, "%craft",
                     CI18N::get("uihelpItemMPCraftRequirement"));
    // No Craft at all
    else
      strFindReplace(itemText, "%craft", CI18N::get("uihelpItemMPNoCraft"));
  } break;
  case ITEMFAMILY::SHIELD: {
    // strFindReplace(itemText, "%skill",
    // CStringManagerClient::getSkillLocalizedName(SHIELDTYPE::shieldTypeToSkill(pIS->Shield.ShieldType))
    // );
    strFindReplace(
        itemText, "%cat",
        CI18N::get("st" + SHIELDTYPE::toString(pIS->Shield.ShieldType)));
    // Protection
    getItemDefenseText(item, itemText);
    // Player buffs
    getBuffText(item, itemText);
  ...
  case ITEMFAMILY::CRAFTING_TOOL: {
    strFindReplace(
        itemText, "%tool",
        CI18N::get("tool" + TOOL_TYPE::toString(pIS->Tool.CraftingToolType)));
    // Player buffs
    getBuffText(item, itemText);
  } break;
  case ITEMFAMILY::HARVEST_TOOL: {
    strFindReplace(
        itemText, "%skill",
        CStringManagerClient::getSkillLocalizedName(pIS->Tool.Skill));
    // Player buffs
    getBuffText(item, itemText);
  } break;
  case ITEMFAMILY::TAMING_TOOL: {
  ...
  case ITEMFAMILY::CONSUMABLE: {
    strFindReplace(itemText, "%consumption_time",
                   toString(pIS->Consumable.ConsumptionTime));
    strFindReplace(itemText, "%overdose_timer_min",
                   toString(pIS->Consumable.OverdoseTimer / 60));
    strFindReplace(itemText, "%overdose_timer_sec",
                   toString(pIS->Consumable.OverdoseTimer % 60));
    // Get Item Consumable infos
    CItemConsumableEffectHelper::getInstance()->getItemConsumableEffectText(
        pIS, itemText, item->getQuality());
  } break;
  case ITEMFAMILY::SCROLL_R2: {
    strFindReplace(itemText, "%r2_description_text",
                   toString(itemInfo.R2ItemDescription));
    strFindReplace(itemText, "%r2_comment_text",
                   toString(itemInfo.R2ItemComment));
  } break;
  case ITEMFAMILY::PET_ANIMAL_TICKET: {
#ifdef RYZOM_FORGE
    string nr = (itemInfo.PetNumber > 0)
                    ? toString(itemInfo.PetNumber)
                    : "(slot)" + toString(item->getIndexInDB());
    strFindReplace(itemText, "%petnumber", nr);
#endif
  } break;
  default: {
    strFindReplace(itemText, "%no_rent",
                   pIS->IsItemNoRent ? CI18N::get("uihelpItemNoRent")
                                     : string(""));
    strFindReplace(itemText, "%descnr",
                   pIS->IsItemNoRent ? CI18N::get("uihelpItemNoRentDesc")
                                     : string(""));
  } break;
  };
  ...
  INVENTORIES::TInventory inventory =
      (INVENTORIES::TInventory)item->getInventoryIndex();
  sint32 slot = item->getIndexInDB();
  string debugText =
      NLMISC::toString("inventory: %s\nslot: %d\n",
...
static void setupEnchantedItem(CSheetHelpSetup &setup, string &itemText) {
  ...
  if (!ctrl || !group)
    return;
  const CClientItemInfo &itemInfo =
      getInventory().getItemInfo(getInventory().getItemSlotId(ctrl));
  IListSheetBase *listBrick = dynamic_cast<IListSheetBase *>(group->getElement(
      group->getId() + setup.PrefixForExtra + INFO_LIST_BRICK));
  ...
  if (!itemInfo.Enchantment.empty()) {
    ...
    if (pIS && pIS->Family == ITEMFAMILY::CRYSTALLIZED_SPELL)
      pPM->buildPhraseDesc(enchantInfo, itemInfo.Enchantment, 0, false,
                           "uihelpPhraseCrystalSpellFormat");
    else
      pPM->buildPhraseDesc(enchantInfo, itemInfo.Enchantment, 0, false,
...
static void setupRawMaterialStats(CSheetHelpSetup &setup) {
  ...
  CInterfaceGroup *groupMp = dynamic_cast<CInterfaceGroup *>(group->getElement(
      group->getId() + setup.PrefixForExtra + INFO_GROUP_MP_STAT));
  if (!groupMp)
    return;
  ...
  if (pIS && pIS->Family == ITEMFAMILY::RAW_MATERIAL) {
    ...
    if (pIS->canBuildSomeItemPart()) {
      ...
      CDBGroupComboBox *pCB = dynamic_cast<CDBGroupComboBox *>(
          groupMp->getElement(groupMp->getId() + ":item_part_choice"));
      if (pCB) {
        pCB->resetTexts();
...
void resetSheetHelp(CSheetHelpSetup &setup) {
  ...
  IListSheetBase *listItem = dynamic_cast<IListSheetBase *>(group->getElement(
      group->getId() + setup.PrefixForExtra + INFO_LIST_ITEM));
  if (listItem) {
    listItem->setActive(false);
  }

  // Hide the item preview by default
  CInterfaceElement *elt = group->getElement(
      group->getId() + setup.PrefixForExtra + INFO_ITEM_PREVIEW);
  if (elt) {
    elt->setActive(false);
  }
  ...
  listBrick = dynamic_cast<IListSheetBase *>(group->getElement(
      group->getId() + setup.PrefixForExtra + INFO_LIST_BRICK_REQUIREMENT));
  if (listBrick) {
    listBrick->setActive(false);
  }
  // Hide the listBrick header by default
  CViewText *view = dynamic_cast<CViewText *>(group->getElement(
      group->getId() + setup.PrefixForExtra + INFO_LIST_BRICK_HEADER));
  if (view) {
    view->setActive(false);
  }

  CInterfaceGroup *groupCosmetic =
      dynamic_cast<CInterfaceGroup *>(group->getElement(
          group->getId() + setup.PrefixForExtra + INFO_GROUP_CHAR_3D));
  if (groupCosmetic)
...
static void setupItemHelp(CSheetHelpSetup &setup) {
  ...
  // get the calling item
  if (!setup.SrcSheet ||
      setup.SrcSheet->getType() != CCtrlSheetInfo::SheetType_Item) {
  ...
  // NB: for raw materials only, must do each once only, must not do it at
  // refresh, cause combo reseted
...
void setupCosmetic(CSheetHelpSetup &setup, CItemSheet *pIS) {
  ...
  if (!(UserEntity->getGender() != pIS->Cosmetic.Gender ||
        UserEntity->people() != people)) {
    CInterfaceGroup *groupCosmetic =
        dynamic_cast<CInterfaceGroup *>(setup.HelpWindow->getElement(
            setup.HelpWindow->getId() + setup.PrefixForExtra +
            INFO_GROUP_CHAR_3D));
    if (groupCosmetic)
    ...
    cs.VisualPropA.PropertySubData.HatModel =
        SCharacter3DSetup::getDB("SERVER:USER:HAIR_TYPE");
    cs.VisualPropA.PropertySubData.HatColor =
        SCharacter3DSetup::getDB("SERVER:USER:HAIR_COLOR");
    if (pIS->ItemType == ITEM_TYPE::HAIR_MALE ||
        pIS->ItemType == ITEM_TYPE::HAIR_FEMALE)
      cs.VisualPropA.PropertySubData.HatModel =
          CVisualSlotManager::getInstance()->sheet2Index(
              CSheetId(setup.SrcSheet->getSheetId()), SLOTTYPE::HEAD_SLOT);
    else if (pIS->ItemType == ITEM_TYPE::TATOO_MALE ||
             pIS->ItemType == ITEM_TYPE::TATOO_FEMALE)
      cs.VisualPropC.PropertySubData.Tattoo = pIS->Cosmetic.VPValue;
    else if (pIS->ItemType == ITEM_TYPE::HAIRCOLOR_MALE ||
             pIS->ItemType == ITEM_TYPE::HAIRCOLOR_FEMALE)
      cs.VisualPropA.PropertySubData.HatColor = pIS->Cosmetic.VPValue;
    else
      nlwarning("<setupItemHelp> Invalid cosmetic item type '%s'",
                ITEM_TYPE::toString(pIS->ItemType).c_str());
...
void setupItemPreview(CSheetHelpSetup &setup, CItemSheet *pIS) {
  ...
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  CCDBNodeBranch *dbBranch =
      NLGUI::CDBManager::getInstance()->getDbBranch(setup.SrcSheet->getSheet());

  CInterfaceElement *elt = setup.HelpWindow->getElement(
      setup.HelpWindow->getId() + setup.PrefixForExtra + INFO_ITEM_PREVIEW);
  if (elt == NULL)
    return;
  ...
  bool scene_inactive = !NLGUI::CDBManager::getInstance()
                             ->getDbProp("UI:SAVE:SHOW_3D_ITEM_PREVIEW")
                             ->getValueBool();
  if (scene_inactive || (pIS->Family != ITEMFAMILY::ARMOR &&
  ...
  CInterface3DScene *sceneI =
      dynamic_cast<CInterface3DScene *>(ig->getGroup("scene_item_preview"));
  if (!sceneI) {
  ...
  if (pIS->Family == ITEMFAMILY::ARMOR) {
    if (pIS->ItemType == ITEM_TYPE::LIGHT_BOOTS ||
        pIS->ItemType == ITEM_TYPE::MEDIUM_BOOTS ||
        pIS->ItemType == ITEM_TYPE::HEAVY_BOOTS) {
      CCDBNodeLeaf *color =
          dbBranch->getLeaf(setup.SrcSheet->getSheet() + ":USER_COLOR", false);
      cs.VisualPropB.PropertySubData.FeetModel =
          CVisualSlotManager::getInstance()->sheet2Index(
              CSheetId(setup.SrcSheet->getSheetId()), SLOTTYPE::FEET_SLOT);
      cs.VisualPropB.PropertySubData.FeetColor = color->getValue32();
      SCharacter3DSetup::setupDBFromCharacterSummary("UI:TEMP:CHAR3D", cs);
      camHeight = -1.15f;
    } else if (pIS->ItemType == ITEM_TYPE::LIGHT_GLOVES ||
               pIS->ItemType == ITEM_TYPE::MEDIUM_GLOVES ||
               pIS->ItemType == ITEM_TYPE::HEAVY_GLOVES) {
      CCDBNodeLeaf *color =
          dbBranch->getLeaf(setup.SrcSheet->getSheet() + ":USER_COLOR", false);
      cs.VisualPropB.PropertySubData.HandsModel =
          CVisualSlotManager::getInstance()->sheet2Index(
              CSheetId(setup.SrcSheet->getSheetId()), SLOTTYPE::HANDS_SLOT);
      cs.VisualPropB.PropertySubData.HandsColor = color->getValue32();
      SCharacter3DSetup::setupDBFromCharacterSummary("UI:TEMP:CHAR3D", cs);
      // cs.VisualPropB.PropertySubData.HandsColor = pIS->Color;
    } else if (pIS->ItemType == ITEM_TYPE::LIGHT_SLEEVES ||
               pIS->ItemType == ITEM_TYPE::MEDIUM_SLEEVES ||
               pIS->ItemType == ITEM_TYPE::HEAVY_SLEEVES) {
      CCDBNodeLeaf *color =
          dbBranch->getLeaf(setup.SrcSheet->getSheet() + ":USER_COLOR", false);
      cs.VisualPropA.PropertySubData.ArmModel =
          CVisualSlotManager::getInstance()->sheet2Index(
              CSheetId(setup.SrcSheet->getSheetId()), SLOTTYPE::ARMS_SLOT);
      cs.VisualPropA.PropertySubData.ArmColor = color->getValue32();
      SCharacter3DSetup::setupDBFromCharacterSummary("UI:TEMP:CHAR3D", cs);
      // cs.VisualPropA.PropertySubData.ArmColor = pIS->Color;
      camHeight = -0.55f;
    } else if (pIS->ItemType == ITEM_TYPE::LIGHT_PANTS ||
               pIS->ItemType == ITEM_TYPE::MEDIUM_PANTS ||
               pIS->ItemType == ITEM_TYPE::HEAVY_PANTS) {
      CCDBNodeLeaf *color =
          dbBranch->getLeaf(setup.SrcSheet->getSheet() + ":USER_COLOR", false);
      cs.VisualPropA.PropertySubData.TrouserModel =
          CVisualSlotManager::getInstance()->sheet2Index(
              CSheetId(setup.SrcSheet->getSheetId()), SLOTTYPE::LEGS_SLOT);
      cs.VisualPropA.PropertySubData.TrouserColor = color->getValue32();
      SCharacter3DSetup::setupDBFromCharacterSummary("UI:TEMP:CHAR3D", cs);
      camHeight = -1.00f;
    } else if (pIS->ItemType == ITEM_TYPE::LIGHT_VEST ||
               pIS->ItemType == ITEM_TYPE::MEDIUM_VEST ||
               pIS->ItemType == ITEM_TYPE::HEAVY_VEST) {
      CCDBNodeLeaf *color =
          dbBranch->getLeaf(setup.SrcSheet->getSheet() + ":USER_COLOR", false);
      cs.VisualPropA.PropertySubData.JacketModel =
          CVisualSlotManager::getInstance()->sheet2Index(
              CSheetId(setup.SrcSheet->getSheetId()), SLOTTYPE::CHEST_SLOT);
    ...
    cs.VisualPropA.PropertySubData.WeaponLeftHand =
        CVisualSlotManager::getInstance()->sheet2Index(
            CSheetId(setup.SrcSheet->getSheetId()), SLOTTYPE::LEFT_HAND_SLOT);
    if (cs.VisualPropA.PropertySubData.WeaponRightHand != 0) {
      CItemSheet *pES =
          SheetMngr.getItem(SLOTTYPE::RIGHT_HAND_SLOT,
                            cs.VisualPropA.PropertySubData.WeaponRightHand);
      if (pES->ItemType == ITEM_TYPE::TWO_HAND_AXE ||
          pES->ItemType == ITEM_TYPE::TWO_HAND_MACE ||
          pES->ItemType == ITEM_TYPE::TWO_HAND_SWORD ||
          pES->ItemType == ITEM_TYPE::MAGICIAN_STAFF ||
          pES->ItemType == ITEM_TYPE::AUTOLAUCH ||
          pES->ItemType == ITEM_TYPE::LAUNCHER ||
          pES->ItemType == ITEM_TYPE::RIFLE)
        cs.VisualPropA.PropertySubData.WeaponRightHand = 0;
    }
    SCharacter3DSetup::setupDBFromCharacterSummary("UI:TEMP:CHAR3D", cs);

  } else if (pIS->Family == ITEMFAMILY::MELEE_WEAPON ||
             pIS->Family == ITEMFAMILY::RANGE_WEAPON) {
    cs.VisualPropA.PropertySubData.WeaponRightHand =
        CVisualSlotManager::getInstance()->sheet2Index(
            CSheetId(setup.SrcSheet->getSheetId()), SLOTTYPE::RIGHT_HAND_SLOT);
    cs.VisualPropA.PropertySubData.WeaponLeftHand = 0;
    SCharacter3DSetup::setupDBFromCharacterSummary("UI:TEMP:CHAR3D", cs);
  } else
    nlwarning("<setupItemPreview> Invalid armour or weapon item type '%s'",
              ITEM_TYPE::toString(pIS->ItemType).c_str());
...
void refreshItemHelp(CSheetHelpSetup &setup) {
  ...
  // BORIS : 06/09/2006 : removed because seams to build a double 'description'
  // in the item info windows
  //	const CItemSheet *pIS= setup.SrcSheet->asItemSheet();
  //	if (pIS)
  //	{
  //		if (pIS->Family == ITEMFAMILY::SCROLL_R2)
  //		{
  //			const R2::TMissionItem *mi =
  //R2::getEditor().getPlotItemInfos((uint32) setup.SrcSheet->getSheetId()); 			if
  //(mi)
  //			{
  //				itemText += CI18N::get("uiRingPlotItemDesc");
  //				itemText += mi->Description.empty() ?
  //CI18N::get("uiRingPlotItemEmpty") 													: mi->Description;
  //				//itemText += "\n@{6F6F}" +
  //CI18N::get("uiRingPlotItemComment") + "\n";
  //				/*
  //				itemText += mi->Comment.empty() ?
  //CI18N::get("uiRingPlotItemEmpty") 												: ("\n" + mi->Comment);
  //												*/
  //			}
  //		}
  //	}
...
static void setupMissionHelp(CSheetHelpSetup &setup) {
  ...
  // setup the item.
  //	CDBCtrlSheet		*ctrlMission=
  //dynamic_cast<CDBCtrlSheet*>(setup.HelpWindow->getCtrl("ctrl_slot"));
  if (setup.DestSheet) {
    setup.SrcSheet->copyAspect(setup.DestSheet);
    setup.DestSheet->setActive(true);
  }
  ...
  if (!setup.SrcSheet->getRootBranch())
    return;
  CCDBNodeLeaf *detailTextLeaf =
      dynamic_cast<CCDBNodeLeaf *>(setup.SrcSheet->getRootBranch()->getNode(
          ICDBNode::CTextId("DETAIL_TEXT")));
  if (!detailTextLeaf)
    return;

  // Change the title according to Mission Client Type
  MISSION_DESC::TIconId iconId =
      (MISSION_DESC::TIconId)setup.SrcSheet->getSheetId();
  MISSION_DESC::TClientMissionType mType =
      MISSION_DESC::getClientMissionType(iconId);
  ...
  CViewTextID *viewTextTitleID =
      dynamic_cast<CViewTextID *>(setup.HelpWindow->getView("text_title_id"));
  if (viewTextTitleID != NULL) {
    CCDBNodeLeaf *titleTextLeaf = dynamic_cast<CCDBNodeLeaf *>(
        setup.SrcSheet->getRootBranch()->getNode(ICDBNode::CTextId("TEXT")));
    if (titleTextLeaf == NULL)
      return;
...
// ***************************************************************************
void refreshMissionHelp(CSheetHelpSetup &setup,
                        const CPrerequisitInfos &infos) {
  ...
  for (uint i = 0; i < infos.Prerequisits.size();) {
    ...
    // check 'or' conditions, if any or (or enclosing 'and') is validated then
    // global block is matched
    uint orIndexMax;
    for (orIndexMax = i + 1; orIndexMax < infos.Prerequisits.size();
         ++orIndexMax) {
    ...
    for (uint j = i; j < orIndexMax; ++j) {
      const string text = setup.HelpWindow->getId() +
                          ":content:scroll_text_id:text_list:" +
                          NLMISC::toString("text_%u", j + 1);
      CViewText *viewText =
          dynamic_cast<CViewText *>(setup.HelpWindow->getElement(text));
      if (viewText) {
      ...
      const string textId = setup.HelpWindow->getId() +
                            ":content:scroll_text_id:text_list:" +
                            NLMISC::toString("text_id_prereq_%u", j + 1);

      CViewTextID *viewTextID =
          dynamic_cast<CViewTextID *>(setup.HelpWindow->getElement(textId));
      if (viewTextID) {
  ...
  for (uint i = (uint)infos.Prerequisits.size(); i < 15; ++i) {
    ...
    const string textId = setup.HelpWindow->getId() +
                          ":content:scroll_text_id:text_list:" +
                          NLMISC::toString("text_id_prereq_%u", i + 1);
    CViewTextID *viewTextID =
        dynamic_cast<CViewTextID *>(setup.HelpWindow->getElement(textId));
    if (viewTextID)
...
void setupCreatorName(CSheetHelpSetup &setup) {
  if (!setup.HelpWindow)
    return;
  CViewTextID *vtid = dynamic_cast<CViewTextID *>(
      setup.HelpWindow->getView(setup.CreatorViewTextID));
  CViewText *vthd =
      dynamic_cast<CViewText *>(setup.HelpWindow->getView("creator_header"));
  if (vtid != NULL) {
    bool bIsRM = false;
    if (setup.SrcSheet) {
      const CItemSheet *pIS = dynamic_cast<const CItemSheet *>(
          SheetMngr.get(CSheetId(setup.SrcSheet->getSheetId())));
      bIsRM = (pIS && pIS->Family == ITEMFAMILY::RAW_MATERIAL);
    }

    // if a RM or not an item, disable the view
    if (!setup.SrcSheet || bIsRM ||
        setup.SrcSheet->getType() != CCtrlSheetInfo::SheetType_Item) {
...
// ***************************************************************************
void fillOutpostBuildingListItem(const std::vector<NLMISC::CSheetId> &mps,
                                 IListSheetBase *listItem,
                                 uint32 qualityLevel) {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  ...
  if (listItem) {
    ...
    for (i = 0; i < mps.size(); i++) {
      CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
          toString("%s:%d:SHEET", branchBase.c_str(), i));
      if (node)
        node->setValue32(mps[i].asInt());
      node = NLGUI::CDBManager::getInstance()->getDbProp(
          toString("%s:%d:QUALITY", branchBase.c_str(), i), false);
      if (node)
        node->setValue32(qualityLevel);
      node = NLGUI::CDBManager::getInstance()->getDbProp(
          toString("%s:%d:PREREQUISIT_VALID", branchBase.c_str(), i), false);
      if (node)
        node->setValue32(1);
    }
    // Reset other to 0.
    for (;; i++) {
      CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
          toString("%s:%d:SHEET", branchBase.c_str(), i), false);
      if (node)
        node->setValue32(0);
      else
        break;
...
void setupOutpostBuildingHelp(CSheetHelpSetup &setup) {
  ...
  sOBText = CI18N::get("uihelpOBFormat_" +
                       COutpostBuildingSheet::toString(pOBS->OBType));

  {
    string timeText;
    timeText =
        toString(pOBS->CostTime / 60) + CI18N::get("uiBotChatTimeMinute");
    if ((pOBS->CostTime % 60) != 0)
      timeText +=
          toString(pOBS->CostTime % 60) + CI18N::get("uiBotChatTimeSecond");
  ...
  strFindReplace(
      sOBText, "%name",
      STRING_MANAGER::CStringManagerClient::getOutpostBuildingLocalizedName(
          pOBS->Id));
  ...
  if (pOBS->OBType == COutpostBuildingSheet::OB_Driller) {
    ...
    IListSheetBase *listItem =
        dynamic_cast<IListSheetBase *>(setup.HelpWindow->getElement(
            setup.HelpWindow->getId() + setup.PrefixForExtra + INFO_LIST_ITEM));

    // setup the bricks
    fillOutpostBuildingListItem(pOBS->Mps, listItem,
                                pOBS->MPLevelOfHighestExtractRate);
  }
}
...
void getSabrinaBrickText(CSBrickSheet *pBR, string &brickText) {
  ...
  if (pBR->getSkill() == SKILLS::unknown) {
    ...
    if (pos0 != string::npos) {
      string::size_type pos1 =
          brickText.find(killSkill, pos0 + killSkill.size());
      if (pos1 != string::npos)
    ...
    if (pBR->UsedSkills.size() == 1)
      strFindReplace(
          brickText, "%skill",
          CStringManagerClient::getSkillLocalizedName(pBR->getSkill()));
    else {
  ...
  if (pBR->SabrinaRelativeCost == 0.f) {
    ...
    strFindReplace(brickText, "%relative_cost",
                   toPercentageText(pBR->SabrinaRelativeCost) + string("%"));
    // Header Cost: cost or credit
    if (pBR->SabrinaRelativeCost >= 0.f)
      strFindReplace(brickText, "%hrel_cost",
                     CI18N::get("uihelpSabrinaRelCost"));
    else
      strFindReplace(brickText, "%hrel_cost",
                     CI18N::get("uihelpSabrinaRelCredit"));
  }

  // Description
  strFindReplace(brickText, "%desc",
                 CStringManagerClient::getSBrickLocalizedDescription(pBR->Id));
  ...
  if (pBR->isFaber() && pBR->isMandatory()) {
    ...
    strFindReplace(
        brickText, "%tool",
        CI18N::get("tool" + TOOL_TYPE::toString(pBR->FaberPlan.ToolType)));
    ...
    if (pBR->FaberPlan.ItemPartMps.empty()) {
      ...
      for (uint i = 0; i < pBR->FaberPlan.ItemPartMps.size(); i++) {
        CSBrickSheet::CFaberPlan::CItemPartMP &mpSlot =
            pBR->FaberPlan.ItemPartMps[i];
    ...
    if (pBR->FaberPlan.FormulaMps.empty()) {
      ...
      for (uint i = 0; i < pBR->FaberPlan.FormulaMps.size(); i++) {
        CSBrickSheet::CFaberPlan::CFormulaMP &mpSlot =
            pBR->FaberPlan.FormulaMps[i];

        // Display the required item
        mpInfo += "@{T4}";
        mpInfo += STRING_MANAGER::CStringManagerClient::getItemLocalizedName(
            mpSlot.ItemRequired);
        mpInfo += "\n";
      }
      // replace in brickText
  ...
  if (pBR->isMagic() && pBR->MagicResistType != RESISTANCE_TYPE::None) {
    ...
    strFindReplace(
        magicResistStr, "%t",
        CI18N::get("rs" + RESISTANCE_TYPE::toString(pBR->MagicResistType)));
  }
 ...
 */
void fillSabrinaPhraseListBrick(const CSPhraseCom &phrase,
                                IListSheetBase *listBrick) {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  ...
  if (listBrick) {
    ...
    for (i = 0; i < phrase.Bricks.size(); i++) {
      ...
      node = NLGUI::CDBManager::getInstance()->getDbProp(
          toString("%s:%d:LOCKED", branchBase.c_str(), i));
      if (node) {
...
void hideListBrickHeader(CSheetHelpSetup &setup) {
  ...
  // get the header text
  CViewText *view = dynamic_cast<CViewText *>(setup.HelpWindow->getElement(
      setup.HelpWindow->getId() + setup.PrefixForExtra +
      INFO_LIST_BRICK_HEADER));
  if (view) {
    view->setActive(false);
  }
}
...
// ***************************************************************************
/*	phraseSheetId: not null if comes from a .sphrase Sheet, used to show
 * progression info
 */
void setupSabrinaPhraseHelp(CSheetHelpSetup &setup, const CSPhraseCom &phrase,
                            uint32 phraseSheetId) {
  CSPhraseManager *pPM = CSPhraseManager::getInstance();
  ...
  // NB: don't add if from bot chat validation (useless cause already filtered
  // by server)
  ...
  if (pPM->allowListBrickInHelp(phrase)) {
    // Get the list of bricks container
    IListSheetBase *listBrick = dynamic_cast<IListSheetBase *>(
        setup.HelpWindow->getElement(setup.HelpWindow->getId() +
                                     setup.PrefixForExtra + INFO_LIST_BRICK));
...
void setupSheetHelp(CSheetHelpSetup &setup) {
  ...
  case CCtrlSheetInfo::SheetType_OutpostBuilding:
    setupOutpostBuildingHelp(setup);
    break;
  default:
...
public:
  virtual void execute(CCtrlBase *pCaller, const string &Params) {
    ...
    if (brickSheetId == pBM->getInterfaceRemoveBrick())
      return;
    // No Info for special "XP catalyzer" or "PVP oupost" interface brick in
    // bonus malus window
    ...
    if (brickName.compare(0, xpCatBrickPrefix.size(), xpCatBrickPrefix) == 0)
      return;
    if (brickName.compare(0, ringXpCatBrickPrefix.size(),
                          ringXpCatBrickPrefix) == 0)
      return;
    if (brickName.compare(0, pvpOutpostBrickPrefix.size(),
                          pvpOutpostBrickPrefix) == 0)
      return;
...
void setConsoModSuccessTooltip(CDBCtrlSheet *cs) {
  ...
  if (CSheetId(cs->getSheetId()).toString() == "mod_melee_success.sbrick") {
    ...
    nodeSM = NLGUI::CDBManager::getInstance()->getDbProp(
        "SERVER:CHARACTER_INFO:SUCCESS_MODIFIER:MELEE", false);
  } else if (CSheetId(cs->getSheetId()).toString() ==
             "mod_range_success.sbrick") {
    ustr = CI18N::get("uittModRangeSuccess");
    nodeSM = NLGUI::CDBManager::getInstance()->getDbProp(
        "SERVER:CHARACTER_INFO:SUCCESS_MODIFIER:RANGE", false);
  } else if (CSheetId(cs->getSheetId()).toString() ==
             "mod_craft_success.sbrick") {
    ustr = CI18N::get("uittModCraftSuccess");
    nodeSM = NLGUI::CDBManager::getInstance()->getDbProp(
        "SERVER:CHARACTER_INFO:SUCCESS_MODIFIER:CRAFT", false);
  } else if (CSheetId(cs->getSheetId()).toString() ==
             "mod_defense_success.sbrick") {
    ustr = CI18N::get("uittModDefenseSuccess");
    nodeSM = NLGUI::CDBManager::getInstance()->getDbProp(
        "SERVER:CHARACTER_INFO:SUCCESS_MODIFIER:DODGE", false);
  } else if (CSheetId(cs->getSheetId()).toString() ==
             "mod_dodge_success.sbrick") {
    ustr = CI18N::get("uittModDodgeSuccess");
    nodeSM = NLGUI::CDBManager::getInstance()->getDbProp(
        "SERVER:CHARACTER_INFO:SUCCESS_MODIFIER:DODGE", false);
  } else if (CSheetId(cs->getSheetId()).toString() ==
             "mod_parry_success.sbrick") {
    ustr = CI18N::get("uittModParrySuccess");
    nodeSM = NLGUI::CDBManager::getInstance()->getDbProp(
        "SERVER:CHARACTER_INFO:SUCCESS_MODIFIER:PARRY", false);
  } else if (CSheetId(cs->getSheetId()).toString() ==
             "mod_forage_success.sbrick") {
    ustr = CI18N::get("uittModForageSuccess");
    nodeSM = NLGUI::CDBManager::getInstance()->getDbProp(
        "SERVER:CHARACTER_INFO:SUCCESS_MODIFIER:ECO:" +
            toString((uint8)ECOSYSTEM::common_ecosystem) + ":FORAGE",
        false);
  } else if (CSheetId(cs->getSheetId()).toString() ==
             "mod_desert_forage_success.sbrick") {
    ustr = CI18N::get("uittModDesertForageSuccess");
    nodeSM = NLGUI::CDBManager::getInstance()->getDbProp(
        "SERVER:CHARACTER_INFO:SUCCESS_MODIFIER:ECO:" +
            toString((uint8)ECOSYSTEM::desert) + ":FORAGE",
        false);
  } else if (CSheetId(cs->getSheetId()).toString() ==
             "mod_forest_forage_success.sbrick") {
    ustr = CI18N::get("uittModForestForageSuccess");
    nodeSM = NLGUI::CDBManager::getInstance()->getDbProp(
        "SERVER:CHARACTER_INFO:SUCCESS_MODIFIER:ECO:" +
            toString((uint8)ECOSYSTEM::forest) + ":FORAGE",
        false);
  } else if (CSheetId(cs->getSheetId()).toString() ==
             "mod_lacustre_forage_success.sbrick") {
    ustr = CI18N::get("uittModLacustreForageSuccess");
    nodeSM = NLGUI::CDBManager::getInstance()->getDbProp(
        "SERVER:CHARACTER_INFO:SUCCESS_MODIFIER:ECO:" +
            toString((uint8)ECOSYSTEM::lacustre) + ":FORAGE",
        false);
  } else if (CSheetId(cs->getSheetId()).toString() ==
             "mod_jungle_forage_success.sbrick") {
    ustr = CI18N::get("uittModJungleForageSuccess");
    nodeSM = NLGUI::CDBManager::getInstance()->getDbProp(
        "SERVER:CHARACTER_INFO:SUCCESS_MODIFIER:ECO:" +
            toString((uint8)ECOSYSTEM::jungle) + ":FORAGE",
        false);
  } else if (CSheetId(cs->getSheetId()).toString() ==
             "mod_primary_root_forage_success.sbrick") {
    ustr = CI18N::get("uittModPrimaryRootForageSuccess");
    nodeSM = NLGUI::CDBManager::getInstance()->getDbProp(
        "SERVER:CHARACTER_INFO:SUCCESS_MODIFIER:ECO:" +
            toString((uint8)ECOSYSTEM::primary_root) + ":FORAGE",
        false);
  }

  if (nodeSM) {
    if (nodeSM->getValue32() < 0)
      strFindReplace(ustr, "%modifier",
                     "@{E42F}-" + toString(nodeSM->getValue32()) + "@{FFFF}");
    else
      strFindReplace(ustr, "%modifier",
                     "@{0F0F}" + toString(nodeSM->getValue32()) + "@{FFFF}");
...
public:
  virtual void execute(CCtrlBase *pCaller, const string &Params) {
    ...
    if (specialTTId == BONUS_MALUS::XpCatalyser)
      CWidgetManager::getInstance()->setContextHelpText(
          CI18N::get("uittXpBonus"));
    else if (specialTTId == BONUS_MALUS::OutpostPVPOn)
      CWidgetManager::getInstance()->setContextHelpText(
          CI18N::get("uittPvpOutpostOn"));
    else if (specialTTId == BONUS_MALUS::OutpostPVPOutOfZone)
      CWidgetManager::getInstance()->setContextHelpText(
          CI18N::get("uittPvpOutpostOutOfZone"));
    else if (specialTTId == BONUS_MALUS::OutpostPVPInRound)
      CWidgetManager::getInstance()->setContextHelpText(
          CI18N::get("uittPvpOutpostInRound"));
    else if (specialTTId == BONUS_MALUS::DeathPenalty) {
      CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
          "SERVER:USER:DEATH_XP_MALUS", false);
      if (node) {
...
public:
  virtual void execute(CCtrlBase *pCaller, const string &Params) {
    ...
    string txt;
    CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
        toString("SERVER:PACK_ANIMAL:BEAST%d:NAME", index));
    if (node && CStringManagerClient::instance()->getDynString(
                    node->getValue32(), txt)) {
      CWidgetManager::getInstance()->setContextHelpText(
          CEntityCL::removeTitleFromName(txt));
    }
  }
};
...
public:
  virtual void execute(CCtrlBase *pCaller, const string &Params) {
    ...
    CCDBNodeBranch *animalsNode = safe_cast<CCDBNodeBranch *>(
        NLGUI::CDBManager::getInstance()->getDB()->getNode(
            ICDBNode::CTextId("SERVER:PACK_ANIMAL"), false));
    ...
    for (uint i = 0; i != nbAnimals; ++i) {
      ...
      if (statusNode &&
          ANIMAL_STATUS::isDead(
              (ANIMAL_STATUS::EAnimalStatus)statusNode->getValue32())) {
        ICDBNode *beastNode = animalsNode->getNode(i);
        //				CCDBNodeLeaf *uidLeaf =
        //safe_cast<CCDBNodeLeaf*>(beastNode->getNode( ICDBNode::CTextId( "UID"
        //) ));
        CCDBNodeLeaf *despawnLeaf = safe_cast<CCDBNodeLeaf *>(
            beastNode->getNode(ICDBNode::CTextId("DESPAWN")));
        if (minTimeRemaining == -1) {
          minTimeRemaining = despawnLeaf->getValue32();
        } else if (minTimeRemaining > despawnLeaf->getValue32()) {
          minTimeRemaining = despawnLeaf->getValue32();
        }
      }
    }
    ...
    string str;
    BOMB_IF(minTimeRemaining < 0, "at least one animal should be dead",
            return;);
...
REGISTER_ACTION_HANDLER(CHandlerAnimalDeadPopupTooltip,
                        "animal_dead_popup_tooltip");
...
public:
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    CEntityCL *selection = EntitiesMngr.entity(UserEntity->selection());
    if (selection != NULL) {
      sint n = PeopleInterraction.TeamList.getIndexFromName(
          selection->getEntityName());
      if (n >= 0) {
...
// ***************************************************************************
static void onMpChangeItemPart(CInterfaceGroup *wnd, uint32 itemSheetId,
                               const string &statPrefixId) {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  ...
  // get the item sheet
  const CItemSheet *pIS =
      dynamic_cast<const CItemSheet *>(SheetMngr.get(CSheetId(itemSheetId)));
  if (!pIS || pIS->Family != ITEMFAMILY::RAW_MATERIAL)
    return;

  // get the group for raw material stat
  CInterfaceGroup *groupMp = dynamic_cast<CInterfaceGroup *>(
      wnd->getElement(wnd->getId() + statPrefixId + INFO_GROUP_MP_STAT));
  if (!groupMp)
    return;
  ...
  // **** Build Icon
  // get the icon
  CViewBitmap *viewBmp = dynamic_cast<CViewBitmap *>(
      groupMp->getElement(groupMp->getId() + ":icon"));
  if (viewBmp) {
    // texture name in config.xml
    viewBmp->setTexture(CWidgetManager::getInstance()->getParser()->getDefine(
        RM_FABER_TYPE::toIconDefineString(faberType)));
  }

  // **** Build text
  // get the text
  CViewText *viewText = dynamic_cast<CViewText *>(
      groupMp->getElement(groupMp->getId() + ":text"));
  if (viewText) {
    string mpCraft;

    // add the Origin filter.
    string originFilterKey =
        "iompf" + ITEM_ORIGIN::enumToString(
                      (ITEM_ORIGIN::EItemOrigin)itemPart.OriginFilter);
  ...
  for (i = 0; i < RM_FABER_STAT_TYPE::NumRMStatType; i++) {
    // get the stat group
    CInterfaceGroup *groupStat = dynamic_cast<CInterfaceGroup *>(
        groupMp->getElement(groupMp->getId() + toString(":stat%d", i)));
    if (groupStat)
  ...
  uint groupIndex = 0;
  for (i = 0; i < RM_FABER_STAT_TYPE::NumRMStatType; i++) {
    RM_FABER_STAT_TYPE::TRMStatType statType =
    ...
    CInterfaceGroup *groupStat =
        dynamic_cast<CInterfaceGroup *>(groupMp->getElement(
            groupMp->getId() + toString(":stat%d", groupIndex)));
    if (groupStat) {
      groupStat->setActive(true);
      // fill text and bar according to stat
      CViewText *statTitle = dynamic_cast<CViewText *>(
          groupStat->getElement(groupStat->getId() + ":text"));
      CDBViewBar *statValue = dynamic_cast<CDBViewBar *>(
          groupStat->getElement(groupStat->getId() + ":bar"));
      if (statTitle)
...
public:
  virtual void execute(CCtrlBase * /* pCaller */, const string &Params) {
    ...
    if (infoWindowIndex >= CInterfaceHelp::_InfoWindows.size())
      return;
    CInterfaceHelp::CInfoWindow &infoWindow =
        CInterfaceHelp::_InfoWindows[infoWindowIndex];
    ...
    if (infoWindow.CtrlSheet) {
      onMpChangeItemPart(infoWindow.Window, infoWindow.CtrlSheet->getSheetId(),
                         dummy.PrefixForExtra);
    }
  }
};
REGISTER_ACTION_HANDLER(CAHItemHelpMpChangeItemPart,
                        "item_help_mp_change_item_part");
...
public:
  virtual void execute(CCtrlBase * /* pCaller */, const string &Params) {
    ...
    CInterfaceGroup *wnd = dynamic_cast<CInterfaceGroup *>(
        CWidgetManager::getInstance()->getElementFromId(wndStr));
...
REGISTER_ACTION_HANDLER(CAHItemBotChatMpChangeItemPart,
                        "item_botchat_mp_change_item_part");
...
void updateStatReport() {
  ...
  if ((ingameTime0() <= time4StatReport) && (ingameTime1() > time4StatReport)) {
    CInterfaceManager *pIM = CInterfaceManager::getInstance();
    CAHManager::getInstance()->runActionHandler("proc", NULL,
                                                "proc_stat_report");
  }
}
...
public:
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    ...
    CCDBNodeLeaf *pVal =
        NLGUI::CDBManager::getInstance()->getDbProp("UI:SAVE:MK_MODE", false);
    if (pVal) {
