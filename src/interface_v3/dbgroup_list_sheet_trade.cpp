...
NLMISC_REGISTER_OBJECT(CViewBase, CDBGroupListSheetTrade, std::string,
                       "list_sheet_trade");

// ***************************************************************************
void CDBGroupListSheetTrade::CSheetChildTrade::init(
    CDBGroupListSheetText *pFather, uint index) {
  // init my parent
  CSheetChild::init(pFather, index);

  CDBGroupListSheetTrade *zeFather =
      safe_cast<CDBGroupListSheetTrade *>(pFather);
...
// ***************************************************************************
bool CDBGroupListSheetTrade::CSheetChildTrade::isInvalidated(
...
// ***************************************************************************
void CDBGroupListSheetTrade::CSheetChildTrade::update(
    CDBGroupListSheetText *pFather) {
...
// ***************************************************************************
void CDBGroupListSheetTrade::CSheetChildTrade::updateViewText(
    CDBGroupListSheetText *pFather) {
  ...
  case CDBCtrlSheet::Item:
    break; // none for item. consider useless
  case CDBCtrlSheet::Pact:
    text = CI18N::get("uiBotChatPact") + text;
    break;
  case CDBCtrlSheet::Skill:
    text = CI18N::get("uiBotChatSkill") + text;
    break;
  case CDBCtrlSheet::GuildFlag:
    text = CI18N::get("uiBotChatSkill") + text;
    break;
  case CDBCtrlSheet::Phrase:
    text = CI18N::get("uiBotChatPhrase") + text;
    break;
  default:
    break;
  }
  ...
  if (Ctrl->getType() == CCtrlSheetInfo::SheetType_OutpostBuilding) {
    ...
    if (pOBS != NULL) {
      ...
      text += string("\n") + pSMC->getOutpostBuildingLocalizedDescription(
                                 CSheetId(Ctrl->getSheetId()));
      text += "\n" + CI18N::get("uiBotChatPrice") +
              NLMISC::formatThousands(toString(pOBS->CostDapper));
      text += CI18N::get("uiBotChatTime") + toString(pOBS->CostTime / 60) +
              CI18N::get("uiBotChatTimeMinute");
  ...
  if (Ctrl->getSheetCategory() == CDBCtrlSheet::Phrase) {
    ...
    bool melee, range;
    pPM->getCombatWeaponRestriction(weaponRestriction, Ctrl->getSheetId(),
                                    melee, range);
    // don't add also if no combat restriction
    if (!weaponRestriction.empty() &&
        weaponRestriction != CI18N::get("uiawrSF")) {
    ...
    if (Ctrl->getSheetCategory() == CDBCtrlSheet::Phrase) {
      ...
      if (Ctrl->getType() == CCtrlSheetInfo::SheetType_Item) {
        ...
        if (pIS && pIS->Family == ITEMFAMILY::GUILD_OPTION) {
          text += "\n" + CI18N::get("uiBotChatSkillPointCost") +
                  toString(pIS->GuildOption.XPCost);
          text += "\n" + CI18N::get("uiBotChatPrice") +
                  NLMISC::formatThousands(toString(pIS->GuildOption.MoneyCost));
          guildOption = true;
        }
      }
      ...
      if (!guildOption) {
        if (LastPrice != -1) {
          ...
          // Append std price
          bool displayMulPrice = pIS && pIS->Stackable > 1 &&
                                 zeFather->getMultiplyPriceByQuantityFlag();
          ...
          if (zeFather->applyFamePriceFactor()) {
            priceFactor = NLGUI::CDBManager::getInstance()
                              ->getDbProp("SERVER:TRADING:FAME_PRICE_FACTOR")
                              ->getValue16() /
                          10000.0f;
          }
          ...
          if (LastPrice > 0) {
            if (displayMulPrice)
              text += "\n" + CI18N::get("uiBotChatPrice") +
                      NLMISC::formatThousands(
                          toString(sint32(LastPrice * priceFactor))) +
                      " (" +
                      NLMISC::formatThousands(toString(
                          sint32(factor) * sint32(LastPrice * priceFactor))) +
                      ")";
            else
              text += "\n" + CI18N::get("uiBotChatPrice") +
                      NLMISC::formatThousands(
                          toString(sint32(factor * LastPrice * priceFactor)));
          }

          if ((LastFactionPointPrice != 0) &&
              (LastFactionType >= PVP_CLAN::BeginClans) &&
              (LastFactionType <= PVP_CLAN::EndClans)) {
            if (LastPrice > 0)
              text += ". ";
            else
              text += "\n";

            text += CI18N::get("uiBotChatFactionType") +
                    PVP_CLAN::toString((PVP_CLAN::TPVPClan)LastFactionType) +
          ...
          if (zeFather->sellerTypeWanted()) {
            // Append retire price if reseller type OK (only if the item belongs
            // to the player)
            if (LastSellerType == BOTCHATTYPE::User ||
              ...
              // append price
              if (pIS && pIS->Stackable > 1 &&
                  zeFather->getMultiplyPriceByQuantityFlag())
                text += CI18N::get("uiBotChatRetirePrice") +
                        NLMISC::formatThousands(toString(LastPriceRetire)) +
                        " (" +
                        NLMISC::formatThousands(
                ...
                text +=
                    CI18N::get("uiBotChatRetirePrice") +
              ...
              strFindReplace(fmt, "%d",
                             toString(LastResaleTimeLeft / RYZOM_DAY_IN_HOUR));
              strFindReplace(fmt, "%h",
                             toString(LastResaleTimeLeft % RYZOM_DAY_IN_HOUR));
              text += "\n" + fmt;
              // force special color (according if retirable or not)
              if (LastSellerType == BOTCHATTYPE::UserRetirable ||
                  LastSellerType == BOTCHATTYPE::ResaleAndUserRetirable)
            ...
            // else display the name of the vendor (not if this is the player
            // himself, to avoid flood)
          ...
          if (zeFather->sellerTypeWanted() &&
              (LastSellerType == BOTCHATTYPE::User ||
               LastSellerType == BOTCHATTYPE::UserRetirable)) {
...
// ***************************************************************************
bool CDBGroupListSheetTrade::CSheetChildTrade::isSheetValid(
    CDBGroupListSheetText *pFather) {
  ...
  if (stf != None) {
    ...
    if (stf == User && LastSellerType != BOTCHATTYPE::User &&
        LastSellerType != BOTCHATTYPE::UserRetirable)
      return false;
    // If NPCAndResale filter, display both NPC, Resale and ResaleAndUser
    // SELLER_TYPE
    if (stf == NPCAndResale && LastSellerType != BOTCHATTYPE::Resale &&
        LastSellerType != BOTCHATTYPE::ResaleAndUser &&
        LastSellerType != BOTCHATTYPE::ResaleAndUserRetirable &&
        LastSellerType != BOTCHATTYPE::NPC)
      return false;
  ...
  if (phraseSheet)
  {
          ...
          {
                  CSBrickSheet *bs = dynamic_cast<CSBrickSheet
  *>(SheetMngr.get(phraseSheet->Bricks[k])); if (bs)
                  {
                          nlwarning("Brick %d has type %s", (int) k,
  BRICK_TYPE::toString(bs->getBrickType()).c_str());
                  }
                  else
                  {
  ...
  if (phraseSheet) {
    ...
    case TrainerAction:
      return (!isUpgradePhrase(*phraseSheet)) &&
             (!isPowerPhrase(*phraseSheet)) && (!isCaracPhrase(*phraseSheet));
      break;
    case TrainerUpgrade:
      return isUpgradePhrase(*phraseSheet);
      break;
...
bool CDBGroupListSheetTrade::isUpgradePhrase(const CSPhraseSheet &phrase) {
  ...
  for (uint k = 0; k < phrase.Bricks.size(); ++k) {
    CSBrickSheet *bs =
        dynamic_cast<CSBrickSheet *>(SheetMngr.get(phrase.Bricks[k]));
    if (bs && bs->isRoot())
      return false;
  }
...
bool CDBGroupListSheetTrade::isPowerPhrase(const CSPhraseSheet &phrase) {
  for (uint k = 0; k < phrase.Bricks.size(); ++k) {
    CSBrickSheet *bs =
        dynamic_cast<CSBrickSheet *>(SheetMngr.get(phrase.Bricks[k]));
    if (bs && bs->getBrickType() != BRICK_TYPE::SPECIAL_POWER)
      return false;
  }
  return true;
}
...
bool CDBGroupListSheetTrade::isCaracPhrase(const CSPhraseSheet &phrase) {
  for (uint k = 0; k < phrase.Bricks.size(); ++k) {
    CSBrickSheet *bs =
        dynamic_cast<CSBrickSheet *>(SheetMngr.get(phrase.Bricks[k]));
    if (bs && bs->getBrickType() == BRICK_TYPE::TRAINING)
      return true;
  }
  return false;
}
...
// ***************************************************************************
bool CDBGroupListSheetTrade::parse(xmlNodePtr cur,
...
void CDBGroupListSheetTrade::checkCoords() {
  ...
  for (set<CSheetChildTrade *>::iterator it = VendorNameIdToUpdate.begin();
       it != VendorNameIdToUpdate.end();) {
