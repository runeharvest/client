...
public:
  ...
  /// Called when the user has selected the Plan. copy bag. the itemPlanBrick
  /// must have "FaberPlan" good infos.
...
private:
  ...
  public:
    ...
    RM_FABER_TYPE::TRMFType
        FaberTypeRequired; // valid if RequirementType==ItemPartReq
    NLMISC::CSheetId
        SpecificItemRequired; // valid if RequirementType==SpecificItemReq
    // For each MpSlot, the Inventory index (points to _InventoryMirror), and
  ...
  uint _MPBuildNumItemPartReq;     // The number of different itempart lines
                                   // requirement
  uint _MPBuildNumSpecificItemReq; // The number of different specific items
                                   // lines requirement
  ...
  void filterSelectionItemPart(std::vector<uint> &mps,
                               RM_FABER_TYPE::TRMFType itemPartFilter,
                               ITEM_ORIGIN::EItemOrigin originFilter);
  void filterSelectionItemSpecific(std::vector<uint> &mps,
  ...
  void updateEmptySlot(uint itemReqLine,
                       CInterfaceGroup *itemReqLineGroup = NULL);
  void updateQuantityView(uint itemReqLine);
  void updateValidButton();

  void deleteMpSlot(uint itemReqLine, uint mpSlot);

  // When the inventory is modified, must do some checks on the current faber
  // execution
  ...
  CItemImage *getInvMirrorItemImage(uint slotIndex, uint &invId,
                                    uint &indexInInv);
...
extern void launchFaberCastWindow(sint32 memoryLine, uint memoryIndex,
                                  CSBrickSheet *rootBrick);
...
// Called when something needs to close the crafting window (does nothing if not
// open)
