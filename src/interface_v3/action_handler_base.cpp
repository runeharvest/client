  ...
  virtual void execute(CCtrlBase *pCaller, const string &Params) {
    ...
    if (gc) {
      NLGUI::CDBManager::getInstance()
          ...
          ->setValue64((gc->isPopuped() || gc->getLayerSetup() == 0) ? 1 : 0);
    } else {
      NLGUI::CDBManager::getInstance()
          ->getDbProp("UI:VARIABLES:GC_POPUP")
          ->setValue64(0);
    ...
    if (CDBCtrlSheet::getDraggedSheet() == NULL) {
      ...
      CGroupMenu *groupMenu = dynamic_cast<CGroupMenu *>(
...
public:
  virtual void execute(CCtrlBase *pCaller, const string &Params) {
    ...
    CGroupEditBox *geb;
    if (pCaller == NULL)
      geb = dynamic_cast<CGroupEditBox *>(
          CWidgetManager::getInstance()->getElementFromId(target));
    else
      geb = dynamic_cast<CGroupEditBox *>(
          CWidgetManager::getInstance()->getElementFromId(pCaller->getId(),
                                                          target));
    if (geb == NULL) {
      nlwarning("<CAHSetKeyboardFocus::execute> Can't get target edit box %s, "
                "or bad type",
                target.c_str());
      return;
    }
...
class CAHSetEditBoxCommand : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string &Params) {
    ...
    if (menuEB)
      menuEB->setCommand(
          getParam(Params, "value"),
          nlstricmp(getParam(Params, "execute"), "true") ? true : false);
  }
};
  ...
  virtual void execute(CCtrlBase *pCaller, const string &Params) {
    ...
    if (sTarget.rfind(':') == string::npos) {
      ...
      if (pIE == NULL)
        return;
      sTarget = pIE->getId() + ":" +
                sTarget.substr(sTarget.rfind(':') + 1, sTarget.size());
    }
...
class CAHSetServerID : public IActionHandler {
  virtual void execute(CCtrlBase *pCaller, const string &Params) {
    ...
    if (CInterfaceExpr::eval(sValue, evValue, NULL)) {
      ...
      if (bRemoveTitle) {
        CStringPostProcessRemoveTitle *pSPPRT =
...
class CAHMilkoMenuResetInterface : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    ...
    pIM->validMessageBox(CInterfaceManager::QuestionIconMsg,
                         CI18N::get("uiQResetUI"),
                         "milko_menu_do_reset_interface", sParam);
  }
};
REGISTER_ACTION_HANDLER(CAHMilkoMenuResetInterface,
...
REGISTER_ACTION_HANDLER(CAHMilkoMenuDoResetInterface,
    ...
    for (uint32 nMasterGroup = 0; nMasterGroup < rVMG.size(); nMasterGroup++) {
      ...
      for (i = 0; i < rV.size(); ++i) {
        ...
        if (pGC == NULL)
          continue;
        if (pGC->isSavable()) {
          // Yoyo: DO NOT force activation of containers who don't want to save
          // their Active state. Usually driven by server.
      ...
      CWidgetManager::getInstance()
          ->getMasterGroup((uint8)nMasterGroup)
          .centerAllContainers();
...
class CAHPopup : public IActionHandler
{
        virtual void execute (CCtrlBase *pCaller, const string &Params)
        {
                ...
                CInterfaceManager *pIM = CInterfaceManager::getInstance();
                CGroupContainer *pGC =
dynamic_cast<CGroupContainer*>(CWidgetManager::getInstance()->getElementFromId(sCont));
                if (pGC == NULL) return;
