...
struct SCharacter3DSetup {
  ...
  // if the name, color or quality changed the instances are rebuilt. It is not
  // for all fxs.
  struct SCharacterPart {
    ...
    std::vector<std::string>
        StatFxBones; // the bones on which to bind the static fx (weird but done
                     // like that in CCharacterCL)
  ...
  // Initialize the structure from a DataBase Branch (look in cpp to see the
  // structure)
  void setupFromDataBase(const std::string &branchName);

  // Initialize the structure from the SERVER:... DataBase Branch (which is
  // messy)
  ...
  static void setupDBFromCharacterSummary(const std::string &branchName,
                                          const CCharacterSummary &cs);
  static void setupCharacterSummaryFromDB(CCharacterSummary &cs,
                                          const std::string &branchName);
  static void setupCharacterSummaryFromSERVERDB(CCharacterSummary &cs,
                                                uint8 entityID = 0);
...
public:
  ...
  void getRotEuler(float &rx, float &ry, float &rz) {
    rx = _CurRotX;
    ry = _CurRotY;
    rz = _CurRotZ;
  }
  ...
  void setPeople(EGSPD::CPeople::TPeople people) {
    _CurrentSetup.People = people;
  }
...
private:
  ...
  void createInstance(TChar3DPart i,
                      const SCharacter3DSetup::SCharacterPart &part);
...
private:
  ...
  // Root is the parent of skeleton (needed because skeleton receive animation
  // values)
