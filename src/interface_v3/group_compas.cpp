...
void CCompassTarget::serial(NLMISC::IStream &f) {
  ...
  f.serial(Pos);
  // for the name, try to save a string identifier if possible, because language
  // may be changed between save & reload
  if (version < 1) {
  ...
  // if language has been modified, then we are not able to display correctly
  // the name, so just reset the compass to north to avoid incoherency
  if (f.isReading()) {
...
CGroupCompas::CGroupCompas(const TCtorParam &param) : CGroupContainer(param) {
  _ArrowShape = NULL;
  std::fill(_TargetTypeColor, _TargetTypeColor + CCompassTarget::NumTypes,
            CRGBA::White);
  ...
  CCDBNodeLeaf *pRC =
      CDBManager::getInstance()->getDbProp("UI:SAVE:RADAR:USE_CAMERA");
  if (pRC) {
...
bool CGroupCompas::wantSerialConfig() const {
  // NB when the user first start the game, the target will befirst set to
  // 'chiang the strong' when missions are being received for the first time. If
  // his fail to occur, we just don't save anything
...
// ***************************************************************************
NLMISC::CVector2f
CGroupCompas::getNorthPos(const NLMISC::CVector2f &userPos) const {
  return NLMISC::CVector2f(userPos.x, userPos.y + 1.f);
}

// ***************************************************************************
void CGroupCompas::draw() {
  if ((uint)_Target.getType() >= CCompassTarget::NumTypes)
    return;
  ...
  NLMISC::CVector userPos((float)userPosD.x, (float)userPosD.y,
                          (float)userPosD.z);
  ...
  case CCompassTarget::Home: {
    // get pos
    CCDBNodeLeaf *pos = NLGUI::CDBManager::getInstance()->getDbProp(
        COMPASS_DB_PATH ":HOME_POINT");
  ...
  if (_Blinking) {
    ...
    if (dt >= COMPASS_BLINK_TIME) {
      ...
      color.blendFromui(color, _NewTargetSelectedColor,
                        (uint8)(blinkAmount * 255.f));
    }
  }
  ...
  if (event.getType() == NLGUI::CEventDescriptor::mouse) {
    ...
    if ((_RadarView != NULL) && (_RadarRangeView != NULL)) {
      if (eventDesc.getEventTypeExtended() ==
        ...
        _RadarPos = NLGUI::CDBManager::getInstance()
                        ->getDbProp("UI:SAVE:RADARZOOM")
                        ->getValue32();
...
void CGroupCompas::setTarget(const CCompassTarget &target) {
  if (_LastDynamicTargetPos != 0xFFFFFFFF) {
    ...
    CBitMemStream out;
    if (GenericMsgHeaderMngr.pushNameToStream("TARGET:COMPASS_NOT_DYNAMIC",
                                              out)) {
      NetMngr.push(out);
...
bool CGroupCompas::parse(xmlNodePtr cur, CInterfaceGroup *parentGroup) {
  ...
  if (ptr)
    _TargetTypeColor[CCompassTarget::North] = convertColor(ptr);
  //
  ptr = xmlGetProp(cur, (xmlChar *)"selection_color");
  if (ptr)
    _TargetTypeColor[CCompassTarget::Selection] = convertColor(ptr);
  //
  ptr = xmlGetProp(cur, (xmlChar *)"home_color");
  if (ptr)
    _TargetTypeColor[CCompassTarget::Home] = convertColor(ptr);
  //
  ptr = xmlGetProp(cur, (xmlChar *)"respawn_color");
  if (ptr)
    _TargetTypeColor[CCompassTarget::Respawn] = convertColor(ptr);
  //
  ptr = xmlGetProp(cur, (xmlChar *)"continent_landmark_color");
  if (ptr)
    _TargetTypeColor[CCompassTarget::ContinentLandMark] = convertColor(ptr);
  //
  ptr = xmlGetProp(cur, (xmlChar *)"mission_landmark_color");
  if (ptr)
    _TargetTypeColor[CCompassTarget::PosTracker] =
        convertColor(ptr); // TODO : see if more colors are needed for animal,
                           // mission target, etc. for now same color for
                           // dyanmic tracking of position
  //
  ptr = xmlGetProp(cur, (xmlChar *)"user_landmark_color");
  if (ptr)
    _TargetTypeColor[CCompassTarget::UserLandMark] = convertColor(ptr);
  //
  ptr = xmlGetProp(cur, (xmlChar *)"new_target_selected_color");
  if (ptr)
    _NewTargetSelectedColor = convertColor(ptr);
  //
...
bool buildCompassTargetFromTeamMember(CCompassTarget &ct, uint teamMemberId) {
  ...
  CCDBNodeLeaf *entityNode = NLGUI::CDBManager::getInstance()->getDbProp(
      toString(TEAM_DB_PATH ":%d:UID", teamMemberId), false);
  CCDBNodeLeaf *nameNode = NLGUI::CDBManager::getInstance()->getDbProp(
      toString(TEAM_DB_PATH ":%d:NAME", teamMemberId), false);
  if (nameNode && nameNode->getValueBool() && entityNode &&
      entityNode->getValue32() != 0 && nameNode) {
    ...
    if (pSMC->getString(nameNode->getValue32(), name))
      ct.Name = CEntityCL::removeTitleAndShardFromName(
          name); // TODO : dynamic support for name
    else
...
bool buildCompassTargetFromAnimalMember(CCompassTarget &ct,
  ...
  if (animalMemberId < MAX_INVENTORY_ANIMAL + 1) {
    ...
    at = (ANIMAL_TYPE::EAnimalType)NLGUI::CDBManager::getInstance()
             ->getDbProp("SERVER:PACK_ANIMAL:BEAST" +
                         toString(animalMemberId - 1) + ":TYPE")
             ->getValue32();
    string sPrefix;
    switch (at) {
    default:
  ...
  CCDBNodeLeaf *statusNode =
      NLGUI::CDBManager::getInstance()->getDbProp(dbBase + ":STATUS", false);
  if (statusNode &&
...
// ***************************************************************************
bool CGroupCompasMenu::parse(xmlNodePtr cur,
                             CInterfaceGroup *parent /*=NULL*/) {
  if (!CGroupMenu::parse(cur, parent))
    return false;
  CXMLAutoPtr compass((const char *)xmlGetProp(cur, (xmlChar *)"compass"));
  if (compass)
    _TargetCompass = (const char *)compass;
  return true;
}

// Helper for sorting landmarks
static inline bool UserLandMarksSortPredicate(const CUserLandMark &lm1,
                                              const CUserLandMark &lm2) {
...
void CGroupCompasMenu::setActive(bool state) {
  if (state && getRootMenu()) {
    ...
    // **** append North/Target etc...
    {
      ...
      getRootMenu()->addLineAtIndex(
          lineIndex++, ct.Name, "set_compas",
          toString("compass=%s|id=%d|menu=%s", _TargetCompass.c_str(),
                   (int)Targets.size() - 1, _Id.c_str()));
      // Home
      CCDBNodeLeaf *pos = NLGUI::CDBManager::getInstance()->getDbProp(
          COMPASS_DB_PATH ":HOME_POINT");
      sint32 px = (sint32)(pos->getValue64() >> 32);
      ...
      // Respawn
      pos = NLGUI::CDBManager::getInstance()->getDbProp(COMPASS_DB_PATH
                                                        ":BIND_POINT");
      ...
      // As of 6/5/2007 : The option to point the selection is always proposed
      // even if no slot is currently targeted
      // if (UserEntity->selection() != CLFECOMMON::INVALID_SLOT &&
      // UserEntity->selection() != 0)
      //{
      ...
      if (entity != NULL)
      {*/
      // string targetName = CI18N::get("uiTargetTwoPoint") +
      // entity->removeTitleAndShardFromName(entity->getEntityName());
      string targetName = CI18N::get("uiTarget");
      ct.setType(CCompassTarget::Selection);
      ct.Name = targetName;
      Targets.push_back(ct);
      getRootMenu()->addLineAtIndex(
          lineIndex++, targetName, "set_compas",
          toString("compass=%s|id=%d|menu=%s", _TargetCompass.c_str(),
                   (int)Targets.size() - 1, _Id.c_str()));
      /*}
}*/
    ...
    if (missionSubMenu) {
      ...
      for (uint m = 0; m < 2; ++m) // deals with solo & group missions
      {
        std::string baseDbPath =
            m == 0 ? MISSIONS_DB_PATH : GROUP_MISSIONS_DB_PATH;
        for (k = 0; k < MAX_NUM_MISSIONS; ++k) {
          for (uint l = 0; l < MAX_NUM_MISSION_TARGETS; ++l) {
            CCDBNodeLeaf *textIDLeaf =
                NLGUI::CDBManager::getInstance()->getDbProp(
                    baseDbPath + toString(":%d:TARGET%d:TITLE", (int)k, (int)l),
                    false);
            if (textIDLeaf) {
              string name;
              if (CStringManagerClient::instance()->getDynString(
                      textIDLeaf->getValue32(), name)) {
                CCDBNodeLeaf *leafPosX =
                    NLGUI::CDBManager::getInstance()->getDbProp(
                        baseDbPath + toString(":%d:TARGET%d:X", (int)k, (int)l),
                        false);
                CCDBNodeLeaf *leafPosY =
                    NLGUI::CDBManager::getInstance()->getDbProp(
                        baseDbPath + toString(":%d:TARGET%d:Y", (int)k, (int)l),
                        false);
                if (leafPosX && leafPosY) {
                  ...
                  ct.Name = name;
                  Targets.push_back(ct);
                  missionSubMenu->addLine(ct.Name, "set_compas",
                                          toString("compass=%s|id=%d|menu=%s",
                                                   _TargetCompass.c_str(),
                                                   (int)Targets.size() - 1,
                                                   _Id.c_str()));
                  selectable = true;
                }
      ...
      if (currCont) {
        ...
        for (k = 0; k < currCont->ContLandMarks.size(); ++k)
          if ((currCont->ContLandMarks[k].Type == CContLandMark::Capital) ||
            ...
            ct.Name = CStringManagerClient::getPlaceLocalizedName(
                currCont->ContLandMarks[k].TitleTextID);
            Targets.push_back(ct);
            landMarkSubMenu->addLineAtIndex(
                contLandMarkIndex++, ct.Name, "set_compas",
                toString("compass=%s|id=%d|menu=%s", _TargetCompass.c_str(),
                         (int)Targets.size() - 1, _Id.c_str()));
            selectable = true;
          }
        // separator?
        if (currCont->ContLandMarks.size() != 0 &&
            currCont->UserLandMarks.size() != 0) {
        ...
        std::sort(sortedLandmarks.begin(), sortedLandmarks.end(),
                  UserLandMarksSortPredicate);
        ...
        for (k = 0; k < sortedLandmarks.size(); ++k) {
          if (sortedLandmarks[k].Type < CUserLandMark::UserLandMarkTypeCount) {
            ...
            Targets.push_back(ct);
            landMarkSubMenus[sortedLandmarks[k].Type]->addLine(
                ct.Name, "set_compas",
                toString("compass=%s|id=%d|menu=%s", _TargetCompass.c_str(),
                         (int)Targets.size() - 1, _Id.c_str()));
            selectable = true;
        ...
        // Not Empty?
        for (k = 0; k < landMarkSubMenus.size(); k++) {
          landMarkSubMenu->setHiddenLine(
              k + contLandMarkIndex, landMarkSubMenus[k]->getNumLines() <= 0);
        }
      }
    ...
    if (teamSubMenu) {
      ...
      for (k = 0; k < MaxNumPeopleInTeam; k++) {
        ...
        if (buildCompassTargetFromTeamMember(ct, k)) {
          Targets.push_back(ct);
          teamSubMenu->addLine(ct.Name, "set_compas",
                               toString("compass=%s|id=%d|menu=%s",
                                        _TargetCompass.c_str(),
                                        (int)Targets.size() - 1, _Id.c_str()));
          selectable = true;
    ...
    if (dialogsSubMenu) {
      ...
      // and add each entry
      const std::vector<CCompassDialogsManager::CCompassDialogsEntry> &entries =
          CCompassDialogsManager::getInstance().getEntries();
      const uint size = (uint)entries.size();
      for (uint i = 0; i < size; ++i) {
        CCompassTarget ct;
        if (CStringManagerClient::instance()->getDynString(entries[i].Text,
                                                           ct.Name)) {
          CSmartPtr<CDialogEntityPositionState> tracker =
              new CDialogEntityPositionState(i);
          ct.setPositionState(tracker);
          Targets.push_back(ct);
          dialogsSubMenu->addLine(
              ct.Name, "set_compas",
              toString("compass=%s|id=%d|menu=%s", _TargetCompass.c_str(),
                       (int)Targets.size() - 1, _Id.c_str()));
          selectable = true;
...
class CCompassDialogsStringCallback : public IStringWaitCallback {
  virtual void onDynStringAvailable(uint /* stringId */,
                                    const std::string &value) {
    uint size = (uint)CCompassDialogsManager::getInstance()._Entries.size();
    for (uint i = 0; i < size; i++) {
      std::string name;
      if (CStringManagerClient::instance()->getDynString(
              CCompassDialogsManager::getInstance()._Entries[i].Text, name)) {
        if (value == name) {
          CCompassDialogsManager::getInstance()._Entries[i] =
              CCompassDialogsManager::getInstance()._Entries.back();
