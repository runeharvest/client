...
struct CPeopleListDesc {
  ...
  std::string
      BaseContainerTemplateName; // name of the template for the base container
  sint InsertPosition; // -1 if the people list should be inserted at the end of
                       // the container list, or the index otherwise
  ...
  // ctor
  CPeopleListDesc()
      : ContactType(Unknown), InsertPosition(-1), Localize(false) {}
};
...
public:
  ...
  /** Add a people to the list, and returns its index or -1 if the creation
   * failed If this is a team mate, tells its index so that ic can be bound to
   * the database in the right location
   */
   ...
   */
  void displayMessage(uint index, const std::string &msg, NLMISC::CRGBA col,
                      uint numBlinks = 0);
  void displayLocalPlayerTell(const std::string &receiver, uint index,
  ...
  // Hide / show the symbol to say that a people is online (if the symbol is
  // available)
  void setOnline(uint index, TCharConnectionState online);
  // Hide / show the symbol to say that a people is blocked (don't receive
  // messages from him)
  ...
  // Get id of the main container. This is the value that was passed during the
  // creation ('Id' field of CPeopleListDesc)
  ...
  // Open or close if opened, the chat corresponding to the entry index in the
  // friend list
...
private:
  struct CPeople {
    CPeople()
        : Container(NULL), Chat(NULL), Online(ccs_offline), Blocked(false),
          ContactId(0) {}
    NLMISC::CRefPtr<CGroupContainer>
        Container; // todo : replace this with a CChatWindow one day, for
                   // consistency
    ...
    bool Blocked;
    uint32 ContactId;
    bool operator<(const CPeople &other) const {
...
private:
  ...
  virtual void childrenMoved(uint srcIndex, uint destIndex,
