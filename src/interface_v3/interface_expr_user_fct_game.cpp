...
DECLARE_INTERFACE_CONSTANT(getMaxDynChanPerPlayer,
                           CChatGroup::MaxDynChanPerPlayer);
DECLARE_INTERFACE_CONSTANT(getMaxNumMissions, MAX_NUM_MISSIONS);
// Outpost Constants
DECLARE_INTERFACE_CONSTANT(getOutpostEnumUnknownOutpostState,
                           OUTPOSTENUMS::UnknownOutpostState);
DECLARE_INTERFACE_CONSTANT(getOutpostEnumPeace, OUTPOSTENUMS::Peace);
DECLARE_INTERFACE_CONSTANT(getOutpostEnumWarDeclaration,
                           OUTPOSTENUMS::WarDeclaration);
DECLARE_INTERFACE_CONSTANT(getOutpostEnumAttackBefore,
                           OUTPOSTENUMS::AttackBefore);
DECLARE_INTERFACE_CONSTANT(getOutpostEnumAttackRound,
                           OUTPOSTENUMS::AttackRound);
DECLARE_INTERFACE_CONSTANT(getOutpostEnumAttackAfter,
                           OUTPOSTENUMS::AttackAfter);
DECLARE_INTERFACE_CONSTANT(getOutpostEnumDefenseBefore,
                           OUTPOSTENUMS::DefenseBefore);
DECLARE_INTERFACE_CONSTANT(getOutpostEnumDefenseRound,
                           OUTPOSTENUMS::DefenseRound);
DECLARE_INTERFACE_CONSTANT(getOutpostEnumDefenseAfter,
                           OUTPOSTENUMS::DefenseAfter);
...
static DECLARE_INTERFACE_USER_FCT(isOpen) {
  ...
  if (!args[0].toString())
    return false;
  CGroupContainer *elm = dynamic_cast<CGroupContainer *>(
...
static DECLARE_INTERFACE_USER_FCT(getCompassText) {
  ...
  // cast as double now.
  sint direction =
      (sint)floor(0.5 + (8.0 * (angle.d[0] + NLMISC::Pi) / (NLMISC::Pi)));
  direction = ((direction % 16) + 16) % 16;
  static const string txts[] = {
      "uiW", "uiWSW", "uiSW", "uiSSW", "uiS", "uiSSE", "uiSE", "uiESE",
      "uiE", "uiENE", "uiNE", "uiNNE", "uiN", "uiNNW", "uiNW", "uiWNW",
  };
...
static DECLARE_INTERFACE_USER_FCT(getDifficultyText) {
  ...
  SENTENCE_APPRAISAL::ESentenceAppraisal sa =
      (SENTENCE_APPRAISAL::ESentenceAppraisal)args[0].getInteger();
...
static DECLARE_INTERFACE_USER_FCT(isCharSelSlotEmpty) {
  ...
  if (nSlotSelected >= CharacterSummaries.size())
    res = true;
  else if (CharacterSummaries[nSlotSelected].Name.empty())
    res = true;
  result.setBool(res);
  return true;
}
...
// ***************************************************************************
// Get for the character selection if the specified character is in a ring
// session or not
static DECLARE_INTERFACE_USER_FCT(isCharSelSlotInRingSession) {
  ...
  bool res = false;
  if (nSlotSelected < CharacterSummaries.size() &&
      !CharacterSummaries[nSlotSelected].Name.empty()) {
    res = CharacterSummaries[nSlotSelected].InRingSession;
  }
  result.setBool(res);
  return true;
}
REGISTER_INTERFACE_USER_FCT("isCharSelSlotInRingSession",
                            isCharSelSlotInRingSession);
...
static DECLARE_INTERFACE_USER_FCT(isCharSelSlotHasEditSession) {
  ...
  if (!args[0].toInteger()) {
    nlwarning(
        "<isCharSelSlotHasEditSession> Can't convert arg 0 to a int value.");
    return false;
  }
...
REGISTER_INTERFACE_USER_FCT("isCharSelSlotHasEditSession",
                            isCharSelSlotHasEditSession);
// ***************************************************************************
...
static DECLARE_INTERFACE_USER_FCT(getClientActionTypeColor) {
  ...
  CInterfaceOptions *options =
      CWidgetManager::getInstance()->getOptions("action_bar_color");
  if (options) {
    const CInterfaceOptionValue &optVal =
        options->getValue(CLIENT_ACTION_TYPE::toString(actType));
...
REGISTER_INTERFACE_USER_FCT("getClientActionTypeColor",
...
static DECLARE_INTERFACE_USER_FCT(getSmoothServerTick) {
  ...
  result.setInteger(
      NetMngr.convertToSmoothServerTick((sint32)args[0].getInteger()));
...
static DECLARE_INTERFACE_USER_FCT(getVSIndex) {
  ...
  SLOTTYPE::EVisualSlot vs = SLOTTYPE::convertTypeToVisualSlot(
      SLOTTYPE::stringToSlotType(args[1].getString()));
...
static DECLARE_INTERFACE_USER_FCT(getKey) {
  ...
  CActionsManager::TActionComboMap::const_iterator it =
      acmap.find(CAction::CName(name.c_str(), param.c_str()));
  if (it != acmap.end()) {
...
static DECLARE_INTERFACE_USER_FCT(isUserEntityDead) {
  ...
  CCDBNodeLeaf *node =
      NLGUI::CDBManager::getInstance()->getDbProp(dbName, false);
  if (node) {
...
static DECLARE_INTERFACE_USER_FCT(getAnimalInventoryStateText) {
  ...
  } else if (!ANIMAL_STATUS::isInventoryAvailable(
...
REGISTER_INTERFACE_USER_FCT("getAnimalInventoryStateText",
                            getAnimalInventoryStateText)
...
static DECLARE_INTERFACE_USER_FCT(isSkillAtMax) {
  ...
  // NB: the 2nd arg is only used as a dependency value (every times any skill
  // change, isSkillAtMax() may change)
...
static DECLARE_INTERFACE_USER_FCT(getBotChatBuyFilter) {
  ...
  result.setString(toString(args[0].getInteger()) + "-" +
                   toString(args[1].getInteger()));
...
static DECLARE_INTERFACE_USER_FCT(getBotChatBuyFilterMPIcon) {
  ...
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  result.setString(CWidgetManager::getInstance()->getParser()->getDefine(
      RM_FABER_TYPE::toIconDefineString(
          RM_FABER_TYPE::TRMFType(args[0].getInteger()))));

  return true;
}
REGISTER_INTERFACE_USER_FCT("getBotChatBuyFilterMPIcon",
                            getBotChatBuyFilterMPIcon)
...
static DECLARE_INTERFACE_USER_FCT(getBotChatBuyFilterMPText) {
  ...
  RM_FABER_TYPE::TRMFType faberType =
      (RM_FABER_TYPE::TRMFType)args[0].getInteger();
...
REGISTER_INTERFACE_USER_FCT("getBotChatBuyFilterMPText",
                            getBotChatBuyFilterMPText)
...
static DECLARE_INTERFACE_USER_FCT(getBotChatItemSheetForItemType) {
  ...
  // get sheet name
  string name =
      CBotChatPageTrade::getItemSheetNameForItemType((ITEM_TYPE::TItemType)a0);
...
REGISTER_INTERFACE_USER_FCT("getBotChatItemSheetForItemType",
...
static DECLARE_INTERFACE_USER_FCT(getSquadCost) {
  ...
  COutpostSquadSheet *pOSS =
      dynamic_cast<COutpostSquadSheet *>(SheetMngr.get(CSheetId(nSheet)));
  if (pOSS == NULL)
    result.setInteger(0);
  else
...
static DECLARE_INTERFACE_USER_FCT(getOutpostPeriod) {
  ...
  OUTPOSTENUMS::TOutpostState status =
      (OUTPOSTENUMS::TOutpostState)args[3].getInteger();
  bool isAttackPeriod = (sint32)args[4].getInteger() != 0;

  // if status wanted is peace or unknow, then "N/A", because there is no attack
  // period in peace mode
  if (status == OUTPOSTENUMS::Peace ||
      status == OUTPOSTENUMS::UnknownOutpostState) {
    result.setString(string(" - "));
    return true;
  }
