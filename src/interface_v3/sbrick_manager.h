...
public:
  ...
  bool isBrickKnown(CSBrickSheet *brick) const {
    ...
    return (getKnownBrickBitField(brick->BrickFamily) &
            (sint64(1) << (brick->IndexInFamily - 1))) != 0;
  }
  /// same with sheetId
  ...
  /// Get the Interface Brick (visual only) used to remove a optional or credit
  /// brick
...
protected:
  ...
  class CBrickContainer : public CSabrinaCom::IBrickContainer {
    ...
    virtual BRICK_FAMILIES::TBrickFamily
    getBrickFamily(NLMISC::CSheetId id, uint &indexInFamily) const;
    virtual BRICK_TYPE::EBrickType getBrickType(NLMISC::CSheetId id) const;
    virtual TOOL_TYPE::TCraftingToolType
    getFaberPlanToolType(NLMISC::CSheetId id) const;
  };
