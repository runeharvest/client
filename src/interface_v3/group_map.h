 ...
 * - Map coordinates : In the [0, 1] range. [0, 0] is the upper left of map, and
 * [1, 1] is the lower right. the corners of the map in world coordinates are
 * given by their zone names (in the continent sheet)
 * - Screen coordinates :  in pixels
 * - Window coordinates : the same as screen coordinate, but relative to that
 * CInterfaceGroup
 *
...
public:
  ...
  struct IDeco {
    /** called when the element is added to the map. If the deco is an interface
     * element, it could add itself to this group child
     */
    virtual void onAdd(CGroupMap & /* owner */) {}
    virtual void onRemove(CGroupMap & /* owner */) {}
    virtual void onPreRender(CGroupMap & /* owner */) {}
    /** Called when the map has been scrolled or scaled. The deco should update
     * its pos here
     *
     */
...
public:
  ...
  virtual ~CGroupMap();
  // Add a decoration to the map. The map will call the 'onAdd' method. When
  // this object is destroyed, it will call the 'onRemove' method
  void addDeco(IDeco *deco);
  // Remove a decoration from the map. This will also call the 'onRemove'
  // method. It is up to the owner to delete it.
  ...
  virtual bool getCtrlsUnder(sint32 x, sint32 y, sint32 clipX, sint32 clipY,
   ...
   * The center of the scale transformation must be given in the map
   * coordinates.
   */
  ...
  /// add a user landmark (returns a pointer on its button).Coordinate are in
  /// the current map (not world coordinates)
  CCtrlButton *addUserLandMark(const NLMISC::CVector2f &pos,
  ...
  void updateUserLandMark(CCtrlButton *button, const ucstring &newName,
                          const CUserLandMark::EUserLandMarkType lmType);
  ...
  // search matching landmark and target it. return true if landmark was
  // targeted
  ...
  void
  setSelectionAxis(bool active,
                   const NLMISC::CVector2f &worldPos = NLMISC::CVector2f::Null);

  // convert a pos in world to a pos in the window, snapped to the best pixel
  // (-> all elements jump to the next pixel at the same time when the map is
  // panned, avoiding annoying flickering)
  void worldToWindowSnapped(sint32 &px, sint32 &py,
                            const NLMISC::CVector2f &src) const;
  void worldToWindow(NLMISC::CVector2f &dest,
                     const NLMISC::CVector2f &src) const;
  void mapToWindowSnapped(sint32 &px, sint32 &py,
                          const NLMISC::CVector2f &src) const;
  ...
  void windowToScreen(sint32 &destX, sint32 &destY, sint32 srcX,
                      sint32 srcY) const;
  // convert a pos in the map to a pos on screen
  void mapToScreen(sint32 &px, sint32 &py, const NLMISC::CVector2f &src) const;
  //
  // convert a pos in the map to a pos relative to the current window (int
  // result)
  ...
  //
  const NLMISC::CVector2f &getVisibleWorldMin() const {
    return _VisibleWorldMin;
  }
  const NLMISC::CVector2f &getVisibleWorldMax() const {
    return _VisibleWorldMax;
  }

  // compute pos of displayed map relative to this group (may be equal are
  // smaller than this group extent depending on the scale.
  void computeMapRectInsideGroup(sint32 &x, sint32 &y, sint32 &w,
                                 sint32 &h) const;

  // From CInterfaceElement : scale and offset must persist between virtual
  // desktops
  ...
  void updateClosestLandMarkMenu(const std::string &menu,
...
private:
  ...
  public:
    ...
    virtual void draw() {}
    void drawPolyButton();
    bool build(const NLLIGO::CPrimZone &concavePoly, CGroupMap *pMap,
  ...
  public:
    ...
    virtual bool isCapturable() const { return HandleEvents; }
    virtual bool wantInstantContextHelp() const {
      return true;
    } // from CCtrlBase : avoid delay when the mouse is over
    CLandMarkButton(const TCtorParam &param) : CCtrlButton(param) {
  ...
  public:
    ...
    CLandMarkText(const TCtorParam &param) : CViewText(param) {
      Type = CContLandMark::Unknown;
      Pos.set(0.f, 0.f);
      SearchMatch = false;
    }
...
private:
  ...
  SMap *_CurMap;
  CContinent *_CurContinent;       // the last continent for which the map was
                                   // displayed (can be NULL if world)
  ...
  bool _IsIsland; // true if current map is an island (island bitmap need not to
                  // be raised to the next power of 2
  ...
  struct SMatchedLandmark {
    SMatchedLandmark(const NLMISC::CVector2f pos, const ucstring &title,
  ...
  enum TMapMode { MapMode_Normal = 0, MapMode_Death, MapMode_SpawnSquad };
...
private:
  ...
  void addLandMark(TLandMarkButtonVect &destList, const NLMISC::CVector2f &pos,
                   const ucstring &title, const CLandMarkOptions &options);
  ...
  void updateLandMarkButton(CLandMarkButton *lmb,
                            const CLandMarkOptions &options);

  // Test title against landmark filter
  bool filterLandmark(const ucstring &title) const;
  bool filterLandmark(const ucstring &title, const std::vector<ucstring> filter,
  ...
  CLandMarkButton *findClosestLandmark(const NLMISC::CVector2f &center,
                                       const ucstring &search, bool startsWith,
                                       const TLandMarkButtonVect &landmarks,
                                       float &closest) const;
  CLandMarkText *findClosestLandmark(const NLMISC::CVector2f &center,
                                     const ucstring &search, bool startsWith,
                                     const TLandMarkTextVect &landmarks,
                                     float &closest) const;

  // update the scale depending on the window size and the user scale
  void updateScale();

  // compute real scale from user scale (this takes in account the size of the
  // window)
  float computeRealScaleFromUserScale(float userScale) const;

  // compute the user scale needed to reach the given real scale on screen (this
  // takes in account the size of the window)
  ...
  // compute _Offset from the _WorldOffset. Then clamp so that region outside
  // the map can't be seen
  ...
  // Update a landmark position from position given from the db as two int32's
  // (before they're divided by 1000)
