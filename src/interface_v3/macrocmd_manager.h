...
public:
  ...
  public:
    ...
    CCommand(const char *name) { Name = name; }
...
public:
  ...
  void addCommand(const std::string &sName, const std::string &sParam,
                  sint32 nPos = -1);
...
public:
  ...
  sint32 getTexIdBack(uint8 nb) {
    return (nb >= _OptBackId.size()) ? _OptBackId[_OptBackId.size() - 1]
                                     : _OptBackId[nb];
  }
  sint32 getTexIdIcon(uint8 nb) {
    return (nb >= _OptIconId.size()) ? _OptIconId[_OptIconId.size() - 1]
                                     : _OptIconId[nb];
  }
  sint32 getTexIdOver(uint8 nb) {
    return (nb >= _OptOverId.size()) ? _OptOverId[_OptOverId.size() - 1]
                                     : _OptOverId[nb];
  }
...
#define GROUP_NEWMACRO_COMMANDS                                                \
  "ui:interface:new_macro:content:cmds:macros:list"
...
#define GROUP_MACROICONCREATION_BACK                                           \
  "ui:interface:macro_icon_creation:IconBackground"
#define GROUP_MACROICONCREATION_ICON                                           \
  "ui:interface:macro_icon_creation:IconForeground"
#define GROUP_MACROICONCREATION_OVER                                           \
  "ui:interface:macro_icon_creation:IconPattern"
#define CTRL_MACROICONCREATION_BUTTON ":but"
#define CTRL_MACROICONCREATION_EDITTEXT                                        \
  "ui:interface:macro_icon_creation:edit_name"
