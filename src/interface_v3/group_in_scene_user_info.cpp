...
NLMISC::CRefPtr<NLMISC::CCDBNodeLeaf>
    CGroupInSceneUserInfo::_GuildIconLeaf[256];

// ***************************************************************************
NLMISC_REGISTER_OBJECT(CViewBase, CGroupInSceneUserInfo, std::string,
                       "in_scene_user_info");
...
public:
  ...
  inline NLMISC::CCDBNodeLeaf *getGuildSymbol() {
    return _GuildSymbol
               ? (&*_GuildSymbol)
               : &*(_GuildSymbol = NLGUI::CDBManager::getInstance()->getDbProp(
                        _DBPrefix + "GUILD_SYMBOL"));
  }
  inline NLMISC::CCDBNodeLeaf *getName() {
    return _Name ? (&*_Name)
                 : &*(_Name = NLGUI::CDBManager::getInstance()->getDbProp(
                          _DBPrefix + "NAME"));
  }
  inline NLMISC::CCDBNodeLeaf *getTitle() {
    return _Title ? (&*_Title)
                  : &*(_Title = NLGUI::CDBManager::getInstance()->getDbProp(
                           _DBPrefix + "TITLE"));
  }
  inline NLMISC::CCDBNodeLeaf *getRPTags() {
    return _RPTags ? (&*_RPTags)
                   : &*(_RPTags = NLGUI::CDBManager::getInstance()->getDbProp(
                            _DBPrefix + "RPTAGS"));
  }
  inline NLMISC::CCDBNodeLeaf *getGuildName() {
    return _GuildName
               ? (&*_GuildName)
               : &*(_GuildName = NLGUI::CDBManager::getInstance()->getDbProp(
                        _DBPrefix + "GUILD_NAME"));
  }
  inline NLMISC::CCDBNodeLeaf *getHP() {
    return _HP ? (&*_HP)
               : &*(_HP = NLGUI::CDBManager::getInstance()->getDbProp(
                        _DBPrefix + "HP"));
  }
  inline NLMISC::CCDBNodeLeaf *getSta() {
    return _Sta ? (&*_Sta)
                : &*(_Sta = NLGUI::CDBManager::getInstance()->getDbProp(
                         _DBPrefix + "STA"));
  }
  inline NLMISC::CCDBNodeLeaf *getSap() {
    return _Sap ? (&*_Sap)
                : &*(_Sap = NLGUI::CDBManager::getInstance()->getDbProp(
                         _DBPrefix + "SAP"));
  }
  inline NLMISC::CCDBNodeLeaf *getFocus() {
    return _Focus ? (&*_Focus)
                  : &*(_Focus = NLGUI::CDBManager::getInstance()->getDbProp(
                           _DBPrefix + "FOCUS"));
  }
  inline NLMISC::CCDBNodeLeaf *getAction() {
    return _Action ? (&*_Action)
                   : &*(_Action = NLGUI::CDBManager::getInstance()->getDbProp(
                            _DBPrefix + "ACTION"));
  }
  inline NLMISC::CCDBNodeLeaf *getMessages() {
    return _Messages
               ? (&*_Messages)
               : &*(_Messages = NLGUI::CDBManager::getInstance()->getDbProp(
                        _DBPrefix + "MESSAGES"));
  }
  inline NLMISC::CCDBNodeLeaf *getPvPLogo() {
    return _PvPLogo ? (&*_PvPLogo)
                    : &*(_PvPLogo = NLGUI::CDBManager::getInstance()->getDbProp(
                             _DBPrefix + "PVP_LOGO"));
  }
  inline NLMISC::CCDBNodeLeaf *getNPCName() {
    return _NPCName ? (&*_NPCName)
                    : &*(_NPCName = NLGUI::CDBManager::getInstance()->getDbProp(
                             _DBPrefix + "NPCNAME"));
  }
  inline NLMISC::CCDBNodeLeaf *getNPCTitle() {
    return _NPCTitle
               ? (&*_NPCTitle)
               : &*(_NPCTitle = NLGUI::CDBManager::getInstance()->getDbProp(
                        _DBPrefix + "NPCTITLE"));
  }
  inline NLMISC::CCDBNodeLeaf *getMissionIcon() {
    return _MissionIcon
               ? (&*_MissionIcon)
               : &*(_MissionIcon = NLGUI::CDBManager::getInstance()->getDbProp(
                        _DBPrefix + "MISSION_ICON"));
  }
  inline NLMISC::CCDBNodeLeaf *getMiniMissionIcon() {
    return _MiniMissionIcon
               ? (&*_MiniMissionIcon)
               : &*(_MiniMissionIcon =
                        NLGUI::CDBManager::getInstance()->getDbProp(
                            _DBPrefix + "MINI_MISSION_ICON"));
  }
...
CRGBA CGroupInSceneUserInfo::BarColor[NumBars] = {
    CRGBA(255, 64, 0),  CRGBA(72, 255, 0),    CRGBA(255, 0, 255),
    CRGBA(0, 128, 255), CRGBA(255, 255, 255),
};
...
CGroupInSceneUserInfo *CGroupInSceneUserInfo::build(CEntityCL *entity) {
  ...
  bool npcFriendAndNeutral =
      (entity->canHaveMissionIcon() || entity->isFauna()) && entity->isFriend();
  ...
  bool rpTags = false;
  bool displayMissionIcons =
      _ConfigSaveInsceneDB[SAVE_FRIEND].getMissionIcon()->getValueBool();
  ...
  if (isForageSource) {
    ...
    name =
        !entityName
             .empty() /*&&
                         NLGUI::CDBManager::getInstance()->getDbProp(dbEntry+"NAME")->getValueBool()*/
        ;
    ...
    // bool displayExtractingParams = true;
    // //forageSource->isExtractionInProgress() /*&&
    // NLGUI::CDBManager::getInstance()->getDbProp(dbEntry+"HP")*/;
    ...
    // For RoleMasters, merchants etc... must display name and function, and
    // nothing else
    for (uint i = 0; i < NumBars; i++)
      bars[i] = false;
    name = !entityName.empty() &&
           _ConfigSaveInsceneDB[dbEntry].getNPCName()->getValueBool();
    symbol = false;
    title = !entityTitle.empty() &&
            _ConfigSaveInsceneDB[dbEntry].getNPCTitle()->getValueBool();
    guildName = false;
    templateName = "in_scene_user_info";
    rpTags = (!entityTag1.empty() || !entityTag2.empty() ||
              !entityTag3.empty() || !entityTag4.empty()) &&
             _ConfigSaveInsceneDB[dbEntry].getRPTags()->getValueBool();
  } else {
    // Base entry in database
    int dbEntry;
    getBarSettings(pIM, user, entity->isPlayer(), _friend, dbEntry, bars);
    name = !entityName.empty() &&
           _ConfigSaveInsceneDB[dbEntry].getName()->getValueBool();
    title = !entityTitle.empty() &&
            _ConfigSaveInsceneDB[dbEntry].getTitle()->getValueBool();
    rpTags = (!entityTag1.empty() || !entityTag2.empty() ||
              !entityTag3.empty() || !entityTag4.empty()) &&
             _ConfigSaveInsceneDB[dbEntry].getRPTags()->getValueBool();
    // if name is empty but not title, title is displayed as name
    if (!title && entityName.empty() && !entityTitle.empty() &&
        _ConfigSaveInsceneDB[dbEntry].getName()->getValueBool())
      title = true;
    templateName = "in_scene_user_info";
    // special guild
  ...
  if (info) {
    ...
    if (info && leftGroup) {
      ...
      fromString(CWidgetManager::getInstance()->getParser()->getDefine(
                     "in_scene_user_info_bar_h"),
                 barHeight);
      fromString(CWidgetManager::getInstance()->getParser()->getDefine(
                     "in_scene_user_info_bar_space"),
                 barSpace);
      fromString(CWidgetManager::getInstance()->getParser()->getDefine(
                     "in_scene_user_bar_length"),
                 CGroupInSceneUserInfo::_BatLength);
      ...
      if (rpTags) {
        ...
        CViewBitmap *bitmap;
        if (pPlayer == NULL ||
            (pPlayer != NULL && pPlayer->getPvpMode() & PVP_MODE::PvpFaction)) {
      ...
      if (isForageSource) {
        ...
        if (!symbol) {
          ...
          if (bitmap) {
            ...
            CForageSourceCL *forageSource =
                static_cast<CForageSourceCL *>(entity);
        ...
        if (bitmap && bitmapOver) {
          // Set the NPC icon
          const CNPCIconCache::CNPCIconDesc &iconDesc =
              CNPCIconCache::getInstance().getNPCIcon(entity, true);
          ...
          if (!symbol) {
            ...
            if (sheet) {
              ...
              NLGUI::CDBManager::getInstance()
                  ->getDbProp(dbLeaf + ":ICON")
                  ->setValue64(entity->getGuildSymbol());
            }
          }
        }
      }
      ...
      if (!isForageSource) {
        ...
        if (entity->isUser() && invisibleLogo) {
          bool invisible = NLGUI::CDBManager::getInstance()
                               ->getDbProp("SERVER:USER:IS_INVISIBLE")
                               ->getValueBool();
        ...
        if (pPlayer != NULL && needPvPLogo) {
          if (pvpFactionLogo) {
            ...
            CViewBitmap *pvpFactionBitmap =
                dynamic_cast<CViewBitmap *>(pvpFactionLogo);
            if (pvpFactionBitmap) {
              if (user) {
                ...
                if (pPlayer->getPvpMode() & PVP_MODE::PvpChallenge)
                  ...
                  pvpFactionBitmap->setTexture("pvp_neutral.tga");
                else if (pPlayer->isAlly() &&
                         (pPlayer->getPvpMode() & PVP_MODE::PvpFactionFlagged))
                  pvpFactionBitmap->setTexture("pvp_ally_flag.tga");
                else if (pPlayer->isAlly() &&
                         (pPlayer->getPvpMode() & PVP_MODE::PvpFaction))
      ...
      if (entity->isUser()) {
        _Value = NLGUI::CDBManager::getInstance()->getDbProp(
            "UI:VARIABLES:CURRENT_SMOOTH_SERVER_TICK");
        _ValueBegin = NLGUI::CDBManager::getInstance()->getDbProp(
            "UI:VARIABLES:SMOOTH_USER_ACT_START");
        _ValueEnd = NLGUI::CDBManager::getInstance()->getDbProp(
            "UI:VARIABLES:SMOOTH_USER_ACT_END");
      }
    ...
    for (i = 0; i < numEntity; i++) {
      ...
      if (entity) {
        ...
        if (character) {
          ...
          bool rebuild = true;
          if (pvpOnly) {
            if (character->getPvpMode() == 0 && character->getOutpostId() == 0)
              rebuild = false;
          }
...
REGISTER_ACTION_HANDLER(CHandlerResetCharacterInScene,
                        "reset_character_in_scene");

// ***************************************************************************
void CGroupInSceneUserInfo::getBarSettings(CInterfaceManager *pIM, bool isUser,
                                           bool isPlayer, bool isFriend,
                                           int &dbEntry, bool *bars) {
  ...
  if (ClientCfg.R2EDEnabled && R2::isEditionCurrent()) {
    ...
    bars[SAP] = (isUser || isFriend) && (isUser || isPlayer) &&
                _ConfigSaveInsceneDB[dbEntry].getSap()->getValueBool();
    bars[STA] = (isUser || isFriend) && (isUser || isPlayer) &&
                _ConfigSaveInsceneDB[dbEntry].getSta()->getValueBool();
    bars[Focus] = (isUser || isFriend) && (isUser || isPlayer) &&
                  _ConfigSaveInsceneDB[dbEntry].getFocus()->getValueBool();
    bars[Action] =
        (isUser) && _ConfigSaveInsceneDB[dbEntry].getAction()->getValueBool();
  }
}
...
void CGroupInSceneUserInfo::setLeftGroupActive(bool active) {
  ...
  getBarSettings(CInterfaceManager::getInstance(), _Entity->isUser(),
                 _Entity->isPlayer(), _Entity->isViewedAsFriend(), dbEntry,
                 barSettings);
...
void CGroupInSceneUserInfo::updateDynamicData() {
  ...
  if (_Entity->getGuildSymbol() != 0) {
    ...
    if (!_GuildIconLeaf[_Entity->slot()]) {
      ...
      _GuildIconLeaf[_Entity->slot()] =
          NLGUI::CDBManager::getInstance()->getDbProp(dbLeaf);
    }
  ...
  double manhattanDistance =
      getManhattanDistance(UserEntity->pos() - _Entity->pos());
  ...
  // If the entity is too far, hide the bars because their value is not updated
  // by the server
  if (manhattanDistance < CLFECOMMON::THRESHOLD_BARS_CLIENT_M) {
    ...
    // or directly from the forage source
      ...
      // NB: forage don't use CBarManager for 2 reasons: useless (forage bars
      // exist only through VP), and complicated since updated at each frame on
      // client (because of smooth transition code)
      CForageSourceCL *forageSource = static_cast<CForageSourceCL *>(_Entity);
      barInfo.Score[SCORES::hit_points] =
          forageSource->getTimeBar(); // Map TimeBar to HP
      barInfo.Score[SCORES::sap] =
          forageSource->getQuantityBar(); // Map QuantityBar to SAP
      barInfo.Score[SCORES::stamina] =
          forageSource->getDBar(); // Map D Bar to Sta
      barInfo.Score[SCORES::focus] =
          forageSource->getEBar(); // Map E Bar to Focus
    }

    // Set the bar
    if (_Bars[HP]) {
      sint value =
          (sint)_BatLength * barInfo.Score[SCORES::hit_points] / RZ_BARS_LENGTH;
      if (!_Entity->isForageSource()) {
        if (value < 0) {
          value = -value;
        ...
        if (_Entity->mode() == MBEHAV::DEATH && !_Entity->isPlayer() &&
            !_Entity->isUser()) {
          if (value > 0) {
            value = 0;
          }
        ...
        if (forageSource->isInExtraTime()) {
          _Bars[HP]->setColorRGBA(forageSource->isInProspectionExtraTime()
                                      ? CRGBA(255, 0, 0)
                                      :                    // red
    ...
    if (_Bars[SAP]) {
      ...
      if (_Entity->isForageSource()) {
        ...
        CForageSourceCL *forageSource = static_cast<CForageSourceCL *>(_Entity);
        string txt(CI18N::get("uittForageContent") +
                   toString(": %u", forageSource->getCurrentQuantity()));
    ...
    if (_Bars[STA]) {
      ...
      if (_Bars[STA]->getW() != value) {
        ...
        if (_Entity->isForageSource()) {
          ...
          color.blendFromuiRGBOnly(
              CRGBA(255, 127, 127), CRGBA(255, 0, 0),
              (RZ_BARS_LENGTH - barInfo.Score[SCORES::stamina]) *
                  (256 / RZ_BARS_LENGTH));
    ...
    if (_Entity->isUser() && _Bars[Action]) {
      ...
      if (end != begin) {
        value = (sint32)_BatLength * (sint32)(value - begin) /
                (sint32)(end - begin);
        clamp(value, 0, (int)_BatLength);
      } else {
        value = 0;
      ...
      CForageSourceCL *forageSource = static_cast<CForageSourceCL *>(_Entity);
      sint32 value = (sint32)_BatLength *
                     (sint32)forageSource->getKamiAngerBar() / RZ_BARS_LENGTH;
      if (_Bars[Action]->getW() != value)
  ...
  if (_Entity && !_Entity->isForageSource()) {
    if (_Entity->slot() == SlotUnderCursor ||
        _Entity->slot() == UserEntity->selection())
      setZBias(-2.f);
    else
      setZBias(0.f);
  }
...
CGroupInSceneUserInfo::newGroupInScene(const std::string &templateName,
                                       const std::string &id) {
  ...
  if (R2::getEditor().getMode() == R2::CEditor::EditionMode && !bypass) {
    // NB : only use the fast version during edition because duplication of
    // CCDBCtrlSheet not implemented now, but we don't use it for the edition
    // !!!!
    CInterfaceElement *prototype =
        CWidgetManager::getInstance()->getElementFromId(
            "ui:interface:" + templateName + "_proto");
    if (prototype && dynamic_cast<CInterfaceGroup *>(prototype)) {
      extern bool
          NoOpForCCtrlSheetInfo_Serial; // CCDBCtrlSheet::serial not
                                        // implemented, but prevent an assert in
                                        // its serial because we don't use it in
                                        // this special case (R2, Edition)
  ...
  if (!groupInfo) {
    ...
    groupInfo = CWidgetManager::getInstance()->getParser()->createGroupInstance(
        templateName, "ui:interface",
        templateParams.empty() ? NULL : &(templateParams[0]),
        (uint)templateParams.size());
  }

  CGroupInSceneUserInfo *info =
      dynamic_cast<CGroupInSceneUserInfo *>(groupInfo);
  if (!info) {
    delete groupInfo;
  }
  return info;
