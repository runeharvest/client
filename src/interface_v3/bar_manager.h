 ...
 *		- From Visual Property, often more frequently updated, but only
 *if Entity is in Vision AND within a certain distance (aka VP threshold which
 *is for instance 30m for Bars)
 *		- From DB (Target, Team, Animal), which are always sent, but
 *less frequently updated The purpose of this manager is to take either the
 *Visual Property or the Database Value, deciding which is the most accurate
 *one. Then the same values are used for the Target, Team and Animal interface,
 *as the 3D InScene interface \author Lionel Berenguier \author Nevrax France
 * \date 2004
 */
class CBarManager {
public:
...
public:
  ...
  /** \name Bar registration. Each Entity is uniquely identified by its
   *dataSetId. The entity can be "connected" through diffent ways (entity in
   *vision, entry in team list etc...). If it was not connected before, its Bars
   *value are reseted WARNING: each of this method assert that the entryId
   *(entityId etc...) is less than the corresponding Max Size (MaxEntity etc...)
   */
  // @{
  // register an entity in Vision. reset bar values to 0, and assign dataSetId
  // to entity
  void addEntity(CLFECOMMON::TCLEntityId entityId, uint dataSetId);
  void delEntity(CLFECOMMON::TCLEntityId entityId);
  // register a TeamMember. NB: no-op if already registered. YOU SHOULD USE
  // updateTeamMemberFromDB()
  void addTeamMember(uint teamMemberId, uint dataSetId);
  void delTeamMember(uint teamMemberId);
  // register a Pet Animal. NB: no-op if already registered. YOU SHOULD USE
  // updateAnimalFromDB()
  void addAnimal(uint paId, uint dataSetId);
  void delAnimal(uint paId);
  // register the target. NB: no-op if already registered. YOU SHOULD USE
  // updateTargetFromDB()
  void addTarget(uint dataSetId);
  void delTarget();
  // @}

  /** called either by VP or DB receive. NB: no-op if the dataSetId was not
   *added through any of preceding fct \param serverTick the server date
   *validity of this info (if too old, skiped) \param scoreFlags an ORed of
   *TScoreFlag. if not set, the value is not relevant (eg: Team DB don't precise
   *Focus)
   */
  void updateBars(uint dataSetId, CBarInfo barInfo,
                  NLMISC::TGameCycle serverTick, uint scoreFlags);

  // do the appropriate addEntry(), delEntry() or/and updateBars(), according to
  // SERVER db change
  void updateTeamMemberFromDB(uint teamMemberId);
  void updateAnimalFromDB(uint paId);
  void updateTargetFromDB();

  // special for the Target. Called when the client select an entity => Out
  // Target Database is flushed with current data
  ...
  void setupUserBarInfo(uint8 msgNumber, sint32 hp, sint32 sap, sint32 sta,
                        sint32 focus);
  /// From last setuped user HP/SAP/STA/FOCUS, and current database MAX, setup
  /// the Bars for the user (slot 0) entry
  void updateUserBars();
...
private:
  ...
  class CBarDataUID {
  public:
    /* What connection are valid. empty() in each case if not, else the index in
     *the array of entries NB: this is a set because some time, a bar data may
     *be connected to multiple entries of same type This typically happens when
     *you dismiss the team member 0 'Paul' while you have a team member 1
     *'Pierre': because of the server array shift,	there will be a short
     *time where TeamMember0= TeamMember1= Pierre Hence the set: Pierre is bound
     *to 2 teammember entries: 0 and 1.
     */
  ...
  public:
    ...
    // Connection input (used only for TargetType, TeamMemberType and
    // AnimalType)
    NLMISC::CCDBNodeLeaf *UIDIn;
    NLMISC::CCDBNodeLeaf
        *PresentIn; // if not NULL, this is an additional test:
                    // if(PresentIn->getValue()==0) => not present
  ...
  public:
    ...
    void clear();
    // connect
    void connectDB(const std::string &baseDBin, const std::string &baseDBout,
                   const std::string &presentDB, const std::string &hpDB,
                   const std::string &sapDB, const std::string &staDB,
