...
// ***************************************************************************
/**
 * Interface to access number of sheet elements, and elements in a ListSheet or
 * ListSheetTrade \author Lionel Berenguier \author Nevrax France \date 2003
 */
...
protected:
  ...
  bool insertSectionGroup(uint sectionGroupIndex, uint sectionId, sint xGroup,
                          sint yGroup);
  void releaseSectionGroups(uint startSectionGroupIndex);
  // if sectionable, the deriver must implement those method
  // min and maxSectionId are exclusive (ie the encoutered sectionId should not
  // be of those values)
  virtual void getCurrentBoundSectionId(sint &minSectionId,
                                        sint &maxSectionId) {
    minSectionId = 0;
    maxSectionId = 0;
  }
  virtual CInterfaceGroup *
  createSectionGroup(const std::string & /* igName */) {
    return NULL;
  }
