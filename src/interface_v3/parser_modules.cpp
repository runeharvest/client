...
bool CIF3DSceneParser::parse(xmlNodePtr cur,
                             NLGUI::CInterfaceGroup *parentGroup) {
...
CIFDDXParser::CIFDDXParser() { parsingStage |= (Resolved | GroupChildren); }
...
CActionCategoryParser::CActionCategoryParser() { parsingStage |= Unresolved; }

CActionCategoryParser::~CActionCategoryParser() {}

bool CActionCategoryParser::parse(xmlNodePtr cur,
                                  NLGUI::CInterfaceGroup *parentGroup) {
  // The category
  CCategory category;
  ...
  if (actionNode) {
    do {
      ...
      if (paramNode) {
        do {
          // The parameter
          CBaseAction::CParameter &parameter =
          ...
          // Parse instance
          xmlNodePtr instanceNode =
              CIXml::getFirstChildNode(paramNode, "instance");
          if (instanceNode) {
            do {
              if (!parser->parseInstance(instanceNode)) {
                // todo hulud interface syntax error
                nlwarning("<CInterfaceParser::parseActionCategory> cannot "
                          "create instance from template");
              }
            } while ((instanceNode =
                          CIXml::getNextChildNode(instanceNode, "instance")));
          }
          ...
          if (valueNode) {
            do {
              // The value
              CBaseAction::CParameter::CValue &value =
              ...
              if (ptr)
                value.Contexts = (const char *)ptr;
              else
                value.Contexts = action.Contexts; // inherit context from action
  ...
  CActionsManager *actionManager =
      ActionsContext.getActionsManager(category.Name);
  if (actionManager) {
// They want to display debug shortcut in final version
...
#endif // FINAL_VERSION
    {
      ...
      for (i = 0; i < category.BaseActions.size(); i++) {
        CAHManager::TFactoryMap::iterator ite =
            pAHFM->FactoryMap.find(category.BaseActions[i].Name);
...
CCommandParser::CCommandParser() { parsingStage |= Unresolved; }

CCommandParser::~CCommandParser() {}

bool CCommandParser::parse(xmlNodePtr cur,
                           NLGUI::CInterfaceGroup *parentGroup) {
  // Parse the key
  bool ret = false;
  ...
  if (ptrName) {
    ...
    if (!ICommand::exists(name) || (CUserCommand::CommandMap.find(name) !=
                                    CUserCommand::CommandMap.end())) {
      // Get the action
      CXMLAutoPtr ptrAction((const char *)xmlGetProp(cur, (xmlChar *)"action"));
      if (ptrAction) {
        // Get the params
        CXMLAutoPtr ptrParams(
            (const char *)xmlGetProp(cur, (xmlChar *)"params"));
        if (ptrParams) {
          CUserCommand::createCommand(ptrName, ptrAction, ptrParams);

          // if prop "ctrlchar" is declared with false, then disable ctrlchar
          // for this command
          CXMLAutoPtr prop(
              (const char *)xmlGetProp(cur, (xmlChar *)"ctrlchar"));
          if ((const char *)prop &&
              (CInterfaceElement::convertBool((const char *)prop) == false))
        ...
        // todo hulud interface syntax error
        nlwarning("<CInterfaceParser::parseCommand> No action for command : %s",
                  (const char *)ptrName);
      }
    }
  } else {
...
CKeyParser::CKeyParser() { parsingStage |= Unresolved; }
  ...
  if (ptrKey) {
    ...
    if (key != KeyCount || isNA) {
      ...
      if (ptrAction) {
        ...
        // Get the context
        CXMLAutoPtr ptrContext(
            (const char *)xmlGetProp(cur, (xmlChar *)"context"));
        std::string context =
            (const char *)ptrContext ? (const char *)ptrContext : "";

        // Add the action
        CCombo combo;
        combo.init(key, (TKeyButton)((shift ? shiftKeyButton : noKeyButton) |
                                     (ctrl ? ctrlKeyButton : noKeyButton) |
                                     (menu ? altKeyButton : noKeyButton)));
        ::CAction::CName actionName((const char *)ptrAction,
                                    ptrParams ? (const char *)ptrParams : "");

        // Get the actions context manager
        CActionsManager *actionManager =
            ActionsContext.getActionsManager(context);
        if (actionManager) {
          bool canAdd = true;
          ...
          if (parser->getDefine("key_def_no_replace") == "1") {
            ...
            if (isNA || actionManager->isComboAssociated(combo) ||
                actionManager->isActionAssociated(actionName))
              // don't replace
              canAdd = false;
          }
        ...
        // todo hulud interface syntax error
        nlwarning("<CInterfaceParser::parseKey> No action for key : %s",
                  (const char *)ptrKey);
      }
    } else {
      // todo hulud interface syntax error
      nlwarning("<CInterfaceParser::parseKey> Unknown key : %s",
                (const char *)ptrKey);
    }
  } else {
    // todo hulud interface syntax error
...
CMacroParser::CMacroParser() { parsingStage |= Unresolved; }
...
CLandmarkParser::CLandmarkParser() { parsingStage |= Unresolved; }

CLandmarkParser::~CLandmarkParser() {}

bool CLandmarkParser::parse(xmlNodePtr cur,
                            NLGUI::CInterfaceGroup *parentGroup) {
