...
// 500 seems to add at max 200Ko in memory (estimation done with the
// WindowTaskManager, so not really accurate)
const uint MaxFlyingText = 500;

// ***************************************************************************
CFlyingTextManager::CFlyingTextManager() { _CharacterWindowOffsetX = 0; }
...
void CFlyingTextManager::initInGame() {
  ...
  for (uint i = 0; i < MaxFlyingText; i++) {
    ...
    CInterfaceGroup *groupInfo =
        CWidgetManager::getInstance()->getParser()->createGroupInstance(
            "in_scene_flying_text", "ui:interface", templateParams);
    // if ok
    if (groupInfo) {
      CGroupInfo gi;
      ...
      if (gi.GroupInScene && gi.GroupInScene) {
        ...
        /* don't link to the interface now. Prefer do this to avoid big number
           of window to parse each frame for nothing because they are hid. This
           save 1ms per frame for 1000 window created but not displayed.... NB:
           the cost of link/unlink instead of a setActive() is negligible in
           effect
        */
...
// ***************************************************************************
void CFlyingTextManager::addFlyingText(void *key, const string &text,
                                       const NLMISC::CVector &pos, CRGBA color,
                                       float scale, sint offsetX) {
