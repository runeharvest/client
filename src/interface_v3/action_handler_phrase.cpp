...
const std::string PhraseCompositionGroup =
...
void debugUpdateActionBar() {
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:USER:ACT_TSTART")
      ->setValue64(NetMngr.getCurrentServerTick());
  NLGUI::CDBManager::getInstance()
      ->getDbProp("SERVER:USER:ACT_TEND")
      ->setValue64(NetMngr.getCurrentServerTick() + 30);
}
...
public:
  virtual void execute(CCtrlBase *pCaller, const string & /* Params */) {
    ...
    // Nb: Yoyo: this slot can come from eiter the Action Book, or the Memorized
    // sentence
...
public:
  virtual void execute(CCtrlBase *pCaller, const string & /* Params */) {
    ...
    // if possible
    if (pPM->hasFreeSlot()) {
      // Set the phrase to edit to 0, to indicate a new phrase. CANNOT
      // "ALLOCATE" THE ID NOW, else may crash if buy a botchatphrase, while
      // editing a new phrase!!!!
      pPM->CompositionPhraseId = 0;

      // If the caller is ctrl sheet (may be a standard button else), and if
      // comes from memory
      CDBCtrlSheet *pCSDst = dynamic_cast<CDBCtrlSheet *>(pCaller);
...
public:
  virtual void execute(CCtrlBase *pCaller, const string & /* Params */) {
    ...
    if (parent) {
      ...
      CDBGroupBuildPhrase *buildGroup =
          dynamic_cast<CDBGroupBuildPhrase *>(parent->getParent());
      if (!buildGroup)
        return;
      ...
      // Action edition case
        ...
        // Then, auto-re_memorize all after, changing Ctrl Gray States if
        // needed, and sending msg to server
...
public:
  virtual void execute(CCtrlBase *pCaller, const string & /* Params */) {
    ...
    if (!pGC)
      return;
    if (!pGC->getHeaderOpened())
      return;
    CDBGroupBuildPhrase *buildPhrase =
        dynamic_cast<CDBGroupBuildPhrase *>(pGC->getHeaderOpened());
    if (!buildPhrase)
      return;
    ...
    if (button && button->getActive() && !button->getFrozen()) {
      ...
      CAHManager::getInstance()->runActionHandler(
          button->getActionOnLeftClick(), button,
          button->getParamsOnLeftClick());
    }
  }
};
REGISTER_ACTION_HANDLER(CHandlerPhraseValidateOnEnter,
    ...
    if (buildSentenceTarget) {
      ...
      // if edition
        ...
        if (phrase.empty()) {
          ...
          if (rootBrick)
            buildSentenceTarget->setRootBrickTypeFilter(
                BRICK_FAMILIES::brickType(rootBrick->BrickFamily));
...
public:
  virtual void execute(CCtrlBase *pCaller, const string & /* Params */) {
    ...
    if (BrickType != FaberPlan) {
      ...
      switch (BrickType) {
      case Root:
        BuildPhraseGroup->validateRoot(brick);
        break;
      case OtherMain:
        BuildPhraseGroup->validateMain(Index, brick);
        break;
      case Param:
        BuildPhraseGroup->validateParam(Index, ParamIndex, brick);
        break;
      case NewOpCredit:
        BuildPhraseGroup->validateNewOpCredit(brick);
        break;
      default:
        break;
      }
...
public:
  virtual void execute(CCtrlBase *pCaller, const string &Params) {
    ...
    // get our father build sentence
    CDBGroupBuildPhrase *buildGroup =
        dynamic_cast<CDBGroupBuildPhrase *>(pCaller->getParent()->getParent());
    if (!buildGroup)
      return;
...
REGISTER_ACTION_HANDLER(CHandlerPhraseSelectMainBrick,
...
REGISTER_ACTION_HANDLER(CHandlerPhraseSelectParamBrick,
...
REGISTER_ACTION_HANDLER(CHandlerPhraseSelectNewBrick,
...
public:
  virtual void execute(CCtrlBase *pCaller, const string & /* Params */) {
    ...
    extern void fillFaberPlanSelection(const std::string &brickDB,
                                       uint maxSelection);
    fillFaberPlanSelection(CDBGroupBuildPhrase::BrickSelectionDB,
...
REGISTER_ACTION_HANDLER(CHandlerPhraseFaberSelectPlan,
...
static void updateAllSPhraseInfo() {
  ...
  if (pIG && pIG->getActive()) {
    CDBGroupBuildPhrase *buildPhrase =
        dynamic_cast<CDBGroupBuildPhrase *>(pIG->getGroup("header_opened"));
    if (buildPhrase)
...
REGISTER_ACTION_HANDLER(CHandlerPhraseUpdateFromHand,
...
REGISTER_ACTION_HANDLER(CHandlerPhraseUpdateFromActionMalus,
...
// **********************************************************************************************************
/** drag'n'drop: true if the src ctrlSheet is a Phrase and the dest is in
 * memory. Or if both are in memory
 */
...
public:
  virtual void execute(CCtrlBase *pCaller, const string &Params) {
    ...
    CDBCtrlSheet *pCSDst = dynamic_cast<CDBCtrlSheet *>(pCaller);
    // can be a phrase id (comes from memory), a phraseSheet (comes from
    // progression), or a macro,
      ...
      if (pCSDst->isSPhraseId() || pCSDst->isMacro()) {
        ...
        // can be a phrase sheet
          ...
          CSPhraseSheet *phrase = dynamic_cast<CSPhraseSheet *>(
              SheetMngr.get(NLMISC::CSheetId(pCSSrc->getSheetId())));
        ...
        // check if incoming macro ok
          ...
          // get the macro
          const CMacroCmd *macroCmd =
              pMM->getMacroFromMacroID(pCSSrc->getMacroId());
          if (macroCmd)
            pCSDst->setCanDrop(true);
        }
      }
...
REGISTER_ACTION_HANDLER(CHandlerCanMemorizePhraseOrMacro,
...
public:
  ...
  virtual void execute(CCtrlBase *pCaller, const string &Params) {
    ...
    if (ctrl && ctrl->isSPhraseIdMemory()) {
      ...
      {
        CAHManager::getInstance()->runActionHandler("forget_phrase_or_macro",
                                                    ctrl);
      }
    }
  }
...
public:
  virtual void execute(CCtrlBase *pCaller, const string &Params);
  void memorizePhraseOrMacro(uint dstMemoryIndex, bool isMacro, sint32 phraseId,
                             sint32 macroId);
  void memorizePhraseSheet(uint dstMemoryIndex, uint32 sheetId);
};
REGISTER_ACTION_HANDLER(CHandlerMemorizePhraseOrMacro,
                        "memorize_phrase_or_macro");

void CHandlerMemorizePhraseOrMacro::execute(CCtrlBase *pCaller,
                                            const string &Params) {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  CSPhraseManager *pPM = CSPhraseManager::getInstance();
  ...
  // NB: THIS IS UGLY BUT WORKS BECAUSE Memory ctrls are first initialized as
  // SPhrase (branchname init)
  ...
  if (src.empty() && (CHandlerPhraseMemoryCopy::haveLastPhraseElement)) {
    ...
    if (!srcIsMacro) {
      ...
      // {Forget(),Delete()} Old phrase, then {Learn(),Memorize()}  new one
      // (Messages are shuffled). Instead, replace the old phrase if needed
      uint16 newPhraseId = (uint16)pPM->getMemorizedPhraseIfLastOrNewSlot(
          dstMemoryLine, dstMemoryIndex);
      if (!newPhraseId)
        return;
      ...
      memorizePhraseOrMacro(dstMemoryIndex, srcIsMacro, newPhraseId,
                            srcMacroId);
    }
  } else {
    CInterfaceElement *pElt =
    ...
    if (!pCSSrc->isSPhraseId() && !pCSSrc->isSPhrase() && !pCSSrc->isMacro())
      return;
...

    {
      ...
      if (pCSSrc->isSPhrase()) {
        ...
        // We may replace a phrase with a macro => must delete the phrase under
        // us
        if (srcIsMacro)
          pPM->fullDeletePhraseIfLast(dstMemoryLine, dstMemoryIndex);

        // memorize the phrase or macro
        memorizePhraseOrMacro(dstMemoryIndex, srcIsMacro, srcPhraseId,
                              srcMacroId);
      }
    }
    // Else the src is a memory too
      ...
      if (pCSSrc->canDragCopy() && pIM->testDragCopyKey()) {
        // if a phrase
        if (!srcIsMacro) {
          // \toto yoyo: There is a Network BUG which prevents us from simply
          // doing {Forget(),Delete()} Old phrase, then {Learn(),Memorize()} new
          // one (Messages are shuffled). Instead, replace the old phrase if
          // needed
          uint16 newPhraseId = (uint16)pPM->getMemorizedPhraseIfLastOrNewSlot(
              dstMemoryLine, dstMemoryIndex);
          if (!newPhraseId)
          ...
          // We may replace a phrase with a macro => must delete the phrase
          // under us
          pPM->fullDeletePhraseIfLast(dstMemoryLine, dstMemoryIndex);
      ...
      // else this is a swap!
        ...
        if (dstPhraseId || dstIsMacro) {
          ...
          memorizePhraseOrMacro(srcMemoryIndex, dstIsMacro, dstPhraseId,
                                dstMacroId);
...
// memorize a spell
void CHandlerMemorizePhraseOrMacro::memorizePhraseOrMacro(uint memoryIndex,
                                                          bool isMacro,
                                                          sint32 phraseId,
                                                          sint32 macroId) {
  CSPhraseManager *pPM = CSPhraseManager::getInstance();
...
void CHandlerMemorizePhraseOrMacro::memorizePhraseSheet(uint memoryIndex,
                                                        uint32 sheetId) {
  CSPhraseManager *pPM = CSPhraseManager::getInstance();
  ...
  // (Messages are shuffled). Instead, replace the old phrase if needed
  uint16 newPhraseId =
      (uint16)pPM->getMemorizedPhraseIfLastOrNewSlot(memoryLine, memoryIndex);
  if (!newPhraseId)
    return;
...
public:
  virtual void execute(CCtrlBase *pCaller, const string &Params) {
    ...
    pIM->validMessageBox(CInterfaceManager::QuestionIconMsg,
                         CI18N::get("uiQDeleteAction"),
                         "do_delete_phrase_or_forget_macro", sParams);
  }
};
REGISTER_ACTION_HANDLER(CHandlerDeletePhraseOrForgetMacro,
...
REGISTER_ACTION_HANDLER(CHandlerDoDeletePhraseOrForgetMacro,
                        "do_delete_phrase_or_forget_macro");

// **********************************************************************************************************
/** Cast a spell from a memory slot (combat or magic)
 *	Called when the user click on a memory slot. Can be called also from
 *CAHRunShortcut, or from MoveToAction system (in CSPhraseManager)
 */
  ...
  virtual void execute(CCtrlBase *pCaller, const string & /* Params */) {
    ...
    // **** Else standart cast
      ...
      if (rootBrick->isMagic()) {
        if (UserControls.isMoving() || (UserEntity && UserEntity->follow() &&
                                        UserEntity->speed() != 0.0)) {
      ...
      // Can't cyclic cast Magic, SpecialPower or Harvest phrases
      // (forage_extraction are auto)
      if (rootBrick->isMagic() || rootBrick->isHarvest() ||
          rootBrick->isSpecialPower())
        cyclic = false;
      ...
      if (UserEntity) {
        ...
        if (target || !rootBrick->isCombat()) {
          ...
          if (rootBrick->isCombat()) {
            ...
            UserEntity->executeCombatWithPhrase(target, memoryLine, memoryIndex,
                                                cyclic);
          }
          // else can cast soon!
          else if (rootBrick->isForageExtraction() &&
                   (!UserEntity->isRiding())) // if mounted, send directly to
                                              // server (without moving) to
                                              // receive the error message
          {
            ...
            if (!target || target->isForageSource()) {
              ...
              // reset any moveTo also (if target==NULL,
              // moveToExtractionPhrase() and therefore resetAnyMoveTo() not
              // called) VERY important if previous MoveTo was a SPhrase MoveTo
              // (because cancelClientExecute() must be called)
              UserEntity->resetAnyMoveTo();

              // Move to targetted source
              if (target)
                UserEntity->moveToExtractionPhrase(
                    target->slot(), MaxExtractionDistance, memoryLine,
                    memoryIndex, cyclic);
            ...
            // Cancel any moveTo(), because don't want to continue reaching the
            // prec entity VERY important if previous MoveTo was a SPhrase
            // MoveTo (because cancelClientExecute() must be called)
            UserEntity->resetAnyMoveTo();
  ...
  virtual void execute(CCtrlBase * /* pCaller */, const string &Params) {
    ...
    if (shortcut >= 0 && shortcut <= 2 * RYZOM_MAX_SHORTCUT) {
      ...
      if (shortcut < RYZOM_MAX_SHORTCUT)
        elm = CWidgetManager::getInstance()->getElementFromId(
            PhraseMemoryCtrlBase + toString(shortcut));
      else
        elm = CWidgetManager::getInstance()->getElementFromId(
            PhraseMemoryAltCtrlBase + toString(shortcut - RYZOM_MAX_SHORTCUT));
...
// ***************************************************************************
/** Called when the user click on a memory slot. Different AH called if the slot
 * is empty or full
 */
...
public:
  virtual void execute(CCtrlBase *pCaller, const string &Params) {
    ...
    if (phraseId) {
      CAHManager::getInstance()->runActionHandler("cast_phrase", pCaller,
                                                  Params);
    }
...
REGISTER_ACTION_HANDLER(CHandlerCastPhraseOrCreateNew,
...
public:
  virtual void execute(CCtrlBase *pCaller, const string & /* Params */) {
    ...
    if (parent) {
      ...
      CDBCtrlSheet *ctrl =
          dynamic_cast<CDBCtrlSheet *>(parent->getCtrl("ctrl_phrase"));
      if (ctrl) {
        CAHManager::getInstance()->runActionHandler(
            ctrl->getActionOnRightClick(), ctrl, ctrl->getParamsOnRightClick());
      }
    }
  }
};
REGISTER_ACTION_HANDLER(CHandlerPhraseLinkCtrlRClick,
...
public:
  virtual void execute(CCtrlBase * /* pCaller */, const string &Params) {
    ...
    // Get the link counter. Used to verify that the client cancel the correct
    // link according to server
    uint8 counter = 0;
    CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
        toString("SERVER:EXECUTE_PHRASE:LINK:%d:COUNTER", index), false);
    if (node)
    ...
    if (!ClientCfg.Local) {
      ...
      pIM->displaySystemInfo(
          toString("PHRASE:CANCEL_LINK %d, %d", index, counter));
    }
  }
};
...
REGISTER_ACTION_HANDLER(CHandlerPhraseUpdateAllMemoryRegenTickRange,
...
public:
  virtual void execute(CCtrlBase *pCaller, const string & /* Params */) {
    ...
    CSPhraseManager *pPM = CSPhraseManager::getInstance();
    CSBrickManager *pBM = CSBrickManager::getInstance();
    CInterfaceElement *pCristalizeMenuOption =
        CWidgetManager::getInstance()->getElementFromId(sCristalizePath);

    if (pCristalizeMenuOption == NULL)
      return;
    ...
    // If the phrase is not empty get the root to known if its a magic phrase or
    // not And if its a magic phrase display the cristalize menu option
    ...
    if (pBrick != NULL) {
      if (pBrick->isMagic()) {
        ...
        CViewTextMenu *vtm =
            dynamic_cast<CViewTextMenu *>(pCristalizeMenuOption);
        if (vtm) {
          CTempInvManager *tim = CTempInvManager::getInstance();
          bool isForageInProgress = tim && tim->isOpened() &&
                                    (tim->getMode() == TEMP_INV_MODE::Forage);
...
REGISTER_ACTION_HANDLER(CHandlerPhraseCheckCanCristalize,
...
REGISTER_ACTION_HANDLER(CHandlerPhraseBookSkillFilter,
...
public:
  virtual void execute(CCtrlBase * /* pCaller */, const string &Params) {
    ...
    if (CInterfaceExpr::eval(expr, value, NULL)) {
      if (!value.toInteger()) {
        nlwarning("<CHandlerPhraseSelectMemory:execute> expression doesn't "
...
public:
  virtual void execute(CCtrlBase * /* pCaller */, const string &Params) {
    CInterfaceManager *pIM = CInterfaceManager::getInstance();
    CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
        "UI:PHRASE:SELECT_MEMORY_2", false);
    if (node) {
      sint32 val;
      fromString(Params, val);
      node->setValue32(val);
...
REGISTER_ACTION_HANDLER(CHandlerPhraseSelectShortcutBar2,
...
DECLARE_INTERFACE_CONSTANT(getPhraseBrickSelectionMax,
                           CDBGroupBuildPhrase::MaxSelection)
...
public:
  virtual void execute(CCtrlBase *pCaller, const string & /* Params */) {
    ...
    string str(STRING_MANAGER::CStringManagerClient::getSBrickLocalizedName(
        CSheetId(ctrlSheet->getSheetId())));
...
REGISTER_ACTION_HANDLER(CHandlerCombatRestrictTooltip,
...
void phraseBotChatBuyBySheet(NLMISC::CSheetId sheetId, uint16 phraseId) {
  ...
  if (!ClientCfg.Local) {
    ...
    // debug:
    CInterfaceManager *pIM = CInterfaceManager::getInstance();
    pIM->displaySystemInfo(toString("PHRASE:BUY_SHEET => ") +
                           toString(phraseId));

    // **** Server emulation
    // learn all bricks of this phrase
    CSPhraseSheet *phrase =
        dynamic_cast<CSPhraseSheet *>(SheetMngr.get(sheetId));
    if (phrase) {
      CSBrickManager *pBM = CSBrickManager::getInstance();
    ...
    NLGUI::CDBManager::getInstance()
        ...
        ->setValue32(counter);
    NLGUI::CDBManager::getInstance()
        ->getDbProp("SERVER:EXCHANGE:COUNTER")
        ->setValue32(counter);
    NLGUI::CDBManager::getInstance()
        ->getDbProp("SERVER:TARGET:CONTEXT_MENU:COUNTER")
        ->setValue32(counter);
    NLGUI::CDBManager::getInstance()
        ->getDbProp("SERVER:USER:COUNTER")
        ->setValue32(counter);
  }
}
...
REGISTER_ACTION_HANDLER(CHandlerPhraseMemoryBeforeMenu,
...
static DECLARE_INTERFACE_USER_FCT(isPhraseMenuNotExecuting) {
  ...
  // if the PhraseId for this menu is equal to either cyclic action, or next
  // action, fails
  CSPhraseManager *pPM = CSPhraseManager::getInstance();
  if (CHandlerPhraseMemoryBeforeMenu::LastPhraseIdMenu != 0) {
    if (pPM->getCycleExecutePhraseId() ==
        CHandlerPhraseMemoryBeforeMenu::LastPhraseIdMenu)
      ok = false;
    if (pPM->getNextExecutePhraseId() ==
        CHandlerPhraseMemoryBeforeMenu::LastPhraseIdMenu)
      ok = false;
  }
...
REGISTER_INTERFACE_USER_FCT("isPhraseMenuNotExecuting",
                            isPhraseMenuNotExecuting)
