...
#endif
  enum TSystemInfoMode { InfoMsg, WarningMsg, ErrorMsg };
...
public:
  ...
  void createLocalBranch(const std::string &fileName,
  ...
  /// Save the game interface (keys and interface config). Called before
  /// continent and entity manager are destroyed.
  void uninitInGame0();

  /// Uninit game interface. Called after continent and entity manager are
  /// destroyed.
  void uninitInGame1();
  ...
  /// update a frame View: check coordinates and draw view. The camera is used
  /// to draw in-scene interfaces. Can be NULL.
  ...
  void loadTextures(const std::string &textFileName,
                    const std::string &uvFileName, bool uploadDXTC = false);

  /// Load texts
  void loadTexts(const std::string &fileName);

  /// Load a set of xml files
  bool parseInterface(const std::vector<std::string> &xmlFileNames, bool reload,
  ...
  void createFileBackup(const std::string &message, const std::string &filename,
                        bool useCopy = false);

  /// select player/shared file name from 'save' folder'
  std::string getSaveFileName(const std::string &module, const std::string &ext,
                              bool useShared = true) const;
  ...
  // Save config to default location, if verbose is true, display message in
  // game sysinfo
  ...
  // Save keys to default location, if verbose is true, display message in game
  // sysinfo
  ...
  public:
    ...
    virtual bool cbIDStringReceived(
        std::string &inOut) = 0; // called when string or id is received (return
                                 // true if valid the change)
  };

  void addServerString(const std::string &sTarget, uint32 id,
                       IStringProcess *cb = NULL);
  void addServerID(const std::string &sTarget, uint32 id,
                   IStringProcess *cb = NULL);
  ...
  /// property is toggled)
  // void enableCaptureElement(CInterfaceElement *pElem, CInterfaceProperty
  // *captureFlag); void disableCaptureElement();

  /** Enable/Disable a single modal window (pointer cannot get out of the
   * window). NB : the keyboard capture is released on both calls. NB : cascaded
   * modal windows are disabled by the call
   */

  /// Handle The Event. return true if the interfaceManager catch it and if must
  /// not send to the Game Action Manager
  ...
  void displaySystemInfo(const std::string &str,
                         const std::string &Category = "SYS");
   ...
   *	ui:interface:message_box must be defined in xml, with a "text" ViewText
   *son
   */
  void messageBox(const std::string &text,
                  const std::string &masterGroup = "ui:interface",
                  TCaseMode caseMode = CaseFirstSentenceLetterUp);
  /** Open a MessageBox. this is a simple ModalWindow with a Ok and a HELP
   *button. The help button with open a browser on ryzom.com faq
   *	ui:interface:message_box_with_help must be defined in xml, with a "text"
   *ViewText son
   */
  void messageBoxWithHelp(const std::string &text,
                          const std::string &masterGroup = "ui:interface",
                          const std::string &ahOnOk = std::string(),
                          const std::string &paramsOnOk = std::string(),
                          TCaseMode caseMode = CaseFirstSentenceLetterUp);

  /** Open a MessageBox with validation question. this is a simple ModalWindow
   *with a Ok / Cancel button ui:interface:valid_message_box must be defined in
   *xml, with a "text" ViewText son, and a "ok" button \param ahOnOk => the
   *action handler to call if ok is pressed. NB: you don't have to call
   *leave_modal in this ah (auto done). \param paramsOnOk => params passed to
   *ahOnOk. \param ahOnCancel => the action handler to call if cancel is
   *pressed. NB: you don't have to call leave_modal in this ah (auto done).
   *	\param paramsOnCancel => params passed to ahOnCancel.
   */
  void validMessageBox(TValidMessageIcon icon, const std::string &text,
                       const std::string &ahOnOk,
                       const std::string &paramsOnOk = std::string(),
                       const std::string &ahOnCancel = std::string(),
                       const std::string &paramsOnCancel = std::string(),
                       const std::string &masterGroup = "ui:interface");

  /** Get the current running validMessageBox OnOk action. empty if no
   *validMessageBox currently opened One can use it to know if it match its
   *system and so if it needs to be closed (with disableModalWindow())
   */
  bool getCurrentValidMessageBoxOnOk(
      std::string &ahOnOk, const std::string &masterGroup = "ui:interface");
  ...
  // This is faster than switching to a new desktop using 'setMode' to update
  // the changes \param mode Index of the virtual desktop
  ...
  // Initialize emote from TextEmotListSheet (create command and insert the GCM
  // entry)
  void initEmotes();
  void uninitEmotes();
  void updateEmotes();

  // For Drag And drop, return true if the "CopyDrag" key is pressed (actually
  // the ctrl key!)
  ...
  /// \name LUA
  // @{
  /// For debug: dump in the sysinfo and nlwarning state of lua. detail range
  /// from 0 to 2 (clamped).
  ...
  uint8 getLocalSyncActionCounterMask() const {
    return _LocalSyncActionCounterMask;
  }
  ...
  bool isItemCaracRequirementMet(CHARACTERISTICS::TCharacteristics type,
                                 sint32 value) {
    // carac requirement?
...
private:
  ...
  private:
    ...
    struct CNodeLocalComp {
      CNode *Node;
      bool operator<=(const CNodeLocalComp &o) const {
        return Node->LocalNode <= o.Node->LocalNode;
      }
      bool operator<(const CNodeLocalComp &o) const {
        return Node->LocalNode < o.Node->LocalNode;
      }
    ...
    struct CNodeServerComp {
      CNode *Node;
      bool operator<=(const CNodeServerComp &o) const {
        return Node->ServerNode <= o.Node->ServerNode;
      }
      bool operator<(const CNodeServerComp &o) const {
        return Node->ServerNode < o.Node->ServerNode;
      }
  ...
  private:
    ...
    void buildRecursLocalLeaves(NLMISC::CCDBNodeBranch *branch,
                                std::vector<NLMISC::CCDBNodeLeaf *> &leaves);
  };
  ...
  public:
    ...
    bool execute(const std::string &rawCommandString,
                 const std::vector<std::string> &args, NLMISC::CLog &log,
                 bool quiet, bool human = true);
...
private:
  ...
  /** This is the GLOBAL Action counter used to synchronize some systems
  ...
  sint32 _LastInGameScreenW,
      _LastInGameScreenH; // Resolution used for last InGame interface
  ...
  // dump printable version of pair key-index in top of stack. if value is a
  // table, recurs (up to recursTableLevel times)
  ...
  NLMISC::CRefPtr<NLMISC::CCDBNodeLeaf>
      _CurrentPlayerCharacLeaf[CHARACTERISTICS::NUM_CHARACTERISTICS];
  ...
  // Pop a new message box. If the message box was found, returns a pointer on
  // it
  void messageBoxInternal(const std::string &msgBoxGroup,
                          const std::string &text,
                          const std::string &masterGroup, TCaseMode caseMode);
