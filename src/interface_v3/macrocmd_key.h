...
public:
  ...
  void
  setTitle(const std::string &uistr); // UI String (uiEditCmd) to be localized

  void deactivate();
  void activate();
  void activateFrom(const std::string &cmdName, const std::string &cmdParams,
                    sint nRef);
...
private:
  ...
  bool getCatIndexAndAM(sint32 nInCatIndex, sint32 &nOutCatIndex,
                        CActionsManager *&nOutAM);
  bool getBaseActionIndex(const CCategory &cat, sint32 nInActIndex,
                          sint32 &nOutActIndex);
...
#define WIN_EDITCMD_COMBO_SECOND_PARAM_EDITBOX                                 \
  ":content:second_param_editbox:eb"
