...
void CSkillManager::initInGame() {
  ...
  nlassert(_UnblockTitle->TitlesUnblock.size() ==
  ...
  for (i = 0; i < CHARACTER_TITLE::NB_CHARACTER_TITLE; ++i) {
    ...
    _TitlesUnblocked[i].UnblockedSkillLists.resize(
        _UnblockTitle->TitlesUnblock[i].SkillsNeeded.size(), false);
    _TitlesUnblocked[i].UnblockedBricks.resize(
        _UnblockTitle->TitlesUnblock[i].BricksNeeded.size(), false);
    _TitlesUnblocked[i].UnblockedMinFames.resize(
        _UnblockTitle->TitlesUnblock[i].MinFames.size(), false);
    _TitlesUnblocked[i].UnblockedMaxFames.resize(
        _UnblockTitle->TitlesUnblock[i].MaxFames.size(), false);
    _TitlesUnblocked[i].UnblockedCiv =
        _UnblockTitle->TitlesUnblock[i].CivNeeded.empty();
    _TitlesUnblocked[i].UnblockedCult =
        _UnblockTitle->TitlesUnblock[i].CultNeeded.empty();
    _TitlesUnblocked[i].UnblockedCharOldness =
        _UnblockTitle->TitlesUnblock[i].CharOldness.empty();
    _TitlesUnblocked[i].UnblockedCharPlayedTime =
        _UnblockTitle->TitlesUnblock[i].CharPlayedTime.empty();
    _TitlesUnblocked[i].UnblockedAccountOldness =
        _UnblockTitle->TitlesUnblock[i].AccountOldness.empty();
    _TitlesUnblocked[i].UnblockedAuthorRating =
        (_UnblockTitle->TitlesUnblock[i].AuthorRating == 0);
    _TitlesUnblocked[i].UnblockedAMRating =
        (_UnblockTitle->TitlesUnblock[i].AMRating == 0);
    _TitlesUnblocked[i].UnblockedOrganizerRating =
        (_UnblockTitle->TitlesUnblock[i].OrganizerRating == 0);
    _TitlesUnblocked[i].UnblockedItemLists.resize(
        _UnblockTitle->TitlesUnblock[i].ItemsNeeded.size(), false);
  }
  ...
  for (i = 0; i < SKILLS::NUM_SKILLS; i++) {
    _SkillValues[i] = pCDBM->getDbProp(
        toString("SERVER:CHARACTER_INFO:SKILLS:%d:SKILL", i), false);
    _SkillBaseValues[i] = pCDBM->getDbProp(
...
// ***************************************************************************
const std::vector<SKILLS::ESkills> &
...
// ***************************************************************************
bool CSkillManager::areSkillOnSameBranch(SKILLS::ESkills s0,
                                         SKILLS::ESkills s1) {
  SKILLS::ESkills parent;

  if ((s0 < 0) || (s0 >= SKILLS::NUM_SKILLS) || (s1 < 0) ||
      (s1 >= SKILLS::NUM_SKILLS))
    return false;
...
// ***************************************************************************
bool CSkillManager::checkBaseSkillMetRequirement(SKILLS::ESkills eSkill,
                                                 uint32 value) {
...
void CSkillManager::onSkillChange() {
  ...
  if (someChange) {
    ...
    for (it = _SkillChangeCallbackSet.begin();
         it != _SkillChangeCallbackSet.end(); it++) {
      (*it)->onSkillChange();
    }
...
void CSkillManager::checkTitleUnblocked(CHARACTER_TITLE::ECharacterTitle i,
  ...
  bool bUnblockedAccountOldness =
      true; //_TitlesUnblocked[i].UnblockedAccountOldness;

  bool bAllUnblocked = bAllUnblockedSkill && bAllUnblockedBrick &&
                       bAllUnblockedItem && bAllUnblockedMinFame &&
                       bAllUnblockedMaxFame && bUnblockedCiv &&
                       bUnblockedCult && bUnblockedCharOldness &&
                       bUnblockedCharPlayedTime && bUnblockedAccountOldness;
  ...
  if (bAllUnblocked != _TitlesUnblocked[i].Unblocked) {
    ...
    if (!IngameDbMngr.initInProgress()) {
      ...
      if (bAllUnblocked && show_message) {
        ...
        string titleStr =
            CHARACTER_TITLE::toString((CHARACTER_TITLE::ECharacterTitle)i);
        bool womenTitle =
            (UserEntity && UserEntity->getGender() == GSGENDER::female);
        const char *newtitle(
            CStringManagerClient::getTitleLocalizedName(titleStr, womenTitle));
...
// ***************************************************************************
void CSkillManager::tryToUnblockTitleFromSkill(SKILLS::ESkills eSkill,
                                               sint32 value) {
  for (uint i = 0; i < CHARACTER_TITLE::NB_CHARACTER_TITLE; ++i) {
    ...
    for (uint j = 0; j < rTU.SkillsNeeded.size(); ++j) // for all skill lists
    {
      if (!_TitlesUnblocked[i].UnblockedSkillLists[j]) // Not already unblocked
      {
        ...
        for (uint k = 0; k < rTU.SkillsNeeded[j].size();
             ++k) // for all skills in current skill list
        {
          ...
          if (sSkill.size() < rTU.SkillsNeeded[j][k].size()) {
            ...
            if (strncmp(sSkill.c_str(), rTU.SkillsNeeded[j][k].c_str(),
                        rTU.SkillsNeeded[j][k].size()) != 0) {
              allSkillsFromListValidated = false;
            }
          }
        }
...
// ***************************************************************************
void CSkillManager::tryToUnblockTitleFromMinFames(uint32 factionIndex,
                                                  sint32 fameValue) {
  for (uint i = 0; i < CHARACTER_TITLE::NB_CHARACTER_TITLE; ++i) {
    CUnblockTitlesSheet::STitleUnblock rTU = _UnblockTitle->TitlesUnblock[i];
    ...
    for (uint j = 0; j < rTU.MinFames.size(); ++j) {
      ...
      break; // there should not be more than one fame prerequisite per faction
             // per title
    }
  }
}
...
// ***************************************************************************
void CSkillManager::tryToUnblockTitleFromMaxFames(uint32 factionIndex,
                                                  sint32 fameValue) {
  for (uint i = 0; i < CHARACTER_TITLE::NB_CHARACTER_TITLE; ++i) {
    CUnblockTitlesSheet::STitleUnblock rTU = _UnblockTitle->TitlesUnblock[i];
    ...
    if (!rTU.CivNeeded.empty()) {
      ...
      if (IngameDbMngr.initInProgress()) {
        if (NLGUI::CDBManager::getInstance()
                ->getDbProp("UI:SAVE:FAME:CIV_ALLEGIANCE")
                ->getValue32() != civNeeded)
          _TitlesUnblocked[i].UnblockedCiv = false;
        continue;
      } else {
        CCDBNodeLeaf *civLeaf = NLGUI::CDBManager::getInstance()->getDbProp(
            "SERVER:FAME:CIV_ALLEGIANCE");
        uint8 civDBValue = civLeaf->getValue8();
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:SAVE:FAME:CIV_ALLEGIANCE")
            ->setValue32((uint32)civDBValue);
...
void CSkillManager::tryToUnblockTitleFromCult(bool show_message) {
  ...
  for (uint i = 0; i < CHARACTER_TITLE::NB_CHARACTER_TITLE; ++i) {
    ...
    if (!rTU.CultNeeded.empty()) {
      ...
      if (IngameDbMngr.initInProgress()) {
        if (NLGUI::CDBManager::getInstance()
                ->getDbProp("UI:SAVE:FAME:CULT_ALLEGIANCE")
                ->getValue32() != cultNeeded)
          _TitlesUnblocked[i].UnblockedCult = false;
        continue;
      } else {
        CCDBNodeLeaf *cultLeaf = NLGUI::CDBManager::getInstance()->getDbProp(
            "SERVER:FAME:CULT_ALLEGIANCE");
        uint8 cultDBValue = cultLeaf->getValue8();
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:SAVE:FAME:CULT_ALLEGIANCE")
            ->setValue32((uint32)cultDBValue);
...
// ***************************************************************************
void CSkillManager::unblockTitleFromServer(
    CHARACTER_TITLE::ECharacterTitle ct) {
...
/// ---------------------------------------------
void CSkillManager::tryToUnblockTitleFromCharOldness(
    uint32 firstConnectedTime) {
  uint32 time = CTime::getSecondsSince1970();
  if (time > firstConnectedTime) {
    if (firstConnectedTime == 0) {
      nlwarning("<CSkillManager::tryToUnblockTitleFromCharOldness> first "
                "connect time is null !");
    }
    ...
    nlwarning("<CSkillManager::tryToUnblockTitleFromCharOldness> bad first "
              "connect time ? %d",
              firstConnectedTime);
  }
}
...
void CSkillManager::tryToUnblockTitleFromCharPlayedTime(uint32 playedTime) {
  for (uint i = 0; i < CHARACTER_TITLE::NB_CHARACTER_TITLE; ++i) {
    ...
    if (!rTU.CharPlayedTime.empty()) {
      ...
      _TitlesUnblocked[i].UnblockedCharPlayedTime =
          (playedTime / (60 * 60 * 24) > requiredPlayedTime);
    }
    checkTitleUnblocked((CHARACTER_TITLE::ECharacterTitle)i);
  }
}
...
/// ---------------------------------------------
void CSkillManager::tryToUnblockTitleFromAccountOldness(
    uint32 /* accountCreationTime */) {}

/// ---------------------------------------------
void CSkillManager::tryToUnblockTitleFromRingRatings(uint32 authorRating,
                                                     uint32 amRating,
  ...
  for (uint i = 0; i < CHARACTER_TITLE::NB_CHARACTER_TITLE; ++i) {
    ...
    _TitlesUnblocked[i].UnblockedAuthorRating =
        rTU.AuthorRating <= authorRating;
    _TitlesUnblocked[i].UnblockedAMRating = rTU.AMRating <= amRating;
    _TitlesUnblocked[i].UnblockedOrganizerRating =
        rTU.OrganizerRating <= masterlessRating;
...
void CSkillManager::tryToUnblockTitleFromItems(bool show_message) {
  ...
  for (uint i = 0; i < CHARACTER_TITLE::NB_CHARACTER_TITLE; ++i) {
    ...
    if (rTU.Reserved || rTU.ItemsNeeded.empty())
      continue;

    for (uint j = 0; j < rTU.ItemsNeeded.size(); ++j) // for all item lists
    {
      //			if (_TitlesUnblocked[i].UnblockedItemLists[j])
      //// Not already unblocked 				continue;

      uint numItemsFromListToValidate = (uint)rTU.ItemsNeeded[j].size();

      for (uint k = 0; k < rTU.ItemsNeeded[j].size();
           ++k) // for all items in current item list
      {
        ...
        for (uint itemSlot = 0; itemSlot < numItems; ++itemSlot) {
          ...
          CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
              branch + ":" + toString(itemSlot) + ":SHEET", false);
          if (node)
          ...
          node = NLGUI::CDBManager::getInstance()->getDbProp(
              branch + ":" + toString(itemSlot) + ":QUALITY", false);
          if (node)
            qualityItem = node->getValue32();

          // sheetid and quality are identical
          if (qualityItem == rTU.ItemsQualityNeeded[j][k] &&
              sheetItem == rTU.ItemsNeeded[j][k].asInt())
      ...
      if (allItemsFromListValidated !=
          _TitlesUnblocked[i].UnblockedItemLists[j]) {
...
#define GROUP_TITLE_COMBO                                                      \
  "ui:interface:info_player_skills:header_opened:title:player_title"
      ...
      for (uint i = 0; i < CHARACTER_TITLE::NB_CHARACTER_TITLE; ++i) {
        if (pSM->isTitleUnblocked((CHARACTER_TITLE::ECharacterTitle)i)) {
          ...
          const char *s =
              CStringManagerClient::getTitleLocalizedName(titleStr, womenTitle);
          pCB->addText(s);
          pSM->_UIUnblockedTitles.push_back(
              (CHARACTER_TITLE::ECharacterTitle)i);
        }
      }
    }
  }
...
public:
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    ...
    if (pCB == NULL)
      return;
    if ((pCB->getSelection() < 0) ||
        (pCB->getSelection() >= (sint32)pSM->_UIUnblockedTitles.size()))
      return;

    nNewTitle = (uint8)pSM->_UIUnblockedTitles[pCB->getSelection()];

    // If new title choosen is different from current title -> Send the message
    // to the server
