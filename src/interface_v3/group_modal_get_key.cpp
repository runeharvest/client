...
NLMISC_REGISTER_OBJECT(CViewBase, CGroupModalGetKey, std::string,
                       "modal_get_key");
...
void CGroupModalGetKey::setActive(bool state) {
  ...
  CViewText *pVT = dynamic_cast<CViewText *>(
      CWidgetManager::getInstance()->getElementFromId(VIEW_TEXT_KEY));
  if (pVT != NULL)
    pVT->setText(string(""));
  pVT = dynamic_cast<CViewText *>(
      CWidgetManager::getInstance()->getElementFromId(VIEW_TEXT_INUSE));
  if (pVT != NULL)
    pVT->setText(string(""));
  CCtrlBaseButton *pCB = dynamic_cast<CCtrlBaseButton *>(
      CWidgetManager::getInstance()->getElementFromId(CTRL_BUTTON_OK));
  if (pCB != NULL)
    pCB->setFrozen(true);
...
bool CGroupModalGetKey::handleEvent(const NLGUI::CEventDescriptor &event) {
  if (event.getType() == NLGUI::CEventDescriptor::key) {
    ...
    if (edk.getKeyEventType() == NLGUI::CEventDescriptorKey::keydown) {
      //			if ((edk.getKey() != KeyCONTROL) &&
      //(edk.getKey() != KeyMENU) && (edk.getKey() != KeySHIFT))
      //			{
      Combo.Key = edk.getKey();
      Combo.KeyButtons = noKeyButton;
      if (edk.getKeyAlt())
        Combo.KeyButtons = (TKeyButton)((uint8)Combo.KeyButtons | altKeyButton);
      if (edk.getKeyCtrl())
        Combo.KeyButtons =
            (TKeyButton)((uint8)Combo.KeyButtons | ctrlKeyButton);
      if (edk.getKeyShift())
        Combo.KeyButtons =
            (TKeyButton)((uint8)Combo.KeyButtons | shiftKeyButton);
      ...
      if (pCurAM != NULL) {
        ...
        if (it != keyShortcut.end()) {
          ...
          if (baseAction && pCurAM->isActionPresentInContext(it->second)) {
            string shortcutName =
                baseAction->getActionLocalizedText(it->second);
            if (pVT != NULL)
              pVT->setText(shortcutName);
          }
        } else {
          if (pVT != NULL)
            pVT->setText(string());
