...
public:
   ...
   */
  static IInterfaceObserver *create(xmlNodePtr node,
                                    CInterfaceGroup *parentGroup) {
    CInterfaceManager *iMngr = CInterfaceManager::getInstance();
    CXMLAutoPtr idObserver(
        (const char *)xmlGetProp(node, (char unsigned *)"action"));
    if (!idObserver) {
    ...
    while (dataTok) {
      ...
      if (NLGUI::CDBManager::getInstance()->getDbProp(data, false) ==
          NULL) // Full path provided ? No.
      {
        ...
        while (parent != NULL) {
          ...
          if (data[0] == ':')
            ...
            sTmp = parent->getId() + ":" + data;
          if (NLGUI::CDBManager::getInstance()->getDbProp(sTmp, false) !=
              NULL) {
            data = sTmp;
            break;
          }
      ...
      if (!obs) {
        CInterfaceObserverFactoryManager::TFactoryMap::const_iterator it =
            CInterfaceObserverFactoryManager::getInstance()->FactoryMap.find(
                (const char *)idObserver);
        if (it ==
            CInterfaceObserverFactoryManager::getInstance()->FactoryMap.end()) {
      ...
      if (!NLGUI::CDBManager::getInstance()->getDB()->addObserver(obs,
                                                                  textId)) {
        return NULL;
      }
   ...
   */
  virtual IInterfaceObserver *createObserver(xmlNodePtr node,
                                             CInterfaceGroup *parent) = 0;
};

/**
 * register a new observer by defining the associated factory and insert the
 * handler name in the global action handler map \param handler : name of the
 * action handler class \param name : name of the action handler in the map
 * (same as the "onaction" value in the XML script)
 */
...
#define REGISTER_OBSERVER(observer, name)                                      \
  ...
  public:                                                                      \
    observer##Factory() {                                                      \
      CInterfaceObserverFactoryManager::getInstance()->FactoryMap.insert(      \
          make_pair(std::string(name), this));                                 \
    };                                                                         \
                                                                               \
  protected:                                                                   \
    IInterfaceObserver *createObserver(xmlNodePtr node,                        \
                                       CInterfaceGroup *parent) {              \
