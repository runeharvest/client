...
CGroupQuickHelp::CGroupQuickHelp(const TCtorParam &param) : CGroupHTML(param) {
  _IsQuickHelp = false;
  initParameters();
    ...
    if (viewBase) {
      ...
      if (adaptor) {
        ...
        for (i = 0; i < groups.size(); i++) {
          if (groups[i]) {
            // Get a paragraph
            CGroupParagraph *paragraph =
                dynamic_cast<CGroupParagraph *>(groups[i]);
            if (paragraph) {
              // Set the text size
              if (i == _CurrentStep)
...
void CGroupQuickHelp::setGroupTextSize(CInterfaceGroup *group, bool selected) {
  ...
  bool linkGlobalColor =
      selected ? LinkColorGlobalColor : _NonSelectedGlobalColor;
  uint fontSize = selected ? _BrowserStyle.Current.FontSize : _NonSelectedSize;
  NLMISC::CRGBA color =
      selected ? _BrowserStyle.Current.TextColor : _NonSelectedColor;
  ...
  case HTML_A:
    ...
    if (_TrustedDomain && elm.hasNonEmptyAttribute("z_action_shortcut")) {
      ...
      CActionsManager *actionManager =
          ActionsContext.getActionsManager(category);
      if (actionManager) {
        const CActionsManager::TActionComboMap &actionCombo =
            actionManager->getActionComboMap();
        CActionsManager::TActionComboMap::const_iterator ite =
            actionCombo.find(CAction::CName(
                elm.getAttribute("z_action_shortcut").c_str(), params.c_str()));
...
// ***************************************************************************
std::string CGroupQuickHelp::getLanguageUrl(const std::string &href,
                                            std::string lang) const {
  std::string uri = href;

  if (uri.size() < 5 || uri.substr(0, 5) == "http://" ||
  ...
  if (uri.substr(uri.size() - 5) == ".html") {
    ...
    // files inside bnp (file:/gamedev.bnp@help_en.html) will always match with
    // CPath::lookup()
    std::string fname;
    size_t pos = uri.find("@");
...
void CGroupQuickHelp::activateCurrentStep() {
  ...
  for (;;) {
    if (_CurrentStep < _Steps.size()) {
      ...
      // A link to follow ?
      if (!step.URL.empty())
        CAHManager::getInstance()->runActionHandler("browse", NULL,
                                                    "url=" + step.URL);
  ...
  void execute(CCtrlBase * /* pCaller */, const std::string &sParams) {
    CInterfaceElement *element =
        CWidgetManager::getInstance()->getElementFromId(
            "ui:interface:quick_help:content:html");
    if (element) {
      // Group HTML ?
    ...
    element = CWidgetManager::getInstance()->getElementFromId(
        "ui:interface:help_browser:content:html");
    if (element) {
      // Group HTML ?
      CGroupQuickHelp *groupQH = dynamic_cast<CGroupQuickHelp *>(element);
      if (groupQH) {
...
class CHandlerRunQuickHelp : public IActionHandler {
  void execute(CCtrlBase * /* pCaller */, const std::string &sParams) {
    ...
    string buttonId = CWidgetManager::getInstance()->getParser()->getDefine(
        "quick_help_buttons");
    if (!buttonId.empty()) {
      // Get the button id
      CInterfaceElement *element =
