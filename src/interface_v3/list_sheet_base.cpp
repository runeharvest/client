...
// ***************************************************************************
bool IListSheetBase::insertSectionGroup(uint sectionGroupIndex, uint sectionId,
                                        sint xGroup, sint yGroup) {
  // if not exist, create new
  nlassert(sectionGroupIndex <= _SectionGroups.size());
  if (sectionGroupIndex == _SectionGroups.size()) {
    CInterfaceGroup *group =
        createSectionGroup(toString("section_group%d", sectionGroupIndex));
    if (!group)
      return false;
