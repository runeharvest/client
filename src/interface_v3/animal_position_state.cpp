...
CEntityCL *CUIDEntityPositionState::getEntity() {
  ...
  // else check that the cache slot is OK with this UID
    ...
    // Is data slot changed ? animal no more in vision -> try to acquire it at
    // next pass
...
void CTeammatePositionState::build(const std::string &baseDB) {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  _Present =
      NLGUI::CDBManager::getInstance()->getDbProp(baseDB + ":NAME", false);
...
void CAnimalPositionState::build(const std::string &baseDB) {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  _Status =
      NLGUI::CDBManager::getInstance()->getDbProp(baseDB + ":STATUS", false);
...
bool CAnimalPositionState::getPos(sint32 &px, sint32 &py) {
  // only if the animal is spawned
  if (dbOk() && ANIMAL_STATUS::isSpawned(
                    (ANIMAL_STATUS::EAnimalStatus)_Status->getValue32())) {
    return CPositionState::getPos(px, py);
  } else {
    px = py = 0;
    return false;
...
// ***************************************************************************
void CNamedEntityPositionState::build(CCDBNodeLeaf *name, CCDBNodeLeaf *x,
                                      CCDBNodeLeaf *y) {
  _Name = name;
  _X = x;
  _Y = y;
...
CEntityCL *CDialogEntityPositionState::getEntity() {
  if (CCompassDialogsManager::getInstance().getEntries().size() >= _DialogIndex)
    return NULL;
  return EntitiesMngr.getEntityByName(
      CCompassDialogsManager::getInstance().getEntries()[_DialogIndex].Text);
}
