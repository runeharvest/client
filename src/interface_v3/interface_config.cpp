...
/*
        ...
        Version 0:
                - first version (well version not added but see loadconfig()
   hack).
*/
...
void CInterfaceConfig::SCont::setFrom(CGroupContainer *pGC) {
  ...
  if ((pGC->getLayerSetup() > 0) && (!pGC->isPopable())) {
    // The container is contained by another one and cannot be transformed in
    // root container
    ContainerMode = 1;
    Opened = pGC->isOpen();
...
void CInterfaceConfig::SCont::setTo(CGroupContainer *pGC) {
  if (ContainerMode == 0) {
    ...
    // Set the W and H only if resizer is enabled (else always take from
    // scripts)
    if (pGC->getEnabledResizer()) {
      sint w = W;
      sint h = H;
      // use the Popup min and max, it it is not popable.... (yoyo: what a
      // mess....)
      if (!pGC->isPopable())
    ...
    if (pGC->isLockable())
      pGC->setLocked(Locked);
    pGC->setMovable(Movable);

    CCtrlScroll *pSB = dynamic_cast<CCtrlScroll *>(pGC->getCtrl("sb"));
    if (pSB != NULL)
      pSB->setTrackPos(ScrollPos);
    //
    pGC->touch(TouchFlag);
    //
    if (BackupedPositionValid) {
    ...
    // Container that just need Opened, Active and ActiveSavable state to be
    // retrieved
    pGC->setOpen(Opened);
    if (ActiveSavable) {
      pGC->setActive(false);
...
private:
  ...
  void visit(CInterfaceElement *elem) {
    ...
    if (!elem->wantSerialConfig())
      return; // has something to save ?
    ...
    // the object may do some 'serialPtr', this would cause the second serial to
    // have a different size because the object would already have been recorded
    // in the ptr table of the stream
...
void CInterfaceConfig::CDesktopImage::fromCurrentDesktop() {
  ...
  const vector<CWidgetManager::SMasterGroup> &rVMG =
  ...
  CInterfaceGroup *topWindow = CWidgetManager::getInstance()->getTopWindow(
      CWidgetManager::getInstance()->getLastTopWindowPriority());
...
void CInterfaceConfig::CDesktopImage::toCurrentDesktop() {
  ...
  for (uint k = 0; k < GCImages.size(); ++k) {
    CGroupContainer *pGC = dynamic_cast<CGroupContainer *>(
        CWidgetManager::getInstance()->getElementFromId(GCImages[k].Id));
    if (pGC != NULL)
  ...
  if (Version >= 1) {
    ...
    if (!topWindowName.empty()) {
      CInterfaceGroup *window = dynamic_cast<CInterfaceGroup *>(
  ...
  for (uint k = 0; k < numElemWithConfig; ++k) {
    ...
    if (!elem) {
      nlwarning("Element %s not found while loading config, skipping datas",
                elemID.c_str());
      ...
      try {
        ...
        nlwarning("Element %s config in stream are too recent to be read by "
                  "the application,  config ignored",
                  elemID.c_str());
...
// ***************************************************************************
void CInterfaceConfig::CDesktopImage::updateGroupContainerImage(
    CGroupContainer &gc) {
  bool updated = false;
  for (uint k = 0; k < GCImages.size(); ++k) {
...
// ***************************************************************************
void CInterfaceConfig::CDesktopImage::removeGroupContainerImage(
    const std::string &groupName) {
  GCImages.erase(std::remove_if(GCImages.begin(), GCImages.end(),
                                CGroupContainerImageMatch(groupName)),
                 GCImages.end());
}
...
// ***************************************************************************
void CInterfaceConfig::streamToDataBase(NLMISC::IStream &f,
  ...
  if (pDB != NULL) {
    ...
    for (uint32 i = 0; i < nbLeaves; ++i) {
      ...
      // If there is a define RESET_VER_dbName that exist for this DB, check if
      // version is OK
      bool wantRead = true;
      ...
      if (CWidgetManager::getInstance()->getParser()->isDefineExist(defVerId)) {
        uint32 dbVer;
        fromString(
            CWidgetManager::getInstance()->getParser()->getDefine(defVerId),
            dbVer);
        // if the version in the file is older than the version this db want,
        // abort read
      ...
      if (wantRead) {
        CCDBNodeLeaf *pNL =
            NLGUI::CDBManager::getInstance()->getDbProp(leafTmp.Name, false);
        if (pNL != NULL)
          leafTmp.setTo(pNL);
      }
    }
