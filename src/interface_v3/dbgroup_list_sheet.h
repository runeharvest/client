...
public:
  ...
  virtual ~CDBGroupListSheet(); // AJM: make base class destructors virtual to
                                // avoid memory leaks!
  ...
  /** (useful for list only) Force the validity of an element, even if its
   *sheetId==0 (empty slot displayed instead) NB: invalidateCoords() called if
   *state is changed
   */
...
protected:
  ...
  bool isChildValid(uint i) const {
    if (i >= _SheetChildren.size())
      return false;
    return _SheetChildren[i]->ForceValid ||
           _SheetChildren[i]->isSheetValid(
               const_cast<CDBGroupListSheet *>(this));
  }
  ...
  void insertSectionGroupList(sint firstSectionId, sint lastSectionId,
                              uint &groupSectionIndex, sint &yItem, sint xStart,
                              sint &xItem);
};
