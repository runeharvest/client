...
bool CSkillValue::andWith(const CSkillValue &sv) {
  ...
  // else if the skills are one same branch
    ...
    // The skillvalue are compatible if the ancestor skill has a lesser value
    // than the son
...
void CReqSkillFormula::CSkillValueAnd::andV(const CSkillValueAnd &a) {
  // for each skill value of a, AND with each skill value of this. if only one
  // is compatible, don't append the and member!
    ...
    if (it->AndSkills.size() == 1) {
      ...
      for (itb = OrSkills.begin(); itb != OrSkills.end();) {
        if (itb != it) {
          ...
          for (itSv = itb->AndSkills.begin(); itSv != itb->AndSkills.end();
               itSv++) {
            CSkillValue sv = *itSv;
...
bool CReqSkillFormula::evaluate() const {
  ...
  std::list<CSkillValueAnd>::const_iterator itOr(OrSkills.begin()),
      endOr(OrSkills.end());
  ...
  for (; itOr != endOr; itOr++) {
    ...
    std::list<CSkillValue>::const_iterator itAnd(itOr->AndSkills.begin()),
        endAnd(itOr->AndSkills.end());
    ok = true;
...
bool CReqSkillFormula::isSkillValueTrained(const CSkillValue &sv) const {
  ...
  if (currentValue >= reqValue)
    ...
    // cause the currentValue could reach this val (eg SM 15), but it actually
    // doesn't
    if (reqValue <= maxValue)
      return false;
    // else if the sv is SM 90 for example, we have to test if one of sons is
    // OK.
    else {
