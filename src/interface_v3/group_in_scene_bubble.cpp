...
void contextHelp(const std::string &name) {
  ...
  if ((ClientCfg.Local || !IngameDbMngr.initInProgress()) &&
      NLGUI::CDBManager::getInstance()
          ->getDbProp("UI:SAVE:ENTITY:CONTEXT_HELP")
          ->getValueBool()) {
    ...
    for (;;) {
      ...
      if (CWidgetManager::getInstance()->getParser()->isDefineExist(
              defineTarget) &&
          CWidgetManager::getInstance()->getParser()->isDefineExist(
              defineUrl)) {
        string target =
            CWidgetManager::getInstance()->getParser()->getDefine(defineTarget);
        string url =
            CWidgetManager::getInstance()->getParser()->getDefine(defineUrl);

        CInterfaceElement *elementTarget =
        ...
        if (elementTarget && elementTarget->getActive()) {
          ...
          if (url.empty() == false) {
            ...
            completeURL = completeURL.substr(0, completeURL.size() -
                                                    5); // Substract the ".html"
            completeURL += "_" + ClientCfg.getHtmlLanguageCode() + ".html";
            // Add bubble
            InSceneBubbleManager.addContextHelpHTML(
                completeURL, target,
                CWidgetManager::getInstance()
                    ->getSystemOption(CWidgetManager::OptionTimeoutContextHtml)
                    .getValSInt32());
          }
...
// ***************************************************************************
// Helper
void CGroupInSceneBubbleManager::fadeWindow(CInterfaceGroup *group,
                                            double timeout) {
...
// ***************************************************************************
// Helper
void CGroupInSceneBubbleManager::alignMessagePopup(vector<CPopup> &rList,
                                                   bool bCenter) {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
...
void CGroupInSceneBubbleManager::release() {
  ...
  for (i = 0; i < _MessagePopupCentered.size(); i++) {
    ...
    if (_MessagePopupCentered[i].Group->getParent())
      _MessagePopupCentered[i].Group->getParent()->delGroup(
          _MessagePopupCentered[i].Group);
    else
      ...
      if (!_BubblePopup[i].Target.empty()) {
        ...
        if (target) {
          ...
          if (!target->getActive() ||
            ...
            CWidgetManager::getInstance()->unMakeWindow(_BubblePopup[i].Group);
            if (_BubblePopup[i].Group->getParent())
              _BubblePopup[i].Group->getParent()->delGroup(
                  _BubblePopup[i].Group);
            else
              delete _BubblePopup[i].Group;
  ...
  for (i = 0; i < _GroupToDelete.size(); i++) {
    ...
    for (j = 0; j < _DynBubbles.size(); j++) {
      if (_DynBubbles[j].Bubble == _GroupToDelete[i]) {
        ...
        CEntityCL *pEntity =
            EntitiesMngr.getEntityByCompressedIndex(_DynBubbles[j].BotUID);
...
void CGroupInSceneBubbleManager::addSkillPopup(uint skillId, sint delta,
                                               uint time) {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  ...
  templateParams.push_back(
      std::pair<std::string, std::string>("skillid", toString(skillId)));
  templateParams.push_back(
      std::pair<std::string, std::string>("delta", toString(delta)));

  CInterfaceGroup *group =
      CWidgetManager::getInstance()->getParser()->createGroupInstance(
          "skill_popup", "ui:interface",
          templateParams.empty() ? NULL : &(templateParams[0]),
          (uint)templateParams.size());
  if (group) {
    // Skill name
    CViewText *pViewSkillName =
        dynamic_cast<CViewText *>(group->getView("name"));
    if (pViewSkillName != NULL)
      pViewSkillName->setText(
          STRING_MANAGER::CStringManagerClient::getSkillLocalizedName(
              (SKILLS::ESkills)skillId));

    // Skill value
    CCDBNodeLeaf *skillLeaf = NLGUI::CDBManager::getInstance()->getDbProp(
        "SERVER:CHARACTER_INFO:SKILLS:" + toString(skillId) + ":BaseSKILL",
        false);
    if (skillLeaf) {
    ...
    if (pViewSkillMax != NULL)
      pViewSkillMax->setText(
          toString(pSM->getMaxSkillValue((SKILLS::ESkills)skillId)));
...
void CGroupInSceneBubbleManager::addMessagePopup(const string &message,
                                                 CRGBA color, uint time) {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();

  // default timeout?
  if (time == 0)
    time = CWidgetManager::getInstance()
               ->getSystemOption(CWidgetManager::OptionTimeoutMessages)
               .getValSInt32();
...
void CGroupInSceneBubbleManager::addMessagePopupCenter(const string &message,
                                                       CRGBA color, uint time) {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  ...
  CInterfaceGroup *group =
      CWidgetManager::getInstance()->getParser()->createGroupInstance(
          "message_popup_center", "ui:interface",
          templateParams.empty() ? NULL : &(templateParams[0]),
          (uint)templateParams.size());
  if (group) {
    // Skill name
...
CGroupInSceneBubbleManager::CPopupContext *
CGroupInSceneBubbleManager::buildContextHelp(const string &templateName,
  ...
  if (target) {
    ...
    h = (target->getXReal() <
         ((sint)width - target->getXReal() - target->getWReal()))
            ? "l"
            : "r";
    v = (target->getYReal() <
         ((sint)height - target->getYReal() - target->getHReal()))
            ? "b"
            : "t";
  ...
  if (group) {
    ...
    if (target) {
      ...
      group->setPosRef((h == "r")   ? (v == "t") ? Hotspot_TR : Hotspot_BR
                       : (v == "t") ? Hotspot_TL
                                    : Hotspot_BL);
      group->setParentPosRef((h == "l")   ? (v == "b") ? Hotspot_TR : Hotspot_BR
                             : (v == "b") ? Hotspot_TL
                                          : Hotspot_BL);
...
void CGroupInSceneBubbleManager::addContextHelp(const string &message,
                                                const string &targetName,
                                                uint time) {
  std::string finalMessage = message;
  CInterfaceElement *target;
  CPopupContext *context = CGroupInSceneBubbleManager::buildContextHelp(
      "context_help_", targetName, target, time);
  if (context) {
...
void CGroupInSceneBubbleManager::addContextHelpHTML(const string &url,
                                                    const string &targetName,
                                                    uint time) {
  // Ignore this
  if (_IgnoreContextHelp.find(url) == _IgnoreContextHelp.end()) {
    CInterfaceElement *target;
    CPopupContext *context = CGroupInSceneBubbleManager::buildContextHelp(
        "context_help_html_", targetName, target, time);
    if (context) {
      CInterfaceManager *pIM = CInterfaceManager::getInstance();
      CAHManager::getInstance()->runActionHandler(
          "browse", NULL,
          "name=" + context->Group->getId() +
              ":header_opened:window:html|url=" + url);
...
void CGroupInSceneBubbleManager::ignoreContextHelp(
    CInterfaceGroup *groupToRemove) {
  uint i;
  for (i = 0; i < _BubblePopup.size(); i++) {
...
void CGroupInSceneBubbleManager::chatOpen(uint32 nUID,
                                          const std::string &ucsText,
                                          uint bubbleTimer) {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();

  CCharacterCL *pChar = dynamic_cast<CCharacterCL *>(
      EntitiesMngr.getEntityByCompressedIndex(nUID));
  if (pChar == NULL || nUID == CLFECOMMON::INVALID_CLIENT_DATASET_INDEX)
    return;
  if (bubbleTimer == 0)
    bubbleTimer = CWidgetManager::getInstance()
  ...
  bool show = false;
  if (pChar->isUser())
    show = NLGUI::CDBManager::getInstance()
               ->getDbProp("UI:SAVE:INSCENE:USER:MESSAGES")
               ->getValueBool();
  else if (pChar->isFriend())
    show = NLGUI::CDBManager::getInstance()
               ->getDbProp("UI:SAVE:INSCENE:FRIEND:MESSAGES")
               ->getValueBool();
  else
    show = NLGUI::CDBManager::getInstance()
               ->getDbProp("UI:SAVE:INSCENE:ENEMY:MESSAGES")
               ->getValueBool();

  if (show) {
    // Check that we can create the new bubble (if a dynamic bubble is already
    // present do not replace it !
    CGroupInSceneBubble *pCharBubble = pChar->getBubble();
    if (pCharBubble != NULL)
      if (strnicmp(pCharBubble->getId().c_str(),
                   "ui:interface:in_scene_dyn_bubble", 32) == 0)
        return;
...
void CGroupInSceneBubbleManager::dynChatOpen(uint32 nBotUID, uint32 nBotName,
                                             const vector<uint32> &DynStrs) {
  ...
  if (pos == _DynBubbles.size()) {
    ...
    CInterfaceGroup *group =
        CWidgetManager::getInstance()->getParser()->createGroupInstance(
            "dyn_3dbulle_L", "ui:interface", templateParams);
    if (group == NULL) {
  ...
  for (j = 0; j < 8; ++j) {
    pVT = dynamic_cast<CViewTextID *>(
        bubble->getElement(id + "opt" + toString(j)));
    if (pVT != NULL) {
      pVT->setActive(false);
      pVT->setTextId(0);
    }
    pCL = dynamic_cast<CCtrlLink *>(
        bubble->getElement(id + "optb" + toString(j)));
    if (pCL != NULL)
      pCL->setActive(false);
  }
...
void CGroupInSceneBubbleManager::webIgChatOpen(uint32 nBotUID, string text,
  ...
  if (pos == _DynBubbles.size()) {
    ...
    CInterfaceGroup *group =
        CWidgetManager::getInstance()->getParser()->createGroupInstance(
            "webig_3dbulle_L", "ui:interface", templateParams);
    if (group == NULL) {
  ...
  for (j = 0; j < 8; ++j) {
    pVT =
        dynamic_cast<CViewText *>(bubble->getElement(id + "opt" + toString(j)));
    if (pVT != NULL) {
      pVT->setActive(false);
  ...
  for (j = 0; j < strs.size(); ++j) {
    ...
    if (pVT != NULL) {
      ...
      if (pCL != NULL) {
        ...
        pCL->setParamsOnLeftClick(
            "name=ui:interface:web_transactions:content:html|show=0|url=" +
            links[j]);
        pCL->setActive(true);
      }
    }
...
uint32 CGroupInSceneBubbleManager::CDynBubble::getOptionStringId(uint option) {
  ...
  CViewTextID *pVT = dynamic_cast<CViewTextID *>(
      Bubble->getElement(id + "opt" + toString(option)));
  if (!pVT)
    return 0;
...
NLMISC_COMMAND(testDynChatOpen1, "", "") {
  ...
  if (selection != NULL) {
    ...
    InSceneBubbleManager.dynChatOpen(selection->dataSetId(), 315,
                                     DescAndOptions);
  }
  return true;
}
...
void CGroupInSceneBubbleManager::dynChatClose(uint32 nBotUID) {
  ...
  // Note: if we receive the impulsion close after a new impulsion open, the NPC
  // could fail to turn towards the user entity if there was no frame in between
  // (because the turning is done by CUserEntity::updatePreCamera()).
  UserEntity->interlocutor(CLFECOMMON::INVALID_SLOT);
}

// ***************************************************************************
CGroupInSceneBubbleManager::CDynBubble *
CGroupInSceneBubbleManager::getDynBubble(uint32 nDynBubbleNb) {
  uint32 value;
...
// ***************************************************************************
uint32 CGroupInSceneBubbleManager::dynChatGetOptionStringId(uint32 nBubbleNb,
                                                            uint option) {
  CDynBubble *pDB = getDynBubble(nBubbleNb);
  if (pDB == NULL)
    return 0;
...
void CGroupInSceneBubbleManager::CDynBubble::skip() {
  ...
  if (nNbOptions == 1) {
    CInterfaceManager *pIM = CInterfaceManager::getInstance();
    CAHManager::getInstance()->runActionHandler("dynchat_click_option", Bubble,
                                                "0");
  }
}
...
class CAHDynChatClickOption : public IActionHandler {
  virtual void execute(CCtrlBase *pCaller, const string &Params) {
    ...
    uint32 optStrId =
        InSceneBubbleManager.dynChatGetOptionStringId(nBubbleNb, nOpt);
    if (!optStrId)
      return;
    ...
    if (isBGDownloadEnabled()) {
      ...
      if (!pSMC->getDynString(optStrId, result)) {
        return; // shouldn't happen since the button isn't visible as long as
                // the text has not been received ...
      }
...
// ***************************************************************************
NLMISC_REGISTER_OBJECT(CViewBase, CGroupInSceneBubble, std::string,
                       "in_scene_bubble");
...
void CGroupInSceneBubble::next() {
  if (_TextParts.empty())
    return;
  if (_CurrentPart >= (_TextParts.size() - 1))
    return;
  _CurrentPart++;
  setRawText(_TextParts[_CurrentPart]);
  displayNextAndSkip(!isOnLastPage());
...
void CGroupInSceneBubble::setRawText(const string &text) {
  ...
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  CInterfaceElement *pVTIE = CWidgetManager::getInstance()->getElementFromId(
      getId() + ":header_opened:window:text");
...
void CGroupInSceneBubble::displayNextAndSkip(bool show) {
  ...
  CInterfaceElement *pIE = CWidgetManager::getInstance()->getElementFromId(
      getId() + ":header_opened:window:but_next");
  if (pIE != NULL)
    pIE->setActive(show);
  pIE = CWidgetManager::getInstance()->getElementFromId(
      getId() + ":header_opened:window:but_skip");
  if (pIE != NULL)
    pIE->setActive(show);
  pIE = CWidgetManager::getInstance()->getElementFromId(
      getId() + ":header_opened:window:text");
...
class CHandlerBubbleNext : public IActionHandler {
  void execute(CCtrlBase *pCaller, const std::string & /* sParams */) {
    ...
    if (strnicmp(pBubble->getId().c_str(), "ui:interface:in_scene_dyn_bubble",
                 32) == 0) {
...
class CHandlerCharacterBubble : public IActionHandler {
  void execute(CCtrlBase * /* pCaller */, const std::string &sParams) {
    ...
    uint duration;
    if (sTime.empty())
      duration = CWidgetManager::getInstance()
                     ->getSystemOption(CWidgetManager::OptionTimeoutBubbles)
                     .getValSInt32();
    else
...
class CHandlerContextHelp : public IActionHandler {
  void execute(CCtrlBase * /* pCaller */, const std::string &sParams) {
    ...
    InSceneBubbleManager.addContextHelp(
        itext, targetName,
        CWidgetManager::getInstance()
            ->getSystemOption(CWidgetManager::OptionTimeoutContext)
            .getValSInt32());
  }
};
