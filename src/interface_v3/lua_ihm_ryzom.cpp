...
// to get rid of you_must_not_use_assert___use_nl_assert___read_debug_h_file
// messages
#include <cassert>
#ifdef assert
#undef assert
...
public:
  void execute(CCtrlBase *pCaller, const std::string &sParams) {
    ...
    // most often action handlers are called from xml files => lot of redundant
    // script
...
static DECLARE_INTERFACE_USER_FCT(lua) {
  ...
  script = retId + "= " + script;
  // execute a small script here,   because most often exprs are called from xml
  // files => lot of redundant script
...
int CLuaIHMRyzom::luaClientCfgIndex(CLuaState &ls) {
  ...
  if (v->size() != 1) {
    ...
    throw ELuaWrappedFunctionException(
        &ls, "Access to array inside client.cfg not supported.");
  }
...
int CLuaIHMRyzom::luaClientCfgNewIndex(CLuaState &ls) {
  ...
  throw ELuaWrappedFunctionException(&ls,
                                     "Can't write into config file from lua.");
}
...
void CLuaIHMRyzom::createLuaEnumTable(CLuaState &ls, const std::string &str) {
  ...
  CSString s = str;
  // Create table recursively (ex: 'game.TPVPClan' will check/create the table
  // 'game' and 'game.TPVPClan')
  p = s.splitTo('.', true);
...
void CLuaIHMRyzom::RegisterRyzomFunctions(NLGUI::CLuaState &ls) {
  ...
  // *** Register the metatable for access to client.cfg (nb nico this may be
  // more general later -> access to any config file ...)
  ls.pushGlobalTable();
...
static sint32 getTargetSlotNr() {
  ...
  CCDBNodeLeaf *node =
      NLGUI::CDBManager::getInstance()->getDbProp(dbPath, false);
...
static CEntityCL *getSlotEntity(uint slot) { return EntitiesMngr.entity(slot); }
...
int CLuaIHMRyzom::getUI(CLuaState &ls) {
  ...
  CInterfaceElement *pIE =
      CWidgetManager::getInstance()->getElementFromId(eltStr);
  ...
  if (!pIE) {
    ...
    if (verbose) {
      ...
      debugInfo(NLMISC::toString("%s : getUI(): '%s' not found",
                                 stackContext.c_str(), eltStr.c_str()));
    }
  } else {
...
int CLuaIHMRyzom::formatUI(CLuaState &ls) {
  ...
  if (!CWidgetManager::getInstance()->getParser()->solveDefine(
          propVal, newPropVal, defError)) {
    throw ELuaIHMException("formatUI(): Can't find define: '%s'",
                           defError.c_str());
  }

  // *** format with any additional parameter and #1,    #2,    #3 etc...
  // search backward,    starting from bigger param to replace (thus avoid to
  // replace #1 before #13 for instance...)
  ...
  while (stackIndex > 1) {
    ...
    // For stack param 4,    the param index is 3 (because stack param 2 is the
    // param No 1)
    sint paramIndex = stackIndex - 1;

    while (NLMISC::strFindReplace(
        newPropVal, NLMISC::toString("#%d", paramIndex), paramValue))
      ;
...
int CLuaIHMRyzom::dumpUI(CLuaState &ls) {
  ...
  CLuaIHM::check(ls, CLuaIHM::isUIOnStack(ls, 1),
                 "dumpUI() requires a UI object in param 1");
  ...
  if (!pIE)
    ...
    nlassert(pIE->pinfo);
    debugInfo(NLMISC::toString(
        "UI: %x. %s. RefPtrCount: %d", pIE, pIE->getId().c_str(),
...
int CLuaIHMRyzom::validMessageBox(CLuaState &ls) {
  ...
  CLuaIHM::check(ls, CLuaIHM::pop(ls, msg),
                 "validMessageBox : ucstring wanted as first parameter");
#else
  CLuaIHM::checkArgType(ls, funcName, 1, LUA_TSTRING);
#endif
  ...
  CInterfaceManager *im = CInterfaceManager::getInstance();
#ifdef RYZOM_LUA_UCSTRING
  im->validMessageBox(CInterfaceManager::QuestionIconMsg, msg.toUtf8(),
                      ls.toString(2), ls.toString(3), ls.toString(4),
                      ls.toString(5), ls.toString(6));
#else
  im->validMessageBox(CInterfaceManager::QuestionIconMsg, ls.toString(1),
                      ls.toString(2), ls.toString(3), ls.toString(4),
                      ls.toString(5), ls.toString(6));
#endif
  return 0;
...
int CLuaIHMRyzom::setTextFormatTaged(CLuaState &ls) {
  ...
  CLuaIHM::check(ls, CLuaIHM::isUIOnStack(ls, 1),
                 "setTextFormatTaged() requires a UI object in param 1");
  CInterfaceElement *pIE = CLuaIHM::getUIOnStack(ls, 1);
  ...
  if (ls.isString(2)) {
    ...
    if (!CLuaIHM::pop(ls, text)) {
      CLuaIHM::check(
          ls, false,
          "setTextFormatTaged() requires a string or a ucstring in param 2");
    }
  }
#else
  string text;
  ...
  if (!vt)
    throw ELuaIHMException("setTextFormatTaged(): '%s' is not a CViewText",
                           pIE->getId().c_str());
...
int CLuaIHMRyzom::initEmotesMenu(CLuaState &ls) {
  ...
  CGroupMenu *pInitRootMenu = dynamic_cast<CGroupMenu *>(
      CWidgetManager::getInstance()->getElementFromId(emoteMenu));
  pInitRootMenu->reset();

  for (std::list<CEmoteStruct>::const_iterator it = entries.begin();
       it != entries.end(); it++) {
...
int CLuaIHMRyzom::enableModalWindow(CLuaState &ls) {
  ...
  CLuaIHM::check(ls, CLuaIHM::isUIOnStack(ls, 1),
                 "enableModalWindow() requires a UI object in param 1");
  CLuaIHM::checkArgType(ls, funcName, 2, LUA_TSTRING);
  ...
  if (pIE) {
    ...
    if (ctrl) {
      ...
      CInterfaceGroup *group = dynamic_cast<CInterfaceGroup *>(
          CWidgetManager::getInstance()->getElementFromId(modalId));
      ...
      if (group) {
        ...
        nlwarning("<CLuaIHMRyzom::enableModalWindow> Couldn't find group %s",
                  modalId.c_str());
      }
    }
  }
...
int CLuaIHMRyzom::getGroundAtMouse(CLuaState &ls) {
  ...
  if (CTool::isInScreen(x, y)) {
    ...
    viewport.getRayWithPoint(cursX, cursY, worldViewRay.Origin,
                             worldViewRay.Dir, camMatrix, camFrust);
...
int CLuaIHMRyzom::disableContextHelpForControl(CLuaState &ls) {
  ...
  CLuaIHM::check(
      ls, CLuaIHM::isUIOnStack(ls, 1),
      "disableContextHelpForControl() requires a UI object in param 1");

  // retrieve args
  CInterfaceElement *pIE = CLuaIHM::getUIOnStack(ls, 1);

  // go
  CWidgetManager::getInstance()->disableContextHelpForControl(
      dynamic_cast<CCtrlBase *>(pIE));
...
int CLuaIHMRyzom::isPlayerNewbie(CLuaState &ls) {
  ...
  CInterfaceManager *im = CInterfaceManager::getInstance();
  ls.push(NLGUI::CDBManager::getInstance()
...
int CLuaIHMRyzom::getSheet2idx(CLuaState &ls) {
  ...
  if (sheetId.buildSheetId(sheedtName)) {
    uint32 idx = CVisualSlotManager::getInstance()->sheet2Index(
        sheetId, (SLOTTYPE::EVisualSlot)slotId);
    ls.push(idx);
  } else
    return 0;
...
int CLuaIHMRyzom::getWeatherValue(CLuaState &ls) {
  ...
  if (ContinentMngr.cur()) {
    weather = ::getBlendedWeather(currDay, currHour, *WeatherFunctionParams,
...
int CLuaIHMRyzom::getIndexInDB(CLuaState &ls) {
  ...
  CLuaIHM::check(ls, CLuaIHM::isUIOnStack(ls, 1),
                 "getIndexInDB() requires a UI object in param 1");
...
int CLuaIHMRyzom::createGroupInstance(CLuaState &ls) {
  ...
  ENUM_LUA_TABLE(params, it) {
    if (!it.nextKey().isString()) {
      nlwarning("%s : bad key encountered with type %s, string expected.",
                funcName, it.nextKey().getTypename());
      continue;
    }

    if (!it.nextValue().isString()) {
      nlwarning("%s : bad value encountered with type %s for key %s, string "
                "expected.",
                funcName, it.nextValue().getTypename(),
                it.nextKey().toString().c_str());
      continue;
    }

    templateParams.push_back(std::pair<std::string, std::string>(
        it.nextKey().toString(),
        it.nextValue().toString())); // strange compilation bug here when I use
                                     // std::make_pair ... :(
  }
  CInterfaceManager *im = CInterfaceManager::getInstance();
  CInterfaceGroup *result =
      CWidgetManager::getInstance()->getParser()->createGroupInstance(
          ls.toString(1), ls.toString(2), templateParams);
...
int CLuaIHMRyzom::createRootGroupInstance(CLuaState &ls) {
  ...
  CInterfaceGroup *result =
      CWidgetManager::getInstance()->getParser()->createGroupInstance(
          ls.toString(1), "ui:interface:" + string(ls.toString(2)),
          templateParams);
  ...
  if (!result) {
    ...
    result->updateCoords();
    CWidgetManager::getInstance()->addWindowToMasterGroup("ui:interface",
                                                          result);
    CInterfaceGroup *pRoot = dynamic_cast<CInterfaceGroup *>(
...
int CLuaIHMRyzom::createUIElement(CLuaState &ls) {
  ...
  CInterfaceElement *result =
      CWidgetManager::getInstance()->getParser()->createUIElement(
          ls.toString(1), ls.toString(2), templateParams);
...
int CLuaIHMRyzom::displayBubble(CLuaState &ls) {
  ...
  InSceneBubbleManager.webIgChatOpen((uint32)ls.toInteger(1), ls.toString(2),
                                     strs, links);
...
int CLuaIHMRyzom::launchContextMenuInGame(CLuaState &ls) {
  ...
  CLuaIHM::check(ls, ls.isString(1),
                 "launchContextMenuInGame() requires a string in param 1");
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
...
int CLuaIHMRyzom::parseInterfaceFromString(CLuaState &ls) {
  ...
  CLuaIHM::check(ls, ls.isString(1),
                 "parseInterfaceFromString() requires a string in param 1");
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
...
int CLuaIHMRyzom::updateAllLocalisedElements(CLuaState &ls) {
  ...
  if (ClientCfg.R2EDVerboseParseTime) {
    nlinfo("%.2f seconds for 'updateAllLocalisedElements'",
...
int CLuaIHMRyzom::getCompleteIslands(CLuaState &ls) {
  ...
  // load entryPoints
  CScenarioEntryPoints scenarioEntryPoints =
      CScenarioEntryPoints::getInstance();
  const CScenarioEntryPoints::TCompleteIslands &islands =
      scenarioEntryPoints.getCompleteIslands();

  CScenarioEntryPoints::TCompleteIslands::const_iterator island(
      islands.begin()),
      lastIsland(islands.end());
  ...
  for (; island != lastIsland; ++island) {
    ...
    for (uint e = 0; e < island->EntryPoints.size(); e++) {
      const CScenarioEntryPoints::CShortEntryPoint &entryPoint =
          island->EntryPoints[e];
      ls.newTable();
...
int CLuaIHMRyzom::getIslandId(CLuaState &ls) {
  ...
  CLuaIHM::checkArgCount(ls, funcName, 1);
  CLuaIHM::check(ls, ls.isString(1),
                 "getIslandId() requires a string in param 1");
...
// ***************************************************************************
//
// addShape("shape", .x, .y, .z, "angle", .scale, collision?, "context", "url",
// highlight?, transparency?, "texture", "skeleton")
//
//********
int CLuaIHMRyzom::addShape(CLuaState &ls) {
  ...
  CShapeInstanceReference instref = EntitiesMngr.createInstance(
      shape, CVector(x, y, z), context, url, collision, idx);
  ...
  if (!instance.empty()) {
    ...
    if (primitive) {
      ...
      primitive->setSize((bbox.getMax().x - bbox.getMin().x) * scale,
                         (bbox.getMax().y - bbox.getMin().y) * scale);
...
sint32 CLuaIHMRyzom::getDbProp(const std::string &dbProp) {
  ...
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  CCDBNodeLeaf *node =
      NLGUI::CDBManager::getInstance()->getDbProp(dbProp, false);
...
void CLuaIHMRyzom::setDbProp(const std::string &dbProp, sint32 value) {
  ...
  if ((dbProp.compare(0, dbServer.size(), dbServer) == 0) ||
    ...
    if (dbProp.compare(0, dbLocalR2.size(), dbLocalR2) != 0) {
      nlstop;
      throw ELuaIHMException("setDbProp(): You are not allowed to write on "
...
void CLuaIHMRyzom::addDbProp(const std::string &dbProp, sint32 value) {
  ...
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  CCDBNodeLeaf *node =
      NLGUI::CDBManager::getInstance()->getDbProp(dbProp, true);
...
void CLuaIHMRyzom::debugInfo(const std::string &cstDbg) {
  ...
  if (ClientCfg.DisplayLuaDebugInfo) {
    ...
    if (ClientCfg.LuaDebugInfoGotoButtonEnabled) {
      CInterfaceManager *pIM = CInterfaceManager::getInstance();
      lua_State *ls =
          CLuaManager::getInstance().getLuaState()->getStatePointer();
      lua_Debug luaDbg;

      if (lua_getstack(ls, 1, &luaDbg)) {
        if (lua_getinfo(ls, "lS", &luaDbg)) {
          // add a command button to jump to the wanted file
          dbg = createGotoFileButtonTag(luaDbg.short_src, luaDbg.currentline) +
                dbg;
        }
      }
    }
...
void CLuaIHMRyzom::rawDebugInfo(const std::string &dbg) {
  ...
  if (ClientCfg.DisplayLuaDebugInfo) {
    ...
    if (!dbg.empty() && dbg[0] == '@') {
      ...
      NLMISC::InfoLog->displayRawNL(
          LuaHelperStuff::formatLuaErrorSysInfo(dbg).c_str());
    }
  ...
  if (ClientCfg.DisplayLuaDebugInfo) {
    ...
    while (lua_getstack(ls, stackLevel, &dbg)) {
      if (lua_getinfo(ls, "lS", &dbg)) {
        std::string result =
            createGotoFileButtonTag(dbg.short_src, dbg.currentline) +
            NLMISC::toString("%s:%d:", dbg.short_src, dbg.currentline);
        rawDebugInfo(result);
      }
...
// ***************************************************************************
void CLuaIHMRyzom::getCallStackAsString(int startStackLevel /*=0*/,
                                        std::string &result) {
...
std::string CLuaIHMRyzom::getDefine(const std::string &def) {
  ...
  if (ClientCfg.DisplayLuaDebugInfo &&
      !CWidgetManager::getInstance()->getParser()->isDefineExist(def))
...
// ***************************************************************************
void CLuaIHMRyzom::messageBox(const ucstring &text,
                              const std::string &masterGroup, int caseMode) {
...
// ***************************************************************************
void CLuaIHMRyzom::messageBoxWithHelp(
    const ucstring &text, const std::string &masterGroup) // TODO: Lua UTF-8
{
  // H_AUTO(Lua_CLuaIHM_messageBox)
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
...
// ***************************************************************************
bool CLuaIHMRyzom::executeFunctionOnStack(CLuaState &ls, int numArgs,
                                          int numRet) {
  ...
  if (dumpFunction) {
    ...
    nlwarning((std::string(ar.what) + ", at line " + toString(ar.linedefined) +
               " in " + std::string(ar.source))
                  .c_str());
  }
...
// ***************************************************************************
#ifdef RYZOM_LUA_UCSTRING
ucstring CLuaIHMRyzom::replacePvpEffectParam(const ucstring &str,
                                             sint32 parameter)
#else
std::string CLuaIHMRyzom::replacePvpEffectParam(const std::string &str,
                                                sint32 parameter)
#endif
{
...
#ifdef RYZOM_BG_DOWNLOADER
// ***************************************************************************
void CLuaIHMRyzom::pauseBGDownloader() { ::pauseBGDownloader(); }

// ***************************************************************************
void CLuaIHMRyzom::unpauseBGDownloader() { ::unpauseBGDownloader(); }
...
void CLuaIHMRyzom::requestBGDownloaderPriority(uint priority) {
  ...
  CBGDownloaderAccess::getInstance().requestDownloadThreadPriority(
      (BGDownloader::TThreadPriority)priority, false);
}
...
bool CLuaIHMRyzom::isPlayerSlotNewbieLand(uint32 slot) {
  if (slot > CharacterSummaries.size()) {
    throw ELuaIHMException("isPlayerSlotNewbieLand(): Invalid slot %d",
                           (int)slot);
  }
...
ucstring CLuaIHMRyzom::getSkillLocalizedName(sint32 skillId) {
  ...
  return ucstring(STRING_MANAGER::CStringManagerClient::getSkillLocalizedName(
      (SKILLS::ESkills)skillId));
}
#else
std::string CLuaIHMRyzom::getSkillLocalizedName(sint32 skillId) {
  // H_AUTO(Lua_CLuaIHM_getSkillLocalizedName)
  return STRING_MANAGER::CStringManagerClient::getSkillLocalizedName(
      (SKILLS::ESkills)skillId);
}
#endif
...
// ***************************************************************************
sint32 CLuaIHMRyzom::getMagicResistChance(bool elementalSpell,
                                          sint32 casterSpellLvl,
                                          sint32 victimResistLvl) {
  ...
  */
  sint32 chanceToHit =
      pPM->getSuccessRate(elementalSpell ? CSPhraseManager::STResistMagic
                                         : CSPhraseManager::STResistMagicLink,
...
sint32 CLuaIHMRyzom::getDodgeParryChance(sint32 attLvl, sint32 defLvl) {
  ...
  sint32 chance = pPM->getSuccessRate(CSPhraseManager::STDodgeParry,
                                      defLvl - attLvl, false);
  clamp(chance, 0, 100);
...
// ***************************************************************************
void CLuaIHMRyzom::browseNpcWebPage(const std::string &htmlId,
                                    const std::string &urlIn,
                                    bool addParameters, double timeout) {
  // H_AUTO(Lua_CLuaIHM_browseNpcWebPage)
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  CGroupHTML *groupHtml = dynamic_cast<CGroupHTML *>(
      CWidgetManager::getInstance()->getElementFromId(htmlId));
  ...
  if (groupHtml) {
    ...
    if (addParameters && !webig) {
      ...
      url += ((url.find('?') != string::npos) ? "&" : "?") + string("shard=") +
             toString(ShardId) + string("&user_login=") + userName +
...
ucstring CLuaIHMRyzom::getDynString(sint32 dynStringId) {
  ...
  string result;
  STRING_MANAGER::CStringManagerClient::instance()->getDynString(dynStringId,
                                                                 result);
...
bool CLuaIHMRyzom::isDynStringAvailable(sint32 dynStringId) {
  ...
  string result;
  bool res = STRING_MANAGER::CStringManagerClient::instance()->getDynString(
      dynStringId, result);
  return res;
}

// ***************************************************************************
bool CLuaIHMRyzom::isFullyPatched() { return AvailablePatchs == 0; }
...
string CLuaIHMRyzom::getGuildMemberGrade(sint32 nMemberId) {
  ...
  return EGSPD::CGuildGrade::toString(
      CGuildManager::getInstance()->getGuildMembers()[nMemberId].Grade);
}
...
// ***************************************************************************
std::string CLuaIHMRyzom::getCharacterSheetSkel(const std::string &sheet,
                                                bool isMale) {
  // H_AUTO(Lua_CLuaIHM_getCharacterSheetSkel)
  const CEntitySheet *sheetPtr = SheetMngr.get(CSheetId(sheet));
  const CCharacterSheet *charSheet =
      dynamic_cast<const CCharacterSheet *>(sheetPtr);

  if (charSheet)
    return charSheet->getSkelFilename();

  const CRaceStatsSheet *raceStatSheet =
      dynamic_cast<const CRaceStatsSheet *>(sheetPtr);

  if (raceStatSheet)
    return raceStatSheet->GenderInfos[isMale ? 0 : 1].Skelfilename;
...
sint CLuaIHMRyzom::getCharacterSheetRegionForce(const std::string &sheet) {
  ...
  const CCharacterSheet *charSheet =
      dynamic_cast<const CCharacterSheet *>(SheetMngr.get(CSheetId(sheet)));
...
#ifdef RYZOM_LUA_UCSTRING
        w->setCommand("tell " +
                          CEntityCL::removeTitleFromName(player.toUtf8()) + " ",
                      false);
#else
        w->setCommand("tell " + CEntityCL::removeTitleFromName(player) + " ",
                      false);
#endif
...
bool CLuaIHMRyzom::isRingAccessPointInReach() {
  ...
  if (BotChatPageAll->RingSessions->RingAccessPointPos == CVector::Null)
    return false;
...
sint32 CLuaIHMRyzom::getTargetLevel() {
  ...
  if (target->isPlayer()) {
    CInterfaceManager *pIM = CInterfaceManager::getInstance();
    CCDBNodeLeaf *pDbPlayerLevel = NLGUI::CDBManager::getInstance()->getDbProp(
        CWidgetManager::getInstance()->getParser()->getDefine(
            "target_player_level"));
    return pDbPlayerLevel ? pDbPlayerLevel->getValue32() : -1;
  } else {
    CCharacterSheet *pCS =
        dynamic_cast<CCharacterSheet *>(SheetMngr.get(target->sheetId()));
    ...
    if (!target->properties().attackable())
      return -1;
...
sint64 CLuaIHMRyzom::getTargetVpa() {
  ...
  sint64 prop =
      NLGUI::CDBManager::getInstance()
          ->getDbProp("SERVER:Entities:E" + toString("%d", getTargetSlotNr()) +
...
bool CLuaIHMRyzom::isPlayerInPVPMode() {
  ...
  return (UserEntity->getPvpMode() & PVP_MODE::PvpFaction ||
          UserEntity->getPvpMode() & PVP_MODE::PvpFactionFlagged ||
          UserEntity->getPvpMode() & PVP_MODE::PvpZoneFaction) != 0;
}
...
bool CLuaIHMRyzom::isTargetInPVPMode() {
  ...
  return (target->getPvpMode() & PVP_MODE::PvpFaction ||
          target->getPvpMode() & PVP_MODE::PvpFactionFlagged ||
          target->getPvpMode() & PVP_MODE::PvpZoneFaction) != 0;
}

// ***************************************************************************
std::string CLuaIHMRyzom::createGotoFileButtonTag(const char *fileName,
                                                  uint line) {
  ...
  if (ClientCfg.LuaDebugInfoGotoButtonEnabled) {
    ...
    return toString("/$$%s|%s|lua|%s('%s',   %d)$$/",
                    ...
                    ClientCfg.LuaDebugInfoGotoButtonFunction.c_str(), fileName,
                    line);
  }
...
// ***************************************************************************
void CLuaIHMRyzom::setDbRGBA(const std::string &dbProp,
  ...
  if ((dbProp.compare(0, dbServer.size(), dbServer) == 0) ||
    ...
    if (dbProp.compare(0, dbLocalR2.size(), dbLocalR2) != 0) {
      nlstop;
      throw ELuaIHMException("setDbRGBA(): You are not allowed to write on "
                             "'SERVER:...' or 'LOCAL:...' database");
    }
  }
  // write to the db
  CCDBNodeLeaf *node =
      NLGUI::CDBManager::getInstance()->getDbProp(dbProp, true);
  if (node)
    node->setValue64(color.R + (color.G << 8) + (color.B << 16) +
                     (color.A << 24));
  return;
}
