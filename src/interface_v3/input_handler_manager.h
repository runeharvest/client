...
/**
 * CInputManager. Class used to manage Input Handlers. its purpose is to manage
 * all input handler CInputManager get hardware events from input handlers, when
 * they call CInputManager::ProcessEvent CInputMangager convert the incoming
 * events into actions and call the appropriate control's callback
 *
 * This conversion is made through the reading of a config file describing the
 * association between events and actions \author Nicolas Brigand \author Nevrax
 * France \date 2002
 */
...
public:
  /// The EventServer Filled with Filtered Messages the InterfaceManager didn't
  /// catch
...
public:
  ...
  /** Return true if the event is handled by Text Edition in the interface
   *Manager All Events are stored in the xml input config file. Basics events
   *such as KeyA, KeyB etc.. should be in this config file. Combo Keys like
   *Ctrl+A (select all) should be in this config file too.
   */
  ...
  /** Same as pumpEvents() but the messages are not processed by the
   *InterfaceManager, but directly posted to FilteredEventServer
   */
  void pumpEventsNoIM();
