...
/** skip a block of character in a string, (same behaviour than when Ctrl-arrow
 * is pressed) It returns the new index
 */
static uint skipUCCharsRight(uint startPos, const ::u32string &str) {
  ...
  while (pos != endIndex && getCharacterCategory(str[pos]) == ccat)
    ++pos;
  // skip spaces
  while (pos != endIndex && str[pos] == ' ')
    ++pos;
  return pos;
}
...
static uint skipUCCharsLeft(uint startPos, const ::u32string &str) {
  ...
  uint ccat = getCharacterCategory(str[pos]);
  // skip characters of the same category
  while (pos != 0 && getCharacterCategory(str[pos - 1]) == ccat)
    --pos;
  return pos;
}
...
protected:
  ...
  void handleUnselection() {
    if (_GroupEdit) {
      // If selection active
      CGroupEditBox *currSelection =
          dynamic_cast<CGroupEditBox *>(CGroupEditBox::getCurrSelection());
...
class CAHEditPreviousChar : public CAHEdit {
  void actionPart() {
    if (_LooseSelection) {
      sint32 minPos =
          min(_GroupEdit->getCursorPos(), _GroupEdit->getSelectCursorPos());
      sint32 maxPos =
          max(_GroupEdit->getCursorPos(), _GroupEdit->getSelectCursorPos());

      // Special Code if left or right: Set the cursor pos to bounds of
      // selection.
      if (minPos != maxPos) {
...
class CAHEditNextChar : public CAHEdit {
  void actionPart() {
    ...
    if (_GroupEdit->getCursorPos() <
        (sint32)_GroupEdit->getInputStringRef().length()) {
      _GroupEdit->setCursorPos(_GroupEdit->getCursorPos() + 1);
      _GroupEdit->setCursorAtPreviousLineEnd(false);
    }
  }
...
class CAHEditPreviousWord : public CAHEdit {
  void actionPart() {
    if (_GroupEdit->getCursorPos() > 0) {
      _GroupEdit->setCursorPos(skipUCCharsLeft(
          _GroupEdit->getCursorPos(), _GroupEdit->getInputStringRef()));
      _GroupEdit->setCursorAtPreviousLineEnd(false);
    }
  }
};
...
class CAHEditNextWord : public CAHEdit {
  void actionPart() {
    if (_GroupEdit->getCursorPos() <
        (sint32)_GroupEdit->getInputStringRef().length()) {
      _GroupEdit->setCursorPos(skipUCCharsRight(
          _GroupEdit->getCursorPos(), _GroupEdit->getInputStringRef()));
      _GroupEdit->setCursorAtPreviousLineEnd(false);
    }
  }
...
class CAHEditGotoLineBegin : public CAHEdit {
  void actionPart() {
    ...
    if (_GroupEdit->getViewText()) {
      sint line = _GroupEdit->getViewText()->getLineFromIndex(
          _GroupEdit->getCursorPos() +
          (uint)_GroupEdit->getPromptRef().length());
      if (line == -1)
        return;
      sint newPos = std::max(_GroupEdit->getViewText()->getLineStartIndex(line),
                             (sint)_GroupEdit->getPromptRef().length());
      if (newPos == -1)
        return;
      _GroupEdit->setCursorPos(newPos -
                               (sint32)_GroupEdit->getPromptRef().length());
      _GroupEdit->setCursorAtPreviousLineEnd(false);
    }
  }
};
    ...
    if (_GroupEdit->getViewText()) {
      if (_GroupEdit->getViewText()->getMultiLine()) {
        sint line = _GroupEdit->getViewText()->getLineFromIndex(
            _GroupEdit->getCursorPos() +
                (uint)_GroupEdit->getPromptRef().length(),
            _GroupEdit->isCursorAtPreviousLineEnd());
        if (line == -1)
          return;
        sint newPos;
        bool endOfPreviousLine;
        _GroupEdit->getViewText()->getLineEndIndex(line, newPos,
                                                   endOfPreviousLine);
        if (newPos != -1) {
          _GroupEdit->setCursorPos(newPos -
                                   (sint32)_GroupEdit->getPromptRef().length());
        ...
        _GroupEdit->setCursorPos((sint32)_GroupEdit->getPromptRef().length() +
                                 (sint32)_GroupEdit->getInputString().length());
      }
    }
  }
};
...
class CAHEditPreviousLine : public CAHEdit {
  void actionPart() {
    if (_GroupEdit->getMaxHistoric() &&
        (!_GroupEdit->getViewText()->getMultiLine())) {
      // Get the start of the string.
      ::u32string startStr =
          _GroupEdit->getInputStringRef().substr(0, _GroupEdit->getCursorPos());

      // Search all historic string that match startStr.
      for (sint i = _GroupEdit->getCurrentHistoricIndex() + 1;
           i < (sint)_GroupEdit->getNumHistoric(); i++) {
        if (_GroupEdit->getHistoric(i).compare(0, _GroupEdit->getCursorPos(),
                                               startStr) == 0) {
      ...
      uint cursorPosInText = _GroupEdit->getCursorPos() +
                             (uint)_GroupEdit->getPromptRef().length();
      if ((_GroupEdit->getCursorPos() ==
               (sint32)_GroupEdit->getInputStringRef().length() &&
           _GroupEdit->getViewText()->getNumLine() == 1) ||
          _GroupEdit->getViewText()->getLineFromIndex(
              cursorPosInText, _GroupEdit->isCursorAtPreviousLineEnd()) ==
              0) // in the first line .. ?
      {
        // .. so do nothing
        return;
      ...
      float cx, cy;
      float height;
      _GroupEdit->getViewText()->getCharacterPositionFromIndex(
          cursorPosInText, _GroupEdit->isCursorAtPreviousLineEnd(), cx, cy,
          height);
      cy += _GroupEdit->getViewText()->getLineHeight();
      uint newCharIndex;
      bool newLineEnd;
      _GroupEdit->getViewText()->getCharacterIndexFromPosition(
          cx, cy, newCharIndex, newLineEnd);
      if (newLineEnd) {
        _GroupEdit->setCursorPos(newCharIndex -
                                 (sint32)_GroupEdit->getPromptRef().length());
        _GroupEdit->setCursorAtPreviousLineEnd(true);
        sint32 newPos = _GroupEdit->getCursorPos();
        clamp(newPos, (sint32)0,
              (sint32)_GroupEdit->getInputStringRef().size());
        _GroupEdit->setCursorPos(newPos);
        return;
      }
      _GroupEdit->setCursorAtPreviousLineEnd(false);
      ...
      float cy0, cy1;
      _GroupEdit->getViewText()->getCharacterPositionFromIndex(
          newCharIndex, _GroupEdit->isCursorAtPreviousLineEnd(), cx0, cy0,
          height);
      _GroupEdit->getViewText()->getCharacterPositionFromIndex(
          newCharIndex + 1, _GroupEdit->isCursorAtPreviousLineEnd(), cx1, cy1,
          height);
      ...
      _GroupEdit->setCursorPos(_GroupEdit->getCursorPos() -
                               (sint32)_GroupEdit->getPromptRef().length());
      sint32 newpos = _GroupEdit->getCursorPos();
...
class CAHEditNextLine : public CAHEdit {
  void actionPart() {
    if ((!_GroupEdit->getViewText()->getMultiLine()) &&
        _GroupEdit->getMaxHistoric() &&
        _GroupEdit->getCurrentHistoricIndex() > 0) {
      // Get the start of the string.
      ::u32string startStr =
          _GroupEdit->getInputStringRef().substr(0, _GroupEdit->getCursorPos());
      ...
      _GroupEdit->getViewText()->getCharacterPositionFromIndex(
          _GroupEdit->getCursorPos() +
              (sint)_GroupEdit->getPromptRef().length(),
          _GroupEdit->isCursorAtPreviousLineEnd(), cx, cy, height);
      if (cy != 0) {
        cy -= height;
        uint newCharIndex;
        bool newLineEnd;
        ...
        if (abs(cx0 - cx) < abs(cx1 - cx) || cy0 != cy1) {
          ...
          _GroupEdit->setCursorPos(
              min(sint32(newCharIndex + 1),
                  sint32(_GroupEdit->getInputStringRef().length() +
                         _GroupEdit->getPromptRef().length())));
        }
        _GroupEdit->setCursorPos(_GroupEdit->getCursorPos() -
                                 (sint32)_GroupEdit->getPromptRef().length());
        sint32 newPos = _GroupEdit->getCursorPos();
...
protected:
  void initPart() { init(); }
  void actionPart() {
    // if selection is activated and not same cursors pos, then cut the
    // selection
    if (CGroupEditBox::getCurrSelection() != NULL &&
        _GroupEdit->getCursorPos() != CGroupEditBox::getSelectCursorPos()) {
      ...
      if (!_GroupEdit->getAHOnChange().empty()) {
        ...
        CAHManager::getInstance()->runActionHandler(
            _GroupEdit->getAHOnChange(), _GroupEdit,
            _GroupEdit->getParamsOnChange());
      }
    }
    // else cut forwards
    else if (_GroupEdit->getCursorPos() <
             (sint32)_GroupEdit->getInputStringRef().length()) {
      ::u32string inputString = _GroupEdit->getInputStringRef();
      ::u32string::iterator it =
          inputString.begin() + _GroupEdit->getCursorPos();
...
class CAHEditSelectAll : public CAHEdit {
  void initPart() { init(); }
  void actionPart() { _GroupEdit->setSelectionAll(); }
};
...
class CAHEditCopy : public CAHEdit {
  void initPart() { init(); }
  void actionPart() { _GroupEdit->copy(); }
  void forwardToEditor() { R2::getEditor().copy(); }
};
...
class CAHEditPaste : public CAHEdit {
  void initPart() { init(); }
  void actionPart() { _GroupEdit->paste(); }
  void forwardToEditor() { R2::getEditor().paste(); }
};
...
class CAHEditExpand : public CAHEdit {
  void initPart() { init(); }
  void actionPart() { _GroupEdit->expand(); }
};
...
class CAHEditExpandOrCycleTell : public CAHEdit {
  ...
  void actionPart() {
    ...
    if (!NLMISC::startsWith(_GroupEdit->getInputString(), "/tell ")) {
      if (_GroupEdit->expand())
        return;
    }
    CInterfaceManager *im = CInterfaceManager::getInstance();
    if (!im->isInGame())
      return;
    // there was no / at the start of the line so try to cycle through the last
    // people on which a tell was done
    const string *lastTellPeople = ChatMngr.cycleLastTell();
    if (!lastTellPeople)
      return;
    // Get chat box from ist edit box
    // If it isn't a user chat or the main chat, just display 'tell' with the
    // name. Otherwise, change the target of the window
...
class CAHEditBack : public CAHEdit {
  void initPart() { init(); }
  void actionPart() { _GroupEdit->back(); }
};
...
REGISTER_ACTION_HANDLER(CAHEditSelectToBlockBegin,
