...
public:
  ...
  bool Requesting;
  CControlSheetInfoWaiter() : IItemInfoWaiter(), Requesting(false) {}
...
public:
  enum TSheetType {
    ...
    // A skill
    SheetType_Skill,
    // Special : the type of the sheet is automatically deduced from its sheet,
    // it drawn like an item
    SheetType_Auto,
    ...
    SheetType_SPhraseId,
    // New Sabrina Phrase. Complete phrase used for bot chat sentence (sheet id
    // represent a CSPhraseSheet)
    SheetType_SPhrase,
...
public:
  ...
  // Each bit is for the associated BRICK_TYPE::EBrickType enum (unnknown bit 0
  // etc...) Default is ALL (ie 0xFFFFFFFF)
  ...
  bool _ReadQuantityFromSheet : 1; // Read quantity from sheet rather than from
                                   // database
  bool _UseQuality : 1;            // is the quality read and displayed ?
  bool _DisplayItemQuality : 1;    // Do we have to display the quality for the
                                // item (false for Cosmetics and Teleport and if
                                // _UseQuality==fasle)?
  bool _DuplicateOnDrag : 1; // when dragged, the item is shown twice : one
                             // version at the mouse position. and another in
                             // the source slot. Useful for items to buy that
                             // are in infinite quantity.
  bool _AutoGrayed : 1; // if true then gray the ctrlSheeet if: 1/ Items: Is the
                        // Item Locked. 2/ Bricks: is the brick Latent.
  bool _HasTradeSlotType : 1; // true is the SLOT_TYPE DB field should be taken
                              // in account

  bool _BrickOverable : 1; // if Type is Brick, display like a button (because
                           // LeftClickable).
  ...
  // For an Item only, this tells what item type this slot accept. Undefined
  // means ALL
...
public:
  ...
  void setListMenuBoth(const std::string &cm) {
    _ListMenuLeft = _ListMenuRight = cm;
  }
  ...
  const std::string &getListMenuRightEmptySlot() {
    return _ListMenuRightEmptySlot;
  }
};
...
public:
  ...
  virtual void draw();
  void drawSheet(sint32 scrX, sint32 scrY, bool draging,
                 bool showSelectionBorder = true);

  virtual bool handleEvent(const NLGUI::CEventDescriptor &event);

  void setActionOnLeftClick(const std::string &ActionHandlerName) {
    _AHOnLeftClick =
        CAHManager::getInstance()->getAH(ActionHandlerName, _AHLeftClickParams);
  }
  void setActionOnRightClick(const std::string &ActionHandlerName) {
    _AHOnRightClick = CAHManager::getInstance()->getAH(ActionHandlerName,
                                                       _AHRightClickParams);
  }
  void setActionOnDrop(const std::string &ActionHandlerName) {
    _AHOnDrop =
        CAHManager::getInstance()->getAH(ActionHandlerName, _AHDropParams);
  }
  void setActionOnCanDrop(const std::string &ActionHandlerName) {
    _AHOnCanDrop =
        CAHManager::getInstance()->getAH(ActionHandlerName, _AHCanDropParams);
  }
  void setParamsOnLeftClick(const std::string &ParamsHandlerName) {
    _AHLeftClickParams = ParamsHandlerName;
  }
  void setParamsOnRightClick(const std::string &ParamsHandlerName) {
    _AHRightClickParams = ParamsHandlerName;
  }
  void setParamsOnDrop(const std::string &ParamsHandlerName) {
    _AHDropParams = ParamsHandlerName;
  }
  void setParamsOnCanDrop(const std::string &ParamsHandlerName) {
    _AHCanDropParams = ParamsHandlerName;
  }

  const std::string &getActionOnLeftClick() const {
    return CAHManager::getInstance()->getAHName(_AHOnLeftClick);
  }
  const std::string &getActionOnRightClick() const {
    return CAHManager::getInstance()->getAHName(_AHOnRightClick);
  }
  const std::string &getActionOnDrop() const {
    return CAHManager::getInstance()->getAHName(_AHOnDrop);
  }
  const std::string &getActionOnCanDrop() const {
    return CAHManager::getInstance()->getAHName(_AHOnCanDrop);
  }
  const std::string &getParamsOnLeftClick() const { return _AHLeftClickParams; }
  const std::string &getParamsOnRightClick() const {
    return _AHRightClickParams;
  }
  ...
  void setListMenuLeft(const std::string &cm) {
    CCtrlSheetInfo::setListMenuLeft(cm);
  }
  void setListMenuRight(const std::string &cm) {
    CCtrlSheetInfo::setListMenuRight(cm);
  }
  void setListMenuBoth(const std::string &cm) {
    CCtrlSheetInfo::setListMenuBoth(cm);
  }
  const std::string &getListMenuLeft() {
    return CCtrlSheetInfo::getListMenuLeft();
  }
  const std::string &getListMenuRight() {
    return CCtrlSheetInfo::getListMenuRight();
  }
  const std::string &getListMenuRightEmptySlot() {
    return CCtrlSheetInfo::getListMenuRightEmptySlot();
  }
  ...
  // Swap the content with another ctrl_sheet (debug): SheetId, Quantity and
  // Quality
  ...
  sint32 getQuality() const {
    return _UseQuality ? (sint32)_Quality.getSInt32() : 0;
  }
  ...
  // Operation on the link between the sheet and database. the string is the
  // branch root of SHEET....
  std::string getSheet() const;
  void setSheet(const std::string &dbBranchId);
  void setSheetFast(const std::string &dbParentBranchId, int sheetNum,
                    int slotNum);
  ...
  REFLECT_STRING_REF("right_click", getActionOnRightClick,
                     setActionOnRightClick);
  REFLECT_STRING_REF("left_click_params", getParamsOnLeftClick,
                     setParamsOnLeftClick);
  REFLECT_STRING_REF("right_click_params", getParamsOnRightClick,
  ...
  REFLECT_STRING_REF("on_can_drop_params", getParamsOnCanDrop,
                     setParamsOnCanDrop);
  ...
  REFLECT_LUA_METHOD("buildCrystallizedSpellListBrick",
                     luaBuildCrystallizedSpellListBrick);
  REFLECT_EXPORT_END
  ...
  void initSheetFast(const std::string &dbParentBranchId, int sheetNum,
                     int slotNum, const CCtrlSheetInfo &ctrlInfo);
  ...
  /** If the branch in setSheet(branch) is of the form ...:# (where # is a
   *number), return this number. The value is hence modified by setSheet().
   *return 0 if not of this form.
   */
  uint32 getIndexInDB() const { return _IndexInDB; }
  /** If the branch in setSheet(branch) is of the form ...:#:# (where # are
   *numbers), return this number. The value is hence modified by setSheet().
   *return 0 if not of this form.
   */
  ...
  // Get the last dropped sheet. The pointer is only valid during the call of
  // the event handler
  // static CDBCtrlSheet	  *getDraggedSheet() { return _LastDraggedSheet;
  // }

  /* Get the last selected sheet that have been draged or right clicked (should
   * be use by menu to get their caller) It is used by the item actions like
   * destroy, move etc..
   */
  ...
  void setUseQuality(bool use) {
    if (use != _UseQuality) {
      _UseQuality = use;
      _NeedSetup = true;
    }
  }
  void setUseQuantity(bool use) {
    if (use != _UseQuantity) {
      _UseQuantity = use;
      _NeedSetup = true;
    }
  ...
  bool isSkill() const {
    return _HasTradeSlotType
               ? _TradeSlotType.getSInt32() == TRADE_SLOT_TYPE::Skill
               : false;
  }
  ...
  SKILLS::ESkills getSkill() const {
    return isSkill() ? (SKILLS::ESkills)getSheetId() : SKILLS::unknown;
  }

  // set special behaviour
  void setBehaviour(TRADE_SLOT_TYPE::TTradeSlotType type);
  TRADE_SLOT_TYPE::TTradeSlotType getBehaviour() const {
    return _HasTradeSlotType
               ? (TRADE_SLOT_TYPE::TTradeSlotType)_TradeSlotType.getSInt32()
               : TRADE_SLOT_TYPE::StandardBehaviour;
  }
  ...
  // get the ListSheet parent. NB: different code from CDBGroupListSheet and
  // CDBGroupListSheetTrade. NULL if not found
  IListSheetBase *getListSheetParent() const;

  // get the index of this sheet in its parent list, or -1 if there's no such
  // list
  ...
  /** According to ActualType, return true if the sheet is valid. eg:
   *getSheetId()!=0 and brick exist for sheet type brucks. Always for Macros and
   *skills.
   */
  ...
  NLMISC::CCDBNodeLeaf *getSlotType() const {
    return _TradeSlotType.getNodePtr();
  }
  ...
  NLMISC::CCDBNodeLeaf *getItemRMClassTypePtr() const {
    return _ItemRMClassType;
  }
  ...
  NLMISC::CCDBNodeLeaf *getItemRMFaberStatTypePtr() const {
    return _ItemRMFaberStatType;
  }
  ...
  sint32 getItemColor() const {
    if (_UserColor)
      return _UserColor->getValue32();
    else
      return -1;
  }
  // set item color (if possible)
  void setItemColor(sint32 val) {
    if (_UserColor)
      _UserColor->setValue32(val);
  }
  ...
  // Get the Actual item name. Localized version of SheetId, or given by server
  // through NAMEID.
  std::string getItemActualName() const;

  /// true if support drag copy (with CTRL). action handler has to check
  /// control.
  ...
  // true if an item that respect Carac requirement. NB: still return true if
  // not an item
  ...
  //
  // ie: "secondsToTimeStringShort"     ->
  // CInterfaceExpr::evalAsString("secondsToTimeStringShort(123)", ret) ie:
  // "lua:secondsToTimeStringShort" ->
  // CInterfaceExpr::evalAsString("lua:secondsToTimeStringShort(123,
  // 'UI:VARIABLES:BONUSES:0')", ret)
...
protected:
  ...
  void setupDisplayAsPhrase(const std::vector<NLMISC::CSheetId> &bricks,
                            const std::string &phraseName);

  // draw a number and returns the width of the drawn number
  sint32 drawNumber(sint32 x, sint32 y, sint32 wSheet, sint32 hSheet,
                    NLMISC::CRGBA color, sint32 value, bool rightAlign = true);
...
protected:
  ...
  CInterfaceProperty _Worned; // if true means that item is worned (red cross,
                              // no longer usable unless it's a tool)
  ...
  sint32 _DispOver2BmpId; // Over Icon N0 2 for bricks / items. Useful for items
                          // when _DispOverBmpId is used to paint user color on
                          // the item.
  ...
  bool _ItemBeastGrayed : 1; // This is an item in a pack animal / beast, that
                             // is unavailable
  bool _ItemWeared : 1;      // Item weared (for bags item)
  bool _Grayed : 1;
  bool _Useable : 1; // if false, means more than grayed: "red-ed" we cannot get
                     // or use it. SBrick, SPhrase and SItem
  ...
  // For an Item only. Useful for LeftHand Filtering: must have a pointer to the
  // right hand
  ...
  // Macro or sentence String compiled as texture Ids and positions, from the
  // _OptString.
  struct CCharBitmap {
    sint32 X, Y;
    sint32 Id;
...
private:
  ...
  void setupCharBitmaps(sint32 maxW, sint32 maxLine, sint32 maxWChar = 1000,
                        bool topDown = false);
  void resetCharBitmaps();
  void displayCharBitmaps(sint32 rdrLayer, sint32 x, sint32 y,
                          NLMISC::CRGBA color);
  ...
  // setup sheet DB. _DbBranchName must be ok, and _SecondIndexInDb and
  // _IndexInDb as well
  void setupSheetDbLinks();

  // 'regen' rendering
  // convert from uv coordinates in the [0, 1] x [0, 1] range to screen coords
  inline void uvToScreen(float x, float y, NLMISC::CVector &screenPos,
                         uint texSize) const;
  // from an angle in the [0, 1] range, return both uv & screen coords for that
  // angle (angle is projected on the side of rectangle of the 'regen' texture)
  void buildPieCorner(float angle, NLMISC::CUV &uv, NLMISC::CVector &pos,
                      uint texSize) const;
  // from a start and end angle in the [0, 1] range, build the set of uv mapped
  // triangles necessary to display that 'pie' NB : output are indeed quads, at
  // the time ofthis writing, uv mappedtri randering not available ... so we
  // turn them into quad for conveniency ...
  uint buildPie(NLMISC::CQuadUV *quv, float startAngle, float endAngle,
                uint texSize);

  // gelper to draw the notify animation
  void drawRotatedQuad(CViewRenderer &vr, float angle, float scale,
                       uint renderLayer, uint32 textureId, sint32 texWidth,
                       sint32 texHeight);
...
/** User type (used with expression system of the interface, see
 * interface_expr.h, that contains a pointer to a CDBCtrlSheet
 */
struct CDBCtrlSheetPtrUserType : public CInterfaceExprUserType {
  ...
  virtual CInterfaceExprUserType *clone() const {
    return new CDBCtrlSheetPtrUserType(*this);
  }
};
