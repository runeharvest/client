...
CChatInputFilter::~CChatInputFilter() {
  // Destruct
  for (std::vector<CChatWindow *>::iterator it = _ListeningWindows.begin();
       it != _ListeningWindows.end(); ++it) {
...
void CChatInputFilter::removeListeningPeopleList(CPeopleList *pl) {
  std::vector<CPeopleList *>::iterator it =
      std::find(_ListeningPeopleList.begin(), _ListeningPeopleList.end(), pl);
...
bool CChatInputFilter::isListeningPeopleList(CPeopleList *pl) const {
  std::vector<CPeopleList *>::const_iterator it =
      std::find(_ListeningPeopleList.begin(), _ListeningPeopleList.end(), pl);
...
void CChatInputFilter::removeListeningWindow(CChatWindow *w) {
  std::vector<CChatWindow *>::iterator it =
      std::find(_ListeningWindows.begin(), _ListeningWindows.end(), w);
...
bool CChatInputFilter::isListeningWindow(CChatWindow *w) const {
  std::vector<CChatWindow *>::const_iterator it =
      std::find(_ListeningWindows.begin(), _ListeningWindows.end(), w);
...
void CChatInputFilter::chatWindowRemoved(CChatWindow *cw) {
  std::vector<CChatWindow *>::iterator it =
      std::find(_ListeningWindows.begin(), _ListeningWindows.end(), cw);
  if (it != _ListeningWindows.end()) {
    _ListeningWindows.erase(it);
  } else {
    nlwarning("<CChatInputFilter::chatWindowRemoved> Not a listening window of "
              "this chat filter");
  }
}

//=============================================================================================================
void CChatInputFilter::displayMessage(const string &msg, NLMISC::CRGBA col,
                                      uint numBlinks /*=0*/,
                                      bool *windowVisible) {
  ...
  for (it = _ListeningWindows.begin(); it != _ListeningWindows.end(); ++it) {
    (*it)->displayMessage(msg, col, FilterType, DynamicChatDbIndex,
                          windowVisibleTmp ? 0 : numBlinks, NULL);
  }
  if (windowVisible)
    *windowVisible = windowVisibleTmp;
}

//=============================================================================================================
void CChatInputFilter::displayTellMessage(
    /*TDataSetIndex &receiverIndex, */ const string &msg, const string &sender,
    NLMISC::CRGBA col, uint numBlinks /*=0*/, bool *windowVisible /*=NULL*/) {
  string senderLwr = NLMISC::toLower(sender);

  // look in people lists
  std::vector<CPeopleList *>::iterator peopleListIt;

  for (peopleListIt = _ListeningPeopleList.begin();
       peopleListIt != _ListeningPeopleList.end(); ++peopleListIt) {
  ...
  // People not found in team or friend list so create/search among free teller
  // (main_chat)

  // If at least one window is visible, no need to make the window blink.
  std::vector<CChatWindow *>::iterator peopleIt;
  for (peopleIt = _ListeningWindows.begin();
       peopleIt != _ListeningWindows.end(); ++peopleIt) {
...
//=============================================================================================================
CChatTargetFilter::~CChatTargetFilter() { reset(); }
...
void CChatTargetFilter::msgEntered(const string &msg, CChatWindow *chatWindow) {
  // Common Target case
  if (ClientCfg.Local) {
    chatWindow->displayMessage(msg, CRGBA::White, _TargetGroup,
                               _TargetDynamicChannelDbIndex);
    return;
  }
  ...
  if (_TargetPartyChat && _TargetPartyChat->getListener()) {
    ...
    // chat to a chat group (say, shout, universe) also for team (with special
    // case in setChatMode) this mode is cached so this should be ok
    ChatMngr.setChatMode(_TargetGroup, ChatMngr.getDynamicChannelIdFromDbIndex(
                                           _TargetDynamicChannelDbIndex));
    // send the string
...
//=============================================================================================================
void CChatTargetFilter::setTargetGroup(CChatGroup::TGroupType groupType,
                                       uint32 dynamicChannelDbIndex,
                                       bool allowUniverseWarning) {
  _TargetPlayer.resize(0);
  if (_TargetPartyChat) {
    _TargetPartyChat->removeObserver(this);
    _TargetPartyChat = NULL;
  ...
  if (_Chat) {
    ...
    case CChatGroup::dyn_chat:
      entry += "DYN:" + NLMISC::toString(dynamicChannelDbIndex);
      break;
    case CChatGroup::say:
      entry += "SAY";
      break;
    ...
    case CChatGroup::universe: {
      ...
      if (allowUniverseWarning)
        NLGUI::CDBManager::getInstance()
            ->getDbProp("UI:VARIABLES:UNIVERSE_CHANEL_WARNING_WANTED")
            ->setValue64(1);
    } break;
    ...
    case CChatGroup::system:
      return; // return with no warning
    default:
      nlwarning("unknown CChatGroup type");
      return;
    }
