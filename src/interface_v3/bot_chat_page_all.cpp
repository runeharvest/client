...
void CBotChatPageAll::initAfterConnectionReady() {
  ...
  // After Connection:Ready is sent, and after loadConfig is done, sent to
  // server the botchat filters
  nlassert(ConnectionReadySent && pIM->isConfigLoaded());

  // reset item part and item type filters (yes, this should not be saved in
  // UI:SAVE.....)
...
NLMISC_COMMAND(bcPage, "Go to the given page in the bot chat", "") {
  ...
  case 3:
    ...
    return true;
  case 4:
    CBotChatManager::getInstance()->setCurrPage(BotChatPageAll->News);
    return true;
  case 5:
    CBotChatManager::getInstance()->setCurrPage(BotChatPageAll->MissionEnd);
    return true;
  case 6:
    CBotChatManager::getInstance()->setCurrPage(BotChatPageAll->DynamicMission);
    return true;
  case 7:
