...
public:
  ...
  // Initialize by loading skills (load the skill_tree stuffs) and organize so
  // in the vector skill == index
  void initInGame();
  void uninitInGame();
  ...
  /// return true if 2 skills are on the same Branch (ie compatible). NB: return
  ...
  /// Get the SkillValue in the database (BUFFED one). Get the max of the skill
  /// and its parent
  uint32 getSkillValueMaxBranch(SKILLS::ESkills eSkill);

  /// Get the SkillValue in the database (unBUFFED one). Get the max of the
  /// skill and its parent
  uint32 getBaseSkillValueMaxBranch(SKILLS::ESkills eSkill);

  /// Get the SkillValue in the database (BUFFED one). Get the max of the skill
  /// and its children NB: warning: slow since recursive
  ...
  /// Get the BaseSkillValue in the database (unBUFFED one). Get the max of the
  /// skill and its children NB: fast O(1) since BaseSkillValue
  ...
  void tryToUnblockTitleFromRingRatings(uint32 authorRating, uint32 amRating,
  ...
  bool isTitleUnblocked(CHARACTER_TITLE::ECharacterTitle ct) {
    return _TitlesUnblocked[ct].Unblocked;
  }

  /// Is the title reserved (can be unblocked only by a server message)
  bool isTitleReserved(CHARACTER_TITLE::ECharacterTitle ct) {
    return _UnblockTitle->TitlesUnblock[ct].Reserved;
  }
