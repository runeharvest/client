...
// ***************************************************************************
void CEncyclopediaManager::updateThema(uint32 nAlbumName,
                                       const CEncyMsgThema &t) {
  ...
  if (pT == NULL) {
    ...
    // If we are not initializing so we must open the encyclopedia on the new
    // thema
    if (!_Initializing) {
      _AlbumNameSelected = nAlbumName;
      _ThemaNameSelected = t.Name;
      CInterfaceManager *pIM = CInterfaceManager::getInstance();
      CInterfaceElement *pContainer = dynamic_cast<CInterfaceElement *>(
...
void CEncyclopediaManager::rebuildAlbumList() {
  ...
  CGroupTree *pTree = dynamic_cast<CGroupTree *>(
...
void CEncyclopediaManager::rebuildAlbumPage(uint32 albumName) {
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("UI:VARIABLES:ENCY:ALBUMBRICK:SHEET")
      ->setValue32(pAlbum->RewardBrick);
  CViewText *pRBVT =
      dynamic_cast<CViewText *>(CWidgetManager::getInstance()->getElementFromId(
          PAGE_ENCY_ALBUM ":reward:desc"));
  if (pRBVT != NULL) {
    STRING_MANAGER::CStringManagerClient *pSMC =
        STRING_MANAGER::CStringManagerClient::instance();
    CUtfStringView desc(
        pSMC->getSBrickLocalizedDescription(CSheetId(pAlbum->RewardBrick)));
...
void CEncyclopediaManager::rebuildThemaPage(uint32 themaName) {
  ...
  // Setup brick reward
  NLGUI::CDBManager::getInstance()
      ->getDbProp("UI:VARIABLES:ENCY:REWARDBRICK:SHEET")
      ->setValue32(pThema->RewardSheet);
  CViewText *pRBVT =
      dynamic_cast<CViewText *>(CWidgetManager::getInstance()->getElementFromId(
          PAGE_ENCY_THEMA ":reward:desc"));
  nlassert(pRBVT != NULL);
  STRING_MANAGER::CStringManagerClient *pSMC =
      STRING_MANAGER::CStringManagerClient::instance();
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("UI:VARIABLES:ENCY:STEPS")
      ->setValue32(nNbSteps);
  ...
  for (i = 0; i < pThema->NbTask; ++i)
    if (pThema->getTaskState((uint8)i) == 2) // 2 == finished
      ++nNbTaskDone;
  NLGUI::CDBManager::getInstance()
      ->getDbProp("UI:VARIABLES:ENCY:DONE")
      ->setValue32(nNbTaskDone);
  ...
  for (i = 0; i < pThema->NbTask; ++i) {
    ...
    CViewTextID *pText = dynamic_cast<CViewTextID *>(
        CWidgetManager::getInstance()->getElementFromId(sTmp + ":desc"));
    nlassert(pText != NULL);
    pText->setTextId(pThema->TaskName[i]);

    // setup task NPC name
    CStringPostProcessNPCRemoveTitle *pSPPRT =
    ...
    CViewBitmap *pBitmap = dynamic_cast<CViewBitmap *>(
        CWidgetManager::getInstance()->getElementFromId(sTmp + ":done"));
...
// ***************************************************************************
// For the group tree we have to check the incoming phrase because no view id
// stored
