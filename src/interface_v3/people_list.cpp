...
CPeopleList::CPeopleList()
    : _ChatWindow(NULL), _ContactType(CPeopleListDesc::Unknown),
      _CurrPeopleID(0), _Savable(false)
...
//==================================================================
bool CPeopleList::create(const CPeopleListDesc &desc,
                         const CChatWindowDesc *chat /* = NULL*/) {
  reset();
  // get father group
  ...
  if (!desc.FatherContainer.empty()) {
    fatherContainer = dynamic_cast<CGroupContainer *>(
        CWidgetManager::getInstance()->getElementFromId(desc.FatherContainer));
    if (!fatherContainer) {
  ...
  CInterfaceGroup *mainIg =
      CWidgetManager::getInstance()->getParser()->createGroupInstance(
          desc.BaseContainerTemplateName, baseId, baseContainerParams);
  ...
  if (chat) {
    ...
    if (!_ChatWindow) {
      ...
      _BaseContainer->attachContainer(_ChatWindow->getContainer(),
                                      desc.InsertPosition);
    }
  }
...
sint CPeopleList::getIndexFromContainerID(const std::string &id) const {
  for (uint k = 0; k < _Peoples.size(); ++k) {
    if (_Peoples[k].Container->getId() == id)
      return k;
  }
  return -1;
}
...
bool CPeopleList::sortExByName(const CPeople &a, const CPeople &b) {
  return NLMISC::compareCaseInsensitive(a.getName(), b.getName()) <
         0; // FIXME: Locale-dependent sort
}
...
bool CPeopleList::isPeopleWindowVisible(uint index) const
{
        ...
        if (_Peoples[index].Container->isOpen())
        {
                ...
                do
                {
                        ...
                        {
                                if (!static_cast<CGroupContainer
*>(ig)->isOpen()) break;
                        }
...
sint CPeopleList::addPeople(const string &name, uint teamMateIndex /*= 0*/) {
  ...
  if (getIndexFromName(name) != -1) {
    nlwarning("<CPeopleList::addPeople> people %s inserted twice.",
              name.c_str());
  }
  ...
  properties.push_back(
      make_pair(string("id"), _ContainerID + "_" + toString(_CurrPeopleID)));
  properties.push_back(make_pair(string("title"), std::string("")));

  if (_ContactType == CPeopleListDesc::Team) {
    properties.push_back(
        make_pair(string("team_mate_index"), toString(teamMateIndex)));
  }

  std::string templateName;
  switch (_ContactType) {
  case CPeopleListDesc::Team:
    templateName = "mate_id";
    break;
  case CPeopleListDesc::Contact:
    templateName = "contact_id_friend";
    break;
  case CPeopleListDesc::Ignore:
    templateName = "contact_id_ignore";
    break;
  default:
  ...
  CInterfaceGroup *group =
      CWidgetManager::getInstance()->getParser()->createGroupInstance(
          templateName, "ui:interface", properties, false);
  if (!group)
    return -1;
  CGroupContainer *gc = dynamic_cast<CGroupContainer *>(group);
  if (!gc) {
    delete group;
    nlwarning("<CPeopleList::addPeople> group is not a container.",
              name.c_str());
    return -1;
  }
...
//==================================================================
void CPeopleList::displayLocalPlayerTell(const string &receiver, uint index,
                                         const string &msg,
                                         uint numBlinks /*=0*/) {
  if (_ContactType == CPeopleListDesc::Ignore) {
    nlwarning("<CPeopleList::displayLocalPlayerTell> bad type, can't display "
              "message.");
    return;
  }
  if (index >= _Peoples.size()) {
  ...
  string csr =
      CHARACTER_TITLE::isCsrTitle(UserEntity->getTitleRaw()) ? "(CSR) " : "";
...
//==================================================================
void CPeopleList::displayMessage(uint index, const string &msg,
                                 NLMISC::CRGBA col,
                                 uint /* numBlinks */ /*= 0*/) {
  if (_ContactType == CPeopleListDesc::Ignore) {
...
void CPeopleList::reset() {
  ...
  if (_BaseContainer) {
    ...
    CGroupContainer *father = static_cast<CGroupContainer *>(
        _BaseContainer->getParent()->getEnclosingContainer());
    if (father) {
...
void CPeopleList::setPeopleMenuEx(
    const std::string &offlineUnblockedMenuName,
...
void CPeopleList::updatePeopleMenu(uint index) {
  ...
  if (_Peoples[index].Container->getHeaderOpened()) {
    _Peoples[index].Container->getHeaderOpened()->setRightClickHandler(
        "active_menu");
    _Peoples[index].Container->getHeaderOpened()->setRightClickHandlerParams(
        "menu=" + *peopleMenu);
  }
  if (_Peoples[index].Container->getHeaderClosed()) {
    _Peoples[index].Container->getHeaderClosed()->setRightClickHandler(
        "active_menu");
    _Peoples[index].Container->getHeaderClosed()->setRightClickHandlerParams(
        "menu=" + *peopleMenu);
  }
}
...
void CPeopleList::setMenu(const std::string &menuName) {
  if (_BaseContainer) {
    if (_BaseContainer->getHeaderClosed()) {
      ...
      _BaseContainer->getHeaderClosed()->setRightClickHandlerParams("menu=" +
                                                                    menuName);
    }
    if (_BaseContainer->getHeaderOpened()) {
      _BaseContainer->getHeaderOpened()->setRightClickHandler("active_menu");
      _BaseContainer->getHeaderOpened()->setRightClickHandlerParams("menu=" +
                                                                    menuName);
    }
  }
  if (_ChatWindow) {
    if (_ChatWindow->getContainer()->getHeaderClosed()) {
      _ChatWindow->getContainer()->getHeaderClosed()->setRightClickHandler(
          "active_menu");
      _ChatWindow->getContainer()
          ->getHeaderClosed()
          ->setRightClickHandlerParams("menu=" + menuName);
    }
    if (_ChatWindow->getContainer()->getHeaderOpened()) {
      _ChatWindow->getContainer()->getHeaderOpened()->setRightClickHandler(
          "active_menu");
      _ChatWindow->getContainer()
          ->getHeaderOpened()
          ->setRightClickHandlerParams("menu=" + menuName);
...
//==================================================================
void CPeopleList::childrenMoved(uint srcIndex, uint destIndex,
                                CGroupContainer * /* children */) {
...
void CPeopleList::setOnline(uint index, TCharConnectionState online) {
  ...
  if (hc != NULL) {
    CViewBitmap *onlineView =
        dynamic_cast<CViewBitmap *>(hc->getView("online"));
    if (onlineView != NULL) {
...
void CPeopleList::openCloseChat(sint index, bool bOpen) {
  ...
  if (_ContactType == CPeopleListDesc::Team)
    _Peoples[index].Chat = pCGW->createFreeTeller(_Peoples[index].getName(),
  ...
  void execute(CCtrlBase *pCaller, const std::string & /* sParams */) {
    ...
    if (text[0] == '/') {
      CChatWindow::_ChatWindowLaunchingCommand =
          NULL; // no CChatWindow instance there ..
      std::string str = text.substr(1);
      NLMISC::ICommand::execute(str, g_log);
      pEB->setInputString(std::string());
      return;
    }
    // Well, we could have used CChatWindow class to handle this, but
    // CPeopleList was written earlier, so for now it is simpler to keep it as
    // it and to just use this action handler to manage user input.
    if (!pCaller || !pCaller->getParent())
      return;
    CGroupContainer *gc = static_cast<CGroupContainer *>(
        pCaller->getParent()->getEnclosingContainer());
    ...
    if (gc) {
      ...
      if (str != ui_interface_free_chat) {
        string str2 =
            gc->getId().substr(gc->getId().rfind('_') + 1, gc->getId().size());
        ...
        uint index;
        if (PeopleInterraction.getPeopleFromContainerID(str, peopleList,
                                                        index)) {
        ...
        string csr(CHARACTER_TITLE::isCsrTitle(UserEntity->getTitleRaw())
                       ? "(CSR) "
                       : "");
        ...
        final += text;
        pWin->displayTellMessage(final, prop.getRGBA(),
                                 pWin->getFreeTellerName(str));

        string s = CI18N::get("youTellPlayer");
        strFindReplace(s, "%name", pWin->getFreeTellerName(str));
        strFindReplace(final, CI18N::get("youTell"), s);
        CInterfaceManager::getInstance()->log(
            final, CChatGroup::groupTypeToString(CChatGroup::tell));
      }
    }
  }
};
