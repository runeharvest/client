...
// ***************************************************************************
const std::string PhraseMemoryViewNextAction =
    "ui:interface:gestionsets:shortcuts:view_next_action";
const std::string PhraseMemoryViewCycleAction =
...
const std::string PhraseMemoryAltCtrlBase =
...
// ***************************************************************************
CSPhraseManager::~CSPhraseManager() { reset(); }
...
void CSPhraseManager::initInGame() {
  ...
  for (i = 0; i < PHRASE_MAX_BOOK_SLOT; i++) {
    CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
        PHRASE_DB_BOOK + ":" + toString(i) + ":PHRASE");
    node->setValue32(0);
  ...
  for (i = 0; i < PHRASE_MAX_MEMORY_SLOT; i++) {
    CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
        PHRASE_DB_MEMORY + ":" + toString(i) + ":PHRASE");
    node->setValue32(0);
    _MemoryDbLeaves[i] = node;
    CCDBNodeLeaf *node_alt = NLGUI::CDBManager::getInstance()->getDbProp(
        PHRASE_DB_MEMORY_ALT + ":" + toString(i) + ":PHRASE");
    node_alt->setValue32(0);
    _MemoryAltDbLeaves[i] = node_alt;
  }

  // Progression Db leaves
  nlctassert(NumProgressType ==
             sizeof(PHRASE_DB_PROGRESSION) / sizeof(PHRASE_DB_PROGRESSION[0]));
  ...
  for (i = 0; i < PHRASE_MAX_PROGRESSION_SLOT; i++) {
    for (uint j = 0; j < NumProgressType; j++) {
      // SHEET
      CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
          PHRASE_DB_PROGRESSION[j] + ":" + toString(i) + ":SHEET");
      node->setValue32(0);
      _ProgressionDbSheets[j][i] = node;
      // LEVEL
      node = NLGUI::CDBManager::getInstance()->getDbProp(
          PHRASE_DB_PROGRESSION[j] + ":" + toString(i) + ":LEVEL");
      node->setValue32(0);
      _ProgressionDbLevels[j][i] = node;
      // LOCKED
      node = NLGUI::CDBManager::getInstance()->getDbProp(
          PHRASE_DB_PROGRESSION[j] + ":" + toString(i) + ":LOCKED");
      node->setValue32(0);
  ...
  // init the UI Next Execute slot
  {
    ...
    node = NLGUI::CDBManager::getInstance()->getDbProp(
        PHRASE_DB_EXECUTE_NEXT_IS_CYCLIC);
    node->setValue32(0);
  ...
  for (i = 0; i < PHRASE_MAX_BOTCHAT_SLOT; i++) {
    CCDBNodeLeaf *nodeSheet = NLGUI::CDBManager::getInstance()->getDbProp(
        PHRASE_DB_BOTCHAT + ":" + toString(i) + ":SHEET");
    CCDBNodeLeaf *nodePrice = NLGUI::CDBManager::getInstance()->getDbProp(
        PHRASE_DB_BOTCHAT + ":" + toString(i) + ":PRICE");
  ...
  for (CSheetManager::TEntitySheetMap::const_iterator it = sm.begin();
    ...
    if (it->second.EntitySheet &&
        it->second.EntitySheet->Type == CEntitySheet::SPHRASE) {
      const_cast<CSPhraseManager *>(this)->buildPhraseFromSheet(
          tmpPhrase, it->first.asInt());
...
// ***************************************************************************
void CSPhraseManager::setPhraseInternal(uint32 slot, const CSPhraseCom &phrase,
                                        bool lock, bool updateDB) {
...
// ***************************************************************************
void CSPhraseManager::setPhrase(uint32 slot, const CSPhraseCom &phrase,
                                bool lock) {
...
// ***************************************************************************
void CSPhraseManager::setPhraseNoUpdateDB(uint32 slot,
                                          const CSPhraseCom &phrase) {
...
void CSPhraseManager::erasePhrase(uint32 slot) {
  ...
  if (slot == _CurrentExecutePhraseIdCycle ||
      slot == _CurrentExecutePhraseIdNext) {
  ...
  if (confirm) {
    ...
    if (!phrase.empty()) {
      ...
      if (rootBrick) {
        ...
        for (uint dstMemoryLine = 0; dstMemoryLine < MEM_SET_TYPES::NumMemories;
             dstMemoryLine++) {
        ...
        // Ugly: must update ALL the memoryBar in case of dstMemoryLine is the
        // selected one
        updateAllMemoryCtrlState();
      }
    }
...
void CSPhraseManager::forgetPhrase(uint32 memoryLine, uint32 memorySlot) {
  ...
  if ((sint32)memoryLine == _SelectedMemoryDB) {
    ...
    if (_CurrentExecuteSlotNext == (sint32)memorySlot ||
        _CurrentExecuteSlotCycle == (sint32)memorySlot)
      updateExecutionDisplay();
  }
}

// ***************************************************************************
uint32 CSPhraseManager::getMemorizedPhrase(uint32 memoryLine,
                                           uint32 memorySlot) const {
  if (memoryLine >= _Memories.size())
    return 0;
  if (memorySlot >= PHRASE_MAX_MEMORY_SLOT)
    return 0;
...
// ***************************************************************************
void CSPhraseManager::memorizePhrase(uint32 memoryLine, uint32 memorySlot,
                                     uint32 slot) {
  if (memorySlot >= PHRASE_MAX_MEMORY_SLOT)
    return;
  if (slot >= _PhraseClient.size())
  ...
  if ((sint32)memoryLine == _SelectedMemoryDB) {
    ...
    if (_CurrentExecutePhraseIdNext == slot ||
        _CurrentExecutePhraseIdCycle == slot)
      updateExecutionDisplay();
  }
}
...
void CSPhraseManager::selectMemoryLineDB(sint32 memoryLine) {
  ...
  if (_SelectedMemoryDB != memoryLine) {
    ...
    // since memory selection changes then must update all the DB and the Ctrl
    // states
    updateMemoryDBAll();
    updateAllMemoryCtrlState();
...
void CSPhraseManager::updateMemoryDBAll() {
  ...
  if (_SelectedMemoryDB == -1 ||
      _SelectedMemoryDB >= (sint32)_Memories.size()) {
    for (uint i = 0; i < PHRASE_MAX_MEMORY_SLOT; i++) {
  ...
  if (_SelectedMemoryDBalt == -1 ||
      _SelectedMemoryDBalt >= (sint32)_Memories.size()) {
    for (uint i = 0; i < PHRASE_MAX_MEMORY_SLOT; i++) {
...
bool CSPhraseManager::isPhraseCastable(uint32 sheetId) const {
  return isPhraseCastable(
      dynamic_cast<CSPhraseSheet *>(SheetMngr.get(NLMISC::CSheetId(sheetId))));
}
...
bool CSPhraseManager::isPhraseCharacBuying(uint32 sheetId) const {
  return isPhraseCharacBuying(
      dynamic_cast<CSPhraseSheet *>(SheetMngr.get(NLMISC::CSheetId(sheetId))));
}

// ***************************************************************************
bool CSPhraseManager::isPhraseCharacBuying(
    class CSPhraseSheet *phraseSheet) const {
  CSBrickManager *pBM = CSBrickManager::getInstance();
...
// ***************************************************************************
void CSPhraseManager::setBookFilter(BRICK_TYPE::EBrickType brickTypeFilter,
...
bool CSPhraseManager::matchBookSkillFilter(const CSPhraseCom &phrase) const {
  ...
  // must have a valid rootBrick. NB: it is an error, but decide to show it
  // (return true)
  if (phrase.empty())
    return true;
  CSBrickSheet *rootBrick = pBM->getBrick(phrase.Bricks[0]);
  ...
  if (_BookBrickTypeFilter != BRICK_TYPE::UNKNOWN) {
    if (BRICK_FAMILIES::brickType(rootBrick->BrickFamily) ==
  ...
  // else filter by Skill
    ...
    // Special for combat
      ...
      // For combat, it is best to show "All Actions that can be used with the
      // weapon associated to the skill"
      if (skillCompatibleWithCombatPhrase(_BookSkillFitler, phrase.Bricks))
        return true;
    }
    // suppose same for magic, craft, and forage
    else {
      // run all the phrase Bricks, if only one match (eg important for magic
      // multi spells), it is ok!
      for (uint i = 0; i < phrase.Bricks.size(); i++) {
        CSBrickSheet *brick = pBM->getBrick(phrase.Bricks[i]);
        if (brick) {
          // if the filter is an ancestor, ok! (NB: if brick skill unknown then
          // it will fail!)
...
void CSPhraseManager::updateBookDB() {
  ...
  while (i < numBookFill) {
    ...
    if (it->first < BookStartSlot || _PhraseClient[it->first].Lock ||
...
// ***************************************************************************
void CSPhraseManager::buildPhraseFromSheet(CSPhraseCom &phrase,
                                           sint32 sheetId) {
  CSPhraseSheet *phraseSheet =
      dynamic_cast<CSPhraseSheet *>(SheetMngr.get(CSheetId(sheetId)));
  if (phraseSheet) {
    // get localized Name
    phrase.Name.fromUtf8(
...
bool CSPhraseManager::isPhraseNextExecuteCounterSync() const {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  sint32 srvVal = NLGUI::CDBManager::getInstance()
...
// ***************************************************************************
void CSPhraseManager::sendMemorizeToServer(uint32 memoryLine, uint32 memorySlot,
                                           uint32 phraseId) {
  // Local simulation
  if (ClientCfg.Local) {
  ...
  // Send the memorize to server
    ...
    if (GenericMsgHeaderMngr.pushNameToStream(sMsg, out)) {
      ...
      out.serial(phrase);
      NetMngr.push(out);
      // nlinfo("impulseCallBack : %s %d %d %d (phrase) sent", sMsg.c_str(),
      // memoryId, slotId, pid);
    } else
...
// ***************************************************************************
void CSPhraseManager::sendForgetToServer(uint32 memoryLine,
                                         uint32 memoryIndex) {
  // Local simulation
  if (ClientCfg.Local) {
  }
  // Send the forget to server.
    ...
    if (GenericMsgHeaderMngr.pushNameToStream(sMsg, out)) {
      ...
      out.serial(slotId);
      NetMngr.push(out);
      // nlinfo("impulseCallBack : %s %d %d sent", sMsg.c_str(), memoryId,
      // slotId);
    } else
      nlwarning("impulseCallBack : unknown message name : '%s'.", sMsg.c_str());
  }
...
bool CSPhraseManager::isPhraseKnown(const CSPhraseCom &phrase) const {
  for (TPhraseMap::const_iterator it = _PhraseMap.begin();
       it != _PhraseMap.end(); ++it) {
    if (it->second == phrase)
      return true;
  }
  return false;
...
string CSPhraseManager::formatBonusMalus(sint32 base, sint32 mod) {
  ...
  {
    str = "@{0F0F}" + toString(base + mod) + "@{FFFF}(" + toString(base) +
          "@{0F0F} + " + toString(mod) + "@{FFFF})";
  } else {
    str = "@{E42F}" + toString(base + mod) + "@{FFFF}(" + toString(base) +
          "@{E42F} - " + toString(mod) + "@{FFFF})";
  }
  return str;
}

// ***************************************************************************
void CSPhraseManager::buildPhraseDesc(string &text, const CSPhraseCom &phrase,
  ...
  if (castable) {
    ...
    if (rootBrick && rootBrick->isCombat()) {
      getCombatWeaponRestriction(weaponRestriction, phrase, usableWithMelee,
                                 usableWithRange);
      ...
      for (uint i = 0; i < RESISTANCE_TYPE::NB_RESISTANCE_TYPE; i++) {
        if (resistMagic[i]) {
          if (first)
            ...
            resList += ", ";
          resList += CI18N::get("rs" + RESISTANCE_TYPE::toString((
    ...
    CCDBNodeLeaf *actMalus =
        _TotalMalusEquipLeaf
            ? &*_TotalMalusEquipLeaf
            : &*(_TotalMalusEquipLeaf =
                     NLGUI::CDBManager::getInstance()->getDbProp(
                         "UI:VARIABLES:TOTAL_MALUS_EQUIP", false));

    // root brick must not be Power or aura, because Action malus don't apply to
    // them (ie leave 0 ActionMalus for Aura or Powers
    ...
    if (rootBrick->isCombat()) {
      // if phrase can be used with in melee and range we choose which one to
      // display according to hand weapon family
      ...
      nodeSM = NLGUI::CDBManager::getInstance()->getDbProp(
          "SERVER:CHARACTER_INFO:SUCCESS_MODIFIER:MAGIC", false);
    }
    if (nodeSM) {
    ...
    if (rootBrick->isForageExtraction()) {
      ...
      nodeSM = NLGUI::CDBManager::getInstance()->getDbProp(
          "SERVER:CHARACTER_INFO:SUCCESS_MODIFIER:ECO:0:FORAGE", false);
      if (nodeSM) {
      ...
      nodeSM = NLGUI::CDBManager::getInstance()->getDbProp(
          "SERVER:CHARACTER_INFO:SUCCESS_MODIFIER:ECO:1:FORAGE", false);
      if (nodeSM)
        successModifier = nodeSM->getValue32();
      ...
      nodeSM = NLGUI::CDBManager::getInstance()->getDbProp(
          "SERVER:CHARACTER_INFO:SUCCESS_MODIFIER:ECO:2:FORAGE", false);
      if (nodeSM)
        successModifier = nodeSM->getValue32();
      if (successModifier == 0)
        successModifier = commonModifier;
      ...
      nodeSM = NLGUI::CDBManager::getInstance()->getDbProp(
          "SERVER:CHARACTER_INFO:SUCCESS_MODIFIER:ECO:3:FORAGE", false);
      if (nodeSM)
        successModifier = nodeSM->getValue32();
      if (successModifier == 0)
        successModifier = commonModifier;
      ...
      nodeSM = NLGUI::CDBManager::getInstance()->getDbProp(
          "SERVER:CHARACTER_INFO:SUCCESS_MODIFIER:ECO:4:FORAGE", false);
      if (nodeSM)
        successModifier = nodeSM->getValue32();
      if (successModifier == 0)
        successModifier = commonModifier;
      ...
      nodeSM = NLGUI::CDBManager::getInstance()->getDbProp(
          "SERVER:CHARACTER_INFO:SUCCESS_MODIFIER:ECO:6:FORAGE", false);
      if (nodeSM)
        successModifier = nodeSM->getValue32();
      if (successModifier == 0)
        successModifier = commonModifier;
  ...
  if (phraseSheetId) {
    // get the text
    string desc =
        STRING_MANAGER::CStringManagerClient::getSPhraseLocalizedDescription(
  ...
  if (phraseSheetId && wantRequirement) {
    ...
    strFindReplace(reqText, "%sp",
                   toString(getCurrentPhraseSheetPrice(phraseSheetId)));
    strFindReplace(reqText, "%basesp",
                   toString(getBasePhraseSheetPrice(phraseSheetId)));

    // build the phrase requirement from this sheetId
    TPhraseReqSkillMap::const_iterator it =
        _PhraseReqSkillMap.find(phraseSheetId);
...
// ***************************************************************************
sint CSPhraseManager::getCraftPhraseSuccessRate(const CSPhraseCom &phrase,
...
// ***************************************************************************
sint CSPhraseManager::getForageExtractionPhraseSuccessRate(
    const CSPhraseCom &phrase, SKILLS::ESkills skill) {
  CSkillManager *pSM = CSkillManager::getInstance();
...
// ***************************************************************************
void CSPhraseManager::getPhraseSapCost(const CSPhraseCom &phrase,
                                       uint32 totalActionMalus, sint &cost,
                                       sint &costMalus) {
  CSBrickManager *pBM = CSBrickManager::getInstance();
...
// ***************************************************************************
void CSPhraseManager::getPhraseStaCost(const CSPhraseCom &phrase,
                                       uint32 totalActionMalus, sint &cost,
                                       sint &costMalus) {
  CSBrickManager *pBM = CSBrickManager::getInstance();
...
// ***************************************************************************
void CSPhraseManager::getPhraseFocusCost(const CSPhraseCom &phrase,
                                         uint32 totalActionMalus, sint &cost,
                                         sint &costMalus) {
  CSBrickManager *pBM = CSBrickManager::getInstance();
...
// ***************************************************************************
void CSPhraseManager::getPhraseHpCost(const CSPhraseCom &phrase,
                                      uint32 totalActionMalus, sint &cost,
                                      sint &costMalus) {
  CSBrickManager *pBM = CSBrickManager::getInstance();
...
// ***************************************************************************
void CSPhraseManager::getPhraseCastTime(const CSPhraseCom &phrase,
                                        uint32 totalActionMalus,
                                        float &castTime, float &castTimeMalus) {
  CSBrickManager *pBM = CSBrickManager::getInstance();
  ...
  // for all bricks get the max "min and Max" cast time, and get the sum of time
  // credit
  ...
  if (sabrinaCost)
    castingTimeFactor =
        (float)sumCastTimeCredit / (float)(sabrinaCost * sabrinaRelativeCost);
  else
  ...
  castTime = (float)(maxCastTime - minCastTime) * castingTimeFactor +
             (float)minCastTime;

  // Compute the malus (positive value)
  castTimeMalus = (float)(maxCastTime - minCastTime) * castingTimeFactor *
                  totalActionMalus / 100;
...
// ***************************************************************************
void CSPhraseManager::getPhraseMagicRange(const CSPhraseCom &phrase,
                                          uint32 totalActionMalus, sint &range,
                                          sint &rangeMalus) {
  CSBrickManager *pBM = CSBrickManager::getInstance();
  ...
  // for all bricks get the min "min and Max" range, and get the sum of range
  // credit
  ...
  if (sabrinaCost)
    rangeFactor =
        (float)sumRangeCredit / (float)(sabrinaCost * sabrinaRelativeCost);
  else
  ...
  float fRange =
      (float)(maxRange - minRange) * (1 - rangeFactor) + (float)minRange;

  // Compute the range, with malus
  float fRangeWithMalus = (float)(maxRange - minRange) * (1 - rangeFactor) *
...
// ***************************************************************************
float CSPhraseManager::getPhraseSumBrickProp(const CSPhraseCom &phrase,
                                             uint propId, bool doAbs) {
  CSBrickManager *pBM = CSBrickManager::getInstance();
  ...
  for (uint i = 0; i < phrase.Bricks.size(); i++) {
    ...
    if (brick) {
      ...
      for (uint j = 0; j < brick->Properties.size(); j++) {
        if (brick->Properties[j].PropId == propId) {
        ...
        } else if (propId == CSBrickManager::getInstance()->StaPropId &&
                   brick->Properties[j].PropId ==
                       CSBrickManager::getInstance()->StaWeightFactorId) {
          CInterfaceManager *im = CInterfaceManager::getInstance();
          if (!_ServerUserDefaultWeightHandsLeaf)
            _ServerUserDefaultWeightHandsLeaf =
                NLGUI::CDBManager::getInstance()->getDbProp(
                    "SERVER:USER:DEFAULT_WEIGHT_HANDS");
          uint32 weight =
              (uint32)(&*_ServerUserDefaultWeightHandsLeaf)->getValue32() /
              10; // weight must be in dg here
          CDBCtrlSheet *ctrlSheet = dynamic_cast<CDBCtrlSheet *>(
              CWidgetManager::getInstance()->getElementFromId(
                  "ui:interface:gestionsets:hands:handr"));
          if (ctrlSheet) {
            const CItemSheet *itemSheet = ctrlSheet->asItemSheet();
            if (itemSheet && (itemSheet->Family == ITEMFAMILY::MELEE_WEAPON ||
                              itemSheet->Family == ITEMFAMILY::RANGE_WEAPON)) {
              weight = (uint32)ctrlSheet->getItemWeight();
            }
          }
          ctrlSheet = dynamic_cast<CDBCtrlSheet *>(
              CWidgetManager::getInstance()->getElementFromId(
                  "ui:interface:gestionsets:hands:handl"));
          if (ctrlSheet) {
            const CItemSheet *itemSheet = ctrlSheet->asItemSheet();
          ...
          if (doAbs)
            ...
            sum += brick->Properties[j].Value * weight / DB_WEIGHT_SCALE +
                   brick->Properties[j].Value2;
        }
      }
    }
  }
...
// ***************************************************************************
sint CSPhraseManager::getSuccessRate(TSuccessTable st, sint level,
                                     bool partialSuccess) {
  ...
  if (_SuccessTableSheet[st]) {
    ...
    for (uint i = 0; i < _SuccessTableSheet[st]->SuccessTable.size(); i++) {
      sint levelDiff =
          level - _SuccessTableSheet[st]->SuccessTable[i].RelativeLevel;
      levelDiff = abs(levelDiff);
      if (levelDiff < bestLevelDiff) {
        bestLevelDiff = levelDiff;
        if (partialSuccess)
          bestProba =
              _SuccessTableSheet[st]->SuccessTable[i].PartialSuccessProbability;
        else
          bestProba =
              _SuccessTableSheet[st]->SuccessTable[i].SuccessProbability;
      }
    }
...
void CSPhraseManager::loadSuccessTable() {
  ...
  _SuccessTableSheet[STCombat] = dynamic_cast<CSuccessTableSheet *>(
      SheetMngr.get(CSheetId("fight_phrase.succes_chances_table")));
  _SuccessTableSheet[STOffensiveMagic] = dynamic_cast<CSuccessTableSheet *>(
      SheetMngr.get(CSheetId("offensive_magic.succes_chances_table")));
  _SuccessTableSheet[STCurativeMagic] = dynamic_cast<CSuccessTableSheet *>(
      SheetMngr.get(CSheetId("curative_magic.succes_chances_table")));
  _SuccessTableSheet[STCraft] = dynamic_cast<CSuccessTableSheet *>(
      SheetMngr.get(CSheetId("craft.succes_chances_table")));
  _SuccessTableSheet[STExtract] = dynamic_cast<CSuccessTableSheet *>(
      SheetMngr.get(CSheetId("extracting.succes_chances_table")));
  _SuccessTableSheet[STResistMagic] = dynamic_cast<CSuccessTableSheet *>(
      SheetMngr.get(CSheetId("magic_resist.succes_chances_table")));
  _SuccessTableSheet[STResistMagicLink] = dynamic_cast<CSuccessTableSheet *>(
      SheetMngr.get(CSheetId("magic_resist_link.succes_chances_table")));
  _SuccessTableSheet[STDodgeParry] = dynamic_cast<CSuccessTableSheet *>(
      SheetMngr.get(CSheetId("dodge_parry.succes_chances_table")));
}
...
// ***************************************************************************
sint CSPhraseManager::getPhraseSuccessRate(TSuccessTable st,
                                           const CSPhraseCom &phrase,
                                           sint skillValue, uint minMpLevel) {
  if (phrase.empty())
    return 0;
...
void CSPhraseManager::updateExecutionDisplay() {
  ...
  // **** test if have to draws the icons, and try to recover if sphrase
  // forgeted or removed.

  // do we have to display icons?
  bool displayCycle = _CurrentExecuteSlotCycle >= 0 &&
                      _SelectedMemoryDB == _CurrentExecuteLineCycle;
  bool displayNext = _CurrentExecuteSlotNext >= 0 &&
                     _SelectedMemoryDB == _CurrentExecuteLineNext;

  // if yes, verify that the actions under the icons are relevant
  if (displayCycle) {
    // if the memory slot is different from the action executed
    if (_Memories[_CurrentExecuteLineCycle].Slot[_CurrentExecuteSlotCycle].Id !=
        _CurrentExecutePhraseIdCycle) {
      // fails!
      displayCycle = false;

      // try to find in the action bar, this action (in case of DragAndDrop for
      // instance)
      for (uint i = 0; i < PHRASE_MAX_MEMORY_SLOT; i++) {
        if (_Memories[_CurrentExecuteLineCycle].Slot[i].Id ==
            _CurrentExecutePhraseIdCycle) {
  ...
  if (displayNext) {
    // if the memory slot is different from the action executed
    if (_Memories[_CurrentExecuteLineNext].Slot[_CurrentExecuteSlotNext].Id !=
      ...
      // instance)
      for (uint i = 0; i < PHRASE_MAX_MEMORY_SLOT; i++) {
        if (_Memories[_CurrentExecuteLineNext].Slot[i].Id ==
            _CurrentExecutePhraseIdNext) {
          // change the slot number
  ...
  CInterfaceElement *viewCycle =
      CWidgetManager::getInstance()->getElementFromId(
          PhraseMemoryViewCycleAction);
  if (viewCycle) {
    CInterfaceElement *ctrl = NULL;
    if (displayCycle)
      ctrl = CWidgetManager::getInstance()->getElementFromId(
          PhraseMemoryViewSlotBase + toString(_CurrentExecuteSlotCycle));
  ...
  CInterfaceElement *viewNext = CWidgetManager::getInstance()->getElementFromId(
      PhraseMemoryViewNextAction);
  if (viewNext) {
    CInterfaceElement *ctrl = NULL;
    if (displayNext)
      ctrl = CWidgetManager::getInstance()->getElementFromId(
          PhraseMemoryViewSlotBase + toString(_CurrentExecuteSlotNext));
    if (displayNext && ctrl) {
  ...
  // **** Display Next Action on Slot in PLAYER window , whatever the selected
  // memory line is
  if (_CurrentExecutePhraseIdCycle > 0 || _CurrentExecutePhraseIdNext > 0) {
    ...
    _NextExecuteIsCyclicLeaf->setValue32(_CurrentExecutePhraseIdNext == 0 ||
                                         _CurrentExecutePhraseIdNext ==
                                             _CurrentExecutePhraseIdCycle);
  } else {
...
// ***************************************************************************
void CSPhraseManager::clientExecute(uint32 memoryLine, uint32 memorySlot,
                                    bool cyclic) {
  if (cyclic) {
...
// ***************************************************************************
void CSPhraseManager::sendExecuteToServer(uint32 memoryLine, uint32 memorySlot,
                                          bool cyclic) {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  ...
  if (ClientCfg.Local) {
    if (cyclic)
      pIM->displaySystemInfo("PhraseCycleCasted:" + toString(memoryLine) + "_" +
                                 toString(memorySlot),
                             "CHK");
    else
      pIM->displaySystemInfo("PhraseNextCasted:" + toString(memoryLine) + "_" +
                                 toString(memorySlot),
                             "CHK");
...
// ***************************************************************************
void CSPhraseManager::executeCraft(
    uint32 memoryLine, uint32 memorySlot, uint32 planSheetId,
    std::vector<CFaberMsgItem> &mpItemPartList,
    std::vector<CFaberMsgItem> specificItemList) {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();

  // **** Do also a clientExecute to display the correct craft action (NB: not
  // cyclic!)
  ...
  // Local simulation
  if (ClientCfg.Local) {
    pIM->displaySystemInfo("PhraseCraftCasted:" + toString(memoryLine) + "_" +
                               toString(memorySlot),
                           "CHK");
    extern void debugUpdateActionBar();
    debugUpdateActionBar();
...
void CSPhraseManager::executeCristalize(uint32 memoryLine, uint32 memorySlot) {
  ...
  if (ClientCfg.Local) {
    pIM->displaySystemInfo("PhraseCristalize:" + toString(memoryLine) + "_" +
                               toString(memorySlot),
                           "CHK");
    extern void debugUpdateActionBar();
    debugUpdateActionBar();
...
void CSPhraseManager::executeDefaultAttack() {
  // Try to find a "default attack" in the memories, and prefer launch it
  // instead
  ...
  if (findDefaultAttack(memoryLine, memorySlot)) {
    ...
    if (ClientCfg.Local) {
      IngameDbMngr.setProp("Entities:E0:P" +
...
void CSPhraseManager::appendCurrentToAckExecute(bool cyclic) {
  ...
  if (cyclic) {
    ...
    nlinfo("ACK-ADD: CYCLIC. counter=%d. size=%d.", entry.Counter,
           _AckExecuteCycleList.size());
#endif
  } else {
...
#ifdef PHRASE_DEBUG_VERBOSE
    nlinfo("ACK-ADD: NEXT. counter=%d. size=%d.", entry.Counter,
           _AckExecuteNextList.size());
#endif
  }
...
// ***************************************************************************
void CSPhraseManager::receiveAckExecuteFromServer(bool cyclic,
                                                  uint counterValue, bool ok) {
  // get the approriated list
  TAckExecuteList &ackList =
      cyclic ? _AckExecuteCycleList : _AckExecuteNextList;
  ...
  if (ok) {
    ...
    // NB: if not found, noop (NB: possible for instance if msg arrives after DB
    // update, or if order of ACK is not good)
#ifdef PHRASE_DEBUG_VERBOSE
    nlinfo("ACK-RECEIVE-OK: %s. counter=%d. size=%d",
           cyclic ? "CYCLIC" : "NEXT", counterValue, ackList.size());
#endif
  }
  // **** server cancelation
  else {
    ...
    if (found) {
      // find the first element in list (start from end) that is not in KO state
      // (ie WaitAck or OK)
...
#ifdef PHRASE_DEBUG_VERBOSE
    nlinfo("ACK-RECEIVE-KO: %s. counter=%d. size=%d",
           cyclic ? "CYCLIC" : "NEXT", counterValue, ackList.size());
#endif
  }
}
    ...
    if (ClientCfg.Local) {
      ...
      if (T1 > pPM->_PhraseDebugEndNextAction) {
        ...
        NLGUI::CDBManager::getInstance()
            ->getDbProp(PHRASE_DB_COUNTER_NEXT)
            ->setValue32(pPM->_PhraseNextExecuteCounter);
      }
      if (T1 > pPM->_PhraseDebugEndCyclicAction) {
        // copy counter
        NLGUI::CDBManager::getInstance()
            ->getDbProp(PHRASE_DB_COUNTER_CYCLE)
            ->setValue32(pPM->_PhraseCycleExecuteCounter);
      }

      sint64 st = NLGUI::CDBManager::getInstance()
                      ->getDbProp("UI:VARIABLES:CURRENT_SERVER_TICK")
                      ->getValue64();
      sint64 stEnd = NLGUI::CDBManager::getInstance()
                         ->getDbProp("SERVER:USER:ACT_TEND")
                         ->getValue64();
...
// ***************************************************************************
void CSPhraseManager::setEquipInvalidation(sint64 serverTick,
...
void CSPhraseManager::updateAllActionRegen() {
  if (_RegenTickRangeTouched) {
    ...
    // nldebug("***** %d ms for CSPhraseManager::updateAllActionRegen", (int)
    // (1000 * CTime::ticksToSecond(endTime - startTime)));
  }
}
...
CSheetId getRightHandItem() {
  ...
  sint32 itemSlot = NLGUI::CDBManager::getInstance()
                        ->getDbProp("LOCAL:INVENTORY:HAND:0:INDEX_IN_BAG")
                        ->getValue32();
...
uint32 getRightHandEffectiveLevel() {
  ...
  if (itemSlot > 0) {
    CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
        "LOCAL:INVENTORY:BAG:" + toString(itemSlot - 1) + ":QUALITY", false);
    if (node)
      // if the right hand item quality is less than our skill value, take
      // it....
...
static sint getRightHandEnchantValue() {
  ...
  if (itemSlot > 0) {
    CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
        "LOCAL:INVENTORY:BAG:" + toString(itemSlot - 1) + ":ENCHANT", false);
    if (node)
      ret = node->getValue32();
  }
...
void CSPhraseManager::updateMemoryCtrlRegenTickRange(uint memorySlot,
  ...
  if (phraseId) {
    ...
    if ((_RegenPhrases.find(phraseId) != _RegenPhrases.end()) &&
        rtr.isEmpty() && !ctrl->getRegenTickRange().isEmpty()) {
      ctrl->startNotifyAnim(); // anim to state that the phraseis available
                               // again
...
// ***************************************************************************
NLMISC::TGameCycle
CSPhraseManager::getPowerDisableTime(const CSPhraseCom &phrase) const {
  float regenTime = 0;
  if (!phrase.empty()) {
    CSBrickManager *pBM = CSBrickManager::getInstance();
      ...
      if (brick) {
        for (uint prop = 0; prop < brick->Properties.size(); ++prop) {
          std::string::size_type endPos =
              brick->Properties[prop].Text.find(":");
          if (endPos != std::string::npos) {
            std::string propName =
                brick->Properties[prop].Text.substr(0, endPos);
...
// ***************************************************************************
NLMISC::TGameCycle
CSPhraseManager::getRegenTime(const CSPhraseCom &phrase) const {
...
// ***************************************************************************
NLMISC::TGameCycle
CSPhraseManager::getTotalRegenTime(const CSPhraseCom &phrase) const {
  CTickRange tickRange = getRegenTickRange(phrase);
  return (tickRange.EndTick - tickRange.StartTick);
}

// ***************************************************************************
void CSPhraseManager::updateMemoryCtrlState(uint memorySlot, CDBCtrlSheet *ctrl,
                                            SKILLS::ESkills itemSkill) {
  /*
          Update Gray State of Memory Ctrls.
          Can Execute only if skill of the phrase and skill of the selected
     Weapon are compatible. Eg: can execute a "FyrosAxe" phrase on a FyrosAxe
     Weapon (skills on same branch) can execute a "Combat" phrase on a FyrosAxe
     Weapon (skills on same branch) can't execute a "MatisAxe" phrase on a
     FyrosAxe Weapon (skills not on same branch) Additionaly, a Magic Phrase is
     always executable
  */
  ...
  if (ctrl && (ctrl->isSPhraseId() || ctrl->isMacro())) {
    ...
    if (!newIsMacro) {
      ...
      bool valid = false;
      if (isGlobalMemPhrase ||
          pSM->areSkillOnSameBranch(phraseRootSkill, itemSkill))
        valid = true;

      // *** Combat: the item must be compatible with the phrase
      if (valid && isCombatPhrase &&
          !skillCompatibleWithCombatPhrase(itemSkill,
                                           getPhrase(phraseId).Bricks))
        valid = false;

      // *** Power: item enchant.
      if (valid && isSpecialPowerPhrase &&
      ...
      if (valid && itemSkill == SKILLS::SC && isFaberPhrase) {
        ...
        if (!phrase.empty()) {
          ...
          valid = pBM->getSabrinaCom().getPhraseFaberPlanToolType(
                      phrase.Bricks) == cttHand;
        }
      }

      // **** Proc Enchantment: valid only if the item in right hand is
      // enchanted
      ...
      if (valid && isCombatPhrase &&
          !phraseUsableWithEmptyHands(getPhrase(phraseId).Bricks)) {
...
void CSPhraseManager::updateAllMemoryCtrlState() {
  ...
  for (uint i = 0; i < PHRASE_MAX_MEMORY_SLOT; i++) {
    // Get the ctrl
    CDBCtrlSheet *ctrl = dynamic_cast<CDBCtrlSheet *>(
    ...
    CDBCtrlSheet *ctrlAlt = dynamic_cast<CDBCtrlSheet *>(
        CWidgetManager::getInstance()->getElementFromId(
            PhraseMemoryAltCtrlBase + toString(i)));
    if (ctrlAlt)
      updateMemoryCtrlState(i, ctrlAlt, itemSkill);
  }
  TTicks endTime = CTime::getPerformanceTime();
  // nldebug("***** %d ms for CSPhraseManager::updateAllMemoryCtrlState", (int)
  // (1000 * CTime::ticksToSecond(endTime - startTime)));
}
...
CDBCtrlSheet *CSPhraseManager::getMemorySlotCtrl(uint memorySlot) {
  ...
  // Get the ctrl
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  return dynamic_cast<CDBCtrlSheet *>(
      CWidgetManager::getInstance()->getElementFromId(PhraseMemoryCtrlBase +
                                                      toString(memorySlot)));
}
...
// ***************************************************************************
SKILLS::ESkills CSPhraseManager::getPhraseRootSkill(
    const std::vector<NLMISC::CSheetId> &phraseBricks) const {
...
// ***************************************************************************
bool CSPhraseManager::skillCompatibleWithCombatPhrase(
    SKILLS::ESkills compareSkill,
    const std::vector<NLMISC::CSheetId> &phraseBricks) const {
  ...
  std::list<CReqSkillFormula::CSkillValueAnd>::iterator it(
      phraseFormula.OrSkills.begin()),
      end(phraseFormula.OrSkills.end());
  for (; it != end; it++) {
    CReqSkillFormula::CSkillValueAnd skillAnd = *it;
    // if the and is not a single value, then no chance to match (eg: SFR&SFM
    // won't match any compareSkill)
...
// ***************************************************************************
bool CSPhraseManager::skillCompatibleWithSpecialPowerPhrase(
    SKILLS::ESkills compareSkill,
    const std::vector<NLMISC::CSheetId> &phraseBricks) const {
  CSkillManager *pSM = CSkillManager::getInstance();

  bool hasEnchantWeapon = false;
  vector<NLMISC::CSheetId>::const_iterator itBrick,
      itBrickEnd = phraseBricks.end();
  ...
  if (hasEnchantWeapon) {
    ...
    if (pSM->isSkillAncestor(SKILLS::SFM, compareSkill) &&
        compareSkill != SKILLS::SFMCAH)
      return true;
    else
      return false;
  }
...
// ***************************************************************************
bool CSPhraseManager::phraseUsableWithEmptyHands(
    const std::vector<NLMISC::CSheetId> &phraseBricks) const {
  vector<NLMISC::CSheetId>::const_iterator itBrick,
      itBrickEnd = phraseBricks.end();
  for (itBrick = phraseBricks.begin(); itBrick != itBrickEnd; ++itBrick) {
...
// ***************************************************************************
void CSPhraseManager::memorizeMacro(uint32 memoryLine, uint32 memorySlot,
                                    uint32 macroId) {
  if (memorySlot >= PHRASE_MAX_MEMORY_SLOT)
    return;
...
// ***************************************************************************
uint32 CSPhraseManager::getMemorizedMacro(uint32 memoryLine,
                                          uint32 memorySlot) const {
  if (memoryLine >= _Memories.size())
    return 0;
  if (memorySlot >= PHRASE_MAX_MEMORY_SLOT)
...
// ***************************************************************************
bool CSPhraseManager::isMemorizedMacro(uint32 memoryLine,
                                       uint32 memorySlot) const {
  if (memoryLine >= _Memories.size())
    return 0;
  if (memorySlot >= PHRASE_MAX_MEMORY_SLOT)
...
uint32 CSPhraseManager::getCurrentPhraseSheetPrice(uint32 phraseSheetId) {
  ...
  CSPhraseSheet *phraseSheet =
      dynamic_cast<CSPhraseSheet *>(SheetMngr.get(CSheetId(phraseSheetId)));
  if (phraseSheet) {
...
void CSPhraseManager::updateBotChatPhrasePrice(uint start, uint end) {
  ...
  for (uint i = start; i < end; i++) {
    if (_BotChatPhraseSheetLeaves[i] && _BotChatPhrasePriceLeaves[i]) {
      uint price = getCurrentPhraseSheetPrice(
          _BotChatPhraseSheetLeaves[i]->getValue32());
...
void CSPhraseManager::updatePhraseProgressionDB() {
  ...
  if (_BookSkillFitler != SKILLS::unknown)
    numProgressFill =
        (uint)_ProgressionPhrases[_BookSkillFitler].Phrases.size();
  uint i;
  ...
  for (i = 0; i < numProgressFill; i++) {
    ...
    bool known = true;
    CSPhraseSheet *phrase = dynamic_cast<CSPhraseSheet *>(
        SheetMngr.get(NLMISC::CSheetId(ppi.SheetId)));
    if (phrase) {
...
void CSPhraseManager::computePhraseProgression() {
  ...
  for (it = SheetMngr.getSheets().begin(); it != SheetMngr.getSheets().end();
       it++) {
    if (it->second.EntitySheet->Type == CEntitySheet::SPHRASE) {
      const CSPhraseSheet *phrase =
          safe_cast<const CSPhraseSheet *>(it->second.EntitySheet);

#ifdef PHRASE_DEBUG_VERBOSE
      static uint pscount = 0;
      nlinfo("**** PROG PHRASE: %4d, %s", pscount++,
             phrase->Id.toString().c_str());
#endif
      ...
      // **** for this phrase, build the required formula, with and without
      // credits
      ...
      /* if the skillFormulaProg is empty(), and if the full formula is not, it
         is because this phrase is a Credit upgrade!!! we DO WANT see it in the
         skill tree

              Additionnaly, for special case of "after critical hit 3" for
         instance, where the credit is more restrictive than the options, take
         the full formula instead. Detect this case simply if the full formula
         is made of only ONE SkillValue
      */
      ...
      for (itSVA = skillFormulaProg.OrSkills.begin();
           itSVA != skillFormulaProg.OrSkills.end(); itSVA++) {
        std::list<CSkillValue>::const_iterator itSV;
        for (itSV = itSVA->AndSkills.begin(); itSV != itSVA->AndSkills.end();
             itSV++) {
          const CSkillValue &sv = *itSV;
          if (sv.Skill != SKILLS::unknown) {
            // get the actual Skill associated to the skillValue (with no sheet
            // data error, it should be == sv.Skill).
            SKILLS::ESkills skill = sv.Skill;
            uint32 value = sv.Value;
            // eg (SME,10) is incorrect (SME range is [20,50]) => take instead
            // (SM,10)
            ...
            /* in case of (SM,25) (eg arise with auras), it means that for
               example, (SMA,25), or (SMB,25) or (SMC,25) are required. Then we
               have to add this progression in all those skills!
                    => insert in those skills only!
            */
            insertProgressionSkillRecurs(skill, value, skillReqLevel,
                                         skillsToInsert);
          }
        }
      }
  ...
  for (uint i = 0; i < SKILLS::NUM_SKILLS; i++) {
    ...
    for (j = 0; j < phraseSortEntry.size(); j++) {
      ...
      CSPhraseSheet *phraseSheet = dynamic_cast<CSPhraseSheet *>(
          SheetMngr.get(CSheetId(pse.ProgressInfo.SheetId)));
      NLMISC::CSheetId brickSheet;
      if (phraseSheet)
        brickSheet =
            pBM->getSabrinaCom().getPhraseBestDisplayBrick(phraseSheet->Bricks);
      ...
      pse.Text = STRING_MANAGER::CStringManagerClient::getSPhraseLocalizedName(
          CSheetId(pse.ProgressInfo.SheetId));
      ...
      for (uint k = 0; k < pse.Text.size(); k++) {
        if ((uint8)pse.Text[k] < (uint8)'\x80' && isalpha(pse.Text[k]))
          ...
          for (; k < pse.Text.size() && (uint8)pse.Text[k] < (uint8)'\x80' &&
                 isdigit(pse.Text[k]);
               k++) {
            number *= 10;
            number += pse.Text[k] - '0';
...
void CSPhraseManager::insertProgressionSkillRecurs(
    SKILLS::ESkills skill, uint32 value, sint *skillReqLevel,
    std::vector<SKILLS::ESkills> &skillsToInsert) {
  CSkillManager *pSM = CSkillManager::getInstance();
  ...
  // Or special case for the MAX 250 value of skills, ie if the skill has no
  // more children
  if (value < pSM->getMaxSkillValue(skill) || children.empty()) {
    // if not used, insert this skill in the "skill used in this phrase
    // progression"
  ...
  // else recurs to children
    ...
    for (uint i = 0; i < children.size(); i++) {
      insertProgressionSkillRecurs(children[i], value, skillReqLevel,
                                   skillsToInsert);
    }
  }
}
...
// ***************************************************************************
void CSPhraseManager::getPhraseLevelFromSection(uint32 section,
                                                uint32 &minLevel,
                                                uint32 &maxLevel) {
  minLevel = section * 5;
...
// ***************************************************************************
void CSPhraseManager::buildPhraseBrickRequirement(
    uint32 phraseSheetId, std::vector<CSheetId> &bricks) {
  CSBrickManager *pBM = CSBrickManager::getInstance();
  bricks.clear();
  ...
  if (phraseSheet) {
    ...
    for (i = 0; i < phraseSheet->Bricks.size(); i++) {
      CSBrickSheet *brick = pBM->getBrick(phraseSheet->Bricks[i]);
      if (brick) {
        // Special case: For "Use Item enchantment" phrase, don't describe
        // requirement
        ...
        for (uint j = 0; j < brick->RequiredBricks.size(); j++) {
          /* if the required brick is not in the current phrase, then it is
             required E.g: if a phrase has brickA and brickB, and if brickB
             require brickA then learning the phrase finally don't require
             brickA! (else won't be possible to learn it if this is the only
             phrase that offers brickA for instance)
          */
...
// ***************************************************************************
void CSPhraseManager::getCombatWeaponRestriction(string &text,
                                                 const CSPhraseCom &phrase,
                                                 bool &usableWithMelee,
                                                 bool &usableWithRange) {
  text.clear();
  ...
  if (rootBrick && rootBrick->isCombat()) {
    ...
    CReqSkillFormula phraseFormula =
        buildCombatPhraseSkillFormula(phrase.Bricks);
    ...
    // sort by skill index. this is the convention, to be sure that we don't
    // search "uiawrSFM_SFR" while "uiawrSFR_SFM" is defined in en.uxt....
...
// ***************************************************************************
CReqSkillFormula CSPhraseManager::buildCombatPhraseSkillFormula(
    const std::vector<NLMISC::CSheetId> &phraseBricks) const {
  CSBrickManager *pBM = CSBrickManager::getInstance();
  CReqSkillFormula phraseFormula;
...
// ***************************************************************************
uint32 CSPhraseManager::getSpellLevel(
    const std::vector<NLMISC::CSheetId> &phraseBricks) const {
  CSBrickManager *pBM = CSBrickManager::getInstance();
  sint32 maxCost = 0;
  for (uint i = 0; i < phraseBricks.size(); i++) {
...
// ***************************************************************************
void CSPhraseManager::getResistMagic(
    bool resistMagic[RESISTANCE_TYPE::NB_RESISTANCE_TYPE],
    const std::vector<NLMISC::CSheetId> &phraseBricks) {
  // reset
  for (uint i = 0; i < RESISTANCE_TYPE::NB_RESISTANCE_TYPE; i++) {
    resistMagic[i] = false;
  }
...
// ***************************************************************************
uint32 CSPhraseManager::getMemorizedPhraseIfLastOrNewSlot(uint32 memoryLine,
                                                          uint32 memoryIndex) {
  // \toto yoyo: There is a Network BUG which prevents us from simply doing
  // {Forget(),Delete()} Old, then {Learn(),Memorize()}  new (Messages are
  // shuffled).
...
// ***************************************************************************
void CSPhraseManager::fullDeletePhraseIfLast(uint32 memoryLine,
                                             uint32 memorySlot) {
...
// ***************************************************************************
string CSPhraseManager::getForageExtractionPhraseEcotypeFmt(
    const CSPhraseCom &phrase) {
  CSBrickManager *pBM = CSBrickManager::getInstance();
  ...
  for (uint i = 0; i < phrase.Bricks.size(); i++) {
    ...
    if (brick) {
      // Parse all properties of the brick, if FG_ECT_SPC, this is a terrain
      // speciliasing
      for (uint j = 0; j < brick->Properties.size(); j++) {
...
// ***************************************************************************
bool CSPhraseManager::findDefaultAttack(uint32 &memoryLine,
                                        uint32 &memorySlot) {
  CSBrickManager *pBM = CSBrickManager::getInstance();
  ...
  if (_SelectedMemoryDB >= 0 &&
      _SelectedMemoryDB < (sint)memoryLineSort.size()) {
...
// ***************************************************************************
uint CSPhraseManager::fillRoleMasterGenericPhrase(uint32 rmFlags,
                                                  uint32 rmRace) {
  CSBrickManager *pBM = CSBrickManager::getInstance();
  ...
  for (it = SheetMngr.getSheets().begin(); it != SheetMngr.getSheets().end();
    ...
    if (it->second.EntitySheet->Type == CEntitySheet::SPHRASE) {
      ...
      for (i = 0; i < phrase->Bricks.size(); i++) {
        CSBrickSheet *brick = pBM->getBrick(phrase->Bricks[i]);
        // if the brick exist, not a Common (no restriction), and not equal to
        // rolemaster race, then can't sell this phrase
...
// ***************************************************************************
void CSPhraseManager::getPhraseSectionBoundFromSkillFilter(sint &minSectionId,
                                                           sint &maxSectionId) {
  CSkillManager *pSM = CSkillManager::getInstance();
  ...
  // NB: 50/5==10 is already exclusive (I mean all actions of DefensiveMagic
  // Skill are 49 max)
...
uint32 CSPhraseManager::getTotalActionMalus(const CSPhraseCom &phrase) const {
  ...
  if (!phrase.Bricks.empty()) {
    CSBrickSheet *rootBrick = pBM->getBrick(phrase.Bricks[0]);
    if (!rootBrick)
      nlerror("Invalid root sbrick in sphrase_com '%s'",
              phrase.Name.toUtf8().c_str());
...
CCDBNodeLeaf *CSPhraseManager::getRegenTickRangeDbLeaf(uint powerIndex) const {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  CCDBNodeLeaf *dbLeaf = NLGUI::CDBManager::getInstance()->getDbProp(
      toString("SERVER:FLAGS:BRICK_TICK_RANGE:%d:TICK_RANGE", (int)powerIndex),
      false);
  return dbLeaf;
}
...
CTickRange CSPhraseManager::getRegenTickRange(uint powerIndex) const {
  ...
  if (dbLeaf) {
    ...
    tickRange.EndTick =
        (NLMISC::TGameCycle)(((uint64)dbLeaf->getValue64()) >> 32);
  }
  return tickRange;
}

// ***************************************************************************
void CSPhraseManager::setRegenTickRange(uint powerIndex,
                                        const CTickRange &tickRange) {
  CCDBNodeLeaf *dbLeaf = getRegenTickRangeDbLeaf(powerIndex);
  if (dbLeaf) {
    uint64 value =
        uint64(tickRange.StartTick) | (uint64(tickRange.EndTick) << 32);
...
int CSPhraseComAdpater::luaGetCastTime(CLuaState &ls) {
  ...
  float castTime;
  float castTimeMalus;
  pPM->getPhraseCastTime(Phrase, pPM->getTotalActionMalus(Phrase), castTime,
                         castTimeMalus);
...
int CSPhraseComAdpater::luaGetCastRange(CLuaState &ls) {
  ...
  sint range;
  sint rangeMalus;
  pPM->getPhraseMagicRange(this->Phrase, pPM->getTotalActionMalus(Phrase),
                           range, rangeMalus);
...
int CSPhraseComAdpater::luaGetHpCost(CLuaState &ls) {
  ...
  sint hpCost;
  sint hpCostMalus;
  pPM->getPhraseHpCost(this->Phrase, pPM->getTotalActionMalus(Phrase), hpCost,
                       hpCostMalus);
...
int CSPhraseComAdpater::luaGetSapCost(CLuaState &ls) {
  ...
  sint sapCost;
  sint sapCostMalus;
  pPM->getPhraseSapCost(this->Phrase, pPM->getTotalActionMalus(Phrase), sapCost,
                        sapCostMalus);
...
int CSPhraseComAdpater::luaGetFocusCost(CLuaState &ls) {
  ...
  sint focusCost;
  sint focusCostMalus;
  pPM->getPhraseFocusCost(this->Phrase, pPM->getTotalActionMalus(Phrase),
                          focusCost, focusCostMalus);
...
int CSPhraseComAdpater::luaGetStaCost(CLuaState &ls) {
  ...
  sint staCost;
  sint staCostMalus;
  pPM->getPhraseStaCost(this->Phrase, pPM->getTotalActionMalus(Phrase), staCost,
                        staCostMalus);
...
NLMISC_COMMAND(regenTickRange,
  ...
  for (uint b = 0; b < 64; ++b) {
    if (powerBitField & (uint64(1) << b)) {
      pPM->setRegenTickRange(
          b, CTickRange(LastGameCycle, LastGameCycle + duration));
    }
  }
