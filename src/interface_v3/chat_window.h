...
struct CChatWindowDesc {
  ...
  std::string
      FatherContainer; // name of the father container. If empty, the chat box
                       // must be added manually in the hierarchy
  std::string ChatTemplate; // Template for the chat interface, or "" to use the
                            // default one
  TTemplateParams ChatTemplateParams; // optional template parameters
  sint InsertPosition; // [optional] -1 if the chat box should be inserted at
                       // the end of the container list, or the index otherwise
  bool ParentBlink;    // [optional] when true, make the parent group blink
  bool Savable;  // should the position of the chat box be saved between session
                 // ? Default is false
...
/** This class can be used to easily manipulate a chat box without having to
 * deal directly with the ui. Each chat box must have a unique identifier. For
 * that reason, they should be created from a CChatWindowManager instance
...
public:
  // a listener to know when a chat window is removed, or when a msg is
  // displayed to it
  struct IObserver {
    // called by a CChatWindow when it is deleted
    virtual void chatWindowRemoved(CChatWindow * /* cw */) {}
    // called by a CChatWindow when a msg has been displayed in it
    // ('displayMessage' has been called)
    // virtual void displayMessage(CChatWindow *cw, const std::string &msg,
    // NLMISC::CRGBA col, uint numBlinks = 0) {}
  };

public:
  // display a message in this chat box with the given color
  virtual void displayMessage(const std::string &msg, NLMISC::CRGBA col,
                              CChatGroup::TGroupType gt,
                              uint32 dynamicChatDbIndex, uint numBlinks = 0,
                              bool *windowVisible = NULL);
  virtual void displayTellMessage(const std::string & /* msg */,
                                  NLMISC::CRGBA /* col */,
                                  const std::string & /* sender */) {}
  virtual void clearMessages(CChatGroup::TGroupType gt,
  ...
  // std::string version for backward compatibility
   ...
   * Don't do it in the dtor, because done automatically at the end of the app
   * by the interface manager. Useful only if querried by the user
   */
  void deleteContainer();
  // get the last chat window from which a command has been called
  static CChatWindow *getChatWindowLaunchingCommand() {
    return _ChatWindowLaunchingCommand;
  }
  ...
  //
  void displayLocalPlayerTell(const std::string &receiver,
                              const std::string &msg, uint numBlinks = 0);

  /// Encode a color tag '@{RGBA}' in the text. If append is true, append at end
  /// of text, otherwise, replace the text
  static void encodeColorTag(const NLMISC::CRGBA &color, std::string &text,
                             bool append = true);
...
protected:
  ...
  friend class CHandlerContactEntry; // TODO : remove this if CChatBox are used
                                     // in people lists
...
public:
  CChatGroupWindow() {}
  // display a message in this chat box with the given color (callback from chat
  // input filter)
  virtual void displayMessage(const std::string &msg, NLMISC::CRGBA col,
                              CChatGroup::TGroupType gt,
                              uint32 dynamicChatDbIndex, uint numBlinks = 0,
                              bool *windowVisible = NULL);
  virtual void displayTellMessage(const std::string &msg, NLMISC::CRGBA col,
                                  const std::string &sender);
  virtual void clearMessages(CChatGroup::TGroupType gt,
                             uint32 dynamicChatDbIndex);
  sint32 getTabIndex();
  ...
  // Free Teller
  NLGUI::CGroupContainer *createFreeTeller(const std::string &winName,
  ...
  bool removeFreeTeller(
      const std::string &containerID); // Return true if free teller found
...
protected:
  ...
  void getAssociatedSubWindow(CChatGroup::TGroupType gt,
                              uint32 dynamicChatDbIndex, NLGUI::CGroupList *&gl,
                              NLGUI::CCtrlTabButton *&tab);
...
public:
   ...
   * \return A pointer on the window, or NULL, if creation failed or if name
   * already exists.
   */
  ...
  /// from a ctrl of a chat box that triggered a menu, or an event, retrieve the
  /// associated chat box
...
inline CChatWindowManager &getChatWndMgr() {
  return CChatWindowManager::getInstance();
}
