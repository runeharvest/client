...
public:
  ...
  CDecal();
  ~CDecal();
  // Set a texture from its filename. It name match a global texture in the view
  // renderer, it will be used ,first
  void setTexture(const std::string &fileName, bool clampU = true,
                  bool clampV = true, bool filtered = true);
  ...
  void setWorldMatrixForArrow(const NLMISC::CVector2f &start,
                              const NLMISC::CVector2f &end, float halfWidth);
  void setWorldMatrixForSpot(const NLMISC::CVector2f &pos, float radius,
  ...
  void
  setCustomUVMatrix(bool on,
                    const NLMISC::CMatrix &matrix = NLMISC::CMatrix::Identity);
  //
...
private:
  ...
  NLMISC::CVector
      _RefPosition; // position for model matrix, computed near camera pos to
                    // avoid z-fight (big translation in the final MVP matrix
                    // leads to z precision problems)
  bool _Touched;
...
private:
  ...
  void render(NL3D::UDriver &drv, NL3D::CShadowPolyReceiver &receiver,
              const std::vector<NLMISC::CPlane> &worldPyramid,
              const std::vector<NLMISC::CVector> &pyramidCorners,
              bool useVertexProgram);
  void renderTriCache(NL3D::IDriver &drv, NL3D::CShadowPolyReceiver &receiver,
                      bool useVertexProgram);
