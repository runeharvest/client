...
void CRosaceComponent::display(bool selected) {
  ...
  if (_Texture) {
    if (selected)
      Driver->drawBitmap(_X, _Y, _W, _H, *_Texture, true,
                         CRGBA(255, 0, 0, 255));
    else
      Driver->drawBitmap(_X, _Y, _W, _H, *_Texture, true,
...
void CRosacePage::previous() {
  _Selected--;
  // If the selected component is not valide selected the last 1 (-1 is no
  // component).
  if (!valide())
...
void CRosacePage::generate() {
  ...
  for (uint i = 0; i < size(); ++i) {
    ...
    _Components[i].setPos((float)(0.5f + x - _W / 2),
                          (float)(0.5f + y - _H / 2));
  }
} // generate //
...
//-----------------------------------------------
// cursorMode :
// Select the component under the position (x,y) or unselect all if nothing at
// this position.
//-----------------------------------------------
...
//-----------------------------------------------
// cursorAngleMode :
// Select elements of the rosace according to the angle generated by mouse
// position and screen center.
//-----------------------------------------------
void CRosacePage::cursorAngleMode(float x, float y) {
  // If the mouse is near center -> unselect all.
  if (x >= 0.5f - 0.01f && x < 0.5f + 0.01f && y >= 0.5f - 0.01f &&
      y < 0.5f + 0.01f)
    _Selected = -1;
...
//-----------------------------------------------
// execute :
// Execute the callback associated to the selected component in the selected
// page.
//-----------------------------------------------
...
// Constructor.
//-----------------------------------------------
CRosace::CRosace() { init(); } // CRosace //

//-----------------------------------------------
// ~CRosace :
// Destructor.
//-----------------------------------------------
CRosace::~CRosace() {} // ~CRosace //
