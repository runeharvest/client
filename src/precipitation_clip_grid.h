...
public:
        ...
        // Example : move(1, 0) will move the array of one column to the left.
The latest column is lost. The first column remains unchanged
        //
        void move(sint offsetX, sint offsetY);
        // Move a part of the array. Values are clamped as necessary
        void moveSubArray(sint dstX, sint dstY, sint srcX, sint srcY, sint
width, sint height);
        ...
        // See which part of array should be updated after its content has been
displaced by the given offset (by a call to move for example).
        // Example: getUpdateRects(0, 1, result) will result the first row as a
result
        //
        void getUpdateRects(sint moveOffsetX, sint moveOffsetY,
std::vector<CRect> &rectsToUpdate);
        // See which parts of array will be discarded if the array is displaced
by the given offset void getDiscardRects(sint moveOffsetX, sint moveOffsetY,
std::vector<CRect> &discardedRects); private: TArrayContainer _Array; uint
_Width; uint _Height; private: inline void checkRect(const CRect &r) const
        {
                nlassert(r.X >= 0 && r.X < (sint32) _Width);
                nlassert(r.Y >= 0 && r.Y < (sint32) _Height);
                nlassert(r.X + r.Width >= 0 && r.X + (sint32)  r.Width <=
(sint32) _Width); nlassert(r.Y + r.Height >= 0 && r.Y + (sint32) r.Height <=
(sint32) _Height);
        }
};
...
*********************************************************************************
template <class T>
void CArray2D<T>::getUpdateRects(sint moveOffsetX, sint moveOffsetY,
std::vector<CRect> &rectsToUpdate)
{
        rectsToUpdate.clear();
        if (moveOffsetX < 0) // moved right ?
        {
                ...
                if (height > 0)
                {
                        ...
                        rectsToUpdate.push_back(CRect((sint32) (_Width - width),
(sint32) (std::max(- moveOffsetY, 0)), (uint32) width, (uint32) height)); #ifdef
NL_DEBUG checkRect(rectsToUpdate.back()); #endif
                }
        }
        else if (moveOffsetX > 0) // moved left ?
        {
                ...
                if (height > 0)
                {
                        //nlwarning("*2");
                        // complete column on the right
                        rectsToUpdate.push_back(CRect(0, (sint32) std::max(-
moveOffsetY, 0), (uint32) width, (uint32) height)); #ifdef NL_DEBUG
        ...
        if (moveOffsetY < 0)
        {
                ...
                //nlwarning("*3");
                rectsToUpdate.push_back(CRect(0, _Height - height, _Width,
height)); #ifdef NL_DEBUG checkRect(rectsToUpdate.back()); #endif
        }
        else
        if (moveOffsetY > 0)
        {
...
*********************************************************************************
template <class T>
void CArray2D<T>::getDiscardRects(sint moveOffsetX, sint
moveOffsetY,std::vector<CRect> &discardedRects)
{
...
*********************************************************************************
template <class T>
void CArray2D<T>::moveSubArray(sint dstX, sint dstY, sint srcX, sint srcY, sint
width, sint height)
{
...
width);
                                ...
                                {
                                        std::reverse_copy(src, src + width,
dst);
                                }
                                src += _Width;
                                dst += _Width;
...
public:
    ...
    // The size is required to be a power of 2
        ...
        const TGridElemList &getGridElemList(sint x, sint y) const { return
_Grid(x & _SizeMask, y & _SizeMask); } uint getSize() const { return
_Grid.getWidth(); }
        // tmp for debug
...
public:
        ...
        //   \param cellSize  heightGridSize width/height of the height grid. It
must be a power of 2
        //   \param cellSize  wrappedHeightGridSize width/height of the wrapped
height grid (for incoming geometry).
        //                    It must be a power of 2.
        //   \param minZ Minimum possible Z
        //
        void init(float cellSize, uint heightGridSize, uint
wrappedHeightGridSize, float minZ = HEIGHT_GRID_MIN_Z); void update(const
CVector &newPos);
...
///////////////////////////////////////////////////////////////////////
private:
        typedef std::map<uint, UVisualCollisionManager::CMeshInstanceColInfo>
TColMeshMap; typedef std::vector<UVisualCollisionManager::CMeshInstanceColInfo>
TColMeshVect; CHeightGridWrapped
_ZGridWrapped; CArray2D<float>
_ZGrid; float
_CellSize; float
_InvCellSize; float
_MinZ; uint
_GridSize; uint
_SizeMask; sint _PosX; sint
_PosY; std::vector<CRect> _UpdateRects; CHashMap<uint64, CTileAddedInfo>
_TileInfos; CPolygon2D _Tri; CPolygon2D::TRasterVect
_Rasters; TColMeshMap
_Meshs; // meshs currenlty inserted in the wrapped grid TColMeshVect
_UpdateMeshs; // mesh to be removed / added to the wrapped grid during the
update (keep there to avoid vector allocation) std::vector<CVector>
_CurrMeshVertices; // vertices of the mesh being inserted CAABBox
_BBox; // bbox containing current grid private: void updateBBox(); void
updateRect(const CRect &rect); void discardRect(const CRect &rect); void
updateCell(sint x, sint y);
        ...
        void	addCollisionMesh(const
UVisualCollisionManager::CMeshInstanceColInfo &colMesh);
};
...
public:
  ...
  struct CGridPoint {
    float
        MeanHeight; // The mean height that precipitation must use at this point
    bool Clipped;   // True if clipped
    CGridPoint(float meanHeight = 0.f, bool clipped = true)
        : MeanHeight(meanHeight), Clipped(clipped) {}
  };
...
public:
  ...
  /** init the grid
   * \param size width & height of the grid so the grid has (size + 1) ^2
   * elements. For example if size is 1, the grid is a simple square and it has
   * four corners. \param gridEltSize width & height of a grid element.
   */
  void initGrid(uint size, float gridEltSizeX, float gridEltSizeY);

  // Update the grid so that it match the position of the user. (this grid is
  // centered at the user pos)
  void updateGrid(const NLMISC::CVector &userPos,
                  NLPACS::UGlobalRetriever *globalRetriever);
...
private:
  void clear();
  void updateGridPart(sint worldX, sint worldY, uint gridX, uint gridY,
  ...
  static TGrid::const_iterator getGridIt(const TGrid &grid, uint x, uint y,
                                         uint size) {
    nlassert(size > 0);
    nlassert(x <= size);
    nlassert(y <= size);
