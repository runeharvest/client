 ...
 */
static inline void blendValue(float &dest, float f1, float f2,
                              float blendFactor) {
...
//==================================================================================
void CWeatherStateClient::setup(
    const CWeatherState &ws,
...
//==================================================================================
void CWeatherStateClient::blend(CWeatherStateClient &dest,
                                const CWeatherStateClient &s1,
                                const CWeatherStateClient &s2,
                                float blendFactor) {
  H_AUTO_USE(RZ_WeatherSetupClient)
  ...
  for (k = 0; k < s1.FXs.size(); ++k) {
    ...
    for (l = 0; l < s2.FXs.size(); ++l) {
      if (s1.FXs[k].Precipitation == s2.FXs[l].Precipitation) {
        ...
        blendValue(dest.FXs[k].Ratio, s1.FXs[k].Ratio, s2.FXs[k].Ratio,
                   blendFactor);
        found = true;
        ++numCommonFXs;
        break;
