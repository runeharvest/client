...
// *********************************************************************************************************
CDisplayerVisualShape::CDisplayerVisualShape(const std::string &shapeName,
                                             float scale,
...
void CDisplayerVisualShape::onPreRender() {
  ...
  if (_Active) {
    ...
    if (!_VisualSnapToGroundDone && SkipFrame == 0) {
      snapToGround(); // force visual snap to ground if not already done + icon
                      // update
    }
  }
  if (!_Touched) {
    if (!_Instance.empty())
      _BBoxMatrix = _Instance.getMatrix();
    else
      _BBoxMatrix = CMatrix::Identity;
    return;
  }
  if (_Active) {
    updateMapDeco();
  ...
  if (!_Instance.empty()) {
    _BBoxMatrix =
        _Instance.getMatrix(); // ensure that bbox and shape displayed at same
                               // pos (events that modify the instance pos may
                               // be received after the display of this frame)
  }
...
void CDisplayerVisualShape::drawBBox(NLMISC::CRGBA color) const {
  ...
  if (getRotateInProgress())
    return; // no drawn while drawing (bbox moved one frame too late, must solve
            // this)
  NLMISC::CAABBox bbox;
...
// *********************************************************************************************************
void CDisplayerVisualShape::onAttrModified(const std::string &name,
                                           sint32 index) {
...
// *********************************************************************************************************
float CDisplayerVisualShape::preciseIntersectionTest(
  ...
  if (_Instance.supportFastIntersect()) {
    float dist2D, distZ;
    if (const_cast<NL3D::UInstance &>(_Instance).fastIntersect(
            worldRayStart, worldRayDir, dist2D, distZ, false)) {
      if (dist2D == 0.f) {
        return distZ;
      }
    }
...
void CDisplayerVisualShape::visualSnapToGround() {
  ...
  if (!_VisualSnapToGroundDone) {
    if (!_Instance.empty()) {
      ...
      // VisualCollisionEntity's  'snap to ground' requires that the entity is
      // not farther than 100 meters from it Eval first coarse height from
      // height map + coarse collision mesh
      ...
      if (coarseSnap) {
        ...
        CTool::TRayIntersectionType interType =
            CTool::computeWorldMapIntersection(pos3f.x, pos3f.y, inter);
      ...
      if (CollisionManager) {
        ...
        snapped = _VisualCollisionEntity->snapToGround(
            pos3f); // refine z from current pos
      }
      //
      if (snapped ||
          interType != CTool::NoIntersection) // take precise version or else
                                              // default to coarsest one
      {
        _WorldPos.z = pos3f.z;
      }
