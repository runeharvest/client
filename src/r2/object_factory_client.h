 ...
 * This object will trigger the right events on its parent when its target is
 * created or erased. (creation is watched too. This is possible because we use
 * instance id, not direct pointer on the object)
 */
class CObjectRefIdClient : public CObjectRefId,
...
// the client factory redefine R2::CObjectRefId so that it can perform its
// observer job on the client
