...
class CRefCountedPackedWorld : public NL3D::CPackedWorld,
...
public:
  static const uint MapSizeDivisor; // size ratio between the island map & the
...
public:
  ...
  // test a convex polygon against the map & water, return true if all positions
  // under the polygon are valid positions. Normal of terrain is not tested.
  bool isValidPoly(const NLMISC::CPolygon2D &poly);
  // Test if the given position is valid (0.5 meter diameter). Test also normal
  // at that position (so good test to create an entity here)
  bool isValidPos(const NLMISC::CVector2f &pos);
  // test a segment against the heightmap & water, normal of terrain is not
  // tested
  bool isValidSegment(const NLMISC::CVector2f start,
                      const NLMISC::CVector2f end);
  // accessibility texture
  NL3D::ITexture *getAccesibilityTexture();
  const NLMISC::CMatrix &
  getWorldToAccessibilityTexMat(bool cropped = false) const {
    return cropped ? _WorldToAccessibilityTexMatCropped
                   : _WorldToAccessibilityTexMat;
  }
...
private:
  NL3D::CPackedWorld *
  reloadPackedIsland(const CScenarioEntryPoints::CCompleteIsland &islandDesc);
...
public:
  // for CWaterMap
  void waterSurfaceAdded(const NLMISC::CPolygon2D &shape,
