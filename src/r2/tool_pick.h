...
/** Tool to select an instance in scene / in the component feature list / in the
 * world map The tool can possibly select a position in scene if there's not
 * entity under the mouse.
 */
class CToolPick : public CTool {
public:
  // build a new pick tool with the given CPicker interface (should not be NULL
  // or assert)
  CToolPick(const std::string &cursCanPickInstance = "r2ed_tool_can_pick.tga",
  ...
  virtual const char *getToolUIName() const {
    return "";
  } // by default, no associated icon in the ui
   ...
   * CTool::checkInstanceUnderMouse() is called -> if no collision is found in
   * scene or in world map, a call is made into lua to retrieve ui instance over
   * which the mouse is. Then the mouse cursor is updated ifthat instance is
   * 'pickable' On a mouse button click, lua check that the current tool is a
   * picking tool. If so, then lua code call the 'pick' method exported below to
   * do the selection.
   */
  ...
  REFLECT_LUA_METHOD("canPick",
                     luaCanPick); // return true if the curent instance under
                                  // the mouse can be selected
  REFLECT_EXPORT_END
