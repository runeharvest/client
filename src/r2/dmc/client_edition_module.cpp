...
CVariable<std::string>
    UserComponentsExamplesDirectory("DSS", "UserComponentsExampleDirectory", "",
                                    "examples/user_componts", 0, true);
CVariable<std::string>
    UserComponentsComponentsDirectory("DSS",
                                      "UserComponentsComponentsDirectory", "",
                                      "examples/user_componts", 0, true);
CVariable<std::string>
    UserComponentsComponentExtension("DSS", "UserComponentsComponentExtension",
                                     "", "gz", 0, true);
CVariable<std::string>
    UserComponentsSourceExtension("DSS", "UserComponentsSourceExtension", "",
                                  "lua", 0, true);

namespace R2 {

// if client request an operation, and the server accept then the client do not
// need to received the same msg he send other client must receive the answer
...
public:
  ...
  // if ok == true, then simulate the replay that The Server would do for other
  // client if ok == false, remove the  Forseen Answer from the answer queue.
  void ack(CClientEditionModule *client, NLNET::IModuleProxy *server,
           TMessageId msgId, bool ok);
  ...
  TMessageId onNodeSet(const std::string &instanceId,
                       const std::string &attrName, const R2::CObject *value);
  TMessageId onNodeInserted(const std::string &instanceId,
                            const std::string &attrName, sint32 position,
                            const std::string &key, const R2::CObject *value);
  TMessageId onNodeErased(const std::string &instanceId,
                          const std::string &attrName, sint32 position);
  TMessageId onNodeMoved(const std::string &instanceId1,
...
class IServerAnswerMsg {
public:
  virtual void ok(CClientEditionModule *client,
                  NLNET::IModuleProxy *server) = 0;
...
public:
  CServerAnswerMsgSet(const std::string &instanceId,
                      const std::string &attrName, const R2::CObject *value)
      : _InstanceId(instanceId), _AttrName(attrName),
        _Value(value ? value->clone() : 0) {}
...
public:
  CServerAnswerMsgInserted(const std::string &instanceId,
                           const std::string &attrName, sint32 position,
                           const std::string &key, const R2::CObject *value)
      : _InstanceId(instanceId), _AttrName(attrName), _Position(position),
        _Key(key), _Value(value ? value->clone() : 0) {}

  void ok(CClientEditionModule *client, NLNET::IModuleProxy *server) {
    client->onNodeInserted(server, _InstanceId, _AttrName, _Position, _Key,
                           _Value.get());
  }
...
public:
  CServerAnswerMsgErased(const std::string &instanceId,
                         const std::string &attrName, sint32 position)
      : _InstanceId(instanceId), _AttrName(attrName), _Position(position) {}
...
public:
  CServerAnswerMsgMoved(const std::string &instanceId1,
                        const std::string &attrName1, sint32 position1,
                        const std::string &instanceId2,
                        const std::string &attrName2, sint32 position2)
      : _InstanceId1(instanceId1), _AttrName1(attrName1), _Position1(position1),
        _InstanceId2(instanceId2), _AttrName2(attrName2),
        _Position2(position2) {}

  void ok(CClientEditionModule *client, NLNET::IModuleProxy *server) {
    client->onNodeMoved(server, _InstanceId1, _AttrName1, _Position1,
                        _InstanceId2, _AttrName2, _Position2);
  }
...
void CServerAnswerForseener::ack(CClientEditionModule *client,
                                 NLNET::IModuleProxy *server, TMessageId msgId,
                                 bool ok) {
...
CServerAnswerForseener::TMessageId
CServerAnswerForseener::onScenarioUploaded(const R2::CObject *hlScenario) {
  bool ok = _Answers
...
CServerAnswerForseener::TMessageId
CServerAnswerForseener::onNodeSet(const std::string &instanceId,
                                  const std::string &attrName,
                                  const R2::CObject *value) {
  bool ok = _Answers
                .insert(std::make_pair(
                    ++_MaxMessageId,
                    new CServerAnswerMsgSet(instanceId, attrName, value)))
                .second;
  nlassert(ok);
  return _MaxMessageId;
}

CServerAnswerForseener::TMessageId CServerAnswerForseener::onNodeInserted(
    const std::string &instanceId, const std::string &attrName, sint32 position,
    const std::string &key, const R2::CObject *value) {
  bool ok =
      _Answers
          .insert(std::make_pair(
              ++_MaxMessageId, new CServerAnswerMsgInserted(
...
CServerAnswerForseener::TMessageId
CServerAnswerForseener::onNodeErased(const std::string &instanceId,
                                     const std::string &attrName,
                                     sint32 position) {
  bool ok = _Answers
                .insert(std::make_pair(
                    ++_MaxMessageId,
                    new CServerAnswerMsgErased(instanceId, attrName, position)))
                .second;
  nlassert(ok);
  return _MaxMessageId;
}

CServerAnswerForseener::TMessageId CServerAnswerForseener::onNodeMoved(
    const std::string &instanceId1, const std::string &attrName1,
    sint32 position1, const std::string &instanceId2,
    const std::string &attrName2, sint32 position2) {
  bool ok = _Answers
                .insert(std::make_pair(++_MaxMessageId,
                                       new CServerAnswerMsgMoved(
...
void CEditorConfig::setDisplayInfo(const std::string &formName,
                                   bool displayInfo) {
  // H_AUTO(R2_CEditorConfig_setDisplayInfo)
  CInterfaceManager *IM = CInterfaceManager::getInstance();
  uint32 index = _NameToId[formName];
  uint32 newValue =
      static_cast<uint32>(NLGUI::CDBManager::getInstance()
                              ->getDbProp("UI:SAVE:R2:DISPLAYINFO")
                              ->getValue32());
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("UI:SAVE:R2:DISPLAYINFO")
      ->setValue32(static_cast<sint32>(newValue));
}
...
void CEditorConfig::setDisplayInfo(uint32 displayInfo) {
  ...
  NLGUI::CDBManager::getInstance()
      ->getDbProp("UI:SAVE:R2:DISPLAYINFO")
      ->setValue32(static_cast<sint32>(displayInfo));
}
...
uint32 CEditorConfig::getDisplayInfo() const {
  ...
  CInterfaceManager *IM = CInterfaceManager::getInstance();
  return static_cast<uint32>(NLGUI::CDBManager::getInstance()
                                 ->getDbProp("UI:SAVE:R2:DISPLAYINFO")
                                 ->getValue32());
}
...
CClientEditionModule::~CClientEditionModule() { release(); }

void CClientEditionModule::init(NLNET::IModuleSocket *clientGw,
...
//{
// public:
//	CSerialFactoryBackup(CObjectFactory *newValue = NULL) :
//ClientFactory(CObjectSerializer::Factory)
//	{
...
bool CClientEditionModule::onProcessModuleMessage(
    IModuleProxy *senderModuleProxy, const CMessage &message) {
  // H_AUTO(R2_CClientEditionModule_onProcessModuleMessage)

  //	CSerialFactoryBackup fb(_Factory); // restore client factory the time to
  //process server msg ...
  //
  //	if (CShareClientEditionItfSkel::onDispatchMessage(senderModuleProxy,
  //message))
  //	{
  //		return;
  //	}
  ...
  if (operationName == "ADV_CONN") {
    ...
    nlinfo("R2CED: user Connected as Client %d In Mode %d",
           bodyConnection.EditSlotId, bodyConnection.Mode);
    ...
    case st_anim:
      ...
      break;
    default:
      nlassert(0 && "SessionType not handled.");
    }
    ...
    if (bodyConnection.SessionType == st_edit) {
      ...
      if (!data && bodyConnection.InCache) {
        ...
        if (sv.setScenarioToLoad("save/r2_buffer.dat", values, md5, signature,
                                 true) &&
            !md5.empty() &&
            CScenarioValidator::AutoSaveSignature == signature) {
          data = _Client->getComLuaModule().loadLocal("save/r2_buffer.dat",
                                                      values);
        } else {
          nlinfo("Data corrupted (the server has refused to accept the "
                 "scenario).");
        }
      }
      _Client->onEditionModeConnected(
          bodyConnection.EditSlotId, bodyConnection.SessionId.asInt(), data,
          bodyConnection.VersionName, bodyConnection.MustTp,
          bodyConnection.InitialActIndex);
      if (bodyConnection.Mode == 2) {
        _Client->onEditionModeDisconnected();
        _Client->onTestModeConnected();
      }
      // warn server that adventure has been connected, so we are ready to
      // receive the TP message
      ...
      // warn server that advneutre has been connected, so we are ready to
      // receive the TP message
    }
    return true;
  }
...
void CClientEditionModule::onTestModeDisconnected(
    NLNET::IModuleProxy * /* moduleProxy */, TSessionId sessionId,
    uint32 lastAct, TScenarioSessionType sessionType) {
...
void CClientEditionModule::onModuleSecurityChange(
...
bool CClientEditionModule::requestTeleportOneCharacterToAnother(
    uint32 sessionId, uint32 sourceCharId, uint32 destCharId) {
  ...
  CShareServerEditionItfProxy proxy(_ServerEditionProxy);
  proxy.teleportOneCharacterToAnother(this, (TSessionId)sessionId, sourceCharId,
                                      destCharId);
  return true;
}
...
*/
  ...
  NLNET::CMessage msg;
  sendMsgToDss(
      CShareServerEditionItfProxy::buildMessageFor_onScenarioUploadAsked(
          msg, messageId, body, true));
  return true;
}

void CClientEditionModule::requestSetNode(const std::string &instanceId,
                                          const std::string &attrName,
                                          CObject *value) {
  ...
  if (!attrName.empty()) {
    ...
    if (instance) {
      CObject *property =
          _PropertyAccessor->getPropertyValue(instance, attrName);
...
void CClientEditionModule::requestSetNodeNoTest(const std::string &instanceId,
                                                const std::string &attrName,
                                                CObject *value) {
  ...
  value2.compress();
  uint32 messageId =
      _ServerAnswerForseener->onNodeSet(instanceId, attrName, value);
  proxy.onNodeSetAsked(this, messageId, instanceId, attrName, value2);
}

void CClientEditionModule::requestEraseNode(const std::string &instanceId,
                                            const std::string &attrName,
                                            sint32 position) {
  ...
  CShareServerEditionItfProxy proxy(_ServerEditionProxy);
  uint32 messageId =
      _ServerAnswerForseener->onNodeErased(instanceId, attrName, position);
  proxy.onNodeEraseAsked(this, messageId, instanceId, attrName, position);
}

void CClientEditionModule::requestInsertNode(const std::string &instanceId,
                                             const std::string &attrName,
                                             sint32 position,
  ...
  if (value) {
    ...
    if (value->getGhost()) {
      // this is a ghost value -> forward directly to client
      getEditor().getDMC().nodeInserted(instanceId, attrName, position, key,
                                        value->clone());
      return;
    }
  }
  ...
  CObjectSerializerServer value2(value);
  value2.compress();
  uint32 messageId = _ServerAnswerForseener->onNodeInserted(
      instanceId, attrName, position, key, value);
  proxy.onNodeInsertAsked(this, messageId, instanceId, attrName, position, key,
                          value2);
}

void CClientEditionModule::requestMoveNode(const std::string &instanceId,
                                           const std::string &attrName,
                                           sint32 position,
  ...
  if (src)
    nlassert(!src->getGhost());
  if (dest)
    nlassert(!dest->getGhost());
  //	CSerialFactoryBackup fb;

  BOMB_IF(_ServerEditionProxy == NULL, "Server Edition Module not connected",
          return);
  CShareServerEditionItfProxy proxy(_ServerEditionProxy);
  uint32 messageId = _ServerAnswerForseener->onNodeMoved(
      instanceId, attrName, position, destInstanceId, destAttrName,
      destPosition);
  proxy.onNodeMoveAsked(this, messageId, instanceId, attrName, position,
                        destInstanceId, destAttrName, destPosition);
}

void CClientEditionModule::requestMapConnection(uint32 scenarioId, bool mustTp,
  ...
  BOMB_IF(_ServerEditionProxy == NULL, "Server Edition Module not connected",
          return);
  CShareServerEditionItfProxy proxy(_ServerEditionProxy);
  proxy.onMapConnectionAsked(this, (TSessionId)scenarioId, mustTp,
                             mustUpdateHighLevel, R2::TUserRole::ur_editor);
}
...
CScenario *CClientEditionModule::getCurrentScenario() const {
  return _Scenario;
}

void CClientEditionModule::addPaletteElement(const std::string &attrName,
...
CObject *
CClientEditionModule::getPropertyValue(CObject *component,
...
CObject *
CClientEditionModule::getPropertyValue(const std::string &instanceId,
                                       const std::string &attrName) const {
  // H_AUTO(R2_CClientEditionModule_getPropertyValue)
...
void CClientEditionModule::onCharModeUpdated(
    NLNET::IModuleProxy * /* senderModuleProxy */, R2::TCharMode mode) {
...
void CClientEditionModule::startScenario(class NLNET::IModuleProxy *proxy,
                                         bool ok, uint32 /* startingAct */,
  ...
  if (ok) {
    if (_SessionType == st_edit) {
      ...
      } else if (_CharMode == TCharMode::Tester ||
                 _CharMode == TCharMode::Player) {
...
void CClientEditionModule::startingScenario(
    class NLNET::IModuleProxy * /* serverProxy */, uint32 charId) {
  ...
  if (_Scenario) {
    // Some times save is refused: be NevraxScenario must not be changed
    {
      ...
      R2::getEditor().getLua().executeScriptNoThrow(
          "r2.Translator.initStartingActIndex()");
      ...
      CObject *hlScenario2 = _Client->getComLuaModule().loadLocal(
          "save/r2_buffer.dat", _LastReadHeader);
    ...
    if (myUserId == (charId >> 4) || ClientCfg.Local) {
      ...
      CUniquePtr<CObject> rtDataPtr(
          _Client->getComLuaModule().translateFeatures(hlScenario, errorMsg));
      ...
      if (ok && sessionType == st_anim) {
        ...
        NLNET::CMessage msg;
        uint32 messageId =
            _ServerAnswerForseener->onScenarioUploaded(hlScenario);
        sendMsgToDss(
            CShareServerEditionItfProxy::buildMessageFor_onScenarioUploadAsked(
                msg, messageId, hlData, false));
      }
      ...
      {
        NLNET::CMessage msg;
        sendMsgToDss(CShareServerEditionItfProxy::buildMessageFor_startScenario(
            msg, ok, header, rtData, _StartingActIndex));
      }
    } else {
...
void CClientEditionModule::updateUserComponentsInfo(
    const std::string &filename, const std::string &name,
    const std::string &description, uint32 timestamp,
  ...
  if (found == _UserComponents.end()) {
    nlwarning("Error: try to update information on a unknown component '%s'",
              filename.c_str());
    return;
  }
...
CUserComponent *CClientEditionModule::getUserComponentByHashMd5(
    const NLMISC::CHashKeyMD5 &md5) const {
  // H_AUTO(R2_CClientEditionModule_getUserComponentByHashMd5)

  TUserComponents::const_iterator first(_UserComponents.begin()),
      last(_UserComponents.end());
  for (; first != last && (first->second->Md5 != md5); ++first) {
  }
  if (first == last) {
    nlwarning("Error: try to upload unknown component '%s'",
              md5.toString().c_str());
    return 0;
  }
...
void CClientEditionModule::onUserComponentUploading(
    NLNET::IModuleProxy * /* senderModuleProxy */, const CHashKeyMD5 &md5) {
...
void CClientEditionModule::onUserComponentRegistered(
    NLNET::IModuleProxy * /* senderModuleProxy */, const CHashKeyMD5 &md5) {
  ...
  if (!userComponent) {
    ...
    R2::getEditor().getLua().executeScriptNoThrow(NLMISC::toString(
        "r2.UserComponentsManager:addUserComponent('%s')", filename.c_str()));
  }
}

void CClientEditionModule::onUserComponentDownloaded(
    NLNET::IModuleProxy *senderModuleProxy, CUserComponent *component) {
  // H_AUTO(R2_CClientEditionModule_onUserComponentDownloaded)

  // case 1 data are compressed
  component->UncompressedData =
      new uint8[component->UncompressedDataLength + 1];
  uLongf dataLength = static_cast<uLongf>(component->UncompressedDataLength);
  sint32 decompressionState = uncompress(
      reinterpret_cast<Bytef *>(component->UncompressedData), &dataLength,
      reinterpret_cast<Bytef *>(component->CompressedData),
      component->CompressedDataLength);

  component->UncompressedDataLength = static_cast<uint32>(dataLength);

  if (decompressionState != Z_OK) {
    nlwarning("Error: the downloaded user component is corrupted '%s'",
...
bool CClientEditionModule::loadUserComponent(const std::string &filename,
                                             bool mustReload) {
  ...
  if (!ok) {
    ...
    nlwarning("Allowed file extension '%s' '%s'", sourceExtension.c_str(),
              componentExtension.c_str());
    return false;
  }
  ...
  if (!compressed) {
    ...
    if (!file) {
      nlwarning("Try to open an invalid file %s (access error)",
                filename.c_str());
      return false;
    }
    ...
    if (fseek(file, 0, SEEK_END) != 0) {
      nlwarning("Try to open an invalid file %s (size error)",
                filename.c_str());
      fclose(file);
      return false;
    }
    ...
    int length = (int)fread(uncompressedFile, sizeof(char),
                            uncompressedFileLength, file);
    if (length < 0) {
    ...
    // REMOVE HEADER
    std::string data((const char *)&uncompressedFile[0],
                     (const char *)&uncompressedFile[uncompressedFileLength]);
    std::string::size_type start = data.find(compiledInfoHeader);
    if (start != std::string::npos) {
      std::string::size_type finish =
          data.find(compiledInfoFooter, start + compiledInfoHeader.length());
      if (finish != std::string::npos) {
        finish += compiledInfoFooter.size();
        data =
            data.substr(0, start) + data.substr(finish, data.size() - finish);
      }
    }
    ...
    if (fread((void *)&uncompressedFileLength, sizeof(uncompressedFileLength),
              1, file) != 1) {
      nlwarning("Error while reading %s", filename.c_str());
    }
  ...
  {
    uLongf destLen =
        uncompressedFileLength + uncompressedFileLength / 1000 + 12;
    Bytef *dest = new Bytef[destLen];
    int ok = compress(dest, &destLen, (Bytef *)uncompressedFile,
                      uncompressedFileLength);
    if (ok != Z_OK) {
      delete[] uncompressedFile;
      delete[] compressedFile;
      nlwarning("Error while reading %s (can't compress data)",
                filename.c_str());
      return false;
    }
  ...
  _UserComponents[filename] =
      new CUserComponent(filename, uncompressedFile, uncompressedFileLength,
                         compressedFile, compressedFileLength);
...
std::string
CClientEditionModule::readUserComponentFile(const std::string &filename) {
...
CUserComponent *CClientEditionModule::getUserComponentByFilename(
...
void CClientEditionModule::saveUserComponentFile(const std::string &filename,
                                                 bool mustCompress) {
  ...
  if (ok) {
    ...
    if (CFile::getExtension(filename) ==
        UserComponentsSourceExtension.toString()) {
      uncompressedName = filename;
      compressedName =
          toString("%s/%s.%s", UserComponentsComponentsDirectory.c_str(),
                   component->Md5Id.toString().c_str(),
                   UserComponentsComponentExtension.c_str());
    } else if (CFile::getExtension(filename) ==
               UserComponentsComponentExtension.toString()) {
      compressedName = filename;
      uncompressedName = toString(
          "%s/%s.%s", UserComponentsExamplesDirectory.c_str(),
    ...
    if (!mustCompress) {
      {
        ...
        if (output) {
          if (fwrite(component->UncompressedData, sizeof(char),
                     component->UncompressedDataLength,
                     output) != component->UncompressedDataLength) {
    ...
    } else {
      {
        ...
        if (output) {
          gzwrite(output, (const voidp)component->UncompressedData,
                  component->UncompressedDataLength);
          gzclose(output);
        }
      }
...
void CClientEditionModule::refreshComponents() {
  ...
  /*
     ...
     CFile::createDirectory(_ScriptDirectory);
          }

          // verify content of directory has not changed
          uint32 lastDirectoryModificationDate =
     CFile::getFileModificationDate(const std::string &filename); if
     (_LastDirectoryModificationDate == lastDirectoryModificationDate )
          ...
          CPath::getPathContent (_ScriptDirectory, false, false, true, result,
     false, false);
...
void CClientEditionModule::ackMsg(NLNET::IModuleProxy *sender, uint32 msgId,
                                  bool ok) {
  _ServerAnswerForseener->ack(this, sender, msgId, ok);
}

void CClientEditionModule::onScenarioUploaded(
...
void CClientEditionModule::onNodeSet(NLNET::IModuleProxy * /* sender */,
...
void CClientEditionModule::onNodeInserted(
    NLNET::IModuleProxy * /* sender */, const std::string &instanceId,
    const std::string &attrName, sint32 position, const std::string &key,
    const R2::CObjectSerializerClient &value) {
  // H_AUTO(R2_CClientEditionModule_onNodeInserted)
  if (_Mute)
    return;
  _Client->nodeInserted(instanceId, attrName, position, key,
                        value.getData()); // todo ownership
}

// The ServerEditionMode erases a node on a hl scenario.
void CClientEditionModule::onNodeErased(NLNET::IModuleProxy * /* sender */,
                                        const std::string &instanceId,
                                        const std::string &attrName,
                                        sint32 position) {
...
void CClientEditionModule::onNodeMoved(NLNET::IModuleProxy * /* sender */,
                                       const std::string &instanceId1,
                                       const std::string &attrName1,
                                       sint32 position1,
                                       const std::string &instanceId2,
                                       const std::string &attrName2,
                                       sint32 position2) {
  // H_AUTO(R2_CClientEditionModule_onNodeMoved)
  if (_Mute)
    return;
  _Client->nodeMoved(instanceId1, attrName1, position1, instanceId2, attrName2,
                     position2);
}

void CClientEditionModule::onQuotaUpdated(
    NLNET::IModuleProxy * /* senderModuleProxy */, uint32 maxNpcs,
    uint32 maxStaticObjects) {
  // H_AUTO(R2_CClientEditionModule_onQuotaUpdated)
  // R2::getEditor().getLua().executeScriptNoThrow(toString("r2.QuotaMgr.onQuotaUpdated(%u,
  // %u)", maxNpcs, maxStaticObjects));
  _MaxNpcs = maxNpcs;
...
std::string CClientEditionModule::getEmoteBehaviorFromEmoteId(
...
void CClientEditionModule::setDisplayInfo(const std::string &formName,
                                          bool displayInfo) {
...
void CClientEditionModule::onDisplayInfoUpdated(
    NLNET::IModuleProxy * /* senderModuleProxy */, uint32 displayInfo) {
...
void CClientEditionModule::onTpPositionSimulated(
    NLNET::IModuleProxy * /* sender */, TSessionId /* sessionId */,
    uint64 /* characterId64 */, sint32 x, sint32 y, sint32 z,
...
void CClientEditionModule::onDisconnected(NLNET::IModuleProxy * /* sender */) {
  ...
  R2::getEditor().getLua().executeScriptNoThrow(
      NLMISC::toString("r2.onDisconnected()"));
}

void CClientEditionModule::onKicked(NLNET::IModuleProxy * /* sender */,
...
void CClientEditionModule::requestSetStringValue(std::string &id,
                                                 std::string &value) {
...
void CClientEditionModule::requestStartAct(uint32 actId) {
  ...
  DROP_IF(_ServerAnimationProxy == NULL,
...
void CClientEditionModule::requestTpPosition(float x, float y, float z) {
  ...
  DROP_IF(_ServerEditionProxy == NULL, "Server Edition Module not connected",
          return);
...
bool CClientEditionModule::requestTpToEntryPoint(uint32 actIndex) {
  ...
  DROP_IF(_ServerEditionProxy == NULL, "Server Edition Module not connected",
          return false);
  if (R2::getEditor().isClearingContent()) {
    return true;
  }
...
void CClientEditionModule::onAnimationModePlayConnected(
    NLNET::IModuleProxy * /* senderModuleProxy */) {
  // H_AUTO(R2_CClientEditionModule_onAnimationModePlayConnected)
  _Client->onAnimationModePlayConnected();
}

void CClientEditionModule::scheduleStartAct(NLNET::IModuleProxy * /* sender */,
                                            uint32 errorId, uint32 actId,
                                            uint32 nbSeconds) {
...
void CClientEditionModule::updateScenarioHeader(
    NLNET::IModuleProxy * /* sender */,
    const TScenarioHeaderSerializer &header) {
  // H_AUTO(R2_CClientEditionModule_updateScenarioHeader)
  _ScenarioHeader = header.Value;
  R2::getEditor().getLua().executeScriptNoThrow(
      "r2.onScenarioHeaderUpdated(r2.getScenarioHeader())");
}

void CClientEditionModule::updateMissionItemsDescription(
    NLNET::IModuleProxy * /* sender */, TSessionId /* sessionId */,
...
void CClientEditionModule::updateActPositionDescriptions(
    NLNET::IModuleProxy * /* sender */,
  ...
  R2::getEditor().getLua().executeScriptNoThrow(
      "r2.onRuntimeActUpdated(r2.getRuntimeActs())");
}

void CClientEditionModule::updateUserTriggerDescriptions(
    NLNET::IModuleProxy * /* sender */,
  ...
  R2::getEditor().getLua().executeScriptNoThrow(
      "r2.onUserTriggerDescriptionUpdated(r2.getUserTriggers())");
}
...
bool CClientEditionModule::requestTriggerUserTrigger(uint32 actId,
                                                     uint triggerId) {
...
void CClientEditionModule::onCurrentActIndexUpdated(
    NLNET::IModuleProxy * /* sender */, uint32 actId) {
  // H_AUTO(R2_CClientEditionModule_onCurrentActIndexUpdated)
  this->_CurrentActIndex = actId;
  R2::getEditor().getLua().executeScriptNoThrow(
      "r2.onCurrentActIndexUpdated(r2.getCurrentActIndex())");
}
...
void CClientEditionModule::updateIncarningList(
    NLNET::IModuleProxy * /* sender */, const std::vector<uint32> &botId) {
  // H_AUTO(R2_CClientEditionModule_updateIncarningList)
  this->_IncarnatingList = botId;
  R2::getEditor().getLua().executeScriptNoThrow(
      "r2.onIncarnatingListUpdated()");
}

void CClientEditionModule::updateTalkingAsList(
    NLNET::IModuleProxy * /* sender */, const std::vector<uint32> &botId) {
...
void CClientEditionModule::systemMsg(NLNET::IModuleProxy * /* sender */,
...
void CClientEditionModule::updateScenarioRingAccess(
    bool ok, const std::string &ringAccess, const std::string &errMsg) {
...
void CClientEditionModule::onRingAccessUpdated(
    NLNET::IModuleProxy * /* moduleSocket */, const std::string &ringAccess) {
...
/*
 */
void CClientEditionModule::addToSaveList(
  ...
  if (!ok) {
    ...
    CShareServerEditionItfProxy serverEditionModule(_ServerEditionProxy);
    serverEditionModule.saveScenarioFile(this, md5,
                                         R2::TScenarioHeaderSerializer(v1));
  }

  // :TODO: VIANNEY add all
  _LastReadHeader.clear();
  _LastReadHeader.push_back(
      std::pair<std::string, std::string>("BodyMD5", sv->getBodyMd5()));
...
/**********************************************************/
void CClientEditionModule::addToUserComponentSaveList(
    const std::string &filename,
    const std::vector<std::pair<std::string, std::string>> &values,
    std::string &body) {
  // H_AUTO(R2_CClientEditionModule_addToSaveList)
  ...
  if (!ok) {
    ...
    CShareServerEditionItfProxy serverEditionModule(_ServerEditionProxy);
    serverEditionModule.saveUserComponentFile(
        this, md5, R2::TScenarioHeaderSerializer(v1));
  }

  return;
}

void CClientEditionModule::saveUserComponentFileAccepted(
    NLNET::IModuleProxy *senderModuleProxy, const std::string &md5,
  ...
  if (ok) {
    ...
    addToUserComponentLoadList(userComponentToSave->getFilename(),
                               new CLoadUserComponentSucceeded(this));
  } else {
    systemMsg(senderModuleProxy, "ERR", "", "uiR2EDServerRefuseToSave");
  }
}

bool CClientEditionModule::addToUserComponentLoadList(
    const std::string &filename,
    CUserComponentValidatorLoadSuccededCallback *cb) {
  ...
  ok = ucv->setUserComponentToLoad(filename, values, md5, signature,
                                   ClientCfg.CheckR2ScenarioMD5);
  if (!ok) {
  ...
  if (!_UserComponentToLoad.insert(std::make_pair(md5, ucv)).second) {
    nlwarning(
        "Ask 2 time the load of the same user component?"); // or the same empty
                                                            // uc file
    delete ucv;
    return false;
  }
...
void CClientEditionModule::loadUserComponentFileAccepted(
    NLNET::IModuleProxy * /* senderModuleProxy */, const std::string &md5,
    bool ok) {
  ...
  if (!ok) {
    nlwarning(
        "The server has refuse the client to load the a file"); // should (can)
                                                                // not happen
    this->systemMsg(0, "ERR", "",
                    "The server has refuse the client to load the a file");
    return;
  }
...
void CClientEditionModule::saveScenarioFileAccepted(
    NLNET::IModuleProxy *senderModuleProxy, const std::string &md5,
    const std::string &signature, bool ok) {
  // H_AUTO(R2_CClientEditionModule_saveScenarioFileAccepted)
...
bool CClientEditionModule::addToLoadList(
    const std::string &filename, CScenarioValidatorLoadSuccededCallback *cb) {
  // H_AUTO(R2_CClientEditionModule_addToLoadList)
  ...
  std::string signature;
  bool ok = true;
  ok = sv->setScenarioToLoad(filename, values, md5, signature,
                             ClientCfg.CheckR2ScenarioMD5);
  if (!ok) {
    return false;
  }
  ...
  if (!_ScenarioToLoad.insert(std::make_pair(md5, sv)).second) {
    nlwarning("Ask 2 time the load of the same scenario?"); // or the same empty
                                                            // scenario
    delete sv;
    return false;
  }
...
void CClientEditionModule::loadScenarioSucceded(
  ...
  for (uint i = 0; i < values.size(); i++) {
    ...
    else if (pair.first == "OwnerMd5")
      initialSeason = pair.second;
    else if (pair.first == "CreatorMD5")
      creatorMD5 = pair.second;
    else if (pair.first == "OtherCharAccess")
      locked = pair.second;
    else if (pair.first == "ModifierMD5")
      modifiedByMD5 = pair.second;
    else if (pair.first == "Name")
      name = pair.second;
  }

  if (ClientCfg.CheckR2ScenarioMD5 && !modifiedByMD5.empty() &&
      !locked.empty() && locked == "RunOnly") {
  ...
  R2::getEditor().getUI().displaySystemInfo(CI18N::get("uiR2EDLoadingScenario"),
                                            "BC");
  CObject *object =
      _Client->getComLuaModule().loadFromBuffer(body, filename, values);
  _LastReadHeader = values;
  if (object) {
    if (!initialIsland.empty() && !initialEntryPoint.empty() &&
        !initialSeason.empty()) {
      //			CShareServerEditionItfProxy
      //serverEditionModule(_ServerEditionProxy);
      //	 			serverEditionModule.teleportWhileUploadingScenario(this,
      //initialIsland, initialEntryPoint, initialSeason);
      R2::getEditor().clearContent();
    }
    _Client->requestUploadScenario(object);
    if (!name.empty() && object && object->isTable() &&
        object->isString("InstanceId")) {
      CObjectString objectName(name);
      _Client->requestSetNode(object->toString("InstanceId"), "Ghost_Name",
                              &objectName);
    }
  }
...
void CClientEditionModule::loadAnimationSucceded(
    const std::string &filename, const std::string &body,
    const CScenarioValidator::TValues &values) {
...
void CClientEditionModule::loadScenarioFileAccepted(
    NLNET::IModuleProxy * /* senderModuleProxy */, const std::string &md5,
    bool ok) {
  // H_AUTO(R2_CClientEditionModule_loadScenarioFileAccepted)
...
void CLoadUserComponentSucceeded::doOperation(
    const std::string &filename, const std::string &body,
    const CScenarioValidator::TValues &header) {
  CLuaState &state = R2::getEditor().getLua();
  state.push((std::string)filename);
  state.push((std::string)body);

  state.newTable();
  CClientEditionModule::TScenarioHeader::const_iterator first(header.begin()),
      last(header.end());
...
void CLoadAnimationSucceded::doOperation(
    const std::string &filename, const std::string &body,
    const CScenarioValidator::TValues &values) {
...
void CLoadScenarioSucceded::doOperation(
    const std::string &filename, const std::string &body,
    const CScenarioValidator::TValues &values) {
...
bool CClientEditionModule::hasCharacterSameCharacterIdMd5(
    const std::string &charIdMd5) const {
...
public:
  CModuleMessageSender(const NLNET::TModuleProxyPtr &proxy,
                       NLNET::IModule *senderModule)
      : _Proxy(proxy), _Sender(senderModule) {}

  void operator()(const NLNET::CMessage &msg) {
    if (!_Proxy) {
      nlwarning("Can not send Message %s to destination",
                msg.getName().c_str());
      return;
    }
    ...
    if (ClientCfg.R2EDDssNetwork == 2) {
      ...
      R2::CShareServerEditionItfProxy::buildMessageFor_forwardToDss(
          handle, charId, msg);
...
void CMessageSpliter::sendSplitedMsg(uint32 charId, const NLNET::CMessage &msg,
...

  {
    ...
    R2::CShareServerEditionItfProxy::buildMessageFor_multiPartMsgHead(
        subMsg, charId, msg.getName(), nbPacket, size);
  ...
  for (; i != nbPacket; ++i) {
    ...
    // send packetSize octet except for the last packet that just send the
    // remainings data
    uint32 dataSend =
        (i + 1) * packetSize > size ? size - packetSize * i : packetSize;

    std::vector<uint8> data(
        const_cast<uint8 *>(&buffer[i * packetSize]),
        const_cast<uint8 *>(&buffer[i * packetSize + dataSend]));
    NLNET::CMessage subMsg;
    R2::CShareServerEditionItfProxy::buildMessageFor_multiPartMsgBody(
        subMsg, charId, i, data);
  ...
  {
    NLNET::CMessage subMsg;
    R2::CShareServerEditionItfProxy::buildMessageFor_multiPartMsgFoot(subMsg,
                                                                      charId);
...
// We received first a multiPartMsgHead, then X multiPartMsgBody, then one
// multiPartMsgFoot
void CClientEditionModule::multiPartMsgHead(NLNET::IModuleProxy * /* sender */,
                                            const std::string &msgName,
                                            uint32 nbPacket, uint32 size) {
  R2::getEditor().getLua().executeScriptNoThrow(
      NLMISC::toString("r2:onMessageSendingStart('%s', %u, %u)",
                       msgName.c_str(), nbPacket, size));
}

void CClientEditionModule::multiPartMsgBody(NLNET::IModuleProxy * /* sender */,
                                            uint32 packetId,
                                            uint32 packetSize) {
  R2::getEditor().getLua().executeScriptNoThrow(NLMISC::toString(
      "r2:onMessageSendingUpdate(%u, %u)", packetId, packetSize));
}

void CClientEditionModule::multiPartMsgFoot(
    NLNET::IModuleProxy * /* sender */) {
  R2::getEditor().getLua().executeScriptNoThrow(
      NLMISC::toString("r2:onMessageSendingFinish()"));
}
