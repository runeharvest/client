...
#define CHECK_LUA_ARG_COUNT(count, funcName)                                   \
  if (args != count) {                                                         \
    nlwarning("%d args required for lua function %s.", count, funcName);       \
    return 0;                                                                  \
  }
#define CHECK_LUA_ARG_COUNT_MAX(count, funcName)                               \
  if (args > count) {                                                          \
    nlwarning("Lua function %s accept at most %d arguments.", funcName,        \
              count);                                                          \
    return 0;                                                                  \
  }
...
void CComLuaModule::initLuaLib() {
  ...
  const luaL_Reg methods[] = {
      ...
      {"requestUploadCurrentScenario",
       CComLuaModule::luaRequestUploadCurrentScenario},
      ...
      {"requestForceEndMultiAction",
       CComLuaModule::luaRequestForceEndMultiAction},
      {"requestNewPendingMultiAction",
       CComLuaModule::luaRequestNewPendingMultiAction},
      ...
      {"getEmoteBehaviorFromEmoteId",
       CComLuaModule::luaGetEmoteBehaviorFromEmoteId},
      ...
      {"getMustVerifyRingAccessWhileLoadingAnimation",
       CComLuaModule::luaGetMustVerifyRingAccessWhileLoadingAnimation},
      ...
      {"hasCharacterSameCharacterIdMd5",
       CComLuaModule::luaHasCharacterSameCharacterIdMd5},
...
bool CComLuaModule::runLuaScript(const std::string &script,
                                 std::string &erromsg) {
...
CComLuaModule *CComLuaModule::getInstance(lua_State *state) {
  ...
  std::map<lua_State *, CComLuaModule *>::const_iterator found(
      _Instance.find(state));
...
CObject *CComLuaModule::translateFeatures(CObject *hlScenario,
  ...
  if (lua_pcall(_LuaState, 1, 1, 0) != 0) {
    errorMsg =
        NLMISC::toString("error running function 'doTranslateFeatures': %s",
                         lua_tostring(_LuaState, -1));
    return 0;
  }
...
sint CComLuaModule::luaReadUserComponentFile(lua_State *state) {
  ...
  std::string ret =
      this2->_Client->getEditionModule().readUserComponentFile(filename);
...
sint CComLuaModule::luaSaveUserComponentFile(lua_State *state) {
  ...
  CComLuaModule *this2 = getInstance(state);
  this2->_Client->getEditionModule().saveUserComponentFile(filename,
                                                           mustCompress);
  return 0;
}
...
sint CComLuaModule::luaUpdateUserComponentsInfo(lua_State *state) {
  ...
  this2->_Client->getEditionModule().updateUserComponentsInfo(
      filename, name, description, timestamp, md5Id);
  return 0;
}
...
sint CComLuaModule::requestInsertNode(lua_State *state, bool isGhost) {
  ...
  CComLuaModule *this2 = getInstance(state);
  nlassert(this2);
  this2->_Client->requestInsertNode(instanceId, attrName, position, key, value);
  delete value;
  return 0;
}
...
sint CComLuaModule::luaRequestEraseNode(lua_State *state) {
  ...
  sint position = -1;
  if (args > 1) {
    attrName = lua_tostring(state, 2);
  }
  if (args > 2) {
    position = static_cast<sint>(lua_tointeger(state, 3));
  }
  CComLuaModule *this2 = getInstance(state);
  nlassert(this2);
...
sint CComLuaModule::luaRequestMoveNode(lua_State *state) {
  ...
  CComLuaModule *this2 = getInstance(state);
  nlassert(this2);
  this2->_Client->requestMoveNode(instanceId, attrName, position, instanceId2,
                                  attrName2, position2);
  return 0;
}

int CComLuaModule::luaRequestNewAction(lua_State *state, bool pending,
                                       uint count) {
  ...
  sint args = lua_gettop(state);
  const char *funcName =
      pending ? "requestNewPendingAction" : "requestNewAction";
  ...
  if (lua_gettop(state) == 1) {
    if (lua_type(state, -1) == LUA_TSTRING) {
      ...
      if (!CLuaIHM::getUCStringOnStack(ls, -1, actionName)) {
        nlwarning("<r2.%s> : ucstring or string expected as action name",
                  funcName);
        return 0;
      }
#else
  ...
  if (pending) {
    if (count > 1) {
      this2->_Client->getActionHistoric().newPendingMultiAction(actionName,
                                                                count);
    } else {
...
void CComLuaModule::setObjectToLua(lua_State *state, CObject *object) {
  ...
  if (object->isRefId()) {
    ...
    lua_pushstring(
        state,
        object->toString().c_str()); // RefIdMetatable, {}, "Value", Value
...
CObject *CComLuaModule::loadLocal(const std::string &filename,
  ...
  CScenarioValidator::TValues::const_iterator first(values.begin()),
      last(values.end());
  std::string name;
  for (; first != last; ++first) {
    if (first->first == "Name") {
...
bool CComLuaModule::load(const std::string &filename) {
  ...
  return _Client->getEditionModule().addToLoadList(
      filename, new CLoadScenarioSucceded(&_Client->getEditionModule()));
}

bool CComLuaModule::loadUserComponent(const std::string &filename) {
  // H_AUTO(R2_CComLuaModule_load)

  return _Client->getEditionModule().addToUserComponentLoadList(
      filename, new CLoadUserComponentSucceeded(&_Client->getEditionModule()));
}

CObject *
CComLuaModule::loadFromBuffer(const std::string &data,
  ...
  if (dump) {
    ...
    testNico.serialBuffer(const_cast<uint8 *>((const uint8 *)&data[0]),
                          (uint)data.size());
  }
  CObject *object = NULL;
  // TMP TMP
...
sint CComLuaModule::luaLoadAnimation(lua_State *state) {
  ...
  bool ok = this2->_Client->getEditionModule().addToLoadList(
      filename,
      new CLoadAnimationSucceded(&this2->_Client->getEditionModule()));
...
sint CComLuaModule::luaSaveUserComponent(lua_State *state) {
  ...
  if (lua_type(state, 2) == LUA_TTABLE) {
    ...
    while (lua_next(state, 2) != 0) {
      ...
      if (lua_type(state, -2) == LUA_TNUMBER &&
        ...
        nlwarning("Error while saving user component file %s",
                  filename.c_str());
      }
      lua_pop(state, 1);
    }
  ...
  this2->_Client->getEditionModule().addToUserComponentSaveList(filename,
                                                                values, body);
...
sint CComLuaModule::luaGetNamespace(lua_State *state) {
  ...
  TSessionId sessionId =
      this2->_Client->getEditionModule().getCurrentAdventureId();
...
sint CComLuaModule::luaGetUserTriggers(lua_State *state) {
  ...
  const TUserTriggerDescriptions &triggers =
      this2->_Client->getEditionModule().getUserTriggers();
...
sint CComLuaModule::luaGetRuntimeActs(lua_State *state) {
  ...
  const TActPositionDescriptions &actPositionDescriptions =
      this2->_Client->getEditionModule().getRuntimeActs();
...
sint CComLuaModule::luaGetCurrentActIndex(lua_State *state) {
  ...
  uint32 currentActIndex =
      this2->_Client->getEditionModule().getCurrentActIndex();
...
sint CComLuaModule::luaGetMaxStaticObjects(lua_State *state) {
  ...
  uint32 maxStaticObjects =
      this2->_Client->getEditionModule().getMaxStaticObjects();
...
sint CComLuaModule::luaGetEmoteBehaviorFromEmoteId(lua_State *state) {
  ...
  luaL_checktype(state, 1, LUA_TSTRING);
  std::string ret =
      this2->_Client->getEditionModule().getEmoteBehaviorFromEmoteId(
...
sint CComLuaModule::luaMustDisplayInfo(lua_State *state) {
  ...
  // this2->_Client->getEditionModule().reserveIdRange(range);
  uint32 isDisplayed =
      this2->_Client->getEditionModule().mustDisplayInfo(formName);
...
sint CComLuaModule::luaHasDisplayInfo(lua_State *state) {
  ...
  uint32 hasDisplayInfo =
      this2->_Client->getEditionModule().hasDisplayInfo(formName);
...
sint CComLuaModule::luaSetStartingActIndex(lua_State *state) {
  ...
  luaL_checktype(state, 1, LUA_TNUMBER);
  this2->_Client->getEditionModule().setStartingActIndex(
      static_cast<uint32>(lua_tointeger(state, 1)));
  return 0;
}
...
sint CComLuaModule::luaGetScenarioHeader(lua_State *state) {
  ...
  const CClientEditionModule::TScenarioHeader &header =
      this2->_Client->getEditionModule().getScenarioHeader();
...
sint CComLuaModule::luaGetIncarnatingList(lua_State *state) {
  ...
  std::vector<uint32> l =
      this2->_Client->getEditionModule().getIncarnatingList();
...
sint CComLuaModule::luaGetSheetRingAccess(lua_State *state) {
  ...
  std::string ringAccess =
      CRingAccess::getInstance().getSheetAccessInfo(sheetClient, sheetServer);
...
sint CComLuaModule::luaGetIslandRingAccess(lua_State *state) {
  ...
  std::string ringAccess =
      CRingAccess::getInstance().getIslandAccessInfo(island);
...
sint CComLuaModule::luaUpdateScenarioAck(lua_State *state) {
  ...
  if (object) {
    ...
    std::map<std::string, int>::const_iterator first(level.begin()),
        last(level.end());
    for (; first != last; ++first) {
    ...
    this2->_Client->getEditionModule().updateScenarioRingAccess(ok, str,
                                                                errMsg);
  }
...
sint CComLuaModule::luaGetCharacterRingAccess(lua_State *state) {
  ...
  std::string access =
      this2->_Client->getEditionModule().getCharacterRingAccess();
...
sint CComLuaModule::luaGetRingAccessAsMap(lua_State *state) {
  ...
  lua_newtable(state);
  std::map<std::string, int>::const_iterator first(accessMap.begin()),
      last(accessMap.end());
  for (; first != last; ++first) {
    lua_pushstring(state, first->first.c_str());
...
sint CComLuaModule::luaVerifyRtScenario(lua_State *state) {
  ...
  std::string charRingAccess =
      this2->_Client->getEditionModule().getCharacterRingAccess();
  bool ok =
      CRingAccess::getInstance().verifyRtScenario(object, charRingAccess, err);
  ...
  switch (err->Type) {
  case CVerfiyRightRtScenarioError::InvalidData:
    lua_pushstring(state, "InvalidData");
    break;
  case CVerfiyRightRtScenarioError::InvalidIslandLevel:
    lua_pushstring(state, "InvalidIslandLevel");
    break;
  case CVerfiyRightRtScenarioError::InvalidBotLevel:
    lua_pushstring(state, "InvalidBotLevel");
    break;
  default:
...
sint CComLuaModule::luaGetEditSessionLink(lua_State *state) {
  ...
  nlassert(this2);
  TSessionId sessionId =
      this2->_Client->getEditionModule().getEditSessionLink();
...
sint CComLuaModule::luaGetMustVerifyRingAccessWhileLoadingAnimation(
    lua_State *state) {
  // H_AUTO(R2_CComLuaModule_luaGetMustVerifyRingAccessWhileLoadingAnimation)
  lua_pushboolean(state,
                  ClientCfg.R2EDMustVerifyRingAccessWhileLoadingAnimation);
  return 1;
}
...
sint CComLuaModule::luaGetIsAnimationSession(lua_State *state) {
  ...
  lua_pushboolean(state, this2->_Client->getEditionModule().getSessionType() ==
                             st_anim);
  return 1;
}
...
sint CComLuaModule::luaHasCharacterSameCharacterIdMd5(lua_State *state) {
  ...
  bool ok =
      this2->_Client->getEditionModule().hasCharacterSameCharacterIdMd5(charId);
  lua_pushboolean(state, ok);
  return 1;
}
