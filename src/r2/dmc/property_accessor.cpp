...
CPropertyAccessor::~CPropertyAccessor() { purgeShadowedValues(); }

CObject *CPropertyAccessor::getPropertyValue(CObject *component,
                                             const std::string &attrName) {
  // H_AUTO(R2_CPropertyAccessor_getPropertyValue)
  return const_cast<CObject *>(
      getPropertyValue((const CObject *)component, attrName));
}

double CPropertyAccessor::getValueAsNumber(CObject *component,
                                           const std::string &attrName) const {
  // H_AUTO(R2_CPropertyAccessor_getValueAsNumber)
  const CObject *object =
      getPropertyValue((const CObject *)component, attrName);
  if (!object || !object->isNumber()) {
    return 0;
  }
  return object->toNumber();
}

sint64 CPropertyAccessor::getValueAsInteger(CObject *component,
                                            const std::string &attrName) const {
  // H_AUTO(R2_CPropertyAccessor_getValueAsInteger)
  const CObject *object =
      getPropertyValue((const CObject *)component, attrName);
  if (!object || !object->isInteger()) {
    return 0;
  }
  return object->toInteger();
}

bool CPropertyAccessor::hasValueInBase(CObject *component,
                                       const std::string &attrName) {
...
const CObject *
CPropertyAccessor::getPropertyValue(const CObject *componentParam,
                                    const std::string &attrName) const {
  // H_AUTO(R2_CPropertyAccessor_getPropertyValue)
  ...
  while (!toRet) {
    ...
    if (base && base->isString()) {
      ...
      if (!baseElement) {
        nlwarning("Can't find base palette element : name = %s",
                  strBase.c_str());
        return NULL;
      }
  ...
  if (toRet) {
    ...
    for (std::vector<CShadowedValue>::const_iterator it =
             _ShadowedValues.begin();
         it != _ShadowedValues.end(); ++it) {
...
void CPropertyAccessor::getPropertyList(CObject *component,
...
void CPropertyAccessor::shadowValue(CObject *shadowedValue,
                                    CObject *localValue) {
...
void CPropertyAccessor::commitValue(CObject *shadowedValue) {
  ...
  for (uint k = 0; k < _ShadowedValues.size(); ++k) {
    if (_ShadowedValues[k].ShadowedValue == shadowedValue) {
      // NB Nico : removed the local copy below because of the new 'instant
      // feddback' mechanism plus, this introduced a bug where the server
      // command was not sent (because new value & local value were equal)

      // copy local value to shadowed value
      //_ShadowedValues[k].ShadowedValue->inPlaceCopy(*_ShadowedValues[k].LocalValue);
      _ShadowedValues[k].ShadowedValue =
          NULL; // delete job done by purgeShadowedValues
      break;
    }
  }
  purgeShadowedValues();
...
struct CDeadShadowedObjectReferenceTest {
  bool operator()(const CPropertyAccessor::CShadowedValue &sw) const {
    return sw.ShadowedValue == NULL;
  }
};
...
void CPropertyAccessor::purgeShadowedValues() {
  ...
  _ShadowedValues.erase(std::remove_if(_ShadowedValues.begin(),
                                       _ShadowedValues.end(),
                                       CDeadShadowedObjectReferenceTest()),
                        _ShadowedValues.end());
}
