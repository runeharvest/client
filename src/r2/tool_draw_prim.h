...
private:
  ...
  CPrimRender _InaccessiblePrim; // inaccessible parts for the primitive being
                                 // drawn (on an invalid pos on heightmap)
  CPrimLook _PrimLook;
  ...
  std::string _CookieKey; // cookie to be attached if creation succeed (empty
                          // string if no cookie)
  CLuaObject _CookieValue;
  CLuaObject _CancelFunc;
  bool _SelectInstance;
  CInstance::TRefPtr
      _ExtendedPrimitive; // road being extended (NULL is this tool is used to
...
private:
  ...
  // for closed polygon only : test if the current shape is valid (e.g not
  // intersecting, with at least 3 points)
  bool isValidPolyShape(bool ignoreLast,
                        std::list<NLMISC::CPolygon> &splitPoly) const;
