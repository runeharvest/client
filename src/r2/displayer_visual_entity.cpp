...
void CDisplayerVisualEntity::onSelect(bool selected) {
  ...
  if (selected && selected != alreadySelected) {
    if (_Entity) {
      // force to snap entity to ground here, because pos is bad when calling
      // "getSelectionDecalRadius" and local bbox cache is updated with wrong
      // value
...
void CDisplayerVisualEntity::updateVisibility() {
  ...
  if (!getActive() || !_Entity)
    return; // not active, so ignore 'VisibleFlag'
...
void CDisplayerVisualEntity::showSelected() {
  ...
  if (!_Entity || getActualSelectionDisplayMode() == CircleSelection) {
    ...
    nlassert(getSelectionType() == LocalSelectBox ||
             getSelectionType() == GroundProjected);
    getEditor().showSelectBox(
        getSelectBox(),
        getInvertedMatrix()
            .inverted() /*, getBlinkColor(CV_SelectedInstanceColor.get())*/);
  }
}
...
void CDisplayerVisualEntity::showHighlighted() {
  ...
  if (!_Entity || getActualSelectionDisplayMode() == CircleSelection) {
    ...
    getEditor().showHighlightBox(
        getSelectBox(),
        getInvertedMatrix()
            .inverted() /*, getBlinkColor(CV_FocusedInstanceColor.get())*/);
  }
}

// *********************************************************************************************************
void CDisplayerVisualEntity::setVisualSelectionBlink(bool on,
                                                     NLMISC::CRGBA color) {
  ...
  if (_PlaceHolder) {
    ...
    if (!_Entity) {
      if (getDisplayFlag(FlagSelected)) {
        setVisualSelectionBlink(true,
                                getBlinkColor(CV_SelectedInstanceColor.get()));
        showSelected();

      } else if (getDisplayFlag(FlagHasFocus)) {
        setVisualSelectionBlink(true,
                                getBlinkColor(CV_FocusedInstanceColor.get()));
        showHighlighted();
      }
      return;
    }
  ...
  if (!_Entity->isAsyncLoading()) {
    if (getActualDisplayMode() == DisplayModeVisible) {
      if (getDisplayFlag(FlagSelected)) {
        ...
        setVisualSelectionBlink(
            false, getBlinkColor(CV_UnselectedInstanceColor.get()));
      }
      ...
      uint32 oldOpacity = CEntityCL::getOpacityMin();
      CEntityCL::setOpacityMin(
          (uint32)(DEFAULT_ENTITY_MIN_OPACITY * _ClipBlend));
...
// *********************************************************************************************************
void CDisplayerVisualEntity::drawBBox(const NLMISC::CMatrix &modelMatrix,
                                      const NLMISC::CAABBox &bbox,
                                      NLMISC::CRGBA colOverZ,
                                      NLMISC::CRGBA colUnderZ) {
  // H_AUTO(R2_CDisplayerVisualEntity_drawBBox)
  // nlwarning("bbox = (%f, %f, %f) - (%f, %f, %f), color = (%d, %d, %d, %d)",
  // bbox.getMin().x, bbox.getMin().y, bbox.getMin().z, 												bbox.getMax().x,
  //bbox.getMax().y, bbox.getMax().z, (int) colOverZ.R, (int) colOverZ.G, (int)
  //colOverZ.B, (int) colOverZ.A);
...
// *********************************************************************************************************
void CDisplayerVisualEntity::drawBBox(NLMISC::CRGBA colOverZ,
                                      NLMISC::CRGBA colUnderZ) {
  // H_AUTO(R2_CDisplayerVisualEntity_drawBBox)
  if (!_Entity)
    return;
  ...
  case CircleSelection: {
    ...
    drawBBox(modelMatrix, getEditor().getLocalSelectBox(*_Entity), colOverZ,
             colUnderZ);
    // draw world version
    colOverZ.avg2(colOverZ, CRGBA::Black);
    colUnderZ.avg2(colUnderZ, CRGBA::Black);
    drawBBox(CMatrix::Identity, getEditor().getSelectBox(*_Entity), colOverZ,
             colUnderZ);
  } break;
  case BoxSelection: {
    // uses local version
...
void CDisplayerVisualEntity::onPostRender() {
  if (_PlaceHolder) {
    // _PlaceHolder->onPostRender();  // do not post render here becauseit would
    // set transparency again, bypassing the changed in 'setClipBlend'
  }
...
void CDisplayerVisualEntity::setClipBlend(float amount) {
  ...
  if (amount == 1.f) {
    ...
    if (!_PlaceHolder) {
      ...
      _PlaceHolder = new CDisplayerVisualShape("r2_entity_place_holder.shape",
                                               scale, false);
      ...
      _PlaceHolder->onPreRender(); // force to create the instance so we can
                                   // make it transparent at start
    }
    if (_PlaceHolder && !_PlaceHolder->getMesh().empty()) {
      makeInstanceTransparent(_PlaceHolder->getMesh(),
                              (uint8)(255.f * (1.f - amount)), amount > 0.5f);
    }
  }
}
...
void CDisplayerVisualEntity::createEntity() {
  ...
  for (; k < 255; ++k) // find an empty slot
  {
    if (!EntitiesMngr.entity(k)) {
      nlwarning("Creating entity in slot %d with sheet %s at position %s with "
                "angle %f",
                (int)k, getSheet().c_str(),
                toString(getPos().asVector()).c_str(), getAngle());
      _Entity =
          CEditor::createEntity(k, NLMISC::CSheetId(getSheet()), getWorldPos(),
                                getAngle(), getPermanentStatutIcon());
  ...
  if (!_Entity) {
    nlwarning("Entity creation failed. This would normally not happeneds. "
              "Sheet '%s', slot %u ",
              getSheet().c_str(), k);
    return;
  }
...
void CDisplayerVisualEntity::updateWorldMapPresence() {
  ...
  R2::CScenarioEntryPoints::CCompleteIsland *currIsland =
      col.getCurrIslandDesc();
  if (currIsland) {
    ...
    if (!_MapDeco.isAddedToMap()) {
      ...
      if (gm) {
        ...
        std::string texName;
        {
          ...
          if (getDisplayedInstance()
                  ->getLuaProjection()
                  .callMethodByNameNoThrow("getSelectBarIcon", 0, 1)) {
...
// *********************************************************************************************************
bool CDisplayerVisualEntity::getActive() const { return _RegisteredInEditor; }
...
GR->retrievePosition(getWorldPos().asVector()); setDisplayFlag(FlagBadPos,
gpos.InstanceId == -1);
}*/
...
bool CDisplayerVisualEntity::isUserCreated() const {
  return getDisplayedInstance()
             ->getLuaProjection()["User"]["Select"]
             .toBoolean() == true;
}

// *********************************************************************************************************
void CDisplayerVisualEntity::onAttrModified(const std::string &name,
                                            sint32 index) {
  if (_PlaceHolder) {
  ...
  if (name == "Angle") {
    ...
    updateName();
  } else if (name == "GabaritHeight" || name == "GabaritTorsoWidth" ||
             name == "GabaritArmsWidth" || name == "GabaritLegsWidth" ||
             name == "GabaritBreastSize" || name == "HairType" ||
             name == "HairColor" || name == "Tattoo" || name == "EyesColor" ||
             name == "MorphTarget1" || name == "MorphTarget2" ||
             name == "MorphTarget3" || name == "MorphTarget4" ||
             name == "MorphTarget5" || name == "MorphTarget6" ||
             name == "MorphTarget7" || name == "MorphTarget8" ||
             name == "JacketModel" || name == "TrouserModel" ||
             name == "FeetModel" || name == "HandsModel" ||
             name == "ArmModel" || name == "WeaponRightHand" ||
             name == "WeaponLeftHand" || name == "JacketColor" ||
             name == "ArmColor" || name == "HandsColor" ||
...
void CDisplayerVisualEntity::updateRaceAndSex() {
  ...
  _Entity = getEditor().createEntity(_Entity->slot(), CSheetId(sheetClient),
                                     getPos().asVector(), getAngle());
  updateEntity();

  // update sex
  SPropVisualA vA;
  const string propNameA = toString("SERVER:Entities:E%d:P%d", _Entity->slot(),
  ...
  EntitiesMngr.updateVisualProperty(0, _Entity->slot(),
                                    CLFECOMMON::PROPERTY_VPA);
}
...
void CDisplayerVisualEntity::updateEntity() {
  ...
  SPropVisualC vC;
  {
    ...
    const string propNameA = toString(
        "SERVER:Entities:E%d:P%d", _Entity->slot(), CLFECOMMON::PROPERTY_VPA);
    const string propNameB = toString(
        "SERVER:Entities:E%d:P%d", _Entity->slot(), CLFECOMMON::PROPERTY_VPB);
    const string propNameC = toString(
        "SERVER:Entities:E%d:P%d", _Entity->slot(), CLFECOMMON::PROPERTY_VPC);
    ...
    vC.PropertySubData.CharacterHeight =
        (uint)getNumber(&getProps(), "GabaritHeight");
    vC.PropertySubData.ArmsWidth =
        (uint)getNumber(&getProps(), "GabaritArmsWidth");
    vC.PropertySubData.TorsoWidth =
        (uint)getNumber(&getProps(), "GabaritTorsoWidth");
    vC.PropertySubData.LegsWidth =
        (uint)getNumber(&getProps(), "GabaritLegsWidth");
    vC.PropertySubData.BreastSize =
        (uint)getNumber(&getProps(), "GabaritBreastSize");
    ...
    vC.PropertySubData.MorphTarget1 =
        (uint)getNumber(&getProps(), "MorphTarget1");
    vC.PropertySubData.MorphTarget2 =
        (uint)getNumber(&getProps(), "MorphTarget2");
    vC.PropertySubData.MorphTarget3 =
        (uint)getNumber(&getProps(), "MorphTarget3");
    vC.PropertySubData.MorphTarget4 =
        (uint)getNumber(&getProps(), "MorphTarget4");
    vC.PropertySubData.MorphTarget5 =
        (uint)getNumber(&getProps(), "MorphTarget5");
    vC.PropertySubData.MorphTarget6 =
        (uint)getNumber(&getProps(), "MorphTarget6");
    vC.PropertySubData.MorphTarget7 =
        (uint)getNumber(&getProps(), "MorphTarget7");
    vC.PropertySubData.MorphTarget8 =
        (uint)getNumber(&getProps(), "MorphTarget8");
    ...
    vA.PropertySubData.JacketColor =
        (uint)getNumber(&getProps(), "JacketColor");
    vA.PropertySubData.TrouserColor =
        (uint)getNumber(&getProps(), "TrouserColor");
...
void CDisplayerVisualEntity::updateName() {
  ...
  // If entity is in an additionnal act, then postfix its name with the name of
  // the act
  if (getDisplayedInstance()->getParentAct() != getEditor().getBaseAct()) {
    R2::CObjectTable *act =
        getDisplayedInstance()->getParentAct()->getObjectTable();
    ...
    if (actNb > 0)
      firstPart =
          CI18N::get("uiR2EDDefaultActTitle") + " " + NLMISC::toString(actNb);
...
// *********************************************************************************************************
void CDisplayerVisualEntity::updatePermanentStatutIcon(
...
void CDisplayerVisualEntity::updateEntityRot() {
  ...
  _Entity->front(CVector((float)cos(angle), (float)sin(angle), 0.f), true, true,
                 true);
  _Entity->dir(_Entity->front(), false, false);
  NLPACS::UMovePrimitive *prim = _Entity->getPrimitive();
  if (prim &&
      prim->getPrimitiveType() == NLPACS::UMovePrimitive::_2DOrientedBox) {
...
std::string CDisplayerVisualEntity::getVisualProperties() const {
  ...
  const std::string propNameA = toString(
      "SERVER:Entities:E%d:P%d", _Entity->slot(), CLFECOMMON::PROPERTY_VPA);
  const std::string propNameB = toString(
      "SERVER:Entities:E%d:P%d", _Entity->slot(), CLFECOMMON::PROPERTY_VPB);
  const std::string propNameC = toString(
      "SERVER:Entities:E%d:P%d", _Entity->slot(), CLFECOMMON::PROPERTY_VPC);
  ...
  const std::string strVPABC = NLMISC::toString(
      "VPA:%016.16" NL_I64 "x\nVPB:%016.16" NL_I64 "x\nVPC:%016.16" NL_I64 "x",
      uVPA, uVPB, uVPC);
...
if (_Entity->skeleton())
{
        sint boneId = _Entity->skeleton()->getBoneIdByName(leader ? "Bip01 Head"
: "Bip01 Spine1"); if (boneId != -1)
        {
...
bone.getLastWorldMatrixComputed(); return worldMat.getPos();
        }
}
*/
...
bool CDisplayerVisualEntity::getLastClip() const {
  ...
  if (!_Entity) {
    return _PlaceHolder ==
           NULL; // is there's a place holder consider entity to be not clipped
  }
  if (getActualSelectionDisplayMode() == BoxSelection)
    return false;
...
// *********************************************************************************************************
float CDisplayerVisualEntity::preciseIntersectionTest(
  ...
  return CEditor::preciseEntityIntersectionTest(*_Entity, worldRayStart,
                                                worldRayDir);
}
...
// *********************************************************************************************************
bool CDisplayerVisualEntity::isInProjection(
  ...
  if (localPos.x < pmin.x || localPos.x > pmax.x || localPos.y < pmin.y ||
      localPos.y > pmax.y)
    return false;
  return true;
}
...
// *********************************************************************************************************
ISelectableObject::TSelectionType
CDisplayerVisualEntity::getSelectionType() const {
  if (!_Entity && _PlaceHolder)
    return _PlaceHolder->getSelectionType();
  // H_AUTO(R2_ISelectableObject_TSelectionType )
  //  If box selection was explicitly asked (not auto mode), uses ground
  //  selection only
  if (_SelectionDisplayMode == BoxSelection)
    return ISelectableObject::GroundProjected;
  // if there's a cylinder primitive, use world box selection, else use local
  // box (for obstacle like barriers)
  return getActualSelectionDisplayMode() == BoxSelection
             ? ISelectableObject::LocalSelectBox
             : ISelectableObject::WorldSelectBox;
}
...
// *********************************************************************************************************
CDisplayerVisualEntity::TSelectionDisplayMode
CDisplayerVisualEntity::getActualSelectionDisplayMode() const {
  ...
  //  If the collision primitive is a rectangle, always display selection as a
  //  rectangle on ground If the collision primitive is a cylinder, use that
  //  cylinder to display that selection on the ground, unless it is too big. In
  //  this case ,uses a box instead
  static volatile float maxRadiusForCircleSelection = 2.f;
  switch (_SelectionDisplayMode) {
  case AutoSelection: {
    float radius = getCylinderRadius();
    if (radius == -1.f || radius >= maxRadiusForCircleSelection)
      return BoxSelection;
...
NLMISC_COMMAND(toggleR2EntityBoxes,
               "toggle display bounding boxes for selected R2 entities", "") {
  if (!args.empty())
    return false;
...
NLMISC_COMMAND(showR2EntityBoxes,
               "display bounding boxes for selected R2 entities", "") {
  if (!args.empty())
    return false;
...
NLMISC_COMMAND(hideR2EntityBoxes,
               "display bounding boxes for selected R2 entities", "") {
  if (!args.empty())
    return false;
