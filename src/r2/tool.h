 ...
 * There's only one tool at a moment and mouse/keyboard events are routed to
 * that tool
 *
...
public:
  ...
  /** Get this tool name in the ui. This name is used to identify the tool in
   * the ui (used by the r2.ToolUI:setActiveToolUIByName function defined in
   * r2ed_ui.lua) May return "" if there's no ui associated with that tool
   */
  ...
  //
  // This update is called at each frame for the current tool just before
  // rendering
  virtual void updateBeforeRender() {}
  // This update is called at each frame for the current tool just after
  // rendering
   ...
   * Default behaviour of this member function is to forward the event to these
  ...
  } // the app window gained the focus (there's no 'focus lost' event here
    // because it reset current tool, so 'CTooll::cancel' will be called
    // instead) IMPORTANT :  Reacting to this should be unnecessary, as lost
    // focus reset the current tool, defaulting to the 'SelectMove' tool, that
  ...
  /**
   * Unlike the other onMousexxx method, these are actually called AFTER the
   * camera event handling has been done. Doing the same in onMouseRightButtonUp
   * or onMouseRightButtonDown instead would require that the user test if mouse
   * button was not released after a camera move (in which case the click should
   * not be handled of course)
   *
   * Usually the onMousexxxButtonUp are ususeful when working in pair with the
   * associated 'mouse button down' event. Such events usually provoke a mouse
   * capture (example : selectMoveTool)
   */
  ...
  // 'startDoubleClickCheck' should be called when the
  // 'onMouseLeftButtonClicked' msg is handled then checkDoubleClick'  should be
  // call on any subsequent onMouseLeftButtonDown. If the result is true then
   ...
   * \param miniMapHandle if not NULL, pointer will be filled with the minimap
   * ui element that is under the mouse \return NULL if there's no instance
   * under the mouse
   */
  static CInstance *
  checkInstanceUnderMouse(IDisplayerUIHandle **miniMapHandle = NULL);

  // helper : handle mouse over instance: highlight them when mouse is over &
   ...
   * - Maintaining the left button down doesn't trigger camera rotation any
   * more. Useful for tools such as 'select', 'move' ...
   */
  ...
  /** Compute a view vector (with its direction z set to 1) from coordinate of
   * the mouse on screen If the mouse is on the island map, then a vector
   * looking down from heights will be returned
   */
  static void computeWorldViewRay(sint32 posX, sint32 posY,
                                  CWorldViewRay &dest);
  // specific test for the world map
  static TRayIntersectionType
  computeWorldMapIntersection(float x, float y, NLMISC::CVector &inter);
   ...
   * \param inter If return type is different from 'NoIntersection', then
   * 'inter' is filled with the collision position
   */
  static TRayIntersectionType
  computeLandscapeRayIntersection(const CWorldViewRay &worldViewRay,
                                  NLMISC::CVector &inter);
  // Get pacs type at the given position, with the given threshold.
  static TRayIntersectionType getPacsType(const NLMISC::CVector &pos,
                                          float threshold,
  ...
  /** handle world map auto-panning feature, should be called whenever auto-pan
   * should be done dx & dy are filled with delta of the map for this frame
   */
...
private:
  /** compute the nearest valid surface at a given position from the island
   * heightmap (heightmap must not be empty or an assertion is raised) \return
   * true if a valid surface was found
   */
  static bool computeNearestValidSurfaceFromHeightMap(float x, float y,
                                                      NLMISC::CVector &inter);
  // trace a ray though the scene, using precise camera collision first, island
  // packed collisions then.
  static bool raytrace(const NLMISC::CVector &segmentStart,
                       const NLMISC::CVector &dir, NLMISC::CVector &inter);
  static bool
  isIslandValidPos(const NLMISC::CArray2D<sint16> &heightMap,
