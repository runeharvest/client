...
// ***************************************************************
void CAutoGroup::update(const CVector &testPos, const std::string &paletteEntry,
                        bool valid) {
  ...
  if (!candidate && _AutoGroup.isAddedToWorldMap()) {
    ...
    CVector pos =
        dv->isCompound() ? dv->getSon(0)->getWorldPos() : dv->getWorldPos();
...
CInstance *CAutoGroup::getGroupingCandidate() {
  ...
  if (!palEntry || !palEntry->isTable())
    return NULL;
  if (getNumber(palEntry, "IsBotObject") == 1)
    return NULL;
  // auto-group feature
  // look in default feature and sort objects by distance
  CInstance *defaultFeatInst =
      getEditor().getDefaultFeature(getEditor().getCurrentAct());
  CInstance *baseDefaultFeatInst =
      getEditor().getDefaultFeature(getEditor().getBaseAct());
  if (!defaultFeatInst || !baseDefaultFeatInst) {
    nlwarning("Can't access to Default Features"); // syntax error in lua was
                                                   // making the client crash
  ...
  if (!components || !baseComponents || !palEntry->isTable())
    return NULL;
  _SortedComponents.clear();
  for (uint k = 0; k < (components->getSize() + baseComponents->getSize());
       ++k) {
    CObject *obj = NULL;
    ...
    if (cs.Dist > CV_AutoGroupMaxDist.get())
      continue;
    cs.Instance = inst;
  ...
  if (subCategoryObj && subCategoryObj->isString()) {
    ...
    // nlwarning("No 'SubCategory' field in palEntry '%s'",
    // paletteEntry.c_str());
  }
  ...
  for (uint k = 0; k < _SortedComponents.size(); ++k) {
    ...
    if (_SortedComponents[k].Instance->isKindOf("Npc")) {
      _SortedComponents[k].Instance->getLuaProjection().callMethodByNameNoThrow(
          "isPlant", 0, 1);
      if (ls.toBoolean(-1) == true)
        continue;
      _SortedComponents[k].Instance->getLuaProjection().callMethodByNameNoThrow(
          "isBotObject", 0, 1);
      if (ls.toBoolean(-1) == true)
    ...
    if (_SortedComponents[k]
            .Instance->getLuaProjection()
            .callMethodByNameNoThrow("getCategory", 0, 1)) {
    ...
    if (_SortedComponents[k]
            .Instance->getLuaProjection()
            .callMethodByNameNoThrow("getSubCategory", 0, 1)) {
      if (ls.isString(-1)) {
        destSubCategory = ls.toString(-1);
      }
      ls.pop();
    }
    if (destSubCategory != subCategory)
      continue;
    // good candidate
...
// ***************************************************************
void CAutoGroup::group(CObject *newEntityDesc,
                       const NLMISC::CVectorD &createPosition) {
  // H_AUTO(R2_CAutoGroup_group)
  CInstance *destGroup = getGroupingCandidate();
  if (!destGroup || !_AutoGroupEnabled)
    return;
  ...
  if (destGroup->isKindOf("NpcGrpFeature")) {
    ...
    CUniquePtr<CObject> newGroup(
        getEditor().getDMC().newComponent("NpcGrpFeature"));
    if (!newGroup.get()) {
    ...
    CLuaState &ls = getEditor().getLua();
    R2::getEditor()
        .getEnv()["PaletteIdToGroupTranslation"]
                 [newEntityDesc->getAttr("Base")->toString()]
        .push();
    if (ls.isString(-1))
    ...
    getEditor().getDMC().requestInsertNode(destGroup->getParentAct()->getId(),
                                           "Features", -1, "", newGroup.get());
    targetGroupId = getString(newGroup.get(), "InstanceId");
    // move target instance in that group (becomes the leader)
    getEditor().getDMC().requestMoveNode(destGroup->getId(), "", -1,
                                         targetGroupId, "Components", -1);
  }
  // move newly created entity into target group
  getEditor().getDMC().requestInsertNode(targetGroupId, "Components", -1, "",
                                         newEntityDesc);
  getEditor().getDMC().getActionHistoric().endAction();
  getEditor().getDMC().flushActions();
}
