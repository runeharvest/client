...
// *********************************************************************************************************
/*static*/ void setDisplayedInstance(CDisplayerBase *displayer,
...
void CInstance::visit(IInstanceVisitor &visitor) {
  ...
  const_cast<CObjectTable *>(_ObjectTable)
      ->visit(instanceVisitor); // 'const' is respected here, because
                                // CInstanceVisitor gives a reference to
                                // CInstance which in turn only allow a const
                                // reference on CObjectTable
  TEST_LUA_PROJ;
}
...
CDisplayerVisual *CInstance::getDisplayerVisual() const {
  ...
  if (!_DisplayerVisual)
    return NULL;
  return NLMISC::safe_cast<CDisplayerVisual *>(
      (CDisplayerBase *)_DisplayerVisual);
}
...
void CInstance::onPreActChanged() {
  ...
  if (_DisplayerUI)
    _DisplayerUI->onPreActChanged();
  if (_DisplayerProperties)
    _DisplayerProperties->onPreActChanged();
  _ToLua.onActChanged();
  TEST_LUA_PROJ;
}
...
void CInstance::onActChanged() {
  ...
  if (_DisplayerUI)
    _DisplayerUI->onActChanged();
  if (_DisplayerProperties)
    _DisplayerProperties->onActChanged();
  _ToLua.onActChanged();
  TEST_LUA_PROJ;
...
void CInstance::onContinentChanged() {
  ...
  if (_DisplayerUI)
    _DisplayerUI->onContinentChanged();
  if (_DisplayerProperties)
    _DisplayerProperties->onContinentChanged();
...
void CInstance::onCreate() {
  ...
  if (_DisplayerUI)
    _DisplayerUI->onCreate();
  if (_DisplayerProperties)
    _DisplayerProperties->onCreate();
  _ToLua.onCreate();
  TEST_LUA_PROJ;
}
...
void CInstance::onPostCreate() {
  ...
  if (_DisplayerUI)
    _DisplayerUI->onPostCreate();
  if (_DisplayerProperties)
    _DisplayerProperties->onPostCreate();
  _ToLua.onPostCreate();
  TEST_LUA_PROJ;
...
void CInstance::onErase() {
  ...
  if (_DisplayerUI)
    _DisplayerUI->onErase();
  if (_DisplayerProperties)
    _DisplayerProperties->onErase();
  _ToLua.onErase();
  TEST_LUA_PROJ;
...
void CInstance::onPreHrcMove() {
  ...
  if (_DisplayerUI)
    _DisplayerUI->onPreHrcMove();
  if (_DisplayerProperties)
    _DisplayerProperties->onPreHrcMove();
  _ToLua.onPreHrcMove();
  TEST_LUA_PROJ;
}
...
void CInstance::onPostHrcMove() {
  ...
  if (_DisplayerUI)
    _DisplayerUI->onPostHrcMove();
  if (_DisplayerProperties)
    _DisplayerProperties->onPostHrcMove();
...
void CInstance::onFocus(bool highlighted) {
  ...
  if (_DisplayerUI)
    _DisplayerUI->onFocus(highlighted);
  if (_DisplayerProperties)
    _DisplayerProperties->onFocus(highlighted);
...
void CInstance::onSelect(bool selected) {
  ...
  if (_DisplayerUI)
    _DisplayerUI->onSelect(selected);
  if (_DisplayerProperties)
    _DisplayerProperties->onSelect(selected);
...
void CInstance::onAttrModified(const std::string &attrName, sint32 attrIndex) {
  ...
  if (attrName == "InstanceId") {
    nlwarning("InstanceId modification not allowed !! Please create a new "
              "object with a new id");
    _Id.clear(); // nevertheless invalidate the cache
  }
  if (_DisplayerVisual)
    getDisplayerVisual()->onAttrModified(attrName, attrIndex);
  if (_DisplayerUI)
    _DisplayerUI->onAttrModified(attrName, attrIndex);
  if (_DisplayerProperties)
    _DisplayerProperties->onAttrModified(attrName, attrIndex);
...
// *********************************************************************************************************
/*void CInstance::onTableModified(const std::string &tableName, const
std::string &keyInTable, sint32 indexInTable)
{
        if (_DisplayerVisual) getDisplayerVisual()->onTableModified(tableName,
keyInTable, indexInTable); if (_DisplayerUI)
_DisplayerUI->onTableModified(tableName, keyInTable, indexInTable); if
(_DisplayerProperties) _DisplayerProperties->onTableModified(tableName,
keyInTable, indexInTable);
}*/

// *********************************************************************************************************
void CInstance::onTargetInstanceCreated(const std::string &refMakerAttr,
  ...
  if (_DisplayerVisual)
    getDisplayerVisual()->onTargetInstanceCreated(refMakerAttr,
                                                  refMakerAttrIndex);
  if (_DisplayerUI)
    _DisplayerUI->onTargetInstanceCreated(refMakerAttr, refMakerAttrIndex);
  if (_DisplayerProperties)
    _DisplayerProperties->onTargetInstanceCreated(refMakerAttr,
                                                  refMakerAttrIndex);
...
// *********************************************************************************************************
void CInstance::onTargetInstanceErased(const std::string &refMakerAttr,
                                       sint32 refMakerAttrIndex) {
  ...
  if (_DisplayerVisual)
    getDisplayerVisual()->onTargetInstanceErased(refMakerAttr,
                                                 refMakerAttrIndex);
  if (_DisplayerUI)
    _DisplayerUI->onTargetInstanceErased(refMakerAttr, refMakerAttrIndex);
  if (_DisplayerProperties)
    _DisplayerProperties->onTargetInstanceErased(refMakerAttr,
                                                 refMakerAttrIndex);
...
// *********************************************************************************************************
void CInstance::onTargetInstancePreHrcMove(const std::string &refMakerAttr,
                                           sint32 refMakerAttrIndex) {
  ...
  if (_DisplayerVisual)
    getDisplayerVisual()->onTargetInstancePreHrcMove(refMakerAttr,
                                                     refMakerAttrIndex);
  if (_DisplayerUI)
    _DisplayerUI->onTargetInstancePreHrcMove(refMakerAttr, refMakerAttrIndex);
  if (_DisplayerProperties)
    _DisplayerProperties->onTargetInstancePreHrcMove(refMakerAttr,
                                                     refMakerAttrIndex);
...
// *********************************************************************************************************
void CInstance::onTargetInstancePostHrcMove(const std::string &refMakerAttr,
                                            sint32 refMakerAttrIndex) {
  ...
  if (_DisplayerVisual)
    getDisplayerVisual()->onTargetInstancePostHrcMove(refMakerAttr,
                                                      refMakerAttrIndex);
  if (_DisplayerUI)
    _DisplayerUI->onTargetInstancePostHrcMove(refMakerAttr, refMakerAttrIndex);
  if (_DisplayerProperties)
    _DisplayerProperties->onTargetInstancePostHrcMove(refMakerAttr,
                                                      refMakerAttrIndex);
...
// *********************************************************************************************************
void CInstance::onTargetInstanceEraseRequested(const std::string &refMakerAttr,
                                               sint32 refMakerAttrIndex) {
  // H_AUTO(R2_CInstance_onTargetInstanceEraseRequested)
  TEST_LUA_PROJ;
  // forward to displayer for convenience & legacy code, but should not be
  // handled by displayers ...
  if (_DisplayerVisual)
    getDisplayerVisual()->onTargetInstanceEraseRequested(refMakerAttr,
                                                         refMakerAttrIndex);
  if (_DisplayerUI)
    _DisplayerUI->onTargetInstanceEraseRequested(refMakerAttr,
                                                 refMakerAttrIndex);
  if (_DisplayerProperties)
    _DisplayerProperties->onTargetInstanceEraseRequested(refMakerAttr,
                                                         refMakerAttrIndex);
  //
...
// *********************************************************************************************************
void CInstance::onTargetInstanceAttrModified(const std::string &refMakerAttr,
                                             sint32 refMakerAttrIndex,
  ...
  TEST_LUA_PROJ;
  if (_DisplayerVisual)
    getDisplayerVisual()->onTargetInstanceAttrModified(
        refMakerAttr, refMakerAttrIndex, targetAttrName, targetAttrIndex);
  if (_DisplayerUI)
    _DisplayerUI->onTargetInstanceAttrModified(refMakerAttr, refMakerAttrIndex,
                                               targetAttrName, targetAttrIndex);
  if (_DisplayerProperties)
    _DisplayerProperties->onTargetInstanceAttrModified(
        refMakerAttr, refMakerAttrIndex, targetAttrName, targetAttrIndex);
  _ToLua.onTargetInstanceAttrModified(refMakerAttr, refMakerAttrIndex,
                                      targetAttrName, targetAttrIndex);
  TEST_LUA_PROJ;
}
...
CEntityCL *CInstance::getEntity() const {
  ...
  TEST_LUA_PROJ;
  CDisplayerVisualEntity *dve =
      dynamic_cast<CDisplayerVisualEntity *>(getDisplayerVisual());
  if (dve) {
...
sint CInstance::getSelectedSequence() const {
  ...
  CLuaObject selected = const_cast<CInstance *>(this)
                            ->getLuaProjection()["User"]["SelectedSequence"];
  sint index = 0;
      ...
      if (behav) {
        ...
        if (activities) {
          if (selectedSequence >= 0 &&
              selectedSequence < (sint)activities->getSize()) {
...
std::string CInstance::getPosInstanceId() const {
  ...
  if (!posObj) {
    nlwarning(
        "<CInstance::getPosInstanceId> can't retrieve position from object");
    return "";
  }
...
REGISTER_ACTION_HANDLER(CAHDeleteSelectedInstance,
                        "r2ed_delete_selected_instance");

// *********************************************************************************************************
// handler to pick an instance
class CAHPickerLua : public IActionHandler {
  //	 TODO nico : replace this action handler by a CTool (in the same way
  //that CToolChoosePosLua derives from CToolChoosePos)
  virtual void execute(CCtrlBase * /* pCaller */, const std::string &sParams) {
    ...
    public:
      ...
      CToolPickLua(const std::string &cursCanPickInstance,
                   ...
                   const std::string &cursCannotPickPos, bool wantMouseUp)
          : CToolPick(cursCanPickInstance, cursCannotPickInstance,
                      cursCanPickPos, cursCannotPickPos, wantMouseUp) {}

      NLMISC_DECLARE_CLASS(CToolPickLua);
      void pick(CInstance &instance) {
        CTool::TSmartPtr holdThis(
            this); // prevent 'setCurrentTool' from deleting 'this'
        getEditor().setCurrentTool(NULL);
        if (!LuaPickFunc.empty()) {
          getEditor().getLua().executeScriptNoThrow(LuaPickFunc + "('" +
                                                    instance.getId() + "')");
        }
      }
      void pick(const CVector &pos) {
        ...
        if (!LuaPickPosFunc.empty()) {
          getEditor().getLua().executeScriptNoThrow(NLMISC::toString(
              "%s(%f, %f, %f)", LuaPickPosFunc.c_str(), pos.x, pos.y, pos.z));
        }
      }
      bool canPick(const CInstance &instance) const {
        ...
        std::string script =
            "return " + LuaTestFunc + "('" + instance.getId() + "')";
    ...
    std::string cursCannotPickInstance =
        getParam(sParams, "CursCannotPickInstance");
    ...
    //
    if (cursCanPickInstance.empty())
      cursCanPickInstance = "r2ed_tool_can_pick.tga";
    if (cursCannotPickInstance.empty())
      cursCannotPickInstance = "curs_stop.tga";
    if (cursCanPickPos.empty())
      cursCanPickPos = "r2ed_tool_pick.tga";
    if (cursCannotPickPos.empty())
      cursCannotPickPos = "r2ed_tool_pick.tga";
    //
    CToolPickLua *picker = new CToolPickLua(
        cursCanPickInstance, cursCannotPickInstance, cursCanPickPos,
        cursCannotPickPos, wantMouseUp == "true");
...
class CAHDumpLuaTable : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const std::string &sParams) {
    ...
    if (getEditor().getSelectedInstance()->getParent()) {
      negativeFilter.insert(getEditor()
                                .getSelectedInstance()
                                ->getParent()
                                ->getLuaProjection()
                                .toPointer());
    }
    getEditor().getSelectedInstance()->getLuaProjection().dump(30,
                                                               &negativeFilter);
  }
};
