...
/**
 * Tool to choose a position on landscape, with transparent entity drawn under
 * the mouse A transparent version of the entity to be created is drawn under
 * the mouse at the chosen position. Additionnally, a set of zone can be created
 * to choose the pos. These zone must be at a valid position on the map to allow
 * validation
 *
 */
...
public:
   ...
   */
  void
  enableMultiPos(const std::string &cursValidMulti = "curs_create_multi.tga");
   ...
   * may take different shapes (for example, it wouldn't show a 'stop' cursor
   * when the palette is under and one has just selected an object to drop)
   */
...
protected:
  CEntityCL *getGhost();
  // for derivers
  virtual void commit(const NLMISC::CVector &createPosition,
                      float createAngle) = 0;
