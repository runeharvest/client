...
// *********************************************************************************************************
CDisplayerLua::CDisplayerLua() { _ToLua._Displayer = this; }
...
bool CDisplayerLua::init(const CLuaObject &parameters) {
  ...
  if (parameters.isString()) {
    getEditor()
        .getEnv()[parameters.toString()]
        .push(); // get method from the R2 env
  } else {
    parameters.push();
  }
  ...
  if (CLuaIHMRyzom::executeFunctionOnStack(ls, 1, 1)) {
    ...
    nlwarning(
        "<CDisplayerLua::init> Error while calling displayer ctor (parameter "
        "should be a r2 method, or the *name* of a r2 method) : param is : ");
    parameters.dump();
    return false;
  }
...
// *********************************************************************************************************
CDisplayerLua *CDisplayerLua::CToLua::getEnclosing() { return _Displayer; }

// *********************************************************************************************************
void CDisplayerLua::CToLua::executeHandler(const CLuaString &eventName,
                                           int numArgs) {
  CLuaState &ls = *getLua();
  ...
  if (!_LuaTable.isValid())
    return; // init failed
  if (_LuaTable[eventName.getStr().c_str()].isNil())
  ...
  if (dumpStackWanted)
    ls.dumpStack();
  CLuaIHMRyzom::executeFunctionOnStack(*_LuaTable.getLuaState(), numArgs + 2,
                                       0);
  if (dumpStackWanted)
    ls.dumpStack();
}
...
// *********************************************************************************************************
void CDisplayerLua::onTargetInstanceCreated(const std::string &refMakerAttr,
...
// *********************************************************************************************************
void CDisplayerLua::onTargetInstanceErased(const std::string &refMakerAttr,
                                           sint32 refMakerAttrIndex) {
...
// *********************************************************************************************************
void CDisplayerLua::onTargetInstanceEraseRequested(
...
// *********************************************************************************************************
void CDisplayerLua::onTargetInstanceAttrModified(
    const std::string &refMakerAttr, sint32 refMakerAttrIndex,
...
// *********************************************************************************************************
void CDisplayerLua::onTargetInstancePreHrcMove(const std::string &refMakerAttr,
                                               sint32 refMakerAttrIndex) {
...
// *********************************************************************************************************
void CDisplayerLua::onTargetInstancePostHrcMove(const std::string &refMakerAttr,
                                                sint32 refMakerAttrIndex) {
