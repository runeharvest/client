...
void CToolPick::updateAfterRender() {
  ...
  if (!instanceUnder || ignoreInstanceUnder) {
    if (isMouseOnUI() && !isMouseOnContainer()) {
      ...
      CVector inter;
      _ValidPos = (ValidPacsPos == computeLandscapeRayIntersection(
                                       worldViewRay, _Intersection));
...
bool CToolPick::onMouseRightButtonClicked() {
  ...
  TSmartPtr holdThis = this; // for safety, prevent real deletion of this before
...
int CToolPick::luaCanPick(CLuaState &ls) {
  ...
  if (_CandidateInstance)
    ls.push(canPick(*_CandidateInstance));
  else
    ls.push(false);
  return 1;
}
