...
// *********************************************************************************************************
void CDisplayerVisualActivitySequence::onAttrModified(
    const std::string &attrName, sint32 /* attrIndex */) {
...
void CDisplayerVisualActivitySequence::update() {
  ...
  for (uint k = 0; k < activities->getSize(); ++k) {
    ...
    if (activityStr == "Stand Still" || activityStr == "Inactive")
      continue;
    //
    ...
    _TraversedPrimInfos.back().Visible =
        nextDV->getActualDisplayMode() != DisplayModeHidden;
    ...
    if (nextDV->getActualDisplayMode() != DisplayModeHidden &&
      ...
      if (nextZone->isKindOf("Region")) {
        ...
        if (!prevZone || !prevZone->isKindOf("Region")) {
          ...
          // just use the couple of vertices for which the distance is the
          // smallest
...
void CDisplayerVisualActivitySequence::addWanderSteps(const CVector &pos) {
  ...
  decal->setWorldMatrixForSpot(CVector2f(pos.x, pos.y),
                               CV_WanderDecalSize.get());
  _Decals.push_back(decal);
}

enum TSequenceState { Hidden = 0, HasFocus, Selected };
static void updateState(TSequenceState &state, CDisplayerVisual *dv) {
  if (dv->getDisplayFlag(CDisplayerVisual::FlagHasFocus))
    state = (TSequenceState)std::max((sint)state, (sint)HasFocus);
  if (dv->getDisplayFlag(CDisplayerVisual::FlagSelected))
    state = (TSequenceState)std::max((sint)state, (sint)Selected);
}

// *********************************************************************************************************
CDisplayerVisual *CDisplayerVisualActivitySequence::getPossibleGroupDV(
    CDisplayerVisual *entityDV) {
  ...
  while (parentGroup) {
    if (dynamic_cast<CDisplayerVisualGroup *>(
            parentGroup->getDisplayerVisual())) {
...
// *********************************************************************************************************
bool CDisplayerVisualActivitySequence::isVisible(CDisplayerVisual *groupDV,
                                                 CDisplayerVisual *entityDV) {
  // H_AUTO(R2_CDisplayerVisualActivitySequence_isVisible)
  if (!entityDV || !groupDV || !_Active)
    return false;
  ...
  if (entityDV->getDisplayFlag(CDisplayerVisual::FlagHideActivities)) {
    return false; // when selected for auto group, the activity won't be
                  // duplicated
  }
  // If in a group and not the leader of this group, or not selected, don't
  // display
  if (entityDV != groupDV) {
    if (groupDV->getSon(0) != entityDV)
      return false;
  }
...
void CDisplayerVisualActivitySequence::onPostRender() {
  ...
  // if this activity sequence is not the selected one in its parent then it is
  // hidden
  ...
  if (state != Hidden && entityDV) {
    ...
    CObject *sequences = getProps().getParent(); // see what is my index in the
                                                 // sequences list of my parent
    if (sequences) {
...
void CDisplayerVisualActivitySequence::onPreRender() {
  ...
  for (uint k = 0; k < _TraversedPrimInfos.size(); ++k) {
    ...
    bool visible =
        (_TraversedPrimInfos[k].PrimDisplay->getActualDisplayMode() !=
         DisplayModeHidden);
...
// *********************************************************************************************************
void CDisplayerVisualActivitySequence::onInstanceCreated(
    CInstance & /* instance */) {
...
// *********************************************************************************************************
void CDisplayerVisualActivitySequence::onInstanceErased(
    CInstance & /* instance */) {
...
// *********************************************************************************************************
void CDisplayerVisualActivitySequence::onAttrModified(
    CInstance & /* instance */, const std::string & /* attrName */,
    sint32 /* attrIndex */) {
  // H_AUTO(R2_CDisplayerVisualActivitySequence_onAttrModified)
  touch();
}
...
void CDisplayerVisualActivitySequence::onUpdate(CGroupMap &owner) {
  ...
  for (uint k = 0; k < _WorldMapEdges.size(); ++k) {
    ...
    _WorldMapEdges[k]->setPattern(
        0.f, (start - end).norm() * invWorldTextureWidth, CCtrlQuad::Repeat);
