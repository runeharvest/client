...
protected:
  mutable uint32
      _TimeStamp; // If this date is not the same than the config file date,
                  // then we know the value should be updated (so, more a
                  // counter than a timestamp, actually)
  std::string _VarName;

private:
  // not copyable
  CConfigVarBase(const CConfigVarBase & /* other */) { nlassert(0); }
  CConfigVarBase &operator=(const CConfigVarBase & /* other */) {
    nlassert(0);
    return *this;
  }
 ...
 * Example :
 *
 * CConfigVarRGBA CV_CreatureDefaultColor("CreatureDefaultColor", CRGBA(255,
 * 255, 255, 0))); // r2_config.lua may contains a 'CreatureDefaultColor' config
 * value
 *
 * creature.setColor(CV_CreatureDefaultColor.get());
 *
 */
template <class T> class CConfigVar : public CConfigVarBase {
public:
  CConfigVar(const char *varName, const T &defaultValue = T())
      : CConfigVarBase(varName), _Default(defaultValue) {}
  CConfigVar(const CConfigVar &other)
      : CConfigVarBase(other._VarName.c_str()), _Value(other._Value),
...
template <class T> const T &CConfigVar<T>::get() const {
  // Relies on R2 'getConfigVarValue' and 'getConfigVarTypename' functions
  // specialization to retrieve the value (see below)
  if (_TimeStamp != CConfigVarBase::getConfigFileTimeStamp()) {
    const CLuaObject &config = getEditor().getConfig();
    if (config.isNil()) {
      nlwarning("Trying to access the config file value '%s' of type '%s' from "
                "r2_config.lua, but the file hasn't been parsed yet, or its "
                "parsing failed, using default value.",
                _VarName.c_str(), getConfigVarTypename(_Default).c_str());
      _Value = _Default;
    } else {
      CLuaObject luaValue = config[_VarName];
      if (!getConfigVarValue(luaValue, _Value)) {
        nlwarning("Can't retrieve value of R2 config var '%s', of type '%s', "
                  "using default value",
                  _VarName.c_str(), getConfigVarTypename(_Default).c_str());
        _Value = _Default;
      }
...
//------------------------------------------------------------------------------------------------
// Float
inline std::string getConfigVarTypename(const float & /* dummy */) {
  return "float";
}
...
//------------------------------------------------------------------------------------------------
// Double
inline std::string getConfigVarTypename(const double & /* dummy */) {
  return "float";
}
...
//------------------------------------------------------------------------------------------------
// sint32
inline std::string getConfigVarTypename(const sint32 & /* dummy */) {
  return "sint32";
}
...
//------------------------------------------------------------------------------------------------
// String
inline std::string getConfigVarTypename(const std::string & /* dummy */) {
  return "string";
}
...
//------------------------------------------------------------------------------------------------
// RGBA
inline std::string getConfigVarTypename(const NLMISC::CRGBA & /* dummy */) {
  return "rgba";
}
