...
void setDisplayedInstance(CDisplayerBase *displayer,
 ...
 *
 * \TODO : find a better name that 'CInstance', something like 'CClientObject',
 * 'CClientView' or 'CEditorObject' ...
 */
...
public:
  ...
  // shortcut to get the "Name" field in utf8 format (or returns an empty string
  // if not present)
  ...
  // Advanced : get the counterpart CObjectTable object	(which is wrapped by the
  // object returned by 'getLuaProjection').
  const CObjectTable *getObjectTable() const { return _ObjectTable; }
  // TMP TMP TMP for lag compensation (should never write in object properties
  // directly...)
  CObjectTable *getObjectTable() {
    return const_cast<CObjectTable *>(_ObjectTable);
  }
  ...
  // if this object is inside a group, get the group leader, returns the object
  // else, or NULL on error
  ...
  // if this object is a logic entity, return its selected sequence, elsedefault
  // to 0
  sint getSelectedSequence() const;
  // resturn selected activity sequence for this entity. If a group entity or a
  // group, return the current sequence for the group
  ...
  CDisplayerBase *getDisplayerProperties() const {
    return _DisplayerProperties;
  }
   ...
   * \param attrName Name of the attribute inside this object, as given by its
   * class definition. If the attribute is an array, then an additionnal
   * parameter gives the index of the element being modified in the array (or -1
   * if the whole array is set) \param attrIndex Index in the element in its
   * array, or -1 if attrName doesn't refer to an array, or if the whole array
   * is modified
   */
   ...
   * nico : I expect this event to be the result of an undo (will be implemented
   * later, server side) may also happen if net msg telling that an instance is
   * created arrives after its targeter is created
   *
   * \param refMaker Name of the attribute that reference the instance (not the
   * instance id) \param refMakerAttrIndex If the reference maker is in an
   * array, then refMakerAttr gives the name of the array, and refMakerAttrIndex
  ...
  void onTargetInstancePreHrcMove(const std::string &refMakerAttr,
                                  sint32 refMakerAttrIndex);
  // Target instance has moved in the hierarchy of objects
  void onTargetInstancePostHrcMove(const std::string &refMakerAttr,
                                   sint32 refMakerAttrIndex);
  ...
  // If this entity is displayed using a CEntityCL instance, return its sheet
  // (Entity may be NULL if more then 254 entities are displayed)
  ...
  void dummySetSelectableFromRoot(
      bool selectable); // for export (this prop is read only)
  ...
  REFLECT_BOOL("SelectableFromRoot", getSelectableFromRoot,
               dummySetSelectableFromRoot);
  REFLECT_BOOL("Ghost", getGhost, setGhost);
  REFLECT_EXPORT_END

  // is this object outnumbering the max number of visible entities (displayed
  // in scene as an exclamation mark)
...
private:
  ...
  public:
    CLuaObject _LuaProjection; // projection in lua of this instance properties
                               // & methods
...
private:
  ...
  // For editor : Create this object from the CObjectTable it materialize in the
  // editor
  CInstance(const CObjectTable *objectTable, CLuaState &ls);
  // copy not supported
  CInstance(const CInstance & /* other */) : NLMISC::CRefCount() {
    nlassert(0);
  }
