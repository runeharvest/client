 ...
 *         If creation is not possible at the mouse position, then there's is a
 * little 'stop' icon shown No instance selection is done
 *
 *         EVENTS :
 *         --------
 *         LCLICK -> create
 ...
 *
 *         NB : we don't do creation on LDOWN or RUP in order to allow camera
 * manipulation
 *
 *     2) Mouse over UI
 *     -----------------
 *         The mouse cursor still has a little star to indicate "creation mode".
 *         Ideally on some actions, the creation is canceled, and the default
 * tool is backuped; NB nico : this is unclear to me atm, is it for button
 * clicks only ? (for example just moving a slider do not seem a good reason to
 * cancel the tool...) so no-op for now (TODO : exception for the minimap, if we
 * want to be able to create by clicking on it ?)
 *
 ...
 *             EVENTS :
 *             --------
 *             nothing to do here, camera event are handled by the
 * 'UserControls' global object If the right button is up without being preceded
 * by a camera move, then the context menu is shown
 *
 *
 ...
 *                      - nothing ? (e.g no selection and the context menu is
 * trigerred by the 'UserControls' global object as usual ? RDOWN while
 * maintained action : cancel the current action and restore state (same
 * behaviour as most apps) LDOWN -> selection + tool dependent (instant action
 * or maintained action) a maintained action (such as moving an entity) will
 * usually capture the mouse. others -> tool dependent
 *
 ...
 *             RDOWN while maintained action : cancel the current action and
 * restore state (same behaviour as most apps)
 *
 *          NB : a tool may want to capture the mouse, in this case it must call
 * CTool::captureMouse and CTool::releaseMouse when it is done
 *
 *      REMARKS:
 *
 *        - Maybe it would be cool to make camera manipulation totally
 * orthogonal to the system, and transparently layered on top of it. This would
 * requires using something like the middle button or some shift / control
 * combination to avoid conflict with the previous events ...
 *
 *     2) Mouse over UI
 *     -----------------
 *         The mouse is dipslayed as usual
 *         Events are taken in account by the UI, not by the tool
 *         This is transparent for CTool derivers, because mouse events won't
 * reach them Clicking in the interface doesn't change the current tool
 *
 *
...
public:
  ...
  enum TAccessMode {
    ...
    AccessOutlandOwner,
    AccessModeCount,
    AccessModeUnknown = AccessModeCount
  };
  ...
  // An entity has been selected with the standard selection system (usually in
  // test mode)
  ...
  /** Update of editor (just after render of the scene and entities update and
   * before rendering of the interface) This is the place to display additionnal
   * visual infos (slection bbox etc ...)
   */
   ...
   */
  bool getVisualPropertiesFromObject(CObject *instance, SPropVisualA &vA,
                                     SPropVisualB &vB, SPropVisualC &vC);
   ...
   *
   *
   * \param baseClass Filter base class for the search : only class that derived
   * from (or are of ) that class will be considered when looking for a new name
   */
  ...
  /** test if a ucstring has the format of a generated name that is : baseName +
   * " " + index Return index on success or -1 on failure
   */
  static sint getGeneratedNameIndex(const std::string &nameUtf8,
                                    const std::string &baseNameUtf8);

  /** Set a local user value (we name it  a "cookie") that should be attached to
   *an instance at creation time (that is, when the object will be created on
   *client -> when the server creation msg is received) The value will be stored
   *in the 'User' lua table. The 'User' table is attached to each instances in
   *the editor and is Read/Write (other properties in client and server and are
   *read only, and should be modified by sending a requestxxx network message)
   * This table is local only (that is, not seen by the server, nor by other
   *clients editing the same map).
   *
   * Example of use :
   *
   * We want that when a road is created, a dialog pop to enter the name of the
   *road.
   *
   * First we must be able to distinguish between roads created by ourselves and
   *road created by other persons editing the map.
   *
   * Secondly we don't want that all roads to have this behaviour (for example,
   *a road in a more complex system would be named automatically)
   *
   *
   ...
   *
   *
   * When creation message is received (may be placed in the 'onCreate' method
   *of a displayer attached to that instance)
   * ==================================================================================================================
   *
   ...
   * end
   *
   * Other example : when an act is created by the user it becomes the current
   *act
   *
   */
  void setCookie(const TInstanceId &instanceId, const std::string &key,
                 CLuaObject &value);
  ...
  struct IInstanceObserver : public NLMISC::CRefCount // refptr'ed observers
  {
    ...
    virtual void onAttrModified(CInstance & /* instance */,
  ...
  /** add a watch to know when an instance is created (the instance is
   * identified by its instance id) returns a handle for removal, or
   * 'BadInstanceObserverHandle' if creation failed
   */
  TInstanceObserverHandle addInstanceObserver(const TInstanceId &instanceId,
  ...
  // Remove an instance observer from its handle (but do not delete it). Return
  ...
  // get table for global variables in lua environment (equivallent to _G lua
  // variable)
   ...
   * property of the table, which is a C++ object, will be accessible from lua
   * (by using metatable)
   */
   ...
   * If the call fails then an error msg is printed in the log, and no arguments
   * are returned
   *
   * \param funcName name of the function to call (must resides in the r2ed
   * table) \param numArgs, numbers of arguments that the functions will receive
   * \param numRet : Number of parameters that the function returns
   *                 As usual the stack will be adjusted to that size after the
   * call
   */
  bool callEnvFunc(const char *funcName, int numArgs, int numRet = 0);
  // Behave like 'callEnvFunc', but call a method instead (r2 is passed as the
  // 'self' parameter, that is)
  ...
  CDynamicMapClient &getDMC() const {
    nlassert(_DMC);
    return *_DMC;
  }

  /** Set a property of an object locally. No network msg is sent, but
   * modification events are triggered to signal that the object has been
   * modified (rooted to one of the CDisplayerBase derived object, attached to
   * the object being modified. see displayer_base.h). Changes to local value
   * must be commited or cancelled when edition is finish by calling
   * 'requestCommitLocalNode' or 'requestRollbackLocalNode' Typical use is by
   * the slider widget : real value is sent accross the network only when the
   * user release the mouse, but local update of object property related to the
   * slider is done each time the slider is modified before release. During the
   * maintained action, no network message are desireable. NB : a copy of input
   * parameter 'value' will be done. Caller is responsible for deleting 'value'
   * after use.
   */
  void requestSetLocalNode(const std::string &instanceId,
                           const std::string &attrName, const CObject *value);
  void requestCommitLocalNode(const std::string &instanceId,
                              const std::string &attrName);
  void requestRollbackLocalNode(const std::string &instanceId,
                                const std::string &attrName);
  ...
  void showSelectBox(const NLMISC::CAABBox &localBox,
                     const NLMISC::CMatrix &worldMat);
  void showHighlightBox(const NLMISC::CAABBox &localBox,
                        const NLMISC::CMatrix &worldMat);
  ...
  //
  /** Set current edition tool. NULL will reset to the default tool (selection
   * tool)
   */
   ...
   * \param fileDescText short description of the script function (for error
   * messages) \return true on success
   */
  bool doLuaScript(const char *filename, const char *fileDescText);
  // helper : create an entity in scene at the given slot & position, replacing
  // any previous entity in that slot

  static CEntityCL *
  createEntity(uint slot, const NLMISC::CSheetId &sheetId,
               const NLMISC::CVector &pos, float heading,
               const std::string &permanentStatutIcon = std::string(""));
  ...
  CInstance *getInstanceUnderPos(float x, float y, float distSelection,
                                 bool &isPlayerUnderCursor);
  // helper : test intersection between an entity and a ray
  static float
  preciseEntityIntersectionTest(CEntityCL &entity,
                                const NLMISC::CVector &worldRayStart,
                                const NLMISC::CVector &worldRayDir);
  // Tmp show the connection window and display a msg in it. An empty msg will
  // close the window
  static void connectionMsg(const std::string &stringId);
  TEntityCustomSelectBoxMap &getEntityCustomSelectBoxMap() {
    return _EntityCustomSelectBoxMap;
  }
  // from a pointer on an entity, retrieve its local selection bbox (possibly
  // redefined in r2_ui_custom_boxes_data.lua
  ...
  bool checkRoomLeft();
  // display an error msg to prompt the user to make room for new objects in its
  // scenario
  void makeRoomMsg();
  ...
  // rename delete auto_save9, rename auto_save1.r2 to auto_save2.r2 and so
  // until auto_save8.r2, copy auto_save.r2 to auto_save1.r2,
  void autoSave();
   ...
   * Plot items are items with SCROLL_R2 as family. Each sheet can be used only
   * once in a scenario (meaning that each icon can only be seen for a single
   * plot item). The name of a plot item can also be change by using the lua
   * command r2:setPlotItemName(sheetId, ucName)
   */
  ...
  // from one class index, returns the index to the derived class (-1 if not
  // found)
...
private:
  ...
  CHashMap<std::string, uint>
      _ClassNameToIndex; // Map each class name to an unique index (filled at
                         // init)
  NLMISC::CArray2D<uint8> _KindOfTable; // Table to test if one class derives
                                        // from another (filled at init)
  std::vector<sint>
      _BaseClassIndices; // for each class, give index of thebase class (vector
                         // ordered by classes indices)

  typedef CHashMultiMap<ucstring, CInstance *, NLMISC::CUCStringHashMapTraits>
      TInstanceByName;
  ...
  std::vector<CSortedInstances>
      _InstancesByDispName; // instances sorted by their display name (private
                            // use)

  // priv for debug
  bool isRegisteredByDispName(CInstance *inst) const;

  // Cookies (local objects attached to instances at creation time, see
  // setCookie)
  struct CCookie {
    std::string Key;
    CLuaObject Value;
  };
  typedef std::list<CCookie>
      TCookieList; // for a single instance, map each key to its value
  ...
  // alternative selection for huge element like particle systems, display a box
  // on ground rather than the selection circle
  ...
  //
  bool _EnteredInSetSelectedInstance; // prevent recursive call from
                                      // CEditor::setSelectedInstance
  bool _Initialized;
  ...
  /** system for local generation of name : for each kind of name, gives the
   * locally allocated ids
   * -> Used to generate name for instances, taking in account instance that
   * have not been added to the scene yet. (that is, requestSetNode message has
   * been sent, but server has not added object to the scenario yet)
   */
  ...
  typedef std::multimap<TInstanceId, IInstanceObserver::TRefPtr>
      TInstanceObserverMap;
  TInstanceObserverMap _InstanceObservers;
  typedef std::map<TInstanceObserverHandle, TInstanceObserverMap::iterator>
      TInstanceObserverHandleMap;
  TInstanceObserverHandleMap
      _InstanceObserverHandles; // map each observer handle into an entry in the
                                // map
  TInstanceObserverHandle
      _InstanceObserverHandleCounter; // current handle to generate when adding
                                      // a new observer
  ...
  bool _WillTP; // true if a teleport should be expected after the scenario has
                // been updated in this case, first season change is ignored,
                // because it is done during the teleport

  bool _ScenarioReceivedFlag;     // for the wait screen ...
  bool _TPReceivedFlag;           // for the wait screen ...
  bool _WaitScenarioScreenWanted; // lua requests that we display the 'wait
                                  // scenario' screen
  bool _WaitScenarioScreenActive; // the 'wait scenario' screen is being
                                  // displayed
  bool _EditionModeDisconnectedFlag;
  CObject *_NewScenario; // new scenario that will be updated just after the
                         // wait screen is over
  uint32
      _NewScenarioInitialAct; // the start at which the user start an edition
                              // session (can be ~= from 1 after a test session)
  bool _PostponeScenarioUpdated;

  // Contextual selection
  //
  // We keep track of the last selected 'logic entity' (npc most of the time),
  // and the last list of primitive through which its current activity sequence
  // goes This way we can handle the following scenario :
  // - create a npc
  ...
  // By keeping the last list of contextual primitive, we ensure that contextual
  // selection remains visible when one click on one element in the currently
  // displayed sequence
...
private:
  ...
  void nodeErased(const std::string &instanceId, const std::string &attrName,
                  sint32 position);

  void nodeInserted(const std::string &instanceId, const std::string &attrName,
                    sint32 position, const std::string &key, CObject *value);

  virtual void nodeSet(const std::string &instanceId,
                       const std::string &attrName, CObject *value);

  void nodeMoved(const std::string &instanceId, const std::string &attrName,
                 sint32 position, const std::string &destInstanceId,
                 const std::string &destAttrName, sint32 destPosition);

  void scenarioUpdated(CObject *highLevel, bool willTP,
                       uint32 startingActIndex);

  // send the needed events to tell that the attr at 'attrName' inside
  // 'parentInstance' (possibly at position (indexInArray') has been modified
  void onAttrModified(CInstance &parentInstance, const std::string &attrName,
                      sint32 indexInArray = -1);

  // send the needed events to tell that an object has been modified (&
  // propagate to parents)
  void onAttrModified(const CObject *value);

  void onResetEditionMode();
  void onEditionModeConnected(uint32 userSlotId, uint32 adventureId,
                              CObject *highLevel,
                              const std::string &versionName, bool willTP,
                              uint32 initialActIndex);
  void onAnimationModeConnected(
  ...
  void onTestModeDisconnected(TSessionId sessionId, uint32 lasAct,
...
public:
  /** private : retrieve lua 'User' table attached to an object (a read / write
   * table) The table is pushed on stack
   */
  static void getLuaUserTableFromObject(CLuaState &ls,
...
private:
  ...
  static int luaDofile(CLuaState &ls); // equivalent of the 'dofile' lua
                                       // function, but with more info output
  static int luaTryFile(
      CLuaState &ls); // same as try file, but do not throw an exception on
                      // error, just return an error message and return false
  ...
  static int luaSnapPosToGround(
      CLuaState
          &ls); // takes x, y, z as parameters and return the snapped position
  ...
  static int luaAddInstanceObserver(
      CLuaState
          &ls); // param 1 = instance id of the instance to observe
                // param 2 = table of an object that will receive notifications.
                // The table should contain methods with the same names than
                // those found in 'IInstanceObserver' (plus parameters  are the
                // same) the method returns a handle for future deletion
  static int luaRemoveInstanceObserver(
      CLuaState &ls); // remove an observer that was previously added by a
                      // 'addInstanceObserver' param 1 = the handle returned by
                      // 'addInstanceObserver' at the registration time returns
                      // a reference to the observer that was registered
  static int luaGenInstanceName(
      CLuaState &ls); // calls CEditor::genInstanceName, same parameters
                      // NB : return has type 'ucstring', so may need to call
                      // :toUtf8() in the lua script
  ...
  static int
  luaSetEditorSeason(CLuaState &ls); // set the weather to display when editing
  ...
  static int luaIsInIslandRect(
      CLuaState &ls); // test if pos is in the current island (not necessarily a
                      // valid pos, but inside the island rect) takes x, y as
                      // entry, returns true on success

  static int luaGetCurrentIslandName(CLuaState &ls);

  static int
  luaWaitScenarioScreen(CLuaState &ls); // display the wait screen after the
                                        // scenario creation has been launched
...
private:
  ...
  void initDecals();
  void showDecal(const NLMISC::CVector2f &pos, float scale, CDecal &decal,
                 const CDecalAnim &decalAnim);
  ...
  // update the display of decals created when the player select an instance in
  // the scene
  void updateSelectingDecals();
  // display of highlight or select box (for selection of huge objects)
  // the CDecalAnim is used to mimic the color cycle seen when standard
  // selection circle is displayed (no CPrimRenderAnim for now)
  void showPrimRender(CPrimRender &dest, const NLMISC::CAABBox &localBox,
                      const NLMISC::CMatrix &worldMat,
  ...
  static bool _IsStartingScenario; // the scenario is an animation scenario
                                   // launch from the ring access point

  // bool _ModeEnabled[ModeCount];

public:
  // private method
  CInstance *getInstanceFromObject(
      const CObject *obj) const; // nb : only table have associated instance
  ...
  // trigger an instance observers for the given instance id. A copy of the
  // observer list is made, thus allowing for safe removal of observer when they
  // are triggered
  ...
  // for CInstance usage : allows to keep a list of instances sorted by their
  // display name
...
// test whether editor is currently enabled and is in EDITION mode. (In this
// mode, selection is managed by the editor rather than by the entity manager)
 ...
 */
template <class T> T *createObjectFromClassName(const std::string &className) {
  if (className.empty())
    return NULL;
  try {
    ...
    if (!inst) {
      nlwarning("<R2::createObjectFromClassName> class %s found in the "
                "registry, but does not match the expected class.",
...
CObject *buildVector(const NLMISC::CVectorD &v,
...
/** Helper : read an enum from a lua string, printing necessary error if no
 * match is found. nil found -> no op
 */
template <class T>
void enumFromLua(const CLuaObject &value, std::pair<std::string, T> *enumTable,
                 uint numEnum, T &dest, const std::string &errorMsgPrefix) {
  if (value.isNil()) {
    return;
  }
  if (!value.isString()) {
    nlwarning("%s : String expected when reading '%s'", errorMsgPrefix.c_str(),
              value.getId().c_str());
    return;
  }
  ...
  nlwarning("%s : Unknown enum %s read from object %s", errorMsgPrefix.c_str(),
            value.toString().c_str(), value.getId().c_str());
}
