...
// ************************************************************************
CObjectRefIdClient::CObjectRefIdClient(const std::string &value)
    : CObjectRefId(""), _ObserverHandle(CEditor::BadInstanceObserverHandle),
      _IndexInParent(-1), _IndexInParentArray(-1), _Enabled(true) {
...
bool CObjectRefIdClient::set(const std::string &key, const std::string &value) {
  ...
  // nlwarning("# Setting CObjectRefIdClient 0x%x value to %s", (int) this,
  // value.c_str());
  if (_Enabled) {
    nlassert(getValue().empty() ==
             (_ObserverHandle == CEditor::BadInstanceObserverHandle));
...
// ************************************************************************
void CObjectRefIdClient::onAttrModified(CInstance & /* instance */,
                                        const std::string &modifiedAttrName,
  ...
  if (!_ParentInstance)
    return;
  _ParentInstance->onTargetInstanceAttrModified(
      nameInParent, indexInArray, modifiedAttrName, modifiedAttrIndexInArray);
}
...
// ************************************************************************
void CObjectRefIdClient::getNameInParent(std::string &name,
                                         sint32 &indexInArray) const {
  ...
  if (_IndexInParent != -1 && _ParentInstance) {
    ...
    // check that index is still valid (true most of the case unless instance
    // has been moved)
    if (_IndexInParent <= (sint32)parentInstanceTable->getSize()) {
      if (_IndexInParentArray == -1) {
        if (parentInstanceTable->getValueAtPos(_IndexInParent) ==
            static_cast<const CObject *>(this)) {
        ...
        if (subObject->isTable()) {
          ...
          if (_IndexInParentArray < (sint32)subTable->getSize()) {
            if (subTable->getValueAtPos(_IndexInParentArray) ==
                static_cast<const CObject *>(this)) {
              name = parentInstanceTable->getKey(_IndexInParent);
  ...
  CObjectTable *parentInstanceTable = _ParentInstance->getObjectTable();
  const CObject *ptrInParent = (parentInstanceTable == this->getParent())
                                   ? static_cast<const CObject *>(this)
                                   : this->getParent();
  // if instance is the direct parent (e.g object is not in an array of the
  // parent)
...
// ************************************************************************
void CObjectTableClient::pushOnLuaStack(CLuaState &state,
                                        CLuaObject &metatable) const {
...
CObject *CObjectTableClient::clone() const {
  ...
  // NB : don't copy the reference because there can be only one
  // CObjectTableClient per instance (other copy are for undo/redo or network)
...
// ************************************************************************
CObjectFactoryClient::CObjectFactoryClient(const std::string &prefix)
