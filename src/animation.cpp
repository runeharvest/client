...
// Destructor
//-----------------------------------------------
CAnimation::~CAnimation() {} // ~CAnimation //

//-----------------------------------------------
// init
//-----------------------------------------------
void CAnimation::init(CAnimationSheet *sheet,
...
// computeAnimation :
//-----------------------------------------------
void CAnimation::computeAnimation(NL3D::UAnimationSet *animationSet,
  ...
  if (id == NL3D::UAnimationSet::NotFound) {
    _Animation = animationSet->addAnimation(animName.c_str(), animName.c_str(),
                                            ClientCfg.DisplayMissingAnimFile);
    if (_Animation == NL3D::UAnimationSet::NotFound) {
      // Up to 100 missing animation kept.
      if (MissingAnim.size() < 100) {
        std::pair<std::set<std::string>::iterator, bool> result =
            MissingAnim.insert(animName);
        if (result.second)
          nlwarning("CAnimation::computeAnimation: Animation '%s' NOT FOUND.",
                    animName.c_str());
      }
    } else {
      if (CAnimationMisc::getAnimationLength(animationSet, _Animation) == 0.0)
        nlwarning("CAnimation::computeAnimation: Animation '%s' LENGTH IS 0.",
                  animName.c_str());
  ...
  if (_Sheet) {
    if (_Sheet->Next.empty())
      ...
      for (i = 0; i < _Sheet->NextWeight.size(); ++i) {
        ...
        if (nextAnim < (sint)animFilterStates.size() &&
            animFilterStates[nextAnim]) {
      ...
      // "concatenate" weights of each index, when the random value is reached
      // we'll have the index to use
      uint32 w = 0;
      ...
      nlwarning("<CAnimation::getNextAnim> can't choose next anim, rand=%d "
                "weigthSum=%d weightCount=%d",
                randWeight, weightSum, _Sheet->NextWeight.size());
    }
  }
...
// resetSoundAnim
//-----------------------------------------------
void CAnimation::resetSoundAnim() { _SoundAnim = CSoundAnimationNoId; }
