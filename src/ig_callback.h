...
/**  A class to manage callback (collisions, constructions ..) with igs of the
 * landscape. Its purpose is to create pacs primitive for objects of the
 * landscape that need them. The matching primitive for an instance is deduced
 * from its shape name. Pacs primitives must be loaded at startup. \author
 * Nicolas Vizerie \author Nevrax France \date 2002
 */
...
public:
  ...
  /** Add an ig with his num ZC. Its callbacks primitive will be created when it
   ...
   */
  void
  addIGsWithNumZC(std::vector<std::pair<NL3D::UInstanceGroup *, sint>> &igs);
  /// Remove all IGs
  void deleteIGs();
  ...
  ///\name Season change
  //@{
  // apply change of season on iug (for now, only change the fxs to match the
  // season)
  void changeSeason();
  //@}
...
private:
  ...
  private:
    ...
    //@{
    virtual std::string transformName(uint index,
    ...
    //@}
    /** Called after all instance have been added, and when their sheets have
     * been retrieved. This is the place to setup parameters from sheets
     */
    void updateFromSheets();
    //
  private:
    TMovePrimitiveVect _MovePrimitives;
    TEntitySheetVect _EntitySheets; // matching sheets for each instance (or
                                    // NULL), they are used only after the
                                    // instance have been added to the scene
    CIGCallback *_Owner;
    NL3D::UInstanceGroup *_IG; // the IG we're looking at
    sint _NumZC;               // >= 0 if valid.
    bool _HasManagedFXs;       // true if there are managed fx in the group,
                               // otherwise, _ManagedFXHandle is not used
...
/** Debug purpose only: create instances in a scene from the content of a move
 * container. The created instances are static. Their pointer can be retrieved
 * in a pointer The following shapes must be available : unit_box.shape : a box
 * of 1 x 1 x 1 centered at the origin unit_cylinder.shape : a cylinder of
 * height 1, radius 1
 */
