...
bool StereoDisplayAttached =
    false;                       // Is stereo display handling the display mode
CSoundManager *SoundMngr = NULL; // the sound manager
NL3D::UMaterial GenericMat;      // Generic Material
NL3D::UTextContext *TextContext =
    NULL; // Context for all the text in the client.
...
NLMISC::CMatrix InvMainSceneViewMatrix; // Matrix to transform from camera space
                                        // to world space
...
std::vector<std::pair<std::string, std::string>> VRDeviceCache;
...
void resetTextContext(const char *font, bool resetInterfaceManager) {
  ...
  TextContext = Driver->createTextContext(
      NLMISC::startsWith(font, "ui") ? font : CPath::lookup(font));
  if (TextContext != NULL)
