...
public:
  NL3D::UTrack
      *PosTrack; // Filled after init : additionnal pos track to animate the fx
...
public:
  ...
  /** helper : create instance matching that sheet. NB : the instance is not
   * sticked neither positionned. 4 user params may be supplied to replace the
   * default ones
   */
  NL3D::UParticleSystemInstance
  createMatchingInstance(const float *customUserParams = NULL) const;
 ...
 * Those fx are intended to be all instanciated at once, hence the small number
 * of instances
 */
