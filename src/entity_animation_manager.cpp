...
//---------------------------------------------------
// getInstance :
// Instanciate CEntityAnimationManager. There can be only one instance
// (singleton) \return CEntityAnimationManager * : Pointer on
...
CEntityAnimationManager *CEntityAnimationManager::getInstance() {
  if (_Instance == 0) {
    ...
    if (Scene)
      ...
      nlwarning(
          "CEntityAnimationManager::getInstance : Scene is not allocated.");
  }
  return _Instance;
} // instance //
...
// \warning If you the kept the pointer given by getInstance, it will be
// invalid.
//-----------------------------------------------
...
void CEntityAnimationManager::release() {
  ...
  // Nothing to release here : all is in the sheet manager so kept during all
  // the time the game is running Release the AnimationSet.
  if (_AnimationSet) {
...
// load :
//---------------------------------------------------
void CEntityAnimationManager::load(NLMISC::IProgressCallback & /* progress */,
  ...
  _AutomatonList = dynamic_cast<CAutomatonListSheet *>(
      SheetMngr.get(CSheetId("automaton.automaton_list")));
  nlassert(_AutomatonList != NULL);

  pushInfoStr("COMPUTING ANIMATION SET ...");
  // Initialize the list of animsets
  CAnimationSetListSheet *pASLS = dynamic_cast<CAnimationSetListSheet *>(
      SheetMngr.get(CSheetId("entities.animset_list")));
  nlassert(pASLS != NULL);
  ...
  // Build 3d Animation Set (called after all animations added in the animation
  // set).
  ...
  /*	// Memory debug
     ...
     rAS.getAnimationStateByIndex(i); uint32 nAnim =
     pAnimState->getNumAnimation(); nTotalAnim += nAnim; for (uint j = 0; j <
     nAnim; ++j)
                          {
                                  CAnimation *pAnim =
     pAnimState->getAnimationByIndex(j); nTotalAnimFx +=
     pAnim->getFXSet().FX.size(); if (!bOnce)
                                  {
                                          bOnce = true;
                                          nlinfo ("%d %d %d", sizeof(rAS),
     sizeof(*pAnimState), sizeof(*pAnim));
                                  }
                          }
                  }
          ...
          nlinfo ("%d %d %d", nTotalAnimSet, nTotalAnimState, nTotalAnim,
     nTotalAnimFx);
  */
} // load //
...
CEntityAnimationManager::mState(const string &automaton, TAnimStateKey key) {
  // Search for the automaton
  std::map<std::string, CAutomatonSheet *>::iterator it =
      _AutomatonList->Automatons.find(automaton);
  if (it != _AutomatonList->Automatons.end()) {
    // Get the state.
    const CAutomatonStateSheet *statePtr = ((*it).second)->state(key);
    if (statePtr == 0) {
      nlwarning("CEntityAnimationManager::mState: the state '%s' does not "
                "exist for the automaton '%s'",
                CAnimationState::getAnimationStateName(key).c_str(),
                automaton.c_str());
      return 0;
    } else
      return statePtr;
...
//
//---------------------------------------------------
void CEntityAnimationManager::processLogic(const std::string &animListName,
TAnimStateKey animationId, CAnimationTime startTimeOffset, CAnimationTime
endTimeOffset, UTrack *&soundTrack, vector<CAnimationTime>& result)
{
        result.clear();
        ...
        if(animSet)
        {
                ...
                if(idAnim != UAnimationSet::NotFound)
                {
                        ...
                        if(anim)
                        {
                                ...
                                soundTrack = anim->getTrackByName(
soundTrackName ); if( soundTrack != NULL )
                                {
                                        UTrackKeyframer * soundTrackKF =
dynamic_cast<UTrackKeyframer *>(soundTrack); if( soundTrackKF == NULL )
                                        {
                                                nlerror("The track %s is not a
key framer track",soundTrackName);
                                        }

                                        soundTrackKF->getKeysInRange(startTimeOffset,
endTimeOffset, result);
                                }
                        }
                }
        }
...
// \warning This method never return a value <= 0.0 and so will return 1.0
// instead. \warning This Method is slower than the one with the animation Id
// instead of the animation Name.
//---------------------------------------------------
double
CEntityAnimationManager::getAnimationLength(const std::string &animName) const {
...
double CEntityAnimationManager::getAnimationLength(uint idAnim) const {
  if (idAnim == NL3D::UAnimationSet::NotFound)
    return 0.f;
...
//---------------------------------------------------
double CEntityAnimationManager::getAnimationAverageSpeed(
...
//---------------------------------------------------
const CAnimationSet *
CEntityAnimationManager::getAnimSet(const std::string &animSet) const {
...
//-----------------------------------------------
void CEntityAnimationManager::serial(NLMISC::IStream & /* f */) {} // serial //
...
void CEntityAnimationManager::createFaceAnimationSet(const CTypeEntity& type)
{
        ...
        if(faceAnimationSet == 0)
                nlwarning("CEntityAnimationManager::createFaceAnimationSet :
cannot create an animation set for the Face.");
...
// \return UAnimationSet * : a pointer on the animation set or 0 if there is not
animation set for this type.
//---------------------------------------------------
UAnimationSet * CEntityAnimationManager::getFaceAnimationSet(const CTypeEntity&
type)
{
        map<CTypeEntity, UAnimationSet *>::iterator itAnimSet =
_FaceAnimationSets.find(type); if(itAnimSet == _FaceAnimationSets.end()) return
0; else return (*itAnimSet).second;
}// getFaceAnimationSet //
...
//---------------------------------------------------
uint CEntityAnimationManager::chooseFaceAnim(const CTypeEntity& type, const
string &emotion)
{
        std::map<CTypeEntity, TFaceEmotions>::iterator itFEPT =
_FaceEmotionsPerType.find(type); if(itFEPT == _FaceEmotionsPerType.end())
        {
                nlwarning("CEntityAnimationManager::chooseFaceAnim : Nothing
associated to the type."); return UAnimationSet::NotFound;
        }
        ...
        {
                nlwarning("CEntityAnimationManager::chooseFaceAnim : Face
Emotion '%s' does not exist for this type.", emotion.c_str()); return
...
// \todo GUIGUI : Check that we can add the same animation more than once with
no problem or do i have to check if the animation is already in.
//-----------------------------------------------
void CEntityAnimationManager::loadFaceAnimations(const CTypeEntity& type, const
char * fileName)
{
        // Get the animation set.
        UAnimationSet * animationSet = getFaceAnimationSet(type);
        if(!animationSet)
        {
                nlwarning("CEntityAnimationManager::loadFaceAnimations :
animationSet is Null for the type -> Face Animations not loaded."); return;
        }
        ...
        fileFormat.insert(make_pair(string("name"),
CSTLoader::STRING)); fileFormat.insert(make_pair(string("filename"),
CSTLoader::STRING)); fileFormat.insert(make_pair(string("percentage"),
CSTLoader::FLOAT));
        ...
        while(cstl.readLine())
        {
                ...
                if(animationSet->addAnimation((animFilename + ".anim").c_str(),
animFilename.c_str()) == UAnimationSet::NotFound)
                        nlwarning("CEntityAnimationManager::loadFaceAnimations :
anim '%s' is not found.", (animFilename + ".anim").c_str());
                // The animation is right added in the set -> add anim in the
right face emotion. else
                {
                        // Attempt on find this name in the map.
                        map<string, CFaceEmotion>::iterator itFaceEmotion =
faceEmotions.find(emotionName);
                        // This emotion already exist -> add new animation to
the emotion. if(itFaceEmotion != faceEmotions.end())
                        {
                                CFaceEmotion &faceEmotion =
(*itFaceEmotion).second; faceEmotion.addAnim(animFilename, percentage);
                        }
                        else
                        {
                                CFaceEmotion faceEmotion;
                                faceEmotion.addAnim(animFilename, percentage);
                                faceEmotions.insert(make_pair(emotionName,
faceEmotion));
                        }
                }
        }
