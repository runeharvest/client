...
class CCallbackClientAdaptor : public CNelCallbackClientAdaptor {
  ...
  CCallbackClientAdaptor(void *containerClass)
      : CNelCallbackClientAdaptor(containerClass), _PassThrue(false) {
    _CallbackClient.setPreDispatchCallback(
        &CCallbackClientAdaptor::cb_preDispatchMessage);
  }

  CCallbackClient &getCallback() { return _CallbackClient; }

  virtual void connect(const NLNET::CInetHost & /* addr */) {
    // do not connect now
  }

  virtual void send(const NLNET::CMessage &buffer,
                    NLNET::TSockId hostid = NLNET::InvalidSockId,
                    bool log = true) {
    if (!_PassThrue) {
      // queue the message for later sending.
      CAutoMutex<CUnfairMutex> mutex(_Mutex);
      nldebug("SB: Pushing a buffer into SendQueue (from %u elts)",
              _SendQueue.size());
      _SendQueue.push(buffer);
    } else {
      // protect socket because update() can be called in the main thread while
      // send() can be called with passthru=true in the session browser thread
      // (by authenticate() for example)
      _CallbackClient.send(buffer, hostid, log);
    }
  }

  // called by the callback client before each message dispatching
  static void cb_preDispatchMessage(CMessage &msgin, TSockId from,
                                    CCallbackNetBase &netbase) {
    CNelCallbackClientAdaptor *adaptor =
        static_cast<CNelCallbackClientAdaptor *>(netbase.getUserData());
    if (adaptor != NULL) {
      CSessionBrowser *sb =
          static_cast<CSessionBrowser *>(adaptor->getContainerClass());
...
CSessionBrowser::CSessionBrowser()
    : CSessionBrowserServerWebClientItf(new CCallbackClientAdaptor(this)),
      _TerminateComm(false), _SignalConnFail(false), _NBReceivedMessage(0),
  ...
  sendMessages:
    ...
    while (!adaptor->_SendQueue.empty()) {
      bool popped = false;
      {
        ...
        // if (adaptor->getCallback().connected()) // don't loose msg if it has
        // not been sent (TODO: ADD?)
        {
          adaptor->_SendQueue.pop();
          popped = true;
        }
      ...
      if (popped)
        nldebug("SB: Popping from SendQueue (now %u elts)",
                adaptor->_SendQueue.size());
      else
    ...
    while (_WaitingForMessage ||
           (adaptor->_SendQueue.empty() &&
            CTime::getSecondsSince1970() - startWait < KeepConnectedTime)) {
      nlSleep(100);
...
void CSessionBrowser::on_preDispatchMessage(CMessage &msgin, TSockId /* from */,
                                            CCallbackNetBase & /* netbase */) {
  ++_NBReceivedMessage;
...
bool CSessionBrowser::waitOneMessage(const std::string &msgName) {
  ...
  if (!_LastReceivedMessageNames.empty()) {
    ...
    bool receivedExpected =
        ...
        (_LastReceivedMessageNames.front() == "JSSRE" &&
         (msgName ==
          "JSSR")); // harcoded: JSSRE may replace JSSR in some responses
    STOP_IF(!receivedExpected, "BIG BAD BUG - Expected SBS message '" +
                                   msgName + "' but received '" +
                                   _LastReceivedMessageNames.front() + "'");
...
bool CSessionBrowser::connected() { return getCallbackAdaptor()->connected(); }
