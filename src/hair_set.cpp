  ...
  for (uint k = 0; k < numHairItem; ++k) {
    ...
    if (item && !item->getShape().empty()) {
      if (item->getShape().find("cheveux", 0) != std::string::npos) {
        ...
        switch (itemName[0]) {
        case 'm':
          _Hairs[Matis].push_back(k);
          break;
        case 't':
          _Hairs[Tryker].push_back(k);
          break;
        case 'z':
          _Hairs[Zorai].push_back(k);
          break;
        case 'f':
          _Hairs[Fyros].push_back(k);
          break;
        }
      }
    }
...
//=========================================================================
sint CHairSet::getHairItemId(EGSPD::CPeople::TPeople gspeople,
                             uint index) const {
  H_AUTO_USE(RZ_HairSet)
  EPeople people = convPeople(gspeople);
  if (people == DontKnow)
    return -1;
  if (index > _Hairs[people].size())
    return -1;
  return (sint)_Hairs[people][index];
}

//=========================================================================
sint CHairSet::getHairItemFromId(EGSPD::CPeople::TPeople gspeople,
                                 uint Id) const {
  H_AUTO_USE(RZ_HairSet)
  EPeople people = convPeople(gspeople);
  if (people == DontKnow)
    return -1;
  TIntArray::const_iterator it =
      std::find(_Hairs[people].begin(), _Hairs[people].end(), Id);
  if (it != _Hairs[people].end())
    return (sint)(it - _Hairs[people].begin());
  else
    return -1;
}
...
bool CHairSet::isHairItemId(uint id) const {
  H_AUTO_USE(RZ_HairSet)
  for (uint k = 0; k < NumPeople; ++k) {
    if (getHairItemFromId(convPeople((CHairSet::EPeople)k), id) != -1)
      return true;
  }
  return false;
}
...
EGSPD::CPeople::TPeople CHairSet::getPeopleFromHairItemID(uint id) const {
  ...
  for (uint k = 0; k < NumPeople; ++k) {
    if (getHairItemFromId(convPeople((CHairSet::EPeople)k), id) != -1)
      return convPeople((CHairSet::EPeople)k);
  }
  return EGSPD::CPeople::EndPeople;
}
