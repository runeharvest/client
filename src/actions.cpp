...
static bool getParam(CBaseAction::CParameter::TType type, string &paramName,
                     string &paramValue, const std::string &argu, uint paramId);
...
bool CActionsManager::addAction(const CAction::CName &name) {
  ...
  pair<TActionsMap::iterator, bool> p =
      _Actions.insert(TActionsMap::value_type(name, action));

  if (!p.second) {
    nlwarning("Action (%s %s) already exist in the action manager.",
              name.Name.c_str(), name.Argu.c_str());
  }
...
bool CActionsManager::addCombo(const CAction::CName &name, const CCombo &combo,
                               bool createAction) {
...
// ***************************************************************************
bool CActionsManager::isActionPresentInContext(
    const CAction::CName &name) const {
  CInterfaceManager *im = CInterfaceManager::getInstance();
  if (!im->isInGame())
    return true; // no filtering done when outgame, because actions.xml not
                 // loaded yet (no base actions where added)
  {
    const CBaseAction *baseAction = getBaseAction(name);
    if (!baseAction)
      return false;
    // see if action valid in current context
    if (!ActionsContext.matchContext(baseAction->Contexts))
      return false;
      ...
      if (parameter.Type == CBaseAction::CParameter::Constant) {
        ...
        for (uint k = 0; k < parameter.Values.size(); ++k) {
          if (parameter.Values[k].Value == paramValue) {
            if (!ActionsContext.matchContext(parameter.Values[k].Contexts))
              return false;
            found = true;
            break;
          }
...
bool CActionsManager::keyPushed(const CEventKeyDown &keyDown) {
  ...
  if (_Enabled) {
    ...
    while ((iteKeyAction != _KeyAction.end()) &&
           (iteKeyAction->first == keyDown.Key)) {
    ...
    if (ite != _ComboAction.end()) {
      ...
      if (isActionPresentInContext(iteAction->first)) {
        // Run the action
        if (iteAction->second.KeyDown &&
            (iteAction->second.Repeat || keyDown.FirstTime)) {
...
void CActionsManager::keyReleased(const CEventKeyUp &keyUp) {
  if (_Enabled) {
    ...
    while (iteWatchedAction != _WatchedActions.end()) {
      ...
      TActionComboMap::iterator iteCombo =
          _ActionCombo.find(iteToDelete->second);
...
void CActionsManager::updateKeyButton(NLMISC::TKeyButton newButtons) {
  ...
  while (iteWatchedAction != _WatchedActions.end()) {
    ...
    while ((iteWatchedAction != _WatchedActions.end()) &&
           (key == iteWatchedAction->first)) {
      // Get the action combo
      TActionComboMap::iterator iteCombo =
          _ActionCombo.find(iteWatchedAction->second);
      if (iteCombo == _ActionCombo.end())
        nlwarning("try to find Name:%s , Argu:%s",
                  iteWatchedAction->second.Name.c_str(),
                  iteWatchedAction->second.Argu.c_str());
      nlassert(iteCombo != _ActionCombo.end());

      // Get the matching note
      sint matchingNote =
          getMatchingNote(iteCombo->second.KeyButtons, newButtons);
      if (matchingNote > bestMatchingNote) {
        // Get the action
        TActionsMap::iterator iteAction =
            _Actions.find(iteWatchedAction->second);
        nlassert(iteAction != _Actions.end());
    ...
    while (iteWatchedActionBegin != iteWatchedAction) {
      // Get the action
      TActionsMap::iterator iteAction =
          _Actions.find(iteWatchedActionBegin->second);
      nlassert(iteAction != _Actions.end());
...
void CActionsManager::addCategory(const CCategory &category) {
  ...
  for (i = 0; i < category.BaseActions.size(); i++) {
    ...
    _ActionCategory.insert(TActionBaseActionMap::value_type(
        category.BaseActions[i].Name, locator));
  }
}
...
void CActionsManager::enable(bool enable) { _Enabled = enable; }
...
bool CBaseAction::isUsableInCurrentContext() const {
  if (ActionsContext.matchContext(Contexts)) {
    ...
    if (!cteParamFound || matchingParamFound)
      return true;
  }
  return false;
}
...
static bool getParam(CBaseAction::CParameter::TType type, string &paramName,
                     string &paramValue, const std::string &argu,
                     uint paramId) {
  ...
  for (i = 0; i < Parameters.size(); i++) {
    ...
    if (getParam(parameter.Type, paramName, paramValue, name.Argu, i)) {
      ...
      case CParameter::Hidden:
        if ((parameter.DefaultValue == paramValue) &&
        ...
        for (j = 0; j < parameter.Values.size(); j++) {
          ...
          if (value.Value == paramValue) {
            ...
            if ((value.LocalizedValue.size() >= 2) &&
                (value.LocalizedValue[0] == 'u') &&
                (value.LocalizedValue[1] == 'i'))
...
const CActionsManager::TComboActionMap &
...
const CActionsManager::TActionComboMap &
...
const CActionsManager::CCategoryLocator *
CActionsManager::getActionLocator(const CAction::CName &name) const {
    ...
    for (i = 0; i < s; i++) {
      ...
      if (getParam(parameter.Type, paramName, paramValue, name.Argu, i)) {
        ...
        case CBaseAction::CParameter::Constant: {
          // If the value of the action param match with one of the values of
          // the base action param so its ok
          uint j;
          for (j = 0; j < parameter.Values.size(); j++) {
...
// ***************************************************************************
const CBaseAction *
CActionsManager::getBaseAction(const CAction::CName &name) const {
...
void CActionsManager::removeBaseAction(const CAction::CName &name) {
  ...
  std::vector<CBaseAction> &baseActions =
      _Categories[pCL->CategoryId].BaseActions;
  baseActions.erase(baseActions.begin() + pCL->BaseActionId);
}

// ***************************************************************************
const CCategory *
CActionsManager::getCategory(const CAction::CName &name) const {
  const CCategoryLocator *pCL = getActionLocator(name);
  if (pCL == NULL)
    return NULL;
...
// ***************************************************************************
void CActionsManager::forceDisplayForAction(const CAction::CName &name,
                                            bool state) {
  if (state)
...
// ***************************************************************************
string
CActionsManager::getActionLocalizedText(const CAction::CName &name) const {
  const CBaseAction *baseAction = getBaseAction(name);
  if (!baseAction)
    return string();
...
CActionsContext::CActionsContext() { _Context = "game"; }

bool CActionsContext::addActionsManager(CActionsManager *actionManager,
                                        const std::string &category) {
  return _ActionsManagers
      .insert(TActionsManagerMap::value_type(category, actionManager))
      .second;
}

// ***************************************************************************
CActionsManager *
CActionsContext::getActionsManager(const std::string &category) const {
...
bool CActionsContext::matchContext(const std::string &contexts) const {
  ...
  for (uint k = 0; k < contextList.size(); ++k) {
    ...
    while (strFindReplace(currContext, "\t", ""))
      ;
    if (nlstricmp(currContext, _Context) == 0)
      return true;
  }
  return false;
}
...
void CActionsContext::removeAllCombos() {
  for (TActionsManagerMap::iterator it = _ActionsManagers.begin();
       it != _ActionsManagers.end(); ++it) {
