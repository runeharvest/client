...
public:
  ...
  static void init(NL3D::UDriver *driver) { _Driver = driver; }
  ...
  CGraph(std::string name, float x, float y, float width, float height,
         ...
         uint page, bool lineMode = false)
      : Name(name), X(x), Y(y), Width(width), Height(height),
        BackColor(backColor), MaxValue(maxValue), Peak(0.0f),
        LineMode(lineMode), PrevY(y), Page(page), Quantum(quantum),
...
public:
  /// Constructor
  CSlotGraph(std::string name, float x, float y, float width, float height,
             NLMISC::CRGBA backColor, NLMISC::TTime quantum, float maxValue,
             bool lineMode, uint8 slot)
      : CGraph(name, x, y, width, height, backColor, quantum, maxValue,
               lineMode),
        _Slot(slot) {}
