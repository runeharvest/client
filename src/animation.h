...
public:
  ...
  enum TAnimId { UnknownAnim = 0xffffffff };
...
public:
  ...
  /// Must apply the Character scale Pos factor to the "pos" channel" of this
  /// animation.
  bool applyCharacterScalePosFactor() const {
    return _Sheet->ApplyCharacterScalePosFactor;
  }
  ...
  bool filterOk(uint32 jobSpecialisation, EGSPD::CPeople::TPeople race) const {
    ...
    if (_Sheet->JobRestriction == 0 &&
        _Sheet->RaceRestriction == EGSPD::CPeople::Unknown)
      return true;
    // if job restriction don't match
    if (_Sheet->JobRestriction != 0 &&
        _Sheet->JobRestriction != jobSpecialisation)
      return false;
    // if race restriction don't match
    if (_Sheet->RaceRestriction != EGSPD::CPeople::Unknown &&
        _Sheet->RaceRestriction != race)
      return false;
    // else filter ok
    return true;
  }
...
private:
  void computeAnimation(NL3D::UAnimationSet *animationSet,
