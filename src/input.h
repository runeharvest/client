...
// get state of mouse button, as a bitfield formatted like NLMISC::TMouseButton
// (modifier keys are not included)
uint GetMouseButtonsState();

// 'nice' the inputs : disable direct input in ctor, then re-enable if needed in
// dtor useful for non-blocking loading. NB : CNiceInputAuto maintains an
// internal counter, so sevral nested CNiceInputAuto's will remain nice
