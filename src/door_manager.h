...
class CDoorManager : public NL3D::ITransformName {
  ...
  struct SDoor {
    ...
    std::vector<NLPACS::UMovePrimitive *>
        Prims;                           // All collisions prims for that door
    NL3D::UInstanceGroup *InstanceGroup; // Instance Group where the door is.
    std::vector<uint32> Instances;       // Shapes making the door
    std::vector<NLMISC::CMatrix>
        InstMat;                       // Matrix of the shapes making the door
...
private:
  virtual std::string transformName(uint index, const std::string &instanceName,
