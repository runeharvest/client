...
// Constructor.
//-----------------------------------------------
CPlayerR2CL::CPlayerR2CL() : CCharacterCL() {
  Type = NPC;
...
bool CPlayerR2CL::build(const CEntitySheet *sheet) // virtual
{
  ...
  if (_Sheet == 0) {
    pushDebugStr(NLMISC::toString(
        "R2 Player '%d' sheet is not a '.creature' -> BIG PROBLEM.", _Slot));
    return false;
  } else
...
//-----------------------------------------------
void CPlayerR2CL::equip(SLOTTYPE::EVisualSlot slot,
                        const std::string &shapeName, const CItemSheet *item) {
  // Check slot.
...
void CPlayerR2CL::equip(SLOTTYPE::EVisualSlot slot, uint index, uint color) {
  ...
  // Default equipment.
    ...
    // sint idx =
    // SheetMngr.getVSIndex(_PlayerSheet->GenderInfos[_Gender].Items[slot],
    // slot);
...
//-----------------------------------------------
void CPlayerR2CL::updateVisualPropertyVpa(
  ...
  if (skeleton() && !ClientCfg.Light) {
    ...
    const CItemSheet *newRightSheet = SheetMngr.getItem(
        SLOTTYPE::RIGHT_HAND_SLOT, visualA.PropertySubData.WeaponRightHand);
    if ((oldRightSheet && newRightSheet &&
         oldRightSheet->Id != newRightSheet->Id) ||
        (!oldRightSheet && newRightSheet)) {
      changeWeapon = true;
    }
    ...
    const CItemSheet *newLeftSheet = SheetMngr.getItem(
        SLOTTYPE::LEFT_HAND_SLOT, visualA.PropertySubData.WeaponLeftHand);
    if ((oldLeftSheet && newLeftSheet &&
         oldLeftSheet->Id != newLeftSheet->Id) ||
        (!oldLeftSheet && newLeftSheet)) {
      changeWeapon = true;
    }
...
//-----------------------------------------------
void CPlayerR2CL::updateVisualPropertyVpb(
    const NLMISC::TGameCycle & /* gameCycle */, const sint64 &prop) {
  // Get the property.
...
void CPlayerR2CL::updateVisualPropertyVpc(
    const NLMISC::TGameCycle & /* gameCycle */, const sint64 &prop) {
  if (skeleton()) {
    // Get the property.
...
// *********************************************************************************************
/*
const char *CPlayerR2CL::getBoneNameFromBodyPart(BODY::TBodyPart part,
BODY::TSide side) const
{
        if (!_PlayerSheet) return CCharacterCL::getBoneNameFromBodyPart(part,
side); return _PlayerSheet->BodyToBone.getBoneName(part, side);
}*/
...
// ***************************************************************************
void CPlayerR2CL::doSetVisualSelectionBlink(bool bOnOff,
