...
CGroundFXManager::CGroundFXManager()
    : _MinSpeed(1.5f), _MaxSpeed(6.f), _SpeedWaterWalkFast(3.f),
      _SpeedWaterSwimFast(2.f), _MaxDist(50.f), _MaxNumFX(10), _NumFX(0),
      _MaxNumCachedFX(10), _NumCachedFX(0), _NumInstances(0),
      _Scene(NULL){
...
void CGroundFXManager::reset() {
  ...
  if (_Scene) {
    ...
    for (it = _ActiveFXs.begin(); it != _ActiveFXs.end(); ++it) {
      if (!it->FX.empty())
        _Scene->deleteInstance(it->FX);
      if (!it->FXUnderWater.empty())
        _Scene->deleteInstance(it->FXUnderWater);
    }
...
// *****************************************************************************
void CGroundFXManager::init(NL3D::UScene *scene, float maxDist, uint maxNumFX,
                            uint fxCacheSize) {
  H_AUTO_USE(RZ_GroundFXManager)
...
CGroundFXManager::TEntityHandle CGroundFXManager::add(CEntityCL *entity) {
  ...
  nlassert(entity);
#ifdef NL_DEBUG
  for (TInstanceList::iterator it = _InstancesList.begin();
       it != _InstancesList.end(); ++it) {
    if (it->Entity == entity) {
...
struct CCmpGroundIDPred {
  inline bool operator()(const CGroundFXSheet &lhs,
                         const CGroundFXSheet &rhs) const {
    return lhs.GroundID < rhs.GroundID;
  }
};

// *****************************************************************************
void CGroundFXManager::CInstance::getFXNameFromGroundType(
    uint32 groundID, std::string &fxName) const {
  H_AUTO_USE(RZ_GroundFXManager)
  fxName.clear();
  if (!Entity)
    return;
  ...
  std::vector<CGroundFXSheet>::const_iterator it =
      std::lower_bound(gfx->begin(), gfx->end(), tmp, CCmpGroundIDPred());
  if (it == gfx->end())
    return;
  if (it->GroundID != groundID)
    return;
  fxName = it->getFXName();
}
...
struct CSortInstancePred {
  bool operator()(CGroundFXManager::TEntityHandle lhs,
                  CGroundFXManager::TEntityHandle rhs) const {
...
// *****************************************************************************
void CGroundFXManager::moveFXInCache(TGroundFXList &ownerList,
                                     TGroundFXHandle fx) {
  H_AUTO_USE(RZ_GroundFXManager)
  if (!fx->FX.empty())
    fx->FX.hide();
  ...
  if (_NumCachedFX == _MaxNumCachedFX) {
    ...
    if (!_CachedFXs.back().FX.empty())
      _Scene->deleteInstance(_CachedFXs.back().FX);
    if (!_CachedFXs.back().FXUnderWater.empty())
      _Scene->deleteInstance(_CachedFXs.back().FXUnderWater);
    _CachedFXs.pop_back();
  } else {
    ++_NumCachedFX;
  }
...
void CGroundFXManager::checkIntegrity() {
  H_AUTO_USE(RZ_GroundFXManager)
  for (TGroundFXList::iterator it = _InactiveFXs.begin();
       it != _InactiveFXs.end(); ++it) {
    if (!it->FX.empty())
      nlassert(!it->FX.hasActiveEmitters());
    if (!it->FXUnderWater.empty())
      nlassert(!it->FXUnderWater.hasActiveEmitters());
  }
  for (TGroundFXList::iterator it = _CachedFXs.begin(); it != _CachedFXs.end();
       ++it) {
    if (!it->FX.empty()) {
      nlassert(!it->FX.hasActiveEmitters());
...
void CGroundFXManager::update(const NLMISC::CVectorD &camPos) {
  ...
  for (TGroundFXList::iterator currIt = _InactiveFXs.begin();
       currIt != _InactiveFXs.end();) {
    nextIt = currIt;
    ++nextIt;
    bool somethingVisible = false;
    if (!currIt->FX.empty() && currIt->FX.isSystemPresent() &&
        currIt->FX.hasParticles()) {
      somethingVisible = true;
    }
    if (!currIt->FXUnderWater.empty() &&
        currIt->FXUnderWater.isSystemPresent() &&
        currIt->FXUnderWater.hasParticles()) {
      somethingVisible = true;
    }
    if (!somethingVisible) {
  ...
  for (TInstanceList::iterator instanceIt = _InstancesList.begin();
       instanceIt != _InstancesList.end(); ++instanceIt) {
    ...
    if (instanceIt->Entity->getLastClip()) {
      ...
      // As a matter of fact, snapToGround is not called at each frame for
      // entities that are not visible, so this causes a
      // very noticeable shift on z for the ground fx when the entity enters the
      // view pyramid.
      if (instanceIt->HasFX) {
      ...
      if (instanceIt->Dist2 > maxDist2) {
        ...
        // if entity is in water, fx can be generated even if the entity hasn't
        // moved
        if (instanceIt->Entity->mode() == MBEHAV::SWIM ||
            instanceIt->Entity->mode() == MBEHAV::MOUNT_SWIM) {
          instanceIt->Mode = CInstance::Swim;
          // retrieve water height
          CContinent *cont = ContinentMngr.cur();
          if (cont) {
            bool splashEnabled;
            if (cont->WaterMap.getWaterHeight(
                    CVector2f((float)entityPos.x, (float)entityPos.y),
                    instanceIt->WaterHeight, splashEnabled)) {
              if (splashEnabled) {
  ...
  CHECK_INTEGRITY
  // sort instances by distance (number of chosen instances is likely to be a
  // lot less than 200, so quick sort is ok)
  std::sort(_SortedInstances.begin(), _SortedInstances.end(),
            CSortInstancePred());
  ...
  for (k = 0; k < numValidInstances; ++k) {
    ...
    case CInstance::Water:
      if (speed == 0.f)
        fxName = "StepSwimIdle.ps";
      else if (speed > _SpeedWaterWalkFast)
        fxName = "StepSwimRun.ps";
      else
        fxName = "StepSwimWalk.ps";
    ...
    if (!fxName.empty()) {
      stdFXName = NLMISC::toLowerAscii(
          NLMISC::CFile::getFilenameWithoutExtension(fxName));
    }
    ...
    if (_SortedInstances[k]->HasFX) {
      if (_SortedInstances[k]->FXHandle->FXName == stdFXName) {
        ...
        if (!_SortedInstances[k]->EmittersActive) {
          // NB : we dont activate emitters has soon as the fx is allocated by
          // an entity, because of the way the fx works. As a matter of fact, if
          // an object move from A to B, the fx may spawn several particles on
          // [AB] in a single frame So if the FX was previously used by another
          // entity, a trail of particles may appear between the 2 entities when
        ...
        if (!_SortedInstances[k]->EmittersUnderWaterActive) {
          ...
          if (!_SortedInstances[k]->FXHandle->FXUnderWater.empty()) {
            if (_SortedInstances[k]->FXHandle->FXUnderWater.isSystemPresent()) {
              _SortedInstances[k]->FXHandle->FXUnderWater.activateEmitters(
                  true);
    ...
    if (createNewFx) {
      ...
      if (!_InactiveFXs.empty()) {
        ...
        for (TGroundFXList::iterator it = _InactiveFXs.begin();
          ...
          if (it->FXName == stdFXName) {
            ...
            if ((_SortedInstances[k]->Entity->pos() -
                 it->FX.getMatrix().getPos())
                    .sqrnorm() <
                MAX_DIST_TO_REUSE_OLD_FX * MAX_DIST_TO_REUSE_OLD_FX) {
      ...
      // We can't reuse inactive fx (those that are shuttingdown), because we
      // wait for all particles to disappear, so that the bbox is null
      if (instanciate && !_CachedFXs.empty()) {
        for (TGroundFXList::iterator it = _CachedFXs.begin();
             it != _CachedFXs.end(); ++it) {
          if (it->FXName == stdFXName) {
            //
            if (!it->FX.empty())
              it->FX.show();
    ...
    // update Pos & matrix
    if (_SortedInstances[k]->HasFX &&
        !_SortedInstances[k]->FXHandle->FX.empty()) {
      ...
      _SortedInstances[k]->FXHandle->FX.setClusterSystem(
          _SortedInstances[k]->Entity->getClusterSystem());
      if (!_SortedInstances[k]->FXHandle->FXUnderWater.empty()) {
        _SortedInstances[k]->FXHandle->FXUnderWater.setMatrix(mat);
        _SortedInstances[k]->FXHandle->FXUnderWater.setClusterSystem(
            _SortedInstances[k]->Entity->getClusterSystem());
      }
      if (_SortedInstances[k]->Mode == CInstance::Ground) {
        float intensity;
      ...
      {
              // adjust z-bias (tmp)
              if (_SortedInstances[k]->FXHandle->FXUnderWater)
      _SortedInstances[k]->FXHandle->FXUnderWater.setZBias(-fxZBias); if
      (_SortedInstances[k]->FXHandle->FX)
      _SortedInstances[k]->FXHandle->FX.setZBias(fxZBias);
      }*/
    }
  }
  ...
  for (uint k = 0; k < Entities.size(); ++k) {
    if (MoveAll || Entities[k].Move) {
      ...
      NLMISC::CVectorD newPos =
          NLMISC::CVectorD(DT * Entities[k].Dir) + Entities[k].Entity->pos();
      NLMISC::CVectorD startToPos =
          newPos - NLMISC::CVectorD(Entities[k].StartPos);
      if (startToPos.norm() > 10.0) {
        newPos =
            NLMISC::CVectorD(Entities[k].StartPos) + 10.0 * startToPos.normed();
      }
...
void CTestGroundFX::displayFXBoxes() const {
  ...
  for (uint k = 0; k < TestGroundFX.Entities.size(); ++k) {
    ...
    drawBox(CVector(-size, -size, -size), CVector(size, size, size),
            CRGBA::Magenta);
    mat.identity();
    ...
    TextContext->render3D(
        mat,
        NLMISC::toString("gfx %d, dist = %.1f", (int)k,
                         (pos - Scene->getCam().getMatrix().getPos()).norm()));
  }
}
...
NLMISC_COMMAND(gfxAdd, "gfxAdd", "<>") {
  ...
  if (entity) {
    ...
    if (node) {
      ...
      node = NLGUI::CDBManager::getInstance()->getDbProp(
          "SERVER:Entities:E" + NLMISC::toString("%d", slot) + ":P" +
              NLMISC::toString("%d", CLFECOMMON::PROPERTY_POSY),
          false);
      if (node) {
        node->setValue64(y);
        node = NLGUI::CDBManager::getInstance()->getDbProp(
            "SERVER:Entities:E" + NLMISC::toString("%d", slot) + ":P" +
                NLMISC::toString("%d", CLFECOMMON::PROPERTY_POSZ),
            false);
        if (node) {
          node->setValue64(z);
    ...
    node = NLGUI::CDBManager::getInstance()->getDbProp(
        "SERVER:Entities:E" + NLMISC::toString("%d", slot) + ":P" +
            NLMISC::toString("%d", CLFECOMMON::PROPERTY_MODE),
        false);
    if (node) {
    ...
    if (dynamic_cast<CPlayerCL *>(entity)) {
      ...
      NLGUI::CDBManager::getInstance()
          ->getDbProp("SERVER:Entities:E" + NLMISC::toString("%d", slot) +
...
NLMISC_COMMAND(gfxMove, "gfxMove", "<>") {
  ...
  if (index > TestGroundFX.Entities.size())
    return false;
  TestGroundFX.Entities[index].Move = true;
  TestGroundFX.Entities[index].Dir.set(NLMISC::frand(1.f), NLMISC::frand(1.f),
                                       0.f);
...
NLMISC_COMMAND(gfxMoveAll, "gfxMoveAll", "<>") {
  for (uint k = 0; k < TestGroundFX.Entities.size(); ++k) {
    TestGroundFX.Entities[k].Dir.set(NLMISC::frand(1.f), NLMISC::frand(1.f),
                                     0.f);
