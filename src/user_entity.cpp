...
extern void beastOrder(const std::string &orderStr,
                       const std::string &beastIndexStr,
...
// Constructor.
//-----------------------------------------------
CUserEntity::CUserEntity() : CPlayerCL() {
  Type = User;
  _Run = false;
  _RunWhenAble = false;
  _WalkVelocity = 1.0f;
...
CUserEntity::~CUserEntity() {
...

  {
    CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
        "SERVER:USER:IS_INVISIBLE", false);
    if (node) {
  ...
  for (uint i = 0; i < EGSPD::CSPType::EndSPType; i++) {
    CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
        toString("SERVER:USER:SKILL_POINTS_%d:VALUE", i), false);
    if (node) {
      ICDBNode::CTextId textId;
  ...
  // Remove the Primitive used for check (because ~CEntityCL() will call
  // CEntityCL::removePrimitive(), not CUserEntity::removePrimitive())
...
bool CUserEntity::build(const CEntitySheet *sheet) // virtual
{
  ...
  if (initPrimitive(0.5f, 2.0f, 0.0f, 0.0f, UMovePrimitive::Slide,
                    (UMovePrimitive::TTrigger)(UMovePrimitive::OverlapTrigger |
                                               UMovePrimitive::EnterTrigger),
                    MaskColPlayer, MaskColPlayer | MaskColNpc | MaskColDoor))
  ...
  if (ClientCfg.Local) {
    ...
    NLGUI::CDBManager::getInstance()
        ->getDbProp("SERVER:Entities:E0:P" +
                    toString("%d", CLFECOMMON::PROPERTY_VPA))
        ->setValue64(*prop); // Set the database
    prop = (sint64 *)&visualB;
    NLGUI::CDBManager::getInstance()
        ->getDbProp("SERVER:Entities:E0:P" +
  ...
  for (uint i = (uint)PVP_CLAN::BeginClans; i <= (uint)PVP_CLAN::EndClans;
       ++i) {
...
//-----------------------------------------------
void CUserEntity::updateVisualPropertyPos(
    const NLMISC::TGameCycle & /* gameCycle */, const sint64 & /* prop */,
    const NLMISC::TGameCycle & /* pI */) {} // updateVisualPropertyPos //

//-----------------------------------------------
// updateVisualPropertyOrient :
// Update Entity Orientation.
//-----------------------------------------------
void CUserEntity::updateVisualPropertyOrient(
    const NLMISC::TGameCycle & /* gameCycle */, const sint64 & /* prop */) {
} // updateVisualPropertyOrient //

void CUserEntity::updateVisualPropertyTargetList(
    const NLMISC::TGameCycle & /* gameCycle */, const sint64 & /* prop */,
    uint /* listIndex */) {}

void CUserEntity::updateVisualPropertyVisualFX(
    const NLMISC::TGameCycle & /* gameCycle */, const sint64 &prop) {
  applyVisualFX(prop);
}
...
//-----------------------------------------------
void CUserEntity::updateVisualPropertyBehaviour(
    const NLMISC::TGameCycle & /* gameCycle */, const sint64 &prop) {
  ...
  if (VerboseAnimUser) {
    nlinfo("UE::updateVPBeha: '%d(%s)'.", (sint)bc.Behav.Behaviour,
           MBEHAV::behaviourToString(bc.Behav.Behaviour).c_str());
  }
  CCDBNodeLeaf *targetList0 = dynamic_cast<CCDBNodeLeaf *>(
      _DBEntry->getNode(CLFECOMMON::PROPERTY_TARGET_LIST_0));
  CCDBNodeLeaf *targetList1 = dynamic_cast<CCDBNodeLeaf *>(
      _DBEntry->getNode(CLFECOMMON::PROPERTY_TARGET_LIST_1));
  CCDBNodeLeaf *targetList2 = dynamic_cast<CCDBNodeLeaf *>(
      _DBEntry->getNode(CLFECOMMON::PROPERTY_TARGET_LIST_1));
  CCDBNodeLeaf *targetList3 = dynamic_cast<CCDBNodeLeaf *>(
      _DBEntry->getNode(CLFECOMMON::PROPERTY_TARGET_LIST_1));
  if (targetList0 && targetList1 && targetList2 && targetList3) {
    uint64 vp[4] = {
        (uint64)targetList0->getValue64(), (uint64)targetList1->getValue64(),
        (uint64)targetList2->getValue64(), (uint64)targetList3->getValue64()};
...
// Update Entity Name.
//-----------------------------------------------
void CUserEntity::updateVisualPropertyName(const NLMISC::TGameCycle &gameCycle,
                                           const sint64 &prop) {
  ...
  /*	if (oldNameId != _NameId)
     ...
     CWidgetManager::getInstance()->getElementFromId("ui:interface:mailbox:content:html");
                  if (element)
                  {
...
//-----------------------------------------------
void CUserEntity::updateVisualPropertyTarget(
    const NLMISC::TGameCycle & /* gameCycle */, const sint64 & /* prop */) {
...
// \warning Read the position or orientation from the database when reading the
// mode (no more updated in updateVisualPropertyPos and
// updateVisualPropertyOrient).
//-----------------------------------------------
void CUserEntity::updateVisualPropertyMode(
    const NLMISC::TGameCycle & /* gameCycle */, const sint64 &prop) {
  // Combat Float Check
  if ((MBEHAV::EMode)prop == MBEHAV::COMBAT_FLOAT) {
    nlwarning(
        "UE:updateVPMode: the user should never have the COMBAT_FLOAT mode.");
    return;
  }
  // New Mode Received.
...
//-----------------------------------------------
void CUserEntity::updateVisualPropertyVpa(const NLMISC::TGameCycle &gameCycle,
                                          const sint64 &prop) {
  CPlayerCL::updateVisualPropertyVpa(gameCycle, prop);

  // Special for user: Disable Character Lod, because always want it at full
  // rez.
  if (!_Skeleton.empty()) {
...
//-----------------------------------------------
void CUserEntity::updateVisualPropertyVpb(const NLMISC::TGameCycle &gameCycle,
                                          const sint64 &prop) {
...
//-----------------------------------------------
void CUserEntity::updateVisualPropertyVpc(const NLMISC::TGameCycle &gameCycle,
                                          const sint64 &prop) {
...
// Update Entity Mount
//-----------------------------------------------
void CUserEntity::updateVisualPropertyEntityMounted(
    const NLMISC::TGameCycle &gameCycle, const sint64 &prop) {
  if (isFighting())
...
// Update Entity Rider
//-----------------------------------------------
void CUserEntity::updateVisualPropertyRiderEntity(
    const NLMISC::TGameCycle &gameCycle, const sint64 &prop) {
  if (isFighting())
...
//-----------------------------------------------
//-----------------------------------------------
void CUserEntity::updateVisualPropertyPvpMode(
    const NLMISC::TGameCycle &gameCycle, const sint64 &prop) {
  CPlayerCL::updateVisualPropertyPvpMode(gameCycle, prop);
  // Additionaly, inform interface of the change
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  // For PVP ZoneFaction
  CCDBNodeLeaf *pDB = NLGUI::CDBManager::getInstance()->getDbProp(
      "UI:VARIABLES:USER:TRACK_PVP_CHANGE_MODE");
  if (pDB) {
    sint32 val = pDB->getValue32();
    pDB->setValue32(val + 1);
  }
  // For Any PVP change
  pDB = NLGUI::CDBManager::getInstance()->getDbProp(
      "UI:VARIABLES:USER:TRACK_PVP_CHANGE_ANY");
  if (pDB) {
    sint32 val = pDB->getValue32();
    pDB->setValue32(val + 1);
  }
...
//-----------------------------------------------
//-----------------------------------------------
void CUserEntity::updateVisualPropertyOutpostInfos(
    const NLMISC::TGameCycle &gameCycle, const sint64 &prop) {
...
//-----------------------------------------------
//-----------------------------------------------
void CUserEntity::updateVisualPropertyPvpClan(
    const NLMISC::TGameCycle &gameCycle, const sint64 &prop) {
...
// \todo GUIGUI : go or teleport the player to the mode position (see how to
// manage it).
//-----------------------------------------------
bool CUserEntity::mode(MBEHAV::EMode m) {
  if (Verbose & VerboseAnim)
    nlinfo("UE::mode: old mode '%s(%d)' new mode '%s(%d)'.",
  ...
  case MBEHAV::MOUNT_SWIM: {
    ...
    if (m != MBEHAV::MOUNT_NORMAL && m != MBEHAV::MOUNT_SWIM) {
      ...
      if (mount) {
        ...
        if (mount->getPrimitive())
          mount->getPrimitive()->setOcclusionMask(
              MaskColNpc); // the mount is an npc
      ...
      if (_Primitive) {
        _Primitive->setRadius(
            std::min(0.5f, (float)(RYZOM_ENTITY_SIZE_MAX / 2)));
    ...
    if (_Mode == MBEHAV::MOUNT_SWIM &&
        (m == MBEHAV::COMBAT || m == MBEHAV::COMBAT_FLOAT)) {
  ...
  // Change the Mode for the user ( if user sits down or stands up we wait in
  // order to play the sit/stand transition anim)
  ...
  case MBEHAV::COMBAT_FLOAT: {
    ...
    // Compute the angle
    const string propName = toString("SERVER:Entities:E%d:P%d", _Slot,
  ...
  case MBEHAV::MOUNT_NORMAL: {
    CCharacterCL *mount =
        dynamic_cast<CCharacterCL *>(EntitiesMngr.entity(parent()));
    if (mount) {
  ...
  case MBEHAV::MOUNT_SWIM: {
    ...
    if (_Mode != MBEHAV::MOUNT_SWIM && _Mode != MBEHAV::MOUNT_NORMAL) {
      if (_Mount !=
          CLFECOMMON::INVALID_SLOT) // if _Mount is still invalid, the following
                                    // code will be done in updatePos()
      {
        setOnMount();
      }
    }
  ...
  bool doSetAnim = true;
  // if user sits down or stands up we set transition anim before changing
  // animset
  if (m == MBEHAV::SIT) {
    setAnim(CAnimationStateSheet::SitMode);
    _Mode = m;
    doSetAnim = false;
  } else if (_Mode == MBEHAV::SIT && m != MBEHAV::DEATH) {
...
void CUserEntity::setOnMount() {
  ...
  if (mount) {
    ...
    if (_Primitive && mount->getPrimitive()) {
      _Primitive->setRadius(std::min(mount->getPrimitive()->getRadius(),
                                     (float)(RYZOM_ENTITY_SIZE_MAX / 2)));
      _Primitive->setHeight(mount->getPrimitive()->getHeight());
      mount->getPrimitive()->setOcclusionMask(
          MaskColNone); // Remove collisions.
    }
    // Now on a mount
    _OnMount = true;
...
CVector CUserEntity::getVelocity() const {
  ...
  // User is swimming
    ...
    if (_R2CharMode == R2::TCharMode::Editer ||
        _R2CharMode == R2::TCharMode::Dm) {
...
void CUserEntity::applyMotion(CEntityCL *target) {
  ...
  if (_MoveToSlot != CLFECOMMON::INVALID_SLOT) {
    ...
    CVectorD aimingPos = target->getAttackerPos(
        angToTarget, attackRadius() + ClientCfg.AttackDist);
    // Aiming Position
    ...
    if (distToAimingPos > 0.5) {
      // Because of Tryker Counter, may increase the Threshold, when the player
      // is stalled
      if (distToAimingPos < _MoveToDist) {
        // If the player is stalled too much time, abort the move and launch the
        // action
        ...
        if (_MoveToColStartTime &&
            (T1 - _MoveToColStartTime >= ClientCfg.MoveToTimeToStopStall))
          targetReached = true;
      } else
        _MoveToColStartTime = 0;
    } else
    ...
    // else continue follow
      ...
      // Look at the entity. delay it after pacs evalCollision(), for correct
      // orientation
      _ForceLookSlot = target->slot();
      // but still estimate now an approximative front (may be used between now
      // and applyForceLook())
    ...
    //		if(target->hasMoved())
    {
      ...
      if (dir2targ != CVectorD::Null) {
        ...
        // orientation
        _ForceLookSlot = target->slot();
        // but still estimate now an approximative front (may be used between
        // now and applyForceLook())
        forceLookEntity(dir2targ, false);
      }
    }
    ...
    const CVectorD aimingPos = target->getAttackerPos(
        angToTarget, attackRadius() + ClientCfg.AttackDist);
    // Aiming Position
    CVectorD dirToAimingPos = aimingPos - pos();
    dirToAimingPos.z = 0.0;
    const double distToAimingPos = dirToAimingPos.norm();
    // If the User was not moving and the distance to re-move is not enough big,
    // stay idle
    if (lastHasMoved || (isFighting() && distToAimingPos >= 0.5) ||
  ...
  // Third Person View
    ...
    _Stages.addStage(NetMngr.getCurrentClientTick() + time,
                     CLFECOMMON::PROPERTY_POSX, x, 0);
    _Stages.addStage(NetMngr.getCurrentClientTick() + time,
                     CLFECOMMON::PROPERTY_POSY, y);
    _Stages.addStage(NetMngr.getCurrentClientTick() + time,
                     CLFECOMMON::PROPERTY_POSZ, z);
    // Move the Mount
    if (mount) {
      mount->_Stages.addStage(NetMngr.getCurrentClientTick() + time,
                              CLFECOMMON::PROPERTY_POSX, x, 0);
      mount->_Stages.addStage(NetMngr.getCurrentClientTick() + time,
                              CLFECOMMON::PROPERTY_POSY, y);
      mount->_Stages.addStage(NetMngr.getCurrentClientTick() + time,
...
//---------------------------------------------------
void CUserEntity::updatePosCombatFloatChanged(
...
void CUserEntity::enableFollow(bool resetCameraRot) {
  ...
  // Disable any autowalk (else when follow re-disabled, it will effect, which
  // is weird)
...
void CUserEntity::resetAnyMoveTo() {
  // if we cancel a MoveToPhrase action, MUST dec the action counter, and hide
  // the Action Icon
  if (_MoveToAction == CUserEntity::CombatPhrase ||
      _MoveToAction == CUserEntity::ExtractRM) {
    // the clientExecute has not been called in case of "ExtractRM autoFind"
    bool autoFindExtractRM =
        _MoveToAction == CUserEntity::ExtractRM &&
        _MoveToPhraseMemoryLine == std::numeric_limits<uint>::max();
...
//-----------------------------------------------
void CUserEntity::moveToCheckStartDist(CLFECOMMON::TCLEntityId slot,
                                       double dist,
  ...
  if (_MoveToSlot != CLFECOMMON::INVALID_SLOT) {
    ...
    // Disable any autowalk (else when moveTo re-disabled, it will effect, which
    // is weird)
    UserControls.autowalkState(false);
...
//-----------------------------------------------
void CUserEntity::moveTo(CLFECOMMON::TCLEntityId slot, double dist,
                         TMoveToAction action) {
  resetAnyMoveTo();
...
//-----------------------------------------------
// moveToMission :
// Method to move to someone else for a mission. NB: if prec action was a
// CombatPhrase action, action counter are decremented
//-----------------------------------------------
void CUserEntity::moveToMission(CLFECOMMON::TCLEntityId slot, double dist,
                                uint32 id) {
  resetAnyMoveTo();
...
//-----------------------------------------------
// moveToMissionRing :
// Method to move to someone else for a ring mission. NB: if prec action was a
// CombatPhrase action, action counter are decremented
//-----------------------------------------------
void CUserEntity::moveToMissionRing(CLFECOMMON::TCLEntityId slot, double dist,
                                    uint32 id) {
  resetAnyMoveTo();
...
// Method to move to someone else.
//-----------------------------------------------
void CUserEntity::moveToCombatPhrase(CLFECOMMON::TCLEntityId slot, double dist,
                                     uint phraseMemoryLine,
                                     uint phraseMemorySlot, bool phraseCyclic) {
  resetAnyMoveTo();
...
// The caller MUST call after CSPhraseManager::clientExecute(), to increment
// action counter
//-----------------------------------------------
void CUserEntity::moveToExtractionPhrase(CLFECOMMON::TCLEntityId slot,
                                         double dist, uint phraseMemoryLine,
                                         uint phraseMemorySlot, bool cyclic) {
  ...
  if (!validForageToolInHand) {
    ...
    if (i != MAX_BAGINV_ENTRIES && ClientCfg.AutoEquipTool) {
      ...
      CInterfaceManager::getInstance()->displaySystemInfo(
          CI18N::get("uiForageToolMissing"), "CHK");
      return;
    }
  }
...
// action counter
//-----------------------------------------------
void CUserEntity::moveToTotemBuildingPhrase(CLFECOMMON::TCLEntityId slot,
                                            double dist, uint phraseMemoryLine,
...
void CUserEntity::moveToAction(CEntityCL *ent) {
  ...
  case CUserEntity::Outpost:
    CLuaManager::getInstance().executeLuaScript(
        "game:outpostBCOpenStateWindow()", 0);
    break;
  // BuildTotem
...
bool CUserEntity::msgForCombatPos(NLMISC::CBitMemStream &out) {
  ...
  //	if(isFighting())
  {
    ...
    if (target)
      wellP = target->isPlacedToFight(UserEntity->pos(), front(),
  ...
  if (wellPosition != wellP) {
    ...
    if (GenericMsgHeaderMngr.pushNameToStream("COMBAT:VALIDATE_MELEE", out)) {
      ...
      nlwarning("UE:msgForCombatPos: unknown message named "
...
void CUserEntity::checkPos() {
  if (PACS && _Primitive) {
      ...
      if (GR->isWaterPosition(gPos, waterHeight)) {
        if (isSwimming() == false) {
          ...
          if (isDead())
            ...
            // \todo GUIGUI : if the move was cancelled, do not switch to
            // swimming mode.
            else if (isRiding()) {
    ...
    // Create the Primitive used to check if the move will be accepted by the
    // server
    ...
    if (_CheckPrimitive) {
      ...
      CVectorD speed = _Primitive->getFinalPosition(dynamicWI) -
                       _CheckPrimitive->getFinalPosition(dynamicWI);

      _CheckPrimitive->move(speed, dynamicWI);
      if (PACS->evalNCPrimitiveCollision(1.0, _CheckPrimitive, dynamicWI) ==
          false)
        nlwarning(
            "UE:checkPos: _CheckPrimitive is a collisionable primitive !!!");

      CVectorD vectDist = _Primitive->getFinalPosition(dynamicWI) -
                          _CheckPrimitive->getFinalPosition(dynamicWI);
      if (vectDist.norm() > 0.001) {
        // The server will not be able to reproduce this move, restoring the
        // last one.
        _HasMoved = false;
        pacsPos(_LastPositionValidated, _LastGPosValidated);
      } else {
...
bool CUserEntity::testPacsPos(CVectorD &posToTest) {
  if (PACS && _Primitive && _CheckPrimitive) {
    ...
    if (PACS->evalNCPrimitiveCollision(1.0, _CheckPrimitive, dynamicWI) ==
        false)
      nlwarning(
          "UE:testPacsPos: _CheckPrimitive is a collisionable primitive !!!");
...
void CUserEntity::tp(const CVectorD &dest) {
  ...
  if (_Primitive) {
    ...
    _Primitive->getGlobalPosition(_LastGPosValidated, dynamicWI);
    // consider this is the last position sent to server (since actually
    // received!)
    _Primitive->getGlobalPosition(_LastGPosSent, dynamicWI);
...
void CUserEntity::updatePos(const TTime &t, CEntityCL *target) {
  ...
  {
          // update anim state at least (needed when a spell is cast to play the
  cast fx at the right time, because they may be visible)
...
//-----------------------------------------------
// updateDisplay :
// Update the PACS position after the evalCollision. The entity position is set
// too. This is fast. If the entity position is too far from its PACS position,
// setGlobalPosition is called. After this call, the position.z is valid.
//-----------------------------------------------
...
//-----------------------------------------------
void CUserEntity::applyBehaviour(
    const CBehaviourContext &behaviourContext) // virtual
{
  ...
  if ((viewMode() == FirstPV) && behaviour.isCombat()) {
    ...
    if (isCurrentBehaviourAttackEnd()) {
      // In First Person (don't care), use a default animation => will choose a
      // default delay of 0.5
    ...
    if (EAM) {
      ...
      if (_CurrentState && _CurrentAnimSet[MOVE]) {
        ...
        if (animStatePtr) {
          animIndex(MOVE, animStatePtr->chooseAnim(_AnimJobSpecialisation,
                                                   people(), getGender(), 0.0));
          if (animIndex(MOVE) != CAnimation::UnknownAnim) {
            if (_PlayList) {
              _PlayList->setAnimation(MOVE, animId(MOVE));
              _PlayList->setTimeOrigin(MOVE, ryzomGetLocalTime() *
                                                 0.001); //_LastFrameTime);
              _AnimAttackOn = true;
              updateVisualDisplay();
            }
          }
  ...
  // In third person view (or camera mode), play the same way than for the
  // others.
  else
...
void CUserEntity::setDead() // virtual
{
  ...
  NL3D::UParticleSystemInstance deathFX =
      FXMngr.instantFX(ClientCfg.DeadFXName);
  if (!deathFX.empty())
  ...
  if (pIM && kamiFameIndex != CStaticFames::INVALID_FACTION_INDEX) {
    CCDBNodeLeaf *pLeafKamiFame = NLGUI::CDBManager::getInstance()->getDbProp(
        toString("SERVER:FAME:PLAYER%d:VALUE", kamiFameIndex - 1), false);
  ...
  if (pIM) {
    CAHManager::getInstance()->runActionHandler(
        "set", NULL, "dblink=UI:VARIABLES:DEATH_WARNING_WANTED|value=1");
  }
} // setDead //
...
void CUserEntity::skillUp() {
  // Play the FX for the death
  NL3D::UParticleSystemInstance skillUpFX =
      FXMngr.instantFX(ClientCfg.SkillUpFXName);
...
//-----------------------------------------------
// startColTimer :
// After a few time, if the user is still in collision with someone else, remove
// collisions with other entitites.
//-----------------------------------------------
...
{
  // Max Defined speed according to the current factor (slow, or speed
  // increased)
...
void CUserEntity::updateSound(const TTime &time) {
  ...
  // step sound of self : refind the sound animations associated to the walk and
  // run animation
  ...
  if (computeAnim && SoundMngr &&
      _CurrentAnimSet[MOVE] !=
          0) // && _SoundId[MOVE] != NLSOUND::CSoundAnimationNoId)
  {
    lastMode = _Run;
    TAnimStateId mode =
        _Run ? CAnimationStateSheet::Run : CAnimationStateSheet::Walk;
    const CAnimationState *animStatePtr =
        _CurrentAnimSet[MOVE]->getAnimationState(mode);
    if (animStatePtr) {
      ::CAnimation::TAnimId animId = animStatePtr->chooseAnim(
          _AnimJobSpecialisation, people(), getGender(), 0);
            ...
            if (!name.empty()) {
              ...
              if (sanim->countMarkers() != 2) {
                ...
                if (warnOnce.find(sanim->getName()) == warnOnce.end()) {
                  nlwarning("Sound animation '%s' has not 2 markers, not a "
                            "biped ? (Display Once)",
                            sanim->getName().c_str());
                  warnOnce.insert(sanim->getName());
                }
              } else {
                stepTime = TTime((sanim->getMarker(1)->getTime() -
                                  sanim->getMarker(0)->getTime()) *
                                 1000);
...
// \param slot : slot now selected (CLFECOMMON::INVALID_SLOT for an empty
// selection). \warning Can be different from the entity targeted (in combat
// mode for example).
//---------------------------------------------------
void CUserEntity::selection(const CLFECOMMON::TCLEntityId &slot) // virtual
{
  // allows reselection in Ring client: even if the selected slots is equal to
  // the selection, the client must send the messages.
  ...
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  NLGUI::CDBManager::getInstance()
      ->getDbProp("UI:VARIABLES:TARGET:SLOT")
      ->setValue64(slot);
  ...
  CCDBNodeLeaf *prop = NLGUI::CDBManager::getInstance()->getDbProp(
      "UI:VARIABLES:TARGET:UID", false);
  if (prop)
    prop->setValue32(tgtEntityId);

  // Bar Manager. Update now the Target View (so it takes VP if data available
  ...
  for (uint k = 0; k < NUM_MISSION_OPTIONS; ++k) {
    CCDBNodeLeaf *missionOption = NLGUI::CDBManager::getInstance()->getDbProp(
        toString("LOCAL:TARGET:CONTEXT_MENU:MISSIONS_OPTIONS:%d:TITLE", (int)k),
        false);
    if (missionOption) {
    ...
    CCDBNodeLeaf *playerGiftNeeded =
        NLGUI::CDBManager::getInstance()->getDbProp(
            toString("LOCAL:TARGET:CONTEXT_MENU:MISSIONS_OPTIONS:%d:PLAYER_"
                     "GIFT_NEEDED",
                     (int)k),
            false);
    if (playerGiftNeeded) {
  ...
  if ((tgtSlot != CLFECOMMON::INVALID_SLOT) && entity) {
    ...
    if (pPlayer) {
      /*// Pvp Mode
      CViewBitmap * tagMode =
      dynamic_cast<CViewBitmap*>(CWidgetManager::getInstance()->getElementFromId("ui:interface:target:pvp_tags:mode"));
      if (tagMode)
      {
...
*/
      ...
      CViewBitmap * tagMode =
      dynamic_cast<CViewBitmap*>(CWidgetManager::getInstance()->getElementFromId("ui:interface:target:pvp_tags:actions"));
      if (tagMode)
      {
              if (pPlayer->getPvpMode()&PVP_MODE::PvpFaction)
  ...
  // clear web page
  prop = NLGUI::CDBManager::getInstance()->getDbProp(
      "LOCAL:TARGET:CONTEXT_MENU:WEB_PAGE_URL", false);
  if (prop)
    prop->setValue32(0);
  prop = NLGUI::CDBManager::getInstance()->getDbProp(
      "LOCAL:TARGET:CONTEXT_MENU:WEB_PAGE_TITLE", false);
  if (prop)
    prop->setValue32(0);
  ...
  for (uint k = 0; k < BOTCHATTYPE::MaxR2MissionEntryDatabase; ++k) {
    prop = NLGUI::CDBManager::getInstance()->getDbProp(
        toString("LOCAL:TARGET:CONTEXT_MENU:MISSION_RING:%d:TITLE", k), false);
    if (prop)
      prop->setValue32(0);
  }
  ...
  // clear programs
  prop = NLGUI::CDBManager::getInstance()->getDbProp(
      "LOCAL:TARGET:CONTEXT_MENU:PROGRAMMES", false);
  if (prop)
    prop->setValue32(0);
  prop = NLGUI::CDBManager::getInstance()->getDbProp(
      "SERVER:TARGET:CONTEXT_MENU:PROGRAMMES");
  if (prop)
    prop->setValue32(0);
...
void CUserEntity::moveToAttack() {
  // **** For clarity, try to launch a "default attack" found in the memory bar
  // instead of an "anonymous" action
  CSPhraseManager *pPM = CSPhraseManager::getInstance();
  ...
  if (ClientCfg.AutoEquipTool) {
    if (inv) {
      ...
      uint32 rightHandSheet = inv->getRightHandItemSheet();
      if (rightHandSheet) {
        validWeaponInHand = inv->isMeleeWeaponItem(rightHandSheet) ||
                            inv->isRangeWeaponItem(rightHandSheet);
      }
...
void CUserEntity::attackWithPhrase() {
  ...
  pPM->sendExecuteToServer(_MoveToPhraseMemoryLine, _MoveToPhraseMemorySlot,
                           _MoveToPhraseCyclic);
  ...
  // Because sendExecuteToServer() has been called, must NOT
  // cancelClientExecute() at resetAnyMoveTo()
  _MoveToAction = CUserEntity::None;
}
...
//-----------------------------------------------
void CUserEntity::assist() { assist(targetSlot()); } // assist //
...
void CUserEntity::disengage() {
  ...
  if (ClientCfg.Local) {
    IngameDbMngr.setProp("Entities:E0:P" + toString(CLFECOMMON::PROPERTY_MODE),
                         MBEHAV::NORMAL); // Mode
    IngameDbMngr.setProp("Entities:E0:P" +
                             toString(CLFECOMMON::PROPERTY_TARGET_ID),
                         _Selection); // Target
...
bool CUserEntity::sit(bool s) {
  ...
  if (ok) {
    CInterfaceManager *pIM = CInterfaceManager::getInstance();
    CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
        "UI:VARIABLES:PLAYER_STAND", false);
    if (node)
...
void CUserEntity::setAFK(bool b, string afkTxt) {
  ...
  if (b) {
    ...
    CInterfaceManager *pIM = CInterfaceManager::getInstance();
    // sint64 start =
    // NLGUI::CDBManager::getInstance()->getDbProp("SERVER:USER:ACT_TSTART")->getValue64();
    // sint64 end =
    // NLGUI::CDBManager::getInstance()->getDbProp("SERVER:USER:ACT_TEND")->getValue64();
    // sint64 type =
    // NLGUI::CDBManager::getInstance()->getDbProp("SERVER:USER:ACT_TYPE")->getValue64();
    // sint64 num =
    // NLGUI::CDBManager::getInstance()->getDbProp("SERVER:USER:ACT_NUMBER")->getValue64();
    if (pIM) {
      if (NLGUI::CDBManager::getInstance()
              ->getDbProp("SERVER:USER:ACT_TYPE")
              ->getValue64() != 0)
        return;
    }
  ...
  if (GenericMsgHeaderMngr.pushNameToStream(msgName, out)) {
    ...
    nlwarning("CUserEntity:setAFK: unknown message named '%s'.",
              msgName.c_str());
...
void CUserEntity::rollDice(sint16 min, sint16 max, bool local) {
  ...
  if (GenericMsgHeaderMngr.pushNameToStream(msgName, out)) {
    ...
    nlwarning("CUserEntity:rollDice: unknown message named '%s'.",
              msgName.c_str());
} // rollDice //

//---------------------------------------------------
// canEngageCombat :
// return true if user can engage melee combat, else return false and display
// system msg
//---------------------------------------------------
...
// \todo GUIGUI : do we have to take care of the user's race kwnowing it can
// favour him ?
//---------------------------------------------------
...
//---------------------------------------------------
void CUserEntity::removePrimitive() // virtual (will not be called by
                                    // ~CEntityCL())
{
...
bool CUserEntity::isBusy() const {
  ...
  CCDBNodeLeaf *nod = NLGUI::CDBManager::getInstance()->getDbProp(
      "SERVER:INVENTORY:EXCHANGE:BEGUN", false);
  if (nod) {
    if (nod->getValueBool())
      return true;
  }
  //	else
  //		nlwarning("UE:isBusy: Cannot get the nod
  //'SERVER:INVENTORY:EXCHANGE:BEGUN'.");
  ...
  /*
          ...
          {
                  nod =
     NLGUI::CDBManager::getInstance()->getDbProp(NLMISC::toString("SERVER:INVENTORY:%d:%d:SHEET",
     INVENTORIES::pickup, i), false); if(nod)
                  {
                          if(nod->getValue32() != 0)
                                  return true;
                  }
                          ...
                          nlwarning("UE:isBusy: Cannot get the nod
     'SERVER:INVENTORY:%d:%d:SHEET'.", INVENTORIES::pickup, i);
          }
          ...
          for(i=0; i<nbSlot; ++i)
          {
                  nod =
     NLGUI::CDBManager::getInstance()->getDbProp(NLMISC::toString("SERVER:INVENTORY:%d:%d:SHEET",
     INVENTORIES::harvest, i), false); if(nod)
                  {
                          if(nod->getValue32() != 0)
                                  return true;
                  }
                          ...
                          nlwarning("UE:isBusy: Cannot get the nod
     'SERVER:INVENTORY:%d:%d:SHEET'.", INVENTORIES::harvest, i);
          }
  */
...
void CUserEntity::updateVisualDisplay() {
  ...
  if (UserControls.isInternalView() || View.forceFirstPersonView()) {
    ...
    if (_Mount != CLFECOMMON::INVALID_SLOT) {
      ...
      CCharacterCL *mount =
          dynamic_cast<CCharacterCL *>(EntitiesMngr.entity(_HiddenMount));
      if (mount)
        mount->displayable(true);
...
void CUserEntity::light() {
  // Create the light
  if (_Light.empty()) {
    // Check there is a skeleton and a bone to stick the light before to create
    // it.
    if (!_Skeleton.empty() && _NameBoneId != -1) {
      ...
      if (!_Light.empty()) {
        ...
        // The player light is the only one who can interact with Lightmapped
        // objects
      ...
      nlwarning(
          "UE:light: there is no skeleton or Name Bone to stick the light.");
  }
...
void CUserEntity::CSpeedFactor::init() {
  ...
  CCDBNodeLeaf *pNodeLeaf = NLGUI::CDBManager::getInstance()->getDbProp(
      "SERVER:USER:SPEED_FACTOR", false);
  if (pNodeLeaf) {
    ...
    if (pNodeLeaf->getValue64() != 0)
      _Value = ((float)pNodeLeaf->getValue64()) /
               100.0f; // may have been received before
  } else
...
void CUserEntity::CMountSpeeds::init() {
  CInterfaceManager *IM = CInterfaceManager::getInstance();
  CCDBNodeLeaf *pNodeLeaf = NLGUI::CDBManager::getInstance()->getDbProp(
      "SERVER:USER:MOUNT_WALK_SPEED", false);
  BOMB_IF(!pNodeLeaf, "MOUNT_WALK_SPEED not found", return);
  if (pNodeLeaf) {
    ICDBNode::CTextId textId;
    pNodeLeaf->addObserver(this, textId);
    _WalkSpeed = ((float)pNodeLeaf->getValue32()) /
                 1000.0f; // may have been received before
  }
  pNodeLeaf = NLGUI::CDBManager::getInstance()->getDbProp(
      "SERVER:USER:MOUNT_RUN_SPEED", false);
  BOMB_IF(!pNodeLeaf, "MOUNT_RUN_SPEED not found", return);
  if (pNodeLeaf) {
    ICDBNode::CTextId textId;
    pNodeLeaf->addObserver(this, textId);
    _RunSpeed = ((float)pNodeLeaf->getValue32()) /
                1000.0f; // may have been received before
  }
}
...
void CUserEntity::CSpeedFactor::update(ICDBNode *node) // virtual
{
  ...
  // nlinfo("SpeedFactor changed to %f / %" NL_I64 "u", _Value,
  // leaf->getValue64());

  // clamp the value (2.0 is the egg item or the level 6 speed up power up,
  // nothing should be faster) commented because ring editor speed is in fact
  // faster
  // if(_Value > 2.0f)
  //{
...
bool CUserEntity::CMountHunger::canRun() const {
  ...
  for (uint i = 0; i != nbAnimals; ++i) {
    ...
    if (((CLFECOMMON::TClientDataSetIndex)uidLeaf->getValue32()) ==
        mountEntity->dataSetId()) {
      CCDBNodeLeaf *hungerLeaf = safe_cast<CCDBNodeLeaf *>(
          beastNode->getNode(ICDBNode::CTextId("HUNGER")));
...
{
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  _WalkSpeed = ((float)(NLGUI::CDBManager::getInstance()
                            ->getDbProp("SERVER:USER:MOUNT_WALK_SPEED")
                            ->getValue32())) /
               1000.0f;
  _RunSpeed = ((float)(NLGUI::CDBManager::getInstance()
                           ->getDbProp("SERVER:USER:MOUNT_RUN_SPEED")
                           ->getValue32())) /
              1000.0f;
}
...
void CUserEntity::CSkillPointsObserver::update(ICDBNode *node) {
  if (FarTP.isFarTPInProgress() ||
      IngameDbMngr
          .initInProgress()) // prevent from displaying at the beginning of a
                             // FarTP (due to RESET_BANK or CDB resetData())
    return;
  ...
  if (leaf) {
    ...
    if (oldValue != 0) {
      ...
      // get the sp title
      const string &spTitle =
          CI18N::get(toString("uiSkillPointsBold%d", SpType));

      // run the popup
      CAHManager::getInstance()->runActionHandler(
          "message_popup", NULL, "text1=" + deltaStr + "|text0=" + spTitle);
...
bool findExtractionActionInMemory(CSPhraseManager *pm, CSBrickManager *bm,
                                  uint memoryLine, uint &index) {
  uint x;
  for (x = 0; x != PHRASE_MAX_MEMORY_SLOT; ++x) {
    ...
    if (!phraseCom.empty()) {
      ...
      if (brickSheet->isForageExtraction() &&
          (!brickSheet->MandatoryFamilies
...
void CUserEntity::extractRM() {
  ...
  if (!autoFindPhrase) {
    ...
    if (!findExtractionActionInMemory(pm, bm, memoryLine, index)) {
      // Search in other memory bar lines (because the auto-equip does not set
      // the current line at once)
  ...
  if (memoryLine != std::numeric_limits<uint>::max()) {
    // Open the forage (but not for care actions). Necessary for the case of
    // redoing an extraction after a Drop All on the same source.
    uint32 phraseId = pm->getMemorizedPhrase(memoryLine, index);
    if (phraseId != 0) {
      const CSPhraseCom &phraseCom = pm->getPhrase(phraseId);
      if (!phraseCom.empty()) {
        CSBrickSheet *rootBrick =
            CSBrickManager::getInstance()->getBrick(phraseCom.Bricks[0]);
        if (rootBrick) {
    ...
    // Cast the extraction. if autoFindPhrase, clientExecute() not already
    // called.
    if (autoFindPhrase) {
    ...
    CInterfaceManager::getInstance()->displaySystemInfo(
        CI18N::get("uiExtractionPhraseMissing"), "CHK");
    return;
  }
}
...
bool CUserEntity::canCastShadowMap() const {
  ...
  // don't cast shadow in first person, but in death mode (third person
  // actually...)
  return viewMode() != FirstPV ||
         UserControls.mode() == CUserControls::DeathMode;
}

// ***************************************************************************
void CUserEntity::forceLookEntity(const NLMISC::CVectorD &dir2targIn,
                                  bool updateHeadPitch, bool /* start */) {
  ...
  if (viewMode() == FirstPV) {
    if (updateHeadPitch && _FollowForceHeadPitch) {
      ...
      if (target) {
        ...
        // don't update to the real head position each frame (else jitter too
        // much cause of target anim)
        CVector targetPos = target->pos() + CVector(0, 0, _FollowHeadOffset);

        // then look at this target
        CVector dirToTarget =
            targetPos - (pos() + CVector(0, 0, UserEntity->eyesHeight()));
        if ((dirToTarget.x != 0.0f) || (dirToTarget.y != 0.0f)) {
          dirToTarget.normalize();
          setHeadPitch(atan2(dirToTarget.z,
                             sqrt(sqr(dirToTarget.x) + sqr(dirToTarget.y))));
        }

        // TestYoyo
        /*if(ClientCfg.Fly!=0.f)
        {
                nlinfo("Uy: %.3f. Hp: %.3f. UPos:(%.3f,%.3f,%.3f).
        TPos:(%.3f,%.3f,%.3f)", UserEntity->frontYaw(),
        UserEntity->getHeadPitch(), pos().x, pos().y, pos().z, targetPos.x,
        targetPos.y, targetPos.z); static	float	uy=0.f; static	float
        hp=0.f; if( fabs(fmod(UserEntity->frontYaw()-uy, 2*Pi))>ClientCfg.Fly ||
                        fabs(fmod(UserEntity->getHeadPitch()-hp,
        2*Pi))>ClientCfg.Fly )
                {
                        nlinfo("YOYOBREAK: ^^^^^^^^^^");
                }
...
void CUserEntity::startForceLookEntity(CLFECOMMON::TCLEntityId slot) {
  ...
  // reorient now (important else may have a time shift because of
  // resetCameraDeltaYaw above)
  CEntityCL *target = EntitiesMngr.entity(slot);
  if (target) {
    /* For complex reason, the target may not be still snapped on the ground.
       snap it now
            - this is because in common case, entities are snap only if they are
       visible (for speed up)
                    => depends on camera
            - but in this case, the camera depends on real entity position
       (headPitch in first person)
    */
    target->snapToGround();
...
void CUserEntity::switchVelocity(bool userRequest) {
  ...
  CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
      "UI:VARIABLES:PLAYER_RUNNING", false);
  if (node)
      ...
      if (_PreviousLeftHandItem.Sheet != 0) {
        for (il = 0; il < MAX_BAGINV_ENTRIES; ++il) {
          if (il != ir &&
              ...
              _PreviousLeftHandItem.Quality ==
                  inv->getBagItem(il).getQuality() &&
...
// ***************************************************************************
void CUserEntity::executeCombatWithPhrase(CEntityCL *target, uint32 memoryLine,
                                          uint32 memoryIndex, bool cyclic) {
  nlassert(target);
  CSPhraseManager *pPM = CSPhraseManager::getInstance();
  ...
  // If melee combat, and if the user entity is not well placed for fight, or if
  // it has changed his target
  if (meleeCombat &&
      (!target->isPlacedToFight(pos(), front(),
                                attackRadius() + ClientCfg.AttackDist) ||
    ...
    // Cancel any moveTo(), because don't want to continue reaching the prec
    // entity VERY important if previous MoveTo was a SPhrase MoveTo (because
    // cancelClientExecute() must be called)
    resetAnyMoveTo();
...
// ***************************************************************************
void CUserEntity::updatePreCollision(const NLMISC::TTime &time,
                                     CEntityCL *target) {
  ...
  if (SoundMngr) {
    ...
    // must end music?
    if (!isDead && SoundMngr->getEventMusicPlayed() == ClientCfg.DeathMusic) {
      SoundMngr->stopEventMusic(ClientCfg.DeathMusic,
...
bool CUserEntity::isInNpcControl() const {
  CInterfaceManager *pIM = CInterfaceManager::getInstance();
  CCDBNodeLeaf *sheet = NLGUI::CDBManager::getInstance()->getDbProp(
      "SERVER:USER:NPC_CONTROL:SHEET", false);
  return sheet &&
         NLMISC::CSheetId(sheet->getValue32()) != NLMISC::CSheetId::Unknown;
}
...
void CUserEntity::updateNpcContolSpeed() {
  ...
  CCDBNodeLeaf *sheet = NLGUI::CDBManager::getInstance()->getDbProp(
      "SERVER:USER:NPC_CONTROL:SHEET", false);
  CCDBNodeLeaf *walk = NLGUI::CDBManager::getInstance()->getDbProp(
      "SERVER:USER:NPC_CONTROL:WALK", false);
  CCDBNodeLeaf *run = NLGUI::CDBManager::getInstance()->getDbProp(
      "SERVER:USER:NPC_CONTROL:RUN", false);
  if (!sheet || !walk || !run) {
    return;
  }
...
void CUserEntity::cancelAllPhrases() {
  ...
  if (GenericMsgHeaderMngr.pushNameToStream("PHRASE:CANCEL_ALL", out)) {
    ...
    nlwarning("<CUserEntity::cancelAllPhrases> unknown message name '%s'",
              "PHRASE:CANCEL_ALL");
  }
}
...
bool CUserEntity::canChangeFront() {
  return !(
      _CurrentBehaviour.Behaviour == MBEHAV::EXTRACTING ||
      (_CurrentBehaviour.Behaviour == MBEHAV::RANGE_ATTACK &&
       _Mode == MBEHAV::COMBAT && !UserControls.isMoving()) ||
      (_CurrentBehaviour.Behaviour >= MBEHAV::MAGIC_CASTING_BEHAVIOUR_BEGIN &&
       _CurrentBehaviour.Behaviour <= MBEHAV::MAGIC_CASTING_BEHAVIOUR_END));
}
...
void CUserEntity::rememberWeaponsInHand() {
  ...
  if (rightItemImg) {
    if (inv->isMeleeWeaponItem(rightItemImg->getSheetID()) ||
        inv->isRangeWeaponItem(rightItemImg->getSheetID())) {
