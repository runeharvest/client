...
struct CClientConfig {
  ...
  bool SkipIntro;
  /// Select character automatically (dont go to create perso) (-1 if no auto
  /// select)
  ...
  /// vector of XML file names that describe the interfaces config for outgame
  /// menus
  ...
  /** Landscape Threshold. NB: threshold is inverted ULandscape::setThreshold(),
   * to be more intelligible in client.cfg MUST USE
   * getActualLandscapeThreshold() to get the real value
   */
  ...
  uint ScreenShotWidth; // If 0 : normal screen shot, else custom screen shot
                        // without interface
  uint ScreenShotHeight;
  bool ScreenShotFullDetail; // If set to true, then load balancing will be
                             // disabled for the duration of the screenshot
  ...
  bool UseEax;
  /// A flag the indicate we whant ADPCM sample (reduce memory by 1/4, but lower
  /// quality)
  bool UseADPCM;
  ...
  /// Time before removing entities collisions from entities with the user in
  /// MS.
  ...
  /// Factor of the walk animation distance after which the entity will start
  /// moving.
  double MinDistFactor;

  /// Scale used to display names.
  float NameScale;
  /// Distance between the name and the extended name (this value is multiply
  /// with the name scale).
  float NamePos;
  ...
  float CameraDistMax;   // distance max of the camera for ryzom player and ring
                         // tester player
  ...
  // Over		: must be displayed at bottom of the screen and in
  // system info window OverOnly		: must be displayed at bottom of
  // the screen Center	; must be displayed at the center of the screen and in
  // system info window Around	; must be displayed in the around chat window
  // CenterAround	; must be displayed at the center of the screen and in
  // around chat window
  ...
  // DEBUG MEMORY
  int CheckMemoryEveryNFrame; // -1 no check (default) else number frame to skip
                              // before a memory checking
  bool LogMemoryAllocation;   // false no log (default) else log each memory
                              // allocation in the file "alloc.memlog"
  ...
  // Use new version of the blend shapes (if true) new version with min,max in
  // race_stats
...
public:
  ...
  // NB: threshold is inverted ULandscape::setThreshold(), to be more
  // intelligible in client.cfg
  float getActualLandscapeThreshold() const;

  // Return LanguageCode but if "wk", then return "en"
  std::string getHtmlLanguageCode() const;

  // return a random loading tip or, if there are not, return the string in
  // argument
