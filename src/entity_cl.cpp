...
NLMISC::CRGBA CEntityCL::_GMTitleColor[CHARACTER_TITLE::EndGmTitle -
                                       CHARACTER_TITLE::BeginGmTitle + 1];
...
void CEntityCL::SInstanceCL::showStaticFXs() {
  for (std::vector<UInstance>::iterator it = StaticFXs.begin();
       it != StaticFXs.end(); ++it) {
    if (!it->empty()) {
      (*it).show();
    }
...
//---------------------------------------------------
//---------------------------------------------------
bool CEntityCL::SInstanceCL::createLoading(const string &strShapeName,
                                           const string &strStickPoint,
                                           sint texture, bool clearIfFail) {
...
//---------------------------------------------------
//---------------------------------------------------
void CEntityCL::SInstanceCL::setColors(sint skin, sint user, sint hair,
                                       sint eyes) {
  ApplyColor = true;
...
NL3D::UInstance CEntityCL::SInstanceCL::createLoadingFromCurrent() {
  ...
  if (ApplyColor) {
    ...
    array[0].second = (uint)ACSkin;
    // User Color
    array[1].first = (uint)1;
    array[1].second = (uint)ACUser;
    // Hair Color
    array[2].first = (uint)2;
    array[2].second = (uint)ACHair;
    // Eyes Color
    array[3].first = (uint)3;
    array[3].second = (uint)ACEyes;
    if (!Loading.empty())
      ColorSlotManager.setInstanceSlot(Loading, array, 4);
  }

  // TODO : there is probably a bug for FX here : if we want to create a loading
  // instance from the current instance and there are some FXs binded to the
  // current instance we have to : 1 - recreate them onto the loading instance
  // or
  // 2 - unbind them from the current instance and rebind them to the loading
  // instance
...
//---------------------------------------------------
void CEntityCL::SInstanceCL::updateCurrentFromLoading(
  ...
  if (!Skeleton.empty()) {
    ...
    if (StickPoint.empty()) {
      if (!Skeleton.bindSkin(Current))
        nlwarning("Cannot bind the skin %s to the skeleton in the slot.",
                  CurrentName.c_str());
    }
    // Try to Stick Object.
    else {
      ...
      if (stickID != -1)
        ...
        nlwarning("Skeleton '%s' is missing bone '%s' for object attachment.",
                  Skeleton.getShapeName().c_str(), StickPoint.c_str());
    }
  }
      ...
      if (!boneName.empty() && !name.empty()) {
        ...
        if (boneID != -1) {
          ...
          if (!instance.empty()) {
            ...
            nlwarning("Can't create static fx %s sticked on bone %s",
                      name.c_str(), boneName.c_str());
          }
        } else {
          nlwarning("Can't find bone %s for static fx %s", boneName.c_str(),
                    name.c_str());
        }
      }
    }
...
//---------------------------------------------------
void CEntityCL::SInstanceCL::makeInstanceTransparent(uint8 opacity,
                                                     uint8 opacityMin) {
  // ensure correct opacity
  opacity = max(opacity, opacityMin);

  // choose to disable ZWrite only when opacity == opacityMin (this that what do
...
void CEntityCL::init() {
  ...
  _TheoreticalOrientation =
      -10.0f; // Init value to know if it has been changed.
...
void CEntityCL::initialize() {
  ...
  initPrimitive(0.5f, 2.0f, 0.0f, 0.0f, UMovePrimitive::DoNothing,
                UMovePrimitive::NotATrigger, MaskColNone, MaskColNone);
...
//
//-----------------------------------------------
bool CEntityCL::initPrimitive(float radius, float height, float length,
                              float width,
                              ...
                              float clipRadius, float clipHeight) {
  // Check primitive, there should not be a primitive because it could have been
  // created with another PACS and crash(continent changed).
  if (_Primitive) {
    nlwarning(
        "ENT:initPrimitive:%d: There is already a primitive -> _Primitive = 0.",
        _Slot);
    _Primitive = 0;
  }
    ...
    if (_Primitive) {
      ...
      if (radius > 0.0f) {
        ...
        _Primitive->setRadius(
            std::min(radius, (float)(RYZOM_ENTITY_SIZE_MAX / 2)));
      ...
      // Maybe be a Box
        ...
        _Primitive->setSize(
            std::min(length, (float)(RYZOM_ENTITY_SIZE_MAX / 2)),
            std::min(width, (float)(RYZOM_ENTITY_SIZE_MAX / 2)));
        _Primitive->setHeight(height + ClientCfg.PrimitiveHeightAddition);
        if (length > width) {
      ...
      nlwarning("CEntityCL::initPrimitive:%d: Cannot create the _Primitive.",
                _Slot);
  } else
    nlwarning("CEntityCL::initPrimitive:%d: PACS not allocated -> _Primitive "
              "not created.",
              _Slot);
  ...
  CAABBox clipBox;
  clipBox.setMinMax(CVector(-clipRadius, -clipRadius, 0.0f),
                    CVector(clipRadius, clipRadius, clipHeight));
  // Compute the local Sphere info
  _ClipRadius = clipBox.getRadius();
  _ClipDeltaZ =
      clipBox.getCenter().z; // NB different from radius, since radius>center.z
...
//-----------------------------------------------
void CEntityCL::updateVisualProperty(
    const NLMISC::TGameCycle &gameCycle, const uint &prop,
    const NLMISC::TGameCycle &predictedInterval) {
  CCDBNodeBranch *nodePtr = IngameDbMngr.getNodePtr();
  if (nodePtr) {
    CCDBNodeBranch *nodeRoot =
        dynamic_cast<CCDBNodeBranch *>(nodePtr->getNode(0));
    if (!nodeRoot) {
      nlwarning("CEntityCL::updateVisualProperty : There is no entry in the DB "
                "for entities (current slot %d).",
                _Slot);
      return;
    }

    CCDBNodeBranch *nodGrp =
        dynamic_cast<CCDBNodeBranch *>(nodeRoot->getNode(_Slot));
    if (nodGrp == 0) {
      nlwarning("CEntityCL::updateVisualProperty : Cannot find the entity '%d' "
                "in the database.",
                _Slot);
      return;
    }
    ...
    CCDBNodeLeaf *nodeProp =
        dynamic_cast<CCDBNodeLeaf *>(nodGrp->getNode(prop));
    if (nodeProp == 0) {
      nlwarning("CEntityCL::updateVisualProperty : Cannot find the property "
                "'%d' for the slot %d.",
                prop, _Slot);
      return;
    }
    ...
    case PROPERTY_POSITION:
      updateVisualPropertyPos(gameCycle, nodeProp->getValue64(),
                              predictedInterval);
      break;
    ...
    case PROPERTY_TARGET_LIST_3:
      updateVisualPropertyTargetList(gameCycle, nodeProp->getValue64(),
                                     prop - PROPERTY_TARGET_LIST_0);
      break;
    ...
    case PROPERTY_OUTPOST_INFOS:
      ...
      //		updateVisualPropertyStatus(gameCycle,
      //nodeProp->getValue64()); 		break;

    default:
      nlwarning("CEntityCL::updateVisualProperty : Unknown Property '%d' for "
                "the entity in the slot '%d'.",
                prop, _Slot);
      break;
    }
  }
...
//-----------------------------------------------
void CEntityCL::updateVisualPropertyContextual(
...
// \param filename : file with the skeleton to apply to the entity or empty
// string to remove a skeleton.
//-----------------------------------------------
USkeleton *CEntityCL::skeleton(const string &filename) {
  ...
  if (!_Skeleton.empty()) {
    ...
    pushDebugStr(toString(
        "Cannot create the Skeleton '%s', the file probably not exist.",
        filename.c_str()));
...
// \param instIdx : if not CEntityCL::BadIndex, the instance will replace the
// one at this index. \return uint32 : index of the instance created, or
// CEntityCL::BadIndex.
//-----------------------------------------------
uint32 CEntityCL::addInstance(const string &shapeName,
                              const std::string &stickPoint, sint texture,
                              uint32 instIdx) {
  ...
  if (instIdx < _Instances.size()) // And so instIdx != CEntityCL::BadIndex
  {
    ...
    if (instIdx != CEntityCL::BadIndex)
      nlwarning("CEntityCL::addInst:%d: given index '%d' is not valid.", _Slot,
                instIdx);
  ...
  if (shapeName.empty()) {
    if (instIdx == CEntityCL::BadIndex)
      nlwarning("CEntityCL::addInst:%d: shape empty but given index was "
                "invalid, so nothing to clean and cannot add an empty shape.",
                _Slot);
  ...
  // else create
    ...
    // load, but dont replace instance if fails (yoyo: I keep the same behavior
    // than before )
    if (!_Instances[idx].createLoading(shapeName, stickPoint, texture, false)) {
      // If fails to load....
      nlwarning("CEntityCL::addInstance : Cannot create the instance '%s' for "
                "the slot %d.",
                shapeName.c_str(), _Slot);
...
void CEntityCL::show(bool s) {
  if (!_Skeleton.empty()) {
    ...
    if (s) {
      for (std::vector<SInstanceCL>::iterator it = _Instances.begin();
...
// todo GUIGUI : for video, it shouldn't be a problem, but we should improve
// with the current which could not be ready yet while loading not done
//-----------------------------------------------
...
void CEntityCL::up(const CVector &vect) {
  ...
  if (vect == CVector::Null) {
    nlwarning("CEntityCL::up : attempt to set the vector up with a vector "
              "Null. Up not changed !");
    return;
  }
...
// \param compute : adjust the param 'vect' to be valid or leave the old front
// unchanged if impossible. \param check : warning if the param 'vect' is not
// valid to be the front (vector Null) even with compute=true. \param forceTurn
// : set front even if the entity cannot turn \return bool : 'true' if the front
// has been filled, else 'false'.
//---------------------------------------------------
bool CEntityCL::front(const CVector &vect, bool compute, bool check,
                      bool forceTurn) {
...
// \param compute : adjust the param 'vect' to be valid or leave the old
// direction unchanged if impossible. \param check : warning if the param 'vect'
// is not valid to be the direction (vector Null) even with compute=true.
...
void CEntityCL::pacsPos(const CVectorD &vect, const UGlobalPosition &globPos) {
  ...
  if (globPos.InstanceId !=
      -1 /*&& ContinentMngr.getCurrentContinentSelectName() == "newbieland"*/) {
    _Primitive->setGlobalPosition(globPos, dynamicWI);
  }
  // NB: else use the 3D position. => the Z may be different than the Z wanted!
  // (according to the pacs pos found)
  else {
  ...
  // If the entity do not fly, update Z value (X,Y should be the same or close
  // and to avoid a pb with a too small move, just keep them)
  if (!flyer())
...
void CEntityCL::pacsMove(const CVectorD &vect) {
  ...
  if (_SetGlobalPositionDone) {
    // Change the position of the entity in PACS and apply the new position to
    // the entity.
    CVectorD deltaPos((vect.x - _FinalPacsPos.x) / DT,
                      (vect.y - _FinalPacsPos.y) / DT, 0);
...
void CEntityCL::pacsFinalizeMove() // virtual
{
  ...
  _Primitive->getGlobalPosition(gPos, dynamicWI);
  if (gPos.InstanceId != -1) {
    if (ClientCfg.UsePACSForAll &&
        (_Mode == MBEHAV::COMBAT || _Mode == MBEHAV::COMBAT_FLOAT)) {
      pos(_FinalPacsPos);
    } else {
      ...
      // If the entity do not fly, update Z value (X,Y should be the same or
      // close and to avoid a pb with a too small move, just keep them)
      if (!flyer())
        pos().z = _FinalPacsPos.z;
    }
  } else {
...
//---------------------------------------------------
void CEntityCL::updateDisplay(CEntityCL * /* parent */) {} // updateDisplay //
...
NL3D::UInstanceGroup *CEntityCL::getClusterSystem() {
  if (!_Skeleton.empty())
    return _Skeleton.getClusterSystem();
  if (!_Instance.empty())
    return _Instance.getClusterSystem();
  return NULL;
}
...
void CEntityCL::snapToGround() {
  ...
  if (!flyer()) {
    ...
    if (needSnap && _CollisionEntity) {
      // for Snap on landscape, use always current 3d position, for snap cache
      // performance.
      vect = pos();
...
//---------------------------------------------------
void CEntityLogicInfo3D::getStaticLightSetup(
  ...
  if (igUnderInstance) {
    ...
    igUnderInstance->getStaticLightSetup(
        sunAmbient, localRetrieverId, gPos.LocalPosition.Surface,
        gPos.LocalPosition.Estimation, pointLightList, sunContribution,
        localAmbient);
  }
  // else retrieve from landscape
    ...
    if (colEnt) {
      colEnt->getStaticLightSetup(sunAmbient, Self->pos(), pointLightList,
...
void CEntityCL::computeCollisionEntity() {
  ...
  if (_CollisionEntity) {
    nlwarning("CEntityCL::computeCollisionEntity: There is already a collision "
              "entity in the slot %d.",
              _Slot);
    _CollisionEntity = 0;
  }
  ...
  if (CollisionManager) {
    ...
    if (_CollisionEntity == 0)
      nlwarning("CEntityCL::computeCollisionEntity : Cannot create the "
                "_CollisionEntity for the slot %d.",
                _Slot);
    else
...
//-----------------------------------------------
double CEntityCL::attackRadius() const { return 0.5; } // attackRadius //
...
float CEntityCL::updateAsyncTexture() {
  ...
  if (_AsyncTextureLoading) {
    ...
    // if all are loaded, then End! don't need to check all instances every
    // frame.
    if (allLoaded) {
      _AsyncTextureLoading = false;
      ...
      for (i = 0; i < _Instances.size(); i++) {
        ...
        if (_Skeleton.empty() && !_Instances[i].Current.empty()) {
          _Instances[i].Current.setPos(
              pos()); // force update right now (else the object would be at 0,
                      // 0, 0 for first frame)
        }
      }

      // additionally, if their is no skeleton (bot objects), may enable cast
      // shadow here
      updateCastShadowMap();
    }
  }
  ...
  // For lesser update, take the refine position (else in 3th person view,
  // rotation will cause lot of update) \todo GUIGUI : make sure pos() will
  // return the world position all the time even when a child.
...
void CEntityCL::addChild(CEntityCL *c) {
  if (c == 0) {
    nlwarning("ENT:addChild:%d: Try to add a child with a Null Pointer.",
              _Slot);
    return;
  }
  ...
  if (skeleton() && c->skeleton()) {
    ...
    if (idBones != -1) {
      ...
      nlwarning("ENT:addChild:%d: the Bone '%s' does not exist", _Slot,
                stickBone.c_str());
  }
} // addChild //
...
void CEntityCL::delChild(CEntityCL *c) {
  if (c == 0) {
    nlwarning("ENT:delChild:%d: Try to remove a child with a Null Pointer.",
              _Slot);
    return;
  }
...
bool CEntityCL::clipped(const std::vector<NLMISC::CPlane> &clippingPlanes,
                        const CVector &camPos) {
  // If the entity still have no position, and still have no mode, entity should
  // not be displayed.
...
//---------------------------------------------------
void CEntityCL::setEntityName(const std::string &name) { _EntityName = name; }
...
void CEntityCL::displayDebug(float x, float &y, float lineStep) // virtual
{
  ...
  y += lineStep;
  // Outpost
  TextContext->printfAt(x, y, "Outpost id:%d side:%s", this->getOutpostId(),
                        OUTPOSTENUMS::toString(this->getOutpostSide()).c_str());
  y += lineStep;
  // Name
  ...
  TextContext->printfAt(x, y, "Target: %d - DataSet Id: %u", _TargetSlot,
                        _DataSetId);
  y += lineStep;
  // Sheet Id
  TextContext->printfAt(x, y, "Sheet: %d(%s)", _SheetId.asInt(),
...
#else
  TextContext->printfAt(x, y, "Pos: %f %f %f (Theoretical : %f %f %f)", pos().x,
                        pos().y, pos().z, _TheoreticalPosition.x,
                        _TheoreticalPosition.y, _TheoreticalPosition.z);
#endif
  y += lineStep;
  // Display the Target Mode.
  TextContext->printfAt(x, y, "Mode: %d(%s) (Theoretical : %d(%s))",
                        (sint)mode(), MBEHAV::modeToString(mode()).c_str(),
                        (sint)_TheoreticalMode,
                        MBEHAV::modeToString(_TheoreticalMode).c_str());
  y += lineStep;
  // Display the Target Behaviour.
  TextContext->printfAt(x, y, "Behaviour: %d(%s)", (sint)behaviour(),
                        MBEHAV::behaviourToString(behaviour()).c_str());
  y += lineStep;
  // Front
#ifndef TMP_DEBUG_GUIGUI
  TextContext->printfAt(x, y, "%f(%f,%f,%f) front", frontYaw(), front().x,
                        front().y, front().z);
#else
  TextContext->printfAt(x, y, "%f(%f,%f,%f) front (Theoretical : %f)",
                        frontYaw(), front().x, front().y, front().z,
                        _TheoreticalOrientation);
#endif
  y += lineStep;
  // Direction
  TextContext->printfAt(x, y, "%f(%f,%f,%f) dir", atan2(dir().y, dir().x),
                        dir().x, dir().y, dir().z);
  y += lineStep;
  // Last Retrieved pacs
  TextContext->printfAt(
      x, y, "Last PACS: (%.4f,%.4f,%.4f) (%d,%d,%.4f,%.4f,%.4f)",
      _LastRetrievedPosition.x, _LastRetrievedPosition.y,
      _LastRetrievedPosition.z, _LastRetrievedPacsPosition.InstanceId,
      _LastRetrievedPacsPosition.LocalPosition.Surface,
      _LastRetrievedPacsPosition.LocalPosition.Estimation.x,
      _LastRetrievedPacsPosition.LocalPosition.Estimation.y,
      _LastRetrievedPacsPosition.LocalPosition.Estimation.z);
  y += lineStep;
  // Current pacs
  UGlobalPosition gp;
  if (_Primitive)
  ...
  TextContext->printfAt(
      x, y, "Current PACS: (%d,%d,%.4f,%.4f,%.4f) %s %.1f", gp.InstanceId,
      gp.LocalPosition.Surface, gp.LocalPosition.Estimation.x,
      gp.LocalPosition.Estimation.y, gp.LocalPosition.Estimation.z,
...
//-----------------------------------------------
//-----------------------------------------------
void CEntityCL::displayDebugPropertyStages(float /* x */, float &y,
                                           float /* lineStep */) {}
...
//-----------------------------------------------
void CEntityCL::onStringAvailable(uint /* stringId */,
  ...
  if (p1 != string::npos) {
    ...
    if (p2 != string::npos) {
      ...
      _Tags = STRING_MANAGER::CStringManagerClient::getTitleInfos(_TitleRaw,
                                                                  womanTitle);
      ...
      CHARACTER_TITLE::ECharacterTitle titleEnum =
          CHARACTER_TITLE::toCharacterTitle(_TitleRaw);
      if (titleEnum >= CHARACTER_TITLE::BeginGmTitle &&
          titleEnum <= CHARACTER_TITLE::EndGmTitle) {
  ...
  if (_Slot == 0) {
    ...
    CViewText *pVT = dynamic_cast<CViewText *>(
        CWidgetManager::getInstance()->getElementFromId(
            "ui:interface:player:header_opened:player_title"));
    if (pVT != NULL)
      pVT->setText(_Title);
...
std::string CEntityCL::removeShardFromName(const std::string &name) {
  ...
  if (!NLMISC::compareCaseInsensitive(name.c_str() + p0 + 1, p1 - p0 - 1,
                                      PlayerSelectedHomeShardName.c_str(),
                                      PlayerSelectedHomeShardName.size()))
...
public:
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    ...
    CEntityCL::_EntitiesColor[CEntityCL::User] =
        pCDBM->getDbProp("UI:SAVE:ENTITY:COLORS:USER")->getValueRGBA();
    CEntityCL::_EntitiesColor[CEntityCL::Player] =
        pCDBM->getDbProp("UI:SAVE:ENTITY:COLORS:PLAYER")->getValueRGBA();
    CEntityCL::_EntitiesColor[CEntityCL::NPC] =
        pCDBM->getDbProp("UI:SAVE:ENTITY:COLORS:NPC")->getValueRGBA();
    CEntityCL::_EntitiesColor[CEntityCL::Fauna] =
        pCDBM->getDbProp("UI:SAVE:ENTITY:COLORS:FAUNA")->getValueRGBA();
    CEntityCL::_EntitiesColor[CEntityCL::ForageSource] =
        pCDBM->getDbProp("UI:SAVE:ENTITY:COLORS:SOURCE")->getValueRGBA();
    CEntityCL::_DeadColor =
        pCDBM->getDbProp("UI:SAVE:ENTITY:COLORS:DEAD")->getValueRGBA();
    CEntityCL::_TargetColor =
        pCDBM->getDbProp("UI:SAVE:ENTITY:COLORS:TARGET")->getValueRGBA();
    CEntityCL::_GroupColor =
        pCDBM->getDbProp("UI:SAVE:ENTITY:COLORS:GROUP")->getValueRGBA();
    CEntityCL::_GuildColor =
        pCDBM->getDbProp("UI:SAVE:ENTITY:COLORS:GUILD")->getValueRGBA();
    CEntityCL::_UserMountColor =
        pCDBM->getDbProp("UI:SAVE:ENTITY:COLORS:MOUNT")->getValueRGBA();
    CEntityCL::_UserPackAnimalColor =
        pCDBM->getDbProp("UI:SAVE:ENTITY:COLORS:BEAST")->getValueRGBA();
    CEntityCL::_PvpEnemyColor =
        pCDBM->getDbProp("UI:SAVE:ENTITY:COLORS:PVPENEMY")->getValueRGBA();
    CEntityCL::_PvpAllyColor =
        pCDBM->getDbProp("UI:SAVE:ENTITY:COLORS:PVPALLY")->getValueRGBA();
    CEntityCL::_PvpAllyInTeamColor =
        pCDBM->getDbProp("UI:SAVE:ENTITY:COLORS:PVPALLYINTEAM")->getValueRGBA();
    CEntityCL::_PvpNeutralColor =
        pCDBM->getDbProp("UI:SAVE:ENTITY:COLORS:PVPNEUTRAL")->getValueRGBA();

    // don't save these colors in .icfg because players can't change them
    CEntityCL::_GMTitleColor[CHARACTER_TITLE::SGM -
                             CHARACTER_TITLE::BeginGmTitle] =
        pCDBM->getDbProp("UI:INTERFACE:ENTITY:COLORS:SGM")->getValueRGBA();
    CEntityCL::_GMTitleColor[CHARACTER_TITLE::GM -
                             CHARACTER_TITLE::BeginGmTitle] =
        pCDBM->getDbProp("UI:INTERFACE:ENTITY:COLORS:GM")->getValueRGBA();
    CEntityCL::_GMTitleColor[CHARACTER_TITLE::VG -
                             CHARACTER_TITLE::BeginGmTitle] =
        pCDBM->getDbProp("UI:INTERFACE:ENTITY:COLORS:VG")->getValueRGBA();
    CEntityCL::_GMTitleColor[CHARACTER_TITLE::SG -
                             CHARACTER_TITLE::BeginGmTitle] =
        pCDBM->getDbProp("UI:INTERFACE:ENTITY:COLORS:SG")->getValueRGBA();
    CEntityCL::_GMTitleColor[CHARACTER_TITLE::G -
                             CHARACTER_TITLE::BeginGmTitle] =
        pCDBM->getDbProp("UI:INTERFACE:ENTITY:COLORS:G")->getValueRGBA();

    CEntityCL::_GMTitleColor[CHARACTER_TITLE::CM -
                             CHARACTER_TITLE::BeginGmTitle] =
        pCDBM->getDbProp("UI:INTERFACE:ENTITY:COLORS:CM")->getValueRGBA();
    CEntityCL::_GMTitleColor[CHARACTER_TITLE::EM -
                             CHARACTER_TITLE::BeginGmTitle] =
        pCDBM->getDbProp("UI:INTERFACE:ENTITY:COLORS:EM")->getValueRGBA();
    CEntityCL::_GMTitleColor[CHARACTER_TITLE::EG -
                             CHARACTER_TITLE::BeginGmTitle] =
        pCDBM->getDbProp("UI:INTERFACE:ENTITY:COLORS:EG")->getValueRGBA();
    CEntityCL::_GMTitleColor[CHARACTER_TITLE::OBSERVER -
                             CHARACTER_TITLE::BeginGmTitle] =
...
bool CEntityCL::isInSameGuild() const {
  ...
  if (guildNameId != 0 && UserEntity &&
      guildNameId == UserEntity->getGuildNameID())
    return true;
  ...
  if (Type == Player || Type == NPC) {
    ...
    if (Type == NPC) {
      ...
      if (isEnemy()) {
        if (getPvpMode() & PVP_MODE::PvpFaction)
          return CRGBA(min(255, _PvpEnemyColor.R + 150),
                       min(255, _PvpEnemyColor.G + 150),
                       min(255, _PvpEnemyColor.B + 150), _PvpEnemyColor.A);
        else
          return _PvpEnemyColor;
      }
    }
    ...
    if (isAlly()) {
      if (getPvpMode() & PVP_MODE::PvpFactionFlagged) {
        if (isInSameLeague())
          return CRGBA(max(0, _PvpAllyColor.R - 100),
                       max(0, _PvpAllyColor.G - 100),
                       max(0, _PvpAllyColor.B - 100), _PvpAllyColor.A);
        return CRGBA(max(0, _PvpAllyInTeamColor.R - 100),
                     max(0, _PvpAllyInTeamColor.G - 100),
                     max(0, _PvpAllyInTeamColor.B - 100),
                     _PvpAllyInTeamColor.A);
      } else {
        if (isInSameLeague())
    ...
    // neutral
    if (isInSameLeague())
      return CRGBA(min(255, _GroupColor.R + 50), min(255, _GroupColor.G + 50),
                   min(255, _GroupColor.B + 50), _GroupColor.A);
...
void CEntityCL::pos(const NLMISC::CVectorD &vect) {
  ...
  if (checkDist != CVectorD::Null) {
    if (checkDist.norm() > ClientCfg.PositionLimiterRadius) {
      checkDist.normalize();
      _PositionLimiter =
          vect + checkDist * ClientCfg.PositionLimiterRadius / 2.0;
    }
  }
  _Position = vect;
} // pos //
  ...
  if (_NameId) {
    ...
    for (i = 0; i < MAX_NUM_MISSIONS; i++)
      for (j = 0; j < MAX_NUM_MISSION_TARGETS; j++) {
        // Get the db prop
        CCDBNodeLeaf *prop =
            EntitiesMngr.getMissionTargetTitleDB(i, j); // NLGUI::CDBManager::getInstance()->getDbProp("SERVER:MISSIONS:"+toString(i)+":TARGET"+toString(j)+":TITLE",
                                                        // false);
        if (prop) {
...
void CEntityCL::makeTransparent(bool t) {
  if (t == true)
    _TranspFactor +=
        ((float)(DT)) / ((float)RZ_TIME_TO_BECOME_TRANSPARENT_IN_SECOND);
  else
    _TranspFactor -=
        ((float)(DT)) / ((float)RZ_TIME_TO_BECOME_TRANSPARENT_IN_SECOND);

  if (_TranspFactor < 0.0)
    _TranspFactor = 0.0;
  if (_TranspFactor > 1.0)
    _TranspFactor = 1.0;
...
CCDBNodeLeaf *CEntityCL::getOpacityDBNode() {
  if (!_OpacityMinNodeLeaf) {
    CInterfaceManager *pIM = CInterfaceManager::getInstance();
    _OpacityMinNodeLeaf = NLGUI::CDBManager::getInstance()->getDbProp(
        "UI:SAVE:USER_CHAR_OPA_MIN", false);
  }
...
//-----------------------------------------------
const char *CEntityCL::getBoneNameFromBodyPart(BODY::TBodyPart part,
  ...
  case BODY::HChest:
    return "Bip01 Spine2";
  case BODY::HArms:
    return side == BODY::Left ? "Bip01 L UpperArm" : "Bip01 R UpperArm";
  case BODY::HHands:
    return side == BODY::Left ? "Bip01 L Hand" : "Bip01 R Hand";
  case BODY::HLegs:
    return side == BODY::Left ? "Bip01 L Calf" : "Bip01 R Calf";
  case BODY::HFeet:
    return side == BODY::Left ? "Bip01 L Foot" : "Bip01 R Foot";
  default:
    break;
  }
  return NULL;
...
CEntityCL::SInstanceCL *CEntityCL::idx2Inst(uint idx) {
  if (idx < _Instances.size()) // CEntityCL::BadIndex is the max so idx < ...
                               // mean idx != CEntityCL::BadIndex too
...
void CEntityCL::updateIsInTeam() {
  ...
  for (uint i = 0; i < MaxNumPeopleInTeam; i++) {
    ...
    // If same Entity uid than the one in the Database, ok the entity is in the
    // Player TEAM!!
...
void CEntityCL::updateIsUserAnimal() {
  ...
  for (uint i = 0; i < MAX_INVENTORY_ANIMAL; i++) {
    ...
    if (uidProp && statusProp && typeProp &&
        uidProp->getValue32() == (sint32)dataSetId() &&
        ANIMAL_STATUS::isSpawned(
            (ANIMAL_STATUS::EAnimalStatus)(statusProp->getValue32()))) {
...
ANIMAL_STATUS::EAnimalStatus CEntityCL::getPackAnimalStatus() const {
  ...
  for (uint i = 0; i < MAX_INVENTORY_ANIMAL; i++) {
    ...
    CCDBNodeLeaf *statusProp = NLGUI::CDBManager::getInstance()->getDbProp(
        toString("SERVER:PACK_ANIMAL:BEAST%d:STATUS", i), false);
    // I must have the same Id, and the animal entry must be ok.
...
void CEntityCL::dataSetId(CLFECOMMON::TClientDataSetIndex dataSet) {
  ...
  if (_Slot != CLFECOMMON::INVALID_SLOT &&
      _DataSetId != CLFECOMMON::INVALID_CLIENT_DATASET_INDEX)
    CBarManager::getInstance()->addEntity(_Slot, _DataSetId);
}

// ***************************************************************************
void CEntityCL::updateVisible(const NLMISC::TTime & /* time */,
                              CEntityCL * /* target */) {
  ...
  if (!R2::isEditionCurrent()) {
    // NB : if editor is enabled, then highlighting is managed in another
    // fashion (there may be multiple-semihighlights)
...
// ***************************************************************************
void CEntityCL::updateSomeClipped(const NLMISC::TTime & /* time */,
                                  CEntityCL * /* target */) {
  // update some shadowmap properties
  updateShadowMapProperties();
}
...
// ***************************************************************************
void CEntityCL::updateClipped(const NLMISC::TTime & /* currentTimeInMs */,
                              CEntityCL * /* target */) {
...
// ***************************************************************************
void CEntityCL::updateVisiblePostPos(
    const NLMISC::TTime & /* currentTimeInMs */, CEntityCL * /* target */) {
  if (R2::isEditionCurrent())
    return; // selection managed by r2 editor in edition mode

  bool bShowReticle = true;

  CCDBNodeLeaf *node =
      (CCDBNodeLeaf *)_ShowReticleLeaf
          ? &*_ShowReticleLeaf
          : &*(_ShowReticleLeaf = NLGUI::CDBManager::getInstance()->getDbProp(
                   "UI:SAVE:SHOW_RETICLE", false));
  ...
  if (bShowReticle && ShowInterface && !isTarget() && isUnderCursor() &&
  ...
  if (!_SelectionFX.empty() || !_MouseOverFX.empty()) {
    ...
    if (!_SelectionFX.empty()) {
      ...
      // Colorize the selection depending of the level of the creature
      {
        ...
        uint8 nForce = NLGUI::CDBManager::getInstance()
                           ->getDbProp("SERVER:TARGET:FORCE_RATIO")
                           ->getValue8();
...
// ***************************************************************************
void CEntityCL::visualSelectionStart() { _VisualSelectionTime = T1; }

// ***************************************************************************
void CEntityCL::visualSelectionStop() { _VisualSelectionTime = 0; }

// ***************************************************************************
bool CEntityCL::canCastShadowMap() const { return ClientCfg.Shadows; }
...
void CEntityCL::updateCastShadowMap() {
  ...
  if (skeleton()) {
    ...
    // NB: must do it at each updateCastShadowMap(), cause don't know if an
    // instance has been added or not
    for (uint i = 0; i < _Instances.size(); i++) {
      if (!_Instances[i].Current.empty()) {
  ...
  if (shadowOn) {
    if (skeleton()) {
      ...
      for (uint i = 0; i < _Instances.size(); i++) {
        if (!_Instances[i].Current.empty()) {
          _Instances[i].Current.setShadowMapDirectionZThreshold(
              _ShadowMapZDirClamp);
...
void CEntityCL::updateShadowMapProperties() {
  /*
          Choose the z clamp direction whether or not the player is on
     "interior" stuff. In "interior" stuff, the ZClamp direction is lesser, to
     avoid some problems of "cast shadow behind the walls"

          Also choose the MaxDepth of shadow map  whether or not the player is
     on "interior" stuff. In "interior" stuff, the MaxDepth is lesser to, to
     avoid some problems with the "bud": when the player go up stairs and when
     in the "bud", the shadow still appears on landscape
  */
  ...
  if (_ShadowMapZDirClamp != zDirClampWanted ||
    ...
    if (dt != 0) {
      ...
      if (_ShadowMapZDirClamp < zDirClampWanted) {
        _ShadowMapZDirClamp +=
            ClientCfg.ShadowZDirClampSmoothSpeed * 0.001f * dt;
        if (_ShadowMapZDirClamp > zDirClampWanted)
...
void CEntityCL::setOrderingLayer(uint layer) {
  if (!_Skeleton.empty())
    _Skeleton.setOrderingLayer(layer);
}
  ...
  if (_LastLocalSelectBoxComputeTime < T1) {
    ...
    if (!_Skeleton.empty()) {
      ...
      if (!_Skeleton.isDisplayedAsLodCharacter()) {
        ...
        if (useBoneSphere) computed =
        _Skeleton.computeRenderedBBoxWithBoneSphere(_LocalSelectBox, false);
        ...
        if (computed)
        {
                ...
                _LocalSelectBox.setCenter(_LocalSelectBox.getCenter() +
        localOffset); static volatile float rot = -1.f; CMatrix rotMat;
        ...
        */
        if (_Skeleton.computeRenderedBBoxWithBoneSphere(_LocalSelectBox,
                                                        false)) {
          found = true;
          // apply local offset due to skeleton animation
          CMatrix invMat = _DirMatrix;
    ...
    if (!found) {
      ...
      _LocalSelectBox.setHalfSize(CVector(
          halfSize.x * scale.x, halfSize.y * scale.y, halfSize.z * scale.z));
      _LocalSelectBox.setCenter(
          CVector(center.x * scale.x, center.y * scale.y, center.z * scale.z));
    }
  }
...
void CEntityCL::getMeshDefaultScale(NLMISC::CVector &scale) const {
  ...
  if (!_Skeleton.empty())
    return; // scale already applied to skeleton
  if (!_Instance.empty()) {
