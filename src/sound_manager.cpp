...
// If you compile using the nel distrib and you don't have the source, you must
// undef this symbol
#if !FINAL_VERSION
...
CSoundManager::CSoundManager(IProgressCallback * /* progressCallBack */)
    : _AudioMixer(NULL), _GroupControllerEffects(NULL),
      _GroupControllerEffectsGame(NULL), _EnvSoundRoot(NULL), _Sources(NULL),
...
CSoundManager::~CSoundManager() {
  ...
  _AttachedSources.clear(); // attached sources already deleted (they are also
                            // in the _Sources map)
...
void CSoundManager::drawSounds(float camHeight) {
  ...
  CClusteredSound *cs =
      static_cast<CAudioMixerUser *>(_AudioMixer)->getClusteredSound();
  // Draw the audible clusters
  {
    CMaterial mat;
    CMaterial mat2;

    CClusteredSound::TClusterStatusMap::const_iterator first(
        cs->getAudibleClusters().begin()),
        last(cs->getAudibleClusters().end());
    for (; first != last; ++first) {
      ...
      CLine lines[12] = {
          CLine(s[0], s[1]), CLine(s[1], s[2]), CLine(s[2], s[3]),
          CLine(s[3], s[0]), CLine(s[4], s[5]), CLine(s[5], s[6]),
          CLine(s[6], s[7]), CLine(s[7], s[4]), CLine(s[0], s[4]),
          CLine(s[1], s[5]), CLine(s[2], s[6]), CLine(s[3], s[7]),
      };
  ...
  /*	float d = float(Pi/10);
          for (float j=0; j<float(Pi*2); j+=d)
                  NL3D::CDRU::drawLine(pos+CVector(LISTEN_DISTANCE*float(sin(j)),
     LISTEN_DISTANCE*float(cos(j)), 0),
     pos+CVector(LISTEN_DISTANCE*float(sin(j+d)),
     LISTEN_DISTANCE*float(cos(j+d)),0),  CRGBA(255,255,255,255), *idriver);

          for (j=0; j<float(Pi*2); j+=d)
                  NL3D::CDRU::drawLine(pos+CVector(PORTAL_INTERPOLATE*float(sin(j)),
     PORTAL_INTERPOLATE*float(cos(j)), 0),
     pos+CVector(PORTAL_INTERPOLATE*float(sin(j+d)),
     PORTAL_INTERPOLATE*float(cos(j+d)),0),  CRGBA(255,255,255,255), *idriver);
  */
  ...
  //				CDRU::drawLinesUnlit(debugLines, mat, *idriver);
                  NL3D::CDRU::drawLine(debugLines[k].V0, debugLines[k].V1,
  CRGBA(0,255,0,255), *idriver);
          }
  */	// Draw listener pos
  mat.setColor(CRGBA(0, 255, 0, 255));
  {
    NL3D::CDRU::drawLine(pos + CVector(2, 2, 0), pos + CVector(-2, -2, 0),
                         CRGBA(255, 255, 0, 255), *idriver);
    NL3D::CDRU::drawLine(pos + CVector(-2, 2, 0), pos + CVector(2, -2, 0),
                         CRGBA(255, 255, 0, 255), *idriver);

    //				CVector front = Camera->getMatrix().getI();
    //		NL3D::CDRU::drawLine(pos, pos+front*4, CRGBA(255,255,0,255),
    //*idriver);
  }
  // draw the virtual sound sound
  {
    ...
    for (; first != last; ++first) {
      ...
      if (css.Direction != CVector::Null) {
        ...
        NL3D::CDRU::drawLine(dest + CVector(0.5f, 0.5f, 0),
                             dest + CVector(-0.5f, -0.5f, 0),
  ...
  // draw the audio path
  {
    ...
    const std::vector<std::pair<NLMISC::CVector, NLMISC::CVector>> &lines =
        cs->getAudioPath();
    std::vector<std::pair<NLMISC::CVector, NLMISC::CVector>>::const_iterator
        first(lines.begin()),
        last(lines.end());
    for (; first != last; ++first) {
      NL3D::CDRU::drawLine(first->first, first->second, CRGBA(0, 255, 0, 255),
                           *idriver);
    }
  }
  // draw the sound source position
  {
    ...
    std::vector<std::pair<bool, CVector>>::iterator first(soundPos.begin()),
        last(soundPos.end());
    for (; first != last; ++first) {
      NL3D::CDRU::drawLine(first->second + CVector(0.5f, 0.5f, 0),
                           first->second + CVector(-0.5f, -0.5f, 0),
                           CRGBA(255, 0, 255, 255), *idriver);
      NL3D::CDRU::drawLine(first->second + CVector(0.5f, -0.5f, 0),
                           first->second + CVector(-0.5f, 0.5f, 0),
                           CRGBA(255, 0, 255, 255), *idriver);
    }
...
UAudioMixer *CSoundManager::getMixer() { return _AudioMixer; }
...
void CSoundManager::loadContinent(const string &name,
...
void CSoundManager::init(IProgressCallback *progressCallBack) {
  ...
  _AudioMixer->init(ClientCfg.MaxTrack, ClientCfg.UseEax, ClientCfg.UseADPCM,
                    progressCallBack, false, driverType,
                    ClientCfg.SoundForceSoftwareBuffer);
  ...
  */
  ...
  _GroupControllerEffectsGame =
      _AudioMixer->getGroupController("sound:effects:game");
  ...
  for (uint i = 0;
       i < NLSOUND::UAudioMixer::TBackgroundFlags::NB_BACKGROUND_FLAGS; ++i)
  ...
  _AudioMixer->enableBackgroundMusicTimeConstraint(
      ClientCfg.EnableBackgroundMusicTimeConstraint);
...
// getSoundName:
//---------------------------------------------------
/*bool CSoundManager::getSoundName( TSound sound, std::string &name, const
CVector& pos ) const
{
        /// \todo Malkav: take ground type into account, and all the different
sounds switch (sound)
        {
                case WALK:
                {
...
//---------------------------------------------------
/*uint32 CSoundManager::addSource( TSound sound, const NLMISC::CVector
&position, bool play, bool loop, const CEntityId &parentId )
{
        ...
        if ( pSource  != NULL )
        {
                ...
                if (parentId != CEntityId::Unknown)
                        _AttachedSources.insert(
TMultiMapEntityToSource::value_type( parentId, source ) );
...
// return 0 if creation failed, sound id if creation was successful
//-----------------------------------------------
CSoundManager::TSourceId
CSoundManager::addSource(const NLMISC::TStringId &soundName,
                         const NLMISC::CVector &position, bool play, bool loop,
                         const CEntityId &id) {
  uint32 retValue = 0;
  ...
  if (pSource == 0) {
    nlwarning("Sound '%s' not found !",
              CStringMapper::unmap(soundName).c_str());
    return retValue;
  }

  // Load the properties for this sound.
  // REMOVED because the pitch and the gain are now in the .nss (don't need .sdf
  // anymore)
...
//-----------------------------------------------
bool CSoundManager::spawnSource(const NLMISC::TStringId &soundName,
  ...
  USource *pSource =
      _AudioMixer->createSource(soundName, true, NULL, NULL, NULL, &context);
  ...
  ////nlinfo ("%.3f spawn source %p", (float)ryzomGetLocalTime ()/1000.0f,
  ///pSource);
...
//
//---------------------------------------------------
/*bool CSoundManager::getSounds( uint32 material, TMoveType moveType, bool soft,
...
//---------------------------------------------------
void CSoundManager::updateEntityPos(const CEntityId &id,
                                    const NLMISC::CVector &pos) {
  TMultiMapEntityToSource::iterator it;
  const std::pair<TMultiMapEntityToSource::iterator,
                  TMultiMapEntityToSource::iterator>
      range = _AttachedSources.equal_range(id);
...
//---------------------------------------------------
void CSoundManager::updateEntityVelocity(const CEntityId &id,
...
//---------------------------------------------------
void CSoundManager::updateEntityDirection(const CEntityId &id,
...
//---------------------------------------------------
// removeEntity :
// remove an entity from the view : delete all the sources attached to that
// entity
//---------------------------------------------------
...
// setSoundPosition :
//---------------------------------------------------
void CSoundManager::setSoundPosition(TSourceId sourceId,
                                     const NLMISC::CVector &position) {
  if (!_PlaySound)
    return;
...
//---------------------------------------------------
/*
bool CSoundManager::setSoundForSource( uint32 sourceId, TSound sound, const
CVector& pos )
{
        // find the sound name
...
//---------------------------------------------------
/*CSoundManager::TSound	CSoundManager::getSoundFromSource( uint32 sourceId )
const
{
...
/*void CSoundManager::loadPositionedSounds()
{
        ...
        while( cstl.readLine() )
        {
                ...
                {
                        nlwarning ("positioned sound: can't find sound: '%s'",
soundName.c_str());
                }
        }
...
void CSoundManager::playPositionedSounds(const CVector & /* pos */) {
  ...
  for (itPSnd = _PositionedSounds.begin(); itPSnd != _PositionedSounds.end();
       ++itPSnd) {
    USource *pSource = _Sources.get(*itPSnd);
    if (!pSource) {
      nlwarning(
          "<CSoundManager::playPositionedSounds>  :  The source %d is unknown",
          *itPSnd);
    } else {
      /*
...
void CSoundManager::update() {
  ...
  // For each weather setup, gives the matching weather sound env to used, when
  // there are precipitation or not
  class CWeatherSetupSoundEnv {
  public:
    NLMISC::TStringId
        SetupName; // setup nem as found is the .weather_setup sheets
  ...
  for (uint k = 0; k < sizeof(weatherSoundLUT) / sizeof(weatherSoundLUT[0]);
       ++k) {
  ...
  if (weatherSetupSoundEnv) {
    ...
    if (!bDisplayOnce) {
      nlinfo("The current weather setup '%s' is unknown !",
             CStringMapper::unmap(weatherState.BestSetupName).c_str());
      bDisplayOnce = true;
    }
  }
  ...
  if (_EnableBackgroundMusicAtTime &&
...
// ***************************************************************************
void CSoundManager::playMusic(const string &fileName, uint xFadeTime,
                              bool async, bool loop,
                              bool forceGameMusicVolume) {
  if (fileName.empty()) {
    stopMusic(xFadeTime);
  } else {
    if (_AudioMixer) {
      // disable before the background music (before is important else may have
      // bug)
...
// ***************************************************************************
void CSoundManager::playEventMusic(const string &fileName, uint xFadeTime,
                                   bool loop) {
  ...
  if (fileName.empty()) {
    ...
    if (_AudioMixer) {
      ...
      if (_EventMusicPlayed.empty() ||
          nlstricmp(fileName, _EventMusicPlayed) != 0 ||
          loop != _EventMusicLoop) {
        // play
...
// ***************************************************************************
void CSoundManager::setGameMusicMode(bool enableBackground,
...
void CSoundManager::updateVolume() {
  if (_AudioMixer) {
    ...
    if (_UseGameMusicVolume)
      ...
      _AudioMixer->setMusicVolume(_GameMusicVolume * _FadeGameMusicVolume *
    ...
    // The event music volume get the game music volume, but is not faded (want
    // music during teleport)
...
void CSoundManager::fadeGameSound(bool fadeIn, sint32 timeFadeMs) {
  ...
  if (!fadeIn)
    dVolumeOverDt *= -1;
  while (fadeIn ? (_FadeGameMusicVolume < 1.f || _FadeSFXVolume < 1.f)
                : (_FadeGameMusicVolume > 0.f || _FadeSFXVolume > 0.f)) {
    // update volume vars
...
/*void CStepSounds::init( const char * fileName )
{
        ...
        {
                nlwarning ("can't load step sound because no material step
file"); return;
        }
        ...
        for( itMaterials = materials.begin(); itMaterials != materials.end();
++itMaterials )
        {
                CMaterialStepSounds materialStepSounds;
                materialStepSounds.load( CPath::lookup(string("sndmat_") +
toString(*itMaterials) + string(".txt")) ); _StepSoundsByMaterial.insert(
make_pair(*itMaterials,materialStepSounds) );
        }
...
//
//---------------------------------------------------
/*bool CStepSounds::getSounds( uint32 material, TMoveType moveType, bool soft,
std::vector<string>& sounds )
{
        nlstopex(("class CStepSounds is not used anymore"));
        map<uint32,CMaterialStepSounds>::iterator itMatSounds =
_StepSoundsByMaterial.find( material ); if( itMatSounds ==
_StepSoundsByMaterial.end() )
        {
                return false;
        }
        else
...
//
//---------------------------------------------------
/*void CMaterialStepSounds::readVar( CConfigFile& cf, TMoveType moveType, bool
soft, char * varName )
{
        try
        {
        ...
        {
                nlwarning("Problem in the sounds by material config file : %s",
e.what ());
        }
...
//
//---------------------------------------------------
bool CMaterialStepSounds::getSounds( TMoveType moveType, bool soft,
vector<string>& sounds )
{
        map<pair<TMoveType,bool>,vector<string> >::iterator itSounds =
_Sounds.find( make_pair(moveType,soft) ); if( itSounds == _Sounds.end() )
        {
                return false;
        }
