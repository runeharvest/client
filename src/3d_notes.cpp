...
void cbNotesChanged() {
  ...
  for (sint i = 0; i < var.size(); i += 3) {
    ...
    if (id == 0) {
      nlwarning("Malformated id 0 in note file '%s', discard all notes",
                ClientCfg.NotesFilename.c_str());
      Notes.Notes.clear();
      return;
    }
    ...
    if (posstr.size() != 3) {
      nlwarning("Malformated position in note number %d in file '%s', discard "
                "all notes",
                id, ClientCfg.NotesFilename.c_str());
      Notes.Notes.clear();
      return;
    }
    ...
    if (note.empty()) {
      nlwarning("Malformated because empty note in note number %d in file "
                "'%s', skipping it",
                id, ClientCfg.NotesFilename.c_str());
    } else {
...
C3DNotes::CNote::CNote(const NLMISC::CVector &pos, const std::string &note,
                       sint id)
...
void C3DNotes::init() {
  if (!ClientCfg.NotesFilename.empty()) {
    ...
    try {
      ...
      nlwarning("Error while loading '%s': %s", ClientCfg.NotesFilename.c_str(),
                e.what());
  ...
  for (list<CNote>::iterator it = Notes.begin(); it != Notes.end(); it++) {
    if ((*it).Bubble == NULL && ((*it).Position - userPos).norm() < 100) {
      ...
      if ((*it).Bubble) {
        ...
        nlinfo("NOTE: id %d pos(%f,%f,%f) note '%s'", (*it).Id,
...
void C3DNotes::release() { Notes.clear(); }
...
void C3DNotes::saveNotes() {
  ...
  for (list<CNote>::iterator it = Notes.begin(); it != Notes.end(); it++) {
    ...
    vals.push_back(toString("%f,%f,%f", (*it).Position.x, (*it).Position.y,
                            (*it).Position.z));
...
void C3DNotes::addNote(const NLMISC::CVector &pos, const string &note,
                       sint id) {
  if (!NotesAvailable)
    return;
...
NLMISC_COMMAND(displayNotes, "display all notes", "") {
  ...
  for (list<C3DNotes::CNote>::iterator it = Notes.Notes.begin();
       it != Notes.Notes.end(); it++) {
    log.displayNL("id %d pos(%f,%f,%f) note '%s'", (*it).Id, (*it).Position.x,
                  (*it).Position.y, (*it).Position.z, (*it).Note.c_str());
  }
