...
//-----------------------------------------------
void CIGCallback::addIGsWithNumZC(
    std::vector<std::pair<NL3D::UInstanceGroup *, sint>> &igs) {
  H_AUTO_USE(RZ_IGCallback)
  _IGInstances.reserve(_IGInstances.size() + igs.size());
  for (uint k = 0; k < igs.size(); ++k) {
...
///===================================================================================
CIGCallback::CIGInstance::CIGInstance(NL3D::UInstanceGroup *ig,
                                      CIGCallback *owner)
...
void CIGCallback::CIGInstance::instanceGroupAdded() {
  ...
  if (!_MovePrimitives.empty())
    return; // already added
  std::vector<NLPACS::UMovePrimitive *> addedPrims;
  uint numInstances = _IG->getNumInstance();
  for (uint k = 0; k < numInstances; ++k) {
    TPacsPrimMap::iterator pbIt = PacsPrims.find(NLMISC::toLowerAscii(
        NLMISC::CFile::getFilenameWithoutExtension(_IG->getShapeName(k))));
    if (pbIt != PacsPrims.end()) {
      ...
      NLPACS::UMoveContainer::getPACSCoordsFromMatrix(pos, angle,
                                                      instanceMatrix);
      // insert the matching primitive block
      addedPrims.clear();
      _Owner->_MoveContainer->addCollisionnablePrimitiveBlock(
          pbIt->second, 0, 1, &addedPrims, angle, pos, true, scale);
      ...
      for (uint i = 0; i < addedPrims.size(); i++) {
        ...
        if (addedPrims[i] && addedPrims[i]->getObstacle() &&
            addedPrims[i]->getTriggerType() ==
                NLPACS::UMovePrimitive::NotATrigger) {
          addedPrims[i]->setHeight(addedPrims[i]->getHeight() +
                                   RZ_PRIM_ZEXT_BLOCK_AVOIDANCE);
        }
      }
      // for future remove
      _MovePrimitives.insert(_MovePrimitives.end(), addedPrims.begin(),
                             addedPrims.end());
    }
  }
...
void CIGCallback::CIGInstance::releaseMovePrimitives() {
  ...
  for (TMovePrimitiveVect::iterator it = _MovePrimitives.begin();
       it != _MovePrimitives.end(); ++it) {
...
void CIGCallback::deleteIGs() {
  H_AUTO_USE(RZ_IGCallback)
  for (TIGInstanceList::iterator it = _IGInstances.begin();
       it != _IGInstances.end(); ++it) {
    delete *it;
  }
  _IGInstances.clear();
}
...
void CIGCallback::CIGInstance::updateFromSheets() {
  ...
  // See for which objects distance should be overriden (object which use a
  // .PLANT sheet)
  ...
  for (uint k = 0; k < numInstances; ++k) {
    if (_EntitySheets[k] && _EntitySheets[k]->Type == CEntitySheet::PLANT) {
      ...
      if (ps->getMaxDist() != -1)
        _IG->setDistMax(k, ps->getMaxDist());
      if (ps->getCoarseMeshDist() != -1)
        _IG->setCoarseMeshDist(k, ps->getCoarseMeshDist());
    }
  }
}
...
void CIGCallback::CIGInstance::updateManagedFXs() {
  ...
  // .PLANT sheet)
  uint numInstances = (uint)_EntitySheets.size();
  // vector of fx that should be managed by the dedicated manager. static for
  // malloc perf
...
bool CIGCallback::enumIGs(IIGEnum *callback) {
  ...
  for (TIGInstanceList::iterator it = _IGInstances.begin();
       it != _IGInstances.end(); ++it) {
    if ((*it)->getIG() != NULL &&
        (*it)->getIG() != (NL3D::UInstanceGroup *)-1) {
...
void CIGCallback::changeSeason() {
  H_AUTO_USE(RZ_IGCallback)
  // for now, this only update managed fxs so that they are displayed the same
  // way on both clients
  for (TIGInstanceList::iterator it = _IGInstances.begin();
       it != _IGInstances.end(); ++it) {
    if ((*it)->getIG() != NULL &&
        (*it)->getIG() != (NL3D::UInstanceGroup *)-1) {
      (*it)->buildSheetVector(); // the sheet vector is deleted after use, so
                                 // need to rebuild it
      (*it)->shutDownFXs();
      (*it)->updateManagedFXs();
      (*it)->eraseSheetVector();
...
void createInstancesFromMoveContainer(
    NL3D::UScene *scene, NLPACS::UMoveContainer *mc,
    std::vector<NL3D::UInstance> *instances /*=NULL*/) {
  H_AUTO_USE(RZ_IGCallback)
  nlassert(scene);
  nlassert(mc);
  ...
  for (uint k = 0; k < prims.size(); ++k) {
    ...
    default:
      nlwarning(
          "createInstancesFromMoveContainer : unsupported type encountered");
      continue;
      break;
    }
    if (!newInstance.empty()) {
      scale.z = prims[k]->getHeight();
      NLMISC::CVectorD worldPos =
          prims[k]->getFinalPosition(prims[k]->getFirstWorldImageV());
      newInstance.setPos(NLMISC::CVector((float)worldPos.x, (float)worldPos.y,
                                         (float)worldPos.z));
