...
public:
  EDownloadTerminationRequested()
      : Exception("Download termination requested") {}
};
...
//=====================================================
CBGDownloaderAccess::~CBGDownloaderAccess() { delete _DownloadCoTask; }

//=====================================================
void CBGDownloaderAccess::init() { release(); }
...
//=====================================================
void CBGDownloaderAccess::startTask(const BGDownloader::CTaskDesc &taskDesc,
...
//=====================================================
bool CBGDownloaderAccess::isTaskEnded(BGDownloader::TTaskResult &result,
                                      ucstring &errorMsg) const // OLD
{
  ...
  try {
    ...
    if (Parent->_WantedTask.State == DLState_GetAndApplyPatch) {
      ...
      if (!isDownloaderProcessRunning() &&
          getDownloaderMode() != DownloaderMode_Slave) {
    ...
    // clear display info, else they would remain for a brief time when arriving
    // on patch screen
    Parent->clearCurrentMessage();
    //
    getTaskResult(Parent->_TaskResult, Parent->_AvailablePatchs,
                  Parent->_MustLaunchBatFile, Parent->_ErrorMsg);
    ...
    if (Parent->_TaskResult == BGDownloader::TaskResult_Success) {
      if (Parent->_WantedTask == DLState_CheckPatch) {
        ...
        if (!Parent->_AvailablePatchs) {
          ...
          // because the downloader may pop a notification balloon here, wait
          // some more before shutting it down

          sleep(2500); // at this time, may last longer because the client
        ...
        bool rosPatch = (Parent->_AvailablePatchs &
                         (1 << BGDownloader::DownloadID_RoS)) != 0;
        bool mainlandPatch = (Parent->_AvailablePatchs &
                              (1 << BGDownloader::DownloadID_MainLand)) != 0;

        if (Parent->_MustLaunchBatFile &&
            !rosPatch) // let be superstitious, should always be true ...
        {
          // no patch but must rebbot ? may happen only if some 'unpacked' file
          // needed by RoS have been deleted
          shutdownDownloader();
        } else if (mainlandPatch) {
          ...
          for (uint catIndex = 0; catIndex < bnpCatSet.categoryCount();
               ++catIndex) {
            const CBNPCategory &cat = bnpCatSet.getCategory(catIndex);
            if (cat.isOptional()) // NB : 'optional' flag meaning has changed :
                                  // it now means 'Mainland Patch' until an enum
                                  // is added
            {
      ...
      if (Parent->_WantedTask.State == DLState_GetAndApplyPatch) {
        ...
        if (Parent->_WantedTask.DownloadBitfield &
            (1 << BGDownloader::DownloadID_RoS)) {
          if (Parent->_MustLaunchBatFile) // let be superstitious, should always
                                          // be true ...
          {
            Parent->_WaitBalloonDisplay = true;
            Parent->_State = State_Finished;
        ...
        } else if (Parent->_WantedTask.DownloadBitfield &
                   (1 << BGDownloader::DownloadID_MainLand)) {
          bool memoryCompressed = CPath::isMemoryCompressed();
          if (memoryCompressed) {
            CPath::memoryUncompress();
          }
          // and redo 'addSearchPath' on data, because of the new bnp files that
          ...
          sleep(5000);
          Parent->_WaitBalloonDisplay = false;
          // stop after the mainland download (hardcoded, but should suffice for
          // now)
          shutdownDownloader();
        }
      }
    ...
    Parent->_ErrorMsg = CI18N::get("uiBGD_ProtocolError") + ucstring(" : ") +
                        ucstring(e.what()); // OLD
    Parent->_DownloadThreadPriority = ThreadPriority_DownloaderError;
...
bool CBGDownloaderAccess::CDownloadCoTask::isDownloaderProcessRunning() {
  // the downloader creates a system-wide mutex, so if present, assume that the
...
void CBGDownloaderAccess::CDownloadCoTask::createDownloaderProcess() {
  ...
  CConfigFile::CVar *manualLaunchVar =
      ClientCfg.ConfigFile.getVarPtr("BackgroundDownloaderManualLaunch");
  if (manualLaunchVar) {
...
void CBGDownloaderAccess::CDownloadCoTask::restartDownloader() {
  ...
  for (;;) {
    void *ryzomInstPIDPtr = NLMISC::CSharedMemory::accessSharedMemory(
        NLMISC::toSharedMemId(RYZOM_PID_SHM_ID));
    if (!ryzomInstPIDPtr)
      break; // only instance running is us
    // try to terminate the other instance
    uint count = 0;
    while (*(DWORD *)ryzomInstPIDPtr == 0) {
      // the other process didn't write its pid into shared mem yet (shared
      // memory is initially filled with zero's) so wait a bit (very unlikely
      // case !!)
      sleep(100);
      ++count;
      if (count == 50) {
        nlwarning("CBGDownloaderAccess::CDownloadCoTask : detected shared "
                  "memory segment, with NULL pid");
        // some problem here ...
    ...
    if (!ok) {
      nlwarning("CBGDownloaderAccess::CDownloadCoTask : detected shared memory "
                "segment, with good pid, but couldn't stop the process");
      // couldn't stop the other client ...
      throw EDownloadException(CI18N::get("uiBGD_MultipleRyzomInstance"));
    }
  }
  // write our pid into shared mem
  Parent->_RyzomInstPIDPtr = CSharedMemory::createSharedMemory(
      NLMISC::toSharedMemId(RYZOM_PID_SHM_ID), sizeof(uint32));
  ...
  *(uint32 *)Parent->_RyzomInstPIDPtr = (uint32)GetCurrentProcessId();
  ...
  if (isDownloaderProcessRunning()) {
    Parent->_DownloaderMsgQueue.init(hWnd, BGDownloader::ClientWndID,
  ...
  for (;;) {
    ...
    // now we can create the message queue because we are sure that it will
    // reach the good app
    Parent->_DownloaderMsgQueue.init(HInstance, BGDownloader::ClientWndID,
                                     BGDownloader::DownloaderWndID);
    sleep(200);
    ...
    while (waitTime <= 32000) {
      Parent->_CurrentMessage.fromUtf8(toString(
          CI18N::get("uiBGD_HandShaking").c_str(), tryIndex, totalTries));
...
//=====================================================
void CBGDownloaderAccess::showDownloader() { sendSimpleMsg(CL_Show); }

//=====================================================
void CBGDownloaderAccess::hideDownloader() { sendSimpleMsg(CL_Hide); }
...
//=====================================================
void CBGDownloaderAccess::CDownloadCoTask::getTaskResult(
    TTaskResult &result, uint32 &availablePatchs, bool &mustLaunchBatFile,
...
void CBGDownloaderAccess::CDownloadCoTask::doTask(const CTaskDesc &taskDesc) {
  ...
  if (downloaderTask.State != DLState_Idle && downloaderTask != taskDesc) {
    // if not the wanted task, ask to stop current task, and wait until
    // completion
    stopTask();
    startTask(taskDesc);
  } else if (downloaderTask.State == DLState_Idle) {
    startTask(taskDesc);
  }
...
//=====================================================
void CBGDownloaderAccess::CDownloadCoTask::startTask(
    const CTaskDesc &taskDesc) {
  CMemStream outMsg(false /* output stream */);
...
void CBGDownloaderAccess::CDownloadCoTask::shutdownDownloader() {
  const uint32 SHUTDOWN_TIMEOUT = 6; // seconds until we consider the downloader
                                     // is hung, and try to shut it down
  ...
  while (isDownloaderProcessRunning()) {
    ...
    yield();
    if ((NLMISC::CTime::getSecondsSince1970() - startTime) >=
        SHUTDOWN_TIMEOUT) {
...
//=====================================================
void CBGDownloaderAccess::CDownloadCoTask::setDownloaderMode(
    TDownloaderMode mode) {
  CMemStream outMsg(false /* output stream */);
...
//=====================================================
void CBGDownloaderAccess::CDownloadCoTask::setDownloaderVerbosity(
    bool verbose) {
  CMemStream outMsg(false /* output stream */);
...
//=====================================================
void CBGDownloaderAccess::CDownloadCoTask::waitMsg(
    BGDownloader::TMsgType wantedMsgType, NLMISC::CMemStream &msg,
    ...
    while (!Parent->_DownloaderMsgQueue.pumpMessage(msg)) {
...
#endif
      {
        if (NLMISC::CTime::getLocalTime() - startTime > timeOutInMs) {
          nlwarning("Time out exceeded while waiting for message of type %d",
                    (int)wantedMsgType);
#ifdef NL_DEBUG
          // nlassert(0);
#endif
  ...
  if (msgType != wantedMsgType) {
    nlwarning("BG DOWNLOADER PROTOCOL ERROR ! Bad message type received. "
              "Expected type is '%d', received type is '%d'",
              (int)wantedMsgType, (int)msgType);
    throw EDownloadException(CI18N::get("uiBGD_ProtocolError"));
  }
}

//=====================================================
bool CBGDownloaderAccess::CDownloadCoTask::defaultMessageHandling(
    BGDownloader::TMsgType msgType, NLMISC::CMemStream &msg) {
  nlassert(msg.isReading());
  switch (msgType) {
  case BGD_Priority: {
...
void CBGDownloaderAccess::CDownloadCoTask::checkDownloaderAlive() {
  if (!Parent->_DownloaderMsgQueue.connected() ||
      !isDownloaderProcessRunning()) {
...
//=====================================================
void CBGDownloaderAccess::CDownloadCoTask::sendSimpleMsg(
    BGDownloader::TMsgType msgType) {
  // nlwarning("##CLIENT SEND simple message of type : %s",
  // toString(msgType).c_str());
...
//=====================================================
void CBGDownloaderAccess::requestDownloadThreadPriority(
    BGDownloader::TThreadPriority newPriority, bool freezeUI) {
...
bool isBGDownloadEnabled() {
  return ClientCfg.BackgroundDownloader && !ClientCfg.Local &&
         ClientCfg.PatchWanted;
}

static BGDownloader::TThreadPriority DownloaderPriorityBackup =
...
void pauseBGDownloader() {
  if (isBGDownloadEnabled() && !DownloaderPaused) {
    ...
    downloader.requestDownloadThreadPriority(
        BGDownloader::ThreadPriority_Paused, true);
