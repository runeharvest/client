...
void CStage::addProperty(uint property, sint64 value, bool replace) {
  ...
  pair<TStage::iterator, bool> result =
      _Stage.insert(make_pair(property, value));
  if (!result.second) {
    // Replace the property if wanted.
    if (replace) {
      TStage::iterator itProp = _Stage.find(property);
      if (itProp == _Stage.end())
        nlwarning("CStage::addProperty: Cannot insert the property '%d' but "
                  "cannot find it too.",
                  property);
      else
        (*itProp).second = value;
...
//-----------------------------------------------
// Return a pair according to the property asked. First element (the bool),
// indicate if the value (second element) is valid. \param uint prop : property
// asked (to get its value). \return pair<bool, sint64> : '.first' == 'true' if
// the property exist so value is valid. '.second' is the value if '.first' ==
// 'true'.
//-----------------------------------------------
...
bool CStage::getPos(NLMISC::CVectorD &pos) const {
  ...
  if (resultY.first == false) {
    nlwarning(
        "CStage:getPos: there is property X in the Stage but no property Y.");
    return false;
  }
  ...
  if (resultZ.first == false) {
    nlwarning("CStage:getPos: there are properties X and Y in the Stage but no "
              "property Z.");
    return false;
  }
...
// \warning If the property already exist, this method does not change the
// value. \warning If the Game Cycle is before the first element, this method do
// not try to add a stage but will try to add a property to the first one.
// \param TGameCycle gameCycle : This is the Key for the stage to search where
// to add the property. \param uint property : property to add. \param uint64
// value : value of the property. \return CStage * : pointer on the stage or 0.
//-----------------------------------------------
CStage *CStageSet::addStage(NLMISC::TGameCycle gameCycle, uint property,
                            sint64 value) {
  ...
  if (!_StageSet.empty()) {
    ...
    if (gameCycle < gcFirst) // Could be egal if we do not replace the property
                             // in the case the property already exist too.
    {
      // Game Cycle to early -> put the property in the first one instead (but
      // do not replace the property in the first 1 if already exist).
      it = _StageSet.begin();
      replace = false;
    }
    // The property will not be inserted before the first one.
      ...
      pair<TStageSet::iterator, bool> result =
          _StageSet.insert(make_pair(gameCycle, CStage()));
      if (!result.second) {
        TStageSet::iterator itFind = _StageSet.find(gameCycle);
        if (itFind == _StageSet.end()) {
          nlwarning("CStageSet::addStage: Cannot insert a stage for the "
                    "gamecycle '%d' but cannot find too.",
                    gameCycle);
          return 0;
        }
  ...
  // There is no stage for the moment.
    ...
    pair<TStageSet::iterator, bool> result =
        _StageSet.insert(make_pair(gameCycle, CStage()));
    if (!result.second) {
      nlwarning("CStageSet::addStage : Strange, map is empty but cannot insert "
...
//-----------------------------------------------
CStage *CStageSet::addStage(NLMISC::TGameCycle gameCycle, uint property,
                            sint64 value, NLMISC::TGameCycle predictedI) {
