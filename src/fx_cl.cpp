 ...
 * Constructor
 */
CFxCL::CFxCL() : CEntityCL(), _FXSheet(NULL), _BadBuild(false) { init(); }
...
bool CFxCL::build(const CEntitySheet *sheet) {
  ...
  // Get the FX filename and user params from the sheet (only the first one of
  // PSList)
  _FXSheet = dynamic_cast<const CFXSheet *>(sheet);
  if ((!_FXSheet) || (_FXSheet->PSList.empty())) {
    _BadBuild = true;
    nlwarning("Bad sheet %s for fx",
 ...
 */
void CFxCL::updateVisualPropertyPos(const NLMISC::TGameCycle & /* gameCycle */,
                                    const sint64 &prop,
  ...
  if (nodeY == 0) {
    nlwarning("ITM:updtVPPos:%d: Cannot find the property 'PROPERTY_POSY(%d)'.",
  ...
  if (nodeZ == 0) {
    nlwarning("ITM:updtVPPos:%d: Cannot find the property 'PROPERTY_POSZ(%d)'.",
  ...
  if (_Instance.empty()) {
    ...
    UParticleSystemInstance fxInst;
    fxInst.cast(_Instance);
    fxInst.setUserParam(0, _FXSheet->PSList[0]
                               .Power[CFXSheet::CPSStruct::StandardIndex]
                               .UserParam0);
    fxInst.setUserParam(1, _FXSheet->PSList[0]
                               .Power[CFXSheet::CPSStruct::StandardIndex]
                               .UserParam1);
  }
  ...
  if (!_Instance.empty()) {
    ...
    // nldebug( "SelectBox: %s %s", _SelectBox.getCenter().asString().c_str(),
    // _SelectBox.getHalfSize().asString().c_str() );

    // Adjust the collision.
    if (_Primitive) {
      _Primitive->setRadius(std::min(
          std::max((_SelectBox.getHalfSize()).x, (_SelectBox.getHalfSize()).y),
