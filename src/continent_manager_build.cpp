...
void primitiveToLM(CContLandMark &lm, IPrimitive *p) {
  ...
  if (pvVect != NULL)
    for (uint32 j = 0; j < p->getNumVector(); ++j)
      lm.Zone.VPoints.push_back(
          CPrimVector(CVector2f(pvVect[j].x, pvVect[j].y)));

  lm.Pos = lm.getZoneCenter();
}

// ***************************************************************************
bool buildLMConts(const std::string &worldSheet,
  ...
  if (bRebuild) {
    ...
    for (uint32 nRegion = 0; nRegion < vRegionFiles.size(); ++nRegion) {
      ...
      for (uint32 nCont = 0; nCont < vContRes.size(); ++nCont) {
        ...
        map<string, sTmpContinent *>::iterator it =
            AllContinents.find(contName);
        if (it != AllContinents.end()) {
          ...
          if (lm.Zone.VPoints.size() != 0)
            ...
            nlwarning("continent %s do not contain any Zone (used for select)",
                      contName.c_str());
          ...
          for (i = 0; i < vGenRes.size(); ++i) {
            ...
            if (lm.Zone.VPoints.size() != 0)
              ...
              nlwarning("region %s do not contain any point",
                        lm.TitleTextID.c_str());
            ...
            IPrimitive *aliasNode =
                getPrimitiveChild(const_cast<IPrimitive *>(vGenRes[i]), pred);
            if (aliasNode) {
            ...
            // nlinfo("aliasToRegionMap -- name = %s : %u", primName.c_str(),
            // alias);
          }
          ...
          for (i = 0; i < vGenRes.size(); ++i) {
            ...
            if (disp != "true")
              continue; // Does the place is displayed

            vGenRes[i]->getPropertyByName("place_type", placeType);

            lm.Type = CContLandMark::Place;
            if (placeType == "Capital")
              lm.Type = CContLandMark::Capital;
            else if (placeType == "Village")
              lm.Type = CContLandMark::Village;
            else if (placeType == "Outpost")
              lm.Type = CContLandMark::Outpost;
            else if (placeType == "Locality")
              lm.Type = CContLandMark::Place;
            else if (placeType == "Street")
              lm.Type = CContLandMark::Street;

            primitiveToLM(lm, vGenRes[i]);
            if (lm.Zone.VPoints.size() != 0)
              pCont->ContLandMarks.push_back(lm);
            else
              nlwarning("place %s do not contain any point",
                        lm.TitleTextID.c_str());
          }
          ...
          for (i = 0; i < vGenRes.size(); ++i) {
            ...
            if (lm.Zone.VPoints.size() != 0)
              ...
              nlwarning("stable %s do not contain any point",
                        lm.TitleTextID.c_str());
          }
        } else {
          nlwarning("cannot find continent %s in %s", contName.c_str(),
                    worldSheet.c_str());
        }
      }
    }
