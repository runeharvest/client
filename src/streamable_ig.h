...
/** A streamable IG hierarchy (hierarchy is used to build clusters hierarchy).
 * It may be loaded synchronously or asynchronously depending on the distance.
 */
...
public:
   ...
   */
  void init(NL3D::UScene *owningScene, const NLMISC::CVector &pos,
            float forceLoadRadius, float loadRadius, float unloadRadius);
  NL3D::UScene *getScene() const { return _Scene; }

  /// Optimisation purpose. If you know how many calls to addIG will be done,
  /// you can call this
  void reserve(uint size);
  /** Add an ig and the name of its parent ig (for clusters linking), or an
   * empty name it no linking is done
   */
  void addIG(const std::string &name, const std::string &parentName,
             const NLMISC::CVector &pos, const NLMISC::CQuat &rot);
  /** Set an additionnal map (ig name -> ig pointer) that will be filled with
   * loaded igs. Setting that pointer to NULL will remove currently loaded igs
   * from the map and will disable it. By default, no map is used
   */
  ...
  uint getIGNum() const { return (uint)_IGs.size(); }
