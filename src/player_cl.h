...
public:
  ...
  virtual bool isAFK() const {
    return (_Mode == MBEHAV::REST || properties().afk());
  }

  /// Return true if this player is in the same faction as the user's (except if
  /// neutral)
...
protected:
  ...
  void equip(SLOTTYPE::EVisualSlot slot, const std::string &shapeName,
             const CItemSheet *item = 0);
  ...
  virtual void updateVisualPropertyVpc(const NLMISC::TGameCycle &gameCycle,
                                       const sint64 &prop);
  /// Update the Visual Property PVP Mode (need special imp for player because
  /// of PVP consider)
