...
public:
  ...
  // Useful for serial
  void initSheet(CEntitySheet *pES, NLMISC::IStream &s,
...
public:
  ...
  /// Load all sheets.
  /// \param needComputeVS : if 'true' this will compute the visual slot file
  /// too.
  void load(NLMISC::IProgressCallback &callBack, bool updatePackedSheet,
            bool needComputeVS = false, bool dumpVSIndex = false);
  /// Load all sheet or a subset of them (if 'extensions' is not NULL, them the
  /// extensions list is taken from that vector)
  void loadAllSheet(NLMISC::IProgressCallback &callBack, bool updatePackedSheet,
                    bool needComputeVS, bool dumpVSIndex,
                    bool forceRecompute = false,
                    const std::vector<std::string> *extensions = NULL);
  /// Load all sheet, without packed sheet, and with a wildcard (for reload
  /// feature)
  void loadAllSheetNoPackedSheet(NLMISC::IProgressCallback &callBack,
   ...
   * \return CEntitySheet * : pointer on the sheet according to the param or 0
   * if any pb.
   */
  ...
  void setOutputDataPath(const std::string &dataPath) {
...
protected:
  /// this structure is fill by the loadForm() function and will contain all the
  /// sheets needed
