...
// Constructor.
//-----------------------------------------------
CPlayerCL::CPlayerCL() : CCharacterCL() {
  Type = Player;
...
const std::vector<CGroundFXSheet> *CPlayerCL::getGroundFX() const {
  switch (getGender()) {
  case 0:
    return &(_PlayerSheet->GenderInfos[0].GroundFX);
  case 1:
    return &(_PlayerSheet->GenderInfos[1].GroundFX);
  default:
    break;
  }
  return NULL;
...
//-----------------------------------------------
bool CPlayerCL::isNeutral() const { return (!isEnemy() && !isAlly()); }

//-----------------------------------------------
// isFriend :
// Return true if this entity is a user's friend.
//-----------------------------------------------
bool CPlayerCL::isFriend() const { return isNeutral() || isAlly(); }
...
bool CPlayerCL::isEnemy() const {
  ...
  if ((getPvpMode() & PVP_MODE::PvpFaction ||
      ...
      (UserEntity->getPvpMode() & PVP_MODE::PvpFaction ||
...
bool CPlayerCL::build(const CEntitySheet *sheet) // virtual
{
  ...
  if (_PlayerSheet == 0) {
    pushDebugStr(NLMISC::toString(
        "Player '%d' sheet is not a '.race_stats' -> BIG PROBLEM.", _Slot));
    return false;
  } else
  ...
  initPrimitive(0.5f, 2.0f, 0.0f, 0.0f, UMovePrimitive::DoNothing,
                UMovePrimitive::NotATrigger, MaskColPlayer, MaskColNone);
...
//-----------------------------------------------
void CPlayerCL::equip(SLOTTYPE::EVisualSlot slot, const std::string &shapeName,
                      const CItemSheet *item) {
  // Check slot.
  ...
  if (!_Instances[s].Loading.empty()) {
    if ((_Instances[s].LoadingName == shapeName) &&
        (_Instances[s].FXItemSheet == item))
      return;
  } else if (!_Instances[s].Current.empty()) {
    if ((_Instances[s].CurrentName == shapeName) &&
        (_Instances[s].FXItemSheet == item))
      return;
  }
  ...
  /* If the object is sticked (ie not a skin), decide to delete the Current
     instance. Why? because the animation is changed according to the equipped
     item.

          Hence, For example, if a sword would be changed for a gun, then the
     new gun animation would take place, while Keeping the old sword shape. BAD.
  */
  if (!stickPoint.empty())
  ...
  if (!_Instances[s].Loading.empty()) {
    ...
    nlwarning("PL::equip(1):%d: cannot create the instance '%s'.", _Slot,
              shapeName.c_str());

  if (!item && (slot != SLOTTYPE::RIGHT_HAND_SLOT) &&
      (slot != SLOTTYPE::LEFT_HAND_SLOT))
...
void CPlayerCL::equip(SLOTTYPE::EVisualSlot slot, uint index, uint color) {
  ...
  if (_Items[slot].Sheet) {
    ...
    std::string shapeName =
        _Gender == GSGENDER::female ? item->getShapeFemale() : item->getShape();

    // use the right type of boots if wearing a caster dress
    if ((slot == SLOTTYPE::FEET_SLOT) &&
      ...
      if (!shapeLegs.empty() &&
          shapeLegs.find("_caster01_") != std::string::npos) {
    ...
    if (!pInst.empty()) {
      ...
      if (slot == SLOTTYPE::HEAD_SLOT && item->Family != ITEMFAMILY::ARMOR)
        ...
        // Set the User Color.
        if (item->Color == -1)
          applyColorSlot(_Instances[slot], skin(), color, _HairColor,
                         _EyesColor);
        // Set the Item Color.
        else if (item->Color != -2)
          applyColorSlot(_Instances[slot], skin(), item->Color, _HairColor,
                         _EyesColor);
  ...
  // Default equipment.
    ...
    sint idx = SheetMngr.getVSIndex(
        _PlayerSheet->GenderInfos[_Gender].Items[slot], slot);
    if (idx != -1) {
      if (SheetMngr.getItem(slot, (uint)idx)) {
        const CItemSheet *itemSheet = SheetMngr.getItem(slot, (uint)idx);

        // If the gender is a female get the right shape else get the default
        // shape.
        equip(slot, _Gender == GSGENDER::female ? itemSheet->getShapeFemale()
                                                : itemSheet->getShape());
      }
    }
  }
...
void CPlayerCL::computeAnimSet() {
  ...
  if (_Gender >= 2) {
    nlwarning("PL:computeAnimSet:%d: not a male or a female (gender=%d).",
              _Slot, _Gender);
    return;
  }
  ...
  if (!::computeAnimSet(_CurrentAnimSet[MOVE], _Mode,
                        _PlayerSheet->GenderInfos[_Gender].AnimSetBaseName,
                        _Items[SLOTTYPE::LEFT_HAND_SLOT].Sheet,
                        _Items[SLOTTYPE::RIGHT_HAND_SLOT].Sheet,
                        !modeWithHiddenItems()))
...
//-----------------------------------------------
void CPlayerCL::updateVisualPropertyVpa(
  ...
  if (_Gender != GSGENDER::male && _Gender != GSGENDER::female) {
    nlwarning(
        "PL::updateVPVpa:%d: neither a male nor a female -> male selected.",
        _Slot);
  ...
  const string replacement =
      STRING_MANAGER::CStringManagerClient::getTitleLocalizedName(
          _TitleRaw, _Gender == GSGENDER::female);
  ...
  if (_CacheSkeletonShapeName !=
      _PlayerSheet->GenderInfos[_Gender].Skelfilename) {
  ...
  if (skeleton() && !ClientCfg.Light) {
    ...
    sint clodId = getLodCharacterId(
        *Scene, _PlayerSheet->GenderInfos[_Gender].LodCharacterName);
    if (clodId >= 0) {
      // Setup Lod Character shape and distance
      skeleton()->setLodCharacterShape(clodId);
      skeleton()->setLodCharacterDistance(
          _PlayerSheet->GenderInfos[_Gender].LodCharacterDistance);
    }
    // Compute the
    computeSomeBoneId();

    // CHEST
    equip(SLOTTYPE::CHEST_SLOT, visualA.PropertySubData.JacketModel,
          visualA.PropertySubData.JacketColor);
    // LEGS
    equip(SLOTTYPE::LEGS_SLOT, visualA.PropertySubData.TrouserModel,
          visualA.PropertySubData.TrouserColor);
    // ARMS
    equip(SLOTTYPE::ARMS_SLOT, visualA.PropertySubData.ArmModel,
          visualA.PropertySubData.ArmColor);
    // HAT
    equip(SLOTTYPE::HEAD_SLOT, visualA.PropertySubData.HatModel,
          visualA.PropertySubData.HatColor);
    // OBJECT in the RIGHT HAND
    _Items[SLOTTYPE::RIGHT_HAND_SLOT].Sheet = SheetMngr.getItem(
        SLOTTYPE::RIGHT_HAND_SLOT, visualA.PropertySubData.WeaponRightHand);
    ...
    if (_Items[SLOTTYPE::RIGHT_HAND_SLOT].Sheet) {
      if (_Gender == GSGENDER::female)
        equip(SLOTTYPE::RIGHT_HAND_SLOT,
              _Items[SLOTTYPE::RIGHT_HAND_SLOT].Sheet->getShapeFemale(),
              _Items[SLOTTYPE::RIGHT_HAND_SLOT].Sheet);
      else
        equip(SLOTTYPE::RIGHT_HAND_SLOT,
              _Items[SLOTTYPE::RIGHT_HAND_SLOT].Sheet->getShape(),
              _Items[SLOTTYPE::RIGHT_HAND_SLOT].Sheet);

      NL3D::UInstance itemInstance =
          (!_Instances[SLOTTYPE::RIGHT_HAND_SLOT].Loading.empty())
              ? _Instances[SLOTTYPE::RIGHT_HAND_SLOT].Loading
              : _Instances[SLOTTYPE::RIGHT_HAND_SLOT].Current;
    ...
    _Items[SLOTTYPE::LEFT_HAND_SLOT].Sheet = SheetMngr.getItem(
        SLOTTYPE::LEFT_HAND_SLOT, visualA.PropertySubData.WeaponLeftHand);
    // Equip the weapon(object/tool).
    if (_Items[SLOTTYPE::LEFT_HAND_SLOT].Sheet) {
      equip(SLOTTYPE::LEFT_HAND_SLOT,
            _Items[SLOTTYPE::LEFT_HAND_SLOT].Sheet->getShape(),
            _Items[SLOTTYPE::LEFT_HAND_SLOT].Sheet);
      NL3D::UInstance itemInstance =
          (!_Instances[SLOTTYPE::LEFT_HAND_SLOT].Loading.empty())
              ? _Instances[SLOTTYPE::LEFT_HAND_SLOT].Loading
              : _Instances[SLOTTYPE::LEFT_HAND_SLOT].Current;
      if (!itemInstance.empty()) {
        // update fxs
        _Items[SLOTTYPE::LEFT_HAND_SLOT].enableAdvantageFX(itemInstance);
        _Items[SLOTTYPE::LEFT_HAND_SLOT].setTrailSize(0);
        //_Items[SLOTTYPE::LEFT_HAND_SLOT].setTrailSize(2 * (uint)
        //visualA.PropertySubData.LTrail);
      }
    } else {
    ...
    if (_Items[SLOTTYPE::HEAD_SLOT].Sheet == 0 ||
        _Items[SLOTTYPE::HEAD_SLOT].Sheet->Family != ITEMFAMILY::ARMOR) {
      CItemSheet *faceItem =
          getItem(_PlayerSheet->GenderInfos[_Gender], SLOTTYPE::FACE_SLOT);
      if (faceItem) {
        string sFaceName;
        ...
        applyColorSlot(_Face, skin(), 0, visualA.PropertySubData.HatColor,
                       0); // Set a default ruflaket color.
      } else
        nlwarning("PL::updateVPVpa:%d: Face Item '%s' does not exist.", _Slot,
                  _PlayerSheet->GenderInfos[_Gender]
                      .Items[SLOTTYPE::FACE_SLOT]
                      .c_str());
    } else {
      // There is a helmet !
      if (!_Face.Loading.empty())
    ...
    string propName;
    propName =
        toString("SERVER:Entities:E%d:P%d", _Slot, CLFECOMMON::PROPERTY_VPB);
    vB = NLGUI::CDBManager::getInstance()->getDbProp(propName)->getValue64();
    propName =
        toString("SERVER:Entities:E%d:P%d", _Slot, CLFECOMMON::PROPERTY_VPC);
...
//-----------------------------------------------
void CPlayerCL::updateVisualPropertyVpb(
    const NLMISC::TGameCycle & /* gameCycle */, const sint64 &prop) {
  // Get the property.
    ...
    if (!itemInstance.empty()) {
      ...
      if (_CurrentBehaviour.Behaviour != MBEHAV::EXTRACTING)
        _Items[SLOTTYPE::RIGHT_HAND_SLOT].setTrailSize(
            visualB.PropertySubData.RTrail);
    }
  }
  ...
  if (_Items[SLOTTYPE::LEFT_HAND_SLOT].Sheet) {
    ...
    if (!itemInstance.empty()) {
      ...
      _Items[SLOTTYPE::LEFT_HAND_SLOT].setTrailSize(
          2 * (uint)visualB.PropertySubData.LTrail);
    }
  }

  if (skeleton()) {
    // HANDS
    equip(SLOTTYPE::HANDS_SLOT, visualB.PropertySubData.HandsModel,
          visualB.PropertySubData.HandsColor);
    // FEET
    equip(SLOTTYPE::FEET_SLOT, visualB.PropertySubData.FeetModel,
          visualB.PropertySubData.FeetColor);
  } else
...
//-----------------------------------------------
void CPlayerCL::updateVisualPropertyVpc(
    const NLMISC::TGameCycle & /* gameCycle */, const sint64 &prop) {
  if (skeleton()) {
    // Get the property.
    ...
    // must recreate the face asynchronously (because of color change / makeup
    // change)
    ...
    if (!inst.empty()) {
      ...
      applyColorSlot(_Face, _Face.ACSkin, _Face.ACUser, _Face.ACHair,
                     visualC.PropertySubData.EyesColor);
      ...
      inst.setBlendShapeFactor(baseName + toString(0),
                               (float)(visualC.PropertySubData.MorphTarget1) /
                                       7.f * (MTmax - MTmin) +
                                   MTmin,
                               true);
      ...
      inst.setBlendShapeFactor(baseName + toString(1),
                               (float)(visualC.PropertySubData.MorphTarget2) /
                                       7.f * (MTmax - MTmin) +
                                   MTmin,
                               true);
      ...
      inst.setBlendShapeFactor(baseName + toString(2),
                               (float)(visualC.PropertySubData.MorphTarget3) /
                                       7.f * (MTmax - MTmin) +
                                   MTmin,
                               true);
      ...
      inst.setBlendShapeFactor(baseName + toString(3),
                               (float)(visualC.PropertySubData.MorphTarget4) /
                                       7.f * (MTmax - MTmin) +
                                   MTmin,
                               true);
      ...
      inst.setBlendShapeFactor(baseName + toString(4),
                               (float)(visualC.PropertySubData.MorphTarget5) /
                                       7.f * (MTmax - MTmin) +
                                   MTmin,
                               true);
      ...
      inst.setBlendShapeFactor(baseName + toString(5),
                               (float)(visualC.PropertySubData.MorphTarget6) /
                                       7.f * (MTmax - MTmin) +
                                   MTmin,
                               true);
      ...
      inst.setBlendShapeFactor(baseName + toString(6),
                               (float)(visualC.PropertySubData.MorphTarget7) /
                                       7.f * (MTmax - MTmin) +
                                   MTmin,
                               true);
      ...
      inst.setBlendShapeFactor(baseName + toString(7),
                               (float)(visualC.PropertySubData.MorphTarget8) /
                                       7.f * (MTmax - MTmin) +
                                   MTmin,
                               true);
    }
    ...
    float characterHeight =
        (float)((sint8)(visualC.PropertySubData.CharacterHeight) - 7) / 7.f;
    float torsoWidth =
        (float)((sint8)(visualC.PropertySubData.TorsoWidth) - 7) / 7.f;
    float armsWidth =
        (float)((sint8)(visualC.PropertySubData.ArmsWidth) - 7) / 7.f;
    float legsWidth =
        (float)((sint8)(visualC.PropertySubData.LegsWidth) - 7) / 7.f;
    float breastSize =
        (float)((sint8)(visualC.PropertySubData.BreastSize) - 7) / 7.f;
    float heightScale, baseHeightScale;
    //	TODO : manage breast size
    GabaritSet.applyGabarit(*skeleton(), _Gender, people(), characterHeight,
                            torsoWidth, armsWidth, legsWidth, breastSize,
                            &heightScale);
...
//-----------------------------------------------
//-----------------------------------------------
void CPlayerCL::updateVisualPropertyPvpMode(const NLMISC::TGameCycle &gameCycle,
                                            const sint64 &prop) {
  ...
  if (isTarget()) {
    ...
    CCDBNodeLeaf *pDB = NLGUI::CDBManager::getInstance()->getDbProp(
        "UI:VARIABLES:USER:TRACK_TARGET_PVP_CHANGE_MODE");
    if (pDB) {
...
//-----------------------------------------------
void CPlayerCL::setPeople(EGSPD::CPeople::TPeople /* people */) {} // people //
...
// *********************************************************************************************
const char *CPlayerCL::getBoneNameFromBodyPart(BODY::TBodyPart part,
                                               BODY::TSide side) const {
  if (!_PlayerSheet)
    return CCharacterCL::getBoneNameFromBodyPart(part, side);
...
float CPlayerCL::getScaleRef() const {
  ...
  if (fyrosRefScale == 0)
    return 1.f;
  return _CustomScalePos *
         (GabaritSet.getRefHeightScale(_Gender, people()) / fyrosRefScale);
}
...
// ***************************************************************************
void CPlayerCL::doSetVisualSelectionBlink(bool bOnOff,
