...
void CGraph::renderGraph() {
  ...
  _Driver->setMatrixMode2D(
      CFrustum(0.0f, ScreenWidth, 0.0f, ScreenHeight, -1.0f, 1.0f, false));
  ...
  float pos = X + Width - 1;
  uint i = 0;
  for (deque<float>::reverse_iterator it = Values.rbegin(); it != Values.rend();
       it++) {
  ...
  CRGBA frontCol(min(BackColor.R * 2, 255), min(BackColor.G * 2, 255),
                 min(BackColor.B * 2, 255), min(BackColor.A * 2, 255));
  ...
  if (TextContext != NULL) {
    ...
    TextContext->printfAt((X + Width + 2) / ScreenWidth, peakval / ScreenHeight,
                          "%.2f", Peak);

    if (DisplayAverageValue) {
      // don't display the average value if they are superposed
      if (avrval + 5 < peakval - 5 || avrval - 5 > peakval + 5) {
        TextContext->printfAt((X + Width + 2) / ScreenWidth,
                              avrval / ScreenHeight, "%.2f", average);
      }
    }

    // Display name
    TextContext->setHotSpot(UTextContext::TopLeft);
    TextContext->printfAt((X + 1) / ScreenWidth,
                          (Y + Height - 1) / ScreenHeight, Name.c_str());
  }
}
