...
//=======================================================
void CSeasonFXSheet::build(const NLGEORGES::UFormElm &item,
  ...
  ok &= item.getValueByName(DontRotateAroundLocalZ,
                            (prefix + "DontRotateAroundLocalZ").c_str());
  ...
  // Anti Crash
  if (CycleDuration <= 0) {
    nlwarning("FX: CPlantSheet '%s' has a season with a CycleDuration<=0: %f "
              "!!!!! setting it to 0.1.",
  ...
  for (uint k = 0; k < 4; ++k) {
    ok &= item.getValueByName(
        UserParamsMin[k],
        (prefix + NLMISC::toString("UserParam%dMin", (int)k)).c_str());
    ok &= item.getValueByName(
        UserParamsMax[k],
        (prefix + NLMISC::toString("UserParam%dMax", (int)k)).c_str());
  }
...
void CSeasonFXSheet::serial(NLMISC::IStream &f) {
  ...
  // Anti Crash. you may recompute the plant.packed_sheets if those assert
  // trigger.
  nlassert(isValidDouble(CycleDuration));
  nlassert(isValidDouble(StartHourMin));
  nlassert(isValidDouble(StartHourMax));
  ...
  nlassert(isValidDouble(ScaleMin.x) && isValidDouble(ScaleMin.y) &&
           isValidDouble(ScaleMin.z));
  nlassert(isValidDouble(ScaleMax.x) && isValidDouble(ScaleMax.y) &&
           isValidDouble(ScaleMax.z));
