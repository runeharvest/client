...
public:
  ...
  struct CArmor {
    ...
    void serial(NLMISC::IStream &f) { f.serialEnum(ArmorType); }
  };
  struct CMeleeWeapon {
  ...
  struct CShield {
    ...
    void serial(NLMISC::IStream &f) { f.serialEnum(ShieldType); }
  };
  struct CTool {
  ...
  struct CPet {
    ...
    void serial(NLMISC::IStream &f) { f.serial(Slot); }
  };

  struct CTeleport {
    TELEPORT_TYPES::TTeleportType Type;

    void serial(NLMISC::IStream &f) { f.serialEnum(Type); }
  };

  struct CScroll {
    std::string Texture;

    void serial(NLMISC::IStream &f) { f.serial(Texture); }
  };
...
public:
  ...
  /// Equipment slot. This is a bitField matching each bit to
  /// SLOTTYPE::TSlotType
  uint64 SlotBF;
  /// texture variant.
  uint32 MapVariant;
  ...
  std::string getShapeFemale() const {
    return ClientSheetsStrings.get(IdShapeFemale);
  }
  std::string getIconBack() const {
    return ClientSheetsStrings.get(IdIconBack);
  }
  std::string getIconMain() const {
    return ClientSheetsStrings.get(IdIconMain);
  }
  std::string getIconOver() const {
    return ClientSheetsStrings.get(IdIconOver);
  }
  std::string getIconOver2() const {
    return ClientSheetsStrings.get(IdIconOver2);
  }
  std::string getIconText() const {
    return ClientSheetsStrings.get(IdIconText);
  }
  ...
  bool hasSlot(SLOTTYPE::TSlotType e) const {
    return (SlotBF & (SINT64_CONSTANT(1) << e)) != 0;
  }

  /// true if the item is faberisable according to family
  bool isFaberisable() const;

  // Get the skill required by the item depending on its type (or
  // unknown_skill_type if no specific skill is required)
  ...
  bool canBuildItemPart(RM_FABER_TYPE::TRMFType e,
  ...
  bool hasCharacRequirement(uint itemLevel,
  ...
  // MP only. return translated text of all item part this MP can build. empty,
  // if can't build anything
