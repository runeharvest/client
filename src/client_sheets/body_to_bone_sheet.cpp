...
static void build(NLMISC::TSStringId &result, const NLGEORGES::UFormElm &item,
...
// *********************************************************************************************
void CBodyToBoneSheet::build(const NLGEORGES::UFormElm &item,
...
// *********************************************************************************************
const char *CBodyToBoneSheet::getBoneName(BODY::TBodyPart part,
                                          BODY::TSide side) const {
  BODY::TBodyPart hominPart = BODY::getMatchingHominBodyPart(part);
  switch (hominPart) {
  case BODY::HHead:
    return ClientSheetsStrings.get(Head);
  case BODY::HChest:
    return ClientSheetsStrings.get(Chest);
  case BODY::HArms:
    return ClientSheetsStrings.get(side == BODY::Left ? LeftArm : RightArm);
  case BODY::HHands:
    return ClientSheetsStrings.get(side == BODY::Left ? LeftHand : RightHand);
  case BODY::HLegs:
    return ClientSheetsStrings.get(side == BODY::Left ? LeftLeg : RightLeg);
  case BODY::HFeet:
    return ClientSheetsStrings.get(side == BODY::Left ? LeftFoot : RightFoot);
  default:
    break;
  }
  return NULL;
