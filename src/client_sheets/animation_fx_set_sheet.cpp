...
// ****************************************************************************************************
void CAnimationFXSetSheet::buildWithPrefix(const NLGEORGES::UFormElm &item,
  ...
  for (uint k = 0; k < MaxNumFX; ++k) {
    ...
    fs.build(item,
             NLMISC::toString((prefix + "FX%d.").c_str(), (int)k).c_str());
    if (!fs.PSName.empty()) {
      FX.push_back(fs);
    }
    item.getValueByName(
        CanReplaceStickMode[k],
        NLMISC::toString((prefix + "CanReplaceStickMode%d").c_str(), (int)k)
            .c_str());
    item.getValueByName(
        CanReplaceStickBone[k],
        NLMISC::toString((prefix + "CanReplaceStickBone%d").c_str(), (int)k)
            .c_str());
  }
}
