...
public:
  ...
  NLMISC::CVector AttackFXOffset; // offset applied to the attack fx (some fxs
                                  // are used for several kinds of guns, and
                                  // need to be offseted differently)
  NLMISC::CVector
      AttackFXRot; // rotation around y axe for attack fxs (in degrees)
  float ImpactFXDelay;
  // ctor
  CItemFXSheet();
...
private:
  ...
  NLMISC::TSStringId
      _Trail; // a trail that is displayed when the item is used to attack
  NLMISC::TSStringId
      _AdvantageFX; // fx to played on some items when the player master it
