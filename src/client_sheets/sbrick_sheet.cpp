...
void CSBrickSheet::build(const NLGEORGES::UFormElm &root) {
  ...
  /*
     ...
     sheetName.find(".sbrick")-2; BrickFamily = BRICK_FAMILIES::toSBrickFamily (
     NLMISC::toUpperAscii(sheetName.substr(0,end)) );
                  if(BrickFamily==BRICK_FAMILIES::Unknown)
                          nlwarning("Unknown Family for SBrick: %s",
     sheetName.c_str());
          }
  */
  ...
  if (isFaber() && (isMandatory() || isRoot())) {
    ...
    // Get Skill Filters
    TRANSLATE_ENUM(FaberPlan.ToolType, TOOL_TYPE::Unknown,
                   TOOL_TYPE::toToolType, "faber.Tool type");
    ...
    for (uint k = 0; k < MAX_FABER_REQ_MP; ++k) {
      sint32 mpQuantity = 0;
      root.getValueByName(mpQuantity,
                          toString("faber.Create.Quantity %d", k + 1).c_str());
      ...
      if (mpQuantity > 0) {
        ...
        TRANSLATE_ENUM_NODB(mpVal.FaberTypeFilter, RM_FABER_TYPE::Unknown,
    ...
    for (uint k = 0; k < MAX_FABER_REQ_MP; ++k) {
      ...
      root.getValueByName(
          mpQuantity,
          toString("faber.Create.Quantity formula %d", k + 1).c_str());
      // if the req quantity is not 0
      if (mpQuantity > 0) {
        ...
        // No error if unknown: filter not used (all MPs match)
        TRANSLATE_VAL(val,
                      toString("faber.Create.MP formula %d", k + 1).c_str());
  ...
  // Read Action Nature
  string val;
  TRANSLATE_ENUM(ActionNature, ACTNATURE::UNKNOWN, ACTNATURE::toActionNature,
                 "Basics.Action Nature");

  // Read CivRestriction
  TRANSLATE_ENUM(CivRestriction, EGSPD::CPeople::Common,
                 EGSPD::CPeople::fromString, "Basics.CivRestriction");
  ...
  for (i = 0; i < Properties.size(); i++) {
    ...
    for (uint j = 0; j < nOpeningProp; j++) {
      ...
      if (text.compare(0, prop.size(), prop) == 0 ||
          (j == 0 &&
           text.compare(0, neededBrickFlag.size(), neededBrickFlag) == 0)) {
  ...
  for (i = 0; i < Properties.size(); i++) {
    ...
    if (text.compare(0, dmgTypeProp.size(), dmgTypeProp) == 0) {
      ...
      if (dt != DMGTYPE::UNDEFINED) {
        ...
        RESISTANCE_TYPE::TResistanceType rt =
    ...
    if (text.compare(0, effectFamProp.size(), effectFamProp) == 0) {
      ...
      strRemoveChar(efStr, ' ');
      EFFECT_FAMILIES::TEffectFamily ef =
...
bool CSBrickSheet::isForageProspection() const {
  return BRICK_FAMILIES::brickType(BrickFamily) ==
...
bool CSBrickSheet::mustDisplayLevel() const {
  ...
  // NB: Yoyo Hack. special interface with indexInFamily==63 means "want to
  // display the level"
  return !(isMandatory() || isRoot() ||
           (BrickFamily >= BRICK_FAMILIES::BeginInterface &&
            BrickFamily <= BRICK_FAMILIES::EndInterface &&
            IndexInFamily != 63) ||
           Level == 0);
}
