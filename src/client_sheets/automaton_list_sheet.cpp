...
CAutomatonStateSheet::CAutomatonStateSheet() {
  ...
  ModeTransition.resize(MBEHAV::NUMBER_OF_MODES,
...
//-----------------------------------------------
void CAutomatonStateSheet::build(const NLGEORGES::UFormElm &item,
                                 const string &baseKeyStr) {
  string stateName;
  ...
  item.getValueByName(AdjustOri,
                      string(baseKeyStr + ".adjust orientation").c_str());
  // Max Animation Duration if there is a destination.
  item.getValueByName(MaxAnimDuration,
                      string(baseKeyStr + ".max duration").c_str());
  ...
  OnMinSpeed.NextStateKey =
      CAnimationStateSheet::getAnimationStateId(stateName);
  // Get state parameter 'on max speed'.
  item.getValueByName(stateName, string(baseKeyStr + ".on max speed").c_str());
  OnMaxSpeed.NextStateKey =
      CAnimationStateSheet::getAnimationStateId(stateName);
  // ON BAD HEADING
  item.getValueByName(stateName,
                      string(baseKeyStr + ".on bad heading forward").c_str());
  OnBadHeadingForward = CAnimationStateSheet::getAnimationStateId(stateName);
  item.getValueByName(stateName,
                      string(baseKeyStr + ".on bad heading backward").c_str());
  OnBadHeadingBackward = CAnimationStateSheet::getAnimationStateId(stateName);
  item.getValueByName(stateName,
                      string(baseKeyStr + ".on bad heading left").c_str());
  OnBadHeadingLeft = CAnimationStateSheet::getAnimationStateId(stateName);
  item.getValueByName(stateName,
                      string(baseKeyStr + ".on bad heading right").c_str());
  OnBadHeadingRight = CAnimationStateSheet::getAnimationStateId(stateName);
  item.getValueByName(BadHeadingMin,
                      string(baseKeyStr + ".bad heading min").c_str());
  item.getValueByName(BadHeadingMax,
                      string(baseKeyStr + ".bad heading max").c_str());

  // ON IMPACT
  item.getValueByName(BreakableOnImpact,
                      string(baseKeyStr + ".breakable on impact").c_str());
  // AT DESTINATION
  item.getValueByName(BrkAtDest,
                      string(baseKeyStr + ".brk at destination").c_str());
  // X,Y,Z FACTOR
  ...
  for (uint mode = 0; mode < MBEHAV::NUMBER_OF_MODES; ++mode) {
    ...
    if (animset != "unknown_mode") {
      ...
      item.getValueByName(
          resultTransition,
          (baseKeyStr + "." + animset + " mode transition").c_str());
      if (resultTransition.empty() == false) {
        ModeConnection[mode] = true;
        ModeTransition[mode] =
            CAnimationStateSheet::getAnimationStateId(resultTransition);
      }
    }
  }
  ...
  BrkOnBigBend = ((OnBigBendLeft != CAnimationStateSheet::UnknownState) ||
                  (OnBigBendRight != CAnimationStateSheet::UnknownState));
  // Min/Max Speed
  OnMinSpeed.Breakable =
      (OnMinSpeed.NextStateKey != CAnimationStateSheet::UnknownState);
  OnMaxSpeed.Breakable =
      (OnMaxSpeed.NextStateKey != CAnimationStateSheet::UnknownState);
...
//-----------------------------------------------
// getModeConnection:
// Return if there is a connection with the mode in parameter from the current
// automaton and filled the transition to use.
//-----------------------------------------------
bool CAutomatonStateSheet::getModeConnection(MBEHAV::EMode mode,
                                             TAnimStateKey &transition) const {
...
void CAutomatonSheet::build(const NLGEORGES::UFormElm &item) {
  ...
  // The state already exist -> Not Insert
    ...
    nlwarning(
        "The state '%s' already exists.",
        CAnimationStateSheet::getAnimationStateName(mas.MoveState).c_str());
  // OTHER
...
CAutomatonSheet::state(const TAnimStateKey &key) const {
  // Search the key.
...
//-----------------------------------------------
//-----------------------------------------------
void CAutomatonSheet::buildStateSet(const NLGEORGES::UFormElm &item,
...
CAutomatonListSheet::~CAutomatonListSheet() {
  std::map<std::string, CAutomatonSheet *>::iterator it = Automatons.begin(),
                                                     itEnd = Automatons.end();
  while (it != itEnd)
    delete (*it++).second;
  Automatons.clear();
}
    ...
    for (uint i = 0; i < size; ++i) {
      ...
      if (list->getArrayValue(automatonType, i)) {
        ...
        for (uint mode = 0; mode < MBEHAV::NUMBER_OF_MODES; ++mode) {
          // Get the Mode Name
          string modeName =
              NLMISC::toLowerAscii(MBEHAV::modeToString((MBEHAV::EMode)mode));
          // Compute the automaton name
          string filename = NLMISC::toLowerAscii(automatonType) + "_" +
                            modeName + ".automaton";
...
//-----------------------------------------------
void CAutomatonListSheet::readDfnStringArray(std::vector<std::string> &states,
