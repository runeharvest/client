...
public:
        ...
        std::string       TitleTextID; // should be converted with CI18N to get
the actual title in ucstring public: void build(const NLGEORGES::UFormElm
&item); void serial(NLMISC::IStream &f);
};*/
...
public:
  ...
  // New Sky system : gives name of the sky sheet per season. If present, this
  // bypass the SkyDay & SkyNight fields
  ...
  // Name of IGs for canopy, depending on season. If the string is empty, then
  // the value in BackgroundIGName is used instead
  ...
  public:
    ...
    float LoadDist;
    float UnloadDist;
    bool EnableRuins; // Allow the display of shape of ruins (else building
                      // shape displayed through bot objects)
