...
static const uint NumUsedVisualSlots =
    sizeof(UsedVisualSlots) / sizeof(UsedVisualSlots[0]);
...
//===============================================================
void CGenderInfo::build(const NLGEORGES::UFormElm &item,
                        const std::string &prefix) {
  // Get equipment items
  // get slots
  for (uint k = 0; k < NumUsedVisualSlots; ++k) {
    if (!item.getValueByName(Items[UsedVisualSlots[k].Slot],
                             (prefix + UsedVisualSlots[k].Suffix).c_str())) {
      nlwarning("Can't get %s from race_stats sheet",
                UsedVisualSlots[k].Suffix);
    }
  }
  // Get skeleton
  ...
  if (!item.getValueByName(AnimSetBaseName,
                           (prefix + "AnimSetBaseName").c_str())) {
    nlwarning("Can't get AnimSetBaseName");
  }

  // Load Lod character name
  if (!item.getValueByName(LodCharacterName,
                           (prefix + "LodCharacterName").c_str()))
    nlwarning("CGenderInfo::build: Key 'LodCharacterName' not found.");

  // Load Lod character apparition distance
  if (!item.getValueByName(LodCharacterDistance,
                           (prefix + "LodCharacterDistance").c_str()))
  ...
  if (!item.getValueByName(CharacterScalePos,
                           (prefix + "CharacterScalePos").c_str()))
    nlwarning("CGenderInfo::build: Key 'CharacterScalePos' not found.");
  for (uint32 i = 0; i < 8; ++i) {
    if (!item.getValueByName(
            BlendShapeMin[i],
            (prefix + "MorphTargetMin" + NLMISC::toString(i)).c_str()))
      nlwarning("CGenderInfo::build: Key 'MorphTargetMin%d' not found.", i);
    if (!item.getValueByName(
            BlendShapeMax[i],
            (prefix + "MorphTargetMax" + NLMISC::toString(i)).c_str()))
...
CRaceStatsSheet::CRaceStatsSheet() {
  std::fill(CharacStartValue,
            CharacStartValue + CHARACTERISTICS::NUM_CHARACTERISTICS, 0);
  // Initialize the type.
...
void CRaceStatsSheet::build(const NLGEORGES::UFormElm &item) {
  ...
  for (uint k = 0; k < NumAttackLists; ++k) {
    ...
    if (item.getValueByName(
            attackListName,
            NLMISC::toString("attack_list%d", (int)k).c_str()) &&
        !attackListName.empty()) {
...
//===============================================================
void CRaceStatsSheet::buildGroundFXs(const NLGEORGES::UFormElm &item,
  ...
  if (item.getNodeByName(&elm, name) && elm) {
    ...
    if (elm->isArray()) {
      ...
      if (elm->getArraySize(groundFXArraySize)) {
        ...
        debug(
            NLMISC::toString("'%s' cannot get the array size.", name.c_str()));
    } else
