 ...
 */
template <class T>
static void GetVillageFormValue(const NLGEORGES::UFormElm &item, T &destItem,
                                const char *name) {
  nlassert(name);
  if (!item.getValueByName(destItem, name))
    nlwarning("VillageSheet : can't get %s value.", name);
}
...
CVillageSheet::CVillageSheet()
    : Altitude(0), ForceLoadDist(0), LoadDist(0), UnloadDist(0), CenterX(0),
      CenterY(0), Width(0), Height(0), Rotation(0) {}
