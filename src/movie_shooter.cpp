...
// ***************************************************************************
CMovieShooter::~CMovieShooter() { clearMemory(); }
...
bool CMovieShooter::init(uint maxMemory) {
  ...
  // Must fill memory with 0, just to ensure the system allocate the virtual
  // pages.
...
bool CMovieShooter::addFrame(double time, UDriver *pDriver) {
  ...
  if (bitmap.getPixels().empty())
    return false;
  return addFrame(time, (CRGBA *)(&bitmap.getPixels()[0]), bitmap.getWidth(),
                  bitmap.getHeight());
}
...
bool CMovieShooter::addFrame(double time, CRGBA *pImage, uint w, uint h) {
  ...
  while (_FirstFrame != NULL && (uint8 *)newFrame <= (uint8 *)_FirstFrame &&
         ((uint8 *)newFrame + totalSize) > (uint8 *)_FirstFrame) {
  ...
  CRGBA *src = pImage;
  for (uint y = h; y > 0; y--) {
    // Precache all the line. NB: correct for 800, since 800*4==3200, ie under
    // the 4K cache size.
...
// ***************************************************************************
uint CMovieShooter::getNumFrames() { return _NumFrames; }

// ***************************************************************************
void CMovieShooter::saveMovie(UDriver *drv, UTextContext *textContext,
                              const char *path, float framePeriod,
                              bool allowLinearInterp,
                              const char *savePrefix /*= "shot_"*/) {
  ...
  while (n >= 2) {
    ...
    if (n >= 2) {
      ...
      double interpValue =
          (time - precFrame->Time) / (nextFrame->Time - precFrame->Time);

      // interpolation is possible only if 2 frames are of same size, and if
      // wanted
      bool interpOk;
      uint w, h;
      w = precFrame->Width;
      h = precFrame->Height;
      interpOk = allowLinearInterp &&
                 (w == nextFrame->Width && h == nextFrame->Height);
      ...
      drv->fillBuffer(bmp0);
      textContext->printfAt(0.05f, 0.80f, "Movie Saving: %d%%",
                            100 - n * 100 / getNumFrames());
      drv->swapBuffers();
    }
...
void CMovieShooter::replayMovie(UDriver *drv, UTextContext *textContext) {
  ...
  while (frame) {
    ...
    drv->fillBuffer(bmp0);
    textContext->printfAt(0.05f, 0.80f, "Movie Replay: %d%%",
                          100 - n * 100 / getNumFrames());
