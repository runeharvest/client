...
// Constructor.
//-----------------------------------------------
CProgressBar::CProgressBar(uint id, float x, float y, float x_pixel,
                           float y_pixel, float w, float h, float w_pixel,
                           float h_pixel, uint range)
...
void CProgressBar::display() {
  ...
  if (_Smooth) {
    uint32 nbUnit =
        (uint32)(ryzomGetLocalTime() - _LastUpdateSmooth) / _SmoothFillRate;
    if (nbUnit > 0) {
      _LastUpdateSmooth = ryzomGetLocalTime();
  ...
  if (utexture) {
    Driver->drawBitmap(_X_Display, _Y_Display, _W_Display, _H_Display,
                       *utexture, true, _BackgroundColor);
  }

  // calculate _W_Display of the progress bar
  const float wBar =
      _W_Display * (static_cast<float>(_TempPos) / static_cast<float>(_Range));
  ...
  Driver->drawBitmap(_X_Display, _Y_Display, _W_Display, _H_Display, *utexture,
