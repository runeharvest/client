...
// Constructor
//-----------------------------------------------
CControlList::CControlList(uint id, float x, float y, float x_pixel,
...
// Destructor
//-----------------------------------------------
CControlList::~CControlList() { clear(); } // Destructor //
...
void CControlList::click(float x, float y, bool &taken) {
  ...
  if (x >= _X_Display && x <= (_X_Display + _W_Display - w) &&
...
// scrollH :
//-----------------------------------------------
void CControlList::scrollH(sint32 scroll) {} // scrollH //
...
void CControlList::display() {
  ...
  /*	UTextureFile *utexture = CInterfMngr::getTexture(1);
     ...
     _H_Display, *utexture, true);
          ...
          for (TListControl::iterator itItem = _Items.begin() ; itItem !=
     _Items.end() ; ++itItem)
          {
                  (*itItem)->show( false );
