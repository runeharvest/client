...
public:
  ...
  /// constructor
  CScrollBar(uint id, float x, float y, float x_pixel, float y_pixel, float w,
             float h, float w_pixel, float h_pixel, bool vertical = true,
             CScrollableControl *ctrl = NULL);
  ...
  /**
   * get the display size of the scroll bar (width if Vertical, Height if
   * horizontal)
   */
  float size() const;
...
private:
  ...
  /// the height (if vertical, width if horizontal) of the button, in proportion
  /// of th window (0-1)
