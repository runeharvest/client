...
protected:
  ...
  /// indicate if the list autowrap it's elements when they are longer than
  /// display size
  bool _AutoWrap;
   ...
   * true), this iterator is always equal to the last item in the list
   */
...
public:
  /// Constructor
  CList(uint id);
  CList(uint id, float x, float y, float x_pixel, float y_pixel, float w,
        float h, float w_pixel, float h_pixel, const CPen &pen);
  CList(uint id, float x, float y, float x_pixel, float y_pixel, float w,
        float h, float w_pixel, float h_pixel, uint32 fontSize, CRGBA color,
        bool shadow);
