...
// Constructor.
//-----------------------------------------------
CCapture::CCapture(uint id) : CControl(id) { init(0); } // CCapture //

//-----------------------------------------------
// CCapture :
// Constructor.
//-----------------------------------------------
CCapture::CCapture(uint id, float x, float y, float x_pixel, float y_pixel,
                   float w, float h, float w_pixel, float h_pixel, uint numFunc,
                   const CPen &pen)
...
void CCapture::display() {
  ...
  const float strWidth =
      TextContext->getStringInfo(index).StringWidth / Driver->getWindowWidth();

  // display string starting by the first character if !insert or display to
  // ensure the input prompt is visible in insert mode calculate the x
  // coordinate where to start displaying
