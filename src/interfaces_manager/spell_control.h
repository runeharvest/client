...
public:
  /// Constructor
  explicit CSpellControl(uint id, float x, float y, float x_pixel,
                         float y_pixel, float w, float h, float w_pixel,
                         float h_pixel, uint16 spacing, const CPen &pen1,
                         const CPen &pen2);
  ...
  explicit CSpellControl(uint id, float x, float y, float x_pixel,
                         ...
                         const CRGBA &color1, bool shadow1, uint32 fontSize2,
                         const CRGBA &color2, bool shadow2);
   ...
   */
  void setButtonColorDisable(const CRGBA &color) {
    _Button.colorDisable(color);
  }
   ...
   * \param CRGBA& color new color when the button is selected
   */
  void setButtonBGModeOn(const CButtonBase::TBG &mode) {
    _Button.bgModeOn(mode);
  }
  /**
   * set the color when 'off'
   * \param CRGBA& color new color when the button is unselected
   */
  void setButtonBGModeOff(const CButtonBase::TBG &mode) {
    _Button.bgModeOff(mode);
  }
  /**
   * set the color when 'disabled'
   * \param CRGBA& color new color when the button is disabled
   */
  void setButtonBGModeDisable(const CButtonBase::TBG &mode) {
    _Button.bgModeDisable(mode);
  }
  //@}
...
private:
  ...
  inline void init(uint16 spacing, const CPen &spellPen,
