...
public:
  ...
  /// Constructor
  CBrickControl(uint id, float x, float y, float x_pixel, float y_pixel,
                float w, float h, float w_pixel, float h_pixel,
                uint numFuncOn = 0, uint numFuncR = 0);
   ...
   * \return CBrickClient* the brick or NULL if no brick is associated with this
   * control
   */
