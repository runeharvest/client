...
/**
 * Class defining list with multiple columns, the number of columns is a param
 * of the constructor (default = 2) \author David Fleury \author Nevrax France
 * \date 2001
 */
...
public:
  ...
  /// Constructor
  CMultiList(uint id, float x, float y, float x_pixel, float y_pixel, float w,
             float h, float w_pixel, float h_pixel, const CPen &pen,
             uint16 nbCol = 2);
  CMultiList(uint id, float x, float y, float x_pixel, float y_pixel, float w,
             float h, float w_pixel, float h_pixel, uint32 fontSize,
             CRGBA color, bool shadow, uint16 nbCol = 2);
  ...
  /**
   * set the max number of memorized lines fo the list,  0 = no limit, default
   * (at creation) is 150 \param the new limit for the number of string in the
   * list
   */
  ...
  /// scroll horizontaly by 'scroll' units in either direction (right if scroll
  /// >0 and left is scroll<0 for example) (implementation dependent)
...
protected:
  ...
  /// indicate if the list is in auto-scroll mode or not (true : the text will
  /// allways scroll to show the last line)
  bool _AutoScroll;

  /// indicate if the list autowarp it's elements when they are longer than
  /// display size
  bool _AutoWrap;
  ...
  /**
   * the ending position in the list for display, by default it's the last item
   * in the list (the newest one) if auto scroll mode is 'on' (_AutoScroll ==
   * true), this iterator is allways equal to the last item in the list
   */
  TItemList::reverse_iterator _EndingIterator;

  /// the relative width of all columns (compared to the total width of the
  /// control)
  // TO DO : relative and/or absolute size of each columns -> params in the osd
  // ".txt"
  ...
  /// the height of a line in pixels (0 = computed for each line (get the info
  /// StringHeight)
  uint16 _LineHeight;
};
