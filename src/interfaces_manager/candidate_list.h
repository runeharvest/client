...
public:
  ...
  CCandidateList(uint id, float x, float y, float x_pixel, float y_pixel,
...
private:
   ...
   * \param ucstring*& the variable that will receive the pointer to the name of
   * the candidate (or NULL if not found) \return bool true if a candidate was
...
private:
  /**
   * list Y coordinates of displayed candidate name, used when a user left click
   * on the control to determine if he clicked on a candidate and which one NB :
   * the second type (ucstring*) is a pointer to the candidate name in
   * _ItemsList
   */
  typedef std::list<std::pair<std::pair<float, float>, const ucstring *>>
      TPairPFloatStr;
