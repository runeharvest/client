...
private:
  ...
  /// The list of OSD (id) that are waiting destruction (they are destroyed at
  /// the beginning of the 'update' method)
  static TUint _OSDWaitingDestruction;

  /// list of the OSD (and their id) that were created since the last call to
  /// update and that are waiting insertion into _OSD and _Windows
...
public:
  ...
  static void init(const TMapFuncCtrl &funcCtrlMap, uint32 width,
                   uint32 height);
  ...
  static void runFuncCtrl(uint num, uint ctrlId, TParam param = 0) {
    ...
    if (it != _CtrlFunctions.end()) {
      ...
      nlwarning("Invalid function called - Num function: %d, Ctrl: %d", num,
                ctrlId);
    }
  }

  /**
   * Create the OSD "id", NB : the OSD is inserted into the _OSD list at the
   * beginning of the next call of 'update()' \param uint the OSD 'id' \param
   * bool true if the window is a pop-up window (default = false) \return COSD*
   * adress of the newly created OSD, or NULL if creation failed
   */
