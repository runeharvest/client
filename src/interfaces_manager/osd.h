...
/**
 * Class to manage an OSD (On Screen Display = Kind of window) for the
 * interfaces. \author Guillaume PUZIN \author Nevrax France \date 2001
 */
class COSD {
public:
  enum ECursor {
  ...
  enum TMode { none = 0, selected, resizable, movable, locked, blocked };
...
private:
  ...
  inline void init(uint id, float x, float y, float x_pixel, float y_pixel,
                   float w, float h, float w_pixel, float h_pixel,
                   float minWidth, float minHeight, bool popUp = false);
  ...
  bool testInRect(float x, float y, float rectX0, float rectY0, float rectX1,
                  float rectY1);
  ...
  void drawBorders(float bSizeW, float bSizeH, float x0, float y0, float x1,
  ...
  void testMode(float x, float y, float rectX0, float rectY0, float rectX1,
                float rectY1);
...
protected:
  ...
  /// boolean indicating if the OSD is a 'pop-up' window, in this case, a click
  /// outside the window close that window
  bool _PopUp;
  ...
  /// Variables to know the offset between the click position and the OSD
  /// borders.
  float _OffsetX;
  float _OffsetY;
  float _OffsetW;
...
public:
  ...
  COSD(uint Id, float x, float y, float x_pixel, float y_pixel, float w,
       float h, float w_pixel, float h_pixel, float minWidth, float minHeight,
       bool popUp = false);
  /// Destructor
  ~COSD();
   ...
   * \warning This method should be called after the update one to be up to
   * date.
   */
  ECursor cursor();
  ...
  /// Return the pointer of the control "id" or 0 if the control "id" doesn't
  /// exit.
