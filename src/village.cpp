...
//===================================================================================
/*virtual*/ bool
CVillage::needCompleteLoading(const NLMISC::CVector &pos) const {
...
//===================================================================================
/*virtual*/ void CVillage::forceUpdate(const NLMISC::CVector &pos,
...
//===================================================================================
bool CVillage::setupFromSheet(
    NL3D::UScene *scene, const CVillageSheet &sheet,
    CStreamableIG::TString2IG *loadedIGMap /*=NULL*/) {
  ...
  for (uint k = 0; k < sheet.IGs.size(); ++k) {
    _IG.addIG(sheet.IGs[k].IgName, sheet.IGs[k].ParentName, CVector::Null,
              CQuat::Identity);
  }
  _Scene = scene;
  _IsOutpost = false;
...
//===================================================================================
void CVillage::forceUnload() { _IG.forceUnload(); }

//===================================================================================
void CVillage::registerObserver(IIGObserver *obs) { _IG.registerObserver(obs); }

//===================================================================================
void CVillage::removeObserver(IIGObserver *obs) { _IG.removeObserver(obs); }
...
//===================================================================================
bool CVillage::enumIGs(IIGEnum *callback) { return _IG.enumIGs(callback); }
...
//===================================================================================
bool CVillage::setupOutpost(NL3D::UScene *scene,
                            const CContinentParameters::CZC &zone,
                            sint32 /* outpost */,
                            CStreamableIG::TString2IG *loadedIGMap) {
  nlassert(!_Scene); // should be built only once
  nlassert(scene);
...
//===================================================================================
void CVillage::setBuildingPosition(uint building, const NLMISC::CQuat &rot,
...
void CVillage::updateBuilding(uint buildingId, NLMISC::CSheetId newForm) {
  ...
  if (building.CurrentSheetId != newForm) {
    ...
    // If the Sheet id is unknown, just remove the old building if there was
    // one.
    if (newForm != CSheetId::Unknown) {
      ...
      const CBuildingSheet *bldSh =
          dynamic_cast<const CBuildingSheet *>(entitySheet);
      if (bldSh) {
