...
/** Manager of 'ground fxs' : fx that are displayed when character walk over
 * sand (actually sawdust), or grass. \author Nicolas Vizerie \author Nevrax
 * France \date 2003
 */
...
public:
   ...
   * \param scene			 The scene from which instance are
   * created \param maxDist           The max dist at which ground fxs are
   * generated \param maxActiveFX       The max number of fxs that are holded by
   * an entity \param maxInactiveFX     The max number of inactive FXs (shuting
   * down fxs & fx ready to use)
   */
  void init(NL3D::UScene *scene, float maxDist, uint maxNumFX,
            uint fxCacheSize);
  ...
  // reset the manager (must call init again for reuse). All handle allocated
  // from add(CEntityCL *entity) becomes invalid after the call.
  void reset();
