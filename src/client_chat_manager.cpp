...
//
//		CDynamicStringInfos *res =
//mng->getDynamicDB().getDynamicStringInfos ((uint32)Args[i]); 		if (res != NULL)
//		{
//			result += " ";
//			result += res->Str;
//		}
...
// }
//
// void CNetworkString::setString (const ucstring &staticStringId,
// CClientChatManager *mng) // OLD
//{
//	CBitMemStream bms;
...
////
////-----------------------------------------------
// uint32 CChatDynamicDatabase::add( uint32 index, ucstring& ucstr,
// vector<bool>& code ) // OLD
//{
//	nlinfo ("receive dynamic string association '%d' '%s'", index,
//ucstr.toString().c_str());
//
//	map<ucstring, uint32>::iterator itIdx = _StringToIndex.find( ucstr ); //
//OLD 	if( itIdx == _StringToIndex.end() )
//	{
//		map<uint32,CDynamicStringInfos *>::iterator itStr = _Data.find(
//index ); 		if( itStr == _Data.end() )
//		{
//			CDynamicStringInfos * dynInfosTmp = new
//CDynamicStringInfos(); 			dynInfosTmp->Index = index; 			dynInfosTmp->Associated =
//true;
//
...
//		else
//		{
//			// we already insert a fake entry usign getInfos(), now
//we set the good value
//
//			// display the index number
...
//	else
//	{
//		nlwarning("<CChatDynamicDatabase::add> the entry %s already
//exists",ucstr.toString().c_str()); 		return 0;
//	}
//
// } // add //
//
...
////
////-----------------------------------------------
// CDynamicStringInfos * CChatDynamicDatabase::getDynamicStringInfos( uint32
// index )
//{
//	if( index == 0 )
//	{
//		nldebug("<CChatDynamicDatabase::getInfos> The index 0 is not a
//valid index"); 		return NULL;
//	}
...
//		else
//		{
//			nlwarning("<CChatDynamicDatabase::getInfos> new infos
//allocation failed for string %d",index); 			return NULL;
//		}
//	}
//
// } // getDynamicStringInfos //
...
void CClientChatManager::chat(const string &strIn, bool isChatTeam) {
  ...
  if (GenericMsgHeaderMngr.pushNameToStream(msgType, bms)) {
    ...
    NetMngr.push(bms);
    // nlinfo("impulseCallBack : %s %s sent", msgType.c_str(),
    // str.toString().c_str());
  } else {
...
void CClientChatManager::tell(const string &receiverIn, const string &strIn) {
  ...
  if (GenericMsgHeaderMngr.pushNameToStream(msgType, bms)) {
    ...
    NetMngr.push(bms);
    // nlinfo("impulseCallBack : %s %s %s sent", msgType.c_str(),
    // receiver.c_str(), str.toString().c_str());
  } else {
...
void CClientChatManager::filter(uint8 filter) {
  ...
  if (GenericMsgHeaderMngr.pushNameToStream(msgType, bms)) {
    ...
    nlwarning(
        "<CClientChatManager::filter> unknown message name : STRING:FILTER");
  }
...
//
//-----------------------------------------------
void CClientChatManager::setChatMode(CChatGroup::TGroupType group,
                                     TChanID dynamicChannelId) {
  uint8 mode = group;
  // mode really changed ?
  if (mode == _ChatMode && dynamicChannelId == _ChatDynamicChannelId)
    return;
  ...
  if (group != CChatGroup::team) {
    ...
    if (GenericMsgHeaderMngr.pushNameToStream(msgType, bms)) {
      ...
      nlwarning("<CClientChatManager::setChatMode> unknown message name : "
                "STRING:CHAT_MODE");
    }
  }
...
// ***************************************************************************
void CClientChatManager::processTellString(NLMISC::CBitMemStream &bms,
                                           IChatDisplayer &chatDisplayer) {
  CChatMsg chatMsg;

  // Serial. For tell message, there is no chat mode, coz we know we are in tell
  // mode !
  ...
  string senderStr;
  bool complete = true;
  complete &= STRING_MANAGER::CStringManagerClient::instance()->getString(
      chatMsg.SenderNameId, senderStr);
  if (!complete) {
...
// ***************************************************************************
void CClientChatManager::processFarTellString(NLMISC::CBitMemStream &bms,
                                              IChatDisplayer &chatDisplayer) {
  CFarTellMsg farTellMsg;

  // Serial. For far tell message, there is no chat mode nor sender index, and
  // the sender is a string literal!
  ...
  // display
  string ucstr;
  buildTellSentence(farTellMsg.SenderName.toUtf8(), farTellMsg.Text.toUtf8(),
                    ucstr);
  chatDisplayer.displayTell(/*chatMsg.CompressedIndex, */ ucstr,
                            farTellMsg.SenderName.toUtf8());
}

// ***************************************************************************
void CClientChatManager::processChatString(NLMISC::CBitMemStream &bms,
                                           IChatDisplayer &chatDisplayer) {
  // before displaying anything, must ensure dynamic channels are up to date
  // NB: only processChatString() have to do this. Other methods cannot be in
  // dyn_chat mode
  ...
  CChatGroup::TGroupType type =
      static_cast<CChatGroup::TGroupType>(chatMsg.ChatMode);
  string senderStr;
  ...
  if (type == CChatGroup::dyn_chat) {
    ...
    sint32 dbIndex =
        ChatMngr.getDynamicChannelDbIndexFromId(chatMsg.DynChatChanID);
    // if the client database is not yet up to date, put the chat message in
    // buffer
    if (dbIndex < 0)
      complete = false;
  }
  ...
  if (!complete) {
    ...
    // nldebug("<impulseChat> Received CHAT, put in buffer : waiting
    // association");
    return;
  }
  ...
  // display
  string ucstr;
  buildChatSentence(chatMsg.CompressedIndex, senderStr,
                    chatMsg.Content.toUtf8(), type, ucstr);
  chatDisplayer.displayChat(chatMsg.CompressedIndex, ucstr,
                            chatMsg.Content.toUtf8(), type,
                            chatMsg.DynChatChanID, senderStr);
}

// ***************************************************************************
void CClientChatManager::processTellString2(NLMISC::CBitMemStream &bms,
                                            IChatDisplayer &chatDisplayer) {
  // serial
  CChatMsg2 chatMsg;
  ...
  complete &= STRING_MANAGER::CStringManagerClient::instance()->getString(
      chatMsg.SenderNameId, senderStr);
  complete &= STRING_MANAGER::CStringManagerClient::instance()->getDynString(
      chatMsg.PhraseId, rawMessage);
  if (!complete) {
    _ChatBuffer.push_back(CChatMsgNode(chatMsg, true));
    nldebug("<impulseTell> Received TELL, put in buffer : waiting association");
    return;
...
// ***************************************************************************
void CClientChatManager::processChatString2(NLMISC::CBitMemStream &bms,
                                            IChatDisplayer &chatDisplayer) {
  CChatMsg2 chatMsg;
  bms.serial(chatMsg);
  if (PermanentlyBanned)
  ...
  if (type == CChatGroup::dyn_chat) {
    nlwarning(
        "Client don't support dyn_chat with CChatMsg2 messages => '%x' aborted",
        chatMsg.PhraseId);
    return;
  }
  ...
  // display
  string ucstr;
  buildChatSentence(chatMsg.CompressedIndex, senderStr, rawMessage, type,
                    ucstr);
  chatDisplayer.displayChat(chatMsg.CompressedIndex, ucstr, rawMessage, type,
                            CEntityId::Unknown, senderStr);
}

// ***************************************************************************
void CClientChatManager::processChatStringWithNoSender(
    NLMISC::CBitMemStream &bms, CChatGroup::TGroupType type,
    IChatDisplayer &chatDisplayer) {
  ...
  bool complete =
      STRING_MANAGER::CStringManagerClient::instance()->getDynString(
          chatMsg.PhraseId, ucstr);
  if (!complete) {
    _ChatBuffer.push_back(CChatMsgNode(chatMsg, false));
    // nldebug("<impulseDynString> Received CHAT, put in buffer : waiting
    // association");
    return;
  }
  ...
  // diplay
  string senderName;
  chatDisplayer.displayChat(INVALID_DATASET_INDEX, ucstr, ucstr, type,
                            CEntityId::Unknown, senderName);
}
...
void CClientChatManager::flushBuffer(IChatDisplayer &chatDisplayer) {
  ...
  // **** Process waiting messages
  {
    ...
    for (itMsg = _ChatBuffer.begin(); itMsg != _ChatBuffer.end();) {
      CChatGroup::TGroupType type =
          static_cast<CChatGroup::TGroupType>(itMsg->ChatMode);
      ...
      if (itMsg->SenderNameId != 0)
        complete &= STRING_MANAGER::CStringManagerClient::instance()->getString(
            itMsg->SenderNameId, sender);
      if (itMsg->UsePhraseId)
        complete &=
            STRING_MANAGER::CStringManagerClient::instance()->getDynString(
                itMsg->PhraseId, content);
      else
      ...
      if (type == CChatGroup::dyn_chat) {
        // retrieve the DBIndex from the dynamic chat id
        sint32 dbIndex =
            ChatMngr.getDynamicChannelDbIndexFromId(itMsg->DynChatChanID);
        // if the client database is not yet up to date, leave the chat message
        // in buffer
        if (dbIndex < 0)
          complete = false;
      }
      ...
      if (complete) {
        ...
        if (itMsg->SenderNameId == 0) {
          ...
          if (itMsg->DisplayAsTell)
            ...
            buildChatSentence(itMsg->CompressedIndex, sender, content, type,
                              ucstr);
        }
        ...
        if (itMsg->DisplayAsTell)
          ...
          chatDisplayer.displayChat(itMsg->CompressedIndex, ucstr, content,
                                    type, itMsg->DynChatChanID, sender);
  ...
  while (dynParamSearch) {
    ...
    if (idx != string::npos) {
      ...
      case 'e': {
        ...
        const string dynStr("???");
        if (huff) {
          nldebug("<CClientChatManager::getString> receiving huffman dynamic "
                  "parameter in static string");
          //						#ifdef OLD_STRING_SYSTEM
          //							_DynamicDB.decodeString(
          //dynStr, bms ); 						#endif
        } else {
          // if( (sint32)bms.length()*8 - bms.getPosInBit() >= 32 )
          {
            ...
            //							#ifdef
            //OLD_STRING_SYSTEM 								dynStr =
            //_DynamicDB.getDynamicStringInfos(nameIndex)->Str; 							#endif
          }
        }
      ...
      case 'u':
      {
              ...
              str.replace( idx, strTmp.size() + 1 + bitCountStr.size(),
      toString(nb) );
      }
      break;
      */
      ...
      */
      ...
      default: {
        nlwarning(
            "<CClientChatManager::getString> The dynamic type %c is unknown",
            c);
      }
      }
      dynParamIdx++;
...
//
//-----------------------------------------------
bool CClientChatManager::getString(string &result, std::vector<uint64> &args,
                                   const string &ucstrbase) {
  result = ucstrbase;
  ...
  while (dynParamSearch) {
    ...
    if (idx != string::npos) {
      ...
      if (dynParamIdx >= args.size()) {
        nlwarning("Missing args for string '%s', only %d args, need arg %d",
                  ucstrbase.c_str(), args.size(), dynParamIdx);
      } else {
        ...
        case 'e': {
          //							#ifdef
          //OLD_STRING_SYSTEM 								CDynamicStringInfos *res =
          //_DynamicDB.getDynamicStringInfos ((uint32)args[dynParamIdx]); 								if
          //(!res->Associated) 							#endif
          finalString = false;
          //							#ifdef
          //OLD_STRING_SYSTEM 								rep = res->Str; 							#endif
        } break;

        // parameter is a string
        case 's': {
          nlwarning(
              "string param not implemented in the vector<uint64> decoding");
        } break;
...
// ***************************************************************************
void CClientChatManager::buildTellSentence(const string &sender,
  ...
  if (sender.empty())
    ...
    if (name.empty()) {
      ...
      CCharacterCL *entity = dynamic_cast<CCharacterCL *>(
          EntitiesMngr.getEntityByName(sender, true, true));
      bool bWoman = entity && entity->getGender() == GSGENDER::female;

      name = STRING_MANAGER::CStringManagerClient::getTitleLocalizedName(
          CEntityCL::getTitleFromName(sender), bWoman);
      {
        // Sometimes translation contains another title
        string::size_type pos = name.find('$');
        if (pos != string::npos) {
          name = STRING_MANAGER::CStringManagerClient::getTitleLocalizedName(
              CEntityCL::getTitleFromName(name), bWoman);
        }
      }
    } else {
      // Does the char have a CSR title?
      csr = CHARACTER_TITLE::isCsrTitle(CEntityCL::getTitleFromName(sender))
                ? "(CSR) "
                : "";
    }
...
// ***************************************************************************
void CClientChatManager::buildChatSentence(
    TDataSetIndex /* compressedSenderIndex */, const string &sender,
    const string &msg, CChatGroup::TGroupType type, string &result) {
  ...
  // get the category if any. Note, in some case (chat from other player), there
  // is not categories and we do not want getStringCategory to return 'SYS'
  // category.
  string finalMsg;
  ...
  if (UserEntity && senderName == UserEntity->getDisplayName()) {
    ...
    if (senderName.empty()) {
      ...
      senderName = STRING_MANAGER::CStringManagerClient::getTitleLocalizedName(
          CEntityCL::getTitleFromName(sender), bWoman);
      {
        // Sometimes translation contains another title
        ...
        if (pos != string::npos) {
          senderName =
              STRING_MANAGER::CStringManagerClient::getTitleLocalizedName(
                  CEntityCL::getTitleFromName(senderName), bWoman);
        }
      }
    }
    ...
    switch (type) {
    case CChatGroup::shout:
      result = cat + csr + senderName + " " + CI18N::get("heShout") + ": " +
               finalMsg;
      break;
    default:
      result =
          cat + csr + senderName + " " + CI18N::get("heSays") + ": " + finalMsg;
      break;
    }
  }
}
...
void CClientChatManager::initInGame() {
  ...
  for (uint i = 0; i < CChatGroup::MaxDynChanPerPlayer; i++) {
    ...
    CCDBNodeLeaf *name = NLGUI::CDBManager::getInstance()->getDbProp(
        toString("SERVER:DYN_CHAT:CHANNEL%d:NAME", i), false);
    CCDBNodeLeaf *id = NLGUI::CDBManager::getInstance()->getDbProp(
        toString("SERVER:DYN_CHAT:CHANNEL%d:ID", i), false);
    if (name && id) {
...
TChanID CClientChatManager::getDynamicChannelIdFromDbIndex(uint32 dbIndex) {
  if (dbIndex >= CChatGroup::MaxDynChanPerPlayer ||
      _DynamicChannelIdLeaf[dbIndex] == NULL)
...
sint32 CClientChatManager::getDynamicChannelDbIndexFromId(TChanID channelId) {
  for (uint i = 0; i < CChatGroup::MaxDynChanPerPlayer; i++) {
    if (_DynamicChannelIdLeaf[i] != NULL) {
      if (uint64(_DynamicChannelIdLeaf[i]->getValue64()) ==
          channelId.getRawId())
        return i;
    }
  }
...
uint32 CClientChatManager::getDynamicChannelNameFromDbIndex(uint32 dbIndex) {
  if (dbIndex >= CChatGroup::MaxDynChanPerPlayer ||
      _DynamicChannelNameLeaf[dbIndex] == NULL)
    return 0;
  else
...
// ***************************************************************************
void CClientChatManager::updateDynamicChatChannels(
    IChatDisplayer &chatDisplayer) {
  // For all channels
  for (uint i = 0; i < CChatGroup::MaxDynChanPerPlayer; i++) {
    // if the NAME is 0, force also an "empty id". because server may not
    // release Id to 0 while NAME is.
...
class CHandlerTell : public IActionHandler {
  void execute(CCtrlBase *pCaller, const string &sParams) {
    ...
    PeopleInterraction.ChatInput.Tell.displayTellMessage(
        /*dsi, */ finalMsg, receiver, prop.getRGBA());
    ...
    CInterfaceManager::getInstance()->log(
        finalMsg, CChatGroup::groupTypeToString(CChatGroup::tell));
  }
};
    ...
    if (pCGW) {
      ...
      if (pUserBut) {
        ...
        case CChatGroup::dyn_chat:
          uint32 textId =
              ChatMngr.getDynamicChannelNameFromDbIndex(dynamicChannelDbIndex);
          string title;
          ...
          if (title.empty()) {
            ...
            m = PeopleInterraction.TheUserChat.Filter.getTargetGroup();
            dynamicChannelDbIndex = PeopleInterraction.TheUserChat.Filter
...
class CHandlerSwapChatMode : public IActionHandler {
  void execute(CCtrlBase * /* pCaller */, const string &sParams) {
    ...
    CCDBNodeLeaf *node = NLGUI::CDBManager::getInstance()->getDbProp(
        "UI:SAVE:CHAT:ENTER_DONT_QUIT_CB", false);
    if (node) {
