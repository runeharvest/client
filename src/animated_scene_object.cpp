...
// Constructor for moving object, create entity (mesh, skeleton) and build
// animation on it
CAnimatedSceneObject::CAnimatedSceneObject(
    const string &ObjectName, const string &ClusterIG, const string &MeshName,
    const string &SkeletonName, const list<string> &Animations,
    const CVector &Position, const CQuat &Rotation, float Distance) {
  ...
  _Rotation = Rotation;
  _Distance =
      Distance * Distance; // square distance for optimize distance calculation
  ...
  // fill animation set
  for (list<string>::const_iterator it = Animations.begin();
       it != Animations.end(); ++it) {
    uint idAnim;
    try {
      idAnim = _AnimationSet->addAnimation((*it + string(".anim")).c_str(),
                                           (*it).c_str());
    } catch (Exception &) {
      idAnim = UAnimationSet::NotFound;
      if (idAnim == UAnimationSet::NotFound) {
        nlwarning("CAnimatedSceneObject::CAnimatedSceneObject Animation %s not "
                  "found for object %s",
                  (*it).c_str(), _ObjectName.c_str());
        continue;
      }
    }
  ...
  if (_Instance == NULL) {
    nlerror("CAnimatedSceneObject::CAnimatedSceneObject createInstance %s "
            "returned 0 -> no shape.",
            _MeshName.c_str());
  } else {
  ...
  if (!_SkeletonName.empty()) {
    ...
    if (_Skeleton == NULL) {
      nlerror("CAnimatedSceneObject::CAnimatedSceneObject createSkeleton %s "
              "returned 0 -> no skeleton.",
              _SkeletonName.c_str());
    }
...
// Constructor for moving object, it not create entity (mesh, skeleton) but
// build animation on pre-exist entity
CAnimatedSceneObject::CAnimatedSceneObject(
    const CEntityId &Id, const list<std::string> &Animations) {
  _Id = Id;
  _ObjectName = "";
  _ClusterIG = "";
  _MeshName = "";
  ...
  if (entity) {
    ...
    // fill animation set
    for (list<string>::const_iterator its = Animations.begin();
         its != Animations.end(); ++its) {
      uint idAnim;
      try {
        idAnim = _AnimationSet->addAnimation((*its + string(".anim")).c_str(),
                                             (*its).c_str());
      } catch (Exception &) {
        idAnim = UAnimationSet::NotFound;
        if (idAnim == UAnimationSet::NotFound) {
          nlwarning("CAnimatedSceneObject::CAnimatedSceneObject Animation %s "
                    "not found for entity CEntityId(%u,%f)",
                    (*its).c_str(), Id.Type, (double)(sint64)Id.Id);
          continue;
        }
      }
...
CAnimatedSceneObject::CAnimatedSceneObject(const CEntityId &Id,
  ...
  for (list<string>::const_iterator its = Animations.begin();
    ...
    try {
      ...
      if (idAnim == UAnimationSet::NotFound) {
        nlwarning("CAnimatedSceneObject::CAnimatedSceneObject Animation %s not "
                  "found for entity CEntityId(%u,%f)",
                  (*its).c_str(), Id.Type, (double)(sint64)Id.Id);
        continue:
      }
    }
...
// Start an animation, if object have more than one animation, it choose one
// randomly
void CAnimatedSceneObject::StartRandomAnimation(double time,
                                                float speedFactor) {
  if (_Transform != NULL) {
    _Transform->unfreezeHRC();
  }
  ...
  if (_Transform != NULL) {
    ...
    UTrack *Track = _AnimationSet->getAnimation(_IdAnimationPlayed)
                        ->getTrackByName("PathRotQuat");
    if (Track == NULL) {
      Track = _AnimationSet->getAnimation(_IdAnimationPlayed)
    ...
    if (entity) {
      if (!entity->flyer()) {
        _AnimTransformation.setPos(_AnimTransformation.getPos() -
                                   CVectorD(0, 0, entity->pos().z));
      }
    }
  }
}
...
void CAnimatedSceneObject::StartAnimation(string &nameAnim, double time,
                                          float speedFactor) {
  if (_Transform != NULL) {
    _Transform->unfreezeHRC();
  }
...
void CAnimatedSceneObject::updateAnimationSequence(
    const list<string> &sequence) {
  for (list<string>::const_iterator it = sequence.begin(); it != sequence.end();
       ++it) {
...
void CAnimatedSceneObject::applyObjectAnimationTimeslice(double time) {
  ...
  if (_Status.ApplyDisplacement == 1) {
    ...
    if (Track != NULL) {
      Track->interpolate((float)(time - _StartAnimationTime) *
  ...
  processLogicTemp((_OldAnimationTime - _StartAnimationTime) *
                       _PlayList->getSpeedFactor(0),
                   (time - _StartAnimationTime) * _PlayList->getSpeedFactor(0));
...
void CAnimatedSceneObject::manageObjectAnimation(double Time, CVector userPos) {
  ...
  if (_IdAnimationPlayed != -1) {
    if (Time >
        (_StartAnimationTime +
         (double)_AnimationSet->getAnimation(_IdAnimationPlayed)->getEndTime() /
             _PlayList->getSpeedFactor(0))) {
      overLoop = Time - _StartAnimationTime -
                 (double)_AnimationSet->getAnimation(_IdAnimationPlayed)
                         ->getEndTime() /
                     _PlayList->getSpeedFactor(0);
      /*			nlinfo("overLoop %f  Time %f  Start %f
         getEndTime() %f  speedFactor() %f", overLoop, Time,
                                      _StartAnimationTime,
                                      _AnimationSet->getAnimation(
         _IdAnimationPlayed )->getEndTime(), _PlayList->getSpeedFactor( 0 ));
      */
      nlassert(overLoop >= 0);

      // play through logic and movement of end of last animation before
      // animation change
  ...
  if (_IdAnimationPlayed == -1) {
    if (_Status.CodeAnim != 1) {
      ...
      float angle =
          (float)((((uint32)Time * 1000) % (uint32)(1000 * 2 * Pi)) * 0.001);
      CMatrix rot;
      rot.identity();
      rot.rotateZ(angle);
      /*
                              CQuat rot = CQuat (CVector (0, 0, 1), angle) *
         _Transform->getRotQuat(); rot.normalize();
      */
...
void CAnimatedSceneObject::applyDeltaRotationAndPosition(CQuat &rot,
                                                         CVector &trans) {
  if (_Transform != NULL) {
    // Animation Matrix
    CMatrix animation;
...
void addAnimatedSceneObject(const string &AnimatedSceneObjectName,
                            const string &ClusterIG, const string &MeshName,
                            const string &SkeletonName,
                            const list<string> &Animations,
                            const CVector &Position, const CQuat &Rotation,
                            float Distance) {
  if (AnimatedSceneObject.find(AnimatedSceneObjectName) ==
      AnimatedSceneObject.end()) {
    CAnimatedSceneObject *o = new CAnimatedSceneObject(
        AnimatedSceneObjectName, ClusterIG, MeshName, SkeletonName, Animations,
...
void addEntityClAnimatedSceneObject(const CEntityId &Id,
                                    const list<string> &Animations) {
  string test = string("CEntityId") + toString(Id.Type) + toString(Id.Id);
  if (AnimatedSceneObject.find(string("CEntityId") + toString(Id.Type) +
                               toString(Id.Id)) == AnimatedSceneObject.end()) {
    CAnimatedSceneObject *o = new CAnimatedSceneObject(Id, Animations);

    if (o != NULL) {
      AnimatedSceneObject.insert(make_pair(
          string("CEntityId") + toString(Id.Type) + toString(Id.Id), o));
    }
  }
}

void addUknowTypeSceneObject(const CEntityId &Id, NL3D::UTransform *Transform,
                             const std::list<std::string> &Animations,
                             bool displacement) {
  if (AnimatedSceneObject.find(string("CEntityId") + toString(Id.Type) +
                               toString(Id.Id)) == AnimatedSceneObject.end()) {
    CAnimatedSceneObject *o =
        new CAnimatedSceneObject(Id, Transform, Animations, displacement);
...
void manageAnimatedSceneObject(double Time) {
  for (TMapCAnimatedSceneObject::iterator it = AnimatedSceneObject.begin();
       it != AnimatedSceneObject.end(); ++it) {
...
void deleteAnimatedSceneObject(const string &AnimatedSceneObjectName) {
  TMapCAnimatedSceneObject::iterator it =
      AnimatedSceneObject.find(AnimatedSceneObjectName);
...
void updateAnimationSequence(const CEntityId &id, const list<string> &sequence,
                             uint32 idSequence) {
  TMapCAnimatedSceneObject::iterator it;

  for (it = AnimatedSceneObject.begin(); it != AnimatedSceneObject.end();
       ++it) {
...
void sendEndSequenceMessage(const CEntityId &id, uint32 idSequence) {
  SEND_MSG_MACRO("ANIME_BAR_MAN_END", SAnimeBarManEnd end; end.Id = id;
                 end.SlotNumber = idSequence; msg.serial(end);, ;, ;)
}
...
//
//---------------------------------------------------
void CAnimatedSceneObject::processLogicTemp(double startOffset,
                                            double endOffset) {
  ...
  //	EAM->processLogic( _AnimationsGlobalId[MOVING], startOffset, endOffset,
  //soundTrack, result);

  // get infos about the anim

  char *soundTrackName = "NoteTrack";
  soundTrack = _AnimationSet->getAnimation(_IdAnimationPlayed)
        ...
        if (!soundTrack->interpolate(*itResult, soundName)) {
          ...
          /*					// if there are step sounds
                                                  if( soundName == "step" )
                                                  {
                                                          // get the material
                                                          UGlobalPosition
             globalPos; getPrimitive()->getGlobalPosition(globalPos, dynamicWI);
                                                          uint32 material =
             GR->getMaterial( globalPos );

                                                          // choose the sound
             type soft/hard bool soft; if( Type.TypeInfo.Sex ==
             CTypeEntity::male ) soft = false; if( Type.TypeInfo.Sex ==
             CTypeEntity::female ) soft = true;

                                                          vector<string> sounds;
                                                          uint32 soundSourceId;

                                                          // choose the sound
             for the move type if( _AnimationsStateKey[MOVING] == "walk" )
                                                          {
                                                                  if(
             SoundMngr->getSounds( material, WALK, soft, sounds ) )
                                                                  {
                                                                          string
             name = chooseRandom(sounds,previousSoundIndex );
                                                                          SoundMngr->spawnSound(
             name , pos ());
                                                                  }
                                                                  else
                                                                  {
                                                                          nlwarning("<CEntityCL::updatePos>
             :  there is no walk step sound for the material %d",material);
                                                                          SoundMngr->spawnSource(
             "Fwrks_Clac_1", pos() );
                                                                  }
                                                          }
                                                          _SoundSourcesId.push_back(
             soundSourceId );
                                                  }
                                                  else*/
          if (soundName.find("snd_") != string::npos) {
            // need to spawn a sound linked to the anim
            SoundMngr->spawnSource(soundName, _Position);
            nlwarning("launching sound for anim event from notetrack '%s'",
                      soundName.c_str());
          } else {
