...
// *********************************************************************************************************
void CWaterMap::init(const NLMISC::CVector2f &minCorner,
  ...
  if (_MinCorner.y > _MaxCorner.y)
    std::swap(_MinCorner.y, _MaxCorner.y);
  if (_MinCorner.y > _MaxCorner.y)
    std::swap(_MinCorner.y, _MaxCorner.y);
  _CellSize = cellSize;
...
// *********************************************************************************************************
bool CWaterMap::getWaterHeight(const NLMISC::CVector2f &pos, float &height,
                               bool &splashEnabled) {
  H_AUTO_USE(RZ_WaterMap)
  if (_Grid.empty())
    return false;
  ...
  if (iy < 0 || iy >= _Height)
    return false;
  const TWaterInfoIndexVect &wiiv =
      _WaterInfoIndexVectVect[_Grid[ix + iy * _Width]];
  if (wiiv.empty())
    return false;
...
// *********************************************************************************************************
void CWaterMap::waterSurfaceAdded(const NLMISC::CPolygon2D &shape,
  ...
  for (uint k = 0; k < _WaterInfoVect.size(); ++k) {
    if (_WaterInfoVect[k].Height == height &&
        _WaterInfoVect[k].Shape.Vertices.size() == shape.Vertices.size()) {
      if (std::equal(wi.Shape.Vertices.begin(), wi.Shape.Vertices.end(),
                     _WaterInfoVect[k].Shape.Vertices.begin())) {
  ...
  if (!rasters.empty()) {
    ...
    for (sint y = 0; y < numRasters; ++y) {
      if ((y + minY) < 0 || (y + minY) >= _Height)
        continue; // outisde of map
      sint lastX = rasters[y].second + 1;
      for (sint x = rasters[y].first; x < lastX; ++x) {
        if (x < 0 || x >= _Width)
          continue; // outside of map
        const TWaterInfoIndexVect *currWI =
            &_WaterInfoIndexVectVect[_Grid[x + (y + minY) * _Width]];
        // see if there's already a list of water surfaces that match the list
        // for that grid cell such a list should contains the surfaces already
        // found for that grid cell + the new one
        sint goodList = -1;
        for (uint k = 0; k < _WaterInfoIndexVectVect.size(); ++k) {
          if (_WaterInfoIndexVectVect[k].size() == currWI->size() + 1) {
            if (std::equal(_WaterInfoIndexVectVect[k].begin(),
                           _WaterInfoIndexVectVect[k].begin() + currWI->size(),
                           currWI->begin()) &&
                _WaterInfoIndexVectVect[k].back() ==
                    _WaterInfoVect.size() - 1) {
        ...
        if (goodList == -1) {
          ...
          // vector has grown up, so currWI pointer becomes invalid -> rebuild
          // it
          currWI = &_WaterInfoIndexVectVect[_Grid[x + (y + minY) * _Width]];
          _WaterInfoIndexVectVect.back().resize(currWI->size() + 1);
          std::copy(currWI->begin(), currWI->end(),
                    _WaterInfoIndexVectVect.back().begin());
          _WaterInfoIndexVectVect.back().back() =
              (uint16)_WaterInfoVect.size() - 1;
...
void CWaterMap::dump(const std::string &filename) {
  ...
  //
  drawDisc(bm, ((float)UserEntity->pos().x - _MinCorner.x) / _CellSize,
           ((float)UserEntity->pos().y - _MinCorner.y) / _CellSize, 2.f,
           CRGBA::Magenta);
  //
  NLMISC::COFile f;
  if (!f.open(filename)) {
...
// ******************************************************************************************************************
void CWaterMap::render(const NLMISC::CVector2f &camPos,
                       float maxDist /*=100.f*/) {
  H_AUTO_USE(RZ_WaterMap)
  if (_Grid.empty())
    return;
  ...
  for (uint x = 0; x < _Width; ++x) {
    for (uint y = 0; y < _Height; ++y) {
      ...
      for (uint l = 0; l < wii.size(); ++l) {
        ...
        NLMISC::CVector bias =
            (float)wii[l] * NLMISC::CVector(0.01f, 0.f, 0.1f);
        drawBox(cornerMin + bias, cornerMax + bias,
                DebugCols[wii[l] % NumDebugCols]);
      }
    }
  }
}
